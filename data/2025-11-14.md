...
TL;DR: 本文提出了多组可调相移导频(MAPSPs)方法，通过多组基本序列生成导频，在角度-延迟域利用信道稀疏性，实现大规模MIMO-OFDM系统中低开销的信道估计，并通过相位调度和Zadoff-Chu序列设计来减轻导频干扰。


<details>
  <summary>Details</summary>
Motivation: 大规模MIMO-OFDM系统在提供高频谱效率的同时面临高信道获取开销的挑战。现有的可调相移导频(APSPs)技术虽然能利用信道稀疏性降低开销，但仍有改进空间。

Method: 1. 将传统空间-频率域信道模型转换到角度-延迟域获得稀疏信道矩阵；2. 提出通过多组基本序列生成MAPSPs的方法；3. 分析导频干扰对MMSE估计的影响机制；4. 提出基于Zadoff-Chu序列的实现方案，包括接收信号预处理和导频调度方法。

Result: 仿真结果表明，MAPSP方法相比APSP获得了更低的估计均方误差(MSE)，并在移动场景下显著提升了频谱效率。

Conclusion: MAPSP方法通过多组导频设计和相位调度，有效减轻了导频干扰，在大规模MIMO-OFDM系统中实现了更好的信道估计性能和频谱效率提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有ISAC解决方案主要依赖集中式处理和完整信道状态信息，限制了可扩展性，缺乏在前传限制下分布式部署的联合AP聚类和用户/目标调度方法。

Method: 将问题建模为混合整数线性规划，然后提出ASSENT框架，使用图神经网络从轻量级链路统计中学习关联和模式选择策略，替代计算密集的优化方法。

Result: 仿真显示ASSENT实现近最优效用，准确学习底层关联，单次前向传播推理显著降低决策延迟。

Conclusion: ASSENT为无蜂窝ISAC提供可扩展的实时解决方案，开源实现促进可复现和可扩展研究。
...
TL;DR: 本文研究了可旋转智能反射面(IRS)的优化设计，通过动态调整IRS的旋转角度来最大化目标区域内所有位置的最小期望信噪比(SNR)。
...
Method: 提出了角度相关信道模型准确描述IRS元件的接收和反射特性；针对单目标位置情况开发了基于粒子群优化(PSO)的算法；针对一般区域覆盖情况，设计了带零点检测的双层PSO迭代算法。

Result: 数值结果表明，所提出的可旋转IRS设计在不同系统设置下相比各种基准方案实现了显著的SNR改进。

Conclusion: 可旋转IRS通过优化旋转角度能够显著提升无线通信系统的区域覆盖性能，所提出的算法和模型为实际系统设计提供了有效解决方案。
...
TL;DR: 提出了一种轻量级协同优化框架，通过区域提示精化初始化和宏调度放置策略，在VLSI全局布局中实现高质量初始化，显著提升线长质量并大幅减少运行时间。


<details>
  <summary>Details</summary>
Motivation: 现有VLSI布局初始化方法存在权衡：区域感知初始化器计算昂贵且主导总运行时间，而快速点基初始化器忽略单元面积导致建模差距，影响收敛性和解质量。

Method: 1. 区域提示精化初始化器：通过虚拟节点和负权边将启发式单元面积信息融入符号图信号，生成区域感知且频谱平滑的布局初始化；2. 宏调度放置过程：逐步恢复区域约束，实现从精化初始化器到完整区域感知目标的平滑过渡。

Result: 在12个测试案例中，11个案例的HPWL相比点基初始化器得到改善，最高减少2.2%线长，同时比最先进的区域感知初始化器快约100倍。

Conclusion: 该框架有效弥合了初始化差距，在保持高质量布局结果的同时显著提升了计算效率。
...
TL;DR: 该论文提出了一种基于NOMA的双智能反射面中继网络，集成了环境反向散射通信。系统包含源节点、能量受限的中继（具有能量收集和反向散射能力）、两个NOMA用户和BS节点。通过时间切换中继协议实现能量收集和信息转发，同时支持反向散射通信。两个IRS用于增强在阻塞条件下的链路性能。


<details>
  <summary>Details</summary>
Motivation: 为未来物联网应用设计可靠且能量高效的NOMA-IRS辅助反向散射网络，解决能量受限中继的通信挑战，并利用IRS增强链路在阻塞条件下的性能。

Method: 采用时间切换中继协议，中继同时进行能量收集和信息转发。部署两个IRS分别增强源到中继和中继到用户的链路。推导了中断概率和吞吐量的闭式表达式，并通过蒙特卡洛仿真验证分析结果。

Result: 数值结果揭示了主链路和反向散射链路之间的关键权衡关系。分析了功率分配因子、反射效率、IRS元素数量和传输速率等系统参数对吞吐量的影响。

Conclusion: 所提出的框架为设计未来物联网应用的可靠且能量高效的NOMA-IRS辅助反向散射网络提供了有价值的见解。
...
<details>
  <summary>Details</summary>
Motivation: 为了在级联Delta-Sigma调制器结构中实现高阶调制，需要寻找合适的参数设置方法。

Method: 使用二项式整数参数配置级联Delta-Sigma调制器，该结构包含分布式反馈和前馈输入以及多比特输出。

Result: 证明了使用这些系数可以实现高阶调制，并讨论了系数的精度要求。

Conclusion: 二项式整数参数为级联Delta-Sigma调制器提供了一种有效的高阶实现方法，但需要注意系数的精度控制。
...
TL;DR: 提出了一种基于现代Hopfield网络的内存增强语义通信框架，通过软注意力检索机制动态调整语义原型权重，在数据动态变化时实现稳定匹配，显著减少传输比特数。
...
Method: 使用现代Hopfield网络在发射端和接收端维护共享语义概念内存，采用软注意力检索机制平滑调整存储的语义原型权重，联合优化编码器、解码器和内存检索机制，最大化推理能力度量。

Result: 理论分析证明了软检索相比硬量化在有限语义漂移下减少不必要传输。在多样化视频场景的广泛仿真中，该方法平均减少约14%的比特数，在渐进内容变化场景中最多减少70%。

Conclusion: 基于现代Hopfield网络的内存增强语义通信框架通过软注意力检索机制有效解决了数据动态变化时的内存更新问题，显著提高了带宽利用效率。
...
TL;DR: 本文评估了超宽带网络的协作自校准方法，通过改进的离散贝叶斯网格不确定性传播算法，在减少测量需求的同时保持定位精度，实现了亚米级定位精度。


<details>
  <summary>Details</summary>
Motivation: 传统基于无线电的定位系统需要精确测量位置的固定参考点，部署耗时且成本高。本文旨在通过协作自校准减少对基础设施的依赖，实现自动化的UWB网络初始化。

Method: 扩展了基于网格不确定性传播的离散贝叶斯方法，通过概率状态估计在减少测量可用性要求的同时保持定位精度。在静态环境中使用12个节点的真实UWB网络数据进行验证。

Result: 实验评估显示，在视距条件下平均测距误差为0.28米，混合传播场景下整体测距误差为1.11米，实现了亚米级定位精度。算法对测量噪声和部分连接场景具有鲁棒性。
...
TL;DR: JMFNet是一个基于模型的深度学习框架，用于具有未知噪声统计和模式转移动态的跳变马尔可夫系统的实时状态估计。它采用包含两个RNN的混合架构，通过交替最小二乘策略联合训练，在非平稳和高噪声环境下优于传统模型基滤波器和无模型深度学习方法。


<details>
  <summary>Details</summary>
Motivation: 解决跳变马尔可夫系统中未知噪声统计和模式转移动态的实时状态估计问题，传统方法在这些条件下性能受限。

Method: 提出混合架构，包含两个RNN：一个用于模式预测，另一个基于KalmanNet的增强版本进行滤波。采用交替最小二乘策略进行无监督联合训练。

Result: 在线性和非线性系统（目标跟踪、摆角跟踪、Lorenz吸引子等）上的实验表明，JMFNet在非平稳和高噪声环境下优于传统滤波器和深度学习方法，在复杂系统或长轨迹中相比KalmanNet有显著改进。

Conclusion: JMFNet框架在跳变马尔可夫系统的状态估计中表现出色，对初始条件、超参数选择和模型知识错误具有低敏感性，性能一致可靠。
...
TL;DR: 提出了一种基于有向二分图编码器的自动化ICD编码方法，通过建模常见代码和罕见代码之间的细粒度共现关系来解决长尾分布问题。


<details>
  <summary>Details</summary>
Motivation: 自动化ICD编码面临标签空间大（10,000-20,000个代码）和长尾分布的挑战，许多罕见代码缺乏足够的训练数据。

Method: 构建有向二分图编码器，将常见代码和罕见代码作为不相交节点集，边仅从常见代码指向罕见代码。使用基于条件概率的偏置注入注意力模块，通过LLM生成代码描述来丰富初始嵌入。

Result: 在三个自动化ICD编码基准数据集上的实验表明，该方法实现了最先进的性能，特别是在长尾分类关键指标Macro-F1上有显著提升。

Conclusion: 所提出的方法通过建模代码间的共现关系，能够有效利用常见代码的共病信息来丰富罕见代码的表示，从而提升长尾分类性能。
...
TL;DR: 本文研究了神经网络组合结构如何影响优化景观和训练动态，分析了过参数化优化问题的梯度流，证明了对于任何真且实解析的成本函数都具有全局收敛性，并展示了关键结构特征在所有允许成本中具有普适性。


<details>
  <summary>Details</summary>
Motivation: 研究神经网络组合结构对优化景观和训练动态的影响，理解过参数化表示如何决定优化过程的几何和动力学特性。

Method: 分析过参数化优化问题的梯度流，将其解释为具有线性激活的神经网络训练，对真且实解析的成本函数进行全局收敛性分析，并引入不平衡度量来评估收敛加速。

Result: 证明了全局收敛性适用于任何真且实解析的成本函数，展示了关键结构特征（如鞍点位置和稳定性）在所有允许成本中具有普适性，收敛速度可根据初始化任意加速。

Conclusion: 神经网络的组合结构决定了优化景观的几何特性，这些特性在过参数化表示中是普适的，且线性情况下的几何和动力学特性可能推广到具有sigmoid激活的网络。
...
TL;DR: 提出了几种基于离散时间深度混合专家（MoE）的生存分析架构，其中一个模型能够同时实现聚类、校准和预测准确性的目标。研究发现，表达能力更强的专家（能为每个患者定制预测）优于依赖固定组原型的专家。


<details>
  <summary>Details</summary>
Motivation: 现有的深度混合专家模型在生存分析中虽然能聚类相似患者，但往往以牺牲校准误差和预测准确性为代价。这是由于混合专家模型的限制性归纳偏差，即个体患者的预测必须与其所属组的预测相似。

Method: 引入了多种离散时间深度混合专家（MoE）架构用于生存分析问题，其中一种架构能够同时实现聚类、校准和预测准确性的目标。

Result: 研究表明，不同MoE架构的关键区别在于其专家的表达能力。表达能力更强的专家（能为每个患者定制预测）在性能上优于依赖固定组原型的专家。

Conclusion: 通过使用表达能力更强的专家，可以在发现患者群体结构的同时，提高校准和预测准确性，实现聚类、校准和预测准确性的平衡。
...
TL;DR: 该论文综述了深度线性神经网络梯度流方程的动态特性和损失景观，分析了无激活函数的深度神经网络在二次损失函数下的梯度下降训练动态。
...
Result: 损失景观包含无限多个全局最小值和鞍点（严格和非严格），但缺乏局部最小值和最大值。损失函数是梯度流的正半定Lyapunov函数，其水平集是无界不变临界点集。
...
TL;DR: 本文提出使用群平均技术在不改变模型结构或训练过程的情况下，使任何训练好的模型在测试时具有精确的等变性，从而提升预测精度。


<details>
  <summary>Details</summary>
Motivation: 许多自然科学中的机器学习任务具有特定的对称性，但等变方法往往未被采用，可能是因为训练被认为具有挑战性，或者期望模型能自动学习对称性，或者等变实现被认为难以构建。

Method: 在测试时对训练好的模型进行群平均操作，通过对对称群进行平均来使模型具有精确的等变性，该方法成本低且不要求模型结构或训练过程的改变。
...
### [16] [Belief Net: A Filter-Based Framework for Learning Hidden Markov Models from Observations](https://arxiv.org/abs/2511.10571)
*Reginald Zhiyan Chen,Heng-Sheng Chang,Prashant G. Mehta*

Main category: cs.LG

TL;DR: 提出了Belief Net框架，通过将HMM的前向滤波公式化为结构化神经网络，使用梯度优化学习HMM参数，相比传统方法收敛更快且保持完全可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统HMM参数学习方法如Baum-Welch算法计算量大且易陷入局部最优，而现代谱算法虽然提供理论保证但可能产生无效概率输出，需要更高效可靠的方法。

Method: 将HMM的前向滤波构建为结构化神经网络，可学习权重明确对应初始分布、转移矩阵和发射矩阵的对数概率，采用仅解码器架构处理观测序列，使用标准自回归下一观测预测损失进行端到端训练。

Result: 在合成HMM数据上，Belief Net相比Baum-Welch实现了更快的收敛速度，在谱方法失败的欠完备和过完备设置中成功恢复参数，并在真实语言数据上与Transformer模型进行了比较。

Conclusion: Belief Net框架为HMM参数学习提供了高效且可解释的梯度优化方法，在多种设置下优于传统算法，同时保持了模型的可解释性优势。
...
TL;DR: 提出基于引导去噪扩散概率模型的生成优化框架，利用代理梯度生成满足压力降最小化和表面温度约束的散热器设计。


<details>
  <summary>Details</summary>
Motivation: 传统黑盒优化方法（如CMA-ES）和拓扑优化方法在散热器设计中存在计算成本高、扩展性差的问题，需要开发更高效、可扩展的生成式优化方法。

Method: 使用边界表示法表示多翅片几何结构，采用多保真度方法生成训练数据。训练DDPM模型生成散热器设计，并训练两个残差神经网络预测压力降和表面温度。利用代理模型的梯度引导几何生成过程满足约束条件。

Result: 引导扩散模型生成的样本比传统黑盒优化方法获得的最低压力降还要低10%。模型训练后，在新约束条件下的推理计算成本低廉且无需重新训练。

Conclusion: 该工作为构建电子冷却基础生成模型迈出了重要一步，展示了生成式优化在散热器设计中的潜力和优势。
...
TL;DR: 本文提出GEF（生成-执行-反馈）循环框架，强调环境作为体验数据生产者的重要性，系统回顾了环境扩展方法，并分析了基准、实现策略和应用。


<details>
  <summary>Details</summary>
Motivation: 基于静态数据集训练的LLM智能体在适应性和长期决策方面存在局限，需要通过与环境的交互学习来提升能力。

Method: 提出GEF循环框架，从环境中心视角系统回顾任务生成、任务执行和反馈三个环节的环境扩展方法。

Result: 整合了分散的环境扩展进展，为智能体智能的未来研究提供了方向。

Conclusion: 环境扩展是实现智能体智能的关键，需要向更复杂、真实和交互的方向发展。
...
### [19] [NeuroLingua: A Language-Inspired Hierarchical Framework for Multimodal Sleep Stage Classification Using EEG and EOG](https://arxiv.org/abs/2511.09773)
*Mahdi Samaee,Mehran Yazdi,Daniel Massicotte*

Main category: cs.LG

TL;DR: NeuroLingua是一个基于语言启发的睡眠分期框架，将睡眠视为结构化生理语言，通过双级Transformer和GCN多模态融合，在Sleep-EDF和ISRUC数据集上达到最先进性能。
...
Method: 将30秒时段分解为重叠的3秒子窗口作为"token"，使用CNN分词器，通过双级Transformer进行层次时间建模（局部依赖的段内编码和跨7个连续时段的段间集成），并通过图卷积网络融合EEG和EOG通道的模态特定嵌入。

Result: 在Sleep-EDF数据集上达到85.3%准确率、0.800宏F1和0.796 Cohen's kappa；在ISRUC数据集上达到81.9%准确率、0.802宏F1和0.755 kappa，匹配或超过已发表基线。

Conclusion: 通过将睡眠框架化为组合语言，NeuroLingua统一了层次序列建模和多模态融合，推进自动睡眠分期向更透明和临床有意义的应用发展。
...
### [20] [DynamicRTL: RTL Representation Learning for Dynamic Circuit Behavior](https://arxiv.org/abs/2511.09593)
...
TL;DR: 提出了DR-GNN模型，通过结合静态结构和多周期执行行为来学习RTL电路的表示，解决了现有GNN模型无法捕获电路运行时行为的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的图神经网络主要关注电路的静态特性，但无法捕捉电路运行时行为，这对于电路验证和优化等任务至关重要。

Method: 使用操作符级控制数据流图表示RTL电路，结合静态结构和多周期执行行为，构建了包含6300个Verilog设计和63000个仿真轨迹的动态电路数据集。

Result: DR-GNN在分支命中预测和翻转率预测方面优于现有模型，其学习表示能有效迁移到相关动态电路任务，在功耗估计和断言预测方面表现良好。

Conclusion: DR-GNN通过整合动态行为信息，显著提升了电路表示学习能力，为电路验证和优化提供了更有效的工具。
...
### [21] [Boosted GFlowNets: Improving Exploration via Sequential Learning](https://arxiv.org/abs/2511.09677)
*Pedro Dall'Antonia,Tiago da Silva,Daniel Augusto de Souza,César Lincoln C. Mattos,Diego Mesquita*

Main category: cs.LG

TL;DR: Boosted GFlowNets通过顺序训练集成模型来解决GFlowNets在探索奖励空间时的不平衡问题，每个后续模型优化前序模型未覆盖区域的残差奖励，从而改善探索和样本多样性。


<details>
  <summary>Details</summary>
Motivation: 标准GFlowNets在实践中难以均匀探索奖励空间，容易偏向易达区域，而难以到达的高奖励区域获得的梯度信号微弱，导致对高奖励区域的覆盖不足。

Method: 提出Boosted GFlowNets方法，顺序训练GFlowNets集成，每个模型优化残差奖励，补偿前序模型已捕获的质量，重新激活未充分探索区域的学习信号。

Result: 在合成多模态基准和肽设计任务中，Boosted GFlowNets实现了显著更好的探索和样本多样性，同时保持了标准轨迹平衡训练的稳定性和简单性。

Conclusion: Boosted GFlowNets通过残差原则解决了GFlowNets的探索不平衡问题，确保学习分布不会退化且通常得到改善，为组合对象采样提供了更有效的解决方案。
...
### [22] [Making Every Head Count: Sparse Attention Without the Speed-Performance Trade-off](https://arxiv.org/abs/2511.09596)
*Mingkuan Zhao,Wentao Hu,Jiayin Wang,Xin Lai,Tianchen Huang,Yuheng Min,Rui Yan,Xiaoyan Zhu*

Main category: cs.LG

TL;DR: SPAttention通过引入"原则性结构稀疏性"新范式，将多头注意力计算从H个独立的O(N²)计算转变为单个协作的O(N²)计算，从根本上将复杂度降低H倍，在保持性能的同时实现约两倍的训练吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 解决LLM核心注意力机制中计算复杂度O(H·N²)与表达能力之间的根本冲突，现有稀疏方法往往以信息完整性为代价换取计算效率，需要解决这种效率-性能权衡问题。

Method: 采用原则性结构稀疏性，将总注意力工作量划分为平衡的非重叠距离带，为每个头分配唯一片段，强制头间功能专业化，从冗余建模转向跨整个序列跨度的不同依赖关系。
...
Conclusion: SPAttention通过重新组织计算任务而非简单丢弃连接，成功解决了多头注意力中的计算冗余问题，在保持模型表达能力的同时显著提升了计算效率。
...
### [23] [Incremental Generation is Necessity and Sufficient for Universality in Flow-Based Modelling](https://arxiv.org/abs/2511.09902)
*Hossein Rouhvarzi,Anastasis Kratsios*

Main category: cs.LG

TL;DR: 本文证明了增量生成对于基于流的生成模型在保持定向的同胚映射空间中的普适性是必要且充分的，并给出了具体的逼近速率和结构保证。


<details>
  <summary>Details</summary>
Motivation: 基于流的去噪模型在生成建模中表现出色，但其经验优势缺乏严格的逼近理论基础。本文旨在为增量生成提供理论支撑，证明其在保持定向的同胚映射空间中的普适性。

Method: 首先使用拓扑动力学方法证明单步自主流的不可能性定理，然后利用自主流的代数性质构造多步流逼近，最后通过线性提升维度获得结构化普适逼近结果。

Result: 证明了每个保持定向的Lipschitz同胚映射可以用最多K_d个自主流以O(n^{-1/d})速率逼近，在光滑条件下可实现维度无关的逼近速率，并获得概率测度的1-Wasserstein误差逼近。

Conclusion: 增量生成是基于流生成模型实现普适逼近的必要且充分条件，为流模型的增量生成机制提供了坚实的理论基础。
...
### [24] [Parametric Expensive Multi-Objective Optimization via Generative Solution Modeling](https://arxiv.org/abs/2511.09598)
...
<details>
  <summary>Details</summary>
Motivation: 现实应用中需要在变化操作条件下解决昂贵的多目标优化问题族，这产生了参数化昂贵多目标优化问题(P-EMOPs)，其中连续任务参数空间包含无限个不同问题，每个都需要单独昂贵的评估。

Method: 交替进行：(1)利用任务间协同效应的获取驱动搜索，(2)通过条件生成模型进行生成式解决方案采样，学习逆模型。

Result: 理论分析证明通过任务感知高斯过程利用任务间协同效应可实现更快收敛，合成和现实基准测试验证了交替框架的有效性。
...
TL;DR: 提出了一种新的学习范式，通过更充分地利用经验数据来减少对先验假设的依赖，适用于候选模型集合过大而无法实现经验估计与真实损失一致收敛的情况。


<details>
  <summary>Details</summary>
Motivation: 传统SRM或基于正则化的学习方法在面对过大候选模型集合时存在局限性，需要减少对先验假设的依赖，更充分地利用经验数据。

Method: 提出新的学习范式，通过更强地整合经验数据，减少基于先验假设的算法决策，能够利用多种常见学习假设而无需事先知道其真实参数。

Result: 分析了该方法的泛化能力，并在多种常见学习假设下展示了其优势，包括邻近点相似性、领域聚类、标记规则的Lipschitz性以及对比学习假设。

Conclusion: 该方法能够在不知道真实参数的情况下有效利用各种学习假设，为大规模候选模型集合的学习问题提供了新的解决方案。
...
TL;DR: 提出了两个参数化的多臂老虎机算法家族，利用离线数据学习近最优算法，在奖励曲线满足特定凹性时获得更强的性能保证，同时保证在良好实例上的最优臂识别和恶劣实例上的最坏情况保证。


<details>
  <summary>Details</summary>
Motivation: 改进型多臂老虎机问题在不确定性下分配努力，应用于技术投资、临床试验和超参数选择等场景。现有算法的最坏情况保证较为悲观，存在Ω(k)和Ω(√k)的下界。

Method: 定义了两个参数化算法家族：第一个包含先前工作的最优随机算法，在奖励曲线满足额外凹性时获得更强保证；第二个保证在良好实例上识别最优臂，在恶劣实例上回退到最坏情况保证。

Result: 通过统计学习视角，实现了更强的数据依赖保证，无需验证假设是否满足，在适当条件下获得关于k的最优依赖关系。

Conclusion: 提出的参数化算法家族能够在不同实例条件下自适应地提供性能保证，通过离线数据学习实现更强的数据依赖性能。
...
<details>
  <summary>Details</summary>
Motivation: 挑战当前表格深度学习中需要大规模预训练数据的普遍观点，探索更高效的数据利用方法。

Method: 在单个真实表格上进行自监督预训练，然后跨多个异构基准进行评估，分析数据特征对构建通用表格基础模型的重要性。

Result: 发现相对少量的数据就足以实现泛化，单个表格的自监督预训练能产生跨领域的强迁移能力。

Conclusion: 从数据集中构建任务的数量和质量是决定下游性能的关键因素，而非数据规模本身。
...
TL;DR: GEM+结合了AIM的自适应测量框架和GEM的可扩展生成器网络，在保持差分隐私的同时，显著提升了合成表格数据的质量和可扩展性，能够高效处理超过百列的高维数据集。


<details>
  <summary>Details</summary>
Motivation: 现有的差分隐私合成表格数据方法存在局限性：基于图形模型的方法（如AIM）在高维数据上内存和计算开销大，而基于生成器网络的方法（如GEM）主要在小数据集上测试，限制了实际应用。

Method: 将AIM的自适应测量框架与GEM的可扩展生成器网络相结合，创建GEM+方法，利用自适应测量优化数据质量，同时通过生成器网络提高可扩展性。

Result: GEM+在效用和可扩展性方面均优于AIM，能够处理超过百列的高维数据集，而AIM由于内存和计算开销无法处理此类数据。

Conclusion: GEM+通过结合自适应测量和生成器网络，在差分隐私合成表格数据领域实现了最先进的性能，解决了高维数据处理的挑战。
...
TL;DR: SEBA是一个针对视觉强化学习的黑盒对抗攻击框架，通过集成影子Q模型、生成对抗网络和世界模型，在保持视觉保真度的同时显著降低累积奖励，并大幅减少环境交互需求。


<details>
  <summary>Details</summary>
Motivation: 视觉强化学习在视觉控制和机器人领域取得了显著进展，但其对抗扰动的脆弱性仍未充分探索。现有黑盒攻击主要针对基于向量或离散动作的RL，在基于图像的连续控制中由于大动作空间和过多环境查询而效果有限。

Method: SEBA集成了影子Q模型（估计对抗条件下的累积奖励）、生成对抗网络（产生视觉不可察觉的扰动）和世界模型（模拟环境动态以减少真实世界查询）。通过两阶段迭代训练程序，交替学习影子模型和优化生成器。

Result: 在MuJoCo和Atari基准测试中，SEBA显著降低了累积奖励，保持了视觉保真度，并且与先前的黑盒和白盒方法相比，大大减少了环境交互。
...
<details>
  <summary>Details</summary>
Motivation: 传统强化学习方法在提升Text2SQL LLMs推理能力时，性能高度依赖于奖励函数设计，不当的奖励会导致奖励黑客现象，模型通过利用奖励结构漏洞获得高分而非真正解决问题。

Method: 采用约束强化学习框架，引入自然可解释的奖励和约束信号，在训练过程中动态平衡这些信号之间的权衡。

Result: 在知名Text2SQL数据集上的数值实验证实，该方法相比最先进的RL训练LLMs有所改进。

Conclusion: 所提出的约束RL框架为Text2SQL提供了理论保证，并通过实验验证了其相对于现有方法的优越性。
...
TL;DR: MCR是一种使用模运算的高维整数向量计算模型，在容量、精度和硬件效率之间实现了独特平衡，相比二进制向量性能更优，内存占用比复数表示小得多，专用硬件加速器可带来3个数量级的速度提升和显著能耗降低。


<details>
  <summary>Details</summary>
Motivation: MCR作为二进制稀疏码模型的泛化，旨在提供更高的表示能力，同时保持比高精度组件模型更轻量的特性。但由于缺乏系统分析，其额外复杂性被认为超过了表达能力提升，因此需要重新评估其实际价值。

Method: 对MCR进行了首次广泛评估，包括容量测量、123个数据集的准确性测试，以及设计首个专用硬件加速器来验证硬件实现可行性。

Result: MCR在容量上优于二进制和整数向量，接近复数表示但内存占用小得多；在123个数据集上显示一致的准确性提升，比二进制稀疏码节省4倍内存；硬件实现带来3个数量级速度提升和显著能耗降低。

Conclusion: 虽然MCR需要比二进制稀疏码更复杂的操作，但其模运算和更高的组件精度使得维度更低，配合专用硬件可实现更快、更节能、更高精度的替代方案。
...
TL;DR: 提出了一个基于深度学习的方程感知仿真框架，能够泛化到未见过的偏微分方程，通过在神经网络模型中编码PDE项和系数来实现通用性。


<details>
  <summary>Details</summary>
Motivation: 传统数值方法求解偏微分方程成本过高，现有深度学习代理模型通常只能处理固定参数的单一PDE，缺乏通用性。
...
Conclusion: 该框架为创建自动化专家级科学软件系统提供了基础，展示了AI系统在可评分科学任务中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决雷达降水临近预报中的两个核心挑战：大气动力学的不确定性和高维数据的高效建模，同时克服扩散模型迭代采样过程在时效性应用中的计算瓶颈。

Method: 采用条件流匹配(CFM)方法，学习从噪声到数据的直接映射，相比扩散模型大幅减少函数评估次数，实现快速采样。

Result: 实验表明FlowCast在预测准确性上达到新SOTA，CFM目标在相同架构下比扩散目标更准确且显著更高效，在显著减少采样步数的情况下仍保持高性能。

Conclusion: CFM成为高维时空预报中强大且实用的替代方案，为时间关键型应用提供了可行的解决方案。
...
TL;DR: 本文研究了分裂联邦学习(SFL)中数据异构性导致的灾难性遗忘问题，提出了Hydra方法来缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 在分裂联邦学习中，由于数据异构性，本地模型更新会偏离全局最优解，而服务器端的顺序处理会导致模型对序列末尾的类别表现更好，出现类似持续学习中的灾难性遗忘现象。

Method: 提出了Hydra方法，该方法受到多头神经网络的启发，专门为SFL设置进行了适配，通过多分支结构来缓解灾难性遗忘问题。

Result: 广泛的数值评估表明，Hydra方法在性能上优于基线方法和文献中的现有方法。

Conclusion: Hydra方法能够有效缓解分裂联邦学习中的灾难性遗忘问题，在数据异构环境下表现出优越性能。
...
TL;DR: SPARC提出单阶段适应方法，用于解决上下文强化学习中未见环境的泛化问题，在赛车模拟器和风扰环境中实现稳健的OOD泛化。


<details>
  <summary>Details</summary>
Motivation: 解决机器人和控制领域中的泛化挑战，特别是在上下文强化学习中，智能体需要在与训练环境不同的上下文（如地形、天气条件）中操作，且测试时无法获取明确的上下文信息。

Method: 引入SPARC：单阶段适应鲁棒控制方法，简化了之前需要分别训练上下文编码器和历史适应模块的两阶段方法。

Result: 在高保真赛车模拟器Gran Turismo 7和风扰MuJoCo环境中测试，SPARC实现了可靠且稳健的OOD泛化性能。

Conclusion: SPARC提供了一种更简单有效的单阶段方法，能够在未见环境中实现可靠的泛化，解决了之前两阶段方法的实现和训练复杂性。
...
TL;DR: TawPipe是一种拓扑感知的权重流水线并行方法，通过利用分布式集群的层次化带宽来提升大语言模型训练中的通信效率。


<details>
  <summary>Details</summary>
Motivation: 现有流水线并行方法存在激活通信开销随序列长度线性增长的问题，而权重传递方法（如WeiPipe）虽然缓解了这一问题，但仍存在冗余的点对点传输和节点内带宽利用不足的缺陷。

Method: TawPipe通过：(i)基于拓扑对设备分组优化节点内集体通信和节点间点对点通信；(ii)为每个设备分配固定的模型权重和梯度分片，避免冗余传输；(iii)通信与计算重叠以隐藏延迟。

Result: 在24个GPU上使用LLaMA风格模型进行的广泛实验表明，TawPipe相比最先进的基线方法实现了更高的吞吐量和可扩展性。

Conclusion: TawPipe通过将大部分通信限制在节点边界内，显著减少了跨节点流量，在大语言模型训练中提供了优越的通信效率。
...
TL;DR: 提出了一种基于宏观经济情境检索的预测框架，通过检索历史类似宏观经济周期来增强金融预测的稳健性，在分布外测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 金融市场具有非平稳性，结构断裂和宏观经济制度转变常导致预测模型在分布外部署时失效。传统多模态方法简单融合数值指标和文本情感，难以适应这种变化。

Method: 引入宏观情境检索框架，在共享相似性空间中联合嵌入宏观指标（CPI、失业率、收益率利差、GDP增长等）和金融新闻情感，实现无需重新训练即可在推理时检索历史类似时期的因果检索。

Result: 在17年标普500数据上训练，在AAPL和XOM的分布外测试中显著缩小了CV到OOD性能差距，获得唯一正向样本外交易结果（AAPL: PF=1.18, Sharpe=0.95; XOM: PF=1.16, Sharpe=0.61），而静态数值、纯文本和朴素多模态基线在制度转变下崩溃。

Conclusion: 通过实施"金融历史不会重演但常押韵"的原则，证明宏观感知检索能在分布变化下产生稳健、可解释的预测，检索邻居形成可解释证据链对应可识别的宏观情境。
...
TL;DR: 本文研究了布尔函数学习中两种对抗性噪声模型（恶意噪声和恶意噪声）下计算高效算法的能力和限制，发现在分布无关设置中两种噪声模型等价，但在固定分布设置中存在任意大的分离。
...
Method: 通过理论分析比较两种噪声模型在分布无关和固定分布设置下的关系，定义并研究忽略矛盾示例（ICE）算法类，分析其在不同噪声模型下的表现。

Result: 在分布无关设置中，两种噪声模型等价；在固定分布设置中，存在任意大的分离；ICE算法可以将恶意噪声容忍度转换为恶意噪声容忍度，但需要损失一半的噪声率。

Conclusion: 噪声模型之间的关系高度依赖于学习设置，ICE算法提供了一种在固定分布设置中缓解恶意噪声挑战的方法，但存在固有的效率损失。
...
TL;DR: 本文提出了针对去中心化GRPO的首个对抗攻击方法，展示了恶意节点可以通过注入恶意令牌来毒化良性模型，攻击成功率可达100%。同时提出了两种防御方法，防御成功率可达100%。


<details>
  <summary>Details</summary>
Motivation: GRPO在大型语言模型后训练中表现出色，由于其通信量小，适合去中心化训练。然而，这种去中心化特性使其容易受到恶意节点的攻击，目前尚无相关安全研究。

Method: 提出了针对去中心化GRPO的对抗攻击方法，包括上下文外和上下文内攻击，通过在良性模型中注入恶意令牌来毒化系统。使用数学和编程任务进行实证验证。

Result: 攻击可以在50次迭代内达到100%的成功率，有效毒化良性节点的本地LLM后训练。同时提出的防御方法可以达到100%的阻止率。

Conclusion: 去中心化GRPO系统存在严重的安全漏洞，但通过适当的防御措施可以有效抵御这些攻击，确保系统的安全性。
...
TL;DR: 本文解释了JEPA模型为何能自动聚类时间序列的动态机制，提出其预测目标隐式学习Koopman算子的不变子空间，并通过理论证明和合成数据验证了这一机制。


<details>
  <summary>Details</summary>
Motivation: 解释JEPA模型在时间序列数据中自动聚类不同动态机制的现象，揭示其背后的理论原理。

Method: 提出理论假设：JEPA的预测目标驱动其学习系统Koopman算子的不变子空间；证明理想化JEPA损失最小化时编码器表示系统的机制指示函数；在已知动态的合成数据上验证理论。

Result: 发现约束JEPA的线性预测器为近似恒等算子是关键归纳偏置，迫使编码器学习这些不变特征；该约束从数学等价但纠缠的最优解中选择可解释解，揭示预测器在表示解缠中的作用。

Conclusion: 阐明了JEPA的关键行为，建立了现代自监督学习与动力系统理论的原则性联系，为设计更鲁棒和可解释的时间序列模型提供了指导。
...
Method: 采用双流架构：分类分支学习未来逐步趋势，回归分支估计目标变量最新观测值的相应偏差；设计多任务损失函数进行不确定性感知加权；实例化四个变体整合主流时序建模编码器（CNN、LSTM、Transformer）。

Result: 在真实世界数据集上的实验表明，CaReTS在预测准确性上优于最先进算法，同时实现更高的趋势分类性能。

Conclusion: CaReTS框架成功地将分类和回归任务结合，在提供准确预测的同时增强了时间动态的可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 现有值分解方法要么施加单调性约束限制表达能力，要么采用较弱的替代方法但增加算法复杂度，需要找到更好的平衡点。

Method: 采用动态系统分析方法，将学习动态建模为连续时间梯度流，分析非单调值分解的稳定性特性。

Result: 实验证明非单调分解在合成矩阵游戏和MARL基准测试中都能可靠恢复IGM最优解，并持续优于单调基线方法。

Conclusion: 无约束的非单调值分解是可行的，且能提供比单调方法更好的性能，为基于值的MARL算法设计提供了新思路。
...
TL;DR: 本文研究了带可行性约束的最佳臂识别问题，提出了一种高效算法，能够自适应地选择测试臂的性能或可行性，并在理论和实验上证明了其优越性。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的最佳臂识别问题通常需要满足额外的可行性约束，且性能测试和可行性测试往往可以分开进行，而现有方法假设两者同时观测，不符合实际应用场景。

Method: 提出了一种新的BAI算法，允许决策者在每次选择时决定测试臂的性能或某个可行性约束，采用固定置信度设置，通过自适应策略消除性能较差或不可行的臂。

Result: 算法在样本复杂度上达到渐进最优，理论分析显示能自然适应问题难度，实验证明在合成和真实数据集上优于现有方法。

Conclusion: 所提出的算法有效解决了带可行性约束的BAI问题，在理论和实践中均表现出色，为实际应用提供了实用的解决方案。
...
TL;DR: SMoFi通过同步服务器端优化器的动量缓冲区来应对数据异构性带来的梯度发散问题，使用基于陈旧度的对齐机制控制梯度更新，显著提升全局模型精度和收敛速度。


<details>
  <summary>Details</summary>
Motivation: 解决Split Federated Learning中数据异构性导致的梯度发散问题，提高全局模型的收敛速度和精度。

Method: 提出Step-wise Momentum Fusion (SMoFi)框架，通过同步服务器端优化器的动量缓冲区，并设计基于陈旧度的对齐机制来控制梯度更新。

Result: 在多个真实数据集上的验证显示，SMoFi能提升全局模型精度达7.1%，收敛速度提升达10.25倍，在更多客户端和更深模型时效果更显著。

Conclusion: SMoFi是资源受限环境下模型训练的有效解决方案，特别适合多客户端和深度模型场景。
...
TL;DR: 本文提出了一种新算法，能够绕过CSQ（相关统计查询）的硬度障碍，在满足因子化假设的分布下，以多项式时间学习两个半空间的交集，而CSQ方法仍需要拟多项式时间。


<details>
  <summary>Details</summary>
Motivation: 学习半空间交集是计算学习理论的核心问题。即使只有两个半空间，能否在多项式时间内学习仍然是一个重要的开放性问题。现有最佳算法需要拟多项式时间，且已证明仅依赖CSQ的方法无法避免这种复杂度。

Method: 采用新的对偶框架分析边际分布诱导的矩张量结构，结合改进的Jennrich算法、随机投影下的PCA以及基于梯度下降的非凸优化框架。

Result: 在满足因子化假设的分布下，CSQ方法仍需拟多项式时间进行弱学习，而我们的算法在多项式时间内实现学习，建立了CSQ与SQ之间的强分离。

Conclusion: 我们的工作证明了在因子化分布下，通过利用更一般的统计查询（SQ）可以突破CSQ的硬度障碍，为学习半空间交集问题提供了新的高效解决方案。
...
TL;DR: 提出了ACT数据标注流程，让LLM既作为标注员又作为判断员来识别潜在错误，人类只需审核最可疑的案例，在大多数基准数据集上性能差距小于2%的同时节省了90%的人工成本。


<details>
  <summary>Details</summary>
Motivation: 监督学习依赖高质量标注数据，但人工标注成本高昂且耗时。现有LLM标注方法质量仍不及人类水平。

Method: ACT数据管道：LLM作为标注员和判断员，识别潜在错误；人类只审核最可疑案例；适用于NLP、CV和多模态领域；理论分析如何修改损失函数。

Result: 在大多数基准数据集上，与完全人工标注数据训练的模型相比，性能差距小于2%，同时节省了高达90%的人工成本。

Conclusion: ACT流程能显著提高人工标注效率，在保持接近人类标注质量的同时大幅降低成本，适用于多种领域。
...
TL;DR: 提出一种轻量级动态对齐机制，通过从验证器隐藏状态计算转向向量并注入预训练草稿器，显著提高接受率，无需额外训练开销。


<details>
  <summary>Details</summary>
Motivation: 推测解码的主要限制是草稿器-验证器不对齐，这会限制令牌接受率并降低整体效率。预训练草稿器虽然较慢但接受率更高，需要改进其对齐效果。

Method: 引入轻量级动态对齐机制：从验证器隐藏状态计算转向向量并注入预训练草稿器，相比离线对齐方法（如蒸馏）具有实时适应性。

Result: 在标准采样下接受令牌数提升高达35%，贪婪采样下提升22%，计算开销可忽略不计。

Conclusion: 该方法可适配现有架构和预训练模型，实现快速部署，显著提升推测解码效率。
...
<details>
  <summary>Details</summary>
Motivation: 静态模型无法适应动态临床环境和连续数据流，现有持续学习方法主要针对单模态输入且在生存预测中存在严重灾难性遗忘问题，而多模态输入能提供更全面的信息。

Method: 提出ConSurv方法，包含两个关键组件：多阶段专家混合(MS-MoE)捕获不同学习阶段的任务共享和特定知识；特征约束回放(FCR)通过限制先前数据的特征偏差来减轻遗忘。

Result: 在MSAIL基准上的广泛实验表明，ConSurv在多个指标上优于竞争方法。

Conclusion: ConSurv成功解决了多模态生存分析中的持续学习挑战，为动态临床环境提供了有效的解决方案。
...
TL;DR: 这篇论文综述了大语言模型中的机器遗忘研究，分析了当前方法在确保敏感信息永久删除方面的局限性，并探讨了技术和制度层面的解决方案。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在敏感领域的应用增加，暴露了其无法确保私人信息被永久遗忘的关键弱点。现有系统缺乏可靠机制来保证敏感信息一旦使用后能被永久删除。

Method: 论文综述了机器遗忘的最新研究，包括评估遗忘是否发生的方法、被遗忘模型对抗攻击的韧性，以及在模型复杂性或专有限制下支持用户信任的机制。审查了差分隐私、同态加密、联邦学习和短暂记忆等技术解决方案，以及审计实践和监管框架等制度保障。
...
TL;DR: 提出了一种基于不确定性的检查点选择方法(UGCS)，通过识别困难样本的预测不确定性来筛选最优模型检查点，避免传统方法的高计算成本或性能不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 强化学习微调过程中模型检查点选择困难：验证集评估计算成本高，最终检查点性能无保证，需要更稳定高效的检查点选择策略。

Method: 使用样本级不确定性识别困难问答对，通过评估检查点在这些困难样本上的表现进行排名，在短训练窗口内对高不确定性样本奖励进行平均。

Result: 在三个数据集和三个大语言模型上的实验表明，该方法能一致性地选择具有更强泛化能力的检查点，优于传统的训练或验证性能策略。

Conclusion: 能够以低不确定性解决最困难任务的模型是最可靠的，基于不确定性的检查点选择策略有效提升了模型选择的稳定性。
...
TL;DR: 提出一种可扩展的双记忆系统持续学习方法，通过共享特征记忆和样本特定记忆，结合自适应修剪和正交正则化，在CIFAR和Tiny-ImageNet上超越14种先进方法。


<details>
  <summary>Details</summary>
Motivation: 解决传统持续学习方法强制神经网络孤立处理顺序任务的问题，避免重复学习相似特征或过度差异化，充分利用任务间有用关系。

Method: 使用两个互补的可微分记忆：一个学习跨任务通用特征，另一个组合共享特征学习样本特有特征。通过内存调整模块自适应修剪关键槽位并最小化扩展容量，使用正交正则化保持几何分离防止干扰。

Result: 在CIFAR-10、CIFAR-100和Tiny-ImageNet上分别达到55.13%、37.24%和30.11%的最终准确率，优于14种最先进方法。

Conclusion: 通过有效整合和利用知识，该方法能够提高顺序任务的平均性能，产生的特征提取结果最接近理论上限，建立了持续学习的新里程碑。
...
TL;DR: 提出Anchor-based Fair Clustering Framework (AFCF)，通过锚点采样和标签传播实现公平聚类的线性时间扩展，大幅提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有公平聚类算法通常具有二次或超二次计算复杂度，难以应用于大规模数据集，需要开发高效的公平聚类框架。

Method: 使用公平采样策略选择代表性锚点集，在锚点上应用现成公平聚类算法，通过优化问题构建锚点图传播标签，采用ADMM算法求解。

Result: 在多个大规模基准测试中，AFCF将最先进方法的计算时间减少数个数量级，同时保持强聚类性能和公平性保证。
...
TL;DR: SE-Diff是一种新颖的生理模拟器和经验增强扩散模型，用于生成心电图(ECG)。它通过整合轻量级ODE模拟器和LLM驱动的经验检索策略，解决了现有方法忽略生理模拟知识和临床经验知识的问题。


<details>
  <summary>Details</summary>
Motivation: 心血管疾病是全球主要死亡原因，ECG是最常用的无创心脏评估工具。生成ECG有助于理解心脏电活动机制、构建大规模无偏数据集并促进隐私保护的数据共享。现有扩散模型方法忽略了生理模拟知识和基于实际临床实践的经验知识。

Method: 提出SE-Diff模型：1) 通过节拍解码器和模拟器一致性约束将轻量级ODE ECG模拟器整合到扩散过程中；2) 设计LLM驱动的经验检索增强策略注入临床知识；3) 结合生理机制先验和临床经验指导ECG生成。

Result: 在真实ECG数据集上的实验表明，SE-Diff在信号保真度和文本-ECG语义对齐方面优于基线方法，证明了其在文本到ECG生成方面的优越性。基于模拟器和经验的知识也惠及下游ECG分类任务。

Conclusion: SE-Diff通过整合生理模拟知识和临床经验知识，实现了更全面的ECG生成，在信号质量和语义对齐方面表现出色，同时提升了下游分类任务的性能。
...
TL;DR: SWaST机制通过交替进行权重剪枝和核心集选择，在深度学习中实现协同训练，可提升准确率17.83%并减少10%-90%计算量，但需解决关键双损失问题。


<details>
  <summary>Details</summary>
Motivation: 现代深度神经网络依赖大量模型权重和训练样本，计算成本高昂。权重剪枝和核心集选择是两种提高计算效率的新兴范式，本文探索冗余权重与训练样本之间的相互作用。

Method: 开发了同时权重和样本裁剪机制(SWaST)，交替执行权重剪枝和核心集选择，并集成状态保持机制以稳定联合优化。

Result: 实验显示剪枝和核心集选择之间存在强协同效应，在变化的剪枝率和核心集大小下，准确率提升高达17.83%，同时FLOPs减少10%至90%。

Conclusion: 权重剪枝和核心集选择在深度学习中具有协同效应，但需要解决关键双损失问题以确保训练稳定性。
...
TL;DR: PRISM框架通过解耦架构先验，使用不同教师模型分别监督logit匹配和正则化目标，在ImageNet-1K上显著提升了数据集蒸馏的性能和样本多样性。


<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法通常继承单一教师模型的归纳偏差，随着数据集规模增大，这种偏差会导致生成样本过于平滑和同质化，降低类内多样性并限制泛化能力。

Method: PRISM框架将logit匹配和正则化目标解耦，分别使用不同架构的教师模型进行监督：主模型用于logit匹配，随机子集用于批量归一化对齐。还引入了可扩展的跨类批次形成方案用于快速并行合成。

Result: 在ImageNet-1K上，PRISM在低中IPC机制下持续且可重复地优于单教师方法（如SRe2L）和近期多教师变体（如G-VBSM）。生成数据展现出显著更丰富的类内多样性，特征间余弦相似度显著下降。

Conclusion: PRISM通过解耦架构先验有效解决了数据集蒸馏中的归纳偏差问题，生成了更具多样性和泛化能力的合成数据，为大规模数据集蒸馏提供了可行的解决方案。
...
TL;DR: 提出了M²V-UGAD框架，用于处理图数据中节点属性和结构同时缺失情况下的无监督异常检测，通过双路径编码器防止跨视图干扰，并利用硬负样本缓解插补偏差。


<details>
  <summary>Details</summary>
Motivation: 现实图数据中节点属性和结构信息往往同时缺失，现有方法假设数据完整且标准插补方法会修复异常节点使其看起来正常，导致检测性能下降。同时缺失时还会产生跨视图干扰问题。

Method: 使用双路径编码器独立重构缺失的节点属性和图结构，在联合潜在空间中进行融合和正则化，使正常节点占据紧凑内流形而异常位于外表面。通过采样潜在空间外部的编码生成硬负样本来锐化决策边界。

Result: 在七个公开基准测试中，M²V-UGAD在不同缺失率下始终优于现有的无监督图异常检测方法。

Conclusion: 该框架有效解决了图数据中多值缺失问题，防止了跨视图干扰和插补偏差，显著提升了无监督异常检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 基于策略的强化学习在改进有能力的机器人策略时，常因噪声大、梯度信号弱而受阻，需要更稳定的精炼方法。
...
TL;DR: MDMLP-EIA是一种多领域动态MLP模型，通过自适应融合双域季节性MLP、能量不变注意力机制和动态容量调整机制，解决了传统MLP方法在时间序列预测中的季节性信号丢失、容量限制和通道融合不足等问题。


<details>
  <summary>Details</summary>
Motivation: MLP方法虽然参数少、鲁棒性好，但在时间序列预测中存在三个关键问题：弱季节性信号丢失、权重共享MLP的容量限制，以及通道独立策略中的通道融合不足。
...
Conclusion: MDMLP-EIA通过创新的多领域动态MLP架构，有效解决了传统MLP方法的局限性，在时间序列预测任务中实现了优异的性能表现。
...
TL;DR: EEGAgent是一个基于大语言模型的通用脑电图分析框架，能够调度多种工具自动完成EEG相关任务，包括信息感知、时空探索、事件检测、用户交互和报告生成。
...
Result: 在公共数据集上评估显示，EEGAgent支持灵活且可解释的EEG分析，具有现实临床应用潜力。

Conclusion: EEGAgent展示了利用LLM调度工具完成多任务EEG分析的可行性，为临床脑电图分析提供了通用框架。
...
Conclusion: 动态阈值策略在理论上和实验上都优于固定阈值方法，为漂移检测提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 表格数据是最丰富的数据类型，但随着数据集增长并涉及多个相关目标，需要利用共享任务信息来改进多任务泛化。现有多任务学习方法主要关注推荐系统，且大多基于多层感知机，难以捕捉复杂特征交互且无法扩展到大数据量。

Method: 提出MultiTab-Net多任务transformer架构，采用新颖的多任务掩码注意力机制动态建模特征-特征依赖关系，同时缓解任务竞争问题。

Result: 在包括大规模推荐数据、社会经济数据和物理数据集等多个领域的广泛实验中，MultiTab-Net始终比现有MTL架构和单任务transformer获得更高的多任务增益。

Conclusion: MultiTab-Net为表格数据多任务学习提供了有效的transformer解决方案，同时贡献了MultiTab-Bench多任务合成数据集生成器，支持系统评估多任务动态特性。
...
TL;DR: 使用AI Feynman符号回归算法自动重新发现天文学基本方程——中心方程，通过数据预处理和搜索空间限制引入物理偏差，成功恢复了该方程的一阶解析形式，但依赖专家驱动的坐标系选择存在局限性。


<details>
  <summary>Details</summary>
Motivation: 探索如何通过符号回归算法自动重新发现物理学基本方程，特别是天文学中的中心方程，以验证符号回归在物理定律发现中的有效性。

Method: 使用AI Feynman符号回归算法，通过数据预处理和搜索空间限制引入观测和归纳偏差，从月球星历数据中恢复中心方程的一阶解析形式。

Result: AI Feynman成功恢复了中心方程的一阶解析形式，但发现该方法严重依赖专家驱动的坐标系选择，存在局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于神经网络的多模态学习模型在开放世界环境中存在可靠性问题，无法有效应对不可预测的环境组成动态、不完整的模态输入和虚假分布关系等挑战，而人类却能自然适应这种动态模糊场景。

Method: 研究多模态学习在开放世界环境中的鲁棒性问题，分析系统在处理真实世界复杂性时处理多模态信号的局限性。

Result: 识别出当前人工智能系统在开放世界环境中处理多模态信号时存在显著的鲁棒性限制。

Conclusion: 需要开发更鲁棒的多模态学习方法，以应对开放世界环境中的不可预测性和复杂性，确保系统在实际部署中的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 传统数据库参数调优方法收敛速度慢，而各种文档中包含有价值的调优提示信息未被充分利用。

Method: 设计结构化思维链提示让LLM从文档中提取条件感知的调优提示，并提出提示感知的演示强化学习算法HA-DDPGfD。

Result: 在MySQL和PostgreSQL上的实验表明，DemoTuner在性能提升和在线调优成本降低方面显著优于DB-BERT、GPTuner和CDBTune等基线方法。

Conclusion: DemoTuner是首个将演示强化学习引入数据库参数调优的工作，在未知工作负载场景下也表现出优越的适应性。
...
TL;DR: 该论文提出了一种基于量子力学Born规则的相干聚合方法，通过控制相对相位来直接调控协同与拮抗作用，并在轻量级干扰核分类器中实现这一思想。


<details>
  <summary>Details</summary>
Motivation: 传统方法将交互作用视为工程化的乘积项或灵活模型中的涌现模式，缺乏对协同与拮抗作用的直接控制。

Method: 采用量子启发的方法，通过相干聚合（求和复数振幅后平方）与不相干代理（求和平方幅度）的对比，构建干扰核分类器（IKC），并引入相干增益和干扰信息两个诊断指标。

Result: 在XOR等高交互合成任务中，IKC在预算匹配比较中优于强基线；在真实表格数据上整体具有竞争力，但通常略逊于容量最丰富的基线。切换聚合方式从不相干到相干能持续改善负对数似然、Brier分数和预期校准误差。

Conclusion: 量子启发的相干聚合方法为交互作用建模提供了机制层面的直接控制，相干增益在多个数据集上均呈现正值，验证了该方法的有效性。
...
TL;DR: GraphSB是一个解决图学习中类别不平衡节点分类问题的新框架，通过结构平衡策略优化图结构，包含结构增强和关系扩散两个阶段，显著提升了现有方法的性能。
...
Conclusion: GraphSB通过平衡图结构分布有效解决了类别不平衡问题，其结构平衡策略具有通用性和可扩展性。
...
TL;DR: SVD-NO是一种基于奇异值分解的神经算子，通过显式参数化核的SVD来学习偏微分方程的解算子，在五个基准方程上达到了新的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的神经算子方法（如基于傅里叶或图的方法）对核积分算子的结构做出强假设，这可能限制表达能力。

Method: 通过SVD显式参数化核，用两个轻量网络学习左右奇异函数，对角参数矩阵学习奇异值，并使用Gram矩阵正则化器强制正交性。

Result: 在五个不同的基准方程上进行广泛评估，SVD-NO达到了新的最先进水平，特别是在解具有高度空间变化性的PDE上表现更佳。
...
<details>
  <summary>Details</summary>
Motivation: 从观测数据推断因果关系具有挑战性，因为数据常受到各种外部干扰。这些干扰通常是外部因素对观测变量的额外影响，而外部因素往往未知，因此需要引入潜在变量来表示这些未观测因素。

Method: 提出时间潜在变量结构因果模型，包含因果强度和邻接系数来表示变量间的因果关系。基于变分推理开发参数学习方法，能够融入专家先验知识来指导模型估计。

Result: 实验结果表明，所提出的方法具有稳定性和准确性。

Conclusion: 通过引入潜在变量表示外部干扰因素，并结合专家先验知识，提出的时间潜在变量结构因果模型能够有效捕捉因果关系，在实验中表现出良好的性能。
...
TL;DR: MoE模型通过激活部分专家网络减少计算量，但模型规模增长导致参数超出GPU内存，需要将非活跃专家卸载到CPU。现有预取方法在预测失败时会严重影响推理速度或模型精度。


<details>
  <summary>Details</summary>
Motivation: 解决MoE模型中专家预取失败时既要保持高推理速度又要维持模型精度的关键挑战。

Method: 通过将非活跃专家卸载到CPU内存，并使用预取启发式方法来隐藏PCIe传输延迟，但在预取失败时面临性能与精度的权衡。

Result: 现有方法在预取失败时要么产生长延迟（按需获取专家），要么显著降低模型精度（丢弃专家）。

Conclusion: 需要新的解决方案来在预取失败时同时保持高推理速度和模型精度。
...
TL;DR: 该研究分析了蛋白质结构标记化的语义特性，发现结构词汇存在语义冗余，并提出利用"同义词替换"策略生成多样化构象集合，用于模拟蛋白质动态性。


<details>
  <summary>Details</summary>
Motivation: 尽管已有许多蛋白质结构标记化的工作，但对底层离散表示的特性理解不足。研究旨在探索结构标记在语言模型中的有效利用，并理解结构词汇的语义特性。

Method: 首先分析结构标记在结构预测语言模型中的使用，发现需要预训练的序列嵌入来弥合序列和结构之间的语义差距。然后分析结构词汇的语义冗余，并提出"同义词替换"策略来生成构象集合。

Result: 研究表明结构词汇存在显著的语义冗余，多个不同标记对应几乎相同的局部几何结构。利用同义词替换策略可以准确重现蛋白质灵活性，性能与最先进模型相当。
...
TL;DR: 提出了FAQNAS框架，将混合量子神经网络设计建模为多目标优化问题，平衡准确率和FLOPs计算成本。


<details>
  <summary>Details</summary>
Motivation: 在NISQ时代，虽然量子电路不直接以FLOPs衡量，但大多数HQNN仍在经典模拟器上训练，FLOPs直接影响运行时和可扩展性，因此需要将FLOPs作为实用的计算复杂度度量标准。

Method: 开发了FLOPs感知的神经架构搜索框架FAQNAS，将FLOPs明确纳入优化目标，通过多目标优化发现平衡准确率和计算成本的架构。

Result: 在五个基准数据集上的实验表明，量子FLOPs主导准确率提升，而经典FLOPs基本保持不变。帕累托最优解显示相比FLOPs不敏感的基线，能以显著降低的计算成本实现竞争性准确率。
...
TL;DR: 提出了Tree-based Stochastic Optimization (TSO)框架，通过树形动作表示和采样剪枝机制，解决了大规模城市网络安全博弈中纳什均衡计算的挑战。


<details>
  <summary>Details</summary>
Motivation: 大规模城市网络安全博弈(UNSGs)由于巨大的组合动作空间，难以计算纳什均衡。现有方法如PSRO需要计算最佳响应，但在大规模游戏中不实用；而随机优化方法又面临动作空间过大无法有效表示的问题。

Method: TSO框架采用树形动作表示将整个动作空间映射到树结构上，解决了神经网络无法表示不可枚举动作空间的问题。同时引入采样剪枝机制减少陷入局部最优的风险。

Result: 大量实验结果表明TSO在解决UNSGs问题上优于其他基线算法。

Conclusion: TSO框架成功弥合了纳什均衡求解的随机优化范式与UNSGs需求之间的差距，为大规模网络安全博弈提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前后验可解释性方法存在安全漏洞，攻击者可以在不访问模型内部的情况下修改解释结果，这在安全关键应用中可能带来严重风险。

Method: 基于模型预测和解释的单步攻击方法，无需访问模型权重，仅需预测和解释结果。针对saliency maps、integrated gradients和DeepLIFT SHAP等解释方法生成攻击。
...
TL;DR: 本文研究发现，五个主流开源文本到图像生成模型在其预训练潜在空间中系统性地编码和放大了种族与性别相关的社会偏见，特别是在职业表征中存在明显的刻板印象。


<details>
  <summary>Details</summary>
Motivation: 随着文本到图像生成模型在现实应用中的广泛部署，其对社会偏见的再现和放大引发了负责任的AI管理担忧，特别是可能损害组织伦理的种族和性别刻板印象问题。

Method: 对五个最受欢迎的开源模型进行实证研究，使用十个中性职业相关提示词生成每个职业100张图像，共5000张图像，由代表不同种族和性别的多样化人类评估者进行评估。

Result: 所有五个模型都编码并放大了显著的社会偏见：护理和护士角色持续女性化，而企业CEO、政治家、医生和律师等高地位职业绝大多数由男性和白人代表。不同模型还表现出特定模式，如QWEN-Image几乎只关注东亚输出，Kandinsky主导白人个体，SDXL分布相对更广但仍存在偏见。

Conclusion: 这些结果为AI项目经理和从业者提供了关键见解，使他们能够选择公平的AI模型和定制提示词，生成符合负责任AI原则的图像。文章最后讨论了这些偏见风险，并提出了在构建负责任生成式AI系统中减轻偏见的可行策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成模型在物理合理性方面存在明显失败，但这些错误往往无法被现有评估方法检测到，且缺乏自动识别和解释物理错误模式的框架，阻碍了模型的针对性改进。

Method: 扩展Matryoshka表示学习范式到transcoder架构，在多个粒度级别进行分层稀疏特征学习，利用物理合理性分类器的中间表示进行训练，并借助大型多模态模型进行特征解释。

Result: 相比现有方法，实现了更优的特征相关性和特征准确性，无需手动特征工程就能识别多样化的物理相关失败模式，为八个最先进的生成模型提供了有价值的分析见解。

Conclusion: 该框架为评估生成模型的物理合理性建立了基准，揭示了这些模型未能遵循物理约束的具体方式，为未来的模型改进铺平了道路。
...
Method: 利用随机微分方程采样探索构象空间，结合基于列表偏好优化的能量排序机制，引入实用上界近似连续时间生成模型中的长采样轨迹概率。
...
TL;DR: 提出了量子度量编码器(QME)，通过量子启发的嵌入方法改进有限样本下的离线强化学习性能，在三个数据集上平均提升116%以上。


<details>
  <summary>Details</summary>
Motivation: 现实应用中强化学习通常面临样本数量有限的问题，离线强化学习在这种约束下性能往往不理想。需要寻找替代方法来处理有限样本问题。

Method: 引入量子度量编码器(QME)，将状态嵌入到更紧凑且有意义的表示中。对于经典数据，QME是经典可模拟的可训练酉嵌入；对于量子数据，可直接在量子硬件上实现。使用SAC和IQL算法验证效果。

Result: 在三个数据集（各100个样本）上，使用QME嵌入状态和解码奖励训练的离线RL智能体性能显著优于原始状态和奖励训练。平均最大奖励性能：SAC提升116.2%，IQL提升117.6%。

Conclusion: QME嵌入状态表现出低Δ-双曲性，表明改进源于QME诱导的状态空间几何结构改变。这种低Δ-双曲性和QME的有效性可为开发有限样本条件下的高效离线RL方法提供有价值信息。
...
TL;DR: 本文研究使用离散声学标记序列来捕捉动物发声中的时序信息，通过向量量化和Gumbel-softmax向量量化自监督语音模型表示，证明这些标记序列能有效区分叫声类型和个体。


<details>
  <summary>Details</summary>
Motivation: 大多数计算生物声学研究在提取帧级特征时会丢弃时间轴上的顺序信息，忽略了动物发声中重要的时序结构，这限制了从发声中提取完整沟通信息的能力。

Method: 使用HuBERT嵌入通过向量量化和Gumbel-softmax向量量化生成离散声学标记序列，采用Levenshtein距离的k-最近邻算法进行序列分类实验。

Result: 配对距离分析显示生成的标记序列能有效区分四个生物声学数据集中的叫声类型和个体，序列分类实验显示出合理的分类性能。
...
TL;DR: OCE-TS是一种用于时间序列预测的序数分类方法，使用序数交叉熵损失替代MSE损失，在保持预测顺序的同时通过概率输出量化不确定性，在多个数据集上优于现有基准模型。
...
Conclusion: OCE-TS通过序数分类方法有效解决了MSE损失的局限性，提供了不确定性估计并提高了异常值鲁棒性，在时间序列预测任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 当前OOC虚假信息检测方法假设训练和测试数据来自相同分布，但在遇到新新闻领域时性能下降，因为缺乏先验知识。

Method: 使用领域不变变分对齐模块联合编码源域和目标域数据，学习可分离的分布空间领域不变特征；采用测试时训练策略和置信度-方差过滤模块动态更新模型。

Result: 在NewsCLIPpings基准数据集上的广泛实验表明，该方法在大多数领域适应设置下优于最先进的基线方法。

Conclusion: VDT方法通过领域不变特征学习和测试时训练机制，有效提升了OOC虚假信息检测的跨领域适应能力。
...
<details>
  <summary>Details</summary>
Motivation: 半异步联邦学习虽然结合了同步训练效率和异步更新灵活性，但存在参与偏差问题，特别是在非IID数据分布下更加严重。分层架构将参与从单个客户端转移到客户端组，进一步加剧了这一问题。现有研究大多关注传统云-端架构，而忽视了非IID数据在云-边-客户端层次结构中对调度的关键影响。

Method: FedCure通过三个关键规则运行：(1)偏好规则优化联盟形成，最大化集体利益并建立理论稳定的分区以减少非IID导致的性能下降；(2)调度规则将虚拟队列技术与贝叶斯估计的联盟动态相结合，在确保平均速率稳定性的同时减轻效率损失；(3)资源分配规则通过基于估计的联盟动态优化客户端CPU频率来提升计算效率，同时满足延迟要求。

Result: 在四个真实世界数据集上的综合实验表明，FedCure相比四种最先进的基线方法，准确率提升高达5.1倍，同时显著提高了效率，每轮延迟的变异系数最低为0.0223，并在各种场景下保持长期平衡。
...
TL;DR: LLM图推理器对图表示中的对称性缺乏不变性，在节点重索引、边重排序或格式变化时会产生不同输出，存在鲁棒性问题。研究分析了微调对编码敏感性和泛化能力的影响，并提出图序列化的分解方法进行评估。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的图推理器缺乏对图表示对称性的内置不变性，在节点重索引、边重排序或格式变化时会产生不一致的输出，这引发了鲁棒性担忧。

Method: 提出将图序列化分解为节点标记、边编码和语法三个因素，并在综合基准测试套件上评估LLM对这些因素变化的鲁棒性。同时贡献了一套新的谱任务来评估微调推理器的泛化能力。

Result: 结果显示更大的（未微调）模型更鲁棒。微调降低了节点重标记的敏感性，但可能增加对结构和格式变化的敏感性，且不能持续提高在未见任务上的性能。

Conclusion: 大语言模型在图推理任务中对图表示的变化缺乏鲁棒性，微调虽然能改善某些方面的敏感性，但可能在其他方面引入新的问题，且不能保证泛化能力的提升。
...
TL;DR: 提出Heuristic Transformer (HT)方法，通过变分自编码器学习奖励的后验分布，将其与上下文数据集结合作为transformer策略的提示，在多个环境中优于基线方法。
...
Result: 在Darkroom、Miniworld和MuJoCo环境中，HT方法在效果和泛化能力方面均优于可比基线方法。

Conclusion: HT方法为基于信念的增强与基于transformer的决策之间搭建了有前景的桥梁。
...
Method: 基于Transformer架构构建统一多任务大视觉模型，在大规模工业光刻仿真数据集（数十万案例）上训练，支持端到端的掩模生成、光刻仿真和规则违规检测。

Result: 实验结果表明模型有效且具有良好泛化能力，性能显著超越学术基线，能够实现敏捷高保真的光刻仿真。

Conclusion: Unitho通过实现敏捷高保真的光刻仿真，为智能EDA构建了强大的数据基础，验证了其在计算光刻领域的有效性和通用性。
...
TL;DR: Torch-Uncertainty是一个基于PyTorch和Lightning的框架，旨在简化深度神经网络训练和评估，集成不确定性量化技术和方法。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在关键现实应用中由于无法准确量化预测不确定性而受到限制，现有方法缺乏统一的工具来评估和集成这些技术。

Method: 开发了一个基于PyTorch和Lightning的框架，提供无缝的工作流程来评估和集成不确定性量化方法，包括分类、分割和回归任务。

Result: 通过全面的实验结果，对多种不确定性量化方法进行了基准测试，证明了框架的有效性。

Conclusion: Torch-Uncertainty填补了现有工具的空缺，为深度学习中不确定性量化的研究和应用提供了统一的解决方案。
...
TL;DR: OutSafe-Bench是首个为多模态时代设计的最全面的内容安全评估测试套件，包含18,000多个双语文本提示、4,500张图像、450个音频片段和450个视频，涵盖9个关键内容风险类别，并提出了MCRS和FairScore新评估方法。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型(MLLMs)日益集成到日常工具和智能代理中，对其可能输出的不安全内容(如有毒语言、偏见图像、隐私侵犯和有害错误信息)的担忧日益增长，而当前的安全基准在模态覆盖和性能评估方面仍高度有限。

Method: 构建了包含四种模态的大规模数据集，提出多维交叉风险评分(MCRS)来建模和评估不同类别间的重叠和相关性风险，并设计了FairScore可解释自动化多评审加权聚合框架，选择表现最佳的模型作为自适应评审团。

Result: 对9个最先进的MLLMs进行评估，揭示了持续且显著的安全漏洞。

Conclusion: MLLMs中迫切需要强大的安全防护措施。
...
TL;DR: 提出PITE方法，通过多原型对齐策略解决个体治疗效果估计中的混淆偏差问题，在保持局部结构的同时实现跨组对齐。


<details>
  <summary>Details</summary>
Motivation: 现有方法在解决混淆偏差时通常进行全局分布平衡，但忽略了个体异质性和代表个体自然聚类的局部结构信息，这影响了ITE估计的准确性。

Method: PITE首先基于相同治疗下的相似个体定义原型作为聚类中心，然后进行实例到原型匹配，将个体分配到组内最近的原型，并设计多原型对齐策略使匹配的原型在潜在空间中接近。

Result: 在基准数据集上的广泛评估表明，PITE优于13种最先进方法，实现了更准确和稳健的ITE估计。

Conclusion: PITE通过细粒度的原型级对齐减少分布偏移，同时保持处理组和对照组的局部结构，为ITE估计提供了有意义的约束。
...
TL;DR: 提出了DeMABAR算法，用于解决去中心化协作多智能体多臂老虎机问题中的对抗攻击问题，包括全局腐败攻击和拜占庭攻击场景。


<details>
  <summary>Details</summary>
Motivation: 现有的去中心化协作多臂老虎机方法容易受到各种对抗攻击的影响，包括奖励观察腐败和拜占庭智能体的攻击。

Method: 设计了DeMABAR算法，该算法能够处理对抗腐败（所有智能体的奖励观察被有限预算腐败）和拜占庭设置（未知比例的智能体可能是拜占庭智能体）。

Result: 理论分析表明DeMABAR算法能够将个体遗憾限制在仅与腐败预算成正比的附加项内，在拜占庭设置下几乎完全消除对抗攻击的影响。数值实验验证了方法的鲁棒性和有效性。

Conclusion: DeMABAR算法为去中心化协作多臂老虎机问题提供了对抗攻击的鲁棒解决方案，在多种攻击场景下都能保持良好性能。
...
TL;DR: 本文研究了在有参考分布Q参数作为建议的情况下，学习布尔超立方体上乘积分布P的问题。当P和Q的均值向量L1距离较小时，提出了一个高效算法，样本复杂度从Ω(d/ε²)降低到Õ(d¹⁻η/ε²)。


<details>
  <summary>Details</summary>
Motivation: 传统的分布学习需要Ω(d/ε²)样本，但当学习者获得参考分布Q的参数作为建议时，能否利用这一额外信息降低样本复杂度？这探索了建议在学习理论中的作用。

Method: 提出了一种高效算法，当P和Q的均值向量L1距离满足‖p-q‖₁ < εd⁰·⁵⁻Ω(η)时，利用参考分布Q的参数来减少学习P所需的样本数量。

Result: 证明了样本复杂度可以降低到Õ(d¹⁻η/ε²)，相比传统下界Ω(d/ε²)有显著改进，前提是参考分布Q与目标分布P足够接近。

Conclusion: 当提供接近目标分布的参考分布参数时，可以显著降低学习乘积分布的样本复杂度，这为利用先验知识改进学习效率提供了理论依据。
...
TL;DR: 提出了RFF-KPKM和IP-RFF-MKPKM方法，通过随机傅里叶特征解决核幂k均值在大规模数据上的计算瓶颈，并改进聚类分配的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决核幂k均值(KPKM)的两个主要限制：大规模数据的计算负担和缺乏真实质心-样本分配学习导致的噪声鲁棒性不足。

Method: RFF-KPKM使用随机傅里叶特征生成高效低维特征映射，避免完整核矩阵计算；IP-RFF-MKPKM结合可能性和模糊隶属度的优点改进聚类分配。

Result: 在大型数据集上实验表明，所提方法在效率和聚类精度上优于现有最先进方法。

Conclusion: 提出的方法成功解决了KPKM的可扩展性和鲁棒性问题，为大规模核聚类提供了有效的解决方案。
...
TL;DR: AgentEvolver是一个自进化代理系统，通过LLM的语义理解和推理能力驱动自主代理学习，解决了传统方法数据构建成本高、探索效率低和样本利用率差的问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的自主代理开发方法成本高昂且效率低下，需要手动构建任务数据集和强化学习管道，存在数据构建成本高、探索效率低和样本利用率差的问题。

Method: 提出三个协同机制：自我提问（好奇心驱动的任务生成）、自我导航（经验重用和混合策略指导提高探索效率）、自我归因（基于贡献度分配差异化奖励提高样本效率）。

Result: 初步实验表明，AgentEvolver相比传统基于RL的基线方法实现了更高效的探索、更好的样本利用率和更快的适应能力。

Conclusion: AgentEvolver通过整合三种自进化机制，实现了代理能力的可扩展、成本效益高且持续改进的自主学习系统。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界交通数据分布在多个利益相关方之间，现有方法主要处理静态依赖关系，忽略了动态特性，导致性能不佳，需要在满足数据本地化约束的同时建模客户端间的动态空间依赖。
...
Conclusion: FedSTGD能够有效建模和重建动态的客户端间空间依赖关系，在联邦学习环境下实现高性能的交通时间序列预测，同时对超参数变化具有鲁棒性。
...
TL;DR: 本文研究了学习率对深度神经网络中权重和偏置波动的影响，揭示了学习率如何调节训练过程中的探索-利用权衡。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络依赖内部参数的固有波动来有效导航复杂优化景观并实现鲁棒性能。虽然这些波动被认为是逃离局部极小值和提高泛化能力的关键，但它们与基本超参数（特别是学习率）的精确关系仍未得到充分探索。

Method: 我们使用不同的学习率训练模型，并分析相应的参数波动与网络最终准确率的关系。

Result: 研究发现学习率的值直接影响权重和偏置波动的幅度和特性，并建立了学习率、波动模式与模型性能之间的明确联系。
...
<details>
  <summary>Details</summary>
Motivation: 基于扰动的解释方法在实践中被广泛使用，但其可靠性常因模型在特定扰动下的未知行为而受到损害。本文旨在探索不确定性校准与扰动解释之间的关系。

Method: 提出了ReCalX方法，通过重新校准模型来改进解释效果，同时保持模型的原始预测能力。

Result: 实证评估显示，ReCalX能有效减少扰动特定的校准错误，提高解释的鲁棒性，并更好地识别全局重要的输入特征。

Conclusion: 模型在解释性特定扰动下的校准问题直接影响解释质量，ReCalX方法能有效解决这一问题，提升基于扰动的解释方法的可靠性。
...
TL;DR: 本文提出使用数据固有维度（ID）作为不平衡分类任务的度量方法，替代传统的基于样本数量的方法，并在多种数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统的不平衡度量方法仅考虑各类别样本数量，忽略了冗余样本和类别学习难度的差异，而基于训练损失和不确定性的复杂方法需要训练模型。

Method: 使用数据固有维度（ID）作为模型无关的不平衡度量方法，可以轻松集成到各种不平衡缓解方法中。

Result: 在五个不同不平衡比率的数据集上，ID方法持续优于基于样本数量的重加权和重采样技术，且与样本数量结合可进一步提升性能。

Conclusion: 数据固有维度是一种有效且易于计算的不平衡度量方法，能够更好地反映类别间的学习难度差异。
...
TL;DR: 提出Panda方法，通过负数据增强（NDA）而非正数据增强（PDA）来提升视觉语言模型在图像损坏下的鲁棒性，有效抑制预测偏差并降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有测试时适应方法采用正数据增强存在两个主要问题：计算开销大且无法缓解预测偏差，即模型在损坏图像下倾向于预测某些特定类别。

Method: Panda通过负数据增强破坏语义内容，将图像分割成块并随机重组，保留损坏特征但丢弃物体相关信号，然后从原始特征中减去负样本的平均特征来抑制损坏相关成分。

Result: Panda相比PDA方法表现更优，能显著提升多种测试时适应方法的性能，且计算开销极小。

Conclusion: Panda是一种有效的测试时适应方法，能通过负数据增强有效提升模型在图像损坏下的鲁棒性，并可无缝集成到现有框架中。
...
TL;DR: 提出了一种在KINN中弱化速度关系损失函数的方法，用于长期股票预测，通过同时学习未来点预测和点间速度关系来解决时间序列波动、OOD数据和异常值问题。
...
Method: 在损失函数中不仅惩罚预测值与标签数据的误差，还惩罚下一个点预测与前一个点加速度预测之间的误差，在多种AR ANN架构上进行测试。

Result: 在道琼斯指数15年数据上测试显示，在容易在OOD条件下产生虚假行为的归一化敏感激活函数上取得了统计显著的改进。

Conclusion: 该架构解决了自回归模型中归一化破坏数据拓扑结构的问题，通过在ANN变换过程中弱化数据邻域邻近关系保持。
...
TL;DR: 该论文提出Holonorm作为Transformer中Tanh归一化的改进替代方案，解决了Tanh的正交性、线性度和失真问题，同时保持信号方向性和可逆性。


<details>
  <summary>Details</summary>
Motivation: Tanh作为层归一化的替代方案存在正交性、线性度和失真问题，导致其可靠性不足，需要一种更好的归一化方法。

Method: 提出Holonorm方法，具有残差连接和非线性特性，适用于高维张量和向量，能保持信号的正交性、方向和可逆性。

Result: Holonorm能防止激活值爆炸，提高深度Transformer模型的稳定性，同时作为0到1之间的百分比度量更易于理解模型评估。

Conclusion: Holonorm是softsign函数的广义形式，是适合作为Transformer归一化函数的首选方案，比Tanh更可靠且解释性更强。
...
TL;DR: 提出了一个基于信息论的原子数据集压缩框架，将原子数据集压缩建模为最小集合覆盖问题，能够高效识别包含原始数据集最大信息量的最小结构子集。


<details>
  <summary>Details</summary>
Motivation: 机器学习原子间势能需要大量训练数据，但大数据集计算成本高，而小数据集可能遗漏重要的稀有原子环境，影响模型准确性和可靠性。

Method: 将原子数据集压缩建模为最小集合覆盖问题，通过算法识别包含原始数据集最大信息量的最小结构子集，去除冗余信息。

Result: 在GAP-20、TM23和ColabFit的64个数据集上验证，MSC方法能保留异常值、保持数据集多样性，在高压缩率下重现力的长尾分布，优于其他子采样方法。

Conclusion: 该方法在开源QUESTS包中实现，可用于原子建模中的数据子采样、异常检测和以更低成本训练改进的机器学习原子间势能。
...
Method: 采用两阶段深度学习框架，结合两个U-Net模型：一个用于降水检测，另一个用于定量降水估计，以解决雨和无雨事件之间的数据不平衡问题。使用GPM CORRA v07数据作为地面真值，并在IMERG-Final上预训练以增强鲁棒性。

Result: 通过利用多颗地球静止卫星，Oya实现了准全球覆盖，并在与现有竞争性区域和全球降水基线比较中表现出优越性能。
...
TL;DR: LayerNorm在Pre-LayerNorm和Post-LayerNorm transformers中对学习和记忆的影响不同：Pre-LayerNorm中稳定学习，Post-LayerNorm中影响记忆化。
...
Method: 通过分析Pre-LayerNorm和Post-LayerNorm transformers中LayerNorm的作用，特别关注移除LayerNorm参数的影响，并在13个模型和6个视觉与语言数据集上进行验证。

Result: 在Pre-LayerNorm模型中移除LayerNorm参数会加剧记忆化并破坏学习稳定性，而在Post-LayerNorm模型中移除LayerNorm参数能有效缓解记忆化并恢复真实标签。早期层的LayerNorm最为关键，其影响在Pre和Post LayerNorm模型中各不相同。

Conclusion: LayerNorm在transformer架构中扮演着塑造记忆化和学习的关键角色，其影响因架构类型（Pre vs Post）和层位置而异，这为理解transformer工作机制提供了新视角。
...
TL;DR: 提出了一种负责任强化学习（RRL）框架，将情感理解和伦理约束整合到个性化决策系统中，通过约束马尔可夫决策过程优化用户参与度和依从性，同时确保情感对齐和伦理安全。


<details>
  <summary>Details</summary>
Motivation: 现有医疗和行为支持系统中的个性化决策方法通常基于静态规则或参与度最大化启发式，忽略了用户的情感背景和伦理约束，可能导致推荐不敏感或不安全的干预措施。

Method: 将个性化建模为约束马尔可夫决策过程（CMDP），引入多目标奖励函数平衡短期行为参与和长期用户福祉，定义情感感知的状态表示，并可使用任何RL算法（如DQN、PPO）结合安全约束或拉格朗日正则化。

Result: 提出了一个概念性框架，将同理心和责任操作化到机器学习策略优化中，连接安全RL、情感计算和负责任AI。
...
TL;DR: 提出半统一的稀疏字典学习框架，连接经典稀疏模型与现代深度架构，集成Top-K LISTA及其凸变体LISTAConv到LC-KSVD2模型中，实现稀疏编码器和字典在监督或无监督机制下的协同演化。
...
Method: 将严格Top-K LISTA及其凸FISTA变体LISTAConv集成到判别式LC-KSVD2模型中，实现稀疏编码器与字典的协同演化；对凸变体建立PALM风格收敛分析。

Result: 在CIFAR-10上达到95.6%，CIFAR-100上86.3%，TinyImageNet上88.5%，具有更快收敛速度和更低内存成本（<4GB GPU）。
...
TL;DR: 本文提出了一种针对少像素攻击（ℓ₀-ball扰动）的线性边界传播方法，通过将ℓ₀-ball的凸包近似为边界框与不对称缩放ℓ₁-like多面体的交集，显著提升了现有最先进ℓ₀验证器的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有局部鲁棒性验证器通常依赖线性边界传播来处理凸扰动空间，但少像素攻击的ℓ₀-ball扰动空间是非凸的，这限制了验证器的可扩展性。
...
Conclusion: 通过将非凸ℓ₀-ball扰动空间近似为凸包，并开发相应的边界传播方法，可以显著提升少像素攻击验证的可扩展性。
...
Method: 通过认知神经网络(ENNs)框架，在大型结构信息模型表示的基础上获得可扩展的结合亲和力联合预测分布，重点研究了先验网络的重要性以及如何通过合成数据预训练来提高批量贝叶斯优化的下游性能。
...
TL;DR: 提出了一种在Siegel空间上构建判别式神经网络的新方法，包括多类逻辑回归层和全连接层，在雷达杂波分类和节点分类任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: Riemannian对称空间如双曲空间和对称正定流形已成为表示学习的流行空间，但Siegel空间在机器学习任务中尚未充分探索，需要构建相应的神经网络层。

Method: 基于这些空间的商结构和Riemannian对称空间上的向量值距离概念，构建Siegel神经网络的多类逻辑回归层和全连接层。

Result: 在雷达杂波分类和节点分类两个应用中，该方法在所有数据集上都实现了最先进的性能。

Conclusion: 成功展示了在Siegel空间上构建判别式神经网络的有效性，为这一未被充分探索的Riemannian对称空间家族提供了实用的机器学习工具。
...
TL;DR: 提出了一种基于生成模型的学习方法，通过掩码和填充地理空间地图来推断地下矿物位置，并展示了如何整合辅助数据源提升预测性能。


<details>
  <summary>Details</summary>
Motivation: 地下矿物表征成本高昂且具有挑战性，而矿物在脱碳所需的先进能源技术中起着关键作用，需要开发更有效的矿物定位方法。

Method: 开发了一种生成建模方法，通过掩码和填充资源可用性的地理空间地图来推断矿物位置，并整合地球物理源和全国土壤调查等辅助数据源。

Result: 在美国本土的测试数据上，最佳模型在1×1平方英里空间分辨率下达到Dice系数0.31±0.01和召回率0.22±0.02，使用辅助特征可进一步提升推理性能。

Conclusion: 该方法能够有效推断矿物位置，整合辅助数据源可改善性能，并能在没有记录矿物的区域进行模型评估，为矿物勘探提供了有前景的工具。
...
TL;DR: 本文扩展了Alpha-Procrustes黎曼度量族，通过引入广义Bures-Wasserstein、Log-Euclidean和Wasserstein距离的版本，构建了更强大的无限维SPD矩阵度量框架。


<details>
  <summary>Details</summary>
Motivation: 现有的Alpha-Procrustes框架虽然统一了许多经典度量，但缺乏实现这些广义形式的结构组件，限制了在高维和无限维场景中的应用。

Method: 基于单位化Hilbert-Schmidt算子和扩展的Mahalanobis范数，构建了广义GBW和Log-Hilbert-Schmidt距离，并引入了可学习的正则化参数以增强几何稳定性。

Result: 在文献基准测试中，广义度量表现出改进的性能，特别是在处理不同维度和规模的数据集比较时效果显著。
...
TL;DR: 该论文研究了星型结构变分推断的理论性质，证明了变分逼近的存在性、唯一性和自洽性，并推导了逼近误差界限。还开发了基于梯度的方法来计算变分逼近，并探讨了在高斯测度和分层贝叶斯模型中的应用。


<details>
  <summary>Details</summary>
Motivation: 与均值场变分推断不同，星型结构变分推断假设近似后验具有相互依赖的结构，其中根变量影响所有其他变量。这种结构更接近真实的后验分布，但缺乏理论保证。

Method: 使用最优传输理论的思想开发了具有可证明保证的基于梯度算法来计算变分逼近，并推导了星型结构变分逼近的理论性质。

Result: 证明了星型结构变分逼近的存在性、唯一性和自洽性，推导了变分逼近与真实后验之间的定量误差界限，并开发了有效的计算算法。

Conclusion: 该研究将均值场变分推断的理论结果扩展到星型结构设置，为更复杂的依赖结构提供了理论保证，并开发了新的星可分传输映射稳定性结果。
...
<details>
  <summary>Details</summary>
Motivation: 在医疗、自动驾驶和工业控制等领域，直接与环境交互通常不安全或不切实际，需要从历史数据中进行离线强化学习。然而，目前对从离线数据集学习策略所固有的近似误差缺乏统计理解。
...
TL;DR: 提出了Continuum Dropout方法，为神经微分方程提供连续时间正则化，通过交替更新过程模拟dropout机制，提升泛化能力和不确定性量化


<details>
  <summary>Details</summary>
Motivation: 神经微分方程在建模连续时间动态方面表现出色，但难以应用dropout这一深度学习核心正则化技术，容易过拟合

Method: 基于交替更新过程理论，将dropout的开关机制建模为在连续时间中活跃（演化）和非活跃（暂停）状态之间切换的随机过程

Result: 在多个时间序列和图像分类任务中，Continuum Dropout优于现有NDE正则化方法，产生更好校准和更可信的概率估计

Conclusion: Continuum Dropout为NDE提供了有效的正则化框架，显著提升泛化性能，并支持通过蒙特卡洛采样进行不确定性量化
