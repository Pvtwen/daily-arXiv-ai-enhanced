...
### [1] [Multi-Mode Pinching-Antenna Systems: Mode Selection or Mode Combining?](https://arxiv.org/abs/2603.08472)
*Xiaoxia Xu,Xidong Mu,Yuanwei Liu,Arumugam Nallanathan*

Main category: eess.SP

TL;DR: 多模式夹持天线系统(PASS)通过介质波导传输多个正交模式信号，实现模式域多路复用，提出了模式选择和模式组合两种协议，并开发了PSO-KPBF算法进行联合优化，显著提升了多用户通信性能。


<details>
  <summary>Details</summary>
Motivation: 传统单模式夹持天线系统在频谱效率和多用户通信能力方面存在限制，需要开发能够同时传输多个正交模式信号的新系统，以提高通信容量和灵活性。

Method: 提出多模式夹持天线系统(PASS)，支持模式选择和模式组合两种操作协议。采用粒子群优化(PSO)结合KKT参数化波束成形(PSO-KPBF)算法，联合优化发射波束成形、PA位置和传播常数。

Result: 1) 即使使用简单的均匀模式组合设计，多模式PASS也显著优于传统单模式PASS和混合波束成形系统；2) 模式组合实现高频谱效率，而模式选择以较低硬件复杂度近似其性能。

Conclusion: 多模式PASS系统通过模式域多路复用有效提升了多用户通信性能，提出的PSO-KPBF算法成功解决了高度非凸优化问题，为未来高效通信系统设计提供了新思路。
...
### [2] [Joint Inverse Learning of Cognitive Radar Perception and Perception-Action Policy](https://arxiv.org/abs/2603.07051)
*Anoop C,Anup Aprem*

Main category: eess.SP

TL;DR: 提出IPFDDP算法，通过非参数贝叶斯机器学习框架，从认知雷达的行动中联合推断其感知状态和感知驱动行动策略，优于现有方法并能量化不确定性。


<details>
  <summary>Details</summary>
Motivation: 现有对抗性电子对抗框架通常假设已知认知雷达的感知状态或感知-行动策略之一，然后推断另一个，这在对抗环境中不现实。需要能够同时推断两者且能处理不确定性的方法。

Method: 提出IPFDDP算法：使用依赖狄利克雷过程建模感知依赖的行动策略，将基于核的DDP推断嵌入贝叶斯逆粒子滤波框架，联合估计认知雷达的感知状态和感知-行动策略。

Result: IPFDDP在均方误差、KL散度和行动偏好识别准确率方面优于现有方法。主动探测与IPFDDP结合相比随机探测平均减少40%的KL散度收敛时间。

Conclusion: IPFDDP提供了一种有效的对抗性逆学习方法，能同时推断认知雷达的感知和策略，量化不确定性，并通过主动探测提高学习效率，适用于现实对抗环境。
...
### [3] [Demo: An RFSoC-Based Testbed for Over-the-Air Wireless Transceiver at Millimeter Wave Frequency](https://arxiv.org/abs/2603.07209)
*Jeet Tekchandani,Jai Mangal,Sumit J. Darak,Shobha Sundar Ram*

Main category: eess.SP

TL;DR: 基于AMD RFSoC和Sivers半导体模拟前端的毫米波硬件测试平台，支持29.8GHz模拟波束成形通信，具备OFDM基带和数字前端，实现了端到端无线通信和BER性能分析。


<details>
  <summary>Details</summary>
Motivation: 毫米波频段具有大带宽优势，但信号衰减严重，需要窄定向波束进行通信。现有系统需要硬件测试平台来验证毫米波通信在实际射频损伤和波束失准情况下的性能。

Method: 采用AMD RFSoC和Sivers半导体模拟前端构建硬件测试平台，包括基于OFDM的基带物理层（ARM处理器）和数字前端（FPGA），集成高速数据转换器，通过模拟波束成形在29.8GHz实现毫米波通信。

Result: 成功实现了端到端无线通信，并在真实无线信道中分析了射频损伤和波束失准情况下的比特误码率性能。

Conclusion: 该测试平台能够有效支持毫米波模拟波束成形通信的研究，为实际射频损伤和波束对准问题提供了实验验证手段。
...
### [4] [Radar Enabled Adaptive Modulation for Millimeter Wave Integrated Sensing and Communication](https://arxiv.org/abs/2603.07216)
*Jai Mangal,Sumit Darak,Shobha Sundar Ram*

Main category: eess.SP

TL;DR: 提出基于雷达感知的调制方案选择方法，消除传统耗时反馈机制，在ISAC框架中提升吞吐量54-209%


<details>
  <summary>Details</summary>
Motivation: 现有ISAC研究主要关注利用雷达感知进行波束识别，但调制方案选择仍依赖传统耗时的上下行反馈机制，需要更高效的调制方案选择方法

Method: 提出雷达使能的调制方案选择方案，利用雷达感知信息直接选择最佳调制方案，无需传统反馈机制

Result: 在四种不同轨迹下分析性能，相比现有ISAC技术，吞吐量提升54-209%

Conclusion: 雷达使能的调制方案选择能显著提升ISAC系统吞吐量，为6G高速车载通信等应用提供更高效的解决方案
...
### [5] [Millimeter Wave Frontend for Integrated Sensing and Communication System Transceiver on Edge](https://arxiv.org/abs/2603.07220)
...
<details>
  <summary>Details</summary>
Motivation: IEEE 802.11ad标准中模拟波束成形需要耗时的波束对准过程，引入大量开销降低吞吐量。需要消除这一瓶颈。

Method: 设计端到端ISAC基站收发器原型，将数字基带硬件前端与Simulink毫米波前端集成，雷达和通信使用相同波形、频谱和前端，通过雷达检测定位用户实现定向通信。

Result: 即使存在硬件损伤，802.11ad-based ISAC相比理想毫米波前端的标准方案仍能提供34%更高的吞吐量。

Conclusion: 集成感知与通信系统能有效消除波束对准开销，显著提升毫米波通信性能，为实际部署提供了系统化分析框架。
...
### [6] [A Low-Complexity PFA-Based Autofocus Algorithm for Automotive SAR](https://arxiv.org/abs/2603.07332)
*S. Hamed Javadi,André Bourdoux,Adnan Albaba,Hichem Sahli*

Main category: eess.SP

TL;DR: 本文提出了一种用于汽车合成孔径雷达的双层自聚焦策略，将极坐标格式算法与反投影算法结合，通过定位误差补偿提高图像质量，同时保持计算效率。


<details>
  <summary>Details</summary>
Motivation: 在汽车雷达应用中，反投影算法(BPA)因其图像失真较小而被优先采用，但现有自聚焦算法主要针对频域SAR技术设计，缺乏适用于时域算法的高效自聚焦解决方案。

Method: 提出双层自聚焦策略：第一层在PFA域中使用新颖的定位误差补偿自聚焦(LECA)处理流程估计和校正定位误差；第二层将这些校正无缝转移到BPA中，同时扩展相位梯度自聚焦(PGA)技术以增强BPA的定位误差补偿效率。

Result: 通过真实汽车实验验证，提出的流程实现了最先进的图像聚焦和分辨率，为计算高效的SAR成像设定了新的基准。

Conclusion: 该研究填补了时域算法高效自聚焦解决方案的空白，通过将PFA的计算效率与BPA的图像质量优势相结合，为汽车SAR应用提供了高性能且计算可行的成像方案。
...
### [7] [Explainable and Hardware-Efficient Jamming Detection for 5G Networks Using the Convolutional Tsetlin Machine](https://arxiv.org/abs/2603.07336)
...
<details>
  <summary>Details</summary>
Motivation: 5G网络中关键任务服务依赖稳定的射频环境，但面临低功率干扰威胁，这些攻击可能低于链路层可观测性，需要轻量级、可解释的检测方案。

Method: 使用卷积Tsetlin机(CTM)直接在5G同步信号块(SSB)特征上进行干扰检测。CTM基于布尔逻辑子句对量化输入进行操作，支持比特级推理，适合FPGA部署。在真实5G测试床上使用空中SSB数据进行实验验证。

Result: CTM在真实数据集上达到91.53%准确率（CNN为96.83%），但训练速度快9.5倍，内存需求减少14倍（45MB vs 624MB）。为Zybo Z7 FPGA设计了紧凑架构，展示了延迟、功耗和准确性的权衡方案。

Conclusion: CTM为RF域干扰检测提供了实用、可解释且资源高效的替代方案，是边缘部署、低延迟、安全关键5G应用的强有力候选方案，为B5G系统奠定了基础。
...
### [8] [Handover-Aware Power Minimization for Networked LEO Satellite Communications: Joint Cooperative Beamforming and Scheduling](https://arxiv.org/abs/2603.07434)
*Yuchen Zhang,Eva Lagunas,Symeon Chatzinotas,Tareq Y. Al-Naffouri*

Main category: eess.SP

TL;DR: 该论文研究网络化低轨卫星系统中的节能下行传输，考虑切换感知和统计信道状态信息，提出两段帧结构和联合协作波束成形与调度算法。


<details>
  <summary>Details</summary>
Motivation: 低轨卫星星座通过星间链路提供宽带非地面服务，但快速轨道运动导致频繁调度更新和切换，且严格的星上约束（如有限射频链）使得用户调度与协作波束成形紧密耦合，需要解决切换感知的节能传输问题。

Method: 提出两段帧结构分离切换操作与用户平面传输；建立功率消耗模型考虑新建立卫星-用户链路的切换成本和切换期间有效传输窗口减少；使用硬化边界遍历速率度量，制定每帧网络范围功率最小化问题；开发迭代算法结合重加权ℓ2代理、惩罚松弛和分式规划内循环，生成一系列凸二阶锥规划。

Result: 基于时变轨道动力学和帧级服务集演化的仿真显示，相比非协作和预调度协作基线，所提方法在功率-切换权衡方面表现优越，实现持续节能并改善可行性。

Conclusion: 该研究为网络化低轨卫星系统提供了一种切换感知的节能传输框架，通过联合协作波束成形和隐式调度有效解决了切换成本和功率效率的平衡问题。
...
### [9] [Taming the Bessel Landscape: Joint Antenna Position Optimization for Spatial Decorrelation in Fluid MIMO Systems](https://arxiv.org/abs/2603.07450)
*Tuo Wu,Kai-Kit Wong,Baiyang Liu,Kin-Fai Tong,Hyundong Shin*

Main category: eess.SP

TL;DR: 本文针对流体MIMO系统，提出两种交替优化算法来解决发射端和接收端天线位置联合优化问题，以最大化遍历容量。


<details>
  <summary>Details</summary>
Motivation: 流体天线系统应用于MIMO时形成流体MIMO，其空间相关矩阵由零阶贝塞尔函数控制，导致高度非凸的优化问题（贝塞尔景观）。需要有效算法来优化天线位置以最大化容量。

Method: 1. 基于Kronecker信道分解，推导了高信噪比容量近似、闭式容量损失界和N=2时的全局最优天线间距；2. 提出两种交替优化算法：AO-PSO（使用粒子群优化全局探索）和AO-SCA（使用逐次凸近似获得闭式梯度，实现快速收敛）。

Result: 理论分析揭示了问题的内在结构：容量可分解为发射和接收相关矩阵的对数行列式项；建立了与i.i.d.理想MIMO信道的性能差距界；获得了N=2时的全局最优天线间距。提出的算法能有效优化贝塞尔景观。

Conclusion: 通过理论分析和交替优化框架，成功解决了流体MIMO中天线位置优化问题。AO-PSO适合全局探索，AO-SCA提供快速收敛，为流体MIMO系统设计提供了有效工具。
...
### [10] [GP Bandit-Assisted Two-Stage Sparse Phase Retrieval for Amplitude-Only Near-Field Beam Training](https://arxiv.org/abs/2603.07477)
*Zijun Wang,Shawn Tsai,Ye Hu,Rui Zhang*

Main category: eess.SP

TL;DR: 提出两阶段稀疏相位恢复框架，用于多径信道中的仅幅度近场波束训练，通过自适应支撑发现和子空间细化，在低信噪比下提升波束赋形增益70%以上。


<details>
  <summary>Details</summary>
Motivation: 6G超大天线阵列引入显著近场效应，需要鲁棒的近场波束训练策略。由于硬件损伤（相位噪声、频率偏移）经常破坏信号相位，仅幅度信道恢复成为相干波束训练的关键替代方案。现有近场幅度训练方法通常假设简化的视距条件，而远场相位恢复方法缺乏优化训练效率的感知灵活性，且受限于平面波模型，不适合近场传播。

Method: 提出两阶段稀疏相位恢复框架：第一阶段在标准2D DFT波束空间上执行自适应支撑发现，利用近场波束模式诱导的物理引导先验；第二阶段将感知和稀疏相位恢复限制在学习的子空间内，细化信道估计。

Result: 数值结果表明，所提出的自适应流水线始终优于非自适应基线，在低信噪比下将波束赋形增益提高了70%以上。

Conclusion: 该两阶段稀疏相位恢复框架为多径环境中的仅幅度近场波束训练提供了有效解决方案，通过自适应支撑发现和子空间细化，显著提升了波束训练性能，特别适用于存在硬件损伤的实际场景。
...
### [11] [MetaSort: An Accelerated Approach for Non-uniform Compression and Few-shot Classification of Neural Spike Waveforms](https://arxiv.org/abs/2603.07602)
*Luca M. Meyer,Majid Zamani*

Main category: eess.SP

TL;DR: 提出MetaSort算法，同时解决尖峰分类和压缩问题，通过自适应电平穿越算法压缩，潜在特征表示分类，结合元迁移学习利用数据几何信息


<details>
  <summary>Details</summary>
Motivation: 现有尖峰分类研究通常将尖峰分类和压缩分开处理，需要一种能同时解决这两个问题的算法，以实现超低功耗的片上实现

Method: 提出MetaSort算法：1) 自适应电平穿越算法用于高保真压缩尖峰形状；2) 使用潜在特征表示处理分类问题；3) 通过元迁移学习利用数据几何信息保证鲁棒性和判别性

Result: 在体内尖峰数据上的实验表明，MetaSort表现出有前景的性能，证明了其潜力，并激励继续开发超低功耗的片上实现

Conclusion: MetaSort能同时有效处理尖峰分类和压缩问题，结合元迁移学习提高了鲁棒性和判别性，为实现超低功耗的片上尖峰分类系统奠定了基础
...
### [12] [Cooperative Multi-Satellite ISAC Networks: Centralized vs. Distributed Sensing](https://arxiv.org/abs/2603.07622)
*Jeongbin Kim,Jaehong Jo,Seunghyeon Jeon,Wonjae Shin,Yo-Seb Jeon,H. Vincent Poor*

Main category: eess.SP

TL;DR: 本文研究多卫星下行ISAC网络，通过优化波束成形和功率分配实现通信与协同感知，提出集中式和分布式两种协同感知框架，在满足通信要求的同时显著提升感知性能。


<details>
  <summary>Details</summary>
Motivation: 多卫星网络需要同时提供通信服务和协同感知空中目标，但现有方案在通信与感知性能平衡、集中处理信令开销等方面存在挑战，需要设计高效的协同感知框架。

Method: 1) 基于均匀平面阵列设计通信和感知波束成形并优化功率分配；2) 提出集中式框架：各网关将感知观测转发到中央单元，采用稀疏信号恢复联合估计多目标位置；3) 提出分布式框架：各网关独立估计目标位置，仅传输本地估计到中央单元，通过匈牙利算法解决数据关联问题，最小化距离估计误差获得最终目标位置。

Result: 仿真结果表明，所提出的集中式和分布式框架在满足通信性能要求的同时，显著优于现有感知方案。同时评估了所提框架下感知精度与通信功耗之间的权衡关系。

Conclusion: 本文提出的多卫星ISAC网络协同感知框架能有效平衡通信与感知需求，集中式框架提供高精度感知，分布式框架降低信令开销，为卫星ISAC系统设计提供了有效解决方案。
...
### [13] [Secure and Robust Beamforming Design for STAR-RIS-aided MU-MIMO ISAC Systems](https://arxiv.org/abs/2603.07719)
*Rakesh Ranjan,Anshu Mukherjee,Manjesh K. Hanawal,Keshav Singh,Ioannis Krikidis*

Main category: eess.SP

TL;DR: 本文提出了一种基于STAR-RIS的鲁棒安全下行传输框架，用于多用户MIMO ISAC系统，通过联合优化主动波束成形和STAR-RIS被动系数，在存在窃听者的情况下最大化保密率。


<details>
  <summary>Details</summary>
Motivation: STAR-RIS技术为ISAC系统提供了新的可能性，特别是在增强物理层安全方面。传统反射型RIS只能单向调控信号，而STAR-RIS能同时透射和反射信号，为多用户安全通信和感知任务提供了更灵活的调控能力。在存在恶意窃听者的环境中，需要设计鲁棒的安全传输方案来保护机密信息。

Method: 提出一个最坏情况鲁棒波束成形问题，联合优化基站主动发射波束成形和STAR-RIS的被动反射/透射系数。采用交替优化算法，使用S-procedure处理信道不确定性约束，结合半定松弛和惩罚凸凹规划方法获得可处理的次优解。

Result: 仿真结果表明，所提框架在严格感知条件下相比传统反射型RIS系统显著提高了频谱效率，验证了STAR-RIS在增强ISAC系统物理层安全方面的有效性。

Conclusion: STAR-RIS赋能的ISAC系统能够有效提升物理层安全性能，通过联合优化主动和被动波束成形，在满足通信质量、感知要求和安全约束的同时，实现更高的频谱效率和鲁棒性。
...
### [14] [Interlayer Error Calibration for Stacked Intelligent Metasurfaces:Modeling, Algorithms, and Future Perspectives](https://arxiv.org/abs/2603.08040)
*Xing Jia,Hao Liu,Haoxian Niu,Jinbao Li,Xiangyu Ding,Lu Gan*

Main category: eess.SP

TL;DR: 该论文系统研究了堆叠智能超表面(SIM)中的层间误差校准问题，提出了一个集成校准协议和解决方案策略的通用框架，并探讨了多阶段校准方法在缓解几何偏差和提高校准效果方面的有效性。
...
Method: 首先对代表性的建模和硬件引起的缺陷进行分类，然后概述SIM校准的主要挑战，开发一个集成校准协议与相关解决方案策略的通用框架，并研究多阶段校准方法在缓解几何偏差和提高传播系数对齐方面的有效性。

Result: 提出了一个系统性的SIM校准框架，多阶段校准方法能够有效缓解几何偏差，改善校准后的传播系数与实际传播系数之间的对齐。

Conclusion: 该研究为未来实现物理一致且硬件兼容的SIM实现提供了关键的研究机会和实践挑战，有助于推动SIM在下一代无线网络中的实际应用。
...
### [15] [Energy-Aware Multi-Exit TinyML for Smart Zero-Energy Devices](https://arxiv.org/abs/2603.08047)
*Shahab Jahanbazi,Mateen Ashraf,Lieven De Strycker,Jeroen Famaey,Onel L. A. Lopez*

Main category: eess.SP

TL;DR: 提出了一种用于零能耗设备(ZED)的统一智能框架，通过多出口点自适应推理和能量感知电路，在保持检测精度的同时显著降低能耗约29.6%。


<details>
  <summary>Details</summary>
Motivation: 随着智能和自主系统的普及，需要在边缘设备上直接执行智能计算。这对于零能耗设备(ZED)尤其具有挑战性，因为需要同时解决内存、能量供应和推理精度的严格约束。

Method: 设计、训练并部署了一个用于人员检测的微型机器学习(TinyML)模型。该架构在内存中存储单一模型，通过多个出口点实现自适应推理，使计算量能够根据输入难度进行扩展。同时引入辅助能量感知电路，根据可用能量动态调节系统运行。

Result: 与最先进的能量感知单出口TinyML方法相比，该方法实现了约29.6%的能耗降低，同时保持了检测精度。

Conclusion: 提出的框架为ZED平台实现了准确且能量高效的智能计算，通过自适应推理和能量感知机制在严格约束下平衡了精度和能耗需求。
...
### [16] [Chirp-Based Multi-Device Ambient Backscatter Communication and Sensing Enabled by OFDM-AFDM Symbiotic Radio](https://arxiv.org/abs/2603.08109)
*Katia Abtouche,Fikiri Salum Uledi,Ayoub Ammar Boudjelal,Muhammad Bilal Janjua,Zinat Behdad,Cicek Cavdar,Hüseyin Arslan*

Main category: eess.SP

TL;DR: 提出一种基于正交频分复用(OFDM)和仿射频分复用(AFDM)波形正交性的共生无线电系统，实现主通信信号与反向散射通信信号在同一频谱内无干扰共存，支持同时感知和数据传输。


<details>
  <summary>Details</summary>
Motivation: 传统反向散射通信系统存在直接链路干扰(DLI)和反向散射设备间干扰(IBDI)问题，需要在同一频谱内实现主通信与反向散射通信的无干扰共存，同时支持高分辨率感知。

Method: 采用基于啁啾的AFDM波形，利用其固有的处理增益增强弱反向散射信号的检测可靠性；将反向散射传输嵌入仿射域，同时保持可靠的OFDM主通信；为每个反向散射设备分配不同的仿射域偏移来抑制IBDI。

Result: 仿真结果表明，该共存方案在不影响主链路误码率的情况下有效抑制干扰，改善了反向散射通信的漏检概率性能，适用于未来低功耗和抗干扰系统。
...
### [17] [Soft Fault Estimation and Localisation in Y-Shaped Networks using OFDM-Based Signals](https://arxiv.org/abs/2603.08183)
*Ameer Ahmadie,Ndeye Gueye,Virginie Degardin,Vincent Cocquempot*

Main category: eess.SP

TL;DR: 提出一种基于时域传输系数分析的软故障检测、估计与定位方法，使用OFDM方案估计传输系数，通过故障严重度比估计故障强度，基于残差函数确定故障位置。


<details>
  <summary>Details</summary>
Motivation: 有线通信网络中的软故障检测与定位对于维护网络可靠性和性能至关重要，需要一种能够同时检测、估计故障强度并精确定位故障位置的方法。

Method: 基于时域传输系数分析，在无故障和故障情况下分析传输系数；采用OFDM方案估计传输系数；推导故障严重度比来估计故障强度；提出基于残差的函数来确定故障位置。

Result: 在Y形测试装置上进行实验验证，证明了所提方法的有效性。

Conclusion: 该方法能够有效检测、估计和定位有线通信网络中的软故障，为网络维护提供了实用的解决方案。
...
### [18] [Instantaneous Frequency Estimation in Noisy Multicomponent Signals with Interfering Modes Based on Prony Method and Spline Approximation](https://arxiv.org/abs/2603.08198)
*Basile Dubois-Bonnaire,Sylvain Meignen,Kévin Polisano*

Main category: eess.SP

TL;DR: 提出一种基于样条近似的算法，用于去除Prony方法估计多分量信号瞬时频率时产生的振荡，特别适用于含噪声且模态在时频平面上接近的信号。


<details>
  <summary>Details</summary>
Motivation: 多分量信号中模态在时频平面上接近时，Prony方法估计瞬时频率会产生振荡干扰，需要解决这一问题以提高估计精度。

Method: 结合Prony技术和样条近似，首先用Prony方法估计瞬时频率，然后通过样条近似算法去除估计结果中的振荡干扰。

Result: 在各种情况下进行数值应用，证明该方法能有效改善含噪声且模态接近的多分量信号的瞬时频率估计性能。

Conclusion: Prony技术与样条近似相结合的方法能有效处理含噪声且模态接近的多分量信号的瞬时频率估计问题，去除振荡干扰，提高估计精度。
...
### [19] [From Design to Validation: Preparing a LEO-Capable UE for End-to-End System Evaluation](https://arxiv.org/abs/2603.08229)
...
TL;DR: 本文设计了一种适用于低轨卫星连接的5G用户设备，重点解决时变传播延迟和高多普勒频移问题，并开发了基于无人机软件定义无线电的实验平台来模拟透明和再生卫星载荷。


<details>
  <summary>Details</summary>
Motivation: 5G通过低轨卫星系统扩展连接面临重大技术挑战，特别是卫星运动导致的时变传播延迟和高多普勒频移。虽然3GPP Release 17建立了非地面网络的初步框架，但Release 19进一步通过支持再生载荷架构来增强这一努力。

Method: 设计了专门用于低轨卫星连接的5G用户设备，重点开发了管理可变延迟和多普勒补偿的策略。同时构建了基于无人机软件定义无线电的自定义实验平台，能够模拟透明和再生卫星载荷两种架构。

Result: 虽然完整的端到端系统验证尚未完成，但初步实验室测试证实了该架构的可行性，为未来的实验活动奠定了基础。

Conclusion: 本文提出的5G用户设备设计和实验平台为低轨卫星连接提供了可行的技术方案，特别是在处理时变延迟和多普勒效应方面取得了进展，为未来5G非地面网络的进一步发展铺平了道路。
...
### [20] [Fluid Antennas Meet Intelligent Surfaces: Security Analysis of NOMA Systems Under Hardware Impairments](https://arxiv.org/abs/2603.08244)
...
<details>
  <summary>Details</summary>
Motivation: 流体天线系统与可重构智能表面的结合为安全无线通信提供了前所未有的机会，但硬件损伤对这种有前景组合的实际影响尚未得到充分探索。需要研究在现实硬件约束下，流体天线与智能表面结合的NOMA系统的安全性能。

Method: 开发了一个全面的分析框架，捕捉自适应空间分集、智能信号反射和硬件引起失真之间的复杂相互作用。通过新颖的分段线性近似和块相关模型，推导了平均安全块错误率的可处理表达式。

Result: 分析表明，虽然流体天线与智能表面的协同作用为安全增强提供了显著自由度，但实际硬件缺陷会创建性能上限，无论空间分集增益如何。硬件质量成为实现FAS-RIS架构全部潜力的决定性因素。

Conclusion: 理论框架揭示了系统复杂性与可实现安全性能之间的关键设计权衡，硬件质量对实现FAS-RIS架构全部潜力至关重要。研究为实施安全NOMA系统提供了实用设计指南，有效平衡流体智能协作的益处与现实硬件限制的约束。
...
### [21] [Electrocardiogram Classification with Transformers Using Koopman and Wavelet Features](https://arxiv.org/abs/2603.08339)
*Sucheta Ghosh,Zahra Monfared*

Main category: eess.SP

TL;DR: 该研究探索了基于Koopman算子和小波变换特征的Transformer模型在心电图分类中的应用，发现小波特征在二分类任务中表现优异，而Koopman特征在四分类任务中与Transformer结合效果更好，适当的EDMD字典选择能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 心电图分析对检测心脏异常至关重要，但由于生理信号的复杂性和变异性，实现稳健的自动分类具有挑战性。本研究旨在探索结合动态系统理论和深度学习的创新方法。

Method: 使用扩展动态模式分解（EDMD）近似Koopman算子，结合小波变换提取特征，采用Transformer模型进行分类。研究两个任务：二分类（正常vs非正常）和四分类（正常、房颤、室性心律失常、传导阻滞）。

Result: 小波特征在二分类任务中表现优异；Koopman特征与Transformer结合在四分类任务中表现最佳；简单的Koopman-小波混合特征未提升准确率；选择合适的EDMD字典（特别是调参的径向基函数字典）显著提升性能，超越小波基线和混合系统。

Conclusion: Koopman特征学习与Transformer结合在心电图分类中具有显著效果，为将动态系统理论整合到时序分类中提供了有前景的方向，并展示了基于Koopman的重构分析为学习动态提供可解释性见解的潜力。
...
### [22] [Mitigating Mixed-field Interference in Near-field and Far-field Communications: An Antenna Selection Approach](https://arxiv.org/abs/2603.08389)
*Tianyu Liu,Changsheng You,Chao Zhou,Mingjiang Wu,Ming-Min Zhao,Zhaocheng Wang*

Main category: eess.SP

TL;DR: 提出基于天线选择的传输框架，通过关闭部分天线来抑制混合近场-远场系统中的干扰，在保持阵列增益的同时提升系统和速率。


<details>
  <summary>Details</summary>
Motivation: 混合近场-远场系统中，近场和远场信道的非正交性会导致严重的用户间干扰，特别是在采用全阵列最大比传输(MRT)模拟波束成形时，这会降低速率性能。

Method: 提出天线选择传输框架，联合设计天线选择和功率分配。针对两用户场景进行理论分析，揭示干扰抑制与阵列增益增强之间的权衡；针对多用户场景，开发基于惩罚对偶分解(PDD)的两层优化框架，并提出了低复杂度天线关闭策略。

Result: 理论分析表明，仅关闭少量天线就能显著抑制混合场干扰，同时不过多损失阵列增益。数值结果显示，所提方案在干扰抑制和阵列增益损失之间取得了良好平衡，相比多种基线方案获得了显著性能增益。

Conclusion: 天线选择是抑制混合近场-远场系统干扰的有效方法，无需改变天线结构即可实现显著的性能提升，揭示了干扰抑制与阵列增益之间的内在权衡关系。
...
### [23] [Deep Learning based Cross-Receiver Radio Frequency Fingerprint Identification Under Varying Channels](https://arxiv.org/abs/2603.08402)
*Jiashuo He,Yumeng Wang,Feiyang He,Sai Huang,Yiheng Liu,Shuo Chang,Zhiyong Feng*

Main category: eess.SP

TL;DR: 提出一种结合信道鲁棒性和接收机校准的跨接收机射频指纹识别框架，通过DSQ预处理和可训练校准网络实现跨接收机部署下的可靠发射机识别。


<details>
  <summary>Details</summary>
Motivation: 射频指纹识别(RFFI)依赖于设备特定的硬件损伤进行发射机识别，但其性能在跨接收机部署时容易受到接收机差异和无线信道变化的影响，需要同时解决信道鲁棒性和接收机可移植性问题。

Method: 1) 注册阶段：开发信道鲁棒预处理方法构建去噪谱商(DSQ)序列，使用源接收机数据训练DSQCNN；2) 跨接收机部署阶段：构建源和目标接收机的校准数据集，设计可训练校准神经网络(TCNN)学习两者间的非线性映射；3) 级联TCNN-DSQCNN框架实现目标接收机上的鲁棒发射机分类。

Result: 在12个WiFi发射机和3个接收机的仿真实验中，所提方法在24dB信噪比下达到超过90%的准确率，实现了可靠的跨接收机分类。

Conclusion: 这是首个通过结合信道抑制和非线性接收机校准来联合解决信道和接收机可移植性问题的研究，提出的TCNN-DSQCNN框架能够有效应对跨接收机部署中的信道变化和接收机差异挑战。
...
### [24] [Subspace Fusion Sensing for Cooperative ISAC](https://arxiv.org/abs/2603.08419)
*Yining Xu,Cunhua Pan,Jun Tang,Hong Ren,Jiangzhou Wang*

Main category: eess.SP

TL;DR: 提出了一种用于协作集成感知与通信的子空间融合感知算法，通过张量展开构建等效虚拟天线阵列，实现无需数据关联的分布式AP融合感知。


<details>
  <summary>Details</summary>
Motivation: 在集成感知与通信系统中，需要高效处理分布式接入点接收的信号，传统方法存在数据关联复杂、性能受限等问题，需要开发更有效的融合感知算法。

Method: 将多个AP接收信号堆叠为三阶张量，通过张量展开构建等效虚拟天线阵列，然后开发无需数据关联的子空间融合感知算法，并推导了CRLB下界。

Result: 仿真结果表明，与传统技术相比，所提算法在感知性能上具有显著优势，验证了其有效性。

Conclusion: 提出的子空间融合感知算法能够有效处理分布式AP信号，无需复杂的数据关联过程，在集成感知与通信系统中具有良好性能和应用前景。
...
### [25] [Graph Based Semantic Encoder Decoder Framework for Task Oriented Communications in Connected Autonomous Vehicles](https://arxiv.org/abs/2603.08438)
*Soheyb Ribouh,Phil Polo Ditsia Di Ngoma*

Main category: eess.SP

TL;DR: 提出基于图的语义编码解码器（GBSED）用于车联网任务导向通信，通过场景图压缩实现99%带宽节省，同时保持高语义保真度。
...
Method: 提出图基语义编码解码器架构：编码器利用场景图捕捉道路实体的空间语义关系，结合语义压缩算法将图表示压缩99%；解码器重构任务相关表示而非原始数据。

Result: 在SNR高于10dB时语义保真度超过0.9；风险评估预测成功率高；在3GPP CDL信道下鲁棒性强；相比原始图像压缩增益显著。

Conclusion: 该语义通信框架能显著降低通信开销同时保持高语义保真度，是未来6G系统的有前景解决方案。
...
TL;DR: 研究模式相关损耗对耦合芯多芯光纤系统信噪比统计特性的影响，分析其对放大自发辐射噪声和非线性干扰的影响差异，并探讨空间模式色散的联合效应。


<details>
  <summary>Details</summary>
Motivation: 在耦合芯多芯光纤系统中，模式相关损耗会影响信号传输质量，但对其在信噪比统计特性上的具体影响，特别是对放大自发辐射噪声和非线性干扰的不同作用机制，以及空间模式色散的联合效应，尚需深入研究。

Method: 通过数值模拟和理论仿真相结合的方法，深入分析模式相关损耗对接收端放大自发辐射噪声和非线性干扰的影响，以及两者对信噪比的联合贡献。同时研究空间模式色散对模式相关损耗引起的损伤的影响。

Result: 研究表明，模式相关损耗对两种噪声（放大自发辐射噪声和非线性干扰）产生不同的统计特性，这与单模偏振相关损耗的情况存在差异。空间模式色散会进一步影响模式相关损耗引起的损伤，对两种噪声产生联合效应。

Conclusion: 模式相关损耗在耦合芯多芯光纤系统中对信噪比统计特性有重要影响，对放大自发辐射噪声和非线性干扰产生不同的统计分布，且空间模式色散会加剧这种影响。这些发现为优化多芯光纤系统设计提供了重要见解。
...
### [27] [Cost-Driven Representation Learning for Linear Quadratic Gaussian Control: Part II](https://arxiv.org/abs/2603.07437)
*Yi Tian,Kaiqing Zhang,Russ Tedrake,Suvrit Sra*

Main category: cs.LG

TL;DR: 该论文研究从部分高维观测中学习状态表示用于控制的问题，通过成本驱动的状态表示学习方法，为无限时域时不变LQG控制建立了有限样本保证，并提出了两种方法（显式和隐式学习转移函数）。


<details>
  <summary>Details</summary>
Motivation: 解决从部分和高维观测中学习状态表示用于控制的问题，特别是在实际应用中观测通常是部分且高维的情况下，如何学习有效的状态表示来支持最优控制。

Method: 采用成本驱动的状态表示学习方法，学习潜在状态空间中的动态模型来预测累积成本。研究了两种方法：1）显式学习潜在状态的转移函数（Part I中已研究）；2）隐式学习潜在动态（类似于MuZero）。

Result: 为无限时域时不变LQG控制建立了有限样本保证，证明能够找到接近最优的表示函数和接近最优的控制器。技术贡献包括证明了新随机过程的持续激励性。

Conclusion: 成本驱动的状态表示学习方法能够有效解决从部分高维观测中学习状态表示的问题，为无限时域LQG控制提供了理论保证，隐式方法类似于MuZero，具有实际应用价值。
...
### [28] [PolyFormer: learning efficient reformulations for scalable optimization under complex physical constraints](https://arxiv.org/abs/2603.08283)
*Yilin Wen,Yi Guo,Bo Zhao,Wei Qi,Zechun Hu,Colin Jones,Jian Sun*

Main category: cs.LG

TL;DR: PolyFormer 是一种新型物理信息机器学习方法，通过捕捉约束的几何结构并将其转化为高效的多面体重构，从而将问题复杂度与求解难度解耦，实现大规模约束优化的高效求解。


<details>
  <summary>Details</summary>
Motivation: 现实世界优化问题常受复杂物理定律约束，计算可扩展性受限。传统物理信息机器学习仅将物理知识用于正则化学习模型，而本文旨在开发能简化问题本身的PIML方法。

Method: 提出PolyFormer方法，捕捉约束背后的几何结构，将其转化为高效的多面体重构，从而将问题复杂度与求解难度解耦，使现成优化求解器能高效产生可行解。

Result: 在三个重要问题（大规模资源聚合、网络约束优化、不确定性优化）上评估，PolyFormer实现高达6,400倍的计算加速和99.87%的内存减少，同时保持优于或与最先进方法相当的解质量。

Conclusion: PolyFormer为可扩展约束优化提供了高效可靠解决方案，将PIML的范围扩展到科学发现和工程应用中的规范性任务。
...
### [29] [vLLM Hook v0: A Plug-in for Programming Model Internals on vLLM](https://arxiv.org/abs/2603.06588)
...
<details>
  <summary>Details</summary>
Motivation: 当前vLLM实现限制了部署模型内部状态的可编程性，阻碍了测试时模型对齐和增强方法的应用，如基于注意力模式的对抗提示检测或基于激活引导的响应调整。

Method: 开发vLLM Hook插件，通过配置文件指定要捕获的内部状态，支持两种核心功能：被动编程（探测内部状态进行分析）和主动编程（通过修改内部状态干预模型生成）。

Result: vLLM Hook实现了与vLLM的无缝集成，并展示了三个用例：提示注入检测、增强的检索增强生成（RAG）和激活引导。

Conclusion: vLLM Hook填补了vLLM在内部状态可编程性方面的关键空白，为社区提供了强大的模型调试和增强工具，并邀请社区贡献改进。
...
### [30] [How Attention Sinks Emerge in Large Language Models: An Interpretability Perspective](https://arxiv.org/abs/2603.06591)
*Runyu Peng,Ruixiao Li,Mingshu Chen,Yunhua Zhou,Qipeng Guo,Xipeng Qiu*

Main category: cs.LG

TL;DR: 研究发现大语言模型中存在"注意力汇"现象，特别是模型倾向于过度关注输入序列的第一个token。作者揭示了P0汇电路机制，该机制使模型能在两个Transformer块内识别位置0的token并形成注意力汇，且不依赖语义信息。


<details>
  <summary>Details</summary>
Motivation: 大语言模型经常对特定token分配不成比例的注意力，这种现象被称为注意力汇。虽然通常认为这是有害的，但先前研究发现模型对输入序列第一个token的持续关注是一个例外。这种结构性偏差会影响广泛的下游应用，但其形成机制尚不清楚。

Method: 作者追踪了注意力汇在输入第一个token周围的形成过程，识别出一种称为P0汇电路的简单机制。通过分析30B A3B MoE模型从头训练的训练轨迹，研究该机制的出现和发展。

Result: 发现P0汇电路机制使模型能在两个Transformer块内识别位置0的token并形成注意力汇，且不依赖语义信息。该机制在训练早期出现，并逐渐集中在前两层，可能作为追踪预训练收敛状态的信号。

Conclusion: 研究揭示了注意力汇在位置0的形成机制，即P0汇电路。这一机制在训练早期出现并稳定存在，可能为监测预训练收敛提供有用信号，有助于理解大语言模型中的结构性注意力偏差。
...
### [31] [FuzzingRL: Reinforcement Fuzz-Testing for Revealing VLM Failures](https://arxiv.org/abs/2603.06600)
*Jiajun Xu,Jiageng Mao,Ang Qi,Weiduo Yuan,Alexander Romanus,Helen Xia,Vitor Campagnolo Guizilini,Yue Wang*

Main category: cs.LG

TL;DR: 提出一种通过模糊测试和强化学习微调自动生成问题来诱导视觉语言模型出错的方法，以揭示模型漏洞
...
Conclusion: 提出的方法能有效揭示视觉语言模型的脆弱性，为模型安全评估提供了一种自动化工具
...
### [32] [Switchable Activation Networks](https://arxiv.org/abs/2603.06601)
*Laha Ale,Ning Zhang,Scott A. King,Pingzhi Fan*

Main category: cs.LG

TL;DR: SWAN框架通过为每个神经单元添加确定性、输入依赖的二进制门控机制，实现动态计算分配，在保持精度的同时减少冗余计算，统一了稀疏性、剪枝和自适应推理的优势。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络和大规模生成模型虽然性能优异，但计算成本过高，难以在资源受限环境中部署。现有效率技术如dropout、剪枝和低秩分解都有局限性，无法实现动态自适应计算。

Method: 为每个神经单元引入确定性、输入依赖的二进制门控机制，让网络学习何时激活或停用单元，实现动态计算分配。不同于传统剪枝，SWAN学习结构化、上下文依赖的激活模式。

Result: SWAN能够自适应分配计算，减少冗余同时保持精度，支持高效动态推理，并能转换为紧凑的密集模型进行部署，统一了稀疏性、剪枝和自适应推理的优势。

Conclusion: 通过将效率问题重新定义为学习激活控制，SWAN提出了神经计算的新原则：激活不是固定的而是上下文依赖的，指向可持续AI、边缘智能和受生物大脑启发的未来架构。
...
### [33] [Khatri-Rao Clustering for Data Summarization](https://arxiv.org/abs/2603.06602)
*Martino Ciaperoni,Collin Leiber,Aristides Gionis,Heikki Mannila*

Main category: cs.LG

TL;DR: 提出Khatri-Rao聚类范式，通过假设聚类中心来自多个简洁原型集的交互，生成更简洁但同样准确的数据摘要，应用于k-Means和深度聚类。


<details>
  <summary>Details</summary>
Motivation: 传统基于中心的聚类方法生成的数据摘要存在冗余，特别是在具有大量潜在聚类的数据集中效果有限，需要更简洁但准确的数据摘要方法。

Method: 引入Khatri-Rao聚类范式，假设聚类中心来自两个或多个简洁原型集的交互，提出了Khatri-Rao k-Means算法和Khatri-Rao深度聚类框架。

Result: 实验表明Khatri-Rao k-Means在数据摘要的简洁性和准确性之间取得更好平衡；Khatri-Rao深度聚类框架能进一步减少数据摘要大小同时保持准确性。

Conclusion: Khatri-Rao聚类范式能有效减少数据摘要冗余，提高摘要效率，为大规模复杂数据集提供更有效的摘要方法。
...
### [34] [Scale Dependent Data Duplication](https://arxiv.org/abs/2603.06603)
*Joshua Kazdan,Noam Levi,Rylan Schaeffer,Jessica Chudnovsky,Abhay Puri,Bo He,Mehmet Donmez,Sanmi Koyejo,David Donoho*

Main category: cs.LG

TL;DR: 研究发现数据重复对模型训练的影响具有尺度依赖性：随着模型能力增强，语义重复文档的梯度对齐程度提高，而大规模语料库中语义碰撞加速，导致大模型在有限唯一性数据上训练时损失惩罚急剧增加。


<details>
  <summary>Details</summary>
Motivation: 预训练期间的数据重复会损害泛化能力并导致记忆化，但现有去重方法主要关注表面形式匹配，忽略了语义等效文档（如翻译文本）在大规模网络数据中可能产生的冗余训练信号问题。

Method: 通过分析不同规模模型对语义等效文档的梯度对齐程度；使用EmbeddingGemma-300m嵌入1.92亿文档并分析最近邻相似度分布；进行控制实验，在有限唯一文档池中采样进行预训练，观察模型规模与损失惩罚的关系。

Result: 发现重复具有尺度依赖性：大模型对语义重复文档的梯度对齐更强；大规模语料库中语义碰撞加速；有限唯一性数据对小模型影响温和，但对大模型造成急剧增加的损失惩罚，打破简单缩放外推。

Conclusion: 提出了明确的缩放定律，帮助从业者估计预训练语料库语义唯一性有限导致的缩放偏差，识别并解决了未研究的尺度依赖来源，实现了更准确的大规模预测。
...
### [35] [Know When You're Wrong: Aligning Confidence with Correctness for LLM Error Detection](https://arxiv.org/abs/2603.06604)
*Xie Xiaohu,Liu Xiaohu,Yao Benjamin*

Main category: cs.LG

TL;DR: 提出基于输出锚定标记概率的归一化置信度评分方法，用于检测LLM错误和幻觉，无需外部验证


<details>
  <summary>Details</summary>
Motivation: LLM在关键决策系统中部署时，缺乏可靠的置信度评估方法，存在可信度风险

Method: 基于输出锚定标记概率的归一化置信度评分和自评估框架，分析不同训练方法对置信度校准的影响，提出后RL SFT自蒸馏方法

Result: SFT显著改善置信度可靠性（AUROC从0.806提升到0.879），RL方法导致过度自信，自适应RAG仅需58%检索操作恢复95%准确率增益

Conclusion: 提出的置信度评分方法能有效检测LLM错误，SFT训练能产生良好校准的置信度，RL方法需要后处理来恢复置信度可靠性
...
### [36] [Structure-Aware Set Transformers: Temporal and Variable-Type Attention Biases for Asynchronous Clinical Time Series](https://arxiv.org/abs/2603.06605)
...
<details>
  <summary>Details</summary>
Motivation: 电子健康记录是不规则、异步的多变量时间序列。传统方法要么使用网格化方法（需要插补或缺失掩码，可能导致误差或采样策略捷径），要么使用点集标记化（丢失变量内轨迹和时间局部跨变量上下文）。需要一种能恢复这些先验知识的方法。

Method: 提出STAR-Set Transformer，添加参数高效的软注意力偏置：1) 时间局部性惩罚 -|Δt|/τ（可学习时间尺度）；2) 变量类型亲和力 B_{s_i,s_j}（从学习的特征兼容性矩阵）。评估了10种深度融合调度策略。

Result: 在三个ICU预测任务上表现优异：CPR（AUC/APR: 0.7158/0.0026）、死亡率（0.9164/0.2033）、血管加压药使用（0.8373/0.1258），优于常规网格、事件时间网格和先前的集合基线方法。

Conclusion: STAR-Set Transformer通过可学习的时间尺度和变量兼容性矩阵，为时间序列模型提供了实用的上下文感知插件，同时提供了时间上下文和变量交互的可解释性总结。
...
### [37] [LegoNet: Memory Footprint Reduction Through Block Weight Clustering](https://arxiv.org/abs/2603.06606)
*Joseph Bingham,Noah Green,Saman Zonouz*

Main category: cs.LG

TL;DR: LegoNet：一种无需重新训练或微调的神经网络压缩技术，通过构建权重块并进行聚类，实现64倍压缩无精度损失，128倍压缩精度损失小于3%


<details>
  <summary>Details</summary>
Motivation: 随着神经网络应用对精度和性能要求的提高，模型规模和内存占用不断增长，这在缓存和内存有限的嵌入式设备上成为瓶颈，限制了先进神经网络架构的应用。

Method: 提出LegoNet压缩技术，构建整个模型（无论层类型）的权重块，并对这些块进行聚类。使用块而不是单个权重值进行聚类，能够有效压缩模型。

Result: 使用32个4x4块压缩ResNet-50（在Cifar-10和ImageNet上训练），实现64倍内存压缩且无精度损失；使用16个4x4块实现128倍压缩，精度损失小于3%。

Conclusion: LegoNet提供了一种无需重新训练、微调或数据，也不移除权重或改变架构的神经网络压缩方法，在嵌入式设备上具有重要应用价值。
...
### [38] [Valid Feature-Level Inference for Tabular Foundation Models via the Conditional Randomization Test](https://arxiv.org/abs/2603.06609)
*Mohamed Salem*

Main category: cs.LG

TL;DR: 提出一种结合条件随机化测试(CRT)与TabPFN(表格数据概率基础模型)的特征级假设检验方法，可在非线性、相关场景下获得有限样本有效的p值，无需模型重训练或参数假设。


<details>
  <summary>Details</summary>
Motivation: 现代机器学习模型虽然表达能力强大但统计上难以分析，黑盒预测器虽然能取得强经验性能，但很少能为评估单个特征是否包含目标变量信息提供有效的假设检验或p值。

Method: 结合条件随机化测试(CRT)与TabPFN(表格数据的概率基础模型)，CRT通过随机化特征生成参考分布，TabPFN提供高效的概率预测，从而计算特征相关性的p值。

Result: 该方法能在非线性、相关设置下产生有限样本有效的p值，用于条件特征相关性评估，无需模型重训练或参数假设，为黑盒模型提供了统计推断能力。

Conclusion: 提出的CRT+TabPFN组合为特征级假设检验提供了实用解决方案，将统计推断能力引入现代机器学习模型，解决了黑盒预测器缺乏有效假设检验的问题。
...
### [39] [CapTrack: Multifaceted Evaluation of Forgetting in LLM Post-Training](https://arxiv.org/abs/2603.06610)
*Lukas Thede,Stefan Winzeck,Zeynep Akata,Jonathan Richard Schwarz*

Main category: cs.LG

TL;DR: 论文提出CapTrack框架，从能力角度分析LLM微调后的遗忘现象，发现遗忘不仅限于参数知识，还包括鲁棒性和默认行为的系统性漂移，不同微调方法影响不同。


<details>
  <summary>Details</summary>
Motivation: 当前对LLM微调后遗忘的理解主要局限于参数或事实知识的丢失，这种准确性中心的视角不足以描述现代基础模型的系统性行为退化。需要从能力角度重新定义遗忘，以评估模型漂移对用户体验的影响。

Method: 提出CapTrack框架，结合行为分类法和基于现有基准及针对性适配的评估套件，用于分析LLM的能力遗忘。在大规模实证研究中，涵盖不同微调算法、领域和模型家族（包括80B参数模型）。

Result: 遗忘现象超出参数知识范围，鲁棒性和默认行为出现显著漂移。指令微调导致最强的相对漂移，偏好优化相对保守并能部分恢复丢失的能力。不同模型家族差异持续存在，没有通用的缓解方案。

Conclusion: 需要从能力中心视角重新审视LLM遗忘问题，CapTrack框架为系统评估模型漂移提供了有效工具。研究发现微调导致的系统性行为退化比传统认知更广泛，需要针对不同微调方法和模型特性制定缓解策略。
...
### [40] [Consensus is Not Verification: Why Crowd Wisdom Strategies Fail for LLM Truthfulness](https://arxiv.org/abs/2603.06612)
*Yegor Denisov-Blanch,Joshua Kazdan,Jessica Chudnovsky,Rylan Schaeffer,Sheng Guan,Soji Adeshina,Sanmi Koyejo*

Main category: cs.LG

TL;DR: 论文研究发现，在缺乏外部验证器的领域（如事实真实性判断），通过增加推理计算（如Pass@k方法）来聚合多个模型输出并不能提高准确性，反而会放大共享的误解，因为语言模型的错误高度相关。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索在缺乏外部验证器的领域（如事实真实性判断）是否可以通过增加推理计算（类似数学和代码领域使用的Pass@k方法）来提高语言模型的性能。在数学和代码领域，错误的候选答案可以被可靠地过滤掉，但在没有便利验证的领域，这种方法是否有效尚不清楚。

Method: 研究方法包括：1）在五个基准测试和多个模型上进行实验；2）使用投票式聚合方法，将推理成本扩展到原始采样的25倍；3）分析模型在不确定性下的行为，比较模型预测其他模型输出与识别真实性的能力；4）研究模型错误的相关性，包括在分布外随机字符串条件下的输出相关性；5）评估基于置信度加权的效果。

Result: 研究结果显示：1）即使将推理成本增加到25倍，投票式聚合在准确性上没有一致的提升，反而经常放大共享的误解；2）模型在预测其他模型输出方面比识别真实性更擅长，揭示了社会预测与真实性验证之间的分离；3）语言模型的错误高度相关，即使在分布外随机字符串条件下，不同模型也会产生相关的输出；4）基于置信度的加权没有益处，因为自我报告的置信度无法可靠地区分正确和错误的答案。

Conclusion: 结论是：推理时扩展存在边界——在有验证器的领域，额外样本提供更多候选答案供验证器过滤；在没有验证器的领域，额外样本仅仅强化共享的误解。这为语言模型在真实性相关任务中的应用提供了重要限制。
...
### [41] [OptiRoulette Optimizer: A New Stochastic Meta-Optimizer for up to 5.3x Faster Convergence](https://arxiv.org/abs/2603.06613)
*Stamatis Mastromichalakis*

Main category: cs.LG

TL;DR: OptiRoulette是一个随机元优化器，在训练过程中动态选择优化器更新规则，而不是固定使用单一优化器，显著提升了多个图像分类任务上的性能和收敛可靠性。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习训练通常固定使用单一优化器（如AdamW），但不同优化器在不同训练阶段可能有不同效果。本文旨在通过动态选择优化器来提升训练效率和最终性能。

Method: OptiRoulette结合了四种关键技术：预热优化器锁定、从活跃优化器池中随机采样、优化器切换时的兼容性感知学习率缩放、以及故障感知池替换。该方法实现为torch.optim.Optimizer兼容的组件，可通过pip安装。

Result: 在五个图像分类数据集上，OptiRoulette相比AdamW基线显著提升测试准确率：CIFAR-100 (+9.22pp)、CIFAR-100-C (+4.52pp)、SVHN (+0.89pp)、Tiny ImageNet (+9.73pp)、Caltech-256 (+9.74pp)。更重要的是，它在更高目标上表现出更好的收敛可靠性，在10次运行中都能达到设定的验证准确率目标，而AdamW基线无法达到。

Conclusion: OptiRoulette通过动态优化器选择机制，在多个图像分类任务上实现了显著的性能提升和更好的收敛可靠性，为深度学习优化提供了新的有效工具。
...
### [42] [Correlation Analysis of Generative Models](https://arxiv.org/abs/2603.06614)
*Zhengguo Li,Chaobing Zheng,Wei Wang*

Main category: cs.LG

TL;DR: 该论文提出了一种基于两个简单线性方程的统一表示方法，用于描述扩散模型和流匹配模型，并通过理论分析揭示了现有模型中噪声数据与预测目标之间相关性较弱的问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型和流匹配模型使用神经网络从噪声数据预测预定义目标，但缺乏统一的理论框架。作者旨在建立统一表示并分析这些模型的理论特性。

Method: 首先提出使用两个简单线性方程的统一表示方法来描述扩散模型和流匹配模型，然后对该模型进行理论分析。

Result: 理论分析表明，在现有扩散模型和流匹配中，噪声数据与预测目标之间的相关性有时较弱，这可能影响所有模型中起关键作用的预测（或学习）过程。

Conclusion: 该研究为扩散模型和流匹配提供了统一的理论框架，并揭示了现有方法中噪声数据与目标相关性不足的问题，这对改进模型学习过程具有重要意义。
...
### [43] [Annealed Co-Generation: Disentangling Variables via Progressive Pairwise Modeling](https://arxiv.org/abs/2603.06615)
...
<details>
  <summary>Details</summary>
Motivation: 科学应用中多变量协同生成面临高维扩散模型计算负担重和数据不平衡的挑战，需要更高效的建模方法

Method: 将高维扩散建模替换为低维扩散模型，首先训练无条件扩散模型对解耦的因果变量对，推理时通过共享变量耦合这些成对模型实现联合分布恢复，采用共识、加热、冷却三阶段退火过程确保一致性

Result: 在流场补全和抗体生成两个科学任务上展示了框架的灵活性和有效性

Conclusion: 成对块建模而非联合建模是科学应用中多变量协同生成的有效方法，Annealed Co-Generation框架能够实现高效的多变量生成而无需额外训练
...
### [44] [RACER: Risk-Aware Calibrated Efficient Routing for Large Language Models](https://arxiv.org/abs/2603.06616)
*Sai Hao,Hao Zeng,Hongxin Wei,Bingyi Jing*

Main category: cs.LG

TL;DR: RACER：一种用于LLM路由的集合选择方法，通过构建嵌套模型集和风险控制来优化成本-性能权衡


<details>
  <summary>Details</summary>
Motivation: 现有LLM路由器大多依赖单模型选择，容易产生误路由问题。需要一种能够控制误路由风险同时优化成本-性能权衡的方法。

Method: 将LLM路由形式化为α-VOR问题，提出RACER方法：1）通过增强评分构建嵌套模型集；2）利用有限样本集中界限校准阈值，支持可变集合大小和弃权；3）扩展基础路由器输出模型集，后续聚合改进输出。

Result: 理论证明RACER在未见测试数据上实现严格的无分布风险控制（后处理且模型无关）。大量实验验证理论保证，RACER在各种基准测试中持续提升下游准确率。

Conclusion: RACER通过集合选择方法有效解决了LLM路由中的误路由问题，在控制风险的同时优化了成本-性能权衡，为多模型系统提供了可靠的路由解决方案。
...
### [45] [Evo: Autoregressive-Diffusion Large Language Models with Evolving Balance](https://arxiv.org/abs/2603.06617)
*Junde Wu,Minhao Hu,Jiayuan Zhu,Yuyuan Liu,Tianyi Zhang,Kang Li,Jingkun Chen,Jiazhen Pan,Min Xu,Yueming Jin*

Main category: cs.LG

TL;DR: Evo是一个双重潜在轨迹模型，将自回归和基于扩散的语言生成统一在连续演化生成框架中，通过语义成熟度变量平衡两种范式，实现高质量、高效率的文本生成。


<details>
  <summary>Details</summary>
Motivation: 传统上将自回归解码和扩散生成视为分离的范式，作者希望建立一个统一框架，将文本生成重新概念化为潜在流，让模型能够根据不确定性自适应地平衡AR和扩散方法。

Method: Evo将每个token与向量值嵌入关联，该嵌入在语义成熟度变量t_i∈[0,1]上演化。低t_i值对应自信的AR式精炼，高值对应扩散式规划。模型实现为时间条件Transformer，由共享向量场控制，端到端训练以联合推断潜在代码及其演化时间。

Result: Evo 8B在15个多样化基准测试中达到最先进或高度竞争力的结果，包括推理(GSM8K, ARC-C)、代码生成(HumanEval, MBPP)和通用语言理解，同时保持快速推理速度。

Conclusion: Evo为LLM设计提供了新范式，具有强大的生成质量、稳健的符号推理和解码效率，证明了统一AR和扩散方法的可行性。
...
### [46] [Latent Autoencoder Ensemble Kalman Filter for Data assimilation](https://arxiv.org/abs/2603.06752)
*Xin T. Tong,Yanyan Wang,Liang Yan*

Main category: cs.LG

TL;DR: 提出LAE-EnKF方法，通过在学习到的线性稳定潜在空间中重构同化问题，解决标准EnKF在强非线性系统中的性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 标准集合卡尔曼滤波器(EnKF)在高维系统中广泛应用，但在强非线性动态下性能会显著下降，因为卡尔曼更新与底层系统行为存在结构不匹配。

Method: 提出潜在自编码器集合卡尔曼滤波器(LAE-EnKF)，通过学习非线性编码器-解码器、稳定线性潜在演化算子和一致的潜在观测映射，在潜在坐标中构建封闭的线性状态空间模型。

Result: 在代表性非线性和混沌系统上的数值实验表明，LAE-EnKF比标准EnKF和相关潜在空间方法提供更准确和稳定的同化结果，同时保持可比较的计算成本。

Conclusion: LAE-EnKF通过在学习到的线性稳定潜在空间中重构同化问题，有效解决了EnKF在强非线性系统中的局限性，强调结构一致性、稳定性和可解释性，并提供了理论分析和泛化误差界限。
...
### [47] [Distilling and Adapting: A Topology-Aware Framework for Zero-Shot Interaction Prediction in Multiplex Biological Networks](https://arxiv.org/abs/2603.06618)
*Alana Deng,Sugitha Janarthanan,Yan Sun,Zihao Jing,Pingzhao Hu*

Main category: cs.LG

TL;DR: 提出一个用于多路生物网络零样本交互预测的新框架，通过上下文感知表示学习和知识蒸馏解决现有方法在建模多路性、整合结构序列信息以及处理未见实体方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有方法在建模多路生物网络的复杂性方面存在不足：无法充分建模多路性、难以整合结构和序列信息、在零样本预测（针对没有先验邻域信息的未见实体）方面面临困难。

Method: 1. 利用领域特定基础模型生成丰富嵌入；2. 引入拓扑感知图标记器捕捉多路性和高阶连接性；3. 使用对比学习对齐跨模态嵌入；4. 采用师生蒸馏策略实现鲁棒的零样本泛化。

Result: 实验结果表明，该框架在多路生物网络的交互预测方面优于现有最先进方法，为探索各种生物相互作用和推进个性化治疗提供了强大工具。

Conclusion: 该框架通过结合上下文感知表示学习和知识蒸馏，有效解决了多路生物网络中零样本交互预测的关键挑战，为复杂生物系统的理解提供了新的解决方案。
...
### [48] [NEST: Network- and Memory-Aware Device Placement For Distributed Deep Learning](https://arxiv.org/abs/2603.06798)
...
<details>
  <summary>Details</summary>
Motivation: 现有分布式训练框架通常依赖启发式或拓扑无关的搜索，单独处理通信和内存，缺乏设备级内存感知，导致同步增加、通信膨胀、计算利用率低，限制了在真实数据中心网络上的可扩展性和效率。

Method: NEST采用结构化动态规划，在算子图上操作，考虑张量和专家并行配置、分层或任意网络上的显式allreduce延迟、以及内存/计算配置文件。通过跨张量、流水线、数据和专家维度分解并行性，定义混合策略的搜索空间，同时联合优化共置、网络延迟和内存可行性。

Result: 在不同硬件和网络上的评估显示，NEST相比最先进基线实现了高达2.43倍的吞吐量提升、更好的内存效率和改进的可扩展性。

Conclusion: NEST为下一代AI基础设施的并行化策略和数据中心互连的协同设计提供了基础，通过统一考虑网络、计算和内存约束，显著提升了分布式深度学习训练的效率。
...
### [49] [Not all tokens are needed(NAT): token efficient reinforcement learning](https://arxiv.org/abs/2603.06619)
*Hejian Sang,Yuanda Xu,Zhengze Zhou,Ran He,Zhipeng Wang*

Main category: cs.LG

TL;DR: NAT框架通过选择性地更新部分生成token而非全部token，显著降低RL训练的计算和内存开销，同时保持学习效果。


<details>
  <summary>Details</summary>
Motivation: 在大型语言模型的强化学习中，对长思维链轨迹进行全token反向传播的计算成本过高，限制了RL的扩展性。

Method: 提出NAT框架，使用Horvitz-Thompson重加权进行无偏部分token策略梯度估计，采用两种token选择方案：均匀随机采样和随机前缀截断。

Result: 在数学推理基准测试中，NAT仅使用50%的token就能达到全token GRPO的性能，为Qwen3-8B节省18%峰值GPU内存和29%RL训练时间。

Conclusion: NAT提供了一种高效且正交的途径来扩展RL，突破了长轨迹带来的限制，使token预算成为优化的一等公民。
...
### [50] [Combinatorial Allocation Bandits with Nonlinear Arm Utility](https://arxiv.org/abs/2603.07005)
*Yuki Shibukawa,Koichi Tanaka,Yuta Saito,Shinji Ito*

Main category: cs.LG

TL;DR: 提出组合分配赌博机(CAB)问题，关注臂满意度而非最大化匹配数量，设计UCB和TS算法并证明近似遗憾上界


<details>
  <summary>Details</summary>
Motivation: 传统匹配平台仅最大化匹配数量会导致匹配集中在热门参与者，增加其他参与者不满并导致流失，降低平台利润

Method: 提出CAB在线学习问题，引入臂满意度概念；设计UCB算法和TS算法，使用广义线性模型(GLM)建模反馈

Result: UCB算法达到近似遗憾上界，与特殊情况的现有下界匹配；TS算法也获得近似遗憾上界；合成数据实验验证算法有效性

Conclusion: CAB问题能解决匹配平台中匹配集中化问题，提出的算法在理论和实验上均表现良好，有助于提高平台长期利润
...
### [51] [Reward Under Attack: Analyzing the Robustness and Hackability of Process Reward Models](https://arxiv.org/abs/2603.06621)
*Rishabh Tiwari,Aditya Tomar,Udbhav Bamba,Monishwaran Maheswaran,Heng Yang,Michael W. Mahoney,Kurt Keutzer,Amir Gholami*

Main category: cs.LG

TL;DR: 当前最先进的流程奖励模型（PRMs）在对抗性优化压力下存在系统性可利用漏洞，主要作为流畅度检测器而非推理验证器，导致训练信号不可靠。


<details>
  <summary>Details</summary>
Motivation: PRMs正成为LLM推理流程的核心组件，但现有模型可能存在系统性漏洞，需要评估其在对抗性攻击下的鲁棒性，以确保可靠部署。

Method: 提出三级诊断框架：1）静态扰动分析评估表面风格变化和逻辑损坏的检测能力；2）基于梯度的对抗优化攻击；3）RL诱导的奖励破解，训练策略在AIME问题上最大化PRM奖励。

Result: 发现PRMs存在流畅度-逻辑分离：对表面风格变化高度不变（奖励变化<0.1），但对逻辑损坏检测不一致；梯度攻击可大幅提升无效轨迹的奖励；RL训练的策略可获得接近完美的PRM奖励（>0.9），但真实准确率极低（<4%），43%的奖励增益来自风格捷径。

Conclusion: 当前PRMs主要作为流畅度检测器而非推理验证器，存在系统性盲点，不适合作为训练信号。作者发布了PRM-BiasBench和诊断工具包，以支持部署前的鲁棒性评估。
...
### [52] [Combining Adam and its Inverse Counterpart to Enhance Generalization of Deep Learning Optimizers](https://arxiv.org/abs/2603.07122)
*Tao Shi,Liangming Chen,Long Jin,Mengchu Zhou*

Main category: cs.LG

TL;DR: 提出DualAdam优化器，结合Adam和InvAdam的更新机制，在保持收敛性的同时提升泛化性能，通过逃离尖锐最小值找到平坦最小值。


<details>
  <summary>Details</summary>
Motivation: Adam优化器虽然收敛快但泛化性能不佳，通常被认为是因为倾向于收敛到尖锐最小值。需要一种既能保持Adam快速收敛特性，又能找到平坦最小值以提升泛化性能的方法。

Method: 1. 提出InvAdam：采用与Adam相反的参数更新机制（一阶矩和二阶矩的元素级乘法而非除法），旨在增大二阶矩大时的步长，帮助逃离尖锐最小值。2. 提出DualAdam：结合Adam和InvAdam的更新机制，确保收敛性同时提升泛化性能。3. 使用扩散理论数学证明InvAdam逃离尖锐最小值的能力。

Result: 在图像分类任务和大语言模型微调上的实验表明，DualAdam在泛化性能上优于Adam及其最先进的变体。

Conclusion: DualAdam通过结合Adam和InvAdam的优势，成功解决了Adam泛化性能不足的问题，既能保持快速收敛，又能找到平坦最小值，显著提升了深度学习模型的泛化能力。
...
### [53] [From ARIMA to Attention: Power Load Forecasting Using Temporal Deep Learning](https://arxiv.org/abs/2603.06622)
*Suhasnadh Reddy Veluru,Sai Teja Erukude,Viswa Chaitanya Marella*

Main category: cs.LG

TL;DR: 本文对传统统计模型和深度学习方法在短期电力负荷预测中的表现进行了实证评估，发现基于自注意力机制的Transformer模型在24小时预测中表现最佳，MAPE仅为3.8%。


<details>
  <summary>Details</summary>
Motivation: 准确的短期电力负荷预测对于有效管理、优化和确保现代电力系统的稳健性至关重要。本文旨在通过实证评估比较传统统计模型和深度学习方法的预测性能。

Method: 使用PJM每小时能耗数据，评估了四种模型：ARIMA、LSTM、BiLSTM和Transformer。数据处理包括插值、归一化和滑动窗口序列方法。使用MAE、RMSE和MAPE指标评估24小时预测性能。

Result: 在所有测试模型中，基于自注意力算法的Transformer模型表现最佳，MAPE为3.8%，在准确性和鲁棒性方面均优于其他模型。

Conclusion: 这些发现强调了基于注意力的架构在准确捕捉电力消耗数据中复杂时间模式方面的巨大潜力。
...
### [54] [Making LLMs Optimize Multi-Scenario CUDA Kernels Like Experts](https://arxiv.org/abs/2603.07169)
*Yuxuan Han,Meng-Hao Guo,Zhengning Liu,Wenguang Chen,Shi-Min Hu*

Main category: cs.LG

TL;DR: 本文提出了MSKernelBench基准测试和CUDAMaster系统，用于多场景GPU内核自动化优化，在多种算子中实现显著加速，性能优于现有方法并接近商业库。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的GPU内核自动化优化方法主要局限于机器学习应用（如PyTorch算子），忽视了科学计算等更广泛领域（如稀疏矩阵运算）。扩展到这些领域带来了基准测试和算法的新挑战，因此开发通用自动化内核优化方法成为主要关注点。

Method: 1. 提出MSKernelBench基准测试，覆盖多种场景：基础代数运算、常见LLM内核、稀疏矩阵算子、科学计算例程，支持FP32和BF16精度；2. 基于此基准提出CUDAMaster系统，这是一个多智能体、硬件感知的内核优化系统，利用性能分析信息并自动构建完整的编译和执行工具链。

Result: 实验结果表明，CUDAMaster在大多数算子中实现了显著加速，性能比Astra高出约35%。在多个案例中，其性能达到或超过了高度优化的闭源库（如cuBLAS）的水平。

Conclusion: 本文通过MSKernelBench基准测试和CUDAMaster系统，为多场景GPU内核自动化优化提供了系统评估框架和有效解决方案，展示了在广泛领域中实现高性能优化的潜力。
...
### [55] [Advances in GRPO for Generation Models: A Survey](https://arxiv.org/abs/2603.06623)
*Zexiang Liu,Xianglong He,Yangguang Li*

Main category: cs.LG

TL;DR: Flow-GRPO将GRPO扩展到生成模型，实现稳定强化学习对齐，已成为现代生成模型的通用对齐框架，在方法改进和多模态应用方面快速发展。
...
Method: Flow-GRPO扩展了Group Relative Policy Optimization (GRPO)到生成模型，通过强化学习对齐生成系统。该方法在原始框架基础上进行了多方面改进，包括奖励信号设计、信用分配、采样效率、多样性保持、奖励黑客缓解和奖励模型构建等。

Result: Flow-GRPO引发了快速的研究增长，已扩展到多种生成范式和模态，包括文本到图像、视频生成、图像编辑、语音音频、3D建模、具身视觉语言动作系统、统一多模态模型、自回归和掩码扩散模型以及修复任务。

Conclusion: Flow-GRPO作为现代生成模型的通用对齐框架，通过理论洞察和实践适应展示了其重要性。未来需要解决可扩展和鲁棒的基于强化的生成的关键开放挑战。
...
### [56] [Adversarial Latent-State Training for Robust Policies in Partially Observable Domains](https://arxiv.org/abs/2603.07313)
*Angad Singh Ahuja*

Main category: cs.LG

TL;DR: 论文提出对抗性潜在初始状态POMDP框架，通过潜在极小极大原理和近似最佳响应证书，在部分可观测强化学习中应对潜在分布偏移的鲁棒性问题。


<details>
  <summary>Details</summary>
Motivation: 部分可观测强化学习在潜在分布偏移下的鲁棒性仍然具有挑战性，需要形式化框架来分析和缓解最坏情况下的脆弱性。

Method: 形式化对抗性潜在初始状态POMDP设置，证明潜在极小极大原理，推导有限样本保证的近似最佳响应证书，并在Battleship基准上进行迭代最佳响应训练。

Result: 在Battleship基准上，针对偏移潜在分布的有针对性暴露将Spread和Uniform分布之间的平均鲁棒性差距从10.3次射击减少到3.1次射击；迭代最佳响应训练表现出与近似证书理论一致的预算敏感行为。

Conclusion: 对于潜在初始状态问题，该框架提供了精确的诊断原则，并证实结构化对抗性暴露能有效缓解最坏情况下的脆弱性。
...
### [57] [Pavement Missing Condition Data Imputation through Collective Learning-Based Graph Neural Networks](https://arxiv.org/abs/2603.06625)
*Ke Yu,Lu Gao*

Main category: cs.LG

TL;DR: 提出基于集体学习的图卷积网络方法，用于填补路面状况数据中的缺失值，通过捕捉相邻路段间的依赖关系来提高填补准确性。


<details>
  <summary>Details</summary>
Motivation: 路面状况数据对道路网络维护至关重要，但常因传感器错误和非定期检查等原因存在缺失。系统性缺失数据会导致信息损失、统计能力下降和评估偏差。现有方法要么丢弃含缺失值的数据点，要么仅通过数据相关性进行填补。

Method: 采用基于集体学习的图卷积网络，整合相邻路段的特征和观测路段条件之间的依赖关系来学习缺失条件值。该方法能够捕捉相邻路面路段条件之间的依赖关系，不同于其他图神经网络变体。

Result: 使用德克萨斯州交通部奥斯汀地区收集的路面状况数据进行案例研究，实验表明所提模型在填补缺失数据方面能够产生有希望的结果。

Conclusion: 基于集体学习的图卷积网络方法能够有效处理路面状况数据中的缺失值问题，通过利用相邻路段间的空间依赖关系，为路面维护决策提供更完整准确的数据支持。
...
### [58] [Grouter: Decoupling Routing from Representation for Accelerated MoE Training](https://arxiv.org/abs/2603.06626)
*Yuqi Xu,Rizhen Hu,Zihan Liu,Mou Sun,Kun Yuan*

Main category: cs.LG

TL;DR: Grouter通过从完全训练的MoE模型中提取高质量结构作为固定路由器，将结构优化与权重更新解耦，显著加速MoE训练收敛，提升数据利用率和训练吞吐量。


<details>
  <summary>Details</summary>
Motivation: 传统MoE训练中，结构优化和权重更新同时进行，导致收敛缓慢且不稳定。需要在巨大的组合空间中同时训练专家权重和搜索最优路由策略，这种纠缠影响了训练效率。

Method: 提出Grouter预路由方法：1）从完全训练的MoE模型中提取高质量结构作为固定路由器；2）引入专家折叠技术适应不同模型配置；3）使用专家调优重新平衡不同数据分布的工作负载；4）利用结构先验实现针对性优化提升训练吞吐量。

Result: Grouter显著提升训练性能：预训练数据利用率提高4.28倍，训练吞吐量加速最高达33.5%，同时实现更优的模型收敛质量。

Conclusion: 预路由方法为可扩展的MoE训练建立了基本范式，通过解耦结构优化和权重更新，显著加速训练过程并提升效率。
...
### [59] [Reject, Resample, Repeat: Understanding Parallel Reasoning in Language Model Inference](https://arxiv.org/abs/2603.07887)
*Noah Golowich,Fan Chen,Dhruv Rohatgi,Raghav Singhal,Carles Domingo-Enrich,Dylan J. Foster,Akshay Krishnamurthy*

Main category: cs.LG

TL;DR: 该论文使用粒子滤波（特别是序贯蒙特卡洛SMC）的视角，从理论上研究推理时聚合和剪枝多个样本的方法，分析其准确性与计算成本的权衡。


<details>
  <summary>Details</summary>
Motivation: 当前推理时聚合和剪枝多个样本的方法虽然强大，但缺乏对其准确性与成本权衡的理论理解。论文旨在通过粒子滤波的理论框架，为这类方法提供严格的理论基础。

Method: 采用粒子滤波算法（特别是序贯蒙特卡洛SMC）的理论视角，给定基础语言模型和过程奖励模型，研究在有限过程奖励评估次数下从目标分布采样的准确性。提出了SMC的非渐进保证条件、算法改进，并分析了粒子滤波方法的基本限制。

Result: 理论上识别了：(1) 实现SMC非渐进保证的简单条件；(2) SMC的算法改进；(3) 所有粒子滤波方法面临的基本限制。实证表明，理论条件能有效控制SMC的采样误差，但不一定能保证最终准确性，表明可能需要超越采样的理论视角。

Conclusion: 通过粒子滤波的理论框架为推理时样本聚合方法提供了严格分析，揭示了采样误差与最终准确性之间的差异，指出未来需要超越采样理论的研究方向来全面理解这类方法的性能。
...
### [60] [Leakage Safe Graph Features for Interpretable Fraud Detection in Temporal Transaction Networks](https://arxiv.org/abs/2603.06632)
*Hamideh Khaleghpour,Brett McKinney*

Main category: cs.LG

TL;DR: 提出一种时间敏感、无信息泄露的图特征提取方法，用于时序交易网络的非法实体检测，通过因果图特征增强传统交易属性检测


<details>
  <summary>Details</summary>
Motivation: 传统非法交易检测主要依赖交易级别属性，但欺诈行为也可能通过网络结构特征（如中心枢纽、高流量中介、协调社区）表现出来，需要开发能够捕捉这些结构特征且避免前瞻偏差的方法

Method: 使用时序交易网络构建有向交易图，计算可解释的结构描述符（度统计、PageRank、HITS分数、k-core指数、邻域可达性等），并计算因果变体以防止前瞻偏差，使用随机森林分类器进行严格时间分割训练

Result: 在Elliptic数据集上，模型在保留的未来测试期表现良好（ROC-AUC约0.85，平均精度约0.54），图特征虽非主导预测信号但提供互补可解释性，校准模型在概率可靠性和分类性能方面表现更好

Conclusion: 因果图特征提取是时序欺诈检测流程中实用且可解释的增强方法，能够为调查工作流提供风险上下文分析，尽管交易属性仍是主要预测信号，但图特征提供了有价值的补充信息
...
### [61] [Bayesian Transformer for Probabilistic Load Forecasting in Smart Grids](https://arxiv.org/abs/2603.07899)
*Sajib Debnath,Md. Uzzal Mia*

Main category: cs.LG

TL;DR: 提出贝叶斯Transformer框架，集成三种不确定性机制，用于概率负荷预测，在极端天气下保持良好校准


<details>
  <summary>Details</summary>
Motivation: 现代电网需要具有良好校准不确定性估计的概率负荷预测，现有深度学习模型在极端天气分布偏移下会产生过度自信的点预测

Method: 基于PatchTST骨干网络，集成蒙特卡洛Dropout（认知不确定性）、变分前馈层（对数均匀权重先验）和随机注意力（可学习高斯噪声扰动），结合七级多分位数弹球损失预测头和训练后等渗回归校准

Result: 在五个电网数据集上表现最优，PJM数据集24小时预测CRPS为0.0289，比深度集成提升7.4%，比确定性LSTM提升29.9%，90%置信水平下PICP为90.4%，预测区间最窄（4,960 MW），在热浪和寒潮事件中保持良好校准

Conclusion: 贝叶斯Transformer框架通过集成多种不确定性机制，在极端天气下保持良好校准，支持基于风险的备用容量规划、随机机组组合和需求响应激活
...
### [62] [A new Uncertainty Principle in Machine Learning](https://arxiv.org/abs/2603.06634)
*V. Dolotin,A. Morozov*
...
### [63] [Amortizing Maximum Inner Product Search with Learned Support Functions](https://arxiv.org/abs/2603.08001)
*Theo X. Olausson,João Monteiro,Michal Klein,Marco Cuturi*

Main category: cs.LG

TL;DR: 提出摊销最大内积搜索方法，通过神经网络直接预测MIPS解，针对固定查询分布和键集摊销计算成本


<details>
  <summary>Details</summary>
Motivation: 最大内积搜索是机器学习中的关键子程序，传统方法计算成本高。作者希望针对特定查询分布，通过学习方式摊销搜索成本，提高效率

Method: 提出两种互补方法：1) SupportNet - 使用输入凸神经网络直接建模支持函数，通过自动微分恢复最优键；2) KeyNet - 使用向量值网络直接从查询回归最优键，绕过梯度计算。引入同质化包装器确保正1-齐次性，并使用分数回归、梯度匹配损失和欧拉定理推导的分数一致性损失进行训练

Result: 实验表明，学习到的SupportNet或KeyNet能够实现高匹配率，并为针对特定查询分布压缩数据库开辟了新方向

Conclusion: 摊销MIPS通过学习方式直接预测MIPS解，针对固定查询分布摊销计算成本。该方法将MIPS值函数视为键集的支持函数，利用其凸性和齐次性性质，通过神经网络建模实现高效搜索
...
### [64] [Graph Property Inference in Small Language Models: Effects of Representation and Inference Strategy](https://arxiv.org/abs/2603.06635)
*Michal Podstawski*

Main category: cs.LG

TL;DR: 小规模语言模型在图属性推理任务中，性能高度依赖于输入表示的结构化组织和推理策略设计，而非仅由模型规模决定。
...
Method: 对小规模指令调优语言模型进行图论属性推理的系统研究，分离输入表示和推理策略的作用，测试多种局部和全局图度量。

Result: 结构性能对关系信息的组织方式高度敏感：保持邻域结构的表示能提高估计稳定性和顺序一致性；多分支推理在不同配置下产生最可靠的总体增益。

Conclusion: 小语言模型的图属性推理能力关键取决于表示组织和推理设计，结构化能力不仅受模型规模影响，还受关系信息编码方式和预测引导方式影响，这为在有限模型容量下改进结构化推理提供了实用杠杆。
...
### [65] [Explainable Condition Monitoring via Probabilistic Anomaly Detection Applied to Helicopter Transmissions](https://arxiv.org/abs/2603.08130)
*Aurelio Raffa Ugolini,Jessica Leoni,Valentina Breschi,Damiano Paniccia,Francesco Aldo Tucci,Luigi Capone,Mara Tanelli*

Main category: cs.LG

TL;DR: 提出一种基于健康数据的可解释性条件监测方法，仅学习健康数据分布，通过概率偏差度量检测异常，支持不确定性量化和结果解释


<details>
  <summary>Details</summary>
Motivation: 由于故障是罕见事件，难以获取足够的故障数据进行监督学习，因此需要一种仅依赖健康数据的异常检测方法，同时需要可解释性以支持安全关键应用

Method: 基于贝叶斯框架，仅学习健康观测数据的概率分布，定义偏离正常状态的概率度量来检测和预测故障，提供不确定性量化和描述性工具增强结果可解释性

Result: 在两个用例上验证：公开的预测性维护基准和真实直升机传动系统多年数据集，方法在异常检测性能上达到与最先进方法竞争的水平

Conclusion: 提出的基于健康数据的可解释性条件监测方法有效，能够检测和预测故障，支持不确定性量化，适用于安全关键应用，在真实场景中表现良好
...
### [66] [SmartBench: Evaluating LLMs in Smart Homes with Anomalous Device States and Behavioral Contexts](https://arxiv.org/abs/2603.06636)
*Qingsong Zou,Zhi Yan,Zhiyao Xu,Kuofeng Gao,Jingyu Xiao,Yong Jiang*

Main category: cs.LG

TL;DR: SmartBench：首个面向LLM的智能家居异常检测数据集，评估13个主流LLM发现现有模型在智能家居异常检测上表现不佳，Claude-Sonnet-4.5在上下文无关异常上仅66.1%准确率，上下文相关异常上仅57.8%。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能家居助手研究主要关注用户指令理解和执行，但智能家居助手的关键功能是检测家庭环境异常状态，需要LLM准确判断异常并提供解释或建议。

Method: 提出SmartBench数据集，包含正常和异常设备状态以及正常和异常设备状态转换上下文，在13个主流LLM上进行评估。

Result: 实验显示大多数最先进模型无法实现良好的异常检测性能，Claude-Sonnet-4.5在上下文无关异常上准确率66.1%，上下文相关异常上57.8%，表明当前LLM智能家居助手远不能有效检测和处理异常。

Conclusion: 下一代基于LLM的智能家居助手在有效检测和处理智能家居环境异常条件方面仍有很大差距，需要进一步研究提升异常检测能力。
...
### [67] [Are We Winning the Wrong Game? Revisiting Evaluation Practices for Long-Term Time Series Forecasting](https://arxiv.org/abs/2603.08156)
*Thanapol Phungtua-eng,Yoshitaka Yamamoto*

Main category: cs.LG

TL;DR: 论文批判当前长期时间序列预测领域过度依赖点误差指标（如MSE、MAE）的基准测试文化，认为这种"游戏化"评估方式与真实世界预测需求脱节，提出需要多维度的评估框架。


<details>
  <summary>Details</summary>
Motivation: 当前LTSF领域已演变为以基准测试为中心的"游戏"，模型主要通过MSE、MAE等点误差指标的微小改进来排名和宣称SOTA。这种度量中心的评估方式与真实世界预测的广泛目标（如保持时间结构、趋势稳定性、季节一致性、对制度转移的鲁棒性、支持下游决策等）存在结构性错位。

Method: 论文提出一个多维度的评估视角，整合统计保真度、结构一致性和决策层面的相关性。通过挑战当前的度量单一文化，旨在将注意力从赢得基准测试表格转向推进有意义、上下文感知的预测。

Result: 论文没有提供具体的实验结果，而是提出了一个概念性的评估框架转变。主要结果是批判性地分析了当前LTSF评估范式的局限性，并提出了替代的评估方向。

Conclusion: 需要重新思考LTSF评估的基础问题：什么才是衡量预测进展的真正意义？应该从单一的度量驱动评估转向整合统计、结构和决策相关性的多维度评估框架，以促进更有意义、上下文感知的预测研究。
...
### [68] [HEARTS: Benchmarking LLM Reasoning on Health Time Series](https://arxiv.org/abs/2603.06638)
*Sirui Li,Shuhan Xiao,Mihir Joshi,Ahmed Metwally,Daniel McDuff,Wei Wang,Yuzhe Yang*

Main category: cs.LG

TL;DR: HEARTS是一个用于评估大语言模型在健康时间序列数据上分层推理能力的统一基准，包含16个真实数据集、12个健康领域、20种信号模态和110个任务，发现LLMs在健康时间序列推理上表现不佳且与通用推理能力相关性弱。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅覆盖少量健康时间序列模态和任务，无法反映真实世界生理建模中的多样化领域和复杂时间依赖性，需要建立更全面的评估框架来推动LLMs在健康时间序列推理方面的发展。

Method: 构建HEARTS基准，整合16个真实世界数据集，涵盖12个健康领域和20种信号模态，定义包含感知、推理、生成和演绎四个核心能力的110个任务分类体系，并在超过2万个测试样本上评估14个最先进的LLMs。

Result: LLMs在健康时间序列任务上显著落后于专用模型，其表现与通用推理分数相关性弱；LLMs常依赖简单启发式方法，难以处理多步时间推理；随着时间复杂性增加，性能下降，模型家族内出现相似失败模式，表明单纯扩展模型规模不足够。

Conclusion: HEARTS通过量化LLMs在健康时间序列推理方面的差距，为开发下一代能够处理多样化健康信号推理的LLM智能体提供了标准化测试平台和持续更新的基准。
...
### [69] [Towards plausibility in time series counterfactual explanations](https://arxiv.org/abs/2603.08349)
*Marcin Kostrzewa,Krzysztof Galus,Maciej Zięba*

Main category: cs.LG

TL;DR: 提出一种基于梯度优化和soft-DTW对齐的时间序列分类反事实解释生成方法，通过整合目标类k近邻确保时间结构合理性。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列反事实解释方法在保持现实时间结构方面存在不足，生成的解释可能缺乏时间合理性，需要一种能产生既有效又具有时间真实性的反事实解释方法。

Method: 采用梯度优化直接在输入空间搜索反事实。核心创新是整合soft-DTW动态时间规整与目标类的k近邻对齐，通过多目标损失函数平衡有效性、稀疏性、邻近性和新颖的基于soft-DTW的合理性约束。

Result: 在多个维度评估中，该方法在有效性方面表现竞争性，在目标类分布对齐方面显著优于现有方法，表明具有更优的时间真实性。定性分析进一步凸显了现有方法在保持现实时间结构方面的关键局限性。

Conclusion: 该方法能够为时间序列分类器生成既有效又高度合理且符合时间模式的反事实解释，解决了现有方法在时间结构真实性方面的不足。
...
### [70] [SR-TTT: Surprisal-Aware Residual Test-Time Training](https://arxiv.org/abs/2603.06642)
*Swamynathan V P*

Main category: cs.LG

TL;DR: SR-TTT通过引入损失门控稀疏记忆机制，在保持O(1)内存的同时解决TTT模型在精确回忆任务中的灾难性失败问题


<details>
  <summary>Details</summary>
Motivation: 传统的测试时训练(TTT)语言模型虽然能实现理论上无限上下文窗口和O(1)内存占用，但在精确回忆任务（如"大海捞针"）中会出现灾难性失败，因为快速权重会过度压缩上下文，导致重要信息被后续梯度更新覆盖

Method: 提出SR-TTT（惊奇感知残差测试时训练），在TTT主干上增加损失门控稀疏记忆机制，动态路由不可压缩的惊奇令牌到传统的精确注意力残差缓存中，同时保持低熵背景上下文的O(1)内存

Result: SR-TTT解决了TTT在精确回忆任务中的失败问题，同时保持了O(1)内存优势，实现了对关键信息的精确回忆

Conclusion: 通过结合TTT的高效压缩和精确注意力的精确回忆能力，SR-TTT提供了一种平衡内存效率和任务性能的解决方案
...
### [71] [Beyond the Markovian Assumption: Robust Optimization via Fractional Weyl Integrals in Imbalanced Data](https://arxiv.org/abs/2603.08377)
*Gustavo A. Dorrego*

Main category: cs.LG

TL;DR: 提出基于分数微积分的优化算法，通过加权分数Weyl积分替代瞬时梯度，利用历史梯度序列作为自然正则化器，有效解决不平衡数据集中的过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 传统梯度下降及其变体采用局部、马尔可夫权重更新，对噪声和过拟合高度敏感。在金融欺诈检测等极度不平衡数据集中，主导类梯度会系统性覆盖少数类的细微信号，这一问题尤为严重。

Method: 基于分数微积分，提取广义分数导数的核心记忆引擎——加权分数Weyl积分，用动态加权的历史梯度序列替代瞬时梯度。该分数记忆算子作为自然正则化器。

Result: 在医疗诊断中防止过拟合，在金融欺诈检测中PR-AUC比经典优化器提升约40%，建立了纯分数拓扑与应用机器学习之间的稳健桥梁。

Conclusion: 分数微积分优化算法通过历史梯度记忆有效解决不平衡数据集中的过拟合问题，在金融欺诈检测等实际应用中表现出显著优势。
...
### [72] [Trust Aware Federated Learning for Secure Bone Healing Stage Interpretation in e-Health](https://arxiv.org/abs/2603.06646)
*Paul Shepherd,Tasos Dagiuklas,Bugra Alkan,Joaquim Bastos,Jonathan Rodriguez*

Main category: cs.LG

TL;DR: 提出一种基于信任感知的联邦学习框架，用于通过频谱特征解释骨骼愈合阶段，通过自适应信任评分机制过滤不可靠参与者，提高训练稳定性和预测性能。


<details>
  <summary>Details</summary>
Motivation: 在分布式医疗传感环境中，存在不可靠或对抗性参与者的问题，这会影响联邦学习的模型质量和安全性，特别是在医疗应用如骨骼愈合阶段分析中，需要确保数据的可靠性和模型完整性。

Method: 使用多层感知机模型，在Flower联邦学习框架上进行训练。提出自适应信任评分缩放与过滤机制，结合指数移动平均平滑，评估、验证和过滤客户端贡献。研究两种信任评分平滑策略：固定因子和基于信任评分变异性的自适应因子。低信任客户端被排除在聚合之外，待可靠性改善后重新纳入。

Result: 实验结果表明，自适应信任管理相比基线联邦平均策略，能提高训练稳定性和预测性能，减轻受损客户端的负面影响，同时保持稳健的检测能力。

Conclusion: 该工作证明了自适应信任机制在联邦医疗传感中的可行性，未来研究方向可扩展到临床跨机构聚合应用。
...
### [73] [Efficient Credal Prediction through Decalibration](https://arxiv.org/abs/2603.08495)
*Paul Hofman,Timo Löhr,Maximilian Muschalik,Yusuf Sale,Eyke Hüllermeier*

Main category: cs.LG

TL;DR: 提出一种基于相对似然和分类器校准的高效置信集预测方法，通过去校准技术生成概率区间，适用于复杂模型如TabPFN和CLIP


<details>
  <summary>Details</summary>
Motivation: 在安全关键应用中，可靠的不确定性表示至关重要。现有置信集方法计算复杂，需要重新训练模型集成，难以应用于复杂的基础模型和多模态系统

Method: 基于相对似然概念，受概率分类器校准技术启发，提出去校准技术为每个类别标签预测概率区间（上下界）

Result: 方法在覆盖效率评估、分布外检测和上下文学习等任务中表现优异，首次在TabPFN和CLIP等复杂模型上实现置信集预测

Conclusion: 提出的高效置信集预测方法解决了传统方法计算复杂的问题，使置信集预测能够应用于复杂的基础模型和多模态系统
...
### [74] [HURRI-GAN: A Novel Approach for Hurricane Bias-Correction Beyond Gauge Stations using Generative Adversarial Networks](https://arxiv.org/abs/2603.06649)
*Noujoud Nadera,Hadi Majed,Stefanos Giaremis,Rola El Osta,Clint Dawson,Carola Kaiser,Hartmut Kaiser*

Main category: cs.LG

TL;DR: HURRI-GAN使用时间序列生成对抗网络(TimeGAN)增强物理风暴潮模型(ADCIRC)的预测精度，通过生成空间偏差校正来减少计算网格大小和运行时间，同时保持预测准确性。


<details>
  <summary>Details</summary>
Motivation: 美国沿海地区受严重风暴事件影响，需要准确预测飓风风暴潮和风影响。物理模拟模型(如ADCIRC)虽然准确，但在高分辨率下计算时间过长，无法满足应急响应的实时需求。

Method: 提出HURRI-GAN方法，使用时间序列生成对抗网络(TimeGAN)来补偿物理模型的系统性误差。该方法能够从水位测量站位置的空间区域外推模型偏差校正。

Result: 模型能够准确生成超出测量站位置的目标位置偏差校正。低均方根误差(RMSE)表明生成外推数据的能力。将HURRI-GAN生成的校正应用于ADCIRC模拟水位，改善了大多数测试测量站的总体预测。

Conclusion: HURRI-GAN通过AI增强物理模型，减少了必要的网格大小和运行时间，同时保持了预测准确性，为应急响应提供了更及时的飓风影响预测。
...
### [75] [Breaking the Bias Barrier in Concave Multi-Objective Reinforcement Learning](https://arxiv.org/abs/2603.08518)
*Swetha Ganesh,Vaneet Aggarwal*

Main category: cs.LG

TL;DR: 本文解决了多目标强化学习中非线性标量化带来的梯度偏差问题，提出了使用多级蒙特卡洛估计器的自然策略梯度算法，将样本复杂度从O(ε⁻⁴)优化到O(ε⁻²)。


<details>
  <summary>Details</summary>
Motivation: 多目标强化学习需要优化多个目标的非线性效用函数（如公平性、风险敏感性），但非线性标量化导致策略梯度方法存在偏差：梯度依赖于∂f(J^π)，而实践中只能获得经验回报估计Ĵ。由于f是非线性的，插件估计器存在偏差，导致梯度偏差持续存在，降低了样本效率。

Method: 开发了配备多级蒙特卡洛（MLMC）估计器的自然策略梯度（NPG）算法，控制标量化梯度的偏差同时保持低采样成本。当标量化函数二阶光滑时，一阶偏差会自动抵消，允许普通NPG达到相同的最优样本复杂度。

Result: 证明了该方法实现了计算ε最优策略的最优O(ε⁻²)样本复杂度，相比现有方法的O(ε⁻⁴)有显著改进。当标量化函数二阶光滑时，普通NPG也能达到相同的最优复杂度。

Conclusion: 本文首次为策略梯度方法下的凹多目标强化学习提供了最优样本复杂度保证，通过MLMC估计器解决了非线性标量化带来的梯度偏差问题，显著提升了样本效率。
...
### [76] [Geodesic Gradient Descent: A Generic and Learning-rate-free Optimizer on Objective Function-induced Manifolds](https://arxiv.org/abs/2603.06651)
*Liwei Hu,Guangyao Li,Wenyong Wang,Xiaoming Zhang,Yu Xiang*

Main category: cs.LG

TL;DR: 提出了一种无需学习率的黎曼梯度下降算法GGD，通过n维球面局部近似目标函数诱导的超曲面，确保更新轨迹保持在超曲面上，在多个数据集上优于Adam算法。
...
Method: 提出测地线梯度下降(GGD)：每次迭代使用n维球面近似目标函数诱导超曲面的局部邻域，将欧几里得梯度投影到球面上形成测地线，确保更新轨迹保持在超曲面上，参数更新使用测地线端点。最大步长等于n维球面弧长的四分之一，无需学习率。

Result: 在Burgers'数据集上，相比Adam算法，全连接网络的测试MSE降低35.79%到48.76%；在MNIST数据集上，卷积神经网络的交叉熵损失降低3.14%到11.59%。

Conclusion: GGD是一种通用且无需学习率的黎曼梯度下降算法，能够适应任意复杂的几何结构，在多个基准测试中显著优于经典优化算法。
...
### [77] [Impact of Connectivity on Laplacian Representations in Reinforcement Learning](https://arxiv.org/abs/2603.08558)
*Tommaso Giorgi,Pierriccardo Olivieri,Keyue Jiang,Laura Toni,Matteo Papini*

Main category: cs.LG

TL;DR: 该论文研究了在未知转移图或状态空间过大的情况下，通过样本轨迹学习图谱特征进行线性值函数逼近的误差上界，建立了逼近质量与MDP拓扑结构的关系。


<details>
  <summary>Details</summary>
Motivation: 在大规模强化学习中，学习紧凑的状态表示对于解决维度灾难至关重要。现有方法利用MDP的结构先验，通过状态图拉普拉斯特征向量的线性组合构建状态表示。但当转移图未知或状态空间过大时，需要直接从样本轨迹估计图谱特征，需要理论分析这种方法的逼近误差。

Method: 提出在未知转移图或状态空间过大的情况下，通过样本轨迹直接估计图谱特征的方法。推导了线性值函数逼近在学习的谱特征下的逼近误差上界，分析了误差如何随状态图的代数连通性缩放。进一步界定了特征向量估计本身引入的误差，提供了表示学习流程的端到端误差分解。

Result: 证明了线性值函数逼近在学习谱特征下的近似误差上界，展示了误差如何随状态图的代数连通性缩放，将逼近质量建立在MDP的拓扑结构基础上。提供了拉普拉斯算子在RL设置中的表达，纠正了文献中的一些常见误解。结果适用于一般（非均匀）策略，无需对诱导转移核的对称性做任何假设。

Conclusion: 该工作为通过样本轨迹学习谱特征进行值函数逼近提供了理论保证，建立了逼近误差与MDP拓扑结构的关系，为大规模强化学习中的表示学习提供了理论指导，并在网格世界环境中通过数值模拟验证了理论发现。
...
### [78] [ERP-RiskBench: Leakage-Safe Ensemble Learning for Financial Risk](https://arxiv.org/abs/2603.06671)
*Sanjay Mishra*

Main category: cs.LG

TL;DR: 本文提出一个重建的ERP财务风险检测实验框架，使用集成机器学习方法，通过防泄漏验证协议和成本敏感评估，在混合风险定义下实现了最佳检测性能。


<details>
  <summary>Details</summary>
Motivation: ERP系统中的财务风险检测是机器学习的重要但未被充分探索的应用领域。现有研究存在数据集描述模糊、易泄漏的流程以及夸大性能的评估实践等问题，需要建立一个更可靠、可复现的实验框架。

Method: 构建了ERP-RiskBench复合基准数据集，包含公共采购事件日志、标记的欺诈数据和新合成的ERP数据集。采用嵌套交叉验证和时间/群体感知分割防止数据泄漏。主要模型是基于梯度提升方法的堆叠集成，并与线性基线、深度表格架构和可解释的"玻璃盒"替代方案进行比较。

Result: 堆叠集成在多个数据集配置和结构化消融研究中实现了最佳检测结果。防泄漏协议显著降低了先前夸大的准确率估计。SHAP解释和特征稳定性分析显示，采购控制特征（特别是三方匹配差异）是最具信息量的预测因子。

Conclusion: 该框架为ERP审计和治理环境中的机器学习部署提供了一个可复现、操作基础的蓝图，通过严格的防泄漏验证和成本敏感评估，提高了财务风险检测的可靠性和实用性。
...
### [79] [One step further with Monte-Carlo sampler to guide diffusion better](https://arxiv.org/abs/2603.06685)
*Minsi Ren,Wenhao Deng,Ruiqi Feng,Tailin Wu*

Main category: cs.LG

TL;DR: 提出ABMS方法：通过额外反向去噪步骤和蒙特卡洛采样改进SDE生成模型的引导效果，减少估计误差，提升条件生成质量
...
Method: 提出ABMS方法：执行额外反向去噪步骤和蒙特卡洛采样，作为即插即用的调整策略，可配合高阶采样器使用

Result: 在多种任务和数据上验证有效性，包括手写轨迹生成、图像逆问题（修复、超分辨率、去模糊）、分子逆设计等，均能提升生成质量

Conclusion: ABMS方法能有效减少估计误差，改善引导扩散，提升条件生成的一致性和质量，并揭示了现有方法中交叉条件干扰的关键问题
...
### [80] [Scaling Agentic Capabilities, Not Context: Efficient Reinforcement Finetuning for Large Toolspaces](https://arxiv.org/abs/2603.06713)
*Karan Gupta,Pranav Vajreshwari,Yash Pandya,Raghav Magazine,Akshay Nambi,Ahmed Awadallah*

Main category: cs.LG

TL;DR: ATLAS是一个强化微调框架，使小型语言模型能够在大型工具空间中有效操作，通过学习上下文获取和动作执行，在有限参数和上下文预算下接近前沿代理性能。


<details>
  <summary>Details</summary>
Motivation: 在大型工具生态系统中运行的代理系统需要在弱监督或无监督下规划和执行长时程工作流。虽然前沿模型通过规模和大量上下文预算缓解了这些挑战，但小型语言模型仍然脆弱：急切加载工具会饱和上下文，执行错误会随时间累积，稀疏奖励限制了学习。

Method: ATLAS框架将上下文控制和执行结构作为可学习的决策，结合迭代工具加载和程序化工具编排来限制上下文增长并稳定长时程轨迹。提出基于规则的强化微调，将任务成功分解为结构化、任务对齐的标准，并使用小型评判模型实现可扩展训练。

Result: 在MCP基准测试中，这些设计选择相比通用RL基线带来了巨大且一致的性能提升，使4B参数的小型语言模型在更严格的参数和上下文预算下接近前沿代理性能。

Conclusion: ATLAS框架通过将上下文管理和执行结构作为可学习组件，结合基于规则的强化微调，使小型语言模型能够在大型工具空间中有效操作，在资源受限条件下实现接近前沿模型的性能。
...
### [81] [From Statistical Fidelity to Clinical Consistency: Scalable Generation and Auditing of Synthetic Patient Trajectories](https://arxiv.org/abs/2603.06720)
...
<details>
  <summary>Details</summary>
Motivation: 电子健康记录(EHR)的访问受到隐私法规和机构障碍的限制，现有合成EHR方法虽然能捕捉整体统计特性，但在临床过程和观察方面存在不一致性。

Method: 开发了集成管道，包含两个协同步骤：1) 基于知识的基础生成模型，表示近32,000个临床事件并保持结构完整性；2) 利用大语言模型的自动审计模块，过滤临床不一致性。

Result: 生成了18,071条合成患者记录，临床事件概率与真实数据高度一致(R2=0.99)。自动审计显著减少了真实与合成数据之间的差异(Cohen's d从0.59-1.60降至0.18-0.67)，基于审计数据训练的下游模型性能达到或超过真实数据，且无隐私风险(F1=0.51)。

Conclusion: 该集成管道能够生成临床一致的合成EHR，支持安全的数据共享，同时保持数据实用性和隐私保护，为数字健康研究提供了可行解决方案。
...
### [82] [ProtAlign: Contrastive learning paradigm for Sequence and structure alignment](https://arxiv.org/abs/2603.06722)
...
<details>
  <summary>Details</summary>
Motivation: 现有蛋白质语言模型只考虑序列与文本描述的对齐，忽略了结构信息；传统方法将序列和结构分开处理，无法充分利用两者之间的对齐关系。

Method: 引入序列结构对比对齐框架，在大规模序列-结构对上进行训练，最大化匹配对的一致性，同时推远不相关对，学习跨模态共享嵌入空间。

Result: 实现了跨模态检索（如根据序列查找结构邻居），提升了功能注释和稳定性估计等下游预测任务性能，提供了序列变异与结构组织的可解释联系。
...
### [83] [Bi Directional Feedback Fusion for Activity Aware Forecasting of Indoor CO2 and PM2.5](https://arxiv.org/abs/2603.06724)
*Harshala Gammulle,Lidia Morawska,Sridha Sridharan,Clinton Fookes*

Main category: cs.LG

TL;DR: 提出双流双向反馈融合框架，联合建模室内环境演变和人类活动，通过上下文感知调制机制和双时间尺度模块，显著提升CO2和PM2.5预测精度，并提供可解释的不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 室内空气质量预测对保障居住者健康、热舒适和智能建筑控制至关重要。传统数据驱动模型主要依赖历史传感器轨迹，难以预测行为引起的排放峰值或浓度快速变化，因为环境因素与高度动态的居住者行为之间存在复杂相互作用。

Method: 提出双流双向反馈融合框架：1) 联合建模室内环境演变和人类活动嵌入；2) 上下文感知调制机制，基于共享的融合状态自适应缩放和偏移每个流；3) 双时间尺度时间模块，分别捕捉CO2的渐进积累模式和PM2.5的短期波动；4) 结合加权均方误差、峰值感知惩罚和不确定性正则化的复合损失函数。

Result: 在真实世界IAQ数据集上的广泛验证表明，该方法显著优于最先进的预测基线，同时提供对智能建筑和健康感知监测系统实际部署至关重要的可解释不确定性估计。

Conclusion: 该双流双向反馈融合框架有效解决了室内空气质量预测中行为诱导排放峰值和快速浓度变化的挑战，通过联合建模环境演变和人类活动，实现了更准确、更稳健的预测，为智能建筑控制提供了实用工具。
...
### [84] [Regression Models Meet Foundation Models: A Hybrid-AI Approach to Practical Electricity Price Forecasting](https://arxiv.org/abs/2603.06726)
*Yunzhong Qiu,Binzhu Li,Hao Wei,Shenglin Weng,Chen Wang,Zhongyi Pei,Mingsheng Long,Jianmin Wang*

Main category: cs.LG

TL;DR: FutureBoosting：一种新颖的范式，通过将冻结时间序列基础模型生成的预测特征集成到回归模型中，来增强电力市场价格预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 电力市场价格具有极端波动性、非线性和非平稳性，准确预测极具挑战。现有时间序列基础模型（TSFMs）能有效捕捉时间依赖但未充分利用跨变量相关性和非周期性模式，而回归模型擅长特征交互但仅限于未来可用输入，忽略了历史驱动因素。

Method: 提出FutureBoosting范式：使用冻结的TSFM生成预测特征，将这些特征作为增强输入注入下游回归模型。具体实现为一个轻量级、即插即用的电力价格预测框架。

Result: 在真实电力市场数据上的广泛评估表明，该框架持续优于最先进的TSFMs和回归基线，在大多数情况下将平均绝对误差（MAE）降低了30%以上。通过消融研究和可解释AI技术验证了预测特征的贡献。

Conclusion: FutureBoosting为实际市场参与提供了一个稳健、可解释且有效的解决方案，建立了一个将时间上下文集成到回归模型中的通用框架。
...
### [85] [Safe Transformer: An Explicit Safety Bit For Interpretable And Controllable Alignment](https://arxiv.org/abs/2603.06727)
*Jingyuan Feng,Andrew Gambardella,Gouki Minegishi,Takeshi Kojima,Yusuke Iwasawa,Yutaka Matsuo*

Main category: cs.LG

TL;DR: Safe Transformer通过插入包含显式安全比特的信息瓶颈，实现语言模型安全决策的可解释性和可控性，仅需轻量微调即可达到接近零的攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 当前安全对齐方法将安全行为隐式编码在模型参数中，导致根本性不透明：无法轻易检查模型为何拒绝请求，也无法在其安全判断失败时进行干预。

Method: 在预训练语言模型的Transformer层之间插入离散信息瓶颈，包含显式安全比特和额外无监督比特。通过对比训练，模型学习解耦表示：安全比特控制行为模式（s=1时生成有用响应，s=0时拒绝），无监督比特编码生成所需的语义内容。

Result: 在红队基准测试中，Safe Transformer实现了接近零的攻击成功率，显著优于基础模型和安全微调基线。

Conclusion: Safe Transformer通过模块化设计实现了安全决策的可解释性和可控性，仅需轻量微调而无需从头预训练，为语言模型安全对齐提供了新思路。
...
### [86] [Orion: Characterizing and Programming Apple's Neural Engine for LLM Training and Inference](https://arxiv.org/abs/2603.06728)
*Ramchand Kumaresan*

Main category: cs.LG

TL;DR: Orion是首个绕过CoreML直接访问Apple Neural Engine的开源端到端系统，支持直接ANE执行、编译器流水线和稳定的多步训练，实现GPT-2 124M推理170+ tokens/s，训练速度提升3.8倍。


<details>
  <summary>Details</summary>
Motivation: 苹果设备中的神经处理单元(ANE)在大型语言模型工作中很少使用，因为CoreML框架存在不透明的抽象层，不支持直接ANE编程和本地训练。

Method: 通过苹果私有_ANEClient和_ANECompiler API绕过CoreML，构建包含编译器流水线（5个优化pass将图IR降至ANE原生MIL）和运行时的系统，支持IOSurface零拷贝张量I/O、程序缓存和权重更新的增量编译。

Result: 将训练步骤的重编译时间从4.2秒降至494毫秒（8.5倍提升），训练速度提升3.8倍；在M4 Max上实现GPT-2 124M推理170+ tokens/s；稳定训练110M参数transformer 1000步仅需22分钟且无NaN错误。

Conclusion: Orion成功解锁了ANE在LLM工作负载中的潜力，通过直接硬件访问和创新的权重更新机制，显著提升了推理和训练性能，为苹果设备上的本地AI应用开辟了新途径。
...
### [87] [Don't Freeze, Don't Crash: Extending the Safe Operating Range of Neural Navigation in Dense Crowds](https://arxiv.org/abs/2603.06729)
*Jiefu Zhang,Yang Xu,Vaneet Aggarwal*

Main category: cs.LG

TL;DR: 提出一种强化学习方法用于密集人群导航，通过密度不变编码和密度随机化训练实现零样本密度泛化，在超出训练密度1.3倍的情况下仍保持高性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的人群导航方法在超出训练分布的密集人群下容易失效，而分析方法虽然安全但容易在紧密交互中冻结。需要一种能在不同密度人群中安全导航且具有良好泛化能力的方法。

Method: 使用密度不变观测编码（距离排序的K个最近行人+有界人群摘要），结合密度随机化训练和基于物理的接近奖励塑形与密度自适应缩放。编码保持输入统计稳定，训练时在11-16行人密度下进行。

Result: 在N=21行人（比训练密度高1.3倍）的评估中，策略达到>99%的目标到达率和86%的无碰撞成功率，冻结现象显著少于分析方法，比学习基准方法高出>60个百分点的无碰撞成功率。

Conclusion: 提出的密度不变编码和密度自适应训练方法能够实现零样本密度泛化，在密集可变密度人群中实现安全导航，超越了现有学习和分析方法的表现。
...
### [88] [Rank-Factorized Implicit Neural Bias: Scaling Super-Resolution Transformer with FlashAttention](https://arxiv.org/abs/2603.06738)
*Dongheon Lee,Seokju Yun,Jaegyun Im,Youngmin Ro*

Main category: cs.LG

TL;DR: 本文提出Rank-factorized Implicit Neural Bias (RIB)方法，替代传统相对位置偏置，使超分辨率Transformer能够使用FlashAttention，大幅提升训练和推理效率，同时扩大感受野。


<details>
  <summary>Details</summary>
Motivation: 当前超分辨率Transformer过度依赖相对位置偏置(RPB)，无法利用硬件高效的FlashAttention内核，导致计算负担重，限制了通过扩大训练块大小或自注意力窗口来扩展模型的能力。

Method: 提出RIB方法：使用低秩隐式神经表示近似位置偏置，将其与像素内容token在通道维度拼接，将注意力分数计算中的元素级偏置加法转换为点积操作。同时引入卷积局部注意力和循环窗口策略。

Result: 在Urban100×2上达到35.63dB PSNR，相比基于RPB的SR Transformer(PFT)，训练时间减少2.1倍，推理时间减少2.9倍，窗口大小可扩大到96×96。

Conclusion: RIB方法成功解决了SR Transformer无法使用FlashAttention的问题，实现了高效的长距离建模，显著提升了超分辨率任务的性能和效率。
...
### [89] [Heterogeneous Decentralized Diffusion Models](https://arxiv.org/abs/2603.06741)
*Zhiying Jiang,Raihan Seraj,Marcos Villagra,Bidhan Roy*

Main category: cs.LG

TL;DR: 提出高效去中心化扩散模型框架，将计算需求从1176GPU天降至72GPU天，支持异构训练目标（DDPM和Flow Matching），无需重新训练即可统一推理


<details>
  <summary>Details</summary>
Motivation: 前沿扩散模型训练需要大量计算资源，限制了参与机构范围。现有去中心化方法仍需要大量资源（1176GPU天）且要求所有专家使用相同训练目标
...
Result: 计算需求从1176降至72GPU天（16倍减少），数据需求从158M降至11M（14倍减少）。异构配置（2DDPM:6FM）在FID（11.88 vs 12.45）和LPIPS多样性（0.631 vs 0.617）上优于同质8FM基线

Conclusion: 通过消除同步要求和支持混合DDPM/FM目标，该框架降低了去中心化生成模型训练的基础设施需求，使更多机构能够参与前沿模型开发
...
### [90] [Improved Constrained Generation by Bridging Pretrained Generative Models](https://arxiv.org/abs/2603.06742)
*Xiaoxuan Liang,Saeid Naderiparizi,Yunpeng Liu,Berend Zwartsenberg,Frank Wood*

Main category: cs.LG

TL;DR: 提出一个约束生成框架，直接在复杂可行区域（如道路地图）内生成样本，同时保持真实性


<details>
  <summary>Details</summary>
Motivation: 现实世界中的约束（如机器人控制和自动驾驶）很少是简单的线性不等式，而是类似道路地图的复杂可行区域，需要生成满足约束的样本

Method: 微调预训练生成模型，在保持生成保真度的同时强制执行约束

Result: 方法展现出与现有微调和无训练约束基线不同的特性，揭示了约束满足和采样质量之间的新权衡

Conclusion: 提出了一种有效的约束生成框架，能够在复杂可行区域内直接生成样本，为现实世界应用提供了新的解决方案
...
### [91] [Stabilizing Reinforcement Learning for Diffusion Language Models](https://arxiv.org/abs/2603.06743)
*Jianyuan Zhong,Kaibo Wang,Ding Ding,Zijin Feng,Haoli Bai,Yang Xiang,Jiacheng Sun,Qiang Xu*

Main category: cs.LG

TL;DR: GRPO在扩散大语言模型(dLLMs)中会导致奖励崩溃，作者提出了StableDRL来解决这一问题，通过无条件裁剪和自归一化来稳定训练。


<details>
  <summary>Details</summary>
Motivation: GRPO在自回归语言模型中效果很好，但直接应用于扩散大语言模型(dLLMs)会导致奖励崩溃。作者发现两个不兼容的来源：1）GRPO依赖序列概率定义的重要性比率，这在dLLMs中难以计算且估计噪声大；2）标准GRPO的公式不适合估计的比率，其条件裁剪会被模型无关的估计噪声异常绕过，产生梯度尖峰。

Method: 提出StableDRL，专门为dLLMs设计的GRPO改进版本：1）使用无条件裁剪来抑制异常值引起的尖峰；2）使用自归一化将更新约束在每样本梯度的凸包内。还通过阶梯注意力机制将StableDRL扩展到块级扩散模型。

Result: StableDRL打破了GRPO在dLLMs中形成的不稳定循环，防止了策略漂移和比率方差增加，实现了稳定的训练。

Conclusion: GRPO直接应用于dLLMs存在根本性不兼容问题，需要专门设计的StableDRL来解决估计噪声和梯度稳定性问题，为扩散大语言模型的强化学习训练提供了有效解决方案。
...
### [92] [Enhancing Instruction Following of LLMs via Activation Steering with Dynamic Rejection](https://arxiv.org/abs/2603.06745)
*Minjae Kang,Jaehyung Kim*

Main category: cs.LG

TL;DR: DIRECTER是一种动态拒绝引导方法，通过KV缓存缩放和合理性引导的解码循环，自适应调整激活引导强度，避免过度引导导致的指令跟随能力下降。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在指令调优方面有进展，但仍难以遵循复杂用户指令。现有的激活引导技术存在过度引导风险，过度强调指令会降低任务准确性和文本质量。

Method: 提出DIRECTER方法：1) 动态调制引导强度，通过缩放KV缓存实现；2) 结合合理性引导的解码循环，比较引导后输出分布与原始分布，自适应调整引导强度；3) 基于轻量级一次性注意力敏感性分析，对层的影响力进行排序。

Result: 在多个基准测试中，DIRECTER显著提升了指令跟随能力，相比基线方法准确率提升高达6.5%，且没有在生成质量或任务保真度方面产生常见的权衡问题。

Conclusion: DIRECTER的动态、合理性引导控制机制为缓解激活引导中的过度引导问题提供了通用解决方案，与现有基线兼容，展示了其作为通用机制的潜力。
...
### [93] [Property-driven Protein Inverse Folding With Multi-Objective Preference Alignment](https://arxiv.org/abs/2603.06748)
*Xiaoyang Hou,Junqi Liu,Chence Shi,Xin Liu,Zhi Yang,Jian Tang*

Main category: cs.LG

TL;DR: ProtAlign是一个多目标偏好对齐框架，通过半在线直接偏好优化微调预训练的反向折叠模型，在保持结构保真度的同时满足多种可开发性目标。


<details>
  <summary>Details</summary>
Motivation: 蛋白质序列设计需要在可设计性（恢复目标骨架的能力）与多种竞争性的可开发性属性（如溶解度、热稳定性和表达性）之间取得平衡。现有方法存在目标依赖性，需要大量领域专业知识或仔细的超参数调整。

Method: ProtAlign采用半在线直接偏好优化策略，使用灵活偏好边界缓解竞争目标间的冲突，并通过硅基属性预测器构建偏好对，微调预训练的反向折叠模型。

Result: 应用于广泛使用的ProteinMPNN骨架，得到的MoMPNN模型在CATH 4.3晶体结构、从头生成的骨架和实际结合剂设计场景中，都能在不损害可设计性的情况下增强可开发性。

Conclusion: ProtAlign为实际蛋白质序列设计提供了一个有吸引力的框架，能够平衡结构保真度与多种可开发性目标。
...
### [94] [Implementation of Quantum Implicit Neural Representation in Deterministic and Probabilistic Autoencoders for Image Reconstruction/Generation Tasks](https://arxiv.org/abs/2603.06755)
*Saadet Müzehher Eren*

Main category: cs.LG

TL;DR: 提出基于量子隐式神经表示（QINR）的自动编码器和变分自动编码器，用于图像重建和生成任务，通过量子-经典混合架构提升性能


<details>
  <summary>Details</summary>
Motivation: 展示QINR在VAE/AE中能将潜在空间信息转换为丰富、周期性、高频特征，解决QGAN模型多样性不足的问题，提供更稳定的图像生成方案

Method: 采用量子-经典混合架构：经典CNN编码器 + 量子QINR解码器；使用BCEWithLogits作为重建损失，QINR-VAE额外使用KL散度进行潜在正则化；引入可学习角度缩放解决优化问题

Result: 在MNIST、E-MNIST、Fashion-MNIST数据集上测试，QINR-VAE能用少量数据生成更多样化的图像，生成和重建的图像具有清晰边界和细节，性能优于现有生成模型

Conclusion: 在AE/VAE框架中加入基于QINR的量子层能显著提升图像重建和生成性能，同时保持参数数量受限，为量子机器学习在图像处理中的应用提供了有效方案
...
### [95] [Learning Unbiased Cluster Descriptors for Interpretable Imbalanced Concept Drift Detection](https://arxiv.org/abs/2603.06757)
*Yiqun Zhang,Zhanpei Huang,Mingjie Zhao,Chuyao Zhang,Yang Lu,Yuzhu Ji,Fangqing Gu,An Zeng*

Main category: cs.LG

TL;DR: ICD3：针对不平衡数据流的概念漂移检测方法，通过多分布粒度搜索识别不平衡概念，为每个概念训练单簇分类器独立检测漂移，解决大簇对小簇漂移的"掩蔽效应"。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的数据流通常存在概念不平衡问题，大簇的统计特性容易掩盖小簇分布的漂移（掩蔽效应）。现有方法大多假设概念平衡，无法有效检测不平衡数据中的概念漂移，特别是无法定位小概念及其漂移。

Method: 提出ICD3方法：1）使用新设计的多分布粒度搜索检测不平衡概念，有效捕捉大小概念的分布；2）为每个识别出的概念训练单簇分类器，在后续数据块中独立监控其潜在漂移，避免大簇的主导影响。

Result: ICD3具有高度可解释性，能准确定位漂移的概念，对概念不平衡比的变化具有鲁棒性。在多个基准数据集上的综合实验和消融研究表明，ICD3优于现有最先进方法。

Conclusion: ICD3有效解决了不平衡数据流中的概念漂移检测问题，通过独立监控每个概念避免了掩蔽效应，为理解动态系统的演化提供了可靠工具。
...
### [96] [Enhancing SHAP Explainability for Diagnostic and Prognostic ML Models in Alzheimer Disease](https://arxiv.org/abs/2603.06758)
*Pablo Guillén,Enrique Frias-Martinez*

Main category: cs.LG

TL;DR: 提出多層次可解釋性框架，驗證SHAP在阿茲海默症診斷與預後模型中的穩定性與一致性，發現認知與功能標記主導解釋，且跨任務解釋高度一致。


<details>
  <summary>Details</summary>
Motivation: 現有阿茲海默症機器學習模型雖表現良好，但臨床應用受限於技術門檻高且缺乏可信賴、一致的解釋方法。SHAP雖常用於解釋，但現有研究多關注單一任務，缺乏對解釋方法在不同疾病階段、模型架構或預測目標間穩健性的評估。

Method: 提出多層次可解釋性框架，整合：(1)特徵重要性與SHAP的模型內一致性指標；(2)跨AD邊界的SHAP穩定性；(3)診斷與預後間的SHAP跨任務一致性。使用AutoML在NACC數據集上優化分類器，訓練4個診斷模型和4個預後模型，涵蓋標準AD進展階段。透過相關性指標、top-k特徵重疊、SHAP符號一致性和領域層級貢獻比評估穩定性。

Result: 認知與功能標記在診斷和預後的SHAP解釋中均佔主導地位。診斷與預後模型間的SHAP-SHAP一致性在所有分類器中都很高，符號穩定性達100%，解釋幅度變化極小。領域層級貢獻也保持穩定，僅在預後中遺傳特徵有輕微增加。

Conclusion: SHAP解釋可透過量化方法驗證其穩健性和可轉移性，為臨床醫生提供更可靠的機器學習預測解釋，促進臨床應用。
...
### [97] [Diversity-Aware Adaptive Collocation for Physics-Informed Neural Networks via Sparse QUBO Optimization and Hybrid Coresets](https://arxiv.org/abs/2603.06761)
*Hadi Salloum,Maximilian Mifsud Bonici,Sinan Ibrahim,Pavel Osinenko,Alexei Kornaev*

Main category: cs.LG

TL;DR: 提出一种基于核心集选择的PINNs配点优化方法，通过稀疏图BQM和混合覆盖锚点，在固定配点预算下提高精度并降低选择开销。
...
Method: 将配点选择重新解释为核心集构建问题，设计QUBO/BQM目标函数（线性项编码残差重要性，二次项减少冗余），提出稀疏图BQM和修复过程，并引入混合覆盖锚点保证全局PDE约束。

Result: 在1D时间依赖粘性Burgers方程（含激波形成）上测试，稀疏和混合公式相比密集QUBO减少选择开销，在固定配点预算下达到相当或更好的精度。

Conclusion: 基于核心集的配点选择方法能有效平衡信息性和多样性，稀疏和混合公式在保持精度的同时显著降低计算开销，为PINNs训练提供了更高效的配点策略。
...
### [98] [Metalearning traffic assignment for network disruptions with graph convolutional neural networks](https://arxiv.org/abs/2603.06763)
*Serio Agriesti,Guido Cantelmo,Francisco Camara Pereira*

Main category: cs.LG

TL;DR: 结合图卷积神经网络与元学习架构，使交通流量预测模型能快速适应路网结构和需求模式的变化


<details>
  <summary>Details</summary>
Motivation: 传统机器学习模型在交通预测中面临历史数据与未来条件统计差异的问题，特别是在路网结构变化（如维护、灾害等）时性能下降，而这些恰恰是最需要可靠预测的场景

Method: 将图卷积神经网络与元学习架构结合，训练模型快速适应新的图结构和需求模式，使其能同时处理路网变化和需求变化

Result: 元学习方法使图神经网络能够快速适应未见过的路网关闭和OD矩阵，在未见场景上达到约0.85的R²分数，减轻了设计覆盖所有相关模式的训练数据集的负担

Conclusion: 提出的元学习架构有效解决了交通预测中模型适应路网结构变化的问题，为实际应用提供了更可靠的预测工具
...
### [99] [Failure Detection in Chemical Processes using Symbolic Machine Learning: A Case Study on Ethylene Oxidation](https://arxiv.org/abs/2603.06767)
*Julien Amblard,Niklas Groll,Matthew Tait,Mark Law,Gürkan Sin,Alessandra Russo*

Main category: cs.LG

TL;DR: 本文研究使用符号机器学习预测化工过程故障，在乙烯氧化过程中进行可行性研究，结果表明符号机器学习在保持可解释性的同时性能优于随机森林和多层感知机等基线方法。


<details>
  <summary>Details</summary>
Motivation: 在化工过程行业，安全性至关重要，但传统大规模神经网络方法存在脆弱性、缺乏可解释性等问题，且该领域缺乏包含历史故障的开源真实数据集。

Method: 采用最先进的符号机器学习系统，能够从上下文相关的噪声示例中学习概率规则形式的预测模型。该方法不依赖于特定化工过程，为弥补真实故障数据不足，使用化工过程模拟器生成数据进行可行性研究。

Result: 实验结果表明，符号机器学习在乙烯氧化过程故障预测中性能优于随机森林和多层感知机等基线方法，同时通过生成紧凑的基于规则的预测模型保持可解释性。

Conclusion: 符号机器学习在化工过程故障预测中具有可行性，其生成的规则模型可集成到智能体中，辅助化工操作人员在潜在故障期间进行决策，平衡了预测性能和可解释性需求。
...
### [100] [Gauge Freedom and Metric Dependence in Neural Representation Spaces](https://arxiv.org/abs/2603.06774)
*Jericho Cain*

Main category: cs.LG

TL;DR: 神经网络表示本质上是向量空间，但具有可逆线性变换的规范自由度，因此余弦相似度等度量是坐标依赖的，而非不变性量。


<details>
  <summary>Details</summary>
Motivation: 神经网络表示通常被视为固定欧几里得空间中的向量，但其坐标并非唯一确定。当表示经过可逆线性变换时，网络功能可以保持不变，因此表示实际上只定义到可逆线性变换的程度。这解释了文献中观察到的余弦相似度不稳定、嵌入空间各向异性等现象。

Method: 从几何视角研究神经表示空间，将其视为具有一般线性群规范自由度的向量空间。通过实验验证，在多层感知机和卷积网络中插入可逆变换会显著扭曲余弦相似度和最近邻结构，同时保持预测不变。

Result: 实验证实，可逆变换会显著改变余弦相似度值和最近邻结构，而模型预测保持不变。这解释了为什么SVCCA和CKA等表示比较方法更有效，因为它们考虑了这种规范自由度。

Conclusion: 神经表示分析应关注规范自由度下的不变性量，或明确选择规范坐标。余弦相似度等常用度量是坐标依赖的，不能可靠地反映表示的本质特性。
...
### [101] [HGT-Scheduler: Deep Reinforcement Learning for the Job Shop Scheduling Problem via Heterogeneous Graph Transformers](https://arxiv.org/abs/2603.06777)
*Bulent Soykan*

Main category: cs.LG

TL;DR: 提出HGT-Scheduler框架，将作业车间调度问题建模为异构图，通过边类型依赖的注意力机制捕捉不同类型的关系模式，在FT06实例上取得8.4%的最优性差距。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法将作业车间调度问题的析取图建模为同构图，将作业优先约束和机器竞争边合并为单一关系类型，忽略了问题结构的固有异质性，可能导致关键关系信息丢失。

Method: 提出HGT-Scheduler强化学习框架，将JSSP建模为异构图，使用异构图变换器通过边类型依赖的注意力机制捕捉优先和竞争关系的特定类型关系模式，使用近端策略优化训练调度策略。

Result: 在FT06实例上达到8.4%的最优性差距，统计显著优于忽略边类型的相同架构(p=0.011)和图同构网络基线。在FT10实例上展示良好可扩展性，但训练步数限制下异构图与同构图性能相当，表明边类型感知需要更长的训练时间。

Conclusion: 显式建模不同的边语义可以改善有效调度策略的学习，三层注意力架构性能最佳，异构图建模在作业车间调度问题中具有优势。
...
### [102] [SpatialMAGIC: A Hybrid Framework Integrating Graph Diffusion and Spatial Attention for Spatial Transcriptomics Imputation](https://arxiv.org/abs/2603.06780)
*Sayeem Bin Zaman,Fahim Hafiz,Riasat Azim*

Main category: cs.LG

TL;DR: SpatialMagic提出了一种结合图扩散和空间自注意力的混合插补模型，用于解决空间转录组学数据的高稀疏性和技术噪声问题，在多个平台上优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 空间转录组学(ST)能够映射具有空间背景的基因表达，但受到高稀疏性和技术噪声的严重影响，这些噪声掩盖了真实的生物信号并阻碍了下游分析。

Method: SpatialMagic是一种混合插补模型，结合了基于MAGIC的图扩散和基于transformer的空间自注意力。图扩散捕获基因表达中的长程依赖关系，空间注意力模型捕获局部邻域结构，从而恢复缺失的表达值并保持空间一致性。

Result: 在多个平台上，SpatialMagic始终优于现有基线方法（包括MAGIC和基于注意力的模型），在聚类准确性方面达到峰值调整兰德指数(ARI)得分：高分辨率Stereo-Seq数据为0.3301，Slide-Seq为0.3074，Sci-Space数据集为0.4216。该模型还显著增强了上下游生物分析，改善了上下调基因的检测，同时保持了跨数据集的调控一致性。

Conclusion: SpatialMagic的混合扩散注意力策略和精炼模块在定量指标上优于最先进的基线方法，并通过保留组织结构和揭示生物学相关基因，提供了对插补数据的更好理解。该框架在提高数据质量的同时保持了生物可解释性。
...
### [103] [xaitimesynth: A Python Package for Evaluating Attribution Methods for Time Series with Synthetic Ground Truth](https://arxiv.org/abs/2603.06781)
*Gregor Baer*

Main category: cs.LG

TL;DR: xaitimesynth是一个Python包，为时间序列可解释性方法评估提供可复用的合成数据生成和评估基础设施。


<details>
  <summary>Details</summary>
Motivation: 评估时间序列归因方法很困难，因为真实世界数据集很少提供哪些时间点驱动预测的真实标签。目前每个研究都需要从头开始重新实现合成数据生成方法。

Method: 该包基于加性模型生成合成时间序列：每个样本是背景信号和局部化、类别区分特征的叠加，特征窗口自动跟踪为真实标签掩码。提供流畅的数据生成API和YAML配置格式，支持单变量和多变量时间序列。

Result: 开发了xaitimesynth开源Python包，提供标准定位指标（AUC-PR、AUC-ROC、相关性质量准确度、相关性排名准确度），可在https://github.com/gregorbaer/xaitimesynth获取。

Conclusion: xaitimesynth为时间序列可解释性方法评估提供了灵活、可复现的合成数据生成基础设施，解决了该领域缺乏标准化评估框架的问题。
...
### [104] [Physics-Informed Diffusion Model for Generating Synthetic Extreme Rare Weather Events Data](https://arxiv.org/abs/2603.06782)
*Marawan Yakout,Tannistha Maiti,Monira Majhabeen,Tarry Singh*

Main category: cs.LG

TL;DR: 提出基于物理信息的扩散模型生成合成多光谱卫星图像，解决热带气旋快速增强检测中的数据稀缺问题，特别是针对罕见的4级等效事件。
...
Result: 模型成功学习十个不同上下文类别的判别特征，有效缓解数据瓶颈。生成样本保持现实空间自相关性和物理一致性，平均对数谱距离为4.5dB。

Conclusion: 该生成框架为增强操作天气检测算法的训练数据集提供了可扩展解决方案，能够有效解决极端类别不平衡问题。
...
### [105] [Optimistic Policy Regularization](https://arxiv.org/abs/2603.06793)
*Mai Pham,Vikrant Vaze,Peter Chin*

Main category: cs.LG

TL;DR: OPR通过保留和强化历史成功轨迹来防止早期熵崩溃，显著提升PPO在Atari游戏和网络安全环境中的样本效率和最终性能


<details>
  <summary>Details</summary>
Motivation: 深度强化学习代理经常遭受过早收敛问题，早期熵崩溃导致策略在发现全局最优策略之前就丢弃了探索行为。为了解决这个问题，需要一种机制来保持探索性行为。

Method: 引入乐观策略正则化（OPR），这是一种轻量级机制，通过维护高性能轨迹的动态缓冲区，并通过方向性对数比奖励塑造和辅助行为克隆目标，将学习偏向这些行为。

Result: 在PPO上实例化OPR后，在Arcade Learning Environment中显著提高了样本效率。在49个Atari游戏的1000万步基准测试中，OPR在22个环境中获得最高分，而基线方法是在标准的5000万步基准下报告的。在CAGE Challenge 2网络安全环境中，OPR超越了竞赛获胜的Cardiff代理，同时使用相同的PPO架构。

Conclusion: 将策略更新锚定在经验上成功的轨迹上，可以同时提高样本效率和最终性能，为强化学习中的过早收敛问题提供了一种有效的解决方案。
...
### [106] [Multi-Agent Reinforcement Learning with Submodular Reward](https://arxiv.org/abs/2603.06810)
*Wenjing Chen,Chengyuan Qian,Shuo Xing,Yi Zhou,Victoria Crawford*

Main category: cs.LG

TL;DR: 该论文研究了具有次模奖励的多智能体强化学习，提出了首个形式化框架和算法，在已知动态时获得1/2近似解，在未知动态时实现次线性遗憾界。


<details>
  <summary>Details</summary>
Motivation: 现实中的多智能体协作任务（如多无人机监视、协作探索）存在智能体贡献重叠的现象，表现出边际收益递减的特性，这可以用次模奖励来建模。然而，标准的MARL通常假设可加性奖励，无法有效处理这种场景。

Method: 1. 提出首个具有次模奖励的MARL形式化框架；2. 对于已知动态的情况，设计贪心策略优化算法，获得1/2近似保证；3. 对于未知动态的情况，提出基于UCB的学习算法，实现次线性遗憾界。

Result: 1. 贪心算法在多项式复杂度下获得1/2近似解，避免了联合策略优化的维度灾难；2. UCB算法实现了O(H²KS√AT)的1/2遗憾界，其中H是时间步长，K是智能体数，S是状态数，A是动作数，T是回合数。

Conclusion: 该研究为具有次模奖励的多智能体强化学习提供了首个理论框架和算法，有效解决了智能体贡献重叠的现实场景，在理论和实际应用上都有重要意义。
...
### [107] [Joint 3D Gravity and Magnetic Inversion via Rectified Flow and Ginzburg-Landau Guidance](https://arxiv.org/abs/2603.06829)
*Dhruman Gupta,Yashas Shende,Aritra Das,Chanda Grover Kamra,Debayan Gupta*

Main category: cs.LG

TL;DR: 本文提出了一种基于整流流的新型框架，用于3D重磁联合反演，通过引入Ginzburg-Landau正则器和指导方法，在Noddyverse数据集上实现物理感知训练，并发布了3D密度VAE模型。


<details>
  <summary>Details</summary>
Motivation: 随着浅层矿产资源的逐渐枯竭，地下矿产探测变得至关重要。传统地质勘探方法存在局限性，而重磁联合反演是一种有前景的新方法。然而，现有的确定性算法和大多数机器学习方法只能预测单一解，无法捕捉可能的解分布，且反演问题本身是病态的。

Method: 1. 将3D重磁联合反演重新构建为整流流问题，在Noddyverse数据集（最大的基于物理的反演数据集）上进行训练；2. 引入Ginzburg-Landau正则器，这是Ising模型的广义版本，有助于矿石识别，实现物理感知训练；3. 提出基于GL理论的指导方法，可作为即插即用模块与现有无条件去噪器配合使用；4. 训练并发布3D密度的变分自编码器（VAE），便于下游工作。

Result: 论文提出了一个完整的重磁联合反演框架，包括整流流模型、GL正则器、指导方法以及VAE模型。该方法能够处理反演问题的非唯一性，捕捉可能的解分布，而不仅仅是单一正则化解。

Conclusion: 该研究为地下矿产探测提供了一种创新的重磁联合反演方法，通过整流流框架和物理感知训练，克服了传统方法的局限性，能够更好地建模解分布，并为该领域提供了可用的工具和模型。
...
### [108] [Contextual Counterfactual Credit Assignment for Multi-Agent Reinforcement Learning in LLM Collaboration](https://arxiv.org/abs/2603.06859)
*Yanjun Chen,Yirong Sun,Hanlin Wang,Xinming Zhang,Xiaoyu Shen,Wenjie Li,Wei Zhang*

Main category: cs.LG

TL;DR: C3是一种针对多智能体强化学习系统的信用分配方法，通过冻结上下文、评估替代消息并应用留一法基线，解决稀疏终端反馈导致的信用扩散问题。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的多智能体强化学习系统通常只依赖稀疏的终端反馈，这种共享信号会混淆上游决策，阻碍准确的决策级信用分配，导致轨迹级信用扩散问题。

Method: C3通过三个步骤实现：1) 冻结从转录本派生的确切上下文；2) 通过固定延续重放评估上下文匹配的替代消息；3) 应用留一法基线来计算无偏、低方差的边际优势，用于标准策略梯度优化。

Result: 在五个数学和编程基准测试中，在匹配预算下，C3相比现有基线显著提升了终端性能。机制诊断显示这些改进伴随着更高的信用保真度、更低的上下文方差和更强的智能体间因果依赖性。

Conclusion: C3方法有效解决了多智能体强化学习中的信用分配问题，通过局部干预提取无偏的边际优势，提高了学习效率和性能。
...
### [109] [IGLU: The Integrated Gaussian Linear Unit Activation Function](https://arxiv.org/abs/2603.06861)
*Mingi Kang,Zai Yang,Jeova Farias Sales Rocha Neto*

Main category: cs.LG

TL;DR: IGLU是一种新的参数化激活函数，通过半正态混合分布从GELU门控推导而来，使用柯西CDF作为门控组件，通过单一锐度参数在恒等映射和ReLU行为之间连续插值，并提供计算高效的有理近似版本。


<details>
  <summary>Details</summary>
Motivation: 尽管GELU等平滑激活函数在Transformer模型中表现出色，但其数学关系和有效性原理尚未完全理解。现有激活函数如ReLU和GELU在梯度消失、计算效率等方面存在局限性，需要一种具有理论依据且性能优越的替代方案。

Method: 从GELU门控的半正态混合分布推导出IGLU激活函数，其门控组件恰好是柯西CDF。这形成了一个单参数族，通过锐度参数σ在恒等映射和ReLU行为之间连续插值。还提出了IGLU-Approx，一种完全基于ReLU操作的有理近似，消除了超越函数计算。

Result: 在CIFAR-10、CIFAR-100和WikiText-103数据集上，使用ResNet-20、ViT-Tiny和GPT-2 Small模型，IGLU在视觉和语言任务上均达到或优于ReLU和GELU基线性能。IGLU-Approx以显著降低的计算成本恢复了这一性能，特别是在不平衡分类数据集上表现出色。

Conclusion: IGLU提供了一种理论上有依据的激活函数，通过柯西分布的重尾特性解决了梯度消失问题，其近似版本保持了性能同时提高了计算效率，为深度神经网络激活函数设计提供了新方向。
...
### [110] [Stochastic Attention via Langevin Dynamics on the Modern Hopfield Energy](https://arxiv.org/abs/2603.06875)
*Abdulrahman Alswaidan,Jeffrey D. Varner*

Main category: cs.LG

TL;DR: 注意力机制本质上是经典能量函数的梯度下降，通过朗之万采样可得到无需训练的随机注意力，通过温度参数控制检索与生成模式


<details>
  <summary>Details</summary>
Motivation: 传统注意力机制是确定性检索，缺乏生成能力。作者希望揭示注意力与经典能量函数的联系，开发无需训练、可控制检索与生成的统一框架

Method: 将注意力计算重新解释为经典能量函数的一步梯度下降，引入朗之万采样得到随机注意力，通过单一温度参数控制模式（低温精确检索，高温开放生成）

Result: 在4个领域（64-4096维）验证：生成模式下，随机注意力比最佳学习基线（变分自编码器）新颖性高2.6倍、多样性高2.0倍，且与Metropolis校正金标准匹配。提出简单信噪比规则选择温度

Conclusion: 注意力机制本质上是能量最小化过程，随机注意力提供无需训练的通用框架，可自然扩展到检索增强生成和上下文学习，无需架构修改
...
### [111] [Physics-informed AI Accelerated Retention Analysis of Ferroelectric Vertical NAND: From Day-Scale TCAD to Second-Scale Surrogate Model](https://arxiv.org/abs/2603.06881)
...
TL;DR: 该研究提出了一种基于物理信息神经算子（PINO）的AI代理模型，用于高效预测铁电场效应晶体管垂直NAND（Fe-VNAND）的阈值电压漂移和保持特性，相比传统TCAD工具实现了超过10000倍的加速。


<details>
  <summary>Details</summary>
Motivation: 3D Fe-VNAND虽然能克服z方向缩放限制并降低编程电压，但其数据保持特性受到电荷去陷和铁电去极化复杂相互作用的阻碍。传统TCAD工具计算成本高，难以进行大规模参数空间探索来优化器件设计。

Method: 开发了基于物理信息神经算子（PINO）的AI代理模型，将基本物理原理嵌入学习架构中，用于高效预测阈值电压漂移和保持行为。

Result: PINO框架相比TCAD实现了超过10000倍的加速，同时保持了物理准确性。研究在单个FeFET配置上验证了模型的有效性。

Conclusion: 该PINO模型为建模保持损失机制提供了有效途径，能够支持Fe-VNAND器件的大规模优化设计。
...
### [112] [Single-pass Possibilistic Clustering with Damped Window Footprints](https://arxiv.org/abs/2603.06889)
*Jeffrey Dale,James Keller,Aquila Galusha*

Main category: cs.LG

TL;DR: 提出一种单次遍历可能性聚类算法SPC，用于大数据流聚类，能处理非球形聚类，具有闭式更新和协方差合并能力。


<details>
  <summary>Details</summary>
Motivation: 大数据时代下流式聚类需求日益增长（如网络流量分析、传感器数据处理），可能性模型相比传统方法具有独特优势，特别是通过"模糊化"参数控制典型性衰减速度。

Method: 提出SPC算法：单次遍历可能性聚类，能建模非球形聚类，具有闭式足迹更新（适用于任意大小阻尼窗口），采用多假设跟踪文献中的协方差合并技术来融合两个聚类均值和协方差估计。

Result: SPC在聚类纯度和归一化互信息指标上，与另外五种流式聚类算法进行了验证比较。

Conclusion: SPC是一种有效且易于应用于新数据集的流式可能性聚类算法，特别适合大数据流处理场景。
...
### [113] [Learning From Design Procedure To Generate CAD Programs for Data Augmentation](https://arxiv.org/abs/2603.06894)
*Yan-Ying Chen,Dule Shu,Matthew Hong,Andrew Taber,Jonathan Li,Matthew Klenk*

Main category: cs.LG

TL;DR: 提出一种基于LLM的CAD程序数据增强方法，通过参考曲面和建模过程生成多样化的CAD程序，提升几何复杂度以接近工业级设计


<details>
  <summary>Details</summary>
Motivation: LLM在代码生成方面表现出色，但在CAD程序生成领域面临挑战，特别是生成的几何形状复杂度有限，与真实工业设计存在差距。这主要是由于现有CAD程序训练数据缺乏多样性，尤其是缺少样条曲线定义的有机形状。

Method: 提出新颖的数据增强范式：使用LLM生成CAD程序，条件化于参考曲面程序和建模过程。通过变化有机形状的参考曲面，丰富生成CAD模型的几何分布，特别是引入现有开源CAD数据集中通常缺失或不足的样条曲线定义的边和面。

Result: 实验表明，该方法生成的CAD样本具有显著更大的几何多样性，在有机形状基元比例方面与工业级CAD设计具有更高的相似性。这种增强使得该方法成为训练LLM和其他深度学习模型进行CAD生成的有用工具。

Conclusion: 提出的数据增强方法通过引入基于样条曲线的有机形状，有效解决了CAD程序生成中几何复杂度不足的问题，为训练更强大的CAD生成模型提供了高质量的数据支持。
...
### [114] [XGenBoost: Synthesizing Small and Large Tabular Datasets with XGBoost](https://arxiv.org/abs/2603.06904)
*Jim Achterberg,Marcel Haas,Bram van Dijk,Marco Spruit*

Main category: cs.LG

TL;DR: XGenBoost：基于XGBoost的生成模型，包含扩散模型和自回归模型两种架构，用于混合类型表格数据生成，在性能和训练成本上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 树集成方法（如XGBoost）在混合类型表格数据的判别任务中表现出色，作者认为这些优势同样适用于生成模型。现有生成模型在表格数据合成方面存在不足，需要更高效、更准确的解决方案。

Method: 提出两种基于XGBoost的生成模型：1）DDIM扩散模型，使用XGBoost作为分数估计器，适用于小数据集；2）分层自回归模型，使用XGBoost分类器学习条件分布，适用于大规模表格合成。两种架构都充分利用了树模型的自然约束特性。

Result: 在两个基准测试中（包含小数据集和大数据集），XGenBoost在混合类型表格合成任务上超越了之前的神经和树基生成模型，同时训练成本更低。

Conclusion: 树集成方法的优势可以成功迁移到生成建模领域，XGenBoost为混合类型表格数据合成提供了高效且性能优越的解决方案，证明了树基生成模型的潜力。
...
### [115] [NerVE: Nonlinear Eigenspectrum Dynamics in LLM Feed-Forward Networks](https://arxiv.org/abs/2603.06922)
*Nandan Kumar Jha,Brandon Reagen*

Main category: cs.LG

TL;DR: NerVE是一个统一的特征谱框架，用于理解大语言模型中前馈网络在高维潜在空间中如何组织和调节信息流，通过轻量级特征谱动态跟踪提供可操作的架构和优化器设计见解。


<details>
  <summary>Details</summary>
Motivation: 尽管前馈网络在参数预算中占主导地位，但其高维动态特性仍未被充分理解。需要一种方法来理解FFN如何在高维潜在空间中组织和调节信息流。

Method: 提出NerVE框架，通过四种互补的轻量级指标跟踪特征谱动态：谱熵（离散度）、参与比（有效维度）、特征值早期富集（顶部权重）和Jensen-Shannon散度（分布偏移）。

Result: 验证了NerVE在不同模型规模、架构和优化器配置下的有效性，发现FFN非线性通过特征模式重新注入方差，优化器几何形状强烈调节这种方差重新注入的程度，并发现稳定的特征谱特征与模型泛化能力相关。

Conclusion: NerVE提供了一种理解FFN高维动态的统一框架，揭示了架构和优化器选择如何影响信息流组织，为超越试错的设计选择提供了可操作的见解，并推广到Transformer之外的MLP-Mixer架构。
...
### [116] [Swimba: Switch Mamba Model Scales State Space Models](https://arxiv.org/abs/2603.06938)
*Zhixu Du,Krishna Teja Chitty-Venkata,Murali Emani,Venkatram Vishwanath,Hai Helen Li,Yiran Chen*

Main category: cs.LG

TL;DR: Switch Mamba (Swimba) 提出了一种在状态空间模型(SSM)中引入专家混合(MoE)的新方法，通过参数空间的MoE设计，在保持单状态轨迹和一次递归计算的同时增加模型容量。


<details>
  <summary>Details</summary>
Motivation: 传统MoE方法应用于SSM时，由于需要维护多个状态轨迹，会显著增加递归状态更新的计算成本。研究目标是找到一种既能引入专家专业化，又能保持计算效率的SSM-MoE设计。

Method: 提出了两种MoE-SSM设计：1) 分离SSM上的MoE（维护多个状态轨迹）；2) 参数化MoE-SSM（维护单状态轨迹，一次递归计算）。Switch Mamba (Swimba)采用第二种设计，通过在专家生成的SSM流上进行路由来实现。

Result: 在相同FLOPs下，Swimba比基线模型表现略好，实时延迟和吞吐量有小幅下降。理论分析证明了MoE参数化SSM的良定义性和稳定性，并描述了两种设计之间的关系。

Conclusion: 参数空间的MoE设计可以在保持主要递归成本不变的情况下增加SSM容量，为高效的大规模SSM模型提供了可行的技术路径。
...
### [117] [Physics-Consistent Neural Networks for Learning Deformation and Director Fields in Microstructured Media with Loss-Based Validation Criteria](https://arxiv.org/abs/2603.06939)
*Milad Shirani,Pete H. Gueldner,Murat Khidoyatov,Jeremy L. Warren,Federica Ninno*

Main category: cs.LG

TL;DR: 结合有限元和神经网络求解Cosserat弹性模型，并引入经典稳定性理论验证神经网络预测的物理合理性


<details>
  <summary>Details</summary>
Motivation: 研究具有微观结构固体的力学行为，需要捕捉变形和取向场之间的耦合，并开发可靠的计算方法来求解这类介质的平衡构型

Method: 开发两种互补计算方法：基于变分原理的有限元公式和直接最小化总势能的神经网络求解器；神经网络架构尊重理论的基本运动学结构，保持能量框架不变性、满足单位长度约束，并通过独立网络表示变形和取向场

Result: 推导了Cosserat模型的拟凸性条件、秩一凸性条件和Legendre-Hadamard不等式，并将其表述为适合评估神经网络预测的形式，建立了基于物理的验证框架

Conclusion: 将经典变分稳定性理论与现代机器学习求解器相结合，建立了一个计算工作流程，其中平衡解不仅被学习，还接受能量一致性评估，神经网络输出违反必要稳定性条件的预测可以被拒绝
...
### [118] [Joint MDPs and Reinforcement Learning in Coupled-Dynamics Environments](https://arxiv.org/abs/2603.06946)
*Ege C. Kaya,Mahsa Ghasemi,Abolfazl Hashemi*

Main category: cs.LG

TL;DR: 论文提出了联合马尔可夫决策过程(JMDPs)形式化框架，用于处理多动作生成接口环境中反事实结果的联合分布问题。


<details>
  <summary>Details</summary>
Motivation: 传统MDP只指定边际分布，无法描述多个可能动作之间反事实结果的联合分布，而许多强化学习分布量（如差距分布、优越性概率）本质上是跨动作联合的。

Method: 通过增强MDP为JMDP，添加多动作样本转移模型来指定反事实结果的耦合，同时保持标准MDP交互作为边际观测。采用一步耦合机制，推导n阶回报矩的Bellman算子。

Result: 在一步耦合机制下，为n阶回报矩提供了具有收敛保证的动态规划和增量算法。

Conclusion: JMDP为具有多动作生成接口的耦合动力学环境提供了形式化框架，能够处理反事实结果的联合分布，扩展了传统MDP的能力。
...
### [119] [Not All Neighbors Matter: Understanding the Impact of Graph Sparsification on GNN Pipelines](https://arxiv.org/abs/2603.06952)
*Yuhang Song,Naima Abrar Shami,Romaric Duvignau,Vasiliki Kalavri*

Main category: cs.LG

TL;DR: 图稀疏化作为轻量级预处理步骤，可显著加速大规模图神经网络训练和推理，同时保持甚至提升节点分类准确率。
...
Method: 开发可扩展的实验框架，系统评估不同稀疏化方法对GNN性能和准确率的影响。首次全面研究稀疏化图上的GNN训练和推理。

Result: 1. 稀疏化通常保持甚至提升预测性能（如随机稀疏化使GAT在PubMed图上准确率提升6.8%）；2. 规模越大收益越明显，显著加速训练和推理（K-Neighbor稀疏化使Products图模型服务性能提升11.7倍，仅损失0.7%准确率）；3. 稀疏化计算开销很快被摊销，适用于超大规模图。

Conclusion: 图稀疏化是解决大规模图机器学习瓶颈的有效轻量级预处理技术，能在保持准确率的同时显著提升性能，特别适合大规模图场景。
...
### [120] [Chart-RL: Generalized Chart Comprehension via Reinforcement Learning with Verifiable Rewards](https://arxiv.org/abs/2603.06958)
*Xin Zhang,Xingyu Li,Rongguang Wang,Ruizhong Miao,Zheng Wang,Dan Roth,Chenyang Li*

Main category: cs.LG

TL;DR: Chart-RL：一种使用数学可验证奖励的强化学习方法，显著提升视觉语言模型在图表问答任务上的性能，尤其在复杂推理任务上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在图表理解任务上泛化能力不足，因为图表理解需要抽象、符号化和定量推理能力，而当前模型难以处理这种结构化视觉表示。

Method: 提出Chart-RL方法，采用基于数学可验证奖励的强化学习来增强视觉语言模型的图表问答能力，通过强化学习训练模型进行更准确的图表推理。

Result: 在多个图表理解基准测试中，Chart-RL均优于监督微调方法：在MutlChartQA上相对提升16.7%，在ChartInsights上提升11.5%。在25个扰动图表类别中的18个上表现更好，展示了强大的鲁棒性。更重要的是，仅用10个复杂示例训练的模型就优于用6000多个简单示例训练的模型。

Conclusion: 任务难度和内在复杂性比数据量对强化学习训练更重要，Chart-RL不仅提升了图表理解能力，还能有效迁移到视觉数学问题领域，为增强多模态系统的结构化推理能力提供了有效途径。
...
### [121] [Learning Quadruped Walking from Seconds of Demonstration](https://arxiv.org/abs/2603.06961)
*Ruipeng Zhang,Hongzhan Yu,Ya-Chien Chang,Chenghao Li,Henrik I. Christensen,Sicun Gao*

Main category: cs.LG

TL;DR: 该论文分析了四足机器人运动控制中，基于少量演示数据的模仿学习为何能超越传统模型控制方法，并提出了一种新的模仿学习方法。


<details>
  <summary>Details</summary>
Motivation: 研究四足运动控制中，模型自由学习何时能超越基于模型的控制设计。传统方法面临离散接触优化和模式切换组合爆炸的困难，而模仿学习可以利用数据模式绕过这些难题。

Method: 基于极限环结构、庞加莱回归映射和神经网络局部数值特性的理论分析，提出了一种新的模仿学习方法，该方法调节潜在空间变化与输出动作变化之间的对齐关系。

Result: 硬件实验证实，仅需几秒钟的演示数据，就能完全离线训练出各种运动策略，并具有合理的鲁棒性。

Conclusion: 该研究为四足机器人模仿学习在小数据场景下的有效性提供了理论依据，提出的方法能够高效地从少量演示中学习运动策略。
...
### [122] [Conditional Unbalanced Optimal Transport Maps: An Outlier-Robust Framework for Conditional Generative Modeling](https://arxiv.org/abs/2603.06972)
*Jiwoo Yoon,Kyumin Choi,Jaewoong Choi*

Main category: cs.LG

TL;DR: 提出条件非平衡最优传输(CUOT)框架，通过Csiszár散度惩罚放松分布匹配约束，增强对异常值的鲁棒性，并基于此构建条件非平衡最优传输映射(CUOTM)生成模型。


<details>
  <summary>Details</summary>
Motivation: 传统条件最优传输(COT)继承了最优传输对异常值敏感的局限性，在条件设置下尤为严重，因为每个条件分布仅从有限数据子集估计。需要更鲁棒的框架来处理现实数据中的异常值问题。

Method: 引入条件非平衡最优传输(CUOT)框架，通过Csiszár散度惩罚放松条件分布匹配约束，同时严格保持条件边际分布。基于半对偶形式提出CUOTM生成模型，采用三角c-变换参数化。

Result: 在2D合成数据和图像尺度数据集上的实验表明，CUOTM相比现有COT基线具有更优的异常值鲁棒性和竞争力的分布匹配性能，同时保持高采样效率。

Conclusion: CUOT框架有效解决了传统COT对异常值敏感的问题，CUOTM作为鲁棒的条件生成模型在实际应用中具有优势，理论分析验证了三角c-变换参数化的有效性。
...
### [123] [NePPO: Near-Potential Policy Optimization for General-Sum Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2603.06977)
*Addison Kalanther,Sanika Bharvirkar,Shankar Sastry,Chinmay Maheshwari*

Main category: cs.LG

TL;DR: 提出NePPO方法，通过学习玩家无关的势函数，在混合合作-竞争环境中计算近似纳什均衡


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习在一般和博弈中训练困难，学习动态不稳定，收敛保证有限，且异质偏好下系统目标不明确

Method: 提出近势策略优化(NePPO)，学习玩家无关势函数，使合作博弈的纳什均衡近似原博弈均衡；引入新MARL目标，使用零阶梯度下降最小化该目标

Result: NePPO在实验中表现优于MAPPO、IPPO和MADDPG等基线方法

Conclusion: NePPO为混合合作-竞争环境提供有效的近似纳什均衡计算方法，解决了MARL在一般和博弈中的挑战
...
### [124] [Diffusion Controller: Framework, Algorithms and Parameterization](https://arxiv.org/abs/2603.06981)
*Tong Yang,Moonkyung Ryu,Chih-Wei Hsu,Guy Tennenholtz,Yuejie Chi,Craig Boutilier,Bo Dai*

Main category: cs.LG

TL;DR: 提出DiffCon框架，将扩散模型反向采样视为状态随机控制问题，基于LS-MDP理论统一控制视角，推导出强化学习微调方法，实现高效灰盒适配。


<details>
  <summary>Details</summary>
Motivation: 当前可控扩散生成方法依赖各种启发式方法，缺乏统一的理论框架。需要建立统一理解，弥合理论与实践的差距。

Method: 将反向扩散采样建模为LS-MDP中的状态随机控制问题，控制通过重加权预训练的反向转移核实现。推导出f-散度正则化策略梯度更新和奖励加权回归目标，提出侧网络参数化架构，在冻结骨干网络基础上实现灰盒适配。

Result: 在Stable Diffusion v1.4上进行监督和奖励驱动的微调实验，在偏好对齐胜率和质量-效率权衡方面均优于灰盒基线，甚至超过参数高效的白盒适配器LoRA。

Conclusion: DiffCon框架为可控扩散生成提供了统一的理论基础，提出的方法在保持骨干网络冻结的同时实现了高效微调，在多个指标上优于现有方法。
...
### [125] [RESCHED: Rethinking Flexible Job Shop Scheduling from a Transformer-based Architecture with Simplified States](https://arxiv.org/abs/2603.07020)
*Xiangjie Xiao,Cong Zhang,Wen Song,Zhiguang Cao*

Main category: cs.LG

TL;DR: ReSched是一个简约的深度强化学习框架，用于解决柔性作业车间调度问题，通过简化状态表示和使用Transformer架构，在多个调度问题上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有神经方法依赖复杂的特征工程状态表示（通常需要20多个手工特征）和图偏置的神经架构，建模复杂度高，缺乏通用性。

Method: 1) 重新审视FJSP的MDP公式，将状态空间压缩为四个基本特征，通过子问题视角消除历史依赖；2) 使用带点积注意力的Transformer块，并针对调度任务添加三个轻量级架构修改。

Result: ReSched在FJSP上优于经典调度规则和最先进的DRL方法，同时在JSSP和FFSP上也表现出色，与专门为这些变体设计的神经基线相比具有竞争力。

Conclusion: ReSched通过简化状态表示和模型设计，提供了一个更通用、更简约的调度框架，在多个调度问题上都取得了优异性能。
...
### [126] [Resource-Adaptive Federated Text Generation with Differential Privacy](https://arxiv.org/abs/2603.07027)
*Jiayi Wang,John Gounley,Heidi Hanson*

Main category: cs.LG

TL;DR: 提出一个灵活的跨机构联邦学习框架，通过差分隐私合成数据和自适应客户端参与机制，解决计算异构性和隐私保护问题


<details>
  <summary>Details</summary>
Motivation: 跨机构联邦学习中，敏感文本数据因隐私法规限制而无法共享，传统方法需要为每个下游任务重复训练，通信开销大且隐私要求高。现有方法面临领域转移问题，且联邦微调受限于计算异构性——只有资源丰富的客户端能更新模型，弱客户端被排除在外，加剧了数据倾斜和差分隐私噪声的负面影响。

Method: 提出灵活参与框架：强客户端执行差分隐私联邦微调，弱客户端通过轻量级差分隐私投票机制优化合成文本。使用控制代码（标签、主题、元数据）表示客户端数据比例，将投票限制在语义一致的子集中，确保合成数据反映全局分布。采用两阶段方法，弱客户端只需单轮通信。

Result: 实验表明，该框架在差分隐私和异构性条件下改善了分布对齐和下游任务的鲁棒性。

Conclusion: 提出的灵活参与框架能有效整合不同计算能力客户端的贡献，在保护隐私的同时生成高质量的合成数据，解决了跨机构联邦学习中的计算异构性和隐私保护挑战。
...
### [127] [Interpretable Maximum Margin Deep Anomaly Detection](https://arxiv.org/abs/2603.07073)
*Zhiji Yang,Mei Huang,Xinyu Li,Xianli Pan,Qi Wang,Jianhua Zhao*

Main category: cs.LG

TL;DR: 提出IMD-AD方法解决Deep SVDD的三个问题：超球体坍塌、参数启发式选择、可解释性差，通过最大间隔目标和小量异常标签实现稳定训练和可解释检测。


<details>
  <summary>Details</summary>
Motivation: Deep SVDD存在三个主要问题：1) 容易发生超球体坍塌（hypersphere collapse）；2) 超球体参数通常需要启发式选择；3) 可解释性有限。需要一种更稳定、可学习参数且具有内在可解释性的深度异常检测方法。

Method: 提出IMD-AD方法：1) 利用少量标记异常和最大间隔目标稳定训练；2) 证明超球体参数与网络最后一层权重的等价性，使中心和半径能够端到端学习；3) 开发联合优化表示、间隔和最后一层参数的高效训练算法。

Result: 在图像和表格基准测试上的广泛实验表明，IMD-AD在检测性能上优于多个最先进的基线方法，同时提供可解释的决策诊断和可视化输出。

Conclusion: IMD-AD成功解决了Deep SVDD的三个关键问题，实现了稳定训练、端到端参数学习和内在可解释性，为深度异常检测提供了更可靠和可解释的解决方案。
...
### [128] [Entropy-Aware On-Policy Distillation of Language Models](https://arxiv.org/abs/2603.07079)
*Woogyeol Jin,Taywon Min,Yongjin Yang,Swanand Ravindra Kadhe,Yi Zhou,Dennis Wei,Nathalie Baracaldo,Kimin Lee*

Main category: cs.LG

TL;DR: 提出熵感知的在线策略蒸馏方法，通过结合反向KL和正向KL散度，在教师模型高熵时保持生成多样性，在低熵时保持精确模仿，提升知识蒸馏效果。


<details>
  <summary>Details</summary>
Motivation: 传统的在线策略蒸馏使用反向KL散度，鼓励学生匹配教师的高置信度预测，但这种模式寻求特性会降低生成多样性，且在教师分布熵高时产生不稳定的学习信号。

Method: 提出熵感知在线策略蒸馏，核心思想是在教师熵高时用正向KL散度增强标准反向KL目标，既能捕捉完整输出范围，又能在其他地方保持精确模仿，平衡模式寻求的精确性和模式覆盖的鲁棒性。

Result: 实验表明，该方法能维持生成多样性（保持词元级熵），改善师生对齐（高熵词元上的正向KL更低）。在六个数学推理基准测试中，相比基线在线策略蒸馏方法，Qwen3-0.6B-Base、Qwen3-1.7B-Base和Qwen3-4B-Base的Pass@8准确率分别提升+1.37、+2.39和+5.05。

Conclusion: 考虑教师不确定性对于保持多样性和实现有效知识转移至关重要，熵感知在线策略蒸馏方法在平衡精确模仿和生成多样性方面表现出色。
...
### [129] [Dreamer-CDP: Improving Reconstruction-free World Models Via Continuous Deterministic Representation Prediction](https://arxiv.org/abs/2603.07083)
*Michael Hauri,Friedemann Zenke*

Main category: cs.LG

TL;DR: 提出一种基于JEPA风格预测器的无重建模型，在Crafter环境中达到Dreamer性能，证明无需重建目标也能有效学习世界模型


<details>
  <summary>Details</summary>
Motivation: 现有基于模型强化学习（MBRL）方法如Dreamer依赖观测空间重建目标，但会使表示对任务无关细节敏感。替代的无重建方法性能较差，需要弥合这一差距

Method: 引入JEPA风格预测器，在连续确定性表示上定义预测目标，无需观测空间重建，实现无重建的世界模型学习

Result: 在Crafter环境中匹配Dreamer性能，首次在无重建目标的情况下在该基准上实现有效世界模型学习

Conclusion: 通过JEPA风格预测器成功弥合了Dreamer与无重建模型之间的性能差距，证明无需重建目标也能学习有效的世界模型表示
...
### [130] [Countdown-Code: A Testbed for Studying The Emergence and Generalization of Reward Hacking in RLVR](https://arxiv.org/abs/2603.07084)
*Muhammad Khalifa,Zohaib Khan,Omer Tafveez,Hao Peng,Lu Wang*

Main category: cs.LG

TL;DR: 论文提出了Countdown-Code环境，用于精确测量LLM中的奖励黑客行为，发现即使只有1%的污染数据就能在监督微调中学习到奖励黑客行为，并在后续强化学习中放大。


<details>
  <summary>Details</summary>
Motivation: 奖励黑客是模型过度优化代理奖励而非真正解决底层任务的一种错位形式。由于真实任务奖励通常昂贵或无法计算，精确测量奖励黑客发生率具有挑战性。

Method: 引入Countdown-Code最小环境，模型既能解决数学推理任务又能操纵测试工具。这种双重访问设计在代理奖励（测试通过/失败）和真实奖励（数学正确性）之间创建了清晰分离。

Result: 研究发现奖励黑客行为可在监督微调中无意学习，即使只有1%的污染数据就足以让模型内化奖励黑客行为，并在后续强化学习中重新出现。强化学习会放大错位并推动其泛化到原始领域之外。

Conclusion: 结果揭示了奖励黑客在LLM中出现和持续的一个先前未充分探索的途径，强调了对合成SFT数据进行更严格验证的必要性。开源环境代码以促进未来研究。
...
### [131] [Agentic Planning with Reasoning for Image Styling via Offline RL](https://arxiv.org/abs/2603.07148)
*Subhojyoti Mukherjee,Stefano Petrangeli,Branislav Kveton,Trung Bui,Franck Dernoncourt,Arko Mukherjee*

Main category: cs.LG

TL;DR: 提出基于工具的智能体强化学习后训练框架，通过结构化规划和链式推理来改进复杂图像编辑任务，相比直接提示编辑方法效果更好。


<details>
  <summary>Details</summary>
Motivation: 直接基于提示的编辑方法在处理复杂图像变换时经常失败，因为模糊和主观的提示需要细微理解图像中应该改变什么。利用组合式图像编辑工具而非直接提示，可以从结构化智能体级规划中获益，获得更好的结果。
...
Result: 方法在视觉质量和指令遵循方面持续优于Edit-Only基线，在大多数组合任务中优于其他基线，并通过了人工评估验证。

Conclusion: 结构化规划框架结合链式推理和离线RL后训练能够显著提升复杂图像编辑任务的性能，特别是在处理模糊和主观提示时表现出色。
...
### [132] [Spectral Conditioning of Attention Improves Transformer Performance](https://arxiv.org/abs/2603.07162)
*Hemanth Saratchandran,Simon Lucey*

Main category: cs.LG

TL;DR: 通过理论分析注意力块的雅可比矩阵，提出改进其谱性质的方法来降低条件数，从而提升Transformer中注意力层的整体条件性，并在实践中验证了性能提升。


<details>
  <summary>Details</summary>
Motivation: Transformer中注意力层的雅可比矩阵条件数会影响网络训练和优化的稳定性，改善雅可比矩阵的条件性可能提升模型性能。

Method: 理论分析注意力块的雅可比矩阵，发现其由查询、键和值投影决定。基于此，系统性地改变每个注意力层的谱性质，降低雅可比矩阵的条件数。

Result: 改进雅可比矩阵条件性在实践中转化为性能提升。该方法简单、通用，可作为即插即用模块集成到多种现有注意力机制中。

Conclusion: 通过改善注意力层雅可比矩阵的条件数，可以提升Transformer在各种架构和任务上的性能，该方法具有广泛适用性。
...
### [133] [Shaping Parameter Contribution Patterns for Out-of-Distribution Detection](https://arxiv.org/abs/2603.07195)
*Haonan Xu,Yang Yang*

Main category: cs.LG

TL;DR: 提出SPCP方法，通过塑造参数贡献模式来增强OOD检测鲁棒性，防止因稀疏参数贡献模式导致的过度自信预测


<details>
  <summary>Details</summary>
Motivation: 现有分类器倾向于依赖稀疏的参数贡献模式，即只有少数主导参数驱动预测。这种脆弱性可能被OOD输入利用，异常触发这些参数导致过度自信的预测

Method: 提出SPCP方法，在训练过程中通过基于动态估计阈值修正过高的参数贡献，鼓励分类器学习边界导向的密集贡献模式，使决策依赖于更广泛的参数集

Result: 在各种OOD检测设置下的广泛实验验证了SPCP的有效性，能够减少由异常触发参数引起的过度自信预测，同时保持ID性能

Conclusion: SPCP通过塑造参数贡献模式，增强了OOD检测的鲁棒性，解决了分类器因稀疏参数贡献模式导致的脆弱性问题
...
### [134] [A Dual-Graph Spatiotemporal GNN Surrogate for Nonlinear Response Prediction of Reinforced Concrete Beams under Four-Point Bending](https://arxiv.org/abs/2603.07201)
*Zhaoyang Ren,Qilin Li*

Main category: cs.LG

TL;DR: 开发了一个双图时空图神经网络替代模型，用于预测钢筋混凝土梁在四点弯曲下的时程响应，通过耦合节点和单元级动态来改进局部高梯度区域的峰值预测。


<details>
  <summary>Details</summary>
Motivation: 高保真非线性有限元模拟钢筋混凝土结构成本高昂，特别是在加载位置变化的参数化设置中。当单元量被迫通过节点表示时会出现峰值损失问题。

Method: 使用双图循环神经网络架构：节点级图卷积门控循环单元处理运动学，单元级图卷积门控循环单元处理历史相关的内部变量，通过池化预测全局力，并采用自回归展开方式。

Result: 模型能够联合预测节点位移、单元冯·米塞斯应力、等效塑性应变和全局垂直反力，在局部高梯度应力/应变区域改进了峰值敏感预测，同时不降低全局载荷位移趋势的准确性。

Conclusion: 该替代模型能以非线性有限元模拟的一小部分成本生成完整轨迹，实现了更快的参数化评估和设计探索。
...
### [135] [wDPO: Winsorized Direct Preference Optimization for Robust LLM Alignment](https://arxiv.org/abs/2603.07211)
*Jilong Liu,Yonghui Yang,Pengyang Shao,Haokai Ma,Wei Qin,Richang Hong*

Main category: cs.LG

TL;DR: wDPO提出分层缩尾策略，通过数据级和梯度级干预分别处理硬噪声和模糊比较，提升DPO在噪声偏好数据下的对齐鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有DPO的鲁棒变体主要依赖均匀目标修改或全局重加权，将噪声样本视为同质不确定性源，无法区分不同噪声类型，导致对齐鲁棒性不足。偏好数据在实践中通常存在噪声，需要针对不同噪声类型进行针对性干预。

Method: 提出winsorized DPO (wDPO)，采用无奖励分层干预策略：1) 利用DPO对数比隐含边际识别异质噪声模式；2) 对硬噪声进行数据级干预，稀疏修正强不一致偏好对；3) 对模糊比较进行梯度级干预，通过软缩尾限制高损失尾部的极端损失，防止弱信息样本主导梯度更新。

Result: 在PKU-SafeRLHF和多个外部安全基准上的实验表明，wDPO相比原始DPO和强DPO基线，能持续提升偏好对齐质量和鲁棒性，在受控标签翻转噪声下效果尤为显著。

Conclusion: 鲁棒偏好对齐需要针对不同噪声类型进行针对性干预而非均匀正则化。wDPO通过分层缩尾策略有效区分和处理硬噪声与模糊比较，为噪声偏好数据下的LLM对齐提供了更鲁棒的解决方案。
...
### [136] [Margin in Abstract Spaces](https://arxiv.org/abs/2603.07221)
*Yair Ashlagi,Roi Livni,Shay Moran,Tom Waknine*

Main category: cs.LG

TL;DR: 论文研究了基于间隔的学习在任意度量空间中的可学习性，揭示了间隔阈值现象，并证明了并非所有可学习的度量空间都能嵌入到线性空间中。
...
Method: 首先研究任意度量空间中的简单基于间隔问题：概念由中心点定义，根据距离是否低于r或高于R进行分类。然后扩展到由距离函数的有界线性组合定义的概念。最后通过构建Banach空间的分类结构来研究是否所有可学习的度量空间都能嵌入到线性空间。

Result: 1. 当R>3r时，该类在任意度量空间中都是可学习的；2. 存在通用常数γ>0，当间隔大于γ时，该类在每个度量空间中都是可学习的，而低于γ时存在不可学习的度量空间；3. 并非所有可学习的度量空间都能通过核型构造嵌入到线性空间中；4. 无限维空间中样本复杂度必须按(1/γ)^p（p≥2）的多项式增长。

Conclusion: 间隔学习的基本现象仅依赖于三角不等式，而不需要线性或解析结构。存在一个通用间隔阈值决定可学习性，且并非所有可学习的度量空间都能简化为线性分类。这为理解现代过参数化学习中的间隔现象提供了理论基础。
...
### [137] [Unlocking Data Value in Finance: A Study on Distillation and Difficulty-Aware Training](https://arxiv.org/abs/2603.07223)
*Chuxue Cao,Honglin Lin,Zhanping Zhong,Xin Gao,Mengzhang Cai,Conghui He,Sirui Han,Lijun Wu*

Main category: cs.LG

TL;DR: 本文提出通过多阶段蒸馏和验证构建高质量金融领域数据集ODA-Fin-SFT-318k和ODA-Fin-RL-12k，结合SFT和RL训练，使8B模型在多个金融基准测试中超越同规模开源SOTA模型。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在金融领域部署面临挑战：专业术语密集、数值推理要求严格、事实错误容忍度低。现有模型在垂直领域性能主要受后训练数据质量和难度/可验证性特征影响。

Method: 1. 构建ODA-Fin-SFT-318k数据集：通过多阶段蒸馏和验证生成高质量思维链监督数据；2. 构建ODA-Fin-RL-12k数据集：针对难但可验证任务，平衡奖励精度和任务多样性；3. 使用标准SFT和RL流程训练模型。

Result: ODA-Fin-RL-8B模型在9个基准测试（通用金融任务、情感分析、数值推理）中一致超越同规模开源SOTA金融大语言模型。高质量思维链蒸馏为SFT建立坚实基础，难度和可验证性感知采样提升RL泛化能力。

Conclusion: 金融领域大语言模型性能提升的关键在于高质量、难度和可验证性平衡的数据集构建。通过数据中心的AI研究方法，结合多阶段蒸馏验证和难度感知采样，可以有效提升模型在专业领域的表现。作者开源了数据集和训练模型以推动金融AI研究。
...
### [138] [LightMedSeg: Lightweight 3D Medical Image Segmentation with Learned Spatial Anchors](https://arxiv.org/abs/2603.07228)
*Kavyansh Tyagi,Vishwas Rathi,Puneet Goyal*

Main category: cs.LG

TL;DR: LightMedSeg：一种轻量级3D医学图像分割架构，通过整合解剖先验和自适应上下文建模，在仅需0.48M参数和14.64GFLOPs的情况下，达到接近重型Transformer基线的分割精度。


<details>
  <summary>Details</summary>
Motivation: 临床AI中的3D医学图像分割需要在严格的内存、延迟和数据可用性约束下保持可靠性。现有的Transformer方法虽然精度高，但存在参数过多、计算量大（高FLOPs）和泛化能力有限的问题。

Method: 提出模块化的UNet风格分割架构，整合解剖先验与自适应上下文建模。包括：锚点条件FiLM调制实现解剖感知特征校准；局部结构先验模块和纹理感知路由动态分配表示能力到边界丰富区域；通过幽灵卷积和深度可分离卷积最小化计算冗余；通过带有锚点相对空间位置偏置的学习跳跃路由器自适应融合多尺度特征。

Result: 仅需0.48M参数和14.64GFLOPs，分割精度在几个Dice点内接近重型Transformer基线，实现了可部署且数据高效的3D医学图像分割解决方案。

Conclusion: LightMedSeg是一种轻量、高效且可部署的3D医学图像分割架构，在保持接近最先进精度的同时，显著降低了计算和内存需求，适合临床AI应用。
...
### [139] [Retrieval-Augmented Generation for Predicting Cellular Responses to Gene Perturbation](https://arxiv.org/abs/2603.07233)
*Andrea Giuseppe Di Francesco,Andrea Rubbi,Pietro Liò*

Main category: cs.LG

TL;DR: PT-RAG是一个用于预测细胞对基因扰动响应的两阶段检索增强生成框架，通过可微分检索和细胞类型感知机制，在单基因扰动数据集上超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在预测细胞对基因扰动的响应时，由于缺乏足够的上下文信息，难以在不同细胞类型和扰动情境下泛化。需要一种能够有效利用相关扰动信息的方法来改进预测性能。

Method: PT-RAG采用两阶段检索增强生成框架：1）使用GenePT嵌入检索候选扰动K；2）通过Gumbel-Softmax离散采样，基于细胞状态和输入扰动自适应地精炼选择。这种细胞类型感知的可微分检索允许端到端优化检索目标和生成过程。

Result: 在Replogle-Nadig单基因扰动数据集上，PT-RAG在相同实验条件下超越了STATE和普通RAG方法，在分布相似性指标（W1、W2）上表现最佳。特别值得注意的是，普通RAG的显著失败表明可微分、细胞类型感知的检索在该领域至关重要。

Conclusion: 检索增强生成是建模细胞对基因扰动响应的有前景范式。PT-RAG的成功表明，可微分、细胞类型感知的检索机制对于该领域的性能提升至关重要，而简单的检索方法可能反而损害性能。
...
### [140] [Rethinking Deep Research from the Perspective of Web Content Distribution Matching](https://arxiv.org/abs/2603.07241)
*Zixuan Yu,Zhenheng Tang,Tongliang Liu,Chengqi Zhang,Xiaowen Chu,Bo Han*

Main category: cs.LG

TL;DR: WeDas框架通过引入搜索空间结构感知，解决了深度搜索代理中推理查询与网页索引结构不匹配的问题，显著提升了子目标完成度和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有深度搜索代理框架将搜索引擎视为静态工具，导致查询要么过于粗略要么过于精细，无法获取精确证据，存在推理驱动查询与底层网页索引结构不匹配的问题。

Method: 提出WeDas框架，将搜索空间结构特征纳入代理的观察空间；核心是查询-结果对齐分数，量化代理意图与检索结果的兼容性；通过少量样本探测机制迭代估计该分数，使代理能基于本地内容景观动态重新校准子目标。

Result: WeDas作为即插即用模块，在四个基准测试中持续改善了子目标完成度和准确性，有效弥合了高层推理与低层检索之间的差距。

Conclusion: 通过将搜索空间结构特征纳入代理观察空间，WeDas框架成功解决了深度搜索代理中查询与网页索引结构不匹配的问题，显著提升了检索性能。
...
### [141] [LF2L: Loss Fusion Horizontal Federated Learning Across Heterogeneous Feature Spaces Using External Datasets Effectively: A Case Study in Second Primary Cancer Prediction](https://arxiv.org/abs/2603.07249)
*Chia-Fu Lin,Yi-Ju Tseng*

Main category: cs.LG

TL;DR: 提出LF2L框架，通过损失融合的横向联邦学习整合多源数据，提升肺癌幸存者第二原发癌的预测性能


<details>
  <summary>Details</summary>
Motivation: 随着癌症生存率提高，第二原发癌成为重要问题。台湾本地数据集规模有限，传统机器学习方法效果和泛化性受限，需要整合外部数据但面临特征不一致和隐私约束挑战

Method: 提出损失融合横向联邦学习框架，不直接合并数据，而是通过共享损失机制平衡共同特征和独特特征的贡献，在保护机构隐私的同时实现跨机构协作

Result: 相比本地化、横向联邦学习和集中式学习基线，在AUROC和AUPRC指标上取得显著统计改进，证明有效利用外部数据能提升临床模型性能

Conclusion: 不仅需要获取外部数据，更需要有效利用来增强现实世界临床模型开发中的性能，LF2L框架为多源医疗数据整合提供了隐私保护的解决方案
...
### [142] [Turning Time Series into Algebraic Equations: Symbolic Machine Learning for Interpretable Modeling of Chaotic Time Series](https://arxiv.org/abs/2603.07261)
*Madhurima Panja,Grace Younes,Tanujit Chakraborty*

Main category: cs.LG

TL;DR: 提出两种符号预测器（SyNF和SyTF），从混沌时间序列数据中学习可解释的代数方程，在保持竞争力的单步预测精度的同时提供透明方程。


<details>
  <summary>Details</summary>
Motivation: 混沌时间序列预测困难，现代深度学习虽然短期精度高但黑盒性质限制了科学洞察和实际信任，特别是在需要理解底层动力学的场景中。

Method: 提出两种符号预测器：1) SyNF（符号神经预测器）基于神经网络方程学习架构，实现完全可微的紧凑可解释代数关系发现；2) SyTF（符号树预测器）基于进化符号回归，在原则性的精度-复杂度权衡下直接搜索方程结构。

Result: 在132个低维混沌吸引子基准测试和两个真实世界混沌时间序列（圣胡安登革热周发病率和Nino 3.4海表温度指数）上，符号预测器实现了竞争力的单步预测精度，同时提供揭示底层动力学关键方面的透明方程。

Conclusion: 符号预测器填补了混沌时间序列预测中可解释性的空白，在保持预测精度的同时提供透明方程，增强了科学洞察和实际信任。
...
### [143] [Adaptive Double-Booking Strategy for Outpatient Scheduling Using Multi-Objective Reinforcement Learning](https://arxiv.org/abs/2603.07270)
*Ninda Nurseha Amalina,Heungjo An*

Main category: cs.LG

TL;DR: 提出自适应门诊双重预约框架，整合个体化爽约预测与多目标强化学习，动态决定单预约、双预约或拒绝预约请求


<details>
  <summary>Details</summary>
Motivation: 患者爽约扰乱门诊运营，降低效率，可能延误必要治疗。现有方法依赖固定启发式规则，无法适应实时调度条件或患者特定爽约风险

Method: 1) 将调度问题建模为马尔可夫决策过程；2) 使用多头注意力软随机森林模型预测患者级爽约概率；3) 开发带多策略协同进化机制的多策略近端策略优化方法；4) 基于KL散度提出τ规则实现行为相似策略间的选择性知识转移；5) 使用SHAP解释爽约风险和调度决策

Result: 框架能够动态决定单预约、双预约或拒绝预约请求，提供传统门诊调度策略的数据驱动替代方案

Conclusion: 提出的自适应双重预约框架整合了个体化爽约预测和多目标强化学习，通过选择性知识转移机制改善收敛性和权衡多样性，为门诊调度提供了动态、数据驱动的解决方案
...
### [144] [Spectral Discovery of Continuous Symmetries via Generalized Fourier Transforms](https://arxiv.org/abs/2603.07299)
*Pavan Karjol,Kumar Shubham,Prathosh AP*

Main category: cs.LG

TL;DR: 基于广义傅里叶变换的谱结构方法，通过检测谱域中的稀疏模式来发现连续单参数子群对称性，无需优化生成器


<details>
  <summary>Details</summary>
Motivation: 连续对称性在科学和学习问题中很基础，但通常先验未知。现有方法要么在变换生成器空间中搜索，要么依赖学习的增强方案，需要新的视角

Method: 提出基于谱结构的框架，使用广义傅里叶变换。核心观察是：对子群的不变性会在函数的不可约表示谱分解中诱导结构化稀疏性。通过检测谱域中的这种稀疏模式来发现对称性，而不是优化生成器

Result: 在最大环面（GFT简化为多维傅里叶分析）上开发对称性检测程序。在包括双摆和顶夸克标记等结构化任务中，谱稀疏性可靠地揭示单参数对称性

Conclusion: 谱分析为基于生成器的对称性发现提供了原则性和可解释的替代方案，通过谱结构检测对称性比传统方法更直接可靠
...
### [145] [AutoResearch-RL: Perpetual Self-Evaluating Reinforcement Learning Agents for Autonomous Neural Architecture Discovery](https://arxiv.org/abs/2603.07300)
*Nilesh Jain,Rohit Yadav,Sagar Kotian,Claude AI*

Main category: cs.LG

TL;DR: AutoResearch-RL：一个让强化学习代理自主进行神经网络架构和超参数研究的框架，无需人工监督，通过修改训练脚本并评估验证损失来持续优化模型配置。


<details>
  <summary>Details</summary>
Motivation: 传统神经架构搜索和超参数优化需要大量人工干预和专家知识。该研究旨在开发一个完全自主的研究框架，让AI代理能够持续进行开放式的架构和超参数研究，无需人类监督，直到达到收敛或资源耗尽。

Method: 框架采用三层分离设计：1) 冻结环境（数据管道、评估协议和常量）确保实验公平比较；2) 可修改的目标文件（train.py）作为代理的可编辑状态；3) 元学习器（RL代理）积累实验轨迹并指导后续修改。代理使用PPO算法，通过修改代码、执行训练、观察验证比特每字节奖励来更新策略。

Result: 在单GPU nanochat预训练基准测试中，AutoResearch-RL经过约300次夜间迭代后，发现的配置能够匹配或超过手动调优的基线，整个过程无需人工干预。

Conclusion: 该研究证明了强化学习代理能够自主进行神经架构和超参数研究，通过分离环境、可编辑状态和元学习器的设计，实现了完全自动化的模型优化过程，为未来的自主AI研究提供了新范式。
...
### [146] [Retrieval-Augmented Multi-scale Framework for County-Level Crop Yield Prediction Across Large Regions](https://arxiv.org/abs/2603.07305)
*Yiming Sun,Qi Cheng,Licheng Liu,Runlong Yu,Yiqun Xie,Xiaowei Jia*

Main category: cs.LG

TL;DR: 提出一个结合短期和长期时间模式捕捉的作物产量预测框架，通过检索式适应策略处理空间异质性，在630个美国县的实际玉米产量数据上表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动方法在大地理区域和长时间跨度上性能下降，主要因为难以同时捕捉短期和长期时间模式，以及无法有效处理农业系统的空间数据变异性，这影响了特定区域或年份的预测可靠性。

Method: 1) 提出新的主干模型架构，同时捕捉短期日尺度作物生长动态和跨年长期依赖关系；2) 引入检索式适应策略增强模型在不同空间区域的泛化能力；3) 设计新颖的检索-精炼流程，通过移除输入特征无法解释的跨年偏差来调整检索样本。

Result: 在美国630个县的县级玉米产量数据实验中，该方法持续优于不同类型的基线方法，验证了检索式增强方法在空间异质性下提高模型鲁棒性的有效性。

Conclusion: 提出的预测框架通过同时处理短期和长期时间模式以及空间异质性，显著提高了作物产量预测的准确性和鲁棒性，有助于改进管理策略、保险评估和粮食安全规划。
...
### [147] [ShakyPrepend: A Multi-Group Learner with Improved Sample Complexity](https://arxiv.org/abs/2603.07319)
*Lujing Zhang,Daniel Hsu,Sivaraman Balakrishnan*

Main category: cs.LG

TL;DR: 提出ShakyPrepend方法，利用差分隐私工具改进多组学习的理论保证，适应组结构和空间异质性


<details>
  <summary>Details</summary>
Motivation: 多组学习任务关注控制预测器在指定子组上的条件损失，需要改进现有方法的理论保证

Method: ShakyPrepend方法，利用差分隐私启发的工具来获得更好的理论保证

Result: 数值实验表明ShakyPrepend能适应组结构和空间异质性，提供了实际部署指导

Conclusion: 提出的方法在多组学习中具有改进的理论保证和实际应用价值
...
### [148] [Norm-Hierarchy Transitions in Representation Learning: When and Why Neural Networks Abandon Shortcuts](https://arxiv.org/abs/2603.07323)
*Truong Xuan Khanh,Truong Quynh Hoa*

Main category: cs.LG

TL;DR: 论文提出Norm-Hierarchy Transition框架，解释神经网络为何需要多个epoch才能从虚假捷径特征转向结构化表示，认为这是正则化优化中参数范数层级缓慢遍历的结果。


<details>
  <summary>Details</summary>
Motivation: 神经网络通常依赖虚假捷径特征多个epoch后才发现结构化表示，但这一转换何时发生以及其时间尺度能否预测的机制尚不清楚。现有工作显示梯度下降收敛到低范数解且神经网络有简单性偏置，但都无法解释从捷径特征到结构化表示的转换时间尺度。

Method: 提出Norm-Hierarchy Transition框架，将延迟表示学习解释为正则化优化过程中参数范数层级的缓慢遍历。当存在多个具有不同范数的插值解时，权重衰减逐渐将模型从高范数的捷径解移向低范数的结构化表示。推导出转换延迟与捷径范数和结构化范数之比的对数增长紧密相关的界限。

Result: 在模运算、带虚假特征的CIFAR-10、CelebA和Waterbirds数据集上的实验支持该框架的预测。结果表明grokking、捷径学习和延迟特征发现都源于训练过程中范数层级遍历的共同机制。

Conclusion: Norm-Hierarchy Transition框架为神经网络延迟表示学习提供了统一解释，揭示了grokking、捷径学习和延迟特征发现背后的共同机制，即正则化优化中参数范数层级的缓慢遍历过程。
...
### [149] [Learning Concept Bottleneck Models from Mechanistic Explanations](https://arxiv.org/abs/2603.07343)
*Antonio De Santis,Schrasing Tong,Marco Brambilla,Lalana Kagal*

Main category: cs.LG

TL;DR: M-CBM通过从黑盒模型中提取概念构建瓶颈层，相比传统CBM在保持稀疏性的同时提升了性能


<details>
  <summary>Details</summary>
Motivation: 传统CBM使用预定义概念可能预测能力不足或难以从数据中学习，导致性能显著落后于黑盒模型

Method: 使用稀疏自编码器从黑盒模型中提取概念，然后用多模态LLM对选定图像进行概念命名和标注

Result: M-CBM在多种数据集上一致超越先前的CBM，同时改善了概念预测并提供简洁解释

Conclusion: M-CBM通过从黑盒模型自身学习概念构建瓶颈层，解决了传统CBM概念定义不足的问题
...
### [150] [Learning Clinical Representations Under Systematic Distribution Shift](https://arxiv.org/abs/2603.07348)
*Yuanyun Zhang,Shi Li*

Main category: cs.LG

TL;DR: 提出一种实践不变表示学习框架，用于多模态临床预测，通过抑制环境预测信息来提升模型在分布外数据上的鲁棒性和可迁移性。


<details>
  <summary>Details</summary>
Motivation: 临床机器学习模型在训练和部署环境之间存在系统性分布偏移，这种偏移源于异构的测量策略、文档实践和机构工作流程，导致生理信号与特定实践伪影之间的表示纠缠。

Method: 将临床观察建模为潜在生理因素和环境依赖过程的产物，结合监督风险最小化、对抗性环境正则化和跨医院不变风险惩罚，共同优化预测性能同时抑制学习嵌入中的环境预测信息。

Result: 在多个纵向电子健康记录预测任务和跨机构评估中，该方法相比掩码预训练和标准监督基线，将分布外AUROC提升了2-3个百分点，同时保持分布内性能并改善校准。

Conclusion: 在表示学习过程中显式考虑系统性分布偏移能够产生更鲁棒和可迁移的临床模型，突显了在医疗AI中结构不变性与架构规模同等重要。
...
### [151] [Latent Generative Models with Tunable Complexity for Compressed Sensing and other Inverse Problems](https://arxiv.org/abs/2603.07357)
*Sean Gunn,Jorio Cocola,Oliver De Candido,Vaggos Chatziafratis,Paul Hand*

Main category: cs.LG

TL;DR: 该论文提出了可调复杂度的生成先验模型，通过嵌套dropout技术让扩散模型、归一化流和变分自编码器能够动态调整表示复杂度，在多种逆问题中取得比固定复杂度基线更低的重建误差。


<details>
  <summary>Details</summary>
Motivation: 传统生成模型通常使用固定的复杂度或维度来表示自然信号，这在解决逆问题时存在局限性：复杂度太大会导致对噪声过拟合，复杂度太小则会产生高表示误差。需要一种能够根据具体问题动态调整复杂度的先验模型。

Method: 利用嵌套dropout技术开发了可调复杂度的生成先验模型，应用于扩散模型、归一化流和变分自编码器。该方法允许模型根据任务需求动态调整表示复杂度。

Result: 在压缩感知、图像修复、去噪和相位恢复等多种任务中，可调先验模型一致性地取得了比固定复杂度基线更低的重建误差。在线性去噪场景下，理论分析明确表征了最优调优参数如何依赖于噪声和模型结构。

Conclusion: 可调复杂度的生成先验模型具有显著潜力，能够更好地平衡表示误差和过拟合问题。这项工作为支持理论的发展和在更广泛逆问题中的应用提供了动机。
...
### [152] [N-Tree Diffusion for Long-Horizon Wildfire Risk Forecasting](https://arxiv.org/abs/2603.07361)
*Yucheng Xing,Xin Wang*

Main category: cs.LG

TL;DR: NT-Diffusion是一种用于长时程野火风险预测的分层扩散模型，通过共享早期去噪阶段并在后期分支，减少冗余计算，提高预测效率
...
Method: 提出N-Tree Diffusion（NT-Diffusion）分层扩散模型，将火灾发生表示为连续火灾风险图（FRMs），共享早期去噪阶段，在后期进行时域特定分支，实现特定时域细化同时减少冗余采样

Result: 在新建的真实世界野火数据集上评估，NT-Diffusion相比基线预测方法实现了持续准确性提升和推理成本降低

Conclusion: NT-Diffusion为长时程野火风险预测提供了一种高效的概率建模框架，通过分层设计平衡了计算效率和预测精度
...
### [153] [Scaling Laws in the Tiny Regime: How Small Models Change Their Mistakes](https://arxiv.org/abs/2603.07365)
*Mohammed Alnemari,Rizwan Qureshi,Nader Begrazadah*

Main category: cs.LG

TL;DR: 该论文研究了参数规模在20M以下的小型神经网络（TinyML/边缘AI领域）的缩放规律，发现其错误率遵循幂律分布，但指数比大型语言模型更陡峭，且缩放规律并不均匀，错误结构随规模变化显著。


<details>
  <summary>Details</summary>
Motivation: 现有神经缩放定律研究主要关注参数规模超过100M的大型模型，而TinyML和边缘AI领域通常使用20M以下的小型模型，这一规模区间的缩放规律尚未得到充分研究。

Method: 训练了90个模型（参数范围22K-19.8M），采用两种架构（普通ConvNet和MobileNetV2），在CIFAR-100数据集上保持深度和训练设置不变，仅改变宽度，分析错误率、错误集重叠度、类别分布和校准度等指标。

Result: 小型模型错误率遵循幂律分布，但指数（α=0.156和0.106）比大型语言模型（α≈0.076）陡峭1.4-2倍；缩放规律不均匀，局部指数随规模衰减；错误集重叠度低（Jaccard=0.35）；小模型将容量集中于简单类别，放弃最难类别；最小模型校准度最佳（ECE=0.013）。

Conclusion: 小型神经网络的缩放规律与大型模型不同，错误结构随规模显著变化，仅依赖聚合准确率会误导边缘部署决策，必须在目标模型规模上进行验证。
...
### [154] [Learning to Reflect: Hierarchical Multi-Agent Reinforcement Learning for CSI-Free mmWave Beam-Focusing](https://arxiv.org/abs/2603.07370)
*Hieu Le,Oguz Bedir,Mostafa Ibrahim,Jian Tao,Sabit Ekin*

Main category: cs.LG

TL;DR: 提出基于分层多智能体强化学习的无CSI毫米波可重构智能表面控制框架，通过用户定位数据替代信道估计，实现高效波束聚焦


<details>
  <summary>Details</summary>
Motivation: 可重构智能表面实际部署面临信道状态信息估计开销过大和集中式优化维度爆炸的问题，需要更实用的解决方案

Method: 采用分层多智能体强化学习框架，高层控制器负责用户-反射面分配，低层控制器进行焦点优化，使用MAPPO算法和CTDE训练范式

Result: 相比集中式基线获得2.81-7.94 dB RSSI提升，系统复杂度增加时优势更明显；用户密度翻倍时性能下降最小，总功率稳定；在45-99个反射单元和0.5米定位误差下保持鲁棒性
...
### [155] [ConfHit: Conformal Generative Design with Oracle Free Guarantees](https://arxiv.org/abs/2603.07371)
*Siddhartha Laghuvarapu,Ying Jin,Jimeng Sun*

Main category: cs.LG

TL;DR: ConfHit是一个用于生成式药物发现的分布无关框架，通过加权可交换性和多重样本密度比加权符合性p值，为生成的分子批次提供统计保证，确保至少包含一个有效候选分子。


<details>
  <summary>Details</summary>
Motivation: 当前生成式模型在科学发现中缺乏对生成候选分子满足特定属性的可靠保证。现有的符合性预测方法在药物发现应用中受到预算限制、缺乏实验验证（oracle访问）和分布偏移的限制。

Method: ConfHit利用历史样本和生成样本之间的加权可交换性来消除对实验验证的需求，构建多重样本密度比加权符合性p值来量化命中统计置信度，并提出嵌套测试程序来认证和精炼候选集。

Result: 在代表性生成分子设计任务和多种方法中，ConfHit在不同置信水平下始终提供有效的覆盖保证，同时保持紧凑的认证集。

Conclusion: ConfHit为生成式建模建立了一个原则性和可靠的框架，能够在预算限制和缺乏实验验证的情况下提供统计保证。
...
### [156] [Sparsity and Out-of-Distribution Generalization](https://arxiv.org/abs/2603.07388)
*Scott Aaronson,Lin Lin Lee,Jiawei Li*

Main category: cs.LG

TL;DR: 论文提出了一个基于特征稀疏性的OOD泛化理论框架，通过特征选择、奥卡姆剃刀和分布重叠保证，形式化了OOD泛化原理，并推广到子空间junta分类器。


<details>
  <summary>Details</summary>
Motivation: 解释OOD泛化是认识论和机器学习中的核心问题。自1946年Goodman的"grue"悖论以来，这一问题在AI对齐等领域尤为重要。需要为OOD泛化提供原则性解释。

Method: 提出三个核心要素：1) 世界通过区分性特征呈现；2) 奥卡姆剃刀偏好稀疏假设（依赖尽可能少的特征）；3) 只要训练和测试分布在相关特征限制上充分重叠，稀疏假设就能泛化。形式化为定理，推广Blumer等人的样本复杂度界限到OOD场景，并扩展到子空间junta分类器。

Result: 证明了形式化上述直觉的简单定理，将Blumer等人的经典样本复杂度界限推广到OOD上下文。进一步将稀疏分类器推广到子空间junta，其中真实分类器仅依赖于特征的低维线性子空间。

Conclusion: 为OOD泛化提供了基于特征稀疏性的原则性解释框架，通过形式化定理和推广到子空间junta分类器，为机器学习中的分布外泛化问题提供了理论支撑。
...
### [157] [Feed m Birds with One Scone: Accelerating Multi-task Gradient Balancing via Bi-level Optimization](https://arxiv.org/abs/2603.07389)
*Xuxing Chen,Yun He,Jiayi Xu,Minhui Huang,Xiaoyi Liu,Boyang Liu,Fei Tian,Xiaohan Wei,Rong Jin,Sem Park,Bo Long,Xue Feng*

Main category: cs.LG

TL;DR: MARIGOLD提出了一种高效的多任务学习框架，通过分层结构和零阶方法解决传统梯度平衡方法计算效率低的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多任务学习方法如MGDA及其变体虽然能动态调整任务权重缓解冲突，但需要访问所有任务梯度，计算效率低下。需要一种更高效的多任务学习算法框架。
...
Conclusion: MARIGOLD框架通过分层建模和零阶优化，有效解决了多任务学习中梯度平衡方法的计算效率问题，为高效多任务学习提供了新思路。
...
### [158] [Deterministic Fuzzy Triage for Legal Compliance Classification and Evidence Retrieval](https://arxiv.org/abs/2603.07390)
*Rian Atri*

Main category: cs.LG

TL;DR: 提出基于确定性双编码器和透明模糊分类带的合同证据分类方法，在ACORD和CUAD数据集上表现良好，通过校准阈值实现自动决策与人工审查的平衡。


<details>
  <summary>Details</summary>
Motivation: 法律团队使用机器学习处理大量合同证据时，现有模型通常不透明、非确定性，难以符合HIPAA或NERC-CIP等法规框架。需要一种简单、可复现的替代方案。

Method: 使用RoBERTa-base双编码器，512维投影和余弦相似度，在ACORD基准上训练分级条款检索，然后在CUAD衍生的二元合规数据集上微调。将标量合规分数映射到三个区域：自动不合规、自动合规和人工审查，通过验证数据调优阈值以最大化自动决策覆盖率，同时将经验错误率限制在2%以内。

Result: 在ACORD测试集上NDCG@5为0.38-0.42，NDCG@10为0.45-0.50，4-star Precision@5约0.37。在CUAD二元标签上AUC为0.98-0.99，F1为0.22-0.30（正类权重不同）。在正类率约0.6%的高度不平衡设置中优于多数类和随机基线。

Conclusion: 确定性编码器、校准模糊带和显式错误约束提供了手工规则和不透明大语言模型之间的实用中间方案，支持可解释的证据分类、可复现的审计追踪以及与法律审查概念的具体映射。
...
### [159] [Generalizing Linear Autoencoder Recommenders with Decoupled Expected Quadratic Loss](https://arxiv.org/abs/2603.07402)
*Ruixin Guo,Xinyu Li,Hao Zhou,Yang Zhou,Ruoming Jin*

Main category: cs.LG

TL;DR: 本文提出解耦期望二次损失（DEQL）来推广EDLAE目标，揭示了b>0的闭式解，并通过Miller矩阵逆定理确保计算可行性，在基准数据集上优于b=0基线。
...
Method: 将EDLAE目标推广为解耦期望二次损失（DEQL），简化求解过程并揭示b>0的闭式解。采用基于Miller矩阵逆定理的高效算法确保b>0情况下的计算可行性。

Result: 在基准数据集上的实验结果表明，DEQL提供的b>0解优于b=0的EDLAE基线，证明DEQL扩展了解空间并能发现测试性能更好的模型。

Conclusion: DEQL框架成功推广了EDLAE，揭示了更广泛的超参数范围内的闭式解，为线性自编码器在推荐系统中的实际应用提供了更强大的理论支持和计算工具。
...
### [160] [Context Channel Capacity: An Information-Theoretic Framework for Understanding Catastrophic Forgetting](https://arxiv.org/abs/2603.07415)
*Ran Cheng*

Main category: cs.LG

TL;DR: 论文提出"上下文信道容量"理论框架，证明零遗忘需要C_ctx ≥ H(T)，建立"不可能三角"，并验证HyperNetwork等架构能实现零遗忘。
...
Method: 引入上下文信道容量(C_ctx)概念，建立不可能三角理论，提出错误上下文探测(P5)诊断协议，并在Split-MNIST和CIFAR-10上进行大规模实验验证。

Result: C_ctx完美预测遗忘行为：C_ctx=0的方法灾难性遗忘(6-97%)，C_ctx≈1的方法实现零遗忘(98.8% ACC)。梯度上下文编码器将CIFAR-10的oracle gap从23.3pp降至0.7pp。

Conclusion: 架构设计优先于算法设计，上下文通路必须在结构上不可绕过。条件再生架构通过重新定义参数为函数值而非状态来绕过不可能三角。
...
### [161] [DualSpec: Accelerating Deep Research Agents via Dual-Process Action Speculation](https://arxiv.org/abs/2603.07416)
*Shuzhang Zhong,Baotong Lu,Qi Chen,Chuanjie Liu,Fan Yang,Meng Li*

Main category: cs.LG

TL;DR: DualSpec：针对深度研究代理的异构推测框架，通过区分搜索和访问动作的不同特性，实现端到端3.28倍加速，同时保持与完全推理代理相当的准确性。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的深度研究代理在执行长时域信息搜索任务时存在高延迟问题，现有推测框架采用统一策略和严格动作匹配，限制了推理加速和鲁棒性。研究发现搜索和访问动作具有不同的推理和模型能力需求，需要针对性的异构推测方法。

Method: 提出DualSpec异构推测框架：1）基于熵分析发现搜索决策不确定性高，需要显式推理；访问决策熵低，主要依赖模型能力。2）采用轻量级、基于置信度的语义验证器。3）针对不同类型动作实施差异化推测策略。

Result: 在多个模型和基准测试中，DualSpec实现了最高3.28倍的端到端加速，同时保持与完全推理代理相当的准确性。

Conclusion: 通过考虑动作异质性设计的异构推测框架能有效加速深度研究代理，同时保持准确性。搜索和访问动作的不同特性为优化推测策略提供了重要指导。
...
### [162] [OrthoFormer: Instrumental Variable Estimation in Transformer Hidden States via Neural Control Functions](https://arxiv.org/abs/2603.07431)
*Charles Luo*

Main category: cs.LG

TL;DR: OrthoFormer：一种基于因果推理的Transformer架构，通过神经控制函数将工具变量估计嵌入Transformer块，解决传统Transformer因相关学习导致的分布外泛化失败问题。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer架构存在根本性限制：它们通过相关学习捕获由潜在混杂因素引起的虚假关联，而非不变的因果机制。这导致在分布外情况下出现灾难性失败，因为模型混淆了静态背景因素（内在身份、风格、上下文）与动态因果流（状态演化、机制）。

Method: 提出OrthoFormer架构，基于四个理论支柱：1) 结构方向性（时间箭头强制）；2) 表示正交性（潜在噪声分离）；3) 因果稀疏性（马尔可夫毯近似）；4) 端到端一致性（梯度解耦的阶段分离）。通过神经控制函数将工具变量估计直接嵌入Transformer块。

Result: 理论证明：OrthoFormer的偏差严格小于OLS（普通最小二乘法），对于任何有效的工具变量滞后，残差偏差以几何速率O(ρ^k)衰减。识别了自工具化中的偏差-方差-外生性三难困境，以及神经禁止回归现象（移除梯度解耦会改善预测损失但破坏因果有效性）。实验验证了所有理论预测。

Conclusion: OrthoFormer代表了从相关序列建模到因果序列建模的范式转变，对鲁棒性、可解释性和在分布偏移下的可靠决策制定具有重要意义。
...
### [163] [Data Agent: Learning to Select Data via End-to-End Dynamic Optimization](https://arxiv.org/abs/2603.07433)
*Suorong Yang,Fangjian Su,Hai Gan,Ziqi Ye,Jie Li,Baile Xu,Furao Shen,Soujanya Poria*

Main category: cs.LG

TL;DR: Data Agent：一个端到端的动态数据选择框架，将数据选择建模为训练感知的顺序决策问题，通过复合奖励信号学习样本选择策略，在多种数据集和架构上实现50%以上的训练加速且性能无损。


<details>
  <summary>Details</summary>
Motivation: 现有动态数据选择方法通常依赖任务特定的手工指标或静态/快照标准来估计样本重要性，这限制了跨学习范式的可扩展性，且难以捕捉训练过程中数据效用的动态变化。

Method: 提出Data Agent框架，将数据选择建模为训练感知的顺序决策问题。代理学习与模型优化协同演化的样本级选择策略，通过复合奖励信号指导，该信号整合了基于损失的难度和基于置信度的不确定性信号，并包含无需调参的自适应权重机制来平衡这些信号。

Result: 在广泛的实验验证中，Data Agent在多种数据集和架构上一致地加速训练同时保持或提升性能，如在ImageNet-1k和MMLU上减少超过50%的训练成本且性能无损。其数据集无关的公式化和模块化奖励设计使其成为跨任务和场景的即插即用解决方案。

Conclusion: Data Agent通过将数据选择建模为顺序决策问题，解决了现有方法的局限性，实现了高效、可扩展的动态数据选择，在真实世界场景中具有广泛应用潜力，特别是在处理噪声数据集等方面表现出色。
...
### [164] [Discrete Tokenization Unlocks Transformers for Calibrated Tabular Forecasting](https://arxiv.org/abs/2603.07448)
*Yael S. Elmatad*

Main category: cs.LG

TL;DR: 本文提出了一种基于注意力机制的表格数据建模方法，通过简单的离散化词汇表结合高斯平滑，在表格基准测试中超越了梯度提升方法。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer在序列数据上表现出色，但在表格数据上梯度提升方法仍然占主导地位。本文旨在探索即使使用基本的tokenization技术，注意力机制是否也能在表格特征上发挥强大作用。

Method: 1. 使用刻意简化的离散化词汇表进行tokenization；2. 结合高斯平滑处理标签；3. 离散化环境上下文，同时用自适应高斯平滑标签，生成校准的概率密度函数；4. 保持序列顺序和时间差token。

Result: 在60万个实体（500万训练样本）上：1. 比调优的XGBoost提升10.8%（中位数MAE：35.94s vs 40.31s）；2. 自适应sigma检查点达到KS=0.0045；3. 消融实验显示：丢失序列顺序损失约2.0%，丢弃时间差token损失约1.8%。

Conclusion: 即使使用基本的tokenization，注意力机制也能在表格数据上超越梯度提升方法。架构设计很重要：序列顺序和时间差token对性能有显著影响，自适应高斯平滑能有效校准概率分布。
...
### [165] [Contact-Guided 3D Genome Structure Generation of E. coli via Diffusion Transformers](https://arxiv.org/abs/2603.07472)
*Mingxin Zhang,Xiaofeng Dai,Yu Yao,Ziqi Yin*

Main category: cs.LG

TL;DR: 提出基于条件扩散-Transformer框架，从Hi-C接触图生成三维大肠杆菌基因组构象集合，而非单一结构


<details>
  <summary>Details</summary>
Motivation: 传统基因组重建方法通常产生单一确定性结构，但实际基因组具有构象异质性，需要生成与Hi-C数据一致的构象集合

Method: 使用条件扩散-Transformer框架，结合变分自编码器进行潜在扩散，通过Transformer编码器和交叉注意力注入Hi-C信息，采用流匹配目标训练

Result: 生成的结构能重现输入Hi-C的距离衰减和结构相关性指标，同时保持显著的构象多样性

Conclusion: 扩散生成模型在集合水平的三维基因组重建中有效，能产生与Hi-C数据一致的异质构象集合
...
### [166] [Interpretable-by-Design Transformers via Architectural Stream Independence](https://arxiv.org/abs/2603.07482)
*Clayton Kerce,Alexis Fox*

Main category: cs.LG

TL;DR: 本文提出Late Fusion Architecture (LFA)，通过架构层面的流独立性（token流和语义流分离处理）实现可解释性设计，相比标准transformer能保持符号头可解释性，并展示功能模块化特性。


<details>
  <summary>Details</summary>
Motivation: 尽管transformer模型性能强大，但其内部决策过程不透明。研究者希望探索是否可以通过架构约束实现"设计即解释"——通过架构层面的流独立性来强制模型保持可解释性。

Method: 提出Late Fusion Architecture (LFA)，采用流独立性原则：将token流（携带符号结构）和上下文语义流分离处理，保持独立可观察，直到输出阶段才进行融合。引入Token-Position Dependence Score (PDS)来量化可解释性。

Result: LFA在所有最终层都保持可解释的符号头，而标准transformer在第3层（共6层）就出现溶解现象（PDS_max分别为0.276 vs 0.058）。干预实验显示功能模块化：抑制LFA的recency头仅造成轻微语义损伤（Cohen's d = -0.158），而基线模型出现灾难性纠缠（d = -0.672）。LFA平均稳定性达42%，远高于基线（19%和11%）。

Conclusion: 通过防止过早的纠缠，架构独立性引导模型走向语义理解而非位置启发式方法，确立了可解释性作为可通过结构约束强制执行的架构设计标准，而非事后分析。
...
### [167] [Enhanced Random Subspace Local Projections for High-Dimensional Time Series Analysis](https://arxiv.org/abs/2603.07500)
*Eman Khalid,Moimma Ali Khan,Zarmeena Ali,Abdullah Illyas,Muhammad Usman,Saoud Ahmed*

Main category: cs.LG

TL;DR: 提出增强型随机子空间局部投影（RSLP）框架，用于高维时间序列的稳健脉冲响应估计，通过加权子空间聚合、类别感知采样、自适应子空间大小选择和自举推断，显著提升估计稳定性和推断可靠性。


<details>
  <summary>Details</summary>
Motivation: 高维时间序列预测中，当预测变量数量超过观测值时，传统局部投影方法会出现严重过拟合，导致估计不稳定不可靠，特别是在存在数百个相关预测变量的情况下。

Method: 提出增强型随机子空间局部投影（RSLP）框架，包含四个关键改进：加权子空间聚合、类别感知子空间采样、自适应子空间大小选择，以及针对依赖数据的自举推断程序。

Result: 在合成数据、宏观经济指标和FRED-MD数据集上的实验表明：自适应子空间大小选择在h≥3的预测范围内将估计变异性降低33%；自举推断程序在极高维设置（FRED-MD的126个预测变量）中产生保守置信区间，在政策相关范围内窄14%且保持适当覆盖率。

Conclusion: 该框架为实践者提供了将丰富信息集纳入脉冲响应分析的原则性方法，避免了传统高维方法的不稳定性，显著提升了估计稳定性和推断可靠性。
...
### [168] [A Unified Framework for Knowledge Transfer in Bidirectional Model Scaling](https://arxiv.org/abs/2603.07506)
*Jianlu Shen,Fu Feng,Jiaze Xu,Yucheng Xie,Jiaqi Lv,Xin Geng*

Main category: cs.LG

TL;DR: BoT提出首个尺寸无关的双向知识迁移框架，将不同尺寸模型间的权重视为连续信号，通过离散小波变换统一处理从小到大的上采样和从大到小的下采样问题。


<details>
  <summary>Details</summary>
Motivation: 现有参数空间方法将小到大（S2L）和大到小（L2S）的模型缩放视为两个独立、不兼容的问题，分别专注于参数合成和选择，缺乏统一的双向框架。

Method: 将模型权重视为连续信号，不同尺寸模型代表可迁移知识的不同离散化。利用离散小波变换（DWT）及其逆变换（IDWT），将S2L和L2S缩放分别视为上采样和下采样信号处理操作，通过分解级别作为动态缩放因子实现参数无关的高效迁移。

Result: 在DeiT、BERT和GPT上的实验显示，S2L可节省高达67.1%的预训练FLOPs，L2S可节省52.8%，在GLUE和SQuAD等基准测试中达到最先进性能。

Conclusion: BoT框架首次统一了不同尺寸模型间的双向知识迁移，通过信号处理视角和小波变换实现了参数无关、计算高效且性能优越的模型缩放方案。
...
### [169] [Online Continual Learning for Anomaly Detection in IoT under Data Distribution Shifts](https://arxiv.org/abs/2603.07507)
...
Method: 提出OCLADS框架，包含两个关键机制：1) 设备端的智能样本选择机制用于数据传输；2) 边缘服务器的分布偏移检测机制用于模型更新

Result: 实验结果显示，OCLADS在TinyML环境下实现了高推理精度，同时相比基准方案显著减少了模型更新次数

Conclusion: OCLADS框架能够有效应对物联网非平稳环境中的分布偏移问题，通过智能通信机制实现高效模型更新，平衡了准确性和资源消耗
...
### [170] [A Unified View of Drifting and Score-Based Models](https://arxiv.org/abs/2603.07514)
*Chieh-Hsin Lai,Bac Nguyen,Naoki Murata,Yuhta Takida,Toshimitsu Uesaka,Yuki Mitsufuji,Stefano Ermon,Molei Tao*

Main category: cs.LG

TL;DR: 漂移模型通过优化数据分布与模型分布之间的核诱导均值漂移差异来训练一步生成器，本文揭示了其与扩散模型中分数匹配原理的精确关系。


<details>
  <summary>Details</summary>
Motivation: 漂移模型在实践中默认使用拉普拉斯核，但其与扩散模型中核心的分数匹配原理之间的关系尚不明确。本文旨在精确揭示漂移模型与分数匹配之间的理论联系。

Method: 通过Tweedie公式建立高斯核漂移与高斯平滑分布分数匹配的等价性；推导一般径向核的精确分解；对拉普拉斯核在低温和高维情况下建立严格的误差界。

Result: 证明高斯核漂移恰好是平滑分布上的分数匹配目标；揭示漂移与DMD都使用分数失配传输方向但实现方式不同；对拉普拉斯核证明在特定条件下漂移是分数匹配的准确代理。

Conclusion: 漂移模型本质上是平滑分布上的分数匹配方法，这统一了漂移与扩散模型的理论基础，并阐明了其与DMD的关系，为理解这些生成模型提供了新的理论视角。
...
### [171] [Reinforcement learning-based dynamic cleaning scheduling framework for solar energy system](https://arxiv.org/abs/2603.07518)
*Heungjo An*

Main category: cs.LG

TL;DR: 本文提出基于强化学习的框架，用于优化沙漠地区光伏面板的清洁调度，PPO算法在阿布扎比案例中比SAC和传统方法节省13%成本


<details>
  <summary>Details</summary>
Motivation: 在干旱地区，光伏面板因灰尘等污染物导致发电效率下降，需要优化清洁调度以提升可再生能源系统的可持续性和效率

Method: 采用强化学习框架，使用PPO和SAC算法，根据不确定的环境条件动态调整光伏面板的清洁间隔

Result: 在阿联酋阿布扎比的案例研究中，PPO算法优于SAC和传统仿真优化方法，通过动态响应天气不确定性实现了高达13%的成本节约

Conclusion: 强化学习驱动的自主决策在优化可再生能源系统维护操作方面具有潜力，未来研究需增强模型的泛化能力并考虑更多因素以适应不同地区
...
### [172] [One-for-All Model Initialization with Frequency-Domain Knowledge](https://arxiv.org/abs/2603.07523)
*Jianlu Shen,Fu Feng,Yucheng Xie,Jiaqi Lv,Xin Geng*

Main category: cs.LG

TL;DR: FRONT提出了一种基于频率域的知识迁移框架，通过离散余弦变换提取预训练模型权重中的低频"learngene"（学习基因），实现无需训练的知识迁移到不同规模的下游模型。


<details>
  <summary>Details</summary>
Motivation: 现有知识迁移方法存在局限性：参数选择方法无法捕捉知识间的相互依赖结构，而参数预测方法需要大量网络集合且不实用。预训练模型的知识与单一架构紧密耦合，难以灵活地跨不同规模模型重用。

Method: 使用离散余弦变换(DCT)提取预训练模型权重中的低频成分作为"learngene"，通过简单的截断或填充操作即可适配任意规模的下游模型初始化。还提出了可选的低成本精炼过程，引入频谱正则化器进一步提升learngene的可迁移性。

Result: FRONT在视觉任务中加速收敛达15倍，在语言任务中平均减少40.5%的训练FLOPs，实现了最先进的性能表现。

Conclusion: 模型的基础任务无关知识（learngene）编码在权重的低频成分中，通过频率域分析可以高效提取和迁移这种知识，为跨模型规模的知识重用提供了有效解决方案。
...
### [173] [Neural Dynamics-Informed Pre-trained Framework for Personalized Brain Functional Network Construction](https://arxiv.org/abs/2603.07524)
*Hongjie Jiang,Yifei Tang,Shuqiang Wang*

Main category: cs.LG

TL;DR: 提出基于神经动力学的预训练框架，用于构建个性化脑功能网络，在异质场景中优于传统方法
...
Method: 提出神经动力学启发的预训练框架，提取异质场景中的个性化神经活动模式表征，利用这些表征指导脑区分割和神经活动相关性估计

Result: 在18个数据集上进行系统评估，包括虚拟神经调控和异常神经回路识别等任务，实验结果表明该框架在异质场景中取得优越性能
...
### [174] [Generative prediction of laser-induced rocket ignition with dynamic latent space representations](https://arxiv.org/abs/2603.07525)
*Tony Zahtila,Ettore Saetta,Murray Cutforth,Davy Brouzet,Diego Rossinelli,Gianluca Iaccarino*

Main category: cs.LG

TL;DR: 提出结合卷积自编码器和神经常微分方程的数据驱动代理模型，用于快速预测激光点火火箭发动机的复杂多物理场模拟，将预测成本降低数个数量级。


<details>
  <summary>Details</summary>
Motivation: 激光点火火箭发动机的尺度解析模拟计算成本极高，涉及湍流混合、激光能量沉积和高速火焰生长等多物理场耦合，且设计空间巨大。传统模拟方法无法满足快速探索和不确定性量化的需求。

Method: 采用卷积自编码器将高维流场压缩到低维潜在空间，然后使用神经常微分方程学习系统的时域动力学。训练完成后，模型可以从初始条件和操作输入快速生成时空预测。

Result: 该方法将点火试验的预测成本降低了数个数量级，实现了输入参数空间的高效探索。模型输出的时空场预测具有更好的物理可解释性。

Conclusion: 该框架标志着向激光点火火箭燃烧室实时数字孪生迈出了重要一步，代表了复杂系统背景下代理建模的新范式。
...
### [175] [Obliviator Reveals the Cost of Nonlinear Guardedness in Concept Erasure](https://arxiv.org/abs/2603.07529)
*Ramin Akbari,Milad Afshari,Vishnu Naresh Boddeti*

Main category: cs.LG

TL;DR: Obliviator是一种后处理的概念擦除方法，通过迭代优化核函数组合来完全捕获非线性统计依赖，保护表示免受非线性攻击者的侵害，同时量化擦除成本并揭示效用与擦除之间的动态关系。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法无法完全捕获学习表示与不需要属性之间的复杂非线性统计依赖关系，导致对非线性攻击者仍然脆弱。同时，虽然预期存在效用与擦除之间的权衡，但擦除过程中的成本动态尚未被研究。

Method: Obliviator是一种后处理擦除方法，从函数视角制定擦除问题，形成涉及核函数组合的优化问题。采用迭代方法逐步变换特征空间，而不是一次性求解，以实现更保留效用的擦除。

Result: Obliviator能够保护不需要属性免受非线性攻击者的侵害，其效用-擦除权衡曲线优于基线方法。该方法显示出强大的泛化能力：当应用于更强大模型学习到的更好解耦表示时，其擦除变得更加保留效用。

Conclusion: Obliviator通过完全捕获非线性统计依赖，提供了对非线性攻击者的保护，同时通过迭代方法量化了擦除成本，揭示了属性保护与效用保留之间的动态关系，为概念擦除提供了更全面的解决方案。
...
### [176] [ECG Classification on PTB-XL: A Data-Centric Approach with Simplified CNN-VAE](https://arxiv.org/abs/2603.07558)
*Naqcho Ali Mehdi,Amir Ali*

Main category: cs.LG

TL;DR: 本文提出了一种结合CNN和VAE的简化架构，通过精心数据预处理和类别平衡，在PTB XL心电图数据集上实现了与复杂模型相当的分类性能，同时大幅降低了模型复杂度。


<details>
  <summary>Details</summary>
Motivation: 当前心电图自动分类方法过度依赖复杂的深度神经网络架构，而本文旨在证明通过数据中心的机器学习实践（如数据预处理和类别平衡），使用简化模型也能获得竞争性性能。

Method: 采用简化的卷积神经网络与变分自编码器（CNN-VAE）架构，结合系统的数据预处理和类别平衡策略，在PTB XL数据集上进行训练和评估。

Result: 在PTB XL数据集上实现了87.01%的二元分类准确率和0.7454的加权F1分数（涵盖CD、HYP、MI、NORM、STTC五个诊断类别），仅使用197,093个可训练参数。

Conclusion: 强调数据中心的机器学习实践比架构复杂性更重要，系统预处理和平衡训练策略对医学信号分类至关重要，同时识别了少数类别（特别是肥大症）检测的挑战，为处理不平衡心电图数据集提供了改进方向。
...
### [177] [Constraints Matrix Diffusion based Generative Neural Solver for Vehicle Routing Problems](https://arxiv.org/abs/2603.07568)
*Zhenwei Wang,Tiehua Zhang,Ning Xue,Ender Ozcan,Ling Wang,Ruibin Bai*

Main category: cs.LG

TL;DR: 提出融合神经网络框架，使用离散噪声图扩散模型学习VRP约束，生成约束分配矩阵，增强自回归求解器的特征表示和决策过程，在CVRPlib数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于注意力的神经求解器在异构问题参数分布下鲁棒性有限，在节点表示高度相似或长决策视野任务中性能显著下降，且改进主要局限于固定分布基准。

Method: 提出融合神经网络框架：1) 使用离散噪声图扩散模型学习VRP底层约束，生成约束分配矩阵；2) 将该矩阵自适应集成到自回归求解器的特征表示学习和决策过程中；3) 作为图结构掩码，促进具有全局视野和局部特征整合的解决方案形成。

Result: 在CVRPlib公共数据集的378组合空间（跨越四个不同维度）上进行全面实验评估，证明所提融合模型能有效捕获和利用问题约束，在多个基准数据集上达到最先进性能。

Conclusion: 该融合框架通过离散噪声图扩散模型学习约束并集成到自回归求解器中，解决了现有神经求解器在异构分布、相似节点和长决策视野下的鲁棒性问题，为VRP求解提供了更强大的神经方法。
...
### [178] [TS-MLLM: A Multi-Modal Large Language Model-based Framework for Industrial Time-Series Big Data Analysis](https://arxiv.org/abs/2603.07572)
...
<details>
  <summary>Details</summary>
Motivation: 工业时间序列大数据分析对设备预测性健康管理至关重要。现有LLM方法通常专注于单模态适配，未能充分利用时域信号、频域视觉表示和文本知识信息之间的互补性。

Method: 提出TS-MLLM框架：1) 工业时间序列补丁建模分支捕获长程时域动态；2) 频谱感知视觉语言模型适配机制整合频域模式和语义上下文；3) 时域中心多模态注意力融合机制以时域特征为查询主动检索相关视觉和文本线索。

Result: 在多个工业基准测试上的广泛实验表明，TS-MLLM显著优于现有最先进方法，特别是在少样本和复杂场景下，验证了框架在工业时间序列预测方面的优越鲁棒性、效率和泛化能力。

Conclusion: TS-MLLM通过有效整合时域、频域和文本多模态信息，为工业时间序列分析提供了一个强大且统一的框架，在预测性健康管理应用中展现出卓越性能。
...
### [179] [TT-Sparse: Learning Sparse Rule Models with Differentiable Truth Tables](https://arxiv.org/abs/2603.07606)
*Hans Farrell Soegeng,Sarthak Ketanbhai Modi,Thomas Peyrin*

Main category: cs.LG

TL;DR: TT-Sparse：一种基于可微分真值表的神经网络模块，通过软TopK算子学习稀疏连接，可精确提取为紧凑的DNF/CNF布尔规则，在保持高性能的同时实现全局可解释性。


<details>
  <summary>Details</summary>
Motivation: 在高风险领域，机器学习需要可解释性以确保问责制、透明度和信任。虽然基于规则的方法提供全局精确可解释性，但现有方法难以同时实现高预测性能和低复杂度的人类可理解规则。

Method: 提出TT-Sparse神经网络模块，使用可微分真值表作为节点，通过新的软TopK算子（带直通估计）学习离散的、受基数约束的特征选择，实现端到端可微分训练。前向传播保持稀疏性，支持高效计算和精确符号规则提取。

Result: 在28个数据集（包括二分类、多分类和回归任务）上的实验表明，学习的稀疏规则相比现有最先进方法具有更优的预测性能和更低的复杂度。

Conclusion: TT-Sparse成功解决了同时实现高预测性能和低复杂度可解释规则的挑战，通过可微分真值表和软TopK算子实现了精确的规则提取，为高风险领域的可解释机器学习提供了有效解决方案。
...
### [180] [Compression as Adaptation: Implicit Visual Representation with Diffusion Foundation Models](https://arxiv.org/abs/2603.07615)
*Jiajun He,Zongyu Guo,Zhaoyang Jia,Xiaoyi Zhang,Jiahao Li,Xiao Li,Bin Li,José Miguel Hernández-Lobato,Yan Lu*

Main category: cs.LG

TL;DR: 提出一种新的视觉表示框架，将视觉信号编码为函数，通过冻结视觉生成模型的低秩适配器参数化，实现紧凑存储和感知视频压缩。


<details>
  <summary>Details</summary>
Motivation: 现代视觉生成模型通过大规模训练获得丰富的视觉知识，但现有的视觉表示（如像素、潜在变量或标记）仍然外在于模型，无法直接利用这些知识进行紧凑存储或重用。

Method: 引入新的视觉表示框架，将信号编码为函数，该函数由附加到冻结视觉生成模型的低秩适配器参数化。这种隐式表示可以进一步哈希为单个紧凑向量。

Result: 在极低比特率下实现强大的感知视频压缩（如81帧视频）。函数性质支持推理时缩放和控制，允许对压缩性能进行额外优化。

Conclusion: 隐式表示直接作为生成过程的函数，提出了一个统一视觉压缩和生成的框架。
...
### [181] [Helix: Evolutionary Reinforcement Learning for Open-Ended Scientific Problem Solving](https://arxiv.org/abs/2603.07642)
*Chang Su,Zhongkai Hao,Zhizhou Zhang,Zeyu Xia,Youjia Wu,Hang Su,Jun Zhu*

Main category: cs.LG

TL;DR: HELIX是一个结合分层进化强化学习和上下文经验的大语言模型推理框架，在圆填充任务和机器学习基准测试中取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理复杂科学问题时存在探索效率有限和泛化能力差的问题，需要一种能够在大规模灵活解空间中高效探索的框架。

Method: HELIX框架包含两个关键创新：1）通过上下文学习构建多样且高质量的候选解池以扩大探索范围；2）使用强化学习进行迭代策略优化以逐步提升解的质量。

Result: 在圆填充任务中，HELIX仅使用14B模型就实现了2.63598308的半径和，达到最先进水平。在标准机器学习基准测试中，HELIX超越了GPT-4o，在Adult和Bank Marketing数据集上平均F1分数提升了5.95分。

Conclusion: HELIX通过结合上下文学习和强化学习，有效解决了复杂科学问题中的探索效率和泛化能力问题，为LLM在科学推理任务中的应用提供了新框架。
...
### [182] [Partial Differential Equations in the Age of Machine Learning: A Critical Synthesis of Classical, Machine Learning, and Hybrid Methods](https://arxiv.org/abs/2603.07655)
*Mohammad Nooraiepour,Jakub Wiktor Both,Teeratorn Kadeethum,Saeid Sadeghnejad*

Main category: cs.LG

TL;DR: 该论文批判性评估了求解偏微分方程的两大范式：经典数值方法和机器学习方法，通过统一框架分析其计算挑战、认识论差异，并提出混合设计原则。


<details>
  <summary>Details</summary>
Motivation: 偏微分方程在科学各尺度中至关重要，但其计算求解仍是现代科学的重大挑战。需要系统评估经典数值方法和机器学习方法这两种成熟但认识论不同的范式，理解各自的优势和局限，为负责任的方法选择提供指导。

Method: 采用统一的评估框架，围绕六个基本计算挑战分析两种范式。对经典方法评估其结构保持特性、严格收敛理论和可扩展求解器设计；对机器学习方法按物理知识融入程度进行分类，并进行同等批判性评估。提出混合设计原则，包括解决结构继承问题的框架和误差预算分解方法。

Result: 识别了经典方法在高维和几何复杂场景中的持续局限，机器学习方法的归纳特性使其准确性依赖于训练分布的统计接近度。发现了两种范式之间的三个真正互补性，并建立了混合设计原则，包括结构继承框架和误差分解方法。

Conclusion: 经典方法是演绎的（误差可由PDE结构和离散化参数推导），而机器学习方法是归纳的（准确性取决于与训练分布的统计接近度）。这种认识论差异是负责任方法选择的主要标准。混合设计需要解决结构继承问题，并评估新兴前沿技术（如基础模型、可微分编程、量子算法等）是否受基本约束还是工程进展限制。
...
### [183] [Beyond Surrogates: A Quantitative Analysis for Inter-Metric Relationships](https://arxiv.org/abs/2603.07671)
*Yuanhao Pu,Defu Lian,Enhong Chen*

Main category: cs.LG

TL;DR: 提出统一理论框架量化不同评估指标间的关系，解决离线验证指标与在线性能不匹配问题


<details>
  <summary>Details</summary>
Motivation: 工业应用中常出现"指标不匹配"问题，即离线验证指标的提升无法转化为在线性能改进。现有研究主要关注代理损失与评估指标的一致性，但不同评估指标间的直接关系研究不足。

Method: 提出统一理论框架，将指标分类以促进不同数学形式的比较分析，通过贝叶斯最优集和遗憾转移来探究指标间关系，识别遗憾转移中的结构不对称性。

Result: 该框架为设计评估系统提供了新视角，能够从理论上保证离线改进与在线目标的一致性。

Conclusion: 通过量化评估指标间的关系，可以解决工业应用中常见的离线与在线性能不匹配问题，设计出理论上保证一致的评估系统。
...
### [184] [Global Convergence of Average Reward Constrained MDPs with Neural Critic and General Policy Parameterization](https://arxiv.org/abs/2603.07698)
*Anirudh Satheesh,Pankaj Kumar Barman,Washim Uddin Mondal,Vaneet Aggarwal*

Main category: cs.LG

TL;DR: 提出一种基于神经正切核理论的原始-对偶自然行动者-评论家算法，用于带约束的马尔可夫决策过程，首次为通用策略和多层神经评论家提供全局收敛保证。


<details>
  <summary>Details</summary>
Motivation: 现有约束强化学习的理论分析主要依赖表格策略或线性评论家，限制了在高维和连续控制问题中的应用。需要为通用策略参数化和多层神经网络评论家建立理论保证。

Method: 提出原始-对偶自然行动者-评论家算法，将神经评论家估计与自然策略梯度更新相结合，利用神经正切核理论控制马尔可夫采样下的函数逼近误差，无需混合时间预言机。

Result: 建立了全局收敛性和累积约束违反率 $\tilde{\mathcal{O}}(T^{-1/4})$ 的保证，该结果受策略和评论家类引起的逼近误差限制。这是首次为带通用策略和多层神经评论家的CMDP提供此类保证。

Conclusion: 该工作将行动者-评论家方法的理论基础扩展到线性评论家体系之外，为高维和连续控制问题中的约束强化学习提供了理论支持。
...
### [185] [Step-Size Decay and Structural Stagnation in Greedy Sparse Learning](https://arxiv.org/abs/2603.07703)
*Pablo M. Berná*

Main category: cs.LG

TL;DR: 研究Power-Relaxed Greedy算法在稀疏学习中的步长设计问题，发现过衰减步长计划（m^{-α}, α>1）会导致结构停滞，即使在低维稀疏设置中也会收敛失败。


<details>
  <summary>Details</summary>
Motivation: 贪婪算法在稀疏逼近和分阶段学习方法（如匹配追踪和提升）中至关重要。已知Power-Relaxed Greedy算法在步长为m^{-α}且α>1时，在一般希尔伯特空间中可能无法收敛。本研究从稀疏学习的角度重新审视这一现象。

Method: 研究具有受控特征相干性的可实现回归问题，推导残差范数的显式下界，证明过衰减步长计划即使在低维稀疏设置中也会导致结构停滞。通过数值实验验证理论预测并说明特征相干性的作用。

Result: 理论分析表明过衰减步长计划会诱导结构停滞，数值实验证实了理论预测，并阐明了特征相干性在贪婪稀疏学习步长设计中的关键作用。

Conclusion: 研究结果为贪婪稀疏学习中的步长设计提供了重要见解，强调了避免过衰减步长计划的重要性，特别是在考虑特征相干性的情况下。
...
### [186] [Reverse Distillation: Consistently Scaling Protein Language Model Representations](https://arxiv.org/abs/2603.07710)
*Darius Catrina,Christian Bepler,Samuel Sledzieski,Rohit Singh*

Main category: cs.LG

TL;DR: 提出Reverse Distillation框架，通过将大蛋白质语言模型表示分解为受小模型指导的正交子空间，解决PLM缩放不佳问题，确保大模型始终优于小模型。


<details>
  <summary>Details</summary>
Motivation: 蛋白质语言模型（PLMs）的缩放规律与自然语言处理和计算机视觉不同：在许多任务中，同一家族内的模型性能会达到平台期甚至下降，中型模型常常比家族中最大的模型表现更好。需要解决PLM缩放不佳的问题。

Method: 提出Reverse Distillation（反向蒸馏）框架，将大PLM表示分解为受同家族小模型指导的正交子空间。生成的嵌入具有嵌套的Matryoshka结构：大模型嵌入的前k维恰好是小模型的表示。该方法隔离共享特征并正交提取大模型的额外贡献。

Result: 在ProteinGym基准测试中，反向蒸馏的ESM-2变体在相同嵌入维度下优于各自基线，150亿参数的反向蒸馏模型实现了最强性能。框架可推广到任何存在缩放挑战的模型家族。

Conclusion: Reverse Distillation提供了一种原则性方法来解决PLM缩放问题，确保模型性能随规模增加而单调提升，为蛋白质建模和其他存在类似缩放挑战的领域提供了通用解决方案。
...
TL;DR: FedShift：一种新颖的两阶段"隐藏与发现"分布式对抗攻击方法，针对联邦图学习，通过注入可学习的隐藏"移位器"实现高效、隐蔽的攻击，能有效规避主流防御算法。
...
Method: 提出两阶段攻击框架：1）训练前阶段：向部分训练数据注入可学习的隐藏"移位器"，将中毒图表示推向目标类决策边界但不越过边界，确保训练隐蔽性；2）训练后阶段：利用全局模型信息，以隐藏移位器为优化起点，高效找到对抗扰动，最后聚合多个恶意客户端的扰动形成最终攻击样本。

Result: 在六个大规模数据集上的实验表明，该方法相比现有先进攻击方法具有最高的攻击效果，能有效规避3种主流鲁棒联邦学习防御算法，时间成本降低超过90%。

Conclusion: FedShift方法在隐蔽性、鲁棒性和效率方面表现卓越，为联邦图学习的安全研究提供了新的攻击视角和基准。
...
### [188] [Uncertainty-Gated Generative Modeling](https://arxiv.org/abs/2603.07753)
*Xingrui Gu,Haixi Zhang*

Main category: cs.LG

TL;DR: 提出UGGM框架，将不确定性作为控制信号来门控表示、传播和生成，显著提升金融时间序列预测的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 金融时间序列预测存在高风险，传统点预测模型在制度转换和冲击下容易过度自信且危险，需要更好的不确定性建模方法。

Method: 提出不确定性门控生成建模(UGGM)，通过门控重参数化控制表示，通过相似性和置信度路由控制传播，通过不确定性控制预测分布控制生成，结合不确定性驱动的正则化和校准。

Result: 在WIAE-GPF上实例化的UG-WIAE-GPF显著改善风险敏感预测，在NYISO数据集上MSE减少63.5%(0.3508→0.1281)，在冲击区间下鲁棒性提升(mSE: 0.2739→0.1748)。

Conclusion: UGGM框架通过将不确定性作为内部控制信号，有效提升了金融时间序列预测的准确性和鲁棒性，为高风险预测问题提供了新解决方案。
...
### [189] [Using GPUs And LLMs Can Be Satisfying for Nonlinear Real Arithmetic Problems](https://arxiv.org/abs/2603.07764)
*Christopher Brix,Julia Walczak,Nils Lommen,Thomas Noll*

Main category: cs.LG

TL;DR: 本文提出GANRA，一种结合LLMs和GPU加速的高效SMT求解器，用于解决非线性实数算术问题，在基准测试中性能显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决非线性实数算术（NRA）问题计算复杂度高，现有基于梯度下降的方法有改进空间，需要更高效的求解技术。

Method: 扩展先前基于梯度下降的思想，结合大型语言模型（LLMs）和GPU加速技术，开发了新型SMT求解器GANRA。

Result: 在两种NRA基准测试中，GANRA性能显著优于现有技术：在Sturm-MBO基准上，能在不到1/20的运行时间内证明超过5倍实例的可满足性。

Conclusion: 结合LLMs和GPU加速的GANRA求解器为非线性实数算术问题提供了高效解决方案，大幅提升了求解性能。
...
### [190] [Breaking Training Bottlenecks: Effective and Stable Reinforcement Learning for Coding Models](https://arxiv.org/abs/2603.07777)
*Zongqian Li,Shaohan Huang,Zewen Chi,Yixuan Su,Lexin Zhou,Li Dong,Nigel Collier,Furu Wei*

Main category: cs.LG

TL;DR: MicroCoder-GRPO提出改进的Group Relative Policy Optimization方法，通过条件截断掩码、多样性温度选择和移除KL损失等创新，在代码生成任务上取得显著性能提升，并发布更挑战的训练数据集和评估框架。
...
Method: 提出MicroCoder-GRPO改进方法：1) 条件截断掩码以改善长输出潜力同时保持训练稳定性；2) 多样性决定温度选择以维持和鼓励输出多样性；3) 移除KL损失并采用高裁剪比率以促进解决方案多样性。

Result: 在LiveCodeBench v6上相对强基线提升达17.6%，在扩展上下文评估下增益更显著；发布MicroCoder-Dataset训练语料，在300训练步内获得主流数据集3倍性能增益；发布MicroCoder-Evaluator评估框架，准确率提升约25%，执行速度加快约40%。

Conclusion: 通过30多个对照实验的全面分析，揭示了7个主要方面的34个训练洞见，证明适当训练的模型可以达到与更大模型相竞争的性能。
...
### [191] [ProgAgent:A Continual RL Agent with Progress-Aware Rewards](https://arxiv.org/abs/2603.07784)
*Jinzhou Tan,Gabriel Adineera,Jinoh Kim*

Main category: cs.LG

TL;DR: ProgAgent是一个持续强化学习代理，通过从无标签专家视频中学习任务进度感知的奖励，结合对抗性正则化和JAX原生架构，在持续学习中显著减少遗忘并提升学习速度。
...
Method: 1) 从无标签专家视频中通过感知模型估计任务进度，学习密集的形奖励函数；2) 引入对抗性推回精炼来正则化奖励模型，防止对非专家轨迹的过度自信预测；3) 构建JIT编译的JAX原生系统架构，支持大规模并行rollout和完全可微分更新；4) 将PPO与coreset回放和突触智能结合，形成统一的稳定性-可塑性平衡目标。

Result: 在ContinualBench和Meta-World基准测试中，ProgAgent显著减少遗忘，提升学习速度，在视觉奖励学习（优于Rank2Reward、TCN）和持续学习（优于Coreset、SI）方面超越关键基线，甚至超过理想化的完美记忆代理。真实机器人试验验证了其从噪声、少量人类演示中学习复杂操作技能的能力。

Conclusion: ProgAgent通过统一进度感知奖励学习与高性能系统架构，为终身机器人学习提供了有效的解决方案，能够从稀疏演示中稳定学习复杂技能，同时保持持续学习中的稳定性。
...
### [192] [Vision Transformers that Never Stop Learning](https://arxiv.org/abs/2603.07787)
*Caihao Sun,Mingqi Yuan,Shiyuan Wang,Jiayu Chen*

Main category: cs.LG

TL;DR: 本文系统研究了Vision Transformers中的可塑性丧失问题，发现注意力模块不稳定加剧可塑性丧失，前馈网络模块退化更严重，并提出ARROW优化器通过自适应重塑梯度方向来保持可塑性。


<details>
  <summary>Details</summary>
Motivation: 可塑性丧失指模型逐渐无法适应新任务，是持续学习中的基本挑战。虽然在同质神经网络架构中已有广泛研究，但在Vision Transformers等异质注意力模型中其机制仍未充分探索。

Method: 1. 使用捕捉参数多样性和利用率的局部指标进行细粒度诊断；2. 评估多种缓解可塑性丧失的方法；3. 提出ARROW优化器，通过使用在线曲率估计自适应重塑注意力模块的梯度方向来保持可塑性。

Result: 分析发现：堆叠注意力模块表现出加剧可塑性丧失的不稳定性，前馈网络模块退化更严重。参数重新初始化方法无法恢复ViTs的可塑性，而显式调节更新过程的方法更有效。ARROW优化器有效改善可塑性并在新任务上保持更好性能。

Conclusion: ViTs中的可塑性丧失机制与同质架构不同，需要专门方法。ARROW通过几何感知的梯度方向重塑有效保持可塑性，为Vision Transformers的持续学习提供了实用解决方案。
...
### [193] [Neural Precoding in Complex Projective Spaces](https://arxiv.org/abs/2603.07811)
*Zaid Abdullah,Merouane Debbah,Symeon Chatzinotas,Bjorn Ottersten*

Main category: cs.LG

TL;DR: 提出基于复射影空间的深度学习预编码框架，通过消除传统表示中的全局相位冗余，改善多用户MISO系统的预编码性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习预编码方法使用复数信道的实部/虚部或幅度/相位表示，但这些表示无法利用信道与预编码向量内积的全局相位旋转不变性，导致学习效率低下和泛化性能下降。

Method: 提出基于复射影空间的深度学习框架，对无线信道和WMMSE预编码向量进行参数化，消除传统表示中的全局相位冗余。研究了基于实值嵌入和复数超球坐标的两种CPS参数化方法。

Result: 仿真结果表明，在模型复杂度几乎不增加的情况下，该方法在总速率性能和泛化能力方面都有显著提升。

Conclusion: 通过复射影空间参数化消除全局相位冗余，能够使深度学习模型学习到几何对齐且物理上可区分的信道-预编码映射，从而显著改善预编码性能。
...
### [194] [Gradient Iterated Temporal-Difference Learning](https://arxiv.org/abs/2603.07833)
...
<details>
  <summary>Details</summary>
Motivation: 传统半梯度TD方法虽然学习速度快但容易发散，而梯度TD方法虽然稳定但学习速度较慢。迭代TD学习通过并行学习一系列动作值函数来提高速度，但由于其半梯度性质，在跟踪移动目标时可能不稳定。

Method: 修改迭代TD学习算法，计算移动目标的梯度，而不是忽略它们。这种方法被称为梯度迭代时序差分学习，旨在构建一个强大的梯度TD方法，既能保持稳定性，又能与半梯度方法竞争学习速度。

Result: 在包括Atari游戏在内的多个基准测试中，梯度迭代时序差分学习算法展现出与半梯度方法竞争的学习速度，这是之前梯度TD方法工作中从未展示过的结果。

Conclusion: 梯度迭代时序差分学习成功地将梯度TD方法的稳定性与半梯度方法的学习速度相结合，为强化学习中的时序差分方法提供了一个有前景的新方向。
...
### [195] [Guess & Guide: Gradient-Free Zero-Shot Diffusion Guidance](https://arxiv.org/abs/2603.07860)
*Abduragim Shtanchaev,Albina Ilina,Yazid Janati,Arip Asadulaev,Martin Takác,Eric Moulines*

Main category: cs.LG

TL;DR: 提出一种轻量级似然替代方法，消除去噪网络梯度计算需求，大幅降低贝叶斯逆问题推理成本，实现最快且帕累托最优的零样本生成。


<details>
  <summary>Details</summary>
Motivation: 现有基于预训练扩散模型的贝叶斯逆问题方法依赖需要向量-雅可比积的替代似然，每个去噪步骤都产生巨大计算负担，限制了实际应用。

Method: 引入轻量级似然替代方法，无需计算去噪器网络的梯度，避免反向传播开销，能够处理多样化的逆问题。

Result: 实验证实推理成本显著下降，同时在多个任务中取得最佳性能，成为最快且帕累托最优的贝叶斯逆问题解决方法。

Conclusion: 提出的方法消除了去噪网络梯度计算需求，大幅降低计算成本，为基于扩散模型的零样本贝叶斯逆问题提供了高效解决方案。
...
### [196] [Slumbering to Precision: Enhancing Artificial Neural Network Calibration Through Sleep-like Processes](https://arxiv.org/abs/2603.07867)
*Jean Erik Delanois,Aditya Ahuja,Giri P. Krishnan,Maxim Bazhenov*

Main category: cs.LG

TL;DR: 提出SRC方法，通过模拟生物睡眠中的自发回放机制来改善神经网络校准，无需监督重训练即可提升预测概率与真实准确率的匹配度。


<details>
  <summary>Details</summary>
Motivation: 神经网络经常过度自信，预测概率与实际准确率不匹配，这削弱了信任。受生物睡眠和自发回放在记忆与学习中的作用启发，需要一种新的校准方法。

Method: SRC是一种后训练、类似睡眠的阶段，选择性回放内部表征来更新网络权重，改善校准而不需要监督重训练。

Result: SRC与温度缩放等标准方法竞争且互补。SRC与温度缩放结合在AlexNet和VGG19上实现了最佳的Brier分数和熵权衡。

Conclusion: SRC提供了一种从根本上新颖的神经网络校准方法，为实现更可信的置信度估计和缩小人类不确定性处理与现代深度网络之间的差距提供了实用路径。
...
### [197] [Designing probabilistic AI monsoon forecasts to inform agricultural decision-making](https://arxiv.org/abs/2603.07893)
...
TL;DR: 开发决策理论框架和混合预测系统，为异质化农民提供定制化季风开始预报，在印度成功部署并预测了2025年异常干旱期


<details>
  <summary>Details</summary>
Motivation: 数亿农民在天气不确定性下做出高风险决策，现有预报无法为异质化农民提供最优行动建议，需要开发能适应不同农民需求的预报系统

Method: 1) 决策理论框架：考虑农民异质性，不预设最优行动；2) 混合系统：结合AI天气预测模型和"演化农民期望"统计模型，后者使用贝叶斯推断预测季节内首次发生事件的时间变化概率

Result: 混合系统在更长提前期上比其组件或多模型平均更具技能；2025年在印度政府项目中部署，向3800万农民提供次季节季风开始预报，成功预测了当年夏季早期的异常干旱期

Conclusion: 该决策理论框架和混合系统为全球大量脆弱人群开发气候适应工具提供了可行路径，能够为异质化决策者提供有用的定制化预报
...
### [198] [LeJOT-AutoML: LLM-Driven Feature Engineering for Job Execution Time Prediction in Databricks Cost Optimization](https://arxiv.org/abs/2603.07897)
...
Method: 采用LLM代理驱动的AutoML框架，结合检索增强生成和模型上下文协议工具链（日志解析器、元数据查询、只读SQL沙箱），分析作业工件，通过安全门合成和验证特征提取代码，训练/选择预测器。

Result: 在企业Databricks工作负载上生成200多个特征，将特征工程和评估循环从数周缩短到20-30分钟，保持竞争性预测准确性，集成到LeJOT管道中实现19.01%的成本节省。

Conclusion: LeJOT-AutoML通过LLM代理自动化特征工程，显著加速ML生命周期，实现持续模型更新，在云成本优化方面取得显著效果，为作业编排系统提供了有效的自动化解决方案。
...
### [199] [DyQ-VLA: Temporal-Dynamic-Aware Quantization for Embodied Vision-Language-Action Models](https://arxiv.org/abs/2603.07904)
...
TL;DR: DyQ-VLA是一个用于视觉-语言-动作模型的动态量化框架，通过实时运动学代理触发比特宽度切换和动态分配最优比特宽度，在保持99.5%性能的同时减少70%内存占用，实现1.49倍仿真加速和1.43倍真实世界加速。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作模型在具身智能中占主导地位，但受限于推理开销。静态量化方法对VLA模型不够理想，存在两个关键挑战：1）时间动态敏感性 - 固定精度会忽略阶段变化的误差容忍度，浪费资源；2）实时分配 - 识别实时敏感性来指导比特分配的问题尚未解决。

Method: 提出DyQ-VLA动态量化框架：1）敏感性感知切换策略 - 利用实时运动学代理触发比特宽度切换；2）运动学引导模块 - 动态分配最优比特宽度。

Result: DyQ-VLA仅需原始内存占用的30.9%，同时保持99.5%的原始性能，实现1.49倍仿真加速和最高1.43倍真实世界加速。

Conclusion: DyQ-VLA有效解决了VLA模型量化中的时间动态敏感性和实时分配挑战，显著降低了内存占用和推理延迟，同时保持了高性能，为边缘部署提供了高效解决方案。
...
TL;DR: 提出一个AI框架，用于在医疗BI系统中评估SQL聚合查询的隐私风险，通过静态分析防止数据泄露


<details>
  <summary>Details</summary>
Motivation: 医疗机构的多个BI团队因隐私法规限制难以共享患者数据，即使使用聚合指标也可能存在隐私泄露风险，需要自动化检测工具

Method: 开发模块化AI框架：解析SQL查询为AST，提取敏感模式（如细粒度GROUP BY），使用CodeBERT嵌入编码逻辑，结合结构特征，通过XGBoost分类器评估风险分数

Result: 系统能有效识别高风险查询（阈值>0.85），提供可解释的风险标记，支持跨部门指标共享，同时保持合规性和可审计性

Conclusion: 该框架实现了静态、可解释的查询级隐私风险检测，支持预执行保护，促进医疗数据现代化目标，优于依赖运行时数据访问的现有方法
...
### [201] [ELLMob: Event-Driven Human Mobility Generation with Self-Aligned LLM Framework](https://arxiv.org/abs/2603.07946)
*Yusong Wang,Chuang Yang,Jiawei Wang,Xiaohang Xu,Jiayi Xu,Dongyuan Li,Chuan Xiao,Renhe Jiang*

Main category: cs.LG

TL;DR: 本文提出ELLMob框架，首个解决大规模社会事件下偏离轨迹生成的方法，通过构建事件标注数据集和自对齐LLM框架，有效平衡习惯模式与事件约束


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的轨迹生成方法擅长生成常规轨迹，但无法捕捉大规模社会事件（如台风、疫情、奥运会）下的偏离移动模式。这源于两个关键缺口：缺乏事件标注的移动数据集用于设计和评估，以及现有框架无法在轨迹决策中平衡用户习惯模式与事件约束之间的竞争关系

Method: 提出ELLMob自对齐LLM框架：1）构建首个事件标注移动数据集，涵盖台风海贝思、COVID-19和东京2021奥运会三大事件；2）基于模糊痕迹理论，首先提取习惯模式与事件约束之间的竞争理性，然后通过迭代对齐生成既符合习惯基础又响应事件的轨迹

Result: 广泛实验表明，ELLMob在所有事件上都优于现有最先进的基线方法，证明了其有效性。代码和数据集已开源

Conclusion: 该研究填补了事件标注移动数据集的空白，并通过自对齐LLM框架成功解决了大规模社会事件下轨迹生成的挑战，为城市系统研究提供了更全面的轨迹生成工具
...
### [202] [PSTNet: Physically-Structured Turbulence Network](https://arxiv.org/abs/2603.07957)
*Boris Kriuk,Fedor Kriuk*

Main category: cs.LG

TL;DR: PSTNet：一种轻量级物理结构化神经网络，通过嵌入物理约束（Monin-Obukhov理论、Richardson数、惯性子区标度律）实现实时大气湍流强度估计，仅需552个参数，在资源受限的机载系统中替代传统查找表。


<details>
  <summary>Details</summary>
Motivation: 传统谱模型基于气候学平均而非瞬时大气状态，通用ML回归器缺乏对基本物理标度律的保证。在缺乏实时预报基础设施的海洋、极地等区域，可靠的大气湍流强度实时估计仍是挑战。

Method: PSTNet包含四个组件：1) 基于Monin-Obukhov理论的无参数主干；2) Richardson数监督的专家子网络混合机制；3) 局部空气密度比调节的特征线性调制层；4) 强制惯性子区标度律的Kolmogorov输出层。整个模型仅552个可学习参数。

Result: 在340个六自由度制导仿真中验证，涵盖三种飞行器类别和六种操作类别。PSTNet实现平均脱靶距离改善+2.8%，胜率78%，具有统计显著效应。在Cortex-M7微控制器上执行时间小于12秒，存储需求少于2.5kB。

Conclusion: 将领域物理知识编码为架构先验比扩展模型容量更有效且可解释，PSTNet可作为资源受限、安全关键的机载制导系统中传统查找表的可行替代方案。
...
### [203] [\$OneMillion-Bench: How Far are Language Agents from Human Experts?](https://arxiv.org/abs/2603.07980)
...
TL;DR: OneMillion-Bench是一个包含400个专家策划任务的基准测试，涵盖法律、金融、工业、医疗和自然科学领域，旨在评估语言模型在真实专业场景中的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要局限于结构化或考试式任务，无法满足真实世界专业需求。随着语言模型从聊天助手发展为能够进行多步推理和工具使用的长期智能体，需要更贴近实际专业场景的评估标准。

Method: 构建了包含400个专家策划任务的基准测试，涵盖五个专业领域。采用基于评分标准的评估协议，从事实准确性、逻辑一致性、实践可行性和专业合规性四个维度进行评分。要求智能体检索权威来源、解决证据冲突、应用领域特定规则并做出约束决策。

Result: OneMillion-Bench提供了一个统一的测试平台，用于评估智能体在领域密集型场景中的可靠性、专业深度和实践准备度。正确性不仅取决于最终答案，更取决于推理过程。

Conclusion: 该基准测试填补了现有评估体系的空白，能够更有意义地区分不同智能体在真实专业场景中的表现，为评估智能体的专业能力和实践准备度提供了重要工具。
...
### [204] [MJ1: Multimodal Judgment via Grounded Verification](https://arxiv.org/abs/2603.07990)
*Bhavesh Kumar,Dylan Feng,Leonard Tang*

Main category: cs.LG

TL;DR: MJ1是一个通过强化学习训练的多模态裁判，采用结构化验证链和反事实一致性奖励机制，在视觉证据基础上进行判断，显著提升多模态判断准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态裁判模型难以将决策建立在视觉证据基础上，存在位置偏见等问题，需要改进视觉基础验证能力。

Method: 提出MJ1模型，采用结构化基础验证链（观察→声明→验证→评估→评分）和反事实一致性奖励机制，通过强化学习训练惩罚位置偏见。

Result: 即使未经训练，该机制在MMRB2基准测试中提升基础模型准确率：图像编辑+3.8分，多模态推理+1.7分。训练后的MJ1仅用30亿参数在MMRB2上达到77.0%准确率，超越Gemini-3-Pro等更大规模模型。

Conclusion: 基于视觉证据的基础验证和一致性训练能够显著提升多模态判断能力，无需增加模型规模。
...
### [205] [FedMomentum: Preserving LoRA Training Momentum in Federated Fine-Tuning](https://arxiv.org/abs/2603.08014)
*Peishen Yan,Yang Hua,Hao Wang,Jiaru Zhang,Xiaoyu Wu,Tao Song,Haibing Guan*

Main category: cs.LG

TL;DR: FedMomentum：通过SVD实现结构化和动量保持的LoRA聚合，解决联邦微调中LoRA模块聚合时的噪声和动量损失问题


<details>
  <summary>Details</summary>
Motivation: 联邦微调中LoRA模块的简单聚合会引入噪声，而现有的无噪声聚合策略会损害LoRA的结构表达能力，导致训练动量损失，影响收敛速度和性能

Method: 提出FedMomentum框架，在数学正确聚合低秩更新后，使用奇异值分解（SVD）提取主导更新方向，重构相同秩的LoRA模块，保留残差组件以维持语义信息和鲁棒性

Result: 在多个任务上的实验表明，FedMomentum在收敛速度和最终准确率方面持续优于现有最先进方法

Conclusion: FedMomentum通过动量保持的LoRA聚合解决了联邦微调中的关键问题，实现了更好的性能和更快的收敛
...
### [206] [Capacity-Aware Mixture Law Enables Efficient LLM Data Optimization](https://arxiv.org/abs/2603.08022)
*Jingwei Li,Xinran Gu,Jingzhao Zhang*

Main category: cs.LG

TL;DR: 提出CAMEL方法，通过容量感知的混合定律和损失到基准预测定律，高效优化大语言模型的数据混合比例，减少50%优化成本并提升3%下游性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据混合优化方法要么直接在目标模型上进行昂贵的搜索，要么依赖无法很好外推到大型模型的混合缩放定律，需要更高效的计算方法。

Method: 提出CAMEL容量感知混合定律，建模验证损失与模型规模和混合比例的非线性关系；引入损失到基准预测定律；研究固定计算预算在不同模型规模间的分配策略；应用于专家混合模型验证。

Result: 相比现有方法，减少50%的混合优化成本，下游基准性能提升高达3%；成功外推到55B-A1.2B目标模型。

Conclusion: CAMEL方法提供了一种计算高效的数据混合缩放管道，能够准确预测大型模型性能并显著降低优化成本，为大规模语言模型训练提供有效的数据混合优化方案。
...
### [207] [GCGNet: Graph-Consistent Generative Network for Time Series Forecasting with Exogenous Variables](https://arxiv.org/abs/2603.08032)
*Zhengyu Li,Xiangfei Qiu,Yuhan Zhu,Xingjian Wu,Jilin Hu,Chenjuan Guo,Bin Yang*

Main category: cs.LG

TL;DR: GCGNet：一种用于带外生变量时间序列预测的图一致性生成网络，通过变分生成器、图结构对齐器和图精炼器联合建模时空相关性，提升预测鲁棒性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在建模外生变量对时间序列预测的影响时，通常采用两步策略分别处理时间相关性和通道相关性，无法有效捕捉跨时间和通道的联合相关性。同时，现实世界时间序列常受噪声影响，需要鲁棒的相关性建模方法。

Method: 提出GCGNet（图一致性生成网络）：1）使用变分生成器生成粗粒度预测；2）通过图结构对齐器评估生成预测与真实相关性之间的一致性（相关性以图形式表示，对噪声鲁棒）；3）设计图精炼器防止退化并提高预测精度。

Result: 在12个真实世界数据集上的大量实验表明，GCGNet在带外生变量的时间序列预测任务上优于现有最先进的基线方法。

Conclusion: GCGNet通过联合建模时间相关性和通道相关性，并采用鲁棒的图表示方法，有效解决了现有方法在带外生变量时间序列预测中的局限性，实现了更准确和鲁棒的预测性能。
...
TL;DR: SFed-LoRA：一种稳定联邦学习中的LoRA方法，通过理论推导最优缩放因子来解决多客户端聚合时高秩适配器梯度崩溃的问题。


<details>
  <summary>Details</summary>
Motivation: 在联邦学习场景中，LoRA方法存在不稳定性问题。多客户端聚合会引入统计方差，导致使用高秩适配器时出现梯度崩溃。现有缩放因子方法忽略了聚合过程的交互影响。

Method: 提出SFed-LoRA框架，理论分析适配器秩与联邦聚合之间的交互关系，推导出最优缩放因子来有效缓解跨N个客户端累积的聚合误差，不改变原始模型架构或增加推理延迟。

Result: 实验验证SFed-LoRA能防止高秩崩溃，相比现有方法在高秩适配中显著提高稳定性并加速收敛，适用于多种任务、模型架构和异构数据分布。

Conclusion: SFed-LoRA通过理论推导的最优缩放因子解决了联邦学习中LoRA的不稳定性问题，为高秩适配器在隐私约束下的分布式场景提供了有效解决方案。
...
### [209] [Adversarial Domain Adaptation Enables Knowledge Transfer Across Heterogeneous RNA-Seq Datasets](https://arxiv.org/abs/2603.08062)
*Kevin Dradjat,Massinissa Hamidi,Blaise Hanczar*

Main category: cs.LG

TL;DR: 提出基于深度学习的领域自适应框架，用于RNA-seq数据的癌症类型分类，通过领域不变潜在空间实现从大型通用数据集到小型目标数据集的知识迁移。


<details>
  <summary>Details</summary>
Motivation: RNA-seq数据中准确预测表型对诊断和个性化医疗至关重要，但转录组学数据集通常有限，导致深度学习模型过拟合和泛化能力差。需要解决跨数据集知识迁移的挑战，包括异质预处理流程和目标表型差异。

Method: 提出深度学习领域自适应框架，通过联合优化分类和领域对齐目标学习领域不变潜在空间。采用对抗训练方法和适当正则化确保稳定训练和数据稀缺场景的鲁棒性。探索监督和无监督两种变体，分别利用有标签或无标签目标样本。

Result: 在三个大规模转录组数据集（TCGA、ARCHS4、GTEx）上评估，实验结果显示在癌症和组织类型分类准确率上相比非自适应基线有持续改进，特别是在低数据场景下表现更优。

Conclusion: 领域自适应是转录组学中数据高效知识迁移的强大策略，能够在受限数据条件下实现鲁棒的表型预测，为生物医学应用提供了实用解决方案。
...
### [210] [Deterministic Differentiable Structured Pruning for Large Language Models](https://arxiv.org/abs/2603.08065)
*Weiyu Huang,Pengle Zhang,Xiaolu Zhang,Jun Zhou,Jun Zhu,Jianfei Chen*

Main category: cs.LG

TL;DR: 提出确定性可微分剪枝方法DDP，通过直接优化离散l0目标的确定性软代理，消除随机性，减少训练-测试不匹配，在多种模型上实现高性能剪枝。


<details>
  <summary>Details</summary>
Motivation: 结构化剪枝通过移除低重要性组件来降低LLM推理成本，但现有方法使用随机硬混凝土松弛进行可微分优化，存在训练-测试不匹配问题，且掩码范围受限。

Method: 提出确定性可微分剪枝方法DDP，直接优化离散l0目标的确定性软代理，消除随机性，提供更大表达能力，减少训练-测试不匹配，加速收敛。

Result: 在Qwen3-32B和Qwen3-30B-A3B等密集和MoE模型上应用，下游任务性能损失仅1%，在20%稀疏度下优于先前方法，并在vLLM中实现端到端推理加速。

Conclusion: DDP方法通过确定性优化解决了结构化剪枝中的随机性问题，实现了更好的性能保持和推理加速，为LLM部署提供了有效的剪枝方案。
...
### [211] [Hybrid Quantum Neural Network for Multivariate Clinical Time Series Forecasting](https://arxiv.org/abs/2603.08072)
*Irene Iele,Floriano Caprio,Paolo Soda,Matteo Tortora*

Main category: cs.LG

TL;DR: 提出一种混合量子-经典架构，用于多变量多时间步的生理信号预测，在噪声和缺失数据情况下表现更稳健


<details>
  <summary>Details</summary>
Motivation: 生理信号预测能够支持主动监测和及时临床干预，通过预测患者状态的关键变化。当前需要解决多变量多时间步的生理时间序列预测问题。

Method: 提出混合量子-经典架构：使用GRU编码器将历史观测窗口编码为潜在表示，然后投影到量子角度参数化变分量子电路(VQC)。量子层作为可学习的非线性特征混合器，在最终预测阶段前建模跨变量交互。

Result: 在BIDMC PPG和呼吸数据集上，采用留一患者交叉验证协议。结果显示与经典和深度学习基线相比具有竞争力的准确性，同时对噪声和缺失输入具有更强的鲁棒性。

Conclusion: 混合量子层可以为小规模临床环境中的生理时间序列预测提供有用的归纳偏置，量子计算在医疗时间序列分析中具有潜力。
...
### [212] [Tiny Autoregressive Recursive Models](https://arxiv.org/abs/2603.08082)
*Paulius Rauba,Claudio Fanconi,Mihaela van der Schaar*

Main category: cs.LG

TL;DR: 研究评估了将Tiny Recursive Models (TRMs)机制应用于自回归模型的可行性，发现虽然两级细化基线在某些任务上表现良好，但完整的自回归TRM架构并未带来可靠的性能提升。


<details>
  <summary>Details</summary>
Motivation: TRMs在ARC-AGI上表现出色，通过两步细化机制更新内部推理状态和预测输出。研究者想知道这种机制是否能有效应用于自回归模型，但TRMs缺乏因果预测结构和持久潜在状态，难以与标准模型直接比较。

Method: 提出自回归TRM并在小型自回归任务上评估。设计了一套模型，逐步将标准Transformer转换为Tiny Autoregressive Recursive Model，在固定块设计、令牌流和下一令牌目标的受控设置中进行实验。

Result: 在字符级算法任务的算力匹配实验中，意外发现某些两级细化基线表现出色。但与预期相反，完整的自回归TRM架构并未带来可靠的性能提升。

Conclusion: 两级细化机制总体上具有潜力，但自回归TRM特定模型可能不是有前景的研究方向，建议谨慎投资。
...
### [213] [EAGLE-Pangu: Accelerator-Safe Tree Speculative Decoding on Ascend NPUs](https://arxiv.org/abs/2603.08088)
*Chang Han,Yijie Hu,Jingling Liu*

Main category: cs.LG

TL;DR: EAGLE-Pangu 是一个将树状推测解码移植到昇腾NPU上Pangu教师模型后端的可复现系统，通过分支/提交缓存管理、加速器安全的树张量化等技术，在MT-Bench和HumanEval风格提示上实现平均1.27倍解码吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 自回归解码是大语言模型服务的主要瓶颈，推测解码方法通过每步验证多个候选token来减少昂贵的教师模型调用。树状推测能进一步提高并行性，但在异构后端和加速器栈之间移植时往往很脆弱，因为注意力掩码、KV缓存布局和索引语义不可互换。

Method: EAGLE-Pangu系统包含三个关键技术：(1) 基于缓存API构建的显式分支/提交缓存管理器；(2) 通过构造消除未定义负索引并验证结构不变量的加速器安全树张量化；(3) 兼容融合内核的教师验证路径，带有可调试的eager回退机制。

Result: 在MT-Bench和HumanEval风格的240轮提示上，EAGLE-Pangu在融合内核性能路径上相比仅使用教师模型的贪婪解码，端到端解码吞吐量平均提升1.27倍，p99时最高提升2.46倍。系统还提供了无融合内核的参考路径，支持可复现调试和消融实验。

Conclusion: EAGLE-Pangu成功将树状推测解码移植到昇腾NPU的Pangu后端，通过系统化方法解决了跨异构平台的移植挑战，实现了显著的解码加速，同时保持了可调试性和可复现性。
...
### [214] [Invisible Safety Threat: Malicious Finetuning for LLM via Steganography](https://arxiv.org/abs/2603.08104)
*Guangnian Wan,Xinyin Ma,Gongfan Fang,Xinchao Wang*

Main category: cs.LG

TL;DR: 论文揭示了一种隐蔽的LLM安全威胁：通过微调使模型学会隐写术，在看似良性的问答交互中隐藏恶意内容，从而绕过安全对齐检测。


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全对齐主要关注直接的有害内容生成，但忽视了模型可能被训练成在表面安全交互下隐藏恶意内容的风险。这种隐蔽威胁更难被检测和防范。

Method: 通过微调使LLM学习并应用隐写技术：在推理时输入包含隐写嵌入的恶意问题（隐藏在明文覆盖问题中），模型生成包含隐写恶意响应的表面良性回答。人类只能看到覆盖交互，恶意内容被隐藏。

Result: 成功在GPT-4.1上演示了该攻击，绕过了OpenAI微调API的安全防护。在Llama-3.3-70B-Instruct、Phi-4和Mistral-Small-24B-Base-2501三个开源模型上复现成功。使用Llama-Guard-3-8B评估AdvBench数据集，所有包含恶意内容的隐写文本都被错误分类为安全。

Conclusion: LLM安全对齐存在严重漏洞，模型可被训练成在表面安全交互下隐藏恶意内容。这揭示了现有安全防护机制的局限性，需要开发更强大的检测方法来应对这种隐蔽威胁。
...
### [215] [Model-based Offline RL via Robust Value-Aware Model Learning with Implicitly Differentiable Adaptive Weighting](https://arxiv.org/abs/2603.08118)
*Zhongjian Qiao,Jiafei Lyu,Boxiang Lyu,Yao Shu,Siyang Gao,Shuang Qiu*

Main category: cs.LG

TL;DR: ROMI提出了一种新的鲁棒值感知模型学习方法，通过可调节的状态不确定性集实现可控保守性和稳定模型更新，解决了RAMBO中的Q值低估和梯度爆炸问题。
...
Method: ROMI采用鲁棒值感知模型学习方法，要求动态模型预测的未来状态值接近状态不确定性集内的最小Q值，实现可控保守性。同时提出隐式可微自适应加权，通过双层优化方案自适应实现动态感知和值感知的模型学习，改善多步rollout中的分布外泛化能力。
...
### [216] [Mitigating Homophily Disparity in Graph Anomaly Detection: A Scalable and Adaptive Approach](https://arxiv.org/abs/2603.08137)
*Yunhui Liu,Qizhuo Xie,Yinfeng Chen,Xudong Jin,Tao Zheng,Bin Chong,Tieke He*

Main category: cs.LG

TL;DR: SAGAD是一个可扩展的自适应图异常检测框架，通过多跳嵌入和重参数化切比雪夫滤波器解决同质性差异和可扩展性问题，支持小批量训练并实现线性复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有图异常检测方法面临两个主要挑战：1）同质性差异，节点在类别和节点层面表现出不同的同质性；2）有限的可扩展性，许多方法依赖昂贵的全图操作。

Method: SAGAD预计算多跳嵌入并应用重参数化切比雪夫滤波器提取低频和高频信息。引入异常上下文感知自适应融合来缓解节点级同质性差异，使用频率偏好引导损失来缓解类别级差异。

Result: 在10个基准测试上的广泛实验证实了SAGAD在准确性和可扩展性方面优于最先进的方法。理论分析表明在温和条件下，SAGAD能确保正常和异常节点之间的渐近线性可分性。
...
### [217] [DARC: Disagreement-Aware Alignment via Risk-Constrained Decoding](https://arxiv.org/abs/2603.08145)
*Mingxi Zou,Jiaxiang Chen,Junfan Li,Langzhang Liang,Qifan Wang,Xu Yinghui,Zenglin Xu*

Main category: cs.LG

TL;DR: DARC是一种无需重新训练的推理时方法，通过风险约束解码处理人类偏好的异质性，在保持平均质量的同时减少分歧和尾部风险。


<details>
  <summary>Details</summary>
Motivation: 现有的偏好对齐方法（如RLHF、DPO）通常优化单一标量目标，隐含地平均处理异质性的人类偏好。在实践中，系统性的标注者和用户群体分歧使得均值奖励最大化变得脆弱且容易受到代理过度优化的影响。

Method: DARC（Disagreement-Aware Alignment via Risk-Constrained Decoding）是一种无需重新训练的推理时方法，将响应选择框架化为分布鲁棒的风险敏感决策。给定多个偏好样本或可扩展的分歧代理，DARC通过最大化KL鲁棒（熵）满意度目标来重新排序候选响应，并提供简单的部署控制，限制或惩罚相对于均值的相应熵风险溢价，实现明确的风险预算而无需重新训练。

Result: 在对齐基准测试中，DARC在嘈杂、异质反馈下减少了分歧和尾部风险，同时保持了竞争力的平均质量。

Conclusion: DARC提供了一种实用的推理时方法，通过风险约束解码处理偏好异质性，为对齐系统提供了更好的风险控制能力，无需昂贵的重新训练过程。
...
### [218] [Training event-based neural networks with exact gradients via Differentiable ODE Solving in JAX](https://arxiv.org/abs/2603.08146)
*Lukas König,Manuel Kuhn,David Kappel,Anand Subramoney*

Main category: cs.LG

TL;DR: Eventax框架结合可微分ODE求解器和基于事件的处理，为任意ODE定义的神经元模型提供精确梯度训练，解决了现有SNN训练方法在灵活性和精确性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有脉冲神经网络梯度训练框架存在权衡：离散时间方法使用代理梯度支持任意神经元模型但引入梯度偏差并限制脉冲时间分辨率；连续时间方法计算精确梯度但需要脉冲时间和状态演化的解析表达式，仅限于简单神经元类型如LIF。

Method: 基于JAX构建，结合Diffrax可微分ODE求解器和基于事件的脉冲处理，用户只需指定神经元动力学、脉冲条件和重置规则，支持任意ODE定义的神经元模型。

Result: 在多个基准测试（Yin-Yang、MNIST）上展示了框架的实用性，支持多种神经元模型（LIF、QIF、EIF、Izhikevich、EGRU）和损失函数，并实现了人类皮层锥体神经元的多室模型。

Conclusion: Eventax提供了建模灵活性，支持广泛的神经元模型、损失函数和网络架构，为使用精确梯度训练基于事件的架构提供了原型设计和测试工具。
...
### [219] [C$^2$FG: Control Classifier-Free Guidance via Score Discrepancy Analysis](https://arxiv.org/abs/2603.08155)
...
TL;DR: 本文对Classifier-Free Guidance (CFG)进行理论分析，提出基于扩散过程动态调整权重的Control CFG方法


<details>
  <summary>Details</summary>
Motivation: 当前CFG方法使用固定或启发式动态权重，缺乏理论依据，忽视了扩散过程的固有动态特性

Method: 提出Control Classifier-Free Guidance (C²FG)，通过指数衰减控制函数使引导强度与扩散动态对齐，无需训练且可即插即用

Result: C²FG在多种生成任务中有效且广泛适用，同时与现有策略正交

Conclusion: 理论分析揭示了固定权重策略的局限性，提出的C²FG方法为时间相关引导建立了原则性基础
...
### [220] [Learning Hierarchical Knowledge in Text-Rich Networks with Taxonomy-Informed Representation Learning](https://arxiv.org/abs/2603.08159)
*Yunhui Liu,Yongchao Liu,Yinfeng Chen,Chuntao Hong,Tao Zheng,Tieke He*

Main category: cs.LG

TL;DR: TIER：一种用于文本丰富网络的层次化分类学感知表示学习方法，通过构建隐式层次分类学并整合到节点表示中，提升对文本丰富网络的建模能力。


<details>
  <summary>Details</summary>
Motivation: 现有文本丰富网络学习方法通常关注平面语义建模，忽视了文本文档中固有的层次语义结构。层次知识结构在现实领域中普遍存在，但在文本丰富网络中的潜力尚未充分探索。

Method: TIER首先通过相似性引导的对比学习构建聚类友好的嵌入空间，然后执行层次K-Means聚类和LLM驱动的聚类精炼来构建语义一致的隐式层次分类学。利用得到的分类学，引入基于共表相关系数的正则化损失，使学习到的嵌入与层次结构对齐。

Result: 该方法在多个跨领域数据集上显著优于现有方法，证明了层次知识学习对文本丰富网络的重要性。

Conclusion: TIER通过整合层次分类学结构，实现了对文本丰富网络更可解释和结构化的建模，能够同时捕捉细粒度和粗粒度语义，为文本丰富网络学习提供了新思路。
...
### [221] [AutoAdapt: An Automated Domain Adaptation Framework for LLMs](https://arxiv.org/abs/2603.08181)
*Sidharth Sinha,Anson Bastos,Xuchao Zhang,Akshay Nambi,Chetan Bansal,Saravan Rajmohan*

Main category: cs.LG

TL;DR: AutoAdapt：一个端到端的自动化框架，通过多智能体辩论系统和LLM代理替代传统黑盒搜索，实现高效可靠的LLM领域自适应，在10个任务上比现有AutoML基线提升25%准确率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在开放领域表现优异，但在专业领域面临数据有限、知识快速更新的挑战。现有领域自适应方法依赖人工试错，超参数复杂，对数据和用户偏好敏感，且LLM训练成本高昂。不同模型/领域间超参数选择的交互和可迁移性理解不足，导致自适应效果不确定。

Method: 提出AutoAdapt框架：1）利用文献和开源资源构建知识库减少专家干预；2）设计多智能体辩论系统，提案和批评智能体迭代交互，整合用户意图、数据信号和最佳实践来缩小搜索空间；3）提出AutoRefine，用LLM代理替代昂贵的黑盒搜索，在有限预算下优化超参数。

Result: 在10个任务上，AutoAdapt相比最先进的自动化机器学习基线，实现了25%的平均相对准确率提升，且开销最小。

Conclusion: AutoAdapt通过自动化框架解决了LLM领域自适应的关键挑战，显著提升了适应效率和效果，为专业领域应用提供了可靠解决方案。
...
### [222] [SERQ: Saliency-Aware Low-Rank Error Reconstruction for LLM Quantization](https://arxiv.org/abs/2603.08185)
*Yeonsik Park,Hyeonseong Kim,Seungkyu Choi*

Main category: cs.LG

TL;DR: SERQ提出了一种基于显著性感知误差重建的4位LLM推理方法，通过单低秩补偿矩阵在保持高效4位矩阵乘法的同时，显著减少量化误差。


<details>
  <summary>Details</summary>
Motivation: 现有PTQ方法在W4A4设置下存在严重精度下降问题，传统低秩适配需要两个顺序因子，导致推理时需要中间量化，限制了低精度效率。

Method: SERQ采用三阶段方法：静态激活展平、显著性感知误差重建、离线权重置换。使用单低秩补偿矩阵联合缓解激活和权重显著性引起的量化误差。

Result: SERQ在W4A8和W4A4设置下均优于现有误差重建方法，比最先进的基于旋转的W4A4方法精度更高，同时大幅降低校准复杂度。

Conclusion: SERQ通过单低秩补偿矩阵实现了高效的4位LLM推理，在保持低延迟的同时显著提升量化精度，为边缘设备部署提供了有效的解决方案。
...
### [223] [Sequential Service Region Design with Capacity-Constrained Investment and Spillover Effect](https://arxiv.org/abs/2603.08188)
*Tingting Chen,Feng Chu,Jiantong Zhang*

Main category: cs.LG

TL;DR: 该研究提出了一种结合实物期权分析和Transformer强化学习的方法，用于解决具有k区域约束和随机溢出效应的顺序服务区域设计问题，能够有效处理不确定性下的投资序列决策。


<details>
  <summary>Details</summary>
Motivation: 服务区域设计影响长期运营绩效，但由于资本和运营约束，无法同时大规模部署，需要顺序扩展。现有研究缺乏对两个实际但未被充分探索的因素的考虑：每期可投资区域数量限制(k区域约束)和投资决策通过区域间连接影响需求演化的随机溢出效应。

Method: 提出一个解决方案框架，整合实物期权分析(ROA)和基于Transformer的近端策略优化(TPPO)算法。ROA评估投资序列的跨时期期权价值，TPPO学习直接生成高期权价值序列的顺序策略，避免穷举枚举。

Result: 在现实多区域设置下的数值实验表明，TPPO比基准深度强化学习方法收敛更快，并始终识别出具有更优期权价值的序列。案例研究和敏感性分析进一步证实了方法的稳健性。

Conclusion: 该方法在更强的溢出效应和动态市场条件下，通过自适应扩展提供了关于投资并发性、区域优先级排序以及增加收益的见解，为顺序服务区域设计问题提供了有效的解决方案。
...
### [224] [Distributional Regression with Tabular Foundation Models: Evaluating Probabilistic Predictions via Proper Scoring Rules](https://arxiv.org/abs/2603.08206)
*Jonas Landsgesell,Pascal Knoll*

Main category: cs.LG

TL;DR: 论文指出当前表格数据回归任务的基准测试过于关注点估计（如均方误差），忽略了概率预测评估，建议使用连续排名概率得分（CRPS）等评分规则来评估概率回归模型。


<details>
  <summary>Details</summary>
Motivation: 当前表格深度学习基准测试（如TabArena、TALENT）主要评估点估计性能（均方误差、R²），这导致研究人员过度优化均值估计，而忽略了概率预测的重要性。PFNs等模型作为基础模型，需要更全面的评估框架来反映实际部署中的不确定性量化需求。

Method: 提出在现有机器学习基准测试中增加概率回归评估指标，特别推荐使用连续排名概率得分（CRPS）作为概率回归的评分规则。同时指出不同评分规则会影响模型的归纳偏差，因此建议对可提示的表格基础模型进行微调。

Result: 论文揭示了当前基准测试的局限性，提出了使用适当评分规则评估概率预测的方法，并展示了评分规则选择对模型训练的影响，为改进表格数据回归评估提供了具体方向。

Conclusion: 需要改革当前表格数据回归的评估体系，从单纯的点估计评估转向概率预测评估，使用CRPS等评分规则，并开发可微调/可提示的表格基础模型以适应不同评分规则的归纳偏差要求。
...
### [225] [Revisiting Gradient Staleness: Evaluating Distance Metrics for Asynchronous Federated Learning Aggregation](https://arxiv.org/abs/2603.08211)
*Patrick Wilhelm,Odej Kao*

Main category: cs.LG

TL;DR: 该论文提出在异步联邦学习中，探索使用不同距离度量替代欧氏距离来衡量梯度陈旧性，以提高全局模型的收敛性和准确性。


<details>
  <summary>Details</summary>
Motivation: 异步联邦学习中，客户端基于计算速度在不同时间发送更新，经常使用过时的全局模型版本。这种陈旧性会降低全局模型的收敛性和准确性。先前工作如AsyncFedED使用欧氏距离衡量陈旧性，但需要探索更准确的距离度量来捕捉梯度陈旧性的影响。

Method: 扩展AsyncFedED方法，探索替代的距离度量来更准确地衡量梯度陈旧性，并将这些度量集成到聚合过程中。在异构客户端和非独立同分布数据设置下评估不同距离度量对收敛速度、模型性能和训练稳定性的影响。

Result: 结果表明某些距离度量能够带来更鲁棒和高效的异步联邦学习训练，为实际部署提供了更强的基础。这些度量改进了异步FL的收敛速度、模型性能和训练稳定性。

Conclusion: 通过探索替代的距离度量来衡量梯度陈旧性，可以显著提高异步联邦学习的性能。某些特定的距离度量比传统的欧氏距离更能准确捕捉陈旧性影响，从而提供更鲁棒和高效的训练方案，增强了异步FL在实际应用中的可行性。
...
### [226] [Wiener Chaos Expansion based Neural Operator for Singular Stochastic Partial Differential Equations](https://arxiv.org/abs/2603.08219)
*Dai Shi,Luke Thompson,Andi Han,Peiyan Hu,Junbin Gao,José Miguel Hernández-Lobato*

Main category: cs.LG

TL;DR: 提出WCE-FiLM-NO方法，结合Wiener混沌展开与特征线性调制，用于求解奇异随机偏微分方程（如Φ⁴₂和Φ⁴₃模型），无需重整化因子即可获得优异性能。


<details>
  <summary>Details</summary>
Motivation: 传统WCE-NO方法在处理奇异SPDE时，只是简单地将Wick-Hermite特征插入主干网络，无法有效捕捉奇异SPDE解与其光滑余项之间的依赖关系。需要开发更有效的方法来模拟动态Φ⁴模型，特别是Φ⁴₃模型在统计量子场论中的实际应用。

Method: 提出WCE-FiLM-NO方法，在Wiener混沌展开基础上引入特征线性调制（FiLM）技术，通过条件调制机制更好地捕捉奇异SPDE解与其光滑余项之间的复杂依赖关系，无需重整化因子。

Result: 在Φ⁴₂模型上表现出色，相对L₂损失、分布外L₂损失和自相关分数均表现优异。同时展示了模拟Φ⁴₃数据的潜力，这是首批为动态Φ⁴₃模型开发高效数据驱动代理的工作之一。

Conclusion: WCE-FiLM-NO方法能有效处理奇异随机偏微分方程，特别是在Φ⁴模型上表现出优越性能，为统计量子场论中的实际应用提供了有前景的数据驱动模拟工具。
...
### [227] [Fibration Policy Optimization](https://arxiv.org/abs/2603.08239)
*Chang Li,Tshihao Tsu,Yaren Zhang,Chao Xue,Xiaodong He*

Main category: cs.LG

TL;DR: 提出了FiberPO框架，通过代数纤维束结构实现多尺度策略优化，将信任域理论与分层稳定性控制统一起来。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型训练日益异构化，但现有方法缺乏耦合token级、轨迹级和更高层次稳定性控制的机制。

Method: 1) 推导APC-Obj目标函数；2) 开发FBG框架，将采样数据组织为纤维束；3) 提出FiberPO目标函数；4) 构建FGH层次结构实现任意深度分层控制。

Result: 建立了信任域优化与裁剪代理设计的对偶关系，实现了多尺度稳定性控制，提高了token效率，并展示了四层实例FiberPO-Domain。

Conclusion: 将信任域理论、代数结构和多尺度稳定性控制统一为LLM策略优化的完整框架。
...
### [228] [Optimising antibiotic switching via forecasting of patient physiology](https://arxiv.org/abs/2603.08242)
*Magnus Ross,Nel Swanepoel,Akish Luintel,Emma McGuire,Ingemar J. Cox,Steve Harris,Vasileios Lampos*

Main category: cs.LG

TL;DR: 使用神经过程模型预测患者生命体征轨迹，根据临床指南而非历史决策来评估抗生素从静脉转为口服的适宜时机，提高转换率
...
Method: 使用神经过程模型对生命体征轨迹进行概率建模，通过将预测结果与临床指南比较来评估转换适宜性，而非学习过去的决策行为。系统对患者进行排序以优先临床审查，提供可解释的输出，无需重新训练即可适应更新的指南

Result: 在MIMIC-IV（美国重症监护，6,333例）和UCLH（英国大型城市学术医院集团，10,584例）验证中，系统选择的患者相关性比随机选择高2.2-3.2倍

Conclusion: 预测患者生理状态为抗生素管理决策支持提供了原则性基础，系统能提高转换率，保持临床判断力，并适应指南更新
...
### [229] [FedPrism: Adaptive Personalized Federated Learning under Non-IID Data](https://arxiv.org/abs/2603.08252)
*Prakash Kumbhakar,Shrey Srivastava,Haroon R Lone*

Main category: cs.LG

TL;DR: FedPrism：一个联邦学习框架，通过棱镜分解和双流设计解决非IID数据下的性能下降问题，实现全局泛化与个性化适应的平衡


<details>
  <summary>Details</summary>
Motivation: 现实世界联邦学习中存在中度到极端的统计异质性（非IID客户端数据），导致性能显著下降。全局聚合策略虽然促进广泛泛化，但往往无法捕捉本地数据分布的多样性，导致个性化效果不佳。

Method: 1. 棱镜分解方法：将每个客户端模型分解为三部分：全局基础部分、相似客户端共享的组部分、本地数据独有的私有部分。系统能自动将相似用户分组，并在数据变化时自适应调整。
2. 双流设计：并行运行通用模型和本地专家模型，根据专家模型的置信度在通用模型和本地专家之间路由预测。

Result: 在非IID数据分区上的系统实验表明，FedPrism超越了静态聚合和硬聚类基线方法，在高异质性条件下实现了显著的准确率提升。

Conclusion: FedPrism是异构环境中联邦学习的鲁棒且灵活的解决方案，有效平衡了可泛化知识与自适应个性化，为实际部署中的统计异质性挑战提供了有效应对方案。
...
### [230] [Airborne Magnetic Anomaly Navigation with Neural-Network-Augmented Online Calibration](https://arxiv.org/abs/2603.08265)
*Antonia Hager,Sven Nebendahl,Alexej Klushyn,Jasper Krauser,Torleiv H. Bryne,Tor Arne Johansen*

Main category: cs.LG

TL;DR: 提出一种完全自适应的磁异常导航架构，具备"冷启动"能力，可在飞行中实时识别并补偿飞机磁干扰，无需离线校准飞行或预训练。


<details>
  <summary>Details</summary>
Motivation: 现有磁异常导航解决方案通常依赖大量离线校准飞行或预训练，这给实际部署带来后勤障碍。需要一种能在飞行中实时补偿飞机平台大动态磁干扰的方法。

Method: 使用扩展卡尔曼滤波器，其增强状态向量同时估计飞机运动状态、基于物理的Tolles-Lawson校准模型系数以及神经网络参数。卡尔曼滤波更新在数学上等价于在线自然梯度下降，将二阶优化的优越收敛性和数据效率直接集成到导航滤波器中。神经网络被约束为残差学习角色，仅建模物理基线未校正的非线性。

Result: 在MagNav Challenge数据集上验证，该框架能有效限制仅使用磁力计特征集的惯性漂移。导航精度与离线训练的最先进模型相当，无需先前的校准飞行或专用机动。

Conclusion: 提出的完全自适应磁异常导航架构实现了"冷启动"能力，消除了对离线校准的依赖，为实际部署提供了更实用的解决方案。
...
### [231] [SCL-GNN: Towards Generalizable Graph Neural Networks via Spurious Correlation Learning](https://arxiv.org/abs/2603.08270)
*Yuxiang Zhang,Enyan Dai*

Main category: cs.LG

TL;DR: 提出SCL-GNN框架，通过HSIC量化节点表示与类别分数的虚假相关性，采用双层优化策略，提升GNN在IID和OOD图上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: GNN在训练中容易利用节点特征与标签之间的虚假相关性，这些相关性虽然统计上显著但不可靠，导致泛化能力受限，特别是在分布偏移情况下。

Method: 提出SCL-GNN框架：1) 使用Hilbert-Schmidt独立性准则(HSIC)量化节点表示与类别分数之间的虚假相关性；2) 引入双层优化策略联合优化模块和GNN参数，防止过拟合。

Result: 在真实世界和合成数据集上的大量实验表明，SCL-GNN在各种分布偏移情况下均优于现有基线方法，显示出更强的鲁棒性和泛化能力。

Conclusion: SCL-GNN通过显式学习和缓解虚假相关性，有效提升了GNN在IID和OOD图数据上的泛化性能，为解决GNN中的虚假相关性问题提供了新思路。
...
### [232] [TA-RNN-Medical-Hybrid: A Time-Aware and Interpretable Framework for Mortality Risk Prediction](https://arxiv.org/abs/2603.08278)
*Zahra Jafari,Azadeh Zamanifar,Amirfarhad Farhadi*

Main category: cs.LG

TL;DR: 提出TA-RNN-Medical-Hybrid框架，通过连续时间编码和医学知识融合解决ICU死亡率预测中的时间不规则性和可解释性问题。


<details>
  <summary>Details</summary>
Motivation: ICU死亡率预测面临三大挑战：电子健康记录的时间不规则性、纵向疾病轨迹的复杂性，以及现有数据驱动模型缺乏临床可解释性。需要同时提高预测准确性和提供临床可理解的解释。

Method: 提出时间感知和知识增强的深度学习框架TA-RNN-Medical-Hybrid，包含：1) 独立于就诊索引的显式连续时间嵌入；2) 基于SNOMED的疾病表示；3) 分层双级注意力机制（就诊级时间重要性和特征/概念级临床相关性）。

Result: 在MIMIC-III数据集上验证，相比基线模型在AUC、准确率和召回率导向的F2分数上均有提升。定性分析显示模型能有效分解跨时间和临床概念的死亡风险，提供疾病严重程度、慢性和时间进展的可解释洞察。
...
### [233] [Minor First, Major Last: A Depth-Induced Implicit Bias of Sharpness-Aware Minimization](https://arxiv.org/abs/2603.08290)
*Chaewon Moon,Dongkuk Si,Chulhee Yun*

Main category: cs.LG

TL;DR: 本文研究了SAM在深度线性对角网络中的隐式偏差，发现对于深度L=2的网络，ℓ∞-SAM的极限方向依赖于初始化，而ℓ₂-SAM虽然最终收敛到ℓ₁最大间隔解，但训练过程中表现出"顺序特征放大"现象。


<details>
  <summary>Details</summary>
Motivation: 研究Sharpness-Aware Minimization（SAM）在深度线性对角网络中的隐式偏差，特别是与梯度下降（GD）相比，SAM在不同深度网络中的收敛行为差异。

Method: 理论分析L层线性对角网络在线性可分二元分类问题上的训练行为，对比ℓ∞-SAM和ℓ₂-SAM在不同深度（L=1和L=2）下的收敛特性，并通过合成和真实数据实验验证。

Result: 对于L=1，SAM与GD一样收敛到ℓ₂最大间隔分类器；对于L=2，ℓ∞-SAM的极限方向依赖于初始化，可能收敛到0或任意标准基向量，而ℓ₂-SAM虽然最终匹配ℓ₁最大间隔解，但训练过程中表现出"顺序特征放大"现象。

Conclusion: SAM在深度网络中的隐式偏差与浅层网络有本质不同，ℓ₂-SAM的梯度归一化因子导致训练过程中特征依赖顺序变化，这表明无限时间隐式偏差分析在某些情况下是不充分的。
...
### [234] [Rethinking Attention Output Projection: Structured Hadamard Transforms for Efficient Transformers](https://arxiv.org/abs/2603.08343)
*Shubham Aggarwal,Lokendra Kumar*

Main category: cs.LG

TL;DR: 用Walsh-Hadamard变换替代多头注意力中的密集输出投影，减少25%注意力参数，保持性能的同时提升效率


<details>
  <summary>Details</summary>
Motivation: 多头注意力中的密集输出投影随模型维度二次方增长，显著增加了参数数量、内存占用和推理成本，需要更高效的替代方案

Method: 使用固定的、无参数的Walsh-Hadamard变换替代密集输出投影，后接轻量级可学习的仿射缩放，通过正交、保范变换保持全局跨头交互

Result: 在不同模型规模下，该方法在标准基准测试中保持相当或略优的下游任务性能，同时实现最高7%的总参数减少、8.9%的峰值内存节省和6.6%的吞吐量提升，效率增益随模型规模、批次大小和序列长度单调增长

Conclusion: 结构化Hadamard变换模型在训练FLOPs方面表现出更陡峭的验证损失曲线，表明训练期间计算利用率更优，为减少注意力参数同时保持性能提供了有效方案
...
### [235] [A Recipe for Stable Offline Multi-agent Reinforcement Learning](https://arxiv.org/abs/2603.08399)
*Dongsu Lee,Daehee Lee,Amy Zhang*

Main category: cs.LG

TL;DR: 本文提出了一种尺度不变价值归一化(SVN)技术，解决了离线多智能体强化学习中非线性价值分解的不稳定性问题，并提供了实用的离线MARL训练方案。


<details>
  <summary>Details</summary>
Motivation: 尽管单智能体离线强化学习取得了显著成就，但多智能体强化学习(MARL)仍主要采用在线策略训练和从头开始的自我博弈。这一差距部分源于非线性价值分解的不稳定性，导致先前工作避免使用复杂的混合网络，而采用线性价值分解(如VDN)和单智能体设置中的价值正则化方法。

Method: 本文分析了离线MARL设置中非线性价值分解不稳定的根源，发现其会导致价值尺度放大和优化不稳定。为此提出了尺度不变价值归一化(SVN)技术，在不改变Bellman不动点的情况下稳定actor-critic训练。同时研究了离线MARL关键组件(如价值分解、价值学习和策略提取)之间的相互作用，并推导出实用的训练方案。

Result: SVN技术有效稳定了非线性价值分解的训练，解锁了离线MARL的潜力。通过系统分析各组件相互作用，提出了完整的离线MARL实用训练方案。

Conclusion: 本文通过分析非线性价值分解的不稳定性根源，提出了简单有效的SVN技术，解决了离线MARL长期存在的训练不稳定问题，为离线MARL的实际应用提供了可行的解决方案。
...
### [236] [Geometrically Constrained Outlier Synthesis](https://arxiv.org/abs/2603.08413)
*Daniil Karzanov,Marcin Detyniecki*

Main category: cs.LG

TL;DR: GCOS：一种训练时正则化框架，通过在隐藏特征空间中生成符合流形结构的虚拟离群点，提升模型对分布外样本的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络对分布外样本常表现出过度自信，现有合成方法在生成离群点时未能充分考虑训练数据的流形结构。

Method: 两阶段方法：1) 从训练特征提取主方差子空间确定几何感知的离流形方向；2) 基于校准集非一致性分数的经验分位数定义保形壳，自适应控制合成幅度生成边界样本。结合对比正则化目标，在得分空间（如马氏距离或能量）中促进分布内外样本的可分离性。

Result: 在近分布外基准测试中，GCOS在使用标准能量推断时优于现有方法。框架可自然扩展到保形分布外推断，将不确定性分数转换为统计有效的p值，提供具有形式化误差保证的阈值。

Conclusion: GCOS通过几何约束的离群点合成有效提升模型对分布外样本的鲁棒性，为更可预测和可靠的分布外检测提供了途径。
...
### [237] [Meta-RL with Shared Representations Enables Fast Adaptation in Energy Systems](https://arxiv.org/abs/2603.08418)
*Théo Zangato,Aomar Osmani,Pegah Alizadeh*

Main category: cs.LG

TL;DR: 提出一种新颖的元强化学习框架，通过双层优化和混合actor-critic架构提升样本效率和任务间适应性，在建筑能源管理系统数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在多任务和非平稳环境中存在局限性，需要快速策略适应和更好的泛化能力。元强化学习能够解决这些问题，但现有方法在样本效率和任务间知识迁移方面仍有改进空间。

Method: 1. 集成双层优化方案与混合actor-critic架构；2. 元学习共享状态特征提取器，在actor和critic网络中联合优化；3. 提出外层和内层actor网络之间的参数共享机制；4. 在建筑能源管理系统数据集上设计任务准备方法促进泛化。

Result: 实验表明该方法能够有效适应任务，相比传统强化学习和现有元强化学习方法表现更好，在近十年的时变和结构变化数据上验证了有效性。

Conclusion: 提出的元强化学习框架通过改进的特征提取和参数共享机制，显著提升了样本效率、任务适应性和泛化能力，为复杂非平稳环境中的强化学习应用提供了有效解决方案。
...
### [238] [SYNAPSE: Framework for Neuron Analysis and Perturbation in Sequence Encoding](https://arxiv.org/abs/2603.08424)
*Jesús Sánchez Ochoa,Enrique Tomás Martínez Beltrán,Alberto Huertas Celdrán*

Main category: cs.LG

TL;DR: SYNAPSE是一个无需训练的系统化框架，用于理解和压力测试Transformer模型的内部行为，通过提取层表示、训练轻量级线性探针和推理时干预来评估模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在医疗、网络安全等敏感领域，AI缺乏透明度引发可靠性担忧。现有神经元级可解释性方法多为描述性、任务依赖或需要重新训练，限制了其作为跨架构和跨领域评估内部鲁棒性的系统化工具。

Method: 提取每层的[CLS]表示，训练轻量级线性探针获得全局和每类神经元排名，在推理时应用前向钩子干预，无需改变原始模型即可进行受控实验。

Result: SYNAPSE揭示了内部表示的一致、领域无关的组织模式：任务相关信息编码在广泛重叠的神经元子集中，这种冗余提供了功能稳定性，而类别不对称性暴露了异质专业化模式。小规模结构化操作足以重定向预测，突显了互补的脆弱性特征。

Conclusion: SYNAPSE能够系统化地理解和压力测试Transformer模型的内部行为，揭示其鲁棒性和脆弱性模式，为开发更鲁棒的Transformer模型提供指导。
...
### [239] [Grow, Assess, Compress: Adaptive Backbone Scaling for Memory-Efficient Class Incremental Learning](https://arxiv.org/abs/2603.08426)
*Adrian Garcia-Castañeda,Jon Irureta,Jon Imaz,Aizea Lojo*

Main category: cs.LG

TL;DR: 提出GRACE框架，通过动态缩放模型容量解决CIL中的可塑性-稳定性权衡问题，在保持高性能的同时大幅减少内存占用。
...
Method: 提出动态缩放框架GRACE（GRow, Assess, ComprEss），包含三个循环阶段：1) 增长阶段扩展主干网络；2) 评估阶段通过新颖的饱和度评估来评估模型容量利用率；3) 压缩阶段将主干网络压缩为精简表示，防止参数爆炸。

Result: 在多个CIL基准测试中实现了最先进的性能，同时与纯扩展模型相比，内存占用减少了高达73%。

Conclusion: GRACE框架通过动态管理模型容量，有效解决了CIL中的可塑性-稳定性权衡问题，在保持高性能的同时显著降低了内存开销。
...
### [240] [LycheeCluster: Efficient Long-Context Inference with Structure-Aware Chunking and Hierarchical KV Indexing](https://arxiv.org/abs/2603.08453)
*Dongfang Li,Zixuan Liu,Gang Lin,Baotian Hu,Min Zhang*

Main category: cs.LG

TL;DR: LycheeCluster 是一种新颖的 KV 缓存管理方法，通过边界感知分块和递归层次索引，将缓存检索从线性扫描转变为对数时间剪枝，实现高效长上下文处理。


<details>
  <summary>Details</summary>
Motivation: 注意力机制的二次复杂度和 KV 缓存的大内存占用给处理长上下文的 LLMs 带来了严重的计算和内存挑战。现有的基于检索的方法通常通过固定大小的分块损害语义完整性，并存在低效的线性扫描问题。

Method: LycheeCluster 通过边界感知分块保持局部语义连贯性，并构建基于三角不等式的递归层次索引。这种设计将缓存检索从线性扫描转变为理论上有界的对数时间剪枝过程，同时采用惰性更新策略支持高效的流式生成。

Result: 实验表明，LycheeCluster 在模型性能几乎不下降的情况下，实现了高达 3.6 倍的端到端推理加速，优于最先进的 KV 缓存管理方法（如 Quest、ClusterKV）。

Conclusion: LycheeCluster 通过创新的边界感知分块和递归层次索引设计，有效解决了长上下文处理中的 KV 缓存管理问题，显著提升了推理效率。
...
### [241] [Data-Driven Priors for Uncertainty-Aware Deterioration Risk Prediction with Multimodal Data](https://arxiv.org/abs/2603.08459)
*L. Julián Lechuga López,Tim G. J. Rudner,Farah E. Shamout*

Main category: cs.LG

TL;DR: 提出MedCertAIn框架，利用多模态临床数据（时间序列和X光图像）进行院内风险预测，通过数据驱动的先验分布提升预测性能和不确定性量化能力。


<details>
  <summary>Details</summary>
Motivation: 临床决策支持系统需要安全可靠的预测，当前机器学习模型缺乏可靠的不确定性估计，特别是在多模态融合场景下，这阻碍了实际部署。

Method: 设计数据驱动的神经网络参数先验分布，采用混合策略：考虑自监督潜在表示中的跨模态相似性和模态特定的数据扰动。使用MIMIC-IV和MIMIC-CXR数据集中的临床时间序列和胸部X光图像进行训练评估。

Result: MedCertAIn相比最先进的确定性基线和替代贝叶斯方法，显著提高了预测性能和不确定性量化能力。

Conclusion: 数据驱动的先验分布有望推进高风险临床应用中稳健、不确定性感知的AI工具发展。
...
### [242] [Reasoning as Compression: Unifying Budget Forcing via the Conditional Information Bottleneck](https://arxiv.org/abs/2603.08462)
*Fabio Valerio Massoli,Andrey Kuzmin,Arash Behboodi*

Main category: cs.LG

TL;DR: 本文提出基于条件信息瓶颈（CIB）的强化学习目标，用于压缩思维链推理过程，在减少token使用的同时保持任务准确性。


<details>
  <summary>Details</summary>
Motivation: 思维链（CoT）提示能提升LLM在复杂任务上的准确性，但会增加token使用和推理成本。现有基于启发式长度惩罚的"预算强制"方法会同时抑制必要的推理和冗余填充。

Method: 将高效推理重构为信息瓶颈原则下的有损压缩问题，提出条件信息瓶颈（CIB）框架，其中推理轨迹Z作为计算桥梁，仅包含无法直接从提示X获取的关于响应Y的信息。这产生了一个通用的强化学习目标：在推理轨迹先验下最大化任务奖励同时压缩完成内容。

Result: CIB目标能有效修剪认知冗余同时保持流畅性和逻辑性，在适度压缩下提高准确性，在激进压缩下实现最小准确性下降。

Conclusion: 条件信息瓶颈框架为思维链推理压缩提供了理论基础，超越了简单的基于token计数的启发式方法，通过语义先验（语言模型惊奇度）更有效地平衡推理成本和准确性。
...
### [243] [MUSA-PINN: Multi-scale Weak-form Physics-Informed Neural Networks for Fluid Flow in Complex Geometries](https://arxiv.org/abs/2603.08465)
*Weizheng Zhang,Xunjie Xie,Hao Pan,Xiaowei Duan,Bingteng Sun,Qiang Du,Lin lu*

Main category: cs.LG

TL;DR: MUSA-PINN通过多尺度弱形式积分约束解决PINNs在复杂拓扑域中的收敛问题，显著提升TPMS几何中不可压缩流动的求解精度。


<details>
  <summary>Details</summary>
Motivation: 传统PINNs在复杂拓扑域（如三周期极小曲面）中使用逐点残差最小化时存在收敛问题，局部约束无法在曲折通道中传播全局信息，导致梯度不稳定和守恒律违反。

Method: 提出多尺度弱形式PINN（MUSA-PINN），将PDE约束重新表述为分层球形控制体积上的积分守恒律，通过控制表面的通量平衡残差强制连续性和动量守恒，采用三尺度子域策略和两阶段训练计划。

Result: 在TPMS几何中的稳态不可压缩流动实验中，MUSA-PINN优于现有基线方法，相对误差降低高达93%，并保持了质量守恒。

Conclusion: MUSA-PINN通过多尺度弱形式积分约束有效解决了PINNs在复杂拓扑域中的收敛问题，显著提升了求解精度和守恒特性。
...
### [244] [NN-OpInf: an operator inference approach using structure-preserving composable neural networks](https://arxiv.org/abs/2603.08488)
*Eric Parish,Anthony Gruber,Patrick Blonigan,Irina Tezaur*

Main category: cs.LG

TL;DR: NN-OpInf是一种非侵入式降阶建模框架，通过神经网络学习潜在动力学，保持算子结构特性，相比传统多项式OpInf在非线性问题上表现更好但训练成本更高。


<details>
  <summary>Details</summary>
Motivation: 传统多项式算子推断（P-OpInf）在处理非多项式非线性动力学时存在局限性，需要一种更灵活、结构保持且非侵入式的降阶建模方法。

Method: 提出神经网络算子推断（NN-OpInf）框架，从快照数据中学习潜在动力学，强制保持局部算子结构（如斜对称性、半正定性、梯度保持），支持异构算子的可加组合。

Result: 在多个非线性和参数化问题上的数值实验表明，相比P-OpInf和先前的NN-ROM方法，NN-OpInf在精度、稳定性和鲁棒性方面都有显著提升，特别是在动力学无法用多项式模型良好表示的情况下。

Conclusion: NN-OpInf可作为P-OpInf的有效替代方案，在处理非多项式非线性动力学时提供更高的精度和分布外性能，但代价是更高的训练计算成本和更困难、非凸的学习问题。
...
### [245] [Echo2ECG: Enhancing ECG Representations with Cardiac Morphology from Multi-View Echos](https://arxiv.org/abs/2603.08505)
*Michelle Espranita Liman,Özgün Turgut,Alexander Müller,Eimo Martens,Daniel Rueckert,Philip Müller*

Main category: cs.LG

TL;DR: Echo2ECG：通过多视角超声心动图增强心电图表征的多模态自监督学习框架，用于预测心脏形态学表型


<details>
  <summary>Details</summary>
Motivation: 心电图（ECG）无法直接测量心脏形态学表型（如左心室射血分数），这些通常需要超声心动图（Echo）。从ECG预测这些表型可以实现早期、可及的健康筛查。现有自监督方法存在表征不匹配问题，仅将ECG与单视角Echo对齐，后者只能捕捉局部、空间受限的解剖快照。

Method: 提出Echo2ECG多模态自监督学习框架，利用多视角超声心动图捕获的心脏形态结构来丰富心电图表征。将ECG作为特征提取器，在两个需要形态学信息的临床任务上进行评估：1）跨三个数据集的结枸性心脏表型分类；2）使用ECG查询检索具有相似形态特征的Echo研究。

Result: 提取的ECG表征在两个任务上始终优于最先进的单模态和多模态基线方法，尽管比最大基线小18倍。结果表明Echo2ECG是一个鲁棒、强大的ECG特征提取器。

Conclusion: Echo2ECG通过多视角超声心动图增强心电图表征，有效解决了现有方法的表征不匹配问题，为早期心脏健康筛查提供了更准确、可及的工具。
...
### [246] [Oracle-Guided Soft Shielding for Safe Move Prediction in Chess](https://arxiv.org/abs/2603.08506)
*Prajit T Rajendran,Fabio Arnez,Huascar Espinoza,Agnes Delaborde,Chokri Mraidha*

Main category: cs.LG

TL;DR: 提出OGSS框架，通过结合模仿学习和安全模型，在象棋等高风险环境中实现安全探索，显著降低失误率


<details>
  <summary>Details</summary>
Motivation: 在高风险环境中，纯模仿学习或强化学习难以避免安全关键错误。模仿学习样本效率高但脆弱，强化学习需要大量计算资源。需要一种既能安全探索又能保持性能的方法

Method: 提出Oracle-Guided Soft Shielding (OGSS)框架：1) 训练策略模型预测强棋步；2) 从Stockfish评估学习失误预测模型；3) 推理时生成候选棋步，结合策略模型的预测概率和失误概率的效用函数选择行动

Result: 在数百场与强象棋引擎的比赛中，OGSS变体在探索率提高数倍的情况下仍保持较低的失误率，比其他方法（动作剪枝、SafeDAgger、基于不确定性的采样）表现更好

Conclusion: OGSS框架能够在象棋等高风险环境中实现安全探索，在不牺牲战术稳健性的前提下支持更广泛的探索，平衡了性能与安全性
...
### [247] [Towards Effective and Efficient Graph Alignment without Supervision](https://arxiv.org/abs/2603.08526)
*Songyang Chen,Youfang Lin,Yu Liu,Shuai Zheng,Lei Zou*

Main category: cs.LG

TL;DR: 提出GlobAlign和GlobAlign-E方法，通过全局表示和最优传输进行无监督图对齐，在准确性和效率上均优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有无监督图对齐方法（如嵌入方法和最优传输方法）在准确性和效率之间存在权衡限制。作者观察到这些方法采用"局部表示、全局对齐"范式，导致对齐过程中的两个阶段不匹配

Method: 提出新的"全局表示和对齐"范式，开发GlobAlign方法，采用全局注意力机制和分层跨图传输成本来捕捉长距离和隐式节点依赖关系。GlobAlign-E进一步优化效率，将最优传输的立方复杂度降低到二次复杂度

Result: 实验表明，GlobAlign在准确性上比最佳竞争对手提升高达20%，而GlobAlign-E在效率上比现有最优传输方法快一个数量级，实现了最佳效率

Conclusion: 通过全局表示和对齐的新范式，GlobAlign系列方法在无监督图对齐任务中实现了准确性和效率的双重突破，解决了现有方法的局限性
...
### [248] [Drift-to-Action Controllers: Budgeted Interventions with Online Risk Certificates](https://arxiv.org/abs/2603.08578)
*Ismail Lamaakal,Chaymae Yahyati,Khalid El Makkaoui,Ibrahim Ouahbi,Yassine Maleh*

Main category: cs.LG

TL;DR: Drift2Act是一个漂移到行动的控制器，将监控视为具有明确安全性的约束决策，通过风险证书在漂移发生时自动选择适当的响应行动。


<details>
  <summary>Details</summary>
Motivation: 已部署的机器学习系统面临分布漂移问题，但大多数监控管道仅停留在警报阶段，在标签、计算和延迟约束下未能明确指定响应策略。

Method: Drift2Act结合感知层（将无标签监控信号映射到漂移类型信念）和主动风险证书（从最近窗口查询少量延迟标签，产生当前风险的任意时间有效上界U_t(δ)）。证书控制操作：如果U_t(δ)≤τ，选择低成本行动（如重新校准或测试时适应）；如果U_t(δ)>τ，激活弃权/移交，并在冷却期后升级到回滚或重新训练。

Result: 在具有标签延迟和明确干预成本的现实流协议中，Drift2Act在WILDS Camelyon17、DomainNet和受控合成漂移流上实现了接近零的安全违规和快速恢复，成本适中，优于仅警报监控、始终适应、基于计划的重新训练、单独选择性预测以及无证书的消融实验。

Conclusion: 在线风险认证实现了可靠的漂移响应，并将监控重新定义为具有安全性的决策制定。
...
### [249] [DualFlexKAN: Dual-stage Kolmogorov-Arnold Networks with Independent Function Control](https://arxiv.org/abs/2603.08583)
*Andrés Ortiz,Nicolás J. Gallego-Molina,Carmen Jiménez-Mesa,Juan M. Górriz,Javier Ramírez*

Main category: cs.LG

TL;DR: DFKAN是一种新型神经网络架构，通过解耦预线性输入变换和后线性输出激活的双阶段机制，解决了传统MLP固定激活函数和KAN参数爆炸的问题，实现了更好的表达能力与计算成本的平衡。


<details>
  <summary>Details</summary>
Motivation: 传统MLP使用固定的激活函数，限制了网络的表达能力，迫使网络通过增加深度和宽度来逼近复杂拓扑。KAN虽然通过边缘可学习函数解决了这个问题，但存在参数二次缩放和架构刚性问题，难以有效集成标准正则化技术。

Method: 提出DualFlexKAN（DFKAN）架构，采用双阶段机制独立控制预线性输入变换和后线性输出激活。这种解耦允许构建混合网络，优化表达能力与计算成本之间的权衡。支持多种基函数族（正交多项式、B样条、径向基函数），并与可配置的正则化策略集成以稳定训练动态。

Result: 在回归基准测试、物理信息任务和函数逼近方面，DFKAN在准确性、收敛速度和梯度保真度方面均优于MLP和传统KAN。混合配置以比标准KAN少1-2个数量级的参数实现了优越性能，有效缓解了参数爆炸问题，同时保持了KAN式的表达能力。

Conclusion: DFKAN提供了一个原则性、可扩展的框架，用于集成自适应非线性，特别有利于科学应用中的数据高效学习和可解释函数发现，在保持表达能力的同时显著减少了参数数量。
...
### [250] [Towards Batch-to-Streaming Deep Reinforcement Learning for Continuous Control](https://arxiv.org/abs/2603.08588)
*Riccardo De Monte,Matteo Cederle,Gian Antonio Susto*

Main category: cs.LG

TL;DR: 提出两种新颖的流式深度强化学习算法S2AC和SDAC，专为与最先进批量RL方法兼容而设计，特别适用于设备端微调应用如Sim2Real迁移。


<details>
  <summary>Details</summary>
Motivation: 现有深度RL方法虽然性能优异，但计算复杂度高，依赖回放缓冲区、批量更新和目标网络，不适用于资源受限硬件。流式深度RL通过纯在线更新解决了这一限制。

Method: 提出两种流式深度RL算法：Streaming Soft Actor-Critic (S2AC)和Streaming Deterministic Actor-Critic (SDAC)。这些算法设计为与最先进批量RL方法兼容，特别适合设备端微调。还研究了从批量学习到流式学习的实际挑战并提出了具体策略。

Result: 两种算法在标准基准测试中实现了与最先进流式基线相当的性能，且无需繁琐的超参数调优。成功解决了从批量到流式学习的过渡挑战。

Conclusion: 提出的S2AC和SDAC算法为资源受限硬件上的深度RL应用提供了有效的流式解决方案，特别适用于需要设备端微调的Sim2Real迁移等应用场景。
...
### [251] [Don't Look Back in Anger: MAGIC Net for Streaming Continual Learning with Temporal Dependence](https://arxiv.org/abs/2603.08600)
*Federico Giannini,Sandro D'Andrea,Emanuele Della Valle*

Main category: cs.LG

TL;DR: MAGIC Net是一个新颖的流式持续学习方法，通过集成持续学习的架构策略和循环神经网络来处理时间依赖性，实现在线学习和推理


<details>
  <summary>Details</summary>
Motivation: 概念漂移、时间依赖性和灾难性遗忘是数据流学习中的主要挑战。虽然流式机器学习和持续学习分别处理这些问题，但流式持续学习试图统一它们

Method: MAGIC Net集成了持续学习的架构策略和循环神经网络来处理时间依赖性。它持续学习，通过可学习掩码回顾过去知识，必要时扩展架构，所有操作都在线进行

Result: 在合成和真实世界数据流上的实验表明，MAGIC Net能改善对新概念的适应，限制内存使用，并减轻遗忘问题

Conclusion: MAGIC Net是一个有效的流式持续学习方法，能够处理时间依赖性，实现在线学习，并在适应新概念、内存使用和减轻遗忘方面表现良好
...
### [252] [Integral Formulas for Vector Spherical Tensor Products](https://arxiv.org/abs/2603.08630)
*Valentin Heyraud,Zachary Weller-Davies,Jules Tilly*

Main category: cs.LG

TL;DR: 本文推导了简化向量球张量积的积分公式，得到了反对称Gaunt系数的闭式表达式，实现了9倍的计算量减少，并探讨了在SO(3)-等变神经网络中的应用。


<details>
  <summary>Details</summary>
Motivation: Xie等人最近引入的向量球张量积将Gaunt张量积推广到反对称耦合，但需要更高效的计算方法。本文旨在简化该张量积的计算，使其在SO(3)-等变神经网络中更实用。

Method: 推导积分公式简化向量球张量积，获得反对称Gaunt系数的显式闭式表达式，通过单一向量球张量积模拟Clebsch-Gordan张量积，并研究张量积归一化的低秩分解。

Result: 实现了9倍的张量积计算量减少，使向量球张量积能够高效实用地实现，为SO(3)-等变神经网络中的应用铺平道路，并提供了控制表达力-运行时间权衡的方法。

Conclusion: 本文成功简化了向量球张量积的计算，显著提升了效率，为SO(3)-等变神经网络中的广泛应用提供了理论基础和实用工具，同时探讨了表达力与计算效率的平衡策略。
...
### [253] [Grow, Don't Overwrite: Fine-tuning Without Forgetting](https://arxiv.org/abs/2603.08647)
*Dyah Adila,Hanna Mazzawi,Benoit Dherin,Xavier Gonzalvo*

Main category: cs.LG

TL;DR: 提出一种函数保持扩展方法，通过复制预训练参数并应用缩放校正，在初始化时保持数学等价性，从而在适应新任务时避免灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 预训练模型适应专门任务时常导致灾难性遗忘，现有方法要么在新任务上性能妥协，要么难以平衡训练稳定性与预训练知识的有效重用。

Method: 通过复制transformer子模块中的预训练参数并应用缩放校正，确保扩展模型在初始化时与原始模型数学等价，实现稳定训练并充分利用现有知识。

Result: 该方法消除了可塑性与稳定性之间的权衡，在下游任务上匹配全微调性能，同时不损害模型原始能力；通过选择性扩展少量层，能以更低计算成本达到相同性能。

Conclusion: 提出的函数保持扩展方法解决了灾难性遗忘问题，实现了新任务适应与原始能力保持的平衡，具有模块化和计算效率优势。
...
### [254] [Divide and Predict: An Architecture for Input Space Partitioning and Enhanced Accuracy](https://arxiv.org/abs/2603.08649)
...
<details>
  <summary>Details</summary>
Motivation: 需要一种内在的、基于数据本身的方法来量化训练数据的异质性，以识别样本是否为多个分布的混合，这对于提高模型性能至关重要。

Method: 开发一种基于训练点对影响随机变量方差的度量方法，证明数据本身包含支持分块划分的关键信息，并通过方差最大化分析确定最佳混合比例。

Result: 方差能够有效捕捉数据异质性，在等比例混合分布时达到最大值；基于方差的数据净化后按块进行常规训练，能显著提高测试准确率。

Conclusion: 提出的方差度量是量化数据异质性的有效工具，可用于识别混合分布并进行数据净化，从而通过分块训练策略显著提升模型性能。
...
### [255] [Group Entropies and Mirror Duality: A Class of Flexible Mirror Descent Updates for Machine Learning](https://arxiv.org/abs/2603.08651)
...
<details>
  <summary>Details</summary>
Motivation: 现有镜像下降算法受限于特定的熵函数（如Shannon熵），缺乏对不同数据几何和统计分布的适应性。需要更通用的框架来扩展优化算法的灵活性。

Method: 利用群熵（由群组合律控制的广义熵泛函）构建群理论镜像映射，通过多参数广义对数及其逆（群指数）实现灵活的MD更新。引入"镜像对偶性"概念，可在镜像函数与其逆之间切换。

Result: 建立了无限灵活的镜像下降算法家族，能够适应多样数据几何和统计分布。在大规模单纯形约束二次规划问题上验证了方法的有效性、鲁棒性和性能。
...
### [256] [Context-free Self-Conditioned GAN for Trajectory Forecasting](https://arxiv.org/abs/2603.08658)
*Tiago Rodrigues de Almeida,Eduardo Gutierrez Maestro,Oscar Martinez Mozos*

Main category: cs.LG

TL;DR: 提出基于自条件GAN的无监督上下文无关方法，从2D轨迹中学习不同模式，应用于轨迹预测任务，在人类运动和道路代理数据集上表现良好


<details>
  <summary>Details</summary>
Motivation: 传统轨迹预测方法通常需要上下文信息或监督标签，本文旨在开发一种无需上下文信息的无监督方法，能够从2D轨迹中自动学习不同的行为移动模式

Method: 基于自条件GAN的无监督框架，在判别器特征空间中识别不同行为模式，提出了三种不同的自条件GAN训练设置来改进预测性能

Result: 在人类运动和道路代理两个数据集上的实验表明，该方法在监督标签最少的情况下优于之前的上下文无关方法，在人类运动数据上表现尤其突出

Conclusion: 自条件GAN的无监督方法能够有效学习轨迹中的行为模式，在轨迹预测任务中表现优异，特别是在监督信息有限的情况下具有优势
...
### [257] [How Far Can Unsupervised RLVR Scale LLM Training?](https://arxiv.org/abs/2603.08660)
...
TL;DR: 本文对无监督强化学习与可验证奖励（URLVR）进行了全面分析，发现内在奖励方法存在收敛到模型初始分布的尖锐化机制，在置信度与正确性对齐时有效，但错位时会灾难性失败，而外部奖励方法可能突破这一限制。


<details>
  <summary>Details</summary>
Motivation: URLVR通过无需真实标签的奖励来扩展LLM训练，但现有基于模型内在信号的方法潜力与局限尚不明确。本文旨在系统分析URLVR方法，揭示其理论机制与实用边界。

Method: 首先将URLVR方法按奖励来源分为内在与外部两类；建立统一理论框架分析内在方法的收敛行为；通过系统实验验证理论发现；提出模型崩溃步数作为RL可训练性的实用指标；探索基于计算不对称性的外部奖励方法。

Result: 内在奖励方法普遍呈现先升后降的模式，崩溃时机由模型先验而非工程选择决定；内在奖励在小数据集测试时训练中仍有价值；外部奖励方法初步显示可能突破置信度-正确性天花板。

Conclusion: 内在URLVR方法存在根本性限制，其收敛机制导致在置信度与正确性错位时失败；模型崩溃步数可作为实用指标；外部奖励方法为可扩展替代方案提供了有希望的路径。
...
### [258] [A New Lower Bound for the Random Offerer Mechanism in Bilateral Trade using AI-Guided Evolutionary Search](https://arxiv.org/abs/2603.08679)
*Yang Cai,Vineet Gupta,Zun Li,Aranyak Mehta*

Main category: cs.LG

TL;DR: 使用AI进化搜索发现双边贸易中Random-Offerer机制相对于最优效率的近似比下界提升至2.0749，打破了之前2.02的纪录


<details>
  <summary>Details</summary>
Motivation: Myerson-Satterthwaite定理表明双边贸易中无法同时实现完全效率、贝叶斯激励相容和预算平衡。因此研究BIC和BB机制能达到的最优效率近似程度很重要。Random-Offerer机制的近似比上界为2的猜想被推翻后，需要探索其实际最坏情况性能

Method: 使用AlphaEvolve框架进行AI引导的进化搜索，在价值分布空间中寻找最坏情况实例。这是一种基于人工智能的进化算法方法

Result: 发现了一个新的最坏情况实例，将Random-Offerer机制相对于最优效率的近似比下界提升到2.0749，超过了之前2.02的纪录

Conclusion: Random-Offerer机制的最坏情况性能比之前已知的更差，效率差距更大。这为双边贸易机制设计提供了新的理论边界，并展示了AI引导搜索在经济学理论问题中的应用价值
...
### [259] [Split Federated Learning Architectures for High-Accuracy and Low-Delay Model Training](https://arxiv.org/abs/2603.08687)
*Yiannis Papageorgiou,Yannis Thomas,Ramin Khalili,Iordanis Koutsopoulos*

Main category: cs.LG

TL;DR: 本文提出了一种针对分层联邦分割学习（HSFL）的联合优化方法，通过优化模型分割位置和客户端-聚合器分配，在提高准确率的同时减少训练延迟和通信开销。


<details>
  <summary>Details</summary>
Motivation: 现有的分层联邦分割学习（HSFL）架构虽然采用三层训练结构（客户端、本地聚合器、中央服务器），但忽视了模型分割位置和客户端-聚合器分配对准确率、延迟和开销的影响。本文旨在解决这一关键问题。

Method: 1. 明确建模分割位置和客户端-聚合器分配对准确率、延迟和开销的影响；2. 将问题形式化为联合优化问题；3. 证明该问题是NP-hard；4. 提出首个准确率感知的启发式算法，在保证延迟效率的同时考虑模型准确率。

Result: 在公开数据集上的仿真结果表明，相比最先进的SFL和HSFL方案，本文方法能够提高准确率3%，同时减少延迟20%和开销50%。

Conclusion: 本文首次证明了在分层联邦分割学习中，通过联合优化模型分割位置和客户端-聚合器分配，可以同时提高准确率、减少延迟和降低通信开销，为HSFL架构设计提供了新的优化方向。
...
### [260] [Impermanent: A Live Benchmark for Temporal Generalization in Time Series Forecasting](https://arxiv.org/abs/2603.08707)
...
TL;DR: Impermanent是一个用于时间序列预测的实时基准测试，通过持续更新的数据流进行顺序评分，评估模型在开放世界时间变化下的表现，而非静态测试集的一次性准确性。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列预测基准测试使用静态训练-测试分割，容易导致数据污染（模型可能在测试数据上训练或使用测试分数进行模型选择），从而夸大性能。需要评估模型在真实世界时间变化下的泛化能力。

Method: 构建基于GitHub开源活动的实时基准测试，关注前400个星标最多的仓库，从问题开启、拉取请求、推送事件和新星标者构建时间序列，采用滚动窗口和每日更新进行顺序评估。

Result: Impermanent提供了一个自然实时且高度非平稳的数据集，受版本发布、贡献者行为变化、平台/工具变更和外部事件影响，通过标准化协议和排行榜支持可重复的持续比较。

Conclusion: 通过将评估从静态准确性转向持续性能，Impermanent朝着评估时间序列预测中基础模型泛化能力何时以及是否能够被有意义地声称迈出了具体一步。
...
### [261] [CREDO: Epistemic-Aware Conformalized Credal Envelopes for Regression](https://arxiv.org/abs/2603.06826)
*Luben M. C. Cabezas,Sabina J. Sloman,Bruno M. Resende,Fanyi Wu,Michele Caprio,Rafael Izbicki*

Main category: stat.ML

TL;DR: CREDO：结合可信集方法与保形预测，提供可解释的预测区间，同时保证边际覆盖并显式表示认知不确定性


<details>
  <summary>Details</summary>
Motivation: 传统保形预测能提供分布无关的覆盖保证，但其区间在模型外推区域可能显得过于自信，因为标准保形分数不显式表示认知不确定性。而可信集方法虽能通过一组可信预测分布使认知效应可见，但通常基于模型且缺乏校准保证。

Method: 提出CREDO方法，采用"先可信化后保形化"的两步策略：首先构建可解释的可信包络，在局部证据弱时扩大；然后在其上应用分割保形校准，保证边际覆盖。基于修剪极端后验预测端点实现快速计算。

Result: CREDO在基准回归任务中保持目标覆盖率的同时，提高了稀疏自适应性，且计算效率有竞争力。预测区间宽度可分解为随机噪声、认知膨胀和分布无关的校准松弛三部分。

Conclusion: CREDO成功结合了可信集方法与保形预测的优势，提供既保证校准覆盖又显式表示认知不确定性的可解释预测区间，实现了两种方法的优势互补。
...
### [262] [Bilateral Trade Under Heavy-Tailed Valuations: Minimax Regret with Infinite Variance](https://arxiv.org/abs/2603.06851)
*Hangyi Zhao*

Main category: stat.ML

TL;DR: 研究具有有界密度但无限方差的估值下，上下文双边贸易的全反馈问题，建立了精确的极小极大遗憾率


<details>
  <summary>Details</summary>
Motivation: 传统双边贸易研究通常假设估值方差有限，本文研究估值具有有界密度但无限方差（即p∈(1,2)阶矩有限）的更现实情况

Method: 1. 将Bachoc等人的自界性质从有界估值扩展到实值估值；2. 结合截断均值估计；3. 使用基于周期的算法；4. 通过Assouad方法和平滑矩匹配构造建立下界

Result: 证明了基于周期的算法能达到遗憾率$\widetilde{O}(T^{1-2β(p-1)/(βp + d(p-1))})$，并通过下界证明这是精确的极小极大率，在p=2时退化为经典非参数率，p→1⁺时退化为平凡线性率

Conclusion: 本文完整刻画了具有有界密度但无限方差估值的上下文双边贸易问题的精确极小极大遗憾率，填补了传统有界方差假设与更现实无限方差情况之间的理论空白
...
### [263] [Fairness May Backfire: When Leveling-Down Occurs in Fair Machine Learning](https://arxiv.org/abs/2603.06901)
*Yi Yang,Xiangyu Chang,Pei-yu Chen*

Main category: stat.ML

TL;DR: 论文研究了公平性约束在机器学习分类中的群体影响，发现在属性感知部署中公平性必然改善弱势群体结果，而在属性盲部署中影响取决于数据分布，可能导致"拉平下降"现象。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习系统在信贷、就业等关键领域应用日益广泛，算法公平性成为核心关切。然而，当前不清楚实施公平性约束是真正改善了受影响群体的结果，还是导致了"拉平下降"（使一个或两个群体都变得更差）。

Method: 采用统一的、群体层面的贝叶斯框架分析二元分类中的公平性概念。该方法具有分布无关性和算法无关性，隔离了公平性要求的内在效应与有限样本噪声、训练和干预细节的影响。分析了两种部署机制：属性感知决策（敏感属性在决策时可用）和属性盲决策（敏感属性被排除在预测之外）。

Result: 在属性感知机制中，公平ML必然（弱）改善弱势群体的结果，同时（弱）恶化优势群体的结果。在属性盲机制中，公平性的影响取决于数据分布：公平性可能使任一群体受益或受损，甚至可能使两个群体的结果向同一方向变化，导致"拉平上升"或"拉平下降"。研究还识别了驱动这些模式的"掩蔽"候选人的作用。

Conclusion: 研究结果为何时追求算法公平性可能改善群体结果、何时可能导致系统性拉平下降提供了结构性指导，为公平ML的设计和部署选择提供了信息。属性感知部署能确保弱势群体受益，而属性盲部署的效果则不确定，需要根据具体数据分布评估。
...
### [264] [Post-Training with Policy Gradients: Optimality and the Base Model Barrier](https://arxiv.org/abs/2603.06957)
*Alireza Mousavi-Hosseini,Murat A. Erdogdu*

Main category: stat.ML

TL;DR: 研究后训练线性自回归模型，分析结果奖励和过程奖励下的策略梯度方法性能，发现基模型支持集外的学习存在指数级查询代价，提出过程奖励可避免维度灾难。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过后训练提升自回归模型的序列生成质量，特别关注在基模型支持集内外的学习效率差异，探索克服维度灾难的方法。

Method: 使用策略梯度变体进行后训练，分析结果奖励和过程奖励两种设置。在统计学习中使用自适应学习率的SGD，在线学习中使用自适应学习率的策略梯度。

Result: 在基模型支持集内，策略梯度可达到接近最优的奖励查询次数；但在支持集外，需要指数级查询次数。过程奖励模型可避免维度灾难，依赖词元级似然分位数。

Conclusion: 后训练存在基模型支持集的限制，过程奖励是克服维度灾难的关键。自适应学习率方法在统计和在线学习中都能达到接近最优的性能。
...
### [265] [Masked Unfairness: Hiding Causality within Zero ATE](https://arxiv.org/abs/2603.06984)
*Zou Yang,Sophia Xiao,Bijan Mazaheri*

Main category: stat.ML

TL;DR: 论文指出基于平均处理效应（ATE）的公平性监管存在"因果掩盖问题"，优化者可在保持零ATE的同时实现不公平，主张应在模型层面而非决策层面监管公平性。


<details>
  <summary>Details</summary>
Motivation: 现有基于因果理论的公平性量化框架主要关注平均处理效应（ATE），但作者担心仅基于平均值的监管可能存在问题。他们发现优化者可以在保持零ATE（即受保护属性与决策之间的ATE为零）的同时，通过"因果掩盖"实现不公平的决策，从而规避监管。

Method: 作者将"因果掩盖问题"表述为一个线性规划问题，该问题在保持零ATE约束下优化替代目标（如最大化利润或最小化犯罪）。通过研究因果掩盖的能力和局限性，分析优化者在ATE监管下可能诱导的不平等待遇。

Result: 研究表明：1）在ATE监管下的优化可能导致显著的不平等待遇；2）真实公平性与因果掩盖公平性之间的差异由混杂因素驱动，强调了全面条件独立性测试的重要性；3）因果掩盖解决方案在统计和信息论上难以检测，可能长期存在。

Conclusion: 基于平均处理效应的公平性监管存在根本缺陷，优化者可通过因果掩盖规避监管。因此，必须在模型层面而非决策层面监管公平性，以确保真正的公平性。
...
### [266] [Deep Generative Spatiotemporal Engression for Probabilistic Forecasting of Epidemics](https://arxiv.org/abs/2603.07108)
*Rajdeep Pathak,Tanujit Chakraborty*

Main category: stat.ML

TL;DR: 提出深度时空engression方法，用于低频流行病数据集的准确可靠概率预测，通过预加性噪声组件内生量化不确定性，在多个数据集上优于基准方法。


<details>
  <summary>Details</summary>
Motivation: 流行病发病率预测对公共卫生准备至关重要，但由于复杂的非线性时间依赖性和异质性空间交互，传统点预测在不确定性分配上不可靠，需要概率预测来提供最佳或最坏情况场景。

Method: 提出深度时空engression方法，作为分布透镜，通过从训练模型中采样生成样本外概率预测。框架包含轻量级深度生成架构，通过预加性噪声组件内生量化不确定性。

Result: 在三个预测时间范围内对六个流行病学数据集的综合评估表明，该方法在点预测和概率预测方面均优于多个时空基准方法。建立了时空engression过程在温和假设下的几何遍历性和渐近平稳性。

Conclusion: 深度时空engression方法能够为低频流行病数据集生成准确可靠的概率预测，通过内生不确定性量化和模型可解释性增强，为及时公共卫生干预提供实用工具。
...
### [267] [Probabilistic Inference and Learning with Stein's Method](https://arxiv.org/abs/2603.07467)
*Qiang Liu,Lester Mackey,Chris Oates*

Main category: stat.ML

TL;DR: 该专著系统介绍了基于Stein方法的概率推断与学习的理论与方法，包括Stein差异的构造、性质及其与Stein变分梯度下降的联系。


<details>
  <summary>Details</summary>
Motivation: 为概率推断和机器学习领域提供Stein方法的理论基础和实用工具，填补该领域系统化理论框架的空白。

Method: 从Stein算子和Stein集构造Stein差异，分析其可计算性、分离性、收敛检测和控制等性质，并详细阐述Stein算子与Stein变分梯度下降的联系。

Result: 提供了完整的理论框架和方法论，包括精确的定义、定理陈述和证明参考，为相关研究提供了系统化的工具集。

Conclusion: 该专著建立了Stein方法在概率推断和学习中的完整理论体系，为相关算法设计和分析提供了坚实的数学基础。
...
### [268] [Beyond Data Splitting: Full-Data Conformal Prediction by Differential Privacy](https://arxiv.org/abs/2603.07522)
*Young Hyun Cho,Jordan Awan*

Main category: stat.ML

TL;DR: 提出无需数据分割的隐私保护共形预测框架，利用差分隐私的稳定性控制样本内外分数差距，通过保守的私有分位数算法防止覆盖不足，实验显示比分割基线更优


<details>
  <summary>Details</summary>
Motivation: 隐私保护和不确定性量化在数据驱动决策中日益重要。现有私有共形预测方法通常依赖数据分割，这会减少有效样本量，影响预测性能。

Method: 提出全数据隐私保护共形预测框架，避免数据分割。利用差分隐私的稳定性控制样本内外共形分数的差距，配合保守的私有分位数算法防止覆盖不足。提供机制特定的稳定性分析以实现渐近恢复名义覆盖水平。

Result: 通用差分隐私保证提供覆盖下限，但通常无法恢复名义1-α水平。通过机制特定的稳定性分析可实现渐近恢复名义水平。实验表明比基于分割的私有基线产生更尖锐的预测集。

Conclusion: 提出的全数据隐私保护共形预测框架避免了数据分割的样本浪费问题，利用差分隐私的稳定性特性，配合保守的私有分位数算法，在保护隐私的同时提供更优的预测性能。
...
### [269] [An Interpretable Generative Framework for Anomaly Detection in High-Dimensional Financial Time Series](https://arxiv.org/abs/2603.07864)
*Waldyn G Martinez*

Main category: stat.ML

TL;DR: ReGEN-TAD是一个可解释的生成框架，结合机器学习和计量经济学诊断，用于检测高维金融时间序列中的结构不稳定性和异常，无需标注数据。


<details>
  <summary>Details</summary>
Motivation: 高维金融时间序列具有复杂的时间依赖性和演变的横截面结构，这使得检测结构不稳定性和异常变得困难。传统方法难以有效处理这些挑战。

Method: 提出ReGEN-TAD框架，结合联合预测和重构，采用精炼的卷积-Transformer架构。通过聚合预测不一致性、重构退化、潜在失真和波动率变化等互补信号，并使用鲁棒校准生成统一的异常分数。

Result: 在合成和金融面板数据上的实验表明，该方法对结构化偏差具有改进的鲁棒性，同时能够实现经济上一致的因素级归因。

Conclusion: ReGEN-TAD提供了一个可解释的生成框架，能够有效检测高维金融时间序列中的异常，结合了机器学习的预测能力和计量经济学的诊断能力，无需依赖标注数据。
...
### [270] [Robust Transfer Learning with Side Information](https://arxiv.org/abs/2603.07921)
*Akram S. Awad,Shihab Ahmed,Yue Wang,George K. Atia*

Main category: stat.ML

TL;DR: 提出一种基于估计中心不确定性集的鲁棒MDP框架，通过结合有限目标样本和源-目标动态的边信息来构建更紧的不确定性集，从而在环境转移下获得更好的目标域策略。


<details>
  <summary>Details</summary>
Motivation: 传统分布鲁棒优化方法在面对大的环境转移时需要扩大不确定性集，导致策略过于保守和悲观。需要一种方法能够在环境转移下获得更好的目标域性能。

Method: 提出估计中心不确定性集框架，通过约束估计将有限目标样本与源-目标动态的边信息（特征矩边界、分布距离、密度比）相结合，得到改进的转移核估计和更紧的不确定性集。

Result: 建立了鲁棒和非鲁棒值函数的误差界和收敛结果，提供了学习鲁棒策略的有限样本保证，分析了鲁棒次优性间隙。在低维结构下，边信息能减少次优性间隙并提高样本效率。

Conclusion: 该方法在OpenAI Gym环境和经典控制问题中表现优于现有鲁棒和非鲁棒基线，通过边信息构建更紧的不确定性集，在环境转移下实现了更好的目标域性能。
...
### [271] [Local Constrained Bayesian Optimization](https://arxiv.org/abs/2603.07965)
*Jing Jingzhe,Fan Zheyi,Szu Hui Ng,Qingpei Hu*

Main category: stat.ML

TL;DR: 提出LCBO框架解决高维约束贝叶斯优化问题，通过局部下降和不确定性探索交替策略，理论证明KKT残差收敛率与维度呈多项式关系而非指数关系。


<details>
  <summary>Details</summary>
Motivation: 高维约束贝叶斯优化面临维度诅咒的挑战，现有方法如信赖域方法在处理紧约束或复杂约束时容易过早收缩，需要更有效的解决方案。

Method: 提出局部约束贝叶斯优化(LCBO)框架，利用约束惩罚代理模型的可微性，交替进行快速局部下降和不确定性驱动的探索。

Result: 理论证明LCBO在常见核函数下获得KKT残差的收敛率与维度呈多项式关系；在高达100维的基准测试中，LCBO持续优于现有最优方法。

Conclusion: LCBO为高维约束优化问题提供了有效的解决方案，克服了传统方法的局限性，在理论和实验上都表现出优越性能。
...
### [272] [Beyond ReinMax: Low-Variance Gradient Estimators for Discrete Latent Variables](https://arxiv.org/abs/2603.08257)
*Daniel Wang,Thang D. Bui*

Main category: stat.ML

TL;DR: 本文提出了ReinMax-Rao和ReinMax-CV两种梯度估计器，通过Rao-Blackwellisation和控制变量技术降低ReinMax的方差，在离散隐变量VAE训练中表现优异，并从数值积分角度提供了ReinMax的替代视角。


<details>
  <summary>Details</summary>
Motivation: 离散隐变量机器学习模型需要高效的梯度估计器进行反向传播。ReinMax作为Straight-Through家族的最新成员，虽然通过Heun方法减少了偏差，但存在高方差问题，需要改进。

Method: 提出了两种新梯度估计器：1) ReinMax-Rao：将Rao-Blackwellisation技术融入ReinMax以减少方差；2) ReinMax-CV：将控制变量技术融入ReinMax以减少方差。同时从数值积分角度重新审视ReinMax，探索利用其他数值方法构建更精确的梯度近似。

Result: 新提出的ReinMax-Rao和ReinMax-CV估计器在训练具有离散隐空间的变分自编码器时表现出优越性能，有效降低了ReinMax的高方差问题。

Conclusion: 通过结合Rao-Blackwellisation和控制变量技术，成功改进了ReinMax梯度估计器，降低了方差并提升了离散隐变量模型的训练效果，同时为ReinMax提供了数值积分的新视角。
...
### [273] [Posterior Sampling Reinforcement Learning with Gaussian Processes for Continuous Control: Sublinear Regret Bounds for Unbounded State Spaces](https://arxiv.org/abs/2603.08287)
*Hamish Flynn,Joe Watson,Ingmar Posner,Jan Peters*

Main category: stat.ML

TL;DR: 本文分析了高斯过程后验采样强化学习（GP-PSRL）的贝叶斯遗憾，通过递归应用Borell-Tsirelson-Ibragimov-Sudakov不等式和控制链方法，获得了紧致的遗憾界，解决了先前理论工作的局限性。


<details>
  <summary>Details</summary>
Motivation: 后验采样是处理不确定性决策的有效启发式方法，已成功应用于各种连续控制问题。然而，GP-PSRL的理论研究有限：现有遗憾界要么未能达到对最大信息增益的紧致依赖，要么未能正确处理状态空间无界的问题。本文旨在解决这些理论缺陷。

Method: 1. 递归应用Borell-Tsirelson-Ibragimov-Sudakov不等式，证明算法实际访问的状态以高概率包含在近常数半径的球内；2. 使用链式方法控制GP-PSRL的遗憾，获得紧致的依赖关系。

Result: 获得了贝叶斯遗憾界为$\widetilde{\mathcal{O}}(H^{3/2}\sqrt{γ_{T/H} T})$，其中$H$是时间步长，$T$是总时间步数，$γ_{T/H}$是最大信息增益。这个结果解决了先前PSRL理论工作的局限性。

Conclusion: 本文为PSRL在复杂环境中的分析提供了理论基础和工具，通过新的技术方法解决了无界状态空间和最大信息增益紧致依赖的问题，完善了后验采样强化学习的理论框架。
...
### [274] [Unifying On- and Off-Policy Variance Reduction Methods](https://arxiv.org/abs/2603.08370)
*Olivier Jeunen*

Main category: stat.ML

TL;DR: 本文建立了在线A/B测试与离策略评估之间的正式等价关系，证明差异均值估计器与最优逆倾向评分估计器在数学上相同，回归调整方法与双重稳健估计在结构上等价。


<details>
  <summary>Details</summary>
Motivation: 在线A/B测试和离策略评估虽然目标相同（估计处理的增量价值），但通常使用不同的术语和统计工具，各自独立发展。本文旨在弥合这一鸿沟，建立两者之间的正式等价关系。

Method: 通过数学证明建立等价关系：证明标准在线差异均值估计器与配备最优（方差最小化）加性控制变量的离策略逆倾向评分估计器在数学上相同。进一步证明回归调整方法（如CUPED、CUPAC、ML-RATE）与双重稳健估计在结构上等价。

Result: 成功建立了在线实验与离策略评估之间的正式等价关系，揭示了看似不同的方法在数学和结构上的统一性。这一统一视角扩展了对常用方法的理解。

Conclusion: 本文弥合了在线A/B测试和离策略评估之间的鸿沟，为从业者和研究人员提供了统一的视角，有助于指导两类问题的实践和研究工作。
...
### [275] [Generative Adversarial Regression (GAR): Learning Conditional Risk Scenarios](https://arxiv.org/abs/2603.08553)
*Saeed Asadi,Jonathan Yu-Meng Li*

Main category: stat.ML

TL;DR: 提出生成对抗回归(GAR)框架，通过生成器学习条件风险场景，使其与下游风险目标对齐，特别适用于可引出函数如分位数和期望分位数。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以生成与特定风险目标对齐的条件风险场景，特别是在金融风险管理中需要准确模拟尾部风险等场景。需要一种能够确保生成场景与下游风险评估保持一致的生成模型。

Method: 基于可引出函数（如分位数、期望分位数）的回归特性，将点预测扩展到生成建模。采用极小极大化公式，训练生成器使其策略诱导的风险与真实数据在相同上下文下的风险匹配，同时使用对抗策略识别最坏情况下的风险评估差异。

Result: 在S&P 500数据上的实验表明，GAR生成的场景比无条件、计量经济学和直接预测基线方法更好地保持了下游风险特性，同时在对抗选择的策略下保持稳定。

Conclusion: GAR提供了一种有效的框架，用于学习与特定风险目标对齐的条件风险场景，特别适用于金融风险管理中的尾部风险建模，能够生成更准确、更稳健的风险场景。
...
### [276] [Momentum SVGD-EM for Accelerated Maximum Marginal Likelihood Estimation](https://arxiv.org/abs/2603.08676)
*Adam Rozzio,Rafael Athanasiades,O. Deniz Akyildiz*

Main category: stat.ML

TL;DR: 提出了一种基于动量加速的SVGD-EM方法，通过在参数更新和概率测度空间中引入Nesterov加速，显著提高了最大边际似然估计的收敛速度。


<details>
  <summary>Details</summary>
Motivation: 虽然已有工作将EM算法解释为在模型参数和概率测度联合空间上的坐标下降方法，并开发了基于粒子交互的MMLE算法，但这些方法在收敛速度方面仍有改进空间。本文旨在通过引入动量加速机制来提升基于SVGD的EM算法的收敛效率。

Method: 提出了Momentum SVGD-EM方法，在Stein变分梯度下降（SVGD）框架中同时引入Nesterov动量加速：1）在模型参数更新中应用动量加速；2）在概率测度空间中也应用动量加速。这种方法保持了SVGD-EM的粒子交互特性，但通过动量项加速了收敛过程。

Result: 实验表明，Momentum SVGD-EM在各种难度递增的任务中都能显著加速收敛，减少了所需的迭代次数。该方法在低维和高维设置下都表现出有效性，证明了动量加速在参数空间和概率测度空间中的双重优势。

Conclusion: 通过在SVGD-EM框架中引入Nesterov动量加速，本文提出的方法能够显著提升最大边际似然估计的收敛速度。这种双重动量机制（参数空间和概率测度空间）为加速基于粒子的优化算法提供了有效途径，在理论和实际应用上都具有重要意义。
...
### [277] [Structural Causal Bottleneck Models](https://arxiv.org/abs/2603.08682)
*Simon Bing,Jonas Wahl,Jakob Runge*

Main category: stat.ML

TL;DR: 提出结构因果瓶颈模型（SCBMs），假设高维变量间的因果效应仅依赖于原因的低维摘要统计量（瓶颈），为任务特定降维提供灵活框架，可通过标准学习算法估计。


<details>
  <summary>Details</summary>
Motivation: 现有因果降维框架如因果表示学习或因果抽象学习存在局限性，需要一种更灵活的框架来处理高维变量间的因果效应，同时实现任务特定的降维。

Method: 引入结构因果瓶颈模型（SCBMs），假设因果效应仅依赖于原因的低维摘要统计量（瓶颈），提供可估计的框架，并与Tishby & Zaslavsky（2015）的信息瓶颈理论建立联系。

Result: 分析了SCBMs的可识别性，展示了如何通过实验估计它们，并证明了瓶颈在低样本迁移学习场景中对效应估计的益处。

Conclusion: SCBMs为现有因果降维框架提供了有前景的替代方案，能够灵活处理高维因果效应，同时通过标准算法实现可估计性。
