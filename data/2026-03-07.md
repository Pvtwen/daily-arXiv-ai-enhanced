...
TL;DR: 提出基于分段波导夹持天线系统(SWAN)的三重混合波束成形架构，联合优化数字、模拟和夹持波束成形，用于上行多用户MIMO通信，包含全连接和部分连接两种结构。


<details>
  <summary>Details</summary>
Motivation: 传统混合波束成形和夹持天线系统在性能和能效方面存在局限，需要一种新的架构来同时优化数字、模拟和夹持波束成形，以提升多用户MIMO通信的上行性能。

Method: 1) 全连接结构：采用WMMSE和ZF方法优化三重混合波束成形，数字部分用闭式解，模拟部分用黎曼流形优化，夹持部分用高斯-赛德尔搜索；2) 部分连接结构：提出交错拓扑，采用WMMSE设计，用逐元素相位校准替代黎曼流形优化；3) 推导两种结构的速率缩放规律。

Result: 1) SWAN三重混合波束成形在两种结构下均优于传统混合波束成形和传统夹持天线系统；2) 部分连接结构在段数较多时能在和速率与能耗间取得良好平衡；3) 可达速率不一定随段数增加而增加。

Conclusion: 提出的SWAN三重混合波束成形架构有效提升了上行多用户MIMO性能，部分连接结构在能效方面具有优势，为大规模天线系统设计提供了新思路。
...
### [2] [Near-Optimal Low-Complexity MIMO Detection via Structured Reduced-Search Enumeration](https://arxiv.org/abs/2603.05441)
*Logeshwaran Vijayan*

Main category: eess.SP

TL;DR: 提出一种结构化降维搜索策略，在8×8 MIMO系统内实现接近最大似然检测的性能，复杂度与星座大小呈线性关系


<details>
  <summary>Details</summary>
Motivation: 高阶MIMO系统中最大似然检测的计算复杂度随发射层数和星座大小呈指数增长，在实际系统中难以实现

Method: 采用结构化降维搜索策略，通过构建候选列表（3|X|、4|X|、8|X|分别对应3×3、4×4、8×8系统）来近似ML检测

Result: 在i.i.d.瑞利衰落信道下，该方法能接近ML检测性能，即使在高信道条件数下也能保持良好表现

Conclusion: 结构化降维搜索策略为实际MIMO系统提供了一种可行的近ML检测方案，并讨论了软LLR生成和FEC交互的扩展应用
...
### [3] [Intensity Fluctuation Spectra as a Design Guide for Nonlinear-Tolerant Constellation Shaping](https://arxiv.org/abs/2603.04699)
...
<details>
  <summary>Details</summary>
Motivation: 相干光纤链路中的非线性从根本上由信号强度的时域统计和频谱结构驱动。现有研究缺乏将成形星座的块级能量统计与PSD低频特征联系起来的统一框架，无法有效指导非线性抑制的频谱-时间协同设计。
...
Result: 得到了降低低频含量的设计规则，提供了成形和非成形系统的最优符号速率定律。蒙特卡洛模拟验证了理论结果，并确认了CCDM（抑制DC）与ESS（中等块长度下存在有限DC基座）的不同低频行为。
...
### [4] [MIMO Channel Prediction via Deep Learning-based Conformal Bayes Filter](https://arxiv.org/abs/2603.04764)
...
<details>
  <summary>Details</summary>
Motivation: 现有信道预测方法存在固有局限：传统卡尔曼滤波方法易受模型失配影响，深度学习方法会产生过度自信的预测，需要一种能提供可靠不确定性估计的解决方案

Method: 提出深度学习保形贝叶斯滤波(DCBF)框架，整合深度学习预测、保形分位数回归和贝叶斯滤波，实现校准先验和观测的原则性融合

Result: 仿真结果表明，DCBF显著改善了深度学习预测性能，并优于卡尔曼滤波方法，能够提供具有校准不确定性的可靠信道预测

Conclusion: DCBF框架有效解决了现有信道预测方法的局限性，通过集成深度学习和贝叶斯滤波，实现了可靠的信道预测和不确定性校准
...
TL;DR: 提出基于CYGNSS卫星DDM噪声底最大值的新RFI检测方法，通过多卫星并发检测和时间持续性验证提高可靠性，在GPS干扰测试和持续RFI区域表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: GNSS系统的RFI干扰日益严重，需要有效的空间探测方法。CYGNSS星座的DDM噪声底数据可用于RFI检测，但现有方法（如均值法、峰度法）可能稀释异常信号或灵敏度不足。

Method: 1) 采用最大值策略：选择四个同时GNSS反射中噪声底最大值而非均值；2) 两级验证框架：多卫星并发检测（至少两颗卫星独立标记同一区域）和时间持续性验证（10秒窗口内持续超过阈值）；3) 建立地面干扰源与卫星间斜距几何分析的理论基础。

Result: 在2025年5月CYGNSS Level 1数据测试中：1) 在白沙导弹靶场检测到其他方法几乎未发现的RFI；2) 在中东地区标记了62%的历元，优于均值法（46%）和峰度法（33%）；3) 能够检测逐渐增强干扰的早期开始和先前未报告的异常模式。

Conclusion: 基于最大值的DDM噪声底分析方法为空间RFI检测提供了更灵敏可靠的方法，能够有效识别GPS干扰测试和持续RFI区域，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 传统N阶梳状抽取器的通带下垂严重依赖于抽取因子M，这限制了其在实际应用中的灵活性，需要一种不依赖M的下垂补偿方法

Method: 在梳状抽取器的积分级联一个对称3抽头FIR滤波器，滤波器系数仅作为阶数N的函数，不依赖于抽取因子M

Result: 该方法能有效降低通带下垂对抽取因子M的依赖性，且可轻松应用于各种新型梳状抽取器和下垂补偿滤波器设计方法

Conclusion: 提出的去相关方法提供了一种简单有效的解决方案，增强了梳状抽取器在不同抽取因子下的性能一致性
...
TL;DR: 论文揭示了分布式多目标跟踪系统中基于轨迹共识方法的身份级漏洞，提出了标签劫持攻击概念，展示了攻击者如何通过注入欺骗轨迹来破坏网络中的目标身份一致性。


<details>
  <summary>Details</summary>
Motivation: 分布式多目标跟踪系统在有限视场传感器网络中常面临标签不一致问题，现有基于轨迹共识的方法通过运动学和标签一致性匹配来缓解此问题，但这些方法在对抗条件下的行为尚未得到充分研究。

Method: 提出标签劫持攻击概念，借鉴雷达中的经典牵引欺骗技术，形式化攻击隐蔽性概念，并推导基于优化的攻击策略。通过三传感器网络案例研究验证攻击效果。

Result: 案例研究表明，所提出的攻击能成功破坏标签一致性并降低跟踪精度，实现目标身份冒充。攻击能够有效影响分布式多目标跟踪系统的共识层。

Conclusion: 这项工作揭示了分布式多目标跟踪框架中共识层的安全漏洞，强调了在共识层重新思考鲁棒性的必要性，为未来安全增强的跟踪系统设计提供了重要启示。
...
TL;DR: 设计并仿真了一种基于IHP SG13G2 SiGe BiCMOS技术的模拟8-PAM解映射器，实现了0.33 pJ/bit的能效和1Gbit/s的数据速率。


<details>
  <summary>Details</summary>
Motivation: 频谱高效通信系统需要多级调制格式，接收端通常需要解调器提取软信息。虽然数字解调器常见，但模拟实现也具有潜力，本文旨在设计并验证模拟8-PAM解映射器的可行性。

Method: 基于文献中的4-PAM设计进行改进和推广，提出全MOSFET结构的8-PAM解映射器，使用开源IC设计工具进行设计和仿真，采用IHP SG13G2 SiGe BiCMOS技术实现。

Result: 仿真结果显示，在1Gbit/s数据速率下实现了0.33 pJ/bit的能效，验证了模拟8-PAM解映射器的可行性和性能优势。

Conclusion: 成功设计并验证了模拟8-PAM解映射器，展示了模拟实现多级调制解调器的潜力，为高效能通信系统提供了新的实现方案。
...
TL;DR: 提出基于级数解析延拓的AR过程杂波模拟方法，通过预计算输入预失真来补偿AR滤波，实现非高斯相关杂波序列的准确快速模拟。


<details>
  <summary>Details</summary>
Motivation: 线性滤波（特别是AR过程）在模拟具有特定PDF和ACF的杂波序列时，会不可避免地扭曲输入分布，导致PDF不准确或只能适用于简单ACF。需要解决这些挑战。

Method: 1. 基于AR过程输入输出关系推导输入矩和累积量，分别围绕零展开拉普拉斯变换(LT)的矩展开和对数LT的累积量展开；2. 通过Padé逼近(PA)对两种级数展开进行解析延拓以恢复全复平面上的LT；3. 利用累积量展开恢复的LT，通过随机变量变换方法快速模拟AR输入非高斯白序列，实现高效AR过程。

Result: 模拟结果表明，所提策略能够实现非高斯相关杂波序列的准确快速模拟。特别指出，传统的矩展开PA延拓在LT强振荡时可能不准确，而对数LT通常结构更简单，累积量展开的延拓提供了更稳定准确的替代方案。

Conclusion: 提出的基于级数解析延拓的策略能够有效解决AR过程杂波模拟中的输入分布失真问题，通过准确预计算输入预失真补偿AR滤波，实现了准确且高效的非高斯相关杂波序列模拟。
...
TL;DR: 提出双馈点夹持天线系统（DF-PAS），通过在波导两端设置两个馈电点并根据用户位置动态选择，有效缩短波导内传播距离，减轻衰减导致的功率损耗，提升系统性能。


<details>
  <summary>Details</summary>
Motivation: 传统单馈点夹持天线系统（SF-PAS）的性能受到波导内衰减的严重限制，特别是在长波导应用中，这种衰减会显著降低可达数据速率。需要一种不改变波导结构或PA驱动机制的方法来缓解衰减问题。

Method: 提出DF-PAS架构，每个波导在两端设置两个馈电点，根据用户位置动态选择馈电点以缩短波导内传播距离。在单波导场景下推导了高信噪比近似下的遍历速率闭式解和最优PA位置及馈电点选择方案。在多波导场景下，提出了联合优化馈电点选择、PA放置和波束赋形的两阶段优化框架，结合贪婪馈电点切换、基于梯度的PA放置和WMMSE波束赋形。

Result: 仿真结果表明，提出的DF-PAS在各种网络配置下始终优于传统SF-PAS，验证了其作为缓解PAS无线网络中波导内衰减的实用且可扩展解决方案的有效性。

Conclusion: DF-PAS通过简单的双馈电点设计和动态选择机制，在不改变波导结构的情况下有效缓解了波导内衰减问题，为PAS系统提供了性能提升的实用解决方案，特别适用于长波导应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 传统直接预测模型使用均方误差等点式目标，无法捕捉图结构信号中复杂的时空依赖关系。现有频率域方法（如FreDF）虽然缓解了时间自相关，但忽略了空间和跨时空交互作用。

Method: 提出FreST Loss，基于联合傅里叶变换将监督扩展到联合时空谱域。该方法在统一的谱域中对齐模型预测与真实值，有效解耦空间和时间上的复杂依赖关系。

Result: 理论分析表明该公式减少了时域训练目标相关的估计偏差。在六个真实世界数据集上的实验证明，FreST Loss是模型无关的，能够持续改进最先进的基线方法，更好地捕捉整体时空动态。

Conclusion: FreST Loss通过频率增强的时空训练目标，在联合谱域中进行监督，有效解决了图结构信号预测中的时空依赖问题，为时空预测任务提供了通用的改进方案。
...
TL;DR: 提出基于深度神经网络的EINN方法，通过将候选平衡态作为输入来推断系统参数，从而检测临界阈值和灾难性状态转变。


<details>
  <summary>Details</summary>
Motivation: 传统检测临界转变的方法（如前向模拟或分岔分析）计算量大且受参数采样限制，需要更高效灵活的方法来识别复杂动力系统中的临界阈值。

Method: 提出平衡态信息神经网络（EINNs），与传统方法相反，将候选平衡态作为输入，训练深度神经网络推断满足平衡条件的对应系统参数，通过分析学习到的参数景观来检测临界阈值。

Result: 在展示鞍结分岔和多稳态的非线性系统上验证了EINNs的能力，表明该方法能够恢复与即将发生的转变相关的参数区域。
...
TL;DR: FedEMA-Distill：一种服务器端联邦学习方法，通过指数移动平均和知识蒸馏解决异构数据和对抗性客户端问题，显著提升准确率、减少通信轮次和降低上行负载


<details>
  <summary>Details</summary>
Motivation: 联邦学习在客户端持有异构非IID数据或存在对抗性客户端时性能下降，导致客户端漂移、收敛缓慢和高通信开销。需要一种既支持模型异构又具备对抗鲁棒性的通信高效方法。

Method: 服务器端结合全局模型的指数移动平均(EMA)和客户端上传预测logits的集成知识蒸馏。客户端仅上传压缩的logits（在小规模公共代理数据集上评估），无需修改客户端软件，支持不同模型架构。服务器端使用坐标中位数或修剪均值聚合logits以增强对抗鲁棒性。

Result: 在CIFAR-10、CIFAR-100、FEMNIST和AG News数据集上，Dirichlet-0.1标签偏斜下，相比基线方法：准确率提升最高达+5%（CIFAR-10）和+6%（CIFAR-100）；达到目标准确率所需通信轮次减少30-35%；每轮客户端上行负载降至0.09-0.46 MB（比传输完整模型权重减少约一个数量级）。在10-20%拜占庭客户端存在时仍能稳定训练。

Conclusion: 时间平滑与仅logits聚合的结合提供了一种通信高效、对抗鲁棒的联邦学习管道，部署友好且兼容安全聚合和差分隐私，因为只交换聚合或混淆的模型输出。
...
TL;DR: Delta-Crosscoder改进了模型差异分析方法，通过结合BatchTopK稀疏性、基于delta的损失和隐式对比信号，能更有效地识别微调导致的内部表示变化，特别是在窄微调场景下。


<details>
  <summary>Details</summary>
Motivation: 现有Crosscoders方法在处理窄微调（行为变化局部化且不对称）时存在困难，需要更有效的模型差异分析方法来识别微调如何改变模型的内部表示。

Method: Delta-Crosscoder结合了BatchTopK稀疏性、基于delta的损失（优先考虑模型间变化的方向）以及来自匹配输入上配对激活的隐式对比信号。

Result: 在10个模型生物（包括合成虚假事实、涌现错位、潜意识学习和禁忌词猜测）上评估，Delta-Crosscoder能可靠地分离对微调行为负因果责任的潜在方向，并实现有效缓解，优于SAE基线，与非SAE方法相当。

Conclusion: Crosscoders仍然是模型差异分析的强大工具，Delta-Crosscoder的改进使其在窄微调场景下表现更优，能有效识别和缓解微调引起的行为变化。
...
TL;DR: 论文提出非对称注意力机制，将查询和键的维度降低到远小于值的维度，因为选择操作本质上是低维度的，而值传递需要高维语义表示。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer注意力中查询、键和值使用相同维度，但作者认为这三者功能不同：查询和键用于产生注意力权重（选择），而值用于传递语义信息。选择操作本质上是低维的，不需要与值相同的维度。

Method: 提出非对称注意力机制，将查询和键的维度降低到远小于值的维度。通过SVD压缩和QK微调技术，对现有模型进行压缩，减少键缓存占用。在七个实验中验证假设，包括位置选择任务、内容检索、语言建模和大型模型压缩。

Result: 实验表明：1）位置选择任务只需每头1维；2）内容检索需要约log₂N维；3-4）语言建模中d_select = d_model/4仅增加4.3%困惑度，减少75% QK参数；5）GPT-2的键比查询更可压缩；6）LLaMA模型验证架构通用性；7）Mistral-7B实现75%键缓存节省，仅2.0%质量损失。

Conclusion: 非对称注意力机制能显著减少键缓存占用，提高GPU并发用户数（7B模型可节省25GB KV缓存，增加约60%并发用户）。该方法通过降低查询和键维度，在保持模型质量的同时大幅减少计算和存储开销。
...
TL;DR: 提出一种在边缘设备上持久化存储4位量化KV缓存的方法，解决多智能体LLM系统内存不足问题，通过直接恢复缓存消除重复预填充计算，实现高达136倍的加速。


<details>
  <summary>Details</summary>
Motivation: 边缘设备上多智能体LLM系统面临内存管理问题：设备RAM太小无法同时容纳所有智能体的KV缓存。例如在Apple M4 Pro上，10.2GB缓存预算只能容纳3个8K上下文的FP16智能体，10个智能体工作流需要不断驱逐和重载缓存，每次驱逐都需要完整的重新预填充计算（4K上下文需15.7秒）。

Method: 系统包含三个组件：1) 块池提供每个智能体隔离的4位量化KV缓存（safetensors格式）；2) BatchQuantizedKVCache支持多个智能体量化缓存的并发推理；3) 跨阶段上下文注入，在不同对话阶段间累积注意力状态而无需重新计算。将KV缓存持久化到磁盘并直接恢复到注意力层。

Result: 缓存恢复将首令牌生成时间减少高达136倍（Gemma：4K-32K上下文下22-136倍；DeepSeek：4K-32K下11-76倍；Llama：4K-16K下24-111倍；1K下3-10倍）。4位量化相比FP16能在固定设备内存中容纳4倍多的智能体上下文。实际Q4 KV缓存的困惑度变化：Gemma -0.7%，Llama +2.8%，DeepSeek +3.0%。

Conclusion: 通过持久化4位量化KV缓存和直接缓存恢复，显著解决了边缘设备上多智能体LLM系统的内存瓶颈问题，大幅减少了首令牌延迟，同时保持了可接受的精度损失。该方法开源在https://github.com/yshk-mxim/agent-memory。
...
TL;DR: Flowers是一种基于多头扭曲的PDE求解器架构，不使用傅里叶乘子、点积注意力或卷积混合，通过预测位移场扭曲特征实现自适应全局交互，在计算效率和性能上优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统PDE求解器架构（如基于傅里叶、卷积或注意力机制的方法）在计算效率和适应性方面存在局限。Flowers旨在通过物理启发的设计，实现高效、自适应的全局交互，特别适用于流体和波动等物理问题。

Method: Flowers完全基于多头扭曲构建：每个头预测位移场并扭曲混合输入特征；位移预测是逐点进行的，无需空间聚合；非局部性仅通过稀疏采样实现（每个头一个源坐标）；通过多尺度残差块堆叠扭曲层，实现线性成本的全局交互。

Result: 在广泛的2D和3D时变PDE基准测试中表现优异，特别是在流体和波动问题上。紧凑的1700万参数模型在相似规模下优于傅里叶、卷积和注意力基线；1.5亿参数变体在参数、数据和训练计算量更少的情况下超越了最近的transformer基础模型。

Conclusion: Flowers通过创新的多头扭曲架构，为PDE求解器设计提供了新的范式，实现了计算效率与性能的良好平衡，在物理启发的神经网络架构方面具有重要价值。
...
TL;DR: SOLID是一个基于掩码条件扩散的框架，仅从稀疏观测中学习时空动态，无需密集场数据或预插值，通过双重掩码目标实现后验采样，在严重稀疏条件下显著提升概率误差。
...
Method: SOLID采用掩码条件扩散框架，每个去噪步骤都以测量值及其位置为条件。引入双重掩码目标：(i)强调在未观测空白区域学习，(ii)对输入和目标提供最可靠锚点的重叠像素进行加权。采用严格的稀疏条件路径，实现与测量一致的全场后验采样。

Result: 在严重稀疏条件下，SOLID实现了概率误差的数量级改进，产生校准的不确定性图（ρ > 0.7），相比需要密集训练数据的方法有显著优势。

Conclusion: SOLID证明了仅从稀疏观测中端到端学习时空动态的可行性，通过掩码条件扩散和双重掩码目标，在严重稀疏条件下实现了准确的后验采样和校准的不确定性估计。
...
<details>
  <summary>Details</summary>
Motivation: 联邦微调大型语言模型面临两大挑战：1) 本地更新需要大量VRAM存储梯度；2) 频繁模型交换导致通信开销大

Method: 提出ZorBA框架：1) 使用零阶优化避免存储梯度；2) 异构块激活机制，服务器分配不同transformer块子集给客户端；3) 共享随机种子和梯度有限差分减少通信开销
...
TL;DR: 提出自适应分割联邦学习框架，通过服务器分担计算、自适应模型分割和资源分配，优化收敛速度、延迟和能耗


<details>
  <summary>Details</summary>
Motivation: 联邦学习中客户端计算资源有限导致训练延迟高、能耗大，需要利用服务器资源并优化资源分配来提升学习性能和效率

Method: 提出ASFL框架，利用中央服务器训练部分模型，支持自适应模型分割和资源分配；理论分析收敛率并建立联合优化问题；设计OOE-BCD算法迭代求解

Result: 相比五种基线方案，ASFL收敛更快，总延迟降低达75%，能耗降低达80%

Conclusion: ASFL框架通过自适应模型分割和资源分配，有效提升了联邦学习的性能和效率，为资源受限的无线网络环境提供了实用解决方案
...
TL;DR: 提出一个可解释的集成学习框架，使用临床、生活方式和代谢特征进行阿尔茨海默病分类，XGBoost、随机森林和软投票集成表现最佳，SHAP分析识别出关键特征。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病早期准确检测具有挑战性，需要可靠且可解释的预测框架来支持临床决策。

Method: 采用集成学习框架，包含严格预处理、高级特征工程、SMOTE-Tomek混合类别平衡，使用五种集成算法（随机森林、XGBoost、LightGBM、CatBoost、Extra Trees）和深度人工神经网络，通过分层验证选择模型。

Result: 集成方法优于深度学习，XGBoost、随机森林和软投票集成在准确性、敏感性和F1分数方面表现最强，SHAP分析显示MMSE、功能评估年龄和多个工程交互特征最具影响力。

Conclusion: 该框架为阿尔茨海默病预测提供了可靠且透明的解决方案，在临床决策支持应用中具有强大潜力。
...
TL;DR: 提出非经典网络（NCnet），一种经典神经网络架构，在典型实验设置下稳定表现出非经典统计行为，通过CHSH不等式的S统计量衡量非经典性，发现其源于多任务共享隐藏层神经元的梯度竞争。


<details>
  <summary>Details</summary>
Motivation: 受量子力学中测量不兼容性和贝尔家族不等式的启发，探索经典神经网络是否能够表现出非经典统计行为，为理解深度网络内部交互和训练动态提供新视角。

Method: 设计非经典网络（NCnet），一种简单的经典神经网络架构，在典型可解释的实验设置下运行。通过CHSH不等式的S统计量测量非经典性，分析隐藏层神经元在多任务共享时的梯度竞争机制。

Result: 在低资源条件下，S值随资源增加逐渐上升并接近经典上界2；当模型接近足够性能的临界规模时，S可能暂时超过2；随着资源继续增加，S渐近衰减并在2附近波动。模型容量不足时，S与泛化性能正相关，S首次接近2的区间通常对应良好泛化。

Conclusion: 非经典统计为理解深度网络内部交互和训练动态提供了新颖视角，即使没有显式通信的物理连接，任务头也能通过局部损失振荡隐式感知其他任务头的训练任务，导致训练结果中的非局域相关性。
...
Method: 考虑结构化设置，约束对手从固定但表达能力强的函数类R中选择标签。提出新学习算法，使用ERM预言机高效运行，采用截断熵正则化的Frank-Wolfe归约和混合鞅差序列的新尾界分析工具。

Result: 算法获得的遗憾界与学习者的假设类H和对手的标签类R导出的类的Rademacher复杂度相关。作为关键推论，给出了在高维动作集但具有低维结构支付函数的随机零和博弈中计算均衡的预言机高效算法。

Conclusion: 本文在混合学习设置中向同时实现统计最优性和计算效率迈出了重要一步，通过结构化约束和新的分析工具，为混合在线学习问题提供了更优的解决方案。
...
TL;DR: 提出HARR方法，通过将异质数值和类别属性投影到统一可学习的多空间中，实现混合数据的聚类分析，无需参数且收敛有保证。


<details>
  <summary>Details</summary>
Motivation: 现实聚类任务中常见包含数值和类别属性的混合数据。数值属性在欧氏空间中有明确定义，而类别属性则嵌入在隐式空间中。现有方法要么将异质属性编码为单一类型，要么定义统一度量，未能揭示它们的内在联系。

Method: 提出异质属性重构与表示学习范式(HARR)，将异质属性转换为同质状态进行距离度量学习，并将学习与聚类集成。将每个属性的值投影到统一可学习的多空间中，更精细地表示和学习类别数据的距离度量。

Result: HARR方法在准确性和效率方面表现出优越性，能够更有效地自适应不同的聚类数量k，且无需参数、收敛有保证。

Conclusion: HARR通过揭示异质属性间的连接，提供了一种有效的混合数据聚类方法，优于现有方法。
...
TL;DR: 该论文建立了K-Means算法与可微RBF神经网络之间的严格等价关系，通过Γ收敛证明RBF目标函数在温度参数趋近于0时收敛到K-Means解，并提出使用Entmax-1.5解决数值稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 传统K-Means算法是离散的分区方法，无法直接嵌入深度学习架构进行端到端优化。需要建立K-Means与可微神经网络之间的理论联系，以实现在深度学习框架中联合优化表示和聚类。

Method: 通过重新参数化K-Means目标函数，将其嵌入平滑加权损失中，证明RBF目标函数Γ收敛到K-Means解。使用Entmax-1.5替代Softmax解决低温区数值不稳定问题，保持Voronoi分区结构。

Result: 理论证明RBF梯度更新在极限情况下恢复精确的K-Means质心更新规则，产生相同的训练轨迹。实验验证软RBF质心单调收敛到K-Means固定点，提供端到端可微分聚类的统一框架。

Conclusion: 该工作建立了离散分区与连续优化之间的理论桥梁，使K-Means能够直接嵌入深度学习架构，实现表示和聚类的联合优化，为端到端可微分聚类提供了理论基础。
...
TL;DR: VSPrefill：一种基于垂直斜线结构的稀疏注意力机制，通过轻量级训练实现长上下文推理加速，在保持98.35%准确率的同时获得4.95倍速度提升。
...
Method: 提出VSPrefill机制，使用垂直斜线结构模式构建稀疏掩码。通过紧凑的VSIndexer模块从增强RoPE的键值表示中预测上下文感知的重要性分数，以线性复杂度构建稀疏掩码而不修改主干参数。推理时采用自适应累积阈值策略分配每层稀疏预算，并通过融合内核执行带实时索引合并的注意力。
...
Conclusion: VSPrefill在准确性和效率之间建立了新的帕累托前沿，为长上下文推理提供了一种有效的稀疏注意力解决方案。
...
TL;DR: 该论文建立了线性非高斯模型中任意潜在结构和循环图的等价性判据，并开发了首个无需结构假设的潜在变量因果发现方法。


<details>
  <summary>Details</summary>
Motivation: 现有潜在变量因果发现方法依赖强结构假设（如特定指标模式或交互限制），缺乏一般性方法的核心障碍是没有等价性表征。不知道什么可识别，就无法设计识别方法。

Method: 提出边缘秩约束作为新工具，建立任意潜在结构和循环图的分布等价性图形判据，提供遍历整个等价类的程序，并开发从数据中恢复模型（直到等价性）的算法。

Result: 在线性非高斯模型中首次实现了无需结构假设的潜在变量等价性表征，并开发了相应的因果发现算法，填补了该领域的重要空白。

Conclusion: 这是首个在无结构假设的参数化设置中建立潜在变量等价性表征的工作，为更一般的因果发现方法奠定了基础，提供了新工具和算法实现。
...
TL;DR: MAD-SmaAt-GNet在SmaAt-UNet基础上，通过多模态编码器和物理平流组件改进降水临近预报，MSE降低8.9%


<details>
  <summary>Details</summary>
Motivation: 传统数值天气预报计算成本高且未能充分利用大量天气数据，深度学习模型在降水临近预报中展现出潜力，但现有CNN模型如SmaAt-UNet仍有改进空间，需要更好地整合多源天气数据和物理约束

Method: 提出MAD-SmaAt-GNet，在SmaAt-UNet基础上：(1)增加额外编码器处理多模态天气变量；(2)集成基于物理的平流组件确保预测的物理一致性

Result: 每个扩展单独都能改进降雨预报，组合使用效果更佳。相比基线SmaAt-UNet，四步降水预报（最长4小时）的MSE降低8.9%。多模态输入对短预报时效特别有益，平流组件在长短预报时效上都能提升性能

Conclusion: MAD-SmaAt-GNet通过整合多模态天气数据和物理约束，显著提升了降水临近预报的准确性，为深度学习天气预报提供了更有效的架构
...
### [29] [Non-Euclidean Gradient Descent Operates at the Edge of Stability](https://arxiv.org/abs/2603.05002)
*Rustem Islamov,Michael Crawshaw,Jeremy Cohen,Robert Gower*

Main category: cs.LG

TL;DR: 论文通过方向平滑性视角解释边缘稳定性现象，将广义锐度概念扩展到任意范数，统一了多种优化器的EoS分析框架。


<details>
  <summary>Details</summary>
Motivation: 边缘稳定性现象在深度学习中广泛存在但理论基础不完整，需要统一的框架来解释不同优化器中的EoS行为。

Method: 使用方向平滑性理论框架，将广义锐度定义扩展到任意范数，涵盖vanilla GD、预处理GD、ℓ∞-下降、块坐标下降、谱GD等多种优化器。

Result: 实验表明非欧几里得GD在神经网络训练中也表现出渐进锐化并围绕阈值2/η振荡的现象，验证了广义锐度框架的有效性。

Conclusion: 该框架提供了一个统一的几何感知谱度量，能够跨不同优化器解释边缘稳定性现象，为理解深度学习优化提供了新视角。
...
TL;DR: 研究发现大语言模型在上下文学习中容易受到错误示例影响，即使只有一个错误示例也会导致性能显著下降。模型内部会同时编码正确和错误规则，但在后期层才形成预测置信度，揭示了双阶段计算结构。


<details>
  <summary>Details</summary>
Motivation: 上下文学习使大语言模型能够通过少量示例执行新任务，但示例本身可能包含噪声和冲突，使这种能力变得脆弱。为了理解模型如何处理此类冲突，研究需要模型推断底层模式的演示依赖任务。

Method: 使用线性探针和logit lens分析模型内部处理过程，识别不同阶段的关键注意力头。通过针对性消融验证发现，包括早期到中间层的易受攻击头和后期层的易受影响头。

Result: 模型在单个错误示例下性能显著下降。中间层同时编码正确和错误规则，但只在后期层形成预测置信度。识别出两类关键注意力头：易受攻击头对错误敏感，易受影响头会减少对正确预测的支持。消融少量关键头可将性能提升超过10%。

Conclusion: 大语言模型在上下文学习中存在系统性误导行为，具有双阶段计算结构。早期层编码规则信息，后期层形成预测置信度。识别出的注意力头机制解释了模型在冲突证据下的推理失败，为改进模型鲁棒性提供了方向。
...
### [31] [Federated Causal Discovery Across Heterogeneous Datasets under Latent Confounding](https://arxiv.org/abs/2603.05149)
...
<details>
  <summary>Details</summary>
Motivation: 由于数据隐私法规和跨站点异质性的限制，传统因果发现方法需要集中式数据集，这在多数据集场景中难以实现。

Method: 1. fedCI：使用联邦迭代重加权最小二乘法估计广义线性模型参数，进行条件独立性似然比检验；2. fedCI-IOD：将IOD算法扩展为联邦版本，替换其元分析策略，支持存在潜在混杂因素的分布式因果发现。

Result: 方法在保护隐私的同时显著提升统计功效，性能接近完全集中式分析，并能缓解局部样本量小带来的伪影问题。

Conclusion: 提供了完整的工具包（Python包、R实现、Web应用），为联邦条件独立性测试和因果发现提供了实用、用户友好的解决方案。
...
### [32] [Towards Explainable Deep Learning for Ship Trajectory Prediction in Inland Waterways](https://arxiv.org/abs/2603.04472)
*Tom Legel,Dirk Söffker,Roland Schätzle,Kathrin Donandt*

Main category: cs.LG

TL;DR: 该研究提出了一种基于LSTM的船舶轨迹预测模型，通过引入可训练船舶领域参数来增强模型可解释性，在拥挤内河航道环境下进行5分钟预测，最终位移误差约40米。


<details>
  <summary>Details</summary>
Motivation: 在拥挤的内河航道环境中，准确的船舶轨迹预测对安全至关重要。虽然深度学习模型在预测精度上有所提升，但这些模型通常缺乏可解释性，可能掩盖不准确的逻辑并削弱对其可靠性的信心。

Method: 采用基于LSTM的船舶轨迹预测模型，并引入可训练的船舶领域参数，这些参数提供了对交互船舶隐藏状态基于注意力融合的洞察。模型变体通过标准位移误差统计进行评估，并分析生成的船舶领域值的合理性。

Result: 在5分钟预测范围内，最终位移误差约为40米，与类似研究表现相当。虽然船舶间注意力架构提高了预测精度，但使用学习到的船舶领域值为相遇中船舶分配的权重与预期存在偏差，表明精度提升并非完全由预测轨迹与附近船舶轨迹之间的因果关系驱动。

Conclusion: 该模型通过其内在可解释的设计展示了良好的解释能力。未来工作将集中于利用该架构进行反事实分析，并纳入更复杂的注意力机制。
...
### [33] [Towards a data-scale independent regulariser for robust sparse identification of non-linear dynamics](https://arxiv.org/abs/2603.05201)
*Jay Raut,Daniel N. Wilke,Stephan Schmidt*

Main category: cs.LG

TL;DR: STCV算法通过使用无量纲统计指标替代传统基于幅度的阈值处理，解决了数据归一化对稀疏回归方法（特别是SINDy）的破坏性影响，使系统辨识对数据缩放具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 数据归一化是工程和科学应用中常见且必要的预处理步骤，但会严重扭曲基于幅度的稀疏回归方法发现控制方程的能力。这一问题在SINDy框架中尤为突出，数据缩放与测量噪声的相互作用破坏了稀疏性核心假设，导致发现模型变得稠密、不可解释且物理上不正确。

Method: 提出了序列化变异系数阈值法（STCV），这是一种新颖的计算高效稀疏回归算法。STCV用无量纲统计指标"系数存在性"（CP）替代传统的基于幅度的阈值处理，CP评估模型库中候选项的统计有效性和一致性。这种从幅度到统计显著性的转变使发现过程对任意数据缩放具有不变性。

Result: 通过对典型动力系统和实际工程问题（包括物理质量-弹簧-阻尼器实验）的综合基准测试，证明STCV在归一化、噪声数据集上始终显著优于标准STLSQ和E-SINDy方法。STCV能够成功识别正确的稀疏物理定律，而其他方法则会失败。
...
### [34] [Activity Recognition from Smart Insole Sensor Data Using a Circular Dilated CNN](https://arxiv.org/abs/2603.04477)
*Yanhua Zhao*
...
<details>
  <summary>Details</summary>
Motivation: 智能鞋垫配备压力传感器、加速度计和陀螺仪，为监测人类步态和姿势提供了非侵入性方法。需要开发有效的活动分类系统来处理这些多模态时间序列数据。

Method: 使用圆形扩张卷积神经网络(CDCNN)处理来自智能鞋垫的多模态时间序列数据。模型处理160帧窗口，包含24个通道（18个压力、3个加速度计、3个陀螺仪轴）。与使用扁平化数据的极端梯度提升树(XGBoost)模型进行比较。

Result: 在四类活动分类任务（站立、行走、坐姿、串联）中，CDCNN模型在独立于受试者的评估中达到86.42%测试准确率，而XGBoost模型为87.83%。排列特征重要性分析显示惯性传感器（加速度计和陀螺仪）对分类有显著贡献。

Conclusion: 该方法适用于嵌入式部署和实时推理，为智能鞋垫的活动分类提供了有效的深度学习解决方案，展示了多模态传感器数据在活动识别中的价值。
...
### [35] [SurvHTE-Bench: A Benchmark for Heterogeneous Treatment Effect Estimation in Survival Analysis](https://arxiv.org/abs/2603.05483)
...
<details>
  <summary>Details</summary>
Motivation: 生存分析中的异质性治疗效果估计在精准医学和个体化政策制定等高风险应用中至关重要，但由于删失、未观测反事实和复杂识别假设等挑战，现有评估方法分散且不一致，需要统一的评估标准。

Method: 构建包含三个模块的基准测试：(1) 具有已知真实值的模块化合成数据集，系统变化因果假设和生存动态；(2) 结合真实世界协变量与模拟治疗和结果的半合成数据集；(3) 来自双胞胎研究和HIV临床试验的真实世界数据集。

Result: 在合成、半合成和真实世界设置下，首次对生存HTE方法在不同条件和现实假设违反情况下进行了严格比较，为因果生存方法建立了公平、可复现和可扩展的评估基础。

Conclusion: SurvHTE-Bench填补了生存分析中异质性治疗效果估计评估的空白，为研究者提供了统一的评估框架，数据与代码已开源，有助于推动该领域的方法发展和实际应用。
...
TL;DR: 提出MTDP框架，通过两阶段多教师蒸馏，利用视觉和时间序列等成熟基础模型来引导EEG基础模型预训练，显著减少数据需求并提升性能。


<details>
  <summary>Details</summary>
Motivation: EEG数据收集成本高、信噪比低，传统基于掩码重建的自监督预训练方法难以扩展和捕捉神经语义。能否利用成熟模态的基础模型来引导EEG模型预训练？

Method: 提出多教师蒸馏预训练(MTDP)框架：第一阶段使用可学习门控网络融合不同教师模型（如DINOv3和Chronos）的表示，通过掩码潜在去噪目标；第二阶段将融合表示蒸馏到EEG基础模型中。

Result: 在9个下游任务和12个数据集上的评估表明，MTDP-based EEG基础模型优于自监督对比方法，且仅需25%的预训练数据。

Conclusion: 通过利用成熟模态的基础模型进行蒸馏，可以有效解决EEG数据稀缺和噪声问题，显著提升EEG基础模型的预训练效率和性能。
...
TL;DR: 提出對比學習框架，將X射線光譜與科學文獻對齊，建立共享多模態表徵，提升物理變量估計並識別罕見天體


<details>
  <summary>Details</summary>
Motivation: 天文學家擁有大量多模態數據（圖像、光譜、時間序列）和文獻，但這些數據源很少被系統性整合。科學文本涵蓋的物理背景比光譜更廣泛多樣，建立連接具有挑戰性

Method: 提出對比學習框架，將X射線光譜與從科學文獻提取的領域知識對齊，建立共享多模態表徵。採用專家混合策略，結合單模態和共享表徵

Result: 光譜檢索文本的Recall@1%達到20%，證明有意義的模態對齊是可能的。融合光譜和文本數據使20個物理變量估計比單模態光譜基線提升16-18%。多模態潛在空間識別出高優先級跟蹤目標

Conclusion: 該框架成功將光譜與文獻對齊，加速罕見或理解不足天體的解釋。共享潛在空間有效編碼物理信息，專家混合策略表現優異。框架可擴展到其他科學領域
...
TL;DR: 提出Invariant Causal Routing (ICR)因果治理框架，用于识别跨异质环境稳定的政策-规范关系，通过反事实推理和不变因果发现来设计可解释、可审计的政策规则，在在线市场经济中引导社会规范向期望方向发展。


<details>
  <summary>Details</summary>
Motivation: 在线市场经济中，公平曝光、持续参与和平衡再投资等社会规范对长期稳定至关重要。这些规范从无数微观互动中涌现，难以进行因果归因和政策迁移，需要一种能够识别跨环境稳定因果关系的治理方法。

Method: 提出Invariant Causal Routing (ICR)框架，整合反事实推理和不变因果发现，分离真实因果效应与虚假相关性，构建可解释、可审计的政策规则，在异质智能体仿真中验证有效性。

Result: 在基于真实数据校准的异质智能体仿真中，ICR相比相关性或覆盖率基线方法，能产生更稳定的规范、更小的泛化差距和更简洁的规则，证明因果不变性为治理提供了原则性和可解释的基础。

Conclusion: 因果不变性为在线市场经济的治理提供了原则性基础，ICR框架能够识别跨环境稳定的政策-规范关系，设计出可解释、可审计且有效的干预措施，有助于引导社会规范向期望方向发展。
...
TL;DR: KG-WISE：针对大规模知识图谱的GNN推理加速系统，通过分解模型、基于查询子图结构部分加载组件，结合LLM生成查询模板，实现28倍加速和98%内存降低


<details>
  <summary>Details</summary>
Motivation: 现有GNN推理加速方法（如剪枝、量化、知识蒸馏）存在局限性：1）无法根据查询的结构或语义进行自适应；2）模型作为整体文件加载，无法选择性加载与目标节点语义相关的组件；3）在大规模知识图谱上导致数据加载冗余和计算浪费

Method: 1）将训练好的GNN模型分解为细粒度组件，可根据查询子图结构部分加载；2）利用大语言模型生成可重用的查询模板，提取每个任务语义相关的子图；3）实现查询感知的紧凑模型实例化

Result: 在6个大规模知识图谱（最多4200万节点、1.66亿边）上评估，相比最先进系统：推理速度提升最高28倍，内存使用降低98%，同时在商业和开源LLM上保持或提高准确性

Conclusion: KG-WISE提出了一种任务驱动的推理范式，通过模型分解和语义感知的组件加载，有效解决了大规模知识图谱上GNN推理的数据加载冗余和计算浪费问题，实现了显著的性能提升
...
TL;DR: LPWM是一种自监督的以对象为中心的世界模型，能够从视频数据中自主发现关键点、边界框和对象掩码，适用于决策制定任务。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型在真实世界多对象数据集上的扩展性有限，且缺乏对场景分解的无监督学习能力。需要一种能够自主发现对象表示并适用于决策制定的可扩展世界模型。

Method: 提出LPWM架构，通过端到端训练从视频中学习，支持对动作、语言和图像目标的灵活条件化。采用新颖的潜在动作模块建模随机粒子动力学，自主发现关键点、边界框和对象掩码。

Result: 在多样化的真实世界和合成数据集上取得了最先进的结果。展示了在目标条件模仿学习等决策制定任务中的应用能力。

Conclusion: LPWM成功扩展了自监督对象中心世界模型到真实世界多对象数据集，不仅实现了先进的随机视频建模，还直接适用于决策制定任务，为无监督场景理解和决策提供了新途径。
...
TL;DR: 研究探讨了持续学习中灾难性遗忘与结构坍塌之间的相关性，通过测量权重和激活的有效秩来验证两者之间的强关联。


<details>
  <summary>Details</summary>
Motivation: 当前持续学习方法大多通过任务准确率评估，忽略了模型内部结构变化。研究表明结构坍塌会导致可塑性丧失，这可能与遗忘相关，因为网络失去扩展特征空间学习新任务的能力，被迫覆盖现有表征。

Method: 通过测量权重和激活的有效秩(eRank)来评估结构坍塌。在split MNIST和Split CIFAR-100基准上测试了四种架构(MLP、ConvGRU、ResNet-18、Bi-ConvGRU)，分别使用SGD、Learning-without-Forgetting和Experience Replay三种训练策略。

Result: 结果显示遗忘与结构坍塌之间存在强相关性，不同的持续学习策略以不同效率帮助模型保持容量和性能。

Conclusion: 灾难性遗忘与结构坍塌密切相关，通过有效秩测量可以更好地理解持续学习中的内部结构变化，不同策略在防止结构坍塌方面具有不同效果。
...
TL;DR: 提出一种多模态AI框架，在不依赖敏感个人信息的情况下检测重复记录，适用于受GDPR和HIPAA等隐私法规限制的场景。


<details>
  <summary>Details</summary>
Motivation: 重复记录在CRM和医疗保健领域带来重大挑战，传统去重方法依赖姓名、邮箱、社保号等直接标识符，但在GDPR和HIPAA等严格隐私法规下，这些个人信息受到限制或掩码，使得传统方法失效。

Method: 提出新颖、可扩展的多模态AI框架，利用三种不同模态：1) 使用预训练DistilBERT模型从文本字段（姓名、城市）提取语义嵌入；2) 从用户登录时间戳提取行为模式；3) 通过分类嵌入编码设备元数据。采用后期融合方法结合这些异构模态，并通过无监督密度聚类算法DBSCAN进行聚类。

Result: 在专门设计的反映隐私保护约束的合成CRM数据集上评估，与传统字符串匹配基线相比，多模态框架表现出良好性能，实现了良好的F1分数，能够有效识别数据中的重复记录，尽管存在变化和噪声。

Conclusion: 该方法提供了一种隐私合规的实体解析解决方案，支持安全数字基础设施，增强公共卫生分析的可靠性，促进政府和企业在伦理AI方面的采用。非常适合整合到国家健康数据现代化工作中，符合隐私优先创新的更广泛目标。
...
TL;DR: 该研究探索了PDE基础模型在惯性约束聚变逆问题中的应用，通过微调预训练模型并添加任务特定头，实现了从多模态观测中准确重建高光谱图像和估计系统参数。


<details>
  <summary>Details</summary>
Motivation: 当前PDE基础模型的下游评估主要关注前向问题（如自回归预测），而忽略了逆问题。本研究旨在探索这些模型在惯性约束聚变（ICF）逆问题中的应用，即从多模态观测中估计系统参数。

Method: 使用JAG基准数据集，微调PDE基础模型并训练轻量级任务特定头，联合重建高光谱X射线图像和回归系统参数。通过数据缩放实验（5%-100%训练集）评估性能。

Result: 微调模型实现了准确的高光谱重建（测试MSE 1.2e-3）和强大的参数估计性能（R^2高达0.995）。数据缩放实验显示，随着训练数据增加，重建和回归损失持续改善，在低数据区域获得最大边际收益。

Conclusion: 从预训练的MORPH权重微调优于从头训练相同架构，表明基础模型初始化提高了ICF中数据有限逆问题的样本效率，为PDE基础模型在逆问题中的应用提供了有力证据。
...
TL;DR: PPO增强时序序列模型（Transformer/SSM）以应对传感器故障导致的分布漂移，理论证明奖励退化边界，实验显示Transformer策略在传感器失效时优于MLP/RNN/SSM基线


<details>
  <summary>Details</summary>
Motivation: 现实RL系统需处理观测流中的分布漂移，但多数策略架构隐含假设完全观测和无噪声状态。本文研究PPO在持续传感器故障导致部分可观测性和表示漂移下的鲁棒性

Method: 用时序序列模型（Transformer和状态空间模型SSM）增强PPO，使策略能从历史推断缺失信息；理论分析传感器故障过程的无限时域奖励退化概率边界

Result: 在MuJoCo连续控制基准测试中，严重传感器丢失情况下，基于Transformer的序列策略在鲁棒性上显著优于MLP、RNN和SSM基线，即使大部分传感器不可用仍保持高回报

Conclusion: 时序序列推理为传感器不可靠导致的观测漂移提供了原则性和实用的可靠操作机制，Transformer策略在部分可观测环境下表现出优越的鲁棒性
...
TL;DR: VeNRA提出可验证数值推理代理，通过类型化事实账本和双重锁定算法解决金融RAG中的算术错误和语义混淆问题，实现零幻觉金融推理


<details>
  <summary>Details</summary>
Motivation: 标准RAG架构在金融领域存在两大根本限制：LLM的算术能力不足，以及稠密向量检索中的分布语义混淆（如将"净利润"映射到"净销售额"）。在确定性金融领域，99%准确率仍无法建立操作信任

Method: 1. 引入类型化通用事实账本(UFL)从检索概率文本转向检索确定性变量；2. 双重锁定算法进行数学边界约束；3. VeNRA Sentinel：30亿参数SLM用于法医审计Python执行轨迹；4. 对抗模拟训练方法，程序化破坏黄金财务记录模拟生态错误；5. 单次分类范式加可选后思考；6. 微分块损失算法解决反向思维链训练中的损失稀释问题

Result: 实现了零幻觉金融推理，解决了金融RAG中的算术不准确和语义混淆问题，Sentinel模型能在单令牌测试预算内审计执行轨迹，损失算法稳定了极端差异惩罚下的梯度

Conclusion: VeNRA通过类型化事实检索、双重锁定算法和专门训练的审计模型，为高风险的金融领域提供了可验证的数值推理解决方案，解决了传统RAG在确定性金融推理中的根本缺陷
...
TL;DR: 提出使用合成点云数据训练深度回归网络，直接估计森林地块木材体积和地上生物量，相比传统间接方法显著提高精度


<details>
  <summary>Details</summary>
Motivation: 现有基于异速生长模型的间接方法存在测量不确定性和方程近似性问题，无法充分反映树木特征和森林条件的变异性，需要更准确的森林生物量估算方法

Method: 创建合成3D森林地块并转换为点云数据，使用PointNet、PointNet++、DGCNN和PointConv等深度回归网络进行训练，然后将训练好的网络应用于真实激光雷达数据

Result: 在合成数据上达到1.69%-8.11%的MAPE；在真实数据上与实地测量相比差异为2%-20%，而传统间接方法差异高达27%-85%

Conclusion: 合成数据与深度学习结合的方法在森林碳估算方面具有高效和可扩展的潜力，相比传统方法显著提高了精度
...
TL;DR: 提出了TREDBench工程回归数据集基准，发现工程数据与非工程数据存在领域差异，提出基于嵌入的合成数据筛选方法，仅用合成数据微调TabPFN 2.5，在工程回归任务上显著提升性能和数据效率。


<details>
  <summary>Details</summary>
Motivation: 工程应用中的预测建模长期依赖定制模型和小规模表格数据，限制了大规模学习方法的应用。现有表格基础模型的预训练合成数据分布与工程数据的统计结构不匹配，限制了向工程回归任务的迁移。

Method: 1) 构建TREDBench：包含83个真实世界表格回归数据集，带有工程/非工程标签；2) 使用TabPFN 2.5的数据集级嵌入研究领域结构；3) 提出嵌入引导的合成数据筛选方法：生成并识别"工程类似"的合成数据集；4) 仅使用筛选出的合成任务对TabPFN 2.5进行持续预训练。

Result: 在35个工程回归数据集上，仅使用合成数据的适应方法显著提升了预测准确性和数据效率：在29/35个数据集上优于TabPFN 2.5，在27/35个数据集上优于AutoGluon，平均数据效率增益分别为1.75倍和4.44倍。

Conclusion: 原则性的合成数据筛选可以将程序生成器转化为领域相关的"数据引擎"，使基础模型能够在真实数据收集是主要瓶颈的数据稀缺科学和工业领域中实现改进。
...
TL;DR: 深度矩阵分解（深度线性神经网络）在矩阵补全任务中，网络深度通过耦合动力学机制增强低秩偏置，深度≥3的网络会收敛到秩1解，而深度2网络在预训练后可能失去可塑性。


<details>
  <summary>Details</summary>
Motivation: 研究深度矩阵分解（深度线性神经网络）作为简化测试平台，探索网络深度如何影响训练动态。尽管问题简单且重要，现有理论主要关注浅层（深度2）模型，未能完全解释深层网络中观察到的隐式低秩偏置现象。

Method: 通过分析梯度流在块对角观测下的动态，研究深度矩阵分解中的耦合动力学机制。证明深度≥3的网络除非对角初始化，否则都会表现出耦合动态，且收敛到秩1解当且仅当动态是耦合的。

Result: 发现耦合动力学是深度网络低秩偏置的关键机制，且随着深度增加而增强。深度≥3的网络会收敛到秩1解，而深度2网络在预训练后可能失去可塑性，即使后续训练满足耦合条件也无法收敛到低秩解。

Conclusion: 深度矩阵分解通过耦合动力学机制产生低秩偏置，深度≥3的网络能避免可塑性损失，而深度2网络在特定预训练条件下会失去可塑性。这为理解深度神经网络中的隐式正则化提供了理论洞见。
...
TL;DR: 提出改进Dreamer模型的概率方法，通过并行探索多个潜在状态和保持互斥未来假设，在MPE SimpleTag环境中实现4.5%性能提升和28%方差降低


<details>
  <summary>Details</summary>
Motivation: Dreaming（想象学习）能让智能体从想象经验中学习，实现更鲁棒和样本高效的世界模型学习。现有Dreamer模型在潜在状态探索和未来假设处理方面有改进空间，需要更好的概率方法来处理不确定性。

Method: 采用概率方法改进Dreamer模型：1）并行探索多个潜在状态；2）保持互斥未来的不同假设，同时保留连续潜在变量的梯度特性。使用粒子方法（如粒子数K）来处理不确定性。

Result: 在MPE SimpleTag领域中，该方法比标准Dreamer模型表现更好，获得4.5%的分数提升和28%的回合回报方差降低。

Conclusion: 提出的概率方法能有效改进Dreamer模型的性能，但存在局限性：最优超参数（如粒子数K）如何随环境复杂度扩展，以及如何捕捉世界模型中的认知不确定性，这些是未来工作的方向。
...
TL;DR: 提出Count Bridges方法，用于整数计数数据的生成建模和反卷积，特别适用于生物测量中的聚合观测数据


<details>
  <summary>Details</summary>
Motivation: 许多生物测量技术（如RNA测序）产生整数计数数据，但这些数据通常是多个细胞的聚合测量，而研究者需要单细胞分辨率。现有生成模型（如扩散模型）难以直接处理整数数据，且缺乏系统性的反卷积方法

Method: 引入Count Bridges——一种整数上的随机桥过程，作为计数数据的扩散式模型精确类比，具有闭式条件分布用于高效训练和采样。通过期望最大化方法直接从聚合测量训练，将单元级计数作为潜变量

Result: 在整数分布匹配基准测试中取得最先进性能，优于流匹配和离散流匹配基线。成功应用于单细胞基因表达数据建模（核苷酸分辨率）和空间转录组斑点反卷积为单细胞计数谱

Conclusion: Count Bridges为生物计数数据的生成建模和反卷积提供了理论基础，可跨尺度和模态应用，解决了聚合观测数据的反卷积问题
...
TL;DR: 本文揭示了预训练模型对不可学习样本(UEs)的免疫漏洞，并提出BAIT方法通过双层优化绑定扰动与错误标签，有效克服预训练先验影响，保持数据不可学习性。


<details>
  <summary>Details</summary>
Motivation: 不可学习样本(UEs)作为数据保护策略，通过添加微小扰动误导模型学习虚假相关性而非真实语义。然而，作者发现当学习从预训练模型开始时，UEs存在根本性漏洞——预训练先验提供的丰富语义表示能让模型绕过UEs引入的捷径，捕获真实特征，从而抵消不可学习性。

Method: 提出BAIT（绑定人工扰动到错误目标）方法，采用双层优化框架：内层优化将扰动样本与真实标签关联，模拟标准数据-标签对齐；外层优化主动破坏这种对齐，通过强制实施错误标签-扰动绑定，将样本映射到指定的错误目标。这种机制有效覆盖先验的语义指导，迫使模型依赖注入的扰动，防止获取真实语义。

Result: 在标准基准测试和多个预训练骨干网络上的广泛实验表明，BAIT能有效减轻预训练先验的影响，并保持数据的不可学习性。

Conclusion: 本文揭示了预训练模型对UEs的免疫漏洞，并提出BAIT方法通过双层优化绑定扰动与错误标签，有效克服预训练先验的影响，为数据保护提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 科学应用需要传输模型能够泛化到训练时未见过的源分布和目标分布对，而传统方法通常只能处理训练时见过的特定分布对。

Method: 提出分布条件化传输（DCT）框架，将传输映射条件化于学习到的源分布和目标分布的嵌入表示上。该方法与底层传输机制无关，支持从流匹配到分布散度模型（如Wasserstein、MMD）等多种模型。

Result: 在合成基准测试和四个生物学应用中展示了DCT的实际性能优势：单细胞基因组学中的批次效应转移、质谱流式细胞术数据的扰动预测、造血过程中克隆转录动态学习、以及T细胞受体序列进化建模。

Conclusion: DCT框架能够实现分布对间的泛化传输，支持半监督学习，并在多个实际生物学应用中表现出优越性能，为科学应用中的分布传输问题提供了灵活有效的解决方案。
...
Result: 在三个独立数据集（SHHS、CFS、MrOS，总n=9,815）上，KindSleep在估计AHI得分方面表现优异（R2=0.917，ICC=0.957），在OSA严重程度分类方面始终优于现有方法，加权F1分数在0.827到0.941之间。
...
Method: 引入了ConTSG-Bench基准测试框架，包含大规模、对齐良好的数据集，涵盖多种条件模态和语义抽象层次，使用全面的指标评估生成保真度和条件遵循度。

Result: 通过定量基准测试和深入分析，揭示了当前方法的特征和局限性，特别是在复杂条件下精确结构可控性和下游任务效用方面存在关键挑战。

Conclusion: ConTSG-Bench为条件时间序列生成提供了首个系统评估框架，指出了该领域的关键挑战和研究方向，特别是在精确结构控制和复杂条件下的实用价值方面。
...
TL;DR: 提出一个基于分布风险敏感强化学习的均衡器参数优化框架，结合信息瓶颈潜在表示和条件风险价值优化，显著提升高速内存系统信号完整性


<details>
  <summary>Details</summary>
Motivation: 现有均衡器参数优化方法存在计算成本高、优化期望性能而非最坏情况性能、缺乏不确定性量化等问题，无法满足高速内存系统生产规模部署需求

Method: 提出分布风险敏感强化学习框架，集成信息瓶颈潜在表示（实现51倍加速）和条件风险价值优化，使用蒙特卡洛dropout量化认知不确定性，通过分位数回归实现最坏情况优化，PAC-Bayesian正则化提供泛化保证

Result: 在8个内存单元的240万个波形上验证，4抽头和8抽头均衡器配置平均提升37.1%和41.5%，最坏情况保证33.8%和38.2%，相比Q学习基线提升80.7%和89.1%，实现62.5%高可靠性分类，消除大多数配置的手动验证

Conclusion: 该框架为生产规模均衡器优化提供了具有认证最坏情况保证的实用解决方案，显著提升高速内存系统信号完整性
...
TL;DR: 提出一种在策略强化学习中高效训练扩散策略的新方法，通过将策略迭代与扩散过程对齐，仅需计算简单高斯概率，解决了扩散模型动作对数似然计算困难的问题。
...
Conclusion: 该方法有效解决了扩散策略在策略强化学习中的关键挑战，提供了一种高效训练扩散策略的新范式，能够处理多模态行为并实现高性能。
...
TL;DR: 该论文提出了一种基于KANs（Kolmogorov-Arnold网络）结构的多级训练方法，通过样条基函数的均匀细化和几何插值算子，实现了比传统方法高几个数量级的训练加速。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络训练缺乏结构保证，难以实现算法加速。KANs通过指定基函数展开学习激活函数，提供了更多可利用的结构，为开发高效训练算法创造了机会。

Method: 首先建立样条基KANs与带power ReLU激活的多通道MLPs之间的等价关系，分析基变换对梯度优化的影响。然后提出多级训练方法：通过均匀细化样条节点定义KANs序列，使用解析几何插值算子在不同模型间传递信息，形成"正确嵌套层次"的架构。

Result: 数值实验表明，多级训练方法在训练可比KANs或MLPs时，相比传统方法能实现几个数量级的精度提升，特别是在物理信息神经网络（PINNs）中效果显著。

Conclusion: 该工作展示了神经网络的原则性设计如何产生可利用的结构，从而开发出能显著提升训练性能的多级算法，为高效神经网络训练提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有解释方法常因缺失性偏差产生不可靠的特征重要性分数，而现有解决方案需要昂贵的重新训练或架构修改。本文挑战这一假设，认为缺失性偏差只是模型输出空间的表面伪影。

Method: 提出MCal方法：冻结基础模型，仅微调简单的线性头来校正输出空间中的缺失性偏差。这是一种轻量级后处理技术。

Result: MCal能持续减少缺失性偏差，在视觉、语言和表格医学基准测试中，其性能与甚至优于先前重量级方法。

Conclusion: 缺失性偏差可作为表面伪影有效处理，无需深度表示修复。MCal提供了一种简单有效的后处理解决方案，适用于多种领域。
...
TL;DR: 论文证明基于梯度的对齐方法天然集中在决定危害的位置，超过危害决策点的位置梯度为零，导致对齐浅层化，并提出基于恢复惩罚的新目标来创建全位置梯度信号。


<details>
  <summary>Details</summary>
Motivation: 解释为什么LLM的安全对齐是浅层的，理解基于梯度的对齐方法为何无法实现深度对齐，以及为什么对齐模型与基础模型之间的KL散度集中在早期token。

Method: 使用序列级危害的鞅分解，推导出对齐梯度的精确表征：位置t的梯度等于条件期望危害与评分函数的协方差。引入危害信息It量化每个位置对危害的影响，并基于恢复惩罚提出新目标。

Result: 证明超过危害决策点的位置梯度信号为零，标准对齐目标无法产生深度对齐，无论优化质量如何。推导出均衡KL散度跟踪危害信息It，并提出能创建全位置梯度信号的新目标。

Conclusion: 基于梯度的对齐方法天然导致浅层对齐，因为梯度集中在危害决策点。需要基于恢复惩罚的新目标才能实现深度对齐，这为经验上成功的数据增强技术提供了理论依据。
...
TL;DR: 本文提出了一个统一框架分析DP-SGD在两层ReLU CNN中的特征学习动态，发现隐私噪声导致次优特征学习，造成性能下降、公平性问题和对抗鲁棒性降低。


<details>
  <summary>Details</summary>
Motivation: 差分隐私学习在实践中会导致性能下降、公平性问题和对抗鲁棒性降低，但这些现象在现代非凸神经网络中的理论基础尚未充分探索。

Method: 引入统一特征中心框架分析DP-SGD在两层ReLU卷积神经网络中的特征学习动态，建立由特征噪声比(FNR)控制的测试损失界限。

Result: 1) FNR不平衡导致不同影响；2) 噪声对语义长尾数据影响更大；3) 噪声注入加剧对抗攻击脆弱性；4) 公开预训练+私有微调范式在特征分布显著偏移时不一定有效。

Conclusion: 隐私噪声通过特征噪声比机制导致次优特征学习，解释了差分隐私学习中的性能、公平性和鲁棒性问题，为理解这些现象提供了理论基础。
...
TL;DR: FedAFD是一个统一的多模态联邦学习框架，通过客户端双级对抗对齐和粒度感知融合，以及服务器端相似性引导的集成蒸馏，解决模态/任务差异和模型异构性问题，提升个性化性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态联邦学习方法忽视客户端个性化性能，难以处理模态/任务差异和模型异构性，需要更有效的框架来提升客户端和服务器端的学习效果。

Method: 客户端采用双级对抗对齐策略对齐局部和全局表示，设计粒度感知融合模块自适应整合全局知识；服务器端提出相似性引导的集成蒸馏机制，基于特征相似性聚合客户端表示并蒸馏到全局模型。
...
<details>
  <summary>Details</summary>
Motivation: 解决室内复杂环境下自主泊车的挑战性问题，传统方法在GPS信号缺失的室内环境中难以实现可靠定位和规划

Method: 采用分布式UWB辅助系统，结合大型语言模型（LLMs）进行路径规划，通过鲁棒的融合定位和轨迹跟踪技术

Result: 系统在真实车辆演示中得到验证，能够在具有挑战性的室内环境中实现可靠的自动泊车

Conclusion: U-Parking系统展示了UWB与LLM技术结合在自主泊车领域的应用潜力，为室内自动驾驶提供了可行的解决方案
...
TL;DR: EVMbench是一个评估AI代理检测、修复和利用智能合约漏洞能力的基准测试，包含117个精选漏洞，使用本地以太坊环境进行程序化评分。


<details>
  <summary>Details</summary>
Motivation: 随着公链智能合约管理大量价值，其漏洞可能导致重大损失。AI代理在读写和运行代码方面能力增强，需要评估它们在这个领域的能力，包括提升安全性和增加风险两方面。

Method: 引入EVMbench评估框架，基于40个代码库中的117个精选漏洞，在最真实的环境中通过测试和区块链状态进行程序化评分，使用本地以太坊执行环境。

Result: 评估了多个前沿AI代理，发现它们能够端到端地发现并利用实时区块链实例中的漏洞。

Conclusion: 发布了代码、任务和工具来支持持续测量这些能力，并为未来的安全工作提供基础。
...
TL;DR: BandPO提出动态概率感知的裁剪区间，解决PPO中固定裁剪边界对低概率高优势动作更新的限制问题


<details>
  <summary>Details</summary>
Motivation: PPO中的固定裁剪边界严格限制了低概率动作的向上更新空间，不成比例地抑制了高优势的尾部策略，导致熵快速崩溃

Method: 引入BandPO，用Band算子替代传统裁剪机制，将f-散度定义的信任区域投影到动态、概率感知的裁剪区间，通过凸优化问题求解

Result: BandPO在多种模型和数据集上持续优于传统裁剪和Clip-Higher方法，同时有效缓解熵崩溃问题

Conclusion: BandPO通过动态概率感知的信任区域投影，解决了PPO中固定裁剪边界的探索瓶颈，为LLM强化学习提供了更稳定的优化框架
...
TL;DR: 本文提出SC-USFL框架，结合语义通信与U型分割联邦学习，解决车联网中通信开销大和隐私风险问题，通过语义压缩和自适应调整提升效率。


<details>
  <summary>Details</summary>
Motivation: 传统车联网中的集中式学习存在通信开销大和隐私风险问题，而分割联邦学习又面临高维特征传输的通信瓶颈和标签隐私担忧。语义通信能够通过仅传输任务相关信息来缓解这些挑战。

Method: 提出语义通信增强的U型分割联邦学习框架（SC-USFL），包含预训练参数冻结的语义通信模块（SCM）用于压缩传输任务相关语义信息，以及网络状态监控模块（NSM）根据信道条件自适应调整语义压缩率。

Result: SC-USFL框架在资源受限的车载环境中展现出平衡通信负载、保护隐私和保持学习性能的良好潜力，通过本地化敏感计算和减少开销来增强标签隐私。

Conclusion: 该框架为车联网中语义通信与分割联邦学习的协同提供了有前景的解决方案，并指出了进一步推进两者协同的关键研究方向。
...
TL;DR: ∇-Reasoner：一种在推理时通过可微分优化改进LLM生成策略的迭代生成框架，相比传统搜索方法显著提升推理性能并减少计算开销
...
Method: 提出∇-Reasoner框架，将可微分优化集成到解码循环中，核心组件DTO利用LLM似然和奖励模型的梯度信号优化文本表示，并结合拒绝采样和加速设计

Result: 在数学推理基准上实现超过20%的准确率提升，同时减少约10-40%的模型调用次数，相比强基线显著更高效

Conclusion: 从零阶搜索转向一阶优化，为增强LLM推理提供了一种成本效益高的新范式
...
TL;DR: 研究评估了三种基于LSTM、GRU和Transformer的序列模型，通过蒙特卡洛dropout或证据输出层进行不确定性量化，用于1型糖尿病患者的血糖预测和不良血糖事件识别。Transformer模型结合证据输出头在预测准确性和不确定性校准方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 将原则性的不确定性量化整合到实时机器学习血糖预测系统中，以提高预测可靠性并识别不良血糖事件，这对于1型糖尿病患者的临床管理至关重要。

Method: 使用三种序列模型架构（LSTM、GRU、Transformer），通过两种不确定性量化方法（蒙特卡洛dropout和证据输出层），在HUPA-UCM糖尿病数据集上进行验证，并使用糖尿病技术学会误差网格进行临床风险评估。

Result: 基于Transformer的模型配备证据输出头提供了最有效的不确定性感知框架，实现了更高的预测准确性和更好的校准不确定性估计，其不确定性大小与预测误差显著相关。

Conclusion: 研究表明将原则性不确定性量化整合到实时机器学习血糖预测系统中具有重要价值，能够提供更可靠的风险评估和临床决策支持。
...
TL;DR: 提出一种名为WaterSIC的新型量化算法，通过信息论分析量化压缩问题，采用类似"注水"策略为权重矩阵的不同列分配不同量化比特率，在1-4比特量化中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有GPTQ等量化算法与信息论极限可能存在任意大的性能差距，需要设计更接近理论极限的量化方法来压缩大型语言模型的线性层权重。

Method: 提出WaterSIC算法，基于信息论分析量化压缩问题，采用类似"注水"策略为权重矩阵的不同列分配不同量化比特率，优化压缩长度与输出差异的权衡。

Result: WaterSIC算法在信息论极限的0.255比特速率差距内，在Llama和Qwen系列LLM的1-4比特量化中均达到新的SOTA性能。

Conclusion: 通过信息论指导的列级比特率分配策略，WaterSIC算法显著提升了量化压缩性能，为大型语言模型的高效部署提供了更优的量化解决方案。
...
TL;DR: MOUE引入虚拟宽度作为新的扩展维度，通过跨层复用通用专家池，在固定激活预算下将深度转换为虚拟宽度，相比传统MoE性能提升达1.3%


<details>
  <summary>Details</summary>
Motivation: 传统MoE模型在深度和宽度两个物理维度上的扩展性有限，需要新的扩展维度来突破这一限制，同时保持每个token的计算成本不变

Method: 提出MOUE框架，包含三个核心组件：交错旋转拓扑结构用于结构化专家共享，通用专家负载均衡用于深度感知的暴露校正，以及带轻量轨迹状态的通用路由器用于连贯的多步路由

Result: MOUE在不同扩展规模下比匹配的MoE基线性能提升达1.3%，能够渐进式转换现有MoE检查点并获得高达4.2%的增益，揭示了MoE架构的新扩展维度

Conclusion: MOUE成功引入了虚拟宽度作为MoE的新扩展维度，通过跨层复用专家池有效突破了传统深度和宽度的物理限制，为MoE架构提供了新的扩展路径
...
TL;DR: 提出基于Fisher-Rao流形的加权Karcher平均方法进行模型合并，解决传统参数空间方法的功能合并、表示崩溃和多专家合并问题


<details>
  <summary>Details</summary>
Motivation: 现有权重空间合并方法存在三个主要问题：1) 基于欧几里得坐标的操作无法有效合并预测功能；2) 源检查点差异大时导致表示崩溃（激活方差收缩和有效秩退化）；3) 几何方法难以扩展到N>2的专家合并

Method: 将模型合并形式化为在Fisher-Rao流形上计算加权Karcher平均，等价于最小化预测分布间的KL散度函数距离。提出使用轻量级球形代理的定点算法，保持范数并直接推广到多专家合并

Result: 在各种基准测试和崩溃诊断中，该方法在合并模型数量和异质性增加时保持稳定，始终优于先前基线方法

Conclusion: 通过将模型合并视为流形上的几何问题，提出了一种稳定、可扩展的方法，有效解决了传统参数空间合并方法的局限性
...
TL;DR: RefQuery是一个可扩展的多电器、多任务NILM框架，通过紧凑的电器指纹进行条件分解，允许一个共享模型服务多个电器而无需固定输出集，在资源受限的边缘设备上实现实时操作。


<details>
  <summary>Details</summary>
Motivation: 当前NILM领域虽然采用了深度学习方法，但跨域泛化仍然是一个挑战。现有迁移学习方法通常假设固定的电器集合，缺乏对现实世界部署的灵活性，不适合边缘设备，或者无法扩展到实时操作。

Method: RefQuery框架通过紧凑的电器指纹对分解进行条件化，保持预训练的分解网络完全冻结，仅通过轻量级的反向传播阶段学习每个电器的嵌入来适应目标家庭。这使得一个共享模型可以服务多个电器而无需固定输出集。

Result: 在三个公共数据集上的实验表明，RefQuery在单电器和多电器基线（包括基于Transformer的现代方法）之间提供了强大的精度-效率权衡。
...
TL;DR: 多运营商AMoD系统中，竞争环境通过强化学习框架改变定价和车队调度策略，导致价格降低和车队分布模式变化


<details>
  <summary>Details</summary>
Motivation: 现实AMoD市场将是竞争性的，但现有研究未能捕捉多运营商竞争动态，需要研究竞争对策略学习的影响

Method: 提出多运营商强化学习框架，两个运营商同时学习定价和车队再平衡策略，结合离散选择理论实现乘客分配和需求竞争内生涌现

Result: 竞争显著改变学习行为：相比垄断环境，竞争导致价格降低和车队定位模式差异；基于学习的方法对竞争随机性具有鲁棒性

Conclusion: 竞争从根本上改变AMoD系统学习策略，强化学习方法能有效处理竞争环境中的部分可观测对手策略，为现实竞争市场提供解决方案
...
TL;DR: BA-Logic是一种针对图神经网络的新型干净标签后门攻击方法，通过毒化模型内部预测逻辑而非修改训练标签来实现有效攻击。


<details>
  <summary>Details</summary>
Motivation: 现有图后门攻击需要修改训练标签，这在现实场景中不切实际。干净标签设置下的攻击研究不足，现有方法在该设置下效果不佳，核心问题在于无法毒化GNN模型的预测逻辑。

Method: 提出BA-Logic方法，通过协调毒化节点选择器和逻辑毒化触发器生成器来毒化GNN模型的内部预测逻辑，使触发器对预测变得重要。

Result: 在真实世界数据集上的大量实验表明，该方法能有效提升攻击成功率，在干净标签设置下超越了最先进的图后门攻击方法。

Conclusion: BA-Logic成功解决了干净标签图后门攻击的有效性问题，通过毒化模型预测逻辑而非修改标签，为这一现实但研究不足的领域提供了有效解决方案。
...
TL;DR: 提出Margin Cross-Entropy Loss (MCEL)损失函数，通过显式促进输出层logit间隔分离来提升神经网络对位错误的容忍度，无需依赖训练时的位翻转注入。


<details>
  <summary>Details</summary>
Motivation: 现有通过训练时注入位翻转的方法存在计算开销大、在高错误率下推理精度下降、难以扩展到大型神经网络架构等问题，需要一种更高效、可扩展的位错误容忍方法。

Method: 建立位错误容忍度与输出层分类间隔的直接联系，提出Margin Cross-Entropy Loss (MCEL)损失函数，该函数在保持标准交叉熵损失优化特性的同时，显式促进logit级别的间隔分离，并引入可解释的间隔参数来系统调整鲁棒性。

Result: 在多个复杂度不同的数据集、多样化的神经网络架构和各种量化方案上的实验表明，MCEL显著提高了位错误容忍度，在1%错误率下准确率提升高达15%。

Conclusion: MCEL方法简单易实现、高效，可作为标准CEL的直接替代方案，为神经网络鲁棒性提供了可扩展的理论基础，支持在近似计算和易错内存系统上的高效部署。
...
TL;DR: 该论文对多任务学习中的感知器模型进行了渐近分析，揭示了多任务组合如何通过隐式正则化改善泛化性能，并延缓双下降现象。


<details>
  <summary>Details</summary>
Motivation: 多任务学习通过共享相关任务间的共同信息来改善泛化误差，但挑战在于如何找到能够有效揭示这种共同信息的数学表达。本文旨在精确分析多任务感知器学习模型，理解多任务组合带来性能提升的根本原因。

Method: 采用精确的渐近分析方法，研究误设感知器学习模型的多任务学习公式。通过理论分析证明多任务组合等价于传统单任务学习加上额外的正则化项。

Result: 理论分析表明：1）多任务组合在渐近意义上等价于带有额外正则化项的传统公式；2）这些正则化项有助于改善泛化性能；3）多任务组合能够延缓双下降现象，并在渐近情况下缓解该现象。

Conclusion: 多任务学习通过隐式正则化机制改善泛化性能，其核心优势在于任务组合产生的正则化效应能够延缓并缓解双下降现象，为理解多任务学习的理论基础提供了新见解。
...
<details>
  <summary>Details</summary>
Motivation: 传统ISAC友好干扰方法需要窃听者的信道状态信息或精确到达角估计，这在实践中难以获取。现有方法对雷达感知不确定性不够鲁棒，且模型复杂度高，难以在实际系统中部署。
...
Result: 仿真表明：1) 显著提高保密速率；2) 降低块错误率；3) 对CSI不确定性和角度估计误差具有强鲁棒性；4) 模型压缩后性能损失可忽略；5) 即使在噪声AoA下也能满足CRLB约束。

Conclusion: 该深度学习驱动的友好干扰框架在实用ISAC损伤条件下有效，通过利用雷达反馈和神经网络优化，实现了无需窃听者信息的物理层安全增强，具有高鲁棒性和低复杂度。
...
<details>
  <summary>Details</summary>
Motivation: 传统RL智能体在单一固定奖励函数下训练，对奖励函数误设敏感，且难以适应任务偏好的变化，需要更鲁棒和可适应的训练框架

Method: 提出奖励条件化强化学习(RCRL)，通过条件化智能体于奖励参数化，从共享的离策略回放数据中学习多个奖励目标，使单个策略能表示奖励特定的行为
...
<details>
  <summary>Details</summary>
Motivation: 单位超球面上的聚类在许多领域都有重要应用（如基因表达分析、文本和图像分类），但传统聚类方法没有考虑球面的几何结构，不适合处理这类数据

Method: 基于d维广义Kuramoto模型，开发了一种专门用于单位球面S^{d-1}上数据点聚类的新算法

Result: 在合成和真实数据集上验证了方法的有效性，与传统聚类方法相比，在聚类准确度方面取得了相似或更好的结果

Conclusion: 提出的基于广义Kuramoto模型的算法能够有效处理单位超球面上的聚类问题，考虑了球面的几何特性，在准确度上优于或相当于传统方法
...
TL;DR: Aura是一个通用框架，通过三部分编码机制将异构外部信息整合到时间序列模型中，在航空维护场景中显著提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: 实际应用中时间序列预测需要整合多维甚至多模态的外部因素，这些异构信息具有不同的交互模式，传统单模态时间序列模型难以有效捕捉这些复杂关系。

Method: 基于航空维护场景中识别出的三种不同类型外部因素，Aura采用定制的三部分编码机制，根据外部信息与目标时间序列的交互模式进行组织和编码，将异构特征嵌入到成熟的时间序列模型中。

Result: 在中国南方航空公司三年大规模工业数据集（涵盖波音777和空客A320机队）上的实验表明，Aura在所有基线方法中均达到最先进的性能，并展现出卓越的适应性。

Conclusion: Aura作为通用增强框架，在航空安全和可靠性领域具有广泛应用潜力，能够有效整合异构外部信息以提升时间序列预测的准确性。
...
TL;DR: 提出基于最优生成流量的Aumann-Shapley归因理论，通过Wasserstein-2测地线解决基线选择问题，实现严格流形对齐和语义一致性


<details>
  <summary>Details</summary>
Motivation: 传统Shapley归因方法因启发式基线选择产生离流形伪影，现有生成方法存在几何效率低下和离散化漂移问题，需要建立严格的流形上归因理论

Method: 提出基于最优生成流量的Aumann-Shapley归因理论，证明梯度线积分是满足效率和几何公理（特别是重参数化不变性）的唯一函数形式；通过选择动能最小化的Wasserstein-2测地线（从先验分布到数据分布）解决路径模糊性

Result: 该方法在实验中优于基线方法，实现严格的流形对齐（流一致性误差趋近于零）和优越的语义对齐（结构感知总变差），恢复加性模型的经典Shapley值，并提供对流量近似误差的可证明稳定性边界

Conclusion: 通过将基线选择重构为变分问题，建立了基于最优传输流量的规范归因族，为后验XAI提供了严格流形对齐的Shapley归因理论框架
...
<details>
  <summary>Details</summary>
Motivation: 深度强化学习在机器人控制中应用困难，主要因为设计有效奖励函数复杂。现实任务需要同时优化多个目标，需要精确调整权重以获得期望策略特性

Method: 提出两阶段奖励课程：第一阶段使用简化的任务专用奖励函数训练智能体，确保有效探索；第二阶段引入包含能量效率等辅助行为目标的完整奖励。分析不同过渡策略，发现重用阶段间样本对训练稳定性至关重要

Result: 在DeepMind Control Suite、ManiSkill3和移动机器人环境中验证，方法简单有效，显著优于直接在完整奖励上训练的基线方法，对特定奖励权重具有更高鲁棒性

Conclusion: 两阶段奖励课程方法能有效解决多目标强化学习中的奖励设计问题，通过解耦任务目标与行为目标，实现更稳定高效的策略学习
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习在大规模模型（如Vision Transformer）中每轮通信开销过高，需要降低通信复杂度

Method: 将模型参数分成多个块（包括共享块），每个客户端只上传特定参数块；提出FedBCGD+算法，加入客户端漂移控制和随机方差缩减

Result: 理论分析显示通信复杂度比现有方法降低1/N倍（N为参数块数），收敛速度更快；实验结果表明优于现有最先进算法

Conclusion: 首次提出参数块通信方法训练大规模深度模型，有效降低通信开销，为联邦学习中的大规模模型训练提供了高效解决方案
...
TL;DR: Alt-FL提出了一种结合差分隐私、同态加密和合成数据的联邦学习隐私保护框架，通过三种新的交织方法实现隐私保护、学习质量和效率的平衡。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中隐私保护、学习质量和效率难以平衡：差分隐私会降低学习质量，同态加密带来巨大系统开销。需要一种能灵活权衡这些因素的隐私保护框架。

Method: 提出Alt-FL框架，包含三种交织方法：隐私交织(PI)、带差分隐私的合成交织(SI/DP)、带同态加密的合成交织(SI/HE)。使用基于轮次的交织策略结合DP、HE和合成数据。

Result: 在LeNet-5模型和CIFAR-10、Fashion-MNIST数据集上评估，针对多种重建攻击。结果显示：在高隐私保护级别下，PI实现最平衡的权衡；在中等隐私要求下，基于DP的方法更优。

Conclusion: Alt-FL提供了灵活的隐私-效率权衡方案，为不同隐私和资源约束下的隐私保护联邦学习方法选择提供了依据。
...
TL;DR: 提出一种可训练的特征量化层，用于压缩神经网络输入特征，减少边缘设备到服务器的数据传输，实现5-16倍压缩而不显著损失精度。


<details>
  <summary>Details</summary>
Motivation: 物联网中机器学习应用对计算和内存资源需求增长，但边缘设备资源有限。现有方法依赖将数据传输到远程服务器，但受带宽、延迟和能耗限制。需要减少数据传输量的解决方案。

Method: 提出任务特定的可训练特征量化层，使用sigmoid函数近似阶跃函数实现可训练量化阈值。通过连接多个sigmoid输出（称为位级软量化），实现与神经网络集成的可训练量化值。允许每个输入特征按用户定义位数量化。

Result: 相比标准量化方法表现更优，同时保持接近全精度模型的准确率。根据数据集不同，相比32位输入可实现5-16倍压缩，且无显著性能损失。

Conclusion: 提出的可训练特征量化层能有效压缩边缘设备到服务器的数据传输，在保持模型准确性的同时显著减少带宽需求，适用于资源受限的物联网环境。
...
TL;DR: 将AI监管构建为不确定条件下的机制设计问题，通过许可证机制让ML提供商基于模型性能下注，实现完美市场结果：不合格提供商自我排除，合格提供商参与。


<details>
  <summary>Details</summary>
Motivation: 高风险的ML应用需要严格监管，但策略性ML提供商常规避监管以降低开发成本。需要设计可执行的AI监管框架来解决这一挑战。

Method: 提出监管机制框架：将模型经验证据映射到市场份额许可证，提供商从许可证集合中选择，本质上是对模型满足监管要求的能力下注。证明机制具有完美市场结果当且仅当不合格分布集合构成一个信任集（闭凸概率测度集）。

Result: 建立了监管机制与不合格分布集合之间的对偶关系，连接了机制设计和不精确概率理论。通过实验验证了机制在监管虚假特征预测和公平性方面的实际应用。

Conclusion: 该框架为可执行的AI监管提供了理论基础，在机制设计和不精确概率的交叉领域提供了新见解，为AI监管发展奠定了基础。
...
TL;DR: 提出Stable-LoRA方法，通过权重收缩优化策略解决LoRA特征学习中的不稳定性问题，在保持非零初始化优势的同时提升性能。


<details>
  <summary>Details</summary>
Motivation: 尽管LoRA在实践中表现良好，但其理论基础不足，特别是在特征学习稳定性方面。研究发现LoRA在适当超参数和初始化下可以实现自稳定，但必要的非零初始化会破坏这种自稳定性，导致次优性能。

Method: 提出Stable-LoRA，一种权重收缩优化策略。通过在训练初期逐步收缩矩阵A，动态增强LoRA特征学习的稳定性，同时保留非零初始化的优势。

Result: 实验表明Stable-LoRA在各种模型和任务中持续优于其他基线方法，且不增加内存使用，计算开销可忽略不计。
...
TL;DR: 提出IAENet框架，用于术中不良事件的多标签早期预警，通过改进的TAFiLM模块融合静态与动态数据，并使用LCRLoss缓解类别不平衡，在5/10/15分钟预警任务上显著提升性能。
...
Method: 1) 构建首个多标签术中不良事件数据集MuAE；2) 提出基于Transformer的IAENet框架，包含改进的TAFiLM模块用于静态协变量与动态变量的鲁棒融合及复杂时序依赖建模；3) 引入带共现正则化的标签约束重加权损失(LCRLoss)缓解事件内不平衡并增强共现事件的结构一致性。
...
TL;DR: 通过干预性架构修改研究Transformer中的grokking现象，发现限制表征幅度和消除数据依赖注意力路由可以显著加速或完全消除延迟泛化


<details>
  <summary>Details</summary>
Motivation: 研究Transformer架构中的结构性自由度如何影响grokking（延迟泛化）现象，采用干预性方法而非事后分析来验证假设

Method: 1) 引入完全有界的球形拓扑，强制L2归一化；2) 均匀注意力消融，用均匀分布替代数据依赖的查询-键路由；3) 使用非交换S5置换组合作为负对照

Result: 球形拓扑将grokking开始时间减少20倍以上；均匀注意力消融模型在所有种子中实现100%泛化，完全绕过grokking延迟；但球形约束在S5任务中不加速泛化

Conclusion: 消除记忆阶段强烈依赖于架构先验与任务内在对称性的对齐，架构自由度显著影响grokking，提供了训练动态的结构性预测视角
...
<details>
  <summary>Details</summary>
Motivation: NVIDIA的2:4稀疏张量核心虽然能提供2倍吞吐量，但需要严格的50%剪枝，这会严重损害LLM推理精度（如Qwen3从54%降至15%）。而更温和的(2N-2):2N模式（如6:8，25%剪枝）能保持精度，但缺乏硬件支持，只能回退到密集计算，无法从稀疏性中获益。

Method: SlideSparse采用滑动窗口分解技术，将任意(2N-2):2N权重块重构为N-1个重叠的2:4兼容窗口，实现无损精度转换；激活提升技术将相应的激活重排融合到每令牌量化中，成本几乎为零。系统集成到vLLM框架中。
...
Conclusion: SlideSparse成功解锁了(2N-2):2N稀疏模式在商用GPU上的张量核心加速，建立了保持精度的LLM加速实用路径，代码已开源。
...
TL;DR: RIMs（递归推理机）是一种结合神经主干与递归推理机制的新框架，将TRMs表达为其特例并通过重加权组件提升性能，在多个推理基准上表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统神经推理器（如TRMs）与随机推理系统各有优势，前者依赖神经主干，后者使用专门的推理规则。本文旨在桥接这两种范式，通过将经典推理引擎的递归推理机制显式整合到神经框架中，以提升复杂问题的推理能力。

Method: 提出RIMs（递归推理机）框架，该框架显式地融入了受经典推理引擎启发的递归推理机制。将TRMs表达为RIMs的一个实例，并通过引入重加权组件对其进行扩展，从而增强推理能力。

Result: RIMs在多个具有挑战性的推理基准上（包括ARC-AGI-1、ARC-AGI-2和Sudoku Extreme）取得了更好的性能。此外，在表格数据分类等其他任务上也表现出色，甚至超越了TabPFNs。

Conclusion: RIMs成功地将经典推理引擎的递归推理机制整合到神经框架中，不仅统一了TRMs，还通过重加权组件提升了性能，为神经推理系统的发展提供了新的方向。
...
TL;DR: 提出两阶段联邦学习框架，通过行为聚类和联邦平均训练LSTM模型，用于分布式风电场的短期功率预测，保护数据隐私并处理异质性。


<details>
  <summary>Details</summary>
Motivation: 短期风电功率预测对电网调度和市场运营至关重要，但集中化收集涡轮机数据存在隐私、成本和异质性等问题。需要一种既能保护数据隐私又能处理分布式风电场异质性的解决方案。

Method: 两阶段联邦学习框架：第一阶段使用双轮盘选择初始化结合递归自动分割细化，基于长期行为统计对涡轮机进行聚类；第二阶段通过联邦平均为每个聚类训练特定的LSTM模型。

Result: 在丹麦400个独立涡轮机上的实验表明，DRS-auto方法能发现行为一致的聚类组，在保持数据本地性的同时达到有竞争力的预测精度。行为感知聚类始终优于地理分区，并与强大的k-means++基线方法相当。

Conclusion: 该框架为异构分布式风电场提供了一种实用的隐私友好解决方案，通过行为聚类和联邦学习实现了准确的短期功率预测，同时保护了数据隐私。
...
TL;DR: 论文提出超越词错误率(WER)的ASR评估方法，引入样本难度指数(SDI)来量化人口统计和声学因素对模型失败的影响，揭示隐藏的系统性偏见。


<details>
  <summary>Details</summary>
Motivation: 传统ASR系统主要使用词错误率(WER)评估，但这种方法存在局限性：1）无法捕捉语义保真度；2）掩盖了"多样性税"问题，即边缘化和非典型说话者因系统性识别失败而承受不成比例的负担；3）原始标记级指标忽略了语义层面的评估。

Method: 1）系统评估更广泛的非线性和语义指标；2）引入样本难度指数(SDI)，量化内在人口统计和声学因素如何驱动模型失败；3）通过数据制图映射SDI；4）使用EmbER和SemDist等指标揭示WER忽略的隐藏系统性偏见和模型间分歧。

Result: 研究发现EmbER和SemDist指标能够暴露WER忽略的隐藏系统性偏见和模型间分歧。通过SDI和数据制图，可以识别哪些人口统计和声学特征导致模型失败，为模型审计提供量化工具。

Conclusion: 这是构建稳健审计框架的第一步，使开发者能够在部署前审计和缓解ASR差异，实现前瞻性安全分析。新指标和方法为更全面、公平的ASR评估提供了基础。
...
TL;DR: Whisperer框架通过扩散模型预处理输入图像来增强冻结OCR模型的性能，使用行为克隆方法学习改进策略，在300k退化文本图像上实现了8%的字符错误率绝对降低。


<details>
  <summary>Details</summary>
Motivation: 冻结预训练模型虽然稳定高效，但在特定任务上由于数据分布不匹配而表现不佳。需要一种方法能够在不修改模型权重的情况下，通过优化输入来提升冻结下游模型的性能。

Method: 提出Whisperer视觉提示框架，使用扩散模型作为预处理器来调整输入像素空间。采用四阶段训练课程，通过行为克隆方法学习随机发现的改进策略：随机采样扩散中间输出，选择偶然改善CER的结果，然后训练模型复制这些成功案例。

Result: 在300k退化合成文本图像数据集上，实现了8%绝对（10.6%相对）的字符错误率降低，超越了CLAHE等手工设计的基线方法。整个训练过程仅需60 GPU小时，具有很高的样本效率。

Conclusion: 通过"低语"方式向冻结OCR模型输入优化后的图像，可以在不修改模型权重的情况下显著提升性能。该方法将随机发现的成功改进通过行为克隆系统化，为冻结模型适配提供了新思路。
...
TL;DR: 该论文首次建立了辩论与RLAIF之间的形式化联系，通过知识分歧的几何参数化分析辩论优势，发现当模型知识共享时辩论退化为RLAIF，知识分歧时辩论优势呈现从二次到线性的相变。


<details>
  <summary>Details</summary>
Motivation: AI安全领域提出了辩论和RLAIF两种可扩展监督方法，但缺乏形式化框架来关联它们或描述辩论何时具有优势。需要理解何时对抗性监督协议是合理的。

Method: 通过模型表示子空间之间的主角度参数化知识分歧的几何结构，推导辩论优势的精确闭式解。分析三种知识分歧机制（共享、单边、组合）并证明存在性结果。

Result: 当模型共享相同训练语料时，辩论退化为类似RLAIF的单智能体方法；当模型知识分歧时，辩论优势呈现从二次（辩论益处可忽略）到线性（辩论至关重要）的相变。证明了辩论可以实现任一模型单独无法达到的结果，但也发现在组合机制中足够强的对抗性激励会导致协调失败。

Conclusion: 首次建立了辩论与RLAIF的形式化联系，为理解对抗性监督协议的合理性提供了几何基础，并连接到从具有互补信息的模型中引出潜在知识的问题。
...
TL;DR: WavSLM是一个语音语言模型，通过将自监督WavLM表示量化和蒸馏到单个码本中，并优化自回归下一块预测目标，在单个令牌流中联合建模语义和声学信息，无需文本监督或文本预训练。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型展示了简单自回归训练可以实现可扩展和连贯的生成，但将此范式扩展到语音仍然具有挑战性，因为语义和声学信息存在纠缠。现有语音语言模型大多依赖文本监督、分层令牌流或复杂混合架构，偏离了在文本领域证明有效的单流生成预训练范式。

Method: 将自监督WavLM表示量化和蒸馏到单个码本中，然后优化自回归下一块预测目标。模型在单个令牌流中联合建模语义和声学信息，无需文本监督或文本预训练。

Result: 在一致性基准测试和语音生成方面实现了有竞争力的性能，同时使用更少的参数、更少的训练数据，并支持流式推理。

Conclusion: WavSLM展示了简单自回归训练范式可以扩展到语音领域，在单个令牌流中联合建模语义和声学信息，无需复杂的架构或文本监督，为语音语言模型提供了更简洁有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有时间序列聚类解释方法（基于特征归因或元数据）无法识别跨越聚类边界的转变，而反事实解释主要局限于监督学习场景，缺乏无监督聚类的统一解释框架。

Method: 提出GALACTIC框架：1) 局部层面通过聚类感知优化目标生成尊重目标聚类和基础聚类分配的反事实扰动；2) 全局层面基于最小描述长度(MDL)目标选择代表性反事实解释，利用超模性证明和贪心算法获得理论保证。

Result: 在UCR Archive上的实验表明，GALACTIC相比现有方法能生成显著更稀疏的局部反事实解释和更简洁的全局摘要，首次为聚类时间序列提供统一的反事实解释方法。

Conclusion: GALACTIC填补了无监督时间序列聚类反事实解释的空白，通过统一的局部-全局框架提供可解释的聚类边界转变分析，具有理论保证和实际有效性。
...
<details>
  <summary>Details</summary>
Motivation: 金融数据集常存在偏见，可能导致自动化系统做出不公平决策。需要一种方法既能生成合成金融数据，又能减轻与受保护属性相关的偏见。

Method: 提出FairFinGAN框架，基于Wasserstein GAN（WGAN），通过分类器将公平性约束直接整合到训练过程中，确保生成的合成数据既公平又保持下游预测任务的实用性。

Result: 在五个真实金融数据集上评估，与现有GAN数据生成方法相比，FairFinGAN在公平性指标上表现更优，且没有显著损失数据实用性。
...
TL;DR: 提出几何感知量化框架GAQ，通过解耦量化方案和对称感知训练，在压缩等变图神经网络的同时保持SO(3)对称性，实现高效准确的分子模拟。


<details>
  <summary>Details</summary>
Motivation: 等变图神经网络在分子模拟中很重要，但高计算成本和内存瓶颈限制了其应用。传统量化方法会破坏旋转敏感特征的SO(3)等变结构，导致误差和违反守恒定律。

Method: 提出几何感知量化框架GAQ：1) 幅度-方向解耦量化方案，分离不变长度和等变方向；2) 对称感知训练策略，对标量和矢量特征采用不同量化方案；3) 鲁棒的注意力归一化机制稳定低比特梯度。

Result: 在rMD17基准测试中，W4A8模型达到FP32基线精度(9.31 meV vs 23.20 meV)，局部等变误差比朴素量化降低30倍以上。在消费硬件上实现2.39倍推理加速和4倍内存减少。

Conclusion: GAQ框架成功压缩和加速等变模型，同时严格保持连续对称性，实现了稳定、能量守恒的分子动力学模拟，为纳米级时间尺度模拟提供了可行方案。
...
TL;DR: 提出基于信息流指导的KV缓存选择性重计算方法，通过注意力范数信号识别关键token，优化长上下文问答中的检索增强生成效率


<details>
  <summary>Details</summary>
Motivation: 长上下文问答中的检索增强生成受限于推理时对大规模检索上下文的预填充计算瓶颈。现有方法依赖启发式规则或表示差异，未能建模所选token是否能有效影响生成过程

Method: 将选择性KV重计算建模为信息流问题，利用查询的注意力范数信号识别语义相关且结构位置能传播信息的token；在推理一致的RoPE几何下计算，重建检索块的位置分配并引入信息流指导的块重排序策略

Result: 在LLM和VLM基准测试中，在可比效率预算下相比先前方法取得一致性能提升

Conclusion: 通过信息流指导的KV缓存选择性重计算，有效解决了长上下文RAG的推理效率瓶颈，实现了更好的性能-效率权衡
...
TL;DR: TS-BOSS：一种基于分数的时间序列因果发现方法，扩展了BOSS算法，通过置换搜索和grow-shrink树缓存实现高效的时间序列因果结构学习。


<details>
  <summary>Details</summary>
Motivation: 观测数据中的因果结构学习对科学和政策领域至关重要，但时间序列设置因时间依赖性带来挑战。需要将静态因果发现方法扩展到时间序列场景。

Method: TS-BOSS是BOSS的时间序列扩展，执行基于置换的动态贝叶斯网络结构搜索，利用grow-shrink树缓存中间分数计算，保持BOSS的可扩展性和性能。

Result: 在合成数据实验中，TS-BOSS在高自相关机制下特别有效，在可比精度下始终获得更高的邻接召回率。理论保证在适当假设下建立了TS-BOSS的可靠性。
...
TL;DR: 提出对抗性框架解决HAR中的主体间变异性问题，通过对抗任务鼓励主体不变特征表示，提升对新个体的泛化能力


<details>
  <summary>Details</summary>
Motivation: 人类活动识别中，由于不同个体执行相同活动的方式存在差异（主体间变异性），模型对新个体的泛化能力面临挑战

Method: 提出新颖的深度对抗框架，将主体间变异性概念整合到对抗任务中，鼓励学习主体不变的特征表示

Result: 在三个成熟的HAR数据集上使用留一主体交叉验证，性能优于先前方法；对抗任务有效减少了特征空间中的主体间变异性

Conclusion: 提出的对抗框架能有效处理HAR中的主体间变异性问题，提升模型对新个体的泛化能力，优于现有对抗任务方法
...
<details>
  <summary>Details</summary>
Motivation: 节点相似性估计是网络分析和图机器学习的基础任务，应用于聚类、社区检测、分类和推荐等。现有方法在稀疏、噪声或异构网络中可能表现不佳，需要一种更稳健、可解释且通用的节点相似性表示方法。

Method: 基于起始节点锚定的随机游走方法，通过Jaccard相似性衡量邻居结构相似性，偏向向结构相似邻居转移。将游走视为随机邻域采样器，生成部分节点排名，使用鲁棒排名聚合构建可解释的节点到节点亲和矩阵。

Result: 在合成图（随机块模型、LFR基准图）、表格数据的k近邻图和蛋白质相互作用网络上评估，TopKGraphs在稀疏、噪声或异构网络中表现稳健，相比标准相似性度量（Jaccard、Dice）、基于扩散的方法（个性化PageRank）和嵌入方法（Node2Vec）具有竞争性或更优性能。
...
TL;DR: 该论文质疑Sheaf神经网络(SNNs)中学习限制映射的必要性，通过引入固定为单位映射的Identity Sheaf Network基线，发现在异质图基准上性能与复杂SNN变体相当，且过平滑问题并未如理论预测那样更严重。


<details>
  <summary>Details</summary>
Motivation: 重新审视Sheaf神经网络的设计复杂性，质疑学习限制映射是否必要，因为过平滑问题也可以通过残差连接和归一化等技术缓解。研究旨在验证简单单位映射是否足以达到复杂SNN变体的性能。

Method: 引入Identity Sheaf Network基线，将所有限制映射固定为单位映射，与多种SNN变体在五个流行的异质图基准上进行性能比较。使用Rayleigh商作为归一化度量来比较不同模型的过平滑程度。

Result: 在五个异质图基准上，Identity Sheaf Network基线取得了与各种SNN变体相当的性能。使用Rayleigh商分析显示，训练后的网络中，基于扩散分析的SNN理论预测行为并未在经验上体现，Identity Sheaf Networks并未比SNN对应模型遭受更严重的过平滑问题。

Conclusion: 学习限制映射的额外复杂性可能不是必要的，简单的单位映射足以达到与复杂SNN变体相当的性能。理论预测的SNN优势在经验中并未明显体现，建议重新评估Sheaf神经网络设计的必要性。
...
TL;DR: 提出一种针对医疗表格数据的可解释原型神经网络模型，通过可训练的特征分块学习有意义的原型部分，实现可解释的概念预测，在保持竞争力的分类性能的同时提供透明度。


<details>
  <summary>Details</summary>
Motivation: 在医疗等关键领域，模型预测的可解释性与准确性同等重要。现有基于原型的深度神经网络主要针对计算机视觉任务，需要适应医疗记录这类表格数据，特别是需要处理诊断结果的离散化规范。

Method: 提出针对医疗表格数据的新模型，采用可训练的特征分块方法，从结构化数据中学习有意义的原型部分。这些原型表示为二进制或离散化的特征子集，使模型能够用人类可读的术语表达原型，与临床语言和基于案例的推理保持一致。模型通过将患者描述与网络潜在空间中学到的原型进行比较来进行可解释的概念预测。

Result: 在医疗基准数据集上的实验表明，该模型在分类性能上与广泛使用的基线模型具有竞争力，同时提供了透明度，弥合了临床决策支持中预测性能与可解释性之间的差距。

Conclusion: 提出的模型为医疗表格数据提供了一种固有的可解释神经网络方法，通过原型学习实现了概念驱动的预测，在保持良好性能的同时增强了临床决策的透明度和可信度。
...
TL;DR: OPSDC是一种通过自我蒸馏压缩推理过程的方法，让模型学会更简洁地推理，无需真实答案、token预算或难度估计器，仅通过自我蒸馏就能自动压缩简单问题同时保留困难问题的思考过程。


<details>
  <summary>Details</summary>
Motivation: 推理模型虽然会"大声思考"，但很多输出是噪声甚至有害的。冗余的token不仅浪费计算资源，还可能累积错误，因此需要一种方法让模型学会更简洁地推理。

Method: OPSDC（On-Policy Self-Distillation for Reasoning Compression）的核心思想是：让同一个模型在"简洁"指令下生成教师logits，然后在学生自己的推理过程中最小化每个token的反向KL散度。整个过程仅基于自我蒸馏，无需额外监督。

Result: 在Qwen3-8B和Qwen3-14B上，MATH-500任务实现了57-59%的token压缩，同时准确率提升9-16个百分点。在AIME 2024上，14B模型准确率提升10个百分点，压缩率达到41%。

Conclusion: OPSDC展示了自我蒸馏在推理压缩中的有效性，能够自动区分问题难度，对简单问题进行激进压缩，对困难问题保留必要的思考过程。研究表明推理模型的大量输出不仅是冗余的，甚至是有害的，每个不必要的token都可能累积错误。
...
TL;DR: 提出LWAIL方法，通过动态感知的潜在空间Wasserstein距离匹配状态分布，仅需少量无动作专家轨迹即可达到专家级性能。


<details>
  <summary>Details</summary>
Motivation: 传统模仿学习需要大量中等至高质量演示数据以及专家动作，这些在实际应用中往往难以获取。需要开发能够在仅使用少量状态轨迹（无动作信息）的情况下实现有效模仿的方法。

Method: 提出LWAIL框架：1) 预训练阶段使用随机生成的状态数据训练ICVF，构建动态感知的潜在空间；2) 在该潜在空间中使用Wasserstein距离进行状态分布匹配；3) 通过对抗学习实现模仿，仅需1个或少量状态轨迹。

Result: 在多个MuJoCo环境中实验表明，LWAIL优于现有的Wasserstein-based IL方法和对抗IL方法，仅用少量状态轨迹就能达到专家级性能。

Conclusion: LWAIL通过动态感知潜在空间的Wasserstein距离匹配，显著减少了对专家演示数据的依赖，仅需少量状态轨迹即可实现高效模仿学习，为实际应用提供了更可行的解决方案。
...
TL;DR: 提出一种Kraus结构输出层，将通用序列模型的隐藏表示转换为完全正定保迹量子操作，确保物理有效的量子态更新，在参数漂移场景下显著提升状态估计质量。


<details>
  <summary>Details</summary>
Motivation: 实时重构条件量子态是量子反馈控制的基础要求，但标准随机主方程求解器需要精确模型和已知参数，对参数失配敏感。神经网络序列模型可以拟合这些随机动力学，但无约束预测器可能违反物理性（如正定性或迹约束），导致不稳定展开和非物理估计。

Method: 提出Kraus结构输出层，将通用序列骨干网络（RNN、GRU、LSTM、TCN、ESN、Mamba）的隐藏表示转换为完全正定保迹量子操作。该层通过构造确保物理有效的状态更新，并与Neural ODE作为比较基线。

Result: 在参数漂移的随机轨迹上评估，揭示了门控机制、线性递归和全局注意力之间的权衡。在所有模型中，Kraus-LSTM表现最佳，比其无约束对应物提高状态估计质量7%，同时在非平稳状态下保证物理有效的预测。

Conclusion: Kraus结构输出层为量子态估计提供了物理约束的序列建模框架，在参数漂移场景下显著提升性能并确保物理有效性，其中Kraus-LSTM架构表现最优。
...
TL;DR: 研究中文开源大语言模型的政治审查行为，评估诚实诱导和谎言检测技术，发现某些方法能提高真实回答但无法完全消除虚假响应。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要在人工构造的撒谎模型上评估诚实诱导和谎言检测技术，但这些人工构造可能无法反映自然发生的欺骗行为。本研究利用中国开发者开发的开源LLM作为测试平台，这些模型因政治审查训练而经常在敏感话题上产生虚假回答，为研究自然欺骗行为提供了真实场景。

Method: 使用Qwen3等中文开源LLM作为测试平台，这些模型在法轮功、天安门抗议等政治敏感话题上经常产生虚假回答。评估多种诚实诱导技术（如无聊天模板采样、少样本提示、通用诚实数据微调）和谎言检测技术（如让审查模型自我分类、基于无关数据训练的线性探针）。

Result: 诚实诱导方面：无聊天模板采样、少样本提示和通用诚实数据微调最可靠地增加真实回答。谎言检测方面：让审查模型自我分类其回答的性能接近未审查模型的上限，基于无关数据训练的线性探针是更便宜的替代方案。最强诚实诱导技术也能迁移到DeepSeek R1等前沿开源模型。但所有技术都无法完全消除虚假响应。

Conclusion: 中文开源LLM的政治审查行为为研究自然欺骗提供了有价值的测试平台。某些诚实诱导和谎言检测技术能有效提高模型真实性，但无法完全解决虚假回答问题。研究强调了在自然欺骗场景中评估这些技术的重要性，并发布了所有提示、代码和转录文本。
...
TL;DR: 提出三阶段框架：先用廉价不完美标签进行监督预训练，再通过自监督学习微调，以解决优化和仿真问题中机器学习代理模型的标签质量和收敛难题。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习代理模型方法（监督/自监督学习、软/硬可行性约束）面临昂贵高质量标签依赖或困难优化景观的固有挑战，需要在标签质量和模型性能之间权衡。

Method: 三阶段框架：1) 收集廉价不完美标签；2) 监督预训练；3) 自监督学习微调。理论分析表明标签只需将模型置于吸引盆内，证明只需少量不精确标签和训练轮次。

Result: 在非凸约束优化、电网运行和刚性动力系统等挑战性领域验证，显示该方法带来更快收敛、改进的准确性、可行性和最优性，总离线成本降低高达59倍。

Conclusion: 提出的简单三阶段策略有效解决了机器学习代理模型在优化和仿真问题中的标签质量与性能权衡问题，显著降低训练成本并提升模型质量。
...
TL;DR: POET-X：一种可扩展且内存高效的变体，通过正交等价变换显著降低计算成本，使单张H100 GPU能够训练十亿参数LLM


<details>
  <summary>Details</summary>
Motivation: POET框架虽然提供强大的训练稳定性，但其原始实现由于密集矩阵乘法导致高内存消耗和计算开销，需要更高效的实现

Method: POET-X是POET的可扩展变体，通过优化正交等价变换实现，显著降低计算成本，同时保持谱保持特性

Result: POET-X在单张Nvidia H100 GPU上成功预训练十亿参数LLM，而AdamW等标准优化器在相同设置下内存不足，同时保持了POET的泛化和稳定性优势

Conclusion: POET-X为大规模语言模型训练提供了高效稳定的解决方案，显著提升了吞吐量和内存效率，解决了LLM训练中的核心挑战
...
TL;DR: 提出基于字典模式熵(DPE)的因果方向推断框架，整合算法信息论和香农信息论，通过最小化模式层面不确定性实现符号序列的因果发现。


<details>
  <summary>Details</summary>
Motivation: 符号序列的因果方向发现具有挑战性，因为缺乏函数模型和噪声假设。需要一种能够处理符号序列、识别驱动因果关系的具体子模式的方法。

Method: 提出字典模式熵(DPE)框架，将因果关系解释为候选原因中紧凑、基于规则的模式系统地约束效应。构建方向特定的字典，使用基于熵的度量量化其影响，通过最小不确定性准则推断因果方向。

Result: 在多种合成系统（延迟位翻转扰动、AR(1)耦合、1D斜帐篷映射、稀疏过程）中始终实现可靠性能，优于或匹配基于AIT的竞争方法。在生物和生态数据集中表现有竞争力。

Conclusion: 最小化模式层面不确定性提供了一个稳健、可解释且广泛适用的因果发现框架，特别适用于符号序列的因果方向推断。
...
TL;DR: 该研究从概率机器学习角度分析Collatz总停止时间τ(n)，开发了贝叶斯负二项回归和基于奇块分解的生成模型两种方法，发现低阶模结构是τ(n)异质性的关键驱动因素。


<details>
  <summary>Details</summary>
Motivation: 从概率机器学习视角研究Collatz总停止时间τ(n)的统计特性，特别是其偏斜、过度分散和算术异质性，旨在建立预测模型并理解其分布规律。

Method: 1. 贝叶斯分层负二项回归(NB2-GLM)：使用log n和n mod 8作为协变量预测τ(n)，通过后验和后验预测分布量化不确定性。2. 基于奇块分解的机制生成近似：对奇数m，将3m+1分解为2^{K(m)}m'，随机化块长度，通过Dirichlet-multinomial更新校准。

Result: 在测试数据上，NB2-GLM比奇块生成器获得显著更高的预测似然。将块长度分布条件化到m mod 8后，生成器的分布拟合明显改善，表明低阶模结构是τ(n)异质性的关键驱动因素。

Conclusion: Collatz总停止时间τ(n)表现出显著的算术异质性，低阶模结构是其分布特性的重要决定因素。贝叶斯回归模型在预测性能上优于机制生成模型，但后者通过条件化模结构可显著改善拟合效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有非马尔可夫时间序列学习方法（如RNN、神经控制微分方程、Transformer）依赖隐式记忆机制，存在可解释性差、长时训练困难的问题，需要一种原则性的显式特征表示。

Method: 提出Volterra签名VSig(x;K)，通过时间核K加权输入路径x，将其展开到张量代数中，利用Volterra-Chen恒等式建立理论框架，支持核技巧和线性状态空间ODE表示。

Result: 证明了注入性（增强下的可识别性）和无限维路径空间上的通用逼近定理；展示了核技巧的适用性，内积可通过双参数积分方程计算；对指数型核，Volterra签名可解线性ODE；在实际和合成数据动态学习任务中优于经典路径签名基线。

Conclusion: Volterra签名作为历史依赖系统的稳健、计算可行的特征映射，结合时间重参数化不变性，为数据科学提供了理论保证和实用价值，在动态学习任务中表现优异。
...
TL;DR: 提出了一种利用预测信息增强独立性检验的方法，在保持最坏情况鲁棒性的同时，当预测准确时显著提高样本效率。


<details>
  <summary>Details</summary>
Motivation: 传统的非参数独立性检验在有限样本情况下样本复杂度很高，与支撑集大小呈多项式关系。作者希望利用预测信息来改进这一状况，同时保持最坏情况下的有效性。

Method: 采用增强分布检验框架，设计包含辅助预测信息的独立性检验器。提出了：1）基于预测误差自适应减少样本复杂度的二元离散分布独立性检验器；2）扩展到高维多元设置的d个随机变量独立性检验；3）匹配的最优样本复杂度下界证明。

Result: 所设计的检验器在保持最坏情况鲁棒性的同时，当预测准确时能显著提高样本效率。证明了检验器达到最优样本复杂度。

Conclusion: 通过利用预测信息，可以在不牺牲最坏情况有效性的前提下，显著提高独立性检验的样本效率，为统计推断提供了新的增强框架。
...
TL;DR: 卷积网络的局部性和权重共享改变了梯度下降边缘稳定性现象中的隐式正则化，使其能在高维球面数据上泛化，而全连接网络在该场景下会过拟合。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明，全连接网络的边缘稳定性正则化仅由全局输入几何决定，无法防止在高维球面等困难分布上的过拟合。本文旨在探索局部性和权重共享如何改变这一现象。

Method: 通过理论分析证明，当感受野大小m相对于环境维度d保持较小时，具有局部性和权重共享的网络能在球面数据上泛化，泛化率为n^{-1/6 + O(m/d)}。同时分析自然图像的块几何，验证标准卷积设计产生的块分布适合这种稳定性机制。

Result: 证明局部性和权重共享将学习到的滤波器耦合到低维块流形，从而绕过环境空间的高维度。理论泛化率表明卷积网络能在全连接网络必然失败的场景下成功泛化。

Conclusion: 权重共享将滤波器耦合到低维块流形，使卷积网络能够利用边缘稳定性机制实现泛化，这系统解释了卷积网络相对于全连接基线的优越泛化能力。
...
TL;DR: 本文研究了过参数化ReLU模型在随机高维数据上梯度下降的隐式偏差，发现其近似于最小L2范数解，与数据维度相关。


<details>
  <summary>Details</summary>
Motivation: 过参数化机器学习模型（如神经网络）的训练目标通常存在多个全局最小值。梯度下降等优化算法会收敛到特定的全局最小值，这种选择被称为"隐式偏差"。先前研究要么表明在最坏情况下不存在隐式偏差，要么在正交数据下恰好对应最小L2范数解。本文旨在填补这两个极端之间的空白。

Method: 使用原始-对偶分析框架，仔细跟踪预测值、数据张成系数的演化及其相互作用。分析表明，在随机高维数据上，ReLU激活模式会快速稳定，从而能够表征梯度下降的隐式偏差。

Result: 对于足够高维的随机数据，梯度下降的隐式偏差以高概率近似于最小L2范数解，两者之间的差距为Θ(√(n/d))，其中n是训练样本数，d是特征维度。

Conclusion: 本文建立了随机高维数据下浅层ReLU模型梯度下降隐式偏差的理论特征，填补了最坏情况分析和正交数据分析之间的空白，为理解过参数化模型的优化行为提供了新的理论洞见。
...
TL;DR: 提出一种在个体化决策规则中整合人口统计平等和条件人口统计平等约束的新框架，通过扰动无约束最优解实现高效计算


<details>
  <summary>Details</summary>
Motivation: 个体化决策规则在个性化营销、医疗保健和公共政策等社会应用中日益普及，但基于有偏数据训练的算法可能对性别、种族或语言等敏感属性定义的少数群体造成不成比例的伤害，存在歧视风险

Method: 提出新框架，将人口统计平等和条件人口统计平等约束纳入最优个体化决策规则估计中，通过扰动无约束最优解获得理论最优解，实现计算高效

Result: 推导了策略价值和公平约束项的收敛率，通过综合模拟研究和俄勒冈州健康保险实验的实证应用验证了方法的有效性

Conclusion: 提出的框架能够有效解决个体化决策规则中的公平性问题，在保持决策效果的同时满足人口统计平等约束，为公平机器学习提供了实用解决方案
...
Method: 提出贝叶斯监督因果聚类（BSCC）方法，以处理效应作为结果指导聚类过程，识别协变量特征和处理效应均相似的个体亚组

Result: 在模拟数据集和真实世界数据集（第三次国际卒中试验）上评估BSCC，验证了该框架的实际应用价值

Conclusion: BSCC能够有效识别在协变量特征和处理效应方面均相似的个体亚组，为个性化决策提供支持
...
TL;DR: 提出使用非对称Shapley值作为临床预测中特征重要性评估的更好方法，特别适用于基因组学与临床变量混合的高维预测模型


<details>
  <summary>Details</summary>
Motivation: 传统临床预测中评估高维特征（如基因组学）重要性时，仅通过将其添加到传统临床变量中观察预测性能变化的方法存在问题，因为它没有考虑变量间的共线性或已知的依赖方向性

Method: 提出使用非对称Shapley值来量化混合维度预测模型中的特征重要性，特别关注疾病状态作为基因组效应的中介变量这一临床相关场景，并推导了计算局部和全局非对称Shapley值的高效算法

Result: 局部非对称Shapley值被证明对推理非常有用，而全局非对称Shapley值通过将预测性能指标分解为各特征的贡献来提供解释性，以结直肠癌患者无进展生存期预测为例进行了说明

Conclusion: 非对称Shapley值是评估临床预测模型中高维特征重要性的更合适替代方法，能够更好地处理变量间的共线性和依赖关系，为临床决策提供更可靠的解释
...
TL;DR: 论文提出了四种不同的可容许性几何结构，并证明它们两两不嵌套，表明可容许性标准是相对且不可约简的。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索序列和无分布推断中不同可容许性几何结构之间的关系，理解它们之间的兼容性和差异，揭示可容许性标准的相对性本质。

Method: 通过几何分析方法，比较四种可容许性标准：Blackwell风险支配、随时有效可容许性、边际覆盖有效性和Cesàro可接近性可容许性，并证明它们之间的分离定理。

Result: 证明了四种可容许性程序类别两两不嵌套，每种几何都有不同的最优性证书，鞅相干性在不同标准中具有不同的必要性条件。

Conclusion: 可容许性是标准相对的，不同几何结构在空间、偏序和性能度量上操作不同的约束集，导致它们几何上不兼容，不存在统一的可容许性概念。
...
TL;DR: 本文为经验风险最小化决策树建立了全面的统计理论，包括高维回归和分类中的尖锐oracle不等式、最小化最优速率，并引入了一种新的函数类来捕捉稀疏性、各向异性平滑度和空间异质性。


<details>
  <summary>Details</summary>
Motivation: 尽管全局最优经验风险最小化决策树在计算上变得可行且经验上成功，但其统计性能的严格理论保证仍然有限。本文旨在为ERM决策树开发全面的统计理论。

Method: 使用基于经验局部化Rademacher复杂度的新颖均匀集中框架，建立尖锐的oracle不等式。在子高斯性假设下推导主要结果，并在重尾噪声设置下提供鲁棒保证。

Result: 建立了ERM决策树的尖锐oracle不等式，表征了可解释性-准确性权衡。在PSHAB函数类上推导了最小化最优速率，该函数类能捕捉稀疏性、各向异性平滑度和空间异质性。

Conclusion: 研究结果为ERM决策树的最优性提供了原则性基础，并引入了经验过程工具，这些工具广泛适用于其他高度自适应、数据驱动的程序。
...
TL;DR: 本文从统计学角度综述了生成式AI模型产生的合成数据的使用现状，分析了合成数据在支持下游发现、推断和预测时的有效使用条件，指出了常见陷阱，并提出了原则性使用框架和实践建议。


<details>
  <summary>Details</summary>
Motivation: 生成式AI模型的兴起极大地扩展了合成数据在科学、工业和政策领域的可用性和使用。这些发展为数据分析开辟了新的可能性，但也引发了关于合成数据何时能够以有效、可靠和原则性方式使用的基本统计问题。

Method: 本文从统计学角度综述了当前合成数据生成和使用的现状，调查了现代生成模型的主要类别、预期用例和优势，同时强调了它们的局限性和特征性失败模式。还考察了将合成数据视为真实观测替代品时出现的常见陷阱。

Result: 论文识别了合成数据使用中的关键问题，包括模型误设导致的偏差、不确定性衰减和泛化困难。基于这些见解，讨论了合成数据原则性使用的新兴框架。

Conclusion: 论文提出了实践建议、开放问题和注意事项，旨在指导方法开发者和应用研究人员。强调了在明确假设下使用合成数据的重要性，以确保其能够有意义地支持下游发现、推断和预测。
...
TL;DR: 论文提出后验温度调节作为奇异统计模型的统一响应框架，将WAIC、WBIC和奇异波动等经典指标与热力学响应函数联系起来，为奇异贝叶斯学习中的复杂性、预测变异性和结构重组提供解释框架。


<details>
  <summary>Details</summary>
Motivation: 奇异统计模型（如混合模型、矩阵分解、神经网络）违反常规渐近理论，参数不可识别且Fisher几何退化。虽然奇异学习理论通过实对数典型阈值和奇异波动等不变量描述边际似然行为，但这些量难以操作解释。同时，WAIC和WBIC等广泛使用的准则与底层奇异几何脱节。

Method: 提出后验温度调节诱导后验分布的单参数变形，其相关可观测量生成热力学响应函数层次结构。建立通用协方差恒等式连接温度期望的导数与后验波动，将WAIC、WBIC和奇异波动纳入统一响应框架。形式化可观测量代数，商掉不可识别方向，在奇异模型中构造结构上有意义的序参数。

Result: 在对称高斯混合、降秩回归和过参数化神经网络等典型奇异示例中，经验证明温度调节下的相变行为：序参数坍缩、敏感性峰值、复杂性度量与后验几何结构重组对齐。奇异学习理论的经典量获得自然热力学解释：RLCT主导自由能斜率，奇异波动对应温度自由能曲率，WAIC测量预测波动。

Conclusion: 热力学响应理论为解释奇异贝叶斯学习中的复杂性、预测变异性和结构重组提供了自然组织框架，将传统统计准则与底层几何结构统一起来，为奇异模型分析提供了新的操作工具和理论视角。
