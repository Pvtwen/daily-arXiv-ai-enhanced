...
Result: 方案提高了数据符号能量和频谱效率，相比现有方案具有更低的误码率和复杂度。

Conclusion: 差分通信方案在Zak-OTFS系统中有效，提升了性能和效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据驱动方法在新数据上表现不佳，且物理通道通常时变，但低延迟和有限计算资源限制了在线训练。因此，设计离线训练的神经网络以在未知通道上稳健运行。

Method: 提出生成合成训练数据集的设计准则，确保神经网络在新通道上达到特定MSE。进一步提出基准设计，适应不同通道特性。

Result: 神经网络在不同复杂度的架构下均实现稳健泛化，适用于固定和可变延迟扩展的无线通道。

Conclusion: 离线训练的神经网络设计方法无需通道信息或参数更新，适用于实际无线通信场景。
...
<details>
  <summary>Details</summary>
Motivation: 需要统一处理多种生物信号（如ECG和EEG）并解决类别不平衡问题，以提升患者评估的准确性和同步监测能力。
...
<details>
  <summary>Details</summary>
Motivation: 城市环境中GNSS定位常因建筑物遮挡和多径效应导致误差，需改进现有方法以提高定位精度。

Method: 使用RF、GBDT和SVM三种模型对GPS信号进行LOS/NLOS分类，仅当所有分类器一致且置信度超阈值时才选择卫星。

Result: 实验表明，该方法显著提高了定位成功率和接收器包含率，尽管因卫星数量减少导致位置边界略有增加。

Conclusion: 该方法有效提升了城市GNSS环境中的定位可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在联合感知与通信优化中缺乏全局最优性或需要额外降秩步骤，本文旨在解决这一问题。

Method: 采用SDR优化框架，并引入独立波束成形策略作为性能基准。
...
Method: 提出MM-AFDM-IM方案，动态选择不同chirp的星座模式，并通过星座模式选择和chirp激活的动态模式传递额外信息比特。此外，讨论了模式选择策略并推导了误码率上限。

Result: 仿真结果表明，MM-AFDM-IM在频谱和能量效率上优于传统方案。

Conclusion: MM-AFDM-IM是一种有效的AFDM优化方案，显著提升了性能，适用于高移动性通信场景。
...
<details>
  <summary>Details</summary>
Motivation: 传统VLP因调制LED的高成本和操作复杂性限制了广泛应用，uVLP通过利用未调制光源解决了这些问题。

Method: 论文分析了uVLP的基本原理，与传统VLP对比，并根据接收器技术分类为强度法和成像法，提出了一种分类法。

Result: uVLP提供了一种低成本、高效且易于部署的室内定位方案，但仍需解决技术挑战。
...
<details>
  <summary>Details</summary>
Motivation: 均匀线性阵列只能估计方位角，而UCA能提供360度方位角和仰角信息，因此需要一种高效的方法来估计二维DOA。

Method: 通过量化方位角并计算协方差，结合显式公式估计仰角，计算简单。

Result: 数值结果表明，该方法能有效估计方位角和仰角，并可作为高精度多维搜索的起点，且在非均匀噪声下仍有效。

Conclusion: 该方法计算简单，适用于实时处理，且在噪声环境下表现良好。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型无法准确捕捉多频操作和时变调制下的散射行为，需要更先进的建模方法。

Method: 扩展多端口S参数模型，结合结构散射、互耦和非数字调制，适用于时变多端口结构。

Result: 模型验证了其准确性和实用性，适用于广泛的通信和传感系统。

Conclusion: 该模型为多频和时变调制系统提供了精确的分析和优化工具。
...
Method: 使用FCN估计重叠事件数量，结合CS模块重构信号，并通过磁流式细胞术（MFC）验证。

Result: 相比传统算法，恢复率提高21%，分类准确性超过97%。

Conclusion: 该框架为下一代非光学单细胞传感平台奠定了基础，扩展了细胞术在精准医疗中的应用。
...
<details>
  <summary>Details</summary>
Motivation: 电价短期预测对电力市场竞价策略至关重要，但现有数据驱动方法依赖输入数据质量，研究探索异步电价数据的潜在价值。

Method: 采用先进模型集成方法，结合德国-卢森堡、奥地利和瑞士等市场的早期GCT电价数据，对比利时和瑞典市场进行预测。

Result: 预测精度显著提升（比利时22%，瑞典9%），且在极端市场条件下同样有效；但频繁模型校准带来高计算成本。

Conclusion: 异步电价数据能有效提升预测精度，但需权衡计算成本；研究为欧洲互联电力市场的竞价策略优化提供了实用指导。
...
<details>
  <summary>Details</summary>
Motivation: 探索长时间强化学习对小语言模型在多样化推理任务中的影响，并识别有效训练的关键因素。

Method: 采用可验证奖励任务、改进的GRPO、KL正则化、剪裁比和周期性参考策略重置等技术。

Result: 模型在数学、编程和逻辑任务中分别提升14.7%、13.9%和54.8%。

Conclusion: 研究为小语言模型的长期强化学习提供了实用技术，并公开模型以促进进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 识别并行计算的局限性，特别是在数学推理、物理模拟和顺序决策等需要依赖计算步骤的任务中。

Method: 通过复杂性理论形式化并行与顺序计算的区分，并分析现有并行架构的局限性。

Result: 证明并行计算在顺序性任务中存在根本性限制。

Conclusion: 强调在AI发展中，除了并行计算，还需要重视顺序计算的扩展，以应对复杂推理任务。
...
Result: 验证测试结果显示该框架在启动思维过程方面具有潜力。

Conclusion: 结合心理意象的机器思维框架为自主推理和多领域知识整合提供了新方向。
...
Result: 实验表明，该方法在准确性和处理时间上优于七种现有方法，且在线K-d树融合进一步提升了效率。

Conclusion: IncA-DES框架有效解决了数据流中的概念漂移问题，并在实际应用中表现出色。
...
Result: Assay2Mol在生成候选配体分子方面优于现有方法，且更易合成。

Conclusion: Assay2Mol为早期药物发现提供了一种高效且实用的工具。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统k均值聚类在排名向量上的局限性，为个性化和大规模决策提供支持。

Method: 提出KRCA算法，结合k均值初始解和分支定界法，优化聚类效果和计算效率。

Result: KRCA在合成和真实数据集上表现优于基线方法，计算速度快且质量高。

Conclusion: KRC问题及其算法在理论和实践上均有重要意义，为未来研究提供了基础。
...
TL;DR: 论文研究了[0,1]值回归问题，提出了新的损失函数（betting loss），实现了无需方差知识的方差自适应边界。


<details>
  <summary>Details</summary>
Motivation: 探讨在[0,1]值回归问题中，是否存在一种损失函数能够实现优于一阶边界的方差依赖边界。

Method: 提出了一种新的损失函数（betting loss），并验证其性能。
...
<details>
  <summary>Details</summary>
Motivation: 异构表格数据集在元学习中的表示仍是一个开放问题，现有方法依赖通用表示，本文提出针对特定元任务的表示学习方法。

Method: 提出两种方法：深度度量学习和地标重建，要求表示能捕捉地标特性。

Result: 实验表明，提出的编码器能有效学习与地标对齐的表示，但在元任务中性能提升不明显。

Conclusion: 方法虽能学习特定表示，但直接应用于元任务时效果有限。
...
<details>
  <summary>Details</summary>
Motivation: 尽管PINNs在结合物理原理和数据驱动建模方面表现出色，但其在训练域外的外推性能较差，且对激活函数的选择高度敏感。

Method: 采用迁移学习方法在扩展训练域内使用少量精心选择的配置点，并提出一种自适应激活函数，为标准激活函数的线性组合。

Result: 实验表明，该方法在外推域内平均减少了40%的相对L2误差和50%的平均绝对误差，且计算成本未显著增加。

Conclusion: 该方法有效提升了PINNs的外推性能，为复杂科学和工程问题提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM微调的任务数据集混合比例选择依赖人工启发式方法，缺乏原则性和可扩展性。

Method: TASKPGM利用行为差异（如Jensen Shannon Divergence和Pointwise Mutual Information）建模任务关系，通过MRF框架优化任务比例。

Result: 在Llama 2和Mistral上，TASKPGM在MMLU和BIGBench等评估套件中表现一致优于传统方法。

Conclusion: TASKPGM不仅提升性能，还提供了任务影响和混合组成的可解释性，是LLM高效微调的有力工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有DCT方法局限于离散一维空间，难以处理复杂数据（如图像），且MMD在多分布对评估中信息不足。
...
Conclusion: Bootseer有效解决LLM训练启动瓶颈，提升效率。
...
<details>
  <summary>Details</summary>
Motivation: 探索推理微调模型中回溯行为的机制，以理解其增强能力的来源。

Method: 通过识别基础模型中的特定方向，并利用该方向操控蒸馏后的推理模型，分析其回溯行为。
...
<details>
  <summary>Details</summary>
Motivation: 解决神经接口数据共享中的隐私问题，探索联邦学习在高带宽神经解码中的应用。

Method: 引入基于联邦学习的神经解码，并在开放和闭环场景中系统评估其性能与隐私。

Result: 开放环中FL显著优于本地学习，闭环中需调整方法，本地学习表现更好但隐私风险更高。
...
<details>
  <summary>Details</summary>
Motivation: 数据异构性因多种冲突因素而普遍存在，其复杂性需要专家介入解决。现有方法多关注数据结构和模式冲突，忽视了数据转换的关键作用。随着AI应用的扩展，高效数据准备和转换变得至关重要。

Method: 系统分类并分析了解决数据格式差异的策略，探讨了每种策略的挑战。

Result: 揭示了数据异构性的复杂性及其对AI学习效率的影响，强调了数据转换的重要性。

Conclusion: 数据转换在解决异构性问题中至关重要，需进一步研究以优化AI数据准备流程。
...
<details>
  <summary>Details</summary>
Motivation: 用户活动序列是推荐系统中的重要信号，但工业推荐系统面临规模、成本和延迟的挑战，需要高效的基础模型。

Method: 预训练一个20B+参数的Transformer模型，结合创新技术如DCAT，优化基础设施和算法。

Result: PinFM提高了600%的吞吐量，并提升了20%的新物品参与度，已部署服务于5亿用户。
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络的训练动力学在理论上尚不清晰，需要一种严谨的框架来连接SGD的微观行为与权重矩阵的宏观演化。

Method: 开发了连续时间矩阵值SDE框架，推导了奇异值平方遵循Dyson布朗运动的精确SDE，并描述了稳态分布为具有幂律尾的伽马型密度。
...
Method: 引入动态修剪框架，基于任务难度和跨模态语义一致性自适应选择样本，利用预训练多模态模型监督。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习损失函数高度非凸，现有优化算法易陷入局部最优或梯度问题，导致性能不佳。

Method: 引入辅助变量表示每层的输出及其导数，将深度架构分解为一系列浅层架构，并开发交替方向算法。

Result: 理论分析证明LySep模型与原深度模型一致，高维数值结果验证了其在损失最小化和减少解误差方面的优势。
...
Method: 通过数据为中心的方法、神经架构设计、学习策略、多模态学习和基于LLM的建模系统，对代表性方法进行了分类和回顾。

Result: 总结了数据质量增强、结构和时间表示、自监督学习以及与临床知识的整合等方面的进展，并突出了新兴趋势。

Conclusion: 该综述为AI驱动的EHR建模和临床决策支持提供了结构化路线图，并指出了开放挑战。
...
Conclusion: 该研究为中小企业风险建模提供理论和实践支持，为监管机构和投资者提供数据驱动工具。
...
Result: FLDmamba在时间序列预测基准测试中表现优于基于Transformer和其他Mamba的架构。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有基于学习的无损压缩器压缩比不足、吞吐量低和鲁棒性差的问题。

Method: 提出PMKLC，包含自动化多知识学习框架、GPU加速编码器、数据块分区和逐步模型传递机制，以及两种压缩模式（PMKLC-S和PMKLC-M）。

Result: 在15个真实数据集上测试，PMKLC-S/M平均压缩比提升达73.609%和73.480%，吞吐量提升达3.036倍和10.710倍，且具有最佳鲁棒性和内存效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统降阶建模在网格变化时的局限性，同时弥补神经网络算子在误差量化上的不足。

Method: 结合降阶建模和算子学习，提出RONOM框架，并建立离散化误差界限。

Result: RONOM在输入泛化、空间超分辨率和离散化鲁棒性上表现优异，且在时间超分辨率场景中提供新见解。

Conclusion: RONOM框架成功结合了降阶建模和算子学习的优势，为偏微分方程求解提供了更灵活和鲁棒的方法。
...
Method: 通过RND生成内在奖励，利用专家状态转移的嵌入差异作为奖励信号。

Result: 在D4RL基准测试中表现优异，性能接近传统奖励标注方法。

Conclusion: ReLOAD为离线强化学习提供了一种高效且无需手工奖励标注的解决方案。
...
Result: 发现较大的学习率使NTK的领先特征向量与训练目标更对齐。

Conclusion: 研究增强了对深度学习中梯度下降训练动态的理解。
...
Method: 采用双分支Transformer架构分别处理CSI的幅度和相位信息，并通过预处理（异常值去除、平滑和相位校准）提升信号质量。
...
<details>
  <summary>Details</summary>
Motivation: 探讨行为克隆（BC）与强化学习（RL）的联系，改进监督微调（SFT）方法。

Method: 提出重要性加权监督微调（iw-SFT），优化RL目标的下界，并支持质量评分数据训练。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于头皮电极的EEG生物识别系统虽然安全，但使用不便，因此探索更用户友好的耳-EEG替代方案。

Method: 从耳-EEG信号中提取时间和频谱特征，并通过全连接深度神经网络进行主体识别。

Result: 在现有唯一适用于生物识别的耳-EEG数据集上，实验平均准确率为82%。
...
Method: 提出SmoothSplit和ParametricSplit两种激活函数，具备拓扑切割能力，适用于低维数据场景。

Result: 实验表明，ParametricSplit在低维数据中表现优于传统激活函数，且在高维数据中保持竞争力。
...
Method: 提出VIDAR框架，包括大规模扩散视频预训练和掩码逆向动力学模型，利用多视角视频预训练并提取动作相关信息。
...
<details>
  <summary>Details</summary>
Motivation: 在高风险应用中，机器学习预测的解释质量影响用户信任和决策。现有解释可能难以理解或可信，因此需要一种机制拒绝低质量解释。

Method: 引入LtX框架，配备拒绝器评估解释质量。ULER通过学习人类评分和特征相关性判断，设计拒绝器以反映人类对解释质量的评估。
...
Method: Fremer通过在频域处理复杂周期性模式，实现了高效和准确的预测。

Result: Fremer在多个指标上优于现有模型（如MSE、MAE、SMAPE），并在实际测试中降低了延迟和资源消耗。
...
<details>
  <summary>Details</summary>
Motivation: 随着机器学习模型复杂度增加，解释性下降，需要透明化的预测方法。

Method: 利用预测不确定性（分为数据相关和模型相关）指导解释方法选择，并结合不确定性量化框架。

Result: 实验表明，该方法能提升传统机器学习和深度学习场景中解释的鲁棒性和可达性。

Conclusion: 不确定性驱动的解释方法为复杂模型提供了更可靠的解释工具。
...
<details>
  <summary>Details</summary>
Motivation: DNA数据存储因高密度和长寿命而具有潜力，但合成、存储和测序过程中的错误需要高效算法校正。

Method: 预训练语言模型于合成数据，并在真实数据上微调，以适应技术特定的错误模式。

Result: TReconLM优于现有最佳算法，包括深度学习方法，显著提高无错误序列恢复比例。
...
Method: 1. 引入预训练的稳定策略（$\piphi$）提供离策略经验，正则化目标策略（$\pion$）训练；2. 重用零奖励样本，将其作为专家策略指导的独立批次。
...
<details>
  <summary>Details</summary>
Motivation: MCMC算法在机器学习的规划、优化和推理中广泛应用，但其高计算成本限制了大规模问题和实际应用的可行性，现有加速方案在硬件灵活性或系统效率上存在不足。

Method: MC²A通过扩展处理器性能模型分析工作负载多样性，设计参数化硬件架构支持MCMC内核，并引入新型Gumbel采样器消除指数和归一化操作。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM错误检测方法未能有效处理传播错误，影响最终结论的可靠性。

Method: 引入ARES框架，基于先前已验证的前提判断每个步骤，提供统计保证。

Result: 在四个基准测试中表现最佳（72.1% Macro-F1，提升8.2点），在长推理链中尤其有效（90.3% F1，提升27.6点）。

Conclusion: ARES显著提升了传播错误的检测能力，尤其在长推理链中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 提升AI模型在医疗应用中的安全性和透明度，特别是在放射学领域，决策影响重大。

Method: 使用Matryoshka-SAE分析MAIRA-2模型的内部表示，并通过大规模自动解释识别临床相关特征。

Result: 识别出医疗设备、病理特征等临床概念，但“steering”控制效果有限。

Conclusion: 研究为理解MAIRA-2的内部机制提供了初步见解，推动了模型透明度的提升。
...
Method: 使用WaveletInception-BiLSTM网络，结合LWPT提取频谱特征和1D Inception提取多尺度特征，通过BiLSTM建模时序依赖。
...
Conclusion: FedGA有效提升了联邦学习的公平性，且不影响整体性能。
...
TL;DR: 提出了一种残差学习方法，通过训练次级稀疏自编码器（SAE）来捕捉主SAE在领域特定文本中遗漏的特征，显著提升了模型性能。
...
Method: 训练一个次级SAE，专门建模预训练SAE在领域特定文本上的重构误差，并在推理时将两个模型的输出相加。

Result: 实验表明，该方法在多个专业领域中显著改善了交叉熵和解释方差指标，同时保持了通用任务的性能。

Conclusion: 该方法为研究者提供了选择性增强SAE在特定领域可解释性的能力，为LLM的机制解释开辟了新途径。
...
TL;DR: 论文提出了一种框架，通过评估每种关系与不同几何变换的匹配程度，为知识图谱中的关系分配最佳变换或通过多数投票选择一种变换类型。


<details>
  <summary>Details</summary>
Motivation: 当前知识图谱嵌入模型在几何变换的使用上不足，未考虑关系特定的变换，导致模型表现受限。

Method: 提出一种框架，通过排名评估关系与几何变换的匹配度，并利用注意力机制学习低维向量空间中的关系特定几何变换。

Result: 在三个基准知识图谱和一个真实金融知识图谱上验证了模型的有效性，性能与领先模型相当。

Conclusion: 该框架通过关系特定的几何变换提升了知识图谱嵌入模型的性能。
...
Method: 使用时序卷积自编码器提取特征，基于隐空间和残差空间训练二分类和多分类器，结合置信度校准、OOD检测和误报控制技术。

Result: 在模拟数据上验证了方法的有效性，但仍需真实数据测试以确保成熟度。

Conclusion: 该方法是一个有前景的初步成果，未来需进一步验证其实际应用效果。
...
Method: CFR框架基于不确定性阈值划分数据集，系统分析不确定性如何影响自然性解释。

Result: CFR在AnthroProtect数据集中为灌木林、森林和湿地分配了更高相关性，且不确定性增加时解释性下降。
...
TL;DR: 该论文提出了一种新的分类法，用于解耦Transformer架构在长期时间序列预测（LTSF）中的设计，并通过实验验证了最佳架构选择。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer模型在LTSF任务中的架构设计多样且与时间序列特定设计紧密耦合，难以单独评估架构本身的影响。

Method: 提出一种分类法，考虑注意力机制、预测聚合、预测范式和归一化层等关键方面，并通过实验比较不同架构。
...
TL;DR: 研究了基于无限图G的假设类的PAC和在线学习性，其中每个副本通过G的顶点置换生成。主要结果表明，有限支持副本的PAC可学习性等价于G的自同构平凡性，并推导出在线学习性。此外，还通过无限随机图的扩展性质松弛，刻画了两顶点置换不可学习的图类，并证明了k>2时k顶点置换与2顶点置换的可学习性等价。


<details>
  <summary>Details</summary>
Motivation: 探索无限图在置换操作下的学习性，特别是有限支持置换对PAC和在线学习性的影响，以及自同构平凡性的作用。

Method: 通过分析无限图G的置换副本，研究其PAC和在线学习性，利用自同构平凡性和无限随机图的扩展性质松弛进行刻画。

Result: 有限支持置换的PAC可学习性等价于G的自同构平凡性，且k>2时k顶点置换与2顶点置换的可学习性等价。

Conclusion: 研究揭示了无限图在置换操作下的学习性特征，为图学习理论提供了新的分类和复杂性分析工具。
...
TL;DR: DASViT提出了一种可微分搜索方法，用于优化Vision Transformer架构，解决了传统方法的计算资源高和时间长的问题。


<details>
  <summary>Details</summary>
Motivation: 传统NAS方法在ViT架构搜索中面临创新性不足、计算资源需求高和时间长的问题，DASViT旨在解决这些局限性。

Method: DASViT采用可微分架构搜索方法，专注于ViT的架构优化，避免了离散搜索的高成本。
...
<details>
  <summary>Details</summary>
Motivation: 现有XAI技术通常缺乏确定性、模型无关性和收敛保证，且可能在掩蔽时降低性能，因此需要一种更鲁棒的XAI方法。

Method: MUPAX采用测度论公式化，通过结构化扰动分析发现输入固有模式并消除虚假关系，适用于任意维度和损失函数。

Result: MUPAX在音频分类、图像分类、医学图像分析和解剖标志检测等任务中表现优异，不仅能保持还能提升模型准确性。

Conclusion: MUPAX通过生成精确、一致且可理解的解释，为构建可解释和可信的AI系统迈出了重要一步。
...
TL;DR: 提出了一种跨模态知识蒸馏框架，通过原型相似性模块和任务特定蒸馏头解决EEG学习中的模态和标签不一致问题，提升了情感回归和分类性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有图生成方法在可解释性上有限，难以理解结构决策背后的逻辑。

Method: NGTM将图表示为潜在主题的混合，每个主题定义语义子结构的分布，结合全局结构变量生成图。

Result: NGTM在生成质量上具有竞争力，同时支持细粒度控制和可解释性。

Conclusion: NGTM通过主题级调整实现了图生成的高质量和可解释性，适用于分子设计等领域。
...
TL;DR: 论文介绍了NonverbalTTS（NVTTS）数据集，包含17小时的非语言声音标注数据，用于提升表达性语音合成模型的多样性。
...
Method: 通过自动检测和人工验证，从VoxCeleb和Expresso中提取数据，并整合ASR、NV标记、情感分类和多标注者转录融合的流程。

Result: 在NVTTS数据集上微调的TTS模型性能与闭源系统相当，通过人类评估和自动指标验证。
...
<details>
  <summary>Details</summary>
Motivation: 在大型语言模型（LLM）时代，对齐问题成为提升模型可靠性、可控性和能力的关键挑战。逆向强化学习（IRL）在LLM对齐中的应用引发了广泛研究兴趣。

Method: 通过介绍RL基础概念，分析IRL在LLM对齐中的最新进展，讨论关键挑战与机遇，并探讨数据集、基准、评估指标等实践问题。

Result: 总结了稀疏奖励RL的文献，提出了未解决的挑战和未来研究方向。

Conclusion: 本文为LLM对齐领域提供了结构化综述，指出了通过RL和IRL技术改进对齐的潜在方向。
...
TL;DR: 提出MoTM（时间流模型混合）方法，利用隐式神经表示（INRs）解决时间序列缺失值填充任务，特别是在分布偏移下的表现。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列基础模型主要关注预测任务，而跨域缺失值填充任务研究不足，需填补这一空白。

Method: 结合多个独立训练的INRs和一个岭回归器，通过混合已见模式适应新时间序列的缺失值填充。

Result: 在多种缺失场景（如块状和点状缺失、可变采样率）下，MoTM表现出鲁棒的域内和跨域泛化能力。
...
TL;DR: 提出了一种基于排序算法的置换空间核函数框架，引入Merge Kernel降低复杂度，并通过轻量级描述符提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前置换空间的贝叶斯优化依赖Mallows核，其复杂度高且计算效率低，需改进。

Method: 基于排序算法生成核函数，提出Merge Kernel（复杂度Θ(n log n)），并加入三种轻量级描述符。

Result: Merge Kernel在多项基准测试中优于Mallows核，提供更紧凑且高效的解决方案。

Conclusion: 新框架显著提升了置换空间贝叶斯优化的效率和效果。
...
TL;DR: TRENN是一种新型的时空关系架构，用于联合建模团队动态，并通过多任务扩展（MT-TRENN）预测多个团队构念，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 团队建模是AI与社会科学交叉领域的核心挑战，现有方法无法满足实际需求，需同时建模动态与关系并提供可解释的见解。

Method: TRENN结合自动时间图提取器、时空关系编码器、团队构念预测解码器和两个解释模块；MT-TRENN扩展为多任务头。

Result: 实验表明，TRENN和MT-TRENN在性能上显著优于仅依赖时间或关系信息的方法，并提供可解释的见解。
...
TL;DR: GeoReg模型结合卫星影像和地理空间数据，利用大语言模型（LLM）解决数据稀缺问题，通过特征提取和分类提升社会经济指标估计效果。
...
Method: 整合多源数据（卫星影像、地理信息），利用LLM提取特征并分类相关性，结合线性估计和非线性变换优化模型。

Result: 在三个不同发展水平的国家实验中，模型表现优于基线方法，尤其在数据稀缺的低收入国家。
...
<details>
  <summary>Details</summary>
Motivation: 神经网络对输入和权重扰动的高度敏感性可能导致对抗样本脆弱性、训练发散和过拟合等问题。现有方法尚未成功在训练过程中为现代架构（如Transformer）提供Lipschitz约束。

Method: 开发了计算高效的工具来维护范数约束的权重矩阵，并训练具有Lipschitz约束的Transformer模型。研究了优化器（如AdamW和Muon）对性能的影响。

Result: 使用Muon优化器可以降低Lipschitz边界并保持性能。在2M参数的Transformer上实现了60%的验证准确率，但在145M参数模型中，为匹配基准性能需放宽Lipschitz约束。

Conclusion: Lipschitz约束的Transformer在训练中无需稳定性措施（如层归一化），但大规模模型仍需权衡Lipschitz边界与性能。
...
TL;DR: 本文提出了一种基于学习的方法来识别状态估计中的噪声特性，以解决协方差矩阵选择困难的问题，并在实时车辆状态估计中进行了测试。


<details>
  <summary>Details</summary>
Motivation: 准确的状态估计需要仔细考虑过程和测量模型的不确定性，但这些特性通常不为人知，需要有经验的设计师来选择协方差矩阵。协方差矩阵选择错误可能影响估计算法的准确性，甚至导致滤波器发散。识别噪声特性一直是一个挑战性问题。

Method: 提出了一种基于学习的方法论，使用不同的损失函数来识别噪声特性。该方法区别于传统的通过涉及创新序列的优化算法来识别未知协方差矩阵的方法，而是利用学习方法来确定过程和测量模型的随机特性。

Result: 在实时车辆状态估计任务中测试了所提出方法的性能，验证了不同损失函数的学习方法在噪声特性识别方面的有效性。

Conclusion: 基于学习的噪声特性识别方法为解决状态估计中协方差矩阵选择问题提供了新的解决方案，在实时车辆状态估计应用中显示出良好的性能表现。
...
TL;DR: 研究了具有有限阶矩随机初始化权重的深度神经网络的有限维分布（FDDs），在高斯近似下给出了Wasserstein-$1$范数的收敛界。


<details>
  <summary>Details</summary>
Motivation: 探索深度神经网络在随机初始化权重下的有限维分布行为，特别是在宽度趋向无穷时的极限性质。

Method: 假设激活函数为Lipschitz连续，允许各层宽度以任意相对速率增长，推导FDDs与高斯极限的近似界。

Result: 在宽度比例增长的特殊情况下，收敛速率为$n^{-({1}/{6})^{L-1} + \epsilon}$。
...
TL;DR: 论文提出了一种名为“自平衡神经网络”（Sbnet）的新方法，通过模型自身从平衡网络中获取伪倾向得分，解决了传统方法中倾向得分模型可能错误的问题，并在模拟和真实数据中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在观察性研究中，混杂变量和工具变量会影响处理和结果，导致平均处理效应估计偏差。传统方法依赖倾向得分模型，但存在模型错误的风险。

Method: 提出Sbnet方法，利用平衡网络作为前馈神经网络的关键部分，一步估计平均处理效应，并引入多伪倾向得分框架。

Result: 在三种模拟设置和真实数据集上，Sbnet表现优于现有方法。

Conclusion: Sbnet通过自平衡机制有效解决了倾向得分模型错误的问题，提高了平均处理效应的估计准确性。
...
TL;DR: 提出了一种统一的贝叶斯框架，用于识别线性时变系统，通过分解系统响应为后验均值和随机波动项，结合现代机器学习技术进行推断。


<details>
  <summary>Details</summary>
Motivation: 解决线性时变系统识别这一具有挑战性的不适定逆问题，并提供一种量化不确定性的方法。

Method: 将系统脉冲响应建模为随机过程，利用贝叶斯神经网络和高斯过程，采用可扩展的变分推断进行推断。

Result: 实验表明，该框架能稳健地从单次噪声观测中推断LTI系统特性，在模拟环境噪声层析成像问题中优于经典方法，并能成功跟踪连续变化的LTV脉冲响应。
...
TL;DR: 论文研究了逻辑回归模型中部分输入缺失时的预测问题，提出了一种模式逐模式（PbP）策略，并与其他方法进行了比较。


<details>
  <summary>Details</summary>
Motivation: 解决逻辑回归模型中部分输入缺失时的预测问题，填补现有研究的空白。

Method: 采用模式逐模式（PbP）策略，学习每个缺失模式的逻辑模型，并与常数插补、迭代插补、完整案例分析等方法进行比较。

Result: 理论证明PbP能准确逼近贝叶斯概率；实验表明小样本时均值插补可用作基线，非线性迭代插补（MICE.RF.Y）表现更优，大样本时PbP在高斯混合数据中最佳。
...
Method: 引入RAPD捕捉随机向量对的关联性，并基于此提出RASD分布，进一步提出RASGW及其变体（如IWRASGW）。
