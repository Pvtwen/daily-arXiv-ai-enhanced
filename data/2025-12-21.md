...
TL;DR: 提出LRM-MSL框架，通过构建局部无线电地图、二值化和连通分量分析实现多源分离，再使用数值坐标回归网络进行单源定位，解决城市环境中任意数量信号源定位问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么无法有效利用接收信号强度信息，要么缺乏对任意数量信号源的通用性，难以解决城市环境中的多源定位问题。

Method: 1) 构建仅保留信号源周围RSS信息的局部无线电地图并二值化；2) 应用连通分量分析实现多源分离，将MSL问题转化为多个单源定位任务；3) 设计数值坐标回归网络执行单源定位任务。

Result: 构建了VaryTxLoc数据集进行验证，实验结果表明LRM-MSL是准确有效的方法，优于现有最先进方法。

Conclusion: LRM-MSL框架能够处理任意数量的信号源，在城市环境多源定位问题上表现出色，提供了通用的解决方案。
...
TL;DR: TinyMyo：一个轻量级表面肌电信号基础模型，基于Transformer编码器架构，仅3.6M参数，通过自监督预训练实现多任务泛化，并首次部署在超低功耗微控制器上。


<details>
  <summary>Details</summary>
Motivation: 表面肌电信号（EMG）在多个领域有广泛应用，但现有方法在跨受试者、记录系统和采集协议方面缺乏鲁棒性。现有EMG基础模型局限于单一任务且难以在嵌入式平台部署。

Method: 提出TinyMyo轻量级基础模型，基于Transformer编码器架构，使用公开数据集进行自监督预训练。模型仅3.6M参数，通过最小化任务特定头部适配，同一骨干网络可处理多个下游任务。

Result: 在多个数据集上达到或超越SOTA：NinaPro DB5（89.4±0.16%）、UCI-EMG（97.56±0.32%）、EPN-612（96.74±0.09%）。首次在超低功耗微控制器（GAP9）上部署EMG基础模型，平均功耗36.45mW。

Conclusion: TinyMyo提供了一个轻量级、可部署的EMG基础模型，能够泛化到多个下游任务，为EMG社区提供了灵活的研究资源，并展示了在嵌入式设备上的实际应用潜力。
...
TL;DR: 开发了一种最小化数字定制芯片，能产生稳定的6Hz振荡来抑制癫痫发作，为可穿戴式癫痫干预设备提供了概念验证。


<details>
  <summary>Details</summary>
Motivation: 全球有超过5000万癫痫患者，尽管药物治疗有所进展，但仍有大量患者无法控制发作，需要替代的神经调控策略。现有系统依赖复杂的模拟电子设备或高功率刺激硬件，限制了临床应用。

Method: 使用公开的EEG癫痫数据集提取并平均模拟发作波形，数字化后模拟神经前端接口，通过Saleae Logic分析仪与定制芯片的数字输出直接对接。芯片脉冲序列重采样并通过低通重构产生模拟6Hz波形，进行时域和频域分析。

Result: 芯片能施加窄带6Hz节律，覆盖癫痫发作活动的宽带频谱特征，证明数字定制芯片能有效调控癫痫发作活动。

Conclusion: 该研究为低功耗数字定制化神经调控提供了概念验证，为简化、可穿戴的癫痫干预设备开辟了潜在途径，适用于精准医疗和未来医疗设备。
...
TL;DR: 提出了一种名为"concurrence"的统计依赖度量方法，通过构建分类器来区分时间序列中时间对齐与错齐的片段，从而检测复杂非线性关系


<details>
  <summary>Details</summary>
Motivation: 生物系统通常表现出复杂的非线性相互作用，现有方法需要先验知识或大量数据才能捕捉这些关系，需要一种无需先验知识或大数据就能检测依赖关系的方法

Method: 引入"concurrence"标准：如果能够构建一个分类器来区分从两个时间序列中提取的时间对齐与错齐片段，则认为这两个时间序列存在依赖关系

Result: 该方法在理论上与依赖关系相关，能够暴露多种信号（fMRI、生理和行为数据）之间的关系，无需特定参数调整或大量数据

Conclusion: concurrence可以成为跨学科科学分析的标准方法，因为它能够检测广泛信号谱中的关系，无需先验知识或大数据
...
TL;DR: 提出一种基于RIS和协作干扰器的同时保密与隐蔽通信方案，通过生成扩散模型和深度强化学习优化功率分配，在用户移动性下平衡安全性和容量性能。


<details>
  <summary>Details</summary>
Motivation: 在无线通信中同时实现保密性和隐蔽性具有挑战性，传统方法难以在用户移动场景下平衡安全容量和检测错误概率。需要一种能适应动态环境并优化两者性能的方案。

Method: 采用RIS辅助网络和协作干扰器，推导平均最小检测错误概率和平均保密容量的闭式表达式，提出基于生成扩散模型和深度强化学习的算法，优化功率分配以平衡性能指标。

Result: 仿真结果显示，所提算法相比传统DDPG方法收敛更快、性能更优，能有效平衡安全性和容量性能，验证了方案在同时实现保密和隐蔽通信方面的有效性。

Conclusion: 提出的SSACC方案通过RIS和协作干扰器，结合GDM-DRL算法，成功实现了在用户移动场景下同时优化保密性和隐蔽性，为未来安全通信系统提供了有效解决方案。
...
TL;DR: 提出TSLT方法，通过传输截断稀疏对数来减少分布式推测解码中的上行链路通信开销，同时保持接受率和推理性能。


<details>
  <summary>Details</summary>
Motivation: 在AI-RAN中，设备-边缘协作推理采用推测解码技术，但现有分布式方案在每个步骤传输完整词汇表对数，导致显著的上行链路开销。

Method: 提出截断稀疏对数传输(TSLT)策略：先稀疏化再采样，仅传输截断候选集的logits和索引。理论证明TSLT能保持接受率，并扩展到多候选情况以提高接受概率。

Result: 实验显示TSLT显著减少上行链路通信，同时保持端到端推理延迟和模型质量，证明其在未来AI-RAN系统中实现可扩展、通信高效的分布式LLM推理的有效性。

Conclusion: TSLT为AI-RAN系统中的分布式LLM推理提供了一种通信高效的解决方案，通过减少上行链路开销而不牺牲性能，支持可扩展的设备-边缘协作推理。
...
TL;DR: 提出CPMamba框架，基于选择性状态空间模型，用于MIMO-OFDM系统中的高效信道预测，在保持线性计算复杂度的同时实现最先进的预测精度。


<details>
  <summary>Details</summary>
Motivation: 在高移动性场景中，快速时变信道的准确预测对于抵抗信道老化和保证通信质量至关重要。现有方法存在复杂度高、无法准确建模信道时间变化的问题。

Method: 提出CPMamba架构：1）使用专门设计的特征提取和嵌入网络从历史CSI中提取特征；2）采用堆叠的残差Mamba模块进行时间建模；3）利用输入依赖的选择性机制动态调整状态转移，有效捕获CSI间的长程依赖关系。

Result: 在3GPP标准信道模型下的仿真结果表明，CPMamba在所有场景中都实现了最先进的预测精度，具有优异的泛化性和鲁棒性。相比现有基线模型，参数量减少约50%，同时达到相当或更好的性能。

Conclusion: CPMamba通过选择性状态空间模型实现了高效的信道预测，显著降低了实际部署的门槛，为高移动性场景中的MIMO-OFDM系统提供了有效的信道预测解决方案。
...
TL;DR: 提出一种针对带非负和和为1约束的线性光谱解混问题的专用算法，特别适用于监督解混（大字典情况），并扩展支持最小丰度约束


<details>
  <summary>Details</summary>
Motivation: 实践中，特别是监督解混（大字典）时，由于丰度的非负性，解往往稀疏，这促使使用主动集求解器。针对问题特定特征，设计专用算法相比通用求解器可获得更好的计算性能

Method: 设计一种针对线性光谱解混问题的专用算法，将非负约束扩展到更广泛的最小丰度约束，采用主动集求解器

Result: 未在摘要中明确说明具体结果，但暗示该专用算法相比通用求解器能获得更好的计算性能

Conclusion: 提出了一种针对带非负和和为1约束的线性光谱解混问题的专用算法，扩展了约束条件并优化了计算性能
...
<details>
  <summary>Details</summary>
Motivation: 6G高速移动场景（如高铁、无人机）对OFDM通信系统提出挑战，传统方法难以处理高速移动带来的ICI、分数延迟和多普勒频移问题，需要开发更鲁棒的接收机架构。

Method: 1) 采用延迟-多普勒域叠加导频方案，仅添加单个导频进行信道估计；2) 推导分离的分数延迟-多普勒估计算法；3) 提出基于Landweber迭代的低复杂度均衡方法，利用信道固有结构。

Result: 仿真结果表明，所提接收机架构在高达1000km/h的各种移动条件下实现鲁棒通信性能，相比现有方法提高了有效吞吐量。

Conclusion: 该研究为6G高速移动OFDM通信提供了一种有效的接收机架构，通过延迟-多普勒叠加导频和低复杂度均衡算法，解决了高速移动环境中的信道估计和均衡难题。
...
TL;DR: 提出结合Woodbury公式和自适应随机SVD的低复杂度预编码框架，用于LEO卫星大规模天线阵列，相比传统RZF预编码计算量减少61%，仅轻微影响和速率性能。


<details>
  <summary>Details</summary>
Motivation: LEO卫星大规模天线阵列需要计算高效且自适应的预编码技术来应对动态信道变化并提升频谱效率。传统RZF预编码的Gram矩阵求逆计算量大，限制了实时实现。

Method: 结合Woodbury公式和自适应随机SVD（arSVD）的低复杂度框架。Woodbury公式利用低秩扰动降低求逆复杂度，arSVD动态提取主导奇异分量，进一步提升计算效率。

Result: 蒙特卡洛仿真显示，相比传统RZF预编码的完整矩阵求逆，该方法计算量减少高达61%，仅带来轻微的和速率性能下降。

Conclusion: WB-arSVD为下一代卫星通信提供了可扩展且高效的解决方案，便于在功率受限环境中实现实时部署。
...
TL;DR: 提出自适应鲁棒主成分分析(A-RPCA)预处理方法，提升基于信道状态信息的物理层认证性能，通过信息协调框架显著降低错误概率


<details>
  <summary>Details</summary>
Motivation: 解决CSI-PLA中信道状态信息在时域上的变化和不一致性问题，提升物理层认证的准确性和可靠性

Method: 基于鲁棒主成分分析(RPCA)开发自适应RPCA预处理方法，结合基于信息协调的PLA框架，利用Polar码的高斯近似设计短码长Slepian-Wolf解码器

Result: 相比无预处理和无协调的基线方案，A-RPCA显著降低协调后错误概率，在LOS和NLOS场景下检测概率均达到1，优于PCA、鲁棒PCA、自编码器和ReProCS等现有方法

Conclusion: 提出的A-RPCA预处理方法能有效缓解CSI变化问题，显著提升物理层认证性能，在合成和真实数据集上都表现出优越性能
...
TL;DR: 提出一个基于MCRB分析位置物理层认证主动攻击影响的框架，重点关注AoA认证场景下攻击对性能界限的影响


<details>
  <summary>Details</summary>
Motivation: 现有位置物理层认证系统容易受到主动攻击，需要量化分析攻击对认证性能的影响，为系统设计提供理论指导

Method: 使用误设Cramér-Rao界分析框架，针对单天线用户AoA认证场景，考虑具有L个天线单元的欺骗攻击者，假设确定性导频信号

Result: 推导出MCRB的闭式表达式，发现攻击引入的惩罚项不依赖于信噪比，而是取决于攻击者位置、阵列几何和攻击预编码向量

Conclusion: MCRB框架能有效量化主动攻击对物理层认证性能的影响，为抗攻击认证系统设计提供理论依据
...
TL;DR: 提出了一种集成复数变分模态分解算法，结合时序卷积网络和空间注意力机制，用于特定辐射源识别，在仅有10个符号且无需先验知识的情况下达到96%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习SEI方法依赖大量数据或先验信息，在实际场景中面临标记数据有限的挑战。需要开发在数据稀缺情况下仍能有效工作的识别方法。

Method: 1. 集成复数变分模态分解算法：分解和重构复数值信号以逼近原始发射信号，实现更准确的特征提取
2. 时序卷积网络：有效建模序列信号特征
3. 空间注意力机制：自适应加权信息丰富的信号片段
4. 分支网络：利用其他数据的预训练权重，减少对辅助数据集的需求

Result: 1. 在模拟数据上的消融实验验证了模型各组成部分的有效性
2. 在公开数据集上，仅使用10个符号且无需任何先验知识的情况下，达到了96%的准确率

Conclusion: 该方法在数据稀缺情况下实现了高性能的特定辐射源识别，为物理层安全提供了有效的解决方案，特别适用于实际场景中标记数据有限的情况。
...
Method: 将符号回归与Disciplined Convex Programming（DCP）规则集集成，强制所有发现的候选模型表达式遵守DCP组合规则，确保输出表达式在构造时就是全局凸的

Result: 能够发现比传统固定参数凸表达式更宽松、更准确的功能形式的凸替代模型，产生可解释、可验证且灵活的凸模型

Conclusion: DiscoverDCP框架通过数据驱动方式自动生成保证凸性的模型，适用于安全关键的控制和优化任务，避免了后验凸性验证的计算难题
...
TL;DR: 提出一个结合量子情感分析、决策变换器和策略模型选择的混合集成框架，在标普500预测中达到60.14%方向准确率，比单个模型提升3.10%


<details>
  <summary>Details</summary>
Motivation: 金融市场预测具有高噪声、非平稳性和市场效率等挑战，大多数模型难以超过55-57%的准确率。现有方法存在架构多样性不足、情感分析能力有限和弱预测器影响等问题

Method: 1) 采用架构多样性策略，结合LSTM、决策变换器、XGBoost、随机森林和逻辑回归等多种学习算法；2) 使用4量子比特变分量子电路增强情感分析；3) 通过智能过滤排除准确率低于52%的弱预测器；4) 基于置信度的过滤策略（6+模型共识）

Result: 在2020-2023年市场数据上，框架达到60.14%方向准确率，比相同架构模型组合（52.80%）显著提升。量子情感分析为每个模型带来0.8-1.5%增益。Top-7模型集成（60.14%）优于所有35个模型组合（51.2%）。McNemar检验显示统计显著性（p<0.05），初步回测显示夏普比率1.2 vs 买入持有策略0.8

Conclusion: 架构多样性比数据集多样性更重要，量子增强的情感分析能提升预测性能，智能模型选择对集成效果至关重要。该框架在金融市场预测中具有实际交易潜力
...
TL;DR: 论文提出利用DPO的数学结构简化LLM数据估值的Shapley值计算，解决传统方法计算成本过高的问题


<details>
  <summary>Details</summary>
Motivation: LLM训练中数据是重要资产，但数据估值（特别是Shapley值计算）通常需要大量模型重训练，计算成本过高，尤其对于大型模型。需要找到更高效的数据估值方法。

Method: 利用Direct Preference Optimization (DPO)的特定数学结构，实现可扩展的Shapley值计算，显著降低计算复杂度。

Result: 证明DPO的数学结构能够大幅简化数据估值的Shapley值计算，为LLM数据估值提供高效解决方案。

Conclusion: DPO的结构特性使得LLM数据估值变得可行，为数据所有者决策和协作训练中的利益分配问题提供了实用工具。
...
### [17] [SHARe-KAN: Holographic Vector Quantization for Memory-Bound Inference](https://arxiv.org/abs/2512.15742)
*Jeff Smith*

Main category: cs.LG

TL;DR: SHARe-KAN框架通过Gain-Shape-Bias向量量化解决KANs的内存瓶颈问题，结合硬件感知编译器实现88倍内存减少，同时保持原始精度。


<details>
  <summary>Details</summary>
Motivation: Kolmogorov-Arnold Networks (KANs)面临基本的内存墙问题：其学习的基函数导致参数数量激增，产生极高的带宽需求，阻碍了在内存受限环境中的部署。传统剪枝方法在KANs上失效，因为其全息拓扑结构使得信息分布在样条干涉中而非局部化到特定边。

Method: 提出SHARe-KAN框架，采用Gain-Shape-Bias向量量化来利用函数冗余同时保持密集拓扑结构。结合LUTHAM硬件感知编译器进行静态内存规划。

Result: 实现88倍运行时内存减少（从1.13GB降至12.91MB），在PASCAL VOC上匹配未压缩基线精度。在NVIDIA Ampere架构上分析显示>90%的L2缓存驻留，证明工作负载已从基于样条架构固有的DRAM带宽约束中解耦。

Conclusion: SHARe-KAN成功解决了KANs的内存瓶颈问题，通过创新的量化方法和硬件感知编译技术，在保持精度的同时大幅减少内存需求，使KANs能够在内存受限环境中部署。
...
### [18] [TENG++: Time-Evolving Natural Gradient for Solving PDEs With Deep Neural Nets under General Boundary Conditions](https://arxiv.org/abs/2512.15771)
*Xinjie He,Chenggong Zhang*

Main category: cs.LG

TL;DR: 将时间演化自然梯度（TENG）框架扩展到处理Dirichlet边界条件，结合自然梯度优化与数值时间步进方案（Euler和Heun方法），通过边界条件惩罚项精确实施约束，在热方程上验证了方法的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 传统数值方法在处理高维或复杂PDE问题时存在困难，而物理信息神经网络（PINNs）虽然通过嵌入物理约束提供了替代方案，但在实现高精度和处理复杂边界条件方面仍面临挑战。

Method: 扩展时间演化自然梯度（TENG）框架以处理Dirichlet边界条件，将自然梯度优化与数值时间步进方案（Euler和Heun方法）相结合，在损失函数中加入边界条件惩罚项来精确实施约束。

Result: 在热方程实验中，Heun方法由于其二阶修正表现出更高的准确性，而Euler方法在简单场景中具有更高的计算效率，验证了所提方法的有效性。

Conclusion: 这项工作为将框架扩展到Neumann和混合边界条件以及更广泛的PDE类别奠定了基础，推进了基于神经网络的求解器在现实世界问题中的应用。
...
### [19] [How Do Graph Signals Affect Recommendation: Unveiling the Mystery of Low and High-Frequency Graph Signals](https://arxiv.org/abs/2512.15744)
*Feng Liu,Hao Cang,Huanhuan Yuan,Jiaqing Fan,Yongjing Hao,Fuzhen Zhuang,Guanfeng Liu,Pengpeng Zhao*

Main category: cs.LG

TL;DR: 论文研究了图信号在推荐系统中的角色，证明了低频和高频图信号对推荐性能的等效性，提出了频率信号缩放器和空间翻转方法来优化图神经网络推荐模型。


<details>
  <summary>Details</summary>
Motivation: 当前谱图神经网络在推荐系统中通常归功于低通滤波，但近期研究表明高频信号也很重要。低频和高频图信号在推荐中的具体作用尚不明确，需要深入研究。

Method: 1) 理论证明低频和高频图信号在推荐任务中的等效性；2) 提出频率信号缩放器（plug-and-play模块）调整图信号滤波函数以微调用户-物品对的平滑度；3) 提出空间翻转方法恢复图嵌入的表达能力。

Result: 在四个公开数据集上的大量实验验证了所提方法的有效性。证明仅使用低频或高频图信号中的任意一种就足以实现有效的推荐。

Conclusion: 低频和高频图信号在推荐中具有等效作用，都通过平滑用户-物品对相似性来贡献。提出的频率信号缩放器和空间翻转方法能有效提升推荐性能，且与任何GNN模型兼容。
...
### [20] [Provably Extracting the Features from a General Superposition](https://arxiv.org/abs/2512.15987)
*Allen Liu*

Main category: cs.LG

TL;DR: 提出一种高效查询算法，能从叠加函数中恢复特征方向，适用于任意叠加和一般响应函数
...
Method: 开发一种在傅里叶空间中搜索的算法，通过迭代细化搜索空间来定位隐藏的特征方向。算法从噪声oracle访问函数f，要求特征方向不近似相同

Result: 算法能有效识别所有非退化响应的特征方向并重建函数f，适用于比先前结果更一般的设置（任意叠加和一般响应函数）

Conclusion: 该算法为从叠加函数中学习特征提供了有效的解决方案，突破了过完备机制下的传统算法限制
...
TL;DR: LLaDA2.0是一个通过系统转换自回归模型构建的离散扩散大语言模型，包含16B和100B参数的MoE变体，支持并行解码，性能优越且已开源。


<details>
  <summary>Details</summary>
Motivation: 为了解决从头训练大模型的成本问题，同时保持知识继承和高效部署，提出通过转换预训练自回归模型来构建离散扩散大语言模型的新范式。

Method: 采用三步块级WSD训练方案：渐进增加块大小的块扩散（预热）、大规模全序列扩散（稳定）、回归紧凑块扩散（衰减），并结合SFT和DPO进行后训练对齐，得到16B和100B的MoE变体。

Result: 成功构建了LLaDA2.0-mini（16B）和LLaDA2.0-flash（100B）两个指令调优的MoE变体，保留了并行解码优势，在性能与效率方面表现优异。

Conclusion: LLaDA2.0建立了一种通过转换预训练AR模型构建离散扩散大语言模型的新范式，实现了知识继承与高效部署的平衡，为前沿规模部署提供了可行方案。
...
### [22] [CauSTream: Causal Spatio-Temporal Representation Learning for Streamflow Forecasting](https://arxiv.org/abs/2512.16046)
*Shu Wan,Reepal Shah,John Sabo,Huan Liu,K. Selçuk Candan*

Main category: cs.LG

TL;DR: CauStream是一个用于因果时空径流预测的统一框架，通过联合学习径流因果图和动态路由图，在三个美国主要河流流域上超越了现有方法，尤其在长期预测中表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习模型虽然预测性能强，但忽略了物理过程，限制了可解释性和泛化能力。最近的因果学习方法虽然整合了领域知识，但通常依赖固定的因果图，无法适应数据变化。

Method: 提出CauStream框架，联合学习：(1)气象强迫与径流之间的因果图；(2)捕捉站点间动态依赖关系的路由图。在非参数设置下建立了这些因果结构的可识别性条件。

Result: 在三个美国主要河流流域和三个预测时间尺度上评估，CauStream始终优于现有最先进方法，预测时间窗口越长，性能差距越大，表明对未见条件有更强的泛化能力。

Conclusion: CauStream不仅提供准确的预测，还能学习到与领域知识一致的可解释因果图，为因果时空建模提供了原则性基础，可扩展到广泛的科学和环境应用中。
...
<details>
  <summary>Details</summary>
Motivation: 异构材料微结构逆向设计是病态且计算昂贵的问题，传统生成模型难以支持快速稳定的确定性物理信息逆向求解

Method: 提出Janus框架，结合深度编码器-解码器与可分离的KHRONOS预测头，学习同时支持生成逆向和物理预测的等距潜在流形，通过联合目标实现潜在空间解耦

Result: 在MNIST上实现高保真重建和准确分类；在热导率标记的微结构设计中，前向预测R²=0.98，逆向解满足目标属性1%相对误差，潜在空间可视化显示低维解耦流形
...
### [24] [Multivariate Uncertainty Quantification with Tomographic Quantile Forests](https://arxiv.org/abs/2512.16383)
*Takuya Kanazawa*
...
Conclusion: 与经典方向分位数方法不同，TQF使用单一模型覆盖所有方向而不施加凸性限制，能够产生非凸分位数区域。
...
### [25] [D3G: Diverse Demographic Data Generation Increases Zero-Shot Image Classification Accuracy within Multimodal Models](https://arxiv.org/abs/2512.15747)
*Javon Hickmon*

Main category: cs.LG

TL;DR: D3G：一种无需训练、零样本的方法，通过生成多样化人口统计数据来提升CLIP等预训练多模态模型的分类准确性并减少人口统计偏差


<details>
  <summary>Details</summary>
Motivation: 当前图像分类任务面临低容量模型欠拟合、细粒度分类表现不佳、跨模态数据质量要求高、数据集人口统计不平衡导致预测偏差等问题，这些问题在零样本图像分类中可能导致有害的人口统计偏差

Method: 提出Diverse Demographic Data Generation (D3G)方法，使用CLIP作为基础多模态模型，Stable Diffusion XL作为生成模型，在推理时生成多样化人口统计数据来提升分类性能

Result: 在推理时提供多样化人口统计数据能够改善模型性能，并探索了不同人口统计特征对准确率指标的影响

Conclusion: D3G方法能够有效提升零样本图像分类的准确性，同时减少预训练多模态模型中的人口统计偏差，为公平AI系统提供了一种训练免费的解决方案
...
### [26] [Stackelberg Learning from Human Feedback: Preference Optimization as a Sequential Game](https://arxiv.org/abs/2512.16626)
*Barna Pásztor,Thomas Kleine Buening,Andreas Krause*

Main category: cs.LG

TL;DR: SLHF是一种新的偏好优化框架，将对齐问题建模为领导者策略和跟随者策略之间的序贯博弈，相比RLHF和NLHF能捕获更丰富的偏好结构。


<details>
  <summary>Details</summary>
Motivation: 现有偏好优化方法如RLHF（基于标量奖励）和NLHF（基于同时博弈均衡）存在局限性，需要一种能捕获更丰富偏好结构、支持推理时精炼且对非传递性偏好更鲁棒的新框架。

Method: 将偏好优化建模为序贯博弈：领导者策略承诺一个动作，跟随者策略根据领导者的动作条件性地响应。将问题分解为跟随者的精炼问题和领导者对抗对手的优化问题。

Result: 在大型语言模型上的实验表明，SLHF在多样偏好数据集上实现强对齐，从0.5B到8B参数规模都有效，且推理时精炼能力可跨模型家族迁移而无需微调。

Conclusion: SLHF通过序贯博弈设计提供了比RLHF和NLHF更一致、对数据更敏感、对非传递性偏好更鲁棒的偏好优化框架，支持推理时精炼和跨模型迁移。
...
### [27] [Surely Large Multimodal Models (Don't) Excel in Visual Species Recognition?](https://arxiv.org/abs/2512.15748)
*Tian Liu,Anwesha Basu,James Caverlee,Shu Kong*

Main category: cs.LG

TL;DR: 提出POC方法，利用LMMs对FSL专家模型的预测进行后验校正，在VSR任务中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 视觉物种识别需要专业标注但数据稀缺，FSL专家模型表现优于LMMs，但发现LMMs能有效校正专家模型的错误预测

Method: 提出POC方法：使用FSL专家模型的top预测作为提示，结合softmax置信度和少量视觉示例，让LMMs重新排序预测结果

Result: 在5个VSR基准测试中，POC比现有FSL方法准确率提升+6.4%，无需额外训练、验证或人工干预

Conclusion: POC作为即插即用模块能显著增强现有FSL方法，证明了LMMs在专家模型后验校正中的有效性
...
### [28] [A Special Case of Quadratic Extrapolation Under the Neural Tangent Kernel](https://arxiv.org/abs/2512.15749)
*Abiel Kim*

Main category: cs.LG

TL;DR: 研究发现ReLU MLP在NTK机制下，对于靠近原点的外推点表现出二次外推行为，这与远离原点时的线性外推形成对比。


<details>
  <summary>Details</summary>
Motivation: 现有研究已充分分析了ReLU MLP在线性外推方面的机制，但对NTK机制下原点附近外推行为的分析相对不足。由于NTK诱导的无限维特征映射不具有平移不变性，原点附近的外推与远离原点的外推是不同的特殊情况，需要专门研究。

Method: 在神经正切核(NTK)机制下，分析ReLU多层感知机在外推时的行为。特别关注原点附近这一特殊情况，并与远离原点的情况进行对比，利用特征映射的旋转不变性来研究这两种极端情况。

Result: 发现了在NTK机制下，当评估点靠近原点时，ReLU MLP表现出二次外推行为，这与远离原点时的线性外推形成鲜明对比，揭示了两种不同的外推机制。

Conclusion: ReLU MLP在NTK机制下的外推行为取决于评估点与原点之间的距离：远离原点时表现为线性外推，靠近原点时则表现出二次外推。这两种特殊情况代表了ReLU NTK外推的典型极端界限。
...
### [29] [GLOW: Graph-Language Co-Reasoning for Agentic Workflow Performance Prediction](https://arxiv.org/abs/2512.15751)
...
Conclusion: GLOW通过统一图结构和语义建模，有效解决了Agentic Workflows性能预测的挑战，为自动化AW生成提供了可扩展的解决方案。
...
TL;DR: 提出TAO-Net两阶段自适应OOD分类网络，通过混合OOD检测和LLM语义增强提示，实现加密流量的细粒度分类，显著提升对新应用识别性能。


<details>
  <summary>Details</summary>
Motivation: 加密流量分类面临新应用不断涌现的挑战，现有方法基于预定义类别，无法有效处理OOD流量模式。虽然有些方法将未知流量归为"其他"类别，但缺乏细粒度分类能力。

Method: 提出两阶段自适应OOD分类网络：第一阶段采用混合OOD检测机制，结合基于transformer的层间变换平滑性和特征分析；第二阶段利用大语言模型和语义增强提示策略，将OOD分类转化为生成任务。

Result: 在三个数据集上，TAO-Net达到96.81-97.70%宏精度和96.77-97.68%宏F1，显著优于先前方法（仅44.73-86.30%宏精度），特别是在识别新兴网络应用方面表现优异。

Conclusion: TAO-Net通过创新的两阶段设计，有效解决了加密流量分类中的OOD问题，实现了对已知和未知流量的准确细粒度分类，为处理不断变化的网络应用环境提供了有效解决方案。
...
TL;DR: 提出两种基于Kolmogorov-Arnold Networks的可视化工具PKAN和MKAN，用于增强高维数据集的解释性，超越传统相关性分析


<details>
  <summary>Details</summary>
Motivation: 高维数据集中变量间的复杂关系（高维度和共线性）使得数据解释面临挑战，传统相关性分析方法在捕捉非线性关系和提供可解释性方面存在局限

Method: 基于Kolmogorov-Arnold Networks开发两种可视化工具：PKAN（成对KAN矩阵）用于表征变量间的非线性关联，MKAN（多元KAN贡献矩阵）作为非线性特征排序工具，量化输入变量对目标变量的相对贡献

Result: 实验比较表明，PKAN和MKAN比Pearson相关性和互信息方法产生更稳健和信息丰富的结果，能捕捉关系的强度和函数形式

Conclusion: 这些工具支持模型开发流程中的预处理和后处理，促进隐藏物理模式的发现和领域知识指导的模型开发，增强了复杂数据集的解释性和简约性
...
TL;DR: ReactorFold：将核反应堆燃料组件设计转化为序列建模问题，使用语言模型生成新颖设计，能自主调整设计参数并发现传统方法无法触及的不对称配置。


<details>
  <summary>Details</summary>
Motivation: 传统核反应堆设计方法受限于固定的人工定义配置空间，无法发现根本性的新设计拓扑结构。需要一种能超越人类设计约束、探索更大设计空间的方法。

Method: 将燃料组件设计转化为序列建模问题，使用语言模型结合蒙特卡洛数据、参数高效微调和直接偏好优化（DPO）进行训练，学习压水堆组件的潜在结构。

Result: DPO对齐的模型展现出设计空间扩展能力：尽管只在固定钆棒数量的配置上训练，却能自主调整钆库存以满足严格的功率峰值约束。模型还发现了高性能的不对称配置，挑战传统的对称装载启发式方法。

Conclusion: 语言模型能够内化因果物理关系，超越人类施加的设计约束，为核反应堆设计开辟了新的可能性，能够访问传统搜索方法无法触及的设计领域。
...
### [33] [BUILD with Precision: Bottom-Up Inference of Linear DAGs](https://arxiv.org/abs/2512.16111)
...
<details>
  <summary>Details</summary>
Motivation: 从观测数据中学习有向无环图(DAG)结构是因果发现、统计信号处理和机器学习中的核心问题。在线性高斯结构方程模型(SEM)且噪声方差相等的条件下，该问题是可识别的，但需要有效算法来利用精度矩阵的特殊结构进行DAG恢复。

Method: 提出BUILD算法：1) 利用观测数据精度矩阵的特殊结构；2) 自底向上逐步识别叶节点及其父节点；3) 剪除叶节点及相关边；4) 重复过程直至完全重构DAG。针对有限数据估计精度矩阵的问题，采用周期性重新估计策略以增强鲁棒性。

Result: 在具有挑战性的合成基准测试中，BUILD算法在DAG学习方面表现优于现有最先进算法，同时提供了明确的复杂度控制机制。

Conclusion: BUILD算法通过利用线性高斯SEM下精度矩阵的特殊结构，提供了一种确定性的自底向上DAG学习方法，在有限数据条件下通过重新估计策略保持鲁棒性，为DAG结构学习提供了有效解决方案。
...
TL;DR: 提出TMvRKM模型，通过正则最小二乘方法高效确定最优分离超平面，解决传统多视图SVM的计算和泛化挑战


<details>
  <summary>Details</summary>
Motivation: 传统多视图支持向量机在高维空间中使用核技巧时难以有效捕捉决策边界，容易产生错误，且难以处理多视图数据集中的视图不一致问题

Method: 提出多视图孪生限制核机(TMvRKM)，结合核机器和多视图框架优势，采用正则最小二乘方法替代传统二次规划问题，包含耦合项平衡多视图误差，集成早期和晚期融合策略

Result: 在UCI、KEEL和AwA基准数据集上测试，实验和统计分析显示TMvRKM具有卓越的泛化性能，在所有场景中优于基线模型

Conclusion: TMvRKM通过高效的计算方法和有效的多视图信息整合，成功解决了传统多视图核方法的计算效率和泛化挑战
...
### [35] [Batch Normalization-Free Fully Integer Quantized Neural Networks via Progressive Tandem Learning](https://arxiv.org/abs/2512.16476)
*Pengfei Sun,Wenyu Jiang,Piew Yoong Chee,Paul Devos,Dick Botteldooren*

Main category: cs.LG

TL;DR: 提出一种无批归一化(BN)的全整数量化神经网络训练方法，通过渐进式逐层蒸馏实现仅使用整数运算的推理，适用于边缘设备部署。


<details>
  <summary>Details</summary>
Motivation: 现有量化神经网络(QNNs)大多依赖批归一化(BN)层，这阻碍了真正的整数运算部署。虽然已有尝试移除BN的方法，但往往无法恢复BN的稳定性和准确性，且施加了特定约束。

Method: 提出渐进式逐层蒸馏方案：从预训练的带BN教师模型出发，使用逐层目标和渐进补偿训练学生模型，使其完全使用整数运算推理且不含BN操作。

Result: 在ImageNet数据集上使用AlexNet，无BN模型在激进量化下获得有竞争力的Top-1准确率。该方法可直接集成到标准量化工作流中。

Conclusion: 该方法实现了端到端的整数运算推理，适用于资源受限的边缘和嵌入式设备部署，解决了量化神经网络中BN依赖的问题。
...
### [36] [Yantra AI -- An intelligence platform which interacts with manufacturing operations](https://arxiv.org/abs/2512.15758)
*Varshini Krishnamurthy*

Main category: cs.LG

TL;DR: 该论文开发了一个用于XRIT的智能生产系统，集成了机器学习模型（随机森林分类器和孤立森林）进行预测性维护和异常检测，结合Streamlit实时数据可视化，并整合GPT-4虚拟助手提供实时决策支持。


<details>
  <summary>Details</summary>
Motivation: 工业4.0快速发展，智能生产需要解决能源管理、预测性维护和AI驱动决策支持等关键问题，以提高运营效率和减少停机时间。

Method: 开发智能生产系统，集成随机森林分类器进行预测性维护，孤立森林进行异常检测，使用Streamlit实现实时数据可视化仪表板，并整合GPT-4构建AI虚拟助手。

Result: 系统使用模拟数据进行测试，显著提高了工作效率、能源管理和维修规划能力，系统设计为可扩展，适合XRIT生产环境的实时部署。

Conclusion: 系统成功解决了智能生产中的关键问题，未来工作将专注于实时数据集成和系统优化改进。
...
### [37] [Exploiting Radio Frequency Fingerprints for Device Identification: Tackling Cross-receiver Challenges in the Source-data-free Scenario](https://arxiv.org/abs/2512.16648)
...
<details>
  <summary>Details</summary>
Motivation: 边缘计算中基于深度学习的RFFI模型在实际部署时面临跨接收器性能显著下降的问题，因为不同接收器的硬件特性导致分布偏移。现有方法需要源域数据，但实际场景中源数据可能不可用。

Method: 提出MS-SHOT方法：1) 构建约束伪标签框架；2) 理论分析发现目标域性能对伪标签质量敏感；3) 采用动量中心引导的软伪标签生成；4) 施加全局结构约束以鼓励自信且多样化的预测。

Result: 在真实数据集上的大量实验表明，MS-SHOT在准确性和鲁棒性上始终优于现有方法，特别是在处理目标域标签偏移或非均匀类分布场景时表现突出。

Conclusion: MS-SHOT为源数据不可用的跨接收器RFFI适应提供了实用且可扩展的解决方案，有效解决了目标域标签分布未知或非均匀的关键限制。
...
### [38] [Semantic-Constrained Federated Aggregation: Convergence Theory and Privacy-Utility Bounds for Knowledge-Enhanced Distributed Learning](https://arxiv.org/abs/2512.15759)
*Jahidul Arafat*

Main category: cs.LG

TL;DR: 提出语义约束联邦聚合(SCFA)框架，将领域知识约束融入联邦学习，在非IID数据下提升收敛速度并改善隐私-效用权衡。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习在非IID数据下收敛缓慢，且现有解决方案将所有客户端更新同等对待，忽略了语义有效性。需要将领域知识约束融入分布式优化以提高性能。

Method: 引入语义约束联邦聚合(SCFA)框架，将领域知识约束融入联邦学习优化过程。使用知识图谱编码约束（从ISA-95和MASON本体中提取3000个约束），在理论分析中证明收敛率O(1/sqrt(T) + ρ)，其中ρ表示约束违反率。

Result: 理论分析显示约束将有效数据异质性降低41%，通过假设空间缩减因子θ=0.37改善隐私-效用权衡。在ε=10的差分隐私下，约束正则化将效用保持在非隐私基线的3.7%内（标准联邦学习为12.1%退化），提升2.7倍。在博世生产数据实验中，实现22%更快收敛、41.3%模型发散减少，约束违反阈值ρ<0.05时保持90%最优性能。

Conclusion: SCFA是首个为基于约束的联邦学习建立收敛理论的方法，通过领域知识约束显著改善非IID数据下的联邦学习性能，理论预测与实证观察高度一致(R²>0.90)。
...
TL;DR: 本文介绍了"无量纲学习"方法，这是一种结合经典量纲分析与现代机器学习的数据驱动框架，用于从实验数据中发现无量纲数和标度律，并通过正则化技术获得可解释的物理定律。


<details>
  <summary>Details</summary>
Motivation: 传统实验数据分析往往难以自动发现物理变量之间的无量纲关系和标度律。本文旨在开发一种结合经典量纲分析与现代机器学习的方法，从实验测量数据中自动发现紧凑的物理定律，揭示变量之间的量纲不变性。

Method: 方法首先从物理量的测量数据出发，识别将变量组合成无量纲群的基本方式，然后使用神经网络发现哪些组合能最好地预测实验输出。关键创新是一种正则化技术，鼓励学习到的系数取简单的可解释值（如整数或半整数），使发现的定律既准确又具有物理意义。

Result: 该方法系统研究了测量噪声和离散采样对发现过程的影响，证明正则化方法对实验不确定性具有鲁棒性。方法成功处理了具有单个或多个无量纲数的情况，揭示了不同但等效的表示如何捕捉相同的底层物理。方法能够从实验数据中发现准确且物理可解释的标度律。

Conclusion: 尽管取得了进展，但仍面临关键挑战：管理识别多个无量纲群的计算成本、理解数据特征的影响、自动化选择相关输入变量，以及为实验人员开发用户友好的工具。本教程既是教育资源，也是研究人员将无量纲学习应用于实验数据的实用指南。
...
TL;DR: 提出一个可解释的机器学习框架，基于计算流体动力学特征进行空间血栓风险评估，能够在保持计算效率和机制透明性的同时，将局部流动特征与血栓风险联系起来。
...
Method: 引入基于计算流体动力学流动特征的可解释机器学习框架，使用逻辑回归模型结合结构化特征选择流程，推导出紧凑且物理可解释的特征集（包括非线性特征组合）。

Result: 模型能够重现标记的风险分布，识别与血栓风险增加相关的不同流动特征集。即使仅在单个轴向泵操作点上训练，当应用于离心泵时，模型也能预测合理的血栓易发区域。

Conclusion: 可解释机器学习可以在保持计算效率和机制透明性的同时，将局部流动特征与血栓风险联系起来。低计算成本实现了快速的血栓形成性筛查，无需重复或昂贵的模拟。该框架补充了基于物理的血栓建模，为将可解释机器学习集成到CFD驱动的血栓分析和设备设计工作流程提供了方法基础。
...
TL;DR: CSA-TTA：一种跨样本增强测试时适应框架，通过利用其他个体的低血压事件来改善术中低血压的个性化预测，解决了罕见事件导致的不可靠测试时训练问题。


<details>
  <summary>Details</summary>
Motivation: 术中低血压（IOH）具有重要的手术风险，但由于患者特异性变异，准确预测仍然具有挑战性。测试时适应（TTA）为个性化预测提供了有前景的方法，但IOH事件的罕见性往往导致不可靠的测试时训练。

Method: 提出CSA-TTA框架：1）构建跨样本库，将历史数据分割为低血压和非低血压样本；2）采用粗到细检索策略构建测试时训练数据：先使用K-Shape聚类识别代表性聚类中心，然后基于当前患者信号检索前K个语义相似样本；3）在训练中集成自监督掩码重建和回顾性序列预测信号，增强模型对快速和细微术中动态的适应性。
...
Conclusion: CSA-TTA通过跨样本增强有效解决了IOH预测中罕见事件导致的测试时适应不可靠问题，显著提升了预测性能，特别是在零样本场景下表现突出，为术中低血压的个性化预测提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型全微调成本高、内存需求大，而现有参数高效微调方法（如LoRA）将训练限制在有限子空间，有时会降低性能。对于小型语言模型，效率提升更为重要，需要更有效的微调方法。

Method: AdaGradSelect基于梯度自适应选择要更新的Transformer块。它结合了基于Dirichlet的采样（考虑块过去被更新的频率）和epsilon-greedy探索策略，早期探索不同块，后期逐渐聚焦到最重要的块上。

Result: 实验表明，AdaGradSelect比全微调训练速度快约12%，GPU内存使用减少35%，性能接近全微调。在GSM8K数据集上，相比LoRA（rank 256）平均提升约3%，在MATH数据集上达到相似准确率。

Conclusion: AdaGradSelect为小型语言模型提供了比传统微调方法更有效和资源高效的替代方案，在保持性能的同时显著降低了计算成本。
...
TL;DR: 提出基于图神经网络(GNN)的混合孪生方法，用少量稀疏空间测量数据学习物理模型与真实现象间的"无知模型"差异，提高模拟精度


<details>
  <summary>Details</summary>
Motivation: 传统有限元等物理模型存在未建模效应和简化假设导致的误差，而纯数据驱动方法需要大量高质量时空数据，这在现实中难以获取

Method: 采用混合孪生方法，用GNN学习物理模型与真实现象间的"无知模型"差异，利用GNN处理稀疏空间测量的优势，无需密集时空参数数据

Result: 在非线性传热问题中，GNN成功捕捉无知模型，在不同网格、几何和载荷位置下都能泛化修正，显著提高模拟精度和可解释性

Conclusion: GNN混合孪生方法能用少量稀疏数据有效学习物理模型的误差，实现准确模拟，减少数据需求，提高泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 扩散策略在具身控制中表现出色，但存在高推理延迟和计算成本问题。现有静态加速方法（如量化）无法处理动态具身任务，而推测解码虽无损且自适应，但在扩散策略中尚未充分探索。需要解决如何在时间变化的任务难度下以更低成本匹配基础模型去噪质量，以及如何在具身环境中动态调整计算的问题。

Method: 提出时序感知强化学习推测扩散策略（TS-DP）：1）针对动态环境中任务难度随时间变化的问题，蒸馏Transformer草稿器模仿基础模型，替代昂贵的去噪调用；2）基于RL的调度器通过调整推测参数来适应时间变化的任务难度，在保持准确性的同时提高效率。

Result: 在多样化具身环境中的广泛实验表明，TS-DP实现高达4.17倍的推理加速，草稿接受率超过94%，推理频率达到25Hz，实现无性能下降的实时扩散控制。

Conclusion: TS-DP是首个为扩散策略引入时序自适应推测解码的框架，成功解决了动态具身任务中的计算效率问题，实现了高性能实时控制，为具身AI的实际应用提供了有效解决方案。
...
Conclusion: 金融表格模型对对抗攻击脆弱，对抗训练是有效的防御策略，但只能部分缓解问题
...
TL;DR: 本文系统评估了t-SNE中九种不同核函数在生物序列数据降维中的表现，发现余弦相似性核在可视化质量、运行效率和下游分析任务中均优于传统高斯核和隔离核。


<details>
  <summary>Details</summary>
Motivation: 传统t-SNE使用的高斯核缺乏数据依赖性且计算开销大，限制了其在分类生物序列数据上的可扩展性和有效性。虽然已有研究提出隔离核作为替代，但它可能无法最优地捕捉序列相似性，因此需要系统评估不同核函数在生物序列数据降维中的表现。

Method: 研究评估了九种不同核函数在t-SNE中的应用，使用三种嵌入方法：One-Hot Encoding、Spike2Vec和minimizers。通过主观可视化和客观指标（包括邻域保持分数）进行评估，并在六个不同的生物数据集上通过分类和聚类实验验证结果。

Result: 余弦相似性核在总体上优于其他核函数（包括高斯核和隔离核），具有更优的运行效率和更好的低维空间成对距离保持能力。核选择不仅影响可视化质量，还显著影响下游分析任务，余弦相似性核在不同数据类型和嵌入策略中表现出最稳健的性能。

Conclusion: 余弦相似性核是t-SNE应用于大规模生物序列分析的最优选择，它提供了更好的可视化质量、计算效率和下游分析性能，特别适合处理大规模生物序列数据。
...
TL;DR: 该特刊介绍了符号回归在物理科学中的应用，涵盖自动方程发现、涌现现象建模和计算仿真替代模型构建，并讨论了方法学挑战和未来方向。
...
Method: 特刊包含综述性介绍，涵盖符号回归的概念基础、与传统回归方法的对比、在物理科学中的主要应用案例（如有效理论推导、经验函数形式和替代模型构建），以及方法学考虑（搜索空间设计、算子选择、复杂度控制、特征选择、与现代AI方法集成）。

Result: 特刊收集的论文展示了符号回归在物理科学中的广泛应用，包括自动方程发现、涌现现象建模和计算昂贵仿真的紧凑替代模型构建。同时识别了当前挑战，如可扩展性、噪声鲁棒性、过拟合和计算复杂度。

Conclusion: 符号回归在物理科学中正加速发展并日益重要。未来方向包括整合对称性约束、渐近行为和其他理论信息。特刊论文共同展示了符号回归的快速进展及其在物理科学中不断增长的相关性。
...
<details>
  <summary>Details</summary>
Motivation: 当前离散序列建模（如DNA、蛋白质、语言）存在三种主要扩散方法：离散空间扩散、欧几里得空间高斯扩散和单纯形扩散。这些方法具有不同的算法、理论结构和权衡取舍，缺乏统一的理论框架，使得实践者难以在不同模型间切换。

Method: 构建理论框架，将三种离散扩散方法统一为Wright-Fisher种群遗传模型的不同参数化形式，发现单纯形扩散和高斯扩散是该模型的两个大种群极限。利用数十年数学遗传学文献开发稳定的单纯形扩散方法，并训练可在测试时在任意域执行扩散的单一模型。

Result: Wright-Fisher单纯形扩散比之前的单纯形扩散模型更稳定，在条件DNA生成任务上表现更好。同时，训练的多域模型在单个域上的表现与专门训练的模型相当。

Conclusion: 该研究成功构建了统一三种离散扩散方法的理论框架，解决了模型选择困境，使实践者能够灵活切换不同扩散域，同时提高了单纯形扩散的稳定性。
...
TL;DR: 提出DSO方法，使用强化学习优化激活转向，在推理时控制VLMs和LLMs的偏见与性能权衡，实现最先进的公平性-能力平衡。


<details>
  <summary>Details</summary>
Motivation: 生成模型（如VLMs）在决策时受人口属性偏见影响（如难以识别女性医生），而现有激活转向方法难以纠正需要跨人口群体等概率结果的偏见。用户需要在偏见缓解与模型性能之间进行权衡，需要推理时可控制的偏见减少方法。

Method: 提出直接转向优化（DSO），使用强化学习寻找线性变换来转向激活，专门针对偏见缓解而设计，同时保持对模型性能的控制。

Result: DSO在VLMs和LLMs上实现了最先进的公平性与能力权衡，为从业者提供推理时对权衡的控制。

Conclusion: 直接优化转向策略比依赖预定义启发式方法更有效，为模型行为控制提供了更有效的偏见干预方法。
...
<details>
  <summary>Details</summary>
Motivation: 真菌DNA条形码分类面临极端挑战：标注稀疏、类群分布长尾。传统监督学习方法难以泛化到未见物种，且难以捕捉数据的层次结构特征。

Method: 1. 基于状态空间模型架构构建基础模型；2. 采用预训练-微调范式，利用部分标注数据；3. 微调阶段整合多项增强技术：层次标签平滑、加权损失函数、MycoAI的多头输出层。

Result: 在具有明显分类分布偏移的挑战性真菌分类基准测试中，BarcodeMamba+在所有分类级别上都优于现有方法，每个增强组件都带来了显著的性能提升。
...
TL;DR: 论文提出并研究了上下文半监督学习(IC-SSL)，展示了Transformer能够利用未标记的上下文数据学习鲁棒的上下文相关表示，显著提升低标签场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 现有理论研究主要关注有明确标签对的监督学习场景，但实践中Transformer在标签稀疏或缺失时仍表现良好，这表明未标记的上下文演示中存在关键结构，需要研究Transformer如何利用未标记上下文进行表示学习。

Method: 引入并研究上下文半监督学习(IC-SSL)框架，其中少量标记示例伴随大量未标记点，通过Transformer架构利用未标记上下文学习上下文相关的鲁棒表示。

Result: Transformer能够利用未标记上下文学习鲁棒的上下文相关表示，实现准确预测，在低标签场景下显著提升性能。

Conclusion: 该研究为理解Transformer如何在ICL框架中利用未标记上下文进行表示学习提供了基础性见解，展示了半监督方法在上下文学习中的潜力。
...
TL;DR: SALVE框架：通过稀疏自编码器发现模型原生特征，用Grad-FAM验证，通过权重空间干预实现精确控制，并推导关键抑制阈值进行鲁棒性诊断。
...
Method: 使用ℓ₁正则化自编码器学习稀疏、模型原生特征基；用Grad-FAM进行特征级显著性映射验证；利用自编码器结构进行权重空间干预；推导关键抑制阈值α_crit量化类别对主导特征的依赖。
...
TL;DR: AIE4ML：首个针对AMD Versal AIE-ML架构的自动AI模型优化框架，实现接近峰值性能的推理，支持全片上执行和微秒级延迟


<details>
  <summary>Details</summary>
Motivation: AMD Versal AI Engine（AIE）上的高效AI推理面临挑战，包括紧密耦合的VLIW执行、显式数据路径和本地内存管理。先前工作仅关注第一代AIE内核优化，未解决跨2D阵列的完整神经网络执行问题。

Method: 提出AIE4ML框架：1）单内核层面实现接近架构峰值的性能；2）图级和系统级提供结构化并行化方法，可跨2D AIE-ML阵列扩展并利用专用内存块实现全片上执行；3）设计高效线性层实现，支持融合偏置加法和ReLU激活；4）通过新颖的图放置和搜索算法生成确定性、紧凑且拓扑优化的布局；5）无缝集成hls4ml或PyTorch等高级工具的量化模型。

Result: 在层扩展基准测试中，相对于单内核基线达到98.6%的效率，使用304个AIE块中的296个（97.4%），实现全片上数据移动。在实际模型拓扑评估中，AIE4ML在微秒级延迟约束下提供GPU级吞吐量。

Conclusion: AIE4ML是首个针对AMD AIE-ML架构的全面AI模型优化框架，能够实现接近峰值性能的推理，支持全片上执行，适用于粒子物理实验触发系统等超低延迟环境。
...
TL;DR: 决策树研究中使用的大量预测因子属于受隐私法规监管的数据类别，特别是在医疗保健领域，需要隐私保护方法和治理检查。


<details>
  <summary>Details</summary>
Motivation: 决策树方法广泛用于结构化表格数据，因其可解释性而受到重视，但研究中使用的预测因子（如年龄、诊断代码、位置）越来越多地受到隐私法规的监管。本研究旨在评估实际研究中使用的数据与隐私法规的关联程度。

Method: 收集已发表的决策树研究文献，将每个报告的预测因子分配到受监管的数据类别（如健康数据、生物识别标识符、儿童数据、财务属性、位置轨迹和政府ID），然后将每个类别与欧盟和美国隐私法律的具体条款进行关联分析。

Result: 研究发现许多报告的预测因子属于受监管类别，其中医疗保健领域占比最大，不同行业存在明显差异。分析了流行程度、行业构成和时间模式，并总结了每个法规框架参考年份下的监管对齐时间。

Conclusion: 研究结果为隐私保护方法和治理检查提供了支持，可以为超越决策树的机器学习实践提供信息，强调在数据使用中需要考虑隐私法规的合规性。
...
<details>
  <summary>Details</summary>
Motivation: 在森林环境中，商品RFID系统因信号衰减、多径效应和环境变化导致标签定位困难。现有指纹识别方法需要预先在已知位置部署标签，但在实际应用中（如野火响应）往往无法预先标记位置

Method: 使用高斯过程建模不同环境的RF信号响应特征，构建环境模型字典。提出加权对数似然方法，将未知环境的RF信号与字典中最匹配的环境模型关联，实现标签定位

Result: 能够达到接近GPS的定位精度，使用被动商品RFID可同时追踪数十个野火响应资产，无需预先标记位置，成本远低于GPS

Conclusion: 该方法为森林环境中的资产追踪提供了一种成本效益高、可扩展的解决方案，特别适用于野火响应等无法预先部署已知位置标签的应用场景
...
TL;DR: DR-RL：通过强化学习和在线矩阵扰动理论动态优化LLM中多头自注意力的低秩分解，在保持精度的同时显著减少计算量
...
Method: 使用强化学习代理将秩选择建模为序列策略优化问题，结合在线矩阵扰动理论实现增量秩更新，采用轻量级Transformer策略网络和批量SVD操作

Result: 在保持下游任务精度与全秩注意力统计等价的同时，显著减少浮点运算（FLOPs），尤其在长序列（L>4096）场景下效果明显

Conclusion: DR-RL为多头自注意力提供了自适应效率与理论严谨性结合的方案，是资源受限深度学习中有原则的、数学基础扎实的秩缩减替代方法
...
TL;DR: 提出一种结合灵活高阶有限差分格式和Rollout损失函数的降阶模型方法，用于提升参数化PDE在长时间尺度上的预测精度


<details>
  <summary>Details</summary>
Motivation: 传统数值方法求解复杂偏微分方程计算成本高，现有降阶模型虽然能处理参数化PDE族，但在长时间尺度上预测精度会显著下降

Method: 1) 引入灵活、高阶且计算成本低的有限差分格式；2) 提出Rollout损失函数，训练降阶模型在任意时间尺度上做出准确预测

Result: 在二维Burgers方程上验证了方法的有效性，展示了在长时间尺度上保持预测精度的能力

Conclusion: 通过结合新的数值格式和训练策略，显著提升了降阶模型在长时间预测中的性能，为解决复杂PDE的长时间尺度计算问题提供了有效方案
...
TL;DR: SNAC-Pack是一个自动化神经网络架构搜索框架，专注于FPGA部署，通过结合多阶段搜索和硬件资源评估器，实现精度、资源利用率和延迟的多目标优化，无需对每个候选模型进行耗时的综合。


<details>
  <summary>Details</summary>
Motivation: 现有神经架构搜索方法难以准确优化真实硬件性能，通常依赖比特操作等代理指标，需要开发能够直接针对FPGA部署进行优化的硬件感知搜索框架。

Method: SNAC-Pack整合了神经架构协同设计的多阶段搜索能力和资源利用率与延迟评估器，实现精度、FPGA资源利用率和延迟的多目标优化，避免了为每个候选模型进行耗时综合的需要。

Result: 在高能物理喷注分类任务中，SNAC-Pack模型达到63.84%的精度。在Xilinx Virtex UltraScale+ VU13P FPGA上综合后，该模型保持了基准精度，同时资源利用率与传统BOPs优化模型相当。
...
TL;DR: FTBSC-KGML是一个基于预训练-微调、考虑空间变异性和知识引导的机器学习框架，用于准确量化农业生态系统碳循环，特别适用于具有显著变异性的区域。


<details>
  <summary>Details</summary>
Motivation: 在决策相关尺度上准确且经济地量化农业生态系统碳循环对于气候缓解和可持续农业至关重要。传统方法通常依赖位置无关的参数化和独立训练，未能充分利用迁移学习和输入数据的空间异质性，限制了在具有显著变异性区域的适用性。

Method: 提出FTBSC-KGML框架，通过预训练-微调过程增强KGML-ag，结合站点特定参数。使用多站点遥感GPP、气候和土壤协变量数据进行全局预训练，然后在各州或站点进行微调，学习位置感知表示，提高有限数据下的局部准确性而不牺牲可解释性。

Result: FTBSC-KGML在验证误差上低于纯全局模型，解释能力更一致，能更好地捕捉跨州的空间变异性。

Conclusion: 该工作扩展了先前的SDSA-KGML框架，提出的空间异质性感知迁移学习方案通过预训练-微调过程有效提升了农业生态系统碳循环量化的准确性和适用性。
...
TL;DR: 提出基于强化学习的统一几何设计优化方法，结合技术经济考量优化热管微反应器，将平准化度电成本降低57%以上


<details>
  <summary>Details</summary>
Motivation: 热管微反应器适用于偏远地区，但存在规模不经济问题，财务可行性存疑。需要早期设计阶段就考虑经济和物理分析的综合性方法

Method: 采用统一几何设计优化方法：1) 随机采样训练高斯过程和多层感知机代理模型；2) 在强化学习框架中使用代理模型优化平准化度电成本；3) 约束燃料寿命、停堆裕度、峰值热通量和棒积分峰值因子

Result: 优化器巧妙调整设计参数，在满足约束条件下最小化成本，将平准化度电成本降低超过57%。运行维护成本和资本成本是主要贡献因素，特别是轴向反射层成本和控制鼓材料成本

Conclusion: 该方法有效整合技术经济考量，显著降低热管微反应器成本。未来将进行燃料和热管性能与多目标优化的全面集成，以深入理解约束与成本性能的相互作用
...
TL;DR: 本文探讨如何将可解释人工智能(XAI)集成到大数据分析流程中，用于欺诈检测和风险管理，提出了一个结合可扩展大数据基础设施与上下文感知解释机制的概念框架。


<details>
  <summary>Details</summary>
Motivation: 大数据在金融、保险和网络安全等领域的应用日益广泛，但自动化分析系统缺乏透明度、难以满足监管要求、缺乏可信度，需要将可解释AI集成到大数据分析流程中解决这些问题。

Method: 1. 回顾大数据关键特征和主要分析工具（分布式存储系统、流处理平台、欺诈检测模型）；2. 系统综述常用XAI方法（LIME、SHAP、反事实解释、注意力机制）；3. 分析这些方法在大规模部署时的优缺点；4. 提出集成可扩展大数据基础设施与上下文感知解释机制的概念框架。

Result: 识别出与可扩展性、实时处理、图模型和时间模型可解释性相关的研究空白，提出了解决这些挑战的概念框架，为可扩展XAI、隐私保护解释和标准化评估方法指明了研究方向。

Conclusion: 需要将XAI系统性地集成到大数据分析流程中，特别是在欺诈检测和风险管理领域。未来研究方向包括可扩展XAI、隐私感知解释机制以及可解释欺诈检测系统的标准化评估方法。
...
TL;DR: DeepOSets架构通过适当修改，成为多算子上下文学习器，能够从用户提示中的参数-解示例对中恢复新PDE的解算子，无需权重更新，并且是连续算子类的通用均匀逼近器。


<details>
  <summary>Details</summary>
Motivation: 探索非自回归、非注意力机制的DeepOSets架构在科学机器学习中的上下文学习能力，特别是在PDE求解中的算子学习潜力，填补该领域通用逼近理论研究的空白。

Method: 将DeepSets架构与DeepONets结合形成DeepOSets，通过适当修改使其成为多算子上下文学习器，能够从提示中的示例对学习新PDE的解算子。

Result: DeepOSets能够准确预测训练中未见PDE的参数对应解，在泊松方程和反应扩散方程的正反边值问题实验中表现良好，并且是连续算子类的通用均匀逼近器。

Conclusion: DeepOSets架构展示了强大的上下文学习能力，能够从示例中学习新PDE的解算子，为科学机器学习中的算子学习提供了新的理论保证和实用框架。
...
TL;DR: 研究发现高斯模糊在低精度实现中容易被逆向攻击，隐私性最差；像素化和像素化加噪声在适当粒度下能提供更好的隐私-效用平衡。


<details>
  <summary>Details</summary>
Motivation: 图像数据常包含人脸、车牌等隐私信息，负责任的数据发布需要隐藏这些信息，同时保持数据对模型训练的有用性。当前行业标准的高斯模糊方法可能存在隐私风险。

Method: 比较四种隐私保护算法：高斯模糊、像素化、像素化加噪声（DP-Pix）和裁剪。通过逆向攻击和识别攻击评估隐私性，通过在模糊人脸数据上训练模型的质量评估效用性。

Result: 高斯模糊在低精度实现中最容易被逆向攻击，隐私性最差；像素化和像素化加噪声在适当粒度下能同时提供隐私保护和模型训练效用。

Conclusion: 行业标准的高斯模糊方法隐私性不足，像素化和像素化加噪声是更好的选择。作者开发了Privacy Blur软件包提供建议参数和方法。
...
<details>
  <summary>Details</summary>
Motivation: 市场操纵现在通常源于协调的社交媒体活动而非孤立交易。零售投资者、监管机构和经纪商需要能够将在线叙事和协调模式与市场行为联系起来的工具。

Method: 使用parquet-native管道和Streamlit仪表板，融合Reddit活动、机器人和协调指标以及OHLCV市场特征。由于Reddit API限制，使用校准的合成社交特征匹配事件特征，市场数据使用真实历史数据。

Result: 构建了AIMM-GT数据集（33个标记的股票-交易日），展示了初步的判别能力，AIMM在2021年1月GME挤压峰值前22天发出预警。

Conclusion: 虽然当前标记集较小，但AIMM框架显示了社交媒体驱动市场监控的潜力，并发布了代码、数据集架构和仪表板设计以支持相关研究。
...
<details>
  <summary>Details</summary>
Motivation: 机器遗忘技术虽然能保护请求遗忘的用户权利，但引入了新的隐私风险。先前研究主要关注已遗忘数据的隐私，而对保留数据的隐私风险探索不足。本文旨在填补这一空白，首次揭示机器遗忘在双视图设置下的隐私漏洞。

Method: 从信息论角度引入"隐私知识增益"概念，证明双视图设置能让攻击者获得更多信息。提出DVIA（双视图推理攻击）方法，通过黑盒查询两个模型来提取保留数据的成员信息。DVIA无需训练攻击模型，使用轻量级似然比推理模块进行高效推理。

Result: 在不同数据集和模型架构上的实验验证了DVIA的有效性，并突显了双视图设置固有的隐私风险。攻击者通过查询两个模型可以获得比单独查询任一模型更多的隐私信息，从而放大隐私泄露。

Conclusion: 机器遗忘在双视图设置下存在显著的隐私风险，攻击者可以利用原始模型和遗忘模型的查询差异来推断保留数据的成员信息。这揭示了机器遗忘技术在实际部署中需要考虑的新安全挑战。
...
TL;DR: INTELLECT-3是一个106B参数的专家混合模型（激活12B），通过大规模强化学习训练，在数学、代码、科学和推理基准测试中达到同尺寸模型的最先进性能，并开源了完整的RL基础设施栈。


<details>
  <summary>Details</summary>
Motivation: 开发一个在多种认知任务上表现优异的大规模语言模型，同时构建完整的端到端强化学习基础设施，以支持大规模、高效的RL训练，并促进社区研究和应用。

Method: 使用GLM-4.5-Air-Base作为基础模型，通过大规模强化学习训练106B参数的MoE模型（12B激活）。开发了prime-rl框架支持大规模异步RL训练，从单节点扩展到数千GPU，支持多轮交互和工具使用。使用完整的RL基础设施栈进行SFT和RL训练。

Result: INTELLECT-3在数学、代码、科学和推理基准测试中达到同尺寸模型的最先进性能，超越了多个更大的前沿模型。RL训练成功扩展到512个H200 GPU，保持了高训练效率。

Conclusion: 成功开发了高性能的INTELLECT-3模型和完整的RL基础设施栈，证明了大规模RL训练的有效性，并通过开源促进了社区研究和应用发展。
...
TL;DR: 该研究提出了一种基于图神经网络的多模态框架，通过分析手绘立方体草图来检测阿尔茨海默病，将手绘草图转换为图结构表示，并结合人口统计信息和神经心理学测试分数进行分类。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病的早期和可及性检测仍然是一个关键的临床挑战。立方体复制任务提供了一种简单但信息丰富的视觉空间功能评估方法，但需要更系统化的分析框架来提高检测准确性和可解释性。

Method: 将手绘立方体草图转换为图结构表示，捕捉几何和拓扑特性。使用图神经网络处理节点特征（空间坐标、局部图元拓扑、角度几何），并与年龄、教育程度和神经心理学测试特征通过后期融合模型进行多模态集成。

Result: 基于图的表示提供了强大的单模态基线，显著优于基于像素的卷积模型。多模态集成进一步提高了性能和对类别不平衡的鲁棒性。SHAP可解释性分析确定了特定的图元基序和几何扭曲是关键预测因子，与临床观察到的AD患者立方体绘图紊乱现象一致。

Conclusion: 基于图的立方体复制分析为阿尔茨海默病筛查提供了一种可解释、非侵入性且可扩展的方法，能够有效捕捉AD相关的视觉空间功能障碍特征。
...
TL;DR: stMFG是一个用于空间转录组学分析的多尺度交互融合图网络，通过层间跨视图注意力机制动态整合空间和基因特征，在DLPFC和乳腺癌数据集上优于现有方法。
...
Method: 提出stMFG模型，采用多尺度交互融合图网络，引入层间跨视图注意力机制，在每次卷积后动态整合空间和基因特征。结合跨视图对比学习和空间约束，增强区分能力同时保持空间连续性。
...
<details>
  <summary>Details</summary>
Motivation: 基于排序的零阶优化方法（如CMA-ES、自然进化策略等）在实践中广泛成功，但现有理论分析仅提供渐近结果，缺乏显式收敛率。本文旨在填补这一理论空白。

Method: 分析一个简单的基于排序的零阶算法，采用新颖的分析方法，避免了传统的漂移和信息几何技术。

Result: 对于d维问题：强凸光滑函数达到$\widetilde{\mathcal O}(dL/μ·log(dL/μδ)·log(1/ε))$查询复杂度；非凸光滑函数达到$\mathcal O(dL/ε·log(1/ε))$查询复杂度。这些结果以至少1-δ的概率成立。

Conclusion: 首次为基于排序的零阶优化算法建立了显式非渐近收敛率，为理解这类启发式方法的高效性提供了新见解。
...
TL;DR: 该研究训练了一个机器学习模型来预测地质灾害（如滑坡、雪崩）的运移范围，相比传统数值模拟方法计算速度快100-10000倍，同时能准确预测流动范围和沉积厚度。


<details>
  <summary>Details</summary>
Motivation: 地质灾害（滑坡、雪崩等）的快速运移预测对保护生命、基础设施和生态系统至关重要。现有方法面临计算速度与物理真实性之间的权衡，难以实现大规模快速预测。

Method: 训练机器学习模型，使用超过10万个数值模拟数据，涵盖1万多个真实世界数字高程模型区域，学习预测地质灾害的运移范围和沉积厚度。

Result: 模型能高精度预测流动范围和沉积厚度，计算速度比数值求解器快100-10000倍，能重现关键物理行为（如分流和沉积模式），并能泛化到不同类型的流动、规模和地形。

Conclusion: 神经模拟方法实现了跨多样化真实地形的快速、空间分辨的运移预测，为灾害风险减少和基于影响的预报开辟了新途径，有望将物理真实的地质灾害建模扩展到大规模预警系统所需的时空尺度。
...
<details>
  <summary>Details</summary>
Motivation: 现有开放集分类方法通常将所有OOD样本视为单一类别，但在欺诈检测和医疗诊断等高风险场景中，需要对OOD样本有更深入的了解，包括其可能的标签。这引出了一个关键问题：能否在没有真实标签信息的情况下将OOD检测扩展到OOD分类？

Method: 提出粗到细开放集分类（CFC）框架，包含三个关键组件：1）粗分类器使用LLM提示进行OOD检测和异常标签生成；2）基于GNN的细分类器，使用粗分类器识别的OOD样本进行训练，以增强OOD检测和ID分类；3）通过LLM提示和后处理的OOD标签实现细化的OOD分类。

Result: 实验结果表明，CFC在图和文本领域上比最先进方法提高了10%的OOD检测性能，在图数据集上实现了高达70%的OOD分类准确率。

Conclusion: CFC框架成功地将OOD检测扩展到OOD分类，利用LLM生成语义上有意义的OOD实例，提高了可解释性和实际效用，为高风险应用中的开放集分类提供了有效解决方案。
...
Result: 在多个图分类基准测试中，SEAL始终优于现有的联邦图学习基线方法，为更多参与者提供了性能增益，增强了局部GNN模型的分类准确性和泛化能力。

Conclusion: SEAL算法通过锐度感知优化和维度坍塌缓解，有效解决了联邦图学习中的数据异构性问题，提高了模型在异构数据上的泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: 二阶潜在因子(SLF)模型在处理高维不完整数据时有效，但由于其双线性和非凸性质，优化非常困难。锐度感知最小化(SAM)方法被提出用于寻找平坦的局部最小值，从而改善表示学习模型的泛化能力。

Method: 提出锐度感知SLF(SSLF)模型，包含两个关键思想：1)通过Hessian-vector products获取二阶信息；2)通过设计的Hessian-vector products在曲率(Hessian)中注入锐度项。

Result: 在多个工业数据集上的实验表明，所提出的模型始终优于最先进的基线方法。

Conclusion: SSLF模型通过结合二阶信息和锐度感知优化，有效解决了SLF模型的优化困难问题，并在实际应用中表现出优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前主流的大语言模型后训练量化方法通常对所有网络层采用统一的量化策略，忽略了不同层在算法适用性上的显著差异。为了解决这一限制，需要一种能够考虑层间差异的量化方法。

Method: 提出CKA引导的模块化量化框架，这是一个无需微调、即插即用的算法异构量化方法。该方法独立评估每个层上的多种PTQ算法，使用线性中心核对齐（CKA）作为度量标准，自动为每个层选择最优的量化策略，然后将这些单独优化的策略集成起来构建混合量化模型。

Result: 实验表明，该方法在主流LLM（包括LLaMA和Qwen）上，在困惑度（PPL）和下游任务性能方面，始终优于均匀量化基线和最先进的混合精度方法。

Conclusion: CKA引导的模块化量化通过考虑不同层的算法适用性差异，实现了更优的量化效果，为后训练量化提供了一种有效的异构量化框架。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在安全关键和受监管领域的应用增加，模型中保留的敏感或禁止知识带来了隐私泄露、监管不合规和潜在滥用等风险。现有遗忘技术假设遗忘集和保留集完全分离，但在实际应用中数据分布高度纠缠，传统方法会损害模型效用或无法确保安全。

Method: 提出了选择性表示误导遗忘（SRMU）框架，采用特征感知和方向可控的激活编辑方法。与无差别权重扰动不同，SRMU使用结构化误导向量和激活重要性图，选择性地抑制有害表示同时保留良性表示的效用。

Result: 在WMDP基准测试中，SRMU在低和高纠缠配置下都实现了最先进的遗忘性能，且效用损失最小。在20-30%数据重叠的情况下仍保持有效，而现有基线方法会失效。

Conclusion: SRMU为安全驱动的模型治理、隐私合规和受控知识移除提供了鲁棒基础，适用于新兴的大语言模型应用场景。
...
TL;DR: 开发了首个用于电池寿命预测的基础模型PBT，通过领域知识编码的专家混合层，在15个不同数据集上实现最先进性能


<details>
  <summary>Details</summary>
Motivation: 电池循环寿命的早期预测对加速电池研究、制造和部署至关重要。现有机器学习方法受限于数据稀缺和老化条件多样性导致的异质性，而其他领域的基础模型通过迁移学习实现了广泛泛化，但电池寿命预测领域尚未有基础模型

Method: 提出了预训练电池变压器(PBT)，这是首个用于电池寿命预测的基础模型，通过领域知识编码的专家混合层开发，在最大的公共电池寿命数据库上进行验证

Result: PBT从13个锂离子电池数据集中学习可迁移表示，平均性能优于现有模型19.8%。通过迁移学习，在涵盖不同运行条件、形成协议和化学组成的15个多样化数据集上实现了最先进性能
...
TL;DR: 提出一种针对树集成模型的公平性量化验证方法，相比传统验证仅返回单个反例，该方法能估计所有反例的比例并定位其区域，为诊断和缓解偏见提供重要信息。


<details>
  <summary>Details</summary>
Motivation: 现有公平性验证方法主要针对深度神经网络，且大多只能提供下界估计，性能扩展性差。树集成模型在现实应用中广泛使用，需要专门的高效量化验证技术来评估其公平性。

Method: 利用树集成模型的离散结构特性，提出一种高效的量化技术，能够提供任意时间的上界和下界估计。该方法超越了基于反例引导抽象精化的通用框架，针对树模型进行了专门优化。

Result: 在五个广泛使用的数据集上的实验表明，该方法在有效性和效率方面表现优异。在公平性测试应用中，显著优于最先进的测试技术。

Conclusion: 该研究为树集成模型的公平性量化验证提供了一种高效解决方案，能够同时提供上界和下界估计，相比现有方法在性能和准确性方面都有显著提升，有助于更好地诊断和缓解机器学习模型中的偏见问题。
...
<details>
  <summary>Details</summary>
Motivation: 注意力机制是长上下文LLM推理的主要延迟来源，特别是在推理模型和RAG等日益流行的工作负载中。现有方法需要解决注意力计算效率低下的问题，同时保持模型准确性。

Method: Kascade利用两个关键观察：1）后softmax注意力本质上是稀疏的；2）高权重键的身份在相邻层之间是稳定的。方法包括：在锚定层计算精确的Top-k索引，然后在中间重用层重用这些索引；通过动态规划算法选择锚定层以最大化跨层相似性；支持头感知的Top-k选择和重用；针对预填充和解码注意力进行高效实现优化。
...
Conclusion: Kascade提供了一种有效的训练无关稀疏注意力解决方案，显著加速长上下文LLM推理，同时保持模型准确性，为实际部署提供了实用价值。
...
TL;DR: 提出NDRL（嵌套双智能体强化学习）方法优化棉花灌溉与氮肥管理，通过父智能体宏观决策和子智能体微观动态调节，提升产量和资源利用效率。
...
Method: 提出NDRL方法：父智能体基于预期累积产量效益识别有前景的宏观灌溉施肥行动，减少无效探索；子智能体奖励函数包含量化的水分胁迫因子（WSF）和氮胁迫因子（NSF），使用混合概率分布动态优化每日策略。使用2023-2024年田间实验数据校准验证DSSAT模型模拟真实环境。

Result: 相比最佳基线：模拟产量在2023和2024年均提升4.7%；灌溉水生产率分别提升5.6%和5.1%；氮偏因子生产率分别提升6.3%和1.0%。
...
TL;DR: 提出GLNO框架，通过拉普拉斯谱表示和极-留数分解，将神经算子扩展到非周期、衰减动态和任意黎曼流形上的学习。


<details>
  <summary>Details</summary>
Motivation: 现有神经算子方法在处理非周期激励、瞬态响应以及不规则/非欧几何上的信号时存在困难，需要一种能处理衰减动态和任意流形上函数学习的通用框架。

Method: 提出基于极-留数分解和指数基函数的广义算子学习框架，引入几何拉普拉斯神经算子(GLNO)，将拉普拉斯谱表示嵌入到拉普拉斯-贝尔特拉米算子的特征基中，并设计了网格不变网络架构GLNONet。

Result: 在PDEs/ODEs和真实数据集上的广泛实验表明，该方法相比其他最先进模型具有鲁棒的性能优势。

Conclusion: GLNO框架成功扩展了算子学习的能力，能够处理非周期衰减动态和任意黎曼流形上的问题，无需周期性假设或均匀网格，为复杂几何和动态系统提供了有效的学习工具。
...
TL;DR: 提出多保真度延迟接受方案，通过多保真度神经网络结合不同精度的求解器，在离线阶段训练后，在线阶段仅需低精度求解器评估，避免高保真模拟，加速贝叶斯反问题求解。


<details>
  <summary>Details</summary>
Motivation: 传统基于物理模型的反向不确定性量化计算成本高，特别是涉及偏微分方程时，有限元等全阶模型使MCMC采样计算不可行。数据驱动代理模型受限于高保真数据生成成本，而仅使用低保真数据会降低求解精度。

Method: 扩展多级延迟接受框架，引入多保真度神经网络，将不同精度的求解器预测结合。高保真评估仅限于离线训练阶段，在线阶段通过评估粗求解器并将其输出传递给训练好的神经网络来获得似然评估。

Result: 该方法提高了低保真求解器的近似精度，导致更长的子链长度、更好的混合特性和加速的后验推断。在两个基准反问题上（稳态各向同性地下水流、非稳态反应扩散系统）获得了显著的计算节省。

Conclusion: 提出的多保真度延迟接受方案能够一致地整合异构粗求解器，比标准多级延迟接受提供更大灵活性，有效解决了高保真模拟计算成本问题，加速了贝叶斯反问题的后验推断。
...
TL;DR: 论文提出多智能体系统在金融预测任务中可能产生无法追溯到单个智能体的涌现性偏见，需要开发整体公平性评估方法而非还原论分析。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统在预测任务中表现优异，但缺乏有效的公平性评估方法，在金融等高风险领域部署存在风险，因为偏见决策可能导致监管违规和财务损失。

Method: 通过大规模模拟实验，在不同多智能体配置（包括不同通信和协作机制）下，使用公平性指标评估金融表格数据领域的多智能体预测系统。

Result: 研究发现多智能体系统在金融决策中表现出无法追溯到单个智能体组件的涌现性偏见模式，表明多智能体系统可能展现出真正的集体行为。

Conclusion: 金融多智能体系统的公平性风险是模型风险的重要组成部分，对信用评分和收入估计等任务有实际影响，必须将多智能体决策系统作为整体实体而非通过还原论分析进行评估。
...
TL;DR: 提出基于计算学习技术的风力涡轮机叶片故障检测新方法，比较逻辑回归和聚类两种模型，发现聚类在精度和数据分割方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 风力涡轮机叶片故障检测对系统可靠性至关重要，需要开发有效的早期故障检测方法来提高风能系统的运行效率和安全性。

Method: 采用两种计算学习模型：1）逻辑回归模型，与神经网络、决策树和朴素贝叶斯方法进行比较；2）聚类模型，用于数据分割和模式识别。使用Orange数据挖掘工具实现。

Result: 逻辑回归模型优于神经网络、决策树和朴素贝叶斯方法；聚类模型在精度和数据分割方面表现更优，能更好地捕捉数据底层特征。

Conclusion: 聚类方法可能比监督学习方法更适合风力涡轮机叶片故障检测，整合不同计算学习技术可提高系统可靠性。未来将结合这些方法进一步提高检测精度，并扩展到能源基础设施的其他关键部件。
...
<details>
  <summary>Details</summary>
Motivation: LDA主题模型中主题数量T的选择是一个关键设计决策，它直接影响模型的统计拟合度和可解释性。传统方法通常需要手动调整或使用启发式方法，缺乏系统化的优化框架。
...
Result: 实验表明，虽然GA、ES、PABBO和SABBO最终都能达到相似的困惑度范围，但摊销优化器在样本和时间效率上显著更优。SABBO通常只需一次评估就能识别出接近最优的主题数量，PABBO在几次评估内就能找到有竞争力的配置，而GA和ES需要几乎全部预算才能达到相同水平。

Conclusion: 摊销黑盒优化方法（特别是SABBO）为LDA主题数量选择问题提供了高效且有效的解决方案，显著减少了计算成本，使主题模型调参更加实用。
...
TL;DR: 本文提出了一种结合卷积神经网络（CNN）和更高阶奇异值分解（HOSVD）分类器的白血病细胞检测方法，应用于医疗物联网（IoMT）架构，实现快速安全的急性淋巴细胞白血病（ALL）识别。


<details>
  <summary>Details</summary>
Motivation: 急性淋巴细胞白血病（ALL）是一种致命的血液疾病，早期诊断至关重要。传统人工检测存在人为错误风险且耗时，需要基于人工智能的自动化检测方法来提高诊断效率和准确性。

Method: 采用卷积神经网络（CNN）提取特征，结合新型分类器更高阶奇异值分解（HOSVD）对显微镜血液图像中的ALL细胞和正常细胞进行分类，并将模型部署在医疗物联网（IoMT）架构中。

Result: 在急性淋巴细胞白血病图像数据库（ALL-IDB2）上测试，模型在测试阶段达到了平均98.88%的准确率。

Conclusion: 提出的CNN-HOSVD模型能够高效准确地识别白血病细胞，结合IoMT架构可实现实时医患通信，为白血病早期诊断提供了有效的自动化解决方案。
...
TL;DR: PLSCAN是一种新型密度聚类算法，通过识别HDBSCAN*产生稳定叶簇的所有最小簇大小，无需预设密度阈值，在多个真实数据集上表现优于HDBSCAN*。


<details>
  <summary>Details</summary>
Motivation: 密度聚类算法在探索性数据分析中很有用，但实际应用需要选择超参数（如DBSCAN的密度阈值、HDBSCAN*的最小簇大小），这在没有先验知识的情况下很困难。

Method: 提出PLSCAN算法，应用尺度空间聚类原理，等价于新型度量空间上的持久同调，能高效识别HDBSCAN*产生稳定叶簇的所有最小簇大小。

Result: 在多个真实数据集上，PLSCAN比HDBSCAN*获得更高的平均ARI（调整兰德指数），对互达邻域数变化更不敏感；在低维数据集上计算成本与k-Means竞争，高维时与HDBSCAN*相似。

Conclusion: PLSCAN是一种有效的密度聚类算法，无需预设密度阈值，能自动识别稳定簇，在性能和计算效率上都有优势。
...
TL;DR: 提出Abacus方法，通过预测用户事件的频率分布来增强深度序列模型，解决广告系统中用户购买行为建模的类别不平衡和时序不规则问题。


<details>
  <summary>Details</summary>
Motivation: 展示广告系统中用户购买行为建模面临正样本稀疏和用户行为随机性的挑战，导致严重的类别不平衡和不规则事件时序。现有方法依赖手工特征或仅提取序列信号，忽略了用户意图的细粒度时序演化和有用的事件计数统计。

Method: 提出Abacus方法，通过自监督预训练策略预测用户事件的频率分布。进一步提出混合目标函数，将Abacus与序列学习目标结合，融合聚合统计的稳定性和序列建模的敏感性。

Result: 在两个真实数据集上的实验表明，Abacus预训练优于现有方法，加速了下游任务收敛，混合方法相比基线提升了高达6.1%的AUC。

Conclusion: Abacus方法有效解决了用户行为建模中的类别不平衡和时序不规则问题，通过结合频率分布预测和序列建模，显著提升了广告系统的预测性能。
...
TL;DR: DataFlow是一个统一的LLM驱动数据准备框架，通过系统级抽象和模块化操作符解决现有数据准备流程的随意性问题，显著提升下游LLM性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLM数据准备主要依赖临时脚本和松散的工作流，缺乏原则性抽象、可复现性差，且对模型在环数据生成支持有限，需要更系统化的解决方案。

Method: 提出DataFlow框架，包含系统级抽象、PyTorch风格管道构建API、近200个可重用操作符和六个领域通用管道。引入DataFlow-Agent通过操作符合成、管道规划和迭代验证将自然语言规范转换为可执行管道。

Result: 在六个代表性用例中持续提升下游LLM性能：数学、代码和文本管道优于人工策划数据集和专用合成基线，Text-to-SQL执行准确率提升+3%，代码基准平均提升7%，MATH、GSM8K和AIME提升1-3分。统一10K样本数据集使基础模型超越使用1M Infinity-Instruct数据训练的对应模型。

Conclusion: DataFlow为可靠、可复现和可扩展的LLM数据准备提供了实用高性能基础，为未来以数据为中心的AI发展建立了系统级基础。
...
TL;DR: 该研究对多种机器学习算法（SVM、NB、LR、AdaBoost、XGBoost、SVM_R）与神经符号AI（NeSy）在文本分类任务上进行了比较分析，探索了不同文本表示和特征提取技术，发现NeSy方法在有限数据集上能达到与MLP相当的性能。


<details>
  <summary>Details</summary>
Motivation: 文本分类（如博客性别分类）是机器学习中成熟的研究领域，在市场营销、客户推荐等领域有广泛应用。本研究旨在比较传统机器学习算法与新兴的神经符号AI方法在文本分类任务上的表现。

Method: 1. 比较了SVM、Naive Bayes、Logistic Regression、AdaBoost、XGBoost、SVM_R等传统机器学习算法；2. 探索了TF-IDF、Universal Sentence Encoder、RoBERTa等文本表示方法；3. 使用了Chi-Square、Mutual Information、PCA等特征提取技术；4. 将机器学习/深度学习方法与神经符号AI（NeSy）进行对比分析。

Result: 实验结果表明，神经符号AI（NeSy）方法在有限数据集上能够达到与多层感知器（MLP）相当的性能表现。

Conclusion: NeSy方法在文本分类任务中展现出潜力，未来研究将扩展知识库、嵌入类型范围和超参数配置，以进一步研究NeSy方法的有效性。
...
TL;DR: CLARiTy是一个基于视觉Transformer的模型，用于胸部X光的多标签分类和弱监督定位，通过类特定token和SegmentCAM模块提升小病变检测性能，在NIH数据集上达到SOTA定位效果。


<details>
  <summary>Details</summary>
Motivation: 胸部X光解读面临多标签病理分类和空间定位的挑战，通常受限于区域级标注的稀缺性。需要开发能够在仅有图像级标签的情况下同时完成分类和定位的弱监督方法。

Method: CLARiTy采用视觉Transformer架构，使用多个类特定token生成判别性注意力图，结合SegmentCAM模块利用解剖先验进行前景分割和背景抑制。通过ConvNeXtV2教师模型蒸馏提升效率，在NIH ChestX-ray14数据集上仅使用图像级标签训练。

Result: 在NIH数据集上，CLARiTy-S-16-512在14种病理分类上表现竞争性，在8种病理的弱监督定位上达到SOTA，比先前方法提升50.7%。小病变（如结节和肿块）提升尤其显著。低分辨率变体CLARiTy-S-16-224在保持高效率的同时超越基线。

Conclusion: CLARiTy通过结合ViT的全局上下文建模和卷积背景抑制，超越了CNN-ViT混合方法，能够生成精确、降噪的热力图，在低资源环境下具有应用潜力。
...
TL;DR: SPICE是一个Python框架，重新实现了三种流行的预测过程挖掘深度学习方法，提供可配置的基础框架以实现可复现和鲁棒的模型比较。


<details>
  <summary>Details</summary>
Motivation: 当前基于神经网络的预测过程挖掘方法缺乏可复现性、决策透明度、新数据集集成能力和基准测试，导致不同实现之间难以比较。

Method: 提出SPICE框架，在PyTorch中重新实现三种流行的深度学习方法，设计具有严格可配置性的通用基础框架，支持可复现和鲁棒的模型比较。

Result: 在11个数据集上将SPICE与原始报告指标进行公平比较，验证了框架的有效性。

Conclusion: SPICE框架解决了预测过程挖掘领域的可复现性和比较难题，为未来建模方法提供了标准化评估平台。
...
TL;DR: 提出结合字符级CNN与LightGBM的集成模型，用于钓鱼URL检测，在测试集上达到99.819%准确率，并通过FastAPI实现实时检测服务。
...
Method: 采用集成方法结合字符级CNN和LightGBM：1）提取URL的36个词汇、结构和域名特征；2）使用字符级CNN提取序列特征；3）集成两个模型（LightGBM贡献40%，CNN贡献60%）进行最终预测。

Result: 在19,873个URL的测试集上，集成模型达到：准确率99.819%、精确率100%、召回率99.635%、ROC-AUC 99.947%，性能优于单独模型，且保持极低的误报率。

Conclusion: 提出的集成方法能有效识别现代钓鱼技术，通过FastAPI服务实现实时检测，在保持低误报率的同时显著提升检测性能。
...
TL;DR: 提出级联架构解决高维空间中多谐样条的计算可扩展性问题


<details>
  <summary>Details</summary>
Motivation: 之前的多谐样条方法存在O(N^3)计算复杂度和高维输入空间理论假设失效的问题

Method: 构建由多谐样条包组成的级联架构，提出高效矩阵计算和前向传播方法

Result: 同时解决了计算可扩展性问题，并为未知内在低维度的机器学习回归问题提供了理论依据

Conclusion: 级联多谐样条架构是解决高维回归问题的有效方法，兼具理论合理性和计算效率
...
TL;DR: KOSS：基于卡尔曼最优选择性状态空间模型，通过卡尔曼增益实现上下文感知的选择机制，在序列建模任务中表现出色


<details>
  <summary>Details</summary>
Motivation: 现有的选择性状态空间模型（如Mamba和Mamba-2）虽然性能良好，但缺乏理论依据，且无法支持基于潜在状态动态的上下文感知选择

Method: 提出KOSS模型，将选择机制形式化为潜在状态不确定性最小化问题，采用卡尔曼增益动态调节信息传播，使用全局谱微分进行频域导数估计，以及分段扫描实现硬件高效处理

Result: 在选择性复制任务中达到79%以上准确率（基线低于20%）；在9个长期预测基准上MSE降低2.92-36.23%；在二次监视雷达跟踪案例中验证了在非规则间隔和噪声条件下的鲁棒性

Conclusion: KOSS通过理论驱动的卡尔曼最优选择机制，实现了上下文感知的序列建模，在多个任务中超越了现有最先进模型，并验证了其在实际应用中的有效性
...
TL;DR: 研究针对印尼朝觐服务数字化中的虚假应用诈骗问题，通过机器学习算法自动验证应用真实性，SVM在混合特征提取方法下达到92.3%准确率。
...
Result: SVM算法表现最佳，准确率达92.3%，精确度91.5%，F1分数92.0%。特征分析显示，与合法性相关的特定关键词和高风险权限（如READ PHONE STATE）是最重要的区分特征。
...
TL;DR: NRGPT：将GPT架构与基于能量的建模（EBM）结合，通过能量景观探索进行推理，在简单语言、代数任务和语言建模中表现良好，且具有更强的抗过拟合能力。


<details>
  <summary>Details</summary>
Motivation: GPT架构是语言建模的主流设计，而基于能量的建模（EBM）将推理视为在能量景观上的动态过程。本文旨在通过最小修改将GPT与EBM框架统一起来，结合两者的优势。

Method: 提出eNeRgy-GPT（NRGPT）模型，对GPT设置进行最小修改以融入EBM框架。将推理步骤概念化为在能量景观上对token的探索，在某些条件下这种探索可证明为梯度下降。

Result: 在简单语言（莎士比亚数据集）、代数ListOPS任务和更丰富的OpenWebText语言建模中表现良好。模型显示出更强的抗过拟合能力，仅在非常长的训练后才会出现过拟合。

Conclusion: 成功将GPT与EBM框架统一，提出的NRGPT模型在多种任务上表现优异，且具有更好的泛化能力和抗过拟合特性，为语言建模提供了新的视角。
...
TL;DR: 提出一种基于离散测度空间上向量场理论的几何框架，用于分析复杂系统的时空数据，通过两参数度量族解决传统方法难以处理高维非线性动态的问题。


<details>
  <summary>Details</summary>
Motivation: 复杂系统（如大脑、气候、经济等）具有高维非线性动态，传统降维、相空间重构等方法难以处理其庞大的时空数据，需要新的数据驱动分析框架。

Method: 基于离散测度空间上的向量场理论，提出两参数度量族，支持时间相关图像、图像梯度、图和单纯复形上的实值/向量值函数，结合多维标度进行验证。

Result: 该框架能有效实现降维、模态分解、相空间重构和吸引子表征，在生物和物理系统的数值模拟数据上验证了其有效性。

Conclusion: 为理解复杂动力系统提供了稳健途径，特别适用于传统建模不切实际但实验数据丰富的场景。
...
TL;DR: MEPIC是一种内存高效的位置无关缓存系统，通过分页对齐、块级重计算和RoPE融合等技术，实现跨位置、请求和批次的chunk KV重用，显著减少HBM内存使用。


<details>
  <summary>Details</summary>
Motivation: 现代LLM应用（如深度研究助手、编码代理和RAG系统）反复处理包含共享文档或代码块的长提示历史，对KV缓存造成巨大压力。现有前缀缓存受限于严格前缀匹配，位置无关缓存（PIC）虽然支持任意位置重用，但存在KV发散和内存布局不一致问题，导致HBM节省有限。

Method: 1) 将chunk KV对齐到分页存储；2) 将重计算从token级转移到block级，只有第一个block是请求特定的；3) 通过注意力内核中的RoPE融合移除位置编码；4) 使剩余block完全可共享。

Result: 相比最先进的PIC，在保持相当延迟和准确性的情况下，HBM使用减少高达2倍；对于长提示，减少高达5倍。无需任何模型更改。

Conclusion: MEPIC通过消除大部分重复的chunk KV，实现了跨位置、请求和批次的chunk KV重用，显著提高了内存效率，解决了LLM应用中KV缓存的内存压力问题。
...
TL;DR: TRC是一种基于递归迭代的神经控制架构，通过深度迭代而非参数数量来提升能力，在严格资源约束下实现高性能控制


<details>
  <summary>Details</summary>
Motivation: 神经网络控制器参数规模日益庞大（百万到数十亿参数），而嵌入式航空航天系统有严格的功耗和延迟约束，这种参数扩展是不可行的

Method: 提出Tiny Recursive Control (TRC)架构，基于反直觉原则：能力可以通过迭代深度而非参数数量产生。使用紧凑网络（约150万参数）通过两级分层潜在结构重复应用，通过模拟轨迹和基于跟踪误差校正来细化控制序列

Result: 在非线性控制问题（振荡器稳定和燃料约束的动力下降）上，TRC实现了接近最优的控制成本，GPU推理仅需毫秒级，内存小于10MB，比语言模型基线小两个数量级

Conclusion: 递归推理（之前仅限于离散任务）可以有效地转移到连续控制合成中，为资源受限系统的智能控制提供了新途径
...
TL;DR: LaMer是一个元强化学习框架，通过跨情节训练和上下文策略适应，让LLM智能体在测试时主动探索环境并从反馈中学习，显著提升了在需要探索的任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 传统RL训练的LLM智能体在需要主动探索的多轮长视野任务中表现不佳，难以从试错经验中高效适应。需要一种能让智能体在测试时主动探索并从环境反馈中学习的方法。

Method: LaMer包含两个核心组件：1) 跨情节训练框架，鼓励探索和长期奖励优化；2) 通过反思进行上下文策略适应，让智能体无需梯度更新就能从任务反馈信号中调整策略。

Result: 在多种环境中，LaMer相比RL基线显著提升性能：Sokoban提升11%，MineSweeper提升14%，Webshop提升19%。同时展现出对更具挑战性或未见任务更好的泛化能力。
...
TL;DR: 本文提出知识转换(KT)方法，结合知识蒸馏、主动学习和因果推理，解决在线边缘机器学习中未来未见数据的标签确定问题。


<details>
  <summary>Details</summary>
Motivation: 现有边缘机器学习方法通常假设静态模型在中心训练后部署，无法有效处理未见数据。在线边缘ML允许在边缘设备上直接训练并持续更新，但面临如何为未来未见数据确定标签的关键挑战。

Method: 提出知识转换(KT)混合方法，结合知识蒸馏、主动学习和因果推理。KT在主动学习中充当预言机，将教师模型的知识转换为伪标签来训练学生模型。

Result: 通过两种设置进行模拟实验：(1)使用不太稳定的教师模型；(2)使用相对更稳定的教师模型。结果表明，当给定稳定的教师模型时，学生模型最终能达到其预期最大性能。

Conclusion: KT在以下场景中具有潜在优势：(1)教师任务具有通用性，现有预训练模型可能足够，无需从头训练教师模型；(2)学生任务的标签难以获取或成本高昂。
...
<details>
  <summary>Details</summary>
Motivation: 持续学习系统需要在生命周期中增量获取、更新和利用知识，但灾难性遗忘（学习新任务导致旧任务性能急剧下降）是主要挑战。现有方法主要分为五类，但本文从任务排序这一不同角度切入，研究任务顺序对缓解灾难性遗忘的作用。

Method: 提出一种确定最优任务顺序的方法，利用受神经架构搜索（NAS）启发的零样本评分算法来评估任务序列，通过智能任务排序来减轻灾难性遗忘。

Result: 实验结果表明，智能任务排序能显著减少灾难性遗忘。当与传统持续学习策略结合时，排序方法能提供增强的性能和更强的抗遗忘鲁棒性。

Conclusion: 任务排序是缓解持续学习中灾难性遗忘的有效策略，该方法还可应用于课程学习等其他领域。智能任务排序与传统方法结合能进一步提升持续学习系统的性能。
...
TL;DR: 本文提出一种基于联邦学习（FL）的全球协作方法，用于诊断罕见疾病胶原VI相关肌营养不良症（COL6-RD），通过跨机构协作训练机器学习模型，在保护患者隐私的同时显著提升了诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 罕见疾病（如COL6-RD）的诊断面临数据稀缺和碎片化的挑战，跨医院、机构或国家的数据共享存在严重的隐私、监管和物流障碍，限制了机器学习在罕见疾病诊断中的应用。

Method: 使用Sherpa.ai联邦学习平台，在两个国际组织间建立全球联邦学习倡议，利用患者来源成纤维细胞培养物的胶原VI免疫荧光显微镜图像，在分布式数据集上进行协作模型训练，同时保持患者数据本地化和隐私保护。

Result: 开发的机器学习模型能够将胶原VI患者图像分类为COL6-RD相关的三种主要致病机制组：外显子跳跃、甘氨酸取代和假外显子插入。该模型F1分数达到0.82，显著优于单一机构模型（0.57-0.75）。

Conclusion: 联邦学习相比孤立机构模型显著提高了诊断效用和泛化能力。该方法不仅支持更准确的诊断，还将有助于解释意义未明的变异，并指导测序策略的优先排序以识别新的致病性变异。
...
TL;DR: 该研究分析了基于《纽约时报》语料库训练的分类器中"blacks"标签的偏见问题，发现该标签部分作为"种族主义检测器"运作，但在现代语境下表现不佳，揭示了新闻业采用AI工具时面临的历史偏见再现风险。


<details>
  <summary>Details</summary>
Motivation: 随着AI技术广泛应用于新闻研究和新闻室环境，这些基于历史数据训练的模型可能编码了数十年前的刻板印象和偏见。本研究旨在调查基于《纽约时报》语料库训练的分类器中"blacks"标签的运作机制和潜在偏见影响。

Method: 通过定量和定性方法分析训练语料库中"blacks"标签的使用情况，应用可解释AI技术探究该标签在分类器中编码的概念，并测试其在现代语境下的表现（如COVID-19时期反亚裔仇恨报道和"黑人的命也是命"运动报道）。

Result: 研究发现"blacks"标签部分作为针对某些少数族裔的"种族主义检测器"运作，但在现代示例中表现不佳。该案例揭示了模型嵌入的偏见如何影响新闻室中AI工具的各种潜在应用，如故事发现、受众定位和摘要生成。

Conclusion: 新闻业在采用AI工作流工具时面临根本性张力：如何在利用AI技术优势的同时，降低在新闻报道中复制历史偏见的风险。这要求对AI模型进行批判性审视和偏见检测。
...
TL;DR: 提出PostBC方法，通过建模演示数据的后验分布而非直接模仿动作，为RL微调提供更好的初始化策略，显著提升机器人控制性能


<details>
  <summary>Details</summary>
Motivation: 当前实践中，先在大规模演示数据上预训练策略，然后通过RL微调提升性能。然而，现有研究主要关注微调算法，很少关注如何确保预训练策略能成为RL微调的有效初始化。标准行为克隆(BC)可能无法覆盖演示者的所有动作，影响后续RL微调效果

Method: 提出后验行为克隆(PostBC)方法：不直接拟合观察到的演示动作，而是训练策略来建模演示者行为在演示数据集条件下的后验分布。这种方法确保覆盖演示者的所有动作，同时保持与BC相当的预训练性能。PostBC仅需标准监督学习，可与现代生成模型结合实现

Result: 理论证明：标准BC可能无法确保覆盖演示者动作，而PostBC能保证覆盖。在实际机器人控制基准测试和真实世界机器人操作任务中，PostBC相比标准BC能显著提升RL微调性能

Conclusion: PostBC方法通过建模演示数据的后验分布，为RL微调提供了更有效的策略初始化，在保持预训练性能的同时显著提升微调效果，为从演示到RL微调的流程提供了改进方案
...
TL;DR: RLVR框架中，虚假奖励通过裁剪偏差降低策略熵，使模型输出更自信确定，从而提升LLM推理性能，而单纯的熵最小化不足以带来改进。


<details>
  <summary>Details</summary>
Motivation: 研究RLVR框架中探索-利用权衡的机制，特别是虚假奖励和熵最小化这两个看似矛盾的现象如何都能提升LLM推理性能，但背后的原理尚不清楚。

Method: 分析策略熵与性能的关系，研究虚假奖励是否通过裁剪偏差和模型污染相互作用产生收益，提出奖励错配模型来解释虚假奖励的增强效果。

Result: 虚假奖励通过裁剪偏差降低策略熵，使输出更自信确定，而单纯熵最小化不足以提升性能；虚假奖励在非污染设置下也能增强性能。

Conclusion: 阐明了虚假奖励提升性能的机制，为更有效的RLVR训练提供了原则，解释了探索和利用抑制都能改善推理性能的悖论。
...
TL;DR: 提出BayesSum估计器，将贝叶斯求积扩展到离散域，利用高斯过程先验信息提高样本效率，在多种设置下比蒙特卡洛方法收敛更快。
...
Method: 提出BayesSum估计器，将贝叶斯求积扩展到离散域，通过高斯过程利用被积函数的先验信息来提高样本效率。

Result: 理论推导显示在广泛设置下收敛速度显著快于蒙特卡洛方法。实证表明在多个合成设置以及Conway-Maxwell-Poisson和Potts模型的参数估计中确实需要更少样本。

Conclusion: BayesSum是一种样本效率更高的离散域期望估计方法，能够有效利用先验信息，在理论和实证上都优于现有方法。
...
TL;DR: ZICO是一种从零膨胀计数数据学习网络结构的方法，将每个节点建模为零膨胀广义线性模型，在DAG约束下优化平滑的基于分数的目标函数。


<details>
  <summary>Details</summary>
Motivation: 需要从零膨胀计数数据中学习网络结构，特别是在基因调控网络重建等生物信息学应用中，现有方法在处理零膨胀数据和计算效率方面存在不足。

Method: 将每个节点建模为零膨胀广义线性模型，使用节点似然函数和规范链接，通过可微分的代理约束结合稀疏正则化来强制无环图约束，实现完全向量化和小批量处理。

Result: 在模拟数据上，ZICO实现了优越的性能和更快的运行时间；在基因调控网络重建中，性能与常见算法相当或更好；能够处理更大变量集并在广泛领域中实现实用运行时间。

Conclusion: ZICO是一种高效、可扩展的零膨胀计数数据网络结构学习方法，在计算效率和性能方面优于现有方法，适用于基因调控网络重建等多种应用领域。
...
TL;DR: TL-TARNet通过迁移学习改进个体治疗效果估计，在小型目标数据集上减少误差和偏差


<details>
  <summary>Details</summary>
Motivation: 因果知识在不同环境间的泛化具有挑战性，特别是在大规模数据集估计需要应用于较小或系统性不同环境时。机器学习中的个体治疗效果估计器需要大样本量，限制了在行为科学等小数据集领域的应用。

Method: 提出TL-TARNet（迁移学习-TARNet），通过利用源数据集知识并采用迁移学习适应新环境，改进TARNet的个体治疗效果估计。在模拟实验中考虑源数据集大小、样本大小以及随机和非随机干预设置。

Result: 当存在大型无偏源数据集且目标样本较小时，TL-TARNet相比标准TARNet减少了ITE误差并减弱了偏差。在印度人类发展调查的实证应用中，迁移学习将目标平均ITE估计拉向源ITE估计，减少了无迁移时的估计偏差。

Conclusion: 因果模型的迁移学习可以改进小样本中的个体治疗效果估计，特别是在有大型无偏源数据集可用时。
...
TL;DR: 提出一种用于生成玻璃态材料平衡构型的等变黎曼随机插值框架，结合黎曼随机插值和等变流匹配，显著提升生成性能


<details>
  <summary>Details</summary>
Motivation: 玻璃态材料（无序粒子系统）的平衡构型采样非常缓慢困难，需要开发能够生成具有明确似然平衡构型的生成框架来克服这一障碍

Method: 使用等变黎曼随机插值框架，结合黎曼随机插值和等变流匹配，采用等变图神经网络直接在环面上操作，严格融入周期性边界条件和多组分粒子系统的对称性

Result: 在模型非晶系统上的数值实验表明，强制几何和对称性约束显著改善了生成性能

Conclusion: 该方法成功解决了玻璃态材料平衡构型采样的挑战，为无序粒子系统的生成建模提供了有效解决方案
...
TL;DR: 经验流匹配（FM）采样器存在隐式偏差：经验FM最小化器几乎从不产生梯度场，导致能量次优，且生成样本的动能分布主要由源分布而非数据决定。


<details>
  <summary>Details</summary>
Motivation: 研究流匹配（FM）采样器的隐式偏差，特别是经验FM与理论最优传输（OT）之间的差异。虽然总体FM可能产生类似OT的梯度场速度，但经验FM在实践中存在结构性和能量性偏差。

Method: 通过经验流匹配的视角分析FM采样器，研究经验FM最小化器的数学特性，分析生成样本的动能分布，包括瞬时动能和积分动能，并考虑不同源分布（高斯分布和重尾分布）的影响。

Result: 经验FM最小化器几乎从不产生梯度场，导致能量次优。对于高斯源分布，瞬时和积分动能都呈现指数集中；对于重尾源分布，则呈现多项式尾部。这些行为主要由源分布选择决定，而非数据本身。

Conclusion: 经验FM存在固有的结构性和能量性偏差，这些偏差主要由源分布特性决定而非目标数据。这为理解FM采样器的实际性能提供了数学基础，并揭示了源分布选择对采样质量的重要影响。
