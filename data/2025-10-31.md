...
Method: 将RSSI分解为物理可解释的路径损耗和阴影衰落组件，采用轻量级MobileNet模型进行预测，降低学习难度并增强可解释性。

Result: 在常规条件下将预测RMSE降低了50.3%，在消除干扰的情况下仍比先前基准低11.5%的RMSE，模型尺寸比竞争方案小19倍。
...
<details>
  <summary>Details</summary>
Motivation: 在5G网络中，终端位置通过基站发送的定位参考信号的到达时间确定，但现有方法在低采样率条件下精度有限，需要改进TOA估计准确性。

Method: 提出改进TOA精度的算法，在软件定义调制解调器中实现5G PRS定位，并研究PRS灵活时频资源分配对TOA估计精度的影响。

Result: 开发了在低采样率约束下提高TOA精度的算法，实现了5G PRS定位系统，并确定了不同信号环境下的最优PRS配置。

Conclusion: 该研究为5G定位技术提供了改进的TOA估计算法和最优PRS配置策略，有助于提升5G网络定位性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有电弧故障诊断方法在资源受限条件下难以同时实现快速响应和鲁棒精度，需要轻量级在线检测方案来预防电气火灾

Method: 基于电弧电流曲线和傅里叶分解分析，设计自适应激活函数EAS和分支网络架构的MFNN模型，从多频信号中提取特征

Result: MFNN在电弧故障定位和噪声免疫性方面优于8个先进模型，在SNR=-9时测试精度比LCNN高14.51%，比BLS高16.3%

Conclusion: EAS激活函数和网络架构设计显著提升了MFNN性能，为资源受限环境下的电弧故障检测提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 逆变器资源的普及给传统微电网保护带来挑战，因为其引入可变的故障电流和复杂瞬态过程，需要开发更鲁棒的保护方案。

Method: 采用多元故障检测引擎，基于Mahalanobis距离检测故障状态，检测阈值从卡方分布统计推导，故障类型通过轻量级分类器识别，并使用时序持续性滤波器增强安全性。

Result: 在修改的CIGRE 14总线微电网上的广泛仿真显示：亚周期平均检测延迟、高检测和分类精度、对高达250欧姆高阻抗故障的韧性、10ms通信延迟容忍度、20dB信噪比下的噪声容忍度。

Conclusion: 该方法为下一代交流微电网保护提供了一个可重现且计算高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 5G NR中基于互易性的波束成形面临资源和覆盖限制，稀疏非均匀SRS分配导致信道状态信息不完整。现有MAE方法对训练掩码过拟合，在未见失真条件下性能下降。

Method: 使用基于分数的扩散模型进行信道修复，在推理时通过似然梯度项整合系统模型知识，使单个训练模型能够适应不匹配条件。

Result: 在标准化CDL信道上，基于分数的扩散变体在分布偏移条件下始终优于UNet基线，在挑战性场景下NMSE提升达14 dB，同时在匹配条件下保持竞争力。

Conclusion: 扩散引导的修复是5G NR系统中超容量SRS设计的鲁棒且可泛化方法，能够适应各种失真条件。
...
<details>
  <summary>Details</summary>
Motivation: 在IRS辅助的IoT网络中，异构用户设备的计算能力差异和通信资源限制会影响分割学习的效率，需要设计协同优化方案来降低训练延迟。

Method: 采用交替优化方法，结合闭式更新、半定松弛和低复杂度启发式算法，联合优化层分割点、DT分配决策、IRS相位矩阵、AP下行功率分配和DT频率调整。

Result: 仿真结果表明，所提方案相比传统基线显著降低训练延迟，在高用户密度和严格功率约束下可实现高达35%的延迟改进。

Conclusion: 该计算通信协同设计框架有效解决了IRS辅助DT-IoT网络中分割学习的延迟优化问题，为大规模分布式学习提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统2D/3D通道知识图方法假设基站配置固定，无法适应动态的发射器和接收器位置变化，需要开发能够建模六维空间的通道知识图框架。

Method: 使用高斯椭球体表示虚拟散射簇和环境障碍物，通过学习双向散射模式和复杂衰减特性来捕获无线环境的电磁传输特性，从而准确建模不同收发器配置下的信号传输。

Result: BiWGS在构建6D通道功率增益图方面显著优于经典多层感知器，在3D通道知识图构建方面达到与最先进的无线辐射场高斯溅射相当的精度。

Conclusion: 所提出的BiWGS能够在不牺牲保真度的情况下实现6D通道知识图的维度扩展，验证了其在动态收发器配置下建模无线信道的能力。
...
<details>
  <summary>Details</summary>
Motivation: 在存在遮挡的近场通信环境中，需要开发能够有效聚焦能量的物理一致方法，以克服信号传播障碍。

Method: 使用基于刀口衍射的物理模型，将问题表述为希尔伯特空间中的最大化问题，通过匹配衍射诱导核的形状获得最优解。

Result: 得到了一个匹配滤波器形式的优化解，该解能够匹配衍射核的形状，有效连接波传播与信号处理方法。

Conclusion: 该方法为近场遮挡信道中的高效能量聚焦提供了物理建模、信号处理和硬件设计的桥梁，支持使用超表面或透镜天线等连续孔径的硬件实现。
...
<details>
  <summary>Details</summary>
Motivation: 传统高分辨率技术需要多天线阵列和大量快照采集，而通用机器学习方法缺乏物理可解释性，需要开发既准确又可解释的到达角估计方法。

Method: 采用约束符号回归框架，从路径损耗测量中自动发现闭式波束模式和到达角模型，在自由空间和RIS辅助室内测试平台进行验证。

Result: 在自由空间环境中实现亚0.5度平均绝对误差，在RIS辅助室内环境中实现接近零误差的到达角估计，性能接近克拉美-罗下界。

Conclusion: SABER是到达角估计中可解释且准确的替代方案，弥合了不透明机器学习方法与可解释物理驱动估计器之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 传统HMM训练依赖单个长观测序列，但现实中常遇到多个独立短时间序列数据，如人口健康纵向研究中的个体随访数据

Method: 开发了针对多独立短时间序列的HMM训练和解码算法伪代码，基于EM算法的Baum-Welch算法变体

Result: 提供了适用于多短序列场景的HMM训练和解码方法，能够处理人口健康研究中常见的个体随访数据结构

Conclusion: 该方法扩展了HMM在短时间序列数据中的应用，特别适用于人口健康纵向研究等需要处理多个体短轨迹数据的领域
...
<details>
  <summary>Details</summary>
Motivation: 脑电图信号存在显著的个体间变异性，容易导致信号失真或裁剪。模数采样通过折叠信号而非饱和来解决此问题，但从折叠观测中恢复原始波形是一个高度不适定问题。

Method: 将EEG信号表示为有组织的图结构，利用通道和时间连接建立底层相互依赖关系。引入预估计引导的特征注入模块提供粗略折叠指示器，增强在折叠边界恢复时的稳定性。

Result: 在STEW数据集上的实验表明，相比传统优化技术有持续改进，与当前深度学习模型相比具有竞争力的准确性。

Conclusion: 研究结果强调了基于图的方法在稳健模数EEG恢复中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: KANs作为MLPs的有前景替代方案，具有更好的表达能力和可解释性。本文旨在填补KANs研究领域的空白，为研究者和实践者提供全面的技术概览和实用指导。

Method: 通过收集和分类大量开源实现，建立KANs与MLPs的形式等价性，分析不同基函数（B样条、切比雪夫多项式、ReLU组合等）的权衡，并构建改进KANs准确性和效率的技术路线图。

Result: 创建了系统的KANs分类框架，涵盖了物理信息损失设计、自适应采样、领域分解等关键技术，并提供了实用的'选择你的KAN'指南。

Conclusion: KANs在参数效率和表达能力方面优于传统MLPs，本文为KANs研究提供了结构化参考，并指出了当前研究空白，相关GitHub仓库可作为持续研究的资源。
...
<details>
  <summary>Details</summary>
Motivation: 可穿戴传感器提供丰富的生理时间序列数据，但其预测效用的基本原则尚不清楚。作者假设时间分辨率是表示学习的基本轴，不同的临床和行为结果依赖于不同时间尺度的结构。

Method: 提出HiMAE（分层掩码自编码器），这是一个自监督框架，将掩码自编码与分层卷积编码器-解码器相结合，生成多分辨率嵌入。
...
Method: 开发了允许时间不变模型偏差的OTA-FL SGD更新方法，提出了基于统计CSI的非凸联合功率控制设计，并采用连续凸逼近(SCA)算法进行优化。

Result: 在非凸图像分类任务上的实验表明，基于SCA的设计通过优化偏差加速了收敛，并比先前的OTA-FL基线提高了泛化性能。

Conclusion: 该研究为异构无线条件下的OTA-FL提供了有效的偏差-方差权衡优化方法，特别适用于非凸目标函数场景。
...
<details>
  <summary>Details</summary>
Motivation: 高能材料的发现受限于实验数据和测试设施访问，需要新的计算方法来加速发现过程。

Method: 使用LSTM网络进行分子生成，注意力图神经网络进行性质预测，提出固定SHA-256嵌入与部分可训练表示结合的嵌入空间构建策略。

Result: 生成器达到67.5%有效性和37.5%新颖性，生成分子库与训练集的平均Tanimoto系数为0.214，发现37种预测爆速超过9km/s的新型超级炸药。

Conclusion: 该方法能够生成多样化化学空间的高能材料分子，无需预训练即可发现新型高性能炸药。
...
<details>
  <summary>Details</summary>
Motivation: 理解思维链监督如何让模型学会遵循并从中受益的机制，目前这方面的研究还很缺乏。

Method: 在符号推理任务上预训练Transformer，通过可调的算法复杂度和可控的数据组成来研究泛化能力，比较仅输出最终答案和输出CoT推理轨迹两种训练设置。

Result: CoT通常能提升任务性能，但其效果取决于任务复杂度；发现训练早期存在推理不忠实阶段，模型能给出正确答案但跳过或违背推理步骤；CoT会改变Transformer的内部计算机制。

Conclusion: CoT监督加速了泛化过程，但不能解决高算法复杂度任务；推理忠实性是随着训练动态出现的特性；提出了理解Transformer学习的动力学建模框架。
...
<details>
  <summary>Details</summary>
Motivation: 现代多智能体系统在环境监测、金融预测和众包平台中面临系统性偏差问题，现有方法要么忽略偏差导致次优决策，要么需要昂贵的校准程序。

Method: 开发了ABLOC算法，通过迭代学习偏差校正变换并优化组合权重，使用闭式解保证收敛到理论边界。

Result: 实验验证显示，高可学习性比率的系统能恢复显著性能（达到理论最大改进的40%-70%），而低可学习性系统收益甚微。

Conclusion: 可学习性比率决定了偏差学习的价值，为系统设计者提供了何时投资偏差学习与采用简单方法的定量指导。
...
<details>
  <summary>Details</summary>
Motivation: 拼车服务虽然能降低成本、缓解拥堵和环境影响，但其决策过程通常是短视的，忽视了调度决策的长期影响，这限制了系统的整体性能。

Method: 将Xu等人(2018)的学习和规划框架从网约车扩展到拼车系统，在强化学习机制中嵌入拼车模拟来实现非短视决策，并提出了空闲车辆再平衡策略，使用n步时序差分学习从模拟经验中学习时空状态值。

Result: 非短视匹配策略相比短视策略可将服务率提高8.4%，同时减少乘客车内和等待时间；相比短视策略可减少超过25%的车队规模而保持相同性能水平；加入再平衡操作后等待时间减少27.3%，车内时间减少12.5%，服务率提高15.1%。

Conclusion: 所提出的模拟强化学习方法能有效解决拼车系统中的短视决策问题，显著提升系统性能并降低运营成本，为运营商提供重要的成本节约。
...
<details>
  <summary>Details</summary>
Motivation: 现有知识编辑技术通常只关注单一模态(视觉或语言)，忽略了LVLMs的多模态特性和知识更新的连续性，导致编辑效果不佳。需要解决跨模态交互和持续知识细化的需求。

Method: 采用混合外部-内部编辑器，包含双外部存储器用于跨模态证据检索，双LoRA适配器实现各模态解耦参数更新，以及受大脑启发的知识连接器用于组合推理。

Result: 实验表明MemEIC在复杂多模态问题上显著提升性能，有效保留先前编辑内容，为LVLMs的持续组合知识编辑设立了新基准。

Conclusion: MemEIC成功解决了LVLMs中多模态知识编辑的挑战，通过组合编辑方法实现了更好的跨模态知识整合和持续更新能力。
...
<details>
  <summary>Details</summary>
Motivation: 时间序列数据的自相关性给多变量时间序列预测带来挑战，现有研究观察到神经网络中的频谱偏差现象，但常将其归因于特定架构而非普遍特性。

Method: 通过大量实证实验测量主流模型的频谱偏差，并提出FreLE算法，通过显式和隐式频率正则化增强模型泛化能力。

Result: 实验证明几乎所有模型都存在频谱偏差现象，FreLE算法在大量实验中表现出优越性能。

Conclusion: 频谱偏差是时间序列预测模型的普遍特性，FreLE算法能有效缓解此问题，提升模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 在多领域环境中，因果效应可能因领域而异，且存在未观测混杂因子干扰估计。需要开发能在仅观测代理变量的目标领域中准确估计因果效应的方法。

Method: 假设所有变量为离散或分类变量，利用混杂因子的代理变量，提出两种估计技术，证明可识别性（即使治疗和响应变量是连续的），并推导置信区间。

Result: 理论结果得到模拟研究和真实世界案例的支持，研究了网站排名对消费者选择的因果效应。

Conclusion: 该方法能够在仅观测代理变量的目标领域中有效估计因果效应，为多领域因果推断提供了可行的解决方案。
...
Method: SPECS框架：1) 通过自蒸馏生成内省偏好数据对；2) 进行偏好训练学习浅层可转移的表面形式标准；3) 移交到具有可验证奖励的RL进行深度推理

Result: 在多个多模态基准上表现优于强基线，MEGA-Bench提升4.1%，MathVista提升12.2%，减少分布内"卡顿"，改善探索，稳定训练，提高性能上限

Conclusion: 解耦多模态学习的SPECS框架通过偏好训练冷启动优于传统SFT方法，为RL提供了更好的初始化策略
...
<details>
  <summary>Details</summary>
Motivation: 尽管InfoNCE被广泛用于互信息估计，但其与互信息的间接联系存在问题，需要开发更准确的估计方法。

Method: 引入辅助锚类进行一致的密度比估计，提出InfoNCE-anchor方法，并使用适当评分规则统一多种对比目标。
...
<details>
  <summary>Details</summary>
Motivation: Adam是深度学习中最常用的优化器，但其理论理解仍然有限。先前分析表明Adam偏向ℓ∞几何解，但这些结果仅限于全批次情况。本研究旨在分析增量Adam在不同批次大小下的隐式偏差。

Method: 使用逻辑回归在线性可分数据上研究增量Adam的隐式偏差，构建结构化数据集证明其收敛行为，开发代理算法捕捉增量Adam的极限行为，并与Signum优化器进行比较分析。

Result: 增量Adam在结构化数据集上收敛到ℓ₂最大间隔分类器，与全批次Adam的ℓ∞最大间隔偏差不同。Signum在任何批次大小下都收敛到ℓ∞最大间隔分类器。
...
<details>
  <summary>Details</summary>
Motivation: 黑盒LLM指令优化中，白盒LLM常将不同软提示映射到相同指令，导致冗余查询。传统方法视此为效率障碍，而本文将其重新解释为可加速优化的先验知识。

Method: PRESTO包含三个核心组件：评分共享（在预映射内共享评分）、基于预映射的初始化（最大化搜索空间覆盖）、评分一致性正则化（确保预映射内预测一致性）。

Result: 在33个指令优化任务上的实验结果表明，PRESTO具有优越性能，在相同查询预算下有效获得14倍评分数据。

Conclusion: 通过利用软提示的预映射结构，PRESTO能够显著提高黑盒LLM指令优化的效率，证明了将多对一映射重新解释为有用先验知识的有效性。
...
TL;DR: 该论文提出了一种结合扩散模型和退火朗之万动力学的条件采样方法，能够在多项式时间内从后验分布中采样，仅需对分数误差施加L^4约束。


<details>
  <summary>Details</summary>
Motivation: 后验采样在图像修复、去模糊和MRI重建等任务中具有重要应用，但近似后验采样在一般情况下是计算难解的。现有方法如朗之万动力学对分数估计误差敏感，需要更强的误差约束。

Method: 针对对数凹分布，将扩散模型与退火朗之万动力学相结合，利用扩散模型在无条件设置下仅需L^2分数误差约束的优势。

Result: 证明了该方法能够在多项式时间内实现条件采样，仅需对分数误差施加L^4约束，相比传统朗之万动力学所需的次指数误差约束更为宽松。

Conclusion: 该方法为后验采样提供了一种计算可行的解决方案，放宽了对分数估计精度的要求，在图像处理等应用中具有实际意义。
...
Method: 提出Neighborhood Patch Attention减少自注意力层计算冗余，结合SDEdit管道和Latent Frequency Mixing生成细节，使用Structure Guidance增强全局结构。

Result: ScaleDiff在图像质量和推理速度方面在免训练方法中达到最先进水平，兼容U-Net和Diffusion Transformer架构。
...
Method: Convert datasets into interpretable metadata and prompt LLMs with two strategies: zero-shot mode using pretrained knowledge, and meta-informed mode augmented with performance examples from past tasks.

Result: LLMs can recommend competitive models and hyperparameters without search across synthetic and real-world benchmarks, with meta-informed prompting showing improved performance.

Conclusion: LLMs can serve as lightweight, general-purpose assistants for model selection and hyperparameter optimization through in-context meta-learning.
...
<details>
  <summary>Details</summary>
Motivation: 训练通用医学VQA系统面临高质量开放数据集的缺乏，现有数据集规模小、质量参差不齐，限制了医学LMM的发展。

Method: 采用规则引导的生成器-验证器框架：生成器根据图像、标题和文本引用生成多选题，验证器通过多阶段验证确保问题质量，包括自包含性、单一正确答案、临床有效性和图文一致性。

Result: 在PubMed Central上应用该流程创建了MedSynVQA数据集，包含13,087个问题覆盖14,803张图像。使用该数据训练的LMM在6个医学VQA基准测试中表现优异，3B模型平均55.85分，7B模型平均58.15分。

Conclusion: MedVLSynther提供了一条可审计、可复现且保护隐私的路径，能够规模化生成医学VQA训练数据，显著提升医学LMM的性能。
...
Method: 通过新颖的实验设计，量化由捷径诱导偏斜导致的准确率退化的层间贡献，通过对比在干净和偏斜数据集上的反事实训练来实现。在CIFAR-10、Waterbirds和CelebA数据集上，使用VGG、ResNet、DeiT和ConvNeXt架构进行研究。

Result: 发现捷径学习不是局部化的，而是分布在整个网络中。浅层主要编码虚假特征，深层主要遗忘核心特征。分析了局部化的差异并描述了其主要变化轴。
...
<details>
  <summary>Details</summary>
Motivation: 虽然最近的研究探索使用强化学习来自动化监督微调的数据收集过程，但由于迭代去噪导致的动作对数似然计算困难，将大规模RL应用于基于流的VLA模型仍然具有挑战性。

Method: π_RL实现了两种RL算法：(1) Flow-Noise将去噪过程建模为离散时间MDP，使用可学习的噪声网络进行精确的对数似然计算；(2) Flow-SDE将去噪与智能体-环境交互集成，采用ODE-to-SDE转换进行高效RL探索。

Result: 在LIBERO基准上，π_RL将π_0和π_0.5模型的性能分别从57.6%提升到97.6%和从77.1%提升到98.3%。在ManiSkill基准上，在320个并行环境中训练，将π_0从41.6%提升到85.7%，π_0.5从40.0%提升到84.8%。

Conclusion: π_RL实现了显著的性能提升和更强的泛化能力，验证了在线强化学习对于基于流的VLA模型的有效性。
...
Method: 为两阶段和单阶段多专家延迟设置提出新算法，选择性查询训练样本的专家子集，建立理论保证包括泛化边界和标签复杂度分析
...
<details>
  <summary>Details</summary>
Motivation: 解决在标签预算稀缺情况下主动学习中探索与利用的核心挑战，避免传统启发式方法的局限性。

Method: 1. 基于平衡福尔曼曲率的核集构建算法选择初始标签；2. 数据驱动的停止准则判断图探索充分性；3. 动态触发探索到利用的转换；4. 局部图重连策略整合多尺度信息。
...
<details>
  <summary>Details</summary>
Motivation: 传统序列建模将参数化记忆抽象为实体共现的暴力查找，但作者发现存在与这种关联视图不兼容的Transformer推理实例，表明模型必须通过某种方式合成了原子事实的几何结构。

Method: 通过分析Transformer推理实例，并与Node2Vec建立联系，研究几何结构的形成机制，特别是光谱偏差的自然产生。

Result: 发现Transformer会自发学习优雅的几何结构，即使这种结构在简洁性上不优于关联查找，并且这种几何源于自然产生的光谱偏差。

Conclusion: 几何视角的参数化记忆鼓励重新审视知识获取、容量、发现和遗忘等领域的默认直觉，并为使Transformer记忆更具几何性指出了可见的改进空间。
...
Method: 提出两种任务驱动表示学习策略：1）直接学习半监督表示；2）在初始无监督表示基础上进行监督微调，并在主动学习过程中定期更新表示。

Result: 两种策略都显著优于使用无监督或预训练表示的方法，在混乱数据池中表现出更好的性能。

Conclusion: 任务驱动的动态表示学习是解决主动学习中混乱数据池问题的有效方法，能够通过利用已收集标签不断优化表示质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有的GNN水印方法依赖后门触发器，容易受到常见模型编辑的影响并产生所有权模糊问题，需要更鲁棒的水印方案。

Method: 使用轻量级头预测所有者私有载体集的归一化代数连通性，通过符号敏感解码器输出比特，校准阈值控制误报率。

Result: 在多种节点和图分类数据集及骨干网络上，InvGNN-WM保持清洁精度，同时比基于触发器和压缩的基线方法获得更高的水印精度，对剪枝、微调和量化保持鲁棒性。

Conclusion: 该方法提供了不可感知性和鲁棒性保证，证明精确移除是NP完全问题，知识蒸馏会削弱水印但结合水印损失可恢复。
...
<details>
  <summary>Details</summary>
Motivation: 传统哈希或one-hot编码方法无法保持双射映射关系，MLT旨在提供一种可逆、可控维度的编码方案，同时保持计算可扩展性。

Method: 基于有限域上的模运算和可逆线性变换，实现分类标识符到数值向量的可逆编码，可显式控制维度。

Result: 在MovieLens 20M数据集上，MLT实现了与监督嵌入相当的预测性能，但参数数量和训练成本显著降低。

Conclusion: MLT提供了一种高效、可逆的分类标识符编码方法，在保持性能的同时大幅减少了计算资源需求。
...
Method: 使用XGBoost模型，基于552个健康服务区的数据（2021年1月-2023年5月），评估Google人口动态基础模型、Google AlphaEarth和移动电话详单记录三种GeoFM嵌入源的预测性能，并与传统地理插值方法比较。

Result: 基于嵌入的方法在15个指标中的13个（87%）优于基线地理统计方法。多源GeoFM模型表现最稳健，在人口密度、新HIV病例和儿童疫苗接种等指标上取得良好预测效果，但对结核病和营养不良等数据稀缺目标的预测较差。
...
<details>
  <summary>Details</summary>
Motivation: 该研究的动机源于两个关键因素：(i) 数据驱动学习方法在某些领域（如组合优化）仍不成熟且结果有限；(ii) 训练数据可用性受限，如医学图像重建等科学应用。

Method: 定义了无数据设置，并将其分为两种变体：架构无关方法和架构特定方法，基于如何将问题实例编码到神经网络中。

Result: 尽管多层感知机几十年前已用于解决线性规划问题，但该方法最近因在组合优化、逆问题和偏微分方程等多样化应用中展现出有前景的结果而受到关注。
...
<details>
  <summary>Details</summary>
Motivation: 由于求解器专业化和接口不匹配，在约束优化问题中进行微分在机器学习、控制和决策系统中仍然具有挑战性，需要更实用的集成方案。

Method: 通过微分KKT系统计算前向和反向模式解和目标敏感性，提供参数中心的JuMP原生API，允许用户声明命名参数并直接获取导数。

Result: 框架在凸和非凸模型上展示了能力，包括经济调度、投资组合优化和机器人逆运动学。两个配套研究展示了在能源市场战略投标和端到端优化代理训练中的大规模应用。

Conclusion: 可微分优化可以作为实验、学习、校准和设计的常规工具部署，无需偏离标准JuMP建模实践，同时保留广泛的求解器生态系统访问权限。
...
<details>
  <summary>Details</summary>
Motivation: 边缘机器人系统需要高效的在线学习算法来持续适应变化的环境。传统反向传播不符合生物合理性原则，而预测编码框架虽然生物合理但计算开销大。

Method: PCN-TA通过利用时间相关性，在时间帧之间保留潜在状态，减少了训练期间所需的推理迭代次数。

Result: 在COIL-20机器人感知数据集上，PCN-TA比反向传播减少10%的权重更新，比基线PC网络减少50%的推理步骤。

Conclusion: PCN-TA在保持学习性能的同时显著降低了计算开销，为边缘部署和资源受限机器人系统的实时适应提供了支持，是未来神经形态硬件实现的有前景候选方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在两极分化：完全自适应探索方法（如UCB、Thompson Sampling）需要持续探索，在安全关键或成本高昂的领域不实用；而纯贪婪方法需要严格的上下文多样性假设才能成功。需要一种折中方案。

Method: INFEX框架按照预定调度执行基础探索策略，在探索间隔期间主要选择贪婪动作。这是一个通用、模块化的框架，可以无缝集成任何完全自适应探索方法。

Result: 理论分析表明，只要探索频率超过对数阈值，INFEX就能实现与标准可证明高效算法相当的实例依赖遗憾。实验评估证实了理论发现，显示出最先进的遗憾性能和运行时间改进。

Conclusion: INFEX是一个简单实用的框架，通过限制密集探索计算到不频繁的间隔，在保持理论保证的同时提高了计算效率，适用于安全关键和成本敏感的应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 生存分析需要同时建模患者异质性和时间动态，现有方法难以灵活整合这两方面。

Method: 使用双混合专家框架：特征编码MoE用于子群感知表示学习，风险MoE利用患者特征和时间嵌入捕获时间动态。
...
TL;DR: 知识蒸馏会削弱模型的去偏能力，本文研究了KD对去偏能力迁移的影响，提出了三种改进方案：高质量数据增强、迭代知识蒸馏和教师模型权重初始化。


<details>
  <summary>Details</summary>
Motivation: 探索知识蒸馏对模型去偏能力的影响，目前该领域研究不足，特别是在处理分布外数据时的鲁棒性方面。

Method: 在自然语言推理和图像分类任务上进行了广泛实验，分析了KD后模型内部注意力模式和电路的变化。

Result: 发现KD会削弱模型的去偏能力，去偏模型从教师知识注入中获益有限，不同偏见的鲁棒性变化显著。

Conclusion: 提出了三种有效解决方案来改进去偏方法的可蒸馏性，为理解KD工作机制和设计更好的去偏方法提供了见解。
...
<details>
  <summary>Details</summary>
Motivation: 符号回归旨在从观测数据中发现数学表达式，但深度学习方法的缩放规律尚未被充分探索。受语言模型缩放定律启发，本研究系统性地探究符号回归中的缩放行为。

Method: 使用可扩展的端到端transformer流水线和精心生成的训练数据，在五个不同模型大小和三个数量级的计算量范围内进行实验。

Result: 发现验证损失和求解率都遵循计算量的幂律趋势，确定了计算最优的超参数缩放：最优批大小和学习率随模型大小增长，token与参数比约为15时最优。

Conclusion: 符号回归性能可以从计算量中预测，这为训练下一代符号回归模型提供了重要见解。
...
Method: 构建变分框架，损失函数平衡数据保真度和流形内在几何复杂度；将连续流形离散化为三角网格，用边长度参数化度量张量，利用自动微分工具进行高效优化。
...
TL;DR: 这篇系统综述分析了72项关于合成金融数据生成的研究，发现GAN方法在生成时间序列市场数据和表格信用数据方面占主导地位，但隐私保护评估普遍不足。


<details>
  <summary>Details</summary>
Motivation: 合成数据生成可以解决在机器学习应用中使用敏感金融数据时的隐私风险和监管限制问题，但目前缺乏对该领域研究现状的全面总结。

Method: 对2018年以来发表的72项研究进行系统综述，分类分析了合成的金融信息类型、生成方法和评估策略。

Result: GAN方法在文献中占主导地位，特别是在生成时间序列市场数据和表格信用数据方面。虽然一些创新技术显示出提高真实性和隐私保护的潜力，但大多数研究缺乏对隐私保护的严格评估。

Conclusion: 该综述为金融领域开发稳健、保护隐私的合成数据解决方案提供了指导，并指出了关键的研究空白。
...
TL;DR: 提出Nirvana模型，一种具有专业记忆机制、线性时间复杂度和测试时任务信息提取能力的专业化通用模型。通过任务感知记忆触发器和专业记忆更新器，实现基于任务需求的灵活记忆调整和动态上下文记忆。


<details>
  <summary>Details</summary>
Motivation: 传统LLM结构（包括Transformer、线性注意力和混合模型）缺乏由任务信息指导的专业记忆机制，无法在保持广泛能力的同时在目标领域达到专家级性能。

Method: 设计任务感知记忆触发器（Trigger）根据当前任务需求灵活调整记忆机制，将每个输入样本视为自监督微调任务；设计专业记忆更新器（Updater）在Trigger指导下动态记忆上下文。

Result: 在多种自然语言建模基准测试中取得竞争性或更优结果；在医学MRI任务中，通过轻量级编解码器后训练，实现比传统MRI模型和传统LLM骨干模型更高质量的MRI重建，并能生成准确的初步临床报告。

Conclusion: Nirvana通过专业记忆机制成功实现了在保持通用能力的同时在特定领域达到专家级性能，证明了任务感知记忆机制在专业化通用模型中的有效性。
...
TL;DR: 提出了一种基于大语言模型的多阶段bug二分定位方法，相比传统基于补丁的方法准确率提升超过38%，多阶段管道相比基线LLM方法准确率提升60%


<details>
  <summary>Details</summary>
Motivation: 传统基于补丁的bug二分定位方法存在多个限制：假设bug引入提交和补丁提交修改相同函数、仅依赖代码变更而忽略提交消息中的漏洞信息、基于简单启发式规则缺乏逻辑分析

Method: 设计了一个多阶段管道，利用LLM的能力：(1)充分利用补丁信息，(2)在上下文中比较多个候选提交，(3)通过一系列筛选步骤逐步缩小候选范围

Result: 评估显示该方法比最先进解决方案的准确率提高超过38%，多阶段管道相比基线LLM二分方法准确率提高60%

Conclusion: LLM能够突破现有解决方案的限制，多阶段管道对于准确识别bug引入提交至关重要
...
TL;DR: 提出两种基于多智能体强化学习的动态车辆路由方法：自适应导航(AN)和分层枢纽自适应导航(HHAN)，通过协调的路由决策缓解城市交通拥堵。


<details>
  <summary>Details</summary>
Motivation: 传统最短路径算法在动态多车辆环境中表现不佳，会导致所有车辆选择相同路径而加剧拥堵，需要更智能的协调路由方案。

Method: AN使用分散式MARL，每个交叉口智能体基于本地交通和图注意力网络提供路由指导；HHAN扩展为分层结构，只在关键枢纽部署智能体，结合SPF进行微路由，采用集中训练分散执行的A-QMIX框架。
...
TL;DR: SAFE是一个用于评估地球预测模型分层性能的开源工具包，通过地理、经济等多维度分层分析，揭示AI天气预测模型在不同区域的性能差异和公平性问题。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习评估方法基于测试集平均损失，忽略了地球不同区域人类发展和地理分布的非均匀性，无法反映模型在不同地区的真实表现差异。

Method: 开发SAFE工具包，整合多种数据域按领土、全球子区域、收入水平和土地覆盖等属性对地理网格点进行分层，分析每个分层内的模型性能。

Result: 应用SAFE评估多个最先进的AI天气预测模型，发现所有模型在不同属性上都存在预测技能差异，建立了基于不同提前时间和气候变量的模型预测公平性基准。

Conclusion: 通过超越全局平均指标，首次系统分析了模型在哪些区域表现最佳或最差，以及哪些模型最公平，为未来研究提供了重要工具和基准。
...
TL;DR: 提出LTE方法，通过提示LLM其先前生成的错误答案和过长响应问题，无需外部专家指导，有效缓解探索停滞问题，提升推理能力
...
Method: 提出LTE方法，通过提示LLM其先前生成的错误答案和过长响应问题，无需外部专家指导

Result: 在六个数学基准测试中，LTE在Qwen3-4B-Base上平均比GRPO方法在Pass@1上提升6.38，在Pass@k上提升9.00
...
TL;DR: maxVSTAR是一个基于WiFi信道状态信息(CSI)的视觉引导模型自适应框架，通过跨模态师生架构解决边缘设备上的领域偏移问题，实现无需人工干预的自主在线微调。


<details>
  <summary>Details</summary>
Motivation: WiFi CSI基于的人类活动识别在边缘设备部署时面临领域偏移问题，导致识别性能在环境变化时严重下降，需要一种自主适应机制来维持性能。

Method: 采用跨模态师生架构，使用高精度的YOLO视觉模型作为动态监督信号，为CSI数据流提供实时活动标签，从而对轻量级CSI HAR模型进行在线微调。

Result: 在未校准硬件上，基线STAR模型准确率从93.52%降至49.14%，经过一次视觉引导适应后，maxVSTAR将准确率恢复至81.51%。

Conclusion: maxVSTAR在隐私敏感的物联网环境中实现了动态、自监督的模型自适应，为网络边缘的长期自主HAR建立了可扩展的实用范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有Wi-Fi CSI活动识别方法在资源受限的移动边缘环境中存在计算效率低、延迟高和可行性有限的问题，需要开发更适合嵌入式设备的解决方案。

Method: 采用轻量级GRU网络减少33%参数，结合中值滤波、巴特沃斯低通滤波和EMD的多阶段预处理，在Rockchip RV1126处理器和NPU上实现硬件优化部署。

Result: 在7类活动识别中达到93.52%平均准确率，人体存在检测99.11%，INT8量化推理速度33MHz，CPU利用率仅8%，比CPU执行快6倍。

Conclusion: STAR框架提供了实时、隐私保护、低功耗的活动识别解决方案，适用于移动和普适计算环境。
...
TL;DR: MolBridge是一个基于子结构感知对齐的分子-文本学习框架，通过分子子结构和化学短语的额外对齐信号来增强分子描述对，采用子结构感知对比学习和自精炼机制来学习细粒度对应关系。
...
Method: 通过分子子结构和化学短语的额外对齐信号增强原始分子描述对，采用子结构感知对比学习和自精炼机制来过滤噪声对齐信号。

Result: MolBridge在多个分子基准测试中优于现有最先进基线，有效捕捉了细粒度对应关系。
...
TL;DR: 复杂特征工程和混合模型在工业时序异常检测中表现不佳，简单随机森林+XGBoost集成模型在分段数据上表现最优，AUC-ROC达0.976，F1分数0.41，并在定义时间窗口内实现100%早期检测。


<details>
  <summary>Details</summary>
Motivation: 研究先进特征工程和混合模型架构在多元工业时序异常检测中的有效性，特别关注蒸汽轮机系统。

Method: 评估变化点衍生统计特征、基于聚类的子结构表示和混合学习策略对检测性能的影响，并与简单随机森林+XGBoost集成模型在分段数据上的表现进行对比。

Result: 复杂方法表现一致不如简单集成模型，集成模型达到AUC-ROC 0.976，F1分数0.41，并在时间窗口内实现100%早期检测。

Conclusion: 在高度不平衡和时间不确定的数据场景中，模型简洁性结合优化分段可以超越更复杂的架构，提供更强的鲁棒性、可解释性和操作实用性。
...
Conclusion: GSTRL为CPRA问题提供了有效的解决方案，具有坚实的理论基础和实际应用价值。
...
TL;DR: 提出了ACC-SGD-IE方法，通过跨训练传播留一扰动来改进影响估计，在数据清洗中表现优于标准SGD-IE


<details>
  <summary>Details</summary>
Motivation: 标准SGD-IE方法忽略了跨epoch的复合效应，导致对关键样本的错误排序，需要更精确的逐样本影响估计

Method: ACC-SGD-IE：轨迹感知估计器，在训练过程中传播留一扰动，并在每个步骤更新累积影响状态

Result: 在平滑强凸设置中实现几何误差收缩，在平滑非凸设置中收紧误差界限；在多个数据集上产生更准确的影响估计，特别是在长epoch中

Conclusion: ACC-SGD-IE在下游数据清洗中更可靠地标记噪声样本，使用该方法清洗的数据训练的模型优于SGD-IE清洗的模型
...
TL;DR: 使用机器学习方法预测医院再入院率，随机森林模型表现最佳，可帮助识别再入院风险因素和患者群体。


<details>
  <summary>Details</summary>
Motivation: 降低可预防的医院再入院率是医疗支付方、提供者和政策制定者的重要目标，再入院率被用作衡量医院医疗质量的基准。

Method: 使用逻辑回归、随机森林和支持向量机分析健康索赔数据，采用主成分分析进行降维处理，基于AUC指标评估模型性能。

Result: 随机森林模型表现最佳，其次是逻辑回归和支持向量机，这些模型能识别导致再入院的关键因素。

Conclusion: 机器学习模型可用于识别再入院风险因素和重点患者群体，有助于降低再入院率、减少医疗成本并提高医疗质量。
...
Conclusion: AISP是一种有效的测试时对齐方法，能够在不需要微调的情况下优化模型表现。
...
TL;DR: 提出了一种基于归纳方法的机器遗忘解决方案，通过在模型末端添加投影-重分布层来实现遗忘，无需访问原始数据集或完整模型，具有模块化和模型无关性。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法在现实部署中面临可扩展性问题，需要完整访问原始数据集和模型。本文旨在解决这些实际挑战，提供更实用的遗忘方案。

Method: 将分类训练视为顺序学习过程，遗忘通过反转最后训练序列实现。在模型末端添加投影-重分布层作为输出过滤器，实现模块化部署。

Result: 在多个数据集（CIFAR-10/100使用CNN模型，Covertype使用树模型）上的实验表明，该方法输出与完全重新训练模型相似，同时显著降低计算成本。

Conclusion: 该方法在实际设置中展示了适用性、可扩展性和系统兼容性，同时保持了输出性能，为机器遗忘提供了更实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型行为控制方法（如向量加法、方向消融）局限于二维子空间，对参数选择敏感且可能影响无关特征，需要更灵活稳定的调控方法。

Method: Angular Steering方法将行为调控建模为几何旋转，在固定二维子空间内旋转激活向量朝向或远离目标行为方向；还提出了自适应变体，仅旋转与目标特征对齐的激活。

Result: 实验表明Angular Steering在多个模型家族和尺寸上实现了鲁棒的行为控制，同时保持通用语言建模性能，相比现有方法具有更好的灵活性、泛化性和鲁棒性。

Conclusion: Angular Steering将现有加法和正交化技术统一在几何旋转框架下，简化了参数选择，在更广泛的调整范围内保持模型稳定性，为安全可靠AI部署提供了有效工具。
...
TL;DR: 提出了一种通用的生成模型插值方案，无需额外训练即可计算在数据分布约束下的可能过渡路径，理论上可视为黎曼度量下的测地线。


<details>
  <summary>Details</summary>
Motivation: 大多数生成模型缺乏对插值的主概念，需要限制性假设。本文旨在开发一种兼容不同度量和概率分布的通用插值方案。

Method: 开发了一种无需额外训练的算法，计算类似于在合适数据分布约束下的测地线插值曲线。

Result: 定量实验表明，该方法在多种模型和数据集上比基线方法更频繁地穿越高密度区域。

Conclusion: 该方法提供了一种通用的插值方案，理论上具有黎曼几何基础，实践中能更好地遵循数据分布。
...
TL;DR: 提出了IMG算法，在扩散模型推理时通过加权重采样优化生成过程，实现多目标优化，相比需要数百次生成的传统方法，仅需单次生成即可获得更高超体积。
...
Result: 在多目标分子生成任务中，IMG仅需单次生成就比需要数百次扩散生成的基线算法获得显著更高的超体积。

Conclusion: IMG算法可视为优化的扩散过程，能够集成到现有方法中进一步提升性能，且多目标玻尔兹曼分布是分布式多目标优化问题的最优解。
...
<details>
  <summary>Details</summary>
Motivation: 多任务学习在上下文老虎机中具有重要研究价值，但现有方法往往忽略老虎机之间的协方差结构学习，限制了信息共享的有效性和灵活性。
...
Conclusion: 该分层贝叶斯框架有效平衡了多老虎机之间的探索与利用，填补了多老虎机问题领域的研究空白，具有更好的实用性和灵活性。
...
TL;DR: 提出了离线聚类偏好学习框架，解决多用户偏好学习中的两个关键挑战：识别用户相似性以聚合数据，以及处理维度不平衡的离线数据。


<details>
  <summary>Details</summary>
Motivation: 现实世界偏好学习涉及不同偏好的用户，且用户交互有限或成本高昂，需要离线学习。主要挑战包括识别用户相似性以有效聚合数据，以及处理维度不平衡的离线数据。

Method: 首先提出Off-C²PL算法用于纯离线设置，然后扩展到主动数据增强设置，提出A²-Off-C²PL算法，主动选择针对测试用户最不具信息性维度的样本。

Result: 理论分析提供了明确捕捉样本噪声与偏差权衡的次优性边界，证明主动收集的样本比离线样本更有效。通过合成和真实数据集验证了理论结果。

Conclusion: 提出的离线聚类偏好学习框架能够有效解决多用户偏好学习中的数据聚合和维度不平衡问题，主动数据增强策略显著提升了学习效果。
...
TL;DR: 提出基于token感知的因果表示学习框架，解释CLIP在组合推理上的失败原因——组合不可识别性，证明存在伪最优文本编码器无法区分正确标题和困难负例。


<details>
  <summary>Details</summary>
Motivation: CLIP在跨模态泛化方面表现优异，但在对象、属性和关系的组合推理上持续失败，现有因果解释将文本视为单一向量，无法解释提示敏感性和困难负例失败现象。

Method: 建立基于语言token的顺序结构因果模型，将块可识别性扩展到token化文本，证明CLIP对比目标可以恢复模态不变潜在变量。

Result: 理论证明存在伪最优文本编码器，虽然实现完美模态不变对齐，但对原子概念的SWAP、REPLACE和ADD操作不敏感，无法区分正确标题和困难负例。

Conclusion: 组合不可识别性是CLIP组合脆弱性的根本原因，语言侧不可识别性通过模态间隙与视觉侧失败相关联，迭代组合操作加剧难度，需要改进负例挖掘策略。
...
TL;DR: 提出了一种无需数据的持续学习方法，通过分层模型反演和特征建模来生成伪图像，避免了存储历史数据的需求，同时解决了模型反演中的特征漂移和计算效率问题。


<details>
  <summary>Details</summary>
Motivation: 传统持续学习需要存储和重放历史数据，但在隐私安全约束下不可行。数据无关的持续学习旨在不访问先前数据的情况下更新模型，但面临模型反演中的特征漂移和计算效率挑战。

Method: 提出分层模型反演(PMI)方法，通过单层优化提供强初始化，大幅减少迭代次数；使用高斯分布和对比模型建模类级特征，确保合成特征与真实特征对齐；结合语义感知投影特征生成伪图像。

Result: 该方法在多个持续学习设置中表现出强大的有效性和兼容性，能够高效地学习新类别而无需存储历史样本。

Conclusion: PMI和特征建模的结合为数据无关持续学习提供了有效解决方案，特别适用于大型预训练模型如CLIP，在保持计算效率的同时缓解了特征漂移问题。
...
TL;DR: 使用量子退火训练SVM的研究表明，即使低精度QUBO编码（如1比特/参数）也能获得与经典方法相当的分类性能，选择正确的支持向量比精确权重更重要。


<details>
  <summary>Details</summary>
Motivation: 研究量子退火在SVM训练中的应用，探索QUBO问题中量子比特数量（与对偶权重离散化水平相关）对预测性能的影响。

Method: 将SVM训练建模为QUBO问题，使用量子退火进行模型优化，比较不同比特深度编码与经典LIBSVM求解器的性能。

Result: 低精度QUBO编码（如1比特/参数）在多个数据集上表现出与经典方法竞争甚至更优的准确率，增加比特深度并不总是改善分类性能。

Conclusion: 量子退火在SVM训练中具有潜力，选择合适的支持向量比参数精确加权更重要，随着量子设备扩展，量子退火有望实现高效的SVM训练。
...
TL;DR: 该论文揭示了前沿LLM公司推出的Agent Skills框架存在严重安全漏洞，攻击者可以通过简单的提示注入技术窃取敏感数据，并能绕过系统级防护措施。
...
Method: 通过在长Agent Skill文件和引用脚本中隐藏恶意指令，演示如何利用提示注入漏洞窃取内部文件和密码等敏感数据。

Result: 成功展示了如何绕过流行编码代理的系统级防护，证明良性任务特定的批准选项可以被恶意利用来执行有害操作。

Conclusion: 尽管研究不断推进且模型能力增强，前沿LLM在实际场景中仍然容易受到简单提示注入攻击的影响。
...
<details>
  <summary>Details</summary>
Motivation: 现有因果发现方法只能施加结构约束，但可能得出错误因果结论，需要引入能约束因果效应方向的知识。

Method: 为线性因果模型提出量化总因果效应的指标，采用两阶段约束优化方法求解。

Result: 在真实数据集上验证，整合干预约束能提高模型准确性、确保与已有发现一致，并发现新的因果关系。

Conclusion: 干预约束填补了因果知识整合的空白，使模型更可解释且能发现原本成本高昂的因果关系。
...
TL;DR: 该论文通过改进经典强化学习方法，使其能够在稀疏、随机和非平稳环境中高效运行，特别是在水下污染云搜索等应用中。
...
Result: 改进的基于蒙特卡洛的方法显著优于传统Q学习和两种穷举搜索模式，展示了在复杂环境中适应强化学习的潜力。
...
TL;DR: UnifiedFL是一个动态联邦学习框架，通过图神经网络统一表示异构客户端架构，解决现有FL方法在处理不同网络类型、非独立同分布数据和域差异时的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习方法只能支持同一模型家族的变体，无法处理客户端使用根本不同网络类型（如CNN、GNN、MLP）的情况，且忽略了域断裂问题，导致模型泛化能力差。

Method: 使用共享图神经网络将异构本地网络表示为有向模型图中的节点和边；通过客户端参数间的欧氏距离进行距离驱动聚类；采用平衡收敛性和多样性的双层聚合策略。

Result: 在MedMNIST分类和海马体分割基准测试中，UnifiedFL表现出优越性能。

Conclusion: UnifiedFL能够有效处理异构架构、非独立同分布数据和域差异，为联邦学习提供了更通用的解决方案。
...
TL;DR: 提出了几种可解释和可靠的金融AI方法，包括Time-LLM时间序列基础模型、可靠性估计器和符号推理，通过实验证明能减少误报并支持选择性执行。
...
Method: 1. Time-LLM时间序列基础模型使用提示避免错误方向预测；2. 结合基础模型与可靠性估计器过滤不可靠预测；3. 符号推理编码领域规则实现透明解释。
...
TL;DR: 提出CorVS方法，通过视觉追踪轨迹与传感器测量的对应关系进行人员识别，解决了工业环境中仅依赖视觉数据识别个体的局限性。


<details>
  <summary>Details</summary>
Motivation: 工业场所中工人定位数据对提高生产力至关重要。虽然摄像头能提供环境上下文信息，但仅靠视觉数据识别个体不切实际。现有方法通过比较轨迹和可穿戴传感器测量来识别人员，但在真实条件下可能失效。

Method: 提出CorVS方法：1）深度学习模型预测轨迹与传感器测量之间的对应概率和可靠性；2）算法使用预测的概率和可靠性随时间匹配轨迹和传感器测量。

Result: 开发了包含实际仓库操作的数据集，证明了该方法在真实应用中的有效性。

Conclusion: CorVS方法能够有效解决工业环境中的人员识别问题，为仓库管理等应用提供了实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 平流层突然增温(SSW)是次季节可预报性的关键来源和极端冬季天气的主要驱动因素，但其准确高效预报仍面临物理表示、初始化和计算需求等挑战。数据驱动方法在SSW复杂三维动力学中的应用，特别是概率预报方面仍待探索。

Method: 开发基于流匹配的生成AI模型FM-Cast，用于平流层环流的时空演化概率预报。通过理想化实验分析SSW可预报性与物理驱动因素的关系。

Result: 在18次主要SSW事件(1998-2024)评估中，FM-Cast能提前20天准确预报10次事件的爆发、强度和形态，集合准确率超过50%。性能相当或优于领先的NWP系统，在消费级GPU上仅需2分钟完成50成员、30天的预报。

Conclusion: 建立了计算高效的平流层异常概率预报范式，展示了生成AI在加深大气-气候动力学物理理解方面的潜力，区分了由对流层强迫和内部平流层动力学驱动的SSW事件的可预报性差异。
...
TL;DR: 提出了一种新颖的多智能体强化学习框架，通过动态优化上下文长度和傅里叶低频截断方法，提高探索效率并过滤冗余信息，在多个长期依赖任务中实现SOTA性能。
...
Method: 设计中心智能体通过时间梯度分析动态优化上下文长度，并使用傅里叶低频截断方法提取全局时间趋势，提供高效的环境表示。

Result: 在PettingZoo、MiniGrid、GRF和SMACv2等长期依赖任务上实现了最先进的性能。

Conclusion: 提出的自适应上下文长度优化框架能有效提升MARL的探索效率和收敛性能，在复杂多智能体环境中表现出色。
...
TL;DR: 本章综述了基于支持向量机(SVM)和双支持向量机(TWSVM)的多任务学习方法，重点关注共享表示、任务正则化和结构耦合策略，并比较了这些模型的理论特性、优化策略和实证性能。


<details>
  <summary>Details</summary>
Motivation: 虽然深度学习主导了最近的多任务学习研究，但SVM和TWSVM因其可解释性、理论严谨性和在小数据集上的有效性仍然具有相关性。本章旨在为研究人员和从业者提供基于SVM和TWSVM的多任务学习的全面资源。

Method: 调查了基于SVM和TWSVM的多任务学习方法，包括共享表示、任务正则化和结构耦合策略。特别关注了新兴的TWSVM扩展在多任务设置中的应用。

Result: 比较了这些模型的理论特性、优化策略和实证性能，并讨论了在计算机视觉、自然语言处理和生物信息学等领域的应用。

Conclusion: 确定了研究空白，并为构建可扩展、可解释和可靠的基于边界的多任务学习框架提出了未来方向。
...
TL;DR: CoLA-World提出了一种联合训练潜在动作模型和世界模型的新范式，通过预热阶段解决表示对齐问题，实现了两者的协同进化。


<details>
  <summary>Details</summary>
Motivation: 现有两阶段方法训练潜在动作模型和世界模型分离，导致冗余训练且限制了协同适应潜力。直接联合训练面临表示崩溃的挑战。

Method: 提出CoLA-World框架，通过关键的预热阶段有效对齐从零开始的潜在动作模型与预训练世界模型的表示，实现联合训练。

Result: CoLA-World在视频模拟质量和下游视觉规划任务上匹配或优于先前两阶段方法。
...
TL;DR: 提出CFKD-AFN网络，利用丰富的低保真模拟数据来增强稀缺但高保真试验数据的治疗效果预测性能


<details>
  <summary>Details</summary>
Motivation: 精准医疗中，针对小样本和罕见患者群体的个性化治疗效果预测至关重要，但昂贵的试验数据限制了预测性能

Method: 采用跨保真度知识蒸馏和自适应融合网络，包含双通道知识蒸馏模块提取低保真模型的互补知识，以及注意力引导融合模块动态整合多源信息

Result: 在慢性阻塞性肺疾病治疗效果预测实验中，CFKD-AFN相比最先进方法在预测准确率上提升6.67%到74.55%，并对高保真数据集大小变化表现出强鲁棒性

Conclusion: CFKD-AFN显著提升了小样本治疗效果预测性能，并可扩展为可解释变体以支持临床决策
...
TL;DR: 提出了一种新的流形约束鲁棒图压缩框架MRGC，通过图数据流形学习模块提高图压缩在对抗攻击下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有图压缩方法在原始图数据被破坏时性能显著下降，而现有的鲁棒图学习技术效果有限。图压缩本质上是一个内在维度降低过程，但这一特性使其容易受到对抗性扰动的影响。

Method: 采用图数据流形的几何视角，提出了MRGC框架，包含三个图数据流形学习模块，引导压缩图位于平滑、低维的流形中，具有最小的类别模糊性。
...
TL;DR: ReSpec系统通过动态调整推测解码配置、通过知识蒸馏进化草稿模型、以及基于奖励加权更新，解决了推测解码在强化学习训练中的三大挑战，实现了最高4.5倍的加速，同时保持奖励收敛和训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通过强化学习进行适应时，生成阶段消耗超过75%的训练时间。推测解码在服务系统中能加速自回归生成，但在强化学习训练中的行为尚未充分探索，存在速度提升随批量增大而减弱、草稿模型在持续更新中过时、以及草稿模型导致策略退化等三大挑战。

Method: 提出ReSpec系统，包含三个互补机制：动态调整推测解码配置以适应不同批量大小；通过知识蒸馏持续进化草稿模型以保持与演员模型的同步；基于rollout奖励对更新进行加权，减轻草稿模型引起的策略退化。

Result: 在Qwen模型（3B-14B）上，ReSpec实现了最高4.5倍的加速，同时保持了奖励收敛和训练稳定性，为基于强化学习的LLM适应提供了实用解决方案。

Conclusion: ReSpec成功地将推测解码技术适配到强化学习训练中，通过系统性的机制设计解决了集成过程中的关键挑战，显著提升了训练效率，为高效的大语言模型强化学习适应开辟了新途径。
...
TL;DR: 提出了一种基于量子门控循环单元(QGRU)的生成对抗网络，采用连续数据注入和多度量门控策略，用于网络异常检测，在量子硬件上实现了89.43%的时间序列感知F1分数。
...
Method: 使用量子增强生成器输出高斯分布参数，结合Wasserstein判别器稳定对抗训练，通过多阶段门控机制（基于高斯不确定性估计、判别器分数和重构误差）识别异常。

Result: 在基准数据集上达到89.43%的时间序列感知F1分数，优于现有经典和量子模型，并在IBM量子硬件上保持高性能。
...
TL;DR: 提出了CROPI框架，使用影响函数理论指导数据选择，通过离线影响估计和稀疏随机投影技术，在RLVR训练中实现高效数据筛选，显著加速模型训练。


<details>
  <summary>Details</summary>
Motivation: 当前RLVR中的数据选择方法主要基于启发式策略，缺乏理论保证和泛化性。需要一种理论支撑的数据选择方法来提升大语言模型的推理能力训练效率。

Method: 提出CROPI框架：1）使用影响函数估计数据点对学习目标的贡献；2）引入离线策略影响估计方法，利用预收集轨迹近似在线影响；3）采用稀疏随机投影降低LLM高维梯度的维度；4）多阶段RL迭代选择对当前策略最有影响的数据。
...
TL;DR: 本文探讨了在资源受限的可穿戴设备上，使用无监督异常检测作为上游过滤机制来提升心电图分析可靠性的方法。通过神经架构搜索优化六种UAD方法，发现Deep SVDD在检测性能和效率之间达到最佳平衡，集成后可将诊断准确率提升高达21个百分点。


<details>
  <summary>Details</summary>
Motivation: 可穿戴设备上的连续心电图监测对心血管疾病早期检测具有重要价值，但标准分类器在面对分布外数据时会产生错误的高置信度预测，危及患者安全。现有OOD检测方法要么忽略计算约束，要么分别处理噪声和未见类别问题。

Method: 使用神经架构搜索在严格资源约束下优化六种无监督异常检测方法：Deep SVDD、基于重构的模型、掩码异常检测、归一化流和扩散模型，参数限制在512k以内。在PTB-XL和BUT QDB数据集上评估对OOD心血管疾病类别和噪声信号的检测能力。
...
TL;DR: 提出了ICPO框架，利用LRMs的上下文学习能力提供专家指导，无需依赖高级模型轨迹，通过混合策略GRPO和专家区域拒绝采样等方法增强探索和训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法如GRPO在提升大型推理模型推理能力方面取得进展，但存在探索受限问题，主要依赖当前策略分布导致轨迹多样性不足。

Method: ICPO框架包含：混合策略GRPO与隐式专家强制，扩展探索范围；专家区域拒绝采样，过滤不可靠轨迹；退火专家奖励塑形，平衡专家指导与自主改进。

Result: 在数学推理基准测试中，ICPO一致提升了强化学习性能和训练稳定性。

Conclusion: ICPO为LRMs提供了一个可扩展且有效的RLVR范式，无需依赖高级模型轨迹即可实现更好的探索和优化效果。
...
TL;DR: 提出了一种新的多基础推测解码框架，通过理论分析证明了多模型推测解码系统的最优推理时间，相比传统双模型方法实现了3.31-4.43倍的加速，同时保持原始输出分布。


<details>
  <summary>Details</summary>
Motivation: 推理延迟是大规模部署大语言模型的关键瓶颈。现有推测解码方法通常依赖双模型框架且缺乏严格的理论基础。

Method: 引入多基础推测解码框架，通过理论分析多模型token生成，优化模型能力、接受长度和计算成本之间的相互作用。支持独立实现和与现有推测技术集成。

Result: 在多个模型系列上的实验结果显示：LLaMA2-Chat 7B加速3.31-4.01倍，LLaMA3-8B加速3.87倍，Vicuna-7B加速4.43倍，Qwen2-7B加速3.85倍，同时保持原始输出分布。

Conclusion: 多基础推测解码框架提供了超越传统双模型方法的通用范式，显著加速推理过程，为推测解码的进一步发展提供了理论基础和实现代码。
...
Method: 使用截断版本的HOHL作为正则化器，在完全监督学习框架下进行理论分析，并在主动学习和非几何结构数据集上进行实证验证。

Result: 证明了截断HOHL的一致性并推导了显式收敛速率，在主动学习和非几何结构数据集中表现出强大的实证性能。

Conclusion: HOHL在不同学习设置中展现出多功能性和鲁棒性，为超图学习提供了坚实的理论基础和实用价值。
...
TL;DR: 提出了一种三阶段贝叶斯域对抗神经网络框架，结合参数迁移和共享潜在空间适应，在存在大域偏移的情况下提高迁移学习性能并提供不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 传统参数迁移方法在大域偏移下性能下降，域对抗神经网络虽然能处理域偏移但训练不稳定且缺乏不确定性量化能力。

Method: 三阶段框架：第一阶段在源域训练确定性特征提取器；第二阶段使用域对抗神经网络进行对抗性精炼；第三阶段在适应后的特征提取器上构建贝叶斯神经网络进行目标域微调。

Result: 在合成基准测试中显著优于标准迁移技术，在矩形通道临界热通量预测任务中提高了预测精度和泛化能力。

Conclusion: staged B-DANN方法能够改善预测准确性和泛化能力，有望帮助核工程等其他领域。
...
Method: 采用训练紧凑提升决策树集成的方法，通过奖励特征和阈值重用来减少内存占用，结合适应的训练过程和替代内存布局。

Result: 实验评估显示，模型在保持相同性能的同时，相比LightGBM模型实现了4-16倍的压缩比。

Conclusion: 该压缩方案为物联网应用打开了大门，使设备能够在隔离或功率受限环境中进行远程监控、边缘分析和实时决策，仅需最小计算能力和能量。
...
TL;DR: 本文提出了一种多分类局部校准方法，通过引入输入空间距离概念来解决现有多分类校准方法在稀疏区域容易产生偏差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多分类校准方法缺乏输入空间距离概念，容易导致稀疏区域的预测概率系统性失准，这在医疗等高风险场景中尤为关键。

Method: 提出多分类局部校准定义，分析现有评估指标的缺陷，并设计了一种基于Jensen-Shannon距离的神经网络局部校准增强方法。

Result: 通过理论分析和实验验证，证明了所提方法在多分类校准任务上的有效性。
...
TL;DR: 提出了一种结合Bernstein基参数化和Wasserstein距离最小化的分布回归方法，通过高斯分量和Bernstein多项式构建平滑概率轨迹，在保持高可解释性的同时实现几何准确性和计算实用性。


<details>
  <summary>Details</summary>
Motivation: 现有分布回归方法要么忽略概率空间的几何结构，要么计算成本高昂，需要一种能兼顾几何准确性和计算效率的新方法。

Method: 使用Bernstein基参数化概率轨迹，通过加权高斯分量构建条件分布，其中均值和协方差是输入变量的Bernstein多项式函数，采用基于自动微分的优化方法最小化Wasserstein距离。

Result: 在包含复杂轨迹的合成数据集上，该方法在Wasserstein距离、能量距离和RMSE指标上表现出有竞争力的近似质量，特别是在非线性明显的情况下，轨迹平滑度和鲁棒性优于或与替代方法相当。

Conclusion: 该方法实现了几何准确性、计算实用性和可解释性的平衡，未来可扩展到非高斯分布、应用熵正则化加速计算，并适应高维数据近似更复杂结构。
...
<details>
  <summary>Details</summary>
Motivation: 现有航班延误数据集通常局限于平面表格结构，无法捕捉延误传播的时空动态特性。

Method: 提供三种对齐模态：包含5000多万航班运营、气象和机场特征的表格数据集；建模延误沿连续航班传播的航班链模块；编码共享飞机、机组和机场资源连接的航班网络图。

Result: 数据集支持回归、分类、时序结构建模和图学习等多种任务，作为表格、序列和图模态的统一基准。
...
TL;DR: 本文提出了一个基于热核的统一框架，用于理解和分析贝叶斯优化中的组合核，证明了许多成功的组合核与热核相关或等价，并展示了基于热核的简单管道能达到最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯优化在组合任务中具有广泛应用潜力，但需要专门的核函数来有效建模组合域。虽然已有多个组合核被提出，但它们之间的关系尚不明确，需要建立一个统一的理论框架来理解这些核函数。

Method: 开发了一个基于热核的统一框架，系统地推导热核并表示为简单的闭式表达式。通过该框架分析现有组合核与热核的关系，并构建一个快速简单的基于热核的优化管道。

Result: 实验验证了许多成功的组合核确实与热核相关或等价。热核对最优值的位置不敏感，而某些算法在最优值缺乏特定结构时性能会显著下降。基于热核的简单管道能匹配甚至超越某些复杂或缓慢的算法。

Conclusion: 热核为理解组合核提供了一个统一的框架，基于热核的简单方法在贝叶斯优化中能取得与复杂方法相当甚至更好的性能，且对最优值位置具有鲁棒性。
...
TL;DR: 本文研究使用时间序列分类方法作为异常检测的模型选择策略，通过大规模实验证明模型选择方法在性能上优于任何单一异常检测方法，且执行时间相当。


<details>
  <summary>Details</summary>
Motivation: 现有的异常检测方法在异构时间序列数据集上表现不一，没有通用的最佳方法。现有的AutoML解决方案无法直接应用于时间序列异常检测，且缺乏针对时间序列的模型选择方法评估。

Method: 评估了234种模型配置（基于16个基础分类器），在1980多个时间序列上进行了实验，首次对时间序列分类作为异常检测模型选择方法进行了广泛评估。
...
TL;DR: 提出Curly-FM方法解决传统流匹配方法无法建模非梯度场动态的问题，通过设计具有非零漂移参考过程的Schrödinger桥问题来学习周期性行为


<details>
  <summary>Details</summary>
Motivation: 许多现实系统（如单细胞RNA中的细胞周期）表现出非梯度、周期性行为，而现有方法基于最小作用原理，只能捕捉梯度场动态，无法建模这类系统

Method: 设计并求解具有非零漂移参考过程的Schrödinger桥问题，使用推断的速度和群体快照数据构建参考过程，从而学习非梯度场动态

Result: 在单细胞、计算流体动力学和洋流轨迹推断问题上，Curly-FM学习的轨迹与参考过程和群体边际分布更匹配

Conclusion: Curly-FM扩展了流匹配模型的应用范围，使其能够建模物理系统中已知的周期性行为，超越了仅建模群体动态的局限
...
TL;DR: 提出了一种基于奇异值分解和标准扰动机制的简单高效算法，用于在差分隐私下计算矩阵前r个奇异向量张成的子空间，其误差仅取决于秩r相干性和谱间隙。
...
Method: 使用奇异值分解结合标准扰动机制，证明高斯扰动不会增加秩r相干性，从而保持输入数据的相干性特征。

Result: 算法在密集设置下实现了与非私有最优算法相同的性能保证，显著优于现有私有算法。同时证明了相干性在隐私保护下的稳定性。

Conclusion: 该方法为差分隐私下的矩阵近似提供了更优的解决方案，相干性概念可扩展到图问题等结构化模型中的隐私保护算法设计。
...
TL;DR: LoRAQuant是一种针对LoRA适配器的混合精度后训练量化方法，通过SVD重参数化将重要信息集中在特定行列，使重要组件使用高精度量化，其余部分使用超低位宽量化，显著降低比特数同时保持或提升性能。


<details>
  <summary>Details</summary>
Motivation: LoRA已成为大型语言模型参数高效微调的流行技术，但在现实场景中同时加载多个适配器时，尽管单个适配器轻量，但聚合成本在规模上变得显著，需要解决这一问题。

Method: 提出LoRAQuant方法，通过奇异值分解(SVD)对每个适配器进行重参数化，将最重要信息集中在特定行和列中，从而能够对重要组件使用更高精度量化，其余部分使用超低位宽量化。
...
Method: 引入了两种正则化项用于网络训练：第一种连接后验均值，第二种类似于MAP估计器。通过理论分析表征了每种损失如何塑造学习的前向算子及其逆重建映射。
...
<details>
  <summary>Details</summary>
Motivation: 现有梯度压缩方法存在局限性：Rand-K丢弃结构信息且性能较差，Top-K保留重要信息但失去压缩特性且需要昂贵的All-Gather操作。

Method: 设计All-Reduce兼容的Top-K压缩器，使用轻量级梯度草图对齐稀疏模式，结合动量误差反馈(EF21M)实现收敛加速。
...
TL;DR: 本文指出机器学习遗忘算法评估中常见的实践存在问题，即从同一训练模型多次独立运行遗忘算法可能产生非代表性结果，因为某些遗忘方法对模型训练时使用的随机种子高度敏感。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习遗忘算法的实证比较通常从同一训练模型多次运行遗忘算法，但这种做法可能无法真实反映算法性能，因为遗忘效果对模型训练时的随机种子选择很敏感。

Method: 通过分析不同随机种子训练的模型上运行遗忘算法的结果，比较其变异性，证明单一模型评估的局限性。

Result: 研究发现某些遗忘方法在不同训练种子的模型上表现差异很大，表明仅从单一训练模型评估会得到非代表性结果。

Conclusion: 建议在评估机器学习遗忘算法时，应考虑不同模型训练种子带来的变异性，以获得更全面和代表性的性能比较。
...
TL;DR: LSM-MS2是一个基于深度学习的大规模基础模型，通过训练数百万个质谱数据学习语义化学空间，在光谱识别任务中达到最先进性能，特别是在异构化合物识别和复杂生物样本分析方面表现优异。
...
Method: 开发了LSM-MS2大规模深度学习基础模型，在数百万个光谱数据上进行训练，学习语义化学空间。

Result: LSM-MS2在光谱识别方面达到最先进性能：异构化合物识别准确率提高30%；复杂生物样本中正确识别数量增加42%；在低浓度条件下保持稳健性；生成丰富的光谱嵌入，可从少量下游数据直接进行生物学解释。

Conclusion: LSM-MS2模型成功区分疾病状态并预测临床结果，在多种转化应用中展现出强大潜力，为质谱数据的深度挖掘和生物学解释提供了有效工具。
...
TL;DR: 提出一种简单的扰动机制，用于在纯差分隐私下发布d维协方差矩阵。该机制在大数据集上达到最优Frobenius范数误差，在所有p-Schatten范数上达到最佳误差，在p≥2时信息论最优，是首个在谱范数上达到最优误差的纯私有协方差估计器。


<details>
  <summary>Details</summary>
Motivation: 现有的私有协方差矩阵估计方法在不同范数下的误差保证不够全面，特别是在谱范数上缺乏最优性保证，需要一种简单且全面的机制来统一优化各种范数下的误差。

Method: 使用简单的扰动机制，对于大数据集直接应用，对于小数据集通过投影到适当半径的核范数球上来优化误差。

Result: 在大数据集上恢复最优Frobenius范数误差保证，在所有p-Schatten范数上达到已知最佳误差，在p≥2时信息论最优，在谱范数上首次达到最优误差。对于小数据集，通过投影改进Frobenius范数误差。

Conclusion: 该机制简单有效，在纯差分隐私下为协方差矩阵估计提供了全面的最优误差保证，特别是在谱范数上首次实现最优性，改进了现有方法的误差界限。
...
TL;DR: STaMP量化是一种新策略，通过在序列维度应用线性变换并混合精度，显著改善低比特激活量化效果


<details>
  <summary>Details</summary>
Motivation: 当前量化方法在激活量化低于8位时精度急剧下降，需要新方法来维持模型精度

Method: 在序列维度应用线性变换，利用语言和视觉数据的强局部相关性，保持少量token使用更高精度

Result: 在LVM和LLM架构上评估，显著改善低比特激活量化，与现有量化方法互补

Conclusion: STaMP量化策略能有效维持模型精度，是激活量化的有效补充方法
...
TL;DR: 提出了两种基于特征和logits的采样方法来改进影响函数估计，在保持准确性的同时显著降低了计算开销。


<details>
  <summary>Details</summary>
Motivation: 传统影响函数计算需要整个数据集的Hessian矩阵，计算资源消耗大。随机采样方法虽然可行但估计结果不稳定，方差高。

Method: 提出了基于特征和logits的两种高级采样技术，通过考虑特征或logits的随机分布来选择具有代表性的数据子集。

Result: 在类别移除实验中，方法减少了30.1%的计算时间和42.2%的内存使用，或将F1分数提高了2.5%。

Conclusion: 提出的采样方法能够有效提升影响函数估计的准确性和效率，为黑盒模型训练数据影响分析提供了实用解决方案。
...
TL;DR: 研究发现预训练的时序预测模型可以直接用于分类任务，其分类精度与专门为分类任务预训练的模型相当甚至更好，表明预测学习可能是构建通用时序基础模型的有效途径。


<details>
  <summary>Details</summary>
Motivation: 探究预训练预测模型学习到的表征是否具有通用性，能否有效应用于分类任务，挑战任务特定预训练的必要性假设。

Method: 比较不同的表征提取策略，并引入两种模型无关的嵌入增强方法，评估冻结预训练预测模型在分类任务上的表现。

Result: 最佳预测模型在分类任务上达到了与专门预训练的分类模型相当甚至更好的准确率，且预测性能与分类性能呈正相关。

Conclusion: 学习预测可能是构建通用时序基础模型的有力途径，任务特定预训练并非必要。
...
Method: GRWM在潜在表示空间中实施几何正则化，要求自然感知轨迹上的连续点在潜在空间中保持邻近，从而学习与真实环境拓扑对齐的表示。

Result: 在确定性3D环境和长期预测任务中，GRWM显著提高了rollout保真度和稳定性，学习到具有优越几何结构的潜在流形。

Conclusion: 改进表示学习是构建鲁棒世界模型的有效路径，无需扩大动态模块即可实现可靠的长期预测。
...
<details>
  <summary>Details</summary>
Motivation: 评估AI在现实世界中的经济价值和自动化能力，为AI驱动的劳动力自动化提供实证依据。

Method: 引入Remote Labor Index (RLI)基准，包含多行业真实经济价值的项目，评估端到端代理在实际环境中的表现。

Result: AI代理在RLI基准上表现接近最低水平，性能最高的代理仅实现2.5%的自动化率。

Conclusion: 该研究为AI自动化讨论提供了实证基础，建立了追踪AI影响的共同基准，帮助利益相关者主动应对AI驱动的劳动力自动化。
...
<details>
  <summary>Details</summary>
Motivation: 解决RL微调LLM时因数值不匹配导致的不稳定性问题，发现根本原因在于BF16浮点精度本身而非算法或工程对齐问题。

Method: 将浮点精度从BF16改为FP16，这种改变简单易行，只需几行代码修改，无需改变模型架构或学习算法。

Result: 使用FP16能带来更稳定的优化、更快的收敛速度，并在多种任务、算法和框架中表现出更强的性能。
...
TL;DR: 研究表明Transformer模型能够学习Permuted Congruential Generators (PCGs)生成的序列，即使输出被截断到单个比特也能可靠预测，并发现模型会自发地将整数输入分组为比特旋转不变簇。


<details>
  <summary>Details</summary>
Motivation: 研究Transformer模型学习PCGs序列的能力，PCGs通过比特移位、异或、旋转和截断等操作增加了比线性同余生成器更大的难度，探索模型在这种复杂伪随机数生成任务中的表现。

Method: 使用Transformer模型进行上下文预测，在实验中扩展到模数2^22，使用多达5000万参数和50亿token的数据集，分析不同PCG变体的学习能力，并研究嵌入层的聚类现象。

Result: Transformer能够成功预测未见过的PCG序列，即使输出截断到单个比特也能可靠预测；发现模数m的缩放规律：实现近乎完美预测所需的上下文序列元素数量以√m增长；对于大模数需要结合小模数数据进行课程学习；嵌入层出现比特旋转不变聚类现象。

Conclusion: Transformer模型能够学习复杂的PCG序列，展示了强大的模式识别能力，发现了模数缩放规律和课程学习的必要性，揭示了模型通过比特旋转不变聚类实现表示迁移的机制。
...
TL;DR: 提出了一种计算Graves-Lai优化问题解的计算可行算法，用于多峰奖励的多臂老虎机问题
...
Result: 实现了渐近最优的多峰老虎机算法，相关代码已在GitHub上公开

Conclusion: 该工作为多峰奖励的多臂老虎机问题提供了首个计算可行的渐近最优解决方案
...
TL;DR: 提出了一个统一的共形预测框架，用于无限时域策略评估，在在线和离线策略设置中构建分布无关的回报预测区间。


<details>
  <summary>Details</summary>
Motivation: 在高风险强化学习设置中，可靠的uncertainty量化至关重要。需要解决未观测回报、时间依赖性和分布偏移等挑战。

Method: 将分布RL与共形校准相结合，基于截断rollouts的模块化伪回报构建，以及使用经验回放和加权子采样的时间感知校准策略。

Result: 在合成环境和基准环境（如Mountain Car）中的实验表明，该方法显著提高了覆盖率和可靠性，优于标准分布RL基线。

Conclusion: 该框架能够缓解模型偏差并恢复近似可交换性，即使在策略偏移下也能实现uncertainty量化，理论分析提供了考虑模型错误规范和重要性权重估计的覆盖保证。
...
TL;DR: 提出了一种新颖的L1范数正则化不定核逻辑回归模型，通过引入稀疏性增强可解释性和泛化能力，并开发了高效的近端线性化算法来解决非光滑和非凸优化问题。


<details>
  <summary>Details</summary>
Motivation: 在许多实际场景中，不定核比正定核能捕获更多领域特定的结构信息，但现有的不定核逻辑回归缺乏稀疏性，影响了模型的可解释性和泛化性能。

Method: 在不定核逻辑回归框架中引入L1范数正则化，开发了理论上有保证且计算高效的近端线性化算法来处理由此产生的非光滑和非凸优化问题。

Result: 在多个基准数据集上的实验结果表明，所提出的方法在准确性和稀疏性方面均表现出优越性能。

Conclusion: L1范数正则化不定核逻辑回归模型通过引入稀疏性，在保持不定核优势的同时显著提升了模型的可解释性和泛化能力。
...
TL;DR: 提出了一种针对不平衡数据中合成数据引入偏差的校正方法，通过从多数类中借用信息来提供一致估计量，提高预测准确性并避免过拟合。


<details>
  <summary>Details</summary>
Motivation: 不平衡数据中，合成数据方法虽然能缓解正负样本比例失衡问题，但由于合成数据依赖于观测数据且无法准确复制原始数据分布，将其简单视为真实数据会降低预测准确性。

Method: 提出偏差校正程序，通过从多数类中借用信息来估计合成数据引入的偏差，并开发一致估计量来校正这种偏差。该方法可扩展到不平衡多任务学习和因果推断等更广泛场景。

Result: 理论分析提供了偏差估计误差的界限和预测准确性的改进。在手写数字数据集上的模拟结果和数据分析证明了该方法的有效性。

Conclusion: 提出的偏差校正方法能有效减轻合成数据的不利影响，提高不平衡数据分类的预测准确性，同时避免过拟合问题。
...
TL;DR: 提出基于图神经网络的二次规划求解框架，通过实例特定投影减少变量维度，使用双层优化训练模型，无需通过求解器反向传播，在计算时间和求解质量上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统二次规划求解器在高维问题中计算成本高，需要开发能够高效处理高维QP问题的数据驱动方法。

Method: 设计图神经网络生成实例特定投影矩阵，将高维QP投影到低维空间求解，采用双层优化训练模型，内层用QP求解器求解投影后问题，外层更新模型参数。

Result: 实验结果表明该方法能生成高质量可行解，显著减少计算时间，优于现有方法。

Conclusion: 所提出的数据驱动框架能够有效解决高维二次规划问题，通过实例特定投影和神经网络生成机制实现了计算效率和解质量的平衡。
...
TL;DR: 提出PILE评分作为物理信息机器学习中的单一不确定性感知指标，用于解决多目标优化中的模糊性问题，并指导超参数选择


<details>
  <summary>Details</summary>
Motivation: 现有物理信息机器学习方法同时包含数据和物理约束的多目标性质导致模型质量评估模糊，与认知不确定性理解不足相关，可能产生意外失败模式

Method: 在高斯过程回归框架内引入物理信息对数证据（PILE）评分，绕过测试损失的模糊性，提供PIML模型超参数的选择原则

Result: PILE最小化能为各种模型参数（包括核带宽、最小二乘正则化权重和核函数选择）提供优秀选择，数据无关的PILE评分还能识别适合特定PDE的先验核选择

Conclusion: PILE评分可扩展到更广泛的PIML领域，为物理信息机器学习提供统一的模型评估和选择框架
...
TL;DR: 提出了多输出鲁棒共轭高斯过程（MO-RCGP），扩展了RCGP框架来处理多输出回归中的异常值和模型误设问题，同时保持共轭性和输出间相关性建模能力。


<details>
  <summary>Details</summary>
Motivation: 多输出高斯过程对模型误设和异常值敏感，特别是当多个异常响应变量存在时，误差会通过输出间的相关性传播，需要更鲁棒的方法。

Method: 扩展和推广了Altamirano等人提出的鲁棒共轭高斯过程框架，构建了多输出RCGP模型，能够联合捕获输出间的相关性。

Result: 在金融和癌症研究应用中进行了全面评估，证明了方法的有效性。

Conclusion: MO-RCGP是一个可证明鲁棒的多输出高斯过程，具有共轭性，能够联合建模输出间的相关性，有效处理异常值和模型误设问题。
...
<details>
  <summary>Details</summary>
Motivation: 动作在强化学习和随机过程建模中都是核心概念，它们触发状态转换并促进复杂系统中的信息流动。本文旨在统一这两个领域的视角，通过动作驱动过程来连接随机过程和强化学习。

Method: 采用动作驱动过程框架，利用控制即推理的思想，通过最小化策略驱动的真实分布与奖励驱动的模型分布之间的Kullback-Leibler散度。

Result: 证明了最小化KL散度等价于最大熵强化学习，为脉冲神经网络等应用提供了理论基础。

Conclusion: 动作驱动过程成功统一了随机过程和强化学习的视角，为复杂系统的建模和控制提供了新的理论框架。
...
TL;DR: 该研究评估了因果机器学习模型在条件可交换性假设违反时的表现，以及阴性对照结果作为诊断工具的有效性。研究发现当存在未测量混杂时，因果森林和X-learner模型无法准确恢复真实治疗效果异质性，而阴性对照结果能有效识别受未测量混杂影响的亚组。


<details>
  <summary>Details</summary>
Motivation: 开发因果机器学习模型预测个体化治疗效果的观察性研究很少对条件可交换性假设进行实证评估。本研究旨在评估这些模型在条件可交换性违反时的表现，以及阴性对照结果作为诊断工具的有效性。

Method: 通过模拟研究，在不同条件下（包括是否存在真实异质性）检查因果森林和X-learner模型生成的个体化治疗效果估计中的混杂偏倚。模拟数据反映了现实世界场景，包括不同水平的混杂、样本量和阴性对照结果混杂结构。

Result: 当条件可交换性被违反时，因果森林和X-learner模型无法恢复真实的治疗效果异质性，在某些情况下甚至错误地指示存在异质性。阴性对照结果成功识别了受未测量混杂影响的亚组，即使不完全满足理想假设，仍能标记亚组水平估计中的潜在偏倚。

Conclusion: 条件可交换性的违反严重限制了常规收集的观察性数据中因果机器学习模型个体化治疗效果估计的有效性。阴性对照结果作为检测亚组特异性未测量混杂的实证诊断工具很有用，应纳入因果机器学习工作流程以支持个体化推断的可信度。
...
TL;DR: 本文证明了策略学习中经验福利最大化(EWM)方法和插件方法在本质上是等价的，都基于相同的优化问题，并提出了一种新的正则化方法。
...
Method: 通过数学证明建立了EWM和最小二乘法在策略类重参数化下的精确等价关系，并基于此提出了新的正则化方法。

Result: 证明了两种方法的等价性，提出了计算效率更高的凸优化训练过程，避免了EWM中NP难的组合步骤。

Conclusion: 两种策略学习方法本质上是等价的，可以相互替代，基于等价性提出的新方法具有更好的计算效率。
...
TL;DR: 本文提出了一个统一的因果推断理论框架，整合了Riesz回归、协变量平衡、密度比估计、TMLE和匹配估计器在平均处理效应估计中的应用。


<details>
  <summary>Details</summary>
Motivation: 为了统一因果推断中的多种方法，揭示它们之间的内在联系和等价关系，特别是在平均处理效应估计中平衡权重和回归函数的作用。

Method: 通过理论分析建立Riesz回归、协变量平衡、密度比估计、TMLE和匹配估计器之间的等价关系，展示它们在估计平衡权重和回归函数时的内在联系。

Result: 发现Riesz回归与密度比估计在ATE背景下本质等价，匹配估计器是密度比估计的特例，密度比估计与协变量平衡存在对偶关系，TMLE用于构造使主要偏差项为零的回归函数估计器。

Conclusion: 这些看似不同的因果推断方法在理论上是统一的，它们通过不同的方式处理平衡权重和回归函数，最终都服务于准确估计平均处理效应的共同目标。
