...
Method: 提出了基于介质波导、漏泄同轴电缆、表面波导结构等多种物理实现的广义夹持天线系统，采用机械、电子或混合激活机制，在信号引导介质上形成、重新定位或停用辐射点。

Result: 建立了代表性实现的无线信道模型，展示了与传统天线相比独特的传播和可重构特性，并开发了从单波导到多波导配置的系统架构。

Conclusion: 广义夹持天线系统为下一代无线网络提供了用户中心和动态覆盖能力，但仍面临开放研究挑战，需要进一步探索实际部署路径。
...
<details>
  <summary>Details</summary>
Motivation: 需要同时实现C+L波段系统的精确建模和拉曼泵浦设计，传统方法难以兼顾准确性和效率。

Method: 采用基于迁移学习的Transformer框架，利用Transformer的序列建模能力和迁移学习来提高性能。

Result: 建模的RMSE在0.22 dB以内，峰值到峰值GSNR变化/偏差分别为0.86/0.1 dB。

Conclusion: 该框架能够有效实现C+L波段系统的精确建模和拉曼泵浦设计，性能优于传统方法。
...
Method: 提出基于混合消息传递的框架，通过多重近似来降低计算复杂度，处理用户信道与目标位置之间的复杂依赖关系。
...
<details>
  <summary>Details</summary>
Motivation: 随着数据流量需求增长放缓，移动行业无法仅依赖通信服务维持发展，需要寻找新的增长点。ISAC通过在通信网络中嵌入感知能力，扩展网络功能，支持多样化应用。

Method: 提供ISAC领域的鸟瞰视角，重点分析关键挑战、机遇和应用场景，为研究人员提供指导。

Result: ISAC代表了无线网络的范式转变，将网络角色从单纯数据传输扩展到支持多样化应用的多功能平台。

Conclusion: ISAC是无线网络发展的变革性解决方案，为移动行业提供了新的发展方向，需要进一步探索其关键挑战和应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 利用ZP辅助的时域信道矩阵下三角结构，设计低复杂度的接收机检测器以降低计算复杂度。

Method: 提出了基于最小均方误差(MMSE)的检测器和基于最大比合并的时域(MRC-TD)检测器，并分析了MMSE和最大似然检测器的理论BER性能。

Result: 仿真结果表明，所提出的检测器能够达到与传统基于矩阵求逆的MMSE检测器相同的BER性能，同时显著降低了复杂度。

Conclusion: ZP-AFDM系统可以通过利用时域信道矩阵的特殊结构实现低复杂度高性能的接收机设计。
...
<details>
  <summary>Details</summary>
Motivation: 传统混合波束成形系统存在自由度受限的问题，影响多用户MIMO通信和MIMO雷达感知的性能

Method: 设计了三混合波束成形优化框架，结合数字、模拟和电磁波束成形，采用分数规划和流形优化方法将问题转化为可处理的子问题

Result: 仿真结果表明，与采用全向天线的传统混合波束成形系统相比，所提出的ERA-ISAC系统在感知和通信性能上实现了近10dB的增益

Conclusion: 电磁可重构天线能够有效增强ISAC系统的自由度，显著提升通信和感知性能
...
<details>
  <summary>Details</summary>
Motivation: 为计算传感领域的研究人员提供一个论坛，让他们能够学习、发现和交流新的发现和挑战，特别是在看似不相关的应用领域之间。

Method: 为期3天的研讨会，设有6位主题演讲嘉宾，接受科学演示和展示-讲述演示的扩展摘要。

Result: 成功汇集了光学显微镜、电子显微镜、雷达、天文成像、生物医学成像、遥感以及信号处理等领域的研究人员。

Conclusion: 该研讨会为计算传感领域的研究人员提供了一个宝贵的交流平台，促进了跨学科的合作与知识共享。
...
<details>
  <summary>Details</summary>
Motivation: 现有流体天线理论分析往往假设瞬时重构、完美信道知识等理想条件，但这些假设在实践中很少成立，导致性能评估过于乐观。

Method: 通过对比分析理论假设与实际限制，使用示例和仿真展示性能差异，并提出建模改进、实验验证方法和新型控制算法。

Result: 研究表明忽略实际约束会高估容量、覆盖等性能增益，只有考虑有限驱动时间、不完美信道状态信息等现实因素才能准确评估性能。

Conclusion: 流体天线在B5G/6G和物联网应用中仍极具前景，但必须将实际约束纳入系统设计和性能评估才能充分发挥其潜力。
...
<details>
  <summary>Details</summary>
Motivation: 传统MVDR波束成形在大规模阵列中计算复杂度高（立方级增长），难以实时应用，特别是在信号低于噪声底层的场景（如GPS）。

Method: 利用Sherman-Morrison公式、低秩SVD近似和代数操作，针对信号低于噪声底层的场景设计可扩展算法。
...
Method: 利用载波频率偏移在波束扫描同步信号中的恒定性，通过最大似然估计框架相干聚合多个观测信息来增强目标信号。

Result: Cramer-Rao下界分析和仿真表明，即使信号比干扰弱一千倍以上，仍能实现可靠估计。低空无线电地图案例研究验证了框架的实际有效性。

Conclusion: 该框架能够有效估计被强干扰淹没的弱信号，为无线环境完整感知提供了可行解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决ISI信道中的时间色散导致的符号间干扰问题，以及均衡带来的噪声着色问题，需要开发能够有效处理有色噪声的解码方法。

Method: 基于统计物理学中的近似独立性思想，开发ORBGRAND-AI解码器，使用有序可靠性比特猜测和随机加性噪声解码技术，不采用交织处理。

Result: 在二抽头双码ISI信道和RFView信道中，ORBGRAND-AI能够提供与CA-SCL解码器相当或更低的BLER性能，特别是在各种不完美信道状态信息条件下。

Conclusion: ORBGRAND-AI解码器能够有效处理ISI信道中的有色噪声，无需交织即可实现优异的解码性能，二阶自回归模型能够充分表征RFView信道效应。
...
<details>
  <summary>Details</summary>
Motivation: 物联网设备的快速扩张超过了现有识别方法的能力，在开放世界环境中，流量元数据通常不完整、有噪声或被故意混淆，带来了严重的安全、隐私和网络可追溯性风险。

Method: 使用基于互信息和熵的稳定性评分指导的集成大语言模型为IoT Inspector数据集生成高保真厂商标签，然后通过课程学习对量化LLaMA3.18B模型进行指令调优，以支持稀疏性和长尾厂商分布下的泛化能力。

Result: 模型在2015个厂商上实现了98.25%的top-1准确率和90.73%的宏观准确率，同时对缺失字段、协议漂移和对抗性操作保持弹性。

Conclusion: 指令调优的大语言模型为大规模现实世界设备识别提供了可扩展且可解释的基础。
...
<details>
  <summary>Details</summary>
Motivation: 传统渐进域适应方法通过中间域和自训练缓解域偏移，但存在知识迁移效率低或中间数据不完整的问题，需要更稳健的适应策略。

Method: 引入动态加权机制，使用时变超参数ϱ控制域特定学习强度，结合自训练生成伪标签，优化加权目标函数进行迭代模型更新。

Result: 在旋转MNIST、颜色偏移MNIST、肖像数据集和Cover Type数据集上的实验表明，STDW优于现有基线方法，消融研究验证了ϱ动态调度的重要性。

Conclusion: STDW为稳健的渐进域适应提供了理论见解和实用框架，在动态现实场景中具有应用潜力，能有效减少域偏差并提高泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统p-persistent CSMA协议在多跳异构无线网络中吞吐量预测困难，简化模型低估48-62%吞吐量，精确马尔可夫链分析计算复杂度指数增长，需要可扩展的机器学习方法。

Method: 提出解耦图卷积网络(D-GCN)，用可学习注意力机制替代均值聚合，显式分离节点自身传输概率和邻居干扰效应，捕获复杂多跳干扰模式。

Result: D-GCN达到3.3%归一化平均绝对误差，优于强基线方法，在精确分析方法计算不可行时仍可处理，基于梯度的网络优化达到理论最优值的1%以内。

Conclusion: D-GCN通过结构化解耦设计有效解决了无线网络中干扰传播的建模问题，为大规模网络吞吐量预测和优化提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 假设神经网络在深度特征的高频分量中编码任务相关语义信息，在低频分量中存储领域特定偏见，因此需要分离出可泛化的表示。
...
Result: 在多个基准CL和FS任务上验证了有效性，达到SOTA性能但资源需求大幅降低。

Conclusion: CoLoR-GAN是一个高效且资源友好的持续少样本学习框架，能够有效平衡性能和参数效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决在单一框架中同时实现鲁棒分类和高质量生成建模的挑战，克服传统联合能量模型(JEM)中SGLD训练不稳定和样本质量差的限制。

Method: 采用对抗训练原则，包括：1)用基于对抗训练的稳定方法替代SGLD学习；2)协同对抗训练增强分类鲁棒性；3)两阶段训练解决批归一化与EBM训练的不兼容性。

Result: 在CIFAR-10、CIFAR-100和ImageNet上显著提升对抗鲁棒性，生成质量达到BigGAN水平并接近扩散模型，首次实现基于MCMC的EBM在复杂高分辨率数据集上的高质量生成。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在图像理解方面表现出色，但在长视频处理中受到上下文窗口和计算成本的限制。均匀帧采样会导致信息丢失，而现有的关键帧选择方法通常产生稀疏且时间不连续的帧，忽略了场景连续性且缺乏多尺度帧选择的灵活性。

Method: 首先构建了包含20万个视频高光片段的PeakClips数据集，然后采用三阶段渐进式课程学习：两个监督微调阶段用于时间定位和关键片段感知，接着是强化学习阶段直接优化场景驱动预测策略，无需额外标注。

Result: 在主要的长视频理解基准测试中，K-frames提供了有效、可解释且即插即用的解决方案，支持各种尺度的关键帧选择。

Conclusion: K-frames通过场景驱动的关键帧选择范式，解决了现有方法在保持时间连续性和多尺度灵活性方面的局限性，为长视频理解提供了更好的关键帧选择方案。
...
Method: 将深度神经网络解释为动态系统，通过求解近似动态规划的一阶展开，探索贝尔曼方程的高阶展开，开发出OCNOpt优化器，支持层间反馈策略、博弈论应用和连续时间模型的高阶训练。

Result: 大量实验表明，OCNOpt在保持可管理计算复杂度的同时，在鲁棒性和效率方面优于现有方法，为基于动态系统和最优控制理论的原则性算法设计开辟了新途径。

Conclusion: 通过将反向传播与动态规划联系起来，OCNOpt提供了一种基于最优控制理论的新型优化框架，能够改进神经网络训练的鲁棒性和效率，并为原则性算法设计提供了新的可能性。
...
<details>
  <summary>Details</summary>
Motivation: 现有标签分布学习方法仅针对单视图有标签数据，而多视图场景下同时包含有标签和无标签数据的问题尚未被考虑

Method: 通过计算每个视图的k近邻，利用其他视图的局部结构信息来补充当前视图的近邻集，构建基于图学习的多视图半监督LDL模型

Result: 数值研究表明MVSS-LDL比现有单视图LDL方法获得明显更好的分类性能

Conclusion: 这是首次尝试多视图标签分布学习，通过局部近邻结构互补性，不同视图可以相互提供局部结构信息来补充彼此
...
<details>
  <summary>Details</summary>
Motivation: 解决强化学习在训练和部署环境分布偏移下的决策问题，通过鲁棒马尔可夫决策过程框架优化对抗性转移动态下的性能。

Method: 结合参考策略正则化，在软最大策略类中实现可优化性；采用d-矩形线性MDP公式，结合线性函数逼近和上置信界奖励进行乐观探索。

Result: 理论证明策略优化在鲁棒强化学习中可实现多项式次优性边界和样本效率，与基于价值的方法性能相当；实证结果验证了DR-RPO的鲁棒性。

Conclusion: DR-RPO算法成功地将策略优化方法扩展到鲁棒强化学习领域，提供了理论保证和实证验证，填补了该领域的研究空白。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型合并方法严重依赖缩放超参数λ，但缺乏无需数据的原则性设置方法，实践中往往需要调优λ，这在实际中不可行。

Method: 提出Weight Weaving技术，使用用户定义的池化函数（如平均、随机选择或现有合并方法）在λ搜索空间中对模型权重进行池化。

Result: 在三个ViT变体的三种实验设置中，该方法持续改进多个模型合并方法的性能，在无需数据设置下平均准确率提升高达15.9个百分点。

Conclusion: Weight Weaving具有高模块化特性，对搜索空间约束最小，与现有模型合并方法正交运行，无需评估数据要求。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型归因方法需要模型梯度或重训练，限制了在专有或大规模场景下的应用，需要开发更灵活的非参数方法

Method: 基于数据驱动的非参数归因方法，通过计算生成图像与训练图像在块级相似性来衡量影响，基于最优得分函数的解析形式，支持多尺度表示并通过卷积加速

Result: 实验表明该方法达到强大的归因性能，与基于梯度的方法表现相当，显著优于现有非参数基线方法

Conclusion: 该方法提供空间可解释的归因结果，揭示了训练数据与输出之间的内在关系，独立于特定模型，计算高效且适用性广
...
Method: 从检索系统角度处理该任务，将其制定为分类和排序问题，同时考虑代码分配和优先级排序。

Result: 模型在正确排序主要诊断代码方面的准确率达到47%，优于现有最佳分类器的20%；在分类指标上，微平均和宏平均F1分数分别为0.6065和0.2904，超过之前最佳模型。

Conclusion: 提出的分类与排序框架在识别高优先级代码方面表现出色，显著提升了ICD代码自动分配的准确性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现实环境中测量状态可能成本高昂且对后续结果产生负面影响，需要权衡测量成本与状态信息获取的收益。

Method: 将AOMDP建模为周期性部分可观测MDP，提出基于信念状态的在线RL算法，使用序列蒙特卡洛方法联合估计环境参数和潜在状态的后验分布。

Result: 在数字健康应用中评估算法，智能体决定何时提供数字干预和何时通过调查评估用户健康状态。

Conclusion: 尽管测量有成本，但减少的不确定性可以显著提高样本效率并增加最优策略的价值。
...
<details>
  <summary>Details</summary>
Motivation: 传统的数据保真度损失函数如均方误差或负对数似然寻求与噪声测量的点对点一致性，往往导致对噪声的过度拟合。需要一种能够避免过度拟合噪声的新方法。

Method: 引入分布一致性损失，用基于模型的概率评分对每个测量值进行分布级校准，替代传统的点对点匹配。该损失与现代正则化器兼容，优化方式与传统损失相同。

Result: 在图像去噪中，使用DC损失替代MSE损失无需提前停止训练即可获得更高PSNR；在医学图像重建中，DC损失减少了高度迭代重建中的伪影，并增强了手工正则化的效果。

Conclusion: DC损失为逆问题提供了一个统计基础扎实、性能提升的替代方案，能够避免测量噪声的过度拟合，适用范围包括测量噪声分布已知且测量数据集包含许多独立噪声值的实际逆问题。
...
<details>
  <summary>Details</summary>
Motivation: 为自动微分理论提供几何基础，通过微分几何视角理解AD，为深度学习和科学计算中开发结构保持的微分方法奠定基础。
...
Conclusion: 该框架为自动微分理论提供了几何解释，并为开发结构保持的微分方法提供了理论基础，在计算成本上具有线性优势。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决大语言模型在部署时的高内存需求和计算成本问题，同时保持特定任务上的高性能表现。

Method: 结合三种关键技术：SubLN模块、基于MiniLM的多头注意力蒸馏，以及作为关键预热步骤的持续预训练。

Result: 在多种模型规模上达到与全精度对应模型相当的性能，同时实现高达10倍的内存节省和2.65倍的CPU推理加速。

Conclusion: BitDistill提供了一种高效的方法来部署轻量级但高性能的1.58位精度大语言模型，特别适用于资源受限环境。
...
<details>
  <summary>Details</summary>
Motivation: 现有的药物-靶点亲和力(DTA)预测评估方法主要关注亲和力强度预测精度，但忽略了交互方向预测的重要性。正确捕捉交互作用对于药物分配决策至关重要。

Method: 引入IC-index评估交互方向预测性能，证明预测器对药物和靶点身份的置换等变性会导致无法捕捉交互作用，需要通过整合药物和靶点的侧信息来克服这一局限性。

Result: 在多个生物医学交互数据集上的实验表明，不同类型的亲和力强度预测方法在IC-index上的表现各异，补充了现有预测性能评估指标。

Conclusion: IC-index为DTA预测提供了重要的补充评估维度，强调了在机器学习模型中整合适当侧信息对于捕捉交互作用的重要性。
...
<details>
  <summary>Details</summary>
Motivation: SMoE模型虽然训练高效、延迟低，但参数量大导致内存开销显著，需要研究专家压缩方法。

Method: 提出Router-weighted Expert Activation Pruning (REAP)剪枝准则，综合考虑路由器门值和专家激活范数。

Result: 在20B到1T参数的多种SMoE模型上，REAP在生成基准测试中始终优于合并和其他剪枝方法，特别是在50%压缩率下。在代码生成和工具调用任务中，即使剪枝50%专家也能实现接近无损的压缩。

Conclusion: 专家剪枝是生成任务的更优压缩策略，REAP方法能有效避免合并导致的功能子空间坍塌问题。
...
Method: 通过将矩匹配条件转化为关于边际矩、先验超参数和秩的对数线性方程组，建立秩可识别性与方程组可解性之间的等价关系，并应用于四种基础张量模型。

Result: 证明PARAFAC/CP模型、张量链模型和张量环模型的秩是可识别的，而Tucker模型的秩由于对称拓扑导致系统欠定而不可识别。对于可识别模型，推导了基于观测数据矩的显式闭式秩估计器。

Conclusion: 该方法为张量分解中的秩选择提供了严格的理论框架，通过矩匹配方法成功识别了多种张量模型的秩，并验证了所提出估计器的有效性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Clifford-可操纵CNNs（CSCNNs）的核基不完备，限制了模型的表达能力，需要解决这一问题以提升在伪欧几里得群等变学习中的性能。

Method: 提出条件Clifford-可操纵核，通过输入特征场的等变表示来增强核函数，推导等变约束条件，并采用隐式参数化方法高效求解。

Result: 在流体动力学和相对论电动力学等多个PDE预测任务中，该方法持续优于基线方法，证明了其改进的表达能力。

Conclusion: 条件Clifford-可操纵核有效解决了CSCNNs核基不完备的问题，显著提升了模型在等变学习任务中的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 虽然对于SGD，加倍批量大小相当于将学习率减半，但对于Adam等自适应优化器的最优批量大小调度策略尚不明确，通常需要启发式调优。

Method: 开发了批量大小调度的原则性框架Seesaw：当标准调度器要减半学习率时，Seesaw将学习率乘以1/√2并加倍批量大小。
...
<details>
  <summary>Details</summary>
Motivation: 现有几何感知优化器对同一组内的层使用固定学习率，但实际训练中不同层的局部曲率和锐度存在异质性且动态变化，这可能导致训练效率低下

Method: 在几何感知优化算法基础上，实时估计由线性最小化oracle诱导的对偶范数中的梯度方差，并据此为每组内的层分配时变的噪声自适应分层学习率

Result: 理论分析显示算法达到尖锐收敛率，在LLaMA和GPT等transformer架构上的实证结果表明比最先进优化器收敛更快

Conclusion: 噪声自适应分层学习率方案能有效加速深度神经网络训练，特别是在处理层间异质性和动态变化的局部曲率时表现优异
...
<details>
  <summary>Details</summary>
Motivation: 现有方法假设潜在混杂变量相互独立，或无法正确处理观测变量间存在因果关系的模型，需要开发能够处理更复杂因果结构的算法。

Method: 利用观测数据的高阶累积量来识别因果结构，允许潜在变量间、观测变量间以及两者之间的因果关系。

Result: 广泛的模拟实验和真实世界数据实验验证了所提算法的有效性和实用性。

Conclusion: 该算法能够成功识别包含潜在混杂变量的线性非高斯模型中的因果有向无环图，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: Transformer模型存在二次复杂度问题和长序列依赖处理困难，而现有状态空间模型（如S6）的选择性机制仅依赖当前输入，缺乏对序列历史的有效利用。

Method: COFFEE模型通过状态反馈机制，从内部状态计算选择性，使模型能够基于累积上下文调节动态特性；同时采用高效参数化方法消除S6中的冗余。

Result: 在induction head任务中，COFFEE以少两个数量级的参数和训练序列达到接近完美的准确率；在MNIST上仅用3585参数就达到97%准确率，大幅超越S6。
...
Method: 利用梯度下降的隐式偏置特性，通过梯度上升步骤来逆转特定数据点的影响，并基于KKT条件评估遗忘质量。

Result: 在线性模型和两层神经网络中，适当缩放的梯度上升步骤能够满足新的成功遗忘标准，产生与在保留数据上重新训练相近的模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有因果表示学习的评估方法存在两难困境：要么使用过于简化的合成数据集，要么依赖现实任务的下游性能，缺乏既保持视觉复杂性又能获取真实因果生成过程的方法。

Method: 创建包含静态图像生成、动态物理模拟、机器人操作和交通情况分析四个领域的高保真模拟数据集，提供对底层因果结构的灵活访问，允许用户修改配置以符合CRL的不同假设要求。

Result: 构建了包含24个子场景的综合性测试平台，范围从静态到动态设置、简单到复杂结构、单到多智能体交互，并评估了代表性CRL方法在不同范式下的表现。

Conclusion: 该基准为从业者和新手提供了选择或扩展适当CRL框架的经验见解，帮助解决能从CRL视角受益的特定类型现实问题，弥合了严谨评估与现实应用之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 传统IRT方法使用非线性链接函数会破坏TVD-MI比较的加性结构，引入不必要的曲率，影响评估的准确性和效率。

Method: 基于Gini熵最大化推导出裁剪线性模型，采用盒约束最小二乘公式处理边界饱和问题，使用恒等链接函数保持加性结构。

Result: 在33%覆盖率下，获得0.117±0.008的留一RMSE，同时保持代理排名一致性（Spearman ρ=0.972±0.015），比密集评估减少三分之二的评估次数。

Conclusion: 恒等映射能最佳保持TVD-MI的几何结构，为大语言模型评估提供高效方法，也适用于其他有界响应领域。
...
<details>
  <summary>Details</summary>
Motivation: 解决个性化自然语言理解应用中面临的两个主要挑战：(i) 由于数据保密性或隐私要求导致的有限和/或异构数据，(ii) 边缘设备等参与客户端之间计算资源的差异。
...
Conclusion: FedHFT框架能够有效解决联邦学习中的数据异构和资源异构问题，在保持专有数据本地化的同时实现高性能的协作微调。
...
<details>
  <summary>Details</summary>
Motivation: 研究稀疏maxout网络的表达能力，这类网络捕捉了卷积神经网络和图神经网络的关键特征，旨在理解其计算能力和深度-宽度权衡。

Method: 建立了稀疏maxout网络计算函数与一类虚拟多面体之间的对偶关系，利用多面体几何分析网络表达能力，推导了相关多面体维数的紧界。

Result: 构建了深度层次结构序列，证明了足够深的稀疏maxout网络具有通用性，但在固定入度约束下，如果深度不足，仅靠增加宽度无法弥补稀疏性限制。

Conclusion: 稀疏maxout网络的表达能力与深度密切相关，在固定稀疏约束下，深度是实现通用逼近能力的关键因素，宽度无法完全替代深度的作用。
...
Method: 使用预训练基础模型将非结构化数据转化为有意义的表示，通过稀疏自编码器进行解释，并采用递归分层策略解决多重检验和效应纠缠问题。

Result: 在半合成实验中验证了算法的鲁棒性，并在实验生态学背景下首次成功实现了真实世界科学试验中的无监督因果效应识别。

Conclusion: Neural Effect Search能够从数据中直接发现未知的因果效应，为大规模因果发现提供了新途径。
...
Result: 证明了双层神经网络足以逼近一维反应-扩散方程，三层神经网络足以逼近二维反应-扩散方程，该框架可扩展到椭圆和抛物型方程。
...
<details>
  <summary>Details</summary>
Motivation: 系统性组合泛化是机器学习的核心挑战，也是现代语言模型推理能力的关键瓶颈。论文旨在解决Transformer网络在超出训练分布外的泛化问题。

Method: 提出了四种架构机制：输入自适应循环、算法监督、通过离散瓶颈的锚定潜在表示、显式纠错机制。使用GSM8K风格的模块化算术计算图任务作为测试平台。

Result: 这些机制共同形成了Transformer网络中可扩展的潜在空间推理架构方法，具有强大的算法泛化能力。通过机制可解释性分析揭示了这些机制如何产生稳健的OOD泛化能力。

Conclusion: 提出的架构机制能够有效增强Transformer网络在超出训练分布外的系统性组合泛化能力，为稳健的算法泛化提供了解决方案。
...
Method: TENDE方法利用基于分数的扩散模型学习相关条件分布的概率密度函数，通过条件互信息估计转移熵

Result: 在合成基准测试和真实数据上，TENDE相比现有神经估计器和其他最先进方法展现出更优的准确性和鲁棒性

Conclusion: TENDE提供了一种灵活、可扩展的转移熵估计方法，对底层数据生成过程做出最小假设
...
<details>
  <summary>Details</summary>
Motivation: 在双边市场中，价格敏感的客户和服务器动态到达并加入队列，平台需要设计定价和匹配算法来最大化利润，同时保持合理的队列长度。由于实践中需求和供给曲线可能未知，需要开发在线学习方法来应对这一挑战。

Method: 提出了一种新颖的基于在线学习的定价策略，该策略具有两个关键特征：动态组件优化低遗憾和小队列长度之间的权衡；概率组件解决获取有用样本进行快速学习与保持小队列长度之间的张力。

Result: 证明了在γ∈(0,1/6]范围内，实现了Õ(T^{1-γ})遗憾、Õ(T^{γ/2})平均队列长度和Õ(T^{γ})最大队列长度的权衡，显著优于现有结果。在允许的γ范围内，这种遗憾与平均队列长度的权衡在对数因子内是最优的。

Conclusion: 所提出的在线学习定价策略在双边市场中实现了近乎最优的性能，在平台利润最大化和队列长度控制之间取得了良好平衡，特别适用于需求和供给曲线未知的实际情况。
...
Result: 展示了这些方法主要采用扭曲机制处理扩散过程中的中间分布，并使用各种蒙特卡洛方法来辅助从这些扭曲分布中采样。
...
TL;DR: 提出了一个神经网络驱动的鲁棒优化框架，将数据驱动领域作为约束集成到非线性规划技术中，解决参数化神经网络模型与优化求解器交互时产生的领域不一致解问题。
...
Method: 开发了一个神经网络驱动的鲁棒优化框架，将数据驱动领域作为约束集成到非线性规划技术中。

Result: 应用于1180 MW联合循环燃气电厂，实现了域一致的鲁棒最优解，验证了能源效率平均提高0.76个百分点。将该效率增益扩展到全球燃气电厂，估计每年可减少2600万吨CO₂排放。
...
TL;DR: 本文首次揭示了无监督强化学习中涌现探索的机制，通过理论分析和实验验证了SGCRL算法如何通过学习的表征自动塑造奖励环境来促进探索。


<details>
  <summary>Details</summary>
Motivation: 理解无监督强化学习中涌现探索的机制，特别是SGCRL算法如何在无外部奖励或课程的情况下解决长时程目标达成任务。

Method: 结合理论分析和受控实验，研究SGCRL算法的目标函数，分析其如何通过学习的表征最大化隐式奖励。

Result: SGCRL通过学习的状态空间低秩表征自动修改奖励环境，在到达目标前促进探索，到达后促进利用。这些探索动态源于低秩表征学习而非神经网络函数逼近。
...
TL;DR: 提出ProtGram-DirectGCN框架，通过层次化n-gram图和定向图卷积网络进行蛋白质-蛋白质相互作用预测，在有限训练数据下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有蛋白质-蛋白质相互作用预测方法计算成本高，需要探索更高效的替代方案。

Method: 两阶段图表示学习框架：ProtGram将蛋白质一级结构建模为层次化n-gram图，DirectGCN使用定向图卷积网络学习残基级嵌入，通过注意力池化生成蛋白质级嵌入。

Result: DirectGCN在标准节点分类基准上表现与现有方法相当，在复杂定向图上表现优异；完整框架在PPI预测中展现出强大的预测能力。

Conclusion: ProtGram-DirectGCN框架为蛋白质-蛋白质相互作用预测提供了高效且强大的解决方案，特别适合数据有限的情况。
...
TL;DR: 本文系统评估了不同损失函数对Transformer模型在股票收益排序任务中的影响，为基于排名的投资组合选择提供实用指导。


<details>
  <summary>Details</summary>
Motivation: 量化交易策略依赖准确的股票排序来识别盈利投资。传统损失函数注重预测精度而非排序能力，且缺乏对现代Transformer模型在金融排序任务中损失函数效果的全面比较。

Method: 在S&P 500数据上，系统评估了包括点对点、成对、列表式在内的多种先进损失函数，用于每日股票收益预测，以促进基于排名的投资组合选择。

Result: 研究提供了全面的基准测试，揭示了不同损失函数如何影响模型学习投资组合选择所需的横截面和时间模式的能力。

Conclusion: 该研究为优化基于排名的交易策略提供了实用指导，填补了现代Transformer模型在金融收益排序任务中损失函数效果比较的空白。
...
TL;DR: 本文调查了传统数据表征方法的局限性，并探讨了张量方法作为更鲁棒替代方案的潜力，通过实例展示张量方法如何揭示细微数据特征并提供更好的可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有的统计、结构和基于模型的数据集表征方法往往无法提供创新和可解释性所需的深度理解和洞察。

Method: 调查当前最先进的传统数据分析技术，讨论各种基于张量的方法，并通过实例说明张量方法的应用。

Result: 张量方法能够揭示微妙的数据特征，提供增强的可解释性和可操作智能。
...
TL;DR: 提出了可逆模型合并(RMM)方法，通过构建紧凑基空间而非单一合并权重，解决了低秩压缩模型合并时的性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 传统模型合并方法应用于低秩权重时会导致严重的性能下降，需要新的方法来解决这一问题。

Method: 构建紧凑基空间，通过线性组合恢复原始任务特定模型，提供闭式解选择最优权重基和任务特定系数。

Result: RMM在多样化数据集和模型规模上始终优于现有合并方法，显著保留了低秩压缩模型的性能。

Conclusion: 将合并重新定义为生成可重构模型空间而非单一合并模型，允许在需要时恢复到各个专用模型。
...
Method: 结合专门化智能体与结构化推理，采用基于裁判的共识机制，支持通过配置而非代码更改来定义自定义标注类型，实现动态任务适应。

Result: 在摩根大通部署后消除100万条话语积压，平均与人工标注者达成86%一致，每年节省5000多小时人工工作；在内部意图分类数据集上比传统方法提高13.8% Top-1准确率、15.1% Top-5准确率和16.9% F1分数。

Conclusion: MAFA弥合了理论多智能体系统与实际企业部署之间的差距，为面临类似标注挑战的组织提供了蓝图，使人工标注者能够专注于模糊和低覆盖率的案例。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型擅长生成，但其潜在空间未明确组织用于可解释控制。受对比学习能恢复更解缠和结构化表示的启发，需要组织扩散潜在空间以反映底层动态。
...
TL;DR: 本文分析了联邦学习中的激励机制设计挑战，探讨了经济学和博弈论概念在联邦学习中的应用，以及区块链和深度强化学习等技术解决方案，提出了涵盖集中式和去中心化架构的综合分类法。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽然能保护数据隐私，但面临参与困境问题——参与实体不愿贡献资源或可能搭便车，因此需要设计有效的激励机制来确保系统的可持续性和公平性。

Method: 应用经济学和博弈论概念，结合区块链和深度强化学习等技术，提出基于理论概念的综合分类法，涵盖集中式和去中心化架构。

Result: 建立了全面的激励机制分类体系，展示了激励机制在医疗、智能基础设施、车载网络和区块链去中心化系统等工业应用中的重要性。

Conclusion: 精心设计的激励机制对于联邦学习的实际成功至关重要，目前已有有前景的解决方案，但在构建真正可持续、公平和稳健的联邦学习生态系统方面仍面临重大挑战。
...
TL;DR: 对分子核进行首次全面谱分析，发现更丰富的谱特征并不总是提高预测精度，甚至可能与性能负相关。仅保留前2%特征值即可恢复几乎所有性能，挑战了"更丰富谱带来更好泛化"的启发式观点。
...
Method: 在QM9数据集上对分子指纹、预训练transformer、全局和局部3D表示等七种分子性质进行核岭回归的全面谱分析，使用四种不同谱度量指标，并通过截断核来探究谱与预测性能的关系。

Result: 令人惊讶的是，更丰富的谱特征并不一致提高精度。对于transformer和局部3D表示，谱丰富度甚至与性能呈负相关。在多数核中，仅保留前2%特征值即可恢复几乎所有性能。

Conclusion: 研究结果挑战了"更丰富谱带来更好泛化"的常见启发式观点，揭示了表示、核特征与预测性能之间的微妙关系。这些发现对数据受限的科学和实际任务中如何评估核和自监督学习方法具有指导意义。
...
TL;DR: 论文通过严格形式化分析，揭示了平坦极小值与对抗鲁棒性的复杂关系：平坦性仅保证局部鲁棒性而非全局鲁棒性，且对抗样本常位于模型自信但错误的平坦区域。


<details>
  <summary>Details</summary>
Motivation: 尽管神经网络经验上成功，但仍易受微小对抗扰动攻击。长期以来假设平坦极小值（损失景观中低曲率区域）能提高鲁棒性，但这种联系缺乏严格形式化分析。

Method: 首先推导倒数第二层相对平坦度的闭式表达式，利用此约束损失在输入空间的变化，从而形式化分析整个网络的对抗鲁棒性。通过理论和实验验证，揭示对抗脆弱性的几何结构。

Result: 平坦性仅能保证局部邻域内的对抗鲁棒性，要维持更大范围的鲁棒性，损失需要在数据流形外急剧弯曲。实验验证了理论预测，发现对抗样本常位于模型自信但错误的大平坦区域。

Conclusion: 研究挑战了对平坦性的简化理解，提供了关于平坦性在鲁棒性中作用的细致认识：平坦性仅提供局部保护，全局鲁棒性需要损失景观的特定几何结构。
...
TL;DR: GenCluster是一个可扩展的测试时计算框架，首次使用开源模型在IOI竞赛中达到金牌水平性能


<details>
  <summary>Details</summary>
Motivation: 虽然专有模型声称在IOI竞赛中达到金牌水平，但使用开源模型实现类似性能仍具挑战性，需要透明可复现的评估方法

Method: 结合大规模生成、行为聚类、排序和轮询提交策略，在有限验证预算下高效探索多样化解空间

Result: GenCluster性能随计算资源增加而稳定提升，使用gpt-oss-120b模型首次在IOI 2025中达到金牌水平

Conclusion: 该框架为LLM推理能力的透明可复现评估设立了新基准，缩小了开源与闭源系统之间的差距
...
TL;DR: 提出PhysAttnNet网络，通过物理先验引导的双流注意力机制，结合时间衰减和相位差异建模，提升弹性布拉格防波堤运动响应预测的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习模型在未见海况下泛化能力有限，主要由于忽略了海洋系统的自然衰减现象和波-结构相互作用建模不足。

Method: 使用衰减双向自注意力模块模拟自然衰减，相位差异引导的双向交叉注意力模块捕获波-结构相互作用，通过全局上下文融合模块整合双流，并采用时频混合损失函数训练。
...
TL;DR: 该论文比较了通用型和专业型时间序列基础模型在PPG信号分析中的性能，通过51个任务评估发现专业模型在完全微调场景下胜率高出27%。


<details>
  <summary>Details</summary>
Motivation: 随着基础模型在时间序列分析中应用增多，需要系统比较通用型（跨领域预训练）和专业型（同领域预训练）模型的性能差异，特别是在生理信号PPG分析领域。

Method: 使用51个任务套件全面评估两种模型，涵盖心脏状态评估、实验室值估计和跨模态推理，从7个维度（胜率、平均性能、特征质量等）进行对比分析。

Result: 在完全微调场景下，专业型模型胜率比通用型模型高出27%，但在不同微调策略下表现各有优劣。

Conclusion: 专业型模型在PPG信号分析任务中表现更优，但研究也强调了训练数据选择的重要性，并提供了关于泛化性、公平性和注意力可视化的深入分析。
...
Method: 使用Moore-Penrose伪逆直接估计每层的变换矩阵，然后应用包含六个可解释指标的光谱分析来表征层行为。

Result: 发现解码器模型呈现压缩-扩展循环，而编码器模型保持一致的高秩处理；核分析显示层间功能关系模式，CKA相似性矩阵将层划分为三个阶段：特征提取、压缩和专业化。

Conclusion: CAST提供了与现有方法互补的见解，通过直接矩阵估计和光谱分析揭示了Transformer层的行为模式，有助于更好地理解语言模型的内部工作机制。
...
<details>
  <summary>Details</summary>
Motivation: 当前早期事件预测系统忽略风险评分的时间稳定性，且主要基于表格数据评估，缺乏对风险轨迹行为的测试。
...
TL;DR: 提出了SR-RC方法，将谱残差方法与储层计算结合，在保持学习效率的同时提升时序异常检测性能


<details>
  <summary>Details</summary>
Motivation: 传统储层计算在边缘AI应用中需要过大的储层才能达到足够的异常检测性能，而注意力机制虽然能提高精度但计算量大且影响学习效率

Method: 将无学习的谱残差方法作为自下而上的注意力机制与储层计算集成，形成SR-RC方法

Result: SR-RC在基准任务和真实世界时序数据集上优于传统储层计算和基于谱残差提取值的逻辑回归模型

Conclusion: SR-RC为在边缘AI中部署储层计算进行时序异常检测提供了实用方向，因为谱残差方法与储层计算都适合硬件实现
...
TL;DR: TED++是一个子流形感知框架，通过构建类隐藏特征流形的管状邻域并应用局部自适应排序，有效检测逃避现有防御的隐蔽后门攻击。


<details>
  <summary>Details</summary>
Motivation: 随着深度神经网络在关键应用中广泛使用，隐蔽的后门攻击（中毒训练输入触发恶意行为但看起来良性）构成严重安全风险。现有防御在面对基于距离的微妙异常或干净样本稀缺时容易失效。

Method: TED++为每个类的隐藏特征流形构建管状邻域，从少量干净激活中估计局部"厚度"，然后应用局部自适应排序检测漂移出允许管的激活。通过聚合所有层的LAR调整排名，捕捉输入在演化类子流形上的忠实度。

Result: 在基准数据集和任务上的广泛实验表明，TED++在自适应攻击和有限数据场景下实现最先进的检测性能。即使每类只有5个保留样本，TED++仍能实现近乎完美的检测，AUROC比次优方法提高高达14%。

Conclusion: TED++通过表征"管约束"行为，能够有效检测隐蔽的后门攻击，在有限数据和自适应攻击场景下表现出色。
...
TL;DR: LLM-ERM是一个结合LLM引导搜索和ERM验证的程序学习框架，能在少量样本下学习短程序，而基于梯度的训练方法需要指数级更多样本。


<details>
  <summary>Details</summary>
Motivation: 解决程序学习中样本效率和计算可行性的平衡问题，传统方法要么搜索成本高，要么需要大量样本。

Method: 使用预训练LLM生成k个候选程序，在验证数据上编译检查并选择最佳假设，无需反馈、自适应或梯度。

Result: LLM-ERM仅需200个样本就能解决奇偶性变体、模式匹配和素数测试等任务，而SGD训练的transformer即使有10万个样本也会过拟合。

Conclusion: 语言引导的程序合成恢复了有限类ERM的统计效率，同时保持计算可行性，为学习简洁假设提供了实用途径。
...
TL;DR: 提出了DARTS-GT方法，通过可微架构搜索为图变换器选择每层最优的GNN算子，实现深度异构性，并开发了首个图变换器的定量可解释性框架。


<details>
  <summary>Details</summary>
Motivation: 当前图变换器存在设计僵化、缺乏量化可解释性的问题，固定GNN类型和复杂架构使得性能提升难以区分是有意义的模式还是虚假相关性。

Method: 通过非对称注意力重新设计图变换器，将结构编码与特征表示解耦，使用DARTS为每层选择最优GNN算子，并开发基于因果消融的定量可解释性框架。

Result: 在8个基准测试中，DARTS-GT在4个数据集上达到最先进水平，在其他数据集上保持竞争力，发现的架构揭示了数据集特定模式，且比基线产生更可解释的模型。

Conclusion: 图变换器无需在性能和可解释性之间做出选择，DARTS-GT发现的异构架构在保持高性能的同时提供了更好的可解释性。
...
TL;DR: Stop-RAG：基于价值控制的迭代RAG自适应停止策略，通过马尔可夫决策过程优化检索迭代次数，在保持API兼容性的同时提升多跳问答性能


<details>
  <summary>Details</summary>
Motivation: 迭代RAG虽然能回答复杂多跳问题，但每次额外迭代都会增加延迟、成本和引入干扰证据的风险，现有方法要么使用固定迭代次数，要么依赖不能准确反映检索帮助的置信度代理

Method: 将迭代RAG建模为有限时域马尔可夫决策过程，提出基于价值的控制器Stop-RAG，使用完整轨迹的全宽度前向视图Q(λ)目标进行训练
...
<details>
  <summary>Details</summary>
Motivation: LLM系统在日常AI应用中广泛使用，其性能依赖于精心设计的提示。基于LLM的提示优化器通过迭代优化提示来减少人工努力，但这一优化阶段的安全性尚未得到充分研究。

Method: 使用HarmBench进行系统性分析，发现系统对操纵反馈的脆弱性远高于注入查询。提出了简单的假奖励攻击方法，该方法无需访问奖励模型，并设计了轻量级的高亮防御机制。

Result: 基于反馈的攻击使攻击成功率提高了ΔASR=0.48。假奖励攻击将ΔASR从0.23提升到0.07，同时不降低系统效用。

Conclusion: 研究确立了提示优化管道作为首要攻击面，并强调需要为反馈通道和优化框架提供更强的安全保护措施。
...
TL;DR: 提出了SHaRe-SSM（尖峰谐波谐振-点火状态空间模型），一种用于超长序列建模的第二阶尖峰状态空间模型，在保持性能的同时显著降低能耗。
...
Method: 设计第二阶尖峰SSM，利用并行扫描实现动态系统的稳定高效实现，首次提出基于核的尖峰回归器用于超长序列。

Result: 在18k序列上比第二阶ANN-based SSMs能耗降低73倍，在50k序列上仍保持优异性能，平均表现优于transformer和第一阶SSMs。

Conclusion: SHaRe-SSM为超长序列建模提供了一种高效节能的解决方案，同时系统分析了谐振-点火SSMs中异质性、耗散和守恒的影响。
...
TL;DR: 多模态学习中存在模态不平衡问题，不仅出现在表示学习阶段，更显著体现在决策层。研究发现即使经过充分预训练和平衡优化，模型仍会系统性偏向某些模态（如音频）。这种偏见源于特征空间和决策权重分布的内在差异，而非仅优化动态。


<details>
  <summary>Details</summary>
Motivation: 多模态学习通常面临模态不平衡问题，主导模态在联合优化过程中会压制较弱模态。本文发现这种不平衡不仅发生在表示学习阶段，更显著体现在决策层，影响模型的整体性能。

Method: 通过在音频-视觉数据集（CREMAD和Kinetic-Sounds）上进行实验，分析模型在不同模态上的表现差异，并深入探究特征空间和决策权重分布的内在差异。

Result: 实验表明，即使经过广泛预训练和平衡优化，模型仍然表现出对某些模态（如音频）的系统性偏见。这种偏见源于特征空间和决策权重分布的内在差异，而非仅由优化动态引起。

Conclusion: 在融合阶段聚合未校准的模态输出会导致有偏的决策层权重分配，阻碍较弱模态的有效贡献。建议未来多模态系统应在决策层引入自适应权重分配机制，根据各模态能力实现相对平衡。
...
TL;DR: 提出MergeMoE方法，通过数学优化构建压缩矩阵来压缩MoE模型，在相同压缩比下优于基线方法。


<details>
  <summary>Details</summary>
Motivation: MoE技术在扩展模型规模方面很有前景，但其巨大的内存开销使得压缩成为重要研究方向。

Method: 从专家输出合并的角度分析专家合并，将其解释为在前向计算中插入额外矩阵，并基于此提出使用数学优化构建压缩矩阵的MergeMoE方法。

Result: 在多个MoE模型上评估，MergeMoE在相同压缩比下始终优于基线方法。

Conclusion: 从专家输出合并角度分析专家合并，并提出基于数学优化的MergeMoE方法，能有效压缩MoE模型。
...
TL;DR: 该论文研究发现，在大型语言模型剪枝后，逐层重构注意力机制和MLP组件比完全重训练更高效且性能更好，挑战了避免重训练的传统观点。
...
Method: 在GPT架构上进行广泛计算研究，比较不同重构粒度（单矩阵、组件级、块级）和重构策略，分析内存需求和性能表现。

Result: 发现组件级重构（分别重构注意力机制和MLP）在资源效率和困惑度方面达到帕累托最优，性能优于完全重训练，且内存需求仅为后者的零头。

Conclusion: 重构步骤比剪枝标准更重要，简单剪枝标准配合适当重构可超越复杂方法；挑战了"不惜一切代价避免重训练"的叙事，为LLM剪枝后性能恢复提供了重要见解。
...
TL;DR: 本研究探索使用生成对抗网络（GAN）来复制基于过程模型模拟的河流沉积物，通过消融研究表明深度学习社区的大规模2D图像生成技术可直接应用于3D河流沉积图像，训练稳定且能重现非平稳性和细节特征。


<details>
  <summary>Details</summary>
Motivation: 当前方法难以正确再现地质结构特别是河流沉积物的连续性，而基于过程的模型虽然能模拟地质过程但计算成本高昂，因此需要探索GAN等深度学习方法是否能有效替代。

Method: 使用基于过程模型生成训练数据，应用GAN进行生成建模，并通过消融研究验证深度学习技术从2D到3D的迁移性，同时利用沉积时间等地质属性来监控和验证GAN性能。

Result: GAN训练稳定，生成的样本能够重现河流沉积物的非平稳性和细节特征，没有出现模式崩溃或纯记忆训练数据的问题，且能够遵循叠加定律等地质原理。
...
TL;DR: 该论文对UCI葡萄酒数据集进行了多分类研究，比较了手动梯度下降与scikit-learn优化器的性能，并分析了L1正则化对特征稀疏性的影响，提出了最优5特征子集以实现成本效益部署。
...
Method: 使用One-vs-Rest逻辑回归方法，比较手动梯度下降实现与scikit-learn优化器，量化L1正则化对特征稀疏性的影响，并提出最优特征子集选择。

Result: 手动梯度下降达到92.59%测试精度，scikit-learn提供24倍训练加速和98.15%精度。L1正则化实现54-69%特征减少，仅损失4.63%精度。最优5特征子集实现62%复杂度降低和92-94%精度。

Conclusion: 研究为资源受限环境中的葡萄酒分类提供了平衡全面化学分析与针对性特征测量的实用指南，展示了在保持高精度的同时显著降低成本和时间的可行性。
...
TL;DR: MECo是一个分子优化框架，通过将编辑操作转化为可执行代码来桥接推理和执行，显著提高了分子编辑的准确性和一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的大型语言模型在分子优化中虽然能生成高层次编辑意图，但在执行这些修改时存在困难，特别是在处理非直观表示（如SMILES）时。

Method: MECo采用级联框架：首先生成人类可理解的编辑意图，然后通过代码生成将这些意图转化为可执行的结构编辑。

Result: 在重现化学反应的编辑中达到98%以上的准确率，在下游优化基准测试中一致性提高38-86个百分点至90%以上，成功率高于基于SMILES的基线方法。

Conclusion: MECo通过对齐意图与执行，实现了一致、可控和可解释的分子设计，为药物发现中的高保真反馈循环和人类-AI协作工作流奠定了基础。
...
TL;DR: 提出了一个理论基础的、资源高效的数据选择和重加权框架，通过上下文近似(ICA)估计候选训练样本的损失，无需参考模型或额外微调，在多种训练方法中持续提升模型对齐效果。


<details>
  <summary>Details</summary>
Motivation: 微调大型预训练语言模型时，噪声或偏离目标的训练样本会稀释监督信号。虽然小型精选数据集通常能与大型数据集性能相当，但系统高效识别高价值训练数据的方法仍未被充分探索。

Method: 核心是上下文近似(ICA)，通过在上下文中条件化小型精选验证集来估计模型在候选样本上训练后的验证损失。基于局部线性化，ICA等价于向验证最优的一阶更新。从ICA分数推导每个样本的权重，在模型参数演化时动态重加权梯度更新。

Result: 在SFT、DPO和SimPO等多种训练方法中，以及不同骨干网络和数据集上，基于ICA的重加权方法持续改善模型对齐效果，且开销极小。

Conclusion: ICA框架为数据选择和重加权提供了理论基础和高效实现，显著提升模型对齐效果，但针对快速漂移的在线策略更新存在局限性，为未来工作指明了方向。
...
TL;DR: 提出了Guess2Graph (G2G)框架，使用专家猜测来指导统计测试的顺序而非替代它们，保持了统计一致性同时提升性能。开发了PC-Guess和gPC-Guess两个变体，理论上都能在专家错误时保持正确性，gPC-Guess在专家表现优于随机时能获得更好的有限样本性能。


<details>
  <summary>Details</summary>
Motivation: 因果发现算法在样本有限时表现不佳，现有整合专家知识的方法需要完美的预测或不确定性估计，这在实践中不可靠。

Method: G2G框架使用专家猜测来指导统计测试的顺序，而非替代测试。开发了PC-Guess（增强PC算法）和gPC-Guess（学习增强变体）两个具体实现。

Result: 理论上两种方法都能在专家错误时保持正确性，gPC-Guess在专家表现优于随机时能获得更好的有限样本性能。实证显示两种方法都随专家准确性单调改进，gPC-Guess获得显著更强的增益。

Conclusion: G2G框架通过将专家知识作为指导而非替代统计测试，在保持统计一致性的同时有效提升了因果发现性能，特别是在样本有限的情况下。
...
TL;DR: 提出可逆学习框架，通过可逆性度量和选择性状态回滚机制，提高基于价值的强化学习的鲁棒性和效率，显著减少灾难性错误并提升性能。
...
Method: 使用经验推导的状态-动作可逆性度量Phi(s,a)来量化返回先前状态的可能性，并动态调整TD更新中的惩罚项；结合选择性状态回滚操作，当动作预期回报显著低于估计值时回滚到前一个状态。

Result: 在CliffWalking v0环境中减少99.8%的灾难性跌落，平均回合回报提升55%；在Taxi v3环境中抑制99.9%的非法动作，累积奖励提升65.7%，同时显著降低奖励方差。

Conclusion: 可逆学习框架通过可逆性感知评估和针对性回滚的结合，显著提升了强化学习的安全性和性能，其中回滚机制是实现这些改进的关键组件。
...
<details>
  <summary>Details</summary>
Motivation: 传统分块方法将时间序列划分为相邻块，导致固定表示空间，表示能力不足。需要构建选择性表示空间来灵活包含最具信息量的块。

Method: 提出SRS模块，包含可学习的选择性分块和动态重组技术，自适应选择和重排上下文时间序列块。构建SRSNet模型，由SRS模块和MLP头组成。

Result: SRSNet在多个领域的真实数据集上达到最先进性能。SRS作为即插即用模块也能提升现有基于分块模型的性能。

Conclusion: SRS模块通过选择性表示空间有效提升了时间序列预测性能，可作为通用模块增强现有模型。
...
Method: AEPO包含两个核心组件：(1) 动态熵平衡rollout机制，通过熵预监控自适应分配全局和分支采样预算，并对连续高熵工具调用步骤施加分支惩罚；(2) 熵平衡策略优化，在高熵裁剪项中插入停止梯度操作，并引入熵感知优势估计来优先学习高不确定性token。

Result: 在14个挑战性数据集上，AEPO一致优于7种主流RL算法。仅使用1K RL样本，Qwen3-14B结合AEPO在GAIA上达到47.6%(Pass@1)和65.0%(Pass@5)，在Humanity's Last Exam上达到11.2%(Pass@1)和26.0%(Pass@5)，在WebWalker上达到43.0%(Pass@1)和70.0%(Pass@5)。
...
TL;DR: 提出了MX+格式，一种针对4位块浮点格式的改进方案，通过重新利用异常值的指数位作为扩展尾数来提高精度，在保持低存储开销的同时显著提升语言模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前的低精度数据格式要么需要侵入式软件修改，要么不够通用。4位块浮点格式在处理异常值时性能不佳，需要改进。

Method: MX+格式基于关键洞察：异常值不需要使用其指数字段，因此可以将指数位重新用作扩展尾数，提高异常值的精度。

Result: MX+相比4位MX格式（MXFP4）实现了显著更高的模型性能，存储开销和速度损失可忽略不计。
...
Method: 提出Redundancy-aware Graph Information Bottleneck (ReGIB)，将目标分解为本质信息和无关冗余，通过最小化结构熵减少解耦的冗余，并提出理论上的上下界进行优化。

Result: 在真实世界数据集上的广泛实验显示，RedOUT在OOD检测方面表现优异，平均提升6.7%，在ClinTox/LIPO数据集对上显著超越最佳竞争对手17.3%。

Conclusion: RedOUT通过结构熵最小化有效减少结构冗余，显著提升了图OOD检测性能。
...
TL;DR: 本文研究了使用Hydra（一种双向线性时间结构化状态空间模型）替代TabPFN中的Transformer，以解决Transformer在序列长度上的二次复杂度问题，同时通过双向方法减少对输入顺序的敏感性。


<details>
  <summary>Details</summary>
Motivation: Transformer在表格数据基础模型中表现出色，但其二次复杂度限制了效率。状态空间模型（SSM）具有线性复杂度，但对输入顺序敏感，这在行顺序无语义意义的表格数据中是不理想的。
...
Conclusion: Hydra作为Transformer的替代方案，在保持效率的同时实现了竞争性的预测性能，为表格数据基础模型提供了更高效的架构选择。
...
TL;DR: 提出了一种名为Conformal Labeling的新方法，通过控制AI预测标签的假发现率(FDR)来识别可被证明可信的AI预测实例。


<details>
  <summary>Details</summary>
Motivation: 解决AI模型标注大规模数据集时存在的标签质量问题，现有选择性标注方法缺乏对AI标注质量的保证，导致AI标注子集中错误率过高。

Method: 为每个测试实例构建符合性p值，通过比较AI模型的预测置信度与校准实例中被AI误标的置信度，选择p值低于数据依赖阈值的实例，证明AI预测可信。

Result: 实验表明该方法在各种任务（图像和文本标注、LLM QA）中实现了严格的FDR控制和高检测能力。

Conclusion: Conformal Labeling方法能够理论保证将AI标注的假发现率控制在名义水平以下，确保预定比例的AI分配标签平均上是正确的。
...
TL;DR: Matcha是一种新颖的分子对接流程，结合多阶段流匹配、学习评分和物理有效性过滤，在速度和精度上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有蛋白质-配体结合构象预测方法难以平衡速度、精度和物理合理性，需要更高效的解决方案。

Method: 采用三阶段流匹配模型（在R³、SO(3)和SO(2)几何空间），结合专用评分模型和无监督物理有效性过滤器。

Result: 在Astex和PDBbind测试集上表现出更高的对接成功率和物理合理性，比现代大规模共折叠模型快约25倍。

Conclusion: Matcha提供了一种快速、准确且物理合理的蛋白质-配体对接解决方案，代码已开源。
...
TL;DR: MAHA是一种用于多模态问答的检索增强生成系统，通过模态感知知识图谱结合密集向量检索和图遍历，有效处理包含文本、图像、表格等混合模态的非结构化文档。


<details>
  <summary>Details</summary>
Motivation: 当前RAG系统主要处理单模态文本数据，在处理包含多种模态（文本、图像、表格、公式、图表）的非结构化多模态文档时效果有限，每种模态都包含独特信息。
...
Conclusion: MAHA建立了一个可扩展且可解释的检索框架，通过结合嵌入和显式文档结构，实现了对非结构化多模态数据的模态感知推理，推进了RAG系统的发展。
...
TL;DR: 提出FAL和FAL+两种高效transformer架构，通过重定向第一层MHA输出到后续层的MLP输入，消除每块的MHA-MLP连接，减少通信开销并提高训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有transformer设计在张量并行训练中存在显著通信开销，特别是每块的MHA-MLP连接需要all-reduce通信，影响训练效率。

Method: FAL将第一层MHA输出重定向到后续层的MLP输入，消除每块的MHA-MLP连接；FAL+在FAL基础上将归一化的第一层注意力输出添加到后续层MHA输出中，增强MLP输入质量。

Result: FAL减少多GPU训练时间达44%，单GPU吞吐量提升1.18倍，困惑度优于基线GPT；FAL+在不增加训练时间的情况下实现更低的困惑度。

Conclusion: FAL和FAL+通过重新设计transformer架构，有效减少通信开销，提高训练效率，同时保持或改善模型质量。
...
TL;DR: LeapFactual是一种基于条件流匹配的新型反事实解释算法，能够生成可靠且信息丰富的反事实解释，克服了现有方法的梯度消失、潜在空间不连续等限制，适用于模型不可微分和人类参与的系统。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习和人工智能模型在高风险领域（如医疗和科学研究）的应用日益增多，需要不仅准确而且可解释的模型。现有反事实解释方法存在梯度消失、潜在空间不连续等关键限制。

Method: 提出LeapFactual算法，基于条件流匹配生成反事实解释，采用模型无关的方法，不限于可微分损失函数的模型，还能处理人类在环系统。

Result: 在基准和真实数据集上的广泛实验表明，LeapFactual生成准确且符合数据分布的反事实解释，提供可操作的见解，可靠的反事实样本可作为新训练数据提升模型性能。

Conclusion: 该方法具有广泛适用性，增强了科学知识发现和非专家可解释性，为需要人类标注的领域（如公民科学）扩展了反事实解释的范围。
...
Method: 扩展经典的编码器-解码器架构，加入可逆流组件，支持反事实解释分析。
...
Method: 使用Siegel嵌入将一阶和二阶矩表示为单个对称正定矩阵，在SPD矩阵的共享流形上使用自然几何距离进行同时矩对齐。

Result: 在图像去噪和图像分类基准上验证了方法的有效性，代码已公开。

Conclusion: 提出的黎曼流形距离方法能够更忠实地进行跨域比较，并保持源域和目标域的均值和协方差结构。
...
<details>
  <summary>Details</summary>
Motivation: 解决脑机接口和远程环境监测等应用中，需要在低功耗条件下实现可靠异常检测的问题，同时严格控制误报率。

Method: 采用在线假设检验方法结合e值来维持FDR控制，将传感器查询策略建模为多臂老虎机框架中的最优臂识别问题进行动态优化。

Result: 性能评估表明，该方法能在严格FDR要求下可靠检测异常，同时高效调度传感器通信并实现低检测延迟。

Conclusion: 所提出的框架为神经形态无线传感器网络提供了一种有效的低功耗在线异常检测解决方案，在保持严格FDR控制的同时实现了高效的传感器调度。
...
Method: FedPPA方法：渐进式对齐客户端与全局模型的公共层权重，结合基于熵的加权平均

Result: 在MNIST、FMNIST和CIFAR-10数据集上，FedPPA优于现有联邦学习算法，在个性化适应方面表现优异
...
TL;DR: Tawa是一个自动化编译器，能从高级瓦片化程序生成高性能的warp专用代码，解决了GPU SIMT编程模型与任务并行硬件之间的不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 现代GPU的专用硬件单元支持高性能异步数据流执行，但传统的SIMT编程模型与此不匹配，导致程序开发需要手动编排复杂的低级通信和软件管道，工作量大且容易出错。

Method: 提出Tawa编译器，采用新型IR抽象——异步引用(aref)来表达warp级通信，自动将程序划分为生产者-消费者角色并管理复杂的数据流管道。

Result: 在NVIDIA H100 GPU上评估显示，Tawa实现了高硬件利用率，相比高度优化的cuBLAS GEMM内核获得1.1倍加速，在注意力工作负载上比Triton快1.2倍，且性能与手工优化的CUTLASS C++ FlashAttention-3内核相当。

Conclusion: Tawa能够自动生成高性能的warp专用代码，显著减少了编程工作量，同时保持了与手工优化代码相当的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有工具如INDAGO仅关注最大化故障数量，无法确保发现多样化的故障场景或揭示不同的错误类型。
...
TL;DR: 论文提出未来摘要预测(FSP)方法，通过预测序列的长期未来摘要来改进语言模型的长程推理能力，相比传统下一个词预测(NTP)和多词预测(MTP)在数学、推理和编程任务上表现更好。


<details>
  <summary>Details</summary>
Motivation: 传统下一个词预测(NTP)在长程推理、规划和创意写作方面存在局限，多词预测(MTP)只能捕获短程依赖关系，改进有限。需要一种能预测长期未来信息的方法来提升语言模型的长程能力。

Method: 提出未来摘要预测(FSP)方法，训练一个辅助头来预测长期未来的紧凑表示。探索两种变体：手工制作的摘要(如词袋摘要)和学习的摘要(使用从右到左训练的反向语言模型生成的嵌入)。

Result: 在大规模预训练实验(30亿和80亿参数模型)中，FSP在数学、推理和编程基准测试上都优于NTP和MTP方法。

Conclusion: FSP通过预测长期未来摘要，有效提升了语言模型的长程推理能力，为改进语言模型的长期规划能力提供了有前景的方向。
...
<details>
  <summary>Details</summary>
Motivation: 传统被动众包方法在干扰源定位中存在效率问题，需要开发能够自适应引导移动代理收集高价值测量数据的主动定位方法，同时考虑实际城市环境中的障碍和移动约束。

Method: 修改A*算法为A-UCB*，将采集值纳入轨迹成本计算，生成高采集值的规划路径，结合贝叶斯优化来指导移动代理收集接收信号强度测量值。

Result: 在真实城市场景模拟中，该方法相比无信息基线方法能够用更少的测量实现准确的定位，在不同环境下表现出稳定的性能。

Conclusion: 所提出的主动干扰源定位框架通过采集感知路径规划显著提高了定位效率，为城市环境中的干扰源定位提供了有效的解决方案。
...
Result: 在CIFAR-10、CIFAR-100和Tiny-ImageNet等标准图像分类基准测试中达到SOTA性能，在持续学习和迁移学习场景中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 结构化稀疏性虽然能加速GPU上的训练和推理，但在准确性上仍不及非结构化动态稀疏训练（DST），主要原因是表达能力受限。

Method: 为每个层学习单个排列矩阵，与结构化权重矩阵联合训练，应用于块、N:M和对角线三种典型结构。

Result: 在ImageNet-1K（ViT-B/16）和WikiText-103（GPT-2）上，90-95%稀疏度下达到与非结构化基线（RigL、SET）相当的精度，训练速度提升1.21倍，推理速度提升2.9倍。
...
TL;DR: 提出了ShifTS框架，先处理时间偏移再处理概念漂移，通过软注意力机制从回看和预测时间序列中寻找不变模式，提升时间序列预测准确性。
...
Method: 提出ShifTS框架，先解决时间偏移作为处理概念漂移的前提；使用软注意力机制从回看和预测时间序列中提取不变模式；该方法与具体模型无关。

Result: 在多个数据集上的广泛实验表明，ShifTS能持续提升无关模型的预测准确性，并优于现有的概念漂移、时间偏移及组合基线方法。

Conclusion: ShifTS框架通过先处理时间偏移再处理概念漂移的统一方法，有效提升了时间序列预测的准确性，解决了现有方法在概念漂移处理上的不足。
...
TL;DR: 提出Learned Programmatic Representations (LeaPR)模型，将LLM生成的代码特征与决策树结合，实现可解释的端到端学习，性能可与神经网络竞争。
...
Method: 使用LLM合成代码特征函数，结合决策树预测器。提出两种算法：FunSearch的改进版本学习特征，以及ID3算法的变体在分裂叶节点时按需生成新特征。

Result: 在象棋位置评估、图像和文本分类等任务中，学习到高质量的非神经网络预测器，性能常与神经网络相当。

Conclusion: LeaPR提供了一个灵活的学习范式，能够端到端学习可解释表示，特征和预测结果都可以被轻松检查和理解。
...
TL;DR: 本文证明状态空间模型(SSMs)在纯序列建模中存在理论局限性，无法解决真正长序列生成问题，但通过引入外部工具交互可以克服这一限制，实现长度泛化。


<details>
  <summary>Details</summary>
Motivation: SSMs作为Transformer的替代方案，主要优势在于长上下文和长序列生成的高效性，但作者发现其在纯序列建模中存在理论局限性。

Method: 通过理论分析证明SSMs的局限性，然后提出工具增强SSMs的方法，让模型能够交互式访问外部工具，并配合问题相关的训练数据。

Result: 工具增强的SSMs在算术、推理和编程任务上实现了显著的长度泛化能力，能够处理任意长度和复杂度的任务。

Conclusion: SSMs在结合工具交互的场景下，有望成为Transformer的高效替代方案，特别适用于交互式工具和智能体设置。
...
Result: 在软硬切换设置下测试，带迟滞的P-CHO方案能减少切换失败和乒乓事件，实现准确、低延迟的主动切换。
...
TL;DR: 提出了一种新型随机奖励机及其学习算法，用于处理带噪声的稀疏奖励强化学习问题，相比现有方法表现更优
...
Conclusion: 该方法能有效处理带噪声的奖励函数，保证在极限情况下收敛到最优策略
...
TL;DR: 该论文提出了一种无需重新训练就能从基础模型中移除后门攻击的方法，通过利用后门与良性任务在权重空间中的解耦特性，实现了近乎完美的后门遗忘，同时保持96%的干净准确率。


<details>
  <summary>Details</summary>
Motivation: 基础模型虽然具有广泛泛化能力，但对对抗性扰动和后门攻击高度敏感。现有后门移除方法需要昂贵的微调，且会降低其他任务的性能。本文研究是否能在不损害模型通用能力的情况下移除后门。

Method: 研究发现后门在模型权重空间中与良性任务是解耦的，基于这一发现提出了一种简单的遗忘方法，通过隔离和擦除后门影响来实现后门移除。

Result: 在CLIP模型和常见对抗性触发器的实验中，该方法实现了近乎完美的后门遗忘，平均保持96%的干净准确率。即使在攻击未知的情况下，通过反向工程触发器也能成功移除后门。

Conclusion: 该方法相比现有最先进防御方法，在遗忘效果和干净准确率之间取得了更好的平衡，为保护基础模型安全提供了有效解决方案。
...
TL;DR: 提出Hermite特征结构假设(HEA)，通过分析核特征值和特征函数来预测真实数据集上的核回归学习曲线，并发现MLPs在特征学习阶段也会学习HEA预测的Hermite多项式。


<details>
  <summary>Details</summary>
Motivation: 研究真实数据集上的核回归学习曲线，开发一个理论框架来仅从数据协方差矩阵和目标函数的多项式分解预测模型性能。

Method: 提出Hermite特征结构假设(HEA)，通过分析核特征值和特征函数来近似各向异性数据分布下的核结构，并应用于预测学习曲线。

Result: HEA在真实图像数据上表现良好，能够准确预测核回归的学习曲线，并且发现MLPs也会按照HEA预测的顺序学习Hermite多项式。

Conclusion: HEA框架证明了从数据集结构到模型性能的端到端学习理论对于真实数据集上的非平凡学习算法是可行的。
...
TL;DR: 提出了一种在无导师情况下处理无界奖励的谨慎学习算法，通过选择可信区域和仅在证据证明安全时执行任务，实现高风险环境下的安全部署。


<details>
  <summary>Details</summary>
Motivation: 在高风险AI应用中，单个错误可能造成不可逆损害，而现有序列决策理论假设所有错误都可恢复。标准bandit算法的激进探索在这种场景下可能造成不可逆损害，且导师不一定总是可用。

Method: 提出一个包含弃权选项的双动作上下文bandit模型，使用基于谨慎的算法学习何时不学习：选择可信区域，仅在可用证据不证明有害时执行任务。

Result: 在i.i.d.输入条件下建立了次线性遗憾保证，理论上证明了谨慎探索在学习代理安全部署到高风险环境中的有效性。

Conclusion: 谨慎探索策略能够在无导师情况下有效避免不可逆损害，为高风险环境中的学习代理部署提供了理论保障。
...
TL;DR: 本文提出了一种无需额外训练、仅通过纯采样就能从基础模型中激发推理能力的方法，其性能接近甚至超过RL后训练模型，同时避免了多样性下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注RL后训练中出现的新行为，但本文从不同角度探讨是否能在推理时通过纯采样从基础模型中激发可比的推理能力，而无需额外训练。

Method: 受马尔可夫链蒙特卡洛技术启发，提出了一种简单的迭代采样算法，利用基础模型自身的似然函数进行采样。

Result: 在不同基础模型上，该算法在MATH500、HumanEval和GPQA等单次任务中提供了显著的推理能力提升，性能接近甚至超过RL后训练模型，同时避免了多样性下降问题。

Conclusion: 该方法无需训练、精选数据集或验证器，表明其在易验证领域之外具有广泛适用性。
...
TL;DR: 提出了WeightLens和CircuitLens两种互补方法，通过直接分析权重和特征交互来增强神经网络的可解释性，超越传统基于激活的分析方法。


<details>
  <summary>Details</summary>
Motivation: 现有可解释性方法依赖人工检查且仅限于简单任务，自动化方法又容易忽略特征间交互并过度依赖外部LLM和数据集质量。
...
TL;DR: 本文提出了一种新的扩散强制采样器，用于加速循环深度语言模型的生成，通过在每个前向传递中解码新token，同时并行细化这些token的潜在状态，实现了高达5倍的加速。


<details>
  <summary>Details</summary>
Motivation: 研究循环深度模型与扩散语言模型之间的关系，利用它们的相似性来加速循环深度模型的推理生成。

Method: 基于扩散文献原理开发新的扩散强制采样器，在每个前向传递中解码新token，同时通过循环并行细化潜在状态。

Result: 该采样器可直接应用于现有的35亿参数循环深度transformer，无需调优即可实现高达5倍的加速，且在相同时间预算下比基线自回归生成更具表达能力。

Conclusion: 循环深度模型可以自然地被视为强大的连续（尽管是因果的）扩散语言模型，该采样器为在推理时并行化循环深度模型中的额外计算提供了高效机制。
...
TL;DR: BINNs通过整合SNPs、多组学数据和先验生物学知识，改进了作物基因组预测和选择。该方法在训练时使用多组学数据，推理时仅需基因型数据，显著提高了预测准确性并揭示了非线性生物学关系。


<details>
  <summary>Details</summary>
Motivation: 传统G2P模型依赖直接映射，准确性有限，育种者需进行昂贵的大规模田间试验。现有模型虽然能通过整合中间分子表型提高预测拟合度，但在部署时无法获得此类数据，限制了实际应用。

Method: BINNs编码通路级归纳偏置，训练时利用多组学数据，推理时仅使用基因型数据。通过玉米基因表达和多环境田间试验数据进行验证，并在合成代谢组学基准测试中应用完整领域知识。

Result: 在稀疏数据条件下，BINN将秩相关准确性提高了56%；在合成基准测试中，相比传统神经网络将预测误差降低了75%。BINN能识别GWAS/TWAS无法发现的非线性基因，且潜在变量与实验量高度相关。
...
TL;DR: 提出π-Flow模型，通过策略蒸馏方法解决少步扩散模型中的质量-多样性权衡问题，实现快速准确的ODE积分
...
Method: 修改学生流模型的输出层，预测网络无关策略，在后续子步骤生成动态流速度，使用模仿蒸馏方法匹配师生轨迹

Result: 在ImageNet 256²上达到1-NFE FID 2.85，优于相同DiT架构的MeanFlow；在FLUX.1-12B和Qwen-Image-20B上4 NFEs时保持教师级质量且多样性显著提升

Conclusion: π-Flow通过简单的模仿教师行为实现稳定可扩展训练，避免了质量-多样性权衡问题
...
TL;DR: 该论文提出了一个分析高维优化问题中SGD训练动态的框架，特别关注多类各向异性数据的训练动态，并发现了结构相变现象。


<details>
  <summary>Details</summary>
Motivation: 研究高维数据中SGD训练动态，特别是当数据协方差矩阵具有各向异性结构时，SGD如何影响模型学习过程。

Method: 开发了一个理论框架，通过确定性ODE系统分析SGD动态，研究了三种协方差结构：各向同性、零一模型和幂律分布模型。

Result: 发现在零一模型和幂律模型中，SGD倾向于与类均值在"干净方向"（方差较小的方向）上的投影更紧密对齐，存在结构相变。

Conclusion: 数据协方差的结构性特征显著影响SGD训练动态，各向异性结构会导致SGD优先学习低方差方向的特征。
...
TL;DR: 开发结合光学和SAR数据的森林砍伐检测管道，使用KL展开残差空间构建光学异常图，结合HMM分类森林状态，在亚马逊森林测试显示优于现有混合方法。


<details>
  <summary>Details</summary>
Motivation: 传统统计参数方法需要数据分布先验知识，对高维遥感数据不实用，需要开发不依赖数据分布假设的检测方法。

Method: 使用离散KL展开构建光学数据异常图，基于残差分量分布浓度边界量化异常，结合SAR数据通过隐马尔可夫模型分类森林状态。

Result: 在亚马逊92.19km×91.80km区域测试，混合光学-雷达和纯光学方法均达到高精度，优于现有混合方法，且在云层覆盖区域稀疏光学数据下更稳健。

Conclusion: 提出的混合方法在森林砍伐检测中表现优异，特别在光学数据稀疏情况下具有显著鲁棒性，为多云地区监测提供有效解决方案。
...
TL;DR: 提出了一种新的鲁棒非参数聚类验证框架HD-BWDM，用于解决高维和污染数据中的聚类数量确定问题。


<details>
  <summary>Details</summary>
Motivation: 传统聚类有效性指标（如Calinski-Harabasz、Silhouette等）依赖基于质心的距离，在高维或污染数据中性能下降，需要开发更鲁棒的验证方法。

Method: 结合随机投影和主成分分析来缓解维度灾难，使用修剪聚类和基于中心点的距离来确保对异常值的鲁棒性，并基于Johnson-Lindenstrauss嵌入理论推导一致性结果。

Result: 广泛的模拟实验表明，HD-BWDM在高维投影和污染情况下保持稳定性和可解释性，为传统基于质心的验证标准提供了鲁棒替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 在机器学习中，评估学习回归器的质量具有挑战性，因为无法访问真实数据生成机制，且没有数据驱动的评估方法能确保全局最优性的可实现性。

Method: 基于估计输入变量与残差之间的香农互信息，适用于广泛的加性噪声模型，构建了一个数据驱动的评估框架。

Result: 数值实验证实Information Teacher能够检测全局最优性，这与相对于真实模型的零估计误差条件一致，可作为真实评估损失的替代度量。

Conclusion: Information Teacher提供了一个有原则的替代方案，用于评估回归算法，并提供了传统经验性能指标之外的全局最优性保证。
...
TL;DR: 提出SROF正则化器和RowFed算法，用于个性化联邦多变量学习，通过行向量聚类和稀疏性实现更好的变量级结构恢复。


<details>
  <summary>Details</summary>
Motivation: 现有方法在个性化联邦学习中存在局限性：逐项惩罚忽略跨响应依赖，而矩阵级融合过度耦合客户端。需要一种能平衡变量级结构共享和客户端异质性的方法。

Method: 提出稀疏行向融合(SROF)正则化器，跨客户端聚类行向量并诱导行内稀疏性；开发RowFed算法，将SROF嵌入线性化ADMM框架，支持隐私保护的局部参与。

Result: 理论证明SROF具有oracle性质，能正确恢复变量级分组并保持渐近正态性；RowFed收敛到平稳解，在随机客户端参与下迭代间隙以随参与概率提高的速率收缩。实证显示RowFed在异质环境下持续降低估计和预测误差，增强变量级聚类恢复。

Conclusion: 行向融合是规模化个性化联邦多变量学习的有效透明范式，填补了逐项和矩阵级公式间的空白。
...
<details>
  <summary>Details</summary>
Motivation: 全局因果发现方法虽然能恢复最优调整集，但随着变量数量增加计算成本急剧上升；局部因果发现方法更可扩展但只能获得统计上次优的调整集。需要结合两者优势的方法。

Method: LOAD首先识别目标变量间的因果关系并测试是否仅用局部信息即可识别因果效应。如果可以识别，则利用局部因果发现推断中介变量及其父节点来找到最优调整集；否则基于学习的局部结构返回局部有效的父调整集。

Result: 在合成和真实数据实验中，LOAD在可扩展性上优于全局方法，同时在效应估计准确性上优于局部方法。

Conclusion: LOAD方法成功地将局部方法的计算效率与全局方法的统计最优性相结合，提供了一种既高效又准确的因果效应估计解决方案。
...
TL;DR: 提出了一种结合物理信息深度学习和贝叶斯推理的计算框架，用于识别偏微分方程中具有跳跃不连续性的系数参数。


<details>
  <summary>Details</summary>
Motivation: 传统数值和机器学习方法在处理具有不连续系数的偏微分方程反问题时面临高维性、非线性和不连续参数空间的限制。

Method: 采用双网络架构：主网络逼近PDE解，子网络采样其系数；使用梯度自适应加权策略和马尔可夫动力学方法捕捉复杂时空系统的隐藏状态转移。

Result: 在各种具有跳跃变化系数的PDE上的数值实验表明，该框架相比现有方法具有出色的适应性、准确性和鲁棒性。
...
TL;DR: 提出了KCCSD测试，一种基于核的非参数方法，用于评估具有明确定义分数的概率模型的校准性，无需昂贵的期望近似且能控制I类错误。
...
Method: 使用新的基于分数的概率核函数族，无需概率密度样本，并结合条件拟合优度准则构建KCCSD测试的U统计量。

Result: 在各种合成设置中验证了测试的特性，证明了方法的有效性。

Conclusion: KCCSD测试为概率模型校准评估提供了高效可靠的解决方案，避免了传统方法的计算负担和统计控制问题。
...
TL;DR: 提出基于最优传输理论的互传输依赖(MTD)作为实验设计的新几何框架，克服传统基于互信息方法的局限性


<details>
  <summary>Details</summary>
Motivation: 传统最优实验设计方法依赖概率密度，导致限制性不变性。需要更灵活的几何框架来适应特定下游估计问题

Method: 使用最优传输理论构建互传输依赖(MTD)作为统计依赖度量，通过选择适当几何结构来定制实验设计

Result: 该框架能产生高质量设计，为传统信息论技术提供灵活替代方案

Conclusion: MTD框架为最优实验设计提供了几何基础，具有更好的灵活性和适应性
