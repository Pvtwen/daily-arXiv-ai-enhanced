...
Result: 仿真结果表明，考虑天线运动功耗和可调辐射功率显著提升了PASS性能，并验证了所提优化框架的高效性。
...
<details>
  <summary>Details</summary>
Motivation: 在6G及未来的无线通信中，如何实时优化可重构天线的系数以改善信道性能是一个重大挑战，传统方法需要复杂的信道建模和隐式参数估计。

Method: 采用零阶优化技术，直接优化可重构天线的系数，无需依赖信道建模或隐式参数估计。

Result: 通过RIS和可移动天线的案例研究，展示了零阶优化方法优于现有技术。

Conclusion: 零阶优化为可重构天线技术提供了新的研究方向，具有显著潜力。
...
<details>
  <summary>Details</summary>
Motivation: 在信号处理中，分离叠加信号是一个常见挑战，尤其在音频和雷达领域。本文旨在改进现有SSO方法，以应对更复杂的信号条件。

Method: 通过理论分析噪声对SSO的影响，改进并应用SSO方法分离啁啾信号，验证了其在低信噪比和交叉情况下的有效性。

Result: 在包含7个模拟信号的仿真数据集上进行了测试，展示了方法的可行性和数值结果。

Conclusion: 改进后的SSO方法能够有效处理复杂信号条件，为实际应用提供了理论支持。
...
<details>
  <summary>Details</summary>
Motivation: 探索如何在集成传感与通信系统中，通过STAR-RIS和PESA技术实现高效的空间和时间调制，同时兼顾雷达性能和通信需求。

Method: 利用周期性脉冲信号和STAR-RIS的空间调制，结合正交二进制码本的时间调制，实现目标检测和通信消息嵌入。

Result: 时间调制对雷达性能影响极小，同时提供了不同的传输和错误率权衡。

Conclusion: 该方法为ISAC系统提供了一种有效的解决方案，兼顾了雷达性能和通信需求。
...
<details>
  <summary>Details</summary>
Motivation: 研究如何在基站同时具备感知和通信能力的情况下，通过优化雷达检测器设计，实现更优的系统性能权衡。

Method: 采用多帧雷达检测器（包括检测器、绘图提取器和检测前跟踪处理器），联合处理更多扫描帧以提升频谱效率。

Result: 数值分析验证了该方案的有效性，并评估了可实现的系统性能权衡。

Conclusion: 通过多帧雷达检测器联合处理扫描帧，可在不牺牲感知性能的前提下提升频谱效率。
...
<details>
  <summary>Details</summary>
Motivation: 低空无线网络（LAWN）在物联网（IoT）系统中具有潜力，但实时无线控制面临挑战，需解决通信可靠性和控制精度问题。

Method: 采用模型预测控制（MPC）确保轨迹跟踪精度，分析通信可靠性；通过优化问题联合确定控制策略、功率分配和无人机轨迹；提出交替优化（AO）框架和高效算法。

Result: 通过仿真和实验验证，所提方法在控制性能上优于基线方案。

Conclusion: 提出的方法有效解决了LAWN系统中的实时控制问题，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在解决高速运动目标在LEO卫星通信系统中的参数估计问题，特别是针对目标响应在延迟-多普勒域的稀疏性和范围迁移的影响。

Method: 通过OTFS调制信号处理，推导了高速目标回波的输入输出模型，提出了一种基于块正交匹配追踪和匹配滤波器组的最大似然估计器近似实现方法。

Result: 研究发现目标响应在延迟-多普勒域具有稀疏性，且范围迁移会导致目标响应的扩散。数值实验验证了所提方法的估计性能。

Conclusion: 研究为LEO卫星通信系统中的高速目标参数估计提供了一种有效方法，揭示了目标响应的稀疏结构，并展示了估计器的性能。
...
<details>
  <summary>Details</summary>
Motivation: 研究注意力机制是否在资源分配中必不可少，并探索如何结合GNN和注意力机制以提升学习效率。

Method: 分析集合和数值算法的结构，证明置换等变函数的递归表达形式，并设计了一种与结构对齐的GNN框架。

Result: 发现当干扰未反映在可测参数中时，注意力机制是必要的，并通过仿真验证了所提GNN的学习效率。

Conclusion: 注意力机制在特定情况下是必要的，提出的GNN框架能有效提升资源分配的学习效率。
...
<details>
  <summary>Details</summary>
Motivation: 由于Rabiner等人和Lyons在文献中提供的过渡系数和PSL值存在差异且不准确，本文旨在通过程序重新计算并给出最优值。

Method: 通过程序重新估计并优化低通和带通滤波器的过渡系数及PSL值。

Result: 发现原有文献中的过渡系数和PSL值不准确，并提供了更优的数值（例如，对于N=16和BW=4，最优过渡系数为0.40474097）。

Conclusion: 本文修正了现有文献中的错误，为频率采样滤波器设计提供了更准确的过渡系数和PSL值。
...
<details>
  <summary>Details</summary>
Motivation: 在不增加硬件复杂度的前提下提高频谱效率，通过捏合天线位置模式索引传递额外信息。

Method: 设计了全面的收发机方案，包括信道建模、低复杂度的BOSD检测算法、BER分析及基于流形优化的预编码方法。

Result: 仿真结果表明，PA-IM方案在性能上显著优于传统方案，预编码设计显著降低了系统BER。

Conclusion: PA-IM方案通过创新的索引调制和优化设计，实现了频谱效率和性能的双重提升。
...
Result: AREE算法能以低复杂度快速收敛，频谱效率优于现有方法。
...
Method: 采用离散傅里叶变换（DFT）向量设计码本，并结合DNN进行快速码字选择，以降低计算复杂度。
...
### [13] [Joint Radiation Power, Antenna Position, and Beamforming Optimization for Pinching-Antenna Systems with Motion Power Consumption](https://arxiv.org/abs/2507.02348)
*Yiming Xu,Dongfang Xu,Xianghao Yu,Shenghui Song,Zhiguo Ding,Robert Schober*
...
Result: 仿真结果表明，考虑天线运动功耗和可调辐射功率的PASS设计显著提升了性能。
...
### [14] [Derivative-Free Optimization-Empowered Wireless Channel Reconfiguration for 6G](https://arxiv.org/abs/2507.02243)
*Peilan Wang,Jun Fang,Xianlong Zeng,Bin Wang,Zhi Chen,Yonina C. Eldar*

Main category: eess.SP

TL;DR: 论文提出了一种无导数优化技术（零阶优化），用于直接优化可重构天线系数以塑造无线端到端信道，无需信道建模和隐式环境传播参数估计。


<details>
  <summary>Details</summary>
Motivation: 在6G及未来无线通信中，可重构天线技术（如RIS、MA、FA等）被广泛研究，但如何实时优化其系数以塑造有利的无线信道仍是一个重大挑战。

Method: 采用无导数优化技术（零阶优化），直接优化可重构系数，避免复杂的信道建模和隐式参数估计。

Result: 通过RIS和可移动天线的SISO系统案例研究，展示了零阶优化方法相对于现有技术的优越性。

Conclusion: 无导数优化为可重构天线技术提供了新的研究方向，并展示了其在无线信道重构中的潜力。
...
### [15] [Localized kernel method for separation of linear chirps](https://arxiv.org/abs/2507.02262)
*Eric Mason,Sippanon Kitimoon,Hrushikesh Mhaskar*

Main category: eess.SP

TL;DR: 本文提出了一种改进的信号分离方法，用于处理交叉、低信噪比和不连续情况下的线性调频信号。


<details>
  <summary>Details</summary>
Motivation: 解决信号处理中信号叠加分离的挑战，特别是在音频和雷达信号领域。

Method: 改进和扩展了Chui和Mhaskar的信号分离算子（SSO），用于处理交叉、低信噪比和不连续情况下的线性调频信号。

Result: 理论分析了SSO在噪声下的行为，并通过模拟数据集验证了方法的有效性。

Conclusion: 改进后的SSO方法在处理复杂信号叠加时表现出色，特别是在低信噪比和交叉情况下。
...
### [16] [STAR-RIS Transceivers: Integrated Sensing and Communication with Pulsed Signals](https://arxiv.org/abs/2507.02346)
*Hedieh Taremizadeh,Emanuele Grossi,Luca Venturino*

Main category: eess.SP

TL;DR: 该研究提出了一种结合STAR-RIS和PESA接收器的ISAC收发器，通过空间和时间调制实现雷达探测和通信功能。
...
Result: 在雷达性能影响最小的情况下，实现了通信消息的嵌入和目标探测。

Conclusion: 该方法为ISAC系统提供了一种高效的解决方案，具有实际应用潜力。
...
### [17] [Track-before-detect in RIS-aided Integrated Sensing and Communication](https://arxiv.org/abs/2507.02352)
*Georgios Mylonopoulos,Luca Venturino,Emanuele Grossi,Stefano Buzzi,Ciro D'Elia*
...
<details>
  <summary>Details</summary>
Motivation: 研究如何通过基站的多功能（感知与通信）优化系统性能，特别是在频谱效率和感知性能之间找到更好的平衡。

Method: 采用多帧雷达检测器（包括检测器、绘图提取器和检测前跟踪处理器），联合处理更多扫描以提升频谱效率。

Result: 数值分析验证了该方法的有效性，并评估了可实现的系统性能平衡。

Conclusion: 提出的方法能有效提升用户频谱效率，同时不牺牲感知性能，为多功能基站设计提供了新思路。
...
### [18] [Predictive Control over LAWN: Joint Trajectory Design and Resource Allocation](https://arxiv.org/abs/2507.02374)
*Haijia Jin,Jun Wu,Weijie Yuan,Ruizhi Ruan,Jiacheng Wang,Dusit Niyato,Dong In Kim,Abbas Jamalipour*

Main category: eess.SP

TL;DR: 该论文研究了低空无线网络（LAWN）中无人机通过有限块长传输为多台移动自动导引车（AGV）提供实时无线控制的问题，采用模型预测控制（MPC）和优化方法，提出了一种高效算法。


<details>
  <summary>Details</summary>
Motivation: 低空无线网络（LAWN）在物联网（IoT）系统中具有潜力，但实时无线控制面临挑战，需要解决通信可靠性和控制性能的联合优化问题。

Method: 采用模型预测控制（MPC）确保轨迹跟踪精度，分析通信可靠性，并通过优化问题联合确定控制策略、功率分配和无人机轨迹。提出交替优化（AO）框架，结合投影梯度下降（PGD）和逐次凸近似（SCA）技术求解。

Result: 通过仿真和实验验证，所提方法在控制性能上优于基线方案。

Conclusion: 论文提出的联合优化方法有效解决了LAWN系统中的实时控制问题，具有实际应用潜力。
...
### [19] [Parameter estimation of range-migrating targets using OTFS signals from LEO satellites](https://arxiv.org/abs/2507.02385)
*Tong Ding,Luca Venturino,Emanuele Grossi*
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在解决高速目标在LEO卫星通信系统中的参数估计问题，特别是针对存在距离迁移的情况。

Method: 利用OTFS调制信号的特性，推导了高速目标回波的输入输出模型，提出了一种基于块正交匹配追踪和匹配滤波器组的最大似然估计器近似实现方法。

Result: 目标响应在延迟-多普勒域呈现稀疏结构，距离迁移导致响应扩散。提出的估计方法在数值实验中表现出良好的性能。

Conclusion: 研究为LEO卫星通信系统中的高速目标参数估计提供了一种有效方法，并揭示了目标响应的新特性。
...
### [20] [When Attention is Beneficial for Learning Wireless Resource Allocation Efficiently?](https://arxiv.org/abs/2507.02427)
*Jia Guo,Chenyang Yang*

Main category: eess.SP

TL;DR: 论文探讨了注意力机制在图神经网络（GNNs）中是否必要，通过分析集合上的函数结构和数值算法，发现干扰未反映时可省略注意力机制。


<details>
  <summary>Details</summary>
Motivation: 研究注意力机制在资源分配中的必要性，以提升GNNs的学习效率和泛化能力。

Method: 分析集合上的置换等变函数结构，并重新表达数值算法为递归形式，设计GNN框架。

Result: 证明干扰未反映时可省略注意力机制，并通过仿真验证了所提GNN的学习效率。

Conclusion: 注意力机制在资源分配中并非总是必要，干扰未反映时可简化模型设计。
...
### [21] [Corrections to Published Values of Frequency Sampling Filter Transition Coefficients](https://arxiv.org/abs/2507.02556)
*C. S. Ramalingam*
...
<details>
  <summary>Details</summary>
Motivation: 由于Rabiner等人和Lyons在文献中提供的过渡系数和PSL值存在差异且不准确，本文旨在通过程序计算提供优化的正确值。

Method: 通过程序计算优化过渡系数和PSL值，并与Rabiner等人和Lyons的数据进行对比。

Result: 发现并纠正了文献中的错误数据，提供了优化后的过渡系数和PSL值，例如对于N=16和BW=4的情况，最优值为0.40474097。

Conclusion: 本文提供了频率采样滤波器设计中的准确优化数据，纠正了现有文献中的错误。
...
### [22] [Pinching-Antenna-Assisted Index Modulation: Channel Modeling, Transceiver Design, and Performance Analysis](https://arxiv.org/abs/2507.02641)
*Shuaixin Yang,Yijia Li,Yue Xiao,Yong Liang Guan,Xianfu Lei,Zhiguo Ding*
...
Method: 设计了全面的收发器方案，包括信道建模、低复杂度的BOSD检测算法、BER分析上界推导以及基于流形优化的预编码方法。

Result: 仿真结果表明，PA-IM方案在性能上显著优于传统方案，且预编码设计显著降低了系统的误码率。

Conclusion: PA-IM方案为无线通信提供了一种高效的频谱利用方法，同时保持了低硬件复杂度。
...
### [23] [AREE-Based Decoupled Design of Hybrid Beamformers in mmWave XL-MIMO Systems](https://arxiv.org/abs/2507.02802)
*Jiazhe Li,Nicolò Decarli,Francesco Guidi,Heng Dong,Anna Guerra,Alessandro Bazzi,Zhuoming Li*
...
<details>
  <summary>Details</summary>
Motivation: 混合波束形成在毫米波通信中广泛使用，但其模拟和数字预编码器的耦合限制了现有算法的效率和性能。

Method: AREE算法将混合波束形成问题分解为两个低维子问题，通过迭代消除残差误差实现解耦，并采用低复杂度几何信道SVD算法简化计算。

Result: 仿真结果表明，AREE算法能以低复杂度有效解耦模拟和数字预编码器，快速收敛，且频谱效率优于现有方法。

Conclusion: AREE算法为解决混合波束形成的耦合问题提供了一种高效且性能优越的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 毫米波MIMO系统的直接通信路径易受遮挡，传统ES方法计算复杂且耗时，需更高效的方法优化RIS辅助的预编码设计。

Method: 采用离散傅里叶变换（DFT）向量设计码本，并开发DNN模型加速码字选择，替代传统ES方法。
...
### [25] [Learnable-Differentiable Finite Volume Solver for Accelerated Simulation of Flows](https://arxiv.org/abs/2507.01975)
...
<details>
  <summary>Details</summary>
Motivation: 传统数值求解器需要精细的时空网格以保证稳定性、一致性和收敛性，计算成本高；机器学习方法虽高效，但存在可解释性、泛化性和数据依赖性问题。

Method: LDSolver由两部分组成：(1)可微分有限体积求解器，(2)可学习模块，用于在粗网格上提供等效通量近似和时间误差校正。

Result: 实验表明，即使在有限训练数据下，LDSolver也能在多种流动系统中实现高效模拟，并保持高精度和优越的泛化性能。

Conclusion: LDSolver在粗网格上实现了高效准确的流体模拟，性能优于基线模型，具有广泛的应用潜力。
...
### [26] [Padé Approximant Neural Networks for Enhanced Electric Motor Fault Diagnosis Using Vibration and Acoustic Data](https://arxiv.org/abs/2507.02599)
...
Method: 比较一维CNN、Self-ONNs和PadéNets在公开数据集上的表现，PadéNet设计增强非线性并兼容无界激活函数。
...
### [27] [DKGCM: A Spatio-Temporal Prediction Model for Traffic Flow by Fusing Spatial Node Clustering Method and Fourier Bidirectional Mamba Mechanism](https://arxiv.org/abs/2507.01982)
...
Method: 提出DK-GCN方法，结合DTW和K-means聚类分组交通节点；利用FFT和双向Mamba框架捕捉时间依赖性；引入GRPO强化学习优化训练。

Result: 在三个公开数据集上表现优于多种先进方法。

Conclusion: DKGCM模型显著提升了交通需求预测的准确性，为时空依赖性建模提供了新思路。
...
### [28] [Sample Complexity Bounds for Linear Constrained MDPs with a Generative Model](https://arxiv.org/abs/2507.02089)
...
<details>
  <summary>Details</summary>
Motivation: 解决约束马尔可夫决策过程中最大化期望累积奖励同时满足累积约束的问题，特别是在高维特征空间和表格CMDPs中的高效求解。

Method: 使用原始-对偶框架，结合黑盒无约束MDP求解器（如MDVI），分析松弛可行性和严格可行性两种情况下的样本复杂度。

Result: 松弛可行性下，样本复杂度为Õ(d²/((1-γ)⁴ε²))；严格可行性下，复杂度为Õ(d²/((1-γ)⁶ε²ζ²))，其中ζ为Slater常数。

Conclusion: 该框架在表格CMDPs中能恢复接近最优的样本复杂度，为约束MDP问题提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 社交媒体在选举和危机期间充斥大量虚假信息，现有研究多集中于单模态（文本或图像），而多模态特征组合的研究较少。

Method: 采用早期融合方法，结合文本、图像和社交特征，通过数据增强提取视觉和社交特征，使用无监督和监督机器学习模型进行分类。
...
### [30] [Improving Constrained Generation in Language Models via Self-Distilled Twisted Sequential Monte Carlo](https://arxiv.org/abs/2507.02315)
...
<details>
  <summary>Details</summary>
Motivation: 在受限文本生成中，目标分布集中于基础模型不太可能的输出上，导致学习困难。

Method: 采用自蒸馏方法迭代优化基础模型，使其逐步与目标对齐。

Result: 显著提升了生成质量。

Conclusion: 自蒸馏是解决受限文本生成中稀疏奖励问题的有效方法。
...
### [31] [Positive region preserved random sampling: an efficient feature selection method for massive data](https://arxiv.org/abs/2507.01998)
...
### [32] [Online Conformal Prediction with Efficiency Guarantees](https://arxiv.org/abs/2507.02496)
*Vaidehi Srinivas*
...
Method: 针对任意和可交换输入序列，分别设计算法，分析其覆盖率和区间长度的表现。

Result: 在可交换序列中，算法能接近最优覆盖率和长度；在任意序列中，存在效率与覆盖率的权衡。

Conclusion: 可交换与任意序列的算法表现存在差异，需针对不同场景设计不同策略。
...
### [33] [Continuous Wavelet Transform and Siamese Network-Based Anomaly Detection in Multi-variate Semiconductor Process Time Series](https://arxiv.org/abs/2507.01999)
...
Method: 1. 使用连续小波变换将时间序列数据转换为图像；2. 微调VGG-16架构进行图像分类；3. 构建Siamese网络比较输入图像的嵌入向量。
...
### [34] [Classification by Separating Hypersurfaces: An Entropic Approach](https://arxiv.org/abs/2507.02732)
...
Method: 在超立方体中搜索参数向量，结合熵最小化函数，扩展到多项式曲面以实现复杂边界分类。

Result: 数值实验证明该方法高效且通用，适用于线性和非线性可分任务。

Conclusion: 该方法为分类问题提供了更灵活和鲁棒的解决方案。
...
### [35] [Temporal Chain of Thought: Long-Video Understanding by Thinking in Frames](https://arxiv.org/abs/2507.02001)
...
<details>
  <summary>Details</summary>
Motivation: 尽管现有视觉语言模型（VLMs）能处理约1000帧输入，但在长视频理解中仍难以有效利用序列长度并易受无关内容干扰。

Method: 利用VLM自身迭代识别并提取视频中最相关的帧，用于回答问题。

Result: 在4个多样化视频问答数据集上取得最优结果，尤其在超过1小时的长视频上表现突出，32K上下文窗口优于标准推理的700K窗口2.8分。

Conclusion: 通过推理时计算优化上下文选择，显著提升了长视频问答的准确性和模型性能。
...
### [36] [Contextual Online Pricing with (Biased) Offline Data](https://arxiv.org/abs/2507.02762)
...
<details>
  <summary>Details</summary>
Motivation: 研究如何在存在偏差离线数据的情况下，优化在线定价策略，以提高统计复杂性和性能。

Method: 通过分析离线数据的偏差、规模、分散性等参数，提出了一种Optimism-in-the-Face-of-Uncertainty (OFU)策略，并针对不同情况设计了相应的算法。

Result: 对于标量价格弹性情况，实现了最优的实例依赖遗憾边界；对于一般价格弹性情况，提出了最坏情况下的最优遗憾边界。

Conclusion: 论文首次为带有偏差离线数据的上下文定价问题提供了严格的遗憾保证，其方法也适用于带有偏差离线数据的随机线性bandit问题。
...
### [37] [AIRES: Accelerating Out-of-Core GCNs via Algorithm-System Co-Design](https://arxiv.org/abs/2507.02006)
...
Conclusion: AIRES通过算法和系统协同设计，有效提升了外存SpGEMM的性能，为大规模图数据处理提供了高效解决方案。
...
### [38] [GeoAda: Efficiently Finetune Geometric Diffusion Models with Equivariant Adapters](https://arxiv.org/abs/2507.02085)
...
Result: GeoAda在多种几何控制任务和应用领域（如分子动力学、人体运动预测）中表现优异，优于其他基线方法。

Conclusion: GeoAda为几何扩散模型提供了一种灵活且高效的微调方法，同时保持了原始模型的几何特性和性能。
...
### [39] [Evaluating the Promise and Pitfalls of LLMs in Hiring Decisions](https://arxiv.org/abs/2507.02087)
...
<details>
  <summary>Details</summary>
Motivation: 探讨LLM在招聘中的潜在偏见问题，并提出领域专用模型是否能更好地平衡准确性和公平性。

Method: 通过ROC AUC、Precision-Recall AUC、F1-score等指标评估模型准确性，并通过影响比例分析公平性。

Result: Match Score在准确性（ROC AUC 0.85 vs 0.77）和公平性（最低影响比例0.957 vs 0.809）上均优于LLM。

Conclusion: 领域专用模型和偏见审核对高风险的招聘领域至关重要，通用LLM需谨慎使用。
...
### [40] [Energy-Based Transformers are Scalable Learners and Thinkers](https://arxiv.org/abs/2507.02092)
...
### [41] [Parametric Neural Amp Modeling with Active Learning](https://arxiv.org/abs/2507.02109)
...
<details>
  <summary>Details</summary>
Motivation: 传统吉他放大器建模需要大量数据点，PANAMA旨在通过主动学习策略减少所需数据量。

Method: 使用基于梯度的优化算法确定最佳采样数据点，结合WaveNet架构进行建模。

Result: 在有限样本条件下，该方法有效减少了所需数据点数量。

Conclusion: PANAMA通过主动学习策略和梯度优化，实现了高效且数据节省的吉他放大器建模。
...
### [42] [Scaling Collapse Reveals Universal Dynamics in Compute-Optimally Trained Neural Networks](https://arxiv.org/abs/2507.02119)
...
Result: 发现损失曲线在归一化后高度一致，形成“超级坍缩”，且在不同数据集、架构和学习率计划中均成立。

Conclusion: 超级坍缩现象为神经网络训练提供了普适性规律，可作为超参数优化的实用指标。
...
### [43] [CROP: Circuit Retrieval and Optimization with Parameter Guidance using LLMs](https://arxiv.org/abs/2507.02128)
...
<details>
  <summary>Details</summary>
Motivation: 现代VLSI设计的复杂性导致参数空间庞大，手动优化效率低下且依赖专家经验，亟需自动化解决方案。

Method: CROP包括RTL代码向量化、嵌入检索系统和RAG增强的LLM参数搜索，结合相似设计的先验知识优化搜索过程。
...
### [44] [Generative Latent Diffusion for Efficient Spatiotemporal Data Reduction](https://arxiv.org/abs/2507.02129)
...
Conclusion: 该方法显著降低了存储成本，同时实现了高精度的时空重建。
...
### [45] [Non-exchangeable Conformal Prediction for Temporal Graph Neural Networks](https://arxiv.org/abs/2507.02151)
...
<details>
  <summary>Details</summary>
Motivation: 现有共形预测方法主要针对静态图，忽略了真实图中时间依赖性对交换性假设的违反，限制了其应用。

Method: 提出了基于扩散的非共形性评分，捕捉拓扑和时间不确定性，并开发了效率感知的优化算法。

Result: 在多个真实时序图数据集上验证，NCPNET显著减少预测集大小（如WIKI数据集减少31%），并提高效率。

Conclusion: NCPNET有效扩展了共形预测至动态场景，提升了时序图中的不确定性和效率。
...
### [46] [Statistical Inference for Responsiveness Verification](https://arxiv.org/abs/2507.02169)
...
<details>
  <summary>Details</summary>
Motivation: 机器学习模型在贷款、招聘等场景中因未考虑个体输入变化而导致安全失败，需验证预测的响应性。

Method: 提出基于敏感性分析的响应性验证框架，通过约束干预和分布效应估计模型响应性，支持黑盒模型评估。

Result: 开发算法生成可达点均匀样本，验证方法在累犯预测、器官移植优先级和内容审核等应用中有效。
...
### [47] [Metric Design != Metric Behavior: Improving Metric Selection for the Unbiased Evaluation of Dimensionality Reduction](https://arxiv.org/abs/2507.02225)
...
<details>
  <summary>Details</summary>
Motivation: 现有维度降维评估可能因选择高度相关的指标而产生偏差，影响评估的可靠性。

Method: 通过计算指标间的成对相关性，聚类指标以减少重叠，并从每个聚类中选择代表性指标。

Result: 实验表明该方法提高了评估的稳定性，有效减少了偏差。

Conclusion: 提出的工作流程有助于减轻维度降维评估中的偏差，提升评估的可靠性。
...
### [48] [PhysicsCorrect: A Training-Free Approach for Stable Neural PDE Simulations](https://arxiv.org/abs/2507.02227)
...
<details>
  <summary>Details</summary>
Motivation: 神经网络作为PDE求解的替代模型存在长期预测误差累积问题，导致结果偏离物理真实解。

Method: 提出PhysicsCorrect框架，通过离线预计算雅可比矩阵及其伪逆，以线性化逆问题形式校正PDE残差，降低计算开销。

Result: 在Navier-Stokes、波动方程和Kuramoto-Sivashinsky方程等系统中，预测误差降低100倍，推理时间增加不足5%。

Conclusion: PhysicsCorrect将不稳定神经网络转化为可靠仿真工具，兼具深度学习效率与物理保真度。
...
### [49] [VERBA: Verbalizing Model Differences Using Large Language Models](https://arxiv.org/abs/2507.02241)
...
### [50] [Order Acquisition Under Competitive Pressure: A Rapidly Adaptive Reinforcement Learning Approach for Ride-Hailing Subsidy Strategies](https://arxiv.org/abs/2507.02244)
...
### [51] [Uncertainty-aware Reward Design Process](https://arxiv.org/abs/2507.02256)
...
<details>
  <summary>Details</summary>
Motivation: 传统奖励函数设计方法效率低且不一致，现有基于大型语言模型的方法在数值优化上表现不佳，进化搜索范式则计算开销过大。
...
Conclusion: URDP通过结合大型语言模型的逻辑推理能力和贝叶斯优化的数值优化优势，为自动化奖励设计提供了高效且高质量的解决方案。
...
### [52] [Knowledge Graph-Based Explainable and Generalized Zero-Shot Semantic Communications](https://arxiv.org/abs/2507.02291)
...
Method: 利用知识图语义知识库（KG-SKB）对齐语义特征，结合零样本学习（ZSL）实现无需重新训练的分类。

Result: 在APY数据集上的实验表明，KGZS-SC在多种信噪比下对未见类别的分类性能显著优于现有方法。

Conclusion: KGZS-SC网络通过知识图增强语义表示和推理，有效提升了语义通信的泛化能力和效率。
...
### [53] [Holistic Continual Learning under Concept Drift with Adaptive Memory Realignment](https://arxiv.org/abs/2507.02310)
...
Result: AMR在多个基准数据集上表现优异，性能接近完全重新训练（FR），但显著减少了资源消耗。

Conclusion: AMR是一种可扩展的解决方案，能够在非静态环境中平衡稳定性和适应性。
...
### [54] [Transformer-based EEG Decoding: A Survey](https://arxiv.org/abs/2507.02320)
...
<details>
  <summary>Details</summary>
Motivation: EEG解码是脑机接口研究的前沿，传统机器学习方法逐渐被深度学习取代，Transformer因其处理序列数据的能力在该领域日益流行。

Method: 文章梳理了Transformer的基础及其在EEG解码中的直接应用，详细介绍了与卷积/循环/图神经网络等混合架构，以及定制化Transformer的改进结构。

Result: 总结了Transformer在EEG解码中的研究进展，展示了其在特征学习和任务性能上的优势。

Conclusion: 讨论了当前挑战和未来发展方向，旨在为读者提供清晰的研究现状和未来研究启示。
...
### [55] [DeltaSHAP: Explaining Prediction Evolutions in Online Patient Monitoring with Shapley Values](https://arxiv.org/abs/2507.02342)
...
Conclusion: DeltaSHAP为临床时间序列解释提供了一种高效、实用的解决方案，适用于时间敏感的应用场景。
...
### [56] [Offline Reinforcement Learning with Penalized Action Noise Injection](https://arxiv.org/abs/2507.02356)
...
<details>
  <summary>Details</summary>
Motivation: 离线强化学习因环境交互成本高而受限，扩散模型虽有效但计算需求大，PANI旨在提供更高效替代方案。

Method: 提出PANI方法，通过噪声注入动作覆盖动作空间，并惩罚噪声量，理论证明其解决修改后的MDP问题。

Result: PANI兼容多种离线RL算法，在多个基准测试中表现显著提升。

Conclusion: PANI是一种简单高效的方法，无需复杂扩散模型即可显著提升离线RL性能。
...
### [57] [Deep Reinforcement Learning-Based DRAM Equalizer Parameter Optimization Using Latent Representations](https://arxiv.org/abs/2507.02365)
...
Method: 采用数据驱动的潜在信号表示快速评估信号完整性，并结合无模型的Advantage Actor-Critic强化学习代理优化参数。

Result: 在行业标准DRAM波形上，该方法显著提升了眼图开口面积（42.7%和36.8%），优于现有技术。

Conclusion: 该方法在性能、计算效率和泛化能力上均表现出色，为复杂均衡器架构提供了高效优化方案。
...
### [58] [Improving Consistency in Vehicle Trajectory Prediction Through Preference Optimization](https://arxiv.org/abs/2507.02406)
...
<details>
  <summary>Details</summary>
Motivation: 当前轨迹预测模型在复杂交互场景中难以捕捉代理间的依赖关系，导致预测不一致。

Method: 利用偏好优化微调轨迹预测模型，输入自动计算的预测未来偏好排名。

Result: 在三个数据集上实验表明，显著提升场景一致性，预测精度几乎不受影响。
...
### [59] [S2FGL: Spatial Spectral Federated Graph Learning](https://arxiv.org/abs/2507.02409)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究仅从结构角度处理子图联邦学习，忽略了图信号在空间和谱域的传播问题，导致标签信号中断和谱域客户端漂移。

Method: 提出全局知识库缓解标签信号中断，频率对齐解决谱域客户端漂移，结合形成S2FGL框架。

Result: 在多个数据集上的实验验证了S2FGL的优越性。

Conclusion: S2FGL有效解决了联邦图学习中的空间和谱域挑战，提升了全局模型的性能。
...
### [60] [Variational Kolmogorov-Arnold Network](https://arxiv.org/abs/2507.02466)
...
<details>
  <summary>Details</summary>
Motivation: 解决KANs中基函数数量选择的问题，提升其作为MLP替代的实用性。

Method: 采用变分推断优化问题，通过反向传播自适应学习无限基函数。

Result: 成功将KANs的重要超参数纳入学习过程，扩展了其应用潜力。

Conclusion: InfinityKAN为KANs提供了更灵活和自适应的学习框架。
...
### [61] [Continual Gradient Low-Rank Projection Fine-Tuning for LLMs](https://arxiv.org/abs/2507.02503)
...
Method: 提出GORP策略，结合全秩和低秩参数，在统一低秩梯度子空间中联合更新，扩展优化空间并保持效率。

Result: 在持续学习基准测试中，GORP表现优于现有最先进方法。

Conclusion: GORP有效解决了LLMs持续微调中的效率与表达能力矛盾，同时减轻灾难性遗忘。
...
### [62] [TFOC-Net: A Short-time Fourier Transform-based Deep Learning Approach for Enhancing Cross-Subject Motor Imagery Classification](https://arxiv.org/abs/2507.02510)
...
### [63] [RetrySQL: text-to-SQL training with retry data for self-correcting query generation](https://arxiv.org/abs/2507.02529)
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到SQL任务中缺乏针对SQL的生成模型研究，且自校正生成策略的应用尚未探索。

Method: 通过准备参考SQL查询的推理步骤并人为制造错误数据，结合特殊标记进行预训练。

Result: RetrySQL在整体和执行准确率上提升了4个百分点，并验证了全参数预训练的必要性。

Conclusion: RetrySQL证明了自校正能力在文本到SQL任务中的有效性，为SQL导向语言模型提供了新思路。
...
### [64] [Position: A Theory of Deep Learning Must Include Compositional Sparsity](https://arxiv.org/abs/2507.02550)
*David A. Danhofer,Davide D'Ascenzo,Rafael Dubach,Tomaso Poggio*

Main category: cs.LG

TL;DR: 论文探讨了过参数化深度神经网络（DNNs）成功的关键在于其能够利用目标函数的组合稀疏结构。


<details>
  <summary>Details</summary>
Motivation: 理解DNNs学习动态的基本原理，解释其在高维数据中的成功表现。

Method: 提出DNNs通过利用组合稀疏结构（即目标函数由少量低维输入的函数组成）实现高效学习。

Result: 组合稀疏性是所有高效图灵可计算函数的共同特性，可能存在于当前所有学习问题中。

Conclusion: 深入理解组合稀疏性在深度学习中的作用对构建全面的人工智能理论至关重要。
...
TL;DR: 论文表明层归一化（LN）在GPT-2模型中可以被移除，且对验证损失影响很小，同时有助于提升模型的可解释性。


<details>
  <summary>Details</summary>
Motivation: 研究LN在推理阶段的作用及其对模型可解释性的影响。

Method: 移除GPT-2模型中的所有LN层，并评估验证损失和可解释性。

Result: 移除LN层仅导致验证损失小幅增加（如GPT-2 XL增加0.03交叉熵损失），且可解释性技术效果更直接。

Conclusion: LN层对语言建模作用有限，移除后模型仍能有效运行，有助于更精确的可解释性研究。
...
TL;DR: 扩展了可微布尔逻辑网络（DBNs），引入可训练、参数恒定的可微互连，支持更宽输入层；提出两阶段剪枝方法提升压缩效率。


<details>
  <summary>Details</summary>
Motivation: 解决传统DBNs在输入宽度增长时参数爆炸的问题，同时优化模型压缩效果。

Method: 1. 引入可微互连设计，参数恒定；2. 提出两阶段剪枝：基于SAT的逻辑等价剪枝和基于相似性的数据驱动剪枝。

Result: 扩展后的DBNs支持更宽输入层，剪枝方法在压缩率和准确率上优于基线。

Conclusion: 新方法显著提升了DBNs的可扩展性和压缩效率，适用于资源受限硬件。
...
### [67] [Lost in Latent Space: An Empirical Study of Latent Diffusion Models for Physics Emulation](https://arxiv.org/abs/2507.02608)
*François Rozet,Ruben Ohana,Michael McCabe,Gilles Louppe,François Lanusse,Shirley Ho*

Main category: cs.LG

TL;DR: 该论文研究了在动态系统仿真中应用潜在空间扩散模型的有效性，发现其在高压缩率下仍保持准确性，且优于非生成式方法。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在推理时的高计算成本限制了其作为快速物理仿真器的应用，论文探讨了在潜在空间中进行仿真的可行性。

Method: 通过在自动编码器的潜在空间而非像素空间生成数据，研究了扩散模型在动态系统仿真中的应用。

Result: 潜在空间仿真在高达1000倍的压缩率下仍保持准确性，扩散模型比非生成式方法更准确且预测多样性更强。

Conclusion: 论文总结了训练潜在空间仿真器的关键设计选择，包括架构和优化器，证明了扩散模型在仿真中的潜力。
...
### [68] [L-VAE: Variational Auto-Encoder with Learnable Beta for Disentangled Representation](https://arxiv.org/abs/2507.02619)
*Hazal Mogultay Ozcan,Sinan Kalkan,Fatos T. Yarman-Vural*

Main category: cs.LG

TL;DR: L-VAE是一种可学习变分自编码器，通过学习损失函数的超参数和权重，动态平衡解耦和重构损失，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决β-VAE中超参数η需经验调整的问题，动态优化解耦与重构的权衡。

Method: 提出L-VAE模型，同时学习损失项权重和模型参数，并添加正则化项防止偏差。

Result: 在多个数据集上表现最佳或次佳，成功解耦CelebA中的面部属性。

Conclusion: L-VAE有效平衡重构与解耦，优于现有方法。
...
### [69] [A Matrix Variational Auto-Encoder for Variant Effect Prediction in Pharmacogenes](https://arxiv.org/abs/2507.02624)
*Antoine Honoré,Borja Rodríguez Gálvez,Yoomi Park,Yitian Zhou,Volker M. Lauschke,Ming Xiao*

Main category: cs.LG

TL;DR: 该论文提出了一种基于Transformer的矩阵变分自编码器（matVAE），用于评估蛋白质变体的功能影响，并在DMS数据集上表现出优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 传统VEP方法依赖多序列比对（MSA），但这一假设在药物基因组学中受到挑战，因为某些药物基因的进化压力较低。DMS数据集提供了定量适应性评分，可作为替代方案。

Method: 提出matVAE模型，结合结构化先验，并在33个DMS数据集上评估其性能。还比较了基于MSA和DMS训练的模型，并引入AlphaFold结构数据以提升性能。

Result: matVAE-MSA在零样本预测中优于DeepSequence，且参数更少、计算量更低。结合AlphaFold后，性能进一步提升，接近经过DMS微调的DeepSequence。

Conclusion: DMS数据集有望替代MSA而不显著损失预测性能，需进一步开发DMS数据集并探索其关系以优化变体效应预测。
...
### [70] [Medical Data Pecking: A Context-Aware Approach for Automated Quality Evaluation of Structured Medical Data](https://arxiv.org/abs/2507.02628)
...
<details>
  <summary>Details</summary>
Motivation: EHR数据在流行病学研究和AI训练中的应用日益广泛，但其数据质量问题（如偏差、系统性错误）影响研究结果的可靠性，现有评估方法不足。

Method: 采用Medical Data Pecking方法，结合大语言模型和基础技术生成测试套件，并通过MDPT工具执行测试，识别数据问题。

Result: 在三个数据集（AoU、MIMIC-III、SyntheticMass）上测试，生成55-73个测试，成功识别20-43个数据问题，并分析了测试套件的参考基础和准确性。

Conclusion: 该方法通过结合外部医学知识，提升了数据质量测试的上下文敏感性，为未来扩展（如多模态数据和改进基础方法）奠定了基础。
...
### [71] [High-Order Deep Meta-Learning with Category-Theoretic Interpretation](https://arxiv.org/abs/2507.02634)
*David H. Mguni*
...
<details>
  <summary>Details</summary>
Motivation: 解决传统机器学习依赖人工生成数据的局限性，通过生成虚拟任务和软约束，实现任务的自主探索和泛化。
...
### [72] [On Efficient Bayesian Exploration in Model-Based Reinforcement Learning](https://arxiv.org/abs/2507.02639)
...
<details>
  <summary>Details</summary>
Motivation: 解决强化学习中数据高效探索的挑战，特别是针对认知不确定性的探索奖励设计。

Method: 提出了一种基于信息论的探索奖励方法，结合稀疏变分高斯过程、深度核和深度集成模型的近似方法，并提出了PTS-BE框架。

Result: PTS-BE在稀疏奖励和纯探索任务中显著优于其他基线方法。

Conclusion: 该方法为基于信息增益的探索提供了理论保证，并通过PTS-BE框架实现了样本高效探索。
...
### [73] [Fair Deepfake Detectors Can Generalize](https://arxiv.org/abs/2507.02645)
*Harry Cheng,Ming-Hui Liu,Yangyang Guo,Tianyi Wang,Liqiang Nie,Mohan Kankanhalli*

Main category: cs.LG

TL;DR: 论文提出了一种解决深度伪造检测中公平性与泛化性冲突的新方法DAID，通过因果分析和后门调整，实现了公平性与泛化性的双提升。


<details>
  <summary>Details</summary>
Motivation: 深度伪造检测模型在泛化性和公平性之间存在冲突，现有方法难以同时优化两者。论文首次揭示了公平性与泛化性之间的因果关系，并提出解决方案。

Method: 提出DAID框架，包括人口统计属性感知的数据再平衡和人口统计无关的特征聚合，通过反向倾向加权和对齐损失实现。

Result: 在三个跨域基准测试中，DAID在公平性和泛化性上均优于现有方法。

Conclusion: DAID通过因果分析和干预措施，成功解决了公平性与泛化性的冲突，验证了其理论和实践有效性。
...
### [74] [OmniDraft: A Cross-vocabulary, Online Adaptive Drafter for On-device Speculative Decoding](https://arxiv.org/abs/2507.02659)
...
Method: 采用在线n-gram缓存和混合蒸馏微调解决词汇不匹配问题，结合自适应草拟技术提升速度。
...
### [75] [Guided Generation for Developable Antibodies](https://arxiv.org/abs/2507.02670)
*Siqi Zhao,Joshua Moller,Porfi Quintero-Cadena,Lood van Niekerk*

Main category: cs.LG

TL;DR: 论文提出了一种基于离散扩散模型的抗体序列优化方法，结合SVDD模块提升可开发性。


<details>
  <summary>Details</summary>
Motivation: 优化抗体序列的可开发性（如稳定性、安全性）是临床有效的关键。

Method: 使用离散扩散模型训练自然抗体序列，结合SVDD模块引导生成。

Result: 模型能生成符合自然抗体特征和可开发性要求的序列，SVDD显著提升预测得分。

Conclusion: 该框架为结合结合性和生物物理标准的抗体设计提供了迭代的ML驱动方案。
...
### [76] [Embedding-Based Federated Data Sharing via Differentially Private Conditional VAEs](https://arxiv.org/abs/2507.02671)
*Francesco Di Salvo,Hanh Huyen My Nguyen,Christian Ledig*

Main category: cs.LG

TL;DR: 提出一种基于差分隐私生成模型的数据共享方法，通过基础模型提取紧凑嵌入，降低通信成本和计算开销，支持多样化下游任务。


<details>
  <summary>Details</summary>
Motivation: 深度学习在医学影像中的应用受限于数据稀缺和隐私法规，联邦学习虽支持去中心化训练，但存在高通信成本和任务单一性问题。

Method: 采用差分隐私条件变分自编码器（DP-CVAE）建模全局隐私感知数据分布，通过基础模型提取紧凑嵌入。

Result: 方法在多个特征提取器上验证，提升了隐私性、可扩展性和效率，性能优于传统联邦学习分类器，且嵌入保真度高于DP-CGAN。
...
### [77] [Multi-Agent Reinforcement Learning for Dynamic Pricing in Supply Chains: Benchmarking Strategic Agent Behaviours under Realistically Simulated Market Conditions](https://arxiv.org/abs/2507.02698)
...
<details>
  <summary>Details</summary>
Motivation: 传统ERP系统的静态定价方法忽略了市场参与者之间的战略互动，而单智能体强化学习无法模拟现实供应链的相互依赖性。

Method: 评估了三种MARL算法（MADDPG、MADQN和QMIX）在基于真实电商数据和LightGBM需求预测模型的模拟环境中的表现。

Result: 规则基代理在公平性和价格稳定性上表现最佳，但缺乏竞争动态；MADQN定价最激进，MADDPG则平衡了竞争与公平。
...
### [78] [Fluid Democracy in Federated Data Aggregation](https://arxiv.org/abs/2507.02710)
...
Method: 首先探索现有流体民主协议在联邦学习中的性能，并提出新协议viscous-retained democracy；其次分析流体民主协议的对抗性弱点，并设计FedVRD算法动态限制对手影响。

Result: 新协议在相同假设下优于传统1p1v方法，且能避免影响力积累；FedVRD算法有效减少对手对全局模型的影响。

Conclusion: FedVRD协议在减少资源浪费和对抗性攻击方面表现出色，为联邦学习提供了更高效的解决方案。
...
### [79] [A Forget-and-Grow Strategy for Deep Reinforcement Learning Scaling in Continuous Control](https://arxiv.org/abs/2507.02712)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度强化学习方法存在早期经验过拟合问题，限制了样本效率和泛化能力。受人类婴儿遗忘现象的启发，提出新算法。

Method: 引入两种机制：1) 经验回放衰减（ER Decay）逐步减少早期经验的影响；2) 网络扩展动态增加参数。
...
### [80] [A Comprehensive Machine Learning Framework for Micromobility Demand Prediction](https://arxiv.org/abs/2507.02715)
*Omri Porat,Michael Fire,Eran Ben-Elia*

Main category: cs.LG

TL;DR: 本文提出了一种整合空间、时间和网络依赖性的框架，用于改进无桩电动滑板车的需求预测，准确率比基线模型提高了27%至49%。


<details>
  <summary>Details</summary>
Motivation: 无桩电动滑板车作为环保灵活的交通工具，需求预测对其管理和规划至关重要，但现有研究未能综合空间、时间和网络依赖性。

Method: 提出一个框架，整合空间、时间和网络依赖性，以改进需求预测。

Result: 框架比基线模型提高了27%至49%的预测准确率。
...
### [81] [Hierarchical Multi-Label Contrastive Learning for Protein-Protein Interaction Prediction Across Organisms](https://arxiv.org/abs/2507.02724)
*Shiyi Liu,Buwen Liang,Yuetong Fang,Zixuan Jiang,Renjing Xu*

Main category: cs.LG

TL;DR: HIPPO是一个基于层次对比学习的蛋白质-蛋白质相互作用预测框架，通过多层次的生物表征匹配和对比损失函数，实现了跨物种的高性能预测。


<details>
  <summary>Details</summary>
Motivation: 解决蛋白质-蛋白质相互作用（PPI）预测中数据异质性和稀疏性问题，特别是在跨物种和低数据情况下的挑战。

Method: 提出层次对比学习框架HIPPO，结合蛋白质序列和层次属性，通过多层次的生物表征匹配和数据驱动的惩罚机制优化嵌入空间。

Result: HIPPO在基准数据集上表现优异，优于现有方法，且在低数据情况下表现稳健，具有零样本迁移能力。
...
### [82] [Fast and Simplex: 2-Simplicial Attention in Triton](https://arxiv.org/abs/2507.02754)
*Aurko Roy,Timothy Chou,Sai Surya Duvvuri,Sijia Chen,Jiecao Yu,Xiaodong Wang,Manzil Zaheer,Rohan Anil*

Main category: cs.LG

TL;DR: 论文研究了2-单纯形Transformer架构，通过改进注意力机制提升token效率，优于标准Transformer。


<details>
  <summary>Details</summary>
Motivation: 现代大语言模型依赖海量数据，计算资源不再是唯一瓶颈，需要更高效的token利用架构。

Method: 提出2-单纯形Transformer，将标准点积注意力推广为三线性函数，并通过Triton内核高效实现。

Result: 在数学、编程、推理和逻辑任务中，相同token预算下，2-单纯形Transformer表现优于标准Transformer。

Conclusion: 2-单纯形注意力改变了知识和推理任务的缩放定律指数，验证了其token效率优势。
...
### [83] [Understanding and Improving Length Generalization in Recurrent Models](https://arxiv.org/abs/2507.02782)
*Ricardo Buitrago Ruiz,Albert Gu*

Main category: cs.LG

TL;DR: 论文研究了循环模型（如状态空间模型和线性注意力）在序列长度超出训练上下文长度时性能下降的问题，提出了未探索状态假说，并通过简单训练干预提高了长度泛化能力。


<details>
  <summary>Details</summary>
Motivation: 循环模型理论上可以处理任意长序列，但在超出训练上下文长度时性能显著下降，缺乏长度泛化能力。

Method: 通过实证和理论分析支持未探索状态假说，并测试了简单训练干预（如高斯噪声初始化或不同序列的最终状态初始化）。

Result: 仅用500步后训练（约0.1%的预训练预算），干预措施显著提升了长度泛化能力（如从2k到128k）。
...
### [84] [In-Training Multicalibrated Survival Analysis for Healthcare via Constrained Optimization](https://arxiv.org/abs/2507.02807)
...
<details>
  <summary>Details</summary>
Motivation: 现有生存模型仅在校准群体水平上表现良好，可能导致少数子群体校准不佳，影响临床决策。

Method: GRADUATE将多校准问题转化为约束优化，同时优化校准与区分度，数学证明其解接近最优且可行。

Result: 在真实临床数据集上，GRADUATE优于现有基线模型，展示了其有效性。

Conclusion: GRADUATE解决了子群体校准问题，为生存分析提供了更可靠的模型。
...
### [85] [Replicable Distribution Testing](https://arxiv.org/abs/2507.02814)
*Ilias Diakonikolas,Jingyi Gao,Daniel Kane,Sihan Liu,Christopher Ye*

Main category: cs.LG

TL;DR: 本文系统研究了算法可复制性框架下的分布测试问题，提出了新的可复制算法用于测试离散分布的接近性和独立性，并开发了新的下界方法。


<details>
  <summary>Details</summary>
Motivation: 研究在算法可复制性框架下，如何高效测试概率分布的自然属性，填补现有研究的空白。

Method: 开发新的可复制算法测试分布的接近性和独立性，并提出新的样本复杂度下界证明方法。

Result: 建立了近乎最优的样本复杂度下界，解决了先前工作中的开放性问题。

Conclusion: 本文为可复制性分布测试提供了理论和算法支持，具有广泛的应用潜力。
...
### [86] [ExPO: Unlocking Hard Reasoning with Self-Explanation-Guided Reinforcement Learning](https://arxiv.org/abs/2507.02834)
*Ruiyang Zhou,Shuozhe Li,Amy Zhang,Liu Leqi*

Main category: cs.LG

TL;DR: 论文提出了一种名为ExPO的新方法，通过结合真实答案生成高质量样本，优化语言模型的推理能力，解决了传统强化学习后训练在早期阶段和复杂任务中的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习后训练方法依赖模型初始能力生成正样本，难以解决模型初始失败的复杂推理任务。需要一种方法能够高效探索新推理轨迹。
...
Conclusion: ExPO通过生成高质量样本，有效解决了传统方法的局限性，提升了语言模型在复杂推理任务中的表现。
...
### [87] [LLM-Driven Treatment Effect Estimation Under Inference Time Text Confounding](https://arxiv.org/abs/2507.02843)
*Yuchen Ma,Dennis Frauen,Jonas Schweisthal,Stefan Feuerriegel*

Main category: cs.LG

TL;DR: 论文提出了一种解决训练数据与推理数据不一致导致治疗效果估计偏差的框架，结合大语言模型和双重稳健学习器。


<details>
  <summary>Details</summary>
Motivation: 临床实践中，训练数据通常结构完整，而推理时依赖文本描述，导致治疗效果估计偏差。

Method: 提出新框架，利用大语言模型和双重稳健学习器，解决推理时文本混杂问题。

Result: 实验证明框架在实际应用中的有效性。

Conclusion: 该框架能有效减少因推理时数据不一致导致的偏差，提升治疗效果估计的准确性。
...
### [88] [MvHo-IB: Multi-View Higher-Order Information Bottleneck for Brain Disorder Diagnosis](https://arxiv.org/abs/2507.02847)
...
<details>
  <summary>Details</summary>
Motivation: 高阶相互作用在fMRI数据中的建模可以提升机器学习系统的诊断准确性，但有效提取和利用HOIs仍具挑战性。

Method: 结合信息论中的O信息和矩阵基Renyi熵估计器量化HOIs，设计Brain3DCNN编码器，并采用多视图学习信息瓶颈目标。
...
### [89] [Learnable-Differentiable Finite Volume Solver for Accelerated Simulation of Flows](https://arxiv.org/abs/2507.01975)
*Mengtao Yan,Qi Wang,Haining Wang,Ruizhi Chengze,Yi Zhang,Hongsheng Liu,Zidong Wang,Fan Yu,Qi Qi,Hao Sun*
...
Result: 实验表明，LDSolver在多种流动系统中表现优异，显著优于基线模型。
...
### [90] [Padé Approximant Neural Networks for Enhanced Electric Motor Fault Diagnosis Using Vibration and Acoustic Data](https://arxiv.org/abs/2507.02599)
*Sertac Kilickaya,Levent Eren*
...
<details>
  <summary>Details</summary>
Motivation: 传统方法如加速度计和麦克风在电机状态监测中存在局限性，而具有非线性神经元架构的深度学习模型有望提升诊断性能。

Method: 评估并比较了一维CNN、Self-ONNs和PadéNets三种深度学习架构，使用公开数据集测试。PadéNet设计为增强非线性并兼容无界激活函数。

Result: PadéNets在各项测试中表现最优，诊断准确率高达99.96%。

Conclusion: PadéNets因其增强的非线性和兼容性，显著提升了感应电机故障诊断性能。
...
### [91] [DKGCM: A Spatio-Temporal Prediction Model for Traffic Flow by Fusing Spatial Node Clustering Method and Fourier Bidirectional Mamba Mechanism](https://arxiv.org/abs/2507.01982)
*Siqing Long,Xiangzhi Huang,Jiemin Xie,Ming Cai*

Main category: cs.LG

TL;DR: 提出了一种名为DKGCM的图卷积网络结构，用于提升交通需求预测的准确性，结合时空依赖关系，并通过实验验证其优越性。


<details>
  <summary>Details</summary>
Motivation: 交通系统中复杂的时空关系限制了需求预测模型的性能，因此需要更有效的模型来提升预测准确性。

Method: 提出DK-GCN方法，结合DTW和K-means聚类捕捉空间依赖；在时间尺度上，集成FFT和双向Mamba框架捕捉时间依赖；使用GRPO强化学习优化训练。

Result: 模型在三个公开数据集上表现优于其他先进方法。

Conclusion: DKGCM通过有效捕捉时空依赖关系，显著提升了交通需求预测的准确性。
...
### [92] [Sample Complexity Bounds for Linear Constrained MDPs with a Generative Model](https://arxiv.org/abs/2507.02089)
*Xingtu Liu,Lin F. Yang,Sharan Vaswani*

Main category: cs.LG

TL;DR: 论文提出了一种基于原始-对偶框架的算法，用于解决无限时间折扣约束马尔可夫决策过程（CMDPs），并提供了松弛可行性和严格可行性两种情况的样本复杂度分析。


<details>
  <summary>Details</summary>
Motivation: 解决CMDPs中在满足约束条件下最大化期望累积奖励的问题，提出一种可以结合任意黑盒无约束MDP求解器的通用框架。

Method: 使用原始-对偶框架，结合镜像下降值迭代（MDVI）作为MDP求解器，分析松弛可行性和严格可行性的样本复杂度。

Result: 松弛可行性下，算法能以高概率返回ε-最优策略，样本复杂度为Õ(d²/((1-γ)⁴ε²))；严格可行性下，样本复杂度为Õ(d²/((1-γ)⁶ε²ζ²))，其中ζ为Slater常数。

Conclusion: 该框架在表格CMDPs中也能恢复接近最优的样本复杂度，展示了其在理论和实际中的有效性。
...
### [93] [Multimodal Misinformation Detection Using Early Fusion of Linguistic, Visual, and Social Features](https://arxiv.org/abs/2507.01984)
*Gautam Kishore Shahi*
...
<details>
  <summary>Details</summary>
Motivation: 社交媒体在选举和危机期间充斥虚假信息，现有研究多关注单模态（文本或图像），缺乏对多模态特征组合的探索。
...
### [94] [Improving Constrained Generation in Language Models via Self-Distilled Twisted Sequential Monte Carlo](https://arxiv.org/abs/2507.02315)
*Sooyeon Kim,Giung Nam,Juho Lee*
...
Conclusion: 自蒸馏是解决约束生成中学习挑战的有效方法。
...
### [95] [Positive region preserved random sampling: an efficient feature selection method for massive data](https://arxiv.org/abs/2507.01998)
*Hexiang Bai,Deyu Li,Jiye Liang,Yanhui Zhai*
...
Method: 使用可区分对象对的比例衡量特征集的区分能力，并构造正区域保留样本选择特征子集。

Result: 在11个数据集上验证，能在短时间内找到近似约简，且区分能力高于估计下限。
...
### [96] [Online Conformal Prediction with Efficiency Guarantees](https://arxiv.org/abs/2507.02496)
*Vaidehi Srinivas*

Main category: cs.LG

TL;DR: 研究在线框架下的共形预测问题，直接优化效率，目标是在给定错误覆盖率α和时间T下，输出区间并最小化平均长度，同时在交换序列和任意序列上分析性能差异。


<details>
  <summary>Details</summary>
Motivation: 探索在线共形预测的效率优化问题，填补传统方法与在线场景之间的差距，并分析不同输入序列对算法性能的影响。

Method: 提出确定性算法，针对交换序列和任意序列分别设计策略，分析其覆盖率和效率的权衡。

Result: 交换序列下，算法能接近最优覆盖率和长度；任意序列下，算法需权衡长度和错误率，且无法同时最优处理两种序列。

Conclusion: 在线共形预测的效率优化在交换序列和任意序列下存在显著差异，需针对性设计算法，无法一劳永逸。
...
### [97] [Continuous Wavelet Transform and Siamese Network-Based Anomaly Detection in Multi-variate Semiconductor Process Time Series](https://arxiv.org/abs/2507.01999)
*Bappaditya Dey,Daniel Sorensen,Minjin Hwang,Sandip Halder*
...
<details>
  <summary>Details</summary>
Motivation: 半导体制造过程中数据高维、类别不平衡、噪声多且非平稳，传统方法难以有效检测异常。

Method: 将时间序列转换为图像表示，利用预训练的VGG-16架构构建Siamese网络进行异常检测。
...
### [98] [Classification by Separating Hypersurfaces: An Entropic Approach](https://arxiv.org/abs/2507.02732)
*Argimiro Arratia,Mahmoud El Daou,Henryk Gzyl*

Main category: cs.LG

TL;DR: 论文提出了一种基于熵最小化的新方法，用于在高维空间中寻找分类超平面，适用于线性和非线性可分数据。
...
### [99] [Temporal Chain of Thought: Long-Video Understanding by Thinking in Frames](https://arxiv.org/abs/2507.02001)
*Anurag Arnab,Ahmet Iscen,Mathilde Caron,Alireza Fathi,Cordelia Schmid*
...
Result: 在4个不同的视频问答数据集上取得最优结果，尤其在超过1小时的长视频上表现突出，优于传统推理方法。
...
### [100] [Contextual Online Pricing with (Biased) Offline Data](https://arxiv.org/abs/2507.02762)
*Yixuan Zhang,Ruihao Zhu,Qiaomin Xie*

Main category: cs.LG

TL;DR: 研究基于有偏离线数据的上下文在线定价问题，提出了统计复杂度的度量方法，并设计了最优策略。


<details>
  <summary>Details</summary>
Motivation: 解决在有偏离线数据下，如何优化在线定价策略的问题，填补了现有研究的空白。

Method: 针对标量价格弹性情况，定义了衡量离线数据偏离在线最优值的指标δ²，并提出了Optimism-in-the-Face-of-Uncertainty (OFU)策略。对于一般价格弹性，设计了广义OFU算法。

Result: 证明了OFU策略在标量情况下达到最小最大最优的遗憾界，广义OFU算法在一般情况下也达到最优。当偏差界限V未知时，设计了鲁棒变体。

Conclusion: 首次在有偏离线数据下为上下文定价提供了紧致的遗憾保证，方法也可直接应用于带偏离线数据的随机线性bandits问题。
...
### [101] [AIRES: Accelerating Out-of-Core GCNs via Algorithm-System Co-Design](https://arxiv.org/abs/2507.02006)
*Shakya Jayakody,Youpeng Zhao,Jun Wang*

Main category: cs.LG

TL;DR: 论文提出AIRES，一种算法-系统协同设计方法，用于加速图卷积网络（GCNs）中的稀疏通用矩阵乘法（SpGEMM）计算，解决了现有系统的数据对齐和内存分配瓶颈。


<details>
  <summary>Details</summary>
Motivation: 随着图数据规模的扩大，SpGEMM通常在资源受限系统中以核外方式运行，但现有系统存在高I/O延迟和GPU利用率不足的问题。

Method: AIRES从算法角度提出块级稀疏格式数据对齐和分块算法，系统层面采用三阶段动态调度和双路数据传输策略。

Result: AIRES在真实图处理基准测试中显著优于现有方法，延迟降低高达1.8倍。

Conclusion: AIRES通过算法和系统协同优化，有效提升了核外SpGEMM的计算效率。
...
### [102] [GeoAda: Efficiently Finetune Geometric Diffusion Models with Equivariant Adapters](https://arxiv.org/abs/2507.02085)
*Wanjia Zhao,Jiaqi Han,Siyi Gu,Mingjian Jiang,James Zou,Stefano Ermon*
...
<details>
  <summary>Details</summary>
Motivation: 几何扩散模型在下游任务中的高效微调尚未充分探索，尤其是在不同几何控制条件下的灵活性和参数效率。

Method: GeoAda通过结构化适配器设计，包括控制信号编码、可训练层复制和等变零初始化卷积，仅微调轻量级适配器模块。

Result: GeoAda在多种几何控制类型和应用领域中实现了最先进的微调性能，同时保持了原始任务的准确性。

Conclusion: GeoAda通过SE(3)-等变适配器设计，有效解决了几何扩散模型微调中的过拟合和灾难性遗忘问题，具有广泛适用性。
...
### [103] [Evaluating the Promise and Pitfalls of LLMs in Hiring Decisions](https://arxiv.org/abs/2507.02087)
*Eitan Anzenberg,Arunava Samajpati,Sivasankaran Chandrasekar,Varun Kacholia*
...
<details>
  <summary>Details</summary>
Motivation: 探讨LLM在招聘中的潜在偏见问题，并提出专有模型在减少偏见和提高准确性上的优势。
...
Conclusion: 强调在高风险领域（如招聘）中使用领域专用模型和偏见审计的重要性，并指出准确性与公平性可以兼得。
...
### [104] [Energy-Based Transformers are Scalable Learners and Thinkers](https://arxiv.org/abs/2507.02092)
*Alexi Gladstone,Ganesh Nanduru,Md Mofijul Islam,Peixuan Han,Hyeonjeong Ha,Aman Chadha,Yilun Du,Heng Ji,Jundong Li,Tariq Iqbal*

Main category: cs.LG

TL;DR: 论文提出了一种基于无监督学习的通用推理方法，通过能量最小化实现预测，并在多种任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有推理方法存在模态或问题特定性，且需要额外监督。本文探索是否可以通过无监督学习实现通用推理。

Method: 提出能量基Transformer（EBT），通过能量最小化验证输入与候选预测的兼容性，实现预测。

Result: EBT在训练和推理中均优于现有方法，如Transformer++和Diffusion Transformers，且泛化能力更强。

Conclusion: EBT是一种有前景的新范式，可同时提升模型的学习和推理能力。
...
### [105] [Parametric Neural Amp Modeling with Active Learning](https://arxiv.org/abs/2507.02109)
*Florian Grötschla,Luca A. Lanzendörfer,Longxiang Jiao,Roger Wattenhofer*

Main category: cs.LG

TL;DR: PANAMA是一个基于WaveNet架构的主动学习框架，用于训练端到端的参数化吉他放大器模型，通过优化采样策略减少数据需求。


<details>
  <summary>Details</summary>
Motivation: 传统吉他放大器建模需要大量数据，PANAMA旨在通过主动学习策略最小化所需数据点（如旋钮设置）。

Method: 采用WaveNet-like架构和基于梯度的优化算法，确定最优采样点。

Result: 在有限样本下，该方法有效减少了数据需求并提升了模型性能。

Conclusion: PANAMA为吉他放大器建模提供了一种高效的数据驱动方法。
...
### [106] [Scaling Collapse Reveals Universal Dynamics in Compute-Optimally Trained Neural Networks](https://arxiv.org/abs/2507.02119)
*Shikai Qiu,Lechao Xiao,Andrew Gordon Wilson,Jeffrey Pennington,Atish Agarwala*

Main category: cs.LG

TL;DR: 研究发现，当模型大小和训练时间同步增长时，计算最优训练的神经网络的损失曲线会呈现一种普适性，称为“超级坍缩”。
...
Method: 通过分析不同规模模型的损失曲线，结合学习率衰减和超参数优化，研究其坍缩现象。

Result: 发现损失曲线在归一化后能精确坍缩为一条普适曲线，且超参数优化时坍缩更明显。

Conclusion: 超级坍缩现象为神经网络训练提供了普适规律和优化指标。
...
### [107] [CROP: Circuit Retrieval and Optimization with Parameter Guidance using LLMs](https://arxiv.org/abs/2507.02128)
*Jingyu Pan,Isaac Jacobson,Zheng Zhao,Tung-Chieh Chen,Guanglei Zhou,Chen-Chia Chang,Vineet Rashingkar,Yiran Chen*

Main category: cs.LG

TL;DR: CROP是一个基于大型语言模型（LLM）的自动VLSI设计流程优化框架，通过语义匹配和检索增强生成技术，显著提升了设计效率和质量。
...
Method: CROP包括：1）RTL代码转换为向量表示；2）基于嵌入的语义匹配系统；3）检索增强生成（RAG）的LLM参数搜索系统。

Result: 实验显示，CROP在工业设计中以更少迭代实现更优结果，功耗降低9.9%。
...
### [109] [Non-exchangeable Conformal Prediction for Temporal Graph Neural Networks](https://arxiv.org/abs/2507.02151)
*Tuo Wang,Jian Kang,Yujun Yan,Adithya Kulkarni,Dawei Zhou*
...
### [110] [Statistical Inference for Responsiveness Verification](https://arxiv.org/abs/2507.02169)
*Seung Hyun Cheon,Meredith Stewart,Bogdan Kulynych,Tsui-Wei Weng,Berk Ustun*
...
Result: 开发了算法生成可达点的均匀样本，验证了方法在再犯预测、器官移植优先级和内容审核等应用中的有效性。

Conclusion: 该方法能提升机器学习模型在现实应用中的安全性，支持预测的响应性验证和故障概率估计。
...
### [111] [Metric Design != Metric Behavior: Improving Metric Selection for the Unbiased Evaluation of Dimensionality Reduction](https://arxiv.org/abs/2507.02225)
*Jiyeon Bae,Hyeon Jeon,Jinwook Seo*

Main category: cs.LG

TL;DR: 提出了一种通过聚类评估指标来减少降维投影评估偏见的流程。


<details>
  <summary>Details</summary>
Motivation: 降维投影评估中，高度相关的指标可能导致偏见，需解决这一问题。

Method: 通过计算指标间的相关性、聚类指标并选择代表性指标来减少重叠。

Result: 实验表明该方法提高了评估的稳定性，减少了偏见。

Conclusion: 该流程有效减少了降维投影评估中的偏见。
...
### [112] [PhysicsCorrect: A Training-Free Approach for Stable Neural PDE Simulations](https://arxiv.org/abs/2507.02227)
*Xinquan Huang,Paris Perdikaris*

Main category: cs.LG

TL;DR: PhysicsCorrect是一种无需训练的校正框架，通过基于PDE残差的线性化逆问题解决神经网络长期预测中的误差累积问题，显著提升物理模拟的准确性。
...
Result: 在三种典型PDE系统中，PhysicsCorrect将预测误差降低高达100倍，且推理时间增加不足5%。

Conclusion: PhysicsCorrect能够无缝集成多种神经网络架构，将不稳定的神经代理转化为可靠的物理模拟工具，兼具计算效率和物理保真度。
...
### [113] [VERBA: Verbalizing Model Differences Using Large Language Models](https://arxiv.org/abs/2507.02241)
*Shravan Doda,Shashidhar Reddy Javaji,Zining Zhu*
...
<details>
  <summary>Details</summary>
Motivation: 面对模型泛滥（model lake）现象，用户需要高效比较模型差异，但手动比较成本过高。

Method: 通过LLM采样生成模型差异的文本描述，并设计评估协议验证其信息量。

Result: VERBA在决策树模型上差异描述准确率达80%，结合结构信息后提升至90%。
...
### [114] [Order Acquisition Under Competitive Pressure: A Rapidly Adaptive Reinforcement Learning Approach for Ride-Hailing Subsidy Strategies](https://arxiv.org/abs/2507.02244)
*Fangzhou Shi,Xiaopeng Ke,Xinye Xiong,Kexin Meng,Chang Men,Zhengdan Zhu*

Main category: cs.LG

TL;DR: 论文提出了FCA-RL框架，结合强化学习优化网约车平台的优惠券策略，动态适应市场竞争并满足预算约束。


<details>
  <summary>Details</summary>
Motivation: 网约车平台通过低价排名机制激励服务商降价，但现有研究缺乏动态优化优惠券策略的方法。

Method: 提出FCA-RL框架，整合快速竞争适应（FCA）和强化拉格朗日调整（RLA），并开发RideGym模拟环境。

Result: 实验表明FCA-RL在多种市场条件下优于基线方法，有效优化补贴策略。

Conclusion: FCA-RL为网约车服务商提供了一种动态适应市场的高效优惠券策略优化方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统奖励工程方法效率低且不一致，LLMs在数值优化中表现不佳，进化搜索资源利用低效。

Method: 提出URDP框架，通过自一致性分析量化奖励函数不确定性，引入UABO优化超参数配置，采用双层优化架构。

Result: 在35个任务中，URDP生成更高质量的奖励函数，显著提升设计效率。

Conclusion: URDP有效整合LLMs和贝叶斯优化，解决了奖励函数设计的核心挑战。
...
### [116] [Knowledge Graph-Based Explainable and Generalized Zero-Shot Semantic Communications](https://arxiv.org/abs/2507.02291)
*Zhaoyu Zhang,Lingyi Wang,Wei Wu,Fuhui Zhou,Qihui Wu*
...
Method: 利用知识图谱语义知识库（KG-SKB）对齐语义特征，减少通信开销；接收端采用零样本学习（ZSL）直接分类未见数据。

Result: 在APY数据集上，KGZS-SC网络表现出强大的泛化能力，在不同信噪比下显著优于现有框架。

Conclusion: KGZS-SC网络通过知识图谱增强语义表示和零样本学习，有效提升了语义通信的适应性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 传统持续学习方法假设数据分布静态，忽视了现实数据流的动态性。概念漂移要求模型同时具备稳定性和快速适应性。

Method: 提出AMR方法，通过选择性移除过时样本并补充新样本，动态调整记忆缓冲区，以匹配新的数据分布。
...
<details>
  <summary>Details</summary>
Motivation: EEG解码是脑机接口研究的关键，传统机器学习方法逐渐被深度学习取代，Transformer因其处理序列数据的能力在EEG任务中日益流行。

Method: 通过综述Transformer在EEG解码中的应用，包括其基础架构、与其他深度学习技术的混合架构，以及定制化结构的改进。

Result: 总结了Transformer在EEG解码中的优势与进展，包括特征自动学习和任务性能提升。

Conclusion: 尽管Transformer在EEG解码中表现出色，但仍面临挑战，未来需进一步优化架构和应用场景。
...
### [119] [DeltaSHAP: Explaining Prediction Evolutions in Online Patient Monitoring with Shapley Values](https://arxiv.org/abs/2507.02342)
*Changhun Kim,Yechan Mun,Sangchul Hahn,Eunho Yang*
...
<details>
  <summary>Details</summary>
Motivation: 现有XAI方法无法满足临床时间序列解释任务的需求，DeltaSHAP旨在解决这一问题，帮助理解患者风险变化的原因。

Method: DeltaSHAP改进Shapley值，适应时间序列数据，解释连续预测变化而非孤立预测分数，并提供特征归因的大小和方向。
...
### [120] [Offline Reinforcement Learning with Penalized Action Noise Injection](https://arxiv.org/abs/2507.02356)
*JunHyeok Oh,Byung-Jun Lee*

Main category: cs.LG

TL;DR: 本文提出了一种名为PANI的简单方法，通过注入噪声动作来增强离线强化学习性能，同时惩罚噪声量，避免了扩散模型的高计算成本。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习在环境交互成本高时具有实用性，但现有扩散模型方法计算成本高，需探索更高效的方法。

Method: 提出PANI方法，通过噪声注入动作覆盖整个动作空间，并根据噪声量进行惩罚，理论证明其解决了一个修正的MDP问题。

Result: PANI与多种离线RL算法兼容，在多个基准测试中表现出显著性能提升。

Conclusion: PANI是一种简单高效的方法，无需复杂扩散模型即可提升离线RL性能。
...
### [121] [Deep Reinforcement Learning-Based DRAM Equalizer Parameter Optimization Using Latent Representations](https://arxiv.org/abs/2507.02365)
*Muhammad Usama,Dong Eui Chang*

Main category: cs.LG

TL;DR: 本文提出了一种基于数据驱动和强化学习的动态随机存取存储器（DRAM）系统中信号完整性优化方法，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 高速DRAM系统中信号完整性优化通常计算量大或依赖模型，需要一种更高效的方法。
...
Conclusion: 该方法在性能、计算效率和泛化能力上优于现有技术，核心贡献包括高效的潜在信号完整性度量、无模型强化学习策略及验证的优越性能。
...
### [122] [Improving Consistency in Vehicle Trajectory Prediction Through Preference Optimization](https://arxiv.org/abs/2507.02406)
*Caio Azevedo,Lina Achaji,Stefano Sabatini,Nicola Poerio,Grzegorz Bartyzel,Sascha Hornauer,Fabien Moutarde*

Main category: cs.LG

TL;DR: 论文提出了一种基于偏好优化的轨迹预测模型微调方法，显著提高了场景一致性，同时几乎不牺牲预测精度。
...
Result: 在三个数据集上的实验表明，该方法显著提升了场景一致性，且预测精度几乎不受影响。

Conclusion: 偏好优化是一种有效的方法，可在不增加计算负担的情况下提升轨迹预测的交互一致性。
...
### [123] [S2FGL: Spatial Spectral Federated Graph Learning](https://arxiv.org/abs/2507.02409)
*Zihan Tan,Suyuan Huang,Guancheng Wan,Wenke Huang,He Li,Mang Ye*
...
### [124] [Variational Kolmogorov-Arnold Network](https://arxiv.org/abs/2507.02466)
*Francesco Alesiani,Henrik Christiansen,Federico Errica*
...
<details>
  <summary>Details</summary>
Motivation: KANs基于Kolmogorov-Arnold定理，但实际应用中基函数数量的选择依赖经验，限制了其作为MLP替代方案的潜力。

Method: 提出InfinityKAN，通过变分推断优化自适应学习无限基函数，将超参数纳入学习过程。

Result: 扩展了KANs的适用性，解决了基函数数量选择的限制。
...
### [125] [Continual Gradient Low-Rank Projection Fine-Tuning for LLMs](https://arxiv.org/abs/2507.02503)
*Chenxu Wang,Yilin Lyu,Zicheng Sun,Liping Jing*
...
### [126] [TFOC-Net: A Short-time Fourier Transform-based Deep Learning Approach for Enhancing Cross-Subject Motor Imagery Classification](https://arxiv.org/abs/2507.02510)
*Ahmed G. Habashi,Ahmed M. Azab,Seif Eldawlatly,Gamal M. Aly*
...
<details>
  <summary>Details</summary>
Motivation: 跨被试运动想象分类因个体间脑电图（EEG）模式差异大而具有挑战性，导致分类精度低，限制了无校准脑机接口（BCI）的实际应用。

Method: 采用短时傅里叶变换（STFT）优化预处理和卷积神经网络（CNN）训练，结合平衡批处理策略。

Result: 在多个数据集上取得显著改进，分类准确率分别为67.60%（IV-1）、65.96%（IV-2A）和80.22%（IV-2B）。

Conclusion: 该方法为无校准运动想象分类设定了新基准，并提供了一个开放数据集以推动该领域研究。
...
### [127] [RetrySQL: text-to-SQL training with retry data for self-correcting query generation](https://arxiv.org/abs/2507.02529)
*Alicja Rączkowska,Riccardo Belluzzo,Piotr Zieliński,Joanna Baran,Paweł Olszewski*
...
Method: 通过准备参考SQL查询的推理步骤并人为引入错误，生成包含错误和纠正步骤的数据，对开源编码模型进行持续预训练。

Result: RetrySQL在整体和挑战性执行准确率上提升了4个百分点，且全参数预训练是必要的。
...
### [128] [Position: A Theory of Deep Learning Must Include Compositional Sparsity](https://arxiv.org/abs/2507.02550)
*David A. Danhofer,Davide D'Ascenzo,Rafael Dubach,Tomaso Poggio*
...
<details>
  <summary>Details</summary>
Motivation: 探讨DNNs学习动态的基本原理，特别是组合稀疏性在其成功中的作用。

Method: 通过理论分析，论证组合稀疏性是DNNs成功的关键，并指出其与图灵可计算函数的关联。

Result: 组合稀疏性是DNNs高效学习的基础，但关于其可学习性和优化的关键问题仍需解决。
...
### [129] [Transformers Don't Need LayerNorm at Inference Time: Scaling LayerNorm Removal to GPT-2 XL and the Implications for Mechanistic Interpretability](https://arxiv.org/abs/2507.02559)
*Luca Baroni,Galvin Khara,Joachim Schaeffer,Marat Subkhankulov,Stefan Heimersheim*
...
Result: 移除LN层后验证损失仅小幅增加（如GPT-2 XL增加0.03交叉熵损失），且微调数据需求随模型参数增长呈次线性关系。

Conclusion: GPT-2类模型无需LN层即可运行，移除LN层有助于提升模型可解释性研究。
...
### [130] [Scalable Interconnect Learning in Boolean Networks](https://arxiv.org/abs/2507.02585)
*Fabian Kresse,Emily Yu,Christoph H. Lampert*

Main category: cs.LG

TL;DR: 论文提出了一种可扩展的差分布尔逻辑网络（DBNs），通过可训练差分互连和两阶段剪枝方法，提升了模型效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 解决现有DBNs在输入宽度增加时参数数量增长的问题，同时进一步减小模型规模。

Method: 引入可训练差分互连，保持参数数量恒定；提出两阶段剪枝：基于SAT逻辑等价和基于相似性的数据驱动剪枝。

Result: DBNs可扩展到更宽的层，同时保持高准确性；剪枝方法优于基线，提供更好的压缩-准确性权衡。
...
### [131] [Lost in Latent Space: An Empirical Study of Latent Diffusion Models for Physics Emulation](https://arxiv.org/abs/2507.02608)
*François Rozet,Ruben Ohana,Michael McCabe,Gilles Louppe,François Lanusse,Shirley Ho*
...
Method: 通过潜在空间扩散模型仿真动态系统，测试不同压缩率下的性能，并与非生成式模型对比。

Result: 潜在空间仿真的准确性对高压缩率（高达1000倍）表现出惊人鲁棒性，扩散模型比非生成式模型更准确且预测多样性更强。
...
### [132] [L-VAE: Variational Auto-Encoder with Learnable Beta for Disentangled Representation](https://arxiv.org/abs/2507.02619)
*Hazal Mogultay Ozcan,Sinan Kalkan,Fatos T. Yarman-Vural*

Main category: cs.LG

TL;DR: L-VAE是一种新型模型，通过学习损失函数的超参数和权重，动态平衡解缠和重构损失，优于现有VAE变体。
...
Result: 在多个数据集上表现优于或接近现有VAE变体，成功解缠面部属性。
...
### [133] [A Matrix Variational Auto-Encoder for Variant Effect Prediction in Pharmacogenes](https://arxiv.org/abs/2507.02624)
*Antoine Honoré,Borja Rodríguez Gálvez,Yoomi Park,Yitian Zhou,Volker M. Lauschke,Ming Xiao*

Main category: cs.LG

TL;DR: 提出了一种基于Transformer的矩阵变分自编码器（matVAE），用于评估蛋白质变体的功能影响，并在DMS数据集上验证其性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖多序列比对（MSA），但假设自然变体适应性高，而药物基因组学中某些基因进化压力低，因此需要替代方法。DMS数据集提供了定量适应性评分。

Method: 使用matVAE模型，结合结构化先验，在33个DMS数据集上评估性能，并与DeepSequence等模型对比。

Result: matVAE-MSA在零样本预测中优于DeepSequence，参数更少且计算需求更低；结合AlphaFold结构后性能进一步提升。

Conclusion: DMS数据集有望替代MSA，需进一步开发数据集并探索其关系以提升预测效果。
...
### [134] [Medical Data Pecking: A Context-Aware Approach for Automated Quality Evaluation of Structured Medical Data](https://arxiv.org/abs/2507.02628)
*Irena Girshovitz,Atai Ambus,Moni Shahar,Ran Gilad-Bachrach*

Main category: cs.LG

TL;DR: 论文提出了一种名为Medical Data Pecking的方法，通过结合软件工程中的单元测试和覆盖率概念，评估电子健康记录（EHR）数据的质量，并开发了工具MDPT来自动生成测试并识别数据问题。


<details>
  <summary>Details</summary>
Motivation: EHR数据在流行病学研究和AI训练中的应用日益广泛，但其数据质量问题（如偏差和系统性错误）影响了结果的可靠性，现有方法缺乏系统性评估。

Method: 采用软件工程中的单元测试和覆盖率概念，开发了MDPT工具，包含自动测试生成器和数据测试框架，利用大语言模型生成测试套件并执行测试。

Result: 在三个数据集（AoU、MIMIC-III、SyntheticMass）上评估MDPT，生成55-73个测试，成功识别20-43个数据问题，并对测试套件进行了详细分析。

Conclusion: 该方法通过结合外部医学知识，实现了上下文敏感的数据质量测试，为未来扩展（如支持更多数据模态和改进接地方法）奠定了基础。
...
### [135] [High-Order Deep Meta-Learning with Category-Theoretic Interpretation](https://arxiv.org/abs/2507.02634)
*David H. Mguni*
...
<details>
  <summary>Details</summary>
Motivation: 解决传统机器学习依赖人工生成数据的局限性，通过虚拟任务和软约束实现自主学习和泛化。

Method: 采用生成机制创建虚拟任务，通过元学习器迭代优化约束区域，并利用范畴论中的函子概念构建层次化学习结构。

Result: 框架能够自主生成任务和数据，增强归纳偏置，支持跨任务的抽象和知识迁移。

Conclusion: 该架构有望推动机器学习向通用人工智能发展，支持自主生成任务及其解决方案。
...
### [136] [On Efficient Bayesian Exploration in Model-Based Reinforcement Learning](https://arxiv.org/abs/2507.02639)
*Alberto Caron,Chris Hicks,Vasilios Mavroudis*
...
<details>
  <summary>Details</summary>
Motivation: 解决强化学习中数据高效探索的问题，尤其是针对稀疏奖励或纯探索任务。
...
### [137] [Fair Deepfake Detectors Can Generalize](https://arxiv.org/abs/2507.02645)
*Harry Cheng,Ming-Hui Liu,Yangyang Guo,Tianyi Wang,Liqiang Nie,Mohan Kankanhalli*
...
<details>
  <summary>Details</summary>
Motivation: 深度伪造检测模型在泛化性和公平性上存在冲突，现有方法难以兼顾。论文首次揭示并定义了两者的因果关系，提出通过控制混杂因素实现改进。

Method: 提出DAID框架，包括人口统计感知的数据重平衡（逆倾向加权和子群特征归一化）和人口统计无关的特征聚合（对齐损失抑制敏感信号）。

Result: 在三个跨域基准测试中，DAID在公平性和泛化性上均优于现有检测器，验证了其理论和实践有效性。

Conclusion: DAID通过因果干预解决了公平性与泛化性的冲突，为深度伪造检测提供了新思路。
...
Conclusion: OmniDraft适用于设备端LLM应用，支持“一草稿模型适配所有”范式。
...
### [139] [Guided Generation for Developable Antibodies](https://arxiv.org/abs/2507.02670)
*Siqi Zhao,Joshua Moller,Porfi Quintero-Cadena,Lood van Niekerk*
...
<details>
  <summary>Details</summary>
Motivation: 抗体治疗需要高亲和力、良好的制造性和稳定性，传统方法难以同时满足这些开发性能要求。

Method: 使用离散扩散模型训练自然抗体序列，并引入SVDD模块引导生成具有高开发性能的抗体序列。

Result: 模型能生成符合自然抗体库和临床抗体特征的序列，SVDD显著提升了开发性能评分。

Conclusion: 该框架为抗体设计提供了结合结合性和生物物理性能的迭代优化方法。
...
Method: 采用差分隐私条件变分自编码器（DP-CVAE）建模全局隐私感知数据分布，支持多样下游任务。

Result: 方法在多个特征提取器上验证，提升了隐私性、可扩展性和效率，优于传统联邦学习分类器。

Conclusion: DP-CVAE在保证差分隐私的同时，生成更高保真度的嵌入，且参数需求更低。
...
### [141] [Multi-Agent Reinforcement Learning for Dynamic Pricing in Supply Chains: Benchmarking Strategic Agent Behaviours under Realistically Simulated Market Conditions](https://arxiv.org/abs/2507.02698)
*Thomas Hazenberg,Yao Ma,Seyed Sahand Mohammadi Ziabari,Marijn van Rijswijk*
...
<details>
  <summary>Details</summary>
Motivation: 传统ERP系统依赖静态规则定价，忽视了市场参与者间的战略互动，而现有强化学习研究多为单智能体，无法模拟真实供应链的相互依赖性。

Method: 评估了三种MARL算法（MADDPG、MADQN、QMIX）与静态规则基线的性能，基于真实电商数据和LightGBM需求预测模型的模拟环境。

Result: 规则代理公平性高（Jain指数0.9896）且价格稳定（波动0.024），但缺乏竞争动态；MADQN定价最激进，MADDPG平衡竞争与公平（公平性0.8819）。
...
### [142] [Fluid Democracy in Federated Data Aggregation](https://arxiv.org/abs/2507.02710)
*Aditya Vema Reddy Kesari,Krishna Reddy Kesari*

Main category: cs.LG

TL;DR: 论文提出了一种基于共识的联邦学习协议（FedVRD），通过动态选择最有用的客户端权重，减少数据传输成本，并防止对手影响。


<details>
  <summary>Details</summary>
Motivation: 传统联邦学习机制中，所有客户端都需要传输权重，导致资源浪费。通过共识协议选择有用权重，可以优化效率并增强安全性。

Method: 提出了一种新的流体民主协议（viscous-retained democracy），优于传统1p1v协议，并设计了FedVRD算法动态限制对手影响。

Result: FedVRD在相同假设下优于传统协议，同时有效减少对手对全局模型的负面影响。

Conclusion: FedVRD通过动态选择和限制对手，优化了联邦学习的效率和安全性。
...
### [143] [A Forget-and-Grow Strategy for Deep Reinforcement Learning Scaling in Continuous Control](https://arxiv.org/abs/2507.02712)
*Zilin Kang,Chenyuan Hu,Yu Luo,Zhecheng Yuan,Ruijie Zheng,Huazhe Xu*

Main category: cs.LG

TL;DR: 论文提出FoG算法，通过遗忘早期经验和动态扩展网络容量，解决深度强化学习中的初始偏差问题，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度强化学习方法存在初始偏差问题，即过度拟合早期经验，影响样本效率和泛化能力。受人类婴儿遗忘和神经元生长的启发，提出新算法。
...
Conclusion: FoG通过模仿神经科学的遗忘与生长机制，有效解决了初始偏差问题，提升了深度强化学习的性能。
...
### [144] [A Comprehensive Machine Learning Framework for Micromobility Demand Prediction](https://arxiv.org/abs/2507.02715)
*Omri Porat,Michael Fire,Eran Ben-Elia*

Main category: cs.LG

TL;DR: 本文提出了一种整合空间、时间和网络依赖性的框架，用于改进无桩电动滑板车的需求预测，准确率比基线模型提高27%至49%。


<details>
  <summary>Details</summary>
Motivation: 无桩电动滑板车作为环保灵活的交通工具，需求预测对其管理和城市规划至关重要。现有研究仅关注单一因素，无法全面捕捉需求模式。

Method: 提出一个整合空间、时间和网络依赖性的框架，用于需求预测。

Result: 框架将需求预测准确率提高了27%至49%，并深入揭示了城市微出行使用模式。
...
### [145] [Hierarchical Multi-Label Contrastive Learning for Protein-Protein Interaction Prediction Across Organisms](https://arxiv.org/abs/2507.02724)
*Shiyi Liu,Buwen Liang,Yuetong Fang,Zixuan Jiang,Renjing Xu*
...
<details>
  <summary>Details</summary>
Motivation: 解决蛋白质-蛋白质相互作用预测中数据异质性和稀疏性问题，尤其是在跨物种和低数据情况下的挑战。

Method: 提出HIPPO框架，结合层次对比学习和数据驱动的惩罚机制，对齐蛋白质序列和层次属性，并通过多层次的生物表示匹配优化预测。

Result: 在基准数据集上表现优于现有方法，具有鲁棒性和零样本迁移能力，适用于数据稀疏的物种。

Conclusion: HIPPO为跨物种蛋白质相互作用预测提供了统一框架，能够有效处理稀疏数据，并揭示了层次特征融合的重要性。
...
### [146] [Fast and Simplex: 2-Simplicial Attention in Triton](https://arxiv.org/abs/2507.02754)
*Aurko Roy,Timothy Chou,Sai Surya Duvvuri,Sijia Chen,Jiecao Yu,Xiaodong Wang,Manzil Zaheer,Rohan Anil*

Main category: cs.LG

TL;DR: 论文探讨了2-单纯形Transformer架构，通过改进注意力机制提升令牌效率，在数学、编程和逻辑任务中表现优于标准Transformer。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型依赖大规模数据集，计算资源不再是唯一瓶颈，需要更高效的令牌利用架构。

Method: 提出2-单纯形Transformer，将标准点积注意力推广为三线性函数，并通过Triton内核高效实现。

Result: 在相同令牌预算下，2-单纯形Transformer在数学、编程和逻辑任务中优于标准Transformer，并改变了知识任务的缩放定律指数。

Conclusion: 2-单纯形Transformer通过提升令牌效率，为计算资源有限的任务提供了更优解决方案。
...
### [147] [Understanding and Improving Length Generalization in Recurrent Models](https://arxiv.org/abs/2507.02782)
*Ricardo Buitrago Ruiz,Albert Gu*

Main category: cs.LG

TL;DR: 论文通过实证和理论分析支持了未探索状态假设，并提出简单训练干预方法，显著提升了循环模型的长序列泛化能力。


<details>
  <summary>Details</summary>
Motivation: 循环模型在长序列上的性能下降问题，尤其是超出训练上下文长度时的泛化失败。

Method: 提出简单训练干预方法，如高斯噪声初始化或使用其他序列的最终状态，以增加训练状态的覆盖范围。

Result: 仅用500次训练后步骤，模型即可泛化到比训练上下文长多个数量级的序列（如2k到128k），并在长上下文任务中表现更优。

Conclusion: 该方法为循环模型提供了一种简单高效的长序列泛化解决方案。
...
### [148] [In-Training Multicalibrated Survival Analysis for Healthcare via Constrained Optimization](https://arxiv.org/abs/2507.02807)
*Thiti Suttaket,Stanley Kok*
...
Conclusion: GRADUATE在多校准和性能平衡方面表现出色，弥补了现有模型的不足。
...
### [149] [Replicable Distribution Testing](https://arxiv.org/abs/2507.02814)
*Ilias Diakonikolas,Jingyi Gao,Daniel Kane,Sihan Liu,Christopher Ye*

Main category: cs.LG

TL;DR: 该论文系统地研究了算法可复制性框架下的分布测试问题，提出了新的可复制算法用于测试离散分布的接近性和独立性，并开发了新的下界证明方法。
...
Method: 开发新的可复制算法用于测试分布的接近性和独立性，并提出新的下界证明方法。

Result: 建立了近乎最优的样本复杂度下界，解决了先前工作中的开放性问题。

Conclusion: 论文为可复制分布测试提供了算法和理论支持，具有广泛的应用潜力。
...
### [150] [ExPO: Unlocking Hard Reasoning with Self-Explanation-Guided Reinforcement Learning](https://arxiv.org/abs/2507.02834)
*Ruiyang Zhou,Shuozhe Li,Amy Zhang,Liu Leqi*

Main category: cs.LG

TL;DR: 论文提出了一种名为ExPO的新方法，通过生成与当前策略一致且能提高正确答案预测概率的样本，解决了强化学习后训练中探索新推理轨迹的难题。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的后训练方法依赖模型初始能力生成正样本，难以解决模型初始失败的推理任务。需要探索新推理轨迹以提升模型能力。

Method: 提出Self-Explanation Policy Optimization (ExPO)，通过基于真实答案生成符合当前策略的高质量样本，引导模型探索新推理轨迹。

Result: ExPO在推理任务（如MATH level-5）中显著提升了学习效率和最终性能，优于基于专家示范的方法。

Conclusion: ExPO通过生成高质量样本有效解决了强化学习后训练中的探索问题，提升了模型在复杂推理任务中的表现。
...
### [151] [LLM-Driven Treatment Effect Estimation Under Inference Time Text Confounding](https://arxiv.org/abs/2507.02843)
*Yuchen Ma,Dennis Frauen,Jonas Schweisthal,Stefan Feuerriegel*

Main category: cs.LG

TL;DR: 论文提出了一种解决训练与推断数据不一致导致的治疗效应估计偏差的框架，结合大语言模型和双重稳健学习器。
...
### [152] [MvHo-IB: Multi-View Higher-Order Information Bottleneck for Brain Disorder Diagnosis](https://arxiv.org/abs/2507.02847)
*Kunyu Zhang,Qiang Li,Shujian Yu*
...
<details>
  <summary>Details</summary>
Motivation: 高阶交互（HOIs）在fMRI数据中的建模能提升机器学习系统的诊断准确性，但如何有效提取和利用HOIs仍是一个挑战。

Method: 提出MvHo-IB框架，结合信息论中的O-information和矩阵熵估计器量化HOIs，设计Brain3DCNN编码器，并引入多视图学习信息瓶颈目标。
...
### [153] [Adaptive Iterative Soft-Thresholding Algorithm with the Median Absolute Deviation](https://arxiv.org/abs/2507.02084)
...
Result: 证明了算法的尺度等变性、非唯一性、局部稳定性及局部线性收敛。

Conclusion: 自适应ISTA在理论和实践中均表现出色，具有广泛的应用潜力。
...
### [154] [Hybrid least squares for learning functions from highly noisy data](https://arxiv.org/abs/2507.02215)
*Ben Adcock,Bernhard Hientzsch,Akil Narayan,Yiming Xu*

Main category: stat.ML

TL;DR: 提出了一种结合Christoffel采样和最优实验设计的混合方法，用于高效估计条件期望，并在大噪声数据下表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在小噪声下表现良好，但在大噪声数据下效果不佳，需要更高效的条件期望估计方法。

Method: 结合Christoffel采样和最优实验设计的混合方法，扩展到凸约束设置，并利用自适应随机子空间。

Result: 算法在样本点生成和噪声抑制方面具有最优性，计算效率和样本复杂度优于现有方法。

Conclusion: 理论分析和数值实验验证了方法的有效性，适用于计算金融等领域的随机模拟问题。
...
### [155] [Transfer Learning for Matrix Completion](https://arxiv.org/abs/2507.02248)
*Dali Liu,Haolei Weng*

Main category: stat.ML

TL;DR: 本文研究了矩阵补全中的知识迁移问题，提出了一种利用辅助数据的迁移学习方法，并证明了其最优性。
...
Method: 提出了一种迁移学习流程，利用源数据信息，并开发了高效的检测程序识别相关源数据。

Result: 当源矩阵与目标矩阵接近时，方法优于传统单目标数据方法，且收敛速率最优。

Conclusion: 方法有效且在实际数据中验证了其可行性。
...
### [156] [It's Hard to Be Normal: The Impact of Noise on Structure-agnostic Estimation](https://arxiv.org/abs/2507.02275)
*Jikai Jin,Lester Mackey,Vasilis Syrgkanis*

Main category: stat.ML

TL;DR: 论文研究了在给定黑盒机器学习估计的干扰函数（如混杂因素对治疗和结果的影响）的情况下，如何有效估计治疗效果。研究发现，治疗效果噪声的分布对估计结果有显著影响。


<details>
  <summary>Details</summary>
Motivation: 探讨在结构无关的因果推断中，如何利用机器学习估计干扰函数来优化治疗效果估计，并解决现有方法的局限性。

Method: 基于部分线性模型，分析了双机器学习（DML）估计器在高斯和非高斯治疗噪声下的性能，并提出了新的高阶鲁棒性方法（ACE）。

Result: DML在高斯噪声下是最优的，但在非高斯噪声下表现不佳；提出的ACE方法通过利用高阶累积量实现了对干扰误差的高阶鲁棒性。

Conclusion: 研究揭示了治疗噪声分布对因果推断的重要性，并提出了一种更鲁棒的估计方法，为实际应用提供了改进方向。
...
### [157] [Sparse Gaussian Processes: Structured Approximations and Power-EP Revisited](https://arxiv.org/abs/2507.02377)
*Thang D. Bui,Michalis K. Titsias*

Main category: stat.ML

TL;DR: 本文提出了一种基于块对角结构的稀疏变分高斯过程改进方法，并通过实验验证其性能优于现有对角结构方法。
...
Method: 引入块对角结构的缩放矩阵，并在Power Expectation Propagation（PEP）框架中利用这种结构化近似后验。

Result: 块对角近似在回归实验中表现优于或等同于现有对角近似，且计算成本相当。

Conclusion: 新方法为稀疏GP提供了灵活且高性能的替代方案，适用于不同超参数设置。
...
### [158] [Adaptive Iterative Soft-Thresholding Algorithm with the Median Absolute Deviation](https://arxiv.org/abs/2507.02084)
*Yining Feng,Ivan Selesnick*
...
Result: 证明了算法的尺度等变性、非唯一性、局部稳定性及局部线性收敛性。

Conclusion: 自适应ISTA在理论和实践上均表现出良好的收敛性和稳定性。
...
### [159] [Hybrid least squares for learning functions from highly noisy data](https://arxiv.org/abs/2507.02215)
*Ben Adcock,Bernhard Hientzsch,Akil Narayan,Yiming Xu*
...
Method: 结合Christoffel采样和最优实验设计，提出混合算法，并扩展到凸约束设置和自适应随机子空间。
...
### [160] [Transfer Learning for Matrix Completion](https://arxiv.org/abs/2507.02248)
*Dali Liu,Haolei Weng*

Main category: stat.ML

TL;DR: 本文研究了矩阵补全设置下的知识迁移，提出了一种利用辅助数据的迁移学习方法，并证明了其最优性。
...
Method: 提出基于先验信息的迁移学习程序，利用源矩阵接近目标矩阵的特性，并开发了高效的源数据集检测方法。

Result: 在源矩阵接近目标矩阵时，方法优于传统单一数据方法，且消除了收敛速率中的对数因子。

Conclusion: 方法在理论和实验中均表现出色，适用于未知源数据相关性的场景。
...
### [161] [It's Hard to Be Normal: The Impact of Noise on Structure-agnostic Estimation](https://arxiv.org/abs/2507.02275)
*Jikai Jin,Lester Mackey,Vasilis Syrgkanis*

Main category: stat.ML

TL;DR: 本文研究了结构无关因果推断中处理噪声分布对估计效果的影响，发现双机器学习（DML）估计器在高斯噪声下最优，但在非高斯噪声下表现不佳，并提出新的高阶稳健估计方法。


<details>
  <summary>Details</summary>
Motivation: 探讨在结构无关因果推断中，处理噪声分布如何影响估计效果，尤其是DML估计器在不同噪声条件下的表现。

Method: 基于部分线性模型，分析DML估计器在高斯和非高斯噪声下的性能，并提出新的高阶稳健估计方法（ACE）。

Result: DML在高斯噪声下最优，但在非高斯噪声下表现不佳；ACE方法通过利用高阶累积量实现了对干扰误差的高阶稳健性。
...
### [162] [Sparse Gaussian Processes: Structured Approximations and Power-EP Revisited](https://arxiv.org/abs/2507.02377)
*Thang D. Bui,Michalis K. Titsias*

Main category: stat.ML

TL;DR: 论文提出了一种块对角结构的稀疏变分高斯过程方法，通过改进条件后验密度，提升了性能，同时保持了计算效率。


<details>
  <summary>Details</summary>
Motivation: 稀疏变分高斯过程在扩展GP模型时存在性能瓶颈，通过引入块对角结构的缩放矩阵可以进一步优化。

Method: 扩展了稀疏变分高斯过程，采用块对角缩放矩阵，并结合Power Expectation Propagation（PEP）框架。

Result: 实验表明，块对角近似方法性能优于或与现有对角近似方法相当，计算成本相近。
