...
TL;DR: 提出基于分段波导夹持天线系统的三混合波束赋形架构，联合优化数字、模拟和夹持波束赋形，支持全连接和部分连接结构，在性能和能耗间取得良好平衡。


<details>
  <summary>Details</summary>
Motivation: 传统混合波束赋形和夹持天线系统在性能上存在局限，需要一种能联合优化数字、模拟和夹持波束赋形的新架构来提升上行多用户MIMO通信性能。

Method: 提出SWAN三混合波束赋形架构，针对全连接结构使用WMMSE和ZF方法优化，通过闭式解、黎曼流形优化和高斯-赛德尔搜索分别优化三个组件；针对部分连接结构提出交错拓扑，使用元素级相位校准替代黎曼流形更新。

Result: SWAN三混合波束赋形在FC和PC结构下均优于传统混合波束赋形和传统夹持天线系统；PC结构在段数较多时能在和速率与能耗间取得良好平衡；可达速率不一定随段数增加而增加。

Conclusion: 提出的SWAN三混合波束赋形架构有效提升了上行多用户MIMO性能，PC结构在段数较多时提供了性能与能耗的良好折衷，为实际系统设计提供了重要指导。
...
<details>
  <summary>Details</summary>
Motivation: 相干光纤链路中的非线性主要由信号强度的时域统计特性和频谱结构驱动。现有研究缺乏将整形星座的块级能量统计与PSD低频特征联系起来的统一框架，无法为非线性抑制提供系统性的频谱-时间协同设计指导。

Method: 推导了有限块整形符号（包括CCDM和ESS）的半解析PSD模型，明确揭示了自拍项（依赖于符号能量方差）、符号间拍频项（依赖于平均符号能量）和块诱导能量方差项的贡献。获得了频谱凹陷宽度的紧凑表达式，捕获了块长度、符号速率、脉冲滚降和色散的影响。

Result: 得到了降低低频分量的设计规则，提供了整形和非整形系统的最优符号速率定律。蒙特卡洛仿真验证了理论结果，并确认了CCDM（抑制直流）与ESS（中等块长度下存在有限直流基底）在低频行为上的显著差异。

Conclusion: 该框架统一了先前的时域和频域观点，为选择块长度、符号速率和整形方法以减少高容量WDM系统中的非线性干扰提供了可操作的指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有信道预测方法存在局限性：传统卡尔曼滤波方法易受模型失配影响，而深度学习方法会产生过度自信的预测，需要更可靠的预测方法

Method: 提出深度学习保形贝叶斯滤波(DCBF)，整合深度学习预测、保形分位数回归和贝叶斯滤波，实现校准先验和观测的原则性融合

Result: DCBF显著改善了深度学习预测性能，并优于卡尔曼滤波方法，能够提供具有校准不确定性的可靠信道预测

Conclusion: DCBF框架有效解决了信道预测中的模型失配和过度自信问题，为信道状态信息获取提供了可靠解决方案
...
TL;DR: 该论文提出了一种结构化缩减搜索策略，用于高阶MIMO系统的ML检测，能以线性复杂度实现接近ML的性能。


<details>
  <summary>Details</summary>
Motivation: 高阶MIMO系统中的最大似然检测由于计算复杂度随发射层数和星座大小呈指数增长，在实际应用中不可行。

Method: 采用结构化缩减搜索策略，复杂度与星座大小呈线性关系。通过列表大小控制搜索范围：3x3系统用3|X|，4x4系统用4|X|，8x8系统用8|X|（|X|为星座大小）。

Result: 在i.i.d.瑞利衰落信道上的大量仿真表明，该方法在实用MIMO维度（最高8x8）下能接近ML硬判决性能，即使在高信道条件数下也能匹配完整ML性能。

Conclusion: 该方法为高阶MIMO系统提供了实用的ML检测解决方案，并讨论了软LLR生成和FEC交互的扩展应用。
...
TL;DR: 提出基于CYGNSS卫星DDM噪声底最大值的RFI检测方法，采用两级验证框架，在GPS干扰测试区域和中东地区验证了优于现有方法的检测性能。


<details>
  <summary>Details</summary>
Motivation: GNSS的RFI干扰日益严重，需要更有效的空间检测方法。CYGNSS星座的DDM噪声底数据可用于RFI检测，但现有方法可能稀释异常信号。

Method: 提出最大噪声底策略：选择四个GNSS反射通道中的最高噪声底值而非平均值；采用两级验证：多卫星并发检测（至少两颗卫星独立标记同一区域）和时间持续性验证（10秒窗口内持续超过阈值）。

Result: 在White Sands导弹靶场的GPS干扰测试中，该方法在三个日期检测到RFI，而其他方法检测极少；在中东地区标记了62%的时段，优于均值法（46%）和峰度法（33%）。能检测逐渐增强干扰的早期阶段和异常模式。

Conclusion: 最大噪声底分析方法为空间RFI检测提供了更灵敏可靠的方法，能有效检测GPS干扰和持续性RFI，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 传统N阶梳状抽取器的通带下垂严重依赖于抽取因子M，这限制了其应用灵活性。需要一种方法能够降低这种依赖性，使梳状抽取器在不同抽取因子下都能保持较好的通带特性。

Method: 在梳状抽取器的积分级联一个对称的3抽头FIR滤波器，滤波器的系数仅作为阶数N的函数进行选择。这种方法可以从传统梳状抽取器推导而来，并能轻松应用于任何新近开发的梳状抽取器和下垂补偿滤波器设计方法。

Result: 提出的方法有效降低了梳状抽取器通带下垂对抽取因子M的依赖性，提高了滤波器在不同抽取因子下的性能一致性。

Conclusion: 该方法提供了一种简单有效的解决方案，能够显著改善梳状抽取器的通带特性，且具有良好的兼容性和应用灵活性。
...
TL;DR: 该论文揭示了分布式多目标跟踪系统中标签一致性方法的身份级漏洞，提出了标签劫持攻击概念，并开发了基于优化的攻击策略，展示了攻击对标签一致性和跟踪精度的影响。


<details>
  <summary>Details</summary>
Motivation: 分布式多目标跟踪系统在有限视场传感器网络中常面临标签不一致问题，现有基于度量的轨迹匹配方法虽然能缓解此问题，但其在对抗条件下的行为尚未得到充分研究。论文旨在揭示这些方法的身份级漏洞。

Method: 提出了标签劫持攻击概念，借鉴雷达中的经典牵引欺骗技术，形式化了攻击的隐蔽性概念，并开发了基于优化的攻击策略来制作此类攻击。通过三传感器网络案例研究验证攻击效果。

Result: 案例研究表明，所提出的攻击能够成功劫持目标身份，破坏网络中的标签一致性并降低跟踪精度，实现了目标冒充。攻击对系统性能有显著影响。

Conclusion: 这项工作揭示了分布式多目标跟踪框架在共识层的安全漏洞，强调了需要重新思考共识层的鲁棒性设计，以抵御标签劫持等身份级攻击。
...
Method: 基于文献中的4-PAM设计进行改进和泛化，提出全MOSFET基的8-PAM设计，使用开源IC设计工具进行仿真和设计
...
TL;DR: 提出基于级数解析延拓的AR过程杂波模拟策略，通过精确预计算输入预失真来补偿AR滤波失真，实现非高斯相关杂波序列的准确快速模拟。
...
Method: 1) 基于AR过程的输入输出关系推导AR输入的矩和累积量；2) 通过Padé逼近对矩展开和对数矩展开进行解析延拓；3) 利用累积量展开恢复的拉普拉斯变换，通过随机变量变换方法快速模拟AR输入非高斯白序列。

Result: 模拟结果表明，所提策略能够准确快速地模拟非高斯相关杂波序列。累积量展开的解析延拓比传统的矩展开更稳定准确，特别是在拉普拉斯变换具有强振荡时。

Conclusion: 提出的级数解析延拓策略通过精确预计算输入预失真来补偿AR滤波失真，实现了非高斯相关杂波序列的准确快速模拟，为AR过程杂波模拟提供了新的有效方法。
...
TL;DR: 提出双馈点可调天线系统（DF-PAS），通过在波导两端设置两个馈电点并动态选择，缩短波导内传播距离，有效缓解衰减问题，提升系统性能。


<details>
  <summary>Details</summary>
Motivation: 传统可调天线系统（PAS）在实际应用中受波导内衰减限制，特别是在长波导情况下，衰减会严重降低数据速率。需要在不改变波导结构或天线驱动机制的情况下解决这一问题。

Method: 提出双馈点PAS（DF-PAS），每个波导两端设置两个馈电点，根据用户位置动态选择馈电点。针对单波导和多波导场景分别进行分析：单波导下推导高信噪比近似和最优天线位置闭式解；多波导下提出包含贪婪馈电点切换、梯度天线放置和WMMSE波束成形的两阶段优化框架。

Result: 仿真结果表明，DF-PAS在各种网络配置下均优于传统单馈点PAS（SF-PAS），验证了其作为缓解波导内衰减问题的实用且可扩展解决方案的有效性。

Conclusion: DF-PAS通过动态馈电点选择有效缩短波导内传播距离，显著缓解衰减引起的功率损失，为PAS增强的无线网络提供了一种实用且可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有直接预测模型通常依赖点级目标如均方误差，无法捕捉图结构信号中复杂的时空依赖关系。虽然最近的频域方法如FreDF缓解了时间自相关，但往往忽略了空间和跨时空的相互作用。

Method: 提出FreST Loss，一种频率增强的时空训练目标，通过联合傅里叶变换将监督扩展到联合时空谱域。该方法在统一的谱域中对齐模型预测与真实值，有效解耦空间和时间的复杂依赖关系。

Result: 在六个真实世界数据集上的广泛实验表明，FreST Loss是模型无关的，并且通过更好地捕捉整体时空动态，持续改进了最先进的基线方法。

Conclusion: FreST Loss通过联合时空谱监督有效解决了现有方法在捕捉复杂时空依赖关系方面的局限性，理论分析表明该公式减少了时域训练目标相关的估计偏差。
...
TL;DR: 提出基于深度神经网络的平衡信息神经网络(EINNs)，通过反向推断系统参数来检测临界阈值，替代传统计算密集型方法。


<details>
  <summary>Details</summary>
Motivation: 临界转变是复杂动力系统中不同状态间的突然转变，传统检测方法依赖大量前向模拟或分岔分析，计算成本高且受参数采样限制。

Method: EINN方法将候选平衡状态作为输入，训练深度神经网络推断满足平衡条件的相应系统参数，通过分析学习到的参数景观和平衡映射的可行性/连续性变化来检测临界阈值。

Result: 在表现出鞍结分岔和多稳态的非线性系统上验证了EINNs能够恢复与即将发生的转变相关的参数区域。

Conclusion: EINNs为传统技术提供了灵活替代方案，为高维非线性系统中临界转变的早期检测和结构分析提供了新见解。
...
TL;DR: FedEMA-Distill：一种服务器端联邦学习方法，通过指数移动平均和知识蒸馏处理异构非IID数据和对抗性客户端，显著提升精度、减少通信轮次和负载


<details>
  <summary>Details</summary>
Motivation: 联邦学习在客户端持有异构非IID数据或存在对抗性客户端时性能下降，导致客户端漂移、收敛慢和通信开销大。需要一种兼容性强、通信高效且能抵抗攻击的方法。

Method: 提出FedEMA-Distill：服务器端使用全局模型的指数移动平均（EMA），结合客户端上传的预测logits进行集成知识蒸馏。客户端仅上传压缩的logits，无需修改客户端软件，支持模型异构。服务器端还可使用坐标中位数或截断均值聚合logits以抵抗拜占庭攻击。

Result: 在CIFAR-10、CIFAR-100、FEMNIST和AG News数据集上，Dirichlet-0.1标签倾斜下：top-1准确率提升最高达5-6%；达到目标准确率所需通信轮次减少30-35%；每轮客户端上行负载降至0.09-0.46MB（比传输完整模型权重减少约一个数量级）；可抵抗10-20%拜占庭客户端攻击。

Conclusion: 时间平滑与仅logits聚合的结合提供了通信高效、攻击鲁棒的联邦学习管道，部署友好，兼容安全聚合和差分隐私，因为只交换聚合或混淆的模型输出。
...
TL;DR: Delta-Crosscoder 是一种改进的模型差异分析方法，通过结合 BatchTopK 稀疏性、基于变化的损失函数和配对激活的隐式对比信号，有效识别微调后模型内部表示的变化方向，特别适用于局部化、不对称的窄微调场景。


<details>
  <summary>Details</summary>
Motivation: 现有 Crosscoders 方法在处理窄微调（行为变化局部化且不对称）时存在困难，需要一种能够更精确识别微调引起模型内部表示变化的方法。

Method: Delta-Crosscoder 结合了三种关键技术：1) BatchTopK 稀疏性；2) 基于变化的损失函数，优先考虑模型间变化的潜在方向；3) 配对激活在匹配输入上的隐式对比信号。

Result: 在10个模型（包括合成虚假事实、涌现错位、潜意识学习和禁忌词猜测等场景，涵盖 Gemma、LLaMA、Qwen 等模型，1B-9B 参数规模）上评估，Delta-Crosscoder 能可靠地分离出对微调行为有因果责任的潜在方向，并实现有效缓解，优于基于 SAE 的基线方法，与非 SAE 方法相当。

Conclusion: Crosscoders 仍然是模型差异分析的有力工具，Delta-Crosscoder 的改进使其在处理窄微调场景时表现更优，能够精确识别微调引起的模型内部变化。
...
TL;DR: 论文提出非对称注意力机制，将查询和键的维度降低到远小于值的维度，因为选择操作本质上是低维度的，而值传递需要丰富的语义表示。通过实验验证，在保持性能的同时大幅减少参数和KV缓存。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer注意力中查询、键和值使用相同维度，但作者认为这三者功能不同：查询和键用于产生注意力权重（选择操作），而值用于传递语义信息（值传递）。选择操作本质上是低维度的，只需要O(log N)维度就能区分N个相关模式，因此对称设计是不必要的。

Method: 提出非对称注意力机制，将查询和键的维度(d_select)降低到远小于值的维度(d_model)。通过七个实验验证：1) 位置选择任务；2) 基于内容的检索；3-4) WikiText语言建模；5) GPT-2的SVD压缩；6) LLaMA模型验证；7) Mistral-7B的SVD压缩和微调。对现有模型采用SVD压缩后对QK进行轻量微调。

Result: 实验表明：1) 位置选择任务只需每头1个维度；2) 内容检索需要约log₂N维度；3-4) 在语言建模中，d_select = d_model/4时困惑度仅增加4.3%，同时减少75%的QK参数；5) GPT-2的键比查询更容易压缩；6) LLaMA模型显示相似的退化比例；7) Mistral-7B通过SVD压缩和QK微调实现75%的键缓存节省，仅损失2.0%的质量。对于7B参数模型，可节省25GB KV缓存，支持60%更多并发用户。

Conclusion: 非对称注意力机制是有效的，查询和键的维度可以大幅降低而不显著影响模型性能。这种方法可以显著减少参数数量和KV缓存需求，提高推理效率，对大规模语言模型的部署具有重要实践意义。
...
TL;DR: 提出一种在边缘设备上持久化存储4位量化KV缓存的方法，解决多智能体LLM系统中内存不足问题，通过直接恢复缓存而非重新计算，显著降低首token生成时间。


<details>
  <summary>Details</summary>
Motivation: 边缘设备内存有限，无法同时容纳多个智能体的KV缓存，导致需要频繁驱逐和重新计算，造成严重的延迟问题（每个智能体在4K上下文下需要15.7秒重新预填充）。

Method: 系统包含三个组件：1) 块池提供每个智能体隔离的4位量化KV缓存（safetensors格式）；2) BatchQuantizedKVCache支持对多个智能体量化缓存进行并发推理；3) 跨阶段上下文注入，无需重新计算即可在对话阶段间累积注意力状态。

Result: 缓存恢复将首token生成时间降低高达136倍（Gemma：22-136倍，4K-32K上下文）。4位量化相比FP16在固定内存中容纳4倍多的智能体上下文。困惑度变化：Gemma -0.7%，Llama +2.8%，DeepSeek +3.0%。

Conclusion: 通过持久化4位量化KV缓存和直接恢复机制，有效解决了边缘设备上多智能体LLM系统的内存瓶颈问题，显著提升了系统效率，同时保持了可接受的精度损失。
...
TL;DR: Flowers是一种基于多头warp的PDE求解器架构，通过位移场预测实现自适应全局交互，在2D/3D时间相关PDE基准测试中表现优异，优于傅里叶、卷积和注意力基线模型。


<details>
  <summary>Details</summary>
Motivation: 传统PDE求解器架构（如傅里叶乘法器、点积注意力、卷积混合）存在计算效率或表达能力限制。Flowers旨在设计一种更高效、物理动机更强的架构，通过warp操作实现自适应全局交互，同时保持线性计算成本。

Method: Flowers完全基于多头warp构建：每个头预测位移场并warp混合输入特征。位移预测是点对点的，无需空间聚合；非局部性仅通过稀疏采样实现（每个头一个源坐标）。通过多尺度残差块堆叠warp，实现自适应全局交互的线性成本计算。

Result: 在广泛的2D和3D时间相关PDE基准测试中表现优异，特别是在流体和波动问题上。紧凑的17M参数模型在相似规模下优于傅里叶、卷积和注意力基线；150M参数变体优于参数更多、数据更多、训练计算更大的近期transformer基础模型。

Conclusion: Flowers通过基于warp的架构设计，为PDE求解器学习提供了高效且表达力强的替代方案，在物理动机、计算效率和性能方面均表现出色，为神经算子设计开辟了新方向。
...
TL;DR: SOLID是一个基于掩码条件的扩散框架，直接从稀疏观测中学习时空动态，无需密集场数据或预插值，通过双重掩码目标实现后验采样，在严重稀疏条件下获得校准的不确定性。


<details>
  <summary>Details</summary>
Motivation: 物理场通常只在稀疏、时变的传感器位置观测，使得预测和重建成为不适定且需要不确定性量化的问题。现有方法通常在密集再分析或模拟数据上训练，只在稀疏条件下测试，而SOLID旨在直接从稀疏观测中端到端学习。

Method: SOLID采用掩码条件扩散框架，每个去噪步骤都以测量值及其位置为条件。引入双重掩码目标：(i)强调在未观测空白区域学习，(ii)对输入和目标提供最可靠锚点的重叠像素进行加权。严格的稀疏条件通路支持与测量一致的全场后验采样。

Result: 在严重稀疏条件下，SOLID实现了概率误差的数量级改进，产生校准的不确定性地图（ρ > 0.7），相比仅在密集数据上训练的方法有显著优势。

Conclusion: SOLID展示了直接从稀疏观测学习时空动态的可行性，通过掩码条件扩散和双重掩码目标，实现了准确的后验采样和校准的不确定性量化，为物理场重建和预测提供了新方法。
...
Result: 实验显示ZorBA在VRAM使用上比三个基线方法最多减少62.41%，通信开销低

Conclusion: ZorBA通过零阶优化和异构块激活有效解决了联邦微调中的VRAM和通信瓶颈问题
...
TL;DR: ASFL框架通过自适应模型分割和资源分配，在无线网络中优化联邦学习的延迟和能耗，提升学习性能


<details>
  <summary>Details</summary>
Motivation: 联邦学习中客户端计算资源有限导致训练延迟高、能耗大，需要利用服务器资源并优化资源分配来提升效率
...
<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病的早期准确检测具有挑战性，因为其发病隐匿且进展缓慢。需要开发可靠且透明的预测方法，以支持临床决策。

Method: 使用结构化临床、生活方式和代谢特征，结合严格的预处理、特征工程、SMOTE-Tomek混合类别平衡技术。采用五种集成算法（随机森林、XGBoost、LightGBM、CatBoost、Extra Trees）和深度人工神经网络，通过分层验证选择最佳模型。

Result: 集成方法优于深度学习，XGBoost、随机森林和软投票方法在准确性、敏感性和F1分数方面表现最佳。可解释性分析显示MMSE、功能评估年龄和多个工程交互特征是最具影响力的决定因素。
...
TL;DR: 提出非经典网络（NCnet），一种经典神经网络架构，在典型实验设置下稳定表现出非经典统计行为，通过CHSH不等式的S统计量测量非经典性，发现其源于多任务共享隐藏层神经元的梯度竞争。
...
Method: 设计非经典网络（NCnet），一种简单的经典神经网络架构，在典型可解释的实验设置下，通过多任务共享隐藏层神经元，测量CHSH不等式的S统计量来量化非经典性。

Result: 发现非经典性源于隐藏层神经元的梯度竞争；即使没有物理连接支持显式通信，一个任务头可以通过局部损失振荡隐式感知其他任务头的训练任务，导致训练结果的非局域相关性；在低资源状态下S值随资源增加逐渐接近经典上界2，模型接近临界规模时S可能暂时超过2，资源继续增长时S渐近衰减至2附近波动；模型容量不足时S与泛化性能正相关。

Conclusion: 非经典统计可以为理解深度网络内部交互和训练动态提供新颖视角，S统计量首次接近2的区间通常对应良好的泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: 现实聚类任务中常见包含数值和分类属性的混合数据。数值属性在欧氏空间有明确定义，而分类属性则嵌入在隐式空间中。现有方法要么将异构属性编码为单一类型，要么定义统一度量，未能揭示它们的内在联系。
...
Conclusion: HARR通过揭示异构属性间的联系，将混合数据投影到统一可学习的多空间中，实现了更有效的聚类分析，为混合数据处理提供了新范式。
...
Method: 提出VSPrefill机制，使用注意力分布中的垂直-斜线结构模式。通过紧凑的VSIndexer模块从增强RoPE的键值表示中预测上下文感知的重要性分数，构建线性复杂度的稀疏掩码而不修改主干参数。推理时采用自适应累积阈值策略分配每层稀疏预算，并使用融合内核执行带即时索引合并的注意力。
...
TL;DR: MAD-SmaAt-GNet扩展了SmaAt-UNet，通过多模态编码器和物理平流组件改进降水临近预报，相比基线MSE降低8.9%


<details>
  <summary>Details</summary>
Motivation: 传统数值预报方法计算成本高且未能充分利用大量天气数据，深度学习模型在降水临近预报中显示出潜力，特别是CNN在图像预测任务中表现优异

Method: 基于SmaAt-UNet架构，引入多模态编码器学习多个天气变量，并集成基于物理的平流组件确保物理一致性预测

Result: 每个扩展单独都能改进降雨预报，组合使用效果更佳；相比基线SmaAt-UNet，四步降水预报（最多4小时）的MSE降低8.9%；多模态输入对短期预报特别有益，平流组件在长短预报时域都能提升性能
...
TL;DR: 大语言模型在上下文学习中容易受到单个错误示例的误导，研究发现模型内部存在两阶段计算结构：中间层同时编码正确和错误规则，晚期层才形成预测置信度，特定注意力头是推理失败的关键。
...
Method: 使用线性探测和logit透镜分析模型内部表示，发现模型在中间层同时编码正确和错误规则，但在晚期层才形成预测置信度。识别了两个阶段的注意力头：早期到中间层的"脆弱头"对错误敏感，晚期层的"易受影响头"会显著减少对正确预测的支持。

Result: 单个错误规则演示会导致模型性能显著下降。通过有针对性的消融实验，屏蔽少量识别出的注意力头可以使性能提升超过10%。模型在中间层同时编码正确和错误规则，但在晚期层才形成最终预测置信度。

Conclusion: 大语言模型在上下文学习中存在系统性误导行为，具有两阶段计算结构。特定注意力头是推理失败的关键，这为理解和改进模型的鲁棒性提供了重要见解，通过干预这些头可以显著提升模型对错误示例的抵抗力。
...
TL;DR: 该研究提出了一种基于LSTM的船舶轨迹预测模型，通过引入可学习的船舶领域参数来增强模型的可解释性，特别针对内河航道复杂环境。模型在5分钟预测范围内达到约40米的最终位移误差，但发现船舶间注意力权重与预期存在偏差，表明准确性提升不完全由因果关系驱动。


<details>
  <summary>Details</summary>
Motivation: 在拥挤的内河航道环境中，准确的船舶轨迹预测对安全至关重要。虽然深度学习模型在预测准确性方面取得了进展，但这些模型的可解释性往往被忽视，可能导致隐藏不准确的逻辑并削弱对其可靠性的信心。本研究旨在通过可解释的设计来增强船舶轨迹预测模型的可信度。

Method: 提出基于LSTM的船舶轨迹预测模型，引入可训练的船舶领域参数来提供对交互船舶隐藏状态基于注意力融合的洞察。该模型采用注意力机制架构，通过学习的船舶领域值来分配船舶间交互的权重，并在内河航道复杂相遇场景中进行评估。

Result: 模型在5分钟预测范围内达到约40米的最终位移误差，性能与类似研究相当。船舶间注意力架构确实提高了预测准确性，但发现通过学习的船舶领域值分配给相遇中船舶的权重与预期存在偏差，表明准确性提升不完全由因果关系驱动。

Conclusion: 该研究展示了通过内在可解释设计增强模型解释能力的方法。虽然注意力机制提高了预测准确性，但权重分配的偏差揭示了模型决策逻辑的复杂性。未来工作将集中于利用该架构进行反事实分析，并整合更复杂的注意力机制。
...
### [28] [Activity Recognition from Smart Insole Sensor Data Using a Circular Dilated CNN](https://arxiv.org/abs/2603.04477)
*Yanhua Zhao*
...
<details>
  <summary>Details</summary>
Motivation: 智能鞋垫配备压力传感器、加速度计和陀螺仪，为监测人类步态和姿势提供非侵入性方法，需要有效的活动分类系统来处理这些多模态时间序列数据

Method: 使用圆形扩张卷积神经网络(CDCNN)处理160帧窗口、24通道的多模态数据（18个压力通道、3个加速度计轴、3个陀螺仪轴），并与基于扁平化数据的极端梯度提升树(XGBoost)模型进行对比

Result: CDCNN在四类任务（站立、行走、坐姿、串联步态）的主体独立评估中达到86.42%测试准确率，略低于XGBoost的87.83%；排列特征重要性分析显示惯性传感器对分类贡献显著

Conclusion: 该方法适合嵌入式部署和实时推理，为智能鞋垫活动监测提供了有效的深度学习解决方案，惯性传感器在活动分类中起重要作用
...
### [29] [Standing on the Shoulders of Giants: Rethinking EEG Foundation Model Pretraining via Multi-Teacher Distillation](https://arxiv.org/abs/2603.04478)
*Chenqi Li,Yu Liu,Shuo Zhang,Timothy Denison,Tingting Zhu*

Main category: cs.LG

TL;DR: 提出多教师蒸馏预训练框架MTDP，利用视觉和时间序列基础模型的知识来引导EEG基础模型训练，减少对大量EEG数据的需求


<details>
  <summary>Details</summary>
Motivation: EEG数据收集成本高、信噪比低，传统基于掩码重建的自监督预训练方法难以扩展并捕获神经语义。能否利用其他模态（如视觉、时间序列）已成熟的基础模型来引导EEG模型训练？

Method: 提出两阶段多教师蒸馏预训练框架：第一阶段使用可学习门控网络融合不同教师模型（如DINOv3和Chronos）的表示，通过掩码潜在去噪目标；第二阶段将融合表示蒸馏到EEG基础模型中

Result: 在9个下游任务和12个数据集上的广泛评估表明，MTDP-based EEG基础模型优于自监督对比方法，且仅需25%的预训练数据

Conclusion: 利用其他模态成熟基础模型的知识蒸馏可以有效引导EEG基础模型训练，克服EEG数据稀缺和噪声问题，实现更好的性能
...
TL;DR: 本文提出了一种在混合在线学习设置中同时实现统计最优性和计算效率的新算法，通过约束对手从固定函数类中选择标签，并利用截断熵正则化的Frank-Wolfe方法，获得了与假设类和对手标签类的Rademacher复杂度相关的遗憾界。
...
Method: 提出新的学习算法，约束对手从表达性强但固定的函数类R中选择标签，使用截断熵正则化的Frank-Wolfe方法，并开发了混合鞅差序列的新尾界分析工具，算法在给定ERM预言机的情况下高效运行。

Result: 算法获得了与从学习者假设类H和对手标签类R导出的类的Rademacher复杂度相关的遗憾界，作为关键推论，给出了在高维动作集但收益函数具有低维结构时计算随机零和博弈均衡的预言机高效算法。

Conclusion: 本文在混合学习设置中向同时实现统计最优性和计算效率迈出了重要一步，通过结构化约束对手和开发新的分析工具，为混合在线学习提供了有效的解决方案。
...
TL;DR: 提出對比學習框架，將X射線光譜與科學文獻知識對齊，建立多模態共享表示，提升物理參數估計並識別重要天體目標


<details>
  <summary>Details</summary>
Motivation: 天文學家擁有大量多模態數據（圖像、光譜、時間序列）和文獻，但這些數據源很少被系統整合。科學文本包含比光譜更廣泛多樣的物理背景，建立連接具有挑戰性

Method: 提出對比學習框架，將X射線光譜與從科學文獻提取的領域知識對齊，建立共享多模態表示。採用專家混合策略，結合單模態和共享表示

Result: 在從光譜檢索文本時達到20% Recall@1%，共享潛在空間有效編碼物理信息。融合光譜和文本數據，將20個物理變量的估計比單模態光譜基線提升16-18%。識別出高優先級跟蹤目標，包括候選脈動超亮X射線源和引力透鏡系統

Conclusion: 光譜與文本的對比學習不僅可行，還能加速稀有或理解不足天體的解釋。專家混合策略表現最佳，該框架可擴展到其他需要對齊觀測數據與文獻的科學領域
...
### [32] [K-Means as a Radial Basis function Network: a Variational and Gradient-based Equivalence](https://arxiv.org/abs/2603.04625)
*Felipe de Jesus Felix Arredondo,Alejandro Ucan-Puc,Carlos Astengo Noguez*

Main category: cs.LG

TL;DR: 该论文建立了K-Means算法与可微RBF神经网络之间的严格变分等价关系，通过温度参数控制实现软分配到硬分配的收敛，并引入Entmax-1.5解决数值稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 传统K-Means算法是离散的、不可微的，无法与深度学习架构进行端到端联合优化。需要建立K-Means与连续可微优化之间的理论桥梁，使聚类能够直接嵌入神经网络中进行表示和聚类的联合学习。

Method: 通过重新参数化K-Means目标函数，将失真函数嵌入平滑加权损失中，证明RBF目标在温度参数趋近于0时Γ-收敛到K-Means解。使用Entmax-1.5替代Softmax解决低温区域数值不稳定问题，同时保持Voronoi划分结构。

Result: 理论证明RBF梯度更新在极限情况下精确恢复K-Means质心更新规则，产生相同的训练轨迹。实验验证软RBF质心单调收敛到K-Means固定点，为端到端可微聚类提供了统一框架。

Conclusion: 该工作建立了离散划分与连续优化之间的理论桥梁，使K-Means能够直接嵌入深度学习架构，实现表示和聚类的联合优化，为可微聚类提供了严格的理论基础。
...
### [33] [Invariant Causal Routing for Governing Social Norms in Online Market Economies](https://arxiv.org/abs/2603.04534)
*Xiangning Yu,Qirui Mi,Xiao Xue,Haoxuan Li,Yiwei Shi,Xiaowei Liu,Mengyue Yang*

Main category: cs.LG

TL;DR: 提出Invariant Causal Routing (ICR)因果治理框架，通过识别跨异质环境稳定的政策-规范关系，引导在线市场经济中的社会规范向期望方向发展。


<details>
  <summary>Details</summary>
Motivation: 在线市场经济中，公平曝光、持续参与、平衡再投资等社会规范对长期稳定至关重要，但这些规范源于无数微观交互，难以进行因果归因和政策迁移。

Method: 提出Invariant Causal Routing (ICR)框架，整合反事实推理和不变因果发现，分离真实因果效应与虚假相关，构建可解释、可审计的政策规则。

Result: 在基于真实数据校准的异质智能体模拟中，ICR相比相关性或覆盖度基线，能产生更稳定的规范、更小的泛化差距和更简洁的规则。

Conclusion: 因果不变性为治理提供了原则性和可解释的基础，ICR框架能有效引导社会规范向期望结果发展。
...
TL;DR: 该论文建立了线性非高斯模型中含隐变量和循环的因果图等价性理论，并提出了首个无需结构假设的因果发现方法。
...
Method: 1) 建立线性非高斯模型中任意隐变量结构和循环图的等价性图形准则；2) 引入新工具"边秩约束"填补隐变量因果发现工具箱空白；3) 提供遍历整个等价类的程序；4) 开发从数据恢复模型（至等价类）的算法。

Result: 1) 建立了线性非高斯模型中含隐变量和循环的因果图等价性理论；2) 提出了首个无需结构假设的因果发现方法；3) 提供了代码和交互演示。

Conclusion: 该工作填补了隐变量因果发现中无结构假设等价性表征的空白，为线性非高斯模型提供了首个结构假设自由的发现方法，具有理论和实践意义。
...
### [35] [An LLM-Guided Query-Aware Inference System for GNN Models on Large Knowledge Graphs](https://arxiv.org/abs/2603.04545)
*Waleed Afandi,Hussein Abdallah,Ashraf Aboulnaga,Essam Mansour*

Main category: cs.LG

TL;DR: KG-WISE：一种针对大型知识图谱的任务驱动推理范式，通过分解GNN模型为细粒度组件，结合LLM生成查询模板，实现查询感知的模型实例化，显著提升推理效率并降低内存使用。


<details>
  <summary>Details</summary>
Motivation: 现有GNN推理加速方法（如剪枝、量化、知识蒸馏）存在局限性：它们实例化较小的模型但无法适应单个查询的结构或语义，将模型存储为需要完全加载的单一文件，且无法仅检索与目标节点语义相关的相邻节点和模型组件，导致大型知识图谱上数据加载和计算冗余。

Method: KG-WISE将训练好的GNN模型分解为细粒度组件，可根据查询子图结构部分加载；使用大型语言模型生成可重用的查询模板，为每个任务提取语义相关的子图，实现查询感知且紧凑的模型实例化。

Result: 在包含多达4200万节点和1.66亿边的六个大型知识图谱上评估，KG-WISE相比最先进系统实现了高达28倍的推理加速和98%的内存使用降低，同时在使用商业和开源LLM时保持或提高了准确性。

Conclusion: KG-WISE通过任务驱动的推理范式有效解决了大型知识图谱上GNN推理的效率和内存问题，实现了查询感知的模型实例化，为实际应用提供了高效的解决方案。
...
### [36] [Non-Euclidean Gradient Descent Operates at the Edge of Stability](https://arxiv.org/abs/2603.05002)
*Rustem Islamov,Michael Crawshaw,Jeremy Cohen,Robert Gower*

Main category: cs.LG

TL;DR: 论文通过方向平滑性视角解释边缘稳定性现象，将广义锐度概念扩展到非欧几里得范数，为多种优化器提供了统一的几何感知谱度量。


<details>
  <summary>Details</summary>
Motivation: 边缘稳定性现象在深度学习中广泛观察到，但其理论基础不完整。传统平滑性假设被违反，需要更全面的理论解释。

Method: 使用方向平滑性框架解释EoS，将广义锐度扩展到任意范数，涵盖vanilla GD、预条件GD、ℓ∞-下降、块坐标下降、谱GD和Muon等多种优化器。

Result: 非欧几里得GD在神经网络实验中表现出渐进锐化，随后在阈值2/η附近振荡。框架提供了跨优化器的统一几何感知谱度量。

Conclusion: 方向平滑性为边缘稳定性提供了统一的理论解释，广义锐度框架能够捕捉多种优化器的EoS现象，为理解深度学习优化提供了新视角。
...
### [37] [Federated Causal Discovery Across Heterogeneous Datasets under Latent Confounding](https://arxiv.org/abs/2603.05149)
*Maximilian Hahn,Alina Zajak,Dominik Heider,Adèle Helena Ribeiro*

Main category: cs.LG

TL;DR: 提出fedCI和fedCI-IOD，用于在隐私保护下进行联邦条件独立性检验和因果发现，处理异构数据集和潜在混杂因素。


<details>
  <summary>Details</summary>
Motivation: 解决多数据集因果发现中的数据隐私限制和跨站点异质性挑战，传统方法需要集中式数据集但受法规限制。

Method: 使用联邦迭代重加权最小二乘法估计广义线性模型参数，基于似然比检验进行条件独立性测试；开发fedCI-IOD作为IOD算法的联邦扩展，通过元分析策略聚合证据。

Result: fedCI-IOD在保护隐私的同时显著提升统计功效，性能接近完全池化分析，缓解小样本量问题；提供Python包、R实现和Web应用。

Conclusion: 提出的联邦方法有效解决了隐私保护下的因果发现问题，为分布式异构数据集提供了实用的解决方案。
...
### [38] [Latent Particle World Models: Self-supervised Object-centric Stochastic Dynamics Modeling](https://arxiv.org/abs/2603.04553)
...
<details>
  <summary>Details</summary>
Motivation: 现有世界模型在处理真实世界多物体数据集和决策任务方面存在局限性，需要能够自主发现物体表示并支持多种条件生成的模型。
...
Conclusion: LPWM能够有效学习丰富的场景分解表示，不仅适用于随机视频建模，还能直接应用于决策任务，为自监督世界模型的实际应用提供了新途径。
...
### [39] [Towards a data-scale independent regulariser for robust sparse identification of non-linear dynamics](https://arxiv.org/abs/2603.05201)
*Jay Raut,Daniel N. Wilke,Stephan Schmidt*

Main category: cs.LG

TL;DR: 提出STCV算法解决数据归一化对稀疏回归方法（如SINDy）的负面影响，通过统计显著性而非幅度阈值实现尺度不变的系统辨识。


<details>
  <summary>Details</summary>
Motivation: 数据归一化是常见的预处理步骤，但会严重扭曲基于幅度的稀疏回归方法（如SINDy）发现控制方程的能力。归一化与测量噪声的交互作用破坏了稀疏性假设，导致发现密集、不可解释且物理不正确的模型。

Method: 提出顺序变异系数阈值法（STCV），用无量纲统计指标"系数存在性"（CP）替代传统幅度阈值。CP评估模型库中候选项的统计有效性和一致性，使发现过程对任意数据缩放具有不变性。

Result: 在规范动力系统和实际工程问题（包括物理质量-弹簧-阻尼器实验）上全面基准测试表明，STCV在归一化噪声数据集上始终显著优于标准STLSQ和E-SINDy方法，能成功识别正确的稀疏物理规律。

Conclusion: STCV通过减轻归一化的扭曲效应，使稀疏系统辨识成为更可靠和自动化的现实应用工具，增强了模型的可解释性和可信度。
...
TL;DR: 该研究探讨了持续学习中灾难性遗忘与结构坍塌之间的相关性，通过测量权重和激活的有效秩(eRank)来验证两者关系，并在不同架构和训练策略下进行了实验验证。


<details>
  <summary>Details</summary>
Motivation: 持续学习中灾难性遗忘是主要问题，现有方法多通过任务准确率评估，忽略了内部模型结构。研究表明结构坍塌会导致可塑性丧失，这可能与遗忘相关，因为网络失去扩展特征空间学习新任务的能力，被迫覆盖现有表示。

Method: 通过测量权重和激活的有效秩(eRank)来评估结构坍塌，在MLP、ConvGRU、ResNet-18和Bi-ConvGRU四种架构上，使用SGD、Learning-without-Forgetting(LwF)和Experience Replay(ER)三种策略，在split MNIST和Split CIFAR-100基准上进行实验。

Result: 实验结果表明遗忘与结构坍塌密切相关，不同的持续学习策略以不同效率帮助模型保持容量和性能。

Conclusion: 遗忘与结构坍塌存在强相关性，通过测量eRank可以更好地理解持续学习中的模型行为，不同策略在保持模型容量和性能方面具有不同效率。
...
### [41] [SurvHTE-Bench: A Benchmark for Heterogeneous Treatment Effect Estimation in Survival Analysis](https://arxiv.org/abs/2603.05483)
*Shahriar Noroozizadeh,Xiaobin Shen,Jeremy C. Weiss,George H. Chen*

Main category: cs.LG

TL;DR: SurvHTE-Bench是首个针对右删失生存数据中异质性处理效应估计的综合性基准测试平台，包含合成、半合成和真实数据集，为因果生存方法提供公平、可复现的评估框架。


<details>
  <summary>Details</summary>
Motivation: 生存分析中的异质性处理效应估计在精准医学和个体化政策制定等高风险应用中至关重要，但由于删失、未观测反事实和复杂识别假设等挑战，现有评估方法分散且不一致，缺乏统一的基准测试。

Method: 创建了SurvHTE-Bench基准测试平台，包含：(1)具有已知真实值的模块化合成数据集，系统变化因果假设和生存动态；(2)结合真实世界协变量与模拟处理和结果的半合成数据集；(3)来自双胞胎研究（已知真实值）和HIV临床试验的真实数据集。

Result: 该基准测试首次在合成、半合成和真实世界设置下，对生存HTE方法在不同条件和现实假设违反情况下进行了严格比较，为因果生存方法提供了公平、可复现和可扩展的评估基础。

Conclusion: SurvHTE-Bench填补了生存分析中异质性处理效应估计评估的空白，建立了统一的标准，促进了该领域方法的公平比较和进一步发展。
...
TL;DR: 提出一个多模态AI框架，在不依赖敏感个人信息的情况下检测重复记录，使用语义嵌入、行为模式和设备元数据三种模态，通过DBSCAN聚类实现隐私合规的实体解析。


<details>
  <summary>Details</summary>
Motivation: 重复记录在CRM和医疗保健领域带来重大挑战，传统方法依赖姓名、邮箱等直接标识符，但在GDPR、HIPAA等隐私法规下这些敏感信息被限制或掩码，导致传统方法失效。

Method: 提出一个可扩展的多模态AI框架，包含三种模态：1) 使用预训练DistilBERT模型从文本字段（姓名、城市）提取语义嵌入；2) 从用户登录时间戳提取行为模式；3) 通过分类嵌入编码设备元数据。采用后期融合方法组合这些异构模态，并使用无监督密度聚类算法DBSCAN进行聚类。

Result: 在专门设计的合成CRM数据集上评估，该框架表现出良好性能，取得了不错的F1分数，能够有效识别数据中的重复记录，尽管数据存在变化和噪声。

Conclusion: 该方法提供了隐私合规的实体解析解决方案，支持安全数字基础设施，增强公共卫生分析的可靠性，促进政府和企业的道德AI采用。适合集成到国家健康数据现代化工作中，符合隐私优先创新的更广泛目标。
...
<details>
  <summary>Details</summary>
Motivation: 当前PDE基础模型的下游评估主要关注前向问题（如自回归预测），而逆问题（从观测反推参数）在惯性约束聚变等应用中同样重要但研究不足。本研究旨在探索PDE基础模型在数据有限的ICF逆问题中的应用潜力。

Method: 使用JAG基准数据集，对预训练的PDE基础模型进行微调，并训练一个轻量级任务特定头，联合重建高光谱X射线图像和回归系统参数。通过数据缩放实验（5%-100%训练集）评估性能变化。

Result: 微调模型实现了准确的高光谱重建（测试MSE 1.2e-3）和强大的参数估计性能（R^2高达0.995）。数据缩放实验显示，随着训练数据增加，重建和回归损失持续改善，在低数据区域边际收益最大。使用预训练MORPH权重的微调优于从头训练相同架构。

Conclusion: PDE基础模型初始化显著提高了ICF等数据有限逆问题的样本效率，证明了基础模型在复杂物理逆问题中的实用价值，特别是在低数据区域表现突出。
...
TL;DR: PPO算法在传感器故障导致部分可观测性时表现不佳，通过集成Transformer等时序模型，能显著提升在传感器失效情况下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实强化学习系统需要在观测流存在分布漂移的情况下运行，但大多数策略架构隐含假设完全可观测和无噪声状态。传感器故障会导致部分可观测性和表示偏移，需要研究PPO在这种情况下的鲁棒性。

Method: 将PPO与时序序列模型（包括Transformer和状态空间模型）结合，使策略能够从历史中推断缺失信息。在随机传感器故障过程下，证明了无限时域奖励退化的高概率界限，量化了鲁棒性对策略平滑度和故障持续性的依赖关系。

Result: 在MuJoCo连续控制基准测试中，面对严重的传感器丢失，基于Transformer的序列策略在鲁棒性方面显著优于MLP、RNN和SSM基线，即使大部分传感器不可用时仍能保持高回报。

Conclusion: 时序序列推理为传感器不可靠性导致的观测漂移提供了原则性和实用的可靠操作机制，Transformer架构在应对传感器故障方面表现出色。
...
TL;DR: VeNRA提出了一种可验证的数值推理代理，通过类型化事实账本和双重锁定算法解决金融领域RAG的算术不准确和语义混淆问题，并引入Sentinel模型审计执行痕迹。
...
Method: 1. 引入VeNRA框架，通过类型化通用事实账本(UFL)检索确定性变量而非概率性文本；2. 采用双重锁定算法进行数学边界约束；3. 开发30亿参数的Sentinel模型，通过单令牌测试预算审计Python执行痕迹；4. 使用对抗模拟而非传统幻觉数据集进行训练；5. 采用微分块损失算法解决反向思维链训练中的损失稀释问题。

Result: 论文提出了完整的VeNRA系统架构，包括UFL、双重锁定算法、Sentinel审计模型和对抗训练方法，旨在实现金融领域的零幻觉数值推理。

Conclusion: VeNRA通过将RAG范式从概率文本检索转向确定性变量检索，结合类型化事实账本和执行痕迹审计，为高风险金融领域提供了可验证的数值推理解决方案，解决了传统方法的算术不准确和语义混淆问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于异速生长模型的间接方法存在测量不确定性和方程近似性的限制，无法充分反映树木特征和森林条件的变异性，需要更准确的森林生物量估计方法来支持碳汇监测和气候变化缓解策略。

Method: 使用合成3D森林样地创建具有真实体积的点云数据，通过激光雷达模拟器生成点云，然后训练基于PointNet、PointNet++、DGCNN和PointConv的深度回归网络，最后将训练好的网络应用于真实激光雷达数据进行体积和地上生物量估计。

Result: 在合成数据上，深度回归网络的MAPE为1.69%到8.11%；应用于真实数据时，直接方法的误差为2%到20%，而基于个体树分割和异速生长转换的间接方法以及FullCAM的误差为27%到85%，存在显著低估。

Conclusion: 合成数据与深度学习结合的方法在样地尺度森林碳估计方面具有高效和可扩展的潜力，相比传统间接方法显著提高了精度。
...
TL;DR: 提出TREDBench工程回归数据集基准，通过TabPFN嵌入分析发现工程数据与非工程数据存在差异，提出基于嵌入的合成数据筛选方法，仅用筛选后的合成数据微调TabPFN 2.5，在工程回归任务上显著提升性能和数据效率。


<details>
  <summary>Details</summary>
Motivation: 工程应用中的预测建模长期依赖定制化模型和小型表格数据集，限制了大规模学习方法的应用。现有表格基础模型的预训练合成数据分布与工程数据的统计结构不匹配，限制了向工程回归任务的迁移效果。

Method: 1) 构建TREDBench基准：包含83个真实表格回归数据集，带有工程/非工程专家标签；2) 使用TabPFN 2.5的数据集级嵌入分析领域结构；3) 提出嵌入引导的合成数据筛选方法：生成并识别"工程类似"的合成数据集；4) 仅使用筛选出的合成任务对TabPFN 2.5进行持续预训练。

Result: 在35个工程回归数据集上，仅使用合成数据的适应方法显著提升了预测准确性和数据效率：在29/35数据集上优于TabPFN 2.5，在27/35数据集上优于AutoGluon，平均数据效率分别提升1.75倍和4.44倍。

Conclusion: 工程数据集与非工程数据集存在部分可区分性，而标准程序生成的合成数据集与工程数据集存在显著领域差距。通过原则性的合成数据筛选，可以将程序生成器转化为领域相关的"数据引擎"，使基础模型能够在数据稀缺的科学和工业领域得到改进。
...
TL;DR: 深度矩阵分解（深度线性神经网络）用于矩阵补全，研究网络深度如何影响训练动态。发现耦合动态是深度网络低秩偏置的关键机制，且随深度增强。证明了深度≥3网络存在耦合动态，且收敛到秩1解当且仅当动态耦合。还解释了深度模型避免塑性损失的原因。


<details>
  <summary>Details</summary>
Motivation: 研究深度矩阵分解（深度线性神经网络）作为简化测试平台，探讨网络深度如何影响训练动态。尽管问题简单重要，但先前理论主要关注浅层（深度2）模型，未能完全解释深层网络中观察到的隐式低秩偏置现象。

Method: 通过分析梯度流在块对角观测下的动态，研究深度矩阵分解的耦合动态机制。证明深度≥3的网络除非对角初始化否则会表现出耦合动态，且收敛到秩1解当且仅当动态耦合。同时重新审视矩阵补全中的塑性损失现象。

Result: 发现耦合动态是深度网络低秩偏置的关键机制，且随深度增强。证明了深度≥3网络存在耦合动态，且收敛到秩1解当且仅当动态耦合，解决了Menon（2024）提出的开放性问题。还发现深度模型因低秩偏置而避免塑性损失，而深度2网络在解耦动态下预训练后即使恢复训练也无法收敛到低秩解。

Conclusion: 深度矩阵分解中的耦合动态机制解释了深层网络的隐式低秩偏置，且该机制随深度增强。深度模型因低秩偏置而避免塑性损失，为理解深度网络在矩阵补全任务中的行为提供了理论洞见。
...
TL;DR: 本文提出了一种基于概率方法的Dreamer模型改进，通过并行探索多个潜在状态和保持互斥未来假设，在MPE SimpleTag环境中实现了4.5%的性能提升和28%的回报方差降低。


<details>
  <summary>Details</summary>
Motivation: Dreaming（想象）使智能体能够从想象的经历中学习，实现更鲁棒和样本高效的世界模型学习。然而，现有Dreamer模型在并行探索潜在状态和处理互斥未来假设方面存在局限性，需要改进以提升学习效果。

Method: 采用概率方法对Dreamer模型进行创新：1）实现多个潜在状态的并行探索；2）保持互斥未来的不同假设，同时保留连续潜在变量的良好梯度特性。这些改进使得模型能够更有效地探索状态空间并处理不确定性。

Result: 在MPE SimpleTag环境中评估，改进后的方法相比标准Dreamer模型实现了4.5%的得分提升和28%的回合回报方差降低，表明更稳定和高效的学习性能。

Conclusion: 提出的概率方法有效提升了Dreamer模型的性能，但存在一些限制和未来研究方向，包括超参数（如粒子数K）如何随环境复杂度扩展，以及如何捕捉世界模型中的认知不确定性。
...
TL;DR: 提出了Count Bridges方法，一种用于整数计数数据的随机桥过程，提供精确可处理的扩散式模型，支持从聚合测量中直接训练，应用于单细胞基因表达建模和空间转录组解卷积。


<details>
  <summary>Details</summary>
Motivation: 许多生物测定（如RNA测序）产生整数计数数据，但测量通常是在细胞集合的聚合水平上，而非单细胞分辨率。现有生成框架（如扩散和流匹配）扩展到非欧几里得和离散设置时，如何最佳建模整数数据或系统解卷积聚合观测仍不清楚。

Method: 引入Count Bridges，一种整数上的随机桥过程，提供计数数据的精确可处理扩散式模型，具有闭式条件分布用于高效训练和采样。扩展该框架通过EM式方法直接从聚合测量训练，将单元级计数视为潜变量。

Result: 在整数分布匹配基准测试中实现最先进性能，优于流匹配和离散流匹配基线。成功应用于两个大规模生物学问题：单细胞基因表达数据核苷酸分辨率建模（用于解卷积bulk RNA-seq）和多细胞空间转录组spot解析为单细胞计数谱。
...
TL;DR: 论文发现预训练模型能绕过不可学习样本的保护机制，并提出BAIT方法通过双层优化将人工扰动绑定到错误标签，有效维持数据不可学习性。
...
Method: 提出BAIT（绑定人工扰动到错误目标）的双层优化框架：内层将扰动样本与真实标签关联以模拟标准数据-标签对齐；外层通过强制实施错误标签-扰动绑定，将样本映射到指定的错误目标，覆盖先验的语义指导。

Result: 在标准基准测试和多个预训练骨干网络上的广泛实验表明，BAIT能有效减轻预训练先验的影响，并维持数据的不可学习性。

Conclusion: BAIT通过错误标签-扰动绑定机制，成功克服了预训练先验对不可学习样本的削弱作用，为数据保护提供了更可靠的解决方案。
...
TL;DR: DCT是一个分布条件化传输框架，通过学习源分布和目标分布的嵌入表示，使传输模型能够泛化到训练中未见过的分布对，并支持半监督学习。


<details>
  <summary>Details</summary>
Motivation: 传统传输模型只能处理训练中见过的特定源-目标分布对，但科学应用需要模型能够泛化到未见过的分布对，并且需要利用半监督数据来改进分布预测。

Method: 提出分布条件化传输（DCT）框架，将传输映射条件化于学习到的源分布和目标分布的嵌入表示。该框架与底层传输机制无关，支持从流匹配到分布散度模型（如Wasserstein、MMD）等多种模型。

Result: 在合成基准测试和四个生物学应用中验证了DCT的性能优势：单细胞基因组学中的批次效应转移、质谱流式细胞术数据的扰动预测、造血过程中克隆转录动态学习、以及T细胞受体序列进化建模。

Conclusion: DCT框架能够实现分布传输模型的泛化能力，支持半监督学习，并在多个实际生物学应用中展现出实用性能优势，为科学应用中的分布传输问题提供了灵活有效的解决方案。
...
TL;DR: KindSleep是一个深度学习框架，结合临床知识和单通道血氧信号进行阻塞性睡眠呼吸暂停诊断，在三个大型数据集上表现出色，提供更透明可信的诊断工具。
...
Method: KindSleep首先从原始血氧信号中学习识别临床可解释的概念（如血氧下降指数和呼吸障碍事件），然后将这些AI衍生的概念与多模态临床数据融合，估计呼吸暂停低通气指数。

Result: 在三个大型独立数据集（SHHS、CFS、MrOS，总计9,815人）上评估，KindSleep在估计AHI得分方面表现优异（R2 = 0.917，ICC = 0.957），在OSA严重程度分类上始终优于现有方法，加权F1分数在0.827到0.941之间。
...
Method: 引入ConTSG-Bench基准测试，包含大规模、对齐良好的数据集，涵盖多种条件模态和语义抽象层次，使用全面的生成保真度和条件遵循度指标对代表性生成方法进行系统评估。

Result: 定量基准测试和深入分析揭示了当前方法的特征和局限性，特别是在复杂条件下精确结构可控性和下游任务效用方面存在关键挑战。

Conclusion: ConTSG-Bench为条件时间序列生成提供了标准化评估框架，揭示了当前方法的不足，并指出了未来研究方向，特别是在精确结构控制和复杂条件下下游任务效用方面。
...
TL;DR: 提出基于分布风险敏感强化学习的均衡器参数优化框架，结合信息瓶颈潜在表示与条件风险价值优化，实现高速内存系统信号完整性优化。


<details>
  <summary>Details</summary>
Motivation: 现有均衡器参数优化方法存在计算成本高（眼图评估）、优化期望性能而非最坏情况性能、缺乏不确定性量化等问题，无法满足高速内存系统生产规模部署需求。

Method: 提出分布风险敏感强化学习框架：1) 信息瓶颈潜在表示实现速率-失真最优信号压缩；2) 蒙特卡洛dropout量化认知不确定性；3) 分位数回归实现最坏情况优化；4) PAC-Bayesian正则化保证泛化边界。

Result: 在240万个波形上验证：4抽头和8抽头均衡器配置平均改善37.1%和41.5%，最坏情况保证33.8%和38.2%，比Q学习基线提升80.7%和89.1%。实现62.5%高可靠性分类，消除大部分手动验证。

Conclusion: 该框架为生产规模均衡器优化提供了实用解决方案，具有认证的最坏情况保证，显著提升高速内存系统信号完整性优化的效率和可靠性。
...
TL;DR: 提出一种在on-policy强化学习中高效训练扩散策略的新方法，通过将策略迭代与扩散过程对齐，只需计算简单高斯概率，避免了传统方法中计算扩散模型动作对数似然的计算瓶颈。
...
Method: 提出一种新颖高效的方法，通过将策略迭代与扩散过程对齐，只需评估简单高斯概率即可在on-policy设置中训练扩散策略。该方法还能自然地处理熵正则化，这是扩散策略中通常难以纳入的。

Result: 实验表明，所提方法能够产生多模态策略行为，并在IsaacLab和MuJoCo Playground的各种基准任务上实现了优越性能。

Conclusion: 该方法解决了扩散策略在on-policy强化学习中的关键挑战，提供了一种计算高效且能处理熵正则化的训练范式，为扩散策略在决策问题中的广泛应用铺平了道路。
...
TL;DR: 该论文利用KANs的结构优势，通过多级训练算法显著提升训练速度，特别是针对物理信息神经网络。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络训练速度提升困难，因为缺乏结构保证。KANs通过基函数展开提供了更多结构，这为开发高效训练算法提供了机会。

Method: 建立KANs与MLPs的等价关系，分析基变换对梯度优化的影响，提出基于样条节点均匀细化的多级训练方法，使用几何插值算子在模型间传递信息。

Result: 多级训练方法相比传统方法在精度上获得数量级提升，特别是对物理信息神经网络，训练性能显著改善。

Conclusion: 神经网络的有原则设计可以产生可利用的结构，多级算法能显著提升训练性能，为高效神经网络训练提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有解释方法因缺失性偏差（模型在缺失输入时产生的系统失真）而产生不可靠的特征重要性分数。现有解决方案需要昂贵的重新训练或架构修改，作者认为这可以更简单地解决。

Method: MCal方法：在冻结的基础模型输出上微调一个简单的线性头层，将缺失性偏差视为模型输出空间的表面伪影进行校正。

Result: 该方法能持续减少缺失性偏差，在视觉、语言和表格数据等多种医学基准测试中，与先前重量级方法相比具有竞争力甚至表现更优。

Conclusion: 缺失性偏差可以通过轻量级后处理方法有效校正，无需昂贵的重新训练或架构修改，为解释方法提供了更实用的解决方案。
...
TL;DR: 论文证明基于梯度的对齐训练本质上集中在决定危害的位置，超过危害决策点的位置梯度消失，导致对齐是浅层的而非深层的。


<details>
  <summary>Details</summary>
Motivation: 研究为什么大语言模型的安全对齐是浅层的，解释为什么对齐训练无法产生深度对齐，以及为什么KL散度集中在早期token上。

Method: 使用序列级危害的鞅分解，推导出对齐梯度的精确表征，引入危害信息$I_t$量化每个位置对危害的影响，并基于恢复惩罚提出新的目标函数。

Result: 证明梯度在位置$t$等于条件期望危害与得分函数的协方差，超过危害决策点的位置梯度为零，平衡KL散度跟踪危害信息，新目标函数能在所有位置产生梯度信号。

Conclusion: 标准对齐目标无法产生深度对齐，无论优化质量如何。基于恢复惩罚的目标为经验上成功的数据增强技术提供了理论基础。
...
TL;DR: 本文提出一个统一特征中心框架，分析DP-SGD在两层ReLU CNN中的特征学习动态，发现隐私噪声导致次优特征学习，造成公平性问题和对抗脆弱性。
...
Method: 引入统一特征中心框架分析DP-SGD在两层ReLU卷积神经网络中的特征学习动态，建立由特征噪声比(FNR)控制的理论损失边界。

Result: 隐私噪声导致次优特征学习：1) 类别和子群体间FNR不平衡导致差异影响；2) 噪声对语义长尾数据负面影响更大；3) 噪声注入加剧对抗攻击脆弱性。公共预训练+私有微调范式在特征分布显著偏移时不一定改善性能。

Conclusion: 隐私噪声通过特征噪声比影响特征学习质量，导致公平性问题和对抗脆弱性。需要重新思考差分隐私学习的设计，特别是在存在分布偏移时。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态联邦学习方法忽视了个性化客户端性能，难以处理模态/任务差异和模型异构性，需要更有效的框架来提升客户端和服务器学习效果。

Method: 客户端采用双级对抗对齐策略对齐局部和全局表示，粒度感知融合模块自适应整合全局知识；服务器端使用相似性引导的集成蒸馏机制，基于特征相似性聚合客户端表示并蒸馏到全局模型。

Result: 在IID和非IID设置下的广泛实验表明，FedAFD在客户端和服务器端都实现了优越的性能和效率。

Conclusion: FedAFD通过创新的客户端对齐融合和服务器端集成蒸馏机制，有效解决了多模态联邦学习中的个性化、模态差异和模型异构性问题，为隐私保护的多模态协作学习提供了有效解决方案。
...
Method: 集成大型语言模型辅助规划、鲁棒的融合定位和轨迹跟踪技术，采用分布式架构和超宽带技术进行辅助定位

Result: 通过真实车辆演示验证了系统在挑战性室内环境中的可靠性和有效性

Conclusion: U-Parking系统展示了LLM辅助规划与UWB融合定位相结合的方法能够实现室内复杂环境下的可靠自主泊车
...
TL;DR: EVMbench是一个评估AI代理检测、修补和利用智能合约漏洞能力的基准测试，包含117个精选漏洞，使用基于测试和区块链状态的程序化评分，发现前沿AI代理能够端到端地发现和利用实时区块链漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着公链智能合约管理大量价值，其漏洞可能导致重大损失。AI代理在读写和运行代码方面能力不断增强，需要评估它们在这个领域的能力，包括提升安全性和增加风险两方面。

Method: 引入EVMbench评估框架，基于40个代码库中的117个精选漏洞，在最真实的环境中，使用基于测试和本地以太坊执行环境下区块链状态的程序化评分方法。
...
TL;DR: BandPO提出动态概率感知的裁剪区间，解决PPO固定裁剪边界对低概率高优势动作更新的限制问题
...
Method: 引入BandPO，用Band算子替代标准裁剪机制，将f-散度定义的信任区域投影到动态、概率感知的裁剪区间，通过凸优化问题求解

Result: BandPO在多种模型和数据集上持续优于标准裁剪和Clip-Higher方法，有效缓解熵崩溃问题

Conclusion: BandPO通过动态概率感知的裁剪机制解决了PPO的探索瓶颈，为LLM强化学习提供了更稳定的近端约束方法
...
TL;DR: 论文提出了一种语义通信增强的U型分割联邦学习框架（SC-USFL），用于解决车联网中传统集中式学习的通信开销和隐私问题，通过语义压缩和自适应调整来平衡通信负载、保护隐私并保持学习性能。


<details>
  <summary>Details</summary>
Motivation: 车联网边缘智能对智能交通系统至关重要，但传统集中式学习面临高通信开销和隐私风险。分割联邦学习虽然提供分布式解决方案，但存在高维中间特征传输的通信瓶颈和标签隐私问题。

Method: 提出语义通信增强的U型分割联邦学习框架（SC-USFL），包含预训练的参数冻结编码/解码单元的语义通信模块，仅传输任务相关的语义信息；网络状态监控模块实时调整语义压缩率以适应无线信道变化。

Result: SC-USFL框架在资源受限的车联网环境中，能够有效平衡通信负载、保护隐私并保持学习性能，通过本地化敏感计算减少开销并增强标签隐私。

Conclusion: 语义通信与分割联邦学习的结合为车联网提供了有前景的解决方案，论文还指出了未来研究的关键方向以进一步推进这两项技术的协同发展。
...
Method: 提出∇-Reasoner框架，核心是DTO（可微文本优化），在解码循环中集成对token logits的可微优化，利用LLM似然和奖励模型的梯度信号精炼文本表示，并加入拒绝采样和加速设计

Result: 在数学推理基准上获得超过20%的准确率提升，相比强基线减少约10-40%的模型调用次数

Conclusion: 从零阶搜索到一阶优化的范式转变，为增强LLM推理能力提供了成本效益高的路径
...
TL;DR: 研究评估了三种基于LSTM、GRU和Transformer的序列模型，通过蒙特卡洛dropout或证据输出层进行不确定性量化，用于1型糖尿病血糖预测和不良血糖事件识别。Transformer结合证据输出头在准确性和不确定性校准方面表现最佳。


<details>
  <summary>Details</summary>
Motivation: 研究动机是将不确定性量化整合到实时机器学习血糖预测系统中，以提高预测可靠性并识别不良血糖事件，这对于1型糖尿病患者的临床风险管理至关重要。

Method: 使用三种序列模型架构（LSTM、GRU、Transformer），通过两种不确定性量化方法（蒙特卡洛dropout、Deep Evidential Regression的证据输出层）。在HUPA-UCM糖尿病数据集上进行验证，并使用糖尿病技术学会误差网格进行临床风险评估。

Result: 基于Transformer架构并配备证据输出头的模型表现最佳，具有更高的预测准确性和更好的校准不确定性估计，其不确定性大小与预测误差显著相关。在临床风险评估中也表现最优。

Conclusion: 将原则性不确定性量化整合到实时机器学习血糖预测系统中具有重要价值，能够提供更可靠的预测和更好的临床风险管理能力。
...
TL;DR: 提出WaterSIC算法，通过水填充策略为权重矩阵的不同列分配不同量化比特率，在1-4比特量化中达到信息论极限，显著优于GPTQ算法


<details>
  <summary>Details</summary>
Motivation: 现有GPTQ等量化算法与信息论极限存在较大差距，需要设计更接近理论最优的量化方法，特别是在大语言模型权重压缩中实现更好的精度保持

Method: 提出WaterSIC算法，借鉴信息论中的"水填充"思想，根据输入激活的协方差矩阵为权重矩阵的不同列（输入特征）分配不同的量化比特率，实现更优的压缩长度与输出误差权衡

Result: WaterSIC算法在信息论极限0.255比特范围内，显著优于GPTQ（可能存在任意大差距），在Llama和Qwen系列LLM上，1-4比特量化均达到新的SOTA性能

Conclusion: 通过水填充策略实现列级比特率分配的WaterSIC算法，能够接近信息论极限，为大语言模型的高效低精度量化提供了理论保证和实用解决方案
...
TL;DR: MOUE提出虚拟宽度作为MoE的新扩展维度，通过跨层复用通用专家池，在固定计算预算下将深度转换为虚拟宽度，相比传统MoE有显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统MoE架构的扩展受限于物理深度和宽度维度，需要新的扩展维度来进一步提升模型能力而不增加计算成本。

Method: 提出MOUE框架，包含三个核心组件：交错旋转拓扑结构化的专家共享、通用专家负载平衡进行深度感知的暴露校正、带轻量轨迹状态的通用路由器实现连贯的多步路由。

Result: MOUE在多个扩展场景下比匹配的MoE基线提升高达1.3%，能够渐进转换现有MoE检查点并获得高达4.2%的增益，揭示了MoE架构的新扩展维度。

Conclusion: MOUE成功引入了虚拟宽度作为MoE的新扩展维度，通过跨层复用专家池有效提升了模型性能，为MoE架构的进一步发展提供了新方向。
...
TL;DR: 提出基于Fisher-Rao流形Karcher平均的模型合并方法，解决现有参数空间启发式方法的功能合并、表示崩溃和多专家合并问题


<details>
  <summary>Details</summary>
Motivation: 现有权重空间合并方法本质上是参数空间启发式，存在三个实际问题：1) 欧几里得坐标操作而非功能合并；2) 源检查点差异大时导致表示崩溃；3) 几何方法难以扩展到N>2专家合并

Method: 将模型合并形式化为Fisher-Rao流形上的加权Karcher平均计算，等价于最小化预测分布的KL散度函数距离，提出使用轻量级球形代理的实用定点算法

Result: 在各种基准测试和崩溃诊断中，该方法在合并模型数量和异质性增加时保持稳定，始终优于现有基线方法

Conclusion: 基于Fisher-Rao流形的Karcher平均方法为模型合并提供了更稳定、可扩展的解决方案，解决了现有参数空间方法的根本局限性
...
<details>
  <summary>Details</summary>
Motivation: 非侵入式负荷监测（NILM）面临跨域泛化的挑战，现有迁移学习方法存在固定电器集、缺乏灵活性、不适合边缘设备或实时性差等问题，需要一种更实用的解决方案。

Method: 提出RefQuery框架，使用紧凑的电器指纹作为条件进行分解，保持预训练分解网络完全冻结，仅通过轻量级反向传播阶段学习每个电器的嵌入来适应目标家庭。

Result: 在三个公共数据集上的实验表明，RefQuery在单电器和多电器基准测试（包括现代基于Transformer的方法）中实现了强大的准确性与效率权衡。
...
TL;DR: 该研究提出了一个多运营商强化学习框架，用于分析竞争对自动驾驶按需出行系统的影响，通过集成离散选择理论实现乘客分配和需求竞争的内生演化。


<details>
  <summary>Details</summary>
Motivation: 现实中的AMoD市场将是竞争性的，多个运营商通过战略定价和车队部署争夺乘客。现有研究未能捕捉竞争性市场动态，需要研究竞争对策略学习的影响。

Method: 引入多运营商强化学习框架，两个运营商同时学习定价和车队再平衡策略。集成离散选择理论，使乘客分配和需求竞争从效用最大化决策中内生演化。

Result: 使用多个城市的真实数据实验表明，竞争从根本上改变了学习行为，与垄断设置相比导致更低的价格和不同的车队定位模式。基于学习的方法对竞争的额外随机性具有鲁棒性。

Conclusion: 竞争显著影响AMoD系统的学习行为，基于学习的方法能够有效处理竞争环境中的不确定性，成功收敛到有效策略。
...
TL;DR: 提出BA-Logic方法，在干净标签设置下进行有效的图后门攻击，通过协调中毒节点选择器和逻辑中毒触发器生成器来毒化GNN模型的内部预测逻辑。


<details>
  <summary>Details</summary>
Motivation: 现有图后门攻击需要修改训练标签，这在现实场景中不实用。干净标签图后门攻击是一个现实但研究不足的课题，现有方法在干净标签设置下普遍失败。

Method: 提出BA-Logic方法，包含中毒节点选择器和逻辑中毒触发器生成器，通过协调这两个组件来毒化GNN模型的内部预测逻辑，使触发器对预测变得重要。

Result: 在真实世界数据集上的大量实验表明，该方法有效提高了攻击成功率，在干净标签设置下超越了最先进的图后门攻击竞争对手。

Conclusion: BA-Logic解决了干净标签图后门攻击的有效性问题，通过毒化GNN模型的内部预测逻辑，在现实场景中实现了更实用的后门攻击。
...
<details>
  <summary>Details</summary>
Motivation: 传统通过在训练时注入位翻转错误的方法存在计算开销大、在高错误率下推理精度下降、难以扩展到大型神经网络架构等问题，需要一种更高效、可扩展的位错误容忍方法。

Method: 建立位错误容忍度与输出层分类边界之间的直接联系，提出Margin Cross-Entropy Loss (MCEL)损失函数，该函数在保持标准交叉熵损失优化特性的同时，显式促进logit级别的边界分离，并引入可解释的边界参数来调节鲁棒性。

Result: 在多个不同复杂度的数据集、多样化的神经网络架构和多种量化方案上的实验评估表明，MCEL显著提高了位错误容忍度，在1%错误率下准确率提升高达15%。

Conclusion: MCEL方法简单易实现、高效，可作为标准CEL的直接替代方案，为神经网络鲁棒性提供了可扩展和原则性的替代方案，有助于在近似计算和易错内存系统上的高效部署。
...
TL;DR: 本文对多任务学习中的感知器误设模型进行渐近分析，揭示了多任务组合如何通过等效于带额外正则项的传统公式来提升泛化性能，并实证表明多任务能推迟并渐近缓解双下降现象。


<details>
  <summary>Details</summary>
Motivation: 多任务学习旨在通过利用多个相关任务间的共同信息来改善泛化误差，但挑战在于找到能够揭示任务间共同信息的有效公式。本文旨在精确分析多任务公式的渐近行为，理解多任务组合带来性能提升的根本原因。

Method: 对误设感知器学习模型的多任务公式进行精确渐近分析，证明多任务组合在渐近意义上等价于带有额外正则项的传统单任务公式，并通过实证研究多任务组合对泛化误差的影响。

Result: 理论分析表明，多任务组合在渐近上等价于带有额外正则项的单任务公式，这些正则项有助于提升泛化性能。实证结果显示，多任务组合能够推迟双下降现象的发生，并能在渐近意义上缓解该现象。

Conclusion: 多任务学习通过揭示任务间的共同信息，在渐近意义上等价于带有额外正则项的传统公式，从而改善泛化性能。多任务组合不仅能推迟双下降现象，还能在渐近意义上缓解该现象，为多任务学习的理论理解提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 传统ISAC友好干扰方法需要窃听者的CSI或精确AoA估计，但在实际场景中这些信息往往难以获取。同时，雷达感知存在不确定性，CSI不完美，需要一种更鲁棒的物理层安全方案。

Method: 1) 利用雷达回波反馈引导定向干扰，无需窃听者显式信息；2) 提出雷达感知神经网络，联合优化波束成形和干扰；3) 基于f-散度的非参数Fisher信息矩阵估计器增强鲁棒性；4) 引入量化张量列车编码器，模型大小减少100倍以上；5) 集成非重叠安全方案，专用子带用于通信。

Result: 仿真显示：显著提升保密率、降低误块率，对CSI不确定性和角度估计误差具有强鲁棒性。模型压缩后性能损失可忽略。

Conclusion: 提出的深度学习驱动友好干扰框架在实际ISAC损伤下有效，无需窃听者CSI或精确AoA估计，通过雷达回波反馈和鲁棒神经网络设计实现了物理层安全增强。
...
TL;DR: RCRL框架训练单个智能体优化一族奖励函数，仅使用单一目标收集经验，通过奖励参数化条件化智能体，实现从共享回放数据中学习多个奖励目标


<details>
  <summary>Details</summary>
Motivation: 传统RL智能体在单一固定奖励函数下训练，对奖励错误指定敏感，难以适应变化的任务偏好，需要更鲁棒和可适应的学习框架

Method: 提出奖励条件强化学习(RCRL)，让智能体基于奖励参数化进行条件化学习，从共享的离策略回放数据中学习多个奖励目标，实现单一策略表示奖励特定行为

Result: 在单任务、多任务和基于视觉的基准测试中，RCRL不仅提高了名义奖励参数化下的性能，还能高效适应新的参数化设置

Conclusion: RCRL为学习鲁棒、可引导的策略提供了可扩展机制，无需牺牲单任务训练的简单性
...
TL;DR: 提出基于广义Kuramoto模型的新聚类算法，用于单位超球面上的数据聚类，相比传统方法在聚类准确率上表现相似或更好。
...
Method: 基于d维广义Kuramoto模型，开发了一种新的聚类算法，专门处理表示为单位球面$\mathbf{S}^{d-1}$上点的数据。

Result: 在合成和真实数据集上验证了方法的有效性，与传统聚类方法相比，在聚类准确率方面取得了相似或更好的结果。

Conclusion: 提出的基于广义Kuramoto模型的聚类算法能够有效处理单位超球面数据，考虑了球面的几何结构，在聚类性能上优于或等同于传统方法。
...
TL;DR: Aura框架通过三部分编码机制整合多模态外生信息，提升航空维护场景的时间序列预测性能


<details>
  <summary>Details</summary>
Motivation: 实际工业应用中，时间序列预测需要整合多维甚至多模态的外生因素，但现有单模态模型难以捕捉这些异质交互关系。特别是在航空维护场景中，存在多种不同类型的外生因素以不同交互模式影响时间动态。

Method: 提出Aura通用框架，根据外生信息与目标时间序列的交互模式，显式组织和编码异质外部信息。采用定制的三部分编码机制，将异质特征嵌入到成熟的时间序列模型中，实现非时序上下文的无缝集成。

Result: 在中国南方航空为期三年的大规模工业数据集（涵盖波音777和空客A320机队）上的广泛实验表明，Aura在所有基线方法中始终达到最先进的性能，并展现出卓越的适应性。

Conclusion: Aura作为通用增强框架，在航空安全和可靠性方面具有重要潜力，能够有效整合多模态外生信息提升时间序列预测准确性。
...
Method: 提出基于最优生成流的Aumann-Shapley归因形式理论，证明梯度线积分是满足效率和几何公理（特别是重参数化不变性）的唯一泛函。通过选择将先验分布传输到数据分布的动能最小化Wasserstein-2测地线来解决路径模糊性

Result: 建立了规范的归因族，对加性模型恢复经典Shapley，具有可证明的流近似误差稳定性边界。实验表明优于基线方法，通过消失的流一致性误差实现严格流形保持，并通过结构感知总变差获得更好的语义对齐

Conclusion: 通过将基线选择重构为变分问题，提出了基于最优传输流的流形上归因理论，解决了传统Shapley归因的离流形伪影问题，实现了几何效率和语义对齐的改进
...
TL;DR: 提出兩階段獎勵課程學習方法，先訓練簡化任務獎勵確保有效探索，再引入完整獎勵包含行為相關項，顯著提升多目標強化學習性能


<details>
  <summary>Details</summary>
Motivation: 深度強化學習在機器人控制中面臨設計有效獎勵函數的困難，現實任務通常需要同時優化多個目標，需要精確調整權重以獲得期望策略特性

Method: 提出兩階段獎勵課程學習：第一階段使用簡化的僅任務獎勵函數訓練，確保有效探索；第二階段引入包含能量效率等輔助行為項的完整獎勵。分析不同過渡策略，重用階段間樣本對訓練穩定性至關重要

Result: 在DeepMind Control Suite、ManiSkill3和移動機器人環境中驗證，方法簡單有效，顯著優於直接在完整獎勵上訓練的基線，對特定獎勵權重具有更高魯棒性

Conclusion: 兩階段獎勵課程學習能有效解決多目標強化學習中的獎勵設計難題，通過解耦任務特定目標和行為項，實現更穩定高效的策略學習
...
TL;DR: 提出FedBCGD和FedBCGD+算法，通过参数分块上传降低联邦学习中大模型（如Vision Transformer）的通信开销，通信复杂度比现有方法降低1/N倍
...
Conclusion: 首次提出参数块通信方法训练大规模深度模型，显著降低联邦学习通信开销，代码已开源
...
Result: 在LeNet-5模型和CIFAR-10、Fashion-MNIST数据集上评估，针对多种重建攻击。结果显示：高隐私保护级别下PI实现最平衡的权衡，中等隐私要求下DP方法更优

Conclusion: Alt-FL框架为不同隐私和资源约束下的联邦学习隐私保护方法选择提供了基础，实现了灵活的质量-效率权衡
...
TL;DR: 提出一种可训练的特定任务特征量化层，通过可学习的量化阈值压缩神经网络输入特征，减少边缘设备到服务器的数据传输，在保持精度的同时实现5-16倍压缩。


<details>
  <summary>Details</summary>
Motivation: 物联网中机器学习应用对有限计算和内存资源的需求增长，现有方法仍需远程服务器支持，但数据传输受带宽、延迟和能耗限制，需要有效的边缘压缩方案。

Method: 设计可训练的特征量化层，使用sigmoid函数逼近阶跃函数实现可学习的量化阈值，通过多个sigmoid输出的拼接（称为位级软量化）实现与神经网络集成的可训练量化值。

Result: 相比标准量化方法表现更优，在保持接近全精度模型准确率的同时，相比32位输入实现5-16倍的压缩比，具体取决于数据集。

Conclusion: 提出的可训练特征量化层能有效压缩边缘设备数据，减少服务器传输需求，在保持模型性能的同时显著降低带宽和能耗需求。
...
TL;DR: 将AI监管建模为不确定条件下的机制设计问题，提出监管机制框架，通过让提供商对模型能力下注来选择许可证，实现完美市场结果：排除不合规提供商，保留合规提供商。


<details>
  <summary>Details</summary>
Motivation: 面对高风险的机器学习应用需要严格监管，但策略性的ML提供商经常规避监管以降低开发成本。需要设计可执行的AI监管框架来解决这一挑战。

Method: 将AI监管建模为机制设计问题，引入监管机制框架：将模型的经验证据映射到市场份额许可证。提供商可以从一组许可证中选择，迫使他们对自己的模型满足监管要求的能力下注。

Result: 证明机制具有完美市场结果（排除不合规提供商、保留合规提供商）当且仅当不合规分布集合构成一个信度集（封闭、凸的概率测度集合）。建立了监管机制与不合规分布集合之间的对偶关系。

Conclusion: 该框架为机制设计和不精确概率理论的交叉领域提供了新见解，为开发可执行的AI监管奠定了基础。通过实验验证了在监管虚假特征使用和公平性方面的实际应用。
...
<details>
  <summary>Details</summary>
Motivation: LoRA虽然在实践中有效，但其理论基础不足，特别是在特征学习稳定性方面。研究发现LoRA在适当超参数和初始化下可以实现自稳定，但必要的非零初始化会破坏这种稳定性，导致性能下降

Method: 提出Stable-LoRA方法，采用权重收缩优化策略，在训练初期动态收缩A矩阵，从而增强LoRA特征学习的稳定性

Result: Stable-LoRA在多种模型和任务上一致优于其他基线方法，无需额外内存开销，计算开销可忽略不计

Conclusion: Stable-LoRA有效解决了LoRA特征学习的不稳定性问题，在保持非零初始化优势的同时提升了性能，为LoRA提供了更稳定的理论基础
...
TL;DR: 提出IAENet框架，用于术中不良事件的早期预警，通过多标签学习、时间感知特征融合和标签约束重加权损失，在5-15分钟预警任务上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 术中不良事件早期预警对降低手术风险和改善患者安全至关重要。现有深度学习方法存在三个关键挑战：忽视不良事件间的依赖关系、未充分利用异构临床数据、以及医疗数据固有的类别不平衡问题。

Method: 1. 构建首个术中多标签不良事件数据集(MuAE)，涵盖六种关键事件；2. 提出基于Transformer的多标签学习框架(IAENet)，包含改进的时间感知特征线性调制(TAFiLM)模块，用于静态协变量和动态变量的鲁棒融合及复杂时间依赖建模；3. 引入带共现正则化的标签约束重加权损失(LCRLoss)，有效缓解事件内不平衡并增强频繁共现事件间的结构化一致性。
...
TL;DR: 通过干预性架构修改研究Transformer中的grokking现象，发现限制表示幅度和使用均匀注意力可以显著加速甚至消除延迟泛化，但效果依赖于任务对称性对齐。


<details>
  <summary>Details</summary>
Motivation: 传统机制可解释性主要依赖训练后分析，本文采用干预性方法，通过修改架构拓扑来测试假设，研究Transformer在循环模加法任务中grokking（延迟泛化）现象，探究特定架构自由度是否延长记忆阶段。

Method: 1. 引入完全有界的球形拓扑：在残差流中强制L2归一化，并使用固定温度尺度的解嵌入矩阵，消除基于幅度的自由度。2. 均匀注意力消融：用均匀分布替代数据依赖的查询-键路由，将注意力层简化为连续词袋聚合器。3. 使用非交换S5置换组合作为负对照，验证加速效果是否特定于任务几何对齐。

Result: 1. 球形拓扑将grokking起始时间减少20倍以上（无需权重衰减）。2. 均匀注意力模型在所有种子中实现100%泛化，完全绕过grokking延迟。3. 在S5任务上，球形约束不加速泛化，表明消除记忆阶段强烈依赖于架构先验与任务内在对称性的对齐。

Conclusion: 架构自由度显著影响grokking现象，提供干预性证据表明，通过预测性结构视角可以理解训练动态，消除记忆阶段依赖于架构先验与任务对称性的对齐。
...
<details>
  <summary>Details</summary>
Motivation: NVIDIA的2:4稀疏张量核心虽然提供2倍吞吐量，但需要严格的50%剪枝，这会严重损害LLM推理精度（如Qwen3从54%降至15%）。而更温和的(2N-2):2N模式（如6:8，25%剪枝）能保持精度，但缺乏硬件支持，只能回退到密集执行而无法获得稀疏性优势。

Method: SlideSparse采用两种关键技术：1）滑动窗口分解：将任意(2N-2):2N权重块重构为N-1个重叠的2:4兼容窗口，无精度损失；2）激活提升：将相应的激活重排融合到每令牌量化中，成本几乎为零。

Result: 在计算受限的工作负载中，实测加速比达到1.33倍，接近理论上限N/(N-1)=4/3。在Qwen2.5-7B的6:8权重稀疏度下，证明了(2N-2):2N作为保持精度的LLM加速的实用路径。

Conclusion: SlideSparse首次在商用GPU上为(2N-2):2N稀疏模式家族实现了稀疏张量核心加速，在保持LLM推理精度的同时实现了显著的性能提升，为精度保持的LLM加速提供了实用解决方案。
...
TL;DR: RIMs框架将神经推理与经典递归推理机制结合，扩展了TRMs并提升复杂推理任务性能


<details>
  <summary>Details</summary>
Motivation: 现有神经推理系统（如TRMs）与经典随机推理系统之间存在鸿沟，前者依赖神经主干，后者使用专门的推理规则。需要将两种范式结合以获得更好的推理能力。

Method: 提出递归推理机（RIMs）框架，显式整合经典推理引擎的递归推理机制。通过重加权组件扩展TRMs，使其成为RIMs的一个实例。

Result: 在ARC-AGI-1、ARC-AGI-2和Sudoku Extreme等挑战性推理基准上表现更好。在表格数据分类任务上也优于TabPFNs。

Conclusion: RIMs成功桥接了神经推理与经典推理范式，通过整合递归推理机制显著提升了复杂推理任务的性能。
...
TL;DR: 提出基于双轮盘选择(DRS)初始化和递归自动分割的两阶段联邦学习框架，用于风电功率预测，通过行为聚类解决数据隐私、成本和异构性问题


<details>
  <summary>Details</summary>
Motivation: 短期风电功率预测对电网调度和市场运营至关重要，但集中化风机数据收集存在隐私泄露、成本高昂和数据异构性问题，需要一种保护隐私的分布式解决方案

Method: 两阶段联邦学习框架：第一阶段使用双轮盘选择(DRS)初始化和递归自动分割方法，基于长期行为统计对风机进行聚类；第二阶段通过FedAvg算法训练每个聚类的LSTM模型

Result: 在丹麦400个独立风机上的实验表明，DRS-auto方法能发现行为一致的聚类组，在保持数据本地化的同时获得有竞争力的预测精度。行为感知聚类始终优于地理分区方法，并与k-means++基线方法表现相当

Conclusion: 该框架为异构分布式风机群提供了一种实用的隐私友好解决方案，通过行为聚类实现有效的联邦学习，在保护数据隐私的同时保持预测性能
...
TL;DR: 该论文指出传统ASR评估指标WER的局限性，提出使用非线性语义指标和样本难度指数来揭示系统偏见，建立更鲁棒的审计框架。


<details>
  <summary>Details</summary>
Motivation: 传统ASR系统主要使用词错误率(WER)评估，但这种基于词级统计的指标无法捕捉语义保真度，且会掩盖"多样性税"问题——即边缘化和非典型说话者因系统识别失败而承受不成比例的负担。

Method: 1. 系统评估更广泛的非线性和语义指标；2. 引入样本难度指数(SDI)，量化内在人口统计和声学因素如何驱动模型失败；3. 通过数据制图映射SDI，展示EmbER和SemDist等指标如何揭示WER忽略的隐藏系统偏见和模型间分歧。

Result: 研究发现EmbER和SemDist等指标能够暴露WER忽略的隐藏系统偏见和模型间分歧，SDI能够有效量化不同因素对模型失败的影响。

Conclusion: 这是建立鲁棒审计框架的第一步，使开发者能够在部署前审计和缓解ASR差异，实现前瞻性安全分析。
...
TL;DR: Whisperer框架通过扩散模型预处理输入图像，以行为克隆方式提升冻结OCR模型的性能，在300k退化文本图像上实现8%绝对CER降低
...
Method: 提出Whisperer视觉提示框架，使用扩散模型预处理输入图像。采用四阶段训练课程：通过扩散模型随机探索中间输出，选择偶然提升CER的样本，然后通过行为克隆训练模型复制这些改进

Result: 在300k退化合成文本图像数据集上，相比EasyOCR基线实现8%绝对（10.6%相对）CER降低，超越CLAHE等手工设计基线方法

Conclusion: 通过向冻结OCR模型"低语"输入增强，可以在不修改模型权重的情况下提升不完美分类器的性能，方法样本高效且避免了传统强化学习的缺陷
...
<details>
  <summary>Details</summary>
Motivation: AI安全领域提出了辩论和基于AI反馈的强化学习(RLAIF)两种可扩展监督方法，但缺乏形式化框架来关联这两种方法并明确辩论何时具有优势。

Method: 通过辩论模型间知识分歧的几何参数化，使用模型表示子空间之间的主角度分析辩论价值，推导出辩论优势的精确闭式解。

Result: 当模型共享相同训练语料时，辩论退化为类似RLAIF的单智能体方法；当知识分歧时，辩论优势呈现从二次型到线性型的相变；识别了三种知识分歧机制（共享、单边、组合），并证明辩论可实现单个模型无法达成的结果，但过强的对抗性激励会导致组合机制中的协调失败。

Conclusion: 本文建立了辩论与RLAIF之间的首个形式化联系，为理解对抗性监督协议的合理性提供了几何基础，并连接了在具有互补信息的模型间提取潜在知识的问题。
...
TL;DR: WavSLM：首个基于自监督WavLM表征的单流语音语言模型，无需文本监督或文本预训练，通过量化蒸馏实现语义和声学信息的联合建模


<details>
  <summary>Details</summary>
Motivation: 现有语音语言模型大多依赖文本监督、分层token流或复杂混合架构，背离了文本领域证明有效的单流生成预训练范式。需要将简单自回归训练扩展到语音领域，同时解决语义和声学信息纠缠的挑战。
...
TL;DR: GALACTIC：首个统一框架，用于无监督时间序列聚类的局部和全局反事实可解释性，通过聚类感知优化和最小描述长度目标生成稀疏反事实解释。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列聚类可解释性方法（基于特征归因或元数据）无法识别跨越聚类边界的转变。反事实解释（CEs）虽然能识别改变预测所需的最小时间扰动，但主要局限于监督设置。需要统一框架来桥接无监督时间序列聚类的局部和全局反事实可解释性。

Method: GALACTIC框架：1）局部层面：通过聚类感知优化目标生成扰动，尊重目标和底层聚类分配；2）全局层面：制定代表性CE选择问题，提出最小描述长度（MDL）目标提取非冗余的全局解释摘要，表征聚类间转变。证明MDL目标是超模的，可转化为单调子模集函数，实现高效贪心选择算法。

Result: 在UCR Archive上的广泛实验评估表明，GALACTIC相比为问题调整的最先进基线，能产生显著更稀疏的局部CEs和更简洁的全局摘要，首次通过反事实提供解释聚类时间序列的统一方法。

Conclusion: GALACTIC是首个统一框架，用于无监督时间序列聚类的局部和全局反事实可解释性，通过理论保证的高效算法生成稀疏解释和简洁摘要，填补了该领域的研究空白。
...
TL;DR: FairFinGAN：基于WGAN的金融数据生成框架，通过分类器引入公平性约束，在生成合成数据时减少对受保护属性的偏见，同时保持数据实用性。


<details>
  <summary>Details</summary>
Motivation: 金融数据集常存在偏见，可能导致自动化系统做出不公平决策。需要一种既能生成合成金融数据，又能减轻对受保护属性偏见的方法。
...
<details>
  <summary>Details</summary>
Motivation: 等变图神经网络在分子模拟中至关重要，但高计算成本和内存瓶颈限制了其应用。传统低比特量化会破坏旋转敏感特征的SO(3)等变结构，导致误差和守恒定律违反。

Method: 提出几何感知量化框架GAQ：1) 幅度-方向解耦量化方案，分离不变长度和等变方向；2) 对称感知训练策略，对标量和向量特征采用不同量化调度；3) 鲁棒的注意力归一化机制稳定低比特梯度。

Result: 在rMD17基准测试中，W4A8模型达到FP32基线精度(9.31 meV vs 23.20 meV)，局部等变误差比朴素量化降低30倍以上。在消费级硬件上实现2.39倍推理加速和4倍内存减少。

Conclusion: GAQ框架成功压缩等变模型同时严格保持连续对称性，实现了稳定、能量守恒的分子动力学模拟，为纳米级时间尺度模拟提供了实用解决方案。
...
TL;DR: 提出基于信息流指导的KV缓存选择性重计算方法，通过注意力范数信号识别关键token，优化长上下文RAG推理效率


<details>
  <summary>Details</summary>
Motivation: 长上下文问答中的检索增强生成（RAG）在推理时需要对大量检索到的上下文进行预填充，这成为性能瓶颈。现有方法依赖启发式或表示差异，未能有效建模所选token是否真正影响生成过程。

Method: 将选择性KV重计算视为信息流问题，使用查询的注意力范数信号识别语义相关且结构位置有利于信息传播的token。在推理一致的RoPE几何下计算，重构检索块的位置分配，并引入信息流指导的块重排序策略。

Result: 在LLM和VLM基准测试中，在可比效率预算下相比先前方法取得了一致的性能提升。

Conclusion: 基于信息流指导的KV缓存选择性重计算方法能有效识别关键token，在保持效率的同时提升长上下文RAG的性能。
...
TL;DR: TS-BOSS：基于分数的时间序列因果发现方法，扩展了静态BOSS算法，利用动态贝叶斯网络和grow-shrink树缓存技术，在高自相关场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 观测数据中的因果结构学习对科学和政策领域至关重要，但时间序列数据中的时间依赖性带来了特殊挑战。需要将静态因果发现方法扩展到动态时间序列设置。

Method: TS-BOSS是BOSS算法的时间序列扩展，通过排列搜索动态贝叶斯网络结构，利用grow-shrink树缓存中间分数计算，保持静态设置中的可扩展性和性能。

Result: 在合成数据实验中，TS-BOSS在高自相关机制下特别有效，在可比精度下始终获得更高的邻接召回率。理论保证在适当假设下建立了TS-BOSS的可靠性。

Conclusion: TS-BOSS为时间序列因果发现提供了高性能、可扩展的方法，为将基于稀疏性的排列驱动因果学习理论扩展到动态设置提供了原则性桥梁。
...
TL;DR: 提出对抗性框架降低主体间变异性，提升可穿戴传感器人体活动识别的泛化能力


<details>
  <summary>Details</summary>
Motivation: 人体活动识别面临主体间变异性挑战，即不同个体执行相同活动的方式不同，导致模型对新个体的泛化能力受限

Method: 提出新颖的深度对抗框架，将主体间变异性概念整合到对抗任务中，鼓励学习主体不变的特征表示

Result: 在三个标准HAR数据集上使用留一主体交叉验证，性能优于先前方法；对抗任务有效降低了特征空间中的主体间变异性

Conclusion: 所提出的对抗框架能有效处理主体间变异性问题，提升HAR模型的泛化性能，且优于先前工作中的对抗任务设计
...
TL;DR: TopKGraphs是一种基于起始节点锚定随机游走的节点相似性估计方法，通过Jaccard相似性偏置游走，使用鲁棒排名聚合构建可解释的节点亲和矩阵，在多种网络场景中表现优异。
...
Method: TopKGraphs基于起始节点锚定随机游走，通过Jaccard相似性偏置游走向结构相似邻域的节点转移。不计算稳态分布，而是将游走视为随机邻域采样器，生成部分节点排名，然后使用鲁棒排名聚合构建可解释的节点到节点亲和矩阵。

Result: 在合成图（随机块模型、LFR基准图）、表格数据的k近邻图和蛋白质相互作用网络等多种场景中，TopKGraphs相比标准相似性度量（Jaccard、Dice）、基于扩散的方法（个性化PageRank）和嵌入方法（Node2Vec）都取得了竞争性或更优的性能，在稀疏、噪声或异构网络中表现出鲁棒性。

Conclusion: TopKGraphs是一种多功能且可解释的工具，能够桥接简单的局部相似性度量和更复杂的嵌入方法，促进数据挖掘和网络分析应用，为节点相似性估计提供了非参数、可解释且通用的解决方案。
...
TL;DR: 论文质疑Sheaf Neural Networks (SNNs)中学习限制映射的必要性，提出Identity Sheaf Network基线（所有限制映射固定为恒等映射），在五个异配图基准上表现与SNN变体相当，并通过瑞利商数分析表明恒等映射网络并未出现更严重的过平滑问题。


<details>
  <summary>Details</summary>
Motivation: 重新审视Sheaf Neural Networks的设计动机：虽然SNNs通过引入可学习的限制映射来解决异配图上的过平滑问题，但过平滑也可以通过残差连接和归一化等技术缓解。作者质疑学习复杂限制映射的必要性，并探究是否简单的恒等映射就足够。

Method: 1) 提出Identity Sheaf Network基线，将所有限制映射固定为恒等映射；2) 在五个流行的异配图基准数据集上进行实验，与多种SNN变体进行对比；3) 引入瑞利商数作为归一化度量，用于比较不同模型的过平滑程度。

Result: 1) Identity Sheaf Network在五个异配图基准上取得了与各种SNN变体相当的性能；2) 通过瑞利商数分析发现，训练后的网络中，基于扩散分析的SNNs理论预测行为并未在实证中体现；3) Identity Sheaf Networks并未比SNN对应模型遭受更严重的过平滑问题。

Conclusion: 学习复杂的限制映射可能不是SNNs性能提升的关键因素，简单的恒等映射就能达到相似效果。这挑战了SNNs设计中学习限制映射的必要性，并表明过平滑问题可能通过更简单的机制就能有效缓解。
...
TL;DR: 提出一种针对医疗表格数据的可解释原型学习模型，通过可训练的特征分块学习二进制或离散化的原型特征子集，实现与临床语言对齐的案例推理


<details>
  <summary>Details</summary>
Motivation: 在医疗等关键领域，模型预测的可解释性与准确性同等重要。需要开发专门针对医疗记录表格数据的可解释模型，以弥合预测性能与可解释性之间的差距

Method: 受计算机视觉中原型部分模型的启发，提出针对表格数据的新模型。使用可训练的特征分块方法，从结构化数据中学习有意义的原型部分，这些部分表示为二进制或离散化的特征子集。通过将患者描述与学习到的原型在网络的潜在空间中进行比较，实现基于概念的可解释预测

Result: 在医疗基准数据集上的实验表明，该模型在分类性能上与广泛使用的基线模型相当，同时提供了透明度，弥合了临床决策支持中预测性能与可解释性之间的差距

Conclusion: 提出的神经网络本质上具有可解释性，能够以人类可读的术语表达原型，实现与临床语言和案例推理的对齐，为医疗领域的可信AI决策提供了有效解决方案
...
TL;DR: OPSDC是一种通过自我蒸馏让推理模型变得更简洁的方法，仅通过"be concise"指令和反向KL散度就能实现自动压缩，在保持困难问题推理深度的同时大幅减少token使用


<details>
  <summary>Details</summary>
Motivation: 现有推理模型生成大量冗余甚至有害的推理内容，这些不必要的token不仅浪费计算资源，还可能累积错误。需要一种方法让模型学会更简洁地推理，同时保持对困难问题的深度思考能力。

Method: OPSDC（On-Policy Self-Distillation for Reasoning Compression）方法：1）使用"be concise"指令让模型生成简洁的教师logits；2）在学生自己的推理rollouts上最小化每个token的反向KL散度；3）无需真实答案、token预算或难度估计器，仅通过自我蒸馏实现压缩。

Result: 在Qwen3-8B和Qwen3-14B上，MATH-500数据集上实现57-59%的token压缩，同时准确率提升9-16个绝对百分点。在AIME 2024上，14B模型准确率提升10个百分点，压缩率达到41%。

Conclusion: OPSDC证明了自我蒸馏在推理压缩中的有效性，能够自动区分简单和困难问题，对前者进行激进压缩，对后者保留必要的推理深度。该方法揭示了推理模型中大量生成内容不仅是冗余的，而且可能是有害的。
...
TL;DR: LWAIL是一种新颖的对抗模仿学习框架，通过动态感知的潜在空间进行状态分布匹配，仅需少量状态专家轨迹即可达到专家级性能。


<details>
  <summary>Details</summary>
Motivation: 传统模仿学习方法需要大量中等至高质的专家演示及其动作信息，但这些数据往往难以获取。需要开发能够减少数据需求的方法。

Method: 提出LWAIL框架：1）通过预训练阶段训练意图条件值函数（ICVF），使用少量随机生成的状态数据构建动态感知的潜在空间；2）在该潜在空间中计算Wasserstein距离进行状态分布匹配；3）仅需一个或几个状态专家轨迹即可学习。

Result: 在多个MuJoCo环境中，LWAIL超越了先前基于Wasserstein的IL方法和对抗IL方法，在各种任务上取得了更好的结果。

Conclusion: LWAIL通过动态感知潜在空间中的Wasserstein距离匹配，显著减少了对专家演示数据的依赖，仅需少量状态轨迹即可实现专家级性能，为数据稀缺场景下的模仿学习提供了有效解决方案。
...
TL;DR: 提出Kraus结构输出层，将通用序列模型的隐藏表示转换为完全正定保迹量子操作，确保量子态估计的物理有效性


<details>
  <summary>Details</summary>
Motivation: 传统随机主方程求解器需要精确模型参数且对参数失配敏感，而神经网络序列模型可能违反量子态的物理约束（如正定性、迹约束），导致不稳定预测和非物理估计

Method: 设计Kraus结构输出层，将RNN、GRU、LSTM、TCN、ESN、Mamba等序列模型的隐藏表示转换为完全正定保迹量子操作，确保状态更新的物理有效性；并与Neural ODE基线比较

Result: 在所有模型中，Kraus-LSTM表现最佳，在非平稳状态下将状态估计质量提升7%，同时保证预测的物理有效性；不同门控机制、线性递归和全局注意力之间存在权衡

Conclusion: Kraus结构输出层能够将通用序列模型转换为物理有效的量子态估计器，在参数漂移的非平稳状态下显著提升估计质量并保证物理约束
...
TL;DR: 研究中文开源大语言模型的政治审查行为，评估诚实诱导和谎言检测技术，发现现有方法无法完全消除错误回答


<details>
  <summary>Details</summary>
Motivation: 现有研究主要评估在人工构造的欺骗场景下的诚实诱导和谎言检测技术，但这些人工场景可能与自然发生的欺骗行为不同。本研究希望使用真实存在审查行为的开源中文LLM作为测试平台，评估这些技术的实际效果

Method: 使用中国开发者开发的开源大语言模型（如Qwen3系列），这些模型经过训练会对政治敏感话题进行审查。在这些模型上评估多种诚实诱导技术（如去除聊天模板、少样本提示、通用诚实数据微调）和谎言检测技术（如让审查模型自我分类、基于无关数据训练的线性探测器）

Result: 对于诚实诱导，去除聊天模板、少样本提示和通用诚实数据微调最可靠地增加了真实回答。对于谎言检测，让审查模型自我分类其回答的性能接近未审查模型的上限，基于无关数据训练的线性探测器提供了更经济的替代方案。最强的诚实诱导技术也能迁移到前沿开源模型如DeepSeek R1。但没有任何技术能完全消除错误回答

Conclusion: 使用真实存在审查行为的开源中文LLM作为测试平台，为诚实诱导和谎言检测研究提供了更真实的评估环境。虽然发现了一些有效的技术，但没有任何方法能完全解决大语言模型产生虚假回答的问题，这仍然是需要进一步研究的重要挑战
...
TL;DR: 提出三阶段框架：先用廉价不完美标签进行监督预训练，再通过自监督学习精炼模型，以解决优化和仿真问题的机器学习代理模型训练难题。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习代理方法（监督/自监督学习、软/硬可行性约束）存在依赖昂贵高质量标签或面临困难优化景观等固有挑战，需要平衡这些方法的优缺点。

Method: 三阶段框架：1) 收集廉价不完美标签；2) 监督预训练；3) 自监督学习精炼。理论分析显示标签只需将模型置于吸引盆地，仅需少量不精确标签和训练轮次。

Result: 在非凸约束优化、电网运行和刚性动力系统等挑战性领域验证，显示该策略带来更快收敛、改进的准确性、可行性和最优性，离线总成本降低高达59倍。

Conclusion: 提出的三阶段框架有效解决了现有代理模型方法的局限性，通过结合廉价标签和监督预训练与自监督精炼，显著提升了优化和仿真问题的可扩展解决方案。
...
TL;DR: POET-X：一种可扩展且内存高效的POET变体，通过正交等价变换优化LLM训练，显著降低计算成本和内存消耗，使单张H100 GPU能训练十亿参数模型


<details>
  <summary>Details</summary>
Motivation: 虽然POET框架提供了强大的训练稳定性，但其原始实现由于密集的矩阵乘法导致高内存消耗和计算开销，限制了大规模语言模型的实际应用

Method: 提出POET-X，一种可扩展且内存高效的POET变体，通过正交等价变换优化权重矩阵，显著减少计算成本，同时保持POET的泛化和稳定性优势

Result: POET-X使单张Nvidia H100 GPU能够预训练十亿参数规模的LLM，而标准优化器（如AdamW）在相同设置下会内存不足；在吞吐量和内存效率方面有显著提升

Conclusion: POET-X成功解决了POET框架的计算和内存瓶颈，为大规模语言模型的高效稳定训练提供了实用解决方案，在保持训练稳定性的同时大幅提升了可扩展性
...
TL;DR: 提出基于字典模式熵（DPE）的新框架，从时序观测数据中发现符号序列的因果方向，通过最小化模式级不确定性实现鲁棒、可解释的因果推断。


<details>
  <summary>Details</summary>
Motivation: 符号序列的因果方向发现具有挑战性，因为缺乏函数模型和噪声假设。需要一种能够同时推断因果方向和驱动变化的特定子模式的方法。

Method: 提出DPE框架，整合算法信息论和香农信息论。将因果关系解释为候选原因中紧凑、基于规则的模式的涌现，这些模式系统地约束了效应。构建方向特定的字典，使用基于熵的度量量化其影响，建立确定性模式结构与随机变异性之间的原则性联系。

Result: DPE在多种合成系统（延迟位翻转扰动、AR(1)耦合、1D斜帐篷映射、稀疏过程）中始终实现可靠性能，优于或匹配竞争性AIT方法（ETC_E、ETC_P、LZ_P）。在生物和生态数据集中表现有竞争力，而替代方法在特定基因组设置中显示优势。

Conclusion: 最小化模式级不确定性产生了一个鲁棒、可解释且广泛适用的因果发现框架。DPE能够同时推断因果方向和驱动变化的特定子模式，为符号序列的因果推断提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: Collatz总停止时间τ(n)在n≤10^7范围内表现出偏斜、过度离散和算术异质性，需要从概率机器学习角度进行建模分析
...
Result: 在测试数据上，NB2-GLM比奇块生成模型获得显著更高的预测似然；将块长度分布条件化于m mod 8后，生成模型的分布拟合明显改善，表明低阶模结构是τ(n)异质性的关键驱动因素

Conclusion: 低阶模结构（特别是模8余数）是Collatz总停止时间异质性的核心因素，贝叶斯回归模型在预测性能上优于机制生成模型，但条件化块长度分布能显著改善生成模型的拟合效果
...
TL;DR: 提出Volterra签名作为历史依赖系统的显式特征表示，通过时间核加权路径并利用Volterra-Chen恒等式，获得学习理论保证和计算优势，在动态学习任务中优于经典路径签名基线。


<details>
  <summary>Details</summary>
Motivation: 现有非马尔可夫时间序列学习方法（如RNN、神经控制微分方程、Transformer）依赖隐式记忆机制，难以解释且长时训练困难，需要更原则性的显式特征表示。

Method: 提出Volterra签名VSig(x;K)，通过时间核K加权输入路径x并展开到张量代数，利用Volterra-Chen恒等式建立理论保证，通过核技巧实现高效计算。

Result: 证明了注入性（增强下的可识别性）和无限维路径空间上的通用逼近定理；对指数型核，Volterra签名可解为张量代数中的线性状态空间ODE；在真实和合成数据上优于经典路径签名基线。

Conclusion: Volterra签名作为历史依赖系统的鲁棒、计算可处理的特征映射，具有时间重参数化不变性，为动态学习任务提供了理论保证和实际优势。
...
TL;DR: 本文提出了一种利用预测信息增强独立性检验的方法，在保持最坏情况鲁棒性的同时，当预测准确时显著提高样本效率。


<details>
  <summary>Details</summary>
Motivation: 传统的非参数独立性检验在有限样本情况下样本复杂度很高，随着支持集大小呈多项式增长。本文旨在利用预测信息来改善这一状况，即使预测可能不可靠，也能保持检验的鲁棒性。

Method: 采用增强分布检验框架，设计了包含辅助预测信息的独立性检验器。包括：(1) 基于预测误差自适应降低样本复杂度的二元离散分布独立性检验器；(2) 扩展到高维多变量设置，检验d个随机变量的独立性；(3) 匹配极小极大下界证明最优性。

Result: 提出的检验器在保持最坏情况有效性的同时，当预测准确时能显著提高样本效率。理论分析表明这些检验器达到了最优的样本复杂度。

Conclusion: 通过整合预测信息，本文成功开发了既鲁棒又高效的独立性检验方法，在预测准确时大幅降低样本需求，同时保持最坏情况下的统计保证。
...
TL;DR: 卷积网络通过局部连接和权重共享改变隐式正则化，在球面数据上实现泛化，而全连接网络在此高维场景下会过拟合。
...
Method: 理论分析局部连接和权重共享网络在球面数据上的泛化性能，证明当感受野大小m远小于环境维度d时，这些网络能以n^{-1/6+O(m/d)}的速率泛化。同时分析自然图像的patch几何，验证标准卷积设计产生的patch分布适合这种稳定性机制。

Result: 局部连接和权重共享网络在球面数据上实现泛化，而全连接网络在该场景下必然失败。权重共享将学习到的滤波器与低维patch流形耦合，绕过环境空间的高维度。自然图像的patch分布高度适合这种稳定性机制。

Conclusion: 卷积网络通过局部连接和权重共享改变隐式正则化，使其能够泛化到高维球面数据，这为卷积网络优于全连接基线的泛化能力提供了系统性解释。
...
TL;DR: 本文研究了过参数化ReLU模型在高维随机特征数据上梯度下降的隐式偏差，发现其近似于最小L2范数解，填补了最坏情况无偏差与正交数据下精确最小范数解之间的空白。


<details>
  <summary>Details</summary>
Motivation: 过参数化机器学习模型（如神经网络）的训练目标通常存在多个全局最小值。梯度下降等优化算法会收敛到特定的全局最小值，这种选择被称为"隐式偏差"。先前研究表明，在最坏情况下ReLU模型不存在隐式偏差，而在正交数据下梯度下降恰好收敛到最小L2范数解。本文旨在填补这两个极端情况之间的空白，研究高维随机数据下的隐式偏差特性。

Method: 采用原始-对偶分析框架，仔细跟踪预测值、数据张成系数及其相互作用的演化过程。特别关注ReLU激活模式的快速稳定特性，并利用高维随机数据的概率性质进行分析。

Result: 对于足够高维的随机数据，梯度下降的隐式偏差以高概率近似于最小L2范数解，两者之间的差距为Θ(√(n/d))，其中n是训练样本数，d是特征维度。这表明在高维情况下，梯度下降倾向于选择接近最小范数的解。

Conclusion: 本文在高维随机数据设置下建立了梯度下降隐式偏差的理论特性，证明了其近似最小L2范数解的性质，填补了现有理论研究的空白。原始-对偶分析方法为理解ReLU模型优化动态提供了新视角。
...
<details>
  <summary>Details</summary>
Motivation: 个性化决策规则在医疗、营销等领域的应用日益广泛，但基于有偏数据训练的算法可能对性别、种族等敏感属性定义的少数群体造成歧视性伤害，存在伦理问题

Method: 提出将人口统计平等和条件人口统计平等约束融入最优个性化决策规则估计的新框架，通过向无约束最优解施加扰动来获得理论最优解，实现计算高效

Result: 理论上推导了策略价值和公平约束项的收敛率，通过综合模拟研究和俄勒冈健康保险实验的实证应用验证了方法的有效性

Conclusion: 提出的框架能有效解决个性化决策规则中的公平性问题，在保持计算效率的同时确保算法决策对不同人口群体的公平性
...
<details>
  <summary>Details</summary>
Motivation: 在医疗和政策评估等领域，识别具有相似特征的病人亚组对于个性化决策至关重要。现有方法多依赖无监督聚类，但需要能识别特定结果背景下可操作亚组的监督聚类方法

Method: 提出贝叶斯监督因果聚类方法，以处理效应作为结果指导聚类过程，识别协变量特征和处理效应均相似的个体亚组

Result: 在模拟数据集和真实世界数据集（第三次国际卒中试验）上评估了该框架的实际有用性
...
TL;DR: 提出使用非对称Shapley值作为临床预测中特征重要性评估的更好方法，特别适用于基因组学与临床变量混合的预测模型


<details>
  <summary>Details</summary>
Motivation: 传统评估方法（通过添加特征后预测性能的变化来评估重要性）存在问题，因为它没有考虑变量间的共线性，也没有考虑已知的依赖方向性。在临床预测中，特别是当疾病状态作为基因组效应的中介变量时，需要更合适的方法

Method: 提出使用非对称Shapley值来量化混合维度预测模型中的特征重要性。针对疾病状态作为基因组效应中介变量的临床预测场景，推导了计算局部和全局非对称Shapley值的高效算法

Result: 局部非对称Shapley值被证明对推理非常有用，而全局非对称Shapley值通过将任何预测性能指标分解为特征的贡献来提供解释。以结直肠癌患者无进展生存期预测为例进行了说明

Conclusion: 非对称Shapley值是评估临床预测中特征重要性的更合适替代方法，特别是在处理基因组学与临床变量混合的预测模型时，能够更好地处理共线性和变量间的方向性依赖关系
...
TL;DR: 论文提出了四种不同的可容许性几何结构，并证明它们是两两不嵌套的，可容许性本质上是准则相关的。


<details>
  <summary>Details</summary>
Motivation: 研究序列和无分布推断中的不同可容许性准则，揭示它们在几何结构上的根本差异，证明不存在统一的"最优"概念。

Method: 通过几何分析比较四种可容许性准则：Blackwell风险支配、任意时间有效可容许性、边际覆盖有效性、Cesàro可接近性可容许性。证明分离定理，分析各自的最优性证书。

Result: 四种可容许性类别是两两不嵌套的，每种都有不同的最优性证书。鞅相干性对Blackwell可容许性是必要的，对任意时间有效可容许性是充要的，但对覆盖有效性和CAA可容许性既非必要也非充分。

Conclusion: 可容许性是准则相关的，不存在统一的"最优"概念。四种准则虽然共享优化模板，但约束集在不同空间、偏序和性能度量上操作，导致几何不兼容。
...
TL;DR: 本文为经验风险最小化（ERM）决策树建立了全面的统计理论，包括高维回归和分类中的尖锐oracle不等式、最小化最优速率，并引入新的函数类PSHAB空间来捕捉稀疏性、各向异性光滑性和空间异质性等实际特征。


<details>
  <summary>Details</summary>
Motivation: 尽管全局最优的ERM决策树在计算上已变得可行且实证上成功，但其统计性能的严格理论保证仍然有限。本文旨在为ERM决策树建立全面的统计理论，为其最优性提供原则性基础。

Method: 使用基于经验局部化Rademacher复杂度的新型均匀集中框架，建立尖锐的oracle不等式。引入新的函数类：分段稀疏异质各向异性Besov（PSHAB）空间，并推导在该空间上的最小化最优速率。同时提供在重尾噪声设置下的鲁棒保证。

Result: 建立了ERM决策树的尖锐oracle不等式，界定了相对于最多L个叶子的最佳可能树的超额风险，刻画了可解释性-准确性权衡。推导了在PSHAB空间上的最小化最优速率，该空间明确捕捉了稀疏性、各向异性光滑性和空间异质性三个关键结构特征。

Conclusion: 本文为ERM决策树的最优性提供了原则性基础，并引入了经验过程工具，这些工具广泛适用于其他高度自适应的数据驱动过程。理论结果在亚高斯性假设下建立，同时也提供了在重尾噪声设置下的鲁棒保证。
...
TL;DR: 该论文从统计学角度综述了合成数据的生成与使用现状，分析了生成模型在支持下游任务时的假设条件、优势与局限，并提出了合成数据的规范使用框架和实践建议。
...
Method: 论文采用综述研究方法：1) 调查现代生成模型的主要类别、预期用例和优势；2) 分析合成数据作为真实观测替代品时的常见陷阱；3) 基于这些见解讨论合成数据的规范使用框架。

Result: 论文系统性地识别了合成数据使用中的关键问题，包括模型误设导致的偏差、不确定性衰减、泛化困难等特征性失效模式，并提出了应对这些挑战的见解和框架。

Conclusion: 论文为方法开发者和应用研究者提供了实用建议、开放问题和注意事项，旨在指导合成数据的规范使用，促进该领域在科学、工业和政策领域的健康发展。
...
TL;DR: 该论文提出了一种热力学响应理论框架，将奇异学习理论中的复杂概念（如RLCT、奇异波动）与WAIC、WBIC等实用准则统一起来，通过后验调温技术揭示奇异统计模型中的相变行为。


<details>
  <summary>Details</summary>
Motivation: 奇异统计模型（如混合模型、矩阵分解、神经网络）由于参数不可识别性和Fisher几何退化，违反了常规渐近理论。虽然奇异学习理论通过实对数典型阈值（RLCT）和奇异波动等不变量描述了边际似然行为，但这些量难以操作解释。同时，WAIC和WBIC等广泛使用的准则似乎与底层奇异几何脱节。

Method: 提出后验调温技术，诱导后验分布的单参数变形，其相关可观测量生成热力学响应函数层次结构。建立通用协方差恒等式，将调温期望的导数与后验波动联系起来。形式化可观测量代数，通过商掉不可识别方向来构建结构上有意义的序参数。

Result: 在对称高斯混合、降秩回归和过参数化神经网络等典型奇异模型中，实证展示了调温下的相变行为：序参数塌缩、敏感性峰值、复杂度度量与后验几何结构重组对齐。WAIC、WBIC和奇异波动在统一响应框架中获得解释。

Conclusion: 热力学响应理论为解释奇异贝叶斯学习中的复杂度、预测变异性和结构重组提供了自然的组织框架，将奇异学习理论的经典量（RLCT、奇异波动）与实用准则（WAIC、WBIC）统一起来。
