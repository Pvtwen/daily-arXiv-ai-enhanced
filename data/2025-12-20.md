...
TL;DR: 提出LRM-MSL框架，通过构建局部无线电地图、二值化处理、连通分量分析实现多源分离，再使用数值坐标回归网络进行单源定位，解决城市环境中任意数量信号源定位问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法有效利用RSS信息或缺乏对任意数量信号源的通用性，需要一种准确高效的多源定位方法，特别是在认知无线电等应用场景中。

Method: 1) 构建仅保留信号源周围RSS信息的局部无线电地图并二值化；2) 应用连通分量分析实现多源分离，将MSL问题转化为多个SSL任务；3) 设计数值坐标回归网络执行SSL任务。

Result: 构建了VaryTxLoc数据集进行评估，实验结果表明LRM-MSL方法准确有效，优于现有最先进方法。

Conclusion: LRM-MSL是一个通用框架，能够处理任意数量的信号源，在城市环境多源定位问题中表现出色，代码和数据集已开源。
...
TL;DR: TinyMyo：基于Transformer编码器的轻量级表面肌电信号基础模型，仅3.6M参数，通过自监督预训练实现多任务泛化，并在超低功耗微控制器上部署成功。
...
Method: 提出TinyMyo轻量级基础模型，基于Transformer编码器架构，使用公开数据集进行自监督预训练。通过最小化的任务特定头部适配，同一骨干网络可处理多种下游任务。

Result: 模型在多个数据集上达到或超越SOTA性能：NinaPro DB5 (89.4±0.16%)、UCI-EMG (97.56±0.32%)、EPN-612 (96.74±0.09%)。首次在超低功耗微控制器(GAP9)上部署肌电信号基础模型，平均功耗36.45mW。

Conclusion: TinyMyo为肌电信号社区提供了灵活的基础资源，支持手势分类、运动学回归、语音识别等多任务，实现了模型轻量化和嵌入式部署，有望加速未来研究。
...
TL;DR: 研究人员开发了一种数字定制芯片，可产生稳定的6Hz振荡来干扰癫痫发作的异常神经同步，为低功耗可穿戴癫痫干预设备提供概念验证。


<details>
  <summary>Details</summary>
Motivation: 全球有超过5000万人受癫痫影响，尽管药物治疗有所进展，但仍有大量患者癫痫发作未得到控制。现有神经调节策略通常依赖复杂的模拟电子设备或高功率刺激硬件，需要更简单、低功耗的替代方案。

Method: 使用公开可用的EEG癫痫数据集，提取并平均模拟癫痫发作波形，将其数字化以模拟神经前端，然后与定制设计的数字芯片直接接口。芯片脉冲序列被重采样和低通重构以产生模拟6Hz波形，通过频域和时域分析比较癫痫形态、数字化表示和干扰输出。

Result: 芯片施加的窄带6Hz节律能够覆盖癫痫活动的宽带频谱特征，证明数字定制芯片能够有效干扰癫痫发作的异常神经同步。

Conclusion: 这项研究为低功耗数字定制干扰技术提供了概念验证，为开发简化的可穿戴癫痫中断设备开辟了潜在途径，有望用于精准医疗和未来医疗设备。
...
TL;DR: 提出了一种名为"concurrence"的统计依赖度量方法，通过判断对齐与错位时间序列片段能否被分类器区分来检测信号间的依赖关系，适用于小数据集和复杂非线性交互。


<details>
  <summary>Details</summary>
Motivation: 生物系统通常表现出复杂的非线性交互，现有方法需要先验知识或大数据集才能捕捉这些关系，需要一种能在小数据集下检测复杂依赖关系的通用方法。

Method: 提出"concurrence"准则：如果能够构建一个分类器来区分时间序列的对齐片段和错位片段，则认为两个时间序列存在依赖关系。该方法不需要特定参数调整或大数据。

Result: 该方法在理论上与依赖关系相关联，能够揭示多种信号（fMRI、生理和行为数据）之间的关系，适用于跨学科的科学分析。

Conclusion: concurrence方法可以成为跨学科科学分析的标准方法，能够暴露广泛信号谱中的关系，无需特定参数调整或大量数据。
...
TL;DR: 提出RIS辅助网络中带协作干扰器的同时保密与隐蔽通信方案，通过最大化保密容量和检测错误概率来增强安全性，并设计基于生成扩散模型和深度强化学习的算法来平衡性能。


<details>
  <summary>Details</summary>
Motivation: 在无线通信中，同时实现保密性和隐蔽性具有挑战性。传统方法难以在保密容量和检测错误概率之间取得平衡，特别是在用户移动和窃听者自适应调整检测阈值的最坏情况下。

Method: 1) 提出RIS辅助网络中带协作干扰器的SSACC方案；2) 在最坏情况下推导平均最小检测错误概率和平均保密容量的闭式表达式；3) 设计基于生成扩散模型和深度强化学习的算法来优化功率分配，平衡AMDEP和ASC。

Result: 仿真结果表明，所提算法相比传统DDPG方法收敛更快、性能更优，能有效平衡安全性和容量性能，在用户移动场景下实现高AMDEP和ASC。

Conclusion: 该方案成功解决了RIS辅助网络中同时保密与隐蔽通信的平衡问题，提出的GDM-DRL算法在动态环境下表现出优越性能，为未来安全无线通信系统设计提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在AI原生无线接入网络(AI-RAN)中，设备-边缘协同推理使用推测解码技术，但现有分布式方案在每个步骤传输完整词汇表的对数，导致显著的上行链路开销。

Method: 提出截断稀疏对数传输(TSLT)策略，采用"先稀疏化再采样"方法，只传输截断候选集的稀疏对数及其索引，而非完整词汇表对数。该方法还扩展到多候选情况以提高接受概率。

Result: 实验表明TSLT显著减少了上行链路通信，同时保持了端到端推理延迟和模型质量。理论分析证明在TSLT下接受率得以保留。
...
<details>
  <summary>Details</summary>
Motivation: 现有信道预测方法存在复杂度高、无法准确建模信道时变特性的问题，特别是在高速移动场景下，信道预测对于抵抗信道老化和保证通信质量至关重要

Method: 提出CPMamba框架：使用专门设计的特征提取和嵌入网络从历史CSI中提取特征，采用堆叠的残差Mamba模块进行时序建模，通过输入依赖的选择性机制动态调整状态转移

Result: 在3GPP标准信道模型下的仿真结果表明，CPMamba在所有场景中都达到了最先进的预测精度，具有优异的泛化能力和鲁棒性，相比基线模型参数量减少约50%

Conclusion: CPMamba通过选择性状态空间模型有效捕获CSI间的长程依赖关系，在保持线性计算复杂度的同时实现了高效准确的信道预测，显著降低了实际部署门槛
...
TL;DR: 提出一种专门用于线性光谱解混的主动集算法，支持非负性和最小丰度约束，相比通用求解器具有更好的计算性能


<details>
  <summary>Details</summary>
Motivation: 线性光谱解混在非负性和和为1约束下是凸优化问题，现有算法众多。实践中，特别是在监督解混（即使用大字典）时，由于丰度的非负性，解往往稀疏，这促使使用主动集求解器。考虑到问题的特定特征，设计专用算法相比通用求解器可以获得更好的计算性能

Method: 提出一种专用的主动集算法，将非负性约束扩展到更广泛的"最小丰度约束"，为线性光谱解混问题设计专门的求解器

Result: 论文提出了一种针对线性光谱解混的专用算法，能够处理最小丰度约束，相比通用求解器具有计算性能优势

Conclusion: 通过设计专门针对线性光谱解混问题的算法，特别是扩展约束类型并利用主动集方法，可以获得比通用求解器更好的计算性能，这对于处理大字典的监督解混问题尤其重要
...
TL;DR: 提出一种用于6G高移动场景的OFDM接收机架构，采用时延-多普勒叠加导频方案，考虑ICI、分数时延和多普勒频移，实现高速移动下的鲁棒通信。


<details>
  <summary>Details</summary>
Motivation: 6G高移动场景（速度高达1000 km/h）对OFDM系统提出挑战，现有时延-多普勒叠加导频研究未充分考虑ICI、分数时延和多普勒频移的影响。

Method: 1. 使用时延-多普勒叠加导频方案，在时延-多普勒域添加单个导频进行信道估计；2. 推导分离的分数时延-多普勒估计算法；3. 提出基于Landweber迭代的低复杂度均衡方法，利用信道固有结构。

Result: 仿真结果表明，该接收机架构在各种移动条件下（速度高达1000 km/h）实现鲁棒通信性能，相比现有方法提高了有效吞吐量。

Conclusion: 提出的接收机架构有效解决了6G高移动场景下的OFDM通信挑战，通过创新的导频设计和均衡算法实现了高速移动下的可靠通信和吞吐量提升。
...
TL;DR: 提出一种结合Woodbury公式和自适应随机奇异值分解的低复杂度预编码框架，用于LEO卫星大规模天线阵列，相比传统RZF预编码计算复杂度降低61%，仅带来轻微的和速率性能损失。


<details>
  <summary>Details</summary>
Motivation: 随着大规模有源天线阵列在低地球轨道卫星中的部署增加，需要计算高效且自适应的预编码技术来应对动态信道变化并提高频谱效率。传统正则化迫零预编码虽然广泛使用，但其实时实现受到Gram矩阵求逆计算复杂度的限制。

Method: 开发了一个低复杂度框架，将Woodbury公式与自适应随机奇异值分解相结合。Woodbury公式利用低秩扰动降低求逆复杂度，而自适应随机奇异值分解动态提取主导奇异分量，进一步提高计算效率。

Result: 蒙特卡洛仿真表明，所提方法相比传统采用全矩阵求逆的RZF预编码，计算复杂度降低高达61%，而和速率性能仅有适度下降。

Conclusion: WB-arSVD方法为下一代卫星通信提供了可扩展且高效的解决方案，有利于在功率受限环境中实现实时部署。
...
TL;DR: 提出自适应RPCA预处理方法，提升CSI-PLA认证精度，显著降低错误概率并提高检测概率


<details>
  <summary>Details</summary>
Motivation: 解决CSI-PLA中信道状态信息在时域上的变化和不一致问题，提升物理层认证的准确性
...
Conclusion: 提出的自适应RPCA预处理方法能有效处理CSI变化，显著提升物理层认证性能，在合成和真实数据集上均表现出优越性能
...
TL;DR: 提出一个基于误设定Cramér-Rao界(MCRB)的框架，用于分析主动攻击对基于到达角(AoA)的物理层认证的影响，推导出闭式表达式并揭示攻击引入的惩罚项特性。
...
Method: 使用误设定Cramér-Rao界(MCRB)作为分析工具，考虑单天线用户和M天线ULA验证器，假设确定性导频信号，攻击者具有L天线元素，推导闭式MCRB表达式。

Result: 获得闭式MCRB表达式，发现攻击引入的惩罚项不依赖于信噪比(SNR)，而是取决于攻击者位置、阵列几何结构和攻击者预编码向量。

Conclusion: MCRB框架能有效分析主动攻击对物理层认证的影响，攻击引入的惩罚项揭示了系统脆弱性的关键因素，为安全设计提供理论指导。
...
TL;DR: 提出一种集成复数变分模态分解算法，结合时序卷积网络和空间注意力机制，用于特定发射器识别，在仅有10个符号且无需先验知识的情况下达到96%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的特定发射器识别方法通常依赖大量数据或先验信息，这在现实场景中标记数据有限的情况下存在挑战。

Method: 1. 集成复数变分模态分解算法分解和重构复数值信号以逼近原始发射信号；2. 使用时序卷积网络建模序列信号特征；3. 引入空间注意力机制自适应加权信息丰富的信号段；4. 分支网络利用其他数据的预训练权重，减少对辅助数据集的需求。

Result: 在模拟数据上的消融实验验证了模型各组成部分的有效性。在公开数据集上，仅使用10个符号且无需任何先验知识的情况下，该方法达到了96%的准确率。

Conclusion: 该方法通过集成复数变分模态分解、时序卷积网络和空间注意力机制，在有限标记数据条件下实现了高性能的特定发射器识别，为物理层安全提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法需要后验验证凸性，计算困难；固定参数的凸表达式（如二次函数）形式受限，不够灵活准确

Method: 将符号回归与DCP规则集成，强制所有候选模型表达式遵守DCP组合规则，确保输出表达式天生全局凸

Result: 能够发现比传统固定参数凸表达式更灵活、更准确的凸替代模型，同时保持可解释性和可验证性

Conclusion: 该方法为安全关键控制和优化任务提供了可解释、可验证且灵活的凸模型发现框架
...
TL;DR: 混合集成框架结合量子情感分析、决策变换器和策略模型选择，在S&P500预测中达到60.14%方向准确率，比单个模型提升3.10%


<details>
  <summary>Details</summary>
Motivation: 金融市场预测具有挑战性，大多数模型因高噪声、非平稳性和市场效率难以超过55-57%准确率。现有方法存在三个局限：架构多样性不足、情感分析效果有限、弱预测器影响集成性能

Method: 提出混合集成框架：1) 结合不同学习算法（LSTM、决策变换器、XGBoost、随机森林、逻辑回归）实现架构多样性；2) 使用4量子比特变分量子电路增强情感分析；3) 智能过滤排除准确率低于52%的弱预测器；4) 基于置信度的过滤策略（6+模型共识）

Result: 在2020-2023年市场数据上达到60.14%方向准确率，比相同架构模型组合（52.80%）显著提升。量子情感分析为每个模型带来0.8-1.5%增益。Top-7模型集成（60.14%）优于全部35个模型（51.2%）。McNemar检验证实统计显著性（p<0.05），初步回测显示夏普比率1.2 vs 买入持有0.8

Conclusion: 架构多样性比数据集多样性更重要，量子增强的情感分析提供额外收益，智能模型选择对集成性能至关重要。该框架在多样化市场环境中表现出稳健性，具有实际交易潜力
...
TL;DR: SHARe-KAN框架通过Gain-Shape-Bias向量量化解决Vision KANs的内存瓶颈，结合LUTHAM编译器实现88倍运行时内存减少，同时保持原始精度。


<details>
  <summary>Details</summary>
Motivation: Kolmogorov-Arnold Networks (KANs)面临基本的内存墙问题：其学习的基础函数导致参数数量激增，产生极高的带宽需求，阻碍了在内存受限环境中的部署。传统剪枝方法对Vision KANs无效（10%稀疏度导致mAP从85.23%降至45%）。

Method: 提出SHARe-KAN框架，采用Gain-Shape-Bias向量量化来利用函数冗余同时保持密集拓扑结构。结合LUTHAM硬件感知编译器进行静态内存规划。

Result: 实现88倍运行时内存减少（从1.13 GB降至12.91 MB），在PASCAL VOC数据集上匹配未压缩基线精度。在NVIDIA Ampere架构上分析显示>90% L2缓存驻留，工作负载与DRAM带宽约束解耦。
...
TL;DR: 该论文研究了图信号在推荐系统中的作用，证明低频和高频图信号在推荐任务中具有等效效果，并提出频率信号缩放器和空间翻转方法来优化GNN推荐性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究对低频和高频图信号在推荐系统中的作用认识不清，虽然谱图神经网络在建模图信号方面很有效，但低频和高频信号各自的作用机制尚不明确，需要深入探究。

Method: 1. 理论证明低频和高频图信号在推荐任务中的等效性；2. 提出频率信号缩放器作为即插即用模块，调整图信号滤波函数以微调用户-物品对的平滑度；3. 引入空间翻转方法恢复图嵌入的表达能力。

Result: 实验证明：1. 低频或高频图信号单独使用都能实现有效推荐；2. 提出的方法在四个公共数据集上验证了有效性；3. 频率信号缩放器能与任何GNN模型兼容。

Conclusion: 低频和高频图信号在推荐系统中具有等效作用，都通过平滑用户-物品对相似性来提升推荐性能。提出的频率信号缩放器和空间翻转方法能有效优化GNN推荐模型，且单个频率信号就足以实现良好推荐效果。
...
TL;DR: LLaDA2.0是一个基于离散扩散的大型语言模型，通过系统转换自回归模型扩展到100B参数，采用渐进式块扩散训练方案，并发布了16B和100B两个MoE变体用于实际部署。


<details>
  <summary>Details</summary>
Motivation: 传统从头训练大型扩散模型成本高昂，本文提出通过转换预训练自回归模型来继承知识，实现高效的前沿规模部署，同时保持并行解码优势。

Method: 采用3阶段块级WSD训练方案：渐进增加块大小的块扩散（预热）、大规模全序列扩散（稳定）、回归紧凑块扩散（衰减）。通过SFT和DPO进行后训练对齐，获得优化的MoE变体。

Result: 成功开发了LLaDA2.0-mini（16B）和LLaDA2.0-flash（100B）两个指令调优的MoE变体，在保持并行解码优势的同时，实现了优越的性能和效率，并已开源。

Conclusion: LLaDA2.0建立了从前沿规模自回归模型到离散扩散模型的新范式，通过知识继承和渐进适应实现了高效部署，为大规模扩散模型的实际应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 数据是大语言模型训练的关键资产，但数据估值（特别是计算Shapley值）通常需要大量模型重训练，计算成本极高，这阻碍了数据所有者做出明智的投资决策和公平的利益分配。

Method: 利用直接偏好优化（DPO）训练大语言模型的特定数学结构，开发可扩展的Shapley值计算方法，显著降低计算复杂度。

Result: 证明了对于使用DPO训练的LLMs，数据估值中的Shapley值计算可以大幅简化，从而解锁数据估值与大语言模型交叉领域的许多应用。

Conclusion: DPO的数学特性为高效数据估值提供了独特优势，这使得数据所有者能够更实际地评估数据价值并进行协作，促进了数据估值在大语言模型领域的应用。
...
TL;DR: Janus框架统一生成与预测，通过解缠潜在空间实现异构材料微结构的实时物理引导逆向设计


<details>
  <summary>Details</summary>
Motivation: 异构材料微结构的逆向设计是病态且计算昂贵的问题，现有生成模型难以支持快速稳定的确定性反演

Method: 提出Janus框架，结合深度编码器-解码器架构与可分离的KHRONOS预测头，学习同时适用于生成反演和物理预测的等距潜在流形

Result: 在MNIST上实现高保真重建和准确分类；在热导率微结构设计中达到R²=0.98的前向预测精度，逆向解满足1%相对误差

Conclusion: Janus通过统一预测和生成，实现了实时物理引导的逆向微结构生成，计算成本低于传统优化方法
...
TL;DR: 将时间演化自然梯度（TENG）框架扩展到处理狄利克雷边界条件，通过结合自然梯度优化与数值时间步进方案（欧拉和Heun方法），在热方程上验证了方法的准确性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统数值方法处理高维或复杂PDE问题困难，而物理信息神经网络（PINNs）在实现高精度和处理复杂边界条件方面面临挑战。需要开发既能保持稳定性又能保证精度的新方法。

Method: 扩展TENG框架以处理狄利克雷边界条件，将自然梯度优化与数值时间步进方案（欧拉和Heun方法）结合，在损失函数中加入边界条件惩罚项来精确强制执行狄利克雷约束。

Result: 在热方程实验中，Heun方法由于其二阶校正表现出更高的准确性，而欧拉方法在简单场景中具有计算效率优势。该方法成功实现了狄利克雷边界条件的精确执行。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态模型（如CLIP）在零样本图像分类中面临两个主要问题：1）低容量模型容易欠拟合，在细粒度分类上表现不佳；2）数据集人口统计不平衡会导致预测偏向多数群体，产生有害偏见。
...
Conclusion: D3G方法有效提升了零样本图像分类的准确性，同时减少了预训练多模态模型中的人口统计偏见，为改善模型公平性提供了新思路。
...
TL;DR: 提出一种高效查询算法，能从噪声查询访问中恢复叠加特征方向并重构函数，适用于任意叠加和一般响应函数
...
Method: 开发了一种在傅里叶空间中搜索的算法，通过迭代细化搜索空间来定位隐藏方向vi。算法从噪声查询访问中识别非退化响应的所有特征方向

Result: 算法能高效识别所有非退化响应的特征方向并重构函数f，适用于比所有相关先前结果更一般的设置，只要求vi、vj不近似相同
...
TL;DR: 本文提出了一种名为POC的后处理校正方法，利用大语言模型重新排序专家模型的预测结果，显著提升了视觉物种识别的少样本学习性能。


<details>
  <summary>Details</summary>
Motivation: 视觉物种识别需要大量标注数据，但物种级标注需要领域专业知识，通常只能获得少量标注样本。虽然大语言模型在通用识别任务上表现出色，但在高度专业化的视觉物种识别任务中表现不佳，甚至不如简单的少样本学习专家模型。

Method: 提出POC（Post-hoc Correction）方法：首先使用少样本学习训练专家模型，然后利用大语言模型重新排序专家模型的top预测结果。通过包含softmax置信度分数和少样本视觉示例的丰富提示，让大语言模型校正专家模型的错误预测。

Result: 在五个具有挑战性的视觉物种识别基准测试中，POC方法无需额外训练、验证或人工干预，将少样本学习的准确率提升了+6.4%。该方法可泛化到不同的预训练骨干网络和大语言模型，作为即插即用模块显著增强现有少样本学习方法。

Conclusion: 大语言模型虽然不能直接用于视觉物种识别，但能有效校正专家模型的预测错误。POC方法简单有效，通过结合专家模型和大语言模型的优势，显著提升了少样本学习在视觉物种识别任务中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型虽然预测性能强，但忽略了物理过程，限制了可解释性和泛化能力；而现有因果学习方法通常依赖固定的因果图，无法适应数据变化。
...
Conclusion: CauStream不仅提升了预测性能，还能学习到与领域知识一致的因果图，为理解流域动态提供可解释的洞见，为因果时空建模提供了理论基础，可扩展到广泛的科学和环境应用。
...
TL;DR: 研究发现ReLU MLP在NTK机制下，对于靠近原点的外推评估会出现二次外推现象，这与远离原点的线性外推形成对比。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注ReLU MLP在远离原点的外推行为（线性外推），但在NTK机制下，由于神经正切核的特征映射不具有平移不变性，靠近原点的外推行为是一个未被充分探索的特殊情况。

Method: 在NTK机制下分析ReLU MLP的外推行为，特别关注原点附近的外推情况，利用特征映射的旋转不变性来研究两种极端情况的外推边界。

Result: 发现对于靠近原点的外推评估，ReLU MLP表现出二次外推行为，这与远离原点的线性外推形成鲜明对比。

Conclusion: ReLU MLP在NTK机制下的外推行为存在两种极端情况：远离原点时线性外推，靠近原点时二次外推，这为理解神经网络的外推能力提供了新的理论视角。
...
TL;DR: Tomographic Quantile Forests (TQF) 是一种基于树的多变量回归模型，通过方向投影学习条件分位数，并使用切片Wasserstein距离重建多变量条件分布。


<details>
  <summary>Details</summary>
Motivation: 量化预测不确定性对于AI在现实世界中的安全可信部署至关重要，但多变量目标的条件分布完全非参数估计仍然具有挑战性。

Method: TQF学习方向投影n⊤y的条件分位数作为输入x和单位方向n的函数。在推理时，它聚合多个方向的分位数，并通过高效的交替方案（具有凸子问题）最小化切片Wasserstein距离来重建多变量条件分布。

Result: TQF在合成和真实世界数据集上进行了评估，与经典方向分位数方法不同，它可以用单一模型覆盖所有方向而不强加凸性限制。

Conclusion: TQF是一种非参数、不确定性感知的树基多变量回归模型，能够有效估计多变量条件分布，源代码已在GitHub上发布。
...
<details>
  <summary>Details</summary>
Motivation: Agentic Workflows（AWs）作为解决复杂任务的有前景范式，其自动化生成的可扩展性受到执行评估高成本和延迟的严重限制。现有AW性能预测方法无法同时捕捉AWs中复杂的拓扑依赖关系和深层语义逻辑。

Method: 提出GLOW框架：1）使用图导向的LLM（在图任务上指令调优）提取拓扑感知语义特征；2）将LLM特征与GNN编码的结构表示融合；3）采用对比对齐策略优化潜在空间以区分高质量AWs。

Result: 在FLORA-Bench上的大量实验表明，GLOW在预测准确性和排序效用方面优于最先进的基线方法。

Conclusion: GLOW通过结合GNN的结构建模能力和LLM的推理能力，有效解决了AW性能预测中同时捕捉拓扑依赖和语义逻辑的挑战，为AWs的自动化生成提供了更高效的评估方法。
...
TL;DR: BUILD算法利用观测数据的精度矩阵结构，通过自底向上逐步识别叶节点并剪枝，从线性高斯SEM中确定性地重建DAG
...
Method: 提出BUILD算法：利用观测数据的整体精度矩阵的特殊结构，通过确定性逐步算法识别叶节点及其父节点，然后剪除叶节点及其关联边，迭代进行直到完全重建DAG。为应对有限数据估计精度矩阵时的误差累积，定期重新估计精度矩阵以提高鲁棒性。

Result: 在具有挑战性的合成基准测试中，BUILD算法在DAG学习方面优于现有最先进算法，同时提供了明确的复杂度控制。

Conclusion: BUILD算法利用线性高斯SEM下精度矩阵的结构特性，提供了一种确定性的自底向上DAG重建方法，在准确性和鲁棒性方面表现优异，为DAG学习提供了有效的解决方案。
...
### [30] [Stackelberg Learning from Human Feedback: Preference Optimization as a Sequential Game](https://arxiv.org/abs/2512.16626)
*Barna Pásztor,Thomas Kleine Buening,Andreas Krause*

Main category: cs.LG

TL;DR: SLHF是一种新的偏好优化框架，将对齐问题建模为领导者-跟随者顺序博弈，相比RLHF和NLHF具有一致性、数据敏感性和对不可传递偏好的鲁棒性优势。


<details>
  <summary>Details</summary>
Motivation: 现有偏好优化方法如RLHF（基于标量奖励）和NLHF（基于同时博弈均衡）存在局限性，需要能够捕捉更丰富偏好结构、支持推理时精炼的新框架。

Method: 将对齐问题建模为顺序博弈：领导者策略承诺行动，跟随者策略根据领导者行动条件响应。将偏好优化分解为跟随者的精炼问题和领导者对抗对手的优化问题。

Result: 在大型语言模型实验中，SLHF在多样化偏好数据集上实现强对齐，参数规模从0.5B扩展到8B，推理时精炼能力可跨模型家族迁移而无需微调。

Conclusion: SLHF通过顺序博弈设计提供了优于RLHF和NLHF的解决方案，在一致性、数据敏感性和鲁棒性方面具有优势，支持推理时精炼和跨模型迁移。
...
### [31] [TAO-Net: Two-stage Adaptive OOD Classification Network for Fine-grained Encrypted Traffic Classification](https://arxiv.org/abs/2512.15753)
*Zihao Wang,Wei Peng,Junming Zhang,Jian Li,Wenxin Fang*

Main category: cs.LG

TL;DR: TAO-Net：两阶段自适应OOD加密流量分类网络，通过混合OOD检测机制和语义增强提示策略，实现ID/OOD流量区分和细粒度OOD分类，显著提升新兴应用识别性能。
...
Method: 提出两阶段自适应OOD分类网络：第一阶段采用混合OOD检测机制，结合基于transformer的层间变换平滑度和特征分析来区分ID/OOD流量；第二阶段利用大语言模型和语义增强提示策略，将OOD流量分类转化为生成任务，实现无需预定义标签的细粒度分类。

Result: 在三个数据集上，TAO-Net达到96.81-97.70%的宏精度和96.77-97.68%的宏F1分数，显著优于先前方法（仅44.73-86.30%宏精度），特别是在识别新兴网络应用方面表现突出。

Conclusion: TAO-Net通过创新的两阶段设计和语义增强提示策略，有效解决了加密流量分类中的OOD问题，实现了对已知和未知流量的准确细粒度分类，为处理新兴网络应用提供了有效解决方案。
...
### [32] [Batch Normalization-Free Fully Integer Quantized Neural Networks via Progressive Tandem Learning](https://arxiv.org/abs/2512.16476)
*Pengfei Sun,Wenyu Jiang,Piew Yoong Chee,Paul Devos,Dick Botteldooren*

Main category: cs.LG

TL;DR: 提出了一种无需批归一化(BN)的全整数量化神经网络训练方法，通过渐进式层间蒸馏实现，可直接集成到现有量化流程中，支持端到端的纯整数推理。


<details>
  <summary>Details</summary>
Motivation: 现有量化神经网络(QNNs)虽然通过低比特算术缩小模型并降低推理能耗，但大多数仍依赖批归一化(BN)层，这阻碍了真正的纯整数部署。先前尝试移除BN的方法通常无法完全恢复BN的稳定性和准确性，且往往需要特殊约束。

Method: 提出了一种BN-free的全整数QNN训练方法，采用渐进式层间蒸馏方案。从预训练的BN-enabled教师模型开始，使用层间目标和渐进补偿来训练学生模型，该模型完全使用整数算术进行推理且不包含BN操作。
...
### [33] [KAN-Matrix: Visualizing Nonlinear Pairwise and Multivariate Contributions for Physical Insight](https://arxiv.org/abs/2512.15755)
*Luis A. De la Fuente,Hernan A. Moreno,Laura V. Alvarez,Hoshin V. Gupta*

Main category: cs.LG

TL;DR: 本文提出使用Kolmogorov-Arnold Networks（KANs）增强高维数据解释性，开发了两种可视化工具：PKAN用于分析变量对之间的非线性关联，MKAN用于非线性特征重要性排序。


<details>
  <summary>Details</summary>
Motivation: 高维数据中变量间的复杂关系（高维度和共线性）使得传统相关性分析方法难以充分解释数据，需要更强大的工具来揭示非线性关联和变量重要性。

Method: 基于Kolmogorov-Arnold Networks（KANs）开发了两种可视化工具：1）Pairwise KAN Matrix（PKAN）分析变量对之间的非线性关联；2）Multivariate KAN Contribution Matrix（MKAN）作为非线性特征排序工具，量化输入变量对预测目标变量的相对贡献。

Result: 实验比较表明，PKAN和MKAN比Pearson相关性和互信息方法产生更稳健和信息丰富的结果，能够捕捉关系的强度和函数形式，有助于发现隐藏的物理模式。

Conclusion: PKAN和MKAN工具支持模型开发工作流程的预处理（特征选择、冗余分析）和后处理（模型解释、物理洞察），促进基于领域知识的模型开发，提高了复杂数据集的解释性。
...
### [34] [Exploiting Radio Frequency Fingerprints for Device Identification: Tackling Cross-receiver Challenges in the Source-data-free Scenario](https://arxiv.org/abs/2512.16648)
*Liu Yang,Qiang Li,Luxiong Wen,Jian Yang*

Main category: cs.LG

TL;DR: 提出MS-SHOT方法解决源数据不可用的跨接收器射频指纹识别问题，通过动量中心引导的软伪标签和全局结构约束实现高效适配。


<details>
  <summary>Details</summary>
Motivation: 边缘计算中基于深度学习的RFFI模型在实际部署时面临关键挑战：当应用于不同硬件特性的接收器时，由于接收器变化引入的分布偏移，模型性能会显著下降。现有方法需要源域数据，但实际场景中源数据可能不可用。

Method: 提出MS-SHOT方法：1) 建立约束伪标签SCRFFI适配框架；2) 理论分析发现目标域性能对伪标签质量敏感；3) 采用动量中心引导的软伪标签生成；4) 施加全局结构约束以鼓励自信和多样化的预测；5) 能处理目标域标签偏移或非均匀类别分布。

Result: 在真实世界数据集上的大量实验表明，MS-SHOT在准确性和鲁棒性方面持续优于现有方法，为RFFI中的源数据不可用跨接收器适配提供了实用且可扩展的解决方案。

Conclusion: MS-SHOT方法有效解决了源数据不可用的跨接收器RFFI问题，特别能处理目标域标签偏移和非均匀类别分布，为实际部署提供了可行的解决方案。
...
### [35] [ReactorFold: Generative discovery of nuclear reactor cores via emergent physical reasoning](https://arxiv.org/abs/2512.15756)
*Yoonpyo Lee*

Main category: cs.LG

TL;DR: ReactorFold是一个基于语言模型的生成式核反应堆燃料组件设计框架，通过序列建模、参数高效微调和DPO对齐，能够自主发现传统方法无法触及的新设计拓扑结构。


<details>
  <summary>Details</summary>
Motivation: 传统核反应堆堆芯设计方法（确定性方法、元启发式方法、机器学习辅助方法）在固定的人工定义配置空间中搜索，限制了发现全新设计拓扑结构的能力。需要一种能够超越人类设计约束、自主探索设计空间的方法。

Method: 将燃料组件设计重新定义为语言模型的序列建模问题，使用蒙特卡罗数据、参数高效微调和直接偏好优化（DPO）训练模型，使模型学习压水堆组件的潜在结构，并通过单次前向传播生成候选布局。

Result: DPO对齐的模型展现出设计空间扩展的涌现能力：尽管仅在固定数量钆可燃吸收体（Gd）棒的配置上训练，却能自主调整Gd库存以满足严格的功率峰值约束。模型还发现了高性能的非对称配置，挑战了传统的对称装载启发式方法。

Conclusion: 语言模型能够内化因果物理关系并超越人类强加的设计约束，访问传统搜索方法无法触及的设计体系，为核反应堆设计开辟了新途径。
...
### [36] [Twin Restricted Kernel Machines for Multiview Classification](https://arxiv.org/abs/2512.15757)
*A. Quadir,M. Sajid,Mushir Akhtar,M. Tanveer*

Main category: cs.LG

TL;DR: 提出多视角孪生限制核机(TMvRKM)，通过正则最小二乘方法解决传统多视角SVM的计算和泛化问题，在多个基准数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统多视角学习方法在利用核技巧处理高维空间决策边界时面临挑战，容易产生误差且难以处理视角不一致问题，需要更高效和鲁棒的解决方案。

Method: 提出TMvRKM模型，结合核机与多视角框架优势，采用正则最小二乘方法替代传统二次规划问题，包含耦合项平衡多视角误差，集成早期和晚期融合策略。

Result: 在UCI、KEEL和AwA基准数据集上的实验和统计分析显示，TMvRKM在所有场景中都优于基线模型，展现出卓越的泛化性能。

Conclusion: TMvRKM成功解决了传统多视角核方法的关键计算和泛化挑战，为多视角学习提供了高效且性能优越的新方法。
...
### [37] [Yantra AI -- An intelligence platform which interacts with manufacturing operations](https://arxiv.org/abs/2512.15758)
*Varshini Krishnamurthy*

Main category: cs.LG

TL;DR: 该论文开发了一个用于XRIT的智能生产系统，集成了机器学习模型（随机森林分类器和孤立森林）进行预测性维护和异常检测，使用Streamlit实现实时数据可视化，并整合GPT-4虚拟助手提供决策支持，显著提升了运营效率、能源管理和维修规划能力。


<details>
  <summary>Details</summary>
Motivation: 工业4.0快速发展改变了智能生产，需要解决能源管理、预测性维护和AI驱动决策支持等关键问题，以优化XRIT的生产运营效率。

Method: 开发智能生产系统，集成随机森林分类器进行主动维护，孤立森林进行异常检测，使用Streamlit构建实时数据可视化仪表板，并整合GPT-4虚拟助手提供实时决策支持。

Result: 系统通过模拟数据测试，证明能显著提高工作效率、能源管理和维修规划能力，具备可扩展性，可实时应用于XRIT生产环境。

Conclusion: 该系统成功解决了工业4.0环境下的关键生产问题，未来工作将专注于实时数据集成和系统进一步优化。
...
TL;DR: SCFA框架将领域知识约束融入联邦学习聚合过程，通过减少假设空间和数据异质性，在非IID数据下实现更快收敛和更好的隐私-效用权衡。
...
Method: 提出语义约束联邦聚合(SCFA)框架，将领域知识约束纳入分布式优化。使用知识图谱编码约束（从ISA-95和MASON本体中提取3000个约束），在聚合过程中强制执行语义约束。

Result: 理论证明收敛率O(1/√T + ρ)，约束减少41%数据异质性，假设空间减少因子θ=0.37。在ε=10的差分隐私下，效用仅下降3.7%（标准联邦学习下降12.1%）。实验显示22%更快收敛，41.3%模型发散减少，约束违反率ρ<0.05时保持90%最优性能。

Conclusion: SCFA通过语义约束显著改善联邦学习在非IID数据下的性能，提供理论保证和实际验证，为约束驱动的联邦学习建立了首个收敛理论框架。
...
TL;DR: 本文介绍了一种结合经典量纲分析与现代机器学习的方法——无量纲学习，用于从实验数据中发现无量纲数和标度律，并通过正则化技术获得可解释的物理定律。


<details>
  <summary>Details</summary>
Motivation: 传统实验测量产生大量数据，但缺乏系统方法从中提取简洁的物理定律。需要一种既能利用机器学习能力，又能保持物理可解释性的框架，以发现变量间的量纲不变性和标度关系。

Method: 方法结合经典量纲分析与神经网络：首先从物理量测量值识别基本无量纲组合方式，然后用神经网络发现最能预测实验输出的组合。关键创新是使用正则化技术，使学习系数趋向简单可解释值（如整数、半整数）。

Result: 该方法能有效处理单/多无量纲数情况，揭示不同但等效的物理表示。正则化方法对测量噪声和离散采样具有鲁棒性。成功展示了如何从实验数据中发现紧凑的物理定律。

Conclusion: 无量纲学习是连接实验数据与物理定律的有力框架，但仍面临计算成本、数据特性影响、变量自动选择和用户工具开发等挑战。本教程为研究者提供了应用该方法的实用指南。
...
TL;DR: 提出可解释机器学习框架，基于CFD流场特征进行空间血栓风险评估，通过逻辑回归和特征选择建立紧凑可解释模型，在不同泵型间具有良好迁移性。


<details>
  <summary>Details</summary>
Motivation: 现有计算模型难以将复杂流动条件转化为可靠可解释的血栓风险预测，反映了对特定流动特征如何促进血栓形成和生长的理解不足。

Method: 引入基于CFD流动特征的可解释机器学习框架，采用逻辑回归模型结合结构化特征选择流程，提取紧凑且物理可解释的特征集（包括非线性特征组合），使用已验证的宏观血栓模型的空间风险模式进行训练。

Result: 模型成功复现标注的风险分布，识别出与血栓风险增加相关的不同流动特征集。在离心泵上的应用表明，尽管仅在单轴流泵工况下训练，模型仍能预测合理的血栓易发区域。

Conclusion: 可解释机器学习能够将局部流动特征与血栓风险联系起来，同时保持计算高效和机理透明。低计算成本支持快速血栓生成性筛选，该框架补充了基于物理的血栓建模，为将可解释机器学习集成到CFD驱动的血栓分析和设备设计工作流程提供了方法基础。
...
TL;DR: 提出CSA-TTA框架，通过跨样本增强解决术中低血压预测中测试时训练数据不足的问题，结合自监督学习和序列预测提升模型适应性。


<details>
  <summary>Details</summary>
Motivation: 术中低血压(IOH)预测具有挑战性，因为患者特异性差异大且低血压事件罕见，导致测试时训练不可靠。

Method: 提出CSA-TTA框架：1)构建跨样本库，将历史数据分为低血压和非低血压样本；2)采用粗到细检索策略：先用K-Shape聚类识别代表性聚类中心，再基于当前患者信号检索语义相似的top-K样本；3)结合自监督掩码重建和回顾性序列预测信号进行训练。

Result: 在VitalDB和真实医院数据集上评估，与TimesFM和UniTS等先进模型集成。在VitalDB上，微调场景下Recall和F1分别提升+1.33%和+1.13%，零样本场景下提升+7.46%和+5.07%，表现出强鲁棒性和泛化能力。

Conclusion: CSA-TTA通过跨样本增强有效解决了术中低血压预测中测试时训练数据不足的问题，显著提升了预测性能，特别是在零样本场景下表现突出。
...
TL;DR: AdaGradSelect是一种自适应选择Transformer块进行更新的参数高效微调方法，相比全参数微调训练速度提升12%，GPU内存减少35%，性能接近全微调，在小语言模型上优于LoRA。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型全参数微调成本高、内存需求大，而现有参数高效微调方法（如LoRA）限制在有限子空间中训练，有时会降低性能。对于小语言模型，效率提升更为重要，需要更有效的微调方法。

Method: AdaGradSelect基于梯度自适应选择要更新的Transformer块。它结合了基于Dirichlet的采样（依赖历史更新频率）和epsilon-greedy探索策略，早期探索不同块，后期聚焦重要块。

Result: 训练速度提升约12%，GPU内存减少35%，性能接近全参数微调。在GSM8K数据集上，相比LoRA（rank 256）平均提升约3%（Qwen2.5-0.5B、LLaMA3.2-1B、Phi4-mini-3.8B），在MATH数据集上达到相似准确率。

Conclusion: AdaGradSelect为传统微调方法提供了更有效、资源效率更高的替代方案，特别适合小语言模型的高效微调。
...
TL;DR: 提出基于图神经网络(GNN)的混合孪生方法，用少量数据学习物理模型与真实现象之间的差距（无知模型），提升模拟精度并减少数据需求。


<details>
  <summary>Details</summary>
Motivation: 传统有限元等物理模型存在未建模效应和简化假设，与真实存在差距；纯数据驱动方法需要大量高质量数据，现实中难以获取；需要一种能利用有限数据补充物理模型的方法。

Method: 采用混合孪生方法，将物理模型与数据驱动的无知模型结合；使用图神经网络(GNN)表示无知模型，即使测量点稀疏也能学习空间模式；GNN能处理不同网格、几何和载荷位置。

Result: 在非线性热传导问题上验证，GNN成功捕捉无知模型，能泛化到不同空间配置，显著提高模拟精度和可解释性，同时最小化数据需求。

Conclusion: GNN混合孪生方法有效弥补物理模型与真实间的差距，利用有限数据提升模拟性能，为复杂物理现象建模提供了数据高效且可解释的解决方案。
...
TL;DR: TS-DP提出了一种基于推测解码的时间感知强化学习扩散策略，通过Transformer草稿模型和RL调度器动态适应任务难度，实现4.17倍加速且保持94%以上草稿接受率，达到25Hz实时推理。


<details>
  <summary>Details</summary>
Motivation: 扩散策略在具身控制中表现出色，但存在推理延迟高、计算成本大的问题。现有静态加速方法（如量化）无法处理动态具身任务，而推测解码作为无损自适应方法在扩散策略中尚未充分探索。

Method: 1) 使用Transformer草稿模型模仿基础模型，替代昂贵的去噪调用；2) 基于RL的调度器通过调整推测参数动态适应时变任务难度，在保持准确性的同时提高效率。
...
Result: 结果显示在小的对抗扰动下，模型性能显著下降，通过对抗训练可以实现部分性能恢复

Conclusion: 金融表格模型对对抗攻击脆弱，需要鲁棒性增强技术，对抗训练是有效的防御方法但只能提供部分保护
...
TL;DR: 本文系统评估了t-SNE中九种核函数在生物序列数据降维中的应用，发现余弦相似性核在可视化质量和下游分析任务中表现最优，优于传统的高斯核和隔离核。
...
Method: 本研究全面评估了九种核函数在t-SNE中的应用，使用三种嵌入方法（One-Hot Encoding、Spike2Vec和minimizers）。通过主观可视化和客观指标（包括邻域保持分数）进行评估，并在六个不同的生物数据集上进行分类和聚类实验验证。
...
TL;DR: 该特刊介绍了符号回归在物理科学中的应用，包括自动方程发现、涌现现象建模和计算昂贵模拟的紧凑仿真器构建，并概述了方法学挑战和未来方向。
...
Method: 特刊包含多篇论文，涵盖符号回归在物理科学中的各种应用。方法学考虑包括搜索空间设计、运算符选择、复杂度控制、特征选择，以及与现代AI方法的集成。还讨论了对称性约束、渐近行为和其他理论信息的融入。

Result: 特刊收集的论文展示了符号回归在物理科学中的加速进展和日益增长的相关性，包括有效理论推导、经验函数形式和替代模型构建等应用。

Conclusion: 符号回归在物理科学中显示出巨大潜力，但仍面临可扩展性、噪声鲁棒性、过拟合和计算复杂度等挑战。未来方向包括融入更多理论约束，该领域正在快速发展并在物理科学中日益重要。
...
TL;DR: 本文提出了一个统一框架，将离散扩散、高斯扩散和单纯形扩散三种方法统一为同一基础过程（Wright-Fisher种群遗传模型）的不同参数化形式，解决了现有方法算法分散、理论不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 当前离散序列（如DNA、蛋白质、语言）的扩散建模存在三种主要方法：离散空间扩散、欧几里得空间的高斯扩散、单纯形上的扩散。这些方法具有不同的算法、理论结构和权衡取舍，缺乏统一框架，使得实践者难以在不同模型间切换。

Method: 建立理论框架，将三种离散扩散方法统一为Wright-Fisher种群遗传模型的不同参数化形式。发现单纯形扩散和高斯扩散是该模型的两个大种群极限。利用数十年数学遗传学文献开发稳定的单纯形扩散方法，并训练可在测试时在任意域执行扩散的单一模型。

Result: Wright-Fisher单纯形扩散比先前单纯形扩散模型更稳定，在条件DNA生成任务中表现更优。同时，训练的多域模型在单个域上的表现与专门训练的单域模型相当。

Conclusion: 通过Wright-Fisher种群遗传模型统一了三种离散扩散方法，提供了稳定的单纯形扩散实现，并展示了单一模型可在多个域执行扩散的能力，为实践者提供了更灵活的建模选择。
...
TL;DR: 提出DSO方法，通过强化学习优化激活转向，实现推理时可控的偏见减少，在VLMs和LLMs上取得公平性与性能的最佳权衡。


<details>
  <summary>Details</summary>
Motivation: 生成模型（如视觉语言模型）在决策时容易受到人口统计属性偏见的影响（如难以识别女性医生），而现有激活转向方法难以纠正需要跨人口群体等概率结果的偏见。用户对偏见减少与模型性能的权衡需求各异，需要推理时可控制的偏见减少方法。

Method: 提出直接转向优化（DSO），使用强化学习寻找线性变换来引导激活，专门设计用于减少偏见同时保持对模型性能的控制。

Result: DSO在VLMs和LLMs上实现了公平性与能力之间的最先进权衡，同时为从业者提供了推理时对权衡的控制。

Conclusion: 直接针对控制模型行为进行优化的转向策略比依赖预定义启发式方法更有效，为偏见干预提供了更有效的方法。
...
TL;DR: BarcodeMamba+ 是一个用于真菌DNA条形码分类的基础模型，采用状态空间模型架构，通过预训练和微调范式在数据稀疏环境中显著优于传统监督学习方法。


<details>
  <summary>Details</summary>
Motivation: 真菌DNA条形码分类面临标签稀疏和长尾分布等极端挑战，传统监督学习方法难以泛化到未见物种并捕捉数据的层次结构特性。

Method: 基于状态空间模型架构构建基础模型，采用预训练和微调范式利用部分标记数据，在微调阶段整合层次标签平滑、加权损失函数和MycoAI的多头输出层等增强技术。

Result: 在具有明显分类分布偏移的真菌分类基准测试中，最终模型在所有分类级别上都优于现有方法，每个增强组件都带来显著性能提升。
...
TL;DR: 该论文提出并研究上下文半监督学习(IC-SSL)，探索Transformer如何利用未标记的上下文数据进行表示学习，在低标签情况下显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有理论研究主要关注有明确标签对的监督学习设置，但实践中Transformer在标签稀疏或缺失时仍表现良好，这表明未标记的上下文演示中存在关键结构。需要理解Transformer如何利用未标记上下文进行表示学习。

Method: 引入并研究上下文半监督学习(IC-SSL)，其中少量标记示例伴随大量未标记点。Transformer利用未标记上下文学习鲁棒的、上下文依赖的表示。

Result: Transformer能够利用未标记上下文学习稳健的表示，实现准确预测，在低标签情况下显著提升性能。

Conclusion: 该研究为理解Transformer如何在ICL框架内利用未标记上下文进行表示学习提供了基础性见解，揭示了未标记上下文在提升模型性能中的重要作用。
...
TL;DR: SALVE框架：通过稀疏自编码器无监督学习模型原生特征基，结合Grad-FAM验证特征，实现权重空间干预，可连续调制类别定义和跨类别特征，提供细粒度鲁棒性诊断。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络性能优异但难以解释和控制，需要桥接机制可解释性和模型编辑的统一框架。

Method: 使用ℓ1正则化自编码器学习稀疏、模型原生特征基；用Grad-FAM验证特征；利用自编码器结构进行精确的权重空间干预；推导临界抑制阈值α_crit量化类别对主导特征的依赖。

Result: 在卷积网络（ResNet-18）和Transformer模型（ViT-B/16）上验证，实现对模型行为的一致、可解释控制。

Conclusion: 提出了将特征发现转化为可操作模型编辑的原则性方法，推进透明可控AI系统的发展。
...
TL;DR: AIE4ML：首个将AI模型自动转换为针对AMD AIE-ML架构优化固件的框架，实现接近硬件峰值的性能，支持全片上执行，在粒子物理触发系统等超低延迟环境中提供GPU级吞吐量。


<details>
  <summary>Details</summary>
Motivation: 在AMD Versal AI Engine上进行高效AI推理具有挑战性，因为其紧密耦合的VLIW执行、显式数据路径和本地内存管理。先前工作仅关注第一代AIE内核优化，未解决跨2D阵列的完整神经网络执行问题。

Method: 提出AIE4ML框架：1）单内核级别实现接近架构峰值的性能；2）图级别提供结构化并行化方法，跨2D AIE-ML阵列扩展并利用专用内存瓦片实现全片上执行；3）设计高效线性层实现，支持融合偏置加法和ReLU激活；4）通过新颖的图放置和搜索算法，为设备物理2D网格生成确定性、紧凑、拓扑优化的布局。

Result: 在层扩展基准测试中，达到单内核基线98.6%的效率，使用304个AIE瓦片中的296个（97.4%），实现全片上数据移动。在实际模型拓扑评估中，AIE4ML在微秒级延迟约束下提供GPU级吞吐量。

Conclusion: AIE4ML是首个针对AMD AIE-ML架构的全面AI模型优化框架，支持从hls4ml或PyTorch等工具导入量化模型并保持比特精确性，适用于粒子物理实验触发系统等超低延迟环境。
...
TL;DR: 该研究分析了决策树论文中使用的预测变量，发现许多属于受隐私法规监管的数据类别，支持隐私保护方法和治理检查。


<details>
  <summary>Details</summary>
Motivation: 决策树方法在结构化表格数据中广泛应用且因其可解释性而受到重视，但研究中使用的预测变量（如年龄、诊断代码、位置）越来越多地受到隐私法规的监管。需要了解实际应用中这些数据如何与隐私法律框架相关联。

Method: 收集决策树研究文献，将每个报告的预测变量分配到受监管的数据类别（如健康数据、生物识别标识符、儿童数据、财务属性、位置轨迹和政府ID），然后将每个类别与欧盟和美国隐私法律的具体条款联系起来。

Result: 研究发现许多报告的预测变量属于受监管类别，其中医疗保健领域占比最大，不同行业存在明显差异。分析了流行程度、行业构成和时间模式，并总结了每个法律框架参考年份下的监管对齐时间。

Conclusion: 研究证据支持隐私保护方法和治理检查，可以为超越决策树的机器学习实践提供信息，强调在机器学习应用中需要考虑隐私法规的合规性。
...
<details>
  <summary>Details</summary>
Motivation: 商品RFID系统在森林环境中因信号衰减、多径效应和环境变化导致定位精度差。现有指纹识别方法需要预先在已知位置部署标签，但在实际野火响应中难以实现。
...
Conclusion: 该方法为森林环境中的资产追踪提供了一种经济高效、可扩展的解决方案，特别适用于野火响应等需要快速部署的场景，突破了传统RFID定位需要预先标记位置的限制。
...
TL;DR: DR-RL：通过强化学习动态优化LLM中多头自注意力的低秩分解，在保持精度的同时显著减少计算量


<details>
  <summary>Details</summary>
Motivation: 传统低秩近似方法使用静态秩假设，无法适应不同输入上下文和硬件约束，限制了灵活性和效率

Method: 结合强化学习和在线矩阵扰动理论，RL代理将秩选择建模为序列策略优化问题，使用在线矩阵扰动边界实现增量秩更新，采用轻量级Transformer策略网络和批量SVD操作

Result: 在保持与全秩注意力统计等效的下游准确性的同时，显著减少FLOPs，尤其在长序列场景（L>4096）效果明显

Conclusion: DR-RL为MHSA提供了自适应效率与理论严谨性结合的方案，是资源受限深度学习中启发式秩缩减技术的理论替代方案
...
TL;DR: 提出一种结合高阶有限差分格式和Rollout损失训练的方法，用于提升降阶模型在长时间预测中的准确性，并在2D Burgers方程上验证。


<details>
  <summary>Details</summary>
Motivation: 传统降阶模型(ROMs)虽然能快速近似求解参数化偏微分方程，但在长时间预测中精度会显著下降，需要改进其长期预测能力。

Method: 1) 引入灵活、高阶且计算成本低的有限差分格式；2) 提出Rollout损失函数，训练ROMs在任意时间范围内做出准确预测。

Result: 在2D Burgers方程上验证了该方法，证明能够有效提升降阶模型在长时间预测中的准确性。

Conclusion: 通过结合高阶有限差分格式和Rollout损失训练，成功解决了降阶模型在长时间预测中精度下降的问题，为复杂偏微分方程的快速求解提供了改进方案。
...
TL;DR: SNAC-Pack是一个自动化神经网络设计与FPGA部署的集成框架，通过硬件感知的神经架构搜索实现多目标优化，避免耗时的硬件合成过程。


<details>
  <summary>Details</summary>
Motivation: 现有神经架构搜索方法难以准确优化真实硬件性能，通常依赖代理指标如比特操作数，无法直接针对FPGA部署进行优化。

Method: 结合神经架构协同设计的多阶段搜索能力和资源利用率与延迟估计器，实现跨精度、FPGA资源利用率和延迟的多目标优化，无需为每个候选模型进行耗时的硬件合成。

Result: 在高能物理喷注分类任务中达到63.84%准确率，在Xilinx Virtex UltraScale+ VU13P FPGA上合成后，模型保持基准准确率，资源利用率与传统BOPs优化模型相当。
...
<details>
  <summary>Details</summary>
Motivation: 当前农业生态系统碳循环量化方法存在局限性：传统方法依赖位置无关的参数化和独立训练，未能充分利用迁移学习和输入数据的空间异质性，限制了在具有显著变异性的区域的适用性。需要开发能够准确、经济高效地在决策相关尺度上量化碳循环的方法。

Method: 提出FTBSC-KGML框架，在KGML-ag基础上增加预训练-微调过程和站点特定参数。使用多站点遥感GPP、气候和土壤协变量数据进行预训练-微调。核心是空间异质性感知的迁移学习方案：先全局预训练模型，然后在每个州或站点进行微调，学习位置感知表示，在有限数据下提高局部准确性而不牺牲可解释性。

Result: FTBSC-KGML在验证误差上低于纯全局模型，解释力一致性更高，能更好地捕捉跨州的空间变异性。相比之前的方法，在准确性和空间变异捕捉方面都有改进。

Conclusion: 该工作扩展了之前的SDSA-KGML框架，提出的FTBSC-KGML框架通过结合预训练-微调、空间变异感知和知识引导的机器学习，有效解决了农业生态系统碳循环量化中的迁移学习和空间异质性挑战，为气候缓解和可持续农业提供了更适用的工具。
...
TL;DR: 提出了一种结合强化学习与代理模型的几何设计优化方法，用于降低热管微反应器的平准化电力成本，在两种不同轴向反射体成本情况下均实现LCOE降低超过57%。


<details>
  <summary>Details</summary>
Motivation: 热管微反应器适用于偏远地区供电，但存在规模不经济问题，财务可行性存疑。需要结合经济与物理分析的早期设计迭代来改善这一问题。

Method: 采用强化学习优化框架，结合高斯过程和多层感知机等代理模型，优化平准化电力成本，同时满足燃料寿命、停堆裕度、峰值热通量和棒积分峰值因子等约束条件。

Result: 运行维护成本和资本成本是LCOE的主要贡献者，优化器通过巧妙调整设计参数，在满足约束条件下将LCOE降低了超过57%。轴向反射体和控制鼓材料成本是关键因素。

Conclusion: 该方法能有效优化热管微反应器设计，显著降低电力成本。未来将进一步集成燃料和热管性能与多目标优化，以全面理解约束条件与成本性能之间的相互作用。
...
<details>
  <summary>Details</summary>
Motivation: 大数据在金融、保险和网络安全应用中变得至关重要，但自动化分析系统的依赖引发了透明度、监管合规性和信任问题。需要将XAI集成到大数据分析流程中以解决这些挑战。

Method: 1. 回顾大数据关键特征和主要分析工具（分布式存储系统、流处理平台、欺诈检测模型）；2. 系统综述广泛使用的XAI方法（LIME、SHAP、反事实解释、注意力机制）；3. 分析这些方法在大规模部署时的优势和局限性；4. 提出一个概念框架，将可扩展大数据基础设施与上下文感知解释机制和人类反馈相结合。

Result: 识别了与可扩展性、实时处理以及图和时序模型可解释性相关的研究空白。提出了一个解决这些挑战的概念框架，并确定了可扩展XAI、隐私感知解释和可解释欺诈检测系统标准化评估方法等开放研究方向。

Conclusion: 论文为将XAI集成到大数据分析流程中提供了系统性的方法，特别是在欺诈检测和风险管理领域。提出的概念框架为解决可扩展性、实时处理和复杂模型解释性等挑战提供了方向，并指出了未来研究的关键领域。
...
TL;DR: DeepOSets架构通过结合DeepSets和DeepONets，实现了无需权重更新的多算子上下文学习能力，能够从提示中的示例对中恢复未见PDE的解算子，并证明了其在科学机器学习中的通用逼近性。


<details>
  <summary>Details</summary>
Motivation: 本文旨在探索非自回归、非注意力机制的神经网络架构是否也能实现上下文学习能力，特别是在科学机器学习领域，研究DeepOSets架构是否能够从示例中学习PDE解算子，而无需权重更新。

Method: 采用DeepOSets架构，结合DeepSets的集合学习和DeepONets的算子学习能力，通过适当修改使其成为多算子上下文学习器。该架构能够从用户提示中的参数-解示例对中恢复未见PDE的解算子。

Result: 实验证明DeepOSets能够准确预测训练中未见的PDE对应参数查询的解，在泊松方程和反应扩散方程的正反边值问题上表现良好。更重要的是，证明了DeepOSets是连续算子类上的通用一致逼近器。

Conclusion: DeepOSets架构展示了强大的上下文学习能力，能够从示例中学习PDE解算子而无需权重更新，并且具有通用逼近性质，为科学机器学习中的算子学习提供了新的非注意力机制解决方案。
...
TL;DR: 研究发现高斯模糊在实际低精度实现中可被逆向攻击，隐私性最差；像素化和像素化加噪声在适当粒度下能更好平衡隐私与实用性


<details>
  <summary>Details</summary>
Motivation: 图像数据常包含人脸、车牌等隐私信息，需要在不泄露隐私的前提下保持数据对模型训练的有效性。当前业界标准的高斯模糊方法在实际应用中存在隐私风险

Method: 比较四种模糊化算法：高斯模糊、像素化、像素化加噪声（DP-Pix）和裁剪。通过逆向攻击和识别攻击评估隐私性，通过模型在模糊化数据上的学习质量评估实用性

Result: 高斯模糊在实际低精度实现中最易受逆向攻击，隐私性最差；像素化和像素化加噪声在适当粒度下能提供更好的隐私保护，同时保持模型训练的有效性

Conclusion: 高斯模糊作为业界标准方法存在隐私风险，像素化和像素化加噪声是更好的选择。作者开发了Privacy Blur软件包提供建议参数和方法
...
Result: 构建了AIMM-GT数据集（33个标记的股票-日，涵盖8只股票），展示了初步的区分能力，AIMM在2021年1月GME挤压峰值前22天发出预警。

Conclusion: AIMM框架能够检测社交媒体驱动的市场操纵，提供早期预警，并发布了代码、数据集模式和仪表板设计以支持相关研究。
...
TL;DR: 论文揭示了机器遗忘技术中保留数据的隐私风险，首次提出双视图设置下的隐私泄露问题，并开发了DVIA攻击方法来验证这一威胁。


<details>
  <summary>Details</summary>
Motivation: 机器遗忘技术虽然能响应用户数据删除请求，但现有研究主要关注已遗忘数据的隐私保护，而保留数据的隐私风险尚未充分探索。本文旨在填补这一空白，研究机器遗忘在双视图设置下对保留数据带来的隐私威胁。

Method: 从信息论角度引入"隐私知识增益"概念，证明双视图设置能让攻击者获得比单独查询任一模型更多的信息。提出DVIA（双视图推理攻击）方法，通过黑盒查询原始模型和遗忘模型，使用轻量级似然比推理模块高效提取保留数据的成员信息。

Result: 在不同数据集和模型架构上的实验验证了DVIA的有效性，证实了双视图设置确实会放大隐私泄露风险。攻击无需训练攻击模型，仅通过轻量级推理即可成功提取保留数据的成员信息。

Conclusion: 机器遗忘技术虽然保护了请求遗忘用户的隐私，但在双视图设置下会引入新的隐私风险，特别是对保留数据的隐私威胁。研究揭示了这一被忽视的安全隐患，为未来设计更安全的机器遗忘方法提供了重要启示。
...
<details>
  <summary>Details</summary>
Motivation: 构建一个在多个关键领域（数学、代码、科学、推理）具有最先进性能的高效大型语言模型，同时开源完整的训练基础设施以促进社区发展。

Method: 使用混合专家架构（106B参数，12B激活），基于GLM-4.5-Air-Base模型进行监督微调和强化学习训练，采用新开发的prime-rl框架进行大规模异步RL训练，支持多轮交互和工具使用。

Result: INTELLECT-3在其尺寸范围内在数学、代码、科学和推理基准上达到最先进性能，超越了许多更大的前沿模型，并在512个H200 GPU上实现了高效的RL训练扩展。

Conclusion: INTELLECT-3展示了通过大规模强化学习训练MoE模型的有效性，同时开源完整的RL基础设施栈（包括prime-rl框架、训练配方和环境库）将推动社区在智能体RL研究方面的发展。
...
<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病的早期和可及性检测仍然是临床挑战，立方体复制任务提供了简单但信息丰富的视觉空间功能评估。需要开发更准确、可解释的筛查方法。

Method: 将立方体绘图建模为图结构，节点特征编码空间坐标、局部图元拓扑和角度几何特征，使用图神经网络处理，并与年龄、教育程度和神经心理学测试特征在后期融合模型中集成。

Result: 基于图的表示提供了强大的单模态基线，显著优于基于像素的卷积模型，多模态集成进一步提高了性能和对类别不平衡的鲁棒性。SHAP可解释性分析识别出特定的图元基序和几何变形是关键预测因子。

Conclusion: 基于图的立方体复制分析为阿尔茨海默病筛查建立了一种可解释、非侵入性和可扩展的方法，与临床观察到的AD患者立方体绘图紊乱现象密切对应。
...
TL;DR: stMFG是一种多尺度交互融合图网络，通过层间跨视图注意力动态整合空间和基因特征，结合对比学习和空间约束，在空间转录组学领域识别中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 空间转录组学能在原生组织环境中进行全基因组表达分析，但识别空间域仍面临挑战，因为基因与空间之间存在复杂相互作用。现有方法通常分别处理空间和特征视图，仅在输出层融合，这种"分别编码、后期融合"的范式限制了多尺度语义捕获和跨视图交互。

Method: 提出stMFG（多尺度交互融合图网络），引入层间跨视图注意力机制，在每次卷积后动态整合空间和基因特征。模型结合跨视图对比学习和空间约束，增强区分能力的同时保持空间连续性。
...
<details>
  <summary>Details</summary>
Motivation: 基于排序的零阶优化方法（如CMA-ES、自然进化策略等）在实践中广泛应用且对噪声和单调变换具有强鲁棒性，但现有理论分析仅提供渐近结果，缺乏显式收敛速率分析。

Method: 分析一个简单的基于排序的零阶算法，采用新颖的分析方法（避免传统的漂移和信息几何技术），为强凸和平滑非凸目标函数建立显式查询复杂度界限。

Result: 对于d维问题：强凸函数达到Õ(dL/μ·log(dL/μδ)·log(1/ε))查询复杂度；非凸函数达到O(dL/ε·log(1/ε))查询复杂度，均以至少1-δ的概率成立。

Conclusion: 本文填补了基于排序的零阶优化理论空白，首次提供了显式非渐近查询复杂度分析，为理解这类启发式方法的高效性提供了新视角。
...
TL;DR: 使用机器学习模型预测地质灾害（滑坡、雪崩等）的运移距离，比数值求解器快100-10000倍，能准确预测流动范围和沉积厚度


<details>
  <summary>Details</summary>
Motivation: 地质灾害运移距离预测对保护生命、基础设施和生态系统至关重要，但现有方法面临速度与真实性的权衡，需要既物理真实又计算高效的模型

Method: 训练机器学习模型，基于超过10万个数值模拟和1万多个真实世界数字高程模型芯片的数据，预测地质灾害运移距离

Result: 模型能高精度预测流动范围和沉积厚度，计算速度比数值求解器快100-10000倍，能再现关键物理行为（如分流和沉积模式），并能泛化到不同流动类型、规模和地形

Conclusion: 神经仿真技术为地质灾害建模提供了新途径，能够实现快速、空间分辨的运移距离预测，为大规模早期预警系统开辟了新机会
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将所有OOD样本视为单一类别，但实际应用（如欺诈检测、医疗诊断）需要更深入了解OOD样本的可能标签。关键问题：能否在没有真实标签信息的情况下将OOD检测扩展到OOD分类？

Method: 提出CFC框架，包含三个关键组件：1）粗分类器使用LLM提示进行OOD检测和异常标签生成；2）基于GNN的细分类器，使用粗分类器识别的OOD样本进行训练，增强OOD检测和ID分类；3）通过LLM提示和后处理的OOD标签实现精细化的OOD分类

Result: CFC在图和文本领域的OOD检测上比最先进方法提升10%，在图数据集上的OOD分类准确率达到70%

Conclusion: CFC框架成功解决了在没有真实标签信息的情况下进行OOD分类的问题，利用语义OOD实例提高了可解释性和实用性，在开放世界场景中表现出色
...
TL;DR: SEAL算法通过同时最小化损失函数及其尖锐度，并引入基于相关矩阵的正则化器，解决联邦图学习中的数据异构性和维度坍塌问题，提升GNN模型的分类准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 联邦图学习(FGL)面临数据异构性问题：1)基于经验风险最小化的优化器导致局部模型陷入尖锐谷，削弱对分布外图数据的泛化能力；2)局部图数据表示中的维度坍塌对GNN模型分类能力产生负面影响。

Method: 提出SEAL算法：1)制定考虑局部GNN模型尖锐度的优化目标，同时最小化损失函数及其尖锐度，寻找平坦区域参数；2)引入基于局部表示相关矩阵的正则化器，松弛单个局部图样本生成表示的相关性，缓解维度坍塌。

Result: 在多个图分类基准测试中，SEAL始终优于最先进的FGL基线方法，并为更多参与者提供增益，增强了局部GNN模型的分类准确性和泛化能力。

Conclusion: SEAL算法通过尖锐度感知优化和维度坍塌缓解，有效解决了联邦图学习中的数据异构性问题，提升了GNN模型在联邦环境下的性能表现。
...
TL;DR: 提出Sharpness-aware SLF模型，通过Hessian-vector products获取二阶信息并在曲率中注入锐度项，解决SLF模型优化困难问题，在多个工业数据集上优于现有方法。
...
Conclusion: SSLF模型通过结合二阶信息和锐度感知优化，有效解决了SLF模型的优化困难问题，提高了模型性能。
...
Method: 提出CKA引导的模块化量化框架：1）独立评估每个层上的多种PTQ算法；2）使用线性中心核对齐（CKA）作为度量标准自动选择每层最优量化策略；3）将个体优化策略集成构建混合量化模型。该方法无需微调，即插即用。

Result: 实验表明，该方法在LLaMA和Qwen等主流大语言模型上，在困惑度（PPL）和下游任务性能方面，一致优于均匀量化基线和最先进的混合精度方法。

Conclusion: 通过逐层选择最优量化策略的异构量化方法能有效提升大语言模型的量化效果，CKA引导的模块化量化框架为解决层间算法适用性差异提供了有效解决方案。
...
TL;DR: SRMU是一种新颖的激活编辑框架，通过特征感知和方向可控的扰动，在高度纠缠的数据分布中实现选择性知识遗忘，在保持模型效用的同时确保安全性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在安全关键和受监管领域的应用增加，模型保留敏感或禁止知识带来了隐私泄露、监管不合规和潜在滥用等风险。现有遗忘技术假设遗忘集和保留集完全分离，这在现实操作环境中难以满足，因为数据分布高度纠缠，导致现有方法要么损害模型效用，要么无法确保安全性。

Method: 提出选择性表征误导遗忘框架，采用结构化误导向量和激活重要性图，实现特征感知和方向可控的扰动。与无差别权重扰动不同，SRMU选择性地抑制有害表征，同时保留良性表征的效用。

Result: 在WMDP基准测试中，SRMU在低纠缠和高纠缠配置下均实现了最先进的遗忘性能，且效用损失最小。在20-30%数据重叠的情况下仍保持有效，而现有基线方法已失效。

Conclusion: SRMU为安全驱动的模型治理、隐私合规和受控知识移除提供了稳健基础，适用于新兴的LLM应用场景。该方法在高度纠缠的数据分布中实现了选择性知识遗忘，平衡了安全性和模型效用。
...
TL;DR: 开发了首个电池寿命预测基础模型PBT，通过领域知识编码的专家混合层，在13个锂离子电池数据集上学习可迁移表示，平均性能提升19.8%，在15个多样化数据集上实现SOTA性能。
...
Method: 提出了预训练电池变压器(PBT)，这是首个电池寿命预测基础模型。采用领域知识编码的专家混合层架构，在最大的公共电池寿命数据库上进行验证，从13个锂离子电池数据集学习可迁移表示。

Result: PBT平均性能比现有模型提升19.8%。通过迁移学习，在涵盖各种操作条件、化成协议和化学组成的15个多样化锂离子电池数据集上实现了最先进的性能。
...
TL;DR: 本文提出了一种用于树集成模型的公平性量化验证方法，能够提供任意时间的上下界估计，相比现有方法更高效准确。


<details>
  <summary>Details</summary>
Motivation: 传统公平性验证方法仅返回单个反例，而量化验证能估计所有反例的比例并定位其区域，这对诊断和缓解偏见至关重要。现有量化验证方法主要针对深度神经网络，且存在只能提供下界、性能扩展差等局限性。

Method: 利用树集成模型的离散结构，提出高效的量化技术，能够提供任意时间的上界和下界估计，克服了基于反例引导抽象精化框架的局限性。

Result: 在五个广泛使用的数据集上的实验表明，该方法在有效性和效率方面表现优异。应用于公平性测试时，显著优于最先进的测试技术。

Conclusion: 该方法为树集成模型的公平性量化验证提供了高效解决方案，能够准确估计反例比例并定位偏见区域，有助于更好地诊断和缓解模型偏见。
...
Method: Kascade利用两个关键观察：1) softmax后的注意力本质上是稀疏的；2) 高权重键的身份在相邻层间稳定。方法包括：算法选择锚点层（通过动态规划最大化跨层相似性），在锚点层计算精确Top-k索引，在中间重用层复用这些索引，并采用头感知的Top-k选择和高效实现（瓦片级操作）。

Result: 在H100 GPU上，Kascade相比FlashAttention-3基线实现最高4.1倍解码注意力加速和2.2倍预填充注意力加速。在LongBench和AIME-24等长上下文基准测试中，准确率与密集注意力非常接近。

Conclusion: Kascade是一种有效的训练无关稀疏注意力方法，通过跨层重用Top-k索引显著加速长上下文LLM推理，同时保持高准确性，易于在不同模型间部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有水氮组合优化研究存在两个主要问题：1) 作物生长期间水氮组合优化复杂度高，产量优化效果差；2) 轻度胁迫信号量化困难且反馈延迟，导致水氮动态调节不精确，资源利用效率低

Method: 提出嵌套双智能体强化学习(NDRL)方法：父智能体基于预期累积产量效益识别有前景的宏观灌溉施肥动作，减少无效探索；子智能体奖励函数包含量化的水分胁迫因子(WSF)和氮胁迫因子(NSF)，使用混合概率分布动态优化每日策略。使用2023-2024年田间试验数据校准验证DSSAT模型来模拟真实条件并与NDRL交互

Result: 相比最佳基线方法：模拟产量在2023和2024年均提高4.7%；灌溉水生产率分别提高5.6%和5.1%；氮偏生产力分别提高6.3%和1.0%
...
Method: 提出基于极点-残差分解和指数基函数的广义算子学习框架，引入几何拉普拉斯神经算子(GLNO)，将拉普拉斯谱表示嵌入到拉普拉斯-贝尔特拉米算子的特征基中，并设计了网格不变网络架构GLNONet来实现GLNO。

Result: 在PDEs/ODEs和真实世界数据集上的大量实验表明，该方法相比其他最先进模型具有鲁棒的性能优势。

Conclusion: 提出的GLNO框架成功解决了现有算子学习方法在处理非周期动态和不规则几何时的局限性，为在任意黎曼流形上进行算子学习提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于物理模型的反向不确定性量化计算成本高，特别是涉及偏微分方程时，全阶模型使得MCMC采样计算不可行。数据驱动代理模型受限于高保真数据生成成本，而低保真数据单独使用会降低求解精度。

Method: 提出多保真度延迟接受方案，扩展多级延迟接受框架，引入多保真度神经网络。在离线训练阶段使用高保真评估，在线阶段通过评估粗求解器并将输出传递给训练好的神经网络来获得似然评估，避免额外高保真模拟。

Result: 该方法提高了低保真求解器的近似精度，导致更长的子链长度、更好的混合特性和加速的后验推断。在两个基准反问题（稳态各向同性地下水流、非稳态反应扩散系统）上展示了显著的计算节省。

Conclusion: 多保真度延迟接受方案能够一致地整合异构粗求解器，提供比标准多级延迟接受更大的灵活性，有效解决了高计算成本反问题的挑战。
...
TL;DR: 该论文研究了多智能体系统在金融预测任务中的公平性风险，发现多智能体系统可能表现出无法追溯到单个智能体的集体偏见行为，需要将其作为整体实体而非组件来分析。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统虽然能通过协作决策提高预测性能，但缺乏有效的公平性评估方法，这在高风险金融领域（如消费金融）存在部署风险，因为偏见决策可能导致监管违规和财务损失。

Method: 通过大规模模拟实验，在不同多智能体配置（包括不同通信和协作机制）下，使用公平性指标来测量金融表格数据领域多智能体预测系统的公平性特征。

Result: 研究发现多智能体系统在金融决策中出现了无法追溯到单个智能体组件的偏见模式，表明多智能体系统可能表现出真正的集体行为。公平性风险是金融多智能体系统模型风险的重要组成部分，对信用评分和收入估计等任务有实际影响。

Conclusion: 多智能体决策系统必须作为整体实体进行评估，而不是通过对其组成组件的还原主义分析。公平性风险是多智能体系统模型风险的重要部分，需要开发专门的公平性评估方法。
...
<details>
  <summary>Details</summary>
Motivation: 风力涡轮机叶片故障检测对系统可靠性至关重要，需要开发有效的早期故障检测方法，提高风能行业的安全性和效率。

Method: 采用两种计算学习模型：1) 逻辑回归模型，与神经网络、决策树和朴素贝叶斯方法比较；2) 聚类模型，用于数据分割和模式识别。使用Orange Data Mining等工具实现。

Result: 逻辑回归模型优于神经网络、决策树和朴素贝叶斯方法，能有效识别故障相关模式；聚类模型在精度和数据分割方面表现更优，能更好地捕捉数据底层特征。

Conclusion: 聚类方法可能比监督学习方法更好地捕捉数据特征，提出的方法为风力涡轮机叶片早期故障检测提供了新途径，展示了整合不同计算学习技术提升系统可靠性的潜力。
...
<details>
  <summary>Details</summary>
Motivation: LDA主题模型中主题数量T的选择是一个关键设计决策，它显著影响模型的统计拟合度和可解释性。传统方法通常需要大量试错，研究者希望找到更高效的优化方法来选择最优主题数量。

Method: 将主题数量选择问题形式化为离散黑盒优化问题，其中每个函数评估对应训练一个LDA模型并测量其验证困惑度。在固定评估预算下，比较了四种优化器家族：两种手工设计的进化方法（遗传算法和进化策略）和两种学习的摊销方法（PABBO和SABBO）。

Result: 实验表明，虽然所有优化器最终都能达到相似的困惑度范围，但摊销优化器在样本和时间效率上显著更优。SABBO通常只需一次评估就能识别接近最优的主题数量，PABBO在几次评估内找到竞争性配置，而GA和ES需要几乎全部预算才能达到相同区域。

Conclusion: 摊销黑盒优化方法在LDA主题数量选择问题上具有显著优势，能够以更少的评估次数获得接近最优的结果，这为模型超参数优化提供了更高效的解决方案。
...
TL;DR: 本文提出了一种结合CNN和HOSVD的新方法，在IoMT框架下实现白血病细胞的快速自动分类，在ALL-IDB2数据集上达到98.88%的测试准确率。


<details>
  <summary>Details</summary>
Motivation: 急性淋巴细胞白血病（ALL）是一种致命的血液疾病，早期诊断至关重要。传统人工检查存在人为错误风险且耗时，需要基于人工智能的自动诊断方法来提高准确性和效率。

Method: 采用卷积神经网络（CNN）结合新型分类器高阶奇异值分解（HOSVD），在医疗物联网（IoMT）架构下对显微镜血液图像中的ALL细胞和正常细胞进行分类。

Result: 在急性淋巴细胞白血病图像数据库（ALL-IDB2）上实现测试阶段平均准确率达到98.88%，能够实现患者与临床医生的实时通信。

Conclusion: 提出的CNN-HOSVD框架在IoMT环境下能够有效、快速、安全地识别白血病细胞，为白血病早期诊断提供了高效的自动化解决方案。
...
TL;DR: PLSCAN是一种新的基于密度的聚类算法，通过识别HDBSCAN*产生稳定（叶）簇的所有最小簇大小，无需手动选择超参数，在多个真实数据集上表现优于HDBSCAN*。


<details>
  <summary>Details</summary>
Motivation: 基于密度的聚类算法（如DBSCAN和HDBSCAN*）在探索性数据分析中很有用，但需要选择超参数（如密度阈值或最小簇大小），这在没有先验知识的情况下很困难。需要一种能自动确定合适参数的方法。

Method: 提出PLSCAN算法，基于尺度空间聚类原理，等价于在新度量空间上的持久同调。算法能高效识别HDBSCAN*产生稳定叶簇的所有最小簇大小，从而避免手动参数选择。

Result: 在多个真实数据集上，PLSCAN比HDBSCAN*获得更高的平均ARI（调整兰德指数），对互达邻居数量的变化更不敏感。在低维数据集上计算成本与k-Means竞争，高维时与HDBSCAN*相似。

Conclusion: PLSCAN是一种有效的基于密度聚类算法，能自动确定合适的聚类参数，在保持计算效率的同时提高聚类质量，特别适用于探索性数据分析。
...
TL;DR: Abacus：一种结合自监督预训练和序列学习的混合方法，通过预测用户事件的经验频率分布来改进展示广告中的用户购买行为建模。


<details>
  <summary>Details</summary>
Motivation: 展示广告系统中用户购买行为建模面临挑战：正样本稀疏、用户行为随机性导致类别不平衡和事件时间不规则。现有方法依赖手工"计数器"特征忽略用户意图的细粒度时序演化，而序列模型又忽略了有用的事件计数统计信息。

Method: 提出Abacus方法，通过自监督预训练预测用户事件的经验频率分布。进一步提出混合目标函数，将Abacus与序列学习目标统一，结合聚合统计的稳定性和序列建模的敏感性。

Result: 在两个真实数据集上的实验表明，Abacus预训练优于现有方法，加速下游任务收敛。混合方法相比基线在AUC指标上提升高达+6.1%。

Conclusion: Abacus通过结合事件频率分布预测和序列建模，有效解决了展示广告中用户行为建模的挑战，在性能和收敛速度上均有显著提升。
...
TL;DR: DataFlow是一个统一的LLM驱动数据准备框架，通过模块化、可组合的数据转换和自然语言到管道的自动转换，显著提升下游LLM性能。


<details>
  <summary>Details</summary>
Motivation: 当前LLM数据准备主要依赖临时脚本和松散的工作流程，缺乏原则性抽象、可复现性差，且对模型在环数据生成支持有限，需要更系统化的解决方案。

Method: 提出DataFlow框架，包含系统级抽象、PyTorch风格管道构建API、近200个可重用操作符和六个领域通用管道。引入DataFlow-Agent通过操作符合成、管道规划和迭代验证将自然语言规范自动转换为可执行管道。

Result: 在六个代表性用例中，DataFlow持续提升下游LLM性能：数学、代码和文本管道优于人工策划数据集和专用合成基线，在Text-to-SQL上比SynSQL提升+3%执行准确率，代码基准平均提升7%，MATH、GSM8K和AIME提升1-3个百分点。统一10K样本数据集使基础模型超越使用1M Infinity-Instruct数据训练的对应模型。
...
TL;DR: 该研究比较了多种机器学习算法与神经符号AI在文本分类任务上的性能，发现神经符号AI在小数据集上能达到与MLP相当的效果。


<details>
  <summary>Details</summary>
Motivation: 文本分类（如博客性别分类）是成熟的研究领域，在市场分析、客户推荐等应用中很重要。需要比较传统机器学习算法与新兴的神经符号AI方法的性能。

Method: 比较了SVM、朴素贝叶斯、逻辑回归、AdaBoost、XGBoost和SVM变体（SVM_R）与神经符号AI（NeSy）。探索了TF-IDF、通用句子编码器（USE）、RoBERTa等文本表示方法，以及卡方检验、互信息、主成分分析等特征提取技术。

Result: 实验结果显示，神经符号AI方法在有限数据集上能达到与多层感知机（MLP）相当的性能表现。

Conclusion: 神经符号AI在文本分类任务上具有潜力，未来研究将扩展知识库、嵌入类型范围和超参数配置，进一步探索NeSy方法的有效性。
...
TL;DR: CLARiTy是一种基于视觉Transformer的模型，用于胸部X光的多标签分类和弱监督定位，通过类特定token和SegmentCAM模块实现，在NIH ChestX-ray14数据集上取得竞争性分类性能和SOTA定位性能。


<details>
  <summary>Details</summary>
Motivation: 胸部X光解读面临多标签病理分类和空间定位的挑战，通常受限于区域级（密集）标注的稀缺性。需要一种能够同时处理不同标注粒度任务的模型。

Method: CLARiTy采用视觉Transformer架构，使用多个类特定token生成判别性注意力图，并引入SegmentCAM模块进行前景分割和背景抑制。模型使用图像级标签训练，通过ConvNeXtV2教师模型进行知识蒸馏以提高效率。

Result: 在NIH ChestX-ray14数据集上，CLARiTy-S-16-512在14种病理分类上取得竞争性性能，在8种病理的弱监督定位上达到SOTA，比先前方法提升50.7%。小病理（如结节和肿块）提升尤为显著。低分辨率变体CLARiTy-S-16-224在保持高效率的同时显著超越基线。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于神经网络的预测性流程挖掘技术缺乏可复现性、决策透明度、对新数据集的适应性以及基准测试能力，导致不同实现之间难以进行公平比较。

Method: 提出SPICE框架，在PyTorch中重新实现三种流行的深度学习基线方法，设计具有严格可配置性的通用基础框架，支持可复现和鲁棒的模型比较。

Result: 在11个数据集上比较SPICE与原始报告指标，并使用公平指标进行评估，验证了框架的有效性。

Conclusion: SPICE框架解决了预测性流程挖掘领域中的可复现性和比较难题，为未来建模方法提供了标准化的评估平台。
...
TL;DR: 提出結合字元級CNN與LightGBM的集成模型，用於釣魚網站URL檢測，達到99.819%準確率並提供即時檢測服務
...
Method: 使用集成方法結合字元級CNN（提取序列特徵）和LightGBM（處理36個詞彙、結構和域名特徵），透過FastAPI提供即時檢測服務

Result: 在19,873個URL測試集上達到：準確率99.819%、精確率100%、召回率99.635%、ROC-AUC 99.947%，集成模型中CNN貢獻60%、LightGBM貢獻40%

Conclusion: 提出的集成方法優於單一模型，能有效識別現代釣魚技術並保持極低的誤報率，提供實用的即時檢測解決方案
...
<details>
  <summary>Details</summary>
Motivation: 先前研究表明机器学习回归问题可在随机函数理论框架下解决，最优核函数为多调和样条，但存在O(N^3)计算复杂度和高维输入空间下理论假设失效的限制

Method: 提出由多调和样条包构建的级联架构，同时解决可扩展性问题，并为具有未知内在低维性的问题提供理论依据；提出前向计算和端到端级联微分的高效矩阵程序

Result: 级联架构能够同时处理可扩展性和高维问题，为具有未知内在低维性的回归问题提供理论支持

Conclusion: 提出的级联多调和样条架构有效解决了先前方法的计算复杂度和高维限制问题，为实际应用提供了可行的解决方案
...
TL;DR: KOSS：基于卡尔曼最优选择的状态空间模型，通过最小化潜在状态不确定性实现上下文感知选择，在序列建模任务中显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有选择性状态空间模型（如Mamba和Mamba-2）的选择机制缺乏理论依据，且无法支持基于潜在状态动态的上下文感知选择

Method: 提出KOSS模型，将选择机制形式化为潜在状态不确定性最小化问题，采用卡尔曼增益驱动的连续时间潜在状态更新，结合全局谱微分和分段扫描实现高效计算

Result: 在选择性复制任务中达到79%准确率（基线低于20%）；在9个长期预测基准上MSE降低2.92-36.23%；在二次监视雷达跟踪案例中验证了实际应用鲁棒性

Conclusion: KOSS通过理论驱动的闭环选择机制实现了上下文感知的序列建模，在准确性和稳定性方面均优于现有最先进模型，并具有实际应用价值
...
TL;DR: 本研究针对印尼朝觐服务数字化中的欺诈应用问题，采用机器学习算法（SVM、随机森林、朴素贝叶斯）结合文本和元数据特征，实现应用真实性自动验证，SVM达到92.3%准确率。


<details>
  <summary>Details</summary>
Motivation: 印尼朝觐和副朝服务的快速数字化虽然便利了朝觐者，但也催生了通过伪造移动应用进行的数字欺诈。这些欺诈应用不仅造成经济损失，还通过收集敏感个人数据带来严重的隐私风险。需要自动验证应用真实性的解决方案。

Method: 使用包含官方注册应用和非官方应用的综合数据集，比较三种分类器：支持向量机(SVM)、随机森林(RF)和朴素贝叶斯(NB)。采用混合特征提取方法，结合应用描述的文本分析(TF-IDF)和敏感访问权限的元数据分析。

Result: SVM算法表现最佳，准确率达到92.3%，精确度91.5%，F1分数92.0%。特征分析显示，与合法性相关的特定关键词和高风险权限（如读取手机状态）是最显著的区分特征。

Conclusion: 该系统可作为主动、可扩展的解决方案，增强宗教旅游领域的数字信任，有潜力作为国家验证系统的原型，有效应对数字化朝觐服务中的欺诈应用问题。
...
TL;DR: 提出eNeRgy-GPT (NRGPT)，将GPT架构与能量基模型框架统一，通过能量景观探索进行推理，在多种任务上表现良好且具有抗过拟合特性。


<details>
  <summary>Details</summary>
Motivation: GPT架构是当前最流行的语言建模设计，而能量基模型将推理视为在能量景观上的动态过程。作者希望将这两种范式统一起来，结合两者的优势。

Method: 对GPT设置进行最小修改，提出eNeRgy-GPT (NRGPT)模型。其推理步骤被概念化为在能量景观上对token的探索。在某些条件下，这种探索会变成梯度下降。

Result: 模型在简单语言（莎士比亚数据集）、代数ListOPS任务和更丰富的OpenWebText语言建模任务中表现良好。模型还显示出更强的抗过拟合能力，只在非常长的训练后才会出现过拟合。

Conclusion: 成功将GPT架构与能量基模型框架统一，提出的NRGPT模型在多种语言任务上表现良好，并具有更好的抗过拟合特性，为两种范式的结合提供了有前景的方向。
...
TL;DR: 该论文提出了一种基于离散测度空间上向量场理论的几何框架，用于分析复杂系统的时空数据，通过一个双参数度量族解决传统方法在处理高维非线性数据时的挑战。


<details>
  <summary>Details</summary>
Motivation: 复杂系统（如大脑、气候、经济等）具有高维非线性动力学特性，传统建模方法难以处理其时空数据的复杂性和海量性。虽然信息技术进步使数据驱动方法成为可能，但现有维度降维、相空间重构等方法在处理复杂时空数据时仍面临挑战。

Method: 提出基于离散测度空间上向量场理论的几何框架，引入双参数度量族，适用于数据分析和机器学习。该框架支持时间相关图像、图像梯度、以及定义在图和单纯复形上的实值或向量值函数。

Result: 在生物和物理系统的数值模拟数据上验证了该框架的有效性。结果表明，所提出的度量结合多维尺度分析，能有效实现维度降维、模态分解、相空间重构和吸引子表征等关键分析任务。

Conclusion: 该几何框架为理解复杂动力系统提供了稳健的途径，特别适用于传统建模不切实际但实验数据丰富的场景，为复杂系统分析提供了新的工具和方法。
...
<details>
  <summary>Details</summary>
Motivation: 现代LLM应用（如深度研究助手、编码代理和RAG系统）需要反复处理包含共享文档或代码块的长提示历史，这给KV缓存带来巨大压力。现有前缀缓存和位置无关缓存（PIC）存在限制：前缀缓存需要严格前缀匹配，PIC需要选择性重计算和位置编码调整，导致相同块的KV在不同请求中发散，内存页无法共享，HBM节省有限。

Method: MEPIC采用四种关键技术：1）将块KV对齐到分页存储；2）将重计算从token级转移到块级，只有第一个块是请求特定的；3）通过注意力内核中的RoPE融合移除位置编码；4）使剩余块完全可共享。这些技术消除了HBM中的大部分重复块KV。

Result: MEPIC在保持相当延迟和准确性的情况下，比最先进的PIC减少高达2倍的HBM使用，对于长提示可达5倍，且无需修改模型。

Conclusion: MEPIC通过创新的内存对齐、块级重计算和RoPE融合技术，实现了高效的跨位置、跨请求KV缓存重用，显著降低了LLM应用的内存压力，为处理长提示提供了实用的解决方案。
...
TL;DR: TRC是一种基于递归迭代的神经网络控制架构，通过深度迭代而非参数数量来提升容量，在保持小参数量（约150万）的同时实现接近最优的控制性能。


<details>
  <summary>Details</summary>
Motivation: 神经网络控制器参数数量激增（百万到十亿级），对于有严格功耗和延迟限制的嵌入式航空航天系统来说是不可行的。需要一种既能保持高性能又能在资源受限环境中运行的控制架构。

Method: 提出Tiny Recursive Control (TRC)架构，基于"容量可通过迭代深度而非参数数量实现"的反直觉原则。使用紧凑网络（约150万参数）通过两级分层潜在结构重复应用，通过模拟轨迹并根据跟踪误差进行修正来细化控制序列。相同权重处理每个细化步骤，增加迭代只增加计算而不增加内存。

Result: 在非线性控制问题（振荡器稳定和带燃料约束的动力下降）上评估TRC。在这些领域中，TRC实现接近最优的控制成本，同时在GPU上仅需毫秒级推理时间和不到10MB内存，比语言模型基线小两个数量级。

Conclusion: 递归推理（之前仅限于离散任务）可有效转移到连续控制合成中。TRC展示了通过迭代深度而非参数数量来扩展神经网络控制器容量的可行性，为资源受限的嵌入式系统提供了高效的解决方案。
...
TL;DR: LaMer是一个元强化学习框架，通过跨回合训练和基于反思的上下文策略适应，使LLM智能体能够在测试时主动探索环境并从反馈中学习，显著提升了在需要探索的任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 传统RL训练的LLM智能体在需要主动探索的任务中表现不佳，难以从试错经验中高效适应。需要一种方法让智能体在测试时能够主动探索环境并从反馈中学习。

Method: LaMer包含两个关键组件：1）跨回合训练框架，鼓励探索和长期奖励优化；2）基于反思的上下文策略适应，使智能体能够从任务反馈信号中适应策略而无需梯度更新。

Result: 在多个环境中实验表明，LaMer显著优于RL基线方法，在Sokoban、MineSweeper和Webshop任务上分别获得11%、14%和19%的性能提升。同时表现出对更具挑战性或未见任务的更好泛化能力。

Conclusion: 元强化学习为语言智能体中的探索提供了一种原则性方法，通过学习探索策略实现了对新环境的更鲁棒适应。LaMer框架有效解决了LLM智能体在探索性任务中的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有边缘机器学习方法多为静态模型集中训练后部署，无法有效处理未见数据。在线边缘机器学习需要在边缘设备上持续训练更新，但面临如何为未来未见数据确定标签的关键挑战。

Method: 提出知识转换(KT)方法，将知识蒸馏、主动学习和因果推理相结合。KT作为主动学习中的"预言机"，通过从教师模型转换知识来为学生模型生成伪标签，实现持续学习。

Result: 仿真实验表明：当使用相对稳定的教师模型时，学生模型最终能达到预期最大性能。KT在教师任务通用（可用预训练模型）或学生任务标签获取困难/昂贵时特别有益。

Conclusion: KT方法为在线边缘机器学习中未见数据的标签获取问题提供了有效解决方案，特别适用于教师模型任务通用或学生任务标签获取困难的场景，能够实现边缘设备的持续学习能力。
...
TL;DR: 本文提出通过智能任务排序来缓解持续学习中的灾难性遗忘问题，利用零样本评分算法确定最优任务序列，并与传统方法结合提升性能。


<details>
  <summary>Details</summary>
Motivation: 持续学习系统需要在生命周期中增量获取、更新和利用知识，但灾难性遗忘（学习新任务导致旧任务性能急剧下降）是主要挑战。大多数现有方法分为五类，但本文从不同角度考虑任务呈现顺序对缓解遗忘的影响。

Method: 提出通过任务排序来缓解灾难性遗忘的方法，利用受神经架构搜索（NAS）启发的零样本评分算法来确定最优任务序列。该方法从任务排序角度切入，而非传统的重放、正则化、优化、表示或架构方法。

Result: 结果表明智能任务排序能显著减少灾难性遗忘。当与传统持续学习策略结合时，排序方法能提供增强的性能和抗遗忘鲁棒性。此外，该方法还可应用于课程学习等其他领域。

Conclusion: 任务排序是缓解持续学习中灾难性遗忘的有效策略，与现有方法互补且能增强整体性能。该方法具有广泛的应用潜力，包括课程学习等领域。
...
TL;DR: 利用联邦学习平台对胶原VI相关肌营养不良症进行诊断，通过分布式数据集训练ML模型，实现三种致病机制的分类，性能优于单机构模型。


<details>
  <summary>Details</summary>
Motivation: 罕见病诊断面临数据稀缺和分散的挑战，跨机构数据共享存在隐私、监管和物流障碍，需要一种能保护隐私的协作学习方法。

Method: 采用Sherpa.ai联邦学习平台，在两个国际组织的分布式数据集上进行协作训练，使用患者来源成纤维细胞培养物的胶原VI免疫荧光显微镜图像。

Result: 开发的ML模型能够将胶原VI患者图像分类为三种主要致病机制组：外显子跳跃、甘氨酸替代和假外显子插入，F1分数达到0.82，优于单机构模型（0.57-0.75）。

Conclusion: 联邦学习显著提高了诊断效用和泛化能力，不仅支持更准确的诊断，还有助于解释意义未明的变异和指导测序策略优先顺序，以识别新的致病变异。
...
TL;DR: 该研究分析了基于《纽约时报》语料库训练的AI分类器中"blacks"标签的偏见问题，发现该标签部分充当"种族主义检测器"，但对现代种族议题表现不佳，揭示了新闻业采用AI工具时面临的历史偏见再现风险。


<details>
  <summary>Details</summary>
Motivation: AI模型在新闻研究和新闻编辑室应用中日益普及，但这些基于历史数据训练的模型可能编码了过时的态度和刻板印象。研究者发现基于《纽约时报》语料库训练的分类器中存在"blacks"这一有问题的主题标签，需要深入调查其偏见影响。

Method: 通过定量和定性方法分析训练语料库中"blacks"标签的使用情况，应用可解释AI技术探究该标签在分类器中编码的概念，并评估这些概念对模型使用的影响。

Result: 研究发现"blacks"标签在一定程度上充当了针对某些少数群体的"种族主义检测器"，但在处理现代案例时表现不佳，如COVID-19期间的反亚裔仇恨报道和"黑人的命也是命"运动报道。模型可能产生意外输出，影响新闻发现、受众定位、摘要等多种应用。

Conclusion: 这项案例研究揭示了AI模型中嵌入的偏见问题，暴露了新闻编辑室采用AI工作流程工具时的根本矛盾：如何在利用AI技术的同时降低在新闻报道中复制历史偏见的风险。
...
TL;DR: 提出后验行为克隆（PostBC）方法，通过建模演示者的后验分布而非精确匹配动作，确保对演示动作的覆盖，从而为RL微调提供更好的初始化，显著提升微调性能。


<details>
  <summary>Details</summary>
Motivation: 当前实践通常先在大规模演示数据上预训练策略，然后通过RL微调提升性能。虽然微调算法受到广泛关注，但如何确保预训练策略能成为RL微调的有效初始化却研究不足。本文旨在理解预训练策略如何影响微调性能，以及如何预训练策略以确保它们是有效的初始化。

Method: 提出后验行为克隆（PostBC）方法：不同于标准行为克隆（BC）直接匹配演示者的动作，PostBC训练策略来建模给定演示数据集下演示者行为的后验分布。这种方法确保了对演示动作的覆盖，同时保持了与BC相当的预训练性能。PostBC仅需标准监督学习即可实现，可与现代生成模型结合使用。

Result: 理论分析表明标准BC可能无法确保对演示动作的覆盖（这是有效RL微调的必要条件），而PostBC能确保这种覆盖。在实际机器人控制基准测试和真实世界机器人操作任务中，PostBC相比标准BC能显著提升RL微调性能。

Conclusion: 通过建模演示者的后验分布而非精确匹配动作，PostBC为RL微调提供了更好的初始化策略，在保持预训练性能的同时显著提升了微调效果，为从演示数据预训练策略提供了更有效的方法。
...
TL;DR: RLVR框架中，虚假奖励通过削减偏差降低策略熵，使模型输出更自信确定，从而提升LLM数学推理能力，而单纯的熵最小化不足够。


<details>
  <summary>Details</summary>
Motivation: 研究RLVR框架中探索-利用权衡的机制，特别是虚假奖励和熵最小化这两个看似矛盾但都能提升LLM推理性能的现象背后的原理。

Method: 分析策略熵与性能的关系，研究虚假奖励是否通过削减偏差和模型污染相互作用产生收益，提出奖励错位模型解释虚假奖励的增强效果。

Result: 虚假奖励通过削减偏差降低策略熵，使模型输出更自信确定，而单纯的熵最小化不足以提升性能；虚假奖励在非污染设置下也能增强性能。

Conclusion: 虚假奖励通过削减偏差降低策略熵的机制解释了其在RLVR中的益处，为更有效的RLVR训练提供了原理指导。
...
TL;DR: 提出BayesSum估计器，将贝叶斯积分扩展到离散域，利用高斯过程先验信息提高样本效率，比蒙特卡洛和俄罗斯轮盘方法收敛更快


<details>
  <summary>Details</summary>
Motivation: 解决离散域上难以计算的期望估计问题。现有方法（蒙特卡洛、俄罗斯轮盘）虽然一致但需要大量样本才能获得准确结果，需要更高效的估计器

Method: 提出BayesSum估计器，将贝叶斯积分扩展到离散域，通过高斯过程利用被积函数的先验信息，提高样本效率

Result: 理论上推导出比蒙特卡洛更快的收敛速度，在多种设置下经验验证需要更少样本，包括Conway-Maxwell-Poisson和Potts模型的参数估计

Conclusion: BayesSum是一种高效的离散域期望估计方法，通过利用先验信息显著减少所需样本数量，在理论和实证上都优于现有方法
...
TL;DR: ZICO方法通过零膨胀广义线性模型和可微无环约束，从零膨胀计数数据中学习网络结构，在基因调控网络重构中表现优异。


<details>
  <summary>Details</summary>
Motivation: 处理零膨胀计数数据的网络结构学习问题，特别是在基因调控网络重构等生物信息学应用中，现有方法在准确性和计算效率方面存在局限。

Method: 将每个节点建模为零膨胀广义线性模型，使用基于分数的平滑目标函数，结合可微的无环约束代理和稀疏正则化，实现完全向量化和小批量处理。

Result: 在模拟数据上获得优越性能且运行时间更快，在基因调控网络重构任务中表现与常见算法相当或更好，能够处理更大变量集。

Conclusion: ZICO方法为从零膨胀计数数据中学习网络结构提供了高效、可扩展的解决方案，适用于广泛的领域应用。
...
TL;DR: 论文提出TL-TARNet方法，通过迁移学习改进个体治疗效果估计，特别适用于小样本场景，在模拟和实证中都显示出比标准TARNet更好的性能。


<details>
  <summary>Details</summary>
Motivation: 在大规模数据集估计需要应用于较小或系统不同的环境时，因果知识的泛化具有挑战性。基于模型的个体治疗效果估计器需要大样本量，限制了在行为科学等小数据集领域的应用。

Method: 通过迁移学习扩展Treatment Agnostic Representation Networks (TARNet)，提出TL-TARNet方法，利用源数据集知识并适应新环境设置。

Result: 在模拟实验中，当存在大的无偏源数据集且目标样本较小时，TL-TARNet相比标准TARNet减少了ITE误差并减弱了偏差。在印度人类发展调查的实证应用中，迁移学习使目标平均ITE估计更接近源ITE估计，减少了无迁移时的估计偏差。

Conclusion: 因果模型的迁移学习可以改进小样本中的个体治疗效果估计，为外部有效性关键的应用场景提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 玻璃态材料（非晶材料）是无序粒子系统，缺乏原子周期性，采样其平衡构型是一个缓慢且困难的任务，需要开发能够生成具有明确定义似然的平衡构型的生成框架

Method: 采用等变黎曼随机插值框架，结合黎曼随机插值和等变流匹配，严格结合周期性边界条件和多组分粒子系统的对称性，使等变图神经网络直接在环面上操作

Result: 在模型非晶系统上的数值实验表明，强制几何和对称约束显著改善了生成性能

Conclusion: 该方法为玻璃态材料的平衡构型生成提供了有效的解决方案，通过结合几何和对称约束显著提升了生成模型的性能
...
TL;DR: 经验流匹配（FM）采样器存在隐式偏差：经验FM最小化器几乎从不产生梯度场，导致能量次优；生成样本的动能分布取决于源分布而非数据


<details>
  <summary>Details</summary>
Motivation: 研究流匹配采样器的隐式偏差，特别是经验流匹配与理论最优流之间的差异，分析采样过程中的能量特性

Method: 通过经验流匹配的视角分析流匹配采样器，研究经验FM最小化器的梯度场特性，分析生成样本的动能分布（瞬时和积分动能）

Result: 经验FM最小化器几乎从不产生梯度场，导致能量次优；高斯源分布下动能呈指数集中，重尾源分布下呈多项式尾分布；这些行为主要由源分布而非数据决定

Conclusion: 经验流匹配存在结构和能量偏差，这些偏差主要源于源分布的选择而非数据本身，为理解FM采样器的隐式偏差提供了数学框架
