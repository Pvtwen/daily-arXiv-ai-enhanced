...
Motivation: 传统基于帧的眼动追踪系统在扩展现实(XR)应用中难以满足高精度、低延迟和节能的要求，而事件摄像头提供了超高时间分辨率和低功耗的替代方案。
...
Conclusion: 所提出的图结构学习方法能有效推断主系统的卫星分配策略，为次系统提供可靠的干扰避免机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有堆叠智能表面(SIS)主要用于空间波域信号处理，但缺乏时域处理能力。通过增加可调延迟单元，可以扩展SIS的功能，使其能够处理多径引起的符号间干扰(ISI)。

Method: 在SIS中集成战略调谐的符号级延迟单元，构建延迟增强SIS(DA-SIS)。将其用作模拟均衡器，通过调整延迟参数来消除多径干扰。

Result: DA-SIS能够有效消除多径引起的符号间干扰，通过比特误码率(BER)评估显示其均衡性能。与数字均衡器相比，DA-SIS展现出良好的均衡潜力。

Conclusion: 延迟增强SIS是一个有前景的研究方向，能够扩展智能表面的功能，未来需要进一步研究以实现这一概念的实际应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有手势识别系统功耗高且仅限于离散手势分类，无法满足多日使用需求。EMG和超声融合能结合互补信息，克服单一模态限制，实现更鲁棒可靠的人机交互。

Method: 开发超低功耗可穿戴臂带系统，集成EMG和超声传感器。采用轻量级编码器-解码器架构和多任务学习，同时估计手部和腕部关节角度，使用运动捕捉手套进行地面真值标注。

Result: 在真实传感器重定位条件下，EMG-超声融合的均方根误差为10.6°±2.0°，优于单一EMG（12.0°±1°）和超声（13.1°±2.6°）；R²得分为0.61±0.1，也优于单一模态。

Conclusion: EMG和超声传感器融合能显著提高手部和腕部连续运动跟踪的精度和鲁棒性，为开发直观人机交互系统提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 地雷在冲突地区广泛使用，战后仍对平民构成持续威胁。现有无人机磁力计检测方法面临无人机电子设备磁场干扰的技术挑战。

Method: 采用框架安装的无人机双磁力计载荷，通过WAIC-UP方法消除干扰，再使用RUDE算法检测地雷信号。

Result: 该方法在蒙特卡洛模拟中验证了高保真度弹药检测，计算成本低，简化了磁测载荷设计，并评估了不同飞行高度的性能。

Conclusion: WAIC-UP/RUDE双磁力计方法能够有效解决无人机磁场干扰问题，实现高效的地雷检测。
...
<details>
  <summary>Details</summary>
Motivation: 推动集成感知与通信(ISAC)技术的发展，解决传统固定天线系统在同时进行通信和感知任务时的性能限制，利用可移动天线的灵活性提升系统性能。

Method: 采用基于梯度的元学习(GML)方法来解决大规模非凸优化问题，联合优化发射/接收波束成形向量、上行用户发射功率以及两个基站的可移动天线位置。

Result: 数值结果表明，所提出的元学习方法能够达到最优解的99%性能，且基于可移动天线的方案优于多个基准方法。

Conclusion: 基于可移动天线的全双工ISAC系统在满足通信和感知服务质量要求的同时，能够显著提升系统性能，为实际ISAC应用提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 里德堡原子传感器在射频测量中具有高灵敏度，但其光学接口的单极性特性限制了传统OFDM接收。需要采用单极性OFDM方案来实现兼容的信号传输。

Method: 建立了里德堡原子传感器的AM-AM特性经验近似函数，基于DCO-OFDM框架提出了LODC-OFDM方案，并采用Bussgang定理分析非线性失真。

Result: 推导了泰勒级数展开近似AM/AM曲线和理想预失真情况下的闭式解，实验结果表明理论与实验结果吻合良好。

Conclusion: 提出的LODC-OFDM方案有效解决了里德堡原子传感器的宽带OFDM接收问题，为兼容信号传输提供了可行方案。
...
Conclusion: 该研究扩展并完善了先前结果，为1比特量化SIMO系统的性能分析提供了更全面的理论框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有二维噪声调制方案数据率有限，需要开发更高维度的调制方案来提升通信系统的数据速率和频谱效率。

Method: 使用高斯混合分布的概率作为第三调制维度，每个符号对应三个子信道比特：均值调制第一比特、方差调制第二比特、混合概率调制第三比特。采用阈值检测器、最大似然检测器、峰度检测器和贝叶斯假设检测器进行联合检测。

Result: 仿真结果显示第三子信道比特的误码率与现有二维方案相当，数据率相比广义二次噪声调制器和经典二元KLJN噪声调制器分别提升1.5倍和3倍，峰度检测器能以较低复杂度实现约0.06的误码率。

Conclusion: 所提出的三维噪声调制方案成功扩展了调制维度，在保持较低误码率的同时显著提升了数据速率，为高维噪声调制系统提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究在Wasserstein距离约束下线性测量模型的鲁棒估计问题，该问题可表述为无限维非凸极小极大问题，需要解决鞍点存在性和鲁棒估计器的设计问题。

Method: 通过证明无限维问题与有限维问题的鞍点存在性等价，提出可验证的充要条件（参数可从凸问题及其对偶推导），并引入简化充分条件。在没有鞍点时，通过限制估计器为线性来求解有限维非凸极小极大问题。

Result: 给出了鞍点可能不存在的反例，提出了可验证的充要条件和简化充分条件（当Wasserstein半径足够小时鞍点总是存在）。在没有鞍点时，通过线性估计器获得鲁棒估计的上界和解决方案。数值实验验证了理论结果。

Conclusion: 该研究为Wasserstein距离约束下的鲁棒估计问题提供了理论框架，建立了鞍点存在性的判别条件，并提出了有效的鲁棒线性估计器设计方法。
...
Method: 通过简单叠加两个GQNM调制器的输出，创建16进制噪声调制器，在四个不同的均值和四个不同的方差上调制信息比特。通过选择满足理论可区分性条件的参数来优化性能。

Result: 仿真验证表明，与KLJN调制器和GQNM调制器相比，该方法在比特错误概率(BEP)方面获得了更好的性能。

Conclusion: 通过增加调制器、发射机和接收机检测器的复杂度，可以实现更小的比特错误概率。该方法还可扩展到高于16进制的调制方案，但需要进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多普勒定位算法基于非凸函数，计算复杂且性能受限，需要开发更高效准确的定位方法。

Method: 将ToA测量整合到LS多普勒成本函数中，形成二次凸函数，利用最小化器的线性方程和ToA测量，通过约束LS优化得到发射器位置的闭式解，并提供无人机轨迹的闭式设计。

Result: 仿真实验表明，与文献中其他算法相比，所提算法在定位性能上表现更优。

Conclusion: 该算法通过结合多普勒和ToA测量，实现了高效的凸优化定位，提供了闭式解和轨迹设计，显著提升了定位性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有工作主要关注窄带可移动天线系统，而OFDM系统中天线位置优化面临频率平坦性的挑战，需要适应不同子载波的信道条件。

Method: 将连续位置优化问题离散化为采样点选择问题，采用分支定界框架结合图论方法剪枝次优解，在低信噪比下使用简化的图论算法。

Result: 仿真结果表明，所提算法优于传统固定位置天线，窄带天线位置优化可达到接近最优性能。

Conclusion: 可移动天线技术通过优化天线位置能有效提升OFDM系统性能，提出的离散化方法和图论算法能高效解决该优化问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的数学噪声生成模型只能捕捉加性噪声的部分特征，无法全面统计非周期性异步脉冲噪声，这对提升NB-PLC收发器的脉冲噪声处理至关重要。

Method: 通过商业NB-PLC调制解调器的模拟耦合和带通滤波电路测量噪声构建真实数据集；设计NGGAN模型，使用Wasserstein距离作为损失函数，并优化输入信号长度以适应循环平稳噪声生成。

Result: 仿真结果表明，基于波形特征训练的NGGAN在生成噪声质量方面更接近实际测量数据集，优于传统的PSCGM和FRESH滤波器方法。

Conclusion: NGGAN能够有效学习实际NB-PLC噪声的复杂特性，为数据增强提供了更准确的噪声生成模型。
...
<details>
  <summary>Details</summary>
Motivation: 受Costa脏纸编码工作的启发，旨在解决多址信道中通信和计算同时进行时的干扰问题，实现更高效的集成通信与计算系统。

Method: 基于脏纸编码原理，在发射端预消除计算符号，使得离散数据符号传输和空中计算能够在共同的多址信道上同时进行，采用单输入多输出系统设置。

Result: 仿真验证表明，该方法在数据检测和函数计算的均方误差性能方面显著优于现有最先进的集成通信与计算方案，在比特错误率和均方误差方面都有明显提升。

Conclusion: 提出的"脏纸计算"方案能够有效实现通信与计算的集成，通过预消除干扰达到渐进无干扰的效果，在性能上优于现有技术。
...
<details>
  <summary>Details</summary>
Motivation: 现有干扰缓解方法需要专用训练阶段来估计干扰空间特征，这会降低通信速率，且无法应对在训练阶段不传输或动态改变波束成形的智能干扰器。

Method: 提出了JMD范式，通过联合估计和移除干扰子空间与检测合法传输数据，开发了SANDMAN和MAED两种算法，分别采用不同的信道估计方法。

Result: 广泛仿真表明JMD方法在干扰缓解方面具有显著效果，能够有效对抗智能和多天线干扰器。

Conclusion: JMD方法消除了对专用训练阶段的需求，能够缓解智能和动态多天线干扰器，为MIMO系统提供了更有效的干扰缓解解决方案。
...
<details>
  <summary>Details</summary>
Motivation: AFDM在高移动性场景中表现出优越性能，但现有系统在安全性方面存在不足，需要开发能够动态变化波形参数以增强物理层安全性的新方法。

Method: 使用长周期伪噪声序列控制码本动态生成预啁啾参数，采用参数域扩频而非数据域扩频，并提出同步框架解决时变参数在快速时变信道中的可靠快速同步问题。

Result: 理论推导证明未同步的窃听者无法消除时变参数的非线性影响，仿真结果展示了SE-AFDM在高移动性场景中的安全优势，硬件原型验证了同步框架的有效性。

Conclusion: SE-AFDM系统通过动态预啁啾参数和参数域扩频方法，在保持AFDM原有优势的同时显著增强了物理层安全性，适用于高移动性通信场景。
...
TL;DR: 本文研究使用三极化连续孔径阵列进行联合波达方向(DOA)和姿态感知，通过电磁信息理论建模连续信号，开发了等效连续-离散变换技术，并利用三极化信号的自协方差和互协方差构建三极化谱，显著提升DOA估计性能。


<details>
  <summary>Details</summary>
Motivation: 研究联合DOA和姿态感知，旨在利用三极化连续孔径阵列的电磁信息理论来准确估计目标的波达方向和姿态信息，解决传统方法在连续孔径阵列中的局限性。

Method: 采用电磁信息理论建模三极化连续孔径阵列的接收信号，开发等效连续-离散变换技术进行子空间分解，利用三极化信号的自协方差和互协方差构建三极化谱，并提出两种姿态估计算法。

Result: 数值结果表明所提框架的可行性和优越性，能够实现准确的DOA和姿态估计，其中三极化谱显著提升了DOA估计性能。

Conclusion: 提出的基于三极化连续孔径阵列的联合DOA和姿态感知框架是有效的，姿态信息的可识别性依赖于先验目标快照的可用性，两种姿态估计算法分别适用于有无先验知识的情况。
...
<details>
  <summary>Details</summary>
Motivation: 集成感知与通信(ISAC)的部署给授权感知带来了前所未有的安全漏洞，非法被动雷达感知窃听者可以从目标反射信号中提取感知信息，因此需要开发安全解决方案来保护感知数据。

Method: 采用OFDM信号，设计结构化子载波功率分配方案来塑造安全自相关函数，插入周期性峰值误导非法感知者的距离估计；引入峰值旁瓣电平和积分旁瓣电平作为感知安全性能指标；通过凸优化问题最大化ISAC性能同时保证感知安全水平。

Result: 数值结果验证了所提出的感知安全ISAC信号的有效性，能够显著降低非法感知者的目标估计性能，同时保持合法感知接收器的性能。

Conclusion: 该研究成功实现了感知安全的ISAC框架，在通信、合法感知和感知安全之间建立了三向权衡关系，为ISAC系统的安全部署提供了有效解决方案。
...
Method: 利用最优符号级预编码的闭式解结构和张量等变性，构建从问题表述到解的映射，设计基于注意力的张量等变模块，实现线性计算复杂度

Result: 仿真结果显示，所提框架能捕获最优符号级预编码的显著性能增益，同时实现约80倍加速，并在用户数和符号块长度上保持强泛化能力

Conclusion: 基于张量等变性的深度学习框架能有效解决符号级预编码的高复杂度问题，在性能和效率之间取得良好平衡，且适用于不完美CSI场景
...
<details>
  <summary>Details</summary>
Motivation: 传统基于模型的振动控制方法需要繁琐的系统辨识，而纯RL方法在训练期间可能对结构造成损害。研究发现即使基于错误模型的LQR控制器也比无控制效果好，这启发了混合控制框架的开发。

Method: 提出混合控制框架，集成LQR和RL控制器。LQR策略从随机选择的模型和参数推导，不需要真实或近似结构模型知识，保持整体框架的无模型特性。

Result: 该混合方法消除了对显式系统模型的依赖，同时最小化了朴素RL实现中固有的探索风险。

Conclusion: 这是首个解决RL基振动控制关键训练安全挑战并提供验证解决方案的研究，为无模型振动控制提供了安全有效的实现途径。
...
TL;DR: 将分子动力学模拟重新定义为时间序列预测问题，使用基于位移的轨迹预测方法，结合物理信息损失函数，实现高效且精确的原子轨迹模拟。
...
Method: 将MD模拟作为时间序列预测问题，使用基于位移的轨迹预测，结合DFT参数化的Morse势函数物理信息损失机制来确保物理合理性。

Result: 在多种材料上超越标准基线方法的模拟精度，能够在几分钟内稳定模拟数千个MD步骤，为昂贵的DFT模拟提供了可扩展的替代方案。

Conclusion: 物理知识的融入对于提高原子轨迹预测的可靠性和精度至关重要，该方法为大规模分子动力学模拟提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 离线强化学习数据集常受到对抗性污染、系统错误或低质量样本的影响，导致标准行为克隆和离线RL方法性能下降，需要一种能处理数据污染的鲁棒模仿学习方法。

Method: 使用小型干净参考集通过二元判别器估计轨迹级密度比，将密度比裁剪后作为权重用于行为克隆目标，优先学习干净专家行为，同时降低或丢弃污染数据的影响。

Result: 在连续控制基准测试中，即使在高污染率下，Weighted BC仍能保持接近最优性能，优于传统BC、BCQ和BRAC等基线方法。

Conclusion: Weighted BC提供了一种简单有效的鲁棒模仿学习方法，无需了解污染机制，具有理论保证和实际有效性，适用于安全关键应用。
...
Method: 训练一个轻量级分类器来预测采样风险，根据风险指标动态切换贪婪采样和高温度采样。

Result: 在数学推理任务上的实验表明，选择性采样在保持高温度设置的同时改善了质量-多样性权衡。

Conclusion: 选择性采样方法能够有效解决高精度任务中温度采样导致的准确性下降问题，提升输出质量与多样性的平衡。
...
Method: 采用动态token分配、零样本多智能体提取和条件表解析的LLM驱动工作流，平衡准确性与计算成本。

Result: 从10,000篇文献中提取了27,822条温度分辨性能记录，GPT-4.1准确率最高（热电性能F1=0.91，结构字段F1=0.82），GPT-4.1 Mini在成本大幅降低下性能接近。
...
Result: 在CIFAR-10上验证：1）基准测试显示准确率稳步提升；2）增加客户端重叠可提高准确率并降低攻击成功率；3）信号预算研究表明序列信念更新可缓解弱信号问题。

Conclusion: 深度Q网络在与其他控制器比较中实现了最佳的鲁棒性-准确性权衡。
...
<details>
  <summary>Details</summary>
Motivation: 准确的列车延误预测对铁路运营效率至关重要，现有研究主要关注单个列车延误预测，需要支持更高级别交通管理的车站级延误预测方法。

Method: 设计铁路中心时空图卷积网络(RSTGCN)，包含列车频率感知的空间注意力机制等架构创新和特征集成。

Result: 在印度铁路网络数据集上实验显示，相比现有基线方法在标准指标上取得一致改进。
...
<details>
  <summary>Details</summary>
Motivation: 传统剪枝方法基于参数对损失的影响（如幅度或梯度）进行剪枝，但缺乏对网络表示多样性和效率的系统性考虑。

Method: BB方法为每个单元分配本地流量预算（活动率×扇出），通过约束熵分析推导出选择性-受众平衡方程，使用简单的本地执行器剪枝扇入（降低活动率）或扇出（减少广播）。

Result: BB提高了编码熵和去相关性，在ASR的Transformer、人脸识别的ResNet和突触预测的3D U-Net中，在相同稀疏度下提升了准确率，有时甚至超过密集基线。在电子显微镜图像上，达到了最先进的F1和PR-AUC指标。
...
<details>
  <summary>Details</summary>
Motivation: 虽然先前的研究主要集中在协作性多智能体强化学习，但现实世界应用如追逃、安全和竞争性操作等场景中的对抗性交互同样重要，需要专门的框架来支持这类训练。

Method: 扩展IsaacLab框架以支持对抗性策略训练，引入具有异构智能体和不对称目标能力的对抗性MARL环境套件，集成竞争性异构智能体强化学习与近端策略优化(HAPPO)算法。

Result: 实验表明该框架能够有效建模和训练形态多样的多智能体竞争策略，同时保持高吞吐量和模拟真实性，在多个基准场景中验证了其能力。

Conclusion: 该工作成功开发了一个可扩展的对抗性MARL训练平台，为现实世界中的多智能体竞争场景提供了有效的模拟和训练解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前主流的大模型训练范式只在最后阶段引入强化学习，作者质疑这是否是最优方式，希望将强化学习的探索精神更早地融入预训练过程。

Method: 提出RLP目标，将思维链作为探索动作，奖励信号基于思维链带来的信息增益（下一个token对数似然的提升），实现无需验证器的密集奖励训练。

Result: 在Qwen3-1.7B-Base上预训练RLP，在8个数学科学基准上平均提升19%；在Nemotron-Nano-12B-v2上从42.81%提升到61.32%，科学推理平均提升23%。

Conclusion: RLP成功将强化学习重新定义为预训练目标，弥合了下一个token预测与有用思维链推理之间的差距，证明了在预训练阶段引入强化学习的有效性。
...
### [32] [Low Rank Gradients and Where to Find Them](https://arxiv.org/abs/2510.01303)
...
<details>
  <summary>Details</summary>
Motivation: 传统分析通常假设训练数据和参数是各向同性的，但实际数据往往具有复杂的结构。本文旨在理解在更现实的各向异性和病态条件下，神经网络梯度中存在的低秩结构。

Method: 采用尖峰数据模型，允许数据主体各向异性和病态，不要求数据和权重矩阵独立，同时分析均值场和神经正切核两种缩放机制。

Result: 输入权重梯度近似低秩，由两个秩一成分主导：一个与数据残差对齐，另一个与输入数据中的秩一尖峰对齐。训练数据特性、缩放机制和激活函数决定了这两个成分的平衡。

Conclusion: 标准正则化方法（如权重衰减、输入噪声和雅可比惩罚）有选择性地调节这两个梯度成分，实验验证了理论预测。
...
<details>
  <summary>Details</summary>
Motivation: 研究LAPO框架从视频数据中发现动作表示的可识别性，形式化描述这类表示的期望特性、统计优势以及潜在的不可识别性来源。

Method: 通过熵正则化的LAPO目标，在适当条件下证明能够识别满足期望特性的动作表示。

Result: 证明了熵正则化的LAPO目标能够识别满足期望特性的动作表示，这为离散动作表示在实践中表现良好提供了理论解释。

Conclusion: 熵正则化的LAPO目标在适当条件下能够识别满足期望特性的动作表示，这解释了为什么离散动作表示在实践中表现良好。
...
<details>
  <summary>Details</summary>
Motivation: 理解RNN的内部动态对于提高其可解释性和改进设计至关重要，当前缺乏有效的方法来识别网络中信息传递的关键节点。

Method: 采用信息论方法量化节点间互信息，识别信息中继节点，并在合成和真实时间序列分类任务中应用，使用LSTM和GRU等不同RNN架构，进行节点敲除实验验证功能重要性。

Result: 研究揭示了不同架构中信息中继的独特模式，展示了信息如何随时间处理和维持，节点敲除实验证实了识别节点对网络行为的重要影响。

Conclusion: 该方法不仅增强了我们对RNN复杂机制的理解，还为设计更鲁棒和可解释的神经网络提供了有价值的工具，对可解释人工智能有重要贡献。
...
<details>
  <summary>Details</summary>
Motivation: 对称感知方法（如数据增强和等变架构）假设变换后的数据点在测试分布中具有高概率或"重要性"，但这一假设需要批判性评估。

Method: 开发了一种度量数据集各向异性的方法，使用双样本神经分类器测试来区分原始数据集和随机增强版本，并在合成数据集上验证了该度量。

Result: 在多个基准点云数据集中发现了出乎意料的高度对齐现象；理论上证明分布对称性破坏会阻止不变方法达到最优性能；实证发现等变方法的效益取决于具体数据集。

Conclusion: 理解等变性需要重新思考数据中的对称性偏差，等变方法在某些各向异性数据集上仍有益处，但在其他数据集上则不然。
...
Method: 结合噪声对鲁棒的非对比监督损失(NoiSNCL)和幻影标签消歧(PLD)方案，前者对齐类内表示，后者提供保守的负监督。

Result: 在多个数据集上显著优于现有PU方法，特别是在灾后建筑损伤映射等实际应用中表现突出。
...
### [37] [Lower Bounds on Adversarial Robustness for Multiclass Classification with General Loss Functions](https://arxiv.org/abs/2510.01969)
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗鲁棒性研究主要关注0-1损失，缺乏对更广泛损失函数（如交叉熵损失）的理论分析，限制了鲁棒分类器设计在现实场景中的应用。

Method: 通过推导鲁棒风险最小化问题的对偶和重心重构形式，利用Kullback-Leibler和Tsallis熵作为惩罚项，建立了与α-公平打包问题和广义重心问题的理论联系。

Result: 获得了对抗风险的下界计算方法，在交叉熵损失函数下得到了更紧的下界，并通过数值实验验证了理论结果的有效性。

Conclusion: 该工作扩展了对抗鲁棒性理论框架，为超越0-1损失设置的鲁棒分类器设计提供了理论基础和计算工具。
...
<details>
  <summary>Details</summary>
Motivation: 受人类视觉中微眼动揭示感知动态的启发，探索类似方法检测LLM的潜在错误行为

Method: 使用轻量级位置编码扰动作为探测信号，无需微调或任务特定监督

Result: 在多个先进LLM上验证，能有效检测事实性、安全性、毒性及后门攻击等错误行为，计算效率高

Conclusion: 预训练LLM已编码检测自身错误所需的内在证据，微眼动启发的干预为检测和缓解不良行为提供了途径
...
<details>
  <summary>Details</summary>
Motivation: 单一流模型在不同性质分布上表现不一致，需要一种能适应多种后验形状的稳健变分推理方法

Method: 两阶段训练：1) 顺序专家训练(MAF、RealNVP、RBIG等流模型)；2) 基于似然的自适应全局权重估计，无需逐样本门控或架构修改

Result: 在六种典型后验分布上，AMF-VI相比单一流基线获得更低的负对数似然，在Wasserstein-2距离和最大均值差异指标上表现稳定提升

Conclusion: 自适应混合多样化流模型为跨不同后验族提供稳健变分推理的可靠途径，同时保持每个专家的归纳偏置，方法高效且架构无关
...
Method: 基于注意力稀疏性观察，识别思维链中不同重要性的思维类型，采用混合量化-逐出策略，按思维重要性分配token精度，并随着推理轨迹演化逐步逐出不重要思维的token。

Result: 在数学和编程基准测试中，ThinKV使用不到原始KV缓存5%的大小实现了接近无损的准确率，推理吞吐量比最先进基线高出5.8倍。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型虽然能从PPG信号中提取丰富生理信息，但缺乏生理可解释性，在预测能力、临床解释性和传感器设计之间存在矛盾。

Method: 引入PPGen生物物理模型关联PPG信号与可解释的生理和光学参数，并提出混合摊销推断(HAI)方法进行快速、稳健的参数估计。

Result: 在大量仿真实验中，HAI能够在各种噪声和传感器条件下准确推断生理参数。

Conclusion: PPGen为PPG建模提供了一条路径，既能保持深度学习特征所需的保真度，又能支持临床解释和硬件设计。
...
Method: 将数据分为源域和目标域，提取关键交通特征，使用目标域的小量标记数据微调模型，提出GBBW模型基于与目标域的相似性重新加权源域数据

Result: 在亚利桑那州皮马县57个异质交叉口数据上测试，相比8种最先进ML回归模型和7种基于实例的域适应方法，GBBW框架提供更准确和鲁棒的延误估计
...
<details>
  <summary>Details</summary>
Motivation: 在大规模空间融合和实体匹配任务中，精确的召回控制至关重要，因为遗漏少量真实匹配会破坏下游分析，而过度手动审查会增加成本。

Method: 使用等网格边界框过滤和CSR候选表示减少对枚举，通过确定性xxHash引导样本训练轻量级神经网络排序器，构建可重现的分数十分位分层校准集，集成四种阈值估计器并通过逆方差加权融合。

Result: 在两个真实地籍数据集（约631万和6734万对）上评估，该方法始终将召回率控制在目标范围内的小误差内，减少了相对于其他校准的冗余验证，并在单个TPU v3核心上端到端运行。
...
Method: 通过文献综述方法，分析四种主要3D重建方法：点云模型、网格模型、形状感知模型和体积模型，评估其技术基础、局限性和在不同解剖结构中的应用。

Result: 提供了从心脏到神经到肺部成像的广泛概述，分析了模型在病变解剖中的临床应用，以及训练测试数据的影响，考察了公开数据集、计算需求和评估指标。

Conclusion: 指出了新兴研究方向包括多模态集成和跨模态框架，旨在为研究人员提供结构化概述，推动深度学习向更稳健、可泛化和临床影响力的解决方案发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统SGLD无法偏向平坦最小值，而平坦最小值与深度学习的泛化能力密切相关。

Method: 使用随机权重扰动(RWP)的随机梯度Langevin动力学，通过各向同性高斯噪声扰动参数来优化随机平滑目标，隐式捕获曲率信息。

Result: 理论证明fSGLD的平稳测度集中在Hessian迹正则化损失函数的全局最小化器附近；实验在噪声标签和大规模视觉任务中显示优于或媲美基线算法，计算成本仅为SAM的一半。

Conclusion: fSGLD能有效寻找平坦最小值，提升泛化能力和鲁棒性，同时保持计算效率。
...
### [47] [Quantum-inspired Benchmark for Estimating Intrinsic Dimension](https://arxiv.org/abs/2510.01335)
*Aritra Das,Joseph T. Iosue,Victor V. Albert*

Main category: cs.LG

TL;DR: 提出量子启发的内在维度估计基准QuIIEst，包含无限族拓扑非平凡流形，用于评估IDE方法在复杂流形上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有IDE方法估计差异大，需要在比现有基准更复杂的流形上进行基准测试。

Method: 基于量子光学方法嵌入任意齐次空间，允许曲率修改和加性噪声，构建拓扑非平凡流形家族。

Result: 在QuIIEst流形上，IDE方法普遍比现有基准准确性低，且性能随非均匀曲率增加仅有最小程度下降。

Conclusion: QuIIEst基准具有固有难度，能更好评估IDE方法性能；并在分形Hofstadter蝴蝶上识别了能提取非流形空间有效维度的方法。
...
### [48] [Diffusion Transformers for Imputation: Statistical Efficiency and Uncertainty Quantification](https://arxiv.org/abs/2510.02216)
*Zeqi Ye,Minshuo Chen*

Main category: cs.LG

TL;DR: 本文研究了基于扩散变换器的时间序列插值方法的统计效率，推导了样本复杂度界限，并提出了混合掩码训练策略来提升插值性能。


<details>
  <summary>Details</summary>
Motivation: 扩散生成插值方法在时间序列数据中表现出色，但其在捕捉缺失值与观测值之间复杂时空依赖关系的理论理解仍然有限，需要研究其统计效率和不确定性量化。

Method: 基于条件扩散变换器的近似理论推导统计样本复杂度界限，构建缺失值的置信区域，并通过模拟验证理论发现，提出混合掩码训练策略。

Result: 研究发现插值效率和准确性受缺失模式显著影响，提出的混合掩码训练策略能够有效提升插值性能。

Conclusion: 扩散变换器在时间序列插值中具有统计效率优势，混合掩码训练策略能进一步提升性能，为不确定性量化提供了理论支撑。
...
### [49] [Efficiently Generating Correlated Sample Paths from Multi-step Time Series Foundation Models](https://arxiv.org/abs/2510.02224)
...
TL;DR: 提出一种基于copula的方法，能够从现有的多步时间序列基础模型中高效生成具有相关性的样本路径，相比自回归采样速度快几个数量级，并能改善样本路径质量。
...
Method: 使用copula方法从现有的多步时间序列基础模型中一次性前向生成相关的样本路径。

Result: copula方法生成相关样本路径的速度比自回归采样快几个数量级，并通过缓解误差累积现象提高了样本路径质量。

Conclusion: 基于copula的方法能够高效生成准确、相关的样本路径，解决了现有模型只能预测独立边际分布的问题，同时避免了自回归采样的高计算成本。
...
### [50] [Self-Supervised Representation Learning as Mutual Information Maximization](https://arxiv.org/abs/2510.01345)
*Akhlaqur Rahman Sabby,Yi Sui,Tongzi Wu,Jesse C. Cresswell,Ga Wu*

Main category: cs.LG

TL;DR: 本文从变分互信息下界出发，推导出两种自监督表示学习训练范式SDMI和JMI，为现有方法的架构组件提供了理论解释。


<details>
  <summary>Details</summary>
Motivation: 自监督表示学习虽然经验上成功，但其原理理解不足，现有方法中的预测器网络、停止梯度操作等组件常被视为经验性添加。

Method: 从变分互信息下界推导出两种训练范式：SDMI（需要交替优化和停止梯度）和JMI（允许联合优化和对称架构）。

Result: 证明许多现有SSRL方法是这两种范式的具体实例或近似，预测器网络和统计正则化器分别是SDMI和JMI中互信息目标的可行替代。

Conclusion: 为现有SSRL方法的不同架构组件选择提供了超越启发式便利的理论解释，统一了自监督表示学习的理论框架。
...
### [51] [Drop-Muon: Update Less, Converge Faster](https://arxiv.org/abs/2510.02239)
*Kaja Gruntkowska,Yassine Maziane,Zheng Qu,Peter Richtárik*

Main category: cs.LG

TL;DR: 本文挑战了深度学习优化中全网络更新的传统观念，提出了Drop-Muon方法，通过随机选择部分层进行更新，结合渐进式训练和非欧几里得更新，在理论和实践中都优于全网络更新。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习优化器在每个训练步骤都更新所有网络层，但作者认为这种全网络更新在理论上和实践上都可能是次优的。他们希望开发一种更高效的训练方法。

Method: 提出了Drop-Muon方法，这是一种非欧几里得随机渐进训练框架。在每个训练步骤中只更新随机选择的部分网络层，结合了渐进式训练的效率和层特定的非欧几里得更新。

Result: 在CNN实验中，Drop-Muon始终优于全网络Muon，达到相同准确度的速度提高了1.4倍。理论分析表明，除非层平滑常数满足特定关系，否则全网络更新不是最优的。

Conclusion: 研究结果表明大规模模型可以更高效地训练，挑战了全网络更新的现状，提供了一个高效且理论上有依据的替代方案。
...
### [52] [How to Combat Reactive and Dynamic Jamming Attacks with Reinforcement Learning](https://arxiv.org/abs/2510.02265)
*Yalin E. Sagduyu,Tugba Erpek,Kemal Davaslioglu,Sastry Kompella*

Main category: cs.LG

TL;DR: 使用强化学习来对抗动态反应式干扰，通过Q-learning和DQN自适应调整发射功率、调制方式和信道选择，以维持高吞吐量。
...
Method: 采用强化学习（RL）方法，使用Q-learning处理离散干扰事件状态，使用深度Q网络（DQN）处理基于接收功率的连续状态，通过不同的奖励函数和动作集来适应频谱动态。
...
### [53] [Test-Time Anchoring for Discrete Diffusion Posterior Sampling](https://arxiv.org/abs/2510.02291)
...
TL;DR: 提出锚定后验采样(APS)方法，用于预训练离散扩散基础模型的后验采样，在图像恢复任务中无需重新训练特定任务模型，在标准基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 离散扩散模型提供了统一框架来联合建模分类数据(如文本和图像)，具有更快推理、更精细控制和无需训练的原则性贝叶斯推理等优势，特别适合后验采样。但现有方法面临导数自由引导信号稀疏、连续松弛限制应用、分裂吉布斯采样器维度灾难等挑战。

Method: 引入锚定后验采样(APS)，基于两个关键创新：离散嵌入空间中的量化期望实现梯度式引导，以及自适应解码的锚定重掩码技术。

Result: 在线性和非线性逆问题的标准基准测试中，APS在离散扩散采样器中达到最先进性能，并在无需训练的风格化和文本引导编辑中展示了优势。

Conclusion: APS方法成功克服了现有离散扩散后验采样的局限性，为预训练离散扩散基础模型的后验采样提供了有效解决方案。
...
### [54] [RheOFormer: A generative transformer model for simulation of complex fluids and flows](https://arxiv.org/abs/2510.01365)
...
Method: 利用自注意力机制的生成算子学习方法，学习复杂流体流动的不同空间相互作用和特征，在复杂域中处理各种粘弹性、弹粘塑性力学问题。

Result: RheOFormer能够准确学习不同复杂流体的标量和张量非线性力学，预测其流动的时空演化，即使在有限数据集上训练也表现良好，具有强大的泛化能力和计算效率。

Conclusion: RheOFormer作为稳健的神经代理模型，能够加速预测性复杂流体模拟，推进数据驱动实验，并在广泛应用中实现实时过程优化。
...
### [55] [Diffusion Models and the Manifold Hypothesis: Log-Domain Smoothing is Geometry Adaptive](https://arxiv.org/abs/2510.02305)
*Tyler Farghly,Peter Potaptchik,Samuel Howard,George Deligiannidis,Jakiw Pidstrigach*

Main category: cs.LG

TL;DR: 该论文研究了扩散模型成功背后的机制，基于流形假设，认为扩散模型能够适应数据中的低维几何结构。通过分析分数匹配学习问题，研究隐式正则化的作用，特别是平滑分数函数对数据流形的影响。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在多个领域表现出卓越的泛化能力，但其成功机制尚未完全理解。本文旨在验证流形假设，即扩散模型能够适应数据中的低维几何结构，这是其强大性能的关键原因。

Method: 通过理论分析和实证研究，考察平滑经验分数匹配目标最小化器的作用。重点研究平滑分数函数（等价于在log-密度域中平滑）如何产生沿数据流形的切向平滑。

Result: 研究证实平滑分数函数确实会产生沿数据流形的切向平滑。此外，通过选择适当的平滑方式，可以控制扩散模型泛化所沿的流形。

Conclusion: 该工作为扩散模型成功的流形假设提供了证据，表明通过分数匹配学习问题的表述，扩散模型能够适应数据的低维几何结构，并且这种泛化行为可以通过平滑策略进行控制。
...
### [56] [Selective Underfitting in Diffusion Models](https://arxiv.org/abs/2510.01378)
*Kiwhan Song,Jaeyeon Kim,Sitan Chen,Yilun Du,Sham Kakade,Vincent Sitzmann*

Main category: cs.LG

TL;DR: 本文提出选择性欠拟合概念，认为更好的扩散模型在输入空间的某些区域更准确地近似分数函数，而在其他区域欠拟合，这对理解扩散模型的泛化能力至关重要。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在训练时学习分数函数用于生成样本，但存在一个基本问题：它们实际学习的是哪个分数？如果完全匹配经验分数会简单地复制训练数据而无法生成新样本。

Method: 引入选择性欠拟合概念，通过设计实证干预来验证这一观点，并描述这些区域的特征。

Result: 研究结果表明选择性欠拟合对于理解扩散模型至关重要，为它们的泛化和生成性能提供了新的可测试见解。

Conclusion: 选择性欠拟合是理解扩散模型泛化能力的关键机制，更好的模型在特定区域更准确地近似分数函数，在其他区域则欠拟合。
...
### [57] [Fine-Tuning Masked Diffusion for Provable Self-Correction](https://arxiv.org/abs/2510.01384)
*Jaeyeon Kim,Seunggeun Kim,Taekyun Lee,David Z. Pan,Hyeji Kim,Sham Kakade,Sitan Chen*

Main category: cs.LG

TL;DR: PRISM是一个轻量级的模型无关方法，用于提升掩码扩散模型在推理时的自校正能力，通过定义自校正损失来学习每个token的质量分数，无需强化学习或验证器。
...
Method: PRISM通过定义自校正损失来学习每个token的质量分数，这些分数与MDM在同一前向传播中计算，用于检测低质量token。

Result: PRISM在多个领域和规模上提升了MDM推理性能：数独、无条件文本（170M）以及使用LLaDA的代码生成（8B）。

Conclusion: PRISM是一个有效的、模型无关的自校正方法，能够显著提升掩码扩散模型在推理时的性能。
...
TL;DR: 提出基于Pandora's Box问题的LLM推理时优化框架，通过UCB算法自适应决定停止生成时机，在保持相同性能的同时减少15-35%的生成次数。


<details>
  <summary>Details</summary>
Motivation: LLM生成需要在输出质量和推理成本之间取得平衡，特别是在多生成场景下，需要优化停止生成决策。

Method: 将每次生成视为打开一个有成本但奖励随机的"盒子"，开发UCB风格的Pandora's Box算法，并结合Bradley-Terry变换处理跨提示的奖励缩放问题。

Result: 在AlpacaFarm和HH-RLHF数据集上的实验表明，自适应策略能在保持与Best-of-N采样相同性能的同时，平均减少15-35%的生成次数。
...
TL;DR: 提出了一种基于低秩表示和向量量化的神经压缩重建框架，通过计算效率高的低秩操作显著降低解码阶段的计算开销，消除解码器计算瓶颈，同时保持高质量图像重建。


<details>
  <summary>Details</summary>
Motivation: 当代神经压缩方法虽然实现了令人印象深刻的压缩率，但由于基于卷积的解码器在数据重建过程中的复杂性和巨大计算成本，这类技术的采用受到很大阻碍。

Method: 在带有向量量化的自编码器中融入低秩表示，通过对图像的潜在表示执行一系列计算效率高的低秩操作来高效重建数据。

Result: 该方法显著减少了神经压缩/重建解码阶段的计算开销，在保持高质量图像输出的同时，基本上消除了解码器计算瓶颈。
...
TL;DR: 本文系统综述了边缘人工智能(Edge AI)的发展历程、现状和未来方向，涵盖部署位置、处理能力、应用领域和硬件类型等多维度分类，分析了关键技术、挑战和新兴机遇。


<details>
  <summary>Details</summary>
Motivation: 边缘AI通过在网络边缘设备中嵌入智能，实现近源数据处理，提供实时处理、增强隐私和降低延迟的优势，需要系统梳理该领域的发展脉络和技术框架。

Method: 采用PRISMA指南进行系统性综述，构建包含部署位置、处理能力(如TinyML和联邦学习)、应用领域和硬件类型的多维度分类体系，分析从早期内容分发网络到现代设备智能的演进过程。

Result: 识别了边缘AI的核心使能技术包括专用硬件加速器、优化软件和通信协议，评估了资源限制、安全、模型管理、功耗和连接性等关键挑战，并指出了神经形态硬件、持续学习算法、边云协作和可信集成等新兴机遇。

Conclusion: 边缘AI正从早期内容分发向设备智能演进，面临资源、安全等多重挑战，但神经形态硬件、持续学习等新技术方向为其发展提供了广阔前景，为研究者和实践者提供了全面框架。
...
<details>
  <summary>Details</summary>
Motivation: 虽然Softmax Attention的高效替代方案已被广泛研究，但基于理论洞察的更具表达力机制（即使计算成本更高）的探索相对不足。本文旨在填补这一空白。

Method: 从非参数统计学的测试时回归视角推导出Local Linear Attention (LLA)，提出两种内存高效原语应对计算复杂度，开发FlashLLA硬件高效块状算法，并实现定制化推理内核减少内存开销。

Result: 实验验证显示LLA在测试时回归、上下文回归、关联记忆和状态跟踪任务中有效适应非平稳性，在测试时训练和上下文学习中优于强基线，并展现出在大规模模型中可扩展性和适用性的前景。

Conclusion: LLA作为一种理论驱动的新型注意力机制，在多个任务中表现出色，为大规模模型提供了有前景的可扩展解决方案。
...
TL;DR: SCOPED是一种快速通用的扩散模型OOD检测方法，通过结合雅可比迹和得分函数平方范数，将前向传播次数减少一个数量级，在保持竞争力的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有OOD检测方法计算成本高，限制了在视觉、机器人、强化学习等领域的可靠部署，需要开发更高效的检测方法。

Method: 结合雅可比迹和得分函数平方范数形成单一测试统计量，使用核密度估计估计分布内密度，仅需单次前向传播和一次雅可比向量积，利用Hutchinson迹估计器提高效率。

Result: 在四个视觉基准测试中达到竞争性或最先进的精度-召回分数，在机器人控制任务中能识别奖励函数和训练机制中的分布偏移。
...
<details>
  <summary>Details</summary>
Motivation: 研究MBPO在OpenAI Gym中表现出色但在DeepMind Control Suite中表现不佳的原因，揭示环境特定假设如何影响算法设计，并解决合成数据在模型强化学习中的失败模式。

Method: 识别MBPO失败的两个耦合问题：动态模型与奖励模型尺度不匹配导致评论家低估，以及目标表示选择不当增加模型方差。通过解决这些问题改进MBPO算法。

Result: 解决失败模式后，MBPO在7个DMC任务中的5个超越了SAC，同时保持了在OpenAI Gym中的强性能表现。

Conclusion: 研究呼吁社区开发将MDP任务和环境结构与算法失败模式联系起来的分类法，追求统一解决方案，并澄清基准选择如何影响算法泛化条件。
...
TL;DR: 该论文分析了噪声反馈对LLM偏好优化的影响，提供了在噪声条件下的泛化保证，适用于DPO、IPO、SLiC等多种偏好优化方法。
...
Method: 考虑常见的现实噪声模型（如错误标注和不确定性），分析有限步偏好优化的泛化性能，基于偏好数据分布和样本数量分析噪声率的影响。

Result: 实证验证表明，不同类型的噪声在不同噪声率下会导致泛化性能衰减，研究结果对当代LLM具有实际相关性。

Conclusion: 该研究为开发符合人类偏好的AI系统提供了重要见解，强调在噪声反馈条件下进行偏好优化的必要性。
...
TL;DR: 提出了LSPO算法，一种基于响应长度的动态采样元RLVR方法，用于提升语言模型在推理任务上的训练效果


<details>
  <summary>Details</summary>
Motivation: 受LLM过度思考现象研究的启发，希望通过动态选择训练数据来改进RLVR算法的效率

Method: LSPO算法，在每个训练步骤根据平均响应长度动态选择训练数据

Result: 在多个基础模型和数据集上验证了LSPO能持续提升学习效果，并通过消融研究提供了进一步见解

Conclusion: LSPO是一种有效的元RLVR算法，为未来研究提供了有前景的方向
...
TL;DR: 离线到在线强化学习存在不一致性问题，本文提出稳定性-可塑性原则来解释这种不一致性，强调在在线微调时应保留预训练策略或离线数据集中表现更好的知识，同时保持足够的可塑性。


<details>
  <summary>Details</summary>
Motivation: 离线到在线强化学习的经验行为高度不一致，在一种设置中有效的在线微调设计选择在另一种设置中可能完全失败，需要解释这种不一致性。

Method: 提出稳定性-可塑性原则框架，将在线微调分为三种机制，每种机制需要不同的稳定性特性。通过大规模实证研究验证该框架。

Result: 在63个案例中的45个案例中，实证结果与该框架的预测高度一致，验证了该原则的有效性。

Conclusion: 该工作为离线到在线强化学习中的设计选择提供了基于离线数据集和预训练策略相对性能的原则性框架。
...
TL;DR: 提出了一种基于大语言模型(LLM)的贝叶斯优化方法，使用LoRA微调和变分贝叶斯线性层(VBLL)来处理高维黑盒优化问题，特别是在包含不规则变量(如分类、序数变量)的场景中。


<details>
  <summary>Details</summary>
Motivation: 传统高斯过程(GP)在应对高维、包含不规则变量的黑盒优化问题时表现不佳，而神经网络代理模型虽然有所探索，但大语言模型(LLM)的强大能力尚未被充分利用。

Method: 采用LLM作为代理模型，使用LoRA进行轻量级微调，结合VBLL框架更新线性回归头的后验分布，并设计了加权集成(ENS)方法来自动选择LoRA秩等超参数。

Result: 在各种高维基准测试和真实世界分子优化任务中，所提出的(ENS-)LoRA-VBLL方法表现出色。

Conclusion: 基于LLM的代理模型结合LoRA-VBLL框架，为高维黑盒优化问题提供了一种计算轻量且性能优越的解决方案。
...
TL;DR: PEL-NAS是一种基于LLM的神经架构搜索方法，通过搜索空间分区、架构提示协同进化和零成本预测器，解决了传统HW-NAS方法搜索成本高和LLM驱动方法探索偏差的问题。


<details>
  <summary>Details</summary>
Motivation: 传统超网络方法需要多GPU天，而LLM驱动方法存在探索偏差，无法发现整个搜索空间中不同延迟范围的架构。

Method: 1) 复杂度驱动分区引擎按复杂度划分搜索空间；2) LLM驱动的架构提示协同进化算子，通过知识库更新和引导进化算法；3) 零成本预测器避免从头训练大量候选架构。

Result: 在HW-NAS-Bench上，PEL-NAS实现了更高的HV、更低的IGD，在相似准确率下延迟降低达54%，搜索成本从几天降至几分钟。

Conclusion: PEL-NAS通过搜索空间分区和协同进化策略，有效解决了LLM驱动NAS的探索偏差问题，实现了高效、低成本的硬件感知神经架构搜索。
...
TL;DR: 该论文提出了一个关于机器学习模型攻击可迁移性的基本区别：数据空间攻击可以迁移，而模型表示空间攻击不能迁移（除非表示空间几何对齐）。通过理论证明和四个实验验证了这一假设。


<details>
  <summary>Details</summary>
Motivation: 解释为什么图像对抗样本和文本越狱攻击可以在不同模型间迁移，但图像越狱攻击在视觉语言模型间无法迁移的现象。

Method: 1. 在简单网络设置中数学证明数据空间和表示空间攻击的区别；2. 构建针对图像分类器的表示空间攻击；3. 构建针对语言模型的表示空间越狱攻击；4. 构建针对视觉语言模型的数据空间攻击和表示空间攻击。

Result: 表示空间攻击在攻击目标模型时有效，但无法迁移到其他模型；数据空间攻击可以成功迁移；当视觉语言模型的潜在几何在投影后空间中对齐时，表示空间攻击也可以迁移。

Conclusion: 对抗迁移性不是所有攻击的固有属性，而是取决于其操作域——共享的数据空间与模型独特的表示空间，这对构建更鲁棒的模型具有重要启示。
...
<details>
  <summary>Details</summary>
Motivation: 随着多智能体LLM推理的快速发展，如何有效聚合多个LLM的答案成为一个基本挑战。标准多数投票方法将所有答案同等对待，未能考虑模型间的潜在异质性和相关性。

Method: 设计了两种新聚合算法：最优权重(OW)和逆惊喜流行度(ISP)，利用一阶和二阶信息来改进聚合效果。

Result: 在合成数据集、LLM微调基准测试(UltraFeedback、MMLU)和真实医疗场景(ARMMAN)上，新方法始终优于多数投票，实现了实际性能提升。

Conclusion: 新方法不仅提供了实用的性能增益，还为设计鲁棒的多智能体LLM流水线提供了概念性见解，能够缓解多数投票的固有局限性。
...
TL;DR: 提出一种端到端强化学习方法，用于ICU脓毒症休克患者的双血管升压药给药优化，通过动作空间设计提高临床可解释性和采纳度，在eICU和MIMIC数据集上实现超过15%的生存改善概率。


<details>
  <summary>Details</summary>
Motivation: 解决临床决策支持系统中强化学习应用面临的临床医生对不可行给药决策的质疑，提高强化学习在临床环境中的可操作性和接受度。

Method: 结合离线保守Q学习和新型循环建模的重放缓冲区，设计支持离散、连续和定向给药策略的动作空间，捕捉ICU时间序列数据的时序依赖性。

Result: 在eICU和MIMIC数据集上的实证结果表明，动作空间设计显著影响学习到的行为策略，实现超过15%的生存改善概率，并与既定临床协议保持一致。

Conclusion: 设计的动作空间在保持疗效的同时提高了可解释性和临床采纳度，证明了动作空间设计在临床强化学习应用中的重要性。
...
TL;DR: 提出Flock模型，通过概率节点关系等变性解决知识图谱零样本链接预测问题，在54个知识图谱上达到最先进性能
...
Result: 在诊断数据集Petals上完美解决现有模型失败的问题，在54个知识图谱的实体和关系预测任务中达到最先进性能

Conclusion: 概率节点关系等变性克服了确定性等变性的限制，Flock模型是知识图谱同构不变链接级函数的通用逼近器
...
TL;DR: 该研究开发了一个预测框架，使用FDA兽医药物不良事件报告数据来分类动物用药安全结果（死亡vs恢复），结合数据工程、机器学习和可解释AI方法，旨在提高兽医药物安全性评估。


<details>
  <summary>Details</summary>
Motivation: 确保食品生产动物中药物的安全使用对保护动物福利和人类食品安全至关重要。不良事件可能预示意外的药代动力学或毒代动力学效应，增加食品链中违规残留的风险。

Method: 使用FDA OpenFDA约128万份报告数据，通过预处理管道合并关系表并标准化不良事件。评估了多种监督模型（随机森林、CatBoost、XGBoost、ExcelFormer、大语言模型），采用欠采样和过采样处理类别不平衡，并整合了药物理化性质。

Result: 集成方法和CatBoost表现最佳，精确度、召回率和F1分数均达到0.95。基于AUM的伪标签方法提高了少数类检测能力。SHAP可解释性分析识别了与致命结果相关的生物合理性预测因子。

Conclusion: 该框架表明，结合严格的数据工程、先进机器学习和可解释AI能够实现准确、可解释的兽医安全结果预测，支持FARAD使命，加强残留风险评估，并为监管和临床决策提供信息。
...
TL;DR: CarbonX是一个基于时间序列基础模型的开源工具，用于碳强度预测和插补任务，仅需历史碳强度数据即可在全球214个电网实现零样本预测，MAPE为15.82%。


<details>
  <summary>Details</summary>
Motivation: 现有碳强度预测工具存在三个主要限制：需要电网特定的电力混合数据、依赖单独的电网特定模型难以实现全球覆盖、缺乏不确定性估计。

Method: 利用时间序列基础模型的通用性，仅使用历史碳强度数据和单一通用模型，支持零样本预测和完全微调两种模式。

Result: 在214个全球电网实现零样本预测MAPE 15.82%；在13个基准电网平均MAPE 9.59%，尾部预测MAPE 16.54%，提供95%覆盖率的预测区间；最长可预测21天；插补任务性能比统计基线高1.2-3.9倍。

Conclusion: CarbonX可在数据有限的任何电网轻松使用并提供强大性能，是实用的全球规模脱碳工具。
...
TL;DR: 本文提出了两种改进二值神经网络验证的整数规划方法：新的多分类线性目标函数和基于递归结构的有效不等式生成技术，显著提高了验证效率。


<details>
  <summary>Details</summary>
Motivation: 二值神经网络的验证问题通常被建模为整数规划问题，但传统方法存在大M约束导致的积分间隙问题，限制了验证效果。

Method: 1) 为多分类场景设计新的线性目标函数；2) 利用BNN的递归结构生成有效不等式来改进整数规划公式。

Result: 与现有IP方法相比，在有限时间内能够验证更大范围的输入扰动，提升了BNN的鲁棒性验证能力。

Conclusion: 所提出的两种技术显著改善了二值神经网络验证的整数规划公式，为BNN的鲁棒性分析提供了更有效的工具。
...
TL;DR: 提出了Round-Trip Reinforcement Learning (RTRL)框架，通过将往返一致性作为奖励信号来训练模型，显著提升了化学LLM的性能和一致性。


<details>
  <summary>Details</summary>
Motivation: 现有化学大语言模型在双向任务中存在往返不一致问题，模型学会了单向记忆而非灵活掌握，而往返一致性与主要任务性能强相关。

Method: RTRL框架使用往返转换的成功作为奖励信号训练模型，并提出迭代变体，让前向和反向映射在自改进循环中交替训练。

Result: 实验表明RTRL在监督、自监督和合成数据机制下，相比强基线显著提升了性能和一致性。

Conclusion: 往返一致性不仅是理想属性，更是可训练目标，为构建更稳健可靠的基础模型提供了新路径。
...
TL;DR: 提出了一种绕过提示防护的新攻击方法，该方法能够持续越狱生产模型，同时保持响应质量，揭示了轻量级提示防护在现代LLM架构中的固有攻击面。
...
TL;DR: 提出了一个将Python算法编译为GPU可运行二进制文件的框架，用于6G网络中AI/ML模型的集成，并在PUSCH接收机中通过CNN进行信道估计验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 6G网络向AI原生无线系统转变，需要将数字信号处理和机器学习无缝集成到蜂窝网络软件栈中，使现代网络生命周期更接近AI系统。

Method: 开发了一个鲁棒框架，将基于Python的算法编译成GPU可运行的二进制文件，采用统一方法确保在NVIDIA GPU上的效率、灵活性和最高性能。

Result: 在NVIDIA AI Aerial平台上实现了该方法，通过CNN在数字孪生和实时测试平台中成功执行PUSCH接收机的信道估计功能。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统时间序列预测中需要大量人工预处理、验证和集成的问题，以及现有模型泛化能力差、依赖特定领域的问题，需要一种通用、领域无关且最小化人工干预的框架。

Method: 构建包含四个专业代理的框架：Curator进行LLM引导的数据诊断和预处理；Planner通过多模态诊断和自规划缩小模型选择空间；Forecaster执行模型拟合、验证并自适应选择最佳配置和集成策略；Reporter生成全面透明的报告。

Result: 在8个基准测试中，TSci持续优于统计和基于LLM的基线方法，平均分别降低预测误差10.4%和38.2%，并生成清晰严谨的报告。

Conclusion: TSci将预测工作流程转变为可解释、可扩展的白盒系统，显著提高了预测性能和透明度，为通用时间序列预测提供了有效的解决方案。
...
TL;DR: 本文提出了可执行反事实框架，通过代码和数学问题评估LLM的反事实推理能力，发现从干预推理到反事实推理时SOTA模型准确率下降25-40%，并证明强化学习能有效提升泛化能力。
...
Method: 提出可执行反事实框架，通过代码和数学问题操作化因果推理。构建包含if-else条件的反事实代码问题训练集，并在不同代码结构（如while循环）和数学问题上测试泛化能力。比较监督微调和强化学习的效果。

Result: 从干预推理到反事实推理时，o4-mini和Claude-4-Sonnet等SOTA模型准确率下降25-40%。监督微调虽提升域内性能但损害OOD任务，而强化学习在代码和数学问题上均实现1.5-2倍提升，并能泛化到新领域。

Conclusion: 强化学习能诱导核心认知行为并实现跨领域泛化，为提升LLM反事实推理能力提供了有前景的途径。可执行反事实框架为评估和改进LLM推理能力创建了新前沿。
...
TL;DR: 提出了预测性偏好学习（PPL）方法，通过将人类干预信号传播到未来状态来改进交互式模仿学习，提高学习效率和安全性。


<details>
  <summary>Details</summary>
Motivation: 现有交互式模仿学习方法只纠正当前状态的动作，不调整未来可能更危险状态的动作，需要更有效利用人类干预信号。

Method: PPL将每次人类干预引导到L个未来时间步（偏好视野），假设代理遵循相同动作且人类进行相同干预，通过偏好优化在预期探索的安全关键区域传播专家修正。

Result: 在自动驾驶和机器人操作基准测试中验证了方法的效率和通用性，显著提高了学习效率并减少了所需的人类演示。

Conclusion: 选择合适的偏好视野L可以在风险状态覆盖和标签正确性之间取得平衡，从而限制算法最优性差距，为交互式模仿学习提供了更有效的方法。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型生成的图像通常无法满足用户特定的标量奖励标准（如美学评分），而现有的推理时对齐方法存在奖励黑客问题，即模型产生高分但偏离原始提示的图像。

Method: 提出MIRA方法，引入基于分数的图像空间KL代理正则化，约束采样轨迹以防止奖励黑客，同时推导出使用扩散分数的可处理KL近似。

Result: 在SDv1.5和SDXL模型、多种奖励函数和公共数据集上，MIRA相对于强基线实现了超过60%的胜率，同时保持提示一致性。

Conclusion: MIRA有效解决了推理时对齐中的奖励黑客问题，无需微调即可提升奖励分数，并扩展到不可微分奖励场景。
...
TL;DR: 本文分析了RLHF中KL正则化的不同实现方式，建立了统一框架证明'k1 in reward'是Reverse KL正则化的理论正确实现，而'k3 as loss'只是有偏的一阶近似。


<details>
  <summary>Details</summary>
Motivation: 现有RLHF方法中KL正则化的实现存在混淆，有些方法将KL项作为数值估计而非优化损失，这忽视了其功能性作用。需要建立统一框架来分析不同实现方式的理论基础。

Method: 建立统一框架连接两种实现风格：'kn in reward'（作为奖励系数）和'kn as loss'（作为直接损失函数）。通过梯度等效性分析证明不同实现的理论等价性。

Result: 证明在on-policy条件下，'k2 as loss'与'k1 in reward'梯度等效，都是RKL目标的正确实现。而'k3 as loss'只是有偏的一阶近似。同时指出off-policy实现存在偏差问题并提出修正方法。
...
TL;DR: 提出IBCD框架，利用干预数据通过贝叶斯方法进行因果发现，能处理大规模任务并量化不确定性
...
Method: 使用经验贝叶斯框架，对总因果效应矩阵建模而非完整数据矩阵，采用spike-and-slab horseshoe先验，从观测数据学习网络结构权重，将边作为潜变量

Result: 在模拟实验中优于现有基线方法，在521个基因的CRISPR扰动数据上成功识别出稳健的图结构

Conclusion: IBCD框架在因果发现中表现出优越性能，能够处理大规模任务并提供不确定性量化
...
TL;DR: TetriServe提出了一种针对Diffusion Transformer模型的服务系统，通过步级序列并行和轮次调度机制，在保证图像质量的同时显著提高了服务级别目标的达成率。
...
Method: 提出步级序列并行策略，根据请求的截止时间动态调整并行度；引入基于轮次的调度机制，包括时间离散化、步级并行度适配和请求联合打包。
...
TL;DR: 本研究探讨了将强化学习与蛋白质语言模型结合在蛋白质设计中的效果，发现在奖励准确、策略容量充足且任务有改进空间时，强化学习能显著提升性能，但当奖励噪声大或容量受限时，增益会饱和。


<details>
  <summary>Details</summary>
Motivation: 探索强化学习是否能推动蛋白质语言模型超越其预训练先验，揭示潜在的序列-结构-功能规则，并评估强化学习在蛋白质设计中的采样效率和能力提升。

Method: 在四个蛋白质设计领域（抗菌肽设计、激酶变体优化、抗体工程和逆折叠）中，将多种强化学习算法与不同类别的蛋白质语言模型配对，分析RL对采样效率和监督学习未捕获能力的提升。

Result: 强化学习在所有基准测试中一致提高了成功率和采样效率。性能提升取决于三个因素的交互作用：任务改进空间、奖励保真度和策略容量。当奖励准确且信息丰富、策略容量充足且任务有超越监督学习基线的空间时，改进会按比例扩大；当奖励噪声大或容量受限时，尽管进行了探索，增益也会饱和。

Conclusion: 为蛋白质设计中的强化学习提供了实用指导：在扩展策略规模之前优先考虑奖励建模和校准，根据任务难度匹配算法和正则化强度，并将容量分配到边际收益最大的地方。
...
TL;DR: SPAMP是一个统一的梯度整形框架，通过统计感知的逐层自适应调制和投影，将传统硬阈值梯度裁剪推广为平滑的梯度整形方法。
...
Result: 在图像和语言任务上的大量实验表明，SPAMP在稳定性、收敛性和鲁棒性方面优于现有方法。

Conclusion: SPAMP提供了一个原则性的替代方案，取代了传统的刚性启发式方法，通过统计感知的梯度整形提高了深度学习训练的稳定性和性能。
...
<details>
  <summary>Details</summary>
Motivation: 帕金森病远程监测中的三种噪声（患者测量不准确、环境噪声、数据传输丢失）导致UPDRS评分预测误差增加，需要提高模型的噪声鲁棒性。

Method: 1. 基于选定特征的连续值将原始语音特征分组为有序区间；2. 使用对比对训练多层感知机编码器生成噪声鲁棒特征；3. 将鲁棒特征与原始特征拼接作为增强特征输入UPDRS预测模型。

Result: 广泛的实验表明，NoRo能够成功增强各种下游预测模型在不同噪声环境下的UPDRS预测噪声鲁棒性。

Conclusion: NoRo框架通过对比学习和特征增强有效解决了帕金森病远程监测中的噪声问题，提高了UPDRS评分预测的鲁棒性。
...
TL;DR: 提出基于STT-MTJ的自旋电子真随机数生成器，用于解决生成式AI中伪随机数生成器的安全漏洞，显著减少不安全输出并实现高吞吐量。
...
Method: 使用自旋转移矩磁隧道结(STT-MTJs)生成硬件真随机比特，构建高度并行的FPGA辅助计算系统，并将硬件随机比特集成到生成对抗网络中。

Result: 系统提供兆比特/秒的真随机数，通过NIST随机性测试；在CIFAR-10训练的GAN中，相比低质量随机数生成器基线，不安全输出减少高达18.6倍。

Conclusion: 基于STT-MTJ的系统具有纳秒级切换速度、高能效和可扩展性，有望扩展到超过10^6个并行单元，实现千兆比特/秒吞吐量，适合大型语言模型采样，突显自旋电子RNG作为下一代GAI系统实用安全组件的潜力。
...
TL;DR: 从统计物理学角度研究VAE中的后验坍塌现象，发现这是一种由数据结构和模型超参数共同控制的相变，存在临界超参数阈值区分有意义的潜在推断与坍塌状态。


<details>
  <summary>Details</summary>
Motivation: 深入理解变分自编码器中后验坍塌现象的本质，揭示其并非简单的优化失败，而是数据结构和变分约束相互作用下产生的相变现象。

Method: 通过分析后验坍塌相关平凡解的稳定性，识别临界超参数阈值，并在合成和真实数据集上验证这种临界行为。

Result: 发现了表征近似后验与先验分布之间KL散度不连续性的临界边界，确认了相变的存在，后验坍塌是数据结构和变分约束相互作用下产生的相变。

Conclusion: 后验坍塌是一种相变现象，这一视角为深度生成模型的可训练性和表示能力提供了新的理解。
...
TL;DR: 当前LLM推理后训练采用SFT和RL两阶段独立训练，但研究发现高SFT分数不一定带来RL性能提升，甚至可能更差。作者提出泛化损失和Pass@large k作为更好的RL结果预测指标。


<details>
  <summary>Details</summary>
Motivation: 挑战当前SFT和RL两阶段训练方法的有效性，质疑高SFT分数是否能可靠预测RL性能提升，并寻找更有效的预测指标。

Method: 训练了数百个12B参数模型，使用SFT和GRPO RLVR方法，在7个数学基准上进行了多达256次重复评估，消耗超过100万GPU小时。

Result: 发现泛化损失和Pass@large k比直接使用预RL性能预测RL结果更准确，R²和Spearman相关系数提升高达0.5(2倍)。短样本训练可能获得更好SFT表现但RL结果更差。

Conclusion: SFT分数不是RL性能的可靠预测指标，泛化损失和Pass@large k是更好的替代指标，对实际应用具有重要价值。
...
TL;DR: 大规模实证研究表明：单独使用改写合成数据预训练效果不优于自然网络文本；但1/3改写合成数据与2/3自然文本混合可加速5-10倍；纯教科书风格合成数据在小型数据预算下表现较差；最佳合成数据比例约30%；8B参数生成器效果不输更大模型；改写合成数据未见模型崩溃，但教科书风格数据出现模型崩溃迹象。


<details>
  <summary>Details</summary>
Motivation: 高质量训练数据对LLM扩展至关重要但供应有限，合成数据技术可能突破这一限制，需要实证验证其实际效果和最佳使用方式。

Method: 采用统一协议和扩展法则进行大规模实证研究（>1000个LLM，>10万GPU小时），比较自然网络数据、多种合成数据类型（改写文本、生成教科书）及其混合数据的效果。

Result: 改写合成数据单独使用效果不佳，但与自然文本混合可显著加速训练；教科书风格合成数据在小型数据预算下表现差；最佳合成数据比例约30%；8B参数生成器效果足够；改写合成数据未见模型崩溃，教科书风格数据出现模型崩溃。

Conclusion: 合成数据在预训练中具有条件性优势，需要谨慎选择类型和比例，改写合成数据与自然文本混合使用效果最佳，为实际应用提供了实用指导。
...
TL;DR: 提出了曲率自适应Transformer（CAT），通过轻量级可微分门控机制动态学习每个token在三种几何注意力分支之间的路由，解决了固定几何方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer隐含假设欧几里得几何，限制了在非欧几里得结构数据上的有效性。现有方法需要预先选择单一几何，当数据具有混合几何特性时缺乏灵活性。

Method: CAT使用可微分门控机制动态路由每个token到适当的曲率分支，每个分支采用针对各自流形优化的几何特定操作。

Result: 在知识图谱补全基准测试（FB15k-237, WN18RR）上，CAT相比固定几何基线在MRR和Hits@10指标上提升约10%，参数仅增加5%，推理时间相当。

Conclusion: 学习几何自适应优于任何单一固定几何，CAT为语言、视觉和多模态领域的几何混合架构提供了可扩展和可解释的基础。
...
TL;DR: 提出了一种基于组合模式的水印框架，用于检测和定位水印LLM输出中的后生成编辑，通过词汇表分区和组合模式嵌入水印，并设计轻量级统计方法进行编辑检测和定位。


<details>
  <summary>Details</summary>
Motivation: 在现实场景中，LLM生成的内容可能经过后生成编辑（如人工修订或欺骗攻击），因此需要检测和定位这些修改，以保护水印技术的有效性。

Method: 采用组合模式水印框架，将词汇表划分为不相交子集，在生成过程中通过强制确定性组合模式嵌入水印，并设计全局统计量检测水印，以及轻量级局部统计量标记和定位编辑。

Result: 在开源LLM上针对多种编辑场景进行评估，展示了在编辑定位方面的强大实证性能。

Conclusion: 提出的组合水印方法能有效检测和定位水印LLM输出中的后生成编辑，为保护水印技术提供了实用解决方案。
...
TL;DR: 提出了一种新的支持基分解框架，用于在超越有界条目限制的情况下高效近似注意力机制，解决了软注意力二次复杂度的瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 软注意力的二次复杂度是扩展大语言模型的主要瓶颈。现有子二次注意力近似算法只在有界条目假设下有效，但该假设在实际应用中很少成立，限制了其在现代LLMs中的适用性。

Method: 引入支持基分解框架，利用查询和键矩阵条目呈现亚高斯行为的经验发现，将大条目和小条目分离，在稀疏组件上精确计算，在密集组件上使用多项式近似。

Result: 建立了严格的理论保证，证明了子二次运行时间，并将方法扩展到多阈值设置，消除了所有分布假设。

Conclusion: 该方法为多项式注意力的经验成功提供了首个理论证明，表明软注意力可以通过多个带草图的多项式注意力组合来紧密近似。
...
Method: 结合源预训练模型和大规模视觉语言模型，使用频率感知提示技术处理域偏移，不确定性加权策略解决噪声伪标签问题，KLDA方法缓解灾难性遗忘。

Result: 严格的数值研究表明，该方法显著优于现有需要访问源域样本的方法。

Conclusion: REFEREE方法在无源跨域持续学习场景下表现优异，克服了域偏移、噪声伪标签和灾难性遗忘等问题。
...
TL;DR: 提出了一种名为Elsa的神经网络剪枝方法，能够在保持高模型精度的同时实现高达90%的极端稀疏度，突破了传统方法50-60%稀疏度的瓶颈。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络剪枝方法在大型语言模型中无法超越中等稀疏度水平（50-60%）而不严重损害模型精度，这限制了模型压缩的潜力。

Method: Elsa方法通过识别当前实践中基于替代目标公式的局限性，采用基于ADMM的标准约束优化技术直接解决这些问题。还提出了量化变体Elsa-L以扩展到极大模型。

Result: 在广泛的模型和规模实验中，Elsa相比现有方法取得显著改进：在LLaMA-2-7B模型90%稀疏度下，困惑度比最佳现有方法低7.8倍。Elsa-L可扩展到27B模型并具有理论收敛保证。

Conclusion: 这项工作在推进LLM稀疏度前沿方面取得了有意义的进展，表明在迄今为止探索有限的方向上仍存在显著进一步发展的机会。
...
TL;DR: AsyPPO是一种用于大语言模型强化学习的创新架构，通过使用轻量级mini-critics集合来恢复critic的作用，同时保持在大模型设置中的效率。


<details>
  <summary>Details</summary>
Motivation: 传统RL4LLM方法避免使用显式critic，因为传统价值函数在LLM规模上计算成本高，且在稀疏奖励和长推理视野下经常失败。

Method: 使用在不相交提示分片上训练的轻量级mini-critics集合，利用critic间不确定性来优化策略更新：在critic一致时屏蔽优势，从熵正则化中过滤高分歧状态。

Result: 在仅有5000个样本的开源数据上训练后，AsyPPO在多个基准测试中持续提升学习稳定性和性能，在Qwen3-4b-Base上比经典PPO提升超过6%，在Qwen3-8b-Base和Qwen3-14b-Base上提升约3%。

Conclusion: 这些结果强调了架构创新对于可扩展、高效算法的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 旨在学习强健的时间序列表示，通过在嵌入空间中有效平衡均匀性和容忍度特性，以更好地捕捉时间依赖关系。

Method: 采用层次化设置学习实例级和时间信息，集成温度调度器平衡对比损失，并使用层次化角度边界损失增强正负样本对的几何分离。

Result: 在128个UCR和30个UAE数据集上的分类任务中显著优于现有方法，在异常检测任务中获得有竞争力的结果。

Conclusion: TimeHUT通过层次化均匀性-容忍度平衡有效提升了时间序列表示学习性能，在分类任务中表现优异，异常检测结果也具有竞争力。
...
TL;DR: 提出了ShapKAN框架，使用Shapley值评估KAN网络中节点重要性，解决了传统基于幅度的剪枝方法对输入坐标偏移敏感的问题，实现了有效的网络压缩。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络虽然预测精度高，但黑盒特性难以理解特征-结果关系。KAN网络虽能恢复符号表示，但其独特架构使传统剪枝方法不可靠，特别是对输入坐标偏移敏感。

Method: 提出ShapKAN剪枝框架，利用Shapley值归因来评估节点重要性。该方法以平移不变的方式量化每个节点的实际贡献，确保重要性排名不受输入参数化影响。
...
TL;DR: 提出了一种自适应门控融合网络（AGFN），通过基于信息熵和模态重要性的双门融合机制自适应调整特征权重，有效处理多模态情感分析中模态质量变化的问题。
...
Method: 使用双门融合机制，基于信息熵和模态重要性自适应调整特征权重，减少噪声模态的影响并优先考虑信息丰富的线索。

Result: 在CMU-MOSI和CMU-MOSEI数据集上的实验表明，AGFN在准确率上显著优于强基线，能有效辨别细微情感并具有鲁棒性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决分类优化问题中由于组合性质导致数据覆盖不足的挑战，企业缺乏先验选择模型知识，需要基于历史客户选择数据确定最优分类。

Method: 引入悲观分类优化(PASTA)框架，利用悲观原则，仅要求离线数据分布包含最优分类而非覆盖所有可行分类。

Result: 为多项logit和嵌套logit等广泛使用的选择模型建立了首个有限样本遗憾界，证明了PASTA在样本和模型复杂度方面是最小最大最优的。

Conclusion: PASTA框架在数据覆盖不足的情况下仍能有效优化分类，数值实验表明其性能优于现有基线方法。
...
TL;DR: 提出了分层最优传输(HOT)框架，用于神经网络表示相似性分析，解决了传统方法的不对称性、缺乏全局对齐分数以及网络深度不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 传统表示相似性方法存在三个主要问题：独立对齐每层导致不对称结果、缺乏全局对齐分数、难以处理不同深度的网络。这些问题源于忽略了全局激活结构和限制映射为刚性的一对一层对应关系。

Method: HOT框架联合推断软性的、全局一致的层到层耦合和神经元级传输计划。允许源神经元在多个目标层之间分布质量，同时在边际约束下最小化总传输成本。

Result: 在视觉模型、大语言模型和人类视觉皮层记录上的评估显示，HOT在对齐质量上匹配或超越标准成对匹配方法。揭示了平滑、细粒度的分层对应关系：早期层映射到早期层，深层保持相对位置，深度不匹配通过跨多个层分布表示来解决。

Conclusion: HOT能够在网络架构或深度不同时实现更丰富、更可解释的表示比较，这些结构化模式从全局优化中自然涌现，而在贪心的逐层方法中缺失。
...
TL;DR: ActiNet模型（自监督ResNet-V2+HMM）在手腕加速度计数据的人类活动识别中表现优于传统随机森林+HMM方法，F1分数达0.82，kappa分数0.86，适用于大规模流行病学研究。


<details>
  <summary>Details</summary>
Motivation: 提高手腕加速度计数据的人类活动识别准确性，以支持大规模流行病学研究，探索自监督学习结合隐马尔可夫模型是否能显著改善分类性能。

Method: 使用151名CAPTURE-24参与者的数据，训练18层自监督ResNet-V2模型（ActiNet），结合隐马尔可夫模型平滑处理，通过5折分层组交叉验证评估性能。

Result: ActiNet模型平均宏F1分数0.82，Cohen's kappa分数0.86，显著优于随机森林+HMM的0.77和0.81，结果在不同年龄和性别亚组中一致。

Conclusion: ActiNet模型在手腕加速度计数据中提取活动强度标签方面表现优异，建议在未来的流行病学研究中推广应用。
...
TL;DR: 本文研究了无人机辅助的联邦多模态学习框架，通过联合优化无人机感知调度、功率控制、轨迹规划、资源分配和基站资源管理来最小化系统延迟，并提供了收敛性分析。


<details>
  <summary>Details</summary>
Motivation: 利用无人机进行多模态数据收集和模型训练，克服单模态系统的局限性，提高模型精度和泛化能力，同时优化联邦多模态学习系统的延迟性能。

Method: 提出了一种结合块坐标下降和逐次凸逼近技术的高效迭代优化算法，解决延迟最小化问题的计算复杂性，并提供非凸损失函数下的理论收敛分析。
...
<details>
  <summary>Details</summary>
Motivation: 注意力是大型语言模型和视觉语言模型的核心操作，现有优化方法如FlashAttention主要关注系统层面的I/O优化，缺乏数学保证的算法级加速方法。

Method: 基于基础分解(BD)的矩阵恒等式，将多头投影重新组织为紧凑形式，无需重新训练，仅需4秒离线准备即可实现无损加速。

Result: 在DeepSeek-V2-Lite (16B, FP16)上，BDA实现了32%的键值投影加速和25%的权重减小，端到端困惑度仅增加0.02%(FP16)或0.0004%(FP32)，对模型性能影响可忽略。
...
TL;DR: 提出了首个使用线性函数逼近的鲁棒TD学习算法，解决了分布鲁棒强化学习中函数逼近的收敛性问题，无需生成模型访问，样本复杂度为Õ(1/ε²)。


<details>
  <summary>Details</summary>
Motivation: 现有鲁棒TD学习的收敛保证仅限于表格MDP或依赖限制性折扣因子假设，无法满足实际应用中函数逼近的需求。

Method: 结合双时间尺度随机逼近更新和外循环目标网络更新，支持总变差距离和Wasserstein-l距离不确定性集。

Result: 实现了ε-精确值估计的Õ(1/ε²)样本复杂度，填补了鲁棒RL算法经验成功与非鲁棒对应物理论保证之间的关键差距。

Conclusion: 该算法成功将鲁棒TD学习扩展到线性函数逼近场景，关键思想也可相对直接地扩展到鲁棒Q学习。
...
<details>
  <summary>Details</summary>
Motivation: 传统离散选择模型在准确反映个人决策过程方面面临挑战，需要更有效的方法来建模工作地点选择决策。

Method: 使用深度神经网络方法建模工作地点选择，并与传统离散选择模型进行对比分析。

Result: DNN在工作地点选择建模中显示出作为DCM稳健替代方案的显著潜力。DNN在某些方面优于DCM，而DCM在评估个体属性对工作距离影响时与数据更吻合。DCM在短距离表现更优，DNN在长距离与数据和DCM表现相当。

Conclusion: 在工作地点选择分析中，应根据具体应用需求选择合适的模型，两种模型各有优势。
...
TL;DR: 该研究分析了差分隐私随机梯度下降(DPSGD)对模型公平性的影响，发现直接优化差分隐私模型的超参数虽然不能可靠缓解DPSGD的差异性影响，但相比重用非隐私模型的超参数能改善效用-公平性权衡。


<details>
  <summary>Details</summary>
Motivation: 研究动机是分析DPSGD对模型公平性的影响，验证直接优化差分隐私模型超参数是否能达到与非隐私模型相当的公平性水平，并评估这种方法的泛化性。

Method: 方法包括：1)比较DPSGD对不同性能指标的差异性影响；2)在广泛的超参数设置下分析这种影响；3)扩展到DPSGD-Global-Adapt变体。

Result: 结果显示：不同指标的差异性影响不一定一致；直接优化差分隐私模型超参数不能可靠缓解DPSGD的差异性影响，但能改善效用-公平性权衡；DPSGD-Global-Adapt对超参数选择不稳健。

Conclusion: 结论强调：超参数调优会带来额外隐私泄露，需要谨慎平衡隐私、效用和公平性；直接优化差分隐私模型超参数能改善权衡但非可靠解决方案。
...
Method: 收集现有代码并将其统一到一个通用框架中，通过这种统一视角来系统比较不同方法。

Result: 通过统一框架能够系统性地比较各种方法，突出它们的优势和局限性。

Conclusion: 这项工作为理解不同学习正则化方法的性能提供了有价值的见解，并指出了它们未来的发展潜力，同时提供了实用的指导方针。
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习研究需要多样、具有挑战性且可扩展的环境，而现代视频游戏计算成本高且受CPU限制，不适合大规模实验。

Method: 基于CHIP-8模拟器在JAX中实现经典街机游戏环境，提供端到端的GPU加速替代方案，支持图像环境并涵盖解谜、动作和策略等多种游戏类型。

Result: JAX实现相比传统CPU模拟器实现数量级加速，同时保持原始游戏机制完美保真度，在多个游戏中训练RL代理显示出显著的训练速度和可扩展性改进。

Conclusion: Octax为大规模强化学习实验提供了理想的平台，其模块化设计便于研究人员扩展新游戏或使用大型语言模型生成新环境。
...
TL;DR: 提出两种训练策略解决学习非规范哈密顿动力学时的数值不稳定问题，通过直接学习向量场或通过方案学习时间离散动力学
...
Result: 多个数值测试案例验证了方法学习复杂物理动力学的能力，如陀螺动力学等离子体物理中的引导中心

Conclusion: 识别并解决了学习非规范哈密顿动力学时的数值不稳定问题，提出了有效的训练策略
...
<details>
  <summary>Details</summary>
Motivation: 评估后处理隐私过滤技术在医学AI研究中生成隐私保护合成数据集的有效性，因为其效果尚未得到充分验证。

Method: 对胸部X光合成应用过滤管道进行严格评估，分析过滤器在检测真实图像和训练数据生成的近似重复样本方面的表现。

Result: 当前过滤器特异性有限且不一致，仅对真实图像具有高敏感性，但无法可靠检测训练数据生成的近似重复样本。
...
TL;DR: 提出宽-窄-宽（沙漏）MLP块，将跳跃连接置于扩展维度而非传统窄-宽-窄设计，在保持计算效率的同时实现更优性能参数帕累托前沿。


<details>
  <summary>Details</summary>
Motivation: 挑战传统MLP的窄-宽-窄设计范式，探索跳跃连接在扩展维度而非输入/输出维度操作的新架构。

Method: 设计沙漏MLP块，初始投影将输入提升到扩展维度，跳跃连接在宽维度操作，残差计算通过窄瓶颈进行，投影可保持随机初始化不变。

Result: 沙漏架构在图像生成任务上始终优于传统设计，随着参数增加，最优配置趋向更深网络、更宽跳跃连接和更窄瓶颈。

Conclusion: 重新考虑现代架构中跳跃连接的位置，这种设计理念可扩展到Transformer和其他残差网络。
...
TL;DR: Sparse Query Attention (SQA) 是一种新型注意力架构，通过减少查询头数量来降低注意力机制的计算复杂度，在长序列处理中可实现高达3倍的吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 解决多头注意力机制在长序列处理中的二次计算复杂度问题，现有方法如MQA和GQA虽然解决了内存带宽瓶颈，但未能减少注意力分数计算所需的FLOPs。

Method: 提出SQA架构，减少查询头数量而非键/值头，直接降低注意力机制的计算复杂度，比例与查询头减少量成正比。

Result: 在32k-200k长序列基准测试中，SQA在计算受限场景（预训练、微调、编码器任务）中可实现高达3倍的吞吐量提升，且对模型质量影响极小。

Conclusion: SQA是一种有效的注意力优化方法，可作为构建更高效、可扩展模型的有力工具，在Reactive Transformer架构开发过程中偶然发现。
...
TL;DR: 提出一种顺序不变强化学习框架用于黑盒组合优化，通过随机生成顺序训练多变量自回归生成模型，提高样本效率和搜索空间多样性。


<details>
  <summary>Details</summary>
Motivation: 传统分布估计算法依赖学习显式变量依赖图，成本高且难以有效捕捉复杂交互。需要更高效的方法来处理组合优化问题。

Method: 使用多变量自回归生成模型，在训练时随机采样生成顺序（信息保留dropout），使模型对变量顺序不变。采用广义强化策略优化(GRPO)提供稳定的策略梯度更新。
...
TL;DR: 该论文探索了AutoML与预选模型选择的交叉领域，通过传统模型和大型语言模型(LLM)代理来减少AutoML库的搜索空间，从而降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有的AutoML方法通常依赖耗尽的超参数搜索，而预选预测作为一种有前景的替代方案，能够通过智能预选模型来绕过这种搜索，但目前在文献中尚未得到充分探索。

Method: 利用数据集描述和统计信息，结合传统模型和LLM代理来减少AutoML搜索空间，并在AWS AutoGluon投资组合数据集上进行应用。

Result: 该方法显著减少了计算开销，同时仍能为给定数据集选择最佳模型。

Conclusion: 提出的方法为AutoML工作流程提供了转变，在保持模型选择质量的同时大幅降低了计算成本。
...
TL;DR: CNML是一种新颖的对比学习方法，将模型检查任务作为指导信号，在共享潜在空间中联合嵌入逻辑规范和系统表示，显著优于算法和神经基线。
...
Result: 在行业启发的检索任务中，CNML在跨模态和模态内设置下都显著优于算法和神经基线，学习到的表示能有效迁移到下游任务并泛化到更复杂的公式。
...
TL;DR: 提出了LieNLSD方法，首次能够确定非线性项的无穷小生成元数量及其显式表达式，通过函数库和SVD求解系数矩阵，在动态系统和物理问题中表现优异。
...
Method: 为无穷小群作用指定函数库，求解其系数矩阵。证明微分方程的延拓公式对系数矩阵也是线性的，通过数据中心差分和神经网络雅可比矩阵代入无穷小准则，得到系数矩阵的线性方程组，用SVD求解。

Result: 在顶夸克标记和一系列动态系统上，LieNLSD相比现有方法具有定性优势，将神经PDE求解器的长期滚动精度提高了20%以上，并能指导数据增强。

Conclusion: LieNLSD是首个能够确定非线性无穷小生成元数量及其显式表达式的方法，在对称性发现方面具有重要应用价值。
...
Method: 学习一个生成模型来捕获跨任务共享的基础组件及其统计信息，将学习新任务转化为概率推理问题，无需参数更新。
...
TL;DR: 本文提出了两种具有简单模块化结构的算法，用于解决带在线对抗约束的在线凸优化问题，改进了现有的动态遗憾和累积约束违反边界。


<details>
  <summary>Details</summary>
Motivation: 扩展经典的在线凸优化框架，处理在线对抗约束，在成本和约束函数都由对手任意选择且约束函数可能没有共同可行点的最一般情况下，提供更好的性能保证。

Method: 通过将约束学习问题转化为具有特殊构造的代理成本函数的标准在线凸优化问题实例。

Result: 获得了改进的通用动态遗憾和累积约束违反边界，在最一般的对抗设置下优于现有最佳结果。

Conclusion: 提出的模块化算法在在线约束学习问题中提供了更强的理论保证，特别是在对抗性最强的设置下。
...
TL;DR: 该论文分析了梯度空间的动态特性，发现虽然小部分子空间捕获了大部分梯度能量，但仍有显著部分残差存在，且核心子空间的影响随时间推移和层深增加而减弱。基于这些洞察，作者提出了GrassWalk和GrassJump随机算法，在LLaMA-1B和LLaMA-7B预训练中实现了最先进的内存节省和性能提升。
...
Method: 分析了梯度空间及其子空间的动态特性，发现梯度空间呈现近平坦曲率。基于这些洞察，开发了GrassWalk和GrassJump随机算法，利用子空间特性进行优化。

Result: 在LLaMA-1B和LLaMA-7B预训练中，提出的算法实现了最先进的内存节省，同时提升了模型性能。

Conclusion: 梯度空间分析揭示了重要的动态特性，特别是子空间影响随时间变化和近平坦曲率特征。利用这些特性的随机算法能有效减少内存需求并改善训练效果。
...
<details>
  <summary>Details</summary>
Motivation: 许多自然动态过程只能通过静态样本快照观察，重建其时间演化以揭示潜在动态特性对科学研究至关重要。现有方法在时间轴上传输数据，但在高维情况下可扩展性差且需要严格假设。

Method: 通过推导迭代马尔可夫拟合算法，以新颖的因子化方式将扩散Schrödinger桥匹配扩展到多个边际，提出多边际时间Schrödinger桥匹配(MMtSBM)。

Result: 实验表明MMtSBM在玩具示例上保留了理论特性，在真实世界数据集上达到最先进性能，首次在非常高维图像设置中恢复耦合和动态。

Conclusion: 多边际Schrödinger桥成为从静态数据恢复隐藏动态的实用且原则性方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统诊断模型孤立分析不同数据源，限制了识别跨模态相关性，而多模态整合对早期疾病诊断至关重要。

Method: 使用专用编码器将各模态映射到共享潜在空间，通过多头注意力和残差归一化进行融合，支持多任务预训练和轻松适应新疾病。
...
TL;DR: 本文探索了Tsetlin Machine(TM)在大规模多通道RGB图像分类中的应用，提出了生成局部解释和全局类别表示的方法，在MNIST和CelebA数据集上取得了与深度学习模型相当的竞争力，同时保持了模型的可解释性。


<details>
  <summary>Details</summary>
Motivation: 由于Tsetlin Machine采用有限状态自动机和命题逻辑表示模式，比基于神经网络的算法更具可解释性。卷积TM已在多个数据集上表现出色，本文旨在探索TM架构在大规模多通道图像分类中的适用性。
...
Conclusion: TM在大规模复杂训练环境中表现竞争力，同时保持可解释性。这有助于更好地理解TM子句，并为将TM应用于更复杂多样的数据集提供了见解。
...
TL;DR: 本文首次系统比较了传统图神经网络增强方法和基于大语言模型的图增强方法在复合图缺陷下的表现，提出了RoGRAD框架，通过检索增强生成和迭代图对比学习实现动态优化。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的图数据存在多种缺陷，严重影响GNN性能。现有研究主要关注单一缺陷的鲁棒性，缺乏对复合缺陷下传统方法和LLM增强方法性能的系统比较。

Method: 提出RoGRAD框架，采用检索增强生成技术注入类别一致且多样化的增强数据，通过迭代图对比学习强制学习判别性表示，将LLM增强从静态信号注入转变为动态优化过程。

Result: 实验证明RoGRAD优于传统GNN和LLM增强基线方法，平均性能提升高达82.43%。

Conclusion: LLM增强并非总是优于传统方法，RoGRAD通过迭代式检索增强对比学习框架有效提升了图学习的鲁棒性。
...
Conclusion: 提出的几何感知LoRA扩展方法能够有效学习子空间，与现有微调流程兼容，并在多个任务上表现出优越性能。
...
TL;DR: 提出Moon框架，通过模态转换方法解决多元时间序列异常检测中的挑战，结合数值和图像数据，在效率、准确性和可解释性方面优于现有方法。
...
Method: 提出Moon框架：1) 使用多元马尔可夫转移场将数值时间序列转换为图像表示；2) 采用多模态CNN融合数值和图像数据；3) 基于SHAP的异常解释器识别关键变量。

Result: 在6个真实世界数据集上的实验表明，Moon在效率上提升达93%，准确性提升4%，解释性能提升10.8%，优于6种最先进方法。

Conclusion: Moon框架通过模态转换和特征融合，有效解决了多元时间序列异常检测的挑战，在效率、准确性和可解释性方面均有显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在探索高价值样本方面有效，但由于稀疏和窄化的奖励信号，导致偏好对齐效果不佳。
...
TL;DR: 该论文将后处理模型校准技术引入联邦学习，提出了联邦直方图分箱和温度缩放方法，并在异构客户端和差分隐私设置下评估了校准效果。
...
Method: 将传统集中式校准方法（直方图分箱和温度缩放）迁移到联邦环境，针对强客户端异构性定义了新的操作方法，研究了联邦设置和用户级差分隐私设置。

Result: 研究显示联邦化和差分隐私都会影响校准准确性，提出的加权分箱方法在非差分隐私场景表现最佳，联邦温度缩放在差分隐私联邦学习中效果最好。

Conclusion: 成功将后处理校准技术整合到联邦学习中，针对不同隐私需求提供了相应的优化策略，缓解了异构性带来的校准性能下降问题。
...
TL;DR: PepCompass是一个几何感知的肽探索和优化框架，通过定义解码器诱导的黎曼流形来捕获局部几何结构，提供局部探索方法和优化算法，在体外验证中成功发现了25种高活性抗菌肽。


<details>
  <summary>Details</summary>
Motivation: 传统生成模型在肽发现中忽略解码器诱导的几何结构，依赖平坦欧几里得度量，导致探索和优化失真且低效。现有流形方法假设固定内在维度，这在肽数据实践中严重失败。

Method: 提出PepCompass框架：定义Union of κ-Stable Riemannian Manifolds捕获局部几何；开发二阶黎曼布朗高效采样和切空间突变枚举两种局部探索方法；结合形成局部枚举贝叶斯优化(LE-BO)；引入势最小化测地线搜索(PoGS)沿属性富集测地线插值。

Result: 体外验证证实有效性：PoGS产生4个新种子肽，随后LE-BO优化发现25种高活性肽，具有广谱活性，包括对抗耐药菌株。
...
TL;DR: GraphNC是一个图异常检测框架，通过教师模型的异常分数分布对齐和基于扰动的正态性正则化来校准图的正态性，提高检测准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的半监督图异常检测方法仅基于标注的正常节点学习正态性，容易过拟合给定模式，导致高误报率。需要利用标注和未标注数据共同校准正态性。

Method: 提出GraphNC框架，包含两个组件：异常分数分布对齐(ScoreDA)和基于扰动的正态性正则化(NormReg)。ScoreDA通过对齐教师模型的分数分布优化异常分数，NormReg通过在表示空间最小化扰动一致性损失使正常节点表示更紧凑。

Result: 该方法能有效将正常和异常类别的异常分数拉向两端，产生更可分离的异常分数，同时使正常节点表示更加紧凑。

Conclusion: GraphNC通过联合利用标注和未标注数据，在异常分数和节点表示空间校准图正态性，能够克服现有方法的局限性，提高图异常检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI系统在日常生活中的普及，开发公平无偏的模型变得至关重要。虽然表示学习已被证明是有效去偏的方法，但在表格数据中公平表示学习的研究仍不足。

Method: 设计专门的对比学习框架，通过策略性选择正样本对，结合监督和自监督对比学习来学习公平表示。

Result: 与现有最先进的表格数据对比学习模型相比，显著减少了偏见，同时在准确性和下游任务性能方面保持最小损失。

Conclusion: 该方法在减轻偏见方面有效，能够学习公平表示并应用于各种下游任务，为表格数据的公平AI提供了可行方案。
...
TL;DR: 该论文对密集连接深度神经网络进行数学建模，在深度极限下分析其学习问题，证明了从网络学习问题到连续时间对应问题的收敛性。
...
Result: 证明了最优值的收敛性和极小化子的子序列收敛性，表明密集连接架构可以提供深度模型训练的稳定性。

Conclusion: 为密集连接DNNs提供了数学理论基础，证明了其在深度模型训练中的稳定性优势。
...
TL;DR: 本文提出了一种通过最大似然估计对Flow Matching进行微调的方法，以解决FM训练-推理差距问题，特别是在机器人操作等需要高精度的场景中。


<details>
  <summary>Details</summary>
Motivation: Flow Matching算法在训练和推理之间存在差距，无法在训练阶段评估模型输出，这在高精度场景如机器人操作中尤为明显。同时，FM对预设直线路径的过度追求可能引入刚度等问题。

Method: 通过最大似然估计对FM进行微调，包括直接微调和基于残差的微调方法。基于残差的微调可以通过特定架构设计融入收缩特性，提高模型的鲁棒性和可解释性。

Result: 在图像生成和机器人操作任务上的实验结果表明，该方法可靠地提升了FM的推理性能。

Conclusion: 通过最大似然估计微调FM可以有效缩小训练-推理差距，提高模型在高精度任务中的性能，同时保持FM的简单高效训练特性。
...
TL;DR: KAIROS是一个非自回归时间序列预测框架，直接建模分段级多峰分布，避免了误差累积，实现了实时推理，并在六个基准测试中表现出强大的零样本泛化能力。
...
Method: 提出KAIROS非自回归时间序列预测框架，直接建模分段级多峰分布，避免自回归方法的误差累积问题，同时改进现有非自回归模型的过度平滑预测问题。

Result: 在大规模语料库上训练的KAIROS在六个广泛使用的基准测试中表现出强大的零样本泛化能力，预测性能与类似规模的顶级基础模型相当，但推理成本显著降低。

Conclusion: KAIROS强调了非自回归设计作为时间序列基础模型可扩展范式的重要性，为实时Web应用提供了高效的预测解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 权重空间学习通常需要大量精心构建的已训练模型集合（模型动物园），这需要巨大的计算资源，限制了权重空间表示的规模和灵活性。本文旨在消除这一限制。

Method: 提出了一种新的权重空间骨干网络，专门设计用于处理来自Hugging Face等非结构化模型仓库的高度异构模型，这些模型在架构、数据集和文档方面都存在很大差异。

Result: 在Hugging Face模型上训练的权重空间表示实现了强大的性能，通常优于在实验室生成的模型动物园上训练的骨干网络，并且能够泛化到未见过的数据模态。

Conclusion: 通过证明高质量权重空间表示可以在非结构化环境中学习，表明精心策划的模型动物园并非不可或缺，从而克服了权重空间学习社区当前面临的主要限制。
...
TL;DR: 提出了PENEX（惩罚指数损失），一种新的多类指数损失公式，可作为AdaBoost的替代方案，通过一阶方法优化，在深度神经网络训练中表现出正则化效果。


<details>
  <summary>Details</summary>
Motivation: AdaBoost在实践中随着弱学习器数量增加而泛化良好，但其指数损失难以通过一阶方法优化。现有多类指数损失公式不适合优化，需要新的理论基础的损失函数。

Method: 提出PENEX损失函数，这是一种新的多类指数损失公式，可通过一阶方法优化，能够隐式最大化数据点的边界，并通过梯度增量隐式参数化弱学习器。

Result: 在计算机视觉和语言任务中，PENEX表现出比现有方法更好的正则化效果，计算成本相似。理论和实证都表明PENEX能够隐式最大化边界。

Conclusion: PENEX作为AdaBoost启发的替代方案，在深度神经网络的有效训练和微调中具有潜力，提供了理论依据且易于优化。
...
TL;DR: 使用机器学习模型预测伊朗赞詹省居民天然气消费，混合BiLSTM-XGBoost模型表现最佳。


<details>
  <summary>Details</summary>
Motivation: 伊朗作为全球第二大天然气储量国，面临冬季天然气供应压力和消费控制需求，特别是居民部门占最大消费份额。

Method: 采用LSTM、GRU和混合BiLSTM-XGBoost模型，基于2017-2022年天然气消费和气象数据进行分析预测。

Result: 混合BiLSTM-XGBoost模型在RMSE、MAPE和MPE指标上表现最优，在数据有限场景下也展现稳健性能。

Conclusion: 机器学习方法特别是混合模型能有效预测天然气消费，地理和气候因素对消费预测有重要影响。
...
TL;DR: DECOR是一个用于线性高斯结构方程模型的联合学习框架，能够同时学习有向无环图和相关噪声模型，在存在潜在混杂的情况下实现结构学习。
...
Method: 提出DECOR估计器，基于似然方法，联合学习DAG和相关噪声模型。采用交替优化策略，包含平滑无环图更新和凸噪声更新，可加入bow互补性惩罚或后处理步骤。

Result: 在合成基准测试中，DECOR匹配或优于强基线方法，特别在混杂非普遍时表现稳健，在普遍混杂情况下仍保持竞争力。

Conclusion: DECOR提供了一个完全可微分的统一框架，能够在存在潜在混杂的情况下有效学习因果结构，具有理论保证和实际优势。
...
<details>
  <summary>Details</summary>
Motivation: 开发经济高效的高性能催化剂对于可再生能源存储至关重要，但目前仍面临挑战。

Method: 使用基于机器学习的生成模型Catalyst GFlowNet，通过预测形成能和吸附能来设计催化剂晶体表面。

Result: 在析氢反应的概念验证中成功识别出铂是最有效的已知催化剂。
...
TL;DR: 将扩散模型中的分类器无关引导方法扩展到经典策略梯度方法，提出策略梯度引导(PGG)，通过插值条件分支和无条件分支实现无需重新训练的行为控制。


<details>
  <summary>Details</summary>
Motivation: 将扩散模型中成功的引导机制引入强化学习领域，为在线强化学习提供可控性，无需重新训练即可调节行为。

Method: 在策略梯度中加入无条件分支，通过插值条件分支和无条件分支实现引导，理论证明在优势估计下归一化项消失，得到简洁的引导策略梯度更新公式。

Result: 在离散和连续控制基准测试中，条件丢弃在简单离散任务和低样本场景下有效，但会破坏连续控制的稳定性；适度增大引导系数能一致提升稳定性、样本效率和可控性。
...
TL;DR: GRACE使用大语言模型和进化搜索从专家轨迹中逆向生成可解释的代码奖励函数，相比传统黑盒方法更易调试验证。
...
Method: 利用大语言模型在进化搜索框架中逆向工程生成基于代码的可执行奖励函数，可直接从专家轨迹中学习。

Result: 在BabyAI和AndroidWorld基准测试中高效学习到高精度奖励，在多任务设置下构建复杂奖励API，生成的奖励能产生与真实奖励相当的强策略。

Conclusion: GRACE方法成功生成了可解释、可验证的代码奖励函数，在复杂多任务环境中表现优异，优于模仿学习和在线强化学习方法。
...
TL;DR: 该论文介绍了2025年乔治·B·穆迪PhysioNet挑战赛，旨在开发从心电图识别恰加斯病的算法方法，以解决恰加斯病血清学检测能力有限的问题。


<details>
  <summary>Details</summary>
Motivation: 恰加斯病是一种寄生虫感染，慢性阶段会导致心血管疾病和消化问题。由于血清学检测能力有限，而恰加斯心肌病常在心电图中表现，这为优先检测和治疗患者提供了机会。

Method: 挑战赛提供了多个数据集，包括弱标签的大数据集和强标签的小数据集；采用数据增强技术提高模型鲁棒性和泛化能力；应用评估指标模拟当地血清学检测能力，将机器学习问题构建为分诊任务。

Result: 挑战赛吸引了来自111个团队的630多名参与者，提交了超过1300个参赛作品，代表了全球学术界和工业界的多样化方法。

Conclusion: 该挑战赛通过心电图识别恰加斯病的创新方法，为解决血清学检测能力有限的问题提供了有效途径，并促进了全球研究合作。
...
TL;DR: Poolformer是一个序列到序列模型，用循环层和池化操作替代自注意力机制，有效处理长序列数据，在音频处理任务中表现优于现有先进模型。


<details>
  <summary>Details</summary>
Motivation: 自注意力机制在处理长序列时存在二次复杂度问题，限制了其实际应用。需要开发更高效的序列建模方法来处理长序列数据。

Method: 使用SkipBlocks递归定义模型结构，包含残差块、下池化层、嵌套SkipBlock、上池化层和额外残差块，用循环层替代自注意力机制。

Result: 池化操作显著加速训练过程，改善感知指标（FID和IS），防止过拟合。在原始音频处理任务中优于SaShiMi和Mamba等先进模型。

Conclusion: Poolformer通过池化操作和循环层有效处理长序列，深层处理长程依赖，浅层处理短程特征，未来可扩展到文本、视觉和多模态应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有金融基准主要测试静态知识问答，无法捕捉交易的动态迭代特性，金融领域在LLM代理评估中仍未被充分探索。

Method: 引入StockBench基准，让代理接收每日市场信号（价格、基本面、新闻），做出顺序的买入、卖出或持有决策，使用累计回报、最大回撤和索提诺比率等金融指标评估性能。

Result: 评估显示大多数LLM代理难以超越买入持有基准，但部分模型（GPT-5、Claude-4、Qwen3、Kimi-K2、GLM-4.5）显示出更高回报和更好风险管理的潜力。

Conclusion: 在静态金融知识任务中表现出色不一定能转化为成功的交易策略，StockBench作为开源资源支持可复现性和该领域未来研究。
...
TL;DR: DiFFPO是一个统一的框架，通过强化学习训练掩码扩散大语言模型，使其推理能力更强（furious）且更快（fast）。


<details>
  <summary>Details</summary>
Motivation: 现有方法如d1在训练扩散大语言模型的策略时存在似然性难以处理的问题，需要更准确和高效的近似方法。同时，需要联合训练高效的采样器/控制器来优化推理时的计算效率。

Method: 1. 使用离策略强化学习训练代理策略，提出两阶段似然近似结合重要性采样校正；2. 联合训练扩散大语言模型的采样器，让模型学习为每个提示自适应分配推理阈值，利用其多令牌预测能力。

Result: 在数学和规划基准任务上训练开源大扩散语言模型，获得了更好的样本效率和任务性能，以更少的函数评估次数实现了更高的准确率，优化了推理时计算的帕累托前沿。

Conclusion: DiFFPO框架通过强化学习有效提升了扩散大语言模型的推理能力和效率，实现了性能与计算效率的最佳平衡。
...
TL;DR: 提出C2AL方法，通过分析数据集子结构并利用部分冲突的辅助标签来正则化共享表示，解决大规模推荐模型中用户群体异质性导致的分布不平衡问题，提升对少数群体的建模能力。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据由具有不同条件分布的异质群体组成，但单一全局目标的训练假设用户群体同质化。随着模型规模和复杂度的增加，模型会被中心分布模式主导，忽视头部和尾部区域，导致学习能力受限、注意力权重不活跃或神经元死亡。

Method: 分析数据集子结构，通过辅助学习暴露具有强分布对比的子结构；利用部分冲突的辅助标签来正则化共享表示；定制注意力层学习过程，在提升全局性能的同时保留与少数群体的互信息。

Result: 在包含数十亿数据点的生产数据集上对六个SOTA模型进行评估，因子分解机能够使用所提方法捕获细粒度的用户-广告交互，整体归一化熵降低达0.16%，在目标少数群体上增益超过0.30%。

Conclusion: 注意力机制在因子分解机中可发挥共享嵌入选择的关键作用，通过辅助学习处理分布不平衡问题能有效提升模型对异质用户群体的建模能力。
...
<details>
  <summary>Details</summary>
Motivation: 虽然Transformer是主流架构，但xLSTM等替代方案在十亿参数级别保持竞争力的同时具有线性复杂度，需要系统比较两者的扩展行为

Method: 1) 在80M-7B模型规模和2B-2T训练token范围内研究xLSTM的计算最优和过训练扩展行为；2) 分析最优模型大小对上下文长度的依赖性；3) 分析推理时扩展特性

Result: 在典型LLM训练和推理场景中，xLSTM相比Transformer具有更优的扩展特性，且随着训练和推理上下文增长，xLSTM的优势更加明显

Conclusion: xLSTM在扩展规律上优于Transformer，特别是在长上下文场景中表现更佳，为未来模型设计和部署提供了重要指导
...
Method: 结合长短期记忆自编码器和K-Means聚类的正未标记学习方法，利用3GPP关键性能指标和性能测量计数器作为机器学习模型的特征。

Result: 在基于free5GC和UERANSIM的5G测试床上进行评估，当训练数据包含10%到40%攻击污染时，PUL-Inter-Slice Defender的F1分数超过98.50%，优于同类方案。

Conclusion: PUL-Inter-Slice Defender能够有效检测分布式切片移动性攻击变种，在训练数据被污染的情况下仍保持鲁棒性，为5G网络切片安全提供了可靠解决方案。
...
TL;DR: 本文提出ExGRPO框架，通过识别推理经验的价值指标（正确性和熵），组织并优先处理有价值的经验，使用混合策略目标平衡探索与经验利用，显著提升大型语言模型的推理性能。


<details>
  <summary>Details</summary>
Motivation: 标准的策略训练在单次更新后丢弃经验，导致计算效率低下和不稳定。虽然先前研究强调了重用过去经验的好处，但经验特征在大型推理模型学习动态中的作用仍未充分探索。

Method: 提出ExGRPO框架：识别推理经验的价值指标（正确性和熵），组织并优先处理有价值的经验，使用混合策略目标平衡探索与经验利用。

Result: 在五个骨干模型（1.5B-8B参数）上的实验显示，ExGRPO在数学/通用基准上持续提升推理性能，平均比策略RLVR提高+3.5/7.6分，并在强弱模型上都能稳定训练。

Conclusion: 原则性的经验管理是高效可扩展RLVR的关键要素，ExGRPO通过优化经验利用显著提升了推理模型的性能和训练稳定性。
...
TL;DR: 研究表明，未经修改的Transformer模型直接在笛卡尔坐标上训练，无需预定义图结构或物理先验，就能在分子能量和力预测任务上达到与最先进等变图神经网络相竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 传统图神经网络在分子机器学习中依赖预定义的图结构（如固定半径截断或k近邻），这种硬编码的图结构可能限制表达能力并降低推理速度。研究者希望探索纯Transformer模型能否在没有图结构或物理先验的情况下学习分子性质。

Method: 使用标准Transformer架构直接在笛卡尔坐标上训练，不引入任何预定义的图结构或物理先验，与等变图神经网络在相同的训练计算预算下进行比较。

Result: Transformer在OMol25数据集上达到了与最先进等变图神经网络相竞争的能量和力平均绝对误差。模型自适应地学习了物理一致的模式，如注意力权重随原子间距离反比衰减，并能灵活适应不同分子环境。

Conclusion: Transformer能够自适应地学习图神经网络的许多有利特性，挑战了硬编码图归纳偏置的必要性，为分子建模提供了标准化、可扩展的架构方向。
...
TL;DR: Diffusion^2是一个基于扩散模型的RF信号传播预测方法，使用3D点云数据建模从Wi-Fi到毫米波的多种频率信号传播，比现有方法快27倍且误差仅1.9dB。


<details>
  <summary>Details</summary>
Motivation: RF信号传播建模对于环境理解和无线网络优化至关重要，但复杂环境中的精确预测仍具挑战性，因为信号会受到障碍物的吸收和反射影响。

Method: 提出Diffusion^2方法，使用RF-3D编码器从3D点云中提取RF相关特征，通过多尺度嵌入模拟RF信号传播过程。

Result: 在合成和真实世界测量中的评估显示，Diffusion^2能准确估计不同频段和环境条件下的RF信号行为，误差仅1.9dB，比现有方法快27倍。
...
TL;DR: 提出了一个更完整、现实和具有挑战性的交通预测基准数据集，包含两个主要城市的道路网络，最大数据集包含近10万个路段，比现有数据集增加10倍以上。同时提出了一种无需专门时间序列处理模块的GNN方法，具有更好的可扩展性和预测性能。


<details>
  <summary>Details</summary>
Motivation: 当前交通预测的公开基准数据集存在显著缺陷：缺乏道路连接性信息、道路属性信息有限、路段数量较少且无法满足实际应用需求。现有数据集主要关注城际高速公路，而城市道路网络由于更密集的道路和更复杂的交通模式，提出了更具挑战性的预测任务。

Method: 发布代表两个主要城市道路网络的数据集，包含丰富的道路特征和细粒度的交通流量与速度数据。提出了一种替代方法：使用不带专门时间序列处理模块的图神经网络（GNN），从而实现更好的可扩展性。

Result: 创建了包含近10万个路段的数据集（比现有数据集增加10倍以上），提供了丰富的道路特征和细粒度交通数据。提出的GNN方法在可扩展性和预测性能方面都表现更好，能够有效处理大规模数据集。

Conclusion: 新的数据集和建模见解为交通预测研究提供了宝贵资源，解决了现有基准数据集的局限性，并为大规模城市道路网络的交通预测提供了更现实的测试平台。
...
TL;DR: 本文指出当前LLM幻觉检测方法评估存在缺陷，提出使用多种风险指标、多LLM评委集成、结构化任务以及Elo评级等方法来提高不确定性估计算法评估的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM不确定性估计方法评估存在严重问题，常用的近似正确性函数之间存在显著分歧，导致评估结果不可靠，容易被操纵。

Method: 提出使用多种替代风险指标进行风险相关性实验；在QA任务中集成多个LLM-as-a-judge变体以减少评估偏差；探索结构化任务以及分布外和扰动检测任务；使用Elo评级对不确定性估计方法进行客观总结。

Result: 多LLM评委集成能够减少评估偏差，结构化任务和扰动检测任务提供了更鲁棒和可控的风险指标，Elo评级能够对不确定性估计方法进行客观总结。

Conclusion: 通过提出的多指标评估框架，能够更可靠地评估LLM不确定性估计算法，提高检测幻觉的可靠性。
...
TL;DR: 提出了DialTree-RPO框架，结合强化学习和树搜索来自主发现多样化的多轮攻击策略，显著提升了攻击成功率并揭示了新的攻击轨迹。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在多轮交互场景下容易受到对抗攻击，现有方法主要依赖人工红队测试或预定义模板，无法有效探索复杂对话动态中涌现的新型攻击策略。

Method: 采用基于策略的强化学习框架，结合树搜索技术，将对话建模为序列决策问题，无需人工标注数据即可系统性地探索多轮攻击空间。

Result: 在10个目标模型上相比现有最优方法攻击成功率提升超过25.9%，并能有效发现新的攻击策略。

Conclusion: DialTree-RPO框架能够自主发现多样化的多轮攻击策略，显著提升了对抗攻击的发现能力，为LLM安全评估提供了有效工具。
...
TL;DR: 提出Concept Neuron Selection (CNS)方法，通过识别和微调扩散模型中与目标概念相关的神经元，实现增量式个性化学习，避免灾难性遗忘并保持零样本生成能力。


<details>
  <summary>Details</summary>
Motivation: 在真实应用中增量更新扩散模型具有实用性，但计算上具有挑战性。需要一种能够在持续学习方案中执行个性化的方法。

Method: CNS独特地识别扩散模型中与目标概念密切相关的神经元，以增量方式微调概念神经元，并联合保留先前概念的知识，实现融合自由操作。

Result: 在真实数据集上的评估显示，CNS以最少的参数调整实现了最先进的性能，在单概念和多概念个性化任务中都优于先前方法，同时减少了内存存储和处理时间。

Conclusion: CNS是一种简单有效的持续学习策略，能够在增量设置中高效更新扩散模型，同时保持零样本文本到图像生成能力。
...
TL;DR: 提出了Interactive Training框架，允许在神经网络训练过程中进行实时人工或AI干预，通过控制服务器动态调整超参数、训练数据和模型检查点，提升训练稳定性和适应性。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络训练采用固定的优化方案，缺乏动态响应训练不稳定或问题的灵活性，需要一种能够实时干预的训练框架。
...
Conclusion: EqM通过用统一平衡景观替换时间条件速度，在流模型和能量基模型之间建立了更紧密的桥梁，提供了优化驱动推理的简单途径。
...
<details>
  <summary>Details</summary>
Motivation: 解决模型来源验证和未经授权的蒸馏复制问题，关注模型溯源和知识产权保护。
...
Conclusion: 提出的方法能有效检测知识蒸馏过程，为模型溯源提供实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统LPCA方法在高噪声环境下存在邻域大小选择的临界权衡问题，需要先验几何和噪声特征知识，而这些信息通常不可得。

Method: LEGO方法通过正交化图拉普拉斯低频特征向量的梯度来估计每个数据点的切空间，利用数据的全局结构指导局部切空间估计。

Result: 实验表明LEGO产生的切空间估计比LPCA对噪声更稳健，在流形学习、边界检测和局部本征维度估计等下游任务中表现显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 显式思维链推理计算成本高且包含冗余信息，而潜在推理缺乏有效监督，在复杂自然语言推理中效果有限。

Method: 通过自蒸馏方法，利用压缩KV缓存作为监督信号，将教师模型的KV轨迹与潜在推理学生模型对齐。

Result: 该方法在潜在推理基准上表现优于基线，从纯数学推理到自然语言推理的退化更小，并能扩展到更大模型同时保持效率。

Conclusion: 压缩KV缓存蒸馏为潜在推理提供了可扩展的监督信号，结合了思维链推理的准确性和潜在推理的效率优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有的可实现到不可知转换在任意ε时会产生次优的额外样本复杂度，包含1/ε因子。本文旨在消除这种依赖。

Method: 提出改进的构造方法，利用私有不可知学习中的隐私成本主要来自可实现部分的洞察。

Result: 实现了接近最优的额外样本复杂度O~(VC(C)/α²)，适用于任意ε≤1，消除了1/ε因子。

Conclusion: 在私有不可知学习中，隐私成本主要来自可实现部分，该工作解决了Dwork等人提出的私有预测问题的开放性问题。
...
TL;DR: CADD框架通过将离散状态空间与连续潜空间配对扩散，解决了传统离散扩散模型中信息丢失的问题，用有信息的噪声潜向量替代吸收性[MASK]标记，提升了生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统离散扩散模型将所有未观察状态统一映射为吸收性[MASK]标记，造成'信息空洞'，导致语义信息在去噪步骤间丢失。

Method: 引入连续增强离散扩散(CADD)框架，在离散状态空间基础上配对连续潜空间扩散，使掩码标记表示为有信息的噪声潜向量，在反向步骤中利用连续潜向量作为语义提示指导离散去噪。

Result: 在文本生成、图像合成和代码建模任务中，CADD在定性和定量指标上均优于基于掩码的扩散方法，相比强离散基线获得一致提升。

Conclusion: CADD通过连续潜空间增强有效解决了离散扩散中的信息空洞问题，实现了模式覆盖和模式寻求行为间的可控权衡，显著提升了生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 研究尖峰协方差模型中最小范数插值解的泛化性能，特别是在过参数化情况下，理解尖峰强度和对齐如何影响学习风险。

Method: 使用尖峰协方差数据模型，推导出泛化误差的精确表达式，分析尖峰强度、纵横比c=d/n（特别是c→∞时）和目标对齐的影响。

Result: 建立了良性、温和和灾难性过拟合的完整分类体系。发现在对齐良好的问题中，增加尖峰强度可能在达到良性过拟合前先引发灾难性过拟合。目标-尖峰对齐并不总是有益的。

Conclusion: 目标-尖峰对齐在某些条件下可能有害，这一发现在非线性模型中也得到了实证验证。对齐策略需要谨慎考虑具体条件。
...
<details>
  <summary>Details</summary>
Motivation: 由于工程时间序列受物理规律而非语言规律支配，基于大语言模型的AI基础模型可能无效或低效，需要专门针对工程应用设计的时间序列基础模型。

Method: 基于Wiener、Kallianpur和Rosenblatt的创新表示理论，提出了TS-GPT——一种基于创新表示的生成预训练变换器，用于工程监控和控制。

Result: 在预测美国独立系统运营商的实时节点边际价格方面，TS-GPT表现出有效性，能够从条件概率分布生成未来时间序列样本。

Conclusion: TS-GPT作为专门针对工程时间序列的基础模型，在需要因果操作的实时监控和控制应用中具有实用价值。
...
TL;DR: 本文对现有分类核方法进行了可重复的比较研究，提出了新的评估指标，发现在具有分类输入层级组结构的数据集上，嵌套核方法表现最佳；对于未知或无组结构的情况，提出了基于聚类的目标编码策略。


<details>
  <summary>Details</summary>
Motivation: 设计分类核是高斯过程回归中的主要挑战，现有研究因评估指标、优化过程或数据集不同而难以确定优选方法，且缺乏可复现代码。

Method: 对现有所有分类核方法进行可重复比较研究，提出受优化社区启发的新评估指标，并为未知组结构情况提出基于聚类的目标编码策略。

Result: 在具有分类输入层级组结构的数据集上，嵌套核方法明显优于所有竞争者；在无已知组结构的数据集上，基于聚类的估计策略仍优于其他方法且计算成本低。

Conclusion: 嵌套核方法在已知组结构情况下表现最佳，而基于聚类的目标编码策略在未知组结构情况下是有效的替代方案，均优于现有方法。
...
TL;DR: 本文研究了不完全市场中的复制投资组合构建问题，提出了一种基于AlphaZero的方法，并与行业广泛使用的深度对冲方法进行比较，证明了AlphaZero在非凸环境中的优越性能。


<details>
  <summary>Details</summary>
Motivation: 解决不完全市场中复制投资组合构建的关键问题，该问题在金融工程中有广泛应用，包括定价、对冲、资产负债表管理和能源存储规划等。
...
Conclusion: AlphaZero在非凸环境中优于深度对冲，深度对冲的有效性依赖于凸性假设，而AlphaZero在数据稀缺、容易过拟合的衍生品市场中具有重要优势。
...
TL;DR: 使用动力学平均场理论(DMFT)分析对角线性网络(DLNs)的梯度流动力学，推导出高维渐近动力学的低维有效过程，揭示了损失收敛率与泛化的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 对角线性网络虽然是一个可处理的模型，能捕捉神经网络训练中的非平凡行为，但这些现象通常被孤立研究，整体动力学理解不足。

Method: 采用动力学平均场理论(DMFT)推导高维梯度流动力学的低维有效过程，分析该有效过程来研究DLN动力学。

Result: 获得了关于DLN动力学的新见解，包括损失收敛率及其与泛化的权衡关系，并系统重现了许多先前观察到的现象。

Conclusion: 这项工作深化了对DLNs的理解，并证明了DMFT方法在分析神经网络高维学习动力学方面的有效性。
...
TL;DR: 提出连续时间版本的持久对比散度方法，用于未归一化密度的最大似然估计，通过耦合多尺度随机微分方程系统同时进行参数优化和采样，并提供明确的误差界限。


<details>
  <summary>Details</summary>
Motivation: 为能量基模型训练提供具有明确误差保证的新方法，解决传统方法缺乏理论保证的问题。

Method: 将持久对比散度表达为耦合多尺度随机微分方程系统，使用随机正交Runge-Kutta Chebyshev方法进行高效实现。

Result: 推导出持久对比散度迭代与最大似然解之间的误差界限，获得多尺度系统与平均状态之间差异的一致时间界限。

Conclusion: 该方法为训练能量基模型提供了具有明确误差保证的新框架，在长时间范围内提供精确的误差估计。
...
TL;DR: 提出了自适应SVGD方法，通过交替更新粒子并基于核化Stein差异自适应调整核带宽，解决了传统SVGD对核函数选择敏感的问题。


<details>
  <summary>Details</summary>
Motivation: 传统SVGD方法对核函数选择高度敏感，常用的中位数启发式方法在高维设置中表现不佳，需要更灵活的自适应核参数选择策略。

Method: 提出Ad-SVGD方法，交替执行两个步骤：1) 通过SVGD更新粒子；2) 通过梯度上升最大化KSD来自适应调整核带宽参数。

Result: 理论分析表明该方法在自适应设置下具有收敛性，实证结果证实Ad-SVGD在各种任务中持续优于标准启发式方法。

Conclusion: 基于KSD优化的自适应核参数选择策略能有效提升SVGD性能，特别是在高维贝叶斯推断问题中。
...
TL;DR: 本文研究高维设置下的逆协方差矩阵估计问题，重点分析线性收缩估计器和数据增强估计器，推导了它们的二次误差集中界，并基于随机矩阵理论提出了新的确定性等价方法。


<details>
  <summary>Details</summary>
Motivation: 在高维统计中，逆协方差矩阵估计是一个重要但具有挑战性的问题。传统方法在高维情况下表现不佳，因此需要开发新的估计策略，如线性收缩和数据增强方法。

Method: 使用随机矩阵理论工具，提出了适用于具有特定结构依赖样本的广义解析矩阵的确定性等价方法，为线性收缩估计器和数据增强估计器推导了二次误差的集中界。

Result: 理论分析得出了两类估计器的误差集中界，支持方法比较和超参数调优（如选择最优的人工样本比例）。数值实验验证了理论结果的有效性。

Conclusion: 本文为高维逆协方差矩阵估计提供了理论框架和实用工具，特别是在数据增强和线性收缩方法方面，为实际应用中的超参数选择和模型比较提供了理论依据。
...
TL;DR: 开发了一个混合物理-机器学习框架，结合290万个观测数据，用于预测北极永久冻土变化及其对基础设施的风险评估，在RCP8.5情景下预测平均永久冻土分数下降20.3个百分点。


<details>
  <summary>Details</summary>
Motivation: 北极变暖威胁着北方地区超过1000亿美元的基础设施，现有风险评估框架缺乏时空验证、不确定性量化和运营决策支持能力。

Method: 混合物理-机器学习框架，结合随机森林、直方图梯度提升和弹性网络等集成模型，并开发了混合方法结合学习的气候-永久冻土关系（60%）和物理永久冻土敏感性模型（40%）。

Result: 集成模型达到R²=0.980（RMSE=5.01 pp），在RCP8.5情景下预测平均永久冻土分数下降20.3个百分点，识别出15%的高风险区和25%的中风险区。

Conclusion: 该框架代表了全球最大的验证永久冻土机器学习数据集，提供了首个用于北极基础设施的运营混合物理-机器学习预测系统，并可推广到其他永久冻土区域。
