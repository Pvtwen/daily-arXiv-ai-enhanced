...
Motivation: 研究混合动力系统在小型电动车上的应用潜力，探索可再生能源在交通领域的可行性，为大型车辆的规模化应用提供基准。

Method: 采用实验研究和数据驱动分析，使用信号处理和机器学习技术（包括时间卷积网络）进行性能预测、异常检测和油门级别识别。

Result: 混合系统显著提高了电气性能和可靠性，减少了关键故障，机器学习技术能高精度识别油门级别，电压预测模型可用于充电站规划。

Conclusion: 质子交换膜燃料电池与镍氢电池的集成显著改善了小型电动车的性能，该系统具有离网可再生氢能应用的潜力，可为大型车辆提供参考基准。
...
<details>
  <summary>Details</summary>
Motivation: ISATNs作为6G关键使能技术，大规模部署面临能源消耗和碳排放的可持续性问题，需要更环保的网络编排方案。

Method: 采用数字孪生技术，建立以gCO₂/bit为主要可持续性指标的多时间尺度PDCA循环框架，结合日前预测和实时自适应优化，利用碳感知切换、无人机占空比控制和可再生能源感知边缘放置等ISATN特定控制手段。

Result: 使用真实碳强度数据的仿真结果显示，相比仅考虑QoS的编排，gCO₂/bit降低了29%，同时提高了可再生能源利用率和不利事件下的网络韧性。
...
<details>
  <summary>Details</summary>
Motivation: 为水务公司提供一种能够识别DMA级别异常并以最小检查成本定位漏水的有效策略，解决传统漏水检测方法效率低、成本高的问题。

Method: 采用两阶段方法：首先识别发生漏水的DMA区域，然后在识别出的DMA内预定位具体漏水管道序列；使用AMSI指标检测异常，并优化压力计配置方案。

Result: 策略能够有效限制DMA识别阶段的误报，通过AMSI指标对漏水敏感但对一般劣化不敏感的特性，在意大利阿普利亚地区两个真实供水网络中验证了有效性。

Conclusion: 该两阶段漏水检测策略为水务公司提供了一种实用且成本效益高的解决方案，通过DMA识别和管道序列预定位，显著降低了漏水检测的检查成本。
...
<details>
  <summary>Details</summary>
Motivation: 现代齿轮制造对NVH性能要求严格，传统质量控制方法无法捕捉瞬态加工异常和实时缺陷检测，需要开发在线监测系统。

Method: 使用加速度计连续采集振动数据，采用时频信号分析，比较三种子空间学习方法（PCA、PCA+LDA、R-UMLDA）进行特征提取，然后用SVM分类器预测四个齿轮质量等级。

Result: 在工业环境中收集的实验数据集上，该框架实现了高分类准确率（最高达100%），并能提供与工艺动力学相关的可解释频谱特征。

Conclusion: 该方法为齿轮强力珩磨过程提供了有效的在线监测解决方案，可集成到实时监控和预测性维护系统中。
...
<details>
  <summary>Details</summary>
Motivation: 心脏的复杂动态反映在其电活动中，通过心电图捕捉。本研究旨在理解心电图复杂性如何随心脏病理变化，探索非线性分析在心脏疾病诊断中的应用价值。

Method: 使用PTB-XL大型数据集，从II导联心电图提取非线性指标，并使用Spearman相关性和互信息计算跨通道度量（II、V2、AVL导联）。

Result: 在几乎所有指标中，健康与疾病个体之间以及5个诊断超类之间均发现显著差异（p<.001）。将复杂性量化指标整合到机器学习模型中，AUC从基线0.86提高到非线性指标0.87，包含跨时间序列指标后达到0.90。

Conclusion: 非线性时间序列分析能够有效捕捉心电图复杂性变化，这些复杂性量化指标显著提升了机器学习模型的心脏疾病分类性能，为心脏病理诊断提供了有价值的工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有的StULC信道建模要么关注分离的信道，要么忽略了粒子和湍流对激光传播的综合影响，因此需要建立更全面的信道模型。

Method: 采用分析-蒙特卡洛混合方法：基于扩展惠更斯-菲涅尔原理获得激光通过湍流大气后的强度分布；推导空气-水界面后光子传播方向的闭式概率密度函数；使用蒙特卡洛方法建模水下链路并获得接收平面功率分布。

Result: 数值结果表明，水下粒子浓度对通信性能的影响远大于大气湍流和水下湍流；增加空气-水界面的风速不会显著恶化StULC链路的通信性能。

Conclusion: 成功建立了综合考虑粒子和湍流效应的StULC综合信道模型，发现水下粒子浓度是影响通信性能的主要因素，而界面风速影响较小。
...
<details>
  <summary>Details</summary>
Motivation: Wi-Fi活动识别在多用户场景下受限于空间分辨率，而利用近场效应建立个人设备感知链路时，由于信号具有用户特定特性和不规则模式，神经网络需要跨域微调，这在某些活动类别缺失时尤为困难。

Method: 框架包含三个步骤：预训练阶段扩大类间特征边界增强活动可分性；微调阶段创新锚点匹配机制进行跨域适应，基于不完整活动类别过滤用户特定干扰；最后基于输入样本与锚点的特征相似性进一步改进识别。

Result: 构建了综合数据集进行验证，在活动类别缺失的情况下实现了超过90%的跨域准确率。

Conclusion: WiAnchor通过创新的锚点匹配机制有效解决了Wi-Fi多用户活动识别中的跨域适应问题，特别是在活动类别不完整的情况下仍能保持高准确率。
...
Method: 将流形变换矩阵和源方位角-仰角对联合估计统一到一个优化问题中，采用不精确增广拉格朗日方法求解，完全避免了半定规划。
...
### [9] [CLARAE: Clarity Preserving Reconstruction AutoEncoder for Denoising and Rhythm Classification of Intracardiac Electrograms](https://arxiv.org/abs/2510.17821)
...
<details>
  <summary>Details</summary>
Motivation: 心房内电图常受噪声污染且维度高，限制了实时分析。需要一种能保持波形形态、减少重建伪影并产生可解释嵌入的方法。

Method: 基于三个原则设计的一维编码器-解码器：池化下采样、混合插值-卷积上采样路径和有界潜在空间。

Result: 在495,731个EGM片段上评估，CLARAE在所有节律类型的分类中F1分数超过0.97，潜在空间显示清晰的节律聚类，在去噪任务中表现优异。

Conclusion: CLARAE结合了鲁棒的去噪能力和紧凑的判别性表示，为节律判别、信号质量评估和实时映射等临床工作流程提供了实用基础。
...
<details>
  <summary>Details</summary>
Motivation: 解决导向矢量估计失配和数据协方差矩阵重建问题，提高波束形成在干扰环境下的性能。

Method: 使用到达方向估计和通用线性组合算法重构干扰加噪声协方差矩阵，采用预处理空间采样策略，并用样本协方差矩阵替代预处理矩阵。

Result: 仿真结果表明该方法相比现有方法具有更好的性能表现。

Conclusion: 所提出的PPBSS技术在计算成本和波束模式分析方面表现优异，能够有效处理导向矢量失配问题。
...
<details>
  <summary>Details</summary>
Motivation: EEG数据采集面临传感器成本高、采集时间长和受试者间变异性的挑战，需要解决数据稀缺问题。

Method: 预处理真实EEG数据，训练扩散模型从噪声重建EEG通道，通过信号级和任务级指标评估生成信号质量。

Result: 生成数据在分类任务中准确率超过95%，与真实信号具有低均方误差和高相关性。

Conclusion: 扩散模型生成的合成EEG信号能有效补充数据集，改善EEG-BCI中的分类性能，解决数据稀缺问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统非相干空中计算方法需要超过d次信道使用来计算函数，当数据向量具有稀疏性时，可以利用稀疏性显著降低通信开销。

Method: 使用随机变换传输数据向量的低维投影，通过多数投票空中计算方案估计聚合投影的符号位向量，然后利用1位压缩感知恢复高维实值聚合和。

Result: 理论证明MVCS方案在T=O(kn log(d)/ε²)次信道使用下能以ℓ₂范数误差ε估计聚合数据向量∑xᵢ。
...
<details>
  <summary>Details</summary>
Motivation: 传统自动调制识别方法在复杂噪声环境中，特别是低信噪比条件下表现不佳，需要更鲁棒的解决方案来提高频谱效率。

Method: 提出MCANet多模态协作注意力网络，采用精细化特征提取和全局建模来支持融合策略。

Result: 在多个基准数据集上的实验结果表明，MCANet优于主流AMR模型，在低信噪比条件下具有更好的鲁棒性。

Conclusion: MCANet框架有效解决了复杂噪声环境下的自动调制识别挑战，为认知无线电系统提供了更可靠的调制识别方案。
...
<details>
  <summary>Details</summary>
Motivation: 数字射频存储器电子对抗技术的日益发展对雷达系统生存能力和有效性构成严重威胁，需要能够在低信噪比环境中鲁棒地区分真实目标和复杂干扰信号。

Method: 使用双树复小波变换提取对噪声和信号平移具有鲁棒性的特征，结合注意力机制和预训练骨干网络进行特征精炼，采用监督对比学习策略和原型网络进行分类。

Result: 在-6 dB信噪比下达到90.45%的准确率，通过t-SNE可视化验证了特征可分性，结合时域滑动窗口方法实现了有效的干扰抑制。

Conclusion: AWSPNet在复杂电磁环境中具有实际应用潜力，能够同时进行目标识别和干扰抑制。
...
<details>
  <summary>Details</summary>
Motivation: 物联网网络需要实时异常检测，但传统方法依赖标签数据或集中式数据共享，不适合资源受限的物联网设备。FedSVD旨在解决这些问题。

Method: 利用奇异值分解(SVD)和Grassmann流形优化，在低功耗设备上实现无监督联邦学习，显著减少通信开销和计算成本。

Result: FedSVD性能与深度学习基线相当，但推理延迟降低超过10倍，适合延迟敏感的物联网应用。

Conclusion: FedSVD为物联网网络提供了一种高效、低延迟的无监督异常检测解决方案，特别适合资源受限的边缘设备部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的语义通信主要依赖模拟或半数字传输，与现代数字通信基础设施兼容性差；现有向量量化方法在码本优化时忽略信道状态信息，导致鲁棒性不足。

Method: 在离散无记忆信道下建立VQJSCC框架，将语义特征离散化并直接映射到调制星座符号；CAVQ算法将信道转移概率集成到量化过程中；引入多码本对齐机制处理码本顺序与调制顺序不匹配问题。

Result: 实验结果表明VQJSCC有效缓解数字悬崖效应，在各种调制方案下实现优越的重建质量，在鲁棒性和效率方面均优于现有数字语义通信基准方法。

Conclusion: 提出的通道感知向量量化方法能够显著提升数字语义通信的性能，为现代数字通信基础设施提供更好的兼容性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 工业自动化应用对延迟管理有严格要求，因为设备故障可能带来高风险。需要高效的MAC协议来保证关键数据的及时传输，特别是在存在异构数据的工业环境中。
...
Conclusion: FROG-MAC协议因其能够中断低优先级传输的固有特性，在工业异构无线网络中具有更好的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 在无线传感器网络中，评估MAC协议性能时，需要验证高层理论模拟结果与详细实现结果的一致性，以确保协议在实际部署中的可靠性。

Method: 使用MATLAB进行高层理论模拟，使用TinyOS在Mica2平台上开发详细实现，基于能耗和延迟对ADP-MAC协议进行性能评估。

Result: 高层模拟显示能耗随轮询间隔增加而减少，延迟增加；而详细实现显示能耗和延迟都随轮询间隔增加而增加。

Conclusion: 高层和低层模拟对ADP-MAC的趋势显著不同，表明高层研究缺乏现实假设，需要在协议部署前进行详细实现验证。
...
<details>
  <summary>Details</summary>
Motivation: 模数采样通过将输入信号折叠到有界区间来避免信号削波，但折叠引入的非线性失真在噪声和量化条件下给信号恢复带来挑战，需要开发有效的恢复方法。

Method: 设计了模数软量化展开迭代解码器(mSQUID)，包含软量化模块，通过可微分和可学习的方式引导解趋向折叠范围的离散倍数，结合经典CS求解器和深度学习优势。

Result: 在低采样率和高斯噪声下实现优越的重建性能，在具有不同幅度和不相交频带的信号同时采集和量化的挑战性场景中能有效恢复输入信号，且运行时间显著减少。

Conclusion: mSQUID方法成功解决了模数采样中的信号恢复难题，结合了模型驱动和深度学习的优势，适用于实时资源受限系统，为模数采样技术的实际应用提供了有效解决方案。
...
Method: 采用无线馈电技术为波导供电，开发了无线馈电夹持天线系统（Wi-PASS），通过无线方式为波导提供能量，实现更灵活的部署。

Result: Wi-PASS在室内外多种应用场景中展现出优于传统PASS的优势，数值结果显示Wi-PASS比传统固定天线系统提供更高的数据速率。

Conclusion: Wi-PASS在可行性和性能方面均优于传统系统，为毫米波和太赫兹频段的覆盖扩展提供了实用且经济高效的解决方案，并指出了未来研究方向以推进Wi-PASS的部署。
...
<details>
  <summary>Details</summary>
Motivation: 数据从稀疏假设的退化观测中恢复是一个活跃的研究领域，传统迭代优化方法现在与深度学习技术相辅相成，展开式方法的发展受益于这两个领域。

Method: 在参数化色谱信号数据库上对三种展开式架构进行对比研究，使用适应物理化学峰信号表征的度量标准来评估性能。

Result: 研究突出了这些方法的性能表现，特别是当采用适应物理化学峰信号表征的度量标准时。

Conclusion: 展开式方法结合了传统优化和深度学习的优势，在色谱信号恢复任务中表现出色，特别是使用特定领域度量标准进行评估时。
...
<details>
  <summary>Details</summary>
Motivation: 主成分分析在三维数据降维和去噪中应用广泛，但处理具有任意方向的结构生物学数据时面临挑战，传统方法需要大量旋转数据增强，计算成本过高。

Method: 开发了一个高效且原理性的SO(3)不变PCA框架，通过利用底层代数结构，隐式考虑所有旋转而不需要显式数据增强，计算复杂度仅涉及协方差矩阵条目数的平方根。

Result: 在真实分子数据集上验证了方法的有效性，证明了其处理大规模高维重建问题的能力。

Conclusion: 该方法为处理具有未知方向的三维体积数据提供了一种高效的主成分分析解决方案，显著降低了计算复杂度，为大规模高维重建问题开辟了新可能性。
...
### [23] [Provably Optimal Reinforcement Learning under Safety Filtering](https://arxiv.org/abs/2510.18082)
...
<details>
  <summary>Details</summary>
Motivation: 强化学习在复杂任务中的应用受到缺乏正式安全保证的限制。安全过滤器常被认为会牺牲性能并阻碍学习过程，但作者认为这种安全-性能权衡并非固有。

Method: 定义了安全关键马尔可夫决策过程(SC-MDP)和关联的过滤MDP，其中安全过滤器被视为环境的一部分。证明了在过滤MDP中学习是安全的，标准RL收敛性仍然成立。
...
Method: PRISM耦合了基于分数的扩散预处理器、动态相关阈值图编码器，以及通过通用物理惩罚正则化的预测头。

Result: 在六个标准基准测试中，PRISM实现了持续的最先进性能，在MSE和MAE指标上取得了显著提升。

Conclusion: 该模型在温和条件下证明了诱导时域动态的收缩性，并为图块推导了Lipschitz边界，解释了模型的鲁棒性。
...
Conclusion: 基于执行的验证为工具选择提供了比单独语义相似度更可靠的基础，能在实际应用中实现更稳健的代理性能
...
Result: 在XJTU-SY和PRONOSTIA轴承数据集上验证，CARLE优于多种最先进方法，特别是在动态条件下表现更好。

Conclusion: CARLE框架在RUL预测中表现出优异的性能、鲁棒性和泛化能力，并通过可解释性分析增强了透明度和可信度。
...
<details>
  <summary>Details</summary>
Motivation: 开发能够捕捉参数依赖关系、嵌入物理知识并处理高梯度区域的深度学习代理模型，以替代计算昂贵的数值模拟。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在现实应用中面临隐私安全和知识删除需求，但现有机器遗忘方法难以处理连续异构的遗忘需求以及涉及分散敏感数据的场景，导致域间和域内干扰问题。

Method: 使用任务特定适配器学习来解耦遗忘和保留过程，并采用分层合并策略来缓解目标冲突，实现鲁棒且可适应的遗忘更新。

Result: 在WMDP、MUSE和TOFU基准测试上的综合实验表明，该方法能有效处理异构遗忘请求，同时相比基线方法保持更强的LLM效用。

Conclusion: 提出的联邦遗忘方法为大语言模型提供了一种可扩展且隐私保护的解决方案，能够应对现实世界应用中复杂的遗忘需求。
...
Method: 开发了一个模块化、可扩展的统一基准测试框架，整合代表性注意力内核和上下文并行机制，评估注意力掩码模式和序列长度/分布式规模两个关键维度。

Result: 在最多96个GPU的集群上进行全面实验，实现了可重复的比较，突出了方法特定的权衡，为长上下文LLM训练中的注意力机制设计和部署提供了实用指导。

Conclusion: 该基准测试填补了系统评估注意力机制的空白，为长上下文训练中的注意力优化提供了有价值的见解和指导。
...
<details>
  <summary>Details</summary>
Motivation: 结合MoE架构的稀疏激活优势和LoRA的参数高效微调能力，构建更高效、可扩展且专业化的语言模型，解决传统MoE参数冗余和训练复杂性问题。

Method: 将MoE专家重新定义为任务专用的低秩适配器集合，通过轻量级门控网络学习动态组合这些LoRA适配器参数，采用完全可微分的路由机制实现端到端联合训练。

Result: 提出了L-MoE的正式数学框架，包括可微分路由机制和联合优化目标，创建了参数高效、模块化设计、支持动态技能组合的MoE模型。

Conclusion: L-MoE为构建更高效、可扩展和专业化语言模型提供了新路径，统一了MoE和LoRA两种范式，实现了端到端可训练的轻量级专家混合架构。
...
<details>
  <summary>Details</summary>
Motivation: 传统自动调优依赖进化算法、退火方法等优化算法，但没有单一方法在所有调优任务中表现最佳，设计有效的优化器具有挑战性。

Method: 提出一个框架，使用LLM根据问题描述和搜索空间特征生成专门的优化策略，并通过迭代检查和改进这些策略。

Result: 在4个真实世界自动调优应用和6个硬件平台上评估，LLM生成的优化器能与现有算法竞争，并在许多情况下超越人工设计的算法。

Conclusion: LLM生成的优化算法在自动调优中表现出色，能够显著提升性能，为优化算法设计提供了新范式。
...
Method: 采用分布式训练机制，将绝大多数节点更新与服务器解耦，从而大幅减少节点与服务器之间的通信。

Result: 实验表明，SBVFL相比标准VFL减少了约99%的通信量，同时保持了准确性和鲁棒性。

Conclusion: SBVFL使得在敏感领域（包括医疗、金融、制造、航空航天、网络安全和国防工业）实现实用且保护隐私的垂直联邦学习成为可能。
...
Method: 在理论分析中，作者在线性设置下证明了记忆与组合能力的作用。在实验中，使用神经网络架构在简单数据上进行验证，并观察不同架构的组合能力。

Result: 理论分析表明，记忆与组合能力可以帮助模型对需要长尾特征组合的罕见测试样本做出正确预测，即使这些组合从未在训练数据中出现过。实验证实这一理论洞察可以扩展到线性设置之外，并发现模型的组合能力取决于其架构。
...
<details>
  <summary>Details</summary>
Motivation: 为地球观测领域的神经压缩和表示学习提供标准化评估框架，解决预训练偏差问题，促进社区驱动的评估方法发展。

Method: 基于固定大小嵌入构建任务无关表示，包含三个核心组件：可重用嵌入评估流程、隐藏任务挑战模式和平衡评分系统，并发布了SSL4EO-S12-downstream数据集。

Result: 在2025 CVPR EARTHVISION研讨会上进行了公开挑战，并与最先进的基础模型进行了消融实验，展示了框架的有效性。

Conclusion: NeuCo-Bench为地球观测及其他领域的神经嵌入标准化评估迈出了第一步，支持可重复性研究。
...
<details>
  <summary>Details</summary>
Motivation: 神经活动预测对理解神经系统和实现闭环控制至关重要，但深度学习在该领域的应用仍然有限，需要系统评估其在神经活动预测中的潜力。

Method: 使用8种概率深度学习模型（包括2个基础模型）和4种经典统计模型、2种基线方法，在小鼠皮层自发神经活动的宽场成像数据上进行系统比较。

Result: 在多个预测时间范围内，多个深度学习模型始终优于传统方法，最佳模型能够对未来1.5秒的神经活动提供信息性预测。
...
<details>
  <summary>Details</summary>
Motivation: 现有校准方法通常对所有预测应用全局变换，忽略了预测的异质可靠性，且改进校准与不确定性感知决策之间的关系尚未得到充分探索。

Method: 使用基于邻近性的共形预测按特征空间语义相似性将校准样本分为正确和错误两组，对正确预测应用标准等渗回归，对错误预测应用欠置信正则化等渗回归以降低置信度。
...
<details>
  <summary>Details</summary>
Motivation: 传统掩码去噪器在反向转换时通常按位置分解，这会削弱联合结构并降低少步生成的质量。

Method: 提出LDDMs，将离散token的掩码扩散与连续潜在嵌入的扩散耦合。包含两种实现：FUJI-LDDMs（完全联合去噪）和SEQ-LDDMs（顺序解析潜在和离散链）。

Result: LDDMs在无条件生成指标上优于最先进的掩码离散扩散基线，在低采样预算下特别有效。

Conclusion: 通过结合离散和连续扩散，LDDMs能够更好地建模跨token依赖关系，在少步生成场景中显著提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型数据遗忘方法存在生成质量下降或遗忘不完整的问题，主要原因是它们在所有扩散时间步平等处理遗忘任务，而实际上遗忘在不同时间和频率上的影响是不均衡的。

Method: 采用时间-频率选择性方法，根据模型和场景特点在不同时间-频率范围内进行选择性训练，应用于基于梯度的和偏好优化的目标，涵盖图像级和文本到图像任务。

Result: 该方法在多种设置下验证了改进效果，能够生成具有更高美学质量和更低噪声的样本，同时实现了更好的数据遗忘效果。

Conclusion: 通过分析和方法建立对扩散模型数据遗忘独特挑战的清晰理解，提供了改进评估和遗忘性能的实用策略，并提出了简化的标准化SSCD评估方法。
...
<details>
  <summary>Details</summary>
Motivation: 受自注意力机制与核主成分分析之间联系的启发，重新审视PCA的基本原理，探索新的理论框架和算法。

Method: 使用DC（差分凸）框架，提出PCA的新公式，包括核化版本和样本外应用，将经典QR算法与DCA联系起来，并开发新的PCA算法。

Result: 建立了PCA与DC框架的理论联系，展示了核化能力和样本外适用性，提出了新的PCA算法并在实验中与先进方法比较。

Conclusion: DC框架为PCA提供了新的理论视角，揭示了经典算法与优化方法的深层联系，并扩展了PCA的核化和鲁棒版本。
...
<details>
  <summary>Details</summary>
Motivation: 当前强化学习方法依赖人工标注的偏好数据或标记数据集进行奖励建模，存在可扩展性瓶颈。本文旨在探索在无标签数据上的强化学习，让模型从连续经验流中自主学习。
...
Conclusion: COMPASS为大语言模型从连续经验中学习提供了更可扩展的方向，通过联合强化可信共识答案和高度决定性的推理链，系统性地增强了模型的分析能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有证据深度学习(EDL)方法虽然计算高效，但其Dirichlet分布假设限制了在复杂或意外情况下的鲁棒性，需要更灵活的不确定性表示方法。

Method: 扩展EDL方法，预测灵活的Dirichlet分布（Dirichlet分布的泛化形式）来建模类别概率，提供更具表达力和适应性的不确定性表示。

Result: 在经典、长尾和噪声等多样化评估场景中，F-EDL实现了最先进的不确定性量化性能，显著提升了UQ的泛化能力和可靠性。

Conclusion: F-EDL通过灵活的Dirichlet分布扩展了EDL框架，在保持计算效率的同时，为机器学习模型在关键应用中提供了更可靠和泛化的不确定性量化能力。
...
<details>
  <summary>Details</summary>
Motivation: 可靠的验证数据是现代语言模型能力提升的关键驱动力，但构建可泛化的合成验证数据仍然困难，现有方法依赖特定任务启发式或后处理过滤器，缺乏原则性的通用验证性评估器。

Method: 采用进化式、任务无关、策略引导的可执行检查数据合成框架，通过一致性评估器强制执行人工标注和策略诱导检查之间的一致性，将过滤升级为原则性合成。

Result: 实验表明，使用合成数据训练在RLVR和模型蒸馏训练范式下都有效，在LiveCodeBench和AgentBench-OS任务上取得显著改进。
...
Method: 通过结构化上下文学习设置，分析损失景观以揭示优化行为，包括推导单层线性自注意力模型的全局最优解闭式表达式，证明参数恢复的NP难性，以及解释多层模型作为预条件梯度下降。

Result: 获得了单层线性自注意力模型在扩大参数空间中的全局最优解闭式表达式，证明了恢复最优Transformer参数是NP难问题，提出了多层模型优化多个目标的预条件梯度下降解释。

Conclusion: 单层线性自注意力在表示结构化动态函数方面存在根本性限制，而多层模型能够通过预条件梯度下降优化更复杂的目标，这些理论结果通过简化Transformer得到了数值验证。
...
Method: 两阶段框架：首先使用基于模拟推理训练的神经后验估计器进行贝叶斯参数推断，然后对生成的参数轨迹应用标准变点检测算法

Result: 在Lorenz-63模型上，Param-CPD相比观测空间基线提高了F1分数，减少了定位误差，降低了误报率
...
Method: 定义了统一模型强化学习的六个场景，为统一理解和生成模型的强化学习提供系统性基线。

Result: 开发了UniRL-Zero框架，能够在一个统一模型中同时提升多模态理解和生成能力。

Conclusion: 该工作为统一理解和生成模型的强化学习提供了系统性基准，代码已开源。
...
### [49] [Demystifying Transition Matching: When and Why It Can Beat Flow Matching](https://arxiv.org/abs/2510.17991)
*Jaihoon Kim,Rajarshi Saha,Minhyuk Sung,Youngsuk Park*

Main category: cs.LG

TL;DR: TM在目标分布具有良好分离模式和显著方差时优于FM，特别是在单峰高斯和高斯混合分布中，TM通过随机差异潜在更新保持目标协方差，实现更快的收敛和更低的KL散度。


<details>
  <summary>Details</summary>
Motivation: 尽管Flow Matching(FM)是许多先进生成模型的基础，但最近研究表明Transition Matching(TM)能以更少的采样步骤获得更高质量。本研究旨在探究TM何时以及为何优于FM。

Method: 首先在单峰高斯分布场景下证明TM获得更低的KL散度，分析其随机差异潜在更新机制；然后扩展到高斯混合分布，识别局部单峰性机制；最后通过高斯分布和真实图像视频生成实验验证理论结果。

Result: 理论证明：在单峰高斯分布中，TM获得严格更低的KL散度；在高斯混合分布中，当模式间距离增加时TM优势更明显；当目标方差趋近零时TM优势减弱。实验验证了理论结果。

Conclusion: TM在目标分布具有良好分离模式和显著方差时优于FM，TM的随机更新机制能更好地保持目标协方差，实现更快的收敛和更低的误差。
...
### [50] [Attention-Guided Deep Adversarial Temporal Subspace Clustering (A-DATSC) Model for multivariate spatiotemporal data](https://arxiv.org/abs/2510.18004)
*Francis Ndikum Nji,Vandana Janeja,Jianwu Wang*

Main category: cs.LG

TL;DR: 提出A-DATSC模型，结合深度子空间聚类生成器和质量验证判别器，用于处理具有复杂时间依赖性的多元时空数据聚类问题。
...
Method: 采用U-Net启发的生成器，通过堆叠TimeDistributed ConvLSTM2D层保持时空完整性；使用图注意力变换器自表达网络捕获局部空间关系、全局依赖和长短程相关性；结合质量验证判别器。

Result: 在三个真实世界多元时空数据集上的实验表明，A-DATSC相比最先进的深度子空间聚类模型实现了显著优越的聚类性能。

Conclusion: A-DATSC模型有效解决了现有深度子空间聚类方法的局限性，在多元时空数据聚类任务中表现出色。
...
### [51] [Cross-Domain Long-Term Forecasting: Radiation Dose from Sparse Neutron Sensor via Spatio-Temporal Operator Network](https://arxiv.org/abs/2510.18041)
...
Method: 引入时空算子网络(STONe)，在传感器和目标流形之间定义非线性算子，无需迭代递归即可在长期预测范围内保持稳定。

Result: 在23年全球中子数据上训练，STONe实现了准确的180天预测，推理延迟仅为毫秒级。

Conclusion: 该框架建立了跨域算子推理的通用原则，为物理、气候和能源系统中的复杂时空场实时预测提供了新途径。
...
### [52] [Measure-Theoretic Anti-Causal Representation Learning](https://arxiv.org/abs/2510.18052)
*Arman Behnam,Binghui Wang*

Main category: cs.LG

TL;DR: 提出了ACIA框架，用于反因果表示学习，通过双层设计捕获标签生成观测的机制，并在高维数据中学习跨环境的稳定因果模式。


<details>
  <summary>Details</summary>
Motivation: 反因果设置下的表示学习面临独特挑战，现有方法存在依赖显式因果结构、难以处理高维数据等局限。

Method: 采用双层设计：低层表示捕获标签生成观测的机制，高层表示学习跨环境变化的稳定因果模式；使用干预核处理完美和不完美干预，无需显式因果结构。

Result: 在合成和真实医疗数据集上的实验表明，ACIA在准确性和不变性指标上均优于现有方法。

Conclusion: ACIA为反因果学习提供了理论保证的稳健框架，能有效处理高维数据并实现跨环境泛化。
...
### [53] [Adaptive Divergence Regularized Policy Optimization for Fine-tuning Generative Models](https://arxiv.org/abs/2510.18053)
*Jiajun Fan,Tong Wei,Chaoran Cheng,Yuxin Chen,Ge Liu*

Main category: cs.LG

TL;DR: ADRPO是一种自适应发散正则化策略优化方法，通过基于优势估计自动调整正则化强度，在强化学习微调生成模型时平衡探索与利用，在文本到图像生成、语言模型和多模态推理任务中均取得优异表现。
...
Method: 引入自适应发散正则化策略优化(ADRPO)，基于优势估计自动调整正则化强度：对高价值样本减少正则化，对差样本应用更强正则化，使策略能根据数据质量在探索与积极利用间导航。

Result: 在文本到图像生成中，2B参数的SD3模型在属性绑定、语义一致性、艺术风格迁移和组合控制方面超越4.8B和12B参数的大模型；在LLM微调中展现逃离局部最优的探索能力；在多模态音频推理中，7B模型超越Gemini 2.5 Pro和GPT-4o Audio等大型商业模型。

Conclusion: ADRPO为不同生成架构和模态中的探索-利用挑战提供了有效的即插即用解决方案，在保持生成多样性的同时显著提升模型性能。
...
### [54] [SPACeR: Self-Play Anchoring with Centralized Reference Models](https://arxiv.org/abs/2510.18060)
*Wei-Jer Chang,Akshay Rangesh,Kevin Joseph,Matthew Strong,Masayoshi Tomizuka,Yihan Hu,Wei Zhan*

Main category: cs.LG

TL;DR: SPACeR框架结合预训练tokenized自回归运动模型与自强化学习，在自动驾驶仿真中实现高效、人类化的多智能体行为，比大型生成模型快10倍、参数少50倍。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：基于扩散或tokenized的模仿学习模型计算昂贵、推理慢、难以适应闭环场景；而自强化学习虽然高效但依赖启发式奖励、容易偏离人类驾驶规范。

Method: 使用预训练tokenized自回归运动模型作为集中式参考策略，指导去中心化自强化学习，通过似然奖励和KL散度将策略锚定在人类驾驶分布上。

Result: 在Waymo Sim Agents Challenge中表现优异，推理速度快10倍，参数规模小50倍；在闭环自我规划评估中能有效测量规划器质量。

Conclusion: SPACeR建立了测试自动驾驶策略的新范式，通过快速可扩展的交通仿真平衡了人类行为逼真度与计算效率。
...
### [55] [Fine-tuning Flow Matching Generative Models with Intermediate Feedback](https://arxiv.org/abs/2510.18072)
*Jiajun Fan,Chaoran Cheng,Shuaike Shen,Xiangxin Zhou,Ge Liu*

Main category: cs.LG

TL;DR: AC-Flow是一个用于流匹配模型的演员-评论家框架，通过奖励塑造、双稳定性机制和可扩展评论家加权方案，解决了文本到图像生成中基于中间反馈的微调挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的流模型微调方法主要依赖结果奖励，难以解决信用分配问题；而直接回归累积奖励的方法在在线设置中面临训练不稳定和模型崩溃的问题。

Method: 提出三个关键创新：1）奖励塑造提供标准化学习信号；2）双稳定性机制结合优势裁剪和预热阶段；3）可扩展的广义评论家加权方案，通过Wasserstein正则化保持模型多样性。
...
### [56] [R2L: Reliable Reinforcement Learning: Guaranteed Return & Reliable Policies in Reinforcement Learning](https://arxiv.org/abs/2510.18074)
*Nadir Farhi*

Main category: cs.LG

TL;DR: 提出了一种可靠的强化学习方法，通过最大化累积回报超过给定阈值的概率来确保性能保证，而不是传统的最大化期望回报。


<details>
  <summary>Details</summary>
Motivation: 许多实际应用（如路由、资源分配、风险决策）不仅需要高平均性能，还需要保证成功的概率，而传统RL算法无法提供这种可靠性保证。

Method: 通过状态增强表示将可靠RL问题重新表述为标准RL问题，使现有RL算法（如Q-learning、Dueling Double DQN）可以直接应用而无需新框架。

Result: 理论证明两种表述的等价性，数值实验在可靠路由问题上显示该方法能有效平衡效率和可靠性。
...
### [57] [Batch Distillation Data for Developing Machine Learning Anomaly Detection Methods](https://arxiv.org/abs/2510.18075)
...
TL;DR: 该研究构建了一个用于化工过程异常检测的机器学习实验数据库，包含119个实验的时间序列数据、传感器读数、NMR谱图、音视频记录等，支持先进ML异常检测方法的开发。


<details>
  <summary>Details</summary>
Motivation: 解决化工过程异常检测中缺乏公开实验数据的问题，为机器学习方法开发提供训练数据支持。

Method: 建立实验室规模的间歇精馏装置，进行119个实验，涵盖正常工况和故意引入异常的工况，收集多种传感器数据、NMR浓度谱图、音视频记录等。

Result: 创建了包含丰富实验数据的结构化数据库，包括时间序列数据、测量不确定性估计、非常规数据源和专家标注，并通过Zenodo平台公开共享。
...
### [58] [MEG-GPT: A transformer-based foundation model for magnetoencephalography data](https://arxiv.org/abs/2510.18080)
*Rukuang Huang,Sungjun Cho,Chetan Gohil,Oiwi Parker Jones,Mark Woolrich*

Main category: cs.LG

TL;DR: 提出了MEG-GPT，一个基于Transformer的基础模型，用于建模大规模脑动力学，通过时间注意力和下一个时间点预测来处理MEG数据，在解码任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统方法无法捕捉MEG等模态中复杂的时空模式，而深度学习在语言和视觉领域的成功表明基础模型在大规模应用中的潜力。

Method: 开发了MEG-GPT模型，使用时间注意力和下一个时间点预测，并引入新的数据驱动tokenizer来保持MEG信号的高时间分辨率。在Cam-CAN数据集（N=612）上训练。

Result: 模型能生成具有真实时空频谱特性的数据，在下游解码任务中表现优异，跨会话准确率从0.54提升到0.59，跨被试准确率从0.41提升到0.49。
...
### [59] [Any-Depth Alignment: Unlocking Innate Safety Alignment of LLMs to Any-Depth](https://arxiv.org/abs/2510.18081)
...
<details>
  <summary>Details</summary>
Motivation: LLMs存在浅层对齐问题：在助手轮开始时能直接拒绝有害查询，但一旦有害内容开始生成，这种保护就会崩溃。需要找到方法在任意生成深度确保安全性。
...
Conclusion: ADA是一种有效且高效的推理时防御方法，能够解锁LLMs的固有浅层对齐能力，在任意生成深度提供强大的安全保障。
...
### [60] [Enhancing mortality prediction in cardiac arrest ICU patients through meta-modeling of structured clinical data from MIMIC-IV](https://arxiv.org/abs/2510.18103)
*Nursultan Mamatov,Philipp Kellmeyer*

Main category: cs.LG

TL;DR: 开发结合结构化临床数据和非结构化文本信息的机器学习模型，用于ICU住院死亡率的早期预测，相比仅使用结构化数据，AUC从0.753提升到0.918。
...
Result: 结合结构和文本特征的最终逻辑回归模型AUC达到0.918，相比仅使用结构化数据的0.753有22%的相对提升，决策曲线分析显示在0.2-0.8阈值概率范围内具有优越的标准化净效益。
...
### [61] [Gradient Variance Reveals Failure Modes in Flow-Based Generative Models](https://arxiv.org/abs/2510.18118)
...
<details>
  <summary>Details</summary>
Motivation: 分析Rectified Flows中直线路径目标隐藏的基本失效模式，揭示确定性训练下低梯度方差导致记忆化问题的机制

Method: 研究高斯到高斯传输，通过损失梯度方差分析随机和确定性机制下的向量场偏好，在CelebA数据集上进行实证验证

Result: 确定性插值导致记忆化，即使在所有插值线相交的情况下，推理时也会重现训练时的特定配对；添加小噪声可以恢复泛化
...
### [62] [Efficient Long-context Language Model Training by Core Attention Disaggregation](https://arxiv.org/abs/2510.18121)
*Yonghao Zhuang,Junda Chen,Bo Pang,Yi Gu,Yibo Zhu,Yimin Jiang,Ion Stoica,Eric Xing,Hao Zhang*

Main category: cs.LG

TL;DR: 提出核心注意力解耦(CAD)技术，通过将核心注意力计算从模型中分离并在专用设备上执行，解决长上下文训练中的负载不均衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有系统中核心注意力与其他层共存，在长上下文长度下其二次计算增长导致数据并行和流水线并行组中的负载不均衡和拖尾效应。

Method: 将核心注意力分解为token级任务并分发到专用注意力服务器，动态重新批处理任务以均衡计算而不牺牲内核效率。

Result: 在512个H200 GPU和512k token上下文长度下，DistCA系统将端到端训练吞吐量提升1.35倍，消除了数据并行和流水线并行拖尾，实现近乎完美的计算和内存平衡。
...
### [63] [HyperDiffusionFields (HyDiF): Diffusion-Guided Hypernetworks for Learning Implicit Molecular Neural Fields](https://arxiv.org/abs/2510.18122)
*Sudarshan Babu,Phillip Lo,Xiao Zhang,Aadi Srivastava,Ali Davariashtiyani,Jason Perera,Michael Maire,Aly A. Khan*

Main category: cs.LG

TL;DR: HyperDiffusionFields (HyDiF) 是一个将3D分子构象建模为连续场的框架，使用分子方向场(MDF)表示分子结构，并通过超网络和扩散模型实现分子生成和属性预测。


<details>
  <summary>Details</summary>
Motivation: 传统方法将分子表示为离散原子坐标或图结构，难以支持空间细粒度特征提取和条件生成任务。HyDiF旨在通过连续场表示克服这些限制。

Method: 使用分子方向场(MDF)表示分子，通过分子特定神经隐式场(MNF)建模，采用超网络生成MNF权重，并训练为去噪扩散模型实现生成能力。

Result: 该方法支持无条件生成、结构条件生成(如分子修复)，并能进行空间细粒度特征提取用于分子属性预测，且可扩展到大型生物分子。

Conclusion: HyDiF展示了基于场的分子建模的潜力，为分子生成和属性预测提供了新的连续表示方法。
...
### [64] [Nash Policy Gradient: A Policy Gradient Method with Iteratively Refined Regularization for Finding Nash Equilibria](https://arxiv.org/abs/2510.18183)
...
<details>
  <summary>Details</summary>
Motivation: 传统正则化方法需要将正则化强度逐渐减小到零来逼近纳什均衡，这在实际中往往导致学习不稳定。本文旨在开发一种更稳定的方法。

Method: 固定正则化强度以保持鲁棒性，通过迭代优化参考策略实现收敛。开发了NashPG算法，结合策略梯度方法的通用性，仅依赖当前和参考策略。

Result: 理论证明该方法在双人零和博弈中保证严格单调改进并收敛到精确纳什均衡。在经典基准游戏和大型领域（如Battleship和No-Limit Texas Hold'em）上，NashPG达到可比或更低的可利用性，并获得更高的Elo评分。

Conclusion: NashPG提供了一种稳定有效的方法来寻找纳什均衡，在理论和实验上都表现出色，特别适用于大规模不完全信息博弈。
...
### [65] [ActivationReasoning: Logical Reasoning in Latent Activation Spaces](https://arxiv.org/abs/2510.18184)
...
TL;DR: ActivationReasoning (AR) 框架将显式逻辑推理嵌入LLM的潜在空间中，通过三个步骤：识别潜在概念表示、激活命题、逻辑推理，从而提升模型的可解释性和可控性。
...
Method: AR框架包含三个阶段：(1) 寻找潜在表示：识别潜在概念表示并组织成字典；(2) 激活命题：在推理时检测激活概念并映射到逻辑命题；(3) 逻辑推理：应用逻辑规则推断高阶结构、组合新概念并引导模型行为。

Result: 在多个任务上的评估显示，AR能够随着推理复杂性稳健扩展，泛化到抽象和上下文敏感任务，并在不同模型骨干间迁移。在PrOntoQA、Rail2Country、ProverQA和BeaverTails等任务上均表现良好。

Conclusion: 将逻辑结构嵌入潜在激活不仅提高了透明度，还实现了结构化推理、可靠控制和与期望行为的一致性，为构建更可靠和可审计的AI提供了路径。
...
### [66] [Ensemble based Closed-Loop Optimal Control using Physics-Informed Neural Networks](https://arxiv.org/abs/2510.18195)
...
<details>
  <summary>Details</summary>
Motivation: HJB方程提供了最优控制系统设计的框架，但数值求解计算量大，解析解通常不可得。基于物理信息的神经网络等知识引导的机器学习方法为求解HJB方程提供了新途径。

Method: 采用多阶段集成框架学习最优成本函数和最优控制信号，不使用稳定项，支持单一学习控制信号或集成控制信号策略。

Result: 在稳态时不变两状态连续非线性系统的闭环控制中成功验证，能够处理噪声扰动系统状态和变化的初始条件。
...
### [67] [Joint Optimization of Cooperation Efficiency and Communication Covertness for Target Detection with AUVs](https://arxiv.org/abs/2510.18225)
...
TL;DR: 提出了一种用于水下目标检测的分层行动管理框架，通过联合优化轨迹和功率控制来平衡合作效率与通信隐蔽性。


<details>
  <summary>Details</summary>
Motivation: 解决自主水下航行器(AUV)在合作目标检测中合作效率与通信隐蔽性之间的关键权衡问题。

Method: 采用分层行动管理框架：宏观层面使用马尔可夫决策过程和近端策略优化算法进行任务分配；微观层面使用部分可观测马尔可夫决策过程和多智能体近端策略优化算法调整轨迹和传输功率。

Result: 在集中训练和分散执行范式下，实现了满足能量和移动性约束的自适应隐蔽合作。

Conclusion: 为多AUV系统的高效安全运行提供了理论见解和实用解决方案，对水下隐蔽通信任务执行具有重要意义。
...
### [68] [Towards Fast LLM Fine-tuning through Zeroth-Order Optimization with Projected Gradient-Aligned Perturbations](https://arxiv.org/abs/2510.18228)
*Zhendong Mi,Qitao Tan,Grace Li Zhang,Zhaozhuo Xu,Geng Yuan,Shaoyi Huang*

Main category: cs.LG

TL;DR: 提出P-GAP方法，通过投影梯度对齐扰动的零阶优化实现快速LLM微调，显著减少训练迭代次数和GPU时间


<details>
  <summary>Details</summary>
Motivation: 现有零阶优化方法在梯度估计中存在高方差问题，导致收敛缓慢和性能不佳，需要更高效的LLM微调方法

Method: 首先估计低维梯度空间，然后在投影梯度方向上对齐扰动，减少扰动参数数量和方差

Result: 在分类任务上准确率提升6%，生成任务上准确率提升12%，训练迭代减少81%，GPU时间减少70%
...
### [69] [ACTG-ARL: Differentially Private Conditional Text Generation with RL-Boosted Control](https://arxiv.org/abs/2510.18232)
*Yuzheng Hu,Ryan McKenna,Da Yu,Shanshan Wu,Han Zhao,Zheng Xu,Peter Kairouz*

Main category: cs.LG

TL;DR: 提出ACTG-ARL方法，通过分层框架和锚定强化学习，在差分隐私下生成高质量可控的合成文本，相比之前工作提升20% MAUVE指标。


<details>
  <summary>Details</summary>
Motivation: 现有差分隐私合成文本方法存在统计属性保留不足、效用损失严重、缺乏细粒度控制等问题，需要改进合成文本质量和生成控制能力。

Method: 采用分层框架：特征学习+条件文本生成，使用DP表格合成器和DP微调条件生成器；提出锚定强化学习(ARL)提升指令跟随能力。

Result: 在强隐私保证下，合成文本质量提升20% MAUVE，条件生成器的控制能力显著改善。

Conclusion: ACTG-ARL方法在差分隐私下成功实现了高质量、可控的文本合成，为隐私保护的模型训练和评估提供了有效解决方案。
...
### [70] [Fostering the Ecosystem of AI for Social Impact Requires Expanding and Strengthening Evaluation Standards](https://arxiv.org/abs/2510.18238)
*Bryan Wilder,Angela Zhou*

Main category: cs.LG

TL;DR: 论文主张AI/ML社会影响研究需要更广泛的社会影响定义和更严格的部署系统评估，反对过度强调同时实现部署和方法创新的评审标准。


<details>
  <summary>Details</summary>
Motivation: 当前AI/ML社会影响研究的评审标准过度强调同时实现部署和方法创新，这削弱了更广泛社会影响研究生态系统的可持续性，不利于满足项目合作伙伴的实际需求。

Method: 提出立场性观点，建议研究者和评审者应采纳：1）超越部署的更广泛社会影响概念；2）对部署系统影响进行更严格评估。

Result: 识别了当前评审标准存在的问题，提出了改进方向，旨在促进更可持续的社会影响研究生态系统。

Conclusion: AI/ML社会影响研究需要重新定义评审标准，支持在应用或方法单方面做出贡献的项目，以更好地满足实际社会需求。
...
### [71] [Learning with Dual-level Noisy Correspondence for Multi-modal Entity Alignment](https://arxiv.org/abs/2510.18240)
*Haobin Li,Yijie Lin,Peng Hu,Mouxing Yang,Xi Peng*

Main category: cs.LG

TL;DR: 提出RULE框架解决多模态实体对齐中的双重噪声对应问题，包括实体-属性和图间对应关系的噪声，通过可靠性估计和对应推理提高对齐准确性


<details>
  <summary>Details</summary>
Motivation: 现实世界多模态知识图谱中存在实体-属性和图间对应关系的双重噪声，现有方法假设这些对应关系完美，但实际依赖专家标注存在错误

Method: RULE框架：1）通过双重原则估计对应关系可靠性；2）在属性融合时减轻实体内噪声影响；3）在图间差异消除时防止过拟合噪声对应；4）加入对应推理模块发现图间属性连接

Result: 在五个基准测试上验证了有效性，相比七种最先进方法表现更好

Conclusion: RULE框架能有效处理多模态实体对齐中的双重噪声对应问题，提高实体识别准确性
...
### [72] [Scaling Laws Meet Model Architecture: Toward Inference-Efficient LLMs](https://arxiv.org/abs/2510.18245)
*Song Bian,Tao Yu,Shivaram Venkataraman,Youngsuk Park*

Main category: cs.LG

TL;DR: 本文提出条件缩放定律，将架构信息融入Chinchilla框架，通过优化隐藏大小、MLP与注意力参数分配比例和分组查询注意力等关键架构因素，在保持准确性的同时显著提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型参数规模和训练数据量的增长，推理成本已成为紧迫问题。模型准确性与推理效率之间的权衡关系尚未得到充分探索。

Method: 引入条件缩放定律扩展Chinchilla框架，结合架构信息；开发搜索框架识别同时具备推理效率和准确性的架构；训练超过200个模型（80M到3B参数，8B到100B训练token）来拟合提出的缩放定律。

Result: 条件缩放定律能可靠预测最优架构选择，优化后的模型在相同训练预算下比LLaMA-3.2准确率提高2.1%，推理吞吐量提升42%。

Conclusion: 通过系统优化关键架构因素，可以在不牺牲准确性的前提下显著提升大语言模型的推理效率，为实际部署提供了重要指导。
...
### [73] [NTKMTL: Mitigating Task Imbalance in Multi-Task Learning from Neural Tangent Kernel Perspective](https://arxiv.org/abs/2510.18258)
...
<details>
  <summary>Details</summary>
Motivation: 多任务学习中任务不平衡是一个主要挑战，虽然平衡不同任务的收敛速度是有效方法，但在复杂MTL系统中准确描述训练动态和收敛速度非常困难

Method: 利用神经正切核理论分析MTL训练动态，引入扩展的NTK矩阵，采用谱分析平衡多任务收敛速度，并提出基于共享表示近似的NTKMTL-SR提高训练效率

Result: 在多个基准测试中（包括多任务监督学习和多任务强化学习）实现了最先进的性能

Conclusion: 提出的NTKMTL方法能有效缓解多任务学习中的任务不平衡问题，在保持竞争力的同时提高训练效率
...
### [74] [From Competition to Synergy: Unlocking Reinforcement Learning for Subject-Driven Image Generation](https://arxiv.org/abs/2510.18263)
*Ziwei Huang,Ying Shu,Hao Fang,Quanyu Long,Wenya Wang,Qiushi Guo,Tiezheng Ge,Leilei Gan*

Main category: cs.LG

TL;DR: 提出了Customized-GRPO框架，通过Synergy-Aware Reward Shaping和Time-Aware Dynamic Weighting解决主题驱动图像生成中身份保真度与提示遵循度之间的权衡问题，显著优于朴素GRPO基线。


<details>
  <summary>Details</summary>
Motivation: 解决主题驱动图像生成模型中身份保真度与提示遵循度之间的基本权衡问题，发现朴素GRPO应用会导致竞争性退化，因为静态权重的线性奖励聚合会产生冲突的梯度信号并与扩散过程的时间动态不匹配。

Method: 提出Customized-GRPO框架，包含两个关键创新：(i) Synergy-Aware Reward Shaping - 非线性机制，显式惩罚冲突奖励信号并放大协同信号；(ii) Time-Aware Dynamic Weighting - 通过早期优先提示遵循、后期优先身份保真来对齐优化压力与模型时间动态。

Result: 广泛实验表明该方法显著优于朴素GRPO基线，成功缓解竞争性退化，在保持关键身份特征的同时准确遵循复杂文本提示，实现优越的平衡。

Conclusion: Customized-GRPO框架通过奖励整形和动态加权机制有效解决了主题驱动图像生成中的身份保真度与提示遵循度权衡问题，生成图像在保真度和编辑性方面达到更好的平衡。
...
### [75] [Online Time Series Forecasting with Theoretical Guarantees](https://arxiv.org/abs/2510.18281)
*Zijian Li,Changze Zhou,Minghao Fu,Sanjay Manjunath,Fan Feng,Guangyi Chen,Yingyao Hu,Ruichu Cai,Kun Zhang*

Main category: cs.LG

TL;DR: 提出了一个具有理论保证的在线时间序列预测框架TOT，通过引入潜在变量来应对未知的分布漂移，并设计了模型无关的实现方法。
...
Method: 提出了TOT理论框架，证明引入潜在变量可以降低贝叶斯风险；设计了包含时间解码器和两个独立噪声估计器的模型无关架构，分别建模潜在变量的因果推理和观测变量的混合过程。

Result: 在合成数据上的实验结果支持理论主张；基于多个基线的插件实现在多个基准测试中普遍提升了性能，显示了在实际应用中的有效性。

Conclusion: TOT框架通过引入潜在变量有效应对在线时间序列预测中的分布漂移问题，具有理论保证和实际应用价值。
...
### [76] [Physics-Informed Parametric Bandits for Beam Alignment in mmWave Communications](https://arxiv.org/abs/2510.18299)
*Hao Qin,Thang Duong,Ming Li,Chicheng Zhang*

Main category: cs.LG

TL;DR: 提出了两种基于物理信息的bandit算法pretc和prgreedy，利用毫米波信道的稀疏多径特性进行波束对准和跟踪，在多种场景下优于现有方法。
...
Method: pretc从随机探索阶段开始，然后基于估计的奖励函数选择最优波束；prgreedy在线进行估计并选择当前估计下的最佳波束。两种算法将每条路径参数视为黑盒，基于采样历史奖励维护最优估计。

Result: 在合成DeepMIMO数据集和真实世界DeepSense6G数据集上的实验表明，两种算法在多种信道环境下的广泛场景中优于现有方法。

Conclusion: 所提算法具有通用性和鲁棒性，能够有效利用毫米波信道的稀疏多径特性，在波束对准和跟踪任务中表现优异。
...
### [77] [Towards Identifiability of Hierarchical Temporal Causal Representation Learning](https://arxiv.org/abs/2510.18310)
*Zijian Li,Minghao Fu,Junxian Huang,Yifan Shen,Ruichu Cai,Yuewen Sun,Guangyi Chen,Kun Zhang*

Main category: cs.LG

TL;DR: 提出了CHiLD框架，通过三个条件独立观测来唯一确定分层潜变量的联合分布，解决了现有方法无法从单时间步观测变量恢复分层潜变量联合分布的问题。


<details>
  <summary>Details</summary>
Motivation: 现有时间因果表示学习方法无法捕捉真实世界任务中跨多个抽象层次的分层潜动态，因为它们无法从单时间步观测变量恢复分层潜变量的联合分布。

Method: 使用时间上下文观测变量识别多层潜变量的联合分布，利用分层结构的自然稀疏性识别每层内的潜变量，开发基于变分推理的时间序列生成模型，包含上下文编码器和基于归一化流的分层先验网络。

Result: 在合成和真实世界数据集上的实证评估验证了理论主张，证明了CHiLD在建模分层潜动态方面的有效性。

Conclusion: CHiLD框架能够唯一确定分层潜变量的联合分布，成功解决了分层潜动态建模的挑战，为时间序列分析提供了新的解决方案。
...
### [78] [Higher Embedding Dimension Creates a Stronger World Model for a Simple Sorting Task](https://arxiv.org/abs/2510.18315)
*Brady Bhalla,Honglu Fan,Nancy Chen,Tony Yue YU*

Main category: cs.LG

TL;DR: 研究了嵌入维度如何影响Transformer在强化学习排序任务中内部世界模型的形成，发现更大嵌入维度能产生更忠实、一致和鲁棒的内部表示，并揭示了两种机制：注意力权重最后一行单调编码全局排序，选择的交换操作与编码值的最大相邻差异对齐。


<details>
  <summary>Details</summary>
Motivation: 探究嵌入维度对Transformer在强化学习排序任务中内部世界模型形成的影响，理解模型如何构建结构化内部表示。

Method: 使用强化学习训练Transformer执行冒泡排序式的相邻交换操作，通过数百次实验分析不同嵌入维度下模型的内部表示机制。

Result: 模型即使在小嵌入维度下也能达到高准确率，但更大维度能产生更忠实、一致和鲁棒的内部表示，并观察到两种一致机制：注意力权重最后一行单调编码全局排序，选择的交换操作与编码值的最大相邻差异对齐。

Conclusion: Transformer构建了结构化的内部世界模型，模型大小不仅提升最终性能，还改善了表示质量，为类似算法任务的分析提供了可用的指标和方法。
...
### [79] [Scalable, Explainable and Provably Robust Anomaly Detection with One-Step Flow Matching](https://arxiv.org/abs/2510.18328)
...
<details>
  <summary>Details</summary>
Motivation: 现有连续时间模型（如DTE）在异常检测中推理成本过高，需要一种既保持高精度又降低推理成本的方法。

Method: TCCM学习向固定目标（原点）的时间条件收缩向量，无需在训练和推理中求解常微分方程，使用单步偏差评分策略。

Result: 在ADBench基准测试中，TCCM在检测精度和推理成本之间达到良好平衡，尤其在高维和大规模数据集上优于现有方法。

Conclusion: TCCM提供了一种轻量级、可扩展且可解释的异常检测方法，具有理论鲁棒性保证。
...
### [80] [Why Policy Gradient Algorithms Work for Undiscounted Total-Reward MDPs](https://arxiv.org/abs/2510.18340)
*Jongmin Lee,Ernest K. Ryu*

Main category: cs.LG

TL;DR: 本文分析了无折扣总奖励设置下的策略梯度方法，解决了γ=1时现有理论不适用的问题，通过状态分类和新的瞬态访问度量来建立收敛保证。


<details>
  <summary>Details</summary>
Motivation: 现代基于策略的强化学习算法（特别是大语言模型中的策略优化）使用无折扣总奖励设置（γ=1），但现有理论分析大多假设γ<1，导致理论不适用。

Method: 基于两个关键洞察：(1) 将MDP状态分类为循环状态和瞬态状态，这种分类在采用softmax输出层的策略下保持不变；(2) 用新的瞬态访问度量替代经典的状态访问度量。

Result: 为无折扣期望总奖励无限时域MDPs的策略梯度方法提供了理论分析框架。

Conclusion: 提出的瞬态访问度量和状态分类方法能够有效分析γ=1情况下的策略梯度方法，填补了现有理论空白。
...
### [81] [Computable universal online learning](https://arxiv.org/abs/2510.18352)
*Dariusz Kalociński,Tomasz Steifer*

Main category: cs.LG

TL;DR: 该论文研究了可计算通用在线学习问题，证明了即使假设类在可计算性理论中相对简单，通用在线学习也不意味着可计算通用在线学习，并对可计算通用在线学习的变体进行了精确刻画。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习理论中的学习特性刻画往往将学习视为抽象数学对象，而忽略了关键问题：学习何时可以作为计算机程序实现？本文旨在解决通用在线学习中的可计算实现问题。

Method: 研究通用在线学习模型中的可计算性，分析Adversary（扮演Nature角色）在保持局部一致性的前提下改变策略的情况，要求Learner使用可计算策略实现有限错误。

Result: 证明通用在线学习不蕴含可计算通用在线学习，即使假设类在可计算性理论中相对简单；对可计算通用在线学习的不可知变体和适当变体提供了精确刻画。

Conclusion: 研究结果为在线二分类和相关归纳推理问题提供了更现实的视角，揭示了理论学习特性与实际可计算实现之间的重要差异。
...
### [82] [Ensembling Pruned Attention Heads For Uncertainty-Aware Efficient Transformers](https://arxiv.org/abs/2510.18358)
...
Method: 提出Hydra Ensembles，剪枝Transformer的注意力头创建多样化集成成员，通过新的多头注意力和分组全连接层合并成员，形成紧凑模型。

Result: 在图像和文本分类任务中，Hydra Ensembles在多种架构上持续优于Deep Ensembles。在ImageNet-1k零样本分类中，无需额外训练即超越现有最优方法。

Conclusion: Hydra Ensembles提供了一种高效的集成方法，在保持高性能的同时显著降低了计算和内存成本，为大型模型的不确定性量化提供了可行方案。
...
### [83] [Learning to Flow from Generative Pretext Tasks for Neural Architecture Encoding](https://arxiv.org/abs/2510.18360)
*Sunwoo Kim,Hyunjin Hwang,Kijung Shin*

Main category: cs.LG

TL;DR: FGP是一种新的神经网络架构编码预训练方法，通过训练编码器重构信息流的代理表示，在不使用专门模型结构的情况下捕获信息流，显著提升了编码器性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于信息流的神经网络架构编码器虽然性能优秀，但由于复杂的模型结构导致处理速度较慢，这在实际应用中构成了显著挑战。

Method: 提出FGP预训练方法，训练编码器重构神经网络架构信息流的代理表示，无需专门模型结构即可捕获信息流。

Result: 实验表明FGP将编码器性能提升了高达106%（在Precision-1%指标上），相比仅使用监督学习的相同编码器。

Conclusion: FGP方法有效解决了现有基于信息流的编码器速度慢的问题，同时显著提升了编码器性能，为神经网络架构搜索提供了更实用的解决方案。
...
### [84] [Towards Unsupervised Open-Set Graph Domain Adaptation via Dual Reprogramming](https://arxiv.org/abs/2510.18363)
...
<details>
  <summary>Details</summary>
Motivation: 现有的图域自适应模型主要关注闭集设置，但现实场景中目标域可能包含源域中不存在的类别，因此需要解决开放集图域自适应问题。

Method: GraphRTA框架在图形和模型两侧进行重编程：通过修改目标图结构和节点特征来促进已知类和未知类的分离；通过剪枝领域特定参数来减少对源图的偏见；扩展分类器以包含未知类维度。

Result: 在多个公共数据集上的综合实验表明，所提出的模型相比现有最先进基线方法能够取得满意的性能。

Conclusion: GraphRTA框架有效解决了无监督开放集图域自适应问题，无需手动指定阈值即可识别未知类，并在实验中表现出优越性能。
...
### [85] [Training Diverse Graph Experts for Ensembles: A Systematic Empirical Study](https://arxiv.org/abs/2510.18370)
...
<details>
  <summary>Details</summary>
Motivation: 单个GNN的性能受限于真实图数据中的异质性，而集成多个具有不同泛化模式的GNN可以显著提升性能，但目前缺乏对专家级多样化技术的系统性研究。

Method: 评估20种多样化策略，包括随机重新初始化、超参数调优、架构变化、方向性建模和训练数据划分等方法，在14个节点分类基准上构建200多个集成变体。
...
### [86] [Approximation Rates of Shallow Neural Networks: Barron Spaces, Activation Functions and Optimality Analysis](https://arxiv.org/abs/2510.18388)
*Jian Lu,Xiaohuang Huang*

Main category: cs.LG

TL;DR: 该论文研究了具有指数函数幂次激活函数的浅层神经网络的逼近性质，重点分析了在Barron函数空间中逼近率对维度和函数光滑度的依赖性。


<details>
  <summary>Details</summary>
Motivation: 研究浅层神经网络在Barron函数空间中的逼近能力，特别是激活函数为指数函数幂次时，逼近率与维度、函数光滑度的关系，以理解维度诅咒现象。

Method: 分析ReLU^k激活函数的逼近率，证明在ℓ^1有界系数或光滑度不足条件下无法达到最优逼近率，并在不同范数下建立Barron空间和Sobolev空间的最优逼近率。

Result: 证明了浅层神经网络在特定条件下无法达到最优逼近率，确认了维度诅咒的存在，并在不同函数空间中建立了最优逼近率。
...
### [87] [Learning from N-Tuple Data with M Positive Instances: Unbiased Risk Estimation and Theoretical Guarantees](https://arxiv.org/abs/2510.18406)
...
<details>
  <summary>Details</summary>
Motivation: 弱监督学习通常使用粗粒度的聚合信号而非实例标签。本文研究每个训练样本是包含m个正例的n元组，但只观察到每组的计数m的情况，这种NTMP监督出现在图像分类的区域提议和多实例测量等场景中。

Method: 通过将元组生成过程与潜在实例边缘概率联系起来，推导出闭式无偏风险估计器(URE)，扩展到可变元组大小、可变计数及其组合。引入ReLU校正来提高有限样本稳定性，同时保持渐近正确性。

Result: 在转换为NTMP任务的基准测试中，该方法始终优于代表性的弱监督基线方法，在精确率-召回率和F1分数权衡方面表现良好。在类别先验不平衡和不同元组配置下保持稳健。

Conclusion: 计数监督可以通过理论严谨且实践稳定的目标有效利用，证明了仅使用计数信息也能实现有效的弱监督学习。
...
### [88] [Provable Generalization Bounds for Deep Neural Networks with Adaptive Regularization](https://arxiv.org/abs/2510.18410)
*Adeel Safder*
...
<details>
  <summary>Details</summary>
Motivation: 深度神经网络容易过拟合，需要更好的正则化方法来提高泛化性能，特别是在非凸优化环境中。

Method: MAGDrop：基于当前梯度和累积动量动态调整激活层dropout率的正则化方法。

Result: 在MNIST上达到99.52%测试准确率，CIFAR-10上达到90.63%，比标准dropout和自适应梯度正则化方法高1-2%。

Conclusion: MAGDrop通过动态dropout机制有效提升DNN泛化能力，为高风险应用提供了稳健框架。
...
### [89] [Learning Boltzmann Generators via Constrained Mass Transport](https://arxiv.org/abs/2510.18460)
*Christopher von Klitzing,Denis Blessing,Henrik Schopmans,Pascal Friederich,Gerhard Neumann*

Main category: cs.LG

TL;DR: 提出了一种名为约束质量传输(CMT)的变分框架，用于高效采样高维多模态概率分布，在玻尔兹曼生成器中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有变分方法在采样高维多模态分布时的模式崩溃问题，以及退火方法中的质量传输和调度调优困难。

Method: 引入约束质量传输(CMT)框架，在生成中间分布时对KL散度和熵衰减施加约束，增强分布重叠并防止过早收敛。

Result: 在标准玻尔兹曼生成器基准测试和新引入的ELIL四肽系统上，CMT始终优于最先进的变分方法，有效样本量提高2.5倍以上且避免模式崩溃。

Conclusion: CMT框架通过约束KL散度和熵衰减，有效解决了高维多模态分布采样中的关键挑战，在复杂系统中表现出色。
...
### [90] [Simple and Efficient Heterogeneous Temporal Graph Neural Network](https://arxiv.org/abs/2510.18467)
*Yili Wang,Tairan Huang,Changlong He,Qiutong Li,Jianliang Gao*

Main category: cs.LG

TL;DR: 提出SE-HTGNN模型，通过动态注意力机制将时序建模集成到空间学习中，并利用大语言模型作为先验知识，在异构时序图上实现高效准确的表示学习。


<details>
  <summary>Details</summary>
Motivation: 现有方法采用解耦的时空学习范式，削弱了时空信息交互并导致模型复杂度高，需要一种更有效的异构时序图学习范式。

Method: 创新性地通过动态注意力机制将时序建模集成到空间学习中，保留历史图快照的注意力信息来指导后续计算；同时利用大语言模型捕捉节点类型的隐式属性作为先验知识。

Result: 实验表明SE-HTGNN比最先进的基线方法快10倍，同时保持最佳预测精度。

Conclusion: SE-HTGNN通过集成时空学习和引入大语言模型先验知识，在异构时序图上实现了高效且准确的表示学习。
...
### [91] [Benchmarking Fairness-aware Graph Neural Networks in Knowledge Graphs](https://arxiv.org/abs/2510.18473)
*Yuya Sasaki*

Main category: cs.LG

TL;DR: 该论文对知识图谱上的公平感知图神经网络进行基准测试，发现知识图谱在准确性与公平性权衡方面与其他图数据集不同，且性能受多种因素影响。


<details>
  <summary>Details</summary>
Motivation: 现有公平感知GNN研究未在知识图谱上进行评估，而知识图谱是许多应用中的重要图结构数据。

Method: 从YAGO、DBpedia和Wikidata生成新图，在不同GNN骨干网络和早停条件下基准测试预处理和过程处理方法。

Result: 发现知识图谱在公平感知GNN中显示出与其他图不同的趋势，性能受GNN骨干网络和早停条件影响，预处理方法改善公平性指标，过程处理方法提高预测准确性。

Conclusion: 知识图谱在公平性研究中有独特特性，需要综合考虑方法类型、GNN架构和训练策略来平衡准确性与公平性。
...
### [92] [Safe But Not Sorry: Reducing Over-Conservatism in Safety Critics via Uncertainty-Aware Modulation](https://arxiv.org/abs/2510.18478)
*Daniel Bethell,Simos Gerasimou,Radu Calinescu,Calum Imrie*

Main category: cs.LG

TL;DR: 提出不确定安全评论家(USC)方法，通过不确定性感知的调制和精炼来平衡强化学习的安全性与性能，在保持竞争力的奖励同时减少约40%的安全违规。


<details>
  <summary>Details</summary>
Motivation: 现有方法在强化学习安全探索中存在平衡问题：严格的安全方法会损害任务性能，而优先奖励的方法则频繁违反安全约束，产生平坦的成本梯度阻碍策略改进。

Method: 引入不确定安全评论家(USC)，将不确定性感知的调制和精炼集成到评论家训练中，在不确定和高成本区域集中保守性，同时在安全区域保持尖锐梯度。

Result: 实验显示USC减少约40%的安全违规，同时保持竞争力或更高的奖励，预测与真实成本梯度误差减少约83%。
...
### [93] [Learning to Navigate Under Imperfect Perception: Conformalised Segmentation for Safe Reinforcement Learning](https://arxiv.org/abs/2510.18485)
*Daniel Bethell,Simos Gerasimou,Radu Calinescu,Calum Imrie*
...
Method: 将分布无关的有限样本安全保证集成到语义分割中，生成带有严格漏检界限的校准危险地图，这些地图为下游RL规划诱导风险感知成本场。

Result: 在两个卫星基准测试中，COPPOL相比基线方法将危险覆盖率提高最多6倍，实现近乎完全的不安全区域检测，同时将导航过程中的危险违规减少约50%。
...
### [94] [Alibaba International E-commerce Product Search Competition DILAB Team Technical Report](https://arxiv.org/abs/2510.18499)
*Hyewon Lee,Junghyun Oh,Minkyung Song,Soyoung Park,Seunghoon Han*

Main category: cs.LG

TL;DR: DILAB团队开发的多语言电商搜索系统在最终排行榜中排名第5，综合得分0.8819。该系统采用多阶段流水线设计，包括数据精炼、轻量预处理和自适应建模，在查询类别和查询物品任务中表现稳定且高效。


<details>
  <summary>Details</summary>
Motivation: 解决多语言查询-物品理解中的挑战，提升电商搜索系统在多语言环境下的性能和稳定性。

Method: 采用多阶段流水线：数据精炼阶段增强数据集一致性和类别覆盖；轻量预处理包括语言标记和噪声过滤；建模阶段探索多种架构和微调策略，使用精选验证集优化超参数。

Result: 系统在最终排行榜中排名第5，综合得分0.8819，在查询类别和查询物品任务中均表现稳定且高效，展示了跨语言和跨领域的鲁棒性和适应性。

Conclusion: 系统化的数据管理和迭代评估对多语言搜索系统非常有效，提出的框架在语言和领域间具有鲁棒性和适应性。
...
### [95] [Partial VOROS: A Cost-aware Performance Metric for Binary Classifiers with Precision and Capacity Constraints](https://arxiv.org/abs/2510.18520)
*Christopher Ratigan,Kyle Heuton,Carissa Wang,Lenore Cowen,Michael C. Hughes*

Main category: cs.LG

TL;DR: 本文提出了一种改进的ROC分析方法，用于医院警报系统等应用场景，考虑了精度约束、预测阳性数量上限以及不对称的误报和漏报成本。


<details>
  <summary>Details</summary>
Motivation: 传统ROC分析无法捕捉影响部署的关键因素，如避免误报警疲劳的最低精度约束、医院工作人员容量的预测阳性数量上限，以及不对称的误报和漏报成本。

Method: 展示了满足给定精度和容量约束的分类器子集如何在ROC空间中表示为可行区域，建立了该区域的几何结构，定义了只考虑ROC空间可行部分的性能指标——较差分类器的部分面积。

Result: 在MIMIC-IV数据集上预测死亡风险的实验中，这种成本感知指标在排名医院警报应用中的分类器方面优于替代方法。

Conclusion: 提出的部分VOROS指标能够更好地评估医院警报系统等应用中的分类器性能，考虑了实际部署中的约束和成本因素。
...
### [96] [Pay Attention to the Triggers: Constructing Backdoors That Survive Distillation](https://arxiv.org/abs/2510.18541)
...
<details>
  <summary>Details</summary>
Motivation: LLMs常被用作教师模型进行知识蒸馏，但由于这些教师模型可能来自不可信方，蒸馏过程会带来安全风险。现有LLM后门方法选择的触发词在正常上下文中很少出现，导致后门难以转移到学生模型，这低估了知识蒸馏的安全风险。

Method: 提出了T-MTB后门技术，构建由多个特定令牌组成的复合后门触发器，这些令牌在预期的蒸馏数据集中经常单独出现。这样中毒的教师模型保持隐蔽，而在蒸馏过程中这些令牌的单独存在为后门转移到学生模型提供了足够信号。

Result: 使用T-MTB技术，在越狱和内容调控两种攻击场景以及四个LLM模型家族中，成功演示并广泛研究了可转移后门的安全风险。
...
### [97] [RAISE: A Unified Framework for Responsible AI Scoring and Evaluation](https://arxiv.org/abs/2510.18559)
*Loc Phuc Truong Nguyen,Hung Thanh Do*

Main category: cs.LG

TL;DR: RAISE框架统一评估AI模型在可解释性、公平性、鲁棒性和可持续性四个维度的表现，并聚合为单一责任分数。研究发现不同模型在不同维度存在权衡，没有单一模型在所有责任标准上表现最优。
...
Method: 引入RAISE框架，量化模型在四个维度的表现并聚合为责任分数，在金融、医疗和社会经济数据集上评估三种深度学习模型。

Result: MLP在可持续性和鲁棒性方面表现强劲，Transformer在可解释性和公平性方面表现出色但环境成本很高，Tabular ResNet提供平衡的性能。
...
### [98] [HeFS: Helper-Enhanced Feature Selection via Pareto-Optimized Genetic Search](https://arxiv.org/abs/2510.18575)
*Yusi Fan,Tian Wang,Zhiying Yan,Chang Liu,Qiong Zhou,Qi Lu,Zhehao Guo,Ziqi Deng,Wenyu Zhu,Ruochi Zhang,Fengfeng Zhou*

Main category: cs.LG

TL;DR: 提出了HeFS框架，通过遗传算法和帕累托多目标优化来改进现有特征选择算法的结果，识别被忽略但信息丰富的特征，在多个基准数据集上优于现有方法。
...
Method: 使用遗传算法，结合偏置初始化方案和比率引导变异机制，通过帕累托多目标优化同时最大化预测准确性和特征互补性。

Result: 在18个基准数据集上的实验表明，HeFS能一致识别被忽略但信息丰富的特征，在胃癌分类、药物毒性预测和计算机科学应用等挑战性领域优于现有方法。
...
### [99] [Robustness Verification of Graph Neural Networks Via Lightweight Satisfiability Testing](https://arxiv.org/abs/2510.18591)
*Chia-Hsuan Lu,Tony Tan,Michael Benedikt*

Main category: cs.LG

TL;DR: 本文提出了一种改进图神经网络结构鲁棒性验证的方法，通过使用高效的部分求解器替代复杂的约束求解器，在多项式时间内实现更好的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的图神经网络对抗鲁棒性验证技术依赖于强大的约束求解器（如混合整数规划），这些方法计算成本高且难以扩展。

Method: 使用高效的部分求解器替代复杂的约束求解器，这些部分求解器运行在多项式时间内但可能不完整，从而在保持可扩展性的同时提高验证效率。

Result: 在多种GNN变体和数据集上评估了RobLight工具，结果显示该方法在结构鲁棒性验证方面优于现有技术。

Conclusion: 通过使用多项式时间的不完整求解器，可以在图神经网络的结构鲁棒性验证中实现比现有技术更好的性能，为大规模图学习系统的安全验证提供了可行的解决方案。
...
### [100] [Unrolled-SINDy: A Stable Explicit Method for Non linear PDE Discovery from Sparsely Sampled Data](https://arxiv.org/abs/2510.18611)
*Fayad Ali Banna,Antoine Caradot,Eduardo Brandao,Jean-Philippe Colombier,Rémi Emonet,Marc Sebban*

Main category: cs.LG

TL;DR: 提出Unrolled-SINDy方法，通过解卷方案改进PDE发现的稳定性，解决稀疏时间采样数据的问题。


<details>
  <summary>Details</summary>
Motivation: 现有SINDy方法在处理时间稀疏采样数据时存在局限性，无法解决由于局部截断误差导致的参数恢复问题。

Method: 使用解卷方案将数值时间步长与数据采样率解耦，可通过迭代闭式方法或梯度下降方案实现。

Result: 实验表明该方法具有通用性，在传统SINDy和先进的iNeuralSINDy上都能解决非解卷方法无法处理的问题。

Conclusion: Unrolled-SINDy方法能够有效处理稀疏采样数据，提高PDE发现的稳定性和准确性。
...
### [101] [A Rectification-Based Approach for Distilling Boosted Trees into Decision Trees](https://arxiv.org/abs/2510.18615)
...
Method: 使用称为校正的方法来实现蒸馏过程，与通过重新训练模型实现的蒸馏方法进行比较。

Result: 经验证明该方法提供了有趣的结果。

Conclusion: 该方法在提升树蒸馏为决策树方面表现出良好的潜力，在预测性能和可解释性之间实现了有效平衡。
...
### [102] [Hardness of Learning Regular Languages in the Next Symbol Prediction Setting](https://arxiv.org/abs/2510.18634)
*Satwik Bhattamishra,Phil Blunsom,Varun Kanade*

Main category: cs.LG

TL;DR: 本文研究了在下一个符号预测(NSP)设置下语言的可学习性，该设置提供比传统分类更丰富的标签信息，但证明了学习DFA和布尔公式等概念类仍然是计算困难的。


<details>
  <summary>Details</summary>
Motivation: NSP设置已被用于实证分析神经序列模型，且NSP的高效算法可用于学习语言模型的截断支持。本文旨在将该设置形式化以便进行PAC学习分析。

Method: 通过构造使几乎所有额外标签变得无信息，将传统学习问题归约到NSP标签学习问题，基于密码学假设证明学习困难性。

Result: 证明了在NSP设置下学习DFA和布尔公式等概念类仍然是计算困难的，即使该设置提供了更丰富的标签信息。

Conclusion: 尽管NSP设置提供了比传统分类更丰富的监督信号，但对于某些概念类如DFA的学习仍然保持计算困难性。
...
### [103] [Informed Learning for Estimating Drought Stress at Fine-Scale Resolution Enables Accurate Yield Prediction](https://arxiv.org/abs/2510.18648)
...
Method: 将作物产量建模为时间性水分稀缺的函数，预测作物干旱胁迫和对水分稀缺的敏感性；提出物理信息损失函数确保物理一致性；利用多光谱卫星影像、气象数据和精细尺度产量数据；采用深度集成方法处理模型不确定性。

Result: 该方法在作物产量预测中超越了LSTM和Transformer等最先进模型，R²分数高达0.82，同时保持高可解释性。
...
### [104] [Learning Time-Varying Turn-Taking Behavior in Group Conversations](https://arxiv.org/abs/2510.18649)
...
<details>
  <summary>Details</summary>
Motivation: 现有对话动态模型往往无法推广到单个群体之外，且试图用统一公式描述所有群体的发言行为可能并不合适。
...
TL;DR: 该研究提出了一种轻量级卷积神经网络，用于在资源受限的医疗边缘设备上对同步心电图和心音图进行二分类，显著降低了计算和存储成本。


<details>
  <summary>Details</summary>
Motivation: 心血管疾病早期检测很重要，但现有深度学习模型计算量大，不适合资源受限的医疗边缘设备。需要开发高效、准确的模型来实现设备端推理。

Method: 使用早期数据融合的卷积神经网络，在Physionet Challenge 2016数据集上训练和验证同步ECG和PCG记录的二分类模型。

Result: 与现有技术相比，模型将内存占用和计算成本降低了三个数量级，同时保持有竞争力的准确率，在微控制器上验证了设备端推理的可行性。

Conclusion: 提出的轻量级模型适合医疗边缘设备部署，设备端推理比持续数据流传输更节能，为心血管监测提供了实用解决方案。
...
### [106] [Reasoning Language Model Inference Serving Unveiled: An Empirical Study](https://arxiv.org/abs/2510.18672)
...
<details>
  <summary>Details</summary>
Motivation: 尽管RLLM在解决复杂推理任务方面表现出色，但其服务性能和行为尚未被充分探索，这可能会影响RLLM在实际场景中的部署和使用。

Method: 首先通过试点研究比较RLLM与传统LLM的服务性能，然后调查现有推理优化技术对RLLM的有效性，最后在模拟真实世界工作负载的Gamma分布下进行评估。

Result: 发现RLLM具有显著的内存使用和波动、拖尾请求、自适应运行时间和领域偏好等特征。模型量化和推测解码能提高系统效率且对精度影响小，而前缀缓存和KV缓存量化可能降低小RLLM的精度或服务性能。

Conclusion: 研究为研究社区和工业界提供了推进RLLM推理服务的见解，强调了需要针对RLLM特性开发专门的优化技术。
...
### [107] [Learning Task-Agnostic Representations through Multi-Teacher Distillation](https://arxiv.org/abs/2510.18680)
...
TL;DR: 提出一种任务无关的多教师知识蒸馏框架，通过多数投票目标函数实现，无需任务特定标签或先验知识，在各种模态中提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有嵌入模型因架构、损失函数、输入模态和数据集不同而具有多样性，多教师蒸馏能丰富表示但通常针对特定任务，需要任务无关的通用方法。

Method: 基于多数投票目标函数的多教师蒸馏框架，该函数受学生与教师嵌入间互信息约束，形成任务无关的蒸馏损失。

Result: 在文本、视觉模型和分子建模上的评估表明，该方法能有效利用教师多样性，在分类、聚类、回归等下游任务中获得更好性能。

Conclusion: 提出的任务无关蒸馏框架成功利用教师多样性，训练并发布了最先进的嵌入模型，在各种模态中提升下游任务性能。
...
### [108] [Reinforcement Learning with Imperfect Transition Predictions: A Bellman-Jensen Approach](https://arxiv.org/abs/2510.18687)
*Chenbei Lu,Zaiwei Chen,Tongxin Li,Chenye Wu,Adam Wierman*

Main category: cs.LG

TL;DR: 该论文提出了贝叶斯价值函数和BOLA算法，用于处理具有多步预测的强化学习问题，解决了传统RL方法在处理高维预测信息时的维度灾难问题。


<details>
  <summary>Details</summary>
Motivation: 现实应用中智能体可以获取多步未来状态预测，但传统RL基于单步转移模型，无法有效利用这些预测信息，且直接嵌入预测会导致状态空间指数爆炸。

Method: 提出贝叶斯价值函数来表征最优预测感知策略，开发Bellman-Jensen Gap分析，并设计BOLA算法（两阶段模型：离线贝叶斯价值学习+轻量级在线适应）。

Result: 证明了BOLA算法即使在预测不完美的情况下仍保持样本效率，并在合成MDP和真实世界风能存储控制问题上验证了理论和算法。

Conclusion: 该研究为利用多步预测的强化学习提供了理论框架和实用算法，解决了高维预测信息的有效利用问题。
...
### [109] [OmniCast: A Masked Latent Diffusion Model for Weather Forecasting Across Time Scales](https://arxiv.org/abs/2510.18707)
*Tung Nguyen,Tuan Pham,Troy Arcomano,Veerabhadra Kotamarthi,Ian Foster,Sandeep Madireddy,Aditya Grover*

Main category: cs.LG

TL;DR: OmniCast是一个统一天气预测的时间尺度的概率模型，使用VAE编码和基于扩散的transformer生成未来潜在状态，避免了自回归方法的误差累积问题。


<details>
  <summary>Details</summary>
Motivation: 传统数据驱动的深度学习方法在中期天气预报中表现良好，但在更长的次季节到季节(S2S)尺度上由于自回归方法的误差累积而表现不佳。

Method: OmniCast包含两个组件：VAE模型将原始天气数据编码到低维潜在空间，基于扩散的transformer模型生成未来潜在token序列。训练时随机掩码未来token，推理时通过迭代解掩码生成完整序列。

Result: 在中期范围与领先概率方法竞争性相当，速度快10-20倍；在次季节到季节尺度上在准确性、物理基础和概率指标方面达到最先进性能；能生成长达100年的稳定预测。

Conclusion: OmniCast通过联合空间和时间采样有效缓解了自回归方法的误差累积问题，在多个时间尺度上实现了统一的天气预测，特别是在长期预测方面表现出色。
...
### [110] [Improving the Generation and Evaluation of Synthetic Data for Downstream Medical Causal Inference](https://arxiv.org/abs/2510.18768)
*Harry Amad,Zhaozhi Qian,Dennis Frauen,Julianna Piskorz,Stefan Feuerriegel,Mihaela van der Schaar*

Main category: cs.LG

TL;DR: 提出STEAM方法生成用于医学治疗效果分析的合成数据，满足三个关键需求：保持协变量分布、治疗分配机制和结果生成机制，在复杂数据生成过程中表现优于现有方法。
...
Method: 提出STEAM方法，模拟包含治疗的数据生成过程，并优化三个关键需求：协变量分布、治疗分配机制和结果生成机制的保持。
...
### [111] [CAGE: Curvature-Aware Gradient Estimation For Accurate Quantization-Aware Training](https://arxiv.org/abs/2510.18784)
...
<details>
  <summary>Details</summary>
Motivation: 现有的低比特量化感知训练技术与原生训练之间仍存在较大的精度差距，需要新的方法来减小这一差距。

Method: CAGE在直通估计器梯度基础上增加了曲率感知校正项，基于多目标优化视角平衡损失最小化和量化约束，利用局部曲率信息进行梯度修正。

Result: 在预训练800M参数的Llama风格模型时，CAGE在W4A4量化下比异常值处理方法多恢复了超过10%的量化损失。

Conclusion: 曲率感知梯度校正能够弥补当前异常值处理方法之外的剩余性能差距，为量化训练提供了有效的解决方案。
...
### [112] [Stick-Breaking Embedded Topic Model with Continuous Optimal Transport for Online Analysis of Document Streams](https://arxiv.org/abs/2510.18786)
...
Method: SB-SETM扩展了嵌入式主题模型(ETM)，使用截断stick-breaking构造处理文档-主题分布，自动推断活动主题数量，并引入基于连续最优传输的主题嵌入合并策略。

Result: 数值实验表明SB-SETM在模拟场景中优于基线方法，在2022-2023年俄乌战争新闻语料上进行了广泛测试。

Conclusion: SB-SETM是一种有效的在线主题建模方法，能够处理动态数据流并自动适应主题数量的变化。
...
### [113] [On Biologically Plausible Learning in Continuous Time](https://arxiv.org/abs/2510.18808)
*Marc Gong Bacvanski,Liu Ziyin,Tomaso Poggio*

Main category: cs.LG

TL;DR: 提出连续时间神经模型，统一多种生物学习算法，无需阶段分离。学习依赖时间重叠：突触更新需要输入和误差信号时间重合。稳健学习要求突触可塑性时间尺度超过刺激持续时间1-2个数量级。


<details>
  <summary>Details</summary>
Motivation: 生物学习是连续时间过程，但现有算法模型依赖离散更新和分离的学习阶段。需要开发连续时间模型来更真实地模拟生物学习机制。

Method: 构建连续时间神经模型，统一SGD、FA、DFA、KP等学习算法作为动力学极限情况。通过分析和模拟研究时间重叠对学习的影响。

Result: 连续时间网络在生物时间尺度上稳定学习，即使存在时间不匹配和积分噪声。学习强度随输入与误差信号延迟增加而线性下降。突触可塑性时间窗口需在秒级范围。

Conclusion: 连续时间模型成功统一多种生物学习算法，揭示了时间重叠对学习的关键作用。预测秒级资格迹是生物电路中误差驱动学习的必要条件。
...
### [114] [When LRP Diverges from Leave-One-Out in Transformers](https://arxiv.org/abs/2510.18810)
*Weiqiu You,Siqi Zeng,Yao-Hung Hubert Tsai,Makoto Yamada,Han Zhao*

Main category: cs.LG

TL;DR: 研究发现AttnLRP中的双线性传播规则违反实现不变性公理，且绕过softmax层的CP-LRP与LOO对齐度更高，表明双线性因子化敏感性和softmax传播误差共同削弱LRP在Transformer中近似LOO的能力。


<details>
  <summary>Details</summary>
Motivation: LOO特征重要性度量直观但计算昂贵，LRP作为高效替代方案在现代Transformer中的公理合理性尚未充分检验。

Method: 分析AttnLRP双线性传播规则的理论缺陷，实证验证在线性注意力层中的问题；重新评估CP-LRP作为诊断基线，测试绕过softmax层传播相关性的效果。

Result: 双线性传播规则违反实现不变性公理；绕过softmax层仅通过值矩阵反向传播相关性显著提高了与LOO的对齐度，特别是在中后期Transformer层。

Conclusion: 双线性因子化敏感性和softmax传播误差共同导致LRP在Transformer中难以准确近似LOO特征重要性。
...
### [115] [A Unified Perspective on Optimization in Machine Learning and Neuroscience: From Gradient Descent to Neural Adaptation](https://arxiv.org/abs/2510.18812)
*Jesús García Fernández,Nasir Ahmad,Marcel van Gerven*

Main category: cs.LG

TL;DR: 这篇综述提供了一个关于迭代优化的统一视角，连接经典理论与神经网络训练和生物学习。虽然基于梯度的方法主导机器学习，但零阶优化方法在计算上更轻量，且与生物学习机制更相关。


<details>
  <summary>Details</summary>
Motivation: 统一理解迭代优化在AI和生物学习中的作用，探索零阶优化作为梯度方法的替代方案，特别是在高维设置和生物学习场景中的应用。

Method: 通过分类优化方法（基于使用的导数信息阶数），分析这些方法在神经网络训练中的适应性和学习动态，并将这些见解应用于理解生物学习。

Result: 现代零阶方法能够有效近似梯度，在神经网络模型中达到与反向传播竞争的性能，同时为理解大脑学习提供了数学原理视角。

Conclusion: 零阶优化范式将大脑内在噪声作为计算资源，不仅加深了对自然智能的理解，还对神经形态硬件设计具有重要启示，有助于开发利用硬件噪声的快速节能AI系统。
...
### [116] [Online SFT for LLM Reasoning: Surprising Effectiveness of Self-Tuning without Rewards](https://arxiv.org/abs/2510.18814)
*Mengqi Li,Lei Zhao,Anthony Man-Cho So,Ruoyu Sun,Xiao Li*

Main category: cs.LG

TL;DR: 提出了一种简单的在线监督微调(OSFT)方法，让LLM生成自己的回答并立即进行微调，无需奖励信号，在数学推理任务上达到与强化学习方法相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的强化学习方法需要复杂的奖励设计，而OSFT旨在提供一种无需奖励的高效训练策略，利用模型自身的潜在知识来提升推理能力。

Method: 采用在线监督微调范式，模型生成响应后立即用这些自生成数据进行微调，默认只需一次前向传播，无需外部奖励信号。

Result: 在具有挑战性的数学推理任务上，OSFT达到了与GRPO等强RLVR方法相当的下游性能，消融研究证明了其效率和鲁棒性。

Conclusion: OSFT通过促进模型从预训练中学到的现有偏好（潜在知识），提供了一种高效且有前景的替代复杂奖励训练范式的方法。
...
### [117] [Search Self-play: Pushing the Frontier of Agent Capability without Supervision](https://arxiv.org/abs/2510.18821)
...
TL;DR: 提出搜索自博弈(SSP)方法，让LLM同时扮演任务提出者和问题解决者，通过竞争与合作共同进化搜索能力，无需人工监督即可显著提升搜索代理性能。


<details>
  <summary>Details</summary>
Motivation: 传统RLVR方法依赖精心设计的任务查询和真实答案，需要大量人工标注，限制了RL在智能体场景下的扩展性。现有任务合成方法难以控制生成任务的难度，无法有效支持RL训练。

Method: 采用自博弈训练框架，LLM同时作为任务提出者和问题解决者。任务提出者生成具有明确定义真实答案且难度递增的深度搜索查询，问题解决者处理这些查询。通过检索增强生成(RAG)确保每个查询都有准确答案。

Result: SSP在各种基准测试中显著提升了搜索代理的性能，无需任何监督，在从头开始和持续RL训练设置下均表现优异。

Conclusion: 搜索自博弈(SSP)是实现可扩展智能体RLVR的有效方法，通过竞争与合作机制使任务提出者和解决者共同进化，解决了传统方法对人工标注的依赖问题。
...
### [118] [BO4Mob: Bayesian Optimization Benchmarks for High-Dimensional Urban Mobility Problem](https://arxiv.org/abs/2510.18824)
*Seunghee Ryu,Donghoon Kwon,Seongjin Choi,Aryan Deshwal,Seungmo Kang,Carolina Osorio*

Main category: cs.LG

TL;DR: BO4Mob是一个用于高维贝叶斯优化的新基准框架，专注于大型城市道路网络中的起讫点出行需求估计问题。


<details>
  <summary>Details</summary>
Motivation: 解决从有限交通传感器数据中估计起讫点出行需求这一困难的反向优化问题，特别是在现实世界大规模交通网络中，该问题涉及高维连续空间的优化，且目标评估计算昂贵、随机且不可微分。

Method: 基于美国圣何塞真实道路网络构建了五个场景，输入维度最高达10,100维，使用高分辨率开源交通模拟器，包含现实非线性和随机动态。评估了三种最先进的BO算法和两种非BO基线方法。

Result: 开发了一个实用的基准框架，支持可扩展优化算法的开发及其在数据驱动城市移动模型设计中的应用。

Conclusion: BO4Mob基准框架为高维贝叶斯优化算法的发展和大都市道路网络高分辨率数字孪生等应用提供了重要支持。
...
### [119] [Actor-Free Continuous Control via Structurally Maximizable Q-Functions](https://arxiv.org/abs/2510.18828)
*Yigit Korkmaz,Urvi Bhuwania,Ayush Jain,Erdem Bıyık*

Main category: cs.LG

TL;DR: 提出了一种纯基于价值函数的连续控制框架，通过结构最大化Q函数实现高效稳定的学习，无需单独学习actor网络，在受限动作空间环境中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统基于价值的算法局限于离散动作空间，在连续动作空间中计算不可行；而actor-critic方法虽然流行但训练不稳定。
...
### [120] [A Hybrid Enumeration Framework for Optimal Counterfactual Generation in Post-Acute COVID-19 Heart Failure](https://arxiv.org/abs/2510.18841)
...
<details>
  <summary>Details</summary>
Motivation: 反事实推理为假设性结果提供了数学框架，连接因果推理和预测建模，旨在识别可操作的干预路径以减少PASC相关HF住院。

Method: 结合正则化预测模型与反事实搜索，使用精确枚举和优化方法（NICE和MOC算法）探索高维干预空间。

Result: 模型在2700多名患者中表现出强区分性能（AUROC: 0.88），生成了可解释的患者特异性反事实，量化了修改合并症模式或治疗因素对结果的影响。

Conclusion: 反事实推理可形式化为预测函数的优化问题，为复杂生物医学系统提供严谨、可解释且计算高效的个性化推理方法。
...
### [121] [Retaining by Doing: The Role of On-Policy Data in Mitigating Forgetting](https://arxiv.org/abs/2510.18874)
...
<details>
  <summary>Details</summary>
Motivation: 语言模型通过后训练适应新任务时存在灾难性遗忘风险，需要识别缓解这种现象的指导原则。

Method: 在Llama和Qwen等模型家族上，系统比较SFT和RL在指令遵循、通用知识和算术推理任务中的遗忘模式，并通过建模语言模型为两个分布的混合来分析原因。

Result: 实验显示RL比SFT导致更少的遗忘，且目标任务性能相当或更高。RL的模式寻求特性使其能够保持先验知识完整。

Conclusion: RL对遗忘的鲁棒性源于其使用on-policy数据，使用近似on-policy数据可以更有效地缓解遗忘问题。
...
### [122] [Graphical model for tensor factorization by sparse sampling](https://arxiv.org/abs/2510.17886)
*Angelo Giorgio Cavaliere,Riki Nagasawa,Shuta Yokoi,Tomoyuki Obuchi,Hajime Yoshino*

Main category: stat.ML

TL;DR: 本文研究了基于张量分量稀疏测量的张量分解问题，在随机图交互结构下，针对数据大量缺失的场景（如推荐系统），通过消息传递算法和复制理论分析高维极限下的统计推断性能。


<details>
  <summary>Details</summary>
Motivation: 解决推荐系统等场景中大量数据缺失情况下的张量分解问题，通过随机图设计测量方式，为高维极限下的统计推断提供理论分析框架。

Method: 采用消息传递算法，在贝叶斯最优的师生设置下进行测试，并开发复制理论来检验统计推断性能。

Result: 在密集极限下，复制理论变得精确，能够有效分析统计推断的性能表现。

Conclusion: 提出的方法在高维密集极限下能够有效处理稀疏测量的张量分解问题，为数据缺失场景下的推荐系统等应用提供了理论支持。
...
### [123] [Learning Time-Varying Graphs from Incomplete Graph Signals](https://arxiv.org/abs/2510.17903)
*Chuansen Peng,Xiaojing Shen*

Main category: stat.ML

TL;DR: 提出联合推断时变网络拓扑和填补缺失图信号数据的非凸优化框架，通过ADMM算法实现高效求解，在收敛速度和图学习-信号恢复联合精度上优于现有方法


<details>
  <summary>Details</summary>
Motivation: 解决从部分观测图信号中联合推断时变网络拓扑和填补缺失数据的挑战性问题，克服传统解耦方法的局限性

Method: 提出统一非凸优化框架，同时恢复图拉普拉斯矩阵序列和重建未观测信号；引入融合lasso正则化器捕捉网络动态；开发ADMM算法高效求解

Result: 理论证明ADMM收敛到稳定点，推导非渐近统计保证；实验验证在收敛速度和图学习-信号恢复联合精度上显著优于现有基线方法

Conclusion: 集成方法通过图与信号域间的双向信息流实现优越鲁棒性，特别在高缺失数据场景下表现突出
...
### [124] [Arbitrated Indirect Treatment Comparisons](https://arxiv.org/abs/2510.18071)
*Yixin Fang,Weili He*

Main category: stat.ML

TL;DR: 本文提出了一种新的仲裁间接治疗比较方法，旨在解决MAIC悖论——即不同赞助商分析相同数据时得出相互矛盾结论的现象。该方法通过估计共同目标人群（重叠人群）的治疗效果来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: MAIC方法在健康技术评估中应用日益广泛，但存在MAIC悖论问题：不同赞助商使用相同数据可能得出相互矛盾的治疗效果结论，因为每个赞助商隐式地针对不同的人群。

Method: 提出仲裁间接治疗比较方法，专注于估计共同目标人群（特别是重叠人群）的治疗效果，以解决MAIC悖论。

Result: 新方法能够解决MAIC悖论带来的不一致性问题，确保不同分析者针对相同目标人群得出一致的治疗效果估计。

Conclusion: 仲裁间接治疗比较方法通过明确定义共同目标人群，有效解决了MAIC悖论，为健康技术评估提供了更可靠的分析框架。
...
### [125] [Generalization Below the Edge of Stability: The Role of Data Geometry](https://arxiv.org/abs/2510.18120)
...
### [126] [Beating the Winner's Curse via Inference-Aware Policy Optimization](https://arxiv.org/abs/2510.18161)
*Hamsa Bastani,Osbert Bastani,Bryce McLaughlin*

Main category: stat.ML

TL;DR: 提出一种新的推理感知策略优化方法，通过同时优化预测目标值和统计显著性，解决政策学习中的赢家诅咒问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在政策优化时容易受到赢家诅咒的影响，即优化过程会利用预测误差而非真实改进，导致预测的性能提升无法在下游政策评估中得到证实。

Method: 提出推理感知策略优化，在优化预测目标值的同时，考虑政策在统计上显著优于观察政策的可能性。通过数学刻画这两个目标的帕累托前沿，设计基于机器学习预测反事实结果的策略优化算法。

Result: 通过模拟实验验证了该方法的有效性，能够提供更可靠的政策改进保证。

Conclusion: 推理感知策略优化方法能够有效缓解赢家诅咒问题，为决策者提供在预测性能和统计显著性之间权衡选择的机会，从而提高政策学习的可靠性。
...
### [127] [The Bias-Variance Tradeoff in Data-Driven Optimization: A Local Misspecification Perspective](https://arxiv.org/abs/2510.18215)
*Haixiang Lan,Luofeng Liao,Adam N. Elmachtoub,Christian Kroer,Henry Lam,Haofeng Zhang*

Main category: stat.ML

TL;DR: 该论文首次在局部误设设定下分析了样本平均近似(SAA)、集成估计优化(IEO)和估计后优化(ETO)三种数据驱动随机优化方法的相对性能，揭示了它们之间的偏差-方差权衡关系。


<details>
  <summary>Details</summary>
Motivation: 理解SAA和模型驱动方法(ETO/IEO)在复杂上下文依赖问题中的相对性能，特别是在模型接近正确设定但存在局部误设的情况下，现有研究大多局限于模型完全正确或完全误设的二分情况。

Method: 利用统计学中的连续性理论工具，在局部误设设定下分析三种方法的性能，推导决策偏差的显式表达式，并进一步理解方差的几何特性。

Result: 发现在局部误设下，SAA、IEO和ETO之间存在偏差-方差权衡，偏差和方差的相对重要性取决于局部误设的程度，并识别了(不)重要的误设方向。

Conclusion: 该研究为数据驱动随机优化方法在局部误设情况下的相对性能提供了更细粒度的分析框架，揭示了偏差-方差权衡机制，有助于在实际应用中选择合适的优化方法。
...
### [128] [Learning under Quantization for High-Dimensional Linear Regression](https://arxiv.org/abs/2510.18259)
*Dechen Zhang,Junwei Su,Difan Zou*

Main category: stat.ML

TL;DR: 该论文首次系统性地从理论上研究了低比特量化对高维线性回归中随机梯度下降学习性能的影响，分析了数据、标签、参数、激活和梯度等不同量化目标的影响机制。


<details>
  <summary>Details</summary>
Motivation: 尽管低比特量化在大规模模型高效训练中广泛应用且经验上成功，但对其学习性能影响的理论理解仍然缺乏，即使在最简单的线性回归设置中也是如此。

Method: 建立了一个新颖的分析框架，分析高维线性回归中有限步随机梯度下降(SGD)在数据、标签、参数、激活和梯度等全面量化目标下的表现。

Result: 建立了精确的算法依赖和数据依赖的过剩风险边界，揭示了不同量化对学习的影响：参数、激活和梯度量化在训练中放大噪声；数据量化扭曲数据谱；数据和标签量化引入额外近似和量化误差。

Conclusion: 该理论为理解量化如何塑造优化算法的学习动态提供了强大视角，为在实践硬件约束下进一步探索学习理论铺平了道路。
...
### [129] [Parametrising the Inhomogeneity Inducing Capacity of a Training Set, and its Impact on Supervised Learning](https://arxiv.org/abs/2510.18332)
*Gargi Roy,Dalia Chakrabarty*

Main category: stat.ML

TL;DR: 本文提出了训练数据集的"非均匀性参数"概念，用于量化数据中函数关系的不均匀相关性结构，并证明在非零非均匀性参数下，高斯过程建模必须使用非平稳过程。


<details>
  <summary>Details</summary>
Motivation: 传统的数据"非平稳性"概念不足以描述训练数据中函数关系的复杂相关性结构，需要更精确的参数化方法来指导模型选择。

Method: 定义并计算训练数据集的非均匀性参数，在多个公开数据集上进行验证，并基于高斯过程框架证明非零非均匀性参数需要非平稳过程建模。

Result: 实验表明传统非平稳性数据不一定具有非零非均匀性参数，而非零参数确实影响测试预测的质量和可靠性。

Conclusion: 非均匀性参数是选择适当高斯过程模型的关键指标，非零参数要求使用非平稳过程以获得更好的预测性能。
...
### [130] [Interval Prediction of Annual Average Daily Traffic on Local Roads via Quantile Random Forest with High-Dimensional Spatial Data](https://arxiv.org/abs/2510.18548)
*Ying Yao,Daniel J. Graham*

Main category: stat.ML

TL;DR: 提出一种结合分位数随机森林和主成分分析的区间预测方法，用于估算未测量道路的年平均日交通量，并量化预测不确定性。


<details>
  <summary>Details</summary>
Motivation: 现有交通量估算模型大多只提供点预测，忽略了预测不确定性，无法为交通规划提供可靠的置信区间。

Method: 集成分位数随机森林模型与主成分分析，生成AADT预测区间，提供由估计最小值和最大值界定的合理交通量范围。

Result: 在英格兰和威尔士2000多条次要道路数据上测试，区间覆盖概率达88.22%，归一化平均宽度0.23，Winkler得分7468.47。

Conclusion: 该框架通过结合机器学习和空间高维分析，提高了AADT估算的准确性和可解释性，支持更稳健的交通规划。
...
### [131] [A Frequentist Statistical Introduction to Variational Inference, Autoencoders, and Diffusion Models](https://arxiv.org/abs/2510.18777)
*Yen-Chi Chen*

Main category: stat.ML

TL;DR: 本文从频率主义视角统一解释了变分推断(VI)、变分自编码器(VAE)和去噪扩散模型(DDM)的理论基础，通过经典EM算法展示VI如何作为难处理E步骤的可扩展解决方案，以及VAE和DDM如何自然扩展这一框架。


<details>
  <summary>Details</summary>
Motivation: 当前变分推断的教学在不同学科中存在分裂：统计学中VI被视为贝叶斯后验近似方法，而机器学习中VAE和DDM从频率主义视角开发。这给统计学家理解VAE和DDM原理造成了障碍，需要从频率主义角度对VI进行统一介绍。

Method: 从经典的期望最大化(EM)算法出发，展示VI如何作为难处理E步骤的可扩展解决方案，并解释VAE和DDM作为该框架的深度学习自然扩展。

Result: 建立了从经典统计推断到现代生成AI的桥梁，统一了VI、VAE和DDM在频率主义框架下的理论基础。

Conclusion: 通过频率主义视角为VI、VAE和DDM提供了统一的理论解释，弥合了统计学和机器学习在变分推断教学上的鸿沟。
