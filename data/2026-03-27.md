...
TL;DR: SIMs（堆叠智能超表面）与人工神经网络结构相似，提出可学习的SIM架构用于6G+系统，实现多用户信号分离和抗干扰信号处理。


<details>
  <summary>Details</summary>
Motivation: SIMs作为多层可编程超表面，能在电磁波域进行模拟计算，但其与人工神经网络的深层结构相似性尚未被充分探索。利用这种相似性可以为6G及未来系统开发更高效、智能的无线信号处理方案。

Method: 提出可学习的SIM架构，建立SIM与人工神经网络的类比关系，开发基于SIM的机器学习范式，并设计两种SIM赋能的无线信号处理方案：多用户信号分离和通信信号与干扰信号的区分。

Result: SIM赋能的信号处理系统能以轻量级方式显著提升频谱利用效率和抗干扰能力，为超高效智能无线基础设施铺平道路。

Conclusion: SIMs与人工神经网络的结构相似性为开发新型无线信号处理提供了重要机会，提出的可学习SIM架构和信号处理方案展示了在6G+系统中实现高效、智能无线通信的潜力。
...
TL;DR: 提出基于Transformer架构的混合中心化-联邦学习方法用于人类活动识别，在保护数据隐私的同时提升模型性能


<details>
  <summary>Details</summary>
Motivation: 随着边缘设备普及，大量来自可穿戴设备的私人传感器数据可用于人类活动识别，但需要平衡数据隐私与模型性能

Method: 采用Transformer架构，结合中心化与联邦学习混合方法，在Flower框架中实现，处理非独立同分布数据场景

Result: 混合方法在非独立同分布数据场景下有效提升人类活动识别模型的准确性和鲁棒性，联邦学习性能与中心化模型相当

Conclusion: 联邦学习能在保护数据隐私的同时实现与中心化模型相当的性能，为实际应用中平衡隐私与性能提供了可行方案
...
TL;DR: MuViS是一个用于多模态虚拟传感的领域无关基准测试套件，统一了不同数据集和评估标准，发现现有方法在不同场景下没有普遍优势，需要更通用的虚拟传感架构。


<details>
  <summary>Details</summary>
Motivation: 虚拟传感旨在从可访问的测量中推断难以测量的物理量，是物理系统感知和控制的核心。尽管从第一性原理模型到现代数据驱动方法都有快速进展，但研究仍然孤立，缺乏能够跨过程、模态和传感配置转移的默认方法。

Method: 引入MuViS，一个领域无关的多模态虚拟传感基准测试套件，将不同数据集整合到统一的接口中，实现标准化预处理和评估。使用该框架对梯度提升决策树和深度神经网络等现有方法进行基准测试。

Result: 基准测试显示，现有方法中没有一种提供普遍优势，强调了需要可泛化的虚拟传感架构。不同方法在不同场景下表现各异，没有单一方法在所有情况下都最优。

Conclusion: MuViS作为一个开源、可扩展的平台发布，用于可重复的比较和未来新数据集和模型类的集成。该工作强调了虚拟传感领域需要更通用的架构，并提供了一个标准化评估框架来推动这一方向。
...
TL;DR: 该论文研究使用混合耦合器和移相器构建线性微波网络进行模拟计算，实现了矩阵向量乘法，特别展示了离散傅里叶变换、哈达玛变换和哈尔变换的实现。


<details>
  <summary>Details</summary>
Motivation: 微波模拟计算能够实现超高速计算，突破传统数字计算的限制。通过线性微波网络实现矩阵向量乘法，无需数字操作即可瞬时完成计算。

Method: 使用混合耦合器和移相器两种低成本基础组件构建线性微波网络，推导出这类网络可计算的线性变换的充要条件，为三种重要变换（DFT、Hadamard、Haar）提供系统化设计方法，并制作了4×4 DFT硬件原型。

Result: 建立了混合耦合器和移相器网络可计算线性变换的完整理论框架，成功设计并制作了4×4 DFT硬件原型，测量结果与理论预期高度吻合，验证了模拟计算的可行性。

Conclusion: 通过标准微波组件实现模拟计算机，为低延迟、实时模拟信号处理提供了实用途径，展示了微波模拟计算在高速信号处理中的潜力。
...
TL;DR: 本文研究基于流体天线中继的NOMA下行通信系统性能，通过无人机搭载流体天线系统，采用AF或DF中继降低弱用户中断概率，利用Copula理论建模信道分布。


<details>
  <summary>Details</summary>
Motivation: 传统通信系统中，弱用户由于信道条件差容易发生中断。本文旨在通过无人机搭载的流体天线中继系统，利用NOMA技术提高系统性能，特别是改善弱用户的中断概率。

Method: 1) 构建基于流体天线中继的NOMA下行通信系统；2) 采用两阶段传输：基站到用户和FAR，FAR转发信号；3) 使用AF和DF两种中继方案；4) 应用Copula理论和Gaussian copula函数建模FAS信道统计分布；5) 推导弱用户中断概率的解析表达式。

Result: 1) 推导出AF和DF方案下弱用户中断概率的解析表达式；2) 仿真验证了所提方案优于无FAR的基准方案；3) 数值模拟展示了不同FAR位置和通信中断阈值下的中继方案选择参数值。

Conclusion: 基于流体天线中继的NOMA系统能有效降低弱用户中断概率，Copula理论为FAS信道建模提供了有效方法，系统性能优于传统方案，且中继方案选择参数对系统优化有重要指导意义。
...
TL;DR: 提出一种混合学习与优化的波束训练方法，结合深度学习粗估计与模型优化精修，解决XL阵列近场多径场景下的高精度信道参数估计问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于DFT码本的波束训练方法在远场场景有效，但在XL阵列近场多径场景下，由于接收信号功率模式更复杂，现有方法依赖视距主导信道假设，在多径场景下性能下降。

Method: 提出两阶段混合方法：1) 使用定制U-Net架构从接收功率模式学习多径角度和距离的粗估计，采用排列不变训练解决参数匹配歧义，基于路径存在逻辑估计未知路径数；2) 使用粒子群优化在受限搜索区域精修角度和距离参数，同时用Gerchberg-Saxton算法从接收功率模式恢复多径信道增益。

Result: 数值结果表明，所提混合设计在参数估计精度和可达速率方面显著优于各种基准方法，同时保持低计算复杂度。

Conclusion: 提出的混合学习与优化方法有效解决了XL阵列近场多径场景下的波束训练问题，实现了高精度信道参数估计和低计算复杂度的平衡。
...
TL;DR: 该研究比较了压缩感知框架的关键组件（稀疏基、测量矩阵、重建算法）在机械振动信号中的应用，并首次评估了硬件高效的Wang矩阵，证明其在高压縮比下优于传统矩阵。
...
Method: 对压缩感知框架的关键组件进行综合比较分析：稀疏基、测量矩阵和重建算法。首次引入并评估了为图像压缩开发的硬件高效测量矩阵——Wang矩阵，在机械振动信号中的应用。使用HUMS2023和CETIM齿轮箱数据集进行实验评估。

Result: 实验结果表明，Wang矩阵在重建质量上优于传统的高斯和伯努利矩阵，特别是在高压缩比下，能够获得更高的信噪比(SNR)。

Conclusion: Wang矩阵作为硬件高效的测量矩阵，在机械振动信号的压缩感知应用中表现出色，特别是在高压縮比情况下，为实际工程应用提供了有效的解决方案。
...
TL;DR: 本文提出了一种基于联合解映射的无SIC RSMA接收机，通过直接评估复合星座上的比特向量来避免SIC中的错误传播问题，在MISO原型系统中展示了比传统SIC接收机更强的可靠性和更广的工作范围。
...
Method: 提出了一种基于联合解映射(JD)的无SIC RSMA接收机，该方法直接在复合星座上评估比特向量。使用两用户多输入单输出(MISO)原型系统进行空中测量，系统比较SIC和JD两种接收机。

Result: 实验结果表明，提出的无SIC接收机在更宽的工作范围内提供了更强的可靠性和更好的实用性，所有观察结果均与理论预期一致。

Conclusion: 基于联合解映射的无SIC RSMA接收机能够有效克服传统SIC接收机中的错误传播问题，在实际系统中展现出优越的性能和实用性。
...
TL;DR: 提出JTSMLCEF框架，通过联合优化BD-RIS训练散射矩阵和级联信道估计，在MU-MIMO系统中显著降低导频开销并提升信道估计精度。


<details>
  <summary>Details</summary>
Motivation: BD-RIS虽然增强了波束操控能力，但信道系数增加导致CSI获取困难，现有方法忽略了天线空间相关性和RIS-用户信道统计相关性，导频开销过大。
...
Conclusion: JTSMLCEF框架能有效解决BD-RIS辅助MU-MIMO系统中的信道估计挑战，显著降低导频开销并提升估计精度。
...
TL;DR: 本文研究了具有空间分布用户的上行多用户隐蔽通信系统，通过精确建模用户几何位置和信道方差，证明了广义开关功率控制方案是最优策略，并推导了最小检测错误概率和所需协作用户数的闭式表达式。


<details>
  <summary>Details</summary>
Motivation: 先前研究通常使用平均参数和同质假设来近似信道统计，无法准确表征空间异构的隐蔽环境。本文旨在通过显式建模每个用户的几何位置和信道方差，更精确地分析空间异构环境下的隐蔽通信性能。

Method: 1. 显式建模每个用户的几何位置及对应的用户到Willie和用户到Bob信道方差；2. 数学证明广义开关功率控制方案（联合考虑Bob和Willie信道）是异构用户配置下的最优传输策略；3. 基于最优策略推导最小检测错误概率和满足隐蔽约束所需最小协作用户数的闭式表达式；4. 提出分段搜索算法以显著降低计算复杂度实现精确最优性。

Result: 1. 获得闭式表达式，通过蒙特卡洛仿真验证理论分析；2. 发现用户空间异构性可以增强隐蔽通信性能；3. 考虑用户空间异构性的优化相比基于空间同质假设的优化能显著提升隐蔽通信性能。

Conclusion: 本文通过精确建模空间异构环境，证明了广义开关功率控制方案的最优性，推导了关键性能指标的闭式表达式，并提出高效优化算法。研究发现用户空间异构性实际上有利于隐蔽通信，这为实际系统设计提供了重要指导。
...
TL;DR: 提出一种主动挑战-响应认证框架，通过随机时间验证卫星身份，消除固定测量窗口，利用轨道观测的确定性但不可预测采样特性建立基于物理的信任根。


<details>
  <summary>Details</summary>
Motivation: 随着低地球轨道卫星星座的快速增长，卫星链路在紧急通信、导航、广域数据收集和军事行动等关键服务中的作用日益重要。保护这些链路安全、验证卫星发射器身份已成为基本需求，现有被动方法存在固定测量窗口易受攻击的问题。

Method: 提出主动挑战-响应认证框架，验证者在随机选择的时间（提前未知）检查卫星，消除固定测量窗口。利用轨道观测的确定性但不可预测采样特性，建立基于物理的信任根，将卫星认证从静态特征匹配转化为受轨道动力学约束的时空一致性验证问题。

Result: 该方法能够提供强大的保护，即使对抗轨迹感知的欺骗攻击也有效，通过物理约束的轨道动力学实现可靠的卫星身份认证。

Conclusion: 提出的主动认证框架通过随机时间验证和轨道物理约束，为卫星身份认证提供了更安全可靠的解决方案，特别适用于对抗高级欺骗攻击的场景。
...
TL;DR: 论文提出智能反射即服务(IRaaS)概念，旨在解决RIS在移动网络中定位困难的问题，通过资源池化、云基础设施等技术降低系统复杂度，加速RIS部署。


<details>
  <summary>Details</summary>
Motivation: RIS是6G移动通信的有前景技术，但在移动网络中确定其定位位置面临挑战。为了缓解移动网络日益增长的系统复杂度，需要新的解决方案。

Method: 提出IRaaS概念，采用资源池化、基于服务的架构(SBA)、云基础设施和无模型信号处理等技术，使电信运营商能够按需提供智能反射服务，无需大幅更新现有通信协议。

Result: IRaaS提供了一种新颖的部署策略，为智能反射服务供应商创造新机会，平衡电信运营商和物业所有者的利益，有望从技术和商业角度加速RIS的推广。

Conclusion: IRaaS有望加速RIS的部署，为未来移动通信培育真正的智能无线电环境，从技术和商业角度促进6G网络发展。
...
TL;DR: 该研究通过确定性射线追踪模拟分析了卫星到城市传播信道，重点关注不同城市布局、终端类型和频段下的传播机制、莱斯K因子和时延扩展特性。


<details>
  <summary>Details</summary>
Motivation: 随着6G通信系统向三维网络架构演进，卫星与地面基础设施的融合需要准确的城市环境卫星到地面传播信道模型，特别是在街道层面存在显著遮挡和多径效应的情况下。

Method: 采用确定性射线追踪模拟方法，考虑三种代表性城市布局（密集城市、城市、郊区），分析手持设备、车载终端和固定屋顶接收机等多种用例在不同频段下的传播特性。

Result: 研究发现信道色散和衰落特性对卫星仰角、天线位置和城市形态有强烈且有时非传统的影响，这些结果可用于链路级和系统级仿真的信道模型校准。

Conclusion: 该研究为6G三维网络架构中的卫星到城市传播提供了重要的信道特性分析，强调了考虑卫星仰角、天线位置和城市形态对信道参数影响的必要性。
...
### [14] [Can an Actor-Critic Optimization Framework Improve Analog Design Optimization?](https://arxiv.org/abs/2603.24714)
*Sounak Dutta,Fin Amin,Sushil Panda,Jonathan Rabe,Yuejiang Wen,Paul Franzon*

Main category: cs.LG

TL;DR: ACOF是一个用于模拟电路尺寸优化的演员-评论家框架，通过分离提议和评估角色，将设计者判断引入优化循环，相比现有方法显著提升性能指标并降低遗憾值。


<details>
  <summary>Details</summary>
Motivation: 模拟电路设计面临挑战：微小参数变化需要昂贵的仿真周期，高质量解仅存在于巨大搜索空间的狭窄区域。现有优化器缺乏设计者在决定搜索方向时使用的判断能力。

Method: 提出演员-评论家优化框架（ACOF），分离提议和评估角色：演员建议设计空间的有前景区域，评论家评估这些选择、强制执行设计合法性，并在进展受阻时重定向搜索。

Result: 在测试电路中，ACOF相比最强基线平均提升前10位性能指标38.9%，平均降低遗憾值24.7%，在单个电路上峰值增益达70.5%的性能指标提升和42.2%的遗憾值降低。

Conclusion: 通过将迭代推理与仿真驱动搜索相结合，ACOF为跨挑战性设计空间的自动化模拟尺寸优化提供了更透明、更稳定、更可解释的路径。
...
Method: DyMRL整合欧几里得、双曲和复数空间的时间特定结构特征到关系消息传递框架中，学习深度表示。同时采用先进的双重融合-演化注意力机制，以对称方式为不同时间戳的不同模态分配动态学习权重。

Result: 在构建的四个多模态时序知识图谱基准上，DyMRL在事件预测性能上优于最先进的动态单模态和静态多模态基线方法。

Conclusion: DyMRL通过动态多空间表示学习有效解决了多模态时序知识获取与融合的挑战，为事件预测提供了更准确的多模态知识表示。
...
TL;DR: 本文提出了一种衡量机器学习模型对称性内容的方法，并展示了如何通过最小化归纳偏置注入来提升无约束模型的物理保真度。


<details>
  <summary>Details</summary>
Motivation: 尽管无约束模型在物理模拟中表现出色，但它们不保证满足物理量的基本对称性。需要建立严格的指标来评估这些模型学习到的对称性内容，并诊断其失败模式。

Method: 引入严格的指标来衡量学习表示中的对称性内容，并评估输出满足等变条件的准确性。将这些指标应用于两个基于transformer的无约束模型（原子模拟的图神经网络和粒子物理的PointNet风格架构），分析对称性信息在架构层中的处理和学习过程。

Result: 建立了诊断ML模型频谱失败模式的严格框架。通过战略性注入最小所需归纳偏置，可以在保持无约束架构高表达性和可扩展性的同时，保证物理保真度，实现更优的稳定性和准确性。

Conclusion: 通过分析无约束模型中对称性信息的学习过程，可以战略性地注入最小归纳偏置，在保持模型表达性的同时确保物理保真度，为物理模拟的机器学习模型设计提供了新思路。
...
Result: 在Gaia2基准测试中，ERL比ReAct基线提升7.8%的成功率，任务完成可靠性显著提高，优于现有经验学习方法。选择性检索是关键因素。

Conclusion: 通过单次尝试经验反思提取可迁移启发式规则，能有效实现智能体自我改进，为LLM智能体在专业环境中的适应性提供新思路。
...
TL;DR: 提出基于图神经网络的参数化框架，用于学习无网格离散微分算子，通过多项式矩约束训练，实现几何鲁棒、分辨率无关的算子学习


<details>
  <summary>Details</summary>
Motivation: 传统无网格方法在计算成本与精度之间存在权衡：要么低计算成本但精度有限，要么高精度但计算开销大。需要一种既能保持高精度又具有计算效率的方法

Method: 使用图神经网络框架，通过多项式矩约束（来自截断泰勒展开）进行训练，将局部模板相对位置直接映射到离散算子权重。学习到的算子仅依赖于局部几何，具有分辨率无关性

Result: 神经网络能够学习经典多项式一致性，同时保持对不规则邻域几何的鲁棒性。在标准数值分析诊断中，相比平滑粒子流体动力学方法精度更高，在中等精度范围内相比代表性高阶一致无网格方法具有更好的精度-成本权衡

Conclusion: 该框架成功学习了具有几何鲁棒性的无网格离散微分算子，能够求解弱可压缩Navier-Stokes方程，展示了在实际问题中的适用性
...
<details>
  <summary>Details</summary>
Motivation: 工业精馏过程需要准确、符合物理规律的动态模型用于监控、控制和优化。传统数据驱动方法缺乏物理一致性，而纯物理模型计算复杂。需要结合物理约束与数据驱动方法的优势。

Method: 提出PINN数字孪生框架，在神经网络损失函数中嵌入热力学约束：气液平衡(修正拉乌尔定律)、塔板级质量能量平衡、McCabe-Thiele方法。采用自适应损失权重平衡数据保真度与物理一致性。使用Aspen HYSYS生成的961个时间戳测量数据进行训练。

Result: HX摩尔分数预测RMSE为0.00143(R²=0.9887)，比最佳纯数据驱动基线(LSTM等)降低44.6%。模型严格满足热力学约束，能准确捕捉塔板温度、组成分布动态响应，包括进料板响应、回流比变化和压力瞬变。

Conclusion: PINN数字孪生框架为工业精馏过程提供了鲁棒的实时软测量、模型预测控制和异常检测基础，成功结合了物理约束与数据驱动优势。
...
TL;DR: LLM代理通过直接编辑训练代码在无约束搜索空间中进行超参数优化，与经典HPO方法对比发现，经典方法在固定搜索空间表现更好，但LLM在无约束空间能显著缩小差距。结合LLM与CMA-ES的混合方法Centaur表现最佳，小模型(0.8B)配合经典优化器效果优于大模型(27B)。


<details>
  <summary>Details</summary>
Motivation: 研究动机是比较经典超参数优化(HPO)算法与基于LLM的方法在小语言模型超参数调优上的性能差异，探索LLM在无约束搜索空间中的潜力，以及如何结合两者的优势。

Method: 使用autoresearch作为测试平台，对比CMA-ES、TPE等经典HPO方法与LLM代理方法。LLM代理直接编辑训练源代码实现无约束搜索。提出混合方法Centaur，将CMA-ES的内部状态（均值向量、步长、协方差矩阵）与LLM共享。

Result: 在固定搜索空间中，经典HPO方法（CMA-ES、TPE）始终优于LLM代理。但LLM在无约束搜索空间中能显著缩小差距。混合方法Centaur表现最佳，其0.8B变体优于27B变体。避免内存溢出的方法比高搜索多样性的方法表现更好。

Conclusion: 可靠性比探索广度更重要；小型LLM难以跨试验跟踪优化状态，而经典方法缺乏领域知识；结合LLM与经典优化器的混合方法效果最佳；小模型(0.8B)配合强经典优化器足够有效，大模型在固定搜索空间方法中未提供优势。
...
TL;DR: HYPER-TINYPW：一种压缩即生成方法，用生成的权重替换存储的点积混合器权重，通过微MLP从微小层代码合成内核，大幅减少内存占用，在微控制器上保持运行时性能。


<details>
  <summary>Details</summary>
Motivation: 在微控制器上部署神经网络受限于KB级闪存和SRAM，1x1点积混合器在INT8量化后仍占主导内存。需要解决内存瓶颈，同时保持推理性能。

Method: 采用压缩即生成方法：共享的微MLP在加载时从微小层代码合成点积内核并缓存，使用标准整数运算符执行。保留INT8可分离CNN基线，保持PW1在INT8以稳定早期混合。

Result: 在三个ECG基准测试中，HYPER-TINYPW在约225kB时匹配约1.4MB CNN，缩小6.31倍（减少84.15%字节），保持至少95%大模型宏F1。在32-64kB预算下保持平衡检测。在语音命令上达到96.2%测试准确率。

Conclusion: HYPER-TINYPW通过压缩即生成方法显著减少内存占用，同时保持性能，适用于ECG、语音命令等嵌入式传感任务，为资源受限ML系统提供了有效解决方案。
...
### [22] [Energy-Efficient Hierarchical Federated Anomaly Detection for the Internet of Underwater Things via Selective Cooperative Aggregation](https://arxiv.org/abs/2603.24648)
*Kenechi Omeke,Michael Mollel,Lei Zhang,Qammer H. Abbasi,Muhammad Ali Imran*

Main category: cs.LG

TL;DR: 提出用于水下异常检测的节能分层联邦学习框架，通过传感器-雾节点关联、压缩模型更新和选择性雾节点协作，解决水下通信带宽低、能耗高的问题。
...
Method: 提出三层次架构：1) 可行性感知的传感器-雾节点关联；2) 压缩模型更新传输；3) 雾节点间的选择性协作聚合。该架构将大部分通信限制在短距离集群内，仅当较小集群能从邻近较大邻居受益时才激活雾节点间交换。

Result: 在200个传感器的大规模合成部署中，只有约48%的传感器能直接到达网关，而分层学习通过可行的雾路径保持完全参与。选择性协作匹配始终开启的雾间交换的检测精度，同时减少31-33%的能耗。压缩上传在匹配灵敏度测试中减少总能耗71-95%。三个真实基准测试表明，低开销分层方法在检测质量上保持竞争力。

Conclusion: 分层联邦学习框架为在严重声学通信约束下运行的水下部署提供了实用设计指导，平衡了检测质量、通信能耗和网络参与度，而平面联邦学习定义了最小能耗操作点。
...
### [23] [The Order Is The Message](https://arxiv.org/abs/2603.25047)
*Jordan LeDoux*

Main category: cs.LG

TL;DR: 研究发现训练样本顺序对模型学习有巨大影响：固定顺序策略能在远低于IID样本复杂度下实现高精度，而对抗性顺序则完全抑制学习。模型构建的傅里叶表示与顺序结构相关，这一现象对训练效率、grokking现象和安全风险有重要启示。


<details>
  <summary>Details</summary>
Motivation: 探索训练数据顺序对模型学习能力的影响，特别是在样本复杂度远低于理论下界的情况下，研究顺序结构如何影响模型泛化能力和学习效率。

Method: 在模算术任务（p=9973）上进行控制实验，仅改变训练样本顺序而保持其他所有条件不变。比较固定顺序策略、IID顺序和对抗性顺序三种情况，分析模型在不同顺序下的学习表现和傅里叶表示。

Result: 固定顺序策略在训练集仅占输入空间0.3%的情况下，分别在第487和659个epoch达到99.5%测试精度，远低于IID顺序的理论样本复杂度下界。IID基线在5000个epoch后仅达到0.30%精度。对抗性顺序完全抑制学习。模型构建的傅里叶表示的基本频率与顺序结构相关，这一现象在不同种子和训练集组成中一致出现。

Conclusion: 训练样本顺序是影响模型学习的关键因素，能够显著降低样本复杂度要求。模型能够从顺序结构中提取信息构建泛化表示，这一发现对理解grokking现象、提高训练效率有重要意义，同时也揭示了绕过内容审计的安全风险。
...
### [24] [Amplified Patch-Level Differential Privacy for Free via Random Cropping](https://arxiv.org/abs/2603.24695)
*Kaan Durmaz,Jan Schuchardt,Sebastian Schmidt,Stephan Günnemann*

Main category: cs.LG

TL;DR: 随机裁剪作为数据增强技术，在训练差分隐私模型时能通过概率性排除敏感内容来增强隐私保护，无需改变模型架构或训练过程。
...
Method: 提出图像数据的补丁级相邻关系，推导随机裁剪与差分隐私随机梯度下降（DP-SGD）结合时的紧致隐私边界。分析补丁包含概率，展示其如何与小批量采样组合以降低有效采样率。

Result: 理论分析量化了补丁包含概率，实证验证了补丁级增强在多个分割架构和数据集上改善了隐私-效用权衡。结果表明，将隐私计算与领域结构和现有随机性对齐可以在不增加成本的情况下获得更强的隐私保证。

Conclusion: 随机裁剪的随机性为差分隐私训练提供了第三种随机性来源（除梯度噪声和小批量采样外），通过概率性排除敏感内容来增强隐私保护。将隐私计算与数据领域结构对齐可以以零成本获得更强的隐私保证。
...
### [25] [Vision Hopfield Memory Networks](https://arxiv.org/abs/2603.25157)
...
TL;DR: V-HMN是一种受大脑启发的视觉基础骨干网络，通过整合层次化记忆机制和迭代精炼更新，在保持竞争力的同时提供更好的可解释性、数据效率和生物合理性。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer和Mamba等视觉/多模态基础模型虽然取得了显著进展，但距离人脑计算原理较远，需要大量训练数据且可解释性有限。研究者希望开发更接近大脑计算原理的架构。

Method: 提出Vision Hopfield Memory Network (V-HMN)，包含：1) 局部Hopfield模块（图像块级别的联想记忆动态）；2) 全局Hopfield模块（上下文调制的情景记忆）；3) 预测编码启发的精炼规则（迭代误差校正）。这些记忆模块按层次组织，形成统一框架。

Result: 在公共计算机视觉基准测试中，V-HMN取得了与广泛采用骨干架构相竞争的结果，同时提供更好的可解释性、更高的数据效率和更强的生物合理性。

Conclusion: V-HMN有潜力成为下一代视觉基础模型，并为文本、音频等多模态骨干提供通用蓝图，从而桥接大脑启发计算与大规模机器学习。
...
### [26] [Training LLMs for Multi-Step Tool Orchestration with Constrained Data Synthesis and Graduated Rewards](https://arxiv.org/abs/2603.24709)
...
TL;DR: 提出一个强化学习框架，通过真实API响应缓存构建多步骤工具编排环境，并设计分级奖励机制，显著提升LLM在复杂函数调用序列上的执行准确性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在多步骤工具编排（需要按正确顺序调用多个依赖API并传递中间输出）方面存在挑战，现有方法在完整序列执行上经常失败，参数值错误是主要失败原因。训练模型面临两个障碍：现有环境只关注简单的单轮函数调用且使用模拟数据，二元奖励无法为部分正确提供信号。

Method: 1. 构建基于大规模真实API响应缓存的强化学习环境，支持数据合成管道，可采样具有可控复杂度的有效多步骤编排轨迹，生成效率显著高于无约束方法。2. 提出分级奖励设计，将正确性分解为原子有效性（单个函数调用在不同粒度上的正确性）和编排（工具序列正确且尊重依赖关系）。

Result: 在ComplexFuncBench基准测试中，该方法在轮次准确性上显示出显著改进。消融研究证实两个奖励组件都至关重要：单独使用任一组件的性能都会显著下降。

Conclusion: 该框架通过真实API环境和高粒度奖励设计，有效解决了多步骤工具编排的挑战，为训练LLM处理复杂工作流提供了更有效的解决方案。
...
### [27] [Contrastive Learning Boosts Deterministic and Generative Models for Weather Data](https://arxiv.org/abs/2603.24744)
*Nathan Bailey*
...
<details>
  <summary>Details</summary>
Motivation: 天气数据具有高维度和多模态特性，需要压缩到紧凑共享潜在空间以提高下游任务效率。现有对比学习研究未充分比较其与自编码器等压缩方法的优势，也未探索如何处理稀疏数据（实际数据收集更常见）。

Method: 提出SPARTA方法：1）通过对比损失将稀疏样本与完整样本对齐；2）时间感知批次采样策略；3）循环一致性损失改善潜在空间结构；4）图神经网络融合技术注入领域特定物理知识。

Result: 证明对比学习是稀疏地球科学数据可行且有利的压缩方法，能够增强下游任务性能。

Conclusion: 对比学习为稀疏天气数据创建鲁棒嵌入提供了有效途径，结合领域知识注入和结构化损失函数，显著提升压缩表示的质量和下游任务表现。
...
### [28] [Grokking as a Falsifiable Finite-Size Transition](https://arxiv.org/abs/2603.24746)
*Yuda Bi,Chenyu Zhang,Qiheng Wang,Vince D Calhoun*

Main category: cs.LG

TL;DR: 该研究为深度学习中"顿悟"现象的相变描述提供了可检验的有限尺寸输入，通过将群阶p作为扩展变量，使用谱头尾对比作为序参量，应用凝聚态物理诊断方法证实了相变而非平滑交叉的解释。


<details>
  <summary>Details</summary>
Motivation: 深度学习中的"顿悟"现象（延迟泛化）常被类比为相变，但缺乏可检验的有限尺寸输入和定量验证。本研究旨在为这种相变描述提供可证伪的定量基础，而非仅仅停留在类比层面。

Method: 将群阶p作为可接受的扩展变量，使用保留的谱头尾对比作为表示层面的序参量，应用凝聚态物理风格的诊断链：包括粗网格扫描和密集近临界加法审计，使用类似Binder交叉的方法分析有限尺寸边界。

Result: Binder-like交叉揭示了共享的有限尺寸边界，敏感性比较强烈反对平滑交叉解释（在近临界审计中ΔAIC=16.8）。这表明"顿悟"中的相变语言可以作为定量有限尺寸主张进行检验。

Conclusion: "顿悟"现象的相变描述可以从单纯的类比提升为可定量检验的有限尺寸主张，尽管目前相变阶数仍未解决。这为深度学习中的相变研究提供了新的方法论基础。
...
### [29] [Light Cones For Vision: Simple Causal Priors For Visual Hierarchy](https://arxiv.org/abs/2603.24753)
*Manglam Kartik,Neel Tushar Shah*

Main category: cs.LG

TL;DR: 论文提出Worldline Slot Attention，将物体建模为时空世界线中的持久轨迹，使用洛伦兹几何而非欧几里得几何，在层次物体发现任务上实现6倍性能提升
...
Method: 引入Worldline Slot Attention，将物体建模为时空世界线中的持久轨迹，每个物体在不同层次级别有多个槽位，共享空间位置但具有不同的时间坐标。比较欧几里得和洛伦兹几何结构

Result: 欧几里得世界线准确率仅0.078（低于随机机会0.33），而洛伦兹世界线在三个数据集上达到0.479-0.661，性能提升6倍。洛伦兹几何优于双曲嵌入，表明视觉层次需要因果结构而非树结构
...
### [30] [Transformers in the Dark: Navigating Unknown Search Spaces via Bandit Feedback](https://arxiv.org/abs/2603.24780)
*Jungtaek Kim,Thomas Zeng,Ziqian Lin,Minjae Lee,Chungpa Lee,Jy-yong Sohn,Hyung Il Koo,Kangwook Lee*

Main category: cs.LG

TL;DR: 研究表明Transformer架构理论上能够近似搜索算法，通过训练可以实现不同搜索策略，并能泛化到未见过的条件，任务导向的持续训练可以解锁预训练LLM的完整搜索能力。


<details>
  <summary>Details</summary>
Motivation: 虽然外部搜索算法能增强LLM的问题解决能力，但需要额外组件会使整体流程复杂化。因此研究探索LLM或其底层Transformer架构是否能够近似搜索算法，从而简化问题解决过程。

Method: 引入一个简化框架：未知树搜索与赌博机反馈，其中树扩展和反馈信号由外部指定。在此设置下，从零开始训练Transformer模型来近似不同的搜索策略，并通过任务导向的持续训练微调预训练LLM。

Result: Transformer在理论上具有足够的表达能力来实现不同的搜索策略，能够泛化到更长视野或更深树等未见条件。任务导向的持续训练可以解锁预训练LLM的完整搜索能力。

Conclusion: LLM的Transformer架构能够近似搜索算法，这为简化问题解决流程提供了可能，通过适当的训练可以使LLM具备搜索能力而无需外部组件。
...
### [31] [Local learning for stable backpropagation-free neural network training towards physical learning](https://arxiv.org/abs/2603.24790)
...
Result: FFzero在多层感知机和卷积神经网络上对分类和回归任务具有良好泛化能力，在模拟光子神经网络中展示了无需反向传播的物理原位学习可行性。

Conclusion: FFzero为无需反向传播的物理原位学习提供了可行路径，展示了前向优化下局部学习的有效性，而反向传播在此场景下会失败。
...
### [32] [A Practical Guide Towards Interpreting Time-Series Deep Clinical Predictive Models: A Reproducibility Study](https://arxiv.org/abs/2603.24828)
...
<details>
  <summary>Details</summary>
Motivation: 临床决策需要明确的可解释性来审计深度模型，但现有基准缺乏可扩展性和可重复性，未能系统研究临床任务和模型架构对可解释性的影响。

Method: 开发了一个综合基准，在多样化的临床预测任务和模型架构上评估多种可解释性方法，并通过PyHealth开源框架提供实现。

Result: 发现：(1) 正确使用的注意力机制是高效的解释方法；(2) KernelSHAP和LIME等黑盒解释器在时间序列临床预测任务中计算不可行；(3) 多种可解释性方法可靠性不足。

Conclusion: 提出了改进临床预测流程中可解释性的指导原则，并通过开源框架PyHealth支持研究的可重复性和可扩展性。
...
### [33] [Flow matching on homogeneous spaces](https://arxiv.org/abs/2603.24829)
*Francesco Ruscelli*

Main category: cs.LG

TL;DR: 提出将流匹配扩展到齐次空间（李群商空间）的通用框架，通过将数据分布提升到李群上，将问题转化为李代数上的欧几里得流匹配任务，简化了计算并避免了复杂几何。


<details>
  <summary>Details</summary>
Motivation: 齐次空间（李群的商空间）在机器学习中很重要，但现有的黎曼流匹配方法需要定义和计算预度量或测地线，计算复杂且受限于特定几何结构。

Method: 通过将齐次空间上的数据分布提升到基础李群上，将流匹配任务转化为李群上的问题，然后进一步简化为李代数上的欧几里得流匹配，避免了直接处理齐次空间的复杂几何。

Result: 该方法比黎曼流匹配更简单、更快，且完全内蕴（intrinsic），无需定义预度量或计算测地线，为齐次空间上的生成建模提供了更高效的框架。

Conclusion: 提出的框架成功将流匹配扩展到齐次空间，通过李群提升策略简化了几何复杂性，为在更广泛的几何结构上进行生成建模提供了实用且高效的方法。
...
### [34] [Reaching Beyond the Mode: RL for Distributional Reasoning in Language Models](https://arxiv.org/abs/2603.24844)
*Isha Puri,Mehul Damani,Idan Shenfeld,Marzyeh Ghassemi,Jacob Andreas,Yoon Kim*

Main category: cs.LG

TL;DR: 多答案强化学习训练语言模型在单次前向传播中生成多个候选答案，提高多样性、覆盖率和集合级校准，减少生成多个答案所需的计算成本。


<details>
  <summary>Details</summary>
Motivation: 现实世界任务（如医疗诊断、模糊问答）通常存在多个有效答案或不可约的不确定性，但现有语言模型训练方法往往将答案分布坍缩到单一主导模式，无法有效处理多答案场景。

Method: 提出多答案强化学习方法，修改RL目标使模型在单次前向传播中显式生成多个候选答案，将推理时搜索的某些方面内化到模型的生成过程中。

Result: 在问答、医疗诊断和编程基准测试中，相比单答案训练基线，实现了更好的多样性、覆盖率和集合级校准分数；生成多个答案所需token更少；在编程任务中准确性显著提高。

Conclusion: 多答案RL是推理时扩展程序（如best-of-k）的原则性和计算高效替代方案，能够使语言模型在不确定场景下进行分布推理。
...
### [35] [Learning to Staff: Offline Reinforcement Learning and Fine-Tuned LLMs for Warehouse Staffing Optimization](https://arxiv.org/abs/2603.24883)
*Kalle Kujanpää,Yuying Zhu,Kristina Klinkner,Shervin Malmasi*

Main category: cs.LG

TL;DR: 研究两种机器学习方法优化半自动化仓库分拣系统实时人员配置决策：离线强化学习Transformer策略提升2.4%吞吐量；LLM在抽象状态描述上通过微调达到或略超历史基线


<details>
  <summary>Details</summary>
Motivation: 优化半自动化仓库分拣系统的实时人员配置决策，支持不同抽象层次的运营决策，在提高吞吐量的同时考虑人类可读的决策支持

Method: 两种方法：1) 使用离线强化学习训练基于Transformer的策略，基于详细历史状态表示；2) 探索LLM在抽象、人类可读状态描述上的应用，系统比较提示技术、自动提示优化和微调策略

Result: 离线RL方法在学习的模拟器中实现2.4%吞吐量提升；LLM方法通过监督微调结合DPO在模拟器生成的偏好上，在手工模拟器中达到或略超历史基线性能
...
### [36] [GraphER: An Efficient Graph-Based Enrichment and Reranking Method for Retrieval-Augmented Generation](https://arxiv.org/abs/2603.24925)
...
<details>
  <summary>Details</summary>
Motivation: 当前RAG系统中的语义搜索在处理复杂信息需求时存在不足，特别是当相关证据分散在多个来源时。现有方法如代理检索策略未能充分利用数据结构，而知识图谱方法虽然有效但维护成本高且与生产系统不兼容。

Method: 提出GraphER方法，包含两个阶段：离线索引时独立增强数据对象，查询时对候选对象进行基于图的重排序。该方法不依赖知识图谱，可与标准向量存储无缝集成，且检索器无关，延迟开销可忽略。

Result: 在多个检索基准测试上的实验证明了该方法的有效性。

Conclusion: GraphER通过捕捉语义相似性之外的多种邻近关系，有效解决了RAG系统中复杂信息检索的挑战，同时避免了知识图谱的高维护成本，实现了与现有生产系统的兼容。
...
### [37] [CVA: Context-aware Video-text Alignment for Video Temporal Grounding](https://arxiv.org/abs/2603.24934)
...
Method: 1. 查询感知上下文多样化(QCD)：基于视频-文本相似性构建替换片段池，仅混合语义无关内容；2. 上下文不变边界判别(CBD)损失：对比损失强制时间边界处的语义一致性；3. 上下文增强Transformer编码器(CTE)：分层架构结合窗口自注意力和双向交叉注意力。

Result: 在QVHighlights和Charades-STA等主要VTG基准测试中达到最先进性能，Recall@1(R1)分数比现有方法提升约5个百分点，显著减少假阴性。
...
### [38] [A Systematic Empirical Study of Grokking: Depth, Architecture, Activation, and Regularization](https://arxiv.org/abs/2603.25009)
*Shalima Binta Manir,Anamika Paul Rupa*

Main category: cs.LG

TL;DR: 本文通过系统研究揭示了神经网络"顿悟"现象（从记忆到泛化的延迟转变）主要受优化稳定性和正则化相互作用的影响，而非架构本身。


<details>
  <summary>Details</summary>
Motivation: 目前对神经网络"顿悟"现象的理解仍然有限，部分原因是先前的研究混淆了架构、优化和正则化等因素的作用。本研究旨在通过受控实验系统性地分离这些因素，以澄清"顿悟"现象的本质机制。

Method: 在模97加法任务上进行受控研究，使用匹配且精心调优的训练方案，系统比较不同架构（MLP、残差网络、Transformer）、深度、激活函数和正则化策略的影响。每个配置运行3-5个随机种子。

Result: 1) 深度具有非单调效应：4层MLP无法"顿悟"，而8层残差网络能恢复泛化能力；2) Transformer和MLP的差距在匹配超参数后基本消失（仅1.11倍延迟）；3) 激活函数效应依赖训练机制：GELU比ReLU快4.3倍仅当正则化允许记忆时；4) 权重衰减是主要控制参数，存在狭窄的"恰到好处"区间促使"顿悟"发生。

Conclusion: "顿悟"现象主要由优化稳定性和正则化的相互作用驱动，而非架构本身。研究挑战了以架构为中心的解释，阐明了优化和正则化如何共同控制延迟泛化现象。
...
### [39] [Optimal High-Probability Regret for Online Convex Optimization with Two-Point Bandit Feedback](https://arxiv.org/abs/2603.25029)
*Haishan Ye*

Main category: cs.LG

TL;DR: 本文解决了在线凸优化中两点反馈的强凸函数高概率遗憾界开放问题，首次实现了最优的O(d(log T + log(1/δ))/μ)遗憾界。


<details>
  <summary>Details</summary>
Motivation: 在线凸优化中，两点反馈虽然能估计梯度，但强凸函数的高概率遗憾界一直未解决。主要挑战在于带反馈梯度估计器的重尾特性，使得标准集中分析变得困难。

Method: 论文提出了新的分析方法来处理带反馈梯度估计器的重尾特性，解决了标准集中分析的困难，从而实现了强凸函数的高概率遗憾界。

Result: 首次实现了μ-强凸损失的高概率遗憾界O(d(log T + log(1/δ))/μ)，该结果在时间范围T和维度d上都是极小极大最优的。

Conclusion: 本文解决了在线凸优化中两点反馈强凸函数高概率遗憾界的开放挑战，提供了最优的遗憾界，填补了该领域的重要空白。
...
### [40] [Epistemic Compression: The Case for Deliberate Ignorance in High-Stakes AI](https://arxiv.org/abs/2603.25033)
*Steffen Lukas*

Main category: cs.LG

TL;DR: 论文提出"认知压缩"原则：模型复杂度应与数据"保质期"匹配而非盲目扩展参数，通过"制度指数"区分稳定与不稳定领域，在高风险AI中提倡有原则的简约性。


<details>
  <summary>Details</summary>
Motivation: 基础模型在稳定环境中表现优异，但在需要高可靠性的医疗、金融、政策等领域经常失败。这种"保真度悖论"不仅是数据问题，更是结构性问题。在规则随时间变化的领域，额外的模型容量会放大噪声而非捕捉信号。

Method: 提出"认知压缩"原则：稳健性源于模型复杂度与数据"保质期"的匹配，而非参数扩展。引入"制度指数"区分"变化制度"（不稳定、数据贫乏；简约胜出）和"稳定制度"（不变、数据丰富；复杂可行）。通过架构设计实现简约性，使模型结构本身难以表示超出数据证据的方差。

Result: 在15个高风险领域的探索性综合研究中，该指数与经验上优越的建模策略在86.7%的案例中一致（13/15）。

Conclusion: 高风险AI需要从为扩展而扩展转向有原则的简约性，模型结构应设计为通过架构成本限制过拟合，匹配模型复杂度与数据证据的"保质期"。
...
### [41] [Intern-S1-Pro: Scientific Multimodal Foundation Model at Trillion Scale](https://arxiv.org/abs/2603.25040)
...
TL;DR: Intern-S1-Pro是首个万亿参数科学多模态基础模型，在通用和科学领域均实现全面增强，具备先进智能体能力，掌握100+科学领域专业任务，采用高效RL训练基础设施。


<details>
  <summary>Details</summary>
Motivation: 构建首个万亿参数级别的科学多模态基础模型，旨在融合通用智能与专业科学能力，解决科学领域复杂任务，同时保持通用领域的强大性能。

Method: 采用XTuner和LMDeploy基础设施支持高效强化学习训练，确保万亿参数级别的训练与推理精度一致性，集成多模态理解和智能体能力。

Result: 模型在通用能力上达到开源模型顶级水平，在化学、材料、生命科学、地球科学等100+专业科学任务上超越专有模型，实现通用与专业智能的深度融合。

Conclusion: Intern-S1-Pro作为"可专业化的通用模型"，成功展示了万亿参数规模下通用智能与科学专业能力的有效融合，为科学AI发展提供了重要范例。
...
### [42] [SIGMA: Structure-Invariant Generative Molecular Alignment for Chemical Language Models via Autoregressive Contrastive Learning](https://arxiv.org/abs/2603.25062)
*Xinyu Wang,Fei Dou,Jinbo Bi,Minghu Song*

Main category: cs.LG

TL;DR: SIGMA通过token级对比学习解决分子图线性化表示中的模态不匹配问题，结合IsoBeam搜索消除推理中的同构冗余，在保持序列可扩展性的同时提高图结构保真度。
...
Method: 提出SIGMA框架：1）通过token级对比目标使模型严格识别几何对称性，显式对齐具有相同后缀的前缀的潜在状态；2）引入IsoBeam搜索在推理时动态修剪等效路径，消除同构冗余。

Result: 在标准基准测试中，SIGMA在序列可扩展性和图保真度之间架起桥梁，相比强基线在多参数优化中展现出更优的样本效率和结构多样性。

Conclusion: SIGMA在不放弃高效字符串表示的前提下，通过对比学习和动态剪枝解决了线性化分子生成中的模态不匹配问题，实现了可扩展的图结构感知生成。
...
### [43] [An Explainable Ensemble Learning Framework for Crop Classification with Optimized Feature Pyramids and Deep Networks](https://arxiv.org/abs/2603.25070)
*Syed Rayhan Masud,SK Muktadir Hossain,Md. Ridoy Sarkar,Mohammad Sakib Mahmood,Md. Kishor Morol,Rakib Hossain Sajib*

Main category: cs.LG

TL;DR: 提出一个可解释的集成学习框架，融合优化特征金字塔、深度网络、自注意力机制和残差网络，用于基于土壤和气候特征的作物适宜性预测，在埃塞俄比亚农业数据集上达到98.80%准确率。


<details>
  <summary>Details</summary>
Motivation: 农业面临气候变化、土壤退化和资源枯竭的挑战，需要先进的数据驱动作物分类和推荐解决方案。当前复杂机器学习模型与可操作的农业决策之间存在差距，需要建立可信的AI推荐系统。

Method: 提出可解释集成学习范式，融合优化特征金字塔、深度网络、自注意力机制和残差网络。使用预处理方法（标签编码、IQR异常值去除、标准化、SMOTE平衡），比较多种机器学习模型（逻辑回归、KNN、SVM、决策树、随机森林、梯度提升、相对误差SVM），通过网格搜索和交叉验证调优，构建"最终集成"元集成设计。

Result: 在包含3,867个实例和29个特征的埃塞俄比亚农业数据集上，提出的"最终集成"模型达到98.80%的准确率、精确率、召回率和F1分数，优于KNN（95.56%准确率）等其他模型。可解释AI方法（SHAP和排列重要性）识别出土壤pH、氮和锌等关键特征。

Conclusion: 该范式填补了复杂机器学习模型与可操作农业决策之间的差距，通过可解释AI提供可操作的见解，促进农业可持续性并建立对AI推荐系统的信任。
...
### [44] [Process-Aware AI for Rainfall-Runoff Modeling: A Mass-Conserving Neural Framework with Hydrological Process Constraints](https://arxiv.org/abs/2603.25093)
*Mohammad A. Farmani,Hoshin V. Gupta,Ali Behrangi,Muhammad Jawad,Sadaf Moghisi,Guo-Yue Niu*

Main category: cs.LG

TL;DR: MCP模型通过逐步嵌入物理过程表示（土壤存储、导水性、孔隙度等）提升降雨径流预测精度和可解释性，在15个流域验证中接近LSTM性能但保持物理可解释性。
...
Method: 使用质量守恒感知机（MCP）框架，从最小化MCP公式开始，逐步引入有物理意义的水文过程表示：有界土壤存储、状态相关导水性、可变孔隙度、入渗能力、地表积水、垂直排水和非线性水位动态，构建过程感知MCP模型层次结构。

Result: 逐步增强MCP内部物理结构通常能提升预测性能，但过程表示的影响具有强烈的水文气候依赖性：垂直排水在干旱和雪主导流域显著提升模型技能，但在降雨主导区域降低性能；地表积水影响相对较小；最佳MCP配置接近LSTM基准的预测技能。

Conclusion: 在AI架构中嵌入水文过程约束为可解释和过程感知的降雨径流建模提供了有前景的途径，MCP框架在保持物理可解释性的同时实现了高预测精度。
...
### [45] [Layer-Specific Lipschitz Modulation for Fault-Tolerant Multimodal Representation Learning](https://arxiv.org/abs/2603.25103)
*Diyar Altinses,Andreas Schwung*

Main category: cs.LG

TL;DR: 提出一个数学基础的容错多模态表示学习框架，统一自监督异常检测和错误纠正，通过理论分析扰动传播推导鲁棒性准则，实验证明在传感器损坏下提升异常检测和重建性能。


<details>
  <summary>Details</summary>
Motivation: 工业和安全关键环境中的多模态系统需要在部分传感器故障、信号退化或跨模态不一致时保持可靠性，现有方法缺乏数学基础的理论保证。

Method: 基于扰动传播理论分析推导Lipschitz和Jacobian准则，提出两阶段自监督训练：预训练多模态卷积自编码器保留异常信号，扩展可学习计算块进行纠正和对比目标异常识别，引入层特定Lipschitz调制和梯度剪裁控制敏感性。

Result: 在多模态故障数据集上的实验表明，该方法提高了传感器损坏下的异常检测准确性和重建质量。

Conclusion: 该框架弥合了分析鲁棒性保证与实际容错多模态学习之间的差距，为工业应用提供了数学基础的可靠解决方案。
...
### [46] [SEVerA: Verified Synthesis of Self-Evolving Agents](https://arxiv.org/abs/2603.25111)
*Debangshu Banerjee,Changming Xu,Gagandeep Singh*

Main category: cs.LG

TL;DR: SEVerA框架通过形式化验证保证自进化LLM代理的安全性和正确性，在多个任务上实现零约束违反并提升性能。
...
Method: 提出形式化守卫生成模型(FGGM)，用一阶逻辑指定形式化输出契约，结合拒绝采样和验证回退机制。基于FGGM构建SEVerA三阶段框架：搜索合成候选参数化程序，验证硬约束正确性，梯度优化提升软目标。

Result: 在Dafny程序验证、符号数学合成和策略合规代理工具使用(τ²-bench)等任务上，SEVerA实现零约束违反，性能优于无约束和SOTA基线。
...
TL;DR: 提出HIVE框架，通过历史奖励轨迹和实时熵筛选高效用提示，在数学推理任务中显著提升强化学习的数据效率


<details>
  <summary>Details</summary>
Motivation: 传统RL方法如GRPO中，每个提示需要多个rollout，计算成本高昂，且大部分提示梯度贡献很小。需要筛选高效用提示来提升数据效率

Method: 提出HIVE双阶段框架：1) 利用历史奖励轨迹进行粗筛选；2) 使用提示熵作为实时代理指标，剔除效用过时的实例。关注"学习边缘" - 中等难度和高不确定性的交集区域

Result: 在多个数学推理基准测试和模型上评估，HIVE能显著提升rollout效率，同时不损害性能表现

Conclusion: HIVE框架通过智能提示选择有效解决了RL训练中的计算效率问题，特别适用于大语言模型的推理任务后训练
...
### [48] [Knowledge-Guided Retrieval-Augmented Generation for Zero-Shot Psychiatric Data: Privacy Preserving Synthetic Data Generation](https://arxiv.org/abs/2603.25186)
*Adam Jakobsen,Sushant Gautam,Hugo Lewi Hammer,Susanne Olofsdotter,Miriam S Johanson,Pål Halvorsen,Vajira Thambawita*

Main category: cs.LG

TL;DR: 提出基于知识引导的零样本框架，利用LLM结合DSM-5和ICD-10生成隐私保护的合成精神科数据，无需真实数据，在数据质量和隐私保护方面表现良好。


<details>
  <summary>Details</summary>
Motivation: 医疗AI研究受限于真实患者数据获取困难，现有方法依赖真实数据存在隐私风险，需要开发无需真实数据即可生成高质量合成数据的方法。

Method: 使用基于检索增强生成（RAG）的零样本框架，将LLM与DSM-5和ICD-10知识库结合，生成合成精神科表格数据，无需真实数据训练。

Result: 在六种焦虑相关障碍的实验中，CTGAN在边缘分布和多变量结构上表现最佳，但知识增强LLM在成对结构上具有竞争力，在分离焦虑和社交焦虑中达到最低成对误差。隐私分析显示LLM方法具有适度重叠和低平均链接风险。

Conclusion: 基于临床知识引导的LLM能够在真实数据不可用或无法共享时，生成高质量、隐私保护的合成精神科数据，为医疗AI研究提供可行的替代方案。
...
### [49] [A CDF-First Framework for Free-Form Density Estimation](https://arxiv.org/abs/2603.25204)
*Chenglong Song,Mazharul Islam,Lin Wang,Bing Chen,Bo Yang*

Main category: cs.LG

TL;DR: 提出CDF-first框架，通过估计累积分布函数(CDF)而非概率密度函数(PDF)来解决条件密度估计问题，使用Smooth Min-Max网络参数化CDF，保证有效PDF并支持复杂分布形状。


<details>
  <summary>Details</summary>
Motivation: 传统条件密度估计方法直接估计PDF存在数学上的不适定性：对经验分布求导会放大有限数据集中的随机波动，需要强归纳偏置，限制了表达能力且在假设不成立时失效。

Method: 提出CDF-first框架：1) 估计CDF作为稳定且适定的目标；2) 通过Smooth Min-Max网络参数化CDF；3) 对学习的平滑CDF求导恢复PDF；4) 对多元输出使用带SMM因子的自回归分解。

Result: 实验表明该方法在一系列单变量和多变量任务上优于最先进的密度估计器，能够有效捕捉多模态、非对称性和拓扑复杂性等复杂分布特征。

Conclusion: CDF-first框架通过估计CDF而非PDF解决了条件密度估计的根本挑战，提供了一种稳定、表达能力强的密度估计方法，能够处理复杂分布形状并保证有效的PDF。
...
### [50] [Gap Safe Screening Rules for Fast Training of Robust Support Vector Machines under Feature Noise](https://arxiv.org/abs/2603.25221)
*Tan-Hau Nguyen,Thu-Le Tran,Kien Trung Nguyen*

Main category: cs.LG

TL;DR: 提出针对鲁棒支持向量机(R-SVM)的安全样本筛选规则，通过识别不确定性集完全位于间隔超平面一侧的训练样本来降低计算复杂度，同时不影响最优解。


<details>
  <summary>Details</summary>
Motivation: R-SVM通过最坏情况鲁棒公式处理特征噪声，提高了可靠性但增加了计算成本。需要开发能够减少训练复杂度而不影响最优解的方法。

Method: 基于拉格朗日对偶性而非常用的Fenchel-Rockafellar对偶性，推导出安全样本筛选规则。首先建立理想筛选规则，然后通过将基于GAP的安全区域适应到鲁棒设置中，推导出实用规则。

Result: 实验表明，所提出的方法显著减少了训练时间，同时保持了分类准确性。这是首次将安全筛选技术应用于监督机器学习中的最坏情况鲁棒模型。

Conclusion: 成功开发了针对R-SVM的安全样本筛选规则，有效解决了鲁棒模型计算复杂度高的问题，为鲁棒机器学习模型的效率提升提供了新方法。
...
### [51] [Offline Decision Transformers for Neural Combinatorial Optimization: Surpassing Heuristics on the Traveling Salesman Problem](https://arxiv.org/abs/2603.25241)
*Hironori Ohigashi,Shinichiro Hamada*

Main category: cs.LG

TL;DR: 本文提出了一种基于离线强化学习的组合优化方法，使用Decision Transformer从启发式解数据集中学习，超越了训练所用的经典启发式算法性能。


<details>
  <summary>Details</summary>
Motivation: 组合优化问题（如旅行商问题）在工业中至关重要但属于NP难问题。现有的神经组合优化方法依赖在线强化学习，这限制了部署且未能充分利用数十年的算法知识积累。

Method: 应用离线强化学习框架Decision Transformer，直接从启发式解数据集中学习策略；集成Pointer Network处理实例相关的可变动作空间；采用期望回归对Return-to-Go进行乐观条件化处理。

Result: 实验表明，该方法在旅行商问题上持续产生比训练所用的四种经典启发式算法更高质量的路径，展示了离线强化学习超越现有领域知识性能的潜力。

Conclusion: 离线强化学习能够从现有启发式解数据集中学习并超越其性能，为组合优化问题提供了新的解决方案，突破了传统神经组合优化方法的局限性。
...
### [52] [How Pruning Reshapes Features: Sparse Autoencoder Analysis of Weight-Pruned Language Models](https://arxiv.org/abs/2603.25325)
*Hector Borobia,Elies Seguí-Mas,Guillermina Tormo-Carbó*

Main category: cs.LG

TL;DR: 研究发现语言模型剪枝会优先保留稀疏自编码器中的稀有特征，而非高频通用特征，且Wanda剪枝方法比幅度剪枝更好地保留特征结构。


<details>
  <summary>Details</summary>
Motivation: 尽管权重剪枝是压缩大语言模型的常用技术，但其对模型内部学习表示的影响尚不清楚。本研究旨在系统性地探索非结构化剪枝如何重塑语言模型的特征几何结构。

Method: 使用稀疏自编码器（SAEs）作为可解释性探针，在三个模型系列（Gemma 3 1B、Gemma 2 2B、Llama 3.2 1B）、两种剪枝方法（幅度剪枝和Wanda剪枝）和六个稀疏度级别（0-60%）下，研究了五个研究问题：种子稳定性、特征生存、SAE可迁移性、特征脆弱性和因果相关性。

Result: 最显著的发现是：稀有SAE特征（低激活率）比高频特征在剪枝中存活得更好，在17个实验条件中有11个显示出rho = -1.0的Spearman相关性。这表明剪枝起到了隐式特征选择的作用，优先破坏高频通用特征而保留专门的稀有特征。此外，Wanda剪枝比幅度剪枝更好地保留特征结构（最多3.7倍），预训练的SAE在Wanda剪枝模型上可保持有效性至50%稀疏度，但几何特征生存并不能预测因果重要性。

Conclusion: 剪枝作为隐式特征选择机制，优先保留稀有专门特征而非高频通用特征。Wanda剪枝在特征保留方面优于幅度剪枝。特征几何生存与因果重要性之间的分离对压缩下的可解释性研究具有重要意义。
...
### [53] [From Intent to Evidence: A Categorical Approach for Structural Evaluation of Deep Research Agents](https://arxiv.org/abs/2603.25342)
*Shuoling Liu,Zhiquan Tan,Kun Yi,Hui Wu,Yihan Li,Jiangpeng Yan,Liyuan Chen,Kai Chen,Qiang Yang*

Main category: cs.LG

TL;DR: 该研究提出基于范畴论的深度研究智能体评估框架，通过结构感知基准测试揭示当前AI在复杂信息合成方面的局限性，顶级模型平均准确率仅19.9%


<details>
  <summary>Details</summary>
Motivation: 当前深度研究智能体（DRAs）的评估主要依赖临时性经验基准，缺乏对智能体行为的严格建模，也无法充分测试长时程合成和模糊性解析能力。需要建立理论框架来系统评估DRAs的结构理解能力。

Method: 使用范畴论形式化DRA行为，将深度研究工作流建模为结构保持映射（函子）的组合。基于此理论框架，设计了包含296个问题的机制感知基准，从四个可解释维度测试智能体：顺序连接链遍历、V结构回拉中的交集验证、检索子结构的拓扑排序、以及通过Yoneda探针进行本体证伪。

Result: 对11个领先模型的评估显示基准性能持续偏低，最先进模型平均准确率仅19.9%。研究发现当前AI能力存在明显二分：高级研究流程能成功重新定义动态拓扑排序并展示稳健的本体验证能力，但在多跳结构合成方面几乎普遍崩溃。不同任务间的巨大性能差异表明现有系统仍依赖脆弱启发式而非系统性理解。

Conclusion: 虽然顶级自主智能体现在能够有机统一搜索和推理，但在复杂结构信息的广义掌握方面仍面临严峻挑战。该工作为深度研究智能体的结构能力评估提供了理论基础和基准测试工具。
...
### [54] [Hessian-informed machine learning interatomic potential towards bridging theory and experiments](https://arxiv.org/abs/2603.25373)
*Bangchen Yin,Jian Ouyang,Zhen Fan,Kailai Lin,Hanshi Hu,Dingshun Lv,Weiluo Ren,Hai Xiao,Ji Chen,Changsu Cao*

Main category: cs.LG

TL;DR: Hi-MLIP：一种基于Hessian信息的机器学习原子间势能，通过高效的HINT训练协议显著降低Hessian标签需求，实现可靠的势能面曲率预测，提升过渡态搜索和自由能计算精度。


<details>
  <summary>Details</summary>
Motivation: 势能面的局部曲率对预测分子和材料的实验观测至关重要，但对于复杂系统，传统第一性原理计算曲率仍然遥不可及。需要开发能够可靠捕获曲率的机器学习方法。
...
Result: Hi-MLIP显著改进过渡态搜索，将Gibbs自由能预测提升至接近化学精度（尤其在数据稀缺情况下）。能够准确处理强非谐氢化物，重现声子重整化和超导临界温度，与实验结果高度一致，同时避免了非谐计算的计算瓶颈。

Conclusion: 该框架为增强机器学习原子间势能的曲率感知能力提供了实用途径，在广泛系统中架起了模拟与实验观测之间的桥梁，实现了对热力学和动力学现象的准确分析。
...
### [55] [GlowQ: Group-Shared LOw-Rank Approximation for Quantized LLMs](https://arxiv.org/abs/2603.25385)
...
<details>
  <summary>Details</summary>
Motivation: 现有量化技术（如BitsAndBytes、AWQ、GPTQ）在低比特表示（如4位）时会降低精度，而低秩校正方法（如LQER、QERA、ASER）虽然能缓解此问题，但会恢复所有层并在每个解码器块中插入误差校正模块，增加了延迟和内存开销。

Method: GlowQ采用分组共享低秩近似方法，为每个输入共享组缓存单个共享右因子，只恢复提供最高精度收益的组或层。它每个输入共享组计算一次高精度投影并在其模块中重用，减少了参数和内存开销。还提出了选择性变体GlowQ-S，只在提供最大收益的地方应用缓存的共享模块。

Result: 相比强基线，GlowQ平均减少TTFB 5.6%，增加吞吐量9.6%，在WikiText-2上降低困惑度0.17%，下游任务准确率提高0.42个百分点。选择性模型GlowQ-S进一步减少延迟，TTFB降低23.4%，吞吐量增加37.4%，同时平均保持精度在0.2个百分点内。

Conclusion: GlowQ通过分组共享低秩近似有效解决了量化LLM的精度损失问题，在显著减少延迟和内存开销的同时保持了模型精度，为高效部署大型语言模型提供了实用解决方案。
...
### [56] [Maximum Entropy Behavior Exploration for Sim2Real Zero-Shot Reinforcement Learning](https://arxiv.org/abs/2603.25464)
*Jiajun Hu,Nuria Armengol Urpi,Jin Cheng,Stelian Coros*

Main category: cs.LG

TL;DR: FB-MEBE：一种结合行为探索和正则化批评器的在线零样本强化学习算法，用于四足机器人控制，无需下游任务知识即可生成多样化数据，实现可直接硬件部署的自然策略。


<details>
  <summary>Details</summary>
Motivation: 传统零样本RL需要预收集相关多样的数据集，但在没有下游任务先验知识的情况下难以获得高质量数据。现有无向探索方法产生的数据多样性低，导致下游性能差，且策略无法直接部署到真实硬件。

Method: 基于Forward-Backward算法，提出FB-MEBE算法：1) 使用无监督行为探索策略最大化行为分布的熵以促进探索；2) 引入正则化批评器引导策略产生更自然、物理上合理的行为。

Result: 在模拟下游任务中，FB-MEBE相比其他探索策略获得更好性能，且生成的自然策略无需微调即可直接部署到真实硬件系统。

Conclusion: FB-MEBE通过结合行为探索和正则化，有效解决了在线零样本RL中的数据多样性问题，实现了可直接硬件部署的高质量策略学习。
...
### [57] [Not a fragment, but the whole: Map-based evaluation of data-driven Fire Danger Index models](https://arxiv.org/abs/2603.25469)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法虽然能预测野火发生，但标准评估指标对火灾危险指数预测的实际操作性能衡量有限，且经常忽略误报率这一关键操作因素

Method: 重新审视每日火灾危险指数模型评估范式，提出与真实世界决策相一致的新型评估方法，系统评估火灾活动准确预测和误报率，并采用机器学习模型集成

Result: 机器学习模型集成既能提高火灾识别能力，又能减少误报（虚假警报）

Conclusion: 需要开发更符合实际决策需求的火灾预测模型评估方法，集成机器学习模型是提高预测性能的有效途径
...
### [58] [Causal-INSIGHT: Probing Temporal Models to Extract Causal Structure](https://arxiv.org/abs/2603.25473)
*Benjamin Redden,Hui Wang,Shuyan Li*

Main category: cs.LG

TL;DR: Causal-INSIGHT是一个模型无关的后处理解释框架，用于从训练好的时序预测器中提取模型隐含的、有向的、时滞影响结构


<details>
  <summary>Details</summary>
Motivation: 理解多元时间序列中的有向时序交互对于解释复杂动态系统和基于它们训练的预测模型至关重要。现有方法通常关注数据生成过程的因果结构，而Causal-INSIGHT关注预测器如何利用这些依赖关系进行预测

Method: 通过分析预训练预测器对系统性、干预式输入钳制（clamping）的响应，构建有向时序影响信号。引入Qbic准则（平衡预测保真度和结构复杂度的稀疏感知图选择准则），无需真实图标签

Result: 在合成、模拟和真实基准测试中，Causal-INSIGHT能泛化到不同的骨干架构，保持有竞争力的结构准确性，并在应用于现有预测器时显著改善时序延迟定位

Conclusion: Causal-INSIGHT提供了一种有效的方法来解释预训练时序预测器中的模型隐含影响结构，有助于理解预测器如何利用时序依赖关系进行预测
...
### [59] [How Class Ontology and Data Scale Affect Audio Transfer Learning](https://arxiv.org/abs/2603.25476)
...
<details>
  <summary>Details</summary>
Motivation: 尽管迁移学习在深度学习中广泛应用且效果显著，但其内在工作机制仍不明确，特别是关于何时以及为何迁移学习有效的问题。本研究旨在深入理解迁移学习的工作机制，特别是在音频处理领域的应用。

Method: 在AudioSet数据集的不同子集上进行预训练，然后在三个计算机听觉任务上进行微调：声学场景识别、鸟类活动识别和语音命令识别。研究考察了预训练数据样本数量、类别数量以及预训练与下游任务相似性对迁移效果的影响。

Result: 增加预训练数据的样本数量和类别数量对迁移学习都有积极影响。然而，预训练任务与下游任务之间的相似性对迁移效果的影响更大，这种相似性可以使模型学习到可比较的特征。

Conclusion: 迁移学习的效果不仅取决于预训练数据的规模和多样性，更重要的是预训练任务与目标任务的相似性。任务相似性能够促使模型学习到更相关的特征表示，从而在迁移学习中取得更好的效果。
...
### [60] [Interpretable PM2.5 Forecasting for Urban Air Quality: A Comparative Study of Operational Time-Series Models](https://arxiv.org/abs/2603.25495)
*Moazzam Umer Gondal,Hamad ul Qudous,Asma Ahmad Farhan,Sultan Alamri*

Main category: cs.LG

TL;DR: 研究比较了三种轻量级可解释模型（SARIMAX、Facebook Prophet、NeuralProphet）在北京PM2.5小时预测中的表现，发现Facebook Prophet在滚动更新策略下表现最佳，而在线残差修正后的SARIMAX在固定模型策略下误差最低。
...
Method: 采用泄漏感知预测工作流程，结合时间序列数据分区、预处理、特征选择和外生驱动建模。评估了三种预测模型家族：SARIMAX、Facebook Prophet和NeuralProphet，并在两种自适应机制下测试：每周滚动前向重新拟合和固定模型在线残差修正。

Result: 在滚动前向重新拟合机制下，Facebook Prophet表现最佳（MAE 37.61，RMSE 50.10），且计算时间远少于NeuralProphet。在固定模型机制下，在线残差修正改善了Facebook Prophet和SARIMAX的表现，修正后的SARIMAX误差最低（MAE 32.50，RMSE 46.85）。NeuralProphet在两个机制下都不够准确和稳定。

Conclusion: 轻量级加法预测策略在城市空气质量预测中仍具有高度竞争力，能够在准确性、可解释性和计算效率之间提供实用的平衡。
...
### [61] [Missing-Aware Multimodal Fusion for Unified Microservice Incident Management](https://arxiv.org/abs/2603.25538)
*Wenzhuo Qian,Hailiang Zhao,Ziqi Wang,Zhipeng Gao,Jiayi Chen,Zhiwei Ling,Shuiguang Deng*

Main category: cs.LG

TL;DR: ARMOR是一个针对微服务故障管理的鲁棒自监督框架，专门处理多模态数据缺失场景，通过非对称编码器和缺失感知门控融合机制，在数据不完整情况下保持异常检测、故障分类和根因定位的性能。


<details>
  <summary>Details</summary>
Motivation: 现有统一框架假设多模态数据完整，但实际中网络波动和代理故障常导致数据缺失。传统静态占位符方法会引入噪声，掩盖异常并降低性能。需要开发能处理缺失模态的鲁棒框架。

Method: 提出ARMOR框架：1) 模态特定的非对称编码器，隔离指标、日志和追踪数据的分布差异；2) 缺失感知门控融合机制，使用可学习占位符和动态偏置补偿，防止不完整输入的跨模态干扰；3) 基于掩码引导重建的自监督自回归方法，联合优化异常检测、故障分类和根因定位。

Result: 大量实验表明，ARMOR在完整数据条件下达到最先进性能，即使在严重模态缺失情况下也能保持稳健的诊断准确性。

Conclusion: ARMOR是一个有效的鲁棒自监督框架，能够处理微服务故障管理中的多模态数据缺失问题，在异常检测、故障分类和根因定位任务上表现出色，具有实际应用价值。
...
### [62] [An Integrative Genome-Scale Metabolic Modeling and Machine Learning Framework for Predicting and Optimizing Biofuel-Relevant Biomass Production in Saccharomyces cerevisiae](https://arxiv.org/abs/2603.25561)
*Neha K. Nair,Aaron D'Souza*

Main category: cs.LG

TL;DR: 结合酵母基因组尺度代谢模型与机器学习优化，预测和提升生物质通量的计算框架
...
Method: 整合Yeast9基因组尺度代谢模型、机器学习（随机森林、XGBoost、变分自编码器、生成对抗网络）和优化方法（通量平衡分析、贝叶斯优化）

Result: 机器学习模型预测精度高（R²达0.99989），识别出关键代谢途径，通过优化使生物质通量提升12倍（从0.0858到1.041 gDW/hr）

Conclusion: 基因组尺度模拟、可解释机器学习和生成建模相结合，可显著推进酵母代谢工程
...
### [63] [Revisiting On-Policy Distillation: Empirical Failure Modes and Simple Fixes](https://arxiv.org/abs/2603.25562)
*Yuqian Fu,Haohuan Huang,Kaiwen Jiang,Yuanheng Zhu,Dongbin Zhao*

Main category: cs.LG

TL;DR: 本文重新审视了在线蒸馏（OPD）方法，针对长序列生成中采样标记变体的脆弱性问题，提出了教师top-K局部支持匹配方法，通过截断反向KL和top-p采样等技术改进优化稳定性和下游性能。


<details>
  <summary>Details</summary>
Motivation: 在线蒸馏（OPD）在大语言模型后训练中很有吸引力，因为它基于学生生成的轨迹评估教师反馈。但在长序列设置中，常见的采样标记变体很脆弱：它将分布匹配简化为单标记信号，并且随着轨迹偏离教师常见前缀而变得越来越不可靠。

Method: 从估计器和实现两方面重新审视OPD。理论分析显示标记级OPD相对于序列级反向KL有偏差，但方差界限更紧。提出教师top-K局部支持匹配方法，实现为带top-p采样和特殊标记掩码的截断反向KL。

Result: 在单任务数学推理和多任务智能体+数学训练中，该方法比采样标记OPD产生更稳定的优化和更好的下游性能。玩具研究证实了偏差-方差权衡，更强的未来奖励耦合导致更高的梯度方差和更不稳定的学习。

Conclusion: 采样标记OPD存在三个失效模式：不平衡的单标记信号、对学生生成前缀的不可靠教师指导、以及分词器或特殊标记不匹配导致的失真。教师top-K局部支持匹配方法解决了这些问题，提高了优化稳定性和性能。
...
### [64] [Spatiotemporal System Forecasting with Irregular Time Steps via Masked Autoencoder](https://arxiv.org/abs/2603.25597)
...
<details>
  <summary>Details</summary>
Motivation: 当前数据驱动算法在处理不规则时间步的高维动力系统预测时面临挑战，这些不规则性源于数据缺失、稀疏观测或自适应计算技术，会降低预测准确性。

Method: 提出物理-时空掩码自编码器，结合卷积自编码器提取空间特征和掩码自编码器处理不规则时间序列，利用注意力机制在单次预测中重建完整物理序列。

Result: 在多个模拟数据集和真实海洋温度数据上，该方法在预测准确性、对非线性的鲁棒性和计算效率方面显著优于传统卷积和循环网络方法。

Conclusion: 该方法能够捕捉复杂时空模式而无需领域专业知识，在气候建模、流体动力学、海洋预报、环境监测和科学计算中具有应用潜力。
...
### [65] [Social Hippocampus Memory Learning](https://arxiv.org/abs/2603.25614)
...
<details>
  <summary>Details</summary>
Motivation: 现有异构联邦学习方法通常共享模型参数或中间表示，这可能暴露敏感信息并产生额外开销。需要一种既能实现协作学习又能保护隐私的轻量级方法。

Method: 提出SoHip框架：1) 从本地表示中抽象出个体短期记忆；2) 通过海马体启发机制将短期记忆整合为个体长期记忆；3) 与集体聚合的长期记忆融合以增强本地预测。整个过程只交换轻量级记忆，原始数据和本地模型保留在设备上。

Result: 在两个基准数据集上与七个基线方法比较，SoHip始终优于现有方法，准确率提升最高达8.78%。理论分析证明了收敛性和隐私保护特性。

Conclusion: SoHip通过记忆共享而非模型共享，实现了异构代理间的有效协作学习，在保护隐私的同时减少了通信开销，为联邦学习中的社会机器学习提供了新范式。
...
### [66] [Anchored-Branched Steady-state WInd Flow Transformer (AB-SWIFT): a metamodel for 3D atmospheric flow in urban environments](https://arxiv.org/abs/2603.25635)
...
TL;DR: AB-SWIFT是一种基于Transformer的深度学习模型，用于城市尺度空气流动建模，通过分支结构处理不同大气稳定度条件，在预测精度上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统CFD计算成本高昂，现有深度学习模型难以适应城市几何结构的高度变化和大网格尺寸，需要开发更有效的替代模型。

Method: 提出AB-SWIFT（锚定分支稳态风流Transformer），采用内部分支结构的Transformer架构，专门为大气流动建模设计。在随机城市几何和混合大气稳定度（不稳定、中性、稳定）条件下训练。

Result: 在所有预测场上的精度均优于最先进的Transformer和图神经网络模型，代码和数据已开源。

Conclusion: AB-SWIFT成功解决了城市空气流动建模中的几何变化和网格尺寸挑战，为污染物扩散和风电场建模等应用提供了高效替代方案。
...
TL;DR: U-Balance：利用行为不确定性解决CPS安全监测中极端类别不平衡问题的新方法
...
### [68] [Longitudinal Digital Phenotyping for Early Cognitive-Motor Screening](https://arxiv.org/abs/2603.25673)
*Diego Jimenez-Oviedo,Ruben Vera-Rodriguez,Ruben Tolosana,Juan Carlos Ruiz-Garcia,Jaime Herreros-Rodriguez*

Main category: cs.LG

TL;DR: 提出AI驱动的纵向框架，通过平板交互数据建模儿童认知运动发展轨迹，使用无监督学习识别三种不同表现水平的发育表型，发现低表现群体具有高度稳定性，为早期筛查和干预提供数据驱动基础。


<details>
  <summary>Details</summary>
Motivation: 传统儿童认知运动发展评估依赖主观静态评估，需要更客观连续的监测方法。数字设备提供了通过数字生物标志物进行持续客观监测的机会，以实现早期发现异常发育并及时干预。

Method: 提出AI驱动的纵向框架，收集18个月至8岁儿童在平板设备上的交互数据（包括精细运动控制、反应时间等6个认知运动任务）。应用t-SNE降维和K-Means++无监督聚类识别不同发育表型，并跟踪个体在这些表型间的随时间变化。

Result: 识别出三种不同的发育表型：低、中、高表现水平。纵向跟踪显示低表现群体具有高度稳定性（早期年份>90%保留率），表明早期缺陷倾向于持续存在。高表现群体则显示出更大变异性，可能反映参与度因素。

Conclusion: 验证了在触摸屏数据上使用无监督学习揭示异质性发展路径的有效性。识别的表型可作为认知发展的可扩展、数据驱动代理指标，为早期筛查工具和个性化儿科干预奠定基础。
...
### [69] [On Neural Scaling Laws for Weather Emulation through Continual Training](https://arxiv.org/abs/2603.25687)
*Shashank Subramanian,Alexander Kiefer,Arnur Nigmetov,Amir Gholami,Dmitriy Morozov,Michael W. Mahoney*

Main category: cs.LG

TL;DR: 该论文研究了科学机器学习中的神经缩放定律，特别是在天气预报领域，通过最小化的Swin Transformer架构和持续训练策略，展示了可预测的缩放趋势和计算最优训练方案。


<details>
  <summary>Details</summary>
Motivation: 神经缩放定律在自然语言处理和计算机视觉领域已成为构建基础模型的关键，但在科学机器学习（尤其是天气预报）中的应用尚未充分探索。研究旨在分析科学机器学习中的缩放行为，为资源高效分配提供诊断工具。

Method: 采用最小化、可扩展的通用Swin Transformer架构，使用恒定学习率的持续训练和周期性冷却策略作为高效训练方法。系统探索不同模型和数据规模，构建IsoFLOP曲线，识别计算最优训练方案。

Result: 这种最小化训练方式遵循可预测的缩放趋势，甚至优于标准余弦学习率调度。冷却阶段可用于改善下游性能，如实现更长的多步预测和更精确的谱损失调整。通过IsoFLOP曲线识别了计算最优训练方案。

Conclusion: 神经缩放定律可作为科学机器学习中高效资源分配的重要诊断工具。通过缩放趋势外推到更大规模可揭示性能极限，为天气预报等科学应用提供优化指导。代码已开源以确保可复现性。
...
### [70] [A Unified Memory Perspective for Probabilistic Trustworthy AI](https://arxiv.org/abs/2603.25692)
*Xueji Zhao,Likai Pei,Jianbo Liu,Kai Ni,Ningyuan Cao*

Main category: cs.LG

TL;DR: 论文提出将确定性数据访问视为随机采样的特例，建立统一框架分析AI系统中数据访问与随机性需求，揭示随机性需求降低数据访问效率并导致熵受限操作，提出内存级评估标准并分析新兴概率存内计算架构。
...
Conclusion: 通过统一框架分析数据访问和随机性需求，为可信AI的可扩展硬件提供了路径。概率存内计算方法有望解决传统架构在满足AI系统随机性需求方面的局限性。
...
### [71] [Neural Network Conversion of Machine Learning Pipelines](https://arxiv.org/abs/2603.25699)
*Man-Ling Sung,Jan Silovsky,Man-Hung Siu,Herbert Gish,Chinnu Pittapally*

Main category: cs.LG

TL;DR: 该论文研究将随机森林分类器通过师生学习迁移到神经网络学生模型，实现多任务统一推理引擎


<details>
  <summary>Details</summary>
Motivation: 传统机器学习流水线（如随机森林）与神经网络分离，无法联合优化。希望通过师生学习将非神经网络的机器学习模型知识迁移到神经网络，实现统一推理引擎和多任务优化。

Method: 采用师生学习框架，将随机森林分类器作为教师模型，神经网络作为学生模型。在100个OpenML任务上进行实验，探索不同神经网络拓扑结构，并使用随机森林帮助选择神经网络超参数。

Result: 在大多数任务中，学生神经网络能够成功模仿教师随机森林的性能，前提是能够选择正确的神经网络超参数。随机森林也可有效用于神经网络超参数选择。

Conclusion: 非神经网络机器学习模型可以通过师生学习成功迁移到神经网络，实现流水线组件的联合优化和统一推理引擎，为多任务机器学习系统提供新思路。
...
### [72] [Improving Infinitely Deep Bayesian Neural Networks with Nesterov's Accelerated Gradient Method](https://arxiv.org/abs/2603.25024)
...
<details>
  <summary>Details</summary>
Motivation: 基于随机微分方程的贝叶斯神经网络虽然理论扎实、应用潜力强，但依赖数值SDE求解器会导致大量函数评估，计算成本高且收敛不稳定。

Method: 将Nesterov加速梯度整合到SDE-BNN框架中，并加入NFE相关的残差跳跃连接，加速收敛并减少训练和测试时的函数评估次数。

Result: 在图像分类和序列建模等多种任务上，该模型始终优于传统SDE-BNNs，实现了更低的NFEs和更高的预测准确率。
...
### [73] [Fair regression under localized demographic parity constraints](https://arxiv.org/abs/2603.25224)
*Arthur Charpentier,Christophe Denis,Romuald Elie,Mohamed Hebiri,François HU*

Main category: stat.ML

TL;DR: 提出回归任务中人口统计均等(DP)的松弛方法，通过在有限分位数或阈值上施加约束，而非整个分布，以平衡公平性与准确性


<details>
  <summary>Details</summary>
Motivation: 在回归任务中，完全分布的人口统计均等(DP)通常过于严格，会导致显著的准确性损失。需要一种更灵活的公平性约束方法，能够在特定决策相关的分位数或阈值上实现公平，同时保持预测性能。

Method: 提出(ℓ,Z)-公平预测器，在有限的分位数水平或得分阈值上施加群体CDF约束。通过拉格朗日对偶公式推导最优公平离散预测器的闭式解，并开发基于两个样本(标记样本学习基础回归器，未标记样本校准)的模型无关后处理算法。

Result: 理论分析表明离散化成本随网格细化而消失，风险差距趋近于连续最优解。实验在合成和真实数据集上展示了可解释的公平性-准确性权衡，能够在决策相关的分位数或阈值上进行针对性修正，同时保持预测性能。

Conclusion: 提出的回归任务中DP松弛方法提供了一种灵活且可解释的公平性框架，允许在特定分位数或阈值上实现公平约束，避免了完全分布DP的过度限制性，在保持预测准确性的同时实现有意义的公平性改进。
...
### [74] [Practical Efficient Global Optimization is No-regret](https://arxiv.org/abs/2603.25311)
*Jingyi Wang,Haowei Wang,Nai-Yuan Chiang,Juliane Mueller,Tucker Hartland,Cosmin G. Petra*

Main category: stat.ML

TL;DR: 首次为带正nugget的实用EGO算法建立了累积遗憾上界，证明其对常用核函数具有次线性遗憾，是一种无遗憾算法。


<details>
  <summary>Details</summary>
Motivation: 尽管带正nugget的实用EGO算法在实践中被广泛采用且经验上成功，但迄今为止尚未建立其累积遗憾的理论界限，需要填补这一理论空白。

Method: 分析实用EGO（高斯过程代理模型+期望改进采集函数+正nugget）的累积遗憾上界，特别针对平方指数核和Matérn核（ν>1/2），并研究nugget对遗憾界的影响。

Result: 首次证明了实用EGO具有次线性累积遗憾上界，因此对常用核函数是一种无遗憾算法，并通过数值实验验证了理论发现。

Conclusion: 实用EGO具有理论保证的次线性遗憾性能，nugget的选择对遗憾界有理论意义，为实际应用提供了理论依据。
...
### [75] [A Distribution-to-Distribution Neural Probabilistic Forecasting Framework for Dynamical Systems](https://arxiv.org/abs/2603.25370)
...
<details>
  <summary>Details</summary>
Motivation: 现有预测方法（无论是基于物理模型还是神经网络）本质上是轨迹导向的，预测分布通常通过集合或采样获得，而不是作为动态对象直接演化。需要一种直接操作预测分布的新范式。

Method: 开发了分布到分布（D2D）神经概率预测框架，使用核均值嵌入表示输入分布，混合密度网络参数化输出预测分布，围绕可替换的神经预测模块构建分布编码和解码结构。

Result: 在Lorenz63混沌动力系统上验证，D2D模型能捕捉非线性动力学下的非平凡分布演化，无需显式集合模拟即可产生熟练的概率预测，性能与简化完美模型基准相当甚至更优。

Conclusion: 提出了一种新的概率预测范式，其中预测分布被直接学习和演化，而不是通过基于集合的不确定性传播间接重建。
...
### [76] [Residual-as-Teacher: Mitigating Bias Propagation in Student--Teacher Estimation](https://arxiv.org/abs/2603.25466)
*Kakei Yamamoto,Martin J. Wainwright*

Main category: stat.ML

TL;DR: 论文提出并分析了一种名为"残差作为教师"(RaT)的新方法，用于解决传统学生软匹配(SM)方法中教师模型偏差传播的问题。RaT方法通过让教师估计学生预测的残差，使学生能够模拟解决最优优化问题的近端梯度方案，从而减少教师偏差的影响。


<details>
  <summary>Details</summary>
Motivation: 传统的学生-教师设置中，学生直接匹配教师输出的方法（学生软匹配，SM）会传播教师模型中的系统性偏差或错误设定，从而降低学生的预测性能。需要一种能够减少教师偏差影响的新方法。

Method: 提出残差作为教师(RaT)方法：教师用于估计学生预测的残差，而不是直接匹配教师输出。这种方法使学生能够模拟解决最优优化问题的近端梯度方案。分析了RaT固定点的非渐近超额风险界限，以及学生-教师迭代方案的收敛保证。

Result: 理论分析表明：对于核基学生-教师对，RaT方法达到极小极大最优率，而SM方法在任何样本量下都会产生恒定预测误差。在合成数据和ImageNette分类的协变量偏移实验中验证了理论发现。

Conclusion: RaT方法通过使用教师估计残差而非直接匹配输出，能够有效减少教师偏差的影响，在理论和实验上都优于传统的SM方法，特别是在存在教师模型偏差或错误设定的情况下。
...
### [77] [Adaptive Subspace Modeling With Functional Tucker Decomposition](https://arxiv.org/abs/2603.25530)
*Noah Steidle,Joppe De Jonghe,Mariya Ishteva*

Main category: stat.ML

TL;DR: 提出功能性Tucker分解(FTD)，在Tucker分解中嵌入模态连续性约束，使用RKHS建模连续模态，无需先验基函数，保持多线性子空间结构，应用于领域变化张量分类。
...
Method: 引入功能性Tucker分解(FTD)，使用再生核希尔伯特空间(RKHS)建模连续模态，无需先验基函数，同时保持Tucker模型的多线性子空间结构。通过RKHS驱动表示获得自适应和表达性因子描述。

Result: 该方法在领域变化张量分类中表现出价值，在高光谱成像和多变量时间序列分析的分类任务中证明了其有效性，展示了结构分解与功能适应性结合的优势。

Conclusion: FTD通过嵌入模态连续性约束，结合RKHS建模连续模态，为连续过程产生的张量数据提供了更好的表示方法，在张量分类任务中显示出优越性能。
...
### [78] [The Rules-and-Facts Model for Simultaneous Generalization and Memorization in Neural Networks](https://arxiv.org/abs/2603.25579)
*Gabriele Farné,Fabrizio Boncoraglio,Lenka Zdeborová*

Main category: stat.ML

TL;DR: 提出RAF模型，从统计物理角度理论分析神经网络如何同时学习规则和记忆事实，量化过参数化如何支持这种双重能力


<details>
  <summary>Details</summary>
Motivation: 现代神经网络能同时学习底层规则和记忆特定事实，但对此双重能力的理论理解有限，需要建立理论框架来精确描述这一现象

Method: 引入Rules-and-Facts (RAF)模型，结合教师-学生框架（规则学习）和Gardner容量分析（记忆），其中训练标签部分来自结构化教师规则，部分来自随机标签的无结构事实

Result: 量化了过参数化如何支持同时实现规则恢复和事实记忆：足够的超额容量支持记忆，而正则化和核/非线性选择控制容量在规则学习和记忆间的分配

Conclusion: RAF模型为理解现代神经网络如何同时推断结构和存储稀有/不可压缩信息提供了理论基础
