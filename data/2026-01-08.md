...
TL;DR: 本文分析了在瑞利衰落和高斯白噪声下，采用M-PSK调制的相位量化SIMO系统的平均符号错误概率，推导了QPSK调制的n位相位量化SIMO-MRC系统的精确SEP表达式，并揭示了SIMO-MRC系统与相位量化MISO系统之间的对偶关系。


<details>
  <summary>Details</summary>
Motivation: 研究相位量化SIMO系统在M-PSK调制下的性能分析，特别是在实际系统中由于相位量化导致的性能损失，以及不同接收天线组合方案（MRC和SC）下的性能表现。

Method: 采用新颖的分析方法，推导了QPSK调制的n位相位量化SIMO-MRC系统的精确SEP表达式，获得了高信噪比下的分集增益和编码增益特性。进一步分析了2位相位量化SIMO-SC系统，并揭示了SIMO-MRC与相位量化MISO-MRT系统之间的对偶关系。

Result: 获得了QPSK调制的n位相位量化SIMO-MRC系统的精确SEP表达式和高信噪比特性；得到了2位相位量化SIMO-SC系统的分集和编码增益；发现了SIMO-MRC与相位量化MISO-MRT系统的对偶关系；分析了有限CSI下性能的进一步恶化。

Conclusion: 本文系统分析了相位量化SIMO系统的性能，推导了精确的SEP表达式，揭示了系统间的对偶关系，为实际系统中相位量化带来的性能影响提供了理论分析框架，特别是在有限CSI条件下性能会进一步下降。
...
TL;DR: 提出DT-HDRL框架，通过双变压器模型处理不同时间尺度的信道状态信息和遮挡预测，优化近场通信中的波束聚焦和RIS相位调整，提升频谱效率。


<details>
  <summary>Details</summary>
Motivation: 6G网络中部署超大孔径阵列使通信进入近场区域，信号呈现球面波传播。RIS可动态调整相位以支持近场波束聚焦，但有效利用RIS需要快速CSI估计和主动遮挡缓解，这两者发生在不同时间尺度上（毫秒级CSI变化 vs 秒级遮挡事件），传统单级控制算法难以处理。

Method: 提出双变压器分层框架（DT-HDRL）：1）快速时间尺度变压器处理射线追踪数据进行快速CSI估计；2）视觉变压器分析视觉数据预测即将发生的遮挡；3）分层深度强化学习中，高层控制器选择LoS或RIS辅助的NLoS传输路径并设定目标，低层控制器利用瞬时CSI优化基站波束聚焦和RIS相位偏移。

Result: 仿真结果表明：1）相比单时间尺度基线方法，频谱效率提升约18%；2）提出的遮挡预测器F1分数达到0.92，在动态场景中提供769毫秒的提前预警窗口。

Conclusion: DT-HDRL框架通过双时间尺度协调，在动态条件下最大化频谱效率并确保鲁棒性能，有效解决了近场通信中不同时间尺度控制挑战，为6G网络中的RIS辅助近场通信提供了实用解决方案。
...
TL;DR: HAPS系统在VHetNet中面临能源挑战，本文提出HAPS间的能量收集策略，通过能量充足的HAPS为受限HAPS供能，并分析其性能指标。


<details>
  <summary>Details</summary>
Motivation: 高空平台站(HAPS)系统在未来无线通信网络中至关重要，但在垂直异构网络(VHetNet)架构中作为核心元素面临显著的能源挑战，这成为其运行的主要限制因素。

Method: 提出针对HAPS系统的能量收集策略，使一个HAPS系统能够从另一个不受能源限制的HAPS系统收集能量。通过推导中断概率、遍历容量并使用蒙特卡洛模拟验证，同时探索系统的吞吐量性能。

Result: 研究结果表明，充分利用能量收集的潜力是满足HAPS系统能源需求的可行方法。性能分析验证了所提模型的有效性。

Conclusion: HAPS间的能量收集策略为解决VHetNet架构中HAPS系统的能源限制问题提供了有效解决方案，有助于推动HAPS在下一代无线通信网络中的广泛应用。
...
TL;DR: 该论文提出了一个增强型XPM模型，明确考虑了色散导致的强度波动在频域中的演化，相比先前主要关注时域脉冲形变的模型有所改进，能更准确地预测XPM引起的相位失真和系统性能。


<details>
  <summary>Details</summary>
Motivation: 在高容量波分复用系统中，交叉相位调制是一个关键的非线性损伤，主要由色散引起的强度波动驱动。现有模型主要关注时域脉冲形变，未能充分捕捉强度波动在频域中的演化对XPM的影响。

Method: 提出了一个增强型XPM模型，明确将光纤中强度波动在频域的增长纳入考虑，建立了频域增长与XPM诱导相位失真之间的直接关联，并通过仿真验证模型准确性。

Result: 结果表明，强度波动演化（特别是低频部分）显著影响XPM相位波动谱和相位方差。模型能准确预测不同系统参数下的频谱特性，推导的相位方差能准确预测系统误码率性能。

Conclusion: 为了准确表征XPM损伤，必须对强度波动在频域的演化进行建模。该研究为先进光网络设计提供了指导，强调了频域分析在XPM建模中的重要性。
...
TL;DR: OpenISAC是一个开源实时ISAC实验平台，采用OFDM波形，支持单/双基地延迟多普勒感知，通过OTA同步实现双基地操作，基于USRP硬件和C++/Python软件栈。


<details>
  <summary>Details</summary>
Motivation: ISAC作为6G关键应用场景，理论研究已取得显著进展，但缺乏可访问、开源、实时的实验平台阻碍了进一步研究。需要填补这一空白以推动ISAC发展。

Method: 基于OFDM波形实现单基地和双基地延迟多普勒感知功能，采用创新的OTA同步机制实现无有线连接的双基地操作。平台完全基于开源软件，使用UHD库，支持从低成本B200到高性能X400系列USRP。物理层调制解调器用C++实现高速处理，感知数据流传输到Python环境提供用户友好接口。

Result: 开发了OpenISAC平台，具有灵活参数选择、实时通信和感知操作能力，支持广泛的软件定义无线电硬件，为学术界和研究社区提供了强大的ISAC实验工具。

Conclusion: OpenISAC是一个多功能、高性能的开源平台，能够促进OFDM-ISAC领域的探索和创新，解决了ISAC研究缺乏实验平台的问题。
...
TL;DR: F⁴-CKM：一种基于辐射场渲染的6G信道知识地图构建框架，通过无线辐射表示网络捕捉信道空间频率特性，实现仅基于用户位置的高精度信道预测。


<details>
  <summary>Details</summary>
Motivation: 6G移动通信中，天线阵列规模和带宽增长使得获取准确及时的信道状态信息(CSI)日益困难。现有信道知识地图(CKM)方法在如何有效构建方面仍存在问题，需要新的框架来提升基于用户位置的信道预测能力。

Method: 提出F⁴-CKM框架，具有四个特征：辐射场渲染、空间频率感知、免位置使用、快速学习。核心是将计算机视觉中的辐射场渲染技术适配到射频域，通过新型无线辐射表示(WiRARE)网络捕捉无线信道的空间频率特性，并引入整形滤波模块和角度采样策略。

Result: 大量实验表明，F⁴-CKM在无线信道预测准确性和效率方面显著优于现有基线方法。

Conclusion: F⁴-CKM为6G信道知识地图构建提供了有效的解决方案，通过创新的辐射场渲染技术和空间频率感知机制，实现了高精度、高效率的信道预测，有望缓解6G通信中的CSI反馈负担。
...
TL;DR: 提出了一种用于Zak-OTFS ISAC系统的半盲原子范数去噪方案，用于双基地感知，在分数延迟多普勒偏移环境下实现高精度信道估计和数据检测。


<details>
  <summary>Details</summary>
Motivation: 在双分散环境中，分数延迟多普勒偏移会导致严重的信道扩展，使得精确的信道估计成为实现准确双基地感知和鲁棒通信的关键挑战。现有方法难以在这种高移动性场景下有效工作。

Method: 首先推导了Zak-OTFS在分数延迟多普勒偏移和矩形窗下的离散时间输入输出关系。基于此，将联合信道参数估计和数据检测任务表述为原子范数去噪问题，使用负平方惩罚方法处理非凸离散星座约束。开发了结合majorization-minimization、加速投影梯度和不精确加速近端梯度方法的加速迭代算法。

Result: 仿真结果表明，所提方案实现了超分辨率感知精度，通信性能接近完美信道状态信息下界。算法具有严格的收敛性证明。

Conclusion: 提出的半盲原子范数去噪方案能够有效解决Zak-OTFS ISAC系统在分数延迟多普勒偏移环境下的信道估计挑战，为高移动性场景下的集成感知与通信提供了有前景的解决方案。
...
TL;DR: 该论文推导了典型TTD系统中角度估计的克拉美-罗界，研究了Fisher信息的性质，并利用ML等估计器解决ULA的角度估计问题。


<details>
  <summary>Details</summary>
Motivation: 在联合通信与感知（JC&S）背景下，获取精确的参数估计（如AoA）对于解决初始接入、干扰抑制、用户定位、环境监测和MIMO系统同步等问题至关重要。近年来，TTD系统因在初始接入中快速波束训练和缓解波束斜视而受到关注。

Method: 推导了典型TTD系统中角度估计的克拉美-罗界（CRB），研究了CRB和Fisher信息的性质，并进行了数值评估。最后，利用最大似然（ML）等已建立的估计器来解决均匀线性阵列的角度估计问题。

Result: 论文得出了TTD系统中角度估计的CRB表达式，分析了其性质，并通过数值评估验证了理论结果。展示了ML等估计器在均匀线性阵列上解决角度估计问题的有效性。

Conclusion: 该研究为TTD系统中的角度估计提供了理论性能界限（CRB），分析了估计精度的影响因素，并验证了实际估计器（如ML）在解决角度估计问题中的可行性，对JC&S应用中的参数估计具有重要意义。
...
TL;DR: 提出一种两阶段多波束训练方法，通过稀疏子阵列和密集子阵列设计，在毫米波多用户通信中实现快速准确的角度识别，平衡训练开销与波束识别成功率。
...
Method: 提出两阶段多波束训练方法：第一阶段使用稀疏子阵列生成多个高阵列增益波束识别候选用户角度；第二阶段使用密集子阵列生成灵活转向的宽波束，采用交叉验证方法解决第一阶段剩余的角模糊问题。

Result: 数值结果表明，与现有多波束训练方法相比，所提方法显著提高了波束识别成功率，同时保持甚至减少了所需的波束训练开销。

Conclusion: 该方法有效解决了毫米波多用户通信系统中多波束训练的关键挑战，在训练开销和识别准确性之间取得了良好平衡，为实际系统部署提供了高效解决方案。
...
TL;DR: CSI-MAE：基于掩码自编码器的通用信道基础模型，通过CSI感知与生成实现跨场景泛化，在无线通信中表现出优异的零样本迁移能力


<details>
  <summary>Details</summary>
Motivation: 现有信道基础模型存在三大问题：过度依赖场景特定数据导致泛化能力差、仅关注单一/双任务、缺乏零样本学习能力。需要开发能适应不同无线环境、具有跨场景泛化能力的通用模型。

Method: 提出CSI-MAE模型，基于掩码自编码器架构，在3GPP信道模型数据集上进行训练。模型通过CSI感知和生成整合感知与通信功能，采用轻量级解码器微调策略降低训练成本。

Result: CSI-MAE在多种任务中表现优异：通过轻量级微调可达到或超越监督模型性能；全参数微调时达到最先进水平；在跨场景应用中展现出与监督技术相当的零样本迁移能力。

Conclusion: CSI-MAE作为通用信道基础模型，成功解决了现有模型的局限性，在跨场景泛化、多任务适应和零样本学习方面表现突出，为无线通信创新提供了有力工具。
...
<details>
  <summary>Details</summary>
Motivation: 推动语音欺骗和深度伪造检测技术的研究，通过挑战赛形式促进解决方案的发展，应对日益复杂的语音伪造技术威胁。

Method: 构建新的众包数据库，包含更多说话者和多样录音条件，混合前沿和传统生成语音技术，组织挑战赛收集53个团队的解决方案进行评估。

Result: 许多解决方案表现良好，但在对抗攻击和神经编码/压缩方案下性能下降。挑战赛后进行了校准研究和其他主要挑战的回顾。

Conclusion: ASVspoof挑战赛为语音欺骗检测提供了重要平台，未来需要继续应对对抗攻击和压缩技术带来的挑战，并制定了未来发展路线图。
...
TL;DR: 提出一种基于贪婪选择算法的轻量级模型辅助深度学习架构，用于不完美CSI下的多用户混合下行波束成形和功率分配，通过自适应退火损失函数实现更好的功率-中断权衡。


<details>
  <summary>Details</summary>
Motivation: 传统优化方法在计算上成本高昂，而通用深度网络架构缺乏可解释性且需要大量训练数据。需要一种既能处理不完美CSI和概率中断约束，又具有计算效率和泛化能力的方法。

Method: 提出基于贪婪选择算法的轻量级模型辅助深度学习架构，通过实例自适应增强信号模型来估计CSI误差影响。使用嵌套约束波束成形问题的隐式表示，并推导梯度存在条件。在损失函数中采用基于退火的近似方法替代传统的分位数损失。

Result: 仿真验证表明，所提出的深度网络在信道估计和信道压缩导致的CSI误差下能够达到标称中断水平，同时分配比基准方法更少的功率。单个训练模型能够泛化到不同用户数、QoS要求和CSI质量水平。

Conclusion: 提出的方法通过自适应退火损失函数加速训练并获得更好的功率-中断权衡，实现了在不完美CSI下高效、可泛化的混合波束成形和功率分配。
...
<details>
  <summary>Details</summary>
Motivation: 传统多智能体强化学习方法依赖手工特征提取或原始向量表示，限制了策略在输入顺序和大小方面的可扩展性和效率。需要一种能够有效处理智能体观测空间的方法。

Method: 提出基于图像的强化学习方法，将智能体观测编码为结构化视觉输入，通过神经网络处理这些图像输入，提取空间特征，生成分散式运动控制规则。

Result: 在有限距离和仅方位感知的多智能体聚合任务中，该方法实现了高收敛率，收敛速度接近VariAntNet，在某些场景中是唯一可行的替代方案。

Conclusion: 基于图像的强化学习方法为多智能体分散控制提供了一种有效的解决方案，能够处理结构化视觉输入，在保持群体凝聚力的任务中表现出色。
...
### [14] [Lightweight Transformer Architectures for Edge Devices in Real-Time Applications](https://arxiv.org/abs/2601.03290)
*Hema Hariharan Samson*

Main category: cs.LG

TL;DR: 该综述系统分析了面向边缘设备的轻量级Transformer架构，涵盖模型压缩、量化、剪枝和知识蒸馏等技术，评估了多种轻量变体在边缘硬件上的性能表现和部署策略。
...
Result: 现代轻量级Transformer能达到原模型75-96%的准确率，模型大小减少4-10倍，推理延迟降低3-9倍，可在2-5W功耗的设备上部署。15-40M参数模型达到60-75%的硬件利用率，量化策略和硬件感知NAS是最有效的优化方法。

Conclusion: 轻量级Transformer已能在边缘设备上实现高效部署，稀疏注意力机制、混合精度量化和硬件感知NAS是关键优化策略，6步部署流程可实现8-12倍大小压缩且精度损失小于2%。
...
### [15] [Why LLMs Aren't Scientists Yet: Lessons from Four Autonomous Research Attempts](https://arxiv.org/abs/2601.03315)
*Dhruv Trehan,Paras Chopra*

Main category: cs.LG

TL;DR: 研究团队尝试用6个LLM代理组成的流水线自动生成ML研究论文，4次尝试中3次失败，1次成功并被会议接受，从中识别出6种常见失败模式，并提出4个设计原则


<details>
  <summary>Details</summary>
Motivation: 探索使用LLM代理流水线自主生成机器学习研究论文的可行性，了解AI科学家系统的能力边界和失败模式

Method: 使用6个LLM代理映射科学工作流程的不同阶段，构建端到端的论文生成流水线，进行4次完整尝试

Result: 4次尝试中3次失败（在实施或评估阶段），1次成功完成流水线并被Agents4Science 2025会议接受，通过了人类和多AI评审

Conclusion: 识别出6种常见失败模式，提出4个设计原则以构建更稳健的AI科学家系统，并讨论了自主科学发现的影响
...
### [16] [Ratio-Variance Regularized Policy Optimization for Efficient LLM Fine-tuning](https://arxiv.org/abs/2601.03320)
*Yu Luo,Shuo Han,Yihan Hu,Dong Li,Jianye Hao*

Main category: cs.LG

TL;DR: R²VPO：一种基于策略比率方差约束的RL算法，替代硬裁剪，提升LLM对齐的稳定性和数据效率


<details>
  <summary>Details</summary>
Motivation: 现有基于PPO/GRPO的LLM微调方法使用策略比率硬裁剪，虽然稳定训练但存在两个问题：1）会截断高回报但高分歧动作的梯度，抑制复杂推理中的"顿悟时刻"；2）数据稍有过时就被丢弃，导致严重的样本低效

Method: 重新审视策略优化的信任区域目标，提出显式约束策略比率的方差（二阶中心矩）作为硬裁剪的原则性平滑松弛。基于此提出R²VPO（比率-方差正则化策略优化），这是一个原始-对偶框架，支持稳定的在线学习，并通过动态重加权过时样本而非丢弃它们来实现原则性的离线数据重用

Result: 在DeepSeek-Distill-Qwen-1.5B和openPangu-Embedded系列（1B和7B）等先进LLM微调上评估，在数学推理基准测试中，R²VPO始终实现更优的渐近性能，相比基于裁剪的基线平均相对提升达17%，同时收敛所需rollout数量减少约50%

Conclusion: 比率方差控制是改进RL-based LLM对齐稳定性和数据效率的有前景方向，R²VPO框架为策略优化提供了更原则性的方法
...
### [17] [Aligning Findings with Diagnosis: A Self-Consistent Reinforcement Learning Framework for Trustworthy Radiology Reporting](https://arxiv.org/abs/2601.03321)
*Kun Zhao,Siyuan Dai,Pan Wang,Jifeng Song,Hui Ji,Chenghua Lin,Liang Zhan,Haoteng Tang*

Main category: cs.LG

TL;DR: 提出一个用于放射学报告生成的自洽框架，通过"推理-总结"架构和GRPO优化，显著减少幻觉并提升临床效果


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在放射学报告生成中展现出潜力，但面临架构异构性和事实幻觉问题。现有方法难以严格对齐语言输出与视觉证据，且强化学习方法存在计算成本高或探索有限的问题。

Method: 1) 系统评估确定医学影像的最佳视觉编码器和LLM骨干配置；2) 提出"推理-总结"架构，包含详细发现的思考块和结构化疾病标签的答案块；3) 使用Group Relative Policy Optimization (GRPO)优化；4) 采用多维复合奖励函数，明确惩罚生成叙述与最终诊断之间的逻辑不一致。

Result: 在MIMIC-CXR基准测试中，该方法在临床效果指标上达到最先进性能，相比强监督基线显著减少了幻觉。

Conclusion: 提出的自洽放射学报告生成框架通过系统架构设计和GRPO优化，有效解决了幻觉问题，提升了临床实用性，为医学影像报告生成提供了新思路。
...
### [18] [Learning Shrinks the Hard Tail: Training-Dependent Inference Scaling in a Solvable Linear Model](https://arxiv.org/abs/2601.03764)
*Noam Levi*

Main category: cs.LG

TL;DR: 论文提出潜在实例难度（LID）模型，分析最后一层微调中的神经缩放定律，发现推理失败率呈幂律衰减，其指数随训练样本增加而增长，最终饱和于难度分布尾部决定的固有极限。


<details>
  <summary>Details</summary>
Motivation: 现有神经缩放定律主要关注泛化损失，但缺乏对推理性能（如pass@k）的系统分析。本文旨在理解目标函数内在的实例异质难度如何影响推理性能的缩放行为。

Method: 提出潜在实例难度（LID）模型：每个输入的目标方差由潜在"精度"参数控制，该参数服从重尾分布。通过分析最后一层微调的可解模型，推导泛化损失和pass@k失败率的理论表达式。

Result: 泛化损失遵循标准缩放定律，而pass@k失败率呈幂律衰减k^{-β_eff}。观测指数β_eff随训练样本N增加而增长，最终饱和于难度分布尾部决定的固有极限β。学习过程缩小了误差分布的"困难尾部"。

Conclusion: LID模型揭示了泛化误差与推理性能之间的耦合关系：模型改进使pass@k曲线变陡，直到不可约的目标方差占主导。模型给出了可测试的闭式预测，包括计算分配规则（训练优先于饱和，推理尝试在后），并在模拟和真实数据中得到验证。
...
### [19] [HEEGNet: Hyperbolic Embeddings for EEG](https://arxiv.org/abs/2601.03322)
...
<details>
  <summary>Details</summary>
Motivation: EEG脑机接口面临跨域（如不同受试者）分布偏移导致的泛化能力差问题。EEG数据具有层次结构特性，而双曲空间作为树状结构的连续模拟，能更好地表示层次数据，因此探索双曲几何在EEG解码中的应用。

Method: 提出HEEGNet混合双曲网络架构：1）结合欧几里得和双曲编码器；2）采用新颖的粗到细域适应策略；3）利用双曲空间捕捉EEG的层次结构并学习域不变的双曲嵌入。

Result: 在多个公共EEG数据集（包括视觉诱发电位、情绪识别和颅内EEG）上的实验表明，HEEGNet实现了最先进的性能，验证了双曲嵌入能改善泛化能力。

Conclusion: EEG数据确实表现出双曲特性，双曲嵌入能有效提高EEG解码的泛化性能。HEEGNet通过结合欧几里得和双曲表示，成功捕捉了EEG的层次结构，为跨域EEG解码提供了有效解决方案。
...
### [20] [A Gap Between Decision Trees and Neural Networks](https://arxiv.org/abs/2601.03919)
*Akash Kumar*

Main category: cs.LG

TL;DR: 研究决策树与浅层神经网络之间的几何简单性与准确性权衡：决策树产生轴对齐的决策区域，而浅层ReLU网络通过阈值化预测。使用Radon全变差(RTV)度量几何复杂度，发现决策树指示函数及其连续替代具有无限RTV，而构造的平滑屏障评分函数具有有限RTV并能精确恢复决策区域。


<details>
  <summary>Details</summary>
Motivation: 研究几何简单性（作为可解释性的一种形式）与浅层神经网络准确逼近轴对齐决策树能力之间的潜在冲突。决策树产生基于规则的轴对齐决策区域（盒子的有限并集），而浅层ReLU网络通常作为评分模型训练，通过阈值化获得预测。需要理解这两种方法在几何复杂度上的根本差异。

Method: 使用无限宽度、有界范数、单隐藏层ReLU网络类，通过Radon全变差(RTV)半范数分析几何复杂度。首先分析决策树指示函数及其连续替代（分段线性斜坡平滑、sigmoid平滑、高斯卷积）的RTV性质。然后构造具有有限RTV的平滑屏障评分函数，在温和的管质量条件下证明L1(P)校准边界，并给出基于面测度的显式RTV上界。

Result: 发现硬树指示函数具有无限RTV。在d>1维度上，两种自然的分段连续替代（分段线性斜坡平滑和sigmoid平滑）也具有无限RTV。高斯卷积产生有限RTV，但具有显式的指数维度依赖性。构造的平滑屏障评分函数具有有限RTV，其固定阈值τ=1能精确恢复盒子决策区域。在合成矩形并集上的实验展示了准确性与复杂度之间的权衡。

Conclusion: 决策树的几何简单性与浅层神经网络的准确逼近之间存在根本冲突。通过分离分类（恢复决策集）与评分学习（学习校准评分）两个目标，可以构造具有有限几何复杂度的评分函数来精确恢复决策区域。这为理解神经网络表示能力与可解释性之间的权衡提供了理论框架。
...
### [21] [Extreme-value forest fire prediction A study of the Loss Function in an Ordinality Scheme](https://arxiv.org/abs/2601.03327)
*Nicolas Caron,Christophe Guyeux,Hassan Noura,Benjamin Aynes*

Main category: cs.LG

TL;DR: 提出首个用于法国野火严重程度预测的序数分类框架，通过比较不同损失函数（包括新提出的TDeGPD损失）来改进对罕见高严重度火灾的预测能力。


<details>
  <summary>Details</summary>
Motivation: 野火在空间和严重程度上高度不平衡，预测极端事件特别困难。现有方法难以准确预测罕见但关键的高严重度火灾，需要与法国实际决策需求直接对齐的预测框架。

Method: 引入首个野火严重程度序数分类框架，研究损失函数设计对神经网络预测能力的影响。比较标准交叉熵与多种序数感知目标函数，包括新提出的基于截断离散指数广义帕累托分布的概率TDeGPD损失。在多种架构和真实操作数据上进行广泛基准测试。
...
### [22] [Attention mechanisms in neural networks](https://arxiv.org/abs/2601.03329)
*Hasi Hays*

Main category: cs.LG

TL;DR: 该专论对注意力机制进行了全面严谨的数学分析，涵盖理论基础、计算特性和实际应用，重点讨论了其在NLP、CV和多模态学习中的表现，并指出了当前限制。


<details>
  <summary>Details</summary>
Motivation: 注意力机制代表了神经网络架构的根本范式转变，但缺乏系统性的数学理论分析。需要全面理解其理论基础、计算特性和实际应用，以推动该领域发展。

Method: 采用数学分析方法，系统研究注意力机制的理论基础、计算特性和实现方式。通过实证分析训练特性、缩放规律、注意力模式可视化，并在标准数据集上进行性能基准测试。

Result: 展示了注意力机制在语言建模、双向编码器、序列到序列翻译、视觉Transformer和跨模态任务中的广泛应用。揭示了性能与模型大小和计算量之间的缩放规律，以及注意力模式与语言/视觉结构的关系。

Conclusion: 注意力机制是强大的深度学习工具，但仍面临计算可扩展性、数据效率、系统泛化和可解释性等挑战，需要进一步研究解决这些限制。
...
### [23] [LUT-KAN: Segment-wise LUT Quantization for Fast KAN Inference](https://arxiv.org/abs/2601.03332)
*Oleksandr Kuznetsov*

Main category: cs.LG

TL;DR: LUT-KAN：一种基于查找表的KAN量化方法，通过将边缘函数转换为分段查找表，实现12倍推理加速，同时保持分类精度


<details>
  <summary>Details</summary>
Motivation: KAN网络使用可学习单变量函数（通常为B样条）替代标量权重，虽然准确且可解释，但在CPU上推理成本高，且标准量化工具链难以应用，因为主要计算不是矩阵乘法而是重复的样条基函数评估

Method: 提出LUT-KAN方法：将每个边缘函数转换为分段查找表，采用仿射int8/uint8量化和线性插值；建立明确的推理契约，包括边界约定和越界策略；提出"诚实基线"评估方法，在相同后端优化下比较B样条和LUT评估

Result: 在DoS攻击检测案例中，编译后的模型保持分类质量（F1下降低于0.0002），CPU推理延迟降低12倍（NumPy）和10倍（Numba）；内存开销约为10倍（L=64时）；提供完整的量化方案和边界策略评估

Conclusion: LUT-KAN为KAN网络提供了高效的量化推理方案，显著加速CPU推理同时保持精度，解决了KAN在部署中的计算瓶颈问题，所有代码和工件公开以确保可复现性
...
### [24] [Physics-Informed Gaussian Process Regression for the Constitutive Modeling of Concrete: A Data-Driven Improvement to Phenomenological Models](https://arxiv.org/abs/2601.03367)
...
<details>
  <summary>Details</summary>
Motivation: 广泛使用的KCC等现象学模型依赖经验校准的失效面，缺乏模型形式的灵活性和不确定性量化能力，需要开发更灵活、物理一致的替代方法。

Method: 保留KCC模型的模块化弹塑性结构，用约束高斯过程回归替代经验失效面。通过三轴压缩实验数据训练，引入基于导数的物理约束来确保材料行为的一致性。

Result: 无约束GPR在训练条件附近插值良好，但外推时性能恶化并违反物理约束。物理信息GPR在泛化能力、准确性和可靠性方面显著更好，即使在训练范围外的高围压条件下也能保持良好性能，同时减少预测方差。

Conclusion: 提出的物理信息框架提供了鲁棒、不确定性感知的替代模型，在不牺牲KCC模型可解释性和数值效率的前提下，改善了泛化能力并简化了校准过程，为改进混凝土本构模型提供了实用路径。
...
### [25] [Enhancing Small Dataset Classification Using Projected Quantum Kernels with Convolutional Neural Networks](https://arxiv.org/abs/2601.03375)
*A. M. A. S. D. Alagiyawanna,Asoka Karunananda,A. Mahasinghe,Thushari Silva*

Main category: cs.LG

TL;DR: 提出基于投影量子核的CNN增强方法，在小数据集上显著提升图像分类性能


<details>
  <summary>Details</summary>
Motivation: 传统CNN依赖大规模标注数据，但在数据稀缺场景下性能受限，需要新的特征提取方法

Method: 引入投影量子核（PQK）增强CNN特征提取能力，利用量子计算原理捕获复杂模式

Result: 在1000个训练样本下，PQK-CNN在MNIST达到95%准确率（传统CNN 60%），在CIFAR-10达到90%（传统CNN 12%）
...
### [26] [Weather-Aware Transformer for Real-Time Route Optimization in Drone-as-a-Service Operations](https://arxiv.org/abs/2601.03376)
...
<details>
  <summary>Details</summary>
Motivation: 传统路径规划算法（如A*和Dijkstra）虽然能提供最优解，但计算复杂度高，在动态环境中难以实时应用，特别是在考虑天气因素影响无人机操作的情况下。

Method: 使用经典算法模拟生成的合成数据集训练机器学习和深度学习模型，采用基于Transformer和注意力机制的架构，结合天气启发式信息预测最优下一节点选择，注意力机制动态加权风速、风向、温度等环境因素。

Result: 天气感知模型相比传统算法实现显著计算加速，同时保持路线优化性能，基于Transformer的架构在动态环境约束下表现出更好的适应性。

Conclusion: 该框架为大规模无人机服务运营提供实时、天气响应的路线优化，显著提升了自主无人机系统的效率和安全性。
...
### [27] [SIGMA: Scalable Spectral Insights for LLM Collapse](https://arxiv.org/abs/2601.03385)
*Yi Gu,Lingyou Pang,Xiangkun Ye,Tianyu Wang,Jianyu Lin,Carey E. Priebe,Alexander Aue*

Main category: cs.LG

TL;DR: 提出SIGMA框架，通过分析嵌入Gram矩阵的谱特征来量化LLM训练中的模型崩溃现象，提供可扩展的监测工具。


<details>
  <summary>Details</summary>
Motivation: 合成数据训练LLM导致"模型崩溃"问题，现有方法难以在高维空间中量化预测崩溃发生，需要理论严谨且可扩展的监测框架。

Method: 提出SIGMA框架，通过推导嵌入Gram矩阵谱的确定性和随机性边界，建立数学基础指标来追踪表示空间的收缩，随机公式支持大规模模型的可扩展估计。

Result: SIGMA能有效捕捉向退化状态的转变，为崩溃机制提供理论见解，并为递归训练管道提供实用的可扩展监测工具。

Conclusion: SIGMA为模型崩溃问题提供了数学严谨且可扩展的量化框架，既能增进对崩溃机制的理论理解，又能实际监测大规模基础模型的训练健康状态。
...
### [28] [Inferring Clinically Relevant Molecular Subtypes of Pancreatic Cancer from Routine Histopathology Using Deep Learning](https://arxiv.org/abs/2601.03410)
...
TL;DR: PanSubNet是一个可解释的深度学习框架，直接从H&E染色全切片图像预测胰腺导管腺癌的分子亚型，无需RNA测序，在内部验证中AUC达88.5%，外部验证AUC 84.0%，具有临床部署潜力。


<details>
  <summary>Details</summary>
Motivation: PDAC的分子分型（基底样和经典型）具有预后和预测价值，但RNA测序成本高、耗时长、组织要求严格，限制了临床实践中的应用。需要一种更快速、经济、基于常规病理切片的方法。

Method: 开发PanSubNet深度学习框架，使用1,055例患者的配对H&E切片和RNA-seq数据。采用双尺度架构融合细胞级形态和组织级结构，利用注意力机制进行多尺度表示学习和透明特征归因。使用Moffitt 50基因签名结合GATA6表达作为金标准标签。

Result: 内部验证（五折交叉验证）平均AUC 88.5%，平衡敏感性和特异性。外部验证（TCGA队列）AUC 84.0%，无需微调。模型预测与转录组程序、分化标志物和DNA损伤修复特征一致。预测不确定性反映中间转录状态而非分类噪声。

Conclusion: PanSubNet能够从常规H&E染色切片实现快速、经济的分子分层，提供临床可部署且可解释的工具，支持整合到数字病理工作流程，推进PDAC精准肿瘤学。
...
### [29] [Jailbreaking LLMs Without Gradients or Priors: Effective and Transferable Attacks](https://arxiv.org/abs/2601.03420)
*Zhakshylyk Nurlanov,Frank R. Schmidt,Florian Bernard*

Main category: cs.LG

TL;DR: RAILS：无需梯度或先验知识的随机迭代局部搜索框架，通过token级优化实现高效对抗越狱攻击，支持跨分词器集成攻击


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全评估存在局限性，现有自动攻击方法依赖手工先验或白盒梯度访问，导致对模型鲁棒性估计过高。需要开发不依赖这些限制的攻击方法以更准确评估模型安全性。

Method: 提出RAILS框架，仅基于模型logits进行token级迭代优化。核心创新包括：1）新颖的自回归损失函数确保精确前缀匹配；2）基于历史的选择策略桥接代理优化目标与真实攻击成功率。无需梯度依赖，支持跨分词器集成攻击。

Result: RAILS在多个开源模型上实现接近100%的成功率，对GPT和Gemini等闭源系统具有高黑盒攻击可迁移性。跨分词器集成攻击能发现共享对抗模式，显著提升对闭源系统的可迁移性。

Conclusion: RAILS证明了无需梯度或先验知识的token级迭代优化能够有效实现对抗越狱攻击，为LLM安全评估提供了更准确的方法，并展示了跨分词器集成攻击的潜力。
...
TL;DR: 提出基于注意力图谱的训练无关机制探针，通过代数连通性、平滑度和谱熵等指标，揭示模型处理策略和课程学习中的脆弱性，特别是发现被动结构触发的连通性崩溃现象。


<details>
  <summary>Details</summary>
Motivation: 行为基准测试只能告诉我们模型做什么，但不能揭示其内部工作机制。需要一种训练无关的方法来探测模型的机制性特征，以理解模型如何处理不同语言和结构。

Method: 使用注意力图谱谱分析方法：将每个注意力层视为token图，计算代数连通性(λ₂)、平滑度和谱熵等谱特征，形成"谱指纹"，用于分析12个模型在10种语言上的处理模式。

Result: 发现被动结构触发的连通性崩溃现象，模型在特定课程转换后出现英语特有的语法触发连通性故障；识别出四种重复出现的处理策略；定位到第2层的稀疏"补偿补丁"头；发现拓扑机制主要受分词密度而非语言身份影响。

Conclusion: 注意力图谱谱分析为模型审计和训练机制验证提供了实用工具，揭示了课程学习中的脆弱性和模型处理策略，有助于理解模型内部工作机制和优化训练过程。
...
TL;DR: MoE模型中的专家路由存在结构性偏见，大多数路由质量集中在少数"常设委员会"专家上，而非真正的领域专业化
...
Conclusion: MoE模型存在强烈的集中计算结构性偏见，专业化程度远低于普遍认知；当前训练目标（如强制均匀专家利用的负载均衡损失）可能违背模型的自然优化路径，限制了训练效率和性能
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注单源、视频内推理，而现实世界的新闻消费本质上是多源的：同一事件由不同媒体以互补细节、不同叙事选择和有时相互矛盾的主张进行报道。鲁棒的新闻理解需要模型能够比较不同来源的观点、对齐跨来源的多模态证据并综合多源信息。

Method: 设计了新的问题类型来测试模型从多角度理解多源多模态新闻的能力；开发了混合人机QA生成流程，解决了构建跨源新闻理解大规模数据集时的可扩展性和质量控制问题。

Result: 构建了包含429个新闻组、1,405个视频和2,501个高质量问题的数据集；对闭源和开源多模态模型的综合评估表明，VNU-Bench对当前MLLMs构成了重大挑战。

Conclusion: VNU-Bench填补了多源跨视频新闻理解基准测试的空白，为评估模型在真实世界多源新闻消费场景中的能力提供了重要工具，揭示了当前MLLMs在多源新闻理解方面的局限性。
...
TL;DR: 提出了一种用于用户自定义文本分类的软上下文编码器架构，通过上下文化候选标签和输入查询的软提示表示，在多样化数据集上训练以实现对未见主题集的零样本分类，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 用户自定义文本分类在现实应用中频繁出现（如企业分析、内容审核、领域特定信息检索），但现有方法难以处理用户指定的、先前未见过的类别，需要能够泛化到任意领域未见主题集的解决方案。

Method: 提出软上下文编码器架构，将每个候选标签与标签集和输入查询的静态软提示表示进行上下文化。在多样化、多源数据集上进行训练，使模型能够有效泛化到从任意领域抽取的完全未见主题集的零样本分类。

Result: 在分布内测试数据和多个未见UDTC基准测试上进行评估，模型在所有数据集上都达到了最先进的性能，始终优于或匹配基线方法。

Conclusion: 提出的软上下文编码器架构能够有效解决用户自定义文本分类问题，通过上下文化标签和查询表示，在多样化数据上训练，实现了对未见主题集的强大零样本分类能力。
...
### [34] [An Expectation-Maximization Algorithm for Domain Adaptation in Gaussian Causal Models](https://arxiv.org/abs/2601.03459)
*Mohammad Ali Javidian*

Main category: cs.LG

TL;DR: 提出一种基于EM的DAG感知框架，用于在因果DAG已知但目标变量系统缺失的域转移场景下进行目标变量插补，通过一阶梯度EM更新实现高效参数估计。


<details>
  <summary>Details</summary>
Motivation: 解决在域转移部署场景中，当目标变量系统缺失时如何进行准确插补的问题。传统方法在源域完全观测、目标域目标变量缺失的情况下难以有效利用因果结构信息进行知识迁移。

Method: 提出统一的基于EM的框架，结合源域和目标域数据通过DAG结构将信息从观测变量转移到缺失目标。采用一阶（梯度）EM更新替代计算昂贵的广义最小二乘M步，利用已知因果DAG冻结源域不变机制，仅重新估计受转移影响的那些条件分布。

Result: 在标准局部强凹性和平滑性假设及BWY梯度稳定性条件下，证明一阶EM算子围绕真实目标参数具有局部收缩性，获得几何收敛性，并在协变量转移和局部机制转移的高斯SEM下提供参数误差和目标插补误差的有限样本保证。实验表明在合成七节点SEM、64节点MAGIC-IRRI遗传网络和Sachs蛋白质信号数据上，该方法相比基于源域的贝叶斯网络和Kiiveri式EM基线显著提高了目标插补精度。

Conclusion: 提出的DAG感知一阶EM算法能够有效处理域转移下的目标变量插补问题，在显著域转移场景下获得最大改进，为因果DAG已知但目标变量缺失的迁移学习提供了可扩展的解决方案。
...
### [35] [Hybrid Approach for Driver Behavior Analysis with Machine Learning, Feature Optimization, and Explainable AI](https://arxiv.org/abs/2601.03477)
*Mehedi Hasan Shuvo,Md. Raihan Tapader,Nur Mohammad Tamjid,Sajjadul Islam,Ahnaf Atef Choudhury,Jia Uddin*

Main category: cs.LG

TL;DR: 本文提出了一种混合方法分析驾驶员行为，使用Kaggle数据集，通过随机森林分类器达到95%准确率，结合LIME可解释AI技术识别关键特征，在保持性能的同时提升模型效率。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶员行为分析研究常使用机器学习和深度学习技术，但存在特征优化不足的问题，导致性能与可解释性难以兼顾。需要填补这一空白，开发既能保持高性能又能提供解释性的混合方法。

Method: 使用Kaggle的12,857行18列数据集，经过标签编码、随机过采样和标准缩放等预处理后，测试13种机器学习算法。随机森林分类器表现最佳，然后应用LIME可解释AI技术识别对准确率影响最大的前10个正负特征，并重新训练相同算法。

Result: 随机森林分类器初始准确率达到95%。应用LIME识别关键特征后重新训练，准确率略微下降至94.2%，但模型效率得到提升，证实了在不牺牲性能的情况下改进模型效率的可能性。

Conclusion: 提出的混合模型在驾驶员行为预测能力和可解释性方面都能提供投资回报，为驾驶员行为分析提供了一种既能保持高性能又能提供解释性的有效方法。
...
### [36] [From Bits to Chips: An LLM-based Hardware-Aware Quantization Agent for Streamlined Deployment of LLMs](https://arxiv.org/abs/2601.03484)
...
TL;DR: HAQA是一个自动化框架，利用大语言模型简化模型量化部署流程，通过硬件感知的超参数调优，在保持精度的同时提升推理速度2.3倍


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型部署需求增长，普通用户面临硬件资源限制与模型量化复杂性的双重挑战，需要更友好的自动化解决方案

Method: 提出硬件感知量化代理（HAQA），利用LLM自动优化量化超参数和硬件配置，实现跨平台的适应性量化策略

Result: 在Llama模型上实现最高2.3倍推理加速，提升吞吐量和精度，能自动发现反直觉的最优设置，减少人工工作量

Conclusion: HAQA框架有效解决了量化部署的复杂性，为广泛用户提供了高质量、易用的模型部署方案，具有优秀的硬件适应性
...
### [37] [VeRPO: Verifiable Dense Reward Policy Optimization for Code Generation](https://arxiv.org/abs/2601.03525)
...
<details>
  <summary>Details</summary>
Motivation: 主流代码生成RL方法使用通过/失败结果奖励（执行单元测试）来确保功能正确性，但奖励稀疏性限制了性能提升。现有外部奖励模型方法存在奖励错位和高计算成本问题。

Method: VeRPO通过动态估计每个单元测试的难度权重（基于训练期间执行统计），从通过的单元测试权重总和推导出密集奖励。进一步将密集信号与全局执行结果整合，建立仅依赖可验证执行反馈的稳健密集奖励范式。

Result: 在多样化基准和设置上的实验表明，VeRPO始终优于结果驱动和基于奖励模型的基线方法，在pass@1指标上实现高达+8.83%的提升，时间成本可忽略不计（<0.02%）且零GPU内存开销。

Conclusion: VeRPO成功解决了代码生成RL中的奖励稀疏性问题，通过完全基于可验证执行反馈构建稳健密集奖励，在保持低计算成本的同时显著提升性能。
...
### [38] [Green's-Function Spherical Neural Operators for Biological Heterogeneity](https://arxiv.org/abs/2601.03561)
*Hao Tang,Hao Chen,Hao Li,Chao Li*

Main category: cs.LG

TL;DR: 提出可设计的格林函数框架(DGF)和基于此的格林函数球面神经算子(GSNO)，通过三种算子解（等变、不变、各向异性）在保留球面几何的同时建模现实世界异质性。


<details>
  <summary>Details</summary>
Motivation: 现有球面深度学习方法难以平衡强球面几何归纳偏置与建模现实世界异质性的需求，需要在保留球面几何的同时处理真实世界的异质性问题。

Method: 首先提出可设计的格林函数框架(DGF)作为新的球面算子求解策略，在旋转群下设计系统格林函数。基于DGF提出GSNO，融合三种算子解：1) 等变解（来自等变格林函数）用于对称一致性建模；2) 不变解（来自不变格林函数）消除干扰异质性；3) 各向异性解（来自各向异性格林函数）建模各向异性系统。

Result: 在球面MNIST、浅水方程、扩散MRI纤维预测、皮层分区和分子结构建模等任务上验证了GSNO的优越性，能够适应具有干扰变异性和各向异性的现实世界异质系统，同时保持谱效率。

Conclusion: GSNO通过融合三种格林函数解，在保留球面几何的同时有效建模现实世界异质性，为处理具有干扰变异性和各向异性的复杂系统提供了有效解决方案。
...
### [39] [A Proposed Paradigm for Imputing Missing Multi-Sensor Data in the Healthcare Domain](https://arxiv.org/abs/2601.03565)
*Vaibhav Gupta,Florian Grensing,Beyza Cinar,Maria Maleshkova*

Main category: cs.LG

TL;DR: 该论文系统分析了低血糖预测中多传感器数据的缺失值填补问题，提出根据特征特性和缺失时长定制化填补策略的范式。


<details>
  <summary>Details</summary>
Motivation: 慢性病管理面临挑战，特别是低血糖等并发症需要及时检测。可穿戴传感器提供连续监测，但多传感器数据存在噪声和频繁缺失值问题，影响低血糖预测效果。

Method: 1) 分析现有数据集的局限性，强调低血糖预测关键特征的时间特性；2) 全面分析最先进研究中使用的填补技术；3) 评估机器学习和深度学习在其他医疗场景中的填补方法处理长时间序列数据缺失的潜力；4) 提出系统化范式，根据特征特性和缺失间隔时长定制填补策略。

Result: 通过分析发现，需要研究个体特征的时间动态特性，并实施多种针对特定特征的填补技术，以有效处理数据中固有的异质时间模式。

Conclusion: 论文强调研究个体特征时间动态的重要性，并提出采用多种针对特定特征的填补技术来有效处理数据中的异质时间模式，为低血糖预测中的缺失值处理提供了系统化解决方案。
...
### [40] [Local Intrinsic Dimensionality of Ground Motion Data for Early Detection of Complex Catastrophic Slope Failure](https://arxiv.org/abs/2601.03569)
*Yuansan Liu,Antoinette Tordesillas,James Bailey*

Main category: cs.LG

TL;DR: 提出stLID方法，结合空间和时间信息改进局部内在维度(LID)，用于滑坡灾害检测，显著提高检测精度和提前预警时间


<details>
  <summary>Details</summary>
Motivation: 现有滑坡检测方法主要依赖地表位移数据，但往往无法同时捕捉空间相关性和时间动态特性，导致对复杂滑坡和连续多次滑坡的检测能力不足

Method: 基于sLID方法进行三方面改进：1) 运动学增强-引入速度信息捕捉短期时间依赖；2) 空间融合-贝叶斯估计聚合邻域sLID值；3) 时间建模-提出tLID学习长期动态，最终整合为统一的时空LID(stLID)框架

Result: stLID在滑坡故障检测精度和提前预警时间方面持续优于现有方法

Conclusion: 提出的stLID方法通过同时整合空间和时间信息，显著提升了滑坡灾害检测能力，为复杂滑坡和连续多次滑坡的早期识别提供了有效解决方案
...
### [41] [Variational Inference, Entropy, and Orthogonality: A Unified Theory of Mixture-of-Experts](https://arxiv.org/abs/2601.03577)
*Ye Su,Yong Liu*

Main category: cs.LG

TL;DR: 该论文为MoE模型建立了首个统一理论框架，从贝叶斯和信息论角度推导了Top-k路由和负载均衡的最优性，证明了路由的NP-hard本质，并提出了正交性正则化作为最优工程松弛方案。
...
Method: 从贝叶斯视角将路由问题形式化为稀疏后验近似和先验正则化；从信息论视角将其框架化为最小化路由模糊度和最大化信道容量；将路由定义为NP-hard稀疏子集选择问题；提出几何正交性作为工程松弛方案。

Result: 证明了"相干性障碍"的存在：当专家表示具有高互相干性时，贪婪路由策略理论上无法恢复最优专家子集；形式化验证了在专家特征空间中施加几何正交性足以缩小NP-hard全局最优与多项式时间贪婪近似之间的差距。
...
### [42] [Local Gradient Regulation Stabilizes Federated Learning under Client Heterogeneity](https://arxiv.org/abs/2601.03584)
...
<details>
  <summary>Details</summary>
Motivation: 现实部署中的统计异构性会破坏联邦学习的稳定性，主要原因是客户端优化过程中的本地梯度动态扭曲，导致系统性漂移并阻碍全局收敛

Method: 提出客户端视角的通用框架，通过探索-收敛梯度重聚合(ECGR)方法调节本地梯度贡献，平衡对齐良好和对齐不良的梯度分量，在不增加通信开销的情况下抑制不稳定效应

Result: 理论分析和广泛实验表明，调节本地梯度动态能一致地稳定异构数据分布下的联邦学习，包括在LC25000医学影像数据集上的评估

Conclusion: 本地梯度动态是稳定异构联邦学习系统的关键调节杠杆，通过ECGR等方法平衡梯度分量可以有效提升联邦学习在统计异构环境下的稳定性
...
### [43] [ALERT: Zero-shot LLM Jailbreak Detection via Internal Discrepancy Amplification](https://arxiv.org/abs/2601.03600)
*Xiao Lin,Philip Li,Zhichen Zeng,Tingwei Li,Tianxin Wei,Xuying Ning,Gaotang Li,Yuzhong Chen,Hanghang Tong*

Main category: cs.LG

TL;DR: ALERT：基于特征放大的零样本越狱检测框架，通过逐层、逐模块、逐token放大良性提示与越狱提示的内部特征差异，实现高效检测


<details>
  <summary>Details</summary>
Motivation: 现有越狱检测方法主要依赖训练数据中的越狱模板，但现实场景中新攻击不断涌现，需要解决更具挑战性的零样本检测问题

Method: 提出三层放大框架：1) 识别安全相关层；2) 定位编码零样本判别信号的特定模块；3) 定位信息丰富的安全token。在此基础上构建ALERT检测器，使用两个独立互补的分类器对放大后的表示进行分类

Result: 在三个安全基准测试中，ALERT在所有数据集和攻击策略上均稳定排名前二，平均准确率和F1分数比次优基线至少高出10%，有时高达40%

Conclusion: ALERT通过放大内部特征差异，实现了高效且有效的零样本越狱检测，能更好地应对现实世界中不断演化的新型攻击
...
### [44] [A Comparative Study of Traditional Machine Learning, Deep Learning, and Large Language Models for Mental Health Forecasting using Smartphone Sensing Data](https://arxiv.org/abs/2601.03603)
*Kaidong Feng,Zhu Sun,Roy Ka-Wei Lee,Xun Jiang,Yin-Leng Theng,Yi Ding*

Main category: cs.LG

TL;DR: 该研究首次系统比较了传统机器学习、深度学习和大型语言模型在心理健康预测方面的表现，发现深度学习模型（特别是Transformer）整体表现最佳，而个性化策略能显著提升对严重心理健康状态的预测能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注心理健康检测（对已有状况的响应），而心理健康预测能实现主动干预。智能手机传感提供了一种无干扰、可扩展的方式来追踪与心理健康相关的日常行为，但需要系统评估不同建模方法在心理健康预测中的效果。

Method: 使用College Experience Sensing（CES）数据集，这是迄今为止最全面的大学生心理健康纵向数据集。系统比较了传统机器学习（ML）、深度学习（DL）和大型语言模型（LLM）方法，评估了时间窗口、特征粒度、个性化策略和类别不平衡处理等多个维度。

Result: 深度学习模型（特别是Transformer，Macro-F1 = 0.58）取得了最佳整体性能。LLM在上下文推理方面表现出优势，但在时间建模方面较弱。个性化策略显著改善了严重心理健康状态的预测能力。
...
### [45] [Policy-Guided Search on Tree-of-Thoughts for Efficient Problem Solving with Bounded Language Model Queries](https://arxiv.org/abs/2601.03606)
*Sumedh Pendurkar,Guni Sharon*

Main category: cs.LG

TL;DR: 该论文提出将Levin树搜索算法与思维树框架结合，利用语言模型的概率作为启发式指导搜索，在有限计算预算下减少思维评估次数，提高问题解决效率。
...
Method: 将Levin树搜索算法适配到思维树框架中，利用语言模型的概率分布作为启发式函数来指导树探索。该方法还分析了温度参数对搜索边界的影响。

Result: 在固定语言模型查询预算下，LTS在三个领域（Blocksworld、PrOntoQA、Array Sorting）和四种不同语言模型上，始终达到与基线搜索算法相当或更高的准确率。

Conclusion: LTS在思维树框架中特别有效，能够实现成本效益和时间效率高的问题解决，非常适合延迟敏感和资源受限的应用场景。
...
### [46] [Mathematical Foundations of Polyphonic Music Generation via Structural Inductive Bias](https://arxiv.org/abs/2601.03612)
*Joonwon Seo*

Main category: cs.LG

TL;DR: 本文提出了一种解决复调音乐生成中"缺失中间层"问题的新方法，通过结构归纳偏置，在贝多芬钢琴奏鸣曲案例中验证了音高与手部属性的独立性，并设计了Smart Embedding架构，显著减少了参数数量并提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决复调音乐生成中的"Missing Middle"问题，即现有方法在音乐结构建模上的不足。通过引入结构归纳偏置来改进AI音乐生成，特别是在钢琴音乐这种需要同时处理音高和演奏手部属性的复杂场景。

Method: 1. 使用归一化互信息(NMI=0.167)验证音高与手部属性的独立性
2. 提出Smart Embedding架构，减少48.30%的参数
3. 运用信息理论(损失上界0.153比特)、Rademacher复杂度(泛化边界收紧28.09%)和范畴论进行严格数学证明
4. 通过SVD分析和专家听音研究(N=53)验证效果

Result: 1. 验证损失降低9.47%
2. 参数减少48.30%
3. 数学上证明了模型的稳定性和泛化能力提升
4. 专家听音研究确认了生成音乐的质量

Conclusion: 该方法通过结构归纳偏置有效解决了复调音乐生成的"Missing Middle"问题，建立了理论和应用相结合的双重框架，为基于数学基础的深度学习音乐生成提供了可验证的见解。
...
### [47] [Learning Shortest Paths When Data is Scarce](https://arxiv.org/abs/2601.03629)
*Dmytro Matsypura,Yu Pan,Hanzhao Wang*

Main category: cs.LG

TL;DR: 论文提出了一种利用数字孪生模拟器和有限真实数据校准网络路径成本的方法，通过拉普拉斯正则化最小二乘估计边成本偏差，并提供路径最优性验证和主动学习算法。


<details>
  <summary>Details</summary>
Motivation: 数字孪生等模拟器在大型网络路由决策中应用广泛，但模拟器输出存在系统性偏差，而真实测量数据成本高且稀缺。需要一种方法能够利用丰富的合成样本和有限的真实观测来校准模拟器偏差。

Method: 1. 将模拟器与真实世界的偏差建模为未知的边特定偏差，该偏差在相似性图上平滑变化；2. 使用拉普拉斯正则化最小二乘法估计偏差；3. 建立有限样本误差界；4. 提出可计算的数据驱动证书验证候选路径的近似最优性；5. 针对冷启动场景，开发基于偏差感知的主动学习算法。

Result: 该方法能够在数据稀缺情况下获得校准的边成本估计，建立了误差界并将估计误差转化为路径级次优性保证。在多个道路网络和交通图上的数值实验证明了方法的有效性。

Conclusion: 提出的框架能够有效利用模拟器和有限真实数据校准网络路径成本，提供理论保证和实用算法，特别适用于数据稀缺和冷启动场景，为数字孪生辅助路由决策提供了可靠解决方案。
...
### [48] [Kantorovich-Type Stochastic Neural Network Operators for the Mean-Square Approximation of Certain Second-Order Stochastic Processes](https://arxiv.org/abs/2601.03634)
*Sachin Saini,Uaday Singh*

Main category: cs.LG

TL;DR: 提出一种新型Kantorovich型随机神经网络算子(K-SNNOs)，通过随机神经元而非系数层面引入随机性，用于逼近随机过程，建立了均方收敛性和误差估计。


<details>
  <summary>Details</summary>
Motivation: 传统人工神经网络算子主要用于逼近确定性输入输出函数，但在随机动力学领域的扩展相对不足。需要构建能够继承底层过程概率结构、适合建模和逼近随机信号的算子框架。

Method: 构建Kantorovich型随机神经网络算子(K-SNNOs)，通过由随机积分器驱动的随机神经元引入随机性，而非在系数层面添加随机性。该框架使算子能够继承底层随机过程的概率结构。

Result: 建立了K-SNNOs对目标随机过程的均方收敛性，推导了以连续性模表示的定量误差估计。数值模拟验证了理论结果，展示了样本路径的准确重构和均方误差的快速衰减。

Conclusion: 提出的基于随机神经元的算子框架为随机过程的逼近提供了有效工具，具有理论保证和实际应用价值，图形结果进一步证明了该方法的鲁棒性和有效性。
...
### [49] [ReLA: Representation Learning and Aggregation for Job Scheduling with Reinforcement Learning](https://arxiv.org/abs/2601.03646)
...
<details>
  <summary>Details</summary>
Motivation: 现实制造系统中的作业调度需要将有序作业操作分配给机器，现有解决方案在运行时间和调度质量方面存在不足，特别是当问题规模增大时

Method: 提出ReLA强化学习调度器，采用结构化表示学习和聚合方法：1）使用自注意力和卷积的两种实体内学习模块学习作业操作和机器的多样化表示；2）使用交叉注意力的实体间学习模块；3）在多尺度架构中应用这些模块并聚合输出以支持强化学习决策

Result: 在小、中、大规模作业实例实验中，ReLA在大多数测试设置中实现最佳完工时间。在非大型实例上，将SOTA基线的最优性差距降低13.0%；在大型实例上降低78.6%，平均最优性差距分别降至7.3%和2.1%
...
### [50] [Quantum Classical Ridgelet Neural Network For Time Series Model](https://arxiv.org/abs/2601.03654)
...
<details>
  <summary>Details</summary>
Motivation: 传统时间序列分析方法在处理复杂金融数据时存在局限性，需要更强大的特征提取和预测能力

Method: 将Ridgelet神经网络与单量子比特量子计算方法集成，构建量子处理流水线进行时间序列分析

Result: 在金融时间序列数据上的实验结果显示，该模型相比现有模型具有更优越的性能

Conclusion: 量子计算与Ridgelet变换的结合为时间序列分析提供了有效的新方法，特别是在金融预测领域具有应用潜力
...
### [51] [In Search of Grandmother Cells: Tracing Interpretable Neurons in Tabular Representations](https://arxiv.org/abs/2601.03657)
*Ricardo Knauer,Erik Rodner*

Main category: cs.LG

TL;DR: 该研究提出两种信息论度量来量化神经元对单个概念的显著性和选择性，在表格基础模型TabPFN中发现了一些对高级概念具有中等但统计显著性的神经元，表明可解释神经元可以自然涌现。
...
Method: 提出两种信息论度量：神经元显著性和选择性度量，应用于表格基础模型TabPFN的表征，通过简单的神经元-概念对搜索找到最显著和最具选择性的配对。

Result: 首次证明此类模型中某些神经元对高级概念表现出中等但统计显著的显著性和选择性，这些神经元可以在不依赖复杂可解释性技术的情况下被识别。

Conclusion: 可解释神经元可以自然涌现，在某些情况下可以通过简单方法识别，为理解基础模型的内部表征提供了新途径。
...
### [52] [Group and Exclusive Sparse Regularization-based Continual Learning of CNNs](https://arxiv.org/abs/2601.03658)
...
Method: 提出GESCL方法，包含两个正则化项：1）稳定性正则化防止对过去任务重要的滤波器过度偏离；2）可塑性正则化利用CNN过参数化特性稀疏化网络，调整不重要滤波器以适应新任务。

Result: 在流行的持续学习视觉基准测试中，GESCL在分类准确率和避免灾难性遗忘方面显著优于现有最先进方法。

Conclusion: GESCL通过结合稳定性和可塑性正则化，在固定容量网络中有效解决了持续学习的灾难性遗忘问题，同时保持了计算效率。
...
TL;DR: AMIR-GRPO通过引入基于组内奖励排名的隐式对比正则化器，改进GRPO在复杂推理任务中的表现，解决长度偏差、低质量轨迹惩罚不足和标量目标信息损失等问题。


<details>
  <summary>Details</summary>
Motivation: GRPO在推理密集型任务中存在结构限制：序列级优势归一化引入系统性长度偏差，低质量轨迹惩罚被稀释，标量目标丢弃了组内奖励排名中的丰富成对偏好信息，导致昂贵的rollout监督未被充分利用。

Method: 提出AMIR-GRPO，通过从组内奖励排名直接构建隐式DPO风格对比正则化器来增强GRPO，无需额外标注。该机制放大对低奖励轨迹的抑制，减弱响应级长度偏差，并将每个rollout组转化为更密集的监督约束集。

Result: 在多个数学推理基准测试中，AMIR-GRPO始终优于强GRPO基线，在正确和错误推理链之间产生更清晰的分离，并在标准GRPO解决的实例子集之外提供更广泛的覆盖增益。

Conclusion: AMIR-GRPO通过利用组内奖励排名中的丰富成对偏好信息，有效解决了GRPO在推理任务中的结构限制，显著提升了监督效率和学习效果。
...
TL;DR: 提出SVEAD方法，通过随机Voronoi图分解数据空间，利用局部尺度加权的归一化单元相对距离进行异常检测，实现线性时间复杂度和常数空间复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法在处理具有变化局部密度的数据集时存在局限：基于距离的方法会漏检局部异常，而基于密度的方法需要仔细参数选择且具有二次时间复杂度。

Method: 提出SVEAD方法，构建集成随机Voronoi图，通过局部尺度加权的归一化单元相对距离对点进行评分，将数据空间分解为受限区域并独立检查每个区域。

Result: 在45个数据集上的实验表明，SVEAD优于12种最先进的方法，实现了线性时间复杂度和常数空间复杂度。

Conclusion: 通过几何洞察将数据空间分解为受限区域，SVEAD能够有效检测局部异常，克服了现有方法在处理变化局部密度数据集时的局限性。
...
### [55] [Disentangling Aleatoric and Epistemic Uncertainty in Physics-Informed Neural Networks. Application to Insulation Material Degradation Prognostics](https://arxiv.org/abs/2601.03673)
*Ibai Ramirez,Jokin Alcibar,Joel Pino,Mikel Sanz,Jose I. Aizpurua*

Main category: cs.LG

TL;DR: 提出了一种异方差贝叶斯物理信息神经网络（B-PINN）框架，用于变压器绝缘材料老化预测，能够同时建模认知不确定性和偶然不确定性，提供完整的预测后验分布。


<details>
  <summary>Details</summary>
Motivation: 传统物理信息神经网络（PINNs）在预测健康管理（PHM）中应用受限，主要因为其不确定性量化能力不足。现有PINN方法多为确定性或仅考虑认知不确定性，难以支持风险感知的决策制定。

Method: 提出异方差贝叶斯物理信息神经网络框架，将贝叶斯神经网络与基于物理的残差约束和先验分布相结合。在物理信息学习架构中实现概率推断，能够同时建模认知和偶然不确定性。

Result: 在变压器绝缘老化应用中，该方法相比确定性PINN、基于dropout的PINN和其他B-PINN变体，提供了更好的预测精度和校准的不确定性估计。通过系统敏感性研究分析了边界条件、初始条件和残差采样策略的影响。

Conclusion: 贝叶斯物理信息学习方法在支持变压器资产管理中的不确定性感知预测和知情决策方面具有潜力，能够提供更可靠的不确定性量化。
...
### [56] [Rethinking Recurrent Neural Networks for Time Series Forecasting: A Reinforced Recurrent Encoder with Prediction-Oriented Proximal Policy Optimization](https://arxiv.org/abs/2601.03683)
...
Conclusion: 该方法为工程信息学提供了先进的时间序列预测器，显著提高了RNN模型的时间序列建模能力和预测准确性
...
### [57] [A Pre-trained Reaction Embedding Descriptor Capturing Bond Transformation Patterns](https://arxiv.org/abs/2601.03689)
*Weiqi Liu,Fenglei Cao,Yuan Qi,Li-Cheng Xu*

Main category: cs.LG

TL;DR: RXNEmb是一种基于RXNGraphormer预训练模型的新型反应级描述符，能够学习化学键形成和断裂的内在模式，用于反应指纹分析和可视化


<details>
  <summary>Details</summary>
Motivation: 随着数据驱动的反应预测模型兴起，需要有效的反应描述符来连接真实化学与数字表示，但目前通用的反应级描述符仍然稀缺

Method: 开发RXNEmb描述符，源自RXNGraphormer模型，该模型通过区分真实反应与具有错误键变化的虚构反应进行预训练，从而学习内在的键形成和断裂模式

Result: RXNEmb能够对USPTO-50k数据集进行数据驱动的重新聚类，产生比基于规则分类更能反映键变化相似性的分类；结合降维技术可实现反应空间多样性的可视化；注意力权重分析显示模型关注化学关键位点

Conclusion: RXNEmb作为一种强大且可解释的反应指纹和分析工具，为反应分析和发现中更数据为中心的方法铺平了道路
...
### [58] [Inference Attacks Against Graph Generative Diffusion Models](https://arxiv.org/abs/2601.03701)
*Xiuling Wang,Xin Huang,Guibo Luo,Jianliang Xu*

Main category: cs.LG

TL;DR: 该论文研究了图生成扩散模型中的隐私风险，提出了三种黑盒推理攻击（图重构攻击、属性推理攻击、成员推理攻击），并在真实数据集上验证了攻击有效性，最后提出了防御机制。


<details>
  <summary>Details</summary>
Motivation: 图生成扩散模型在生成复杂图结构方面表现出色，但其隐私风险尚未得到充分探索。作者旨在研究这类模型中的信息泄露问题，揭示潜在的隐私威胁。
...
Result: 实验结果表明这些攻击非常有效，显著优于基线方法。攻击能够成功重构训练图、推断图属性并识别训练集成员，揭示了图生成扩散模型存在严重的隐私泄露风险。

Conclusion: 图生成扩散模型存在显著的隐私风险，提出的三种攻击方法有效揭示了信息泄露问题。同时提出了两种防御机制，在防御强度和模型效用之间取得了比现有方法更好的平衡。
...
### [59] [TreeAdv: Tree-Structured Advantage Redistribution for Group-Based RL](https://arxiv.org/abs/2601.03703)
*Lang Cao,Hui Ruan,Yongqian Li,Peng Chao,Wu Ning,Haonan Song,Renhong Chen,Yitong Li*

Main category: cs.LG

TL;DR: TreeAdv：一种基于树结构优势重分配的强化学习方法，用于提升语言模型在复杂推理任务上的样本效率和推理质量


<details>
  <summary>Details</summary>
Motivation: 传统的GRPO方法将每个轨迹视为独立的扁平序列，为所有token分配相同的序列级优势，这导致样本效率低下，并且会产生冗长冗余的思维链，而不会提高逻辑深度

Method: TreeAdv通过熵驱动的采样方法构建树结构（森林），在高不确定性决策处进行分支，同时在rollouts间共享低不确定性token。然后通过重新分配完整轨迹（所有叶节点）的优势来聚合内部树段的token级优势

Result: 在10个数学推理基准测试中，TreeAdv始终优于GRPO和GSPO，同时在相同的监督、数据和解码预算下使用更少的生成token

Conclusion: TreeAdv通过显式利用组rollouts的树结构进行探索和优势分配，显著提高了基于组的强化学习方法的效率和推理质量
...
### [60] [Investigating Knowledge Distillation Through Neural Networks for Protein Binding Affinity Prediction](https://arxiv.org/abs/2601.03704)
*Wajid Arshad Abbasi,Syed Ali Abbas,Maryum Bibi,Saiqa Andleeb,Muhammad Naveed Akhtar*

Main category: cs.LG

TL;DR: 使用知识蒸馏框架，通过结构信息教师网络指导序列学生网络，提升蛋白质-蛋白质结合亲和力预测性能，在仅有序列数据时接近结构模型的准确度。


<details>
  <summary>Details</summary>
Motivation: 蛋白质-蛋白质结合亲和力预测面临预测准确性与数据可用性之间的权衡。实验解析的蛋白质结构数据缺乏限制了结构基机器学习模型的性能，而序列基方法通常表现较差。需要一种方法能在训练时利用结构信息，但在推理时仅需序列数据。

Method: 提出基于知识蒸馏的回归框架：使用结构信息教师网络指导序列基学生网络的训练。训练时利用结合亲和力标签和中间特征表示进行联合监督，推理时仅需序列数据。在非冗余蛋白质-蛋白质结合亲和力基准数据集上使用Leave-One-Complex-Out交叉验证评估。

Result: 序列基基线模型：Pearson相关系数0.375，RMSE 2.712 kcal/mol；结构基模型：Pearson相关系数0.512，RMSE 2.445 kcal/mol；蒸馏基学生模型：Pearson相关系数0.481，RMSE 2.488 kcal/mol。学生模型显著提升序列基性能，接近结构模型表现。误差分析确认改进的一致性和偏差减少。

Conclusion: 知识蒸馏是向序列基预测器传递结构知识的有效方法，能够缩小序列基与结构基模型之间的性能差距。随着更大数据集的可用性，该方法有望进一步改进。代码已开源供推理使用。
...
### [61] [The Geometry of the Pivot: A Note on Lazy Pivoted Cholesky and Farthest Point Sampling](https://arxiv.org/abs/2601.03706)
*Gil Shabat*
...
Result: 提供了简洁的推导和极简的Python实现，弥合了理论与实践的差距，使Pivoted Cholesky分解在核方法中的几何意义更加清晰。

Conclusion: Pivoted Cholesky分解在核方法中具有明确的几何解释：枢轴选择对应最远点采样，Cholesky因子构造对应正交化过程，这有助于更好地理解和应用该算法。
...
### [62] [R$^3$L: Reflect-then-Retry Reinforcement Learning with Language-Guided Exploration, Pivotal Credit, and Positive Amplification](https://arxiv.org/abs/2601.03715)
*Weijie Shi,Yanxi Chen,Zexi Li,Xuchen Pan,Yuchang Sun,Jiajie Xu,Xiaofang Zhou,Yaliang Li*

Main category: cs.LG

TL;DR: R³L提出了一种新的强化学习方法，通过反思重试机制、关键信用分配和正信号放大，解决了LLM推理和智能体任务中的探索和利用问题。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习方法在LLM推理和智能体任务中存在两个主要问题：探索方面，困难任务成功率低且从头开始的重复rollout成本高；利用方面，轨迹级奖励惩罚了有效前缀，失败主导的样本淹没了正信号，导致训练不稳定。

Method: R³L包含三个核心技术：1）反思重试探索：使用语言反馈诊断错误，将失败尝试转化为成功，从失败点重启减少rollout成本；2）关键信用分配：只更新存在对比信号的分歧后缀，排除共享前缀；3）正信号放大：对成功轨迹进行加权，确保正信号指导优化过程。

Result: 在智能体和推理任务上的实验表明，R³L相比基线方法取得了5%到52%的相对改进，同时保持了训练稳定性。

Conclusion: R³L通过主动合成高质量轨迹、精确信用分配和正信号放大，有效解决了强化学习在LLM推理和智能体任务中的探索和利用难题，显著提升了性能并保持了训练稳定性。
...
### [63] [ETR: Outcome-Guided Elastic Trust Regions for Policy Optimization](https://arxiv.org/abs/2601.03723)
*Shijie Zhang,Kevin Zhang,Zheyuan Gu,Xiang Guo,Rujun Guo,Shaoyu Liu,Guanjun Jiang,Xiaozhao Wang*

Main category: cs.LG

TL;DR: 提出ETR（弹性信任区域）方法，通过动态调整信任区域约束来解决GRPO在RLVR中的结构限制，实现更好的信号利用和探索保持。


<details>
  <summary>Details</summary>
Motivation: 当前RLVR领域主导算法GRPO存在结构限制：采用统一、静态的信任区域约束，这假设信号同质性，与结果驱动学习中优势幅度和方差显著波动的异质性现实不符。静态约束无法充分利用高质量信号，同时抑制噪声不足，常导致快速熵崩溃。

Method: 提出ETR（弹性信任区域）方法，通过双层次弹性构建信号感知的优化景观：微观层面基于优势幅度缩放裁剪边界以加速高置信度路径学习；宏观层面利用组方差隐式分配更大更新预算给处于最优学习区域的任务。

Result: 在AIME和MATH基准测试上的广泛实验表明，ETR持续优于GRPO，实现更高的准确性，同时有效缓解策略熵退化以确保持续探索。

Conclusion: ETR通过动态信任区域约束解决了GRPO的结构限制，更好地适应了RLVR中信号异质性的特点，在保持稳定训练的同时实现了更优的性能和探索能力。
...
### [64] [EDCO: Dynamic Curriculum Orchestration for Domain-specific Large Language Model Fine-tuning](https://arxiv.org/abs/2601.03725)
...
TL;DR: EDCO提出了一种基于推理熵和动态课程编排的LLM微调框架，通过优先选择高推理熵样本并动态调整课程，在多个领域超越传统静态课程学习方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM微调大多采用静态课程学习，缺乏对训练过程中模型需求变化的适应性。需要一种能够动态调整训练样本顺序的方法来提高微调效果。

Method: EDCO框架包含三个核心组件：1) 使用前缀token近似全序列熵的高效熵估计器；2) 基于熵的课程生成器，选择推理熵最高的数据点；3) 在选定课程上优化模型的LLM训练器。框架动态调整课程以适应模型演化需求。

Result: 在通信、医学和法律领域的实验中，EDCO在监督学习和强化学习设置下，对Qwen3-4B和Llama3.2-3B模型的微调效果均优于传统课程策略。高效熵估计将计算时间减少了83.5%，同时保持高精度。

Conclusion: EDCO通过动态课程编排和推理熵优先策略，有效解决了静态课程学习的局限性，为领域特定LLM的微调提供了更高效、适应性更强的方法。
...
### [65] [Probabilistic Transformers for Joint Modeling of Global Weather Dynamics and Decision-Centric Variables](https://arxiv.org/abs/2601.03753)
*Paulius Rauba,Viktor Cikojevic,Fran Bartolic,Sam Levang,Ty Dickinson,Chase Dwelle*

Main category: cs.LG

TL;DR: GEM-2是一个概率Transformer模型，直接学习全球大气动力学和用户决策相关变量，以轻量级架构（2.75亿参数）在天气预测中超越传统数值天气预报模型，并具有计算效率和决策价值优势。


<details>
  <summary>Details</summary>
Motivation: 传统天气预报用户面临困难权衡：许多决策相关目标（如极值、累积量、阈值超限）是大气状态变量的函数，而非状态变量本身。用户需要通过后处理估计这些目标，这可能导致次优结果和结构偏差。核心问题是决策依赖于这些函数分布的模型并未直接学习。

Method: 引入GEM-2概率Transformer，联合学习全球大气动力学和用户直接操作的变量套件。采用CRPS（连续排名概率得分）目标训练，轻量级架构（约2.75亿参数），计算效率高（相比最先进模型有20-100倍训练加速）。
...
Conclusion: GEM-2通过直接学习决策相关变量，提供了一种高效、准确的天气预报方法，解决了传统后处理方法的局限性，在计算效率和预测性能方面都有显著优势。
...
### [66] [Improving Compactness and Reducing Ambiguity of CFIRE Rule-Based Explanations](https://arxiv.org/abs/2601.03776)
*Sebastian Müller,Tobias Schneider,Ruben Kemna,Vanessa Toborek*

Main category: cs.LG

TL;DR: CFIRE算法在构建局部解释的紧凑代理规则模型时存在规则冲突问题，作者提出后剪枝策略消除低贡献或冲突覆盖的规则，获得更小、更清晰的模型
...
Result: 在多个数据集上的实验证实，该方法能显著减小模型规模、降低模糊性，对预测性能影响最小

Conclusion: 通过后剪枝策略改进CFIRE算法，可以获得更紧凑、更清晰的解释模型，同时保持预测性能，满足敏感领域对透明度的需求
...
### [67] [Prompt Tuning without Labeled Samples for Zero-Shot Node Classification in Text-Attributed Graphs](https://arxiv.org/abs/2601.03793)
*Sethupathy Parameswaran,Suresh Sundaram,Yuan Fang*

Main category: cs.LG

TL;DR: 提出ZPT框架，通过通用双模态条件生成器实现零样本节点分类，无需标注数据即可生成合成样本进行提示调优


<details>
  <summary>Details</summary>
Motivation: 文本属性图(TAGs)中的零样本节点分类具有挑战性，因为缺乏标注数据，但实际应用广泛（如社交网络社区检测、文章分组、电商产品分类）

Method: 1) 预训练图-语言模型捕获图结构和节点文本描述；2) 训练条件生成模型学习图-文本双模态联合分布；3) 仅基于类别名称生成合成节点和文本嵌入；4) 使用合成样本进行连续提示调优

Result: 在多个基准数据集上的实验表明，该框架优于现有最先进的基线方法，消融研究验证了双模态生成器的贡献

Conclusion: 提出的ZPT框架通过生成合成样本和提示调优，有效解决了文本属性图中的零样本节点分类问题，代码已开源
...
### [68] [Quantum vs. Classical Machine Learning: A Benchmark Study for Financial Prediction](https://arxiv.org/abs/2601.03802)
*Rehan Ahmad,Muhammad Kashif,Nouhaila Innan,Muhammad Shafique*

Main category: cs.LG

TL;DR: 该论文提出了一个可复现的基准测试框架，系统比较量子机器学习模型与架构匹配的经典模型在三个金融任务上的表现，发现量子方法在数据结构和电路设计良好对齐时能超越经典方法。


<details>
  <summary>Details</summary>
Motivation: 当前量子机器学习在金融领域的应用缺乏系统性的基准测试，需要公平评估当前代QML模型何时能匹配或超越经典方法，明确识别量子方法适用的场景。

Method: 建立可复现的基准测试框架，标准化数据分割、特征和评估指标，在三个金融任务上比较QML与经典模型：1) 美土股票方向性收益预测；2) S&P 500实时交易模拟；3) 已实现波动率预测。

Result: 量子方法在数据结构和电路设计对齐时表现更优：方向性分类中，混合量子神经网络在AAPL和KCHOL上分别提升3.8/3.4和4.9/3.6个AUC/准确率点；实时交易中，QLSTM在4个S&P 500制度中的2个获得更高风险调整收益；波动率预测中，角度编码QSVR在KCHOL上获得最低QLIKE。

Conclusion: 该基准框架清晰识别了当前QML架构能提供实质性改进的场景，以及经典方法仍占主导的领域，为量子金融应用提供了实用指导。
...
### [69] [Detecting Semantic Backdoors in a Mystery Shopping Scenario](https://arxiv.org/abs/2601.03805)
*Arpad Berta,Gabor Danner,Istvan Hegedus,Mark Jelasity*

Main category: cs.LG

TL;DR: 提出了一种检测语义后门的方法，通过创建参考模型池并校准模型距离阈值来识别干净模型，在已知训练数据和训练方法的前提下有效检测语义后门攻击。


<details>
  <summary>Details</summary>
Motivation: 语义后门检测是一个重要但研究较少的问题，相比基于触发模式的后门更难检测。研究动机来源于消费者保护场景，监管机构需要检测机器学习服务提供商是否在模型中植入了后门。

Method: 在已知干净训练数据和训练方法的前提下，创建参考模型池（包含少量干净和中毒模型），校准模型距离阈值。提出多种模型距离计算方法，最佳方法基于请求对抗训练，通过反转模型生成最大化与干净样本距离的输入样本来测量模型距离。

Result: 该方法通常能完全分离干净和中毒模型，在检测语义后门方面优于现有最先进的后门检测器，即使在提供者进行自适应攻击避免检测的场景下也有效。

Conclusion: 提出的方法在消费者保护场景下能有效检测语义后门攻击，特别是结合对抗训练和模型反转技术时效果最佳，为监管机构提供了实用的检测工具。
...
### [70] [Logic Tensor Network-Enhanced Generative Adversarial Network](https://arxiv.org/abs/2601.03839)
*Nijesh Upreti,Vaishak Belle*

Main category: cs.LG

TL;DR: LTN-GAN结合逻辑张量网络和生成对抗网络，通过逻辑约束增强样本生成，在保持生成质量的同时提升逻辑一致性


<details>
  <summary>Details</summary>
Motivation: 传统GAN缺乏融入先验知识和强制执行逻辑一致性的机制，限制了其在需要遵守规则的领域中的应用

Method: 将逻辑张量网络（LTN）与GAN结合，利用LTN将一阶逻辑与神经网络集成，在生成过程中强制执行领域特定的逻辑约束

Result: 在合成数据集（高斯、网格、环）和MNIST数据集上的评估显示，LTN-GAN在遵守预定义逻辑约束方面显著优于传统GAN，同时保持生成样本的质量和多样性

Conclusion: 这项工作突显了神经符号方法在知识密集型领域中增强生成建模的潜力
...
### [71] [Feature-Aware One-Shot Federated Learning via Hierarchical Token Sequences](https://arxiv.org/abs/2601.03882)
...
<details>
  <summary>Details</summary>
Motivation: 现有单次联邦学习方法在真实世界领域（如医学影像）性能不佳，处理非IID数据效率低下，需要更鲁棒高效的解决方案。

Method: 1) 使用预训练视觉编码器将图像压缩为分层token序列；2) 多尺度自回归transformer生成器建模token分布；3) 客户端上传合成序列和本地分类器；4) 服务器端通过知识蒸馏进行全局训练。

Result: 在医学和自然图像数据集上验证，在多种非IID场景中平均准确率比最佳基线方法提升9.58%。
...
### [72] [Spectral Manifold Regularization for Stable and Modular Routing in Deep MoE Architectures](https://arxiv.org/abs/2601.03889)
*Ibrahim Delibasoglu*

Main category: cs.LG

TL;DR: SR-MoE通过谱正则化约束路由流形几何结构，解决MoE架构中的专家坍缩问题，保持模型结构模块化，实现稳定的终身学习。


<details>
  <summary>Details</summary>
Motivation: 混合专家架构虽然能高效扩展神经网络，但存在专家坍缩问题，路由会收敛到少数主导专家，这降低了模型容量并在适应过程中导致灾难性干扰。

Method: 提出谱正则化混合专家方法，通过双重正则化施加几何约束：谱范数约束限制路由函数的Lipschitz连续性，稳定秩惩罚保持专家选择中的高维特征多样性。

Result: 传统线性门控随着深度增加导致专家纠缠（准确率下降达4.72%），而SR-MoE保持结构完整性（平均干扰-0.32%），谱约束促进正向知识转移，实现局部专家更新而不影响全局性能。

Conclusion: SR-MoE为构建高容量、模块化网络提供了通用解决方案，能够实现稳定的终身学习，通过几何约束路由流形来强制结构模块化。
...
### [73] [Adaptive-Boundary-Clipping GRPO: Ensuring Bounded Ratios for Stable and Generalizable Training](https://arxiv.org/abs/2601.03895)
*Chi Liu,Xin Chen*

Main category: cs.LG

TL;DR: 提出ABC-GRPO算法，改进GRPO的裁剪机制，在数学推理任务上表现更优，保持更高熵值防止过早收敛。


<details>
  <summary>Details</summary>
Motivation: 分析发现GRPO的裁剪机制在某些场景下不够理想，通过适当改进可以显著提升算法的灵活性和泛化能力。

Method: 提出自适应边界裁剪GRPO（ABC-GRPO），采用非对称和自适应的方式改进原始GRPO框架的裁剪机制。

Result: 在Qwen3大语言模型的数学推理任务上，ABC-GRPO相比标准GRPO获得更优性能，训练过程中保持显著更高的熵值，保护模型探索能力并缓解过早收敛。

Conclusion: ABC-GRPO通过改进GRPO的裁剪机制，在保持探索能力的同时提升了性能，代码已开源便于复现。
...
### [74] [FOREVER: Forgetting Curve-Inspired Memory Replay for Language Model Continual Learning](https://arxiv.org/abs/2601.03938)
*Yujie Feng,Hao Wang,Jian Li,Xu Chu,Zhaolu Kang,Yiran Liu,Yasha Wang,Philip S. Yu,Xiao-Ming Wu*

Main category: cs.LG

TL;DR: FOREVER提出了一种基于遗忘曲线的持续学习框架，通过模型时间而非训练步数来安排记忆回放，有效缓解大语言模型的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法大多依赖固定的步数启发式策略，但这些策略与模型实际学习进度常常不匹配，因为相同的训练步数可能导致不同程度的参数变化。受人类遗忘曲线启发，作者希望开发更符合模型内部演化规律的记忆回放机制。

Method: FOREVER框架包含三个核心组件：1) 基于优化器更新幅度定义"模型时间"，替代传统的训练步数；2) 遗忘曲线启发的回放调度器，决定何时进行回放；3) 强度感知的正则化机制，自适应控制如何回放。这种方法将回放间隔与模型内部演化对齐。

Result: 在三个持续学习基准测试和0.6B到13B参数范围的模型上进行了广泛实验，结果表明FOREVER能够持续有效地缓解灾难性遗忘问题。

Conclusion: FOREVER通过将遗忘曲线与模型中心时间概念相结合，提供了一种更符合大语言模型学习动态的持续学习框架，相比传统步数启发式方法能更好地保持先前学到的知识。
...
### [75] [Stage-specific cancer survival prediction enriched by explainable machine learning](https://arxiv.org/abs/2601.03977)
*Parisa Poorhasani,Bogdan Iancu*

Main category: cs.LG

TL;DR: 该研究开发了可解释的机器学习模型，用于预测结直肠癌、胃癌和肝癌的分期特异性生存率，使用SHAP和LIME技术揭示特征与癌症分期之间的相互作用。


<details>
  <summary>Details</summary>
Motivation: 传统生存预测模型通常将所有癌症分期样本合并训练，可能高估性能并忽略分期特异性差异。现有机器学习生存分析研究缺乏可解释性和透明度。

Method: 使用SEER数据集，构建可解释的机器学习模型预测分期特异性癌症生存率，应用SHAP和LIME技术进行特征重要性分析和模型解释。

Result: 识别了人口统计学和临床变量在不同癌症分期和类型中对生存率的不同影响，揭示了传统黑盒模型中隐藏的特征-分期相互作用。

Conclusion: 分期特异性模型提供了对每个癌症阶段最重要因素的新见解，增强了模型透明度和临床相关性，支持个性化治疗规划。
...
### [76] [Modeling Behavioral Patterns in News Recommendations Using Fuzzy Neural Networks](https://arxiv.org/abs/2601.04019)
*Kevin Innerebner,Stephan Bartl,Markus Reiter-Haas,Elisabeth Lex*

Main category: cs.LG

TL;DR: 使用模糊神经网络从行为数据中学习可读规则来预测新闻点击，实现透明推荐系统


<details>
  <summary>Details</summary>
Motivation: 当前新闻推荐系统多为黑盒模型，缺乏透明度，难以支持编辑决策制定

Method: 采用模糊神经网络从行为数据中学习可读规则，通过可配置阈值控制规则复杂度，在MIND和EB-NeRD数据集上评估

Result: 相比多个基线模型，能准确预测点击行为，同时学习到人类可读的规则，揭示新闻消费模式

Conclusion: 透明推荐系统能让编辑将内容策划目标与目标受众行为对齐，实现可解释的新闻推荐
...
### [77] [Symbolic Regression for Shared Expressions: Introducing Partial Parameter Sharing](https://arxiv.org/abs/2601.04051)
*Viktor Martinek,Roland Herzog*

Main category: cs.LG

TL;DR: 本文提出了一种改进的符号回归方法，支持多个分类变量和中间层次的参数共享，减少了参数数量并增强了问题理解。


<details>
  <summary>Details</summary>
Motivation: 现有符号回归方法在处理分类变量时存在局限性：要么只允许"非共享"参数（特定于类别值），要么只包含"共享"参数（类别无关）。本文旨在扩展这些方法，考虑多个分类变量并引入中间层次的参数共享，以降低参数数量并揭示更多问题信息。

Method: 提出了一种新的符号回归框架，支持多个分类变量。特别地，在两个分类变量的情况下，引入了中间层次的参数共享机制：某些参数在一个类别中共享但在另一个类别中变化。通过合成数据集测试了该方法在减少数据需求和迁移学习方面的能力，并在天体物理学数据集上进行了实际应用验证。

Result: 在合成数据集上，该方法显著减少了数据需求并展示了良好的迁移学习能力。在天体物理学数据集上，与先前只考虑单个分类变量的研究相比，本文方法获得了相似的拟合质量，但需要显著更少的参数，并提取了更多关于问题的信息。

Conclusion: 提出的多分类变量符号回归方法通过引入中间层次的参数共享，不仅减少了参数数量，还增强了模型的可解释性和问题理解能力，为科学发现提供了更强大的工具。
...
### [78] [LinkD: AutoRegressive Diffusion Model for Mechanical Linkage Synthesis](https://arxiv.org/abs/2601.04054)
*Yayati Jadhav,Amir Barati Farimani*

Main category: cs.LG

TL;DR: 提出一种基于自回归扩散框架的机械连杆机构逆设计方法，通过将机构表示为序列构建的图，结合因果Transformer和去噪扩散概率模型，实现从目标轨迹到连杆机构拓扑和几何参数的自动生成。


<details>
  <summary>Details</summary>
Motivation: 机械连杆机构设计面临连续节点位置、离散拓扑配置和非线性运动学约束的复杂耦合问题。传统优化和启发式方法在组合爆炸的设计空间中计算不可行，需要新的可扩展逆设计方法。

Method: 采用自回归扩散框架：1) 将机构表示为序列构建的图（节点对应关节，边对应刚性连杆）；2) 因果Transformer自回归预测离散拓扑；3) DDPM细化每个节点的空间坐标和边连接；4) 支持自适应试错合成，可选择性重新生成有问题的节点。
...
### [79] [Using Legacy Polysomnography Data to Train a Radar System to Quantify Sleep in Older Adults and People living with Dementia](https://arxiv.org/abs/2601.04057)
...
<details>
  <summary>Details</summary>
Motivation: UWB雷达作为非侵入式睡眠监测工具具有前景，但雷达睡眠数据有限，难以构建跨人群和环境的鲁棒模型

Method: 开发端到端神经网络，结合大规模PSG数据集和雷达数据，采用对抗学习的域适应方法弥合PSG与雷达信号间的知识差距

Result: 在47名老年人（含18名阿尔茨海默病前期或轻度患者）的雷达数据集上，实现79.5%准确率和0.65 Kappa值，分类清醒、REM、浅睡和深睡四阶段

Conclusion: 该方法有效解决了数据变异性和样本量有限的问题，显著提升了自动睡眠分期的可靠性，特别适用于雷达数据有限的情况
...
### [80] [Minimum distance classification for nonlinear dynamical systems](https://arxiv.org/abs/2601.04058)
*Dominique Martinez*

Main category: cs.LG

TL;DR: Dynafit：一种基于核方法的度量学习框架，用于分类由不同非线性动力系统生成的轨迹数据


<details>
  <summary>Details</summary>
Motivation: 解决非线性动力系统生成的轨迹数据分类问题，其中每个类别对应不同的动力系统。传统方法难以直接处理非线性动力学，需要一种能够学习轨迹与底层动力学之间距离度量的方法。

Method: 提出Dynafit方法：1）基于核方法学习训练轨迹与底层动力学之间的距离度量；2）通过近似Koopman算子在核特征空间中全局线性化动力学；3）利用核技巧在特征空间中计算距离度量，不受维度限制；4）可定制核函数以融入对动力学的先验知识。

Result: Dynafit适用于各种涉及非线性动力系统和传感器的分类任务。在三个示例中展示了有效性：1）逻辑映射的混沌检测；2）手写动力学识别；3）视觉动态纹理识别。

Conclusion: Dynafit为非线性动力系统轨迹分类提供了一种有效的核方法框架，能够学习动力学相似性度量，并可通过定制核函数融入领域知识，在多种实际应用中表现出良好性能。
...
### [81] [Causal Data Augmentation for Robust Fine-Tuning of Tabular Foundation Models](https://arxiv.org/abs/2601.04110)
...
<details>
  <summary>Details</summary>
Motivation: 表格基础模型在数据稀缺情况下微调困难，早期停止在稀缺验证数据上难以捕捉真实泛化性能，需要更鲁棒的微调方法

Method: 提出CausalMixFT方法，通过在目标数据集上拟合结构因果模型生成结构一致的合成样本，用因果信息增强有限真实数据，保持特征依赖关系的同时扩展训练多样性

Result: 在33个分类数据集和2300多次微调实验中，CausalMixFT将标准化ROC-AUC中位数从0.10提升到0.12，优于CTGAN、TabEBM和TableAugment等统计生成方法，并将验证-测试性能相关性差距从0.67缩小到0.30
...
### [82] [MORPHFED: Federated Learning for Cross-institutional Blood Morphology Analysis](https://arxiv.org/abs/2601.04121)
...
<details>
  <summary>Details</summary>
Motivation: 在低收入和中等收入国家，自动化血液形态分析可支持血液学诊断，但对染色变异、成像差异和罕见形态的数据集偏移敏感。由于隐私法规和数据共享限制，建立集中式数据集捕捉这种多样性通常不可行。

Method: 引入联邦学习框架，使多个临床机构能够在不交换训练数据的情况下进行协作训练。使用来自多个临床站点的血液涂片，联邦模型学习鲁棒的、领域不变的表示，同时保持完整的数据隐私。
...
### [83] [Clinical Data Goes MEDS? Let's OWL make sense of it](https://arxiv.org/abs/2601.04164)
*Alberto Marfoglia,Jong Ho Jhee,Adrien Coulet*

Main category: cs.LG

TL;DR: MEDS-OWL：一个轻量级OWL本体，将医疗事件数据标准（MEDS）与语义网生态系统连接，支持将MEDS数据集转换为RDF图，实现FAIR数据发布和基于图的临床数据分析。
...
Method: 开发MEDS-OWL本体（13个类、10个对象属性、20个数据属性、24个OWL公理），并实现meds2rdf Python转换库，将MEDS事件转换为符合本体的RDF图。使用颅内动脉瘤破裂患者护理路径的合成临床数据集进行演示，并通过SHACL约束验证结果图。

Result: 成功创建了MEDS-OWL本体和meds2rdf转换工具，能够将MEDS数据转换为FAIR对齐的数据集，支持溯源感知的数据发布和事件临床数据的互操作性。
...
### [84] [Agentic Rubrics as Contextual Verifiers for SWE Agents](https://arxiv.org/abs/2601.04171)
...
Result: 在SWE-Bench Verified的并行TTS评估中，Agentic Rubrics在Qwen3-Coder-30B-A3B上达到54.2%，在Qwen3-32B上达到40.6%，比最强基线至少提升3.5个百分点。评分与真实测试结果一致，还能发现测试未捕获的问题。

Conclusion: Agentic Rubrics为软件工程代理提供了高效、可扩展且细粒度的验证信号，其中代理式上下文收集对于生成代码库特定、明确的评估标准至关重要。
...
### [85] [Robust Physics Discovery from Highly Corrupted Data: A PINN Framework Applied to the Nonlinear Schrödinger Equation](https://arxiv.org/abs/2601.04176)
*Pietro de Oliveira Esteves*

Main category: cs.LG

TL;DR: 基于物理信息神经网络（PINNs）的深度学习框架，能够在严重噪声条件下从非线性薛定谔方程（NLSE）中恢复物理参数，仅用500个稀疏随机采样点即可实现非线性系数β的重建，相对误差小于0.2%。


<details>
  <summary>Details</summary>
Motivation: 传统有限差分方法在噪声环境下因数值导数噪声放大而失效，特别是在实验数据稀缺且噪声严重的时空动力学逆问题中，需要一种能够有效处理高测量不确定性的方法。

Method: 将物理信息神经网络（PINNs）与自动微分相结合，通过物理约束正则化来对抗噪声，使用稀疏随机采样数据点（500个）和20%加性高斯噪声进行训练。

Result: 在β=0.5-2.0范围内和100-1000个训练点条件下，均保持低于1%的准确度；β=1.0时标准差小于0.15%；在NVIDIA Tesla T4上约80分钟完成计算。

Conclusion: 物理正则化能有效过滤高测量不确定性，PINNs可作为传统优化方法的可行替代方案，特别适用于数据稀缺且噪声严重的时空动力学逆问题。
...
### [86] [Lightweight Test-Time Adaptation for EMG-Based Gesture Recognition](https://arxiv.org/abs/2601.04181)
*Nia Touko,Matthew O A Ellis,Cristiano Capone,Alessio Burrello,Elisa Donati,Luca Manneschi*

Main category: cs.LG

TL;DR: 提出一个轻量级测试时适应框架，使用TCN骨干网络，通过因果自适应批归一化、GMM对齐与经验回放、元学习三种策略，显著提升表面肌电信号跨会话解码的稳定性，实现长期可靠的肌电控制。


<details>
  <summary>Details</summary>
Motivation: 表面肌电信号的长期可靠解码面临信号漂移问题（电极移位、肌肉疲劳、姿势变化），现有模型在会话内准确率高但跨会话性能急剧下降，且现有解决方案通常需要大数据集或高计算量，不适合能效型可穿戴设备。

Method: 提出轻量级测试时适应框架，采用TCN骨干网络，包含三种部署就绪策略：1）因果自适应批归一化用于实时统计对齐；2）高斯混合模型对齐与经验回放防止遗忘；3）元学习用于快速少样本校准。

Result: 在NinaPro DB6多会话数据集上评估，该框架显著缩小了跨会话准确率差距且开销最小。经验回放更新在有限数据下提供优越稳定性，元学习在一/二样本机制下仅需当前基准数据的一小部分就能达到竞争性性能。

Conclusion: 这项工作为长期假肢使用建立了通向稳健、"即插即用"肌电控制的路径，轻量级测试时适应框架能有效解决表面肌电信号漂移问题，实现可靠的长期解码。
...
### [87] [On the Identifiability of Regime-Switching Models with Multi-Lag Dependencies](https://arxiv.org/abs/2601.03325)
*Carles Balsells-Rodas,Toshiko Matsui,Pedro A. M. Mediano,Yixin Wang,Yingzhen Li*

Main category: stat.ML

TL;DR: 该论文为深度隐变量模型中的可识别性提供了理论框架，特别针对多滞后机制切换时间序列模型，证明了在无监督设置下机制数量和转移参数的唯一可识别性。


<details>
  <summary>Details</summary>
Motivation: 可识别性是深度隐变量模型可解释性的核心，确保参数化由数据生成分布唯一确定。然而，对于深度机制切换时间序列模型，可识别性研究仍然不足，这限制了这些模型在科学发现中的可信度。

Method: 开发了一个多滞后机制切换模型（RSMs）的通用理论框架，涵盖马尔可夫切换模型（MSMs）和切换动态系统（SDSs）。对于MSMs，将模型构建为时间结构化的有限混合模型；对于SDSs，通过时间结构建立隐变量的可识别性。通过神经网络架构和噪声假设在无监督设置中实现可识别性。

Result: 证明了MSMs中机制数量和多滞后转移的非线性高斯设置下的可识别性；建立了SDSs中隐变量在置换和缩放下的可识别性，从而得到机制依赖的隐因果图（在机制/节点置换下）的可识别性条件。开发了满足假设的灵活变分估计器，并在合成基准测试和神经科学、金融、气候等真实数据集上验证了结果。

Conclusion: 该研究为深度机制切换时间序列模型提供了可识别性理论框架，使得在完全无监督设置下能够获得可信的可解释性分析，这对科学发现至关重要。可识别性确保了模型参数化的唯一性，增强了机制切换模型在真实世界应用中的可信度。
...
### [88] [Microeconomic Foundations of Multi-Agent Learning](https://arxiv.org/abs/2601.03451)
*Nassim Helou*

Main category: stat.ML

TL;DR: 论文提出一种两阶段激励机制，用于解决多智能体学习中的策略外部性问题，通过估计可实施的转移支付来引导长期动态，在温和的遗憾理性条件下实现次线性社会福利遗憾和渐进最优福利。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统越来越多地在市场和制度中运行，其中数据、行为和激励都是内生的。需要为多智能体学习建立经济基础，解决主-代理交互中的策略外部性问题，确保AI在市场和保险中的安全和福利对齐设计。

Method: 提出两阶段激励机制：第一阶段估计可实施的转移支付，第二阶段使用这些转移支付来引导长期动态。在马尔可夫决策过程中考虑策略外部性，主-代理双方随时间学习。机制基于遗憾理性假设和探索条件。

Result: 在温和的遗憾理性假设和探索条件下，该机制实现了次线性社会福利遗憾，从而获得渐进最优福利。仿真表明，即使粗略的激励也能纠正状态外部性下的低效学习。

Conclusion: 激励机制设计对于市场和保险中安全和福利对齐的AI至关重要。该研究为多智能体学习提供了经济基础，展示了如何通过激励设计纠正低效学习，实现渐进最优的社会福利。
...
### [89] [Online Learning with Limited Information in the Sliding Window Model](https://arxiv.org/abs/2601.03533)
*Vladimir Braverman,Sumegha Garg,Chen Wang,David P. Woodruff,Samson Zhou*

Main category: stat.ML

TL;DR: 论文研究了滑动窗口模型中的专家问题，使用2个查询和polylog内存实现了接近最优的遗憾界，并将技术扩展到数据流中的bandit问题。


<details>
  <summary>Details</summary>
Motivation: 受流模型中专家问题研究的启发，本文考虑滑动窗口模型中的专家问题。滑动窗口模型适用于流量监控、疫情追踪、自动交易等应用，其中近期信息比旧数据更有价值。

Method: 提出了在滑动窗口模型中处理专家问题的算法，使用2个查询和polylog(nT)位内存，实现了接近最优的遗憾界。技术扩展到数据流中的bandit问题（q=1）。

Result: 使用2个查询和polylog(nT)内存实现了√(nW)polylog(nT)的遗憾界；对于任意区间I实现了√(n|I|)polylog(nT)遗憾；在bandit设置中实现了nT^{2/3}polylog(T)遗憾；如果最佳专家的损失是随机顺序的，可以进一步改进到最优的O(√(nT))遗憾。

Conclusion: 本文在滑动窗口模型中解决了专家问题，实现了查询和内存效率的平衡，并将技术扩展到数据流中的bandit问题，为流计算环境中的在线学习提供了有效的解决方案。
...
### [90] [A Theoretical and Empirical Taxonomy of Imbalance in Binary Classification](https://arxiv.org/abs/2601.04149)
*Rose Yvette Bandolo Essomba,Ernest Fokoué*

Main category: stat.ML

TL;DR: 提出基于三个基本尺度（不平衡系数η、样本-维度比κ、内在可分性Δ）的理论框架，分析类别不平衡对分类性能的影响，预测四种恶化机制。


<details>
  <summary>Details</summary>
Motivation: 类别不平衡显著降低分类性能，但缺乏统一的理论分析视角。需要从几何角度理解不平衡如何影响分类边界和性能指标。

Method: 从高斯贝叶斯分类器出发，推导闭式贝叶斯误差，分析不平衡如何移动判别边界。提出基于(η,κ,Δ)三参数的理论框架，预测四种恶化机制：正常、轻度、极端、灾难性。

Result: 在平衡的高维基因组数据上，仅改变η而固定κ和Δ，经验性能恶化与理论预测一致：少数类召回率在log(η)超过Δ√κ时崩溃，精确率非对称增加，F1分数和PR-AUC下降符合预测机制。

Conclusion: 三参数(η,κ,Δ)提供了模型无关、几何基础的解释框架，能预测类别不平衡导致的性能恶化机制，为理解和缓解不平衡问题提供理论指导。
