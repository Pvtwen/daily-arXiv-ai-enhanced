...
### [1] [Interference Detection and Exploitation for Multi-User Radar Sensing](https://arxiv.org/abs/2601.21429)
*Laurits Randers,Martin Voigt Vejling,Petar Popovski*

Main category: eess.SP

TL;DR: 提出一种用于频谱交织OFDM的干扰检测与利用算法，通过统计严谨的方法检测干扰并控制族错误率，同时利用干扰估计角度、避开干扰估计时延


<details>
  <summary>Details</summary>
Motivation: 下一代无线网络中，多用户场景下同时传输会在重叠频率上产生相互干扰，导致虚假目标检测和感知精度下降，需要解决集成感知与通信中的干扰问题

Method: 提出干扰检测与利用算法，采用统计严谨的程序检测干扰并控制族错误率，算法利用干扰估计角度，同时避开干扰估计时延

Result: 数值实验表明，所提方法能可靠检测干扰，且时延和角度估计误差接近克拉美-罗下界

Conclusion: 提出的算法有效解决了多用户集成感知与通信中的干扰问题，实现了可靠的干扰检测和接近理论极限的感知性能
...
### [2] [A Time-Domain Dual-Edge Asynchronous Pipelined SAR ADC Featuring Reset-Free Quantization at Multi-GS/s](https://arxiv.org/abs/2601.21308)
*Richard Zeng,Anthony Chan Carusone,Xilin Liu*

Main category: eess.SP

TL;DR: 提出一种双沿无复位量化概念，用于异步流水线SAR时域ADC，通过利用上升和下降信号沿实现单周期内无复位量化，消除复位死区时间，提高高速ADC的性能。


<details>
  <summary>Details</summary>
Motivation: 传统时域ADC需要在采样间进行显式复位，引入死区时间，限制了分辨率、速度和能效。在高采样率下，复位时间成为性能瓶颈，需要新的架构来克服这一限制。

Method: 采用双沿无复位量化概念，在异步流水线SAR时域ADC中同时利用上升和下降信号沿进行量化。包含线性度补偿的双沿电压-时间转换器和具有独立可调上升/下降沿延迟的双沿时间-数字转换器。

Result: 在22nm FD-SOI工艺中实现8位ADC，核心面积0.0089mm²。连续单通道工作3.5GS/s，间歇工作可达10.5GS/s以上。在3.5GS/s时，SNDR为21.6dB，SFDR为32.2dB。

Conclusion: 双沿无复位量化概念可行，性能主要受限于实现层面因素而非架构限制，为高速时域ADC提供了有前景的解决方案。
...
### [3] [Compressed Sensing-Driven Near-Field Localization Exploiting Array of Subarrays](https://arxiv.org/abs/2601.21481)
*Sai Pavan Deram,Jacopo Pegoraro,Javier Lorca Hernando,Jesus O. Lacruz,Joerg Widmer*

Main category: eess.SP

TL;DR: SHARE是一种用于近场ISAC定位的两阶段稀疏恢复算法，通过分层处理解决稀疏子阵列架构中的栅瓣模糊问题，在降低计算复杂度的同时保持大孔径的高分辨率。


<details>
  <summary>Details</summary>
Motivation: 近场ISAC定位需要大孔径阵列，但全数字实现成本高且复杂。稀疏子阵列架构虽能降低成本，但会引入严重的栅瓣模糊问题。

Method: SHARE采用两阶段方法：第一阶段使用单个子阵列进行粗略、无模糊的角度估计以解决栅瓣模糊；第二阶段利用整个稀疏孔径进行局部化的联合角度-距离搜索。

Result: SHARE在定位精度和鲁棒性上显著优于传统的一阶段稀疏恢复方法（如OMP），其整体定位精度与全数字2D-MUSIC算法相当甚至更好，尽管MUSIC使用了完整的未压缩数据。

Conclusion: SHARE为高分辨率近场ISAC系统提供了一条实用路径，在保持大孔径高分辨率的同时解决了稀疏阵列的栅瓣模糊问题。
...
### [4] [Channel Extrapolation for MIMO Systems with the Assistance of Multi-path Information Induced from Channel State Information](https://arxiv.org/abs/2601.21524)
*Yuan Gao,Xinyi Wu,Jiang Jun,Zitian Zhang,Zhaohui Yang,Shugong Xu,Cheng-Xiang Wang,Zhu Han*

Main category: eess.SP

TL;DR: 提出基于CSI多径特性的信道外推框架，通过CSI-to-PDP模块提取功率延迟谱特征，结合MAE架构实现自监督学习，显著提升外推性能


<details>
  <summary>Details</summary>
Motivation: 6G网络中传统信道估计开销大，现有基于视觉/雷达等环境信息的方法存在硬件、隐私和多模态对齐问题，需要不依赖额外模态的信道外推方案

Method: 1) CSI-to-PDP模块将CSI转换为功率延迟谱；2) 提取多径总功率和功率加权延迟作为特征；3) 改进MAE架构，使用独立编码器提取掩码CSI和多径特征，通过交叉注意力融合

Result: 框架显著提升外推性能，推理时间仅增加约0.1ms，在已知CSI比例较小时表现出强泛化能力，优于现有基准方法

Conclusion: 提出的基于CSI多径特性的信道外推框架有效解决了传统方法开销大和额外模态依赖问题，为6G网络提供高效信道获取方案
...
### [5] [Near-Field Positioning for XL-MIMO Uniform Circular Arrays: An Attention-Enhanced Deep Learning Approach](https://arxiv.org/abs/2601.21550)
*Yuan Gao,Xinyu Guo,Han Li,Jianbo Du,Shugong Xu*

Main category: eess.SP

TL;DR: 提出一种基于注意力增强深度学习的XL-MIMO系统精确定位方法，利用双路径通道注意力和空间注意力机制，在近场环境下实现优于现有方法的定位精度。


<details>
  <summary>Details</summary>
Motivation: 6G移动通信中XL-MIMO系统天线数量激增，增加了空间自由度，为无线定位带来机遇，但近场范围扩大挑战了传统远场假设。UCA阵列具有恒定角度分辨率优势，如何利用XL-MIMO扩展孔径和近场效应需要进一步研究。

Method: 采用注意力增强深度学习框架，包含双路径通道注意力机制和空间注意力机制，有效整合通道级和空间级特征。使用输入信号的协方差度量作为模型输入。

Result: 模型在综合仿真中超越了ABPN、NFLnet、CNN和MLP等基准方法。协方差度量在定位精度和模型效率方面优于传统的信道状态信息(CSI)。

Conclusion: 提出的注意力增强深度学习模型能够有效利用XL-MIMO系统的扩展孔径和近场效应，实现高精度定位，协方差度量是比CSI更优的定位输入特征。
...
### [6] [VSE: Variational state estimation of complex model-free process](https://arxiv.org/abs/2601.21887)
*Gustav Norén,Anubhab Ghosh,Fredrik Cumlin,Saikat Chatterjee*

Main category: eess.SP

TL;DR: 提出了一种基于变分推理的变分状态估计方法，使用RNN为无模型的复杂动态过程提供闭式高斯后验分布，在跟踪应用中表现优异。


<details>
  <summary>Details</summary>
Motivation: 针对缺乏合适物理模型的复杂动态过程的状态估计问题，传统方法需要已知系统模型或难以提供闭式后验分布，需要一种数据驱动的闭式估计方法。

Method: 使用两个相互辅助的RNN：一个用于推理阶段提供闭式高斯后验，另一个用于学习阶段；基于变分推理原理，两个RNN相互促进学习，实现无模型的状态估计。

Result: 在随机Lorenz系统的跟踪应用中，VSE方法表现优异，既优于已知系统模型的粒子滤波器，也优于最近提出的其他数据驱动状态估计方法。

Conclusion: VSE方法为无模型复杂动态过程提供了一种有效的闭式高斯后验估计方案，在计算简单性和估计准确性方面都有良好表现。
...
### [7] [Joint Laser Inter-Satellite Link Matching and Traffic Flow Routing in LEO Mega-Constellations via Lagrangian Duality](https://arxiv.org/abs/2601.21914)
*Zhouyou Gu,Jihong Park,Jinho Choi*

Main category: eess.SP

TL;DR: 本文研究了低地球轨道巨型星座中激光星间链路的联合优化问题，考虑激光通信终端的机械限制和全球非均匀流量分布，通过拉格朗日对偶分解方法优化连接匹配和流量路由，显著提升网络吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有激光星间链路方案往往忽略激光通信终端的机械限制和由用户及网关不均匀分布导致的全球非均匀流量分布，导致吞吐量次优和激光通信终端/链路利用率不足，特别是在每颗卫星仅携带少量激光通信终端的情况下。

Method: 将问题建模为NP难的混合整数规划，通过拉格朗日对偶松弛耦合约束，将问题分解为：1) 加权图匹配问题用于激光通信终端连接；2) 加权最短路径路由任务；3) 速率分配的线性规划。拉格朗日乘子反映卫星间的拥塞权重，共同指导匹配、路由和速率分配，使用次梯度下降优化乘子并保证收敛。

Result: 使用真实世界星座和地面数据的仿真显示，该方法相比现有非联合方法将网络吞吐量提升了35%到145%。

Conclusion: 通过联合优化激光通信终端连接和流量路由，考虑实际机械限制和全球流量分布，可以显著提高低地球轨道巨型星座的网络吞吐量，拉格朗日对偶分解方法为解决这一复杂优化问题提供了有效途径。
...
### [8] [Duality-Guided Graph Learning for Real-Time Joint Connectivity and Routing in LEO Mega-Constellations](https://arxiv.org/abs/2601.21921)
*Zhouyou Gu,Jinho Choi,Tony Q. S. Quek,Jihong Park*

Main category: eess.SP

TL;DR: DeepLaDu：基于拉格朗日对偶引导的深度学习框架，通过图神经网络单次前向传播预测链路拥塞价格，实现低轨星座网络中激光星间链路连接、流量路由和速率分配的联合优化，相比传统方法提升20%吞吐量并大幅降低计算时间。


<details>
  <summary>Details</summary>
Motivation: 低轨巨型星座的激光星间链路虽能提供高容量骨干连接，但面临激光通信终端有限、机械指向约束和网络拓扑快速变化等管理挑战，需要实时高效的联合优化方案。

Method: 将问题建模为大规模时变星座图上的混合整数优化，提出拉格朗日对偶分解方法，并开发DeepLaDu框架：使用图神经网络单次前向传播直接推断链路级拥塞价格，采用基于次梯度的边级损失实现可扩展稳定训练。

Result: 在模拟的类星链星座环境中，DeepLaDu相比非联合或启发式基线方法提升20%网络吞吐量，同时匹配迭代对偶优化性能但计算时间降低数个数量级，适合动态低轨网络的实时操作。

Conclusion: DeepLaDu通过深度学习与优化理论结合，有效解决了低轨星座激光星间链路管理的实时联合优化问题，为动态非地面网络提供了高效可行的解决方案。
...
### [9] [Optimal Placement of Movable Antennas for Angle-of-Departure Estimation Under User Location Uncertainty](https://arxiv.org/abs/2601.21997)
...
TL;DR: 本文研究在用户设备位置不确定情况下，使用可移动天线阵列进行角度离开估计，通过理论性能分析和天线位置优化，展示了可移动天线系统相比固定阵列的优越性能。


<details>
  <summary>Details</summary>
Motivation: 传统大型天线阵列存在成本和功耗限制，可移动天线（MA）系统能够克服这些限制。在用户设备（UE）位置不确定的实际场景中，需要研究如何利用MA阵列进行鲁棒的角度离开（AoD）估计。

Method: 1) 通过克拉美-罗界（CRB）推导理论性能极限；2) 在用户设备不确定性区域内优化天线位置以确保鲁棒性能；3) 数值分析比较动态优化天线布局与固定阵列的性能。

Result: 数值结果表明，通过显式考虑不确定性区域来动态优化天线布局，相比固定阵列具有更优越的性能，证明了MA系统能够适应环境变化并超越传统阵列。

Conclusion: 可移动天线系统在用户设备位置不确定的场景下，通过优化天线位置能够实现鲁棒的角度离开估计，相比固定阵列具有显著性能优势，为实际无线通信系统提供了有前景的解决方案。
...
### [10] [Towards Joint Optimization for UAV-Integrated RIS-Assisted Fluid Antenna Systems](https://arxiv.org/abs/2601.22109)
...
<details>
  <summary>Details</summary>
Motivation: 无人机集成到蜂窝网络面临严重的空对地干扰挑战，需要提升通信信号质量
...
Conclusion: FAS辅助的RIS方案能有效提升无人机通信性能，为解决空对地干扰问题提供了有效解决方案
...
### [11] [Rethinking LLM-Driven Heuristic Design: Generating Efficient and Specialized Solvers via Dynamics-Aware Optimization](https://arxiv.org/abs/2601.20868)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM驱动的启发式设计框架存在两个关键限制：1) 仅基于最终质量的端点评估，忽略了收敛过程和运行时效率；2) 分布变化导致的高适应成本，需要为新实例组重新生成专用求解器。

Method: 提出DASH框架，通过收敛感知指标共同优化求解器搜索机制和运行时调度，同时引入Profiled Library Retrieval (PLR)机制，在进化过程中并行归档专用求解器，实现成本有效的热启动。

Result: 在四个组合优化问题上的实验表明，DASH将运行效率提高了3倍以上，同时在多样化问题规模上超越了最先进基线的求解质量。通过基于配置的热启动，DASH在不同分布下保持优越精度，同时将LLM适应成本降低了90%以上。

Conclusion: DASH框架通过收敛感知优化和档案库检索机制，有效解决了现有LLM驱动启发式设计框架的局限性，实现了高效、高性能的求解器生成，显著降低了适应成本。
...
### [12] [Finetune-Informed Pretraining Boosts Downstream Performance](https://arxiv.org/abs/2601.20884)
*Atik Faysal,Mohammad Rostami,Reihaneh Gh. Roshan,Nikhil Muralidhar,Huaxia Wang*

Main category: cs.LG

TL;DR: 提出FIP方法，在预训练时针对下游微调时实际使用的目标模态进行偏置学习，提升单模态下游任务性能


<details>
  <summary>Details</summary>
Motivation: 多模态预训练通常平等对待所有模态，但实际部署中下游任务往往只使用单一模态，导致对实际重要模态的表征学习不足

Method: FIP方法通过为目标模态设置更高的掩码难度、更强的损失权重和更大的解码器容量，在预训练阶段偏置学习目标模态的表征，不修改共享编码器或需要额外监督

Result: 在无线信号星座图的掩码建模任务中，FIP能持续提升下游微调性能，无需额外数据或计算资源

Conclusion: FIP方法简单易实现、架构兼容性好，可广泛应用于多模态掩码建模流程中，有效解决预训练与下游任务模态不匹配问题
...
### [13] [A generative machine learning model for designing metal hydrides applied to hydrogen storage](https://arxiv.org/abs/2601.20892)
...
<details>
  <summary>Details</summary>
Motivation: 现有材料数据库（如Materials Project）中已表征的氢化物数量有限，限制了最优氢存储材料的发现，需要开发新方法来扩展候选材料库
...
### [14] [Is Parameter Isolation Better for Prompt-Based Continual Learning?](https://arxiv.org/abs/2601.20894)
...
Result: 广泛分析和实验结果表明，该方法在效果和效率上一致优于现有的静态分配策略，有效缓解了灾难性遗忘并提高了参数利用率。

Conclusion: 提出的提示共享框架通过动态提示分配和协作优化机制，解决了固定提示分配的限制，为持续学习提供了更高效灵活的解决方案。
...
### [15] [Faster Predictive Coding Networks via Better Initialization](https://arxiv.org/abs/2601.20895)
*Luca Pinchetti,Simon Frieder,Thomas Lukasiewicz,Tommaso Salvatori*

Main category: cs.LG

TL;DR: 提出一种新的预测编码网络初始化技术，通过保留先前训练样本的迭代进展来显著减少训练时间，提高计算效率和最终性能


<details>
  <summary>Details</summary>
Motivation: 预测编码等基于能量的学习算法因其多功能性和数学基础而受到关注，但其迭代性质导致计算需求大，限制了实际应用。本研究旨在解决这一问题，通过改进初始化方法来减少训练时间

Method: 提出一种新的预测编码网络初始化技术，该技术关注神经元初始化的选择，旨在保留先前训练样本的迭代进展，从而减少所需的训练时间

Result: 实验表明，该方法在监督和无监督设置下都能显著提高收敛速度和最终测试损失，为预测编码与反向传播在计算效率和最终性能方面的差异提供了有希望的解决方案

Conclusion: 预测编码网络的初始化选择对训练时间有显著影响，提出的初始化技术能够有效减少计算需求，为预测编码算法的实际应用提供了可行的路径
...
### [16] [TwinWeaver: An LLM-Based Foundation Model Framework for Pan-Cancer Digital Twins](https://arxiv.org/abs/2601.20906)
...
TL;DR: TwinWeaver框架将患者纵向历史序列化为文本，使大语言模型能统一进行事件预测和预后分析，在癌症患者数据上显著提升了预测精度和风险分层能力。
...
Method: 提出TwinWeaver开源框架，将纵向患者历史序列化为文本，使大语言模型能统一进行事件预测和预后分析。在此基础上构建了Genie Digital Twin (GDT)，在93,054名20种癌症类型的患者数据上进行训练。

Result: GDT显著降低预测误差（中位MASE 0.87 vs 基线0.97），改善风险分层（平均C-index 0.703 vs 基线0.662）。在分布外临床试验数据上，零样本匹配基线，微调后表现更优（中位MASE 0.75-0.88，平均C-index 0.672 vs 基线0.648）。

Conclusion: TwinWeaver为纵向临床建模提供了可扩展且透明的基础，通过将患者历史文本化使大语言模型能有效处理临床时间序列，显著提升预测精度并支持可解释的临床推理。
...
### [17] [Noisy but Valid: Robust Statistical Evaluation of LLMs with Imperfect Judges](https://arxiv.org/abs/2601.20913)
*Chen Feng,Minghe Shen,Ananth Balashankar,Carsten Gerner-Beuerle,Miguel R. D. Rodrigues*

Main category: cs.LG

TL;DR: 提出一个"嘈杂但有效"的假设检验框架，用于在LLM法官存在噪声和偏差的情况下，保证有限样本类型I错误控制，实现可靠的大语言模型安全认证。


<details>
  <summary>Details</summary>
Motivation: LLM作为法官进行大规模评估虽然可扩展，但法官的不完美、噪声和偏差会破坏统计保证。需要一种方法在法官存在缺陷的情况下仍能提供可靠的统计认证。

Method: 使用小规模人工标注校准集估计法官的真阳性率和假阳性率，然后将方差校正的临界阈值应用于大规模法官标注数据集，建立理论上保证有限样本类型I错误控制的假设检验框架。

Result: 1) 理论推导出噪声测试比直接评估具有更高统计功效的条件；2) 在Jigsaw Comment、Hate Speech和SafeRLHF数据集上的实验验证了理论；3) 揭示了实际方法与理论"Oracle"之间的显著性能差距。

Conclusion: 该框架首次系统处理了不完美法官设置，提供了可解释的法官可靠性诊断，阐明了评估功效如何依赖于法官质量、数据集大小和认证水平，为LLM法官统计评估提供了更清晰的理解。
...
### [18] [Noninvasive Intracranial Pressure Estimation Using Subspace System Identification and Bespoke Machine Learning Algorithms: A Learning-to-Rank Approach](https://arxiv.org/abs/2601.20916)
...
TL;DR: 开发了一种结合系统辨识和排序约束优化的机器学习算法，通过无创信号（动脉血压、脑血流速度、R-R间期）来估计颅内压，约32%的测试数据误差在2mmHg以内。


<details>
  <summary>Details</summary>
Motivation: 无创准确估计颅内压是重症监护中的主要挑战，需要开发安全且广泛可及的颅内压监测方法，特别是针对急性脑损伤患者。

Method: 提出机器学习框架，使用子空间系统辨识算法建立脑血流动力学模型，通过凸优化学习无创信号特征与估计误差之间的映射函数，并采用创新的排序约束。

Result: 约31.88%的测试数据估计误差在2mmHg以内，34.07%的测试数据误差在2-6mmHg之间，证明了该无创颅内压估计方法的可行性。

Conclusion: 该方法为安全且广泛可及的颅内压监测奠定了基础，但需要进一步验证和技术改进才能临床部署。
...
### [19] [A Theory of Universal Agnostic Learning](https://arxiv.org/abs/2601.20961)
*Steve Hanneke,Shay Moran*

Main category: cs.LG

TL;DR: 该论文建立了不可知设置下二元分类的最优通用速率完整理论，识别出四种最优速率类型：e^{-n}、e^{-o(n)}、o(n^{-1/2})和任意慢速率，并找到了决定概念类属于哪一类的简单组合结构。
...
Method: 通过理论分析识别出四种最优速率类型，并找到决定概念类属于哪一类的简单组合结构，扩展了可实现情况下的理论框架。

Result: 建立了不可知设置下二元分类的最优通用速率完整理论，证明了对于每个概念类，其最优通用收敛速率必属于四种类型之一，并找到了决定这一分类的简单组合结构。

Conclusion: 该研究为不可知设置下的二元分类提供了完整的理论框架，将可实现情况的理论扩展到更一般的不可知设置，揭示了最优速率的四分类结构及其组合决定因素。
...
### [20] [Monotone Optimisation with Learned Projections](https://arxiv.org/abs/2601.20983)
*Ahmed Rashwan,Keith Briggs,Chris Budd,Lisa Kreusser*

Main category: cs.LG

TL;DR: 提出HM-RI网络，通过预测径向逆函数直接替代POA算法中的投影原语，避免传统二分法，加速单调优化问题的求解。


<details>
  <summary>Details</summary>
Motivation: 传统单调优化算法如POA需要显式的目标函数和约束函数，但在许多实际应用中这些函数只能通过数据获得，使得POA难以直接应用。

Method: 提出算法感知学习方法，通过HM-RI网络直接预测POA的投影原语（径向逆函数），该网络强制单调性和齐次性，并提供理论保证。

Result: 在多个单调优化基准测试中（不定二次规划、乘法规划、发射功率优化），相比直接函数估计方法获得显著加速，同时保持解的质量。

Conclusion: HM-RI网络通过利用单调结构，实现了比不利用单调性的基线方法更好的性能，为数据驱动的单调优化提供了有效解决方案。
...
### [21] [Distributional Active Inference](https://arxiv.org/abs/2601.20985)
...
<details>
  <summary>Details</summary>
Motivation: 机器人系统在复杂环境中的最优控制面临两个互补且交织的挑战：感官状态信息的有效组织和远见行动规划。强化学习只解决后者，导致样本效率低下。主动推理是解释生物大脑如何处理这个双重问题的最先进过程理论，但其在人工智能中的应用仅限于现有基于模型方法的扩展。

Method: 提出了一个形式化的强化学习算法抽象，涵盖基于模型、分布式和免模型方法。这个抽象将主动推理无缝整合到分布式强化学习框架中，使其性能优势无需转移动态建模即可实现。

Result: 该方法使主动推理的性能优势在无需转移动态建模的情况下变得可访问，为复杂环境中的机器人控制提供了更高效的解决方案。

Conclusion: 通过将主动推理整合到分布式强化学习框架中，可以在不依赖转移动态建模的情况下获得性能优势，为解决机器人系统在复杂环境中的双重控制挑战提供了新途径。
...
### [22] [Pre-trained Encoders for Global Child Development: Transfer Learning Enables Deployment in Data-Scarce Settings](https://arxiv.org/abs/2601.20987)
*Md Muhtasim Munif Fahim,Md Rezaul Karim*

Main category: cs.LG

TL;DR: 开发首个全球儿童发育预训练编码器，仅需50个样本即可在资源受限环境中实现有效监测，解决数据瓶颈问题
...
Method: 使用联合国儿童基金会调查数据，在44个国家357,709名儿童上训练首个全球儿童发育预训练编码器，应用迁移学习理论解释预训练多样性如何实现少样本泛化

Result: 仅需50个训练样本，预训练编码器平均AUC达0.65，比冷启动梯度提升的0.61高8-12%；500个样本时AUC达0.73；零样本部署到未见国家AUC最高达0.84

Conclusion: 预训练编码器能够显著改变在资源受限环境中监测可持续发展目标4.2.1的可行性，为全球儿童发育监测提供有效解决方案
...
### [23] [Top-k on a Budget: Adaptive Ranking with Weak and Strong Oracles](https://arxiv.org/abs/2601.20989)
*Lutz Oettershagen*

Main category: cs.LG

TL;DR: 论文研究在弱/强双预言机设置下高效识别top-k项的问题，提出自适应认证算法ACE和两阶段方法ACE-W，显著减少昂贵强预言机调用。


<details>
  <summary>Details</summary>
Motivation: 在实际应用中，精确评估项目价值通常成本高昂（如需要专家验证或昂贵模拟）。现有方法要么依赖噪声较大的快速弱预言机，要么过度使用稀缺的高保真强预言机。需要设计算法在保证准确性的同时最小化强预言机调用。

Method: 1. 分析基准方法STC（筛选后认证）；2. 提出ACE算法，自适应地将强查询集中在关键边界项上；3. 提出ACE-W两阶段方法，先自适应分配弱预算，再运行ACE。

Result: 1. STC最多需要m(4ε_max)次强调用；2. 证明任何算法在相同弱不确定性下至少需要Ω(m(ε_max))次强调用；3. ACE达到O(m(4ε_max))上界，实践中显著减少强调用；4. ACE-W进一步降低强成本。

Conclusion: 论文为双预言机设置下的top-k识别问题提供了理论分析和实用算法。ACE和ACE-W在保证理论最优性的同时，通过自适应策略显著减少了昂贵的强预言机调用，具有重要实际应用价值。
...
Method: 提出架构条件化的缩放定律，将损失分解为对深度和宽度的依赖。通过30个不同架构的Transformer模型（1700万到70亿参数）进行实证验证，每个模型在代表性高计算样本上训练。

Result: 发现最优深度缩放为D* ~ C^0.12，最优宽度缩放为W* ~ C^0.34，宽度应比深度增长快2.8倍。发现临界深度现象：超过D_crit ~ W^0.44后增加层数反而增加损失。在70亿参数规模下，64层模型（63.8亿参数）比32层模型（68.6亿参数）性能差0.12 nats。

Conclusion: 深度-宽度权衡在工业规模下仍然存在，模型设计需要平衡深度和宽度。深度错觉现象表明单纯增加深度不一定提升性能，需要遵循架构条件化的缩放定律进行优化设计。
...
### [25] [MADE: Benchmark Environments for Closed-Loop Materials Discovery](https://arxiv.org/abs/2601.20996)
*Shreshth A Malik,Tiarnan Doherty,Panagiotis Tigas,Muhammed Razzak,Stephen J. Roberts,Aron Walsh,Yarin Gal*

Main category: cs.LG

TL;DR: MADE是一个用于基准测试端到端自主材料发现管道的框架，模拟在有限预算下的闭环发现过程，评估算法在寻找热力学稳定化合物方面的效果和效率。


<details>
  <summary>Details</summary>
Motivation: 现有材料发现基准主要评估静态预测任务或孤立的计算子任务，忽略了科学发现固有的迭代和自适应特性。需要一种能够评估完整发现流程的基准框架。

Method: 引入MADE框架，模拟闭环发现活动，其中代理或算法在受限的oracle预算下提出、评估和优化候选材料。将发现形式化为相对于给定凸包寻找热力学稳定化合物的搜索问题，并通过与基线算法比较来评估效果和效率。

Result: MADE框架具有灵活性，用户可以从可互换组件（如生成模型、过滤器和规划器）组合发现代理，能够研究从固定管道到具有工具使用和自适应决策能力的完全代理系统的任意工作流程。

Conclusion: 通过在不同系统家族中进行系统实验，MADE能够对发现管道中的组件进行消融研究，并比较方法随系统复杂性的扩展能力，为端到端自主材料发现提供了有效的基准测试框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM基准将运筹学问题视为一次性代码生成任务，忽略了实际应用中通过分析不可约不可行子系统进行迭代调试的诊断循环，需要更真实的评估框架。

Method: 引入两个新基准：ORDebug评估迭代自校正能力，包含5000+问题和9种错误类型，每次修复后重新执行求解器并重新计算IIS；ORBias评估行为理性，包含2000个报童问题实例，测量与闭式最优策略的系统性偏差。
...
Conclusion: 通过可验证预言机的过程级评估能够实现超越规模效应的针对性训练，为运筹学领域的LLM评估提供了更真实的框架。
...
TL;DR: OATTA提出了一种考虑时序动态的测试时自适应方法，通过贝叶斯估计和时间先验来提升模型在测试流上的性能，同时引入安全机制防止时序证据不足时的性能下降。
...
Method: 将测试时自适应建模为无梯度递归贝叶斯估计任务，使用学习到的动态转移矩阵作为时间先验来优化基础模型预测；引入似然比门控机制，当时序证据不足时回退到基础预测器。

Result: 在图像分类、可穿戴设备与生理信号分析、语言情感分析等多个领域验证了方法的普适性，OATTA能持续提升现有基线方法，最高提升准确率6.35%。

Conclusion: 建模时序动态为测试时自适应提供了正交于传统方法的额外信号，OATTA作为轻量级、模型无关的模块，能以可忽略的计算开销显著提升模型性能。
...
### [28] [Conditional Denoising Model as a Physical Surrogate Model](https://arxiv.org/abs/2601.21021)
...
<details>
  <summary>Details</summary>
Motivation: 传统代理建模面临数据拟合精度与物理一致性之间的权衡。现有物理一致性方法通常将物理定律作为软约束，无法保证严格遵循控制方程，或依赖后处理修正而不学习底层解几何

Method: 引入Conditional Denoising Model (CDM)，通过训练网络从噪声状态恢复干净状态，学习指向有效解子空间的向量场。采用时间无关公式，将推理转化为确定性定点迭代，将噪声近似投影到平衡流形

Result: 在低温等离子体物理和化学基准测试中，CDM比物理一致性基线具有更高的参数和数据效率。去噪目标作为强大的隐式正则化器：尽管训练中从未见过控制方程，模型比使用显式物理损失训练的基线更严格地遵循物理约束

Conclusion: CDM通过学习物理流形几何，提供了一种既保证物理一致性又保持数据拟合精度的新方法，去噪目标作为隐式正则化器比显式物理约束更有效
...
### [29] [SIGMA-PPG: Statistical-prior Informed Generative Masking Architecture for PPG Foundation Model](https://arxiv.org/abs/2601.21031)
*Zongheng Guo,Tao Chen,Yang Jiao,Yi Pan,Xiao Hu,Manuela Ferrario*

Main category: cs.LG

TL;DR: SIGMA-PPG是一个基于统计先验的生成式基础模型，通过对抗性掩码机制和语义一致性约束，解决了PPG信号内在冗余和噪声问题，在12个下游任务中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前PPG信号基础模型面临信号内在冗余和噪声的挑战，标准掩码建模容易产生平凡解，而对比学习方法缺乏形态学精度。

Method: 提出统计先验引导的生成式掩码架构(SIGMA-PPG)，包含先验引导的对抗性掩码机制（使用强化学习教师利用统计先验创建挑战性学习路径）和通过向量量化的语义一致性约束。

Result: 在超过120,000小时数据上预训练后，SIGMA-PPG在12个多样化下游任务中平均性能优于5个最先进的基线方法。

Conclusion: SIGMA-PPG通过创新的对抗性掩码和语义一致性约束，有效解决了PPG信号建模中的冗余和噪声问题，为PPG信号分析提供了强大的基础模型。
...
### [30] [Supervised Guidance Training for Infinite-Dimensional Diffusion Models](https://arxiv.org/abs/2601.20756)
*Elizabeth L. Baker,Alexander Denker,Jes Frellsen*

Main category: cs.LG

TL;DR: 提出了一种在无限维函数空间中基于分数的扩散模型的后验采样方法，通过Doob's h-transform理论框架和监督引导训练实现高效稳定的贝叶斯逆问题求解。


<details>
  <summary>Details</summary>
Motivation: 虽然基于分数的扩散模型已被扩展到无限维函数空间，可用于偏微分方程逆问题，但在贝叶斯逆问题中，如何条件化这些模型以从后验分布中采样仍缺乏理论支持。本文旨在填补这一理论空白。

Method: 1. 假设先验位于Cameron-Martin空间或相对于高斯测度绝对连续；2. 证明可以使用无限维Doob's h-transform进行条件化；3. 提出监督引导训练（Supervised Guidance Training）的无模拟分数匹配目标，以高效稳定地进行后验采样。

Result: 1. 建立了函数空间中扩散模型条件化的理论框架；2. 证明了条件分数可分解为无条件分数和引导项；3. 提出的监督引导训练方法在函数空间的贝叶斯逆问题上展示了有效性。

Conclusion: 本文首次提出了在函数空间中微调训练好的扩散模型以准确从后验分布采样的方法，为无限维函数空间的贝叶斯逆问题提供了理论保证和实用算法。
...
TL;DR: 提出PPR（Predict-Project-Renoise）约束采样框架，在扩散模型生成过程中强制执行物理定律等硬约束，显著减少约束违反并提高样本质量
...
Method: 提出约束采样框架：定义约束前向过程，仅在满足约束的可行集上进行扩散，诱导约束边际分布。提出PPR（Predict-Project-Renoise）迭代算法，通过去噪预测、投影到可行集、再噪声化交替进行，从约束边际分布中采样

Result: 在2D分布、偏微分方程和全球天气预报实验中，PPR将约束违反减少了一个数量级以上，同时提高了样本一致性，更好地匹配真实约束分布，优于基线方法

Conclusion: PPR框架为基于扩散模型的科学模拟提供了有效的约束强制执行机制，能够在保持生成质量的同时确保物理准确性和约束满足
...
### [32] [Thinking in Frames: How Visual Context and Test-Time Scaling Empower Video Reasoning](https://arxiv.org/abs/2601.21037)
...
Method: 通过视频生成模型进行视觉推理，将生成的帧作为初始状态和解决方案之间的中间推理步骤。在迷宫导航（序列离散规划）和七巧板拼图（连续操作）两个任务上进行评估。

Result: 发现三个关键洞察：1）强大的零样本泛化能力；2）有效利用视觉上下文作为显式控制；3）观察到序列规划中的测试时扩展定律，增加生成视频长度可提升对复杂路径的零样本泛化能力。
...
### [33] [Signal from Structure: Exploiting Submodular Upper Bounds in Generative Flow Networks](https://arxiv.org/abs/2601.21061)
*Alexandre Larouche,Audrey Durand*

Main category: cs.LG

TL;DR: 提出SUBo-GFN方法，利用子模性上界训练GFlowNets，显著减少奖励函数查询次数，提高训练数据生成效率


<details>
  <summary>Details</summary>
Motivation: 当奖励函数具有子模结构时，可以利用这一特性获得未观测组合对象的上界估计，从而减少对奖励函数的查询次数，提高GFlowNets的训练效率

Method: 提出SUBo-GFN方法：利用子模性推导未观测组合对象的奖励上界，遵循"面对不确定性的乐观原则"，使用这些上界来训练GFlowNets，显著减少对奖励函数的实际查询

Result: SUBo-GFN在相同奖励函数查询次数下，比传统GFlowNets生成数量级更多的训练数据；在合成和真实世界的子模任务中，在分布匹配和高质量候选生成方面表现出色

Conclusion: 利用奖励函数的子模结构可以显著提高GFlowNets的训练效率，SUBo-GFN方法通过子模上界估计减少了奖励函数查询需求，为具有结构化奖励的生成任务提供了有效解决方案
...
### [34] [Test-Time Adaptation for Unsupervised Combinatorial Optimization](https://arxiv.org/abs/2601.21048)
*Yiqiao Liao,Farinaz Koushanfar,Parinaz Naghizadeh*

Main category: cs.LG

TL;DR: TACO提出了一种测试时自适应框架，统一了通用化模型和实例特定模型两种范式，通过策略性热启动在保持归纳偏置的同时实现快速有效的无监督适应。


<details>
  <summary>Details</summary>
Motivation: 现有无监督神经组合优化方法存在两种分离范式：通用化模型缺乏实例级适应性，实例特定模型无法利用学习到的归纳结构且易陷入局部最优。需要一种方法既能利用通用化模型学到的归纳偏置，又能获得实例级适应的灵活性。

Method: 提出TACO框架，采用策略性热启动方法，部分放松训练参数同时保持归纳偏置，实现快速有效的无监督适应。该框架与模型无关，统一并扩展了现有两种范式。

Result: 在最小顶点覆盖和最大团等经典组合优化问题上，TACO在静态、分布偏移和动态场景下均表现出有效性和鲁棒性。相比简单微调通用模型或从头训练实例特定模型，TACO获得更好的解质量且计算成本可忽略。

Conclusion: TACO成功桥接了通用化和实例特定的无监督神经组合优化，在保持归纳偏置的同时实现了有效的实例级适应，为实际应用提供了实用框架。
...
### [35] [The Powers of Precision: Structure-Informed Detection in Complex Systems -- From Customer Churn to Seizure Onset](https://arxiv.org/abs/2601.21170)
*Augusto Santos,Teresa Santos,Catarina Rodrigues,José M. F. Moura*

Main category: cs.LG

TL;DR: 提出一种基于协方差矩阵幂的机器学习方法，用于从部分观测数据中检测复杂系统中的涌现现象（如癫痫发作、客户流失等），该方法通过学习最优特征表示来揭示潜在的因果结构。


<details>
  <summary>Details</summary>
Motivation: 复杂系统中的涌现现象（癫痫发作、客户流失、疫情爆发）通常源于隐藏的因果相互作用，但数据生成过程未知且只能部分观测，这给早期检测带来了核心挑战。

Method: 从单参数估计器族（经验协方差或精度矩阵的幂）中学习最优特征表示，通过调整参数来捕捉驱动关键事件的底层结构，然后使用监督学习模块对学习到的表示进行分类。

Result: 在癫痫发作检测和客户流失预测任务中取得了有竞争力的结果，证明了方法的有效性。最优协方差幂不仅表现出良好的可识别性，还能捕捉结构特征，实现了预测性能与可解释统计结构的统一。

Conclusion: 该方法不仅能有效检测复杂系统中的涌现现象，还能提供可解释的统计结构，在预测性能和可解释性之间取得了平衡，为理解复杂系统的潜在因果机制提供了新途径。
...
### [36] [SMKC: Sketch Based Kernel Correlation Images for Variable Cardinality Time Series Anomaly Detection](https://arxiv.org/abs/2601.21050)
*Haokun Zhou*

Main category: cs.LG

TL;DR: SMKC框架解决传感器漂移下的多元时间序列异常检测，通过特征哈希和混合核图像将动态输入结构解耦，使用随机投影和最近邻即可实现竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 实际监控系统中传感器经常出现漂移现象（信号出现、消失或重命名），导致观测变量集合动态变化，传统异常检测方法假设静态变量集不再适用。

Method: 1) 使用置换不变特征哈希将原始输入映射为固定大小的状态序列；2) 构建混合核图像通过序列及其导数的成对比较捕获全局时间结构；3) 使用掩码重建和师生预测目标学习正常模式。

Result: 评估发现鲁棒的对数距离通道提供主要判别信号，而余弦表示通常无法捕获足够对比度。使用随机投影和最近邻的SMKC表示与完全训练的基线方法竞争，无需梯度更新。

Conclusion: SMKC表示本身的有效性为资源受限部署提供了实用的冷启动解决方案，将动态输入结构与异常检测器解耦，解决了传感器漂移带来的挑战。
...
### [37] [Missing-Data-Induced Phase Transitions in Spectral PLS for Multimodal Learning](https://arxiv.org/abs/2601.21294)
*Anders Gjølbye,Ida Kargaard,Emma Kargaard,Lars Kai Hansen*

Main category: cs.LG

TL;DR: PLS-SVD在缺失数据下的相位转变：当信号强度超过临界阈值时，可以恢复潜在共享方向，否则无信息


<details>
  <summary>Details</summary>
Motivation: 多模态数据常存在缺失条目，需要研究PLS-SVD在缺失数据下的表现，理解其恢复潜在共享结构的能力

Method: 在比例高维尖峰模型下，研究独立逐项完全随机缺失的PLS-SVD，分析归一化后掩码交叉协方差矩阵的行为

Result: 发现PLS-SVD存在尖锐的BBP型相位转变：低于临界信噪比时主奇异向量无信息，高于阈值时与潜在共享方向对齐，并给出闭式渐近重叠公式

Conclusion: 理论预测与仿真实验一致，为缺失数据下PLS-SVD的性能提供了理论框架和实用指导
...
TL;DR: Snowball是一种数字化的全连接伊辛机，通过双模式MCMC自旋选择和异步更新算法，在AMD Alveo U250加速卡上实现了比现有伊辛机快8倍的时间到解决方案。


<details>
  <summary>Details</summary>
Motivation: 伊辛机作为组合优化加速器面临三个挑战：硬件拓扑限制需要次要嵌入、朴素并行更新可能导致振荡或停滞、有限精度可能阻碍可行映射或降低解质量。这些因素影响了实际部署的时间到解决方案。

Method: 提出Snowball数字伊辛机，采用全连接拓扑，集成双模式马尔可夫链蒙特卡洛自旋选择与异步自旋更新算法，支持宽范围可配置的耦合精度，在AMD Alveo U250加速卡上实现原型。

Result: 在相同基准实例上，Snowball原型相比最先进的伊辛机实现了8倍的时间到解决方案减少，展示了其在实际部署中的高效性。

Conclusion: Snowball通过解决硬件拓扑、自旋选择更新算法和耦合精度三个关键挑战，为伊辛机的实际部署提供了有效的解决方案，显著提升了组合优化问题的求解效率。
...
TL;DR: 本文建立了平均奖励MDP中同步和异步Q-learning的最优样本复杂度保证，通过懒惰化动态采样实现了Õ(ε⁻²)的收敛率


<details>
  <summary>Details</summary>
Motivation: 平均奖励MDP中的Q-learning缺乏收缩性，现有方法要么需要强假设，要么使用折扣或片段MDP作为近似，导致次优样本复杂度或需要未知参数

Method: 提出使用懒惰化动态采样的Q-learning变体，构造实例依赖的半范数，证明在懒惰化MDP变换后Bellman算子在该半范数下单步收缩

Result: 在可达性假设下，建立了同步和异步Q-learning的最优Õ(ε⁻²)样本复杂度保证（忽略对数因子）

Conclusion: 通过懒惰化变换和实例依赖的半范数分析，克服了平均奖励MDP中Q-learning缺乏收缩性的挑战，实现了最优样本复杂度
...
### [40] [Human-LLM Collaborative Feature Engineering for Tabular Data](https://arxiv.org/abs/2601.21060)
...
TL;DR: 提出人类-LLM协作的特征工程框架，将特征转换操作的建议与选择过程解耦，通过显式建模操作效用和不确定性来指导选择，并选择性引入人类专家偏好反馈来提升效果。
...
Method: 提出人类-LLM协作框架：LLM仅生成操作候选，选择过程通过显式建模每个操作的效用和不确定性来指导。在早期效用估计困难时，选择性引入人类专家偏好反馈（比较哪些操作更有前景）来帮助识别更有效的操作。

Result: 在合成研究和真实用户研究中，该框架提升了多种表格数据集上的特征工程性能，并降低了用户在特征工程过程中的认知负荷。

Conclusion: 通过解耦操作建议与选择，并引入人类专家偏好反馈，提出的协作框架能更有效地进行特征工程，提高性能同时减轻用户负担。
...
### [41] [Partial Feedback Online Learning](https://arxiv.org/abs/2601.21462)
*Shihao Shao,Cong Fang,Zhouchen Lin,Dacheng Tao*

Main category: cs.LG

TL;DR: 该论文研究了部分反馈在线学习，其中每个实例有一组正确标签，但学习者每轮只观察到一个正确标签；预测在正确集合内即算正确。论文给出了确定性学习者和随机学习者在集合可实现性机制下的极小极大后悔的近乎完整刻画。


<details>
  <summary>Details</summary>
Motivation: 研究部分反馈在线学习的动机源于实际应用场景，如语言生成，其中多个回答可能都是有效的，但数据只提供一个参考回答。这种设置需要新的理论框架来理解学习能力。

Method: 引入部分反馈Littlestone维度（PFLdim）来刻画确定性学习者的学习能力，开发部分反馈测量粉碎维度（PMSdim）来获得随机学习者的紧界。通过新的集合版本空间视角和辅助维度进行分析。

Result: PFLdim精确地决定了确定性学习者的学习能力和极小极大后悔。PMSdim为随机学习者提供了紧界。论文确定了确保确定性和随机学习能力不可分离的广泛条件，并解决了Raman等人[2024b]的开放问题。

Conclusion: 在集合可实现性机制外，问题可能变得信息论上不可处理，即使对于|H|=2也可能出现线性后悔。这突显了需要新的、对噪声敏感的复杂性度量来有意义地表征集合可实现性之外的学习能力。
...
TL;DR: PPI-SVRG算法结合了PPI和SVRG两种方差缩减技术，在标签稀缺但有预训练模型预测的情况下，通过控制变量方法提升优化性能。


<details>
  <summary>Details</summary>
Motivation: 在标签数据稀缺但预训练模型预测可用的半监督学习场景下，需要有效利用预测信息来提升优化性能。PPI和SVRG都使用控制变量方法减少方差，但分别利用预测和参考梯度。

Method: 证明了PPI和SVRG在数学上的等价性，并开发了PPI-SVRG算法结合两者优势。收敛界分解为标准SVRG速率加上预测不确定性带来的误差下限。

Result: 在均值估计基准测试中，PPI-SVRG在标签稀缺下将MSE降低了43-52%；在MNIST上仅使用10%标签数据时，测试准确率提高了2.7-2.9个百分点。

Conclusion: PPI-SVRG算法有效结合了预测信息和梯度方差缩减，收敛速率仅依赖于损失函数几何结构，预测质量只影响收敛邻域大小，在预测完美时可精确恢复SVRG。
...
### [43] [Textual Equilibrium Propagation for Deep Compound AI Systems](https://arxiv.org/abs/2601.21064)
*Minghui Chen,Wenlong Deng,James Zou,Han Yu,Xiaoxiao Li*

Main category: cs.LG

TL;DR: 本文提出Textual Equilibrium Propagation (TEP)方法，解决深度复合AI系统中文本梯度传播的爆炸和消失问题，通过局部学习和受控适应优化长流程工作流。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本反馈全局传播的方法（如TextGrad）在系统深度增加时性能下降，存在文本梯度爆炸（反馈指数增长）和文本梯度消失（长上下文能力有限导致反馈压缩失真）两大问题，限制了深度复合AI系统的优化效果。

Method: 提出Textual Equilibrium Propagation (TEP)，受能量模型中均衡传播启发，包含两个阶段：1）自由阶段：本地LLM批评者迭代优化提示直至达到均衡；2）推动阶段：应用有界修改强度的近端提示编辑，通过前向信号而非后向反馈链传播任务级目标。

Result: 在长流程QA基准测试和多智能体工具使用数据集上，TEP相比TextGrad等全局传播方法持续提升准确性和效率，且增益随深度增加而增长，同时保持了黑盒LLM组件的实用性。

Conclusion: TEP通过局部学习原则和受控适应机制，有效解决了深度复合AI系统中的文本梯度传播问题，为优化长流程工作流提供了更高效实用的方法，同时避免了全局文本反向传播的计算负担和信号退化。
...
### [44] [Best Arm Identification with LLM Judges and Limited Human](https://arxiv.org/abs/2601.21471)
*Ruicheng Ao,Hongyu Chen,Siyang Gao,Hanwei Li,David Simchi-Levi*

Main category: cs.LG

TL;DR: 提出一种在存在有偏代理（如LLM）和昂贵真实标签情况下的最佳臂识别方法，通过逆概率加权残差估计和自适应审计策略提高效率
...
Method: 开发结合代理分数和逆概率加权残差的均值估计器，构建随时有效置信序列，提出自适应选择和审计臂的算法，采用插件Neyman规则

Result: 证明算法能集中审计不可靠上下文和接近臂，插件Neyman规则实现接近最优审计效率，数值实验验证理论保证和优越性能

Conclusion: 提出的方法解决了有偏代理和选择性真实标签下的BAI问题，通过自适应审计策略显著提高了资源利用效率
...
TL;DR: 本文首次系统综述了图基础模型（GFMs）中的分布外泛化问题，分析了图学习中的分布偏移挑战，并基于任务规范固定与否对现有方法进行了分类整理。


<details>
  <summary>Details</summary>
Motivation: 图学习模型在实际应用中常面临分布偏移问题（如图结构变化、领域语义差异、模态可用性变化、任务形式变化等），导致泛化能力有限。图基础模型通过大规模预训练学习通用表示来解决这一问题，但目前缺乏对分布外泛化问题的系统性综述。

Method: 本文从分布外泛化视角回顾GFMs进展：首先讨论图学习中的分布偏移挑战并建立统一问题设定；然后根据方法是否设计用于固定任务规范或支持跨异构任务泛化来组织现有方法；最后总结相应的分布外处理策略和预训练目标。

Result: 本文系统梳理了GFMs中分布外泛化的研究现状，建立了分类框架，总结了不同方法的核心策略，并回顾了常见的评估协议，为后续研究提供了基础。

Conclusion: 这是首篇针对图基础模型中分布外泛化问题的综述论文，系统性地整理了该领域的研究进展，并指出了未来的研究方向。
...
### [46] [More Bang for the Buck: Improving the Inference of Large Language Models at a Fixed Budget using Reset and Discard (ReD)](https://arxiv.org/abs/2601.21522)
*Sagi Meir,Tommer D. Keidar,Noam Levi,Shlomi Reuveni,Barak Hirshberg*

Main category: cs.LG

TL;DR: 论文提出ReD方法，通过重置和丢弃策略提高LLM在验证任务中的覆盖率效率，减少达到目标覆盖率所需的尝试次数、token数和成本。


<details>
  <summary>Details</summary>
Motivation: 传统pass@k指标在固定预算下存在局限性，无法反映实际覆盖率随成本增长的情况，观察到幂律行为导致覆盖率增长呈现收益递减问题。

Method: 提出Reset-and-Discard (ReD)查询方法，通过重置和丢弃策略优化LLM查询过程，提高任何给定预算下的覆盖率，并能预测节省的尝试次数。

Result: 在三个LLM上的HumanEval实验表明，ReD显著减少了达到期望覆盖率所需的尝试次数、token数和美元成本，同时提供了一种高效测量推理幂律的方法。

Conclusion: ReD方法有效解决了LLM在验证任务中的收益递减问题，提供了一种更高效的查询策略和幂律测量方法，具有实际应用价值。
...
### [47] [LOCUS: Low-Dimensional Model Embeddings for Efficient Model Exploration, Comparison, and Selection](https://arxiv.org/abs/2601.21082)
*Shivam Patel,William Cocke,Gauri Joshi*

Main category: cs.LG

TL;DR: LOCUS是一种为语言模型生成低维向量嵌入的方法，通过注意力机制基于查询编码和评估分数创建紧凑的能力表示，支持动态模型池管理和高效路由


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型生态系统的快速增长，管理和有效利用庞大且动态变化的模型池变得越来越困难。需要一种能够紧凑表示模型能力的方法，支持新模型的快速加入和现有模型的更新，而无需重新训练。

Method: LOCUS采用基于注意力的方法，通过对查询编码和评估分数进行确定性前向传播来生成嵌入向量。该方法使用编码器模型，能够无缝整合新模型到池中，并优化现有模型嵌入而无需重新训练。此外，还训练了一个正确性预测器，利用模型嵌入和查询编码来实现最先进的路由精度。

Result: 实验表明，LOCUS比基线方法需要少4.8倍的查询评估样本来生成信息丰富且鲁棒的嵌入。学习到的嵌入空间具有几何意义：邻近性反映了模型相似性，支持多种下游应用，包括模型比较和聚类、模型组合选择以及不可用模型的弹性代理。

Conclusion: LOCUS提供了一种高效的方法来管理动态的大语言模型生态系统，通过低维嵌入紧凑表示模型能力，支持高效路由和多种下游应用，显著减少了评估样本需求。
...
### [48] [Sampling-Free Privacy Accounting for Matrix Mechanisms under Random Allocation](https://arxiv.org/abs/2601.21636)
...
Result: 通过数值比较，证明该方法在研究和实践中使用的各种矩阵机制上均有效，相比现有方法提供了更严格的隐私保证，特别是在小ε情况下。

Conclusion: 提出的采样自由隐私放大框架克服了现有蒙特卡洛方法的局限性，为矩阵分解差分隐私训练中的随机分配场景提供了更可靠、高效的隐私边界计算方法。
...
### [49] [MapPFN: Learning Causal Perturbation Maps in Context](https://arxiv.org/abs/2601.21092)
*Marvin Sextro,Weronika Kłos,Gabriel Dernbach*

Main category: cs.LG

TL;DR: MapPFN是一个基于先验数据拟合网络的元学习方法，通过在合成数据上预训练来预测单细胞扰动效应，无需梯度优化即可适应新生物环境


<details>
  <summary>Details</summary>
Motivation: 生物系统干预需要能够适应未见生物环境的治疗效应模型，但现有单细胞扰动数据集覆盖的生物学环境有限，且现有方法无法在推理时利用新的干预证据来适应超出训练数据的场景

Method: 提出MapPFN（先验数据拟合网络），在从因果扰动先验生成的合成数据上进行预训练，利用上下文学习预测扰动后分布，无需基于梯度的优化

Result: 尽管仅使用计算机模拟的基因敲除数据进行预训练，MapPFN能够识别差异表达基因，其性能与在真实单细胞数据上训练的模型相当

Conclusion: MapPFN通过元学习和上下文学习的方法，为生物扰动效应建模提供了一种能够适应新环境且无需梯度优化的有效解决方案
...
### [50] [LoRA and Privacy: When Random Projections Help (and When They Don't)](https://arxiv.org/abs/2601.21719)
*Yaxi Hu,Johanna Düngler,Bernhard Schölkopf,Amartya Sanyal*

Main category: cs.LG

TL;DR: Wishart投影机制通过随机投影实现差分隐私，对向量查询无需加噪，但对矩阵查询存在隐私漏洞，低秩投影可增强隐私，LoRA更新属于该机制且不天然具有隐私性。


<details>
  <summary>Details</summary>
Motivation: 研究随机投影机制在差分隐私中的应用，探索是否可以通过随机性本身（而非加噪）实现隐私保护，特别关注低秩投影对隐私的增强效果及其在LoRA微调中的实际意义。

Method: 提出Wishart投影机制：S ↦ M f(S)，其中M服从Wishart分布。分析该机制对向量查询和矩阵查询的隐私性质，研究无噪和有噪两种情况，并探讨低秩投影的隐私放大效应。

Result: 向量查询：无需加噪即可实现差分隐私；矩阵查询：无噪情况下存在隐私漏洞（AUC > 0.99），有噪情况下低秩投影可增强隐私；LoRA更新属于该机制且不天然具有隐私性，但低秩微调比全参数微调更隐私。

Conclusion: 随机投影机制对向量查询有效，但对矩阵查询需要额外噪声；低秩投影可增强隐私保护；LoRA微调不天然具有隐私性，但低秩结构可提供隐私优势；更严格的隐私分析可降低噪声需求并提高准确性。
...
### [51] [Safety Generalization Under Distribution Shift in Safe Reinforcement Learning: A Diabetes Testbed](https://arxiv.org/abs/2601.21094)
*Minjae Kwon,Josephine Lamp,Lu Feng*

Main category: cs.LG

TL;DR: 研究揭示安全强化学习在训练时满足安全约束的算法，在部署到未见患者时存在安全泛化差距，而测试时屏蔽技术能有效恢复安全性


<details>
  <summary>Details</summary>
Motivation: 当前安全强化学习算法通常在固定训练条件下评估，但实际部署时可能面临分布偏移。研究者想探究训练时的安全保证是否能转移到分布偏移的部署环境中，使用糖尿病管理作为安全关键测试平台

Method: 在统一的临床模拟器上对8种安全RL算法进行基准测试，揭示安全泛化差距，并采用测试时屏蔽技术（使用学习到的动态模型过滤不安全动作）来恢复安全性

Result: 测试时屏蔽技术有效恢复了所有算法和患者群体的安全性，对于PPO-Lag和CPO等强基线，在时间范围内获得了13-14%的提升，同时降低了临床风险指数和血糖变异性

Conclusion: 安全强化学习算法在分布偏移下存在安全泛化差距，测试时屏蔽是有效的解决方案。研究提供的模拟器和基准平台可用于研究安全关键控制领域中的分布偏移安全问题
...
### [52] [Why Adam Works Better with $β_1 = β_2$: The Missing Gradient Scale Invariance Principle](https://arxiv.org/abs/2601.21739)
...
<details>
  <summary>Details</summary>
Motivation: Adam优化器已使用近十年，但一个经验事实未得到解释：当动量参数β₁=β₂时，验证分数和训练行为都会改善。虽然已有研究观察到这一模式，但缺乏理论解释。

Method: 形式化梯度尺度不变性的概念，从理论上证明Adam具有一阶梯度尺度不变性的充要条件是β₁=β₂。通过视觉和语言任务实验验证理论，比较不同β参数设置下梯度缩放对更新的影响。

Result: 实验证明当β₁=β₂时，梯度缩放对更新的影响更平滑，支持理论分析。这一发现将Adam的平衡机制与最近强调尺度鲁棒更新的优化器设计原则对齐。

Conclusion: β₁=β₂的选择与梯度尺度不变性密切相关，这解释了Adam优化器长期未解的经验现象，并为未来优化器设计提供了简单而重要的指导原则。
...
### [53] [TRACE: Trajectory Recovery for Continuous Mechanism Evolution in Causal Representation Learning](https://arxiv.org/abs/2601.21135)
*Shicheng Fan,Kun Zhang,Lu Cheng*

Main category: cs.LG

TL;DR: 提出TRACE方法，通过混合专家框架学习连续机制过渡的因果表示，解决了传统方法假设离散机制切换的局限性。
...
Method: 将过渡机制建模为有限原子机制的凸组合，由时变混合系数控制。提出TRACE混合专家框架，每个专家学习一个原子机制，测试时恢复机制轨迹。

Result: 在合成和真实数据上的实验表明，TRACE恢复混合轨迹的相关性高达0.99，显著优于离散切换基线方法。

Conclusion: 该方法能够联合识别潜在因果变量和连续混合轨迹，并能泛化到训练中未观察到的中间机制状态，为连续机制过渡的因果表示学习提供了有效框架。
...
### [54] [ECSEL: Explainable Classification via Signomial Equation Learning](https://arxiv.org/abs/2601.21789)
*Adia Lumadjeng,Ilker Birbil,Erman Acar*

Main category: cs.LG

TL;DR: ECSEL是一种可解释的分类方法，通过学习符号方程形式的正式表达式，既能分类又能提供解释，在符号回归基准上表现优于现有方法，同时保持可解释性。


<details>
  <summary>Details</summary>
Motivation: 许多符号回归基准具有紧凑的符号结构，但现有方法在恢复目标方程和计算效率方面存在不足。需要一种既能保持可解释性又能达到竞争性分类准确率的方法。

Method: ECSEL直接构建结构化的闭式表达式作为分类器和解释器，学习符号方程形式的正式表达式。该方法利用符号结构的高效性，在保持可解释性的同时实现分类功能。

Result: 在标准符号回归基准上，ECSEL比现有方法恢复更多目标方程，计算量显著减少。分类准确率与成熟机器学习模型相当，同时满足全局特征行为、决策边界分析和局部特征归因等理想特性。

Conclusion: ECSEL在基准数据集和电子商务、欺诈检测两个实际案例中证明，学习到的方程能够揭示数据集偏差、支持反事实推理并提供可操作的见解，实现了可解释性与性能的良好平衡。
...
### [55] [Smooth Dynamic Cutoffs for Machine Learning Interatomic Potentials](https://arxiv.org/abs/2601.21147)
*Kevin Han,Haolin Cong,Bowen Deng,Amir Barati Farimani*

Main category: cs.LG

TL;DR: 提出动态截断半径方法，替代传统固定截断半径，通过控制每个原子的邻居数量来减少内存消耗和加速推理，在多种先进MLIP模型上实现约2倍性能提升。


<details>
  <summary>Details</summary>
Motivation: 机器学习原子间势能（MLIPs）在分子动力学模拟中应用广泛，但面临推理时间和内存消耗两大瓶颈。传统方法采用固定截断半径限制了模拟规模，需要突破这一限制以实现更大规模的现实模拟。

Method: 首次提出动态截断半径公式，不再使用固定截断值，而是通过控制每个原子的目标邻居数量来诱导原子图的稀疏性。该方法在MACE、Nequip、Orbv3和TensorNet四种先进MLIP模型上实现。

Result: 动态截断方法显著降低内存消耗（最高2.26倍）和加速推理时间（最高2.04倍），具体提升取决于模型和原子系统。在材料和分子数据集上，与固定截断模型相比精度下降最小。

Conclusion: 动态截断半径方法成功解决了MLIPs的内存和推理瓶颈，挑战了固定截断半径的传统观念，实现了稳定、长时尺度的分子动力学模拟，所有实现和训练代码将开源。
...
### [56] [Low-Rank Plus Sparse Matrix Transfer Learning under Growing Representations and Ambient Dimensions](https://arxiv.org/abs/2601.21873)
*Jinhang Chai,Xuyuan Liu,Elynn Chen,Yujun Yan*

Main category: cs.LG

TL;DR: 论文提出了一种结构化矩阵估计的迁移学习框架，通过将源任务嵌入更高维目标任务，分解为目标参数为嵌入源组件、低维低秩创新和稀疏编辑，开发了锚定交替投影估计器。
...
Method: 提出通用迁移框架：目标参数分解为嵌入源组件、低维低秩创新和稀疏编辑；开发锚定交替投影估计器，在估计低维创新和稀疏修改的同时保留迁移子空间。

Result: 建立了确定性误差界限，分离目标噪声、表示增长和源估计误差，当秩和稀疏性增量较小时获得严格改进的速率；应用于马尔可夫转移矩阵估计和结构化协方差估计，在依赖噪声下获得端到端理论保证和实证验证的一致迁移增益。

Conclusion: 该迁移学习框架有效处理结构化矩阵估计中环境维度和内在表示的同时增长，通过分解参数结构和开发专用估计器，在源任务良好估计的前提下，能够获得改进的估计性能，适用于多种统计学习问题。
...
### [57] [Mobility-Embedded POIs: Learning What A Place Is and How It Is Used from Human Movement](https://arxiv.org/abs/2601.21149)
*Maria Despoina Siampou,Shushman Choudhury,Shang-Ling Hsu,Neha Arora,Cyrus Shahabi*

Main category: cs.LG

TL;DR: ME-POIs框架通过结合语言模型POI嵌入与大规模人类移动数据，学习基于实际使用模式的POI表示，在五个地图丰富化任务中优于纯文本或纯移动性基线。
...
Method: ME-POIs框架将语言模型生成的POI嵌入与大规模人类移动数据结合，通过对比学习将个体访问编码为时间上下文嵌入，并与可学习的POI表示对齐。针对长尾稀疏性问题，提出跨多个空间尺度从邻近频繁访问POI传播时间访问模式的机制。

Result: 在五个新提出的地图丰富化任务评估中，ME-POIs在增强文本嵌入方面始终优于纯文本和纯移动性基线。值得注意的是，仅使用移动数据训练的ME-POIs在某些任务上也能超越纯文本模型。

Conclusion: POI功能是准确且可泛化POI表示的关键组成部分，结合文本和移动数据的ME-POIs框架能够有效捕捉POI的身份和功能，为地理空间基础模型提供更全面的POI表示。
...
### [58] [Can Neural Networks Learn Small Algebraic Worlds? An Investigation Into the Group-theoretic Structures Learned By Narrow Models Trained To Predict Group Operations](https://arxiv.org/abs/2601.21150)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI在数学中的应用多局限于问答式范式，而真正的数学发现是开放式的。为了最大化AI在数学中的进展，需要超越简单问答，探索模型是否能从特定任务中学习到更广泛的数学结构

Method: 使用群运算预测作为基础测试案例，训练神经网络执行模运算或置换组合等群操作。设计一套测试来评估模型是否捕捉群论概念，如单位元、交换性、子群等

Result: 实验发现模型确实能学习到捕捉抽象代数性质的表示。例如，模型能捕捉模运算的交换性，并能训练线性分类器可靠地区分某些子群的元素。但无法提取单位元等概念

Conclusion: 研究结果表明，在某些情况下，即使是小型神经网络的表示也能用于从新的数学对象中提取有趣的抽象结构，为AI在数学发现中的更广泛应用提供了可能性
...
### [59] [Optimistic Transfer under Task Shift via Bellman Alignment](https://arxiv.org/abs/2601.21924)
*Jinhang Chai,Enpei Zhang,Elynn Chen,Yujun Yan*

Main category: cs.LG

TL;DR: 提出RWT方法，通过Bellman对齐解决在线迁移强化学习中的任务不匹配问题，实现统计上可靠的源数据重用


<details>
  <summary>Details</summary>
Motivation: 在线迁移强化学习中，源任务经验通常基于奖励或转移定义相似性，而在线RL算法基于Bellman回归目标操作，导致直接重用源Bellman更新会引入系统性偏差并破坏遗憾保证

Method: 提出重新加权目标(RWT)方法，通过一步Bellman对齐作为在线RL迁移的正确抽象，重新定位延续值并通过测度变化补偿转移不匹配，将任务不匹配减少到固定的一步校正

Result: 在RKHS函数逼近下建立了遗憾界限，其缩放与任务转移的复杂性而非目标MDP相关；在表格和神经网络设置中的实证结果均显示优于单任务学习和朴素池化的持续改进

Conclusion: Bellman对齐作为在线RL的模型无关迁移原则，通过RWT框架实现了方差减少与偏差校正的分离，为统计上可靠的源数据重用提供了理论基础
...
### [60] [Learning to Advect: A Neural Semi-Lagrangian Architecture for Weather Forecasting](https://arxiv.org/abs/2601.21151)
...
TL;DR: PARADIS提出了一种物理启发的全球天气预报模型，通过将网络功能分解为平流、扩散和反应块来施加归纳偏置，使用神经半拉格朗日算子处理平流，在较低训练成本下达到最先进的预测性能。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习天气预报模型采用单一架构，将不同物理机制（平流、传输、扩散混合、热力学过程等）隐式表示在一个大网络中，这种表示对平流处理尤其成问题，因为长距离传输需要昂贵的全局交互机制或深层卷积层。

Method: PARADIS通过功能分解将网络分为平流、扩散和反应块：1）使用神经半拉格朗日算子实现平流，通过球面上的可微分插值进行轨迹传输；2）通过深度可分离空间混合建模扩散过程；3）通过逐点通道交互建模局部源项和垂直相互作用。

Result: PARADIS以较低训练成本达到最先进的预测性能：1度分辨率模型总训练成本不到1个GPU月，在ERA5基准测试中达到或超过0.25度分辨率的传统和机器学习基线模型（包括ECMWF HRES和DeepMind的GraphCast）的性能。

Conclusion: 通过将物理结构融入网络架构的功能分解，PARADIS实现了高效的天气预报，在显著降低计算成本的同时保持或超越了现有最先进模型的性能，为物理启发的机器学习天气预报提供了有前景的方向。
...
### [61] [A Federated Generalized Expectation-Maximization Algorithm for Mixture Models with an Unknown Number of Components](https://arxiv.org/abs/2601.21160)
*Michael Ibrahim,Nagi Gebraeel,Weijun Xie*

Main category: cs.LG

TL;DR: FedGEM：一种联邦广义期望最大化算法，用于在联邦聚类中处理未知聚类数量K和客户端异构重叠聚类的问题。


<details>
  <summary>Details</summary>
Motivation: 联邦聚类中，客户端数据具有异构且可能重叠的聚类结构，且全局聚类数量K未知，现有方法难以有效处理这种复杂场景。

Method: 提出FedGEM算法：客户端本地执行EM步骤，为每个本地组件构建不确定性集合；服务器利用不确定性集合学习客户端间的聚类重叠，通过闭式计算推断全局聚类数量。

Result: 理论分析提供了概率收敛保证；针对各向同性GMM的具体设置，设计了低复杂度计算；实验表明性能与集中式EM相当，优于现有联邦聚类方法。

Conclusion: FedGEM有效解决了联邦聚类中未知聚类数量和异构重叠聚类的问题，具有理论保证和实际性能优势。
...
### [62] [Efficient Simple Regret Algorithms for Stochastic Contextual Bandits](https://arxiv.org/abs/2601.21167)
*Shuai Liu,Alireza Bakhtiari,Alex Ayoub,Botao Hao,Csaba Szepesvári*

Main category: cs.LG

TL;DR: 本文研究了随机上下文逻辑老虎机在简单遗憾目标下的表现，提出了首个达到$\tilde{\mathcal{O}}(d/\sqrt{T})$简单遗憾的确定性算法，以及基于Thompson Sampling的随机算法，其遗憾为$\tilde{\mathcal{O}}(d^{3/2}/\sqrt{T})$，且主要项不依赖于参数规模常数κ。


<details>
  <summary>Details</summary>
Motivation: 虽然线性上下文老虎机已有简单遗憾的理论保证，但逻辑回归设置下尚无类似结果。本文旨在填补这一空白，研究随机上下文逻辑老虎机在简单遗憾目标下的性能，并开发高效的算法。

Method: 结合上下文线性老虎机和自协调分析的思想，提出了首个确定性算法。同时，针对简单遗憾设置设计了新的Thompson Sampling变体，分别应用于线性逻辑老虎机。算法在有限动作集下完全可计算。

Result: 确定性算法达到$\tilde{\mathcal{O}}(d/\sqrt{T})$简单遗憾，随机算法达到$\tilde{\mathcal{O}}(d^{3/2}/\sqrt{T})$简单遗憾，两者主要项均不依赖于参数规模常数κ。随机算法计算成本更低，实验验证了理论保证。

Conclusion: 本文首次为逻辑回归老虎机提供了简单遗憾的理论保证，提出了高效算法，并展示了随机算法在计算效率上的优势，为实际应用提供了理论支持。
...
### [63] [AC2L-GAD: Active Counterfactual Contrastive Learning for Graph Anomaly Detection](https://arxiv.org/abs/2601.21171)
...
<details>
  <summary>Details</summary>
Motivation: 图异常检测面临标签稀缺和极端类别不平衡的挑战。现有图对比学习方法存在两个关键限制：随机增强破坏了正样本对的语义一致性，而简单的负采样产生无信息的简单对比。

Method: 提出AC2L-GAD框架，结合信息论主动选择和反事实生成：1）识别结构复杂节点；2）生成保持异常特征的正样本增强；3）生成正常负样本提供困难对比；4）仅对策略选择子集进行昂贵的反事实生成。

Result: 在9个基准数据集（包括GADBench的真实金融交易图）上，AC2L-GAD达到与最先进基线竞争或更优的性能，在异常表现出复杂属性-结构交互的数据集上表现尤为突出，同时计算开销比全图反事实生成减少约65%。

Conclusion: AC2L-GAD通过主动反事实对比学习有效解决了图异常检测中的语义一致性和对比质量问题，在保持检测性能的同时显著降低了计算成本，为处理复杂属性-结构交互的异常提供了有效解决方案。
...
### [64] [Breaking the Reasoning Horizon in Entity Alignment Foundation Models](https://arxiv.org/abs/2601.21174)
*Yuanning Cui,Zequn Sun,Wei Hu,Kexuan Xin,Zhangjie Fu*

Main category: cs.LG

TL;DR: 提出一种基于并行编码策略的实体对齐基础模型，通过种子对齐对作为局部锚点引导信息流，解决图基础模型在实体对齐任务中的推理范围差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有实体对齐模型缺乏可迁移性，无法对齐未见知识图谱而不需重新训练。虽然图基础模型提供了解决方案，但直接应用于实体对齐效果不佳，主要原因是存在"推理范围差距"：实体对齐需要捕获稀疏异构知识图谱结构中的长程依赖关系，而图基础模型中的链接预测任务则不需要。

Method: 采用并行编码策略，利用种子对齐对作为局部锚点引导信息流，同时初始化并编码两个并行流。通过锚点条件消息传递，利用局部结构邻近性而非全局搜索显著缩短推理轨迹。此外，引入合并关系图来建模全局依赖关系，以及可学习的交互模块进行精确匹配。

Result: 大量实验验证了该框架的有效性，突显了其对未见知识图谱的强大泛化能力。

Conclusion: 提出的基于并行编码策略的实体对齐基础模型成功解决了图基础模型在实体对齐任务中的推理范围差距问题，实现了对未见知识图谱的强泛化能力，为知识图谱融合提供了有效的解决方案。
...
### [65] [Flow Perturbation++: Multi-Step Unbiased Jacobian Estimation for High-Dimensional Boltzmann Sampling](https://arxiv.org/abs/2601.21177)
*Xin Peng,Ang Gao*
...
<details>
  <summary>Details</summary>
Motivation: 连续归一化流(CNFs)在高维系统中进行无偏Boltzmann采样时，由于Jacobian行列式计算需要D次反向传播，可扩展性受限。现有的随机Jacobian估计器如Hutchinson迹估计器虽然减少计算但引入偏差，而最近提出的Flow Perturbation方法无偏但方差高。

Method: 提出Flow Perturbation++方法，这是Flow Perturbation的方差减少扩展。该方法离散化概率流ODE，在每个积分步骤执行无偏的逐步Jacobian估计。这种多步构造保持了Flow Perturbation的无偏性，同时实现了显著更低的估计器方差。

Result: 在顺序蒙特卡洛框架中集成Flow Perturbation++，在1000维高斯混合模型和全原子Chignolin蛋白质上，相比基于Hutchinson和单步Flow Perturbation基线，实现了显著改进的平衡采样。

Conclusion: Flow Perturbation++通过多步离散化和逐步Jacobian估计，在保持无偏性的同时有效降低方差，为高维系统的无偏Boltzmann采样提供了更有效的解决方案。
...
### [66] [Rethinking Refinement: Correcting Generative Bias without Noise Injection](https://arxiv.org/abs/2601.21182)
*Xin Peng,Ang Gao*

Main category: cs.LG

TL;DR: 提出BFR框架，通过两阶段流匹配精炼方法校正生成模型的系统偏差，无需噪声注入或多步重采样，在保持原始ODE轨迹的同时提升样本质量。


<details>
  <summary>Details</summary>
Motivation: 生成模型（如扩散模型和基于流的模型）在高维设置中常表现出系统性偏差，这会降低样本质量。传统精炼方法通常需要噪声注入或多步重采样，我们希望开发一种无需这些操作的后处理偏差校正方法。

Method: 提出Bi-stage Flow Refinement (BFR)框架，包含两种精炼策略：1）针对近似可逆生成器的潜在空间对齐；2）使用轻量级增强训练的数据空间精炼。该方法保持原始ODE轨迹，对生成样本应用确定性校正。

Result: 在MNIST、CIFAR-10和FFHQ（256x256分辨率）上的实验显示，BFR能一致提升保真度和覆盖率。特别地，从FID 3.95的基础样本开始，潜在空间精炼在MNIST上仅使用1次额外函数评估（1-NFE）就达到了最先进的FID 1.46，同时保持了样本多样性。

Conclusion: BFR框架证明，有效的偏差校正可以作为后处理程序实现，无需扰动采样动态，为生成模型的质量提升提供了高效且保持多样性的解决方案。
...
### [67] [Rethinking Self-Training Based Cross-Subject Domain Adaptation for SSVEP Classification](https://arxiv.org/abs/2601.21203)
*Weiguang Wang,Yong Liu,Yingjie Gao,Guangyuan Xu*

Main category: cs.LG

TL;DR: 提出基于自训练的跨被试域适应方法，通过滤波器组欧几里得对齐和双集成自训练框架，结合时频增强对比学习，在SSVEP-BCI中实现最优性能。
...
Method: 1) 滤波器组欧几里得对齐(FBEA)利用SSVEP滤波器组的频率信息；2) 跨被试自训练(CSST)框架：预训练对抗学习(PTAL)对齐源域和目标域分布，双集成自训练(DEST)优化伪标签质量；3) 时频增强对比学习(TFA-CL)增强多增强视图的特征区分性。

Result: 在Benchmark和BETA数据集上的大量实验表明，该方法在不同信号长度下均达到最优性能，证明了其优越性。

Conclusion: 提出的跨被试域适应方法有效解决了SSVEP-BCI中的被试间变异性和标注成本问题，为BCI应用提供了高性能的解决方案。
...
### [68] [A Sheaf-Theoretic and Topological Perspective on Complex Network Modeling and Attention Mechanisms in Graph Neural Models](https://arxiv.org/abs/2601.21207)
*Chuan-Shen Hu*

Main category: cs.LG

TL;DR: 提出基于细胞层理论的框架，用于分析和建模图神经网络中节点特征和边权重的局部一致性与调和性，并提供多尺度扩展来捕捉层次特征交互。
...
Method: 引入细胞层理论框架，通过层结构跟踪局部特征对齐和一致性，提供特征扩散和聚合的拓扑视角。同时提出受拓扑数据分析启发的多尺度扩展，捕捉图模型中的层次特征交互。

Result: 该框架能够基于底层几何拓扑结构和定义在其上的学习信号，对GDL和TDL架构进行联合表征，为节点分类、子结构检测和社区检测等传统任务提供新的分析视角。

Conclusion: 细胞层理论为几何和拓扑深度学习提供了新的分析工具，能够深入理解特征扩散机制，并为未来研究提供了理论基础。
...
### [69] [Temporal Context and Architecture: A Benchmark for Naturalistic EEG Decoding](https://arxiv.org/abs/2601.21215)
*Mehmet Ergezer*

Main category: cs.LG

TL;DR: 研究EEG解码中模型架构与时间上下文的交互，在HBN电影观看数据集上比较5种架构，发现S5在长上下文下参数效率最高，EEGXF在鲁棒性方面表现更好，揭示了效率与鲁棒性的权衡。


<details>
  <summary>Details</summary>
Motivation: 探索在自然主义EEG解码任务中，不同模型架构如何与时间上下文长度相互作用，以及这些架构在现实场景中的鲁棒性表现。

Method: 使用HBN电影观看数据集，在4分类任务上比较CNN、LSTM、稳定化Transformer（EEGXF）、S4和S5五种架构，测试时间片段长度从8秒到128秒。评估零样本跨频率偏移、跨任务OOD输入和留一被试泛化能力。

Result: 随着上下文长度增加，准确率提升：在64秒时，S5达到98.7%±0.6，CNN达到98.3%±0.3，而S5参数数量比CNN少约20倍。S5在跨被试泛化方面更强，但在OOD任务上会做出过度自信的错误预测；EEGXF在频率偏移下更保守稳定，但在分布内校准较差。

Conclusion: 揭示了实用的效率-鲁棒性权衡：S5适合参数效率高的峰值准确率场景；EEGXF在需要鲁棒性和保守不确定性估计的关键应用中更合适。
...
### [70] [Soft Quantization: Model Compression Via Weight Coupling](https://arxiv.org/abs/2601.21219)
*Daniel T. Bernstein,Luca Di Carlo,David Schwab*

Main category: cs.LG

TL;DR: 提出一种通过权重间短程吸引耦合实现神经网络软量化的新方法，仅需两个超参数即可在训练中诱导权重分布离散化，并在ResNet-20/CIFAR-10上优于直方图均衡化后训练量化


<details>
  <summary>Details</summary>
Motivation: 现有模型量化方法通常需要在训练后进行处理，缺乏在训练过程中直接实现离散化的灵活方案。本文旨在探索一种新的量化途径，通过在训练中引入权重间的耦合作用，实现模型的软量化

Method: 在神经网络训练过程中引入权重间的短程吸引耦合，仅依赖两个超参数即可诱导权重分布离散化，实现混合精度量化。该方法被称为"软量化"方案

Result: 在适当超参数范围内，软量化方案在ResNet-20/CIFAR-10上表现优于直方图均衡化后训练量化。该方法能够实现权重分布的快速离散化

Conclusion: 软量化为机器学习模型压缩提供了新管道，并为研究高维损失空间中压缩与泛化权衡提供了新工具。该方法仅需两个超参数即可实现混合精度量化
...
### [71] [PHDME: Physics-Informed Diffusion Models without Explicit Governing Equations](https://arxiv.org/abs/2601.21234)
*Kaiyuan Tan,Kendra Givens,Peilun Li,Thomas Beckers*

Main category: cs.LG

TL;DR: PHDME：一种用于稀疏观测和不完整物理的端口哈密顿扩散框架，通过GP-dPHS学习能量表示，生成物理一致的人工数据训练扩散模型，实现数据稀缺下的准确预测。


<details>
  <summary>Details</summary>
Motivation: 扩散模型为动力系统轨迹预测提供了表达性先验，但在稀疏数据下不可靠。物理信息机器学习（PIML）可提高可靠性，但大多数方法需要训练时的显式控制方程，而复杂非线性动力学往往只有部分已知。

Method: 提出PHDME框架：1）在有限观测上训练高斯过程分布式端口哈密顿系统（GP-dPHS）捕捉基于能量的动力学表示；2）用GP-dPHS生成物理一致的人工数据集用于扩散训练；3）用结构化物理残差损失指导扩散模型；4）训练后扩散模型作为摊销采样器和预测器；5）应用分裂保形校准提供不确定性估计。

Result: 在PDE基准和真实弹簧系统上的实验显示，在数据稀缺条件下提高了准确性和物理一致性。

Conclusion: PHDME框架成功解决了稀疏观测和不完整物理条件下的动力系统预测问题，通过结合端口哈密顿结构先验和扩散模型，实现了数据稀缺下的可靠预测和不确定性量化。
...
### [72] [Understanding Diffusion Models via Ratio-Based Function Approximation with SignReLU Networks](https://arxiv.org/abs/2601.21242)
*Luwei Sun,Dongrui Shen,Jianfe Li,Yulong Zhao,Han Feng*

Main category: cs.LG

TL;DR: 本文为条件生成建模中的比率型泛函（f1/f2）提供了深度神经网络近似理论框架，特别针对扩散模型，使用SignReLU激活函数建立了近似界和收敛率。


<details>
  <summary>Details</summary>
Motivation: 条件生成建模中，目标条件密度常呈现为比率形式f1/f2，其中f1和f2是基于核的边际密度，捕捉结构化交互。这种设置在基于扩散的生成模型中至关重要，需要理论框架来近似这类比率型泛函。

Method: 使用SignReLU激活函数的深度神经网络来近似比率型泛函，利用该激活函数的分段结构特性。在标准正则性假设下，建立L^p(Ω)近似界和收敛率。特别针对DDPM，构建基于SignReLU的神经估计器用于反向过程。

Result: 建立了比率型泛函的近似理论框架，获得了L^p近似界和收敛率。针对DDPM，推导了生成分布与真实数据分布之间超额KL风险的界，将该风险分解为近似误差和估计误差分量。

Conclusion: 该理论框架为基于扩散的生成模型的有限样本训练提供了泛化保证，建立了深度神经网络近似比率型泛函的理论基础，特别适用于条件生成建模中的扩散模型。
...
### [73] [Less Noise, More Voice: Reinforcement Learning for Reasoning via Instruction Purification](https://arxiv.org/abs/2601.21244)
*Yiju Guo,Tianyi Hu,Zexu Sun,Yankai Lin*

Main category: cs.LG

TL;DR: LENS框架通过识别并移除干扰token来提升RLVR的采样效率，然后将净化后的成功rollout转移到原始噪声提示上进行策略优化，显著提升了性能和收敛速度。


<details>
  <summary>Details</summary>
Motivation: 现有的带可验证奖励的强化学习（RLVR）在有限rollout预算下存在探索效率低的问题，导致采样成功率低和训练不稳定。研究发现许多探索失败并非源于问题难度，而是由少量提示token引入的干扰造成的。
...
Conclusion: 通过剪枝干扰token可以显著改善RLVR的探索效率，为RLVR研究提供了新视角。LENS框架在复杂任务中实现了更高的性能和更快的收敛。
...
### [74] [Conditional Generative Framework with Peak-Aware Attention for Robust Chemical Detection under Interferences](https://arxiv.org/abs/2601.21246)
*Namkyung Yoon,Sanghong Kim,Hwangnam Kim*

Main category: cs.LG

TL;DR: 提出基于峰值感知条件生成模型的AI判别框架，用于提高GC-MS在干扰条件下的测量可靠性，通过生成模拟数据增强判别模型性能。


<details>
  <summary>Details</summary>
Motivation: GC-MS在存在干扰物质时测量可靠性下降，导致非特异性峰、保留时间偏移和背景噪声增加，从而降低灵敏度并产生误报。需要提高干扰条件下的GC-MS测量可靠性。

Method: 提出峰值感知条件生成模型框架：1) 峰值感知机制突出GC-MS数据的特征峰；2) 将化学和溶剂信息编码到潜在向量中；3) 使用条件生成对抗网络生成与实验条件一致的合成GC-MS信号；4) 生成模拟数据集用于训练AI判别模型。

Result: 生成数据验证显示余弦相似度和皮尔逊相关系数均高于0.9，同时保持峰值多样性并减少判别模型的误报。生成模型显著提升了AI判别框架的性能。

Conclusion: 提出的峰值感知条件生成模型框架能有效生成高质量的模拟GC-MS数据，显著提高干扰条件下化学物质判别的准确性和可靠性，为有限实验数据条件下的AI判别模型训练提供了有效解决方案。
...
### [75] [Model-Free Neural State Estimation in Nonlinear Dynamical Systems: A Comparative Study of Neural Architectures and Classical Filters](https://arxiv.org/abs/2601.21266)
*Zhuochen Liu,Hans Walker,Rahul Jain*

Main category: cs.LG

TL;DR: 神经网络状态估计器在非线性系统中接近强非线性卡尔曼滤波性能，且推理速度更快，尽管没有系统模型知识


<details>
  <summary>Details</summary>
Motivation: 神经网络模型越来越多地用于控制和决策中的状态估计，但尚不清楚它们在非线性动态系统中是否表现出有原则的滤波器行为。与依赖系统动力学和噪声模型显式知识的经典滤波器不同，神经网络估计器可以仅从数据中训练，无需访问底层系统方程。

Method: 对模型无关的神经网络模型和经典滤波方法进行系统性的实证比较，评估Transformer模型、状态空间神经网络、循环架构与粒子滤波和非线性卡尔曼滤波在多个非线性场景中的表现。

Result: 神经网络模型（特别是状态空间模型）在非线性场景中达到接近强非线性卡尔曼滤波的状态估计性能，并优于较弱的经典基线方法，同时获得显著更高的推理吞吐量。

Conclusion: 神经网络状态估计器在缺乏系统模型知识的情况下，能够实现接近经典滤波方法的性能，并且在计算效率方面具有优势，为实际应用提供了有前景的替代方案。
...
### [76] [EGAM: Extended Graph Attention Model for Solving Routing Problems](https://arxiv.org/abs/2601.21281)
*Licheng Wang,Yuzi Yan,Mingtao Huang,Yuan Shen*

Main category: cs.LG

TL;DR: 提出EGAM模型，扩展图注意力机制，在节点和边上同时使用多头点积注意力，在多种路由问题上达到或超越现有方法性能
...
Method: 提出扩展图注意力模型(EGAM)，使用多头点积注意力同时更新节点和边嵌入，采用自回归编码器-解码器架构，使用带有专门设计基线的策略梯度算法训练

Result: EGAM在多种路由问题上匹配或超越现有方法，特别是在高度约束问题上表现出色，能有效处理复杂图结构

Conclusion: EGAM通过扩展图注意力机制，在节点和边上同时使用注意力，显著提升了神经组合优化求解器在路由问题上的性能
...
### [77] [DUET: Distilled LLM Unlearning from an Efficiently Contextualized Teacher](https://arxiv.org/abs/2601.21283)
*Yisheng Zhong,Zhengbang Yang,Zhuangdi Zhu*

Main category: cs.LG

TL;DR: 提出DUET方法，通过蒸馏技术结合调优和上下文遗忘的优点，实现高效遗忘不良知识同时保持模型实用性


<details>
  <summary>Details</summary>
Motivation: 现有LLM遗忘方法存在局限：传统调优方法计算量大且易灾难性遗忘；上下文遗忘方法轻量但易受提示移除或逆向工程攻击

Method: 提出DUET（Distilled Unlearning from an Efficient Teacher），基于蒸馏的遗忘方法，学习学生模型模仿提示引导的教师行为，有效拒绝不良知识生成同时保留通用领域知识

Result: 在现有基准测试和增强评估协议下，DUET在遗忘和实用性保持方面表现更高，同时比最先进的遗忘方法数据效率高出数量级

Conclusion: DUET结合了调优和上下文遗忘的优点，实现了高效、安全的LLM遗忘，是迈向可信AI的重要进展
...
### [78] [PILD: Physics-Informed Learning via Diffusion](https://arxiv.org/abs/2601.21284)
...
TL;DR: PILD框架将扩散模型与物理约束统一，通过拉普拉斯分布的虚拟残差观测监督训练，并加入条件嵌入模块注入物理信息，显著提升工程科学问题的精度和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在建模复杂数据分布方面表现出色，但其纯数据驱动特性限制了在需要遵循物理定律的实际工程和科学问题中的应用。需要将物理先验知识整合到生成过程中。

Method: 提出PILD框架：1）引入从拉普拉斯分布采样的虚拟残差观测来监督训练；2）加入条件嵌入模块，在去噪网络的多层注入物理信息；3）框架简洁模块化，适用于常微分方程、偏微分方程、代数方程和不等式约束问题。

Result: 在车辆轨迹估计、轮胎力估计、达西流和等离子体动力学等多个工程科学任务上的实验表明，PILD在准确性、稳定性和泛化能力方面显著优于现有的物理信息方法和扩散模型基线。

Conclusion: PILD成功地将扩散模型与物理约束统一起来，为需要遵循物理定律的工程和科学问题提供了一个有效的生成建模框架，具有广泛的适用性和实际应用价值。
...
### [79] [Zenith: Scaling up Ranking Models for Billion-scale Livestreaming Recommendation](https://arxiv.org/abs/2601.21285)
...
TL;DR: Zenith是一个可扩展且高效的推荐排序架构，通过处理少量高维Prime Tokens来学习复杂特征交互，在TikTok Live部署中显著提升了推荐性能。
...
Method: 提出Zenith架构，通过Token Fusion和Token Boost模块处理少量高维Prime Tokens，实现复杂特征交互学习，同时保持最小运行时开销。

Result: 在TikTok Live平台部署的A/B测试显示，CTR AUC提升1.05%，Logloss降低1.10%，用户质量观看会话提升9.93%，质量观看时长提升8.11%。

Conclusion: Zenith通过改进token异质性展现出优于现有排序方法的扩展规律，在保持高效的同时显著提升了推荐系统的预测性能。
...
### [80] [TimeSliver : Symbolic-Linear Decomposition for Explainable Time Series Classification](https://arxiv.org/abs/2601.21289)
*Akash Pandey,Payal Mohapatra,Wei Chen,Qi Zhu,Sinan Keten*

Main category: cs.LG

TL;DR: TimeSliver是一个新颖的可解释性驱动深度学习框架，通过联合利用原始时间序列数据及其符号抽象来构建保持原始时间结构的表示，每个元素线性编码每个时间片段对最终预测的贡献，为时间序列分类提供可解释的重要性分数。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列可解释性方法存在局限性：基于梯度和特征归因的后验方法受参考状态敏感性影响，难以跨数据集泛化，且将时间点独立处理而忽略序列依赖；基于自注意力机制的方法虽然提供时间归因，但注意力权重往往无法提供忠实的时间重要性度量。

Method: TimeSliver框架联合利用原始时间序列数据及其符号抽象，构建保持原始时间结构的表示。该表示中的每个元素线性编码每个时间片段对最终预测的贡献，从而为每个时间点分配有意义的重要性分数。

Result: 在7个不同的合成和真实世界多元时间序列数据集上，TimeSliver在时间归因方面比其他方法高出11%；在26个UEA基准数据集上，预测性能与最先进基线方法相差在2%以内，成为强大且可解释的通用时间序列分类框架。

Conclusion: TimeSliver是一个强大且可解释的时间序列分类框架，既能提供忠实的时间重要性分数，又能保持接近最先进方法的预测性能，解决了现有可解释性方法的局限性。
...
### [81] [Physics-Guided Tiny-Mamba Transformer for Reliability-Aware Early Fault Warning](https://arxiv.org/abs/2601.21293)
*Changyu Li,Dingcheng Huang,Kexuan Yao,Xiaoya Ni,Lijuan Shen,Fei Luo*

Main category: cs.LG

TL;DR: PG-TMT：一种用于旋转机械故障预警的物理引导微型Mamba Transformer模型，通过三分支编码器、物理对齐和极值理论校准，在非平稳条件下实现高精度、可解释的早期故障检测。


<details>
  <summary>Details</summary>
Motivation: 旋转机械的可靠性预测需要在非平稳工况、域偏移（速度/负载/传感器）和严重类别不平衡下保持准确预警，同时控制误报率。现有方法难以同时满足这些要求。

Method: 提出PG-TMT三分支编码器：深度可分离卷积捕捉微瞬变，Tiny-Mamba状态空间分支建模近线性长程动态，轻量级局部Transformer编码跨通道共振。通过解析时频映射将注意力谱与经典轴承故障阶次带对齐，获得物理合理性评分。使用极值理论建模健康分数超限，实现目标误报强度，采用双阈值滞后和最小保持时间抑制抖动。

Result: 在CWRU、Paderborn、XJTU-SY和工业试点数据集上，PG-TMT在无泄漏流式协议下获得更高的精确率-召回率AUC（主要针对不平衡问题），竞争性或更好的ROC AUC，更短的检测平均时间（在相同误报强度下），并表现出强大的跨域迁移能力。

Conclusion: PG-TMT通过物理对齐表示和EVT校准决策规则的结合，为可靠性中心的预测和健康管理提供了校准、可解释且可部署的早期预警系统。
...
### [82] [Grounding and Enhancing Informativeness and Utility in Dataset Distillation](https://arxiv.org/abs/2601.21296)
*Shaobo Wang,Yantai Yang,Guo Chen,Peiru Li,Kaixin Li,Yufa Zhou,Zhaorun Chen,Linfeng Zhang*

Main category: cs.LG

TL;DR: 提出InfoUtil框架，通过博弈论信息度最大化和原则性效用最大化，在理论框架下优化数据集蒸馏，在ImageNet-1K上比SOTA提升6.1%


<details>
  <summary>Details</summary>
Motivation: 当前数据集蒸馏方法多依赖启发式方法，缺乏对原始数据与合成数据之间关系的深入理论探索，需要建立更坚实的理论基础来平衡效率与质量

Method: 提出InfoUtil框架，包含两个核心组件：1) 使用Shapley值进行博弈论信息度最大化，提取样本关键信息；2) 基于梯度范数的原则性效用最大化，选择全局有影响力的样本

Result: 在ImageNet-1K数据集上使用ResNet-18，相比之前的最佳方法实现了6.1%的性能提升

Conclusion: 通过建立信息度和效用的理论框架，InfoUtil能够合成既信息丰富又效用优化的蒸馏数据集，为数据集蒸馏提供了坚实的理论基础和有效的实践方法
...
### [83] [The Surprising Difficulty of Search in Model-Based Reinforcement Learning](https://arxiv.org/abs/2601.21306)
*Wei-Di Chang,Mikael Henaff,Brandon Amos,Gregory Dudek,Scott Fujimoto*

Main category: cs.LG

TL;DR: 该论文挑战了模型强化学习中搜索能直接替代学习策略的传统观点，发现即使模型高度准确，搜索也可能损害性能，而缓解分布偏移比提高模型或价值函数精度更重要。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为模型强化学习的主要障碍是长期预测和误差累积，但作者质疑这一观点，想探究搜索在模型强化学习中的实际作用和局限性。

Method: 通过实验分析搜索在模型强化学习中的效果，发现搜索不是学习策略的即插即用替代品，并基于缓解分布偏移的洞察，识别出实现有效搜索的关键技术。

Result: 研究发现搜索可能损害性能，即使模型高度准确；缓解分布偏移比提高模型或价值函数精度更重要；通过提出的关键技术，在多个流行基准领域实现了最先进的性能。

Conclusion: 模型强化学习成功的关键不是改进模型或价值函数精度，而是有效管理分布偏移，这为设计更有效的模型强化学习算法提供了新方向。
...
### [84] [Transferable Graph Condensation from the Causal Perspective](https://arxiv.org/abs/2601.21309)
...
<details>
  <summary>Details</summary>
Motivation: 现有图数据集压缩方法要求下游应用与原始数据集和任务严格匹配，这在跨任务和跨域场景中往往失效。需要开发能够保持因果信息且具有良好迁移能力的压缩方法。

Method: 1) 通过因果干预从图的空间域提取领域因果不变特征；2) 执行增强压缩操作以充分捕捉原始图的结构和特征信息；3) 通过谱域增强对比学习将因果不变特征注入压缩图中。

Result: 在5个公共数据集和新的FinReport数据集上，TGCC在跨任务和跨域复杂场景中相比现有方法提升达13.41%，在单数据集和任务场景中在6个数据集中的5个上达到最先进性能。

Conclusion: TGCC通过因果不变性特征提取和增强对比学习，成功创建了既高效又可迁移的压缩图数据集，解决了现有方法在跨任务和跨域场景中的局限性。
...
### [85] [Few-Shot Learning for Dynamic Operations of Automated Electric Taxi Fleets under Evolving Charging Infrastructure: A Meta-Deep Reinforcement Learning Approach](https://arxiv.org/abs/2601.21312)
...
Method: 提出GAT-PEARL元强化学习框架：1）使用图注意力网络提取鲁棒的空间表示并建模复杂时空关系；2）采用概率嵌入的actor-critic强化学习，实现无需重新训练的快速推理适应

Result: 在中国成都真实数据上的广泛仿真表明，GAT-PEARL显著优于传统强化学习方法，在未见过的基础设施布局上表现出更好的泛化能力，在动态环境中实现更高的整体运营效率

Conclusion: GAT-PEARL框架成功解决了动态充电网络下的自动驾驶电动出租车车队管理问题，通过元强化学习方法实现了对基础设施布局变化的快速适应，为实际应用提供了有效解决方案
...
### [86] [Distributionally Robust Classification for Multi-source Unsupervised Domain Adaptation](https://arxiv.org/abs/2601.21315)
...
Method: 提出一种分布鲁棒学习框架，同时建模协变量分布和条件标签分布的不确定性。该方法最初受多源域适应启发，但也适用于单源场景，并能与现有UDA方法无缝集成。

Result: 在各种分布偏移场景下的广泛实验表明，该方法始终优于强基线方法，特别是在目标数据极其稀缺的情况下表现尤为突出。

Conclusion: 提出的分布鲁棒学习框架有效解决了无监督域适应中的不确定性挑战，特别是在数据稀缺和虚假相关性场景下表现出优越性能，具有实际应用价值。
...
### [87] [Heterogeneous Vertiport Selection Optimization for On-Demand Air Taxi Services: A Deep Reinforcement Learning Approach](https://arxiv.org/abs/2601.21316)
*Aoyu Pang,Maonan Wang,Zifan Sha,Wenwei Yue,Changle Li,Chung Shue Chen,Man-On Pun*

Main category: cs.LG

TL;DR: 提出统一空陆交通协调框架UAGMC，结合深度强化学习和V2X通信优化空陆联运，相比传统方法减少34%平均出行时间。


<details>
  <summary>Details</summary>
Motivation: 城市空中交通（UAM）能缓解城市拥堵，但需要与地面交通紧密集成。目前对空陆联运系统乘客最优路径策略的研究有限，缺乏系统探索。

Method: 提出统一优化模型整合空陆交通策略选择，考虑多式联运网络动态特性和实时交通状况。基于此提出UAGMC框架，利用深度强化学习和V2X通信优化垂直起降机场选择和动态规划空中出租车路线。

Result: 实验结果显示UAGMC相比传统比例分配方法减少34%平均出行时间，提升整体出行效率，为空陆多式联运系统集成优化提供新见解。

Conclusion: 该工作为空陆交通模式协调推进智能城市出行解决方案奠定坚实基础，相关代码已开源。
...
### [88] [Adversarial Vulnerability Transcends Computational Paradigms: Feature Engineering Provides No Defense Against Neural Adversarial Transfer](https://arxiv.org/abs/2601.21323)
*Achraf Hsain,Ahmed Abdelkader,Emmanuel Baldwin Mbaya,Hamoud Aljamaan*

Main category: cs.LG

TL;DR: 对抗性攻击不仅影响深度神经网络，还能通过特征提取（如HOG）转移到传统机器学习分类器上，且FGSM攻击比PGD更有效，表明对抗性脆弱性是图像分类系统的固有属性。


<details>
  <summary>Details</summary>
Motivation: 研究对抗性攻击是否仅局限于端到端可微的深度神经网络，还是能通过特征工程（如HOG特征提取）转移到传统机器学习分类器上。传统观点认为特征工程可能通过梯度量化和空间分箱过滤高频对抗信号，从而提供保护。

Method: 使用VGG16作为代理模型生成FGSM和PGD对抗样本，测试这些样本对四种传统分类器（KNN、决策树、线性SVM、核SVM）和浅层神经网络的转移攻击效果。在CIFAR-10数据集上评估了八种HOG配置。

Result: 所有传统分类器都遭受了16.6%-59.1%的相对准确率下降，与神经网络间的转移攻击相当。令人惊讶的是，FGSM攻击在100%的传统ML案例中比PGD造成更大性能下降，这与神经网络中PGD通常更强的模式相反。块归一化提供部分缓解但不足够。

Conclusion: 对抗性脆弱性不是端到端可微性的产物，而是图像分类系统的固有属性。特征工程不能提供足够的保护，这对跨计算范式的安全关键部署具有重要意义。
...
### [89] [An introductory Generalization of the standard SVMs loss and its applications to Shallow and Deep Neural Networks](https://arxiv.org/abs/2601.21331)
*Filippo Portera*

Main category: cs.LG

TL;DR: 提出一种用于SVM（支持向量机）的新型凸损失函数，适用于二分类和回归任务，通过引入模式相关性提升泛化性能


<details>
  <summary>Details</summary>
Motivation: 传统SVM损失函数未充分利用数据模式间的相关性信息，希望通过在损失函数中融入模式相关性来提升模型的泛化能力

Method: 提出新的凸损失函数，推导了相应的对偶问题，在小数据集上进行实验验证，并探索了与浅层和深层神经网络结合的可能性

Result: 新损失函数的泛化性能从不差于标准损失函数，多数情况下表现更好，初步验证了模式相关性对提升泛化能力的有效性

Conclusion: 新损失函数具有潜力，值得进一步研究，特别是与神经网络架构结合，但受限于SVM的可扩展性问题，目前只能在小数据集上验证
...
### [90] [Memorization Control in Diffusion Models from Denoising-centric Perspective](https://arxiv.org/abs/2601.21348)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注数据或模型层面的修改，将扩散模型视为孤立预测器。本文从去噪中心化视角研究扩散模型的记忆化问题，发现均匀时间步采样会导致不同去噪步骤学习贡献不均，从而偏向记忆化

Method: 提出一种时间步采样策略，明确控制在去噪轨迹上的学习位置。通过调整置信区间宽度，直接控制记忆化与泛化的权衡

Result: 在图像和1D信号生成任务上的实验表明，将学习重点转向后期去噪步骤能持续减少记忆化，并改善与训练数据的分布对齐

Conclusion: 验证了该方法的通用性和有效性，为控制扩散模型记忆化提供了新的去噪中心化视角
...
### [91] [L2R: Low-Rank and Lipschitz-Controlled Routing for Mixture-of-Experts](https://arxiv.org/abs/2601.21349)
*Minghao Yang,Ren Togo,Guang Li,Takahiro Ogawa,Miki Haseyama*

Main category: cs.LG

TL;DR: L2R提出了一种新的MoE路由框架，通过低秩潜在空间和饱和内积评分来解决传统线性路由在表示匹配、角度集中和尺度敏感评分方面的问题，提高路由稳定性和专家专业化。


<details>
  <summary>Details</summary>
Motivation: 现代MoE系统仍采用原始高维表示空间中的线性路由器，存在表示不匹配、角度集中和尺度敏感评分等问题，这些共同削弱了路由判别能力和稳定的专家专业化。

Method: L2R框架包含：1）在共享低秩潜在路由空间进行专家分配；2）引入饱和内积评分（SIPS）控制路由函数的Lipschitz行为；3）参数高效的多锚点路由机制增强专家表达能力。

Result: 在大规模语言MoE模型和ImageNet上的视觉MoE设置中，L2R一致提高了路由稳定性、专家专业化和整体模型性能。
...
### [92] [Factored Causal Representation Learning for Robust Reward Modeling in RLHF](https://arxiv.org/abs/2601.21350)
*Yupei Yang,Lin Yang,Wanxi Deng,Lin Qu,Fan Feng,Biwei Huang,Shikui Tu,Lei Xu*

Main category: cs.LG

TL;DR: 提出一种基于因果视角的分解表示学习框架，将上下文嵌入分解为因果因素和非因果因素，约束奖励头仅依赖因果成分，并通过对抗训练防止非因果因素编码奖励相关信息，从而学习更稳健的奖励模型并改善RLHF性能。
...
Method: 提出分解表示学习框架：1）将模型上下文嵌入分解为因果因素（足以预测奖励）和非因果因素（捕获奖励无关属性如长度或谄媚偏见）；2）约束奖励头仅依赖因果成分；3）引入对抗头从非因果因素预测奖励，同时应用梯度反转防止其编码奖励相关信息。

Result: 在数学和对话任务上的实验表明，该方法能学习更稳健的奖励模型，并持续改善下游RLHF性能，优于最先进的基线方法。对长度和谄媚偏见的分析进一步验证了该方法在缓解奖励黑客行为方面的有效性。

Conclusion: 从因果视角出发的分解表示学习框架能有效解决奖励模型中的虚假特征问题，提高奖励模型的鲁棒性，从而改善RLHF对齐效果。
...
### [93] [Theoretically Optimal Attention/FFN Ratios in Disaggregated LLM Serving](https://arxiv.org/abs/2601.21351)
*Chendong Song,Meixuan Wang,Hang Zhou,Hong Liang,Yuan Lyu,Zixi Chen,Yuwei Fan,Zijie Zhou*

Main category: cs.LG

TL;DR: AFD架构分离注意力与FFN计算，但性能对资源配比敏感。本文建立分析框架，为rA-1F拓扑推导最优A/F比例，最大化系统吞吐量。


<details>
  <summary>Details</summary>
Motivation: AFD架构虽然能独立扩展内存和计算资源，但其性能对注意力与FFN的资源配比高度敏感。配比不当会导致步骤级阻塞和设备空闲时间，影响系统效率。

Method: 建立可处理的AFD捆绑资源配比分析框架，针对rA-1F拓扑，使用概率工作负载模型推导最优A/F比例的闭式规则。通过轨迹校准的AFD模拟器验证理论。

Result: 理论推导的最优A/F比例与模拟最优值在10%误差内匹配，能持续减少空闲时间，验证了分析框架的有效性。

Conclusion: 本文提出的分析框架能有效确定AFD架构的最优资源配比，解决注意力侧非平稳工作负载与FFN稳定工作负载的协调问题，提升系统吞吐量。
...
### [94] [Expected Improvement via Gradient Norms](https://arxiv.org/abs/2601.21357)
*Joshua Hang Sai Ip,Georgios Makrygiorgos,Ali Mesbah*

Main category: cs.LG

TL;DR: 提出EI-GN，一种基于梯度范数的期望改进获取函数，通过梯度感知辅助目标促进在高性能且接近一阶平稳点的区域采样，提升贝叶斯优化性能。


<details>
  <summary>Details</summary>
Motivation: 传统期望改进(EI)获取函数在贝叶斯优化中存在过度利用倾向，容易收敛到次优平稳点。需要一种既能利用高性能区域信息，又能考虑梯度信息以更好探索优化空间的获取函数。

Method: 提出EI-GN获取函数，将改进原则应用于梯度感知的辅助目标。使用梯度观测学习梯度增强的代理模型，实现从函数评估中获取原则性梯度推断。推导出可处理的闭式表达式，支持高效优化。

Result: 在标准贝叶斯优化基准测试中，EI-GN相比基线方法获得了一致的性能提升。在控制策略学习问题上也展示了良好的适用性。

Conclusion: EI-GN是一种新颖的获取函数，通过结合梯度信息改进了传统EI的局限性，在保持改进原则框架一致性的同时，提升了贝叶斯优化的效率和效果。
...
### [95] [Graph-Free Root Cause Analysis](https://arxiv.org/abs/2601.21359)
*Luan Pham*

Main category: cs.LG

TL;DR: PRISM是一个无需依赖图的根因分析框架，通过考虑故障传播中的延迟累积效应，显著提升诊断准确率。


<details>
  <summary>Details</summary>
Motivation: 现有无需依赖图的RCA方法通常假设根因具有最高异常分数，但当故障传播时，根因的小延迟可能在下游累积成更大的异常，导致该假设失效。

Method: 提出PRISM框架，针对一类基于组件的系统，在无依赖图情况下进行根因分析，并提供理论保证。

Result: 在9个真实数据集上的735个故障测试中，PRISM达到68%的Top-1准确率，比最佳基线提升258%，每次诊断仅需8毫秒。

Conclusion: PRISM是一个简单高效的根因分析框架，在无依赖图情况下通过考虑故障传播特性，显著优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 将Transformer前向传播建模为单位球面上的交互粒子系统，其中时间对应层数，粒子对应词嵌入，单位球面理想化层归一化。研究感知机块在这种几何框架下的影响

Method: 将Transformer前向传播视为单位球面上的粒子系统，在某些权重设置下可视为显式能量的梯度流，利用Wasserstein梯度流处理无限上下文长度（平均场）极限，分析感知机块的作用

Result: 发现临界点通常是原子化的（atomic），并且集中在球面的特定子集上，表现出局部化特征

Conclusion: 在Transformer的几何框架下，感知机块导致系统临界点呈现原子化和局部化特征，这为理解Transformer的动力学行为提供了新的理论视角
...
TL;DR: FedGALA：联邦图与语言对齐框架，通过无监督对比学习在连续嵌入空间对齐GNN与冻结PLM，解决联邦图基础模型中语义-结构正交性与完整性问题，避免量化知识损失，并通过高效提示调优适应下游任务。


<details>
  <summary>Details</summary>
Motivation: 现有联邦图基础模型(FedGFMs)通过向量量化将可泛化知识投影到离散标记空间，导致量化过程中的不可逆知识损失。需要解决图神经网络(GNNs)与预训练语言模型(PLMs)之间的语义-结构正交性与完整性，同时应对分布式环境中严重的数据异构性和通信约束。

Method: 1) 采用无监督对比学习在连续嵌入空间对齐GNN与冻结的PLM，捕获鲁棒可迁移的通用知识；2) 利用通信高效的提示调优机制引导预对齐编码器和冻结PLM，适应多样下游任务，避免全参数微调的高昂开销。
...
TL;DR: 提出DA-SPS模型，通过双阶段处理（目标变量和外部变量）结合SSA分解、LSTM、P-Conv-LSTM和注意力机制，提升多元时间序列预测精度。


<details>
  <summary>Details</summary>
Motivation: 现有多元时间序列预测方法存在两个主要问题：1) 未能有效考虑外部变量对目标变量的影响；2) 未能充分提取基于不同时间模式的复杂序列信息。这些问题限制了预测性能。

Method: DA-SPS模型采用双阶段架构：目标变量处理阶段(TVPS)使用SSA分解序列为趋势和季节性分量，分别用LSTM和P-Conv-LSTM提取特征；外部变量处理阶段(EVPS)用Spearman相关性分析筛选相关变量，再用L-Attention模块(LSTM+注意力机制)分析。最后加权求和并线性映射得到最终预测。

Result: 在四个公共数据集上，DA-SPS模型超越了现有最先进方法。在自收集的笔记本电脑主板测试项目私有数据集上进一步验证了其在实际场景中的有效性。

Conclusion: DA-SPS模型通过有效处理目标变量和外部变量，充分提取复杂时间模式信息，显著提升了多元时间序列预测性能，并在实际工业应用中表现出色。
...
TL;DR: 提出Sim-MSTNet，一种基于sim2real方法的多任务时空网络流量预测模型，通过生成合成数据解决数据稀缺问题，使用领域随机化减少分布差距，并采用注意力机制和动态损失加权来平衡任务。
...
Method: 1) 使用模拟器生成合成数据解决数据稀缺问题；2) 采用领域随机化技术，通过双层优化（样本加权和模型训练）减少合成与真实数据分布差距；3) 引入基于注意力的机制选择性共享任务知识；4) 应用动态损失加权平衡任务目标。

Result: 在两个开源数据集上的大量实验表明，Sim-MSTNet持续优于最先进的基线方法，实现了更高的准确性和泛化能力。

Conclusion: Sim-MSTNet通过sim2real方法有效解决了数据稀缺和任务不平衡问题，在多任务网络流量预测中表现出优越性能。
...
### [100] [Learning to Optimize Job Shop Scheduling Under Structural Uncertainty](https://arxiv.org/abs/2601.21389)
*Rui Zhang,Jianwei Niu,Xuefeng Liu,Shaojie Tang,Jing Yuan*

Main category: cs.LG

TL;DR: 提出UP-AAC方法解决作业车间调度问题中的结构不确定性，采用非对称架构和注意力不确定性感知模型，在基准实例上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注参数不确定性（如处理时间变化），而本文探索作业车间调度中普遍存在的结构不确定性——作业可能遵循多条路径，选择由无法预先知道的情境因素决定。现有方法因信用分配问题而难以应对此挑战。

Method: 提出UP-AAC方法：1）采用非对称架构，演员接收标准随机状态，而评论家接收事后重建的确定性状态；2）设计基于注意力的不确定性感知模型（UPM）增强演员的调度决策；3）评论家学习更准确的价值函数，为演员提供低方差策略梯度。

Result: 在基准实例上的大量实验表明，该方法在减少完工时间方面优于现有方法。

Conclusion: UP-AAC方法有效解决了作业车间调度中的结构不确定性挑战，通过非对称架构和不确定性感知模型实现了更稳定的学习和更好的调度性能。
...
### [101] [Intrinsic Reward Policy Optimization for Sparse-Reward Environments](https://arxiv.org/abs/2601.21391)
...
<details>
  <summary>Details</summary>
Motivation: 在强化学习中，探索至关重要，但在稀疏奖励环境下，简单的探索策略（如噪声注入）往往不足。现有的内在奖励方法存在两个问题：1）与外在奖励结合优化策略时存在不稳定的信用分配问题；2）用于训练分层学习的子策略时存在样本效率低和次优性问题。

Method: 提出内在奖励策略优化（IRPO）框架，利用多个内在奖励直接优化外在奖励的策略，无需预训练子策略。通过替代策略梯度在稀疏奖励环境中提供比真实梯度更丰富的学习信号。

Result: IRPO在离散和连续环境中相比基线方法提高了性能和样本效率，并对IRPO解决的优化问题进行了形式化分析。

Conclusion: IRPO提供了一种有效利用内在奖励进行策略优化的新方法，解决了现有方法在稀疏奖励环境中的局限性，在性能和样本效率方面都有显著提升。
...
### [102] [Mitigating Overthinking in Large Reasoning Models via Difficulty-aware Reinforcement Learning](https://arxiv.org/abs/2601.21418)
*Qian Wan,Ziao Xu,Luona Wei,Xiaoxuan Shen,Jianwen Sun*

Main category: cs.LG

TL;DR: 提出DiPO框架，通过强化学习让大型推理模型能够感知任务难度，自动调整推理深度，减少简单任务中的过度思考现象


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在处理简单任务时存在"过度思考"问题，导致不必要的冗长推理和资源浪费。现有研究低估了任务难度感知的重要性，难以有效分配推理资源
...
Conclusion: DiPO框架通过强化学习让LRM能够感知任务难度并调整生成偏好，有效解决了过度思考问题，提高了推理效率
...
### [103] [Revisiting Diffusion Model Predictions Through Dimensionality](https://arxiv.org/abs/2601.21419)
*Qing Jin,Chaoyang Wang*

Main category: cs.LG

TL;DR: 论文提出k-Diff框架，通过数据驱动方式学习最优预测目标参数k，无需显式维度估计，在图像生成任务中优于固定目标基线方法。


<details>
  <summary>Details</summary>
Motivation: 扩散和流匹配模型中，预测目标从噪声(ε)、速度(v)转向直接数据(x)预测，但缺乏理论解释为何最优目标取决于数据特性。需要建立理论框架解释数据几何特性与最优预测目标的关系。

Method: 提出广义预测公式框架，涵盖任意输出目标（ε、v、x预测为特例）。理论分析数据几何与最优预测目标的关系，并提出k-Diff框架，通过数据驱动方式学习最优预测参数k，无需显式估计内在维度。

Result: 理论证明当环境维度显著超过数据内在维度时，x预测更优。实验表明k-Diff在潜在空间和像素空间图像生成中，在不同架构和数据规模下均优于固定目标基线方法。

Conclusion: 维度是决定最优预测目标的关键因素，但内在维度难以估计。k-Diff提供了一种原则性、自动化的方法，通过数据驱动学习最优预测参数，提升生成性能。
...
### [104] [ConceptMoE: Adaptive Token-to-Concept Compression for Implicit Compute Allocation](https://arxiv.org/abs/2601.21420)
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型对所有token分配均匀计算，忽略了有些序列容易预测而有些需要深度推理的问题。需要一种能动态调整计算分配的方法来提高效率和性能。

Method: 引入ConceptMoE：1) 可学习的chunk模块通过测量token间相似性识别最优边界，按目标压缩比R压缩序列；2) MoE架构实现受控评估，重新分配节省的计算以匹配基线FLOPs；3) 在持续训练中通过layer looping转换预训练MoE

Result: 在语言和视觉语言任务上一致优于标准MoE：语言预训练+0.9分，长上下文理解+2.3分，多模态基准+0.6分。持续训练时增益达+5.5分。R=2时，注意力计算减少R²倍，KV缓存减少R倍，预填充速度提升175%，解码速度提升117%

Conclusion: 自适应概念级处理从根本上改善了大语言模型的效果和效率，最小架构修改使其易于集成到现有MoE中，展示了计算分配的优化潜力
...
### [105] [Lossy Common Information in a Learnable Gray-Wyner Network](https://arxiv.org/abs/2601.21424)
*Anderson de Andrade,Alon Harell,Ivan V. Bajić*

Main category: cs.LG

TL;DR: 提出基于Gray-Wyner网络的可学习三通道编解码器，分离多视觉任务中的共享信息和任务特定信息，减少表示冗余，提升编码效率。


<details>
  <summary>Details</summary>
Motivation: 传统编解码器忽略计算机视觉任务间大量重叠信息，导致表示冗余和低效。Gray-Wyner网络提供分离公共和任务特定信息的理论框架。

Method: 开发可学习三通道编解码器，通过损失公共信息概念表征方法极限，提出平衡学习表示中固有权衡的优化目标。在三个编解码器架构和六个视觉基准上进行比较。

Result: 该方法显著减少冗余，在双任务场景中持续优于独立编码，证明Gray-Wyner理论在现代机器学习中的实用价值。

Conclusion: 将经典信息理论与任务驱动表示学习相结合，为多任务视觉表示提供高效编码框架，验证Gray-Wyner理论在现代机器学习中的适用性。
...
### [106] [From Consistency to Complementarity: Aligned and Disentangled Multi-modal Learning for Time Series Understanding and Reasoning](https://arxiv.org/abs/2601.21436)
*Hang Ni,Weijia Zhang,Fei Wang,Zezhi Shao,Hao Liu*

Main category: cs.LG

TL;DR: MADI是一个多模态大语言模型，通过细粒度对齐和解耦交互增强时间序列理解，在合成和真实世界基准测试中优于通用LLM和专用MLLM。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在时间序列理解中面临跨模态细粒度时间对齐困难和共享/模态特定语义纠缠问题，这阻碍了局部解释和互补推理。

Method: 提出MADI模型，包含三个关键组件：1) 补丁级对齐，建立跨异构模态的物理基础细粒度对应；2) 离散解耦交互，将模态共同语义分离为紧凑离散潜在表示，并自适应协同纯化的模态独特信息；3) 关键令牌高亮，强调信息丰富、查询相关的信号以进行鲁棒推理。

Result: 在合成和真实世界基准测试中，MADI一致性地优于通用大语言模型和时间序列专用多模态大语言模型。

Conclusion: MADI通过细粒度对齐和解耦交互有效解决了时间序列多模态理解中的跨模态集成挑战，实现了更精确的时间序列理解和推理。
...
### [107] [Accurate Network Traffic Matrix Prediction via LEAD: an LLM-Enhanced Adapter-Based Conditional Diffusion Model](https://arxiv.org/abs/2601.21437)
...
<details>
  <summary>Details</summary>
Motivation: 面向6G和AI原生边缘智能，网络运营需要预测性和风险感知的适应能力。流量矩阵预测面临挑战：现有判别模型存在过平滑问题，对不确定性感知有限，在极端突发流量下表现不佳。

Method: 1) "流量转图像"范式：将流量矩阵转换为RGB图像，利用视觉骨干网络建模全局依赖；2) "冻结LLM+可训练适配器"：高效捕获时序语义，计算成本低；3) 双条件策略：精确指导扩散模型生成复杂动态流量矩阵。

Result: 在Abilene数据集上，RMSE比最佳基线降低45.2%，20步预测误差仅从0.1098微增至0.1134；在GEANT数据集上，20步预测RMSE为0.0258，比最佳基线降低27.3%。

Conclusion: LEAD通过创新的LLM增强扩散模型架构，显著提升了网络流量矩阵预测的准确性和不确定性感知能力，为6G和AI原生边缘智能下的网络运营提供了有效的预测解决方案。
...
### [108] [Synthetic Pattern Generation and Detection of Financial Activities using Graph Autoencoders](https://arxiv.org/abs/2601.21446)
*Francesco Zola,Lucia Muñoz,Andrea Venturi,Amaia Gil*

Main category: cs.LG

TL;DR: 研究探索图自编码器在合成数据上学习洗钱模式的能力，发现GAE-GCN在多种模式上表现最稳定。


<details>
  <summary>Details</summary>
Motivation: 洗钱等非法金融活动在交易网络中呈现重复的拓扑模式，但由于真实标注数据稀缺和隐私限制，自动检测这些模式具有挑战性。

Method: 采用两阶段方法：1) 数据生成阶段，使用参数化生成器创建7种已知非法活动模式的合成样本；2) 模型训练验证阶段，为每个模式训练单独的图自编码器，仅依赖重构误差作为学习结构的指标，比较了基于GCN、GraphSAGE和GAT的三种GAE实现。
...
### [109] [SAGE: Sequence-level Adaptive Gradient Evolution for Generative Recommendation](https://arxiv.org/abs/2601.21452)
*Yu Xie,Xing Kai Ren,Ying Qi,Hu Yao*

Main category: cs.LG

TL;DR: 本文提出SAGE优化框架，解决推荐系统中LLM重用困难和OneRec GBPO的对称保守主义问题，通过序列级信号解耦和不对称自适应动态，提升冷启动性能和推荐多样性。


<details>
  <summary>Details</summary>
Motivation: 现有工作如OneRec验证了LLM在推荐系统中的扩展定律，但依赖独立的词汇表，导致无法重用原生LLM架构，维护成本高且扩展性差。同时，OneRec的GBPO优化策略存在"对称保守主义"问题，静态梯度边界抑制了冷启动项目所需的更新动量，且无法防止高噪声环境中的多样性崩溃。

Method: 提出SAGE（序列级自适应梯度演化）统一优化框架：1）序列级信号解耦：结合几何平均重要性比率和解耦的多目标优势，消除token级方差，解决"奖励崩溃"问题；2）不对称自适应动态：构建动态梯度流形，对高潜力冷启动项目应用"提升因子"实现超线性更新，并使用"熵感知惩罚"打破信息茧房。

Result: 理论分析和实证结果表明，SAGE有效释放了冷启动流量并维持了推荐多样性，同时保持了GBPO的数值稳定性。

Conclusion: SAGE框架成功解决了LLM在推荐系统中的架构重用问题和现有优化策略的局限性，为列表式生成推荐提供了有效的统一优化方案，平衡了冷启动性能、多样性和稳定性。
...
### [110] [HER: Human-like Reasoning and Reinforcement Learning for LLM Role-playing](https://arxiv.org/abs/2601.21459)
...
TL;DR: HER框架通过双层思维区分角色第一人称与LLM第三人称思考，结合推理增强数据和人类对齐奖励模型，显著提升LLM角色扮演的认知模拟能力


<details>
  <summary>Details</summary>
Motivation: 当前LLM角色扮演虽然能捕捉角色语气和知识，但模拟行为背后的内在思维仍具挑战。现有方法存在两个主要缺陷：缺乏高质量推理轨迹数据，以及缺乏与人类偏好对齐的可靠奖励信号

Method: 提出HER统一框架，引入双层思维区分角色第一人称与LLM第三人称思考。通过逆向工程构建推理增强的角色扮演数据，建立人类对齐原则和奖励模型。基于Qwen3-32B模型，采用监督学习和强化学习进行训练
...
### [111] [L$^3$: Large Lookup Layers](https://arxiv.org/abs/2601.21461)
*Albert Tseng,Christopher De Sa*

Main category: cs.LG

TL;DR: L³（大型查找层）通过将嵌入表泛化到模型解码器层，引入了一种新的稀疏轴，使用静态基于令牌的路由来聚合学习到的嵌入，在语言建模和下游任务中优于密集模型和同等稀疏的MoE。


<details>
  <summary>Details</summary>
Motivation: 现代稀疏语言模型通常通过混合专家（MoE）层实现稀疏性，但动态硬路由存在硬件效率低、需要辅助损失来稳定训练等问题。相比之下，分词器嵌入表本质上是稀疏的，但缺乏上下文信息。本研究旨在结合两者的优点。

Method: 提出了大型查找层（L³），包含两个主要组件：1）系统友好的架构，支持快速训练和无开销的CPU卸载推理；2）信息论嵌入分配算法，有效平衡速度和质量。L³使用静态令牌路由在上下文相关的方式下聚合每个令牌的学习嵌入。

Result: 在训练包含高达26亿活跃参数的Transformer时，L³在语言建模和下游任务中都显著优于密集模型和同等稀疏的MoE模型。

Conclusion: L³通过将嵌入表泛化到模型解码器层，解锁了新的稀疏轴，能够高效平衡内存和计算，为稀疏语言模型提供了一种有前景的替代方案。
...
### [112] [A block-coordinate descent framework for non-convex composite optimization. Application to sparse precision matrix estimation](https://arxiv.org/abs/2601.21467)
*Guillaume Lauga*

Main category: cs.LG

TL;DR: 提出一个新的块坐标下降(BCD)框架用于非凸复合优化问题，确保目标函数下降并收敛到解，应用于稀疏精度矩阵估计问题，显著减少迭代次数。


<details>
  <summary>Details</summary>
Motivation: 块坐标下降(BCD)方法在大规模优化问题中广泛应用，但在非凸优化方面的理论研究相对不足。需要一个新的BCD框架来处理非凸复合优化问题，特别是稀疏精度矩阵估计等实际问题。

Method: 提出一个新的块坐标下降(BCD)框架，能够包含变量度量近端梯度更新、近端牛顿更新和交替最小化更新。该框架足够通用，可以涵盖图形LASSO问题中最常用的三种求解器：图形ISTA、原始GLasso和QUIC。

Result: 在非凸稀疏精度矩阵估计问题上验证了该框架的价值，提供了收敛保证，并且相比现有方法，达到最先进估计质量所需的迭代次数减少了高达100倍。

Conclusion: 该BCD框架为处理非凸复合优化问题提供了一个通用且有效的解决方案，特别在稀疏精度矩阵估计问题上表现出显著的计算效率提升，具有重要的理论和实际应用价值。
...
### [113] [ETS: Energy-Guided Test-Time Scaling for Training-Free RL Alignment](https://arxiv.org/abs/2601.21484)
*Xiuyu Li,Jinkai Zhang,Mingyang Yi,Yu Li,Longqiang Wang,Yue Wang,Ju Fan*

Main category: cs.LG

TL;DR: 提出一种免训练的推理方法ETS，通过能量引导的测试时缩放直接从最优RL策略采样，避免复杂的RL后训练对齐过程


<details>
  <summary>Details</summary>
Motivation: RL后训练对齐虽然有效但成本高、不稳定且训练过程复杂，需要更简单高效的替代方案

Method: ETS：基于掩码语言建模的转移概率（参考策略模型+能量项），通过在线蒙特卡洛估计能量项，结合重要性采样和现代加速框架降低推理延迟

Result: 在推理、编码和科学基准测试中，ETS在掩码语言建模（包括自回归模型和扩散语言模型）上一致提升生成质量

Conclusion: ETS提供了一种高效、免训练的推理方法，可直接从最优RL策略采样，避免了复杂且不稳定的RL后训练对齐过程
...
### [114] [Task-Awareness Improves LLM Generations and Uncertainty](https://arxiv.org/abs/2601.21500)
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM解码和不确定性估计方法仅在语言空间操作，忽略了自然语言响应背后的结构信息（如离散标签、数值、图结构）

Method: 在任务依赖的潜在结构中直接建模LLM输出，配备相异性度量，计算贝叶斯最优响应（通过潜在空间合成而非从采样生成中选择）

Result: 贝叶斯最优响应在多个任务上持续优于波束搜索等标准解码方法；通过诱导的贝叶斯风险量化不确定性，能更好地捕捉潜在结构变化并与输出质量和正确性对齐
...
### [115] [Cascaded Transfer: Learning Many Tasks under Budget Constraints](https://arxiv.org/abs/2601.21513)
*Eloi Campagne,Yvenn Amara-Ouali,Yannig Goude,Mathilde Mougeot,Argyris Kalogeratos*

Main category: cs.LG

TL;DR: 提出了一种名为级联迁移学习的新多任务学习范式，通过层次化级联结构在任务间传递信息，同时满足预算约束


<details>
  <summary>Details</summary>
Motivation: 在多任务学习场景中，当需要学习大量相关任务且任务间确切关系未知时，需要一种有效的迁移学习方法来提高学习效率和准确性

Method: 使用基于最小生成树的级联迁移机制，将任务组织为根树结构，按照合适的距离度量连接任务，并沿分支分配训练预算

Result: 在合成和真实多任务设置上的实验表明，该方法相比其他方法能够实现更准确、成本效益更高的跨大规模任务集合的适应

Conclusion: 级联迁移学习为多任务学习提供了一种有效的范式，能够在预算约束下实现任务间的有效知识迁移
...
### [116] [A Unified SPD Token Transformer Framework for EEG Classification: Systematic Comparison of Geometric Embeddings](https://arxiv.org/abs/2601.21521)
*Chi-Sheng Chen,En-Jui Kuo,Guan-Ying Chen,Xinyu Zhang,Fan Zhang*

Main category: cs.LG

TL;DR: 该论文分析了对称正定(SPD)矩阵在黎曼流形上的嵌入几何与优化动力学的关系，提出了BWSPD嵌入比Log-Euclidean嵌入具有更好的梯度条件数，并开发了嵌入空间批归一化方法，在统一Transformer框架中验证了理论预测。


<details>
  <summary>Details</summary>
Motivation: EEG信号的空间协方差矩阵是对称正定(SPD)矩阵，位于黎曼流形上，但嵌入几何与优化动力学之间的理论联系尚未被探索。需要建立嵌入选择与梯度条件数和数值稳定性之间的理论联系。

Method: 通过理论分析建立三个结果：1) BWSPD嵌入的梯度条件数为√κ，优于Log-Euclidean的κ；2) 嵌入空间批归一化近似黎曼归一化；3) bi-Lipschitz边界证明BWSPD保持流形距离。在统一Transformer框架中比较BWSPD、Log-Euclidean和欧几里得嵌入，在三个EEG范式(运动想象、ERP、SSVEP)上进行1500+次实验验证。

Result: Log-Euclidean Transformer在所有数据集上达到最先进性能，显著优于经典黎曼分类器和最近的SPD基线。BWSPD提供竞争性精度且训练时间相似。嵌入空间批归一化在56通道ERP数据上提升26%准确率，但在8通道SSVEP数据上效果可忽略，符合通道数依赖预测。

Conclusion: 该研究建立了SPD流形嵌入几何与优化动力学的理论联系，证明了不同嵌入选择的梯度条件数差异，提出的Log-Euclidean Transformer在EEG分类任务中表现优异，为SPD数据处理提供了理论指导和实用框架。
...
TL;DR: 提出一种结合全局奖励和局部奖励优势的方法，通过交互图精细识别个体贡献，改善多智能体强化学习中的合作问题。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中，全局奖励包含所有智能体的贡献，噪声较大且需要信用分配；局部奖励虽然学习更快但可能导致次优解，因为智能体可能短视地优化自身奖励而忽视全局最优性。

Method: 通过智能体间的交互图来更精细地识别个体贡献，既比全局奖励更细致，又能缓解局部奖励带来的合作问题。同时提出了近似构建这种交互图的实用方法。

Result: 实验证明该方法具有灵活性，能够超越传统的局部奖励和全局奖励设置。

Conclusion: 提出的方法结合了全局奖励和局部奖励的优点，通过交互图实现更精细的信用分配，改善了多智能体强化学习中的合作问题。
...
TL;DR: 论文提出了一种新的洛伦兹线性层，解决了现有双曲神经网络中输出范数随梯度下降步骤对数缩放的问题，实现了线性缩放，同时通过双曲激活函数和缓存策略提升了计算效率。


<details>
  <summary>Details</summary>
Motivation: 双曲空间作为分层和鲁棒表示学习的有前景几何结构，其核心挑战是开发既高效又能捕捉双曲空间关键特性的数学公式。现有洛伦兹模型虽然支持快速前向和反向传播，但存在输出双曲范数随梯度下降步骤对数缩放的问题，削弱了双曲几何的优势。

Method: 基于"距离到超平面"公式提出新的洛伦兹线性层，证明其能实现输出双曲范数的线性缩放。同时引入洛伦兹激活函数和新的缓存策略，提升算法效率。

Result: 新方法完全遵循双曲几何，同时缩小了与欧几里得神经网络的计算差距，代码已开源。

Conclusion: 提出的新洛伦兹线性层解决了双曲神经网络的关键缩放问题，实现了真正的双曲几何优势，为双曲表示学习提供了更有效的框架。
...
TL;DR: 提出Expert Modulation新范式，通过文本信号调节专家路由和计算，实现跨模态时间序列预测


<details>
  <summary>Details</summary>
Motivation: 现实世界时间序列动态复杂，现有多模态方法依赖token级融合，但在高质量时间-文本对稀缺、时间序列尺度特征差异大时效果不佳

Method: 提出Expert Modulation范式，基于文本信号调节专家混合模型的路由和专家计算，实现直接高效的跨模态控制

Result: 通过理论分析和实验验证，在多模态时间序列预测任务上取得显著改进
...
TL;DR: HistoPrism：基于HE组织学的跨癌种基因表达预测模型，通过通路级评估验证其生物学相关性


<details>
  <summary>Details</summary>
Motivation: 从HE组织学预测空间基因表达可为测序提供可扩展的临床替代方案，但现有方法局限于单一癌种和方差评估，缺乏对功能相关性的探索

Method: 提出HistoPrism，一种高效的基于transformer的架构，用于从组织学进行跨癌种基因表达预测，并引入通路级基准评估生物学意义

Result: HistoPrism不仅在高变异基因预测上超越现有最佳模型，更重要的是在通路级预测上取得显著提升，证明其能恢复生物学一致的转录组模式

Conclusion: HistoPrism凭借强大的跨癌种泛化能力和改进的效率，为从常规组织学进行临床相关转录组建模设立了新标准
...
TL;DR: SAL是一种结合选择性参数激活和自适应区域划分的训练方法，通过解耦参数空间来减轻梯度干扰，无需权重对称，在深度和大规模模型中表现良好。


<details>
  <summary>Details</summary>
Motivation: 传统反向传播存在生物学上不合理的权重对称要求，并且在密集表示中遭受显著的梯度干扰问题，需要更符合生物学习机制且可扩展的训练方法。

Method: 提出选择性自适应学习（SAL），将参数空间分解为互斥的样本依赖区域，结合选择性参数激活和自适应区域划分，使用改进的反馈对齐来避免权重对称要求。

Result: 在10个标准基准测试中表现出竞争性的收敛速度和改进的分类性能，在深度（128层）和大规模模型（10亿参数）中保持数值一致性和竞争性准确率。

Conclusion: SAL提供了一种生物学上更合理、可扩展的神经网络训练替代方案，有助于研究大规模神经网络训练，解决了反向传播的梯度干扰和权重对称限制。
...
### [122] [Representation Unlearning: Forgetting through Information Compression](https://arxiv.org/abs/2601.21564)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法通常修改模型参数，但存在不稳定、计算成本高、受限于局部近似等问题。需要更可靠、高效的遗忘方法以满足隐私法规和鲁棒性需求。
...
Conclusion: Representation Unlearning通过在表示空间直接操作而非修改参数，为机器遗忘提供了更稳定、高效且实用的解决方案，优于现有参数修改方法。
...
### [123] [FlexCausal: Flexible Causal Disentanglement via Structural Flow Priors and Manifold-Aware Interventions](https://arxiv.org/abs/2601.21567)
*Yutao Jin,Yuang Tao,Junyong Zhai*

Main category: cs.LG

TL;DR: FlexCausal是一个基于块对角协方差VAE的新型因果解耦表示学习框架，使用因子化流先验建模复杂外生噪声分布，结合监督对齐和反事实一致性约束，在合成和真实数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有解耦方法依赖均值场近似和对角后验协方差，解耦所有潜在维度，且假设外生噪声为各向同性高斯分布，无法捕捉真实世界因果因素的复杂非高斯统计特性。
...
Conclusion: FlexCausal通过块对角协方差VAE和因子化流先验，有效解耦因果机制学习与分布统计，实现了更精确的因果结构学习和高质量生成。
...
### [124] [Bridging Functional and Representational Similarity via Usable Information](https://arxiv.org/abs/2601.21568)
*Antonio Almudévar,Alfonso Ortega*

Main category: cs.LG

TL;DR: 该论文提出了一个统一框架，通过"可用信息"的视角量化表示之间的相似性，从功能相似性、表示相似性和任务粒度层次三个维度进行理论综合。


<details>
  <summary>Details</summary>
Motivation: 当前衡量神经网络表示相似性的方法（如CKA、RSA、stitching等）缺乏统一的理论框架。作者旨在通过"可用信息"的概念，为不同相似性度量方法提供理论基础，并揭示它们之间的内在联系。

Method: 基于信息论框架，使用"可用信息"作为核心概念。通过理论证明将stitching性能与条件互信息建立正式联系，将重建度量与标准工具（CKA、RSA）关联为特定约束下的可用信息估计器。引入任务粒度层次概念来统一功能相似性和表示相似性。

Result: 1. 证明stitching性能与条件互信息相关，且stitching具有内在不对称性；2. 显示重建度量和CKA/RSA是特定约束下的可用信息估计器；3. 揭示相似性相对于预测家族容量的相对性；4. 证明表示相似性是功能相似性的充分但不必要条件；5. 通过任务粒度层次统一概念。

Conclusion: 该框架为表示相似性度量提供了统一的理论基础，揭示了不同方法之间的内在联系。表示相似性应被视为功能相似性的极限情况（最大粒度：输入重建），相似性评估需要考虑预测模型的表达能力。
...
### [125] [Shaping capabilities with token-level data filtering](https://arxiv.org/abs/2601.21571)
*Neil Rathi,Alec Radford*

Main category: cs.LG

TL;DR: 通过预训练数据过滤（特别是token级过滤）来减少语言模型的不良能力，该方法在医疗能力移除任务上表现出高效、鲁棒且成本低的特点，且随着模型规模增大效果更显著。


<details>
  <summary>Details</summary>
Motivation: 当前减少语言模型不良能力的方法大多是后处理的，容易被对抗攻击绕过。因此需要在预训练阶段就塑造模型能力，从根本上解决问题。

Method: 1. 采用预训练数据过滤方法，特别是token级过滤而非文档级过滤；2. 使用稀疏自编码器标记token并蒸馏出高质量分类器；3. 在不同规模模型上进行实验验证。

Result: 1. token过滤比文档过滤更有效，在相同不良能力抑制效果下对良性能力影响更小；2. 过滤效果随模型规模增大而增强，最大模型在遗忘领域计算速度降低7000倍；3. 即使经过过滤，模型仍能在遗忘领域进行对齐；4. 方法对噪声标签具有鲁棒性。

Conclusion: 预训练数据过滤是一种高效、鲁棒且可扩展的方法来减少语言模型的不良能力，token级过滤优于文档级过滤，且随着模型规模增大效果更显著，为模型安全提供了新的解决方案。
...
### [126] [Signal-Adaptive Trust Regions for Gradient-Free Optimization of Recurrent Spiking Neural Networks](https://arxiv.org/abs/2601.21572)
*Jinhao Li,Yuhao Sun,Zhiyuan Ma,Hao He,Xinche Zhang,Xing Chen,Jin Li,Sen Song*

Main category: cs.LG

TL;DR: SATR提出一种基于信号自适应信任区域的分布更新规则，通过KL散度约束更新步长，提升脉冲神经网络在强化学习中的训练稳定性与效率


<details>
  <summary>Details</summary>
Motivation: 循环脉冲神经网络是节能控制策略的有前景平台，但训练高维长时程强化学习任务仍具挑战。基于种群的梯度自由优化虽然避免了不可微脉冲动态的反向传播，但有限种群导致梯度估计方差高，可能产生有害的激进更新步骤

Method: 提出信号自适应信任区域方法，通过KL散度约束分布更新，KL散度由估计的信号能量归一化。该方法在强信号时自动扩展信任区域，在噪声主导时收缩。针对伯努利连接分布实例化SATR，并引入二进制脉冲和权重的位集实现以加速训练

Result: 在高维连续控制基准测试中，SATR在有限种群下提升稳定性，达到与PPO-LSTM等强基线竞争的性能。位集实现显著减少训练时间，实现快速RSNN策略搜索

Conclusion: SATR通过信号自适应信任区域机制有效解决了脉冲神经网络强化学习中的训练稳定性问题，结合高效实现方法使RSNN策略搜索更具实用性
...
### [127] [Learning the Mechanism of Catastrophic Forgetting: A Perspective from Gradient Similarity](https://arxiv.org/abs/2601.21577)
*Mutian Yang,Zisen Zhan,Yutong Chen,Haolin Li,Kaiwen Wang,Kaili Zheng,Yuguang Wang,Qi Wang,Jiandong Gao,Ji Wu*

Main category: cs.LG

TL;DR: 提出基于梯度相似度的理论框架解释灾难性遗忘，识别冲突神经元和协作神经元，并基于此提出CNL方法，通过冻结冲突神经元、只更新协作神经元来消除遗忘。
...
Method: 建立基于梯度的理论框架，证明强负梯度相似度是遗忘的根本原因。识别冲突神经元（导致遗忘）和协作神经元（缓解遗忘），提出协作神经学习（CNL）方法：冻结冲突神经元，只更新协作神经元。

Result: 在5个LLM、4个数据集、4个优化器上的实验表明，CNL在in-set设置下实现零遗忘，在out-of-set设置下减少59.1%-81.7%的遗忘。

Conclusion: 梯度相似度理论框架为灾难性遗忘提供了理论基础，CNL方法通过选择性神经元更新有效解决了知识注入中的遗忘问题。
...
### [128] [Evaluating Prediction Uncertainty Estimates from BatchEnsemble](https://arxiv.org/abs/2601.21581)
*Morten Blørstad,Herman Jangsett Mostein,Nello Blaser,Pekka Parviainen*

Main category: cs.LG

TL;DR: BatchEnsemble作为可扩展的不确定性估计方法，在表格和时间序列任务中表现优异，匹配深度集成性能但参数更少、训练推理更快；提出的GRUBE在序列建模中表现类似或更好。
...
Result: BatchEnsemble在不确定性估计性能上匹配深度集成，明显优于Monte Carlo dropout；GRUBE在预测和不确定性估计方面表现相似或更好；相比传统集成方法，参数更少、训练和推理时间更短。

Conclusion: BatchEnsemble和GRUBE是高效的不确定性估计方法，在保持性能的同时显著减少了计算成本和参数数量，适用于表格和时间序列任务。
...
### [129] [CORDS: Continuous Representations of Discrete Structures](https://arxiv.org/abs/2601.21583)
...
Method: 提出CORDS方法，通过可逆映射将空间对象集合转换为连续场：密度场编码对象位置和数量，特征场在相同支持域上携带对象属性。由于映射可逆，模型完全在场空间中操作，同时保持可精确解码为离散集合。

Result: 在分子生成与回归、目标检测、基于模拟的推理以及涉及局部最大值恢复的数学任务等多个领域进行评估，展示了CORDS能够稳健处理未知集合大小并保持竞争性准确度。

Conclusion: CORDS提供了一种新颖的连续推理框架，有效解决了可变大小集合预测问题，通过可逆场表示避免了传统方法的限制，在多个应用领域展现出良好性能。
...
### [130] [Heterogeneity-Aware Knowledge Sharing for Graph Federated Learning](https://arxiv.org/abs/2601.21589)
...
<details>
  <summary>Details</summary>
Motivation: 图联邦学习（GFL）在保护图数据隐私的同时进行分布式图表示学习，但面临多个客户端间节点特征和结构拓扑的异质性挑战。现有方法难以同时处理这两种异质性，需要一种能共享节点特征和结构拓扑知识的解决方案。

Method: 提出FedSSA方法：1）针对节点特征异质性，使用变分模型推断类级节点分布，基于分布聚类客户端并构建簇级代表分布，通过最小化本地与簇级分布的差异实现语义知识共享；2）针对结构异质性，采用谱图神经网络，提出谱能量度量结构信息，基于谱能量聚类客户端并构建簇级谱GNN，通过对齐本地与簇级谱GNN的谱特性实现结构知识共享。

Result: 在6个同配性和5个异配性图数据集上，在非重叠和重叠分区设置下进行实验，FedSSA始终优于11种最先进的方法。

Conclusion: FedSSA通过同时处理节点特征和结构拓扑异质性，有效解决了图联邦学习中的异质性问题，实现了更好的知识共享和模型性能。
...
### [131] [Scalable Power Sampling: Unlocking Efficient, Training-Free Reasoning for LLMs via Distribution Sharpening](https://arxiv.org/abs/2601.21590)
*Xiaotong Ji,Rasul Tutunov,Matthieu Zimmer,Haitham Bou Ammar*

Main category: cs.LG

TL;DR: 提出一种无需外部奖励、无需迭代MCMC的推理增强方法，通过token级缩放低温分布近似全局幂分布，在数学、QA和代码任务上匹配或超越GRPO，推理延迟降低10倍以上。


<details>
  <summary>Details</summary>
Motivation: 现有RL后训练主要依赖分布锐化而非获得新能力，MCMC方法虽能恢复类似性能但计算成本过高，需要一种更高效的理论替代方案。

Method: 提出理论推导：全局幂分布可由token级缩放低温分布近似，其中缩放因子捕获未来轨迹质量。基于此引入无需训练、无需验证器的算法，自回归地锐化基础模型的生成分布。

Result: 在四种LLM的数学、QA和代码任务上评估，该方法匹配或超越一次性GRPO（无需外部奖励），同时相比基于MCMC的采样减少10倍以上推理延迟。

Conclusion: 提出了一种高效的理论替代方案，无需外部奖励或迭代MCMC即可实现推理性能提升，为LLM推理增强提供了实用且可扩展的方法。
...
### [132] [Dynamics Reveals Structure: Challenging the Linear Propagation Assumption](https://arxiv.org/abs/2601.21601)
*Hoyeon Chang,Bálint Mucsányi,Seong Joon Oh*

Main category: cs.LG

TL;DR: 论文研究了神经网络一阶参数更新是否保持逻辑一致性的几何限制，发现组合操作存在根本性障碍，导致特征映射崩溃


<details>
  <summary>Details</summary>
Motivation: 神经网络通过一阶参数更新进行适应，但尚不清楚这种更新是否能保持逻辑一致性。研究旨在探究线性传播假设（LPA）的几何限制，该假设认为局部更新能够一致地传播到逻辑结果。

Method: 采用关系代数研究关系的三个核心操作：否定（翻转真值）、逆（交换参数顺序）和组合（链接关系）。对于否定和逆，证明保证方向无关的一阶传播需要张量分解，将实体对上下文与关系内容分离。对于组合，证明其可简化为合取，并证明任何在线性特征上定义良好的合取必须是双线性的。

Result: 发现组合操作存在根本性障碍：由于双线性与否定不兼容，这迫使特征映射崩溃。这些结果表明知识编辑失败、逆转诅咒和多跳推理问题可能源于LPA固有的结构限制。

Conclusion: 线性传播假设在组合操作上存在根本性限制，这解释了神经网络在逻辑一致性保持方面的困难，并为知识编辑、推理等问题的根源提供了理论解释。
...
### [133] [Beyond Parameter Finetuning: Test-Time Representation Refinement for Node Classification](https://arxiv.org/abs/2601.21615)
*Jiaxin Zhang,Yiqi Wang,Siwei Wang,Xihong Yang,Yu Shi,Xinwang Liu,En Zhu*

Main category: cs.LG

TL;DR: TTReFT是一种新的图神经网络测试时训练框架，通过将适应目标从模型参数转移到潜在表示来解决灾难性遗忘问题，在OOD场景下表现优异。
...
Result: 在五个基准数据集上的广泛实验表明，TTReFT实现了稳定且优越的性能，为图测试时训练建立了新的表示微调范式。
...
### [134] [Breaking the Overscaling Curse: Thinking Parallelism Before Parallel Thinking](https://arxiv.org/abs/2601.21619)
...
<details>
  <summary>Details</summary>
Motivation: 当前并行思维方法为所有样本分配相同的全局并行度N，但样本异质性导致某些样本只需更小的N'就能达到可比性能，造成预算冗余。这种系统级效能与样本级效率的不兼容构成了"过度缩放诅咒"。

Method: 提出T2方法，利用潜在表示在解码前估计每个样本的最优并行度，实现样本级自适应并行度分配，打破过度缩放诅咒。

Result: 实验表明T2显著降低了成本同时保持了可比性能，实现了更高效的并行思维。

Conclusion: T2通过样本级自适应并行度分配有效解决了并行思维中的过度缩放问题，在保持推理质量的同时大幅提升计算效率。
...
### [135] [LAMP: Look-Ahead Mixed-Precision Inference of Large Language Models](https://arxiv.org/abs/2601.21623)
*Stanislav Budzinskiy,Marian Gloser,Tolunay Yilmaz,Ying Hong Tham,Yuanyi Lin,Wenyi Fang,Fan Wu,Philipp Petersen*

Main category: cs.LG

TL;DR: 提出一种自适应混合精度策略，通过选择性地对复合函数中少量关键组件进行高精度计算，显著提升Transformer推理精度


<details>
  <summary>Details</summary>
Motivation: 混合精度计算是当前AI发展的关键，特别是在大语言模型本地部署中。需要解决复合函数浮点计算问题，专注于Transformer推理的精度优化

Method: 基于复合函数f(g(x))的舍入误差分析，提出自适应策略：选择g(x)中少量关键组件进行高精度计算，其余部分使用低精度计算。将该策略应用于Transformer中的不同复合结构

Result: 在GPT-2模型上的数值实验表明，即使非常低的重计算率（少量组件使用高精度）也能将精度提升高达两个数量级

Conclusion: 提出的自适应混合精度策略能有效提升Transformer推理精度，为高效本地部署大语言模型提供了有前景的技术方案
...
### [136] [Training Memory in Deep Neural Networks: Mechanisms, Evidence, and Measurement Gaps](https://arxiv.org/abs/2601.21624)
*Vasileios Sevetlidis,George Pavlidis*

Main category: cs.LG

TL;DR: 该论文提出了一种系统化分析深度学习训练中历史依赖性的框架，包括因果估计、扰动原语和审计协议，旨在实现可移植、因果感知的训练效果测量。


<details>
  <summary>Details</summary>
Motivation: 现代深度学习训练不是无记忆的，优化器状态、数据顺序策略、非凸路径和辅助状态等历史因素会影响训练结果，但缺乏系统化的测量和分析框架。

Method: 引入种子配对、函数空间因果估计量；设计可移植的扰动原语（动量/Adam/EMA/BN的携带/重置、顺序窗口交换、队列/教师调整）；提出包含审计工件的报告检查清单。

Result: 建立了一个协议，用于实现可移植、因果感知、不确定性感知的测量，能够量化训练历史在不同模型、数据和训练机制中的重要性。

Conclusion: 提出了一套完整的测量协议，能够系统化地评估训练历史对深度学习性能的影响，促进更严谨和可复现的研究实践。
...
### [137] [HeRo-Q: A General Framework for Stable Low Bit Quantization via Hessian Conditioning](https://arxiv.org/abs/2601.21626)
*Jinhao Zhang Yunquan Zhang,Zicheng yan,Boyang Zhang,Jun Sun,Daning Cheng*

Main category: cs.LG

TL;DR: HeRo Q算法通过轻量级可学习的旋转压缩矩阵重塑损失函数景观，减少Hessian矩阵最大特征值，显著提升量化鲁棒性，在超低位宽量化中表现优异


<details>
  <summary>Details</summary>
Motivation: 传统后训练量化(PTQ)只关注最小化量化误差，导致"低误差、高损失"的悖论现象，根本原因在于LLM损失景观的Hessian矩阵中少数高曲率方向对扰动极其敏感

Method: 提出Hessian鲁棒量化(HeRo Q)算法，在量化前对权重空间应用轻量级可学习的旋转压缩矩阵，通过联合框架重塑损失景观，减少最大Hessian特征值

Result: 在Llama和Qwen模型上，HeRo Q在标准W4A8设置下优于GPTQ、AWQ、SpinQuant等SOTA方法，在极具挑战的W3A16超低位宽量化中，将Llama3 8B的GSM8K准确率提升至70.15%，有效避免了激进量化中常见的逻辑崩溃

Conclusion: HeRo Q无需架构修改，计算开销可忽略，可无缝集成到现有PTQ流程中，通过重塑损失景观解决量化鲁棒性问题，在超低位宽量化中表现突出
...
### [138] [Generative Design of Ship Propellers using Conditional Flow Matching](https://arxiv.org/abs/2601.21637)
*Patrick Kruger,Rafael Diaz,Simon Hauschulz,Stefan Harries,Hanno Gottschalk*

Main category: cs.LG

TL;DR: 使用条件流匹配的生成式AI进行船舶螺旋桨设计，实现从性能目标到设计参数的反向映射，并生成满足相同性能的多种设计方案。


<details>
  <summary>Details</summary>
Motivation: 传统前向机器学习模型只能根据设计参数预测性能，而工程设计中更需要从性能目标生成设计方案。生成式AI能够解决这一反向设计问题，为机械组件设计提供新方法。

Method: 采用条件流匹配建立设计参数与模拟噪声之间的双向映射，噪声以性能标签为条件。使用涡格法进行数值模拟生成训练数据，并提出基于前向代理模型的伪标签数据增强方法。

Result: 成功生成了具有几乎相同性能特征的不同螺旋桨几何形状，展示了生成式AI在工程设计中的多功能性和潜力。分析了模型精度与数据量之间的权衡。

Conclusion: 条件流匹配的生成式AI方法能够有效解决船舶螺旋桨的反向设计问题，生成满足特定性能目标的多种设计方案，为工程优化设计提供了新工具。
...
### [139] [Seg-MoE: Multi-Resolution Segment-wise Mixture-of-Experts for Time Series Forecasting Transformers](https://arxiv.org/abs/2601.21641)
*Evandro S. Ortigossa,Eran Segal*

Main category: cs.LG

TL;DR: Seg-MoE：一种用于时间序列预测的稀疏混合专家模型，通过连续时间步段的路由机制，比传统的token-wise MoE方法更好地捕捉时间数据的局部性和连续性，在多个长期预测基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在时间序列预测中取得了进展，但难以高效扩展并捕捉长期时间动态。现有的MoE方法采用token-wise路由机制，未能充分利用时间数据的自然局部性和连续性特征。

Method: 提出Seg-MoE，一种稀疏MoE设计，路由和处理连续的时间步段而非独立的token决策。将Seg-MoE层集成到时间序列Transformer中，使每个专家能够直接建模段内交互，与固有时间模式自然对齐。
...
### [140] [Identifiable Equivariant Networks are Layerwise Equivariant](https://arxiv.org/abs/2601.21645)
...
<details>
  <summary>Details</summary>
Motivation: 研究深度神经网络中端到端等变性与层间等变性之间的关系，解释实践中观察到的网络权重等变结构出现的现象。

Method: 基于抽象形式化理论，证明在参数可识别条件下，端到端等变的网络存在参数选择使得各层也具有等变性。该理论是架构无关的。

Result: 证明了端到端等变网络存在参数选择使得各层也具有等变性，为实践中观察到的网络权重等变结构提供了数学解释。

Conclusion: 该研究为神经网络训练中权重等变结构的出现提供了理论依据，解释了实践中观察到的现象，并建立了端到端等变性与层间等变性之间的数学关系。
...
### [141] [SWE-Spot: Building Small Repo-Experts with Repository-Centric Learning](https://arxiv.org/abs/2601.21649)
*Jinjun Peng,Magnus Saebo,Tianjun Zhong,Yi-Jie Cheng,Junfeng Yang,Baishakhi Ray,Simin Chen,Yangruibo Ding*

Main category: cs.LG

TL;DR: 该论文提出Repository-Centric Learning (RCL)范式，通过深度掌握特定代码库而非广度任务学习，训练出SWE-Spot-4B模型，在软件工程任务上超越更大规模开源模型并媲美商业模型。


<details>
  <summary>Details</summary>
Motivation: 当前小型语言模型在隐私敏感和资源受限环境中部署需求增加，但这些模型在处理复杂陌生代码库时存在能力差距。传统的任务中心学习范式通过广度任务训练无法解决这一限制。
...
Result: SWE-Spot-4B打破了现有缩放趋势，在多个软件工程任务上超越了更大规模的开源模型（如Meta的CWM、Qwen3-Coder-30B），并达到或超过了效率导向的商业模型（如GPT-4.1-mini、GPT-5-nano）。RCL展现出更高的训练样本效率和更低的推理成本。
...
### [142] [Gauge-invariant representation holonomy](https://arxiv.org/abs/2601.21653)
*Vasileios Sevetlidis,George Pavlidis*

Main category: cs.LG

TL;DR: 该论文提出了"表示全纯"这一新指标，用于衡量深度网络内部表示在输入空间路径上的几何变化，超越了传统点对点相似度度量方法。


<details>
  <summary>Details</summary>
Motivation: 现有相似度度量方法（如CKA、SVCCA）只能捕捉激活集的点对点重叠，但无法捕捉表示沿输入路径的变化。两个模型在这些指标下可能看起来几乎相同，但对扰动或对抗性攻击的响应却非常不同。

Method: 引入表示全纯这一规范不变统计量，量化特征在输入空间小环路上平行移动时累积的"扭曲"。通过全局白化固定规范，使用共享子空间和仅旋转的Procrustes对齐邻域，然后将结果嵌入完整特征空间。

Result: 证明了全纯对正交（和白化后的仿射）变换的不变性，建立了仿射层的线性零值，并显示全纯在小半径处消失。经验上，全纯随环路半径增加，能区分CKA下相似的模型，并与对抗性和损坏鲁棒性相关。

Conclusion: 表示全纯作为一个实用且可扩展的诊断工具，能够探测学习表示的几何结构，超越了传统的点对点相似度度量。
...
### [143] [TabClustPFN: A Prior-Fitted Network for Tabular Data Clustering](https://arxiv.org/abs/2601.21656)
...
<details>
  <summary>Details</summary>
Motivation: 表格数据聚类面临三大挑战：特征类型异构、数据生成机制多样、缺乏可迁移的归纳偏置。现有方法需要针对每个数据集进行调参，缺乏通用性

Method: 提出TabClustPFN，基于先验拟合网络范式，在灵活的聚类先验生成的合成数据集上预训练，通过摊销贝叶斯推理同时推断聚类分配和聚类数量

Result: 在合成数据和真实表格基准测试中，TabClustPFN优于传统方法、深度方法和摊销聚类基线，在开箱即用的探索性设置中表现出强鲁棒性

Conclusion: TabClustPFN首次将先验拟合网络成功应用于表格数据聚类，实现了无需调参的单次前向传播聚类，为表格数据探索性分析提供了强大工具
...
### [144] [Epistemic Uncertainty Quantification for Pre-trained VLMs via Riemannian Flow Matching](https://arxiv.org/abs/2601.21662)
*Li Ju,Mayank Nautiyal,Andreas Hellander,Ekta Vats,Prashant Singh*

Main category: cs.LG

TL;DR: REPVLM：一种基于黎曼流匹配的视觉语言模型不确定性量化方法，通过嵌入空间的负对数密度估计模型认知不确定性


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型通常是确定性的，缺乏量化认知不确定性的内在机制。认知不确定性反映了模型对其自身表示缺乏知识或无知的程度，这对于可靠部署至关重要。

Method: 提出REPVLM方法，将嵌入的负对数密度作为认知不确定性的代理指标，其中低密度区域表示模型无知。该方法使用黎曼流匹配在VLM嵌入的超球面流形上计算概率密度。

Result: REPVLM实现了不确定性与预测误差之间的近乎完美相关性，显著优于现有基线方法。除了分类任务外，该方法还为分布外检测和自动化数据筛选提供了可扩展的度量标准。

Conclusion: REPVLM为视觉语言模型提供了一种有效的认知不确定性量化框架，通过嵌入空间的密度估计来识别模型的无知区域，为可靠部署和自动化数据管理提供了新工具。
...
### [145] [SENDAI: A Hierarchical Sparse-measurement, EfficieNt Data AssImilation Framework](https://arxiv.org/abs/2601.21664)
...
Method: 提出SENDAI框架：一种分层稀疏测量高效数据同化框架，通过结合模拟先验与学习到的差异校正，从超稀疏传感器观测中重建完整空间状态。

Result: 在六个全球分布的站点上重建MODIS植被指数场，相比传统基线最大SSIM提升185%，相比近期高频方法提升36%。特别在具有尖锐边界和亚季节动态的景观中表现优异，能有效保留诊断相关结构。
...
### [146] [Expected Return Causes Outcome-Level Mode Collapse in Reinforcement Learning and How to Fix It with Inverse Probability Scaling](https://arxiv.org/abs/2601.21669)
*Abhijeet Sinha,Sundari Elango,Dianbo Liu*

Main category: cs.LG

TL;DR: 论文指出标准强化学习期望回报最大化会导致结果层面的模式坍塌，并提出逆概率缩放修正方法IPS-GRPO来解决这一问题


<details>
  <summary>Details</summary>
Motivation: 许多强化学习问题存在多个质量相当的最优解，但标准期望回报最大化训练的策略通常会坍塌到少数结果上。传统解释归因于探索不足或正则化弱，但作者认为这是期望回报目标本身的结构性问题

Method: 提出逆概率缩放（IPS）修正方法，移除学习信号中的结果频率放大效应。具体实现为IPS-GRPO，作为Group Relative Policy Optimization的即插即用修改，无需辅助模型或架构变更

Result: 在推理和分子生成任务中，IPS-GRPO持续减少结果层面的模式坍塌，同时匹配或超越基线性能

Conclusion: 修正目标函数而非添加探索启发式方法是实现可靠多模态策略优化的关键。逆概率缩放能从根本上改变学习动态，产生与奖励成比例的终端分布，防止多模态设置中的坍塌
...
### [147] [LLM4Fluid: Large Language Models as Generalizable Neural Solvers for Fluid Dynamics](https://arxiv.org/abs/2601.21681)
...
TL;DR: LLM4Fluid是一个利用大语言模型作为通用神经求解器的流体动力学时空预测框架，通过降阶建模和模态对齐实现无需重新训练的高精度预测
...
Method: 1) 通过物理信息解纠缠机制的降阶建模将高维流场压缩到紧凑潜空间；2) 使用预训练LLM作为时序处理器进行自回归预测；3) 提出模态对齐策略解决提示与物理序列之间的表示不匹配问题

Result: 在多种流场场景中，LLM4Fluid无需重新训练即可实现最先进的预测精度，展现出强大的零样本学习和上下文学习能力

Conclusion: LLM4Fluid框架证明了LLM作为通用神经求解器在流体动力学中的有效性，为时空建模提供了新的解决方案
...
### [148] [Can Local Learning Match Self-Supervised Backpropagation?](https://arxiv.org/abs/2601.21683)
*Wu S. Zihan,Ariane Delrocq,Wulfram Gerstner,Guillaume Bellec*

Main category: cs.LG

TL;DR: 该论文建立了局部自监督学习与全局反向传播自监督学习之间的联系，提出了新的局部SSL算法变体，在图像数据集上取得了与全局BP-SSL相当的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然端到端的全局反向传播自监督学习已成为现代AI系统的核心训练方法，但局部自监督学习理论在深度神经网络中构建功能性表示方面一直存在困难。研究者希望建立全局规则与局部规则之间的联系。

Method: 首先为深度线性网络开发理论，识别局部SSL算法实现与全局BP-SSL完全相同权重更新的条件。基于理论见解，开发新的局部SSL算法变体，在深度非线性卷积神经网络中近似全局BP-SSL。

Result: 提高局部SSL与全局BP-SSL梯度更新相似性的变体在图像数据集（CIFAR-10、STL-10和Tiny ImageNet）上表现更好。使用CLAPP损失函数的最佳局部SSL规则与使用InfoNCE或CPC类损失函数的可比全局BP-SSL性能相当，并在这些基准测试中改进了局部SSL的最新水平。

Conclusion: 该研究成功建立了局部与全局自监督学习之间的联系，开发出能够近似全局BP-SSL性能的局部SSL算法，为局部学习规则提供了理论支持和实践改进。
...
### [149] [Don't be so Stief! Learning KV Cache low-rank approximation over the Stiefel manifold](https://arxiv.org/abs/2601.21686)
...
TL;DR: StiefAttention是一种KV缓存压缩方法，通过直接最小化解码器层输出重构误差来学习正交投影基，相比现有方法在相同压缩率下显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存压缩方法通常使用SVD风格的代理目标，这些目标不能很好地反映经过softmax、值混合和后续解码器层变换后的端到端重构效果，导致性能不佳。

Method: StiefAttention通过直接最小化解码器层输出重构误差来学习正交投影基，并为每层预计算候选秩的错误-秩分布，支持在用户指定的错误预算下进行灵活的层间秩分配。

Result: 在Llama3-8B上，相同压缩条件下，StiefAttention比EigenAttention在C4困惑度上提升11.9个点，在0-shot MMLU准确率上提升5.4%，产生更低的相对误差和更高的余弦相似度。

Conclusion: StiefAttention通过端到端优化解码器层输出重构误差，实现了比现有方法更有效的KV缓存压缩，在保持性能的同时显著减少内存占用。
...
### [150] [XFACTORS: Disentangled Information Bottleneck via Contrastive Supervision](https://arxiv.org/abs/2601.21688)
*Alexandre Myara,Nicolas Bourriez,Thomas Boyer,Thomas Lemercier,Ihab Bendidi,Auguste Genovesio*

Main category: cs.LG

TL;DR: XFactors：一种弱监督VAE框架，通过对比监督和KL正则化实现选定因子的解耦与控制，无需对抗训练或分类器。


<details>
  <summary>Details</summary>
Motivation: 完全无监督方法在合成数据上成功，但在真实数据上无法恢复语义因子；监督方法依赖对抗目标或辅助分类器，不稳定且难以扩展到大规模属性集。

Method: 基于解耦信息瓶颈视角，将表示分解为残差子空间和因子特定子空间。通过InfoNCE对比损失对齐目标因子，KL正则化对残差和聚合因子子空间施加高斯结构，避免对抗训练。

Result: 在多个数据集上达到最先进的解耦分数，在相应子空间中实现一致的定性因子对齐，支持通过潜在替换进行可控因子交换，并能正确扩展潜在容量。

Conclusion: XFactors提供了一种稳定、可扩展的弱监督解耦方法，无需对抗训练或分类器，在真实数据集CelebA上表现良好。
...
### [151] [Understanding Model Merging: A Unified Generalization Framework for Heterogeneous Experts](https://arxiv.org/abs/2601.21690)
*Qinglun Li,Anke Tang,Miao Zhang,Mengzhu Wang,Quanjun Yin,Li Shen*

Main category: cs.LG

TL;DR: 该论文提出了一个统一的理论框架来解释模型合并的有效性，并基于L2稳定性理论分析了异构超参数环境下合并模型的泛化性能，为实践者提供了调优建议。


<details>
  <summary>Details</summary>
Motivation: 模型合并技术虽然能有效聚合多个微调模型的能力，但在异构超参数环境下缺乏统一的理论解释，且开源微调模型缺乏超参数透明度，使得合并模型性能难以预测。

Method: 采用L2稳定性理论分析异构超参数环境下合并模型x_avg的泛化性能，建立统一理论框架解释现有合并算法，并提出实践调优建议。

Result: 理论分析揭示了不同超参数对合并模型泛化性能的影响，实验在ResNet/ViT系列上验证了理论预测，涉及数千个微调模型和20/8个视觉分类任务。

Conclusion: 该研究为模型合并提供了首个统一理论框架，解释了现有算法的优化原理，并为实践者提供了在预训练到微调流程中构建合并友好模型的具体指导。
...
### [152] [Curriculum Learning for LLM Pretraining: An Analysis of Learning Dynamics](https://arxiv.org/abs/2601.21698)
*Mohamed Elgaar,Hadi Amiri*

Main category: cs.LG

TL;DR: 课程学习主要改变训练数据在固定学习轨迹内的暴露顺序，而非创造新的学习阶段，通过降低梯度噪声和优化不稳定来提升小模型性能，但在大模型中收益减小。


<details>
  <summary>Details</summary>
Motivation: 研究课程学习是否真正改变模型的学习轨迹，还是仅仅重新排列数据在固定轨迹上的暴露顺序，并探索课程学习如何影响不同规模模型的优化稳定性。

Method: 训练Pythia模型（14M-410M参数）300B tokens，使用三种语言学动机的课程（习得年龄、词频、动词变化）与随机排序对比；在1B参数规模上比较随机和VV课程。分析梯度噪声、输出头谱饱和度和优化稳定性。

Result: 不同排序下训练遵循共享的潜在阶段序列，课程主要改变阶段内的数据暴露。小模型中随机排序表现出更高的梯度噪声和更强的后期训练输出头谱饱和，导致较低最终准确率；课程减少这些效应。大模型中饱和差异较小，课程收益缩减。

Conclusion: 课程学习通过稳定阶段内优化而非创造新阶段来帮助训练，特别是在小模型中效果显著。建立了难度调节与优化稳定性之间的联系，课程通过梯度方差控制来提升训练效果。
...
### [153] [Beyond Forgetting: Machine Unlearning Elicits Controllable Side Behaviors and Capabilities](https://arxiv.org/abs/2601.21702)
*Tien Dang,The-Hai Nguyen,Dinh Mai Phuong,Nguyen Minh Phuong,Hoang Thanh-Tung,Le-Minh Nguyen,Naoya Inoue*

Main category: cs.LG

TL;DR: 该论文探讨了表示误导(RM)这类LLM遗忘方法，发现通过操纵遗忘样本的潜在表示，不仅能实现遗忘，还能引发可控的副作用行为和增强的侧向能力。


<details>
  <summary>Details</summary>
Motivation: 现有表示误导方法中目标向量的作用尚未充分探索，作者希望从线性表示假说的角度重新审视RM，研究机器遗忘是否会产生与高层概念相关的可控副作用行为和增强能力。

Method: 基于线性表示假说，识别与高层概念对应的一维表示，在遗忘表示空间中对概念向量进行线性操作，验证机器遗忘是否引发可控的副作用行为和增强的能力。

Result: 实验验证了机器遗忘确实能引发可控的副作用行为（如控制模型的真实性、情感和拒绝能力）和增强的侧向能力（如提升模型的上下文学习能力）。

Conclusion: 这一现象既是潜在的隐藏风险（如果被滥用），也可被利用来开发具有更强能力和可控行为的模型，为模型开发提供了新的机制。
...
### [154] [SmartMeterFM: Unifying Smart Meter Data Generative Tasks Using Flow Matching Models](https://arxiv.org/abs/2601.21706)
...
<details>
  <summary>Details</summary>
Motivation: 智能电表数据对配电网规划运营至关重要，但面临隐私限制、数据损坏、分辨率不足等问题。现有机器学习方法需要为每个生成任务（合成数据、缺失值插补、超分辨率）单独设计和训练模型，导致冗余和低效。

Method: 利用流匹配模型的强大建模能力，提出条件生成统一框架。将不同生成任务视为部分数据观测，通过将观测注入生成过程，用单一模型统一处理插补和超分辨率等任务，无需重新训练。模型针对15分钟分辨率的月度智能电表数据等高维时间序列数据进行训练。

Result: 模型生成的数据不仅与给定观测一致，而且保持真实性，在性能上优于插值方法和专门针对各任务的机器学习基线模型。

Conclusion: 流匹配模型能够有效统一智能电表数据的多种生成任务，通过单一条件生成模型解决数据可用性、完整性和分辨率问题，提高了效率并减少了模型冗余。
...
### [155] [When does predictive inverse dynamics outperform behavior cloning?](https://arxiv.org/abs/2601.21718)
...
<details>
  <summary>Details</summary>
Motivation: 行为克隆在专家演示有限时经常失败，而预测逆动力学模型虽然表现更好，但其优势原因尚不明确，需要理论解释

Method: 提出理论分析框架，证明PIDM引入偏差-方差权衡：未来状态预测引入偏差，但条件化逆动力学模型能显著降低方差。建立状态预测器偏差条件，使PIDM比BC有更低预测误差和更高样本效率

Result: 在2D导航任务中，BC需要比PIDM多5倍（平均3倍）的演示才能达到相当性能；在复杂3D游戏环境中，BC需要比PIDM多66%以上的样本

Conclusion: PIDM通过偏差-方差权衡机制，在有限专家演示下比BC具有更好的样本效率和性能，特别是在有额外数据源时优势更明显
...
TL;DR: 提出一个可解释性框架，从预训练的PFNs中提取显式谱密度和核函数，实现快速高斯过程回归


<details>
  <summary>Details</summary>
Motivation: PFNs虽然能进行高效的摊销推理，但缺乏对其学习到的先验和核函数的透明访问，这限制了它们在下游任务（如基于代理的优化）中的应用

Method: 通过机制分析识别注意力潜在输出是关键中介，提出解码器架构将PFN潜在表示映射到显式谱密度估计，通过Bochner定理获得平稳核函数

Result: 解码器能够恢复复杂的多峰谱混合，产生的显式核函数支持高斯过程回归，精度与PFNs和基于优化的基线相当，但只需单次前向传播

Conclusion: 该框架实现了从PFNs中提取显式核函数，相比基于优化的基线将推理时间减少了数量级，同时保持了可解释性和准确性
...
### [157] [Mixed-Precision Training and Compilation for RRAM-based Computing-in-Memory Accelerators](https://arxiv.org/abs/2601.21737)
...
<details>
  <summary>Details</summary>
Motivation: CIM加速器虽然能直接在内存中执行矩阵向量乘法加速ML工作负载，但交叉阵列的输入和单元位宽非常有限，大多数CIM编译器不支持8位以下量化，导致单个MVM需要多个计算周期，权重无法高效存储在单个交叉单元中

Method: 提出混合精度训练和编译框架，采用基于强化学习的策略搜索合适的量化配置，以平衡延迟和精度

Result: 在最佳情况下，相比现有最先进方案实现最高2.48倍加速，精度损失仅为0.086%

Conclusion: 提出的基于强化学习的混合精度框架能有效解决CIM架构中量化参数搜索空间大的问题，显著提升性能同时保持高精度
...
TL;DR: 提出一个基于关系数据方法的机器学习框架，用于脓毒症预测，强调可解释性并识别患者亚表型


<details>
  <summary>Details</summary>
Motivation: 脓毒症是重症监护中最复杂和异质性的综合征之一，具有多样化的生理轨迹和治疗反应。虽然深度学习模型在脓毒症早期预测中表现良好，但通常缺乏可解释性，并且忽略了潜在的患者亚表型。

Method: 采用关系数据方法：将电子病历中的时序数据视为多元患者日志，用关系数据模式表示；应用命题化技术（基于关系数据领域的经典聚合/选择函数）构建可解释特征以"扁平化"数据；最后使用选择性朴素贝叶斯分类器对扁平化数据进行分类。

Result: 实验验证证明了所提出方法的相关性及其极高的可解释性。解释分为四个层面：单变量、全局、局部和反事实解释。

Conclusion: 该研究提出了一种新颖的关系数据方法框架，不仅提高了脓毒症预测的可解释性，还能识别患者亚表型，为临床决策提供了更透明和实用的工具。
...
TL;DR: FISMO优化器通过结合Fisher信息几何的曲率信息，在保持计算效率的同时实现结构化预条件，在非凸优化中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: Muon优化器通过正交动量更新实现各向同性几何，但丢弃了梯度谱中编码的有价值曲率信息。需要平衡几何结构与自适应性的优化方法。

Method: FISMO将各向同性更新推广到包含各向异性曲率信息，通过Fisher信息几何将优化器更新重新表述为受Kronecker分解Fisher度量约束的信任区域问题。

Result: 在随机非凸设置中建立了收敛保证，证明了期望平方梯度范数的O(1/√T)收敛率，并在图像分类和语言建模基准测试中展示了优于基线的训练效率和最终性能。

Conclusion: FISMO通过结构化预条件成功平衡了几何结构与自适应性，在保持计算可处理性的同时适应局部损失景观几何，实现了优越的优化性能。
...
TL;DR: 提出统一框架将背包和Top-k算子建模为动态规划，通过平滑递归实现可微松弛，支持高效并行算法，证明香农熵是唯一保持置换等变性的正则化选择。


<details>
  <summary>Details</summary>
Motivation: 背包和Top-k算子在神经网络中很有用，但由于它们是分段常数函数，梯度几乎处处为零，难以集成到神经网络中进行端到端训练。

Method: 将算子建模为动态规划，通过平滑递归实现可微松弛；开发高效并行算法支持确定性和随机前向传播，以及向量-雅可比乘积反向传播；理论分析正则化选择。

Result: 提出统一可微框架，证明香农熵是唯一保持置换等变性的正则化选择，识别诱导稀疏选择的正则化器；在决策聚焦学习、约束动态分类RL和离散VAE扩展等任务上验证有效性。

Conclusion: 通过动态规划视角为离散选择算子提供统一可微框架，理论分析正则化选择，实验证明在多个任务上的有效性，为神经网络集成离散选择算子提供新方法。
...
TL;DR: 提出Quantum LEGO Learning框架，将经典神经网络和变分量子电路作为可组合的模块化学习块，实现架构无关的混合量子-经典学习。


<details>
  <summary>Details</summary>
Motivation: 现有混合量子-经典学习模型通常采用紧密耦合架构或任务特定编码器，限制了概念清晰度、通用性和跨学习场景的可迁移性。需要一种更模块化、架构无关的框架来提升灵活性和分析能力。

Method: 提出Quantum LEGO Learning框架：1) 预训练的经典神经网络作为冻结特征块；2) 变分量子电路作为可训练自适应模块，处理结构化表示而非原始输入；3) 开发块级泛化理论，将学习误差分解为近似和估计分量；4) 通过块交换实验系统验证框架。

Result: 1) 理论分析推广了先前张量网络特定结果，识别了量子模块相对于经典头提供表示优势的条件；2) 量子点分类实验显示稳定优化、对量子比特数敏感性降低、对现实噪声的鲁棒性；3) 框架支持高效学习和混合模型的原则性分析。

Conclusion: Quantum LEGO Learning为混合量子-经典学习提供了模块化、架构无关的框架，通过明确分离特征提取和自适应处理角色，实现了更好的概念清晰度、通用性和资源效率，为分析量子优势提供了理论工具。
...
### [162] [NetMamba+: A Framework of Pre-trained Models for Efficient and Accurate Network Traffic Classification](https://arxiv.org/abs/2601.21792)
*Tongze Wang,Xiaohui Xie,Wenduo Wang,Chuyi Wang,Jinzhou Liu,Boyan Huang,Yannan Hu,Youjian Zhao,Yong Cui*

Main category: cs.LG

TL;DR: NetMamba+ 是一个用于网络流量分类的高效框架，采用 Mamba 架构和 Flash Attention 机制，通过多模态流量表示和标签分布感知微调策略，在性能、效率和少样本学习方面均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 随着加密网络流量的快速增长，有效的流量分类对网络安全和服务质量管理至关重要。现有方法面临三个关键挑战：Transformer 架构计算效率低、流量表示不充分（丢失关键字节级特征同时保留有害偏见）、以及处理真实世界数据中长尾分布的能力差。

Method: NetMamba+ 通过三个关键创新解决上述挑战：(1) 采用 Mamba 和 Flash Attention 机制的高效架构，(2) 保留基本流量信息同时消除偏见的多模态流量表示方案，(3) 标签分布感知的微调策略。

Result: 在包含四个主要分类任务的大规模数据集评估中，NetMamba+ 相比最先进的基线方法，F1 分数提升高达 6.44%。同时具有出色的效率，推理吞吐量比最佳基线高 1.7 倍，内存使用相当低。此外，NetMamba+ 展现出优异的少样本学习能力，用更少的标记数据实现更好的分类性能。在线流量分类系统吞吐量达 261.87 Mb/s。

Conclusion: 作为首个将 Mamba 架构应用于网络流量分类的框架，NetMamba+ 为复杂网络环境中的高效准确流量分析开辟了新可能性。
...
### [163] [Knowledge Vector Weakening: Efficient Training-free Unlearning for Large Vision-Language Models](https://arxiv.org/abs/2601.21794)
*Yejin Kim,Dongjun Hwang,Sungmin Cha,Junsuk Choe*

Main category: cs.LG

TL;DR: KVW是一种无需训练的遗忘方法，通过识别并削弱知识向量来移除LVLMs中特定数据的影响，显著提升计算效率。
...
Method: KVW（知识向量削弱）是一种无需训练的遗忘方法，直接干预完整模型而不需要梯度计算。它识别在遗忘集上激活的知识向量，并逐步削弱其贡献，防止模型利用不良知识。

Result: 在MLLMU和CLEAR基准测试中，KVW实现了稳定的遗忘-保留权衡，同时在计算效率上显著优于基于梯度和基于LoRA的遗忘方法。

Conclusion: KVW为大型视觉语言模型提供了一种高效、无需训练的遗忘解决方案，有效解决了隐私和有害内容问题，同时保持了模型性能。
...
### [164] [Effective LoRA Adapter Routing using Task Representations](https://arxiv.org/abs/2601.21795)
*Akash Dhasade,Anne-Marie Kermarrec,Igor Pavlovic,Diana Petrescu,Rafael Pires,Mathis Randl,Martijn de Vos*

Main category: cs.LG

TL;DR: LORAUTER是一个新颖的路由框架，通过任务表示而非适配器特征来选择和组合LoRA适配器，实现高效的任务级路由，在多个任务上超越基线方法并扩展到1500+适配器。
...
Result: 实验表明LORAUTER在多个任务上持续优于基线路由方法：当存在任务对齐适配器时达到Oracle性能的101.2%，在未见任务上提升5.2个百分点，并能扩展到超过1500个适配器的大规模嘈杂适配器池。

Conclusion: LORAUTER通过任务表示而非适配器特征的路由方法，实现了高效、可扩展的LoRA适配器选择和组合，为大规模适配器库的管理和使用提供了有效解决方案。
...
### [165] [Nonparametric LLM Evaluation from Preference Data](https://arxiv.org/abs/2601.21816)
...
Method: 提出DMLEval框架，引入广义平均排名分数(GARS)来泛化常见排名模型，使用去偏机器学习方法进行估计，支持黑盒机器学习方法和预训练LLM评估器，并提供预算约束下的最优数据收集策略。

Result: DMLEval能够产生统计有效的GARS排名分数估计，支持复杂的人类响应（如平局），在合成和真实世界偏好数据集上都表现出理论优势和实证效果。
...
### [166] [DASH: Deterministic Attention Scheduling for High-throughput Reproducible LLM Training](https://arxiv.org/abs/2601.21824)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM训练中确定性注意力实现（如FlashAttention-3）存在显著性能损失，确定性反向传播相比非确定性版本吞吐量下降可达37.9%，主要原因是梯度累积操作需要序列化以保证数值一致性，导致硬件利用率低下。
...
<details>
  <summary>Details</summary>
Motivation: 机器学习模型在处理计算流体动力学等昂贵黑盒仿真时面临多查询挑战，需要以最小计算成本（尽可能少的黑盒样本）确保所需精度。虽然主动学习策略已用于标量问题，但扩展到场预测仍然缺乏或局限于特定场景和模型类型。

Method: 提出一种与模型架构无关的主动学习策略，结合标量参考值的高斯过程模型，同时减少认知模型误差和标量与场预测之间的差异。引入了该方法的不同具体形式，并与仅基于标量值的填充方法进行比较。

Result: 在NASA通用研究模型的误差传播任务中展示了结果，与没有主动学习的方法相比，以显著更小的成本实现了高精度水平。

Conclusion: 提出的主动学习策略能够有效处理场预测问题，在保持高精度的同时显著降低计算成本，为昂贵黑盒仿真的多查询挑战提供了通用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有线性化拉普拉斯近似方法需要计算大型雅可比矩阵，计算成本高，难以扩展到大规模预训练深度神经网络，需要更高效的核近似方法

Method: 使用替代深度神经网络学习紧凑特征表示，使其内积能够复制神经正切核，仅需高效的雅可比-向量乘积进行训练，避免计算完整雅可比矩阵

Result: 实验结果显示与现有LLA近似方法相比，具有相似或改进的不确定性估计和校准性能，通过偏置学习核能显著增强分布外检测能力

Conclusion: 该方法为在LLA背景下寻找比NTK更好的核提供了有效途径，能够在给定预训练DNN的情况下更高效地计算预测不确定性
...
<details>
  <summary>Details</summary>
Motivation: 现实应用如机器人和医疗保健需要在测试时施加安全约束，但现有约束元强化学习缺乏在真实测试任务中确保策略安全性的方法，同时需要降低样本复杂度以实现快速学习
...
<details>
  <summary>Details</summary>
Motivation: 现有元黑盒优化中的奖励函数由人工设计，存在设计偏差和奖励攻击风险，需要自动化方法来发现更好的奖励函数。

Method: 1) 在有效性方面，采用进化式启发式思想，在迭代的LLM程序搜索过程中引入定制化进化范式；2) 在效率方面，引入多任务进化架构支持并行奖励发现，通过跨任务知识共享加速收敛。

Result: 实验结果表明，通过该方法发现的奖励函数能够有效提升现有元黑盒优化工作的性能，证明了奖励设计在元黑盒优化中的重要性。

Conclusion: 使用大型语言模型作为自动奖励发现工具是可行的，进化式搜索和多任务架构能够有效提升奖励函数的质量和发现效率。
...
TL;DR: DiDAE提出了一种无需梯度优化的高效反事实生成框架，通过解耦字典学习编辑基础模型嵌入，结合反事实知识蒸馏提升不平衡数据集性能


<details>
  <summary>Details</summary>
Motivation: 基础模型存在虚假相关性和"Clever Hans"策略问题，现有缓解方法依赖不可用的组标签或计算昂贵的梯度对抗优化，需要更高效的解决方案

Method: 提出视觉解耦扩散自编码器(DiDAE)，将冻结的基础模型与解耦字典学习结合，在解耦方向上编辑嵌入，通过扩散自编码器解码生成多样化反事实

Result: DiDAE能比现有基线更快生成多个解耦反事实，结合反事实知识蒸馏(DiDAE-CFKD)在缓解捷径学习方面达到SOTA，提升不平衡数据集的下游性能

Conclusion: DiDAE提供了一种无需梯度优化的高效反事实生成方法，有效缓解基础模型的捷径学习问题，为不平衡数据集上的模型鲁棒性提升提供了新途径
...
TL;DR: MoHETS：一种用于多变量时间序列长期预测的编码器-仅Transformer，通过稀疏混合异构专家层捕获多尺度时间动态，结合深度卷积专家和傅里叶专家，使用卷积补丁解码器提高参数效率。
...
Method: MoHETS采用编码器-仅Transformer架构，集成稀疏混合异构专家层：1）将时间补丁路由到少量专家网络；2）结合共享深度卷积专家处理序列级连续性；3）路由傅里叶基专家处理补丁级周期结构；4）通过协变量补丁嵌入的交叉注意力纳入外生信息；5）用轻量卷积补丁解码器替换参数密集的线性投影头。

Result: 在七个多变量基准测试和多个预测范围上验证，MoHETS始终实现最先进性能，相比近期强基线平均MSE降低12%，展示了异构专业化在长期预测中的有效性。

Conclusion: MoHETS通过混合异构专家层有效捕获时间序列的多尺度动态，结合卷积解码器提高参数效率和训练稳定性，为多变量时间序列长期预测提供了有效的解决方案。
...
### [173] [Managing Solution Stability in Decision-Focused Learning with Cost Regularization](https://arxiv.org/abs/2601.21883)
*Victor Spitzer,Francois Sanson*

Main category: cs.LG

TL;DR: 论文提出通过正则化估计成本向量来解决决策聚焦学习中扰动强度波动导致的训练失效问题


<details>
  <summary>Details</summary>
Motivation: 决策聚焦学习通过训练模型直接提升决策质量而非预测精度，但组合优化问题的微分是一个核心挑战。现有方法使用扰动近似，但学习过程中扰动强度的波动可能导致训练失效。

Method: 通过引入对估计成本向量的正则化，提高学习过程的鲁棒性和可靠性。该方法基于组合优化中解稳定性的理论联系。

Result: 通过大量数值实验证明，所提出的正则化方法能有效解决扰动波动导致的训练问题，提高学习过程的稳定性和可靠性。

Conclusion: 通过正则化估计成本向量可以解决决策聚焦学习中扰动强度波动导致的训练失效问题，提高学习过程的鲁棒性，为决策聚焦学习提供了更可靠的实现方法。
...
### [174] [Not All Code Is Equal: A Data-Centric Study of Code Complexity and LLM Reasoning](https://arxiv.org/abs/2601.21894)
*Lukas Twist,Shu Yang,Hanqi Yan,Jingzhi Gong,Di Wang,Helen Yannakoudakis,Jie M. Zhang*

Main category: cs.LG

TL;DR: 研究发现代码的结构复杂性（而非代码本身）是提升大语言模型推理能力的关键因素，通过控制代码结构复杂度进行微调比使用结构多样的代码效果更好
...
Method: 研究两种互补设置：1）解决方案驱动复杂性（同一问题的不同解决方案具有不同复杂度）；2）问题驱动复杂性（不同任务具有不同复杂度）。使用圈复杂度和逻辑代码行数构建受控微调数据集，评估多个开源大语言模型在多样化推理基准上的表现

Result: 代码确实能提升推理能力，但结构特性强烈决定其有效性。在83%的实验中，将微调数据限制在特定结构复杂度范围内，比使用结构多样的代码训练效果更好

Conclusion: 代码的结构复杂性是提升推理能力的关键因素，这为通过数据中心的路径改进推理提供了新方向，超越了单纯扩大规模的方法
...
### [175] [A Low-Complexity Plug-and-Play Deep Learning Model for Generalizable Massive MIMO Precoding](https://arxiv.org/abs/2601.21897)
*Ali Hasanzadeh Karkan,Ahmed Ibrahim,Jean-François Frigon,François Leduc-Primeau*

Main category: cs.LG

TL;DR: PaPP是一种即插即用的深度学习预编码框架，通过元学习域泛化和功率感知输入归一化，实现跨站点、功率水平和信道估计误差的通用预编码，无需为每个部署场景重新训练。


<details>
  <summary>Details</summary>
Motivation: 大规模MIMO下行链路预编码在实际部署中存在挑战：传统WMMSE算法计算成本高且对SNR和信道估计质量敏感，而现有DL解决方案缺乏鲁棒性且需要为每个部署站点重新训练。

Method: 提出PaPP框架，结合高容量教师模型和紧凑学生模型，使用自监督损失平衡教师模仿和归一化和速率。采用元学习域泛化和发射功率感知输入归一化进行训练，支持全数字和混合波束成形预编码。

Result: 在三个未见站点的射线追踪数据上，PaPP的全数字和混合波束成形模型均优于传统和深度学习基线方法。经过少量本地未标记样本微调后，PaPP实现了21倍以上的计算能耗降低，并在信道估计误差下保持良好性能。

Conclusion: PaPP为能量高效的大规模MIMO预编码提供了实用解决方案，具有跨站点重用性、计算效率高和对信道估计误差鲁棒的特点。
...
### [176] [Breaking the Regional Barrier: Inductive Semantic Topology Learning for Worldwide Air Quality Forecasting](https://arxiv.org/abs/2601.21899)
...
<details>
  <summary>Details</summary>
Motivation: 全球空气质量预测面临空间异质性极强的问题，现有传导模型对未见区域的泛化能力差，需要解决全球站点级预测的挑战。

Method: 提出OmniAir框架：1) 将不变物理环境属性编码为可泛化的站点身份；2) 动态构建自适应稀疏拓扑；3) 捕捉长程非欧几里得相关性和物理扩散模式；4) 构建WorldAir数据集覆盖全球7,800多个站点。

Result: OmniAir在18个基线模型中实现了最先进的性能，在数据稀疏区域有效填补监测空白，同时保持高效率和高可扩展性，速度比现有模型快近10倍。

Conclusion: OmniAir通过语义拓扑学习框架成功解决了全球空气质量预测的空间异质性和泛化问题，为全球站点级预测提供了高效可扩展的解决方案。
...
### [177] [Hardware-Triggered Backdoors](https://arxiv.org/abs/2601.21902)
*Jonas Möller,Erik Imgrund,Thorsten Eisenhofer,Konrad Rieck*

Main category: cs.LG

TL;DR: 论文提出了一种新型硬件触发后门攻击，利用不同计算硬件间的数值差异，使模型在不同硬件上对相同输入产生不同预测。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型广泛部署在各种计算硬件上，虽然硬件预期产生相同结果，但设计差异会导致推理过程中的微小数值变化。这种差异可能被利用来创建新型后门攻击。

Method: 通过调整模型的决策函数，使其在决策边界附近对目标输入产生不同预测。具体方法是：将决策边界局部移动到目标输入附近，然后利用数值偏差在特定硬件上翻转预测结果。

Result: 实验证明，这种硬件触发后门可以在常见的GPU加速器上可靠创建，揭示了第三方模型使用中的新型攻击向量。

Conclusion: 研究揭示了硬件差异可能被利用来创建隐蔽后门的新攻击向量，并探讨了相应的防御措施来应对这种威胁。
...
### [178] [LoRIF: Low-Rank Influence Functions for Scalable Training Data Attribution](https://arxiv.org/abs/2601.21929)
*Shuangqi Li,Hieu Le,Jingyi Xu,Mathieu Salzmann*

Main category: cs.LG

TL;DR: LoRIF提出了一种低秩影响函数方法，通过利用梯度的低秩结构解决训练数据归因中的存储和计算瓶颈，在保持归因质量的同时实现高达20倍的存储减少和查询加速。


<details>
  <summary>Details</summary>
Motivation: 现有基于梯度的训练数据归因方法（如TRAK、LoGRA）面临两个主要瓶颈：1）存储和加载所有训练样本的投影梯度导致I/O延迟问题；2）形成D×D逆Hessian近似需要O(D²)内存。这两个瓶颈都随投影维度D增加而恶化，但提高D又是提升归因质量所必需的，形成了质量与可扩展性的矛盾。
...
Conclusion: LoRIF通过利用梯度的低秩结构有效解决了梯度基训练数据归因方法的可扩展性瓶颈，使其能够在前沿规模下实用化，为大规模模型的训练数据归因提供了高效解决方案。
...
### [179] [Robust Multimodal Representation Learning in Healthcare](https://arxiv.org/abs/2601.21941)
...
<details>
  <summary>Details</summary>
Motivation: 真实世界医学数据集通常包含来自多个来源的系统性偏差，这对医学多模态表示学习构成重大挑战。现有方法通常专注于有效的多模态融合，而忽略了影响泛化能力的内在偏差特征。

Method: 提出双流特征解相关框架，通过潜在混杂因子引入的结构因果分析来识别和处理偏差。该方法采用具有双流神经网络结构的因果偏差解相关框架，将因果特征与虚假相关性解耦，利用广义交叉熵损失和互信息最小化进行有效解相关。

Result: 在MIMIC-IV、eICU和ADNI数据集上的综合实验展示了性能的持续改进。
...
TL;DR: 提出信息论方法实现维度无关的扩散模型收敛分析，无需几何假设，并基于KL散度重构提出轻量级自适应调度算法LAS
...
Method: 采用信息论方法，通过KL散度分析收敛性，提出基于训练损失的自适应调度算法LAS，无需训练后重计算

Result: 在温和假设下，目标分布与生成分布间的KL散度可被O(H²/K)界定，其中H为香农熵，K为采样步数；LAS在实验中优于常见启发式调度

Conclusion: 信息论方法可实现维度无关的扩散模型收敛分析，LAS作为轻量级自适应调度算法能有效提升采样质量
...
TL;DR: 该论文提出了一种评估概念瓶颈模型可解释性的框架，通过"清晰度"指标衡量下游性能与概念表示稀疏性、精确性之间的权衡，揭示了灵活性与可解释性之间的关键平衡。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型的广泛应用，模型可解释性问题日益突出。当前模型常被视为黑箱，决策过程缺乏深入探究。尽管存在多种可解释性方法，但对学习表示的系统性评估仍然有限，特别是对于被认为能"诱导可解释性"的稀疏性方法。

Method: 提出"清晰度"度量指标，捕捉下游性能与概念表示稀疏性、精确性之间的相互作用。建立使用真实概念标注数据集的可解释性评估框架。研究基于VLM和属性预测器的概念瓶颈模型，以及三种稀疏性诱导策略：每样本ℓ₁、ℓ₀和基于伯努利分布的方法。

Result: 实验揭示了灵活性与可解释性之间的关键权衡关系。即使在可比性能水平下，不同方法可能表现出显著不同的行为特征。该研究为评估概念表示的质量提供了系统框架。

Conclusion: 概念瓶颈模型的可解释性评估需要综合考虑性能、稀疏性和精确性。提出的清晰度指标和评估框架为系统评估模型可解释性提供了新工具，有助于理解不同建模决策对表示学习的影响。
...
TL;DR: 研究探讨了平衡传播训练方法在局部连接晶格架构上的性能，发现稀疏网络能达到与密集网络相当的表现


<details>
  <summary>Details</summary>
Motivation: 人工智能的快速发展导致能耗不可持续增长，这推动了神经形态计算和基于物理的学习机器训练方法的发展。然而，现有理论研究多集中于全连接或密集连接网络，这些架构在实验实现上面临连接性约束的挑战

Method: 采用平衡传播这一广泛使用的基于物理的训练方法，在更现实的架构选择上（特别是局部连接晶格）进行实验。训练XY模型，探索架构对各种基准任务的影响，跟踪训练过程中空间分布响应和耦合的演化

Result: 结果显示，仅具有局部连接的稀疏网络能够达到与密集网络相当的性能表现

Conclusion: 研究结果为在现实环境中进一步扩展基于平衡传播的架构提供了指导方针，表明局部连接架构在实际应用中具有可行性
...
Method: 提出Aleatoric Uncertainty Modeling (AUM)：1) 将每个单模态表示建模为多元高斯分布以捕获随机不确定性；2) 在二部患者-模态图中使用不确定性感知聚合机制开发动态消息传递；3) 自适应聚合信息，动态强调可靠模态。

Result: 在MIMIC-IV死亡率预测上AUC-ROC提升2.26%，在eICU上提升2.17%，优于现有最先进方法。

Conclusion: AUM框架通过显式量化单模态随机不确定性，有效处理医疗多模态学习中的缺失模态问题，实现了可靠信息的自适应聚合。
...
### [184] [Uncertainty-Aware Data-Based Method for Fast and Reliable Shape Optimization](https://arxiv.org/abs/2601.21956)
*Yunjia Yang,Runze Li,Yufei Zhang,Haixin Chen*

Main category: cs.LG

TL;DR: 提出不确定性感知的数据驱动优化框架，通过概率编码器-解码器模型量化预测不确定性，在优化过程中惩罚高不确定性样本，提高空气动力学形状优化的鲁棒性和性能


<details>
  <summary>Details</summary>
Motivation: 传统数据驱动优化依赖训练数据库质量，当优化过程中遇到分布外样本时，代理模型的预测误差可能导致优化误导。需要将不确定性量化纳入优化过程以检测异常样本并增强鲁棒性

Method: 开发概率编码器-解码器代理模型来预测输出不确定性，将这些不确定性集成到模型置信度感知的目标函数中，在数据驱动优化过程中惩罚具有大预测误差的样本

Result: 在两个多点优化问题（改善翼型阻力发散和抖振性能）上评估，UA-DBO能持续减少优化样本的预测误差，相比原始DBO获得更优性能增益，相比基于全计算模拟的多点优化，提供相当的优化效果但显著加速优化速度

Conclusion: 不确定性感知的数据驱动优化框架能有效监控和最小化代理模型不确定性，提高优化鲁棒性，在保持优化效果的同时大幅提升优化效率
...
### [185] [From Tokens to Blocks: A Block-Diffusion Perspective on Molecular Generation](https://arxiv.org/abs/2601.21964)
*Qianwei Yang,Dong Xu,Zhangfan Yang,Sisi Yuan,Zexuan Zhu,Jianqiang Li,Junkai Ji*

Main category: cs.LG

TL;DR: SoftMol是一个用于靶向分子生成的一体化框架，通过软片段表示、块扩散模型和目标感知搜索策略，实现了100%化学有效性、9.7%结合亲和力提升和6.6倍推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有基于GPT的分子语言模型存在两个根本限制：1) 将分子作为序列处理时未能充分捕捉其图结构特性；2) 缺乏明确的靶向生成机制。需要开发能够更好处理分子结构和实现目标感知生成的模型。

Method: 提出SoftMol框架，包含三个核心创新：1) 软片段表示：无规则的SMILES块表示，支持扩散建模；2) SoftBD模型：首个块扩散分子语言模型，结合局部双向扩散和自回归生成；3) 门控蒙特卡洛树搜索：以目标感知方式组装片段。使用精心策划的ZINC-Curated数据集进行训练。

Result: 相比当前最先进模型，SoftMol实现了100%化学有效性，结合亲和力提升9.7%，分子多样性增加2-3倍，推理效率提升6.6倍。

Conclusion: SoftMol通过统一设计分子表示、模型架构和搜索策略，成功解决了现有分子语言模型在结构建模和目标感知方面的局限性，为靶向药物发现提供了高效可靠的生成框架。
...
### [186] [Bridging Graph Structure and Knowledge-Guided Editing for Interpretable Temporal Knowledge Graph Reasoning](https://arxiv.org/abs/2601.21978)
...
Method: 三阶段流程：1) 使用时序GNN识别结构和时序一致的候选路径；2) LLM引导的路径编辑解决逻辑和语义不一致；3) 整合精炼路径生成准确可解释的预测

Result: 在标准TKG基准测试中达到SOTA性能，在ICEWS数据集上Hits@1相对提升5.6%，Hits@3提升8.1%，消融研究证实各组件有效性

Conclusion: IGETR成功结合了GNN的结构时序建模能力和LLM的上下文理解优势，解决了现有方法的结构信息不足和幻觉问题，实现了更准确可靠的时序知识图谱推理
...
### [187] [Investigation into using stochastic embedding representations for evaluating the trustworthiness of the Fréchet Inception Distance](https://arxiv.org/abs/2601.21979)
*Ciaran Bench,Vivek Desai,Carlijn Roozemond,Ruben van Engen,Spencer A. Thomas*

Main category: cs.LG

TL;DR: 该论文研究了FID指标在医学图像应用中的局限性，通过蒙特卡洛dropout计算FID的预测方差，评估其在处理分布外数据时的可靠性。


<details>
  <summary>Details</summary>
Motivation: FID指标广泛用于评估合成图像质量，但其基于ImageNet预训练的InceptionV3模型可能不适用于医学图像。目前尚不清楚该指标在多大程度上无法捕捉图像特征的有意义差异。

Method: 使用蒙特卡洛dropout计算FID的预测方差，同时计算特征嵌入模型潜在表示的预测方差。通过ImageNet1K验证集（以不同强度增强）和其他外部数据集作为测试输入，分析预测方差与分布外程度的相关性。

Result: 预测方差的大小与测试输入相对于训练数据的分布外程度呈现不同程度的相关性，这为评估FID指标的可信度提供了见解。

Conclusion: 该方法能够洞察FID指标在医学图像应用中的信任度，揭示了预测方差与分布外数据程度的相关性，有助于更好地理解和评估FID在非自然图像领域的适用性。
...
### [188] [Investigating Batch Inference in a Sequential Monte Carlo Framework for Neural Networks](https://arxiv.org/abs/2601.21983)
*Andrew Millard,Joshua Murphy,Peter Green,Simon Maskell*

Main category: cs.LG

TL;DR: 本文提出了一种在序列蒙特卡洛（SMC）采样器中逐步引入小批量数据（数据退火）的方法，可在图像分类任务上实现6倍加速训练且精度损失最小。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯神经网络推理中，变分推断虽然计算高效但依赖于变分分布的选择，而粒子方法（如MCMC和SMC）不假设参数化后验但计算成本高，主要原因是需要全批量数据进行似然和梯度评估。

Method: 探索了在SMC采样器的似然和梯度评估中逐步引入更多小批量数据（数据退火）的多种方法，通过渐进式增加数据量来降低计算成本。

Result: 在基准图像分类问题上，使用神经网络实现了高达6倍的训练加速，同时精度损失最小。

Conclusion: 数据退火方法能够显著提高SMC采样器的计算效率，使其在保持精度的同时更适合大规模神经网络推理任务。
...
### [189] [PowerGenie: Analytically-Guided Evolutionary Discovery of Superior Reconfigurable Power Converters](https://arxiv.org/abs/2601.21984)
...
<details>
  <summary>Details</summary>
Motivation: 发现优越的电路拓扑需要探索指数级大的设计空间，传统上依赖人类专家。现有AI方法要么从预定义模板中选择，要么在有限规模内生成拓扑而缺乏严格验证，大规模性能驱动发现尚未充分探索。

Method: 1) 自动化分析框架：无需组件尺寸或SPICE仿真即可确定转换器功能和理论性能极限；2) 进化微调方法：通过适应性选择和唯一性验证，使生成模型与其训练分布共同进化。

Result: PowerGenie在语法有效性、功能有效性、新颖率和性能指标(FoM)方面优于现有方法。发现了一个新型8模式可重构转换器，其FoM比最佳训练拓扑高23%。SPICE仿真确认在8种模式下平均绝对效率提升10%，单个模式下最高提升17%。
...
Method: Elign采用后训练框架：1）用预训练的机器学习力场（MLFF）替代昂贵的DFT评估来提供物理信号；2）通过将物理引导转移到训练阶段，消除运行时的重复查询。具体使用强化学习将反向扩散表述为强化学习问题，引入FED-GRPO（力-能量解耦组相对策略优化）来微调去噪策略，包括基于势能的能量奖励和基于力的稳定性奖励，并进行独立优化和组归一化。

Result: 实验表明，Elign生成的构象具有更低的金标准DFT能量和力，同时提高了稳定性。关键的是，推理速度与无引导采样一样快，因为在生成过程中不需要进行能量评估。

Conclusion: Elign成功地将物理引导的成本分摊到训练阶段，通过预训练的MLFF和强化学习优化，在保持快速推理的同时生成了更符合物理规律的3D分子构象，解决了现有方法在计算效率和物理准确性之间的权衡问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有主动信息收集方法需要为特定建模选择（动力学模型、信念更新过程、观测模型、规划器）设计专门的成本函数，缺乏统一框架。需要一种与建模选择无关的通用信息收集成本。

Method: 提出统一框架，通过显式暴露参数、信念和控制之间的因果依赖关系，将信息收集成本与建模选择解耦。基于Massey定向信息推导通用信息收集成本，仅假设马尔可夫动力学和加性噪声，其他方面与建模选择无关。证明互信息成本是该通用成本的特例。

Result: 建立了互信息成本与线性化贝叶斯估计中信息增益的明确理论联系，为基于互信息的主动学习方法提供了理论依据。在线性、非线性和多智能体系统实验中验证了框架的实用性。

Conclusion: 提出的统一框架成功将信息收集成本与具体建模选择解耦，提供了通用的信息收集成本函数，为主动学习方法的理论分析和实际应用提供了更灵活的基础。
...
TL;DR: 论文提出了一种几何视角来建模非平稳强化学习环境，将环境变化建模为可微同伦路径，并开发了HT-RL和HT-MCTS算法来在线跟踪环境变化，在振荡和切换频繁的环境中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的强化学习环境通常是非平稳的，奖励和动态特性会漂移、加速、振荡并触发最优动作的突然切换。现有理论通常使用粗粒度模型来衡量环境变化程度，而不是局部变化方式，这导致跟踪误差和策略抖动问题。

Method: 采用几何视角建模非平稳折扣马尔可夫决策过程，将环境变化建模为可微同伦路径，跟踪最优贝尔曼不动点的诱导运动。提出了长度-曲率-拐点特征来描述内在复杂性，并基于此开发了HT-RL和HT-MCTS算法，这些轻量级包装器在线估计长度、曲率和近平局接近度的代理指标，并相应调整学习或规划强度。

Result: 实验表明，相比匹配的静态基线方法，HT-RL和HT-MCTS在跟踪性能和动态遗憾方面都有改进，在振荡和切换频繁的环境中获得了最大的性能提升。

Conclusion: 通过几何方法建模环境变化的局部特性（而非仅全局变化程度）能够更好地处理非平稳强化学习问题，提出的同伦跟踪方法在复杂变化环境中表现出优越的稳定性和适应性。
...
TL;DR: 本文提出了一种用于不平衡域泛化（IDG）的负主导对比学习方法，通过增强决策边界分离和强制域间后验一致性来应对域偏移和标签偏移的挑战。


<details>
  <summary>Details</summary>
Motivation: 不平衡域泛化（IDG）旨在同时缓解域偏移和标签偏移问题，这两种偏移共同影响模型的决策边界，特别是在跨域的异构长尾分布下。尽管具有实际重要性，但由于技术复杂性和缺乏理论基础，该领域仍未被充分探索。

Method: 提出负主导对比学习（NDCL）方法：1）在对比学习中更强调负样本作为主要信号，增强类间决策边界分离，避免决策边界偏向多数类；2）通过重加权交叉熵策略鼓励类内紧凑性；3）通过预测中心对齐策略强制跨域后验一致性。

Result: 在基准测试上的严格实验验证了NDCL方法的有效性，代码已开源。

Conclusion: 本文通过理论分析建立了IDG的泛化边界，并提出了NDCL方法直接引导决策边界，有效解决了不平衡域泛化中的域偏移和标签偏移问题。
...
TL;DR: 提出基于率失真理论的Transformer中间表示压缩框架，通过权衡比特率与精度实现高效多设备推理，在语言基准测试中优于复杂基线方法。


<details>
  <summary>Details</summary>
Motivation: Transformer在推理时需要大量计算和内存，跨设备分区推理需要压缩中间表示，但现有方法缺乏理论指导的压缩方案。

Method: 建立率失真理论框架，学习紧凑编码在比特率与精度间权衡；推导率与熵之间差距的理论边界；开发PAC风格边界估计方法。

Result: 在语言基准测试中实现显著节省，某些情况下精度反而提升；不同架构和任务的率表现符合理论边界预测，增强了框架的可解释性。

Conclusion: 提出的率失真框架为Transformer表示编码提供了统一理论视角，将信息论概念扩展到深度学习，为高效推理提供了理论指导的压缩方案。
...
TL;DR: STARS是一种无需训练、推理时干预的方法，通过Stiefel流形优化激活导向，最大化并发生成路径的多样性，避免语言模型的模式崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 语言模型倾向于生成高概率的狭窄输出集，导致生成路径同质化并容易发生模式崩溃。基于采样的策略虽然注入随机性，但仍难以保证多个并发生成运行之间的多样性。

Method: STARS在推理时收集并发生成运行的隐藏激活，在Stiefel流形上联合优化多个加性导向方向。该方法最大化被导向激活的几何体积，同时Stiefel流形诱导导向干预的正交性。为了降低延迟，设计了轻量级的一步更新算法。

Result: 在测试用例生成和科学发现基准测试中，STARS始终优于标准采样方法，在不牺牲质量性能的情况下实现了更大的多样性。

Conclusion: STARS通过流形优化将激活导向转化为探索引擎，有效解决了语言模型生成多样性的问题，为推理时干预提供了新的有效方法。
...
TL;DR: 论文提出了一个机制性框架，从特征编码的几何变换角度解释灾难性遗忘，通过可处理模型分析最佳和最坏情况，并在实际模型中验证。
...
Method: 引入机制性框架，将灾难性遗忘解释为个体特征编码的几何变换；通过可处理模型形式化分析；使用Crosscoders在实际模型（Vision Transformer）上进行案例研究。

Result: 分析表明特征变换通过减少特征分配容量和破坏下游计算读取导致遗忘；深度会加剧遗忘；在CIFAR-10序列任务上的案例研究验证了框架的有效性。

Conclusion: 该工作为持续学习提供了新的特征中心词汇表，从机制角度理解遗忘，为设计更好的持续学习算法提供了理论基础。
...
TL;DR: 提出基于文本嵌入的层次聚类方法，通过自适应选择文档让LLM评估来估计簇质量，证明方法查询高效，实验证明优于其他基于分类器的过滤方法
...
Method: 基于文本嵌入的层次聚类方法，自适应选择文档让LLM评估来估计簇质量。方法查询高效，查询文档数量与最小纯叶子子树大小成正比

Result: 理论证明方法在查询效率上具有优势，实验研究表明该方法优于其他基于分类器的过滤方法

Conclusion: 提出的层次聚类方法为LLM训练数据选择提供了一种高效、可扩展的解决方案，能够有效处理大规模文档集合的质量评估问题
...
Conclusion: ViKeR方法通过视觉引导和关键令牌正则化，解决了现有MLLM遗忘学习方法忽视令牌重要性差异和视觉线索的问题，实现了更有效的隐私保护遗忘学习。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM遗忘方法主要通过预测空间的对齐目标来近似从头训练的行为，虽然能减少遗忘内容生成，但可能只是压制遗忘概念，这些概念仍存在于表示中并与保留知识纠缠

Method: 引入CLReg对比表示正则化器，识别遗忘特征并将其推离保留特征，明确减少遗忘-保留干扰，同时对保留特征进行最小偏移

Result: 在不同规模的LLM和遗忘基准测试中，CLReg减少了遗忘-保留表示纠缠，促进了主流遗忘方法的效果，且不增加额外的隐私风险

Conclusion: 通过重塑表示空间来移除遗忘概念是未来工作的方向，CLReg为减少表示纠缠提供了理论见解和实践方法
...
TL;DR: 提出一种称为"集合逆问题"的新统计问题，并开发基于条件生成模型的非迭代推理方法，用于从观测数据中恢复先验分布，在多个领域展示应用效果。


<details>
  <summary>Details</summary>
Motivation: 解决高能物理中的"展开"问题、全波形反演和未知先验的逆成像问题，这些都需要从受探测器效应扭曲的测量数据中恢复真实的物理分布。

Method: 提出非迭代推理时方法，基于新型条件生成模型（集合逆生成模型），利用观测集合中的集合信息，通过跨多个真值-观测对的训练隐式编码似然模型，避免推理时显式迭代使用前向模型。

Result: 在逆成像、高能物理和全波形反演的合成和真实数据集上进行了基准测试，证明方法能帮助后验推理并推广到未见过的先验分布。

Conclusion: 提出的集合逆生成模型能有效解决集合逆问题，通过利用集合信息和跨先验训练实现后验采样，代码已开源。
...
TL;DR: PerTA：一种基于参数级任务算术的大语言模型遗忘方法，通过重新缩放任务向量来缓解过度遗忘问题，在保持效率的同时提升遗忘效果和模型效用。


<details>
  <summary>Details</summary>
Motivation: 现有任务算术遗忘方法通过减去任务向量来移除隐私信息，但会导致过度遗忘，破坏保留其他信息所需的关键参数。观察到不同参数对遗忘和保留的重要性不同，需要更精细的参数级调整。

Method: 提出参数级任务算术（PerTA），为每个参数重新缩放任务向量，通过梯度（PerTA-grad）或对角Fisher信息近似（PerTA-fisher）估计每个参数对遗忘与保留的相对重要性权重。
...
TL;DR: 利用AdS/CFT对偶的量子引力全息原理，将流匹配算法与AdS物理结合，实现更快收敛和更高质量的生成模型


<details>
  <summary>Details</summary>
Motivation: 探索量子引力全息原理（特别是AdS/CFT对偶）在生成式机器学习中的应用，为流匹配算法提供物理可解释性，并利用AdS物理开发新的生成建模范式

Method: 使用AdS中的标量场体-边界映射来表示数据从基础分布到学习分布的流动，将流匹配算法与AdS物理相结合，在棋盘玩具数据集和MNIST上进行验证

Result: 模型比无物理的流匹配模型实现更快和更高质量的收敛，提供了流匹配的物理可解释版本

Conclusion: AdS物理和几何在开发新的生成建模范式中具有实用价值，为生成式机器学习提供了物理可解释的框架
...
TL;DR: 提出Cross-Fusion Distance (CFD)度量，用于量化表示空间中数据组间的融合程度，能区分融合改变因素和融合保持因素，具有线性计算复杂度。
...
Method: 提出CFD度量，理论分析其不变性和敏感性特性，在合成实验中验证，并在真实域偏移数据集上评估其与下游泛化性能下降的相关性。

Result: CFD在合成实验中验证了理论特性，在真实数据集上比常用替代方法更接近下游泛化性能下降，提供理论可解释的距离度量。

Conclusion: CFD为表示学习提供了理论可靠且可解释的距离度量，能有效量化数据组间的融合程度，特别适用于域偏移场景。
...
<details>
  <summary>Details</summary>
Motivation: 基础模型在机器学习中取得了显著性能，但往往产生过度自信、未校准的预测。传统的集成方法计算成本过高，不适用于大型基础模型，因此需要一种参数高效的不确定性量化方法。

Method: 提出奇异值集成(SVE)方法，假设权重矩阵的奇异向量构成有意义的"知识方向"。冻结预训练模型的奇异向量，仅训练每个集成成员的奇异值来调节各方向对输出的贡献，通过随机初始化和小批量采样自然产生集成多样性。

Result: SVE在NLP和视觉任务上验证，实现了与显式深度集成相当的不确定性量化效果，同时将基础模型的参数增加控制在1%以内，在保持预测准确性的同时改善了校准效果。

Conclusion: SVE为资源受限环境提供了原则性的不确定性估计方法，通过参数高效的隐式集成实现了基础模型的校准改进，为大型模型的不确定性量化提供了实用解决方案。
...
TL;DR: 该论文通过大规模测量生成式AI模型的推理时间和能耗，揭示了不同任务、模型和配置间存在数量级差异，并提出了一个理解时间和能耗底层机制的分析框架。


<details>
  <summary>Details</summary>
Motivation: 能源已成为机器学习计算的关键资源，虽然测量能耗和观察趋势是重要的第一步，但准确理解诊断能耗差异的原因对于优化至关重要。

Method: 在NVIDIA H100和B200 GPU上对46个模型、7个任务、1858种配置进行大规模推理时间和能耗测量研究，并基于观察结果提出了一个分析框架。

Result: 发现数量级的能耗差异：LLM任务类型可导致25倍能耗差异，视频生成有时比图像生成多消耗100倍以上能量，GPU利用率差异可导致3-5倍能耗差异。

Conclusion: 提出了一个理解时间和能耗底层机制的分析框架，强调时间和能耗由内存、利用率等潜在指标决定，这些指标又受算法、软件和硬件层各种因素影响，该框架也适用于功率受限数据中心的关键指标——每瓦吞吐量。
...
Result: 在多种模型架构和任务上的实验显示，潜在空间正则化带来一致改进；GANPO在分布偏移和噪声下提供更稳健的结构反馈，同时保持可比较的下游性能且计算开销小

Conclusion: 潜在空间正则化是改进语言模型偏好优化的有效方法，GANPO通过对抗性训练实现潜在空间正则化，相比传统词级正则化具有更好的鲁棒性
...
### [207] [GeoNorm: Unify Pre-Norm and Post-Norm with Geodesic Optimization](https://arxiv.org/abs/2601.22095)
...
TL;DR: GeoNorm：基于流形优化的Transformer归一化新方法，用测地线更新替代传统归一化，性能更优且计算成本可忽略


<details>
  <summary>Details</summary>
Motivation: Transformer架构中归一化层的位置（Pre-Norm vs Post-Norm）仍是一个开放问题。作者从流形优化的角度重新思考这个问题，将FFN和注意力层的输出解释为优化中的更新方向。

Method: 提出GeoNorm方法，用流形上的测地线更新替代标准归一化。类似学习率调度，为FFN和注意力组件提出层级的更新衰减策略。

Result: 综合实验表明，GeoNorm在Transformer模型中持续优于现有归一化方法。GeoNorm可以无缝集成到标准Transformer架构中，以可忽略的额外计算成本实现性能提升。

Conclusion: GeoNorm通过流形优化视角重新思考归一化，提供了一种更有效的归一化方法，能够提升Transformer性能而几乎不增加计算负担。
...
### [208] [Boosting CVaR Policy Optimization with Quantile Gradients](https://arxiv.org/abs/2601.22100)
*Yudong Luo,Erick Delage*

Main category: cs.LG

TL;DR: 提出一种改进CVaR-PG算法样本效率的方法，通过引入期望分位数项来利用所有采样数据，在可验证风险规避的领域中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统的CVaR-PG算法面临样本效率低下的问题，因为它只关注尾部性能而忽略了大量采样轨迹，导致数据利用率不高。

Method: 通过将CVaR与期望分位数项相结合进行增强。分位数优化允许动态规划公式化，从而能够利用所有采样数据，提高样本效率。

Result: 在具有可验证风险规避行为的领域中，该算法在马尔可夫策略类中显著优于CVaR-PG，并一致优于其他现有方法。

Conclusion: 通过引入期望分位数项来增强CVaR优化，可以在不改变CVaR目标的前提下显著提高样本效率，为风险规避强化学习提供了更有效的解决方案。
...
### [209] [Prior-Informed Flow Matching for Graph Reconstruction](https://arxiv.org/abs/2601.22107)
*Harvey Chen,Nicolas Zilberstein,Santiago Segarra*

Main category: cs.LG

TL;DR: PIFM是一种用于图重建的条件流模型，通过结合嵌入先验与连续时间流匹配来改进部分观测图的重建质量。
...
Method: 基于置换等变的失真-感知理论，首先使用图子或GraphSAGE/node2vec等先验基于局部信息形成邻接矩阵的初始估计，然后应用整流流匹配来细化估计，将其传输到干净图的真实分布并学习全局耦合。

Result: 在不同数据集上的实验表明，PIFM能持续提升经典嵌入方法的性能，在重建准确率上优于这些方法以及最先进的生成基线模型。

Conclusion: PIFM成功地将嵌入先验与流匹配相结合，为图重建问题提供了一种有效解决方案，弥补了传统嵌入方法与现代生成模型之间的差距。
...
### [210] [Value-Based Pre-Training with Downstream Feedback](https://arxiv.org/abs/2601.22108)
*Shuqi Ke,Giulia Fanti*

Main category: cs.LG

TL;DR: V-Pretraining是一种基于价值的模态无关方法，通过轻量级任务设计器选择与下游任务梯度对齐的预训练任务，从而在自监督预训练中引导模型学习相关能力。


<details>
  <summary>Details</summary>
Motivation: 标准预训练优化固定代理目标（如下一个token预测），可能将计算资源分配偏离感兴趣的下游能力。需要一种方法能够利用少量已验证的目标信息来引导昂贵的自监督预训练。

Method: 引入V-Pretraining：基于价值的模态无关方法，通过轻量级任务设计器重新塑造预训练任务，最大化每个梯度步的价值。任务设计器选择预训练任务（如数据增强），使得预训练损失梯度与下游任务计算的梯度对齐，从而引导预训练朝向相关下游能力。

Result: 在语言模型（0.5B-7B参数）上，V-Pretraining在相同更新预算下，仅使用12%的GSM8K训练样本作为反馈，将推理能力（GSM8K测试Pass@1）相对提升高达18%。在视觉自监督学习中，ADE20K上提升1.07 mIoU，降低NYUv2 RMSE，同时提高ImageNet线性准确率，并提供了持续预训练中token效率提升的初步证据。

Conclusion: V-Pretraining能够有效利用少量下游任务信息引导自监督预训练，显著提升模型在下游任务上的表现，同时保持预训练的无标签特性，为更高效的预训练方法提供了新方向。
...
### [211] [Physics Informed Reconstruction of Four-Dimensional Atmospheric Wind Fields Using Multi-UAS Swarm Observations in a Synthetic Turbulent Environment](https://arxiv.org/abs/2601.22111)
*Abdullah Tasim,Wei Sun*

Main category: cs.LG

TL;DR: 使用无人机集群协同测量，通过双向LSTM网络估计局部风速，再结合物理信息神经网络重建四维大气风场，无需专用风速传感器或固定基础设施。


<details>
  <summary>Details</summary>
Motivation: 传统大气风场测量仪器在边界层存在时空空白，单个无人机只能沿飞行轨迹采样，无法完整重建风场。需要一种能够利用无人机集群协同测量来准确重建四维风场的方法。

Method: 提出一个框架：1）使用合成湍流环境和高保真多旋翼仿真生成训练数据；2）用双向LSTM网络从无人机动力学估计局部风分量；3）将估计结果同化到物理信息神经网络中，重建时空连续风场。

Result: 双向LSTM在低风条件下北向和东向分量RMSE为0.064-0.062 m/s，中风时0.122-0.129 m/s，高风时0.271-0.273 m/s。物理信息重建能恢复1000米高度内风场主要时空结构，保持平均流向和垂直切变。五架无人机集群配置下整体RMSE为0.118-0.154 m/s。
...
### [212] [Learning Hamiltonian Flow Maps: Mean Flow Consistency for Large-Timestep Molecular Dynamics](https://arxiv.org/abs/2601.22123)
...
<details>
  <summary>Details</summary>
Motivation: 哈密顿系统的长时间演化模拟受到数值积分稳定性所需的小时间步长的限制，需要克服这一约束以实现更高效的大时间步长模拟。

Method: 引入学习哈密顿流映射的框架，通过施加时间平均哈密顿动力学的平均流一致性条件，直接从独立相空间样本训练，无需访问未来状态或生成昂贵轨迹。

Result: 方法在多种哈密顿系统中得到验证，特别在机器学习力场分子动力学模拟中表现优异，保持相似的训练和推理成本，但支持显著更大的积分时间步长。

Conclusion: 该方法能够直接利用广泛可用的无轨迹MLFF数据集进行训练，为哈密顿系统的大时间步长模拟提供了有效解决方案，突破了传统积分器的稳定性限制。
...
### [213] [SMOG: Scalable Meta-Learning for Multi-Objective Bayesian Optimization](https://arxiv.org/abs/2601.22131)
*Leonard Papenmeier,Petru Tighineanu*

Main category: cs.LG

TL;DR: SMOG：基于多输出高斯过程的可扩展模块化元学习模型，用于多目标贝叶斯优化，通过结构化联合高斯过程先验和灵活残差核传播元数据不确定性。


<details>
  <summary>Details</summary>
Motivation: 在多目标优化中，通常只能通过黑盒访问问题且测量预算有限。许多应用中有相关优化任务的历史数据，这为元学习加速优化提供了机会。贝叶斯优化已分别扩展到元学习和多目标优化，但结合两者的方法——为多目标贝叶斯优化提供元学习先验——仍未被充分探索。

Method: 提出SMOG模型，基于多输出高斯过程，显式学习目标间的相关性。构建跨元任务和目标任务的结构化联合高斯过程先验，在条件化元数据后，得到闭式目标任务先验，并通过灵活残差多输出核增强。支持分层并行训练：元任务高斯过程拟合一次后缓存，实现与元任务数量的线性扩展。
...
### [214] [Pay for Hints, Not Answers: LLM Shepherding for Cost-Efficient Inference](https://arxiv.org/abs/2601.22132)
*Ziming Dong,Hardik Sharma,Evan O'Toole,Jaya Prakash Champati,Kui Wu*

Main category: cs.LG

TL;DR: LLM Shepherding：通过从LLM获取简短前缀（提示）来增强SLM性能，在数学和编程任务上显著降低成本（42-94%），同时保持准确性。


<details>
  <summary>Details</summary>
Motivation: LLM推理成本高限制了大规模部署，而SLM虽然成本低但准确性不足。现有方法（路由和级联）将LLM视为全有或全无的资源，缺乏灵活性。

Method: 提出LLM Shepherding框架：仅从LLM请求简短前缀（提示）提供给SLM。开发两阶段预测器，联合决定是否需要提示以及请求多少token。

Result: 在数学推理（GSM8K、CNK12）和代码生成（HumanEval、MBPP）基准测试中，相比纯LLM推理降低成本42-94%，相比最先进的路由和级联基线实现高达2.8倍成本降低，同时匹配准确性。

Conclusion: LLM Shepherding通过token级预算控制实现SLM-LLM协作，在保持准确性的同时显著降低推理成本，为大规模部署提供了有效解决方案。
...
### [215] [StepShield: When, Not Whether to Intervene on Rogue Agents](https://arxiv.org/abs/2601.22136)
*Gloria Felicia,Michael Eniolade,Jinfeng He,Zitha Sasindran,Hemant Kumar,Milan Hussain Angati,Sandeep Bandarupalli*

Main category: cs.LG

TL;DR: StepShield是首个评估何时检测到违规而非仅仅是否检测到的基准，包含9,213个代码代理轨迹，提出三种时间指标，揭示LLM检测器比静态分析器早干预2.3倍，混合检测器可减少75%监控成本。


<details>
  <summary>Details</summary>
Motivation: 现有安全基准只报告二元准确率，混淆了早期干预和事后分析。在步骤8检测到违规可实现干预，而在步骤48检测只有法医价值。这种关键区别当前基准无法衡量。

Method: 提出StepShield基准，包含9,213个代码代理轨迹（1,278个标注训练对和7,935个轨迹测试集，8.1%恶意率）。恶意行为基于六个类别的真实安全事件。提出三种时间指标：早期干预率(EIR)、干预间隔和节省的令牌数。

Result: 评估显示LLM检测器达到59%早期干预率，而静态分析器只有26%，性能差距2.3倍，标准准确率指标完全无法发现。混合检测器减少75%监控成本，企业规模五年累计节省1.08亿美元。

Conclusion: 通过将评估重点从"是否"转向"何时"，StepShield为构建更安全、更经济可行的AI代理提供了新基础。代码和数据以Apache 2.0许可证发布。
...
### [216] [PRISM: Distribution-free Adaptive Computation of Matrix Functions for Accelerating Neural Network Training](https://arxiv.org/abs/2601.22137)
*Shenghao Yang,Zhichao Wang,Oleg Balabanov,N. Benjamin Erichson,Michael W. Mahoney*

Main category: cs.LG

TL;DR: PRISM框架通过自适应多项式逼近和随机化草图技术加速矩阵函数计算，无需显式谱边界估计，能自动适应演化谱，在Shampoo和Muon优化器中加速训练。


<details>
  <summary>Details</summary>
Motivation: 矩阵函数（如平方根、逆根、正交化）在神经网络训练的预条件梯度方法中起核心作用。现有迭代算法避免显式特征分解，主要依赖矩阵乘法以适配GPU加速器，但需要进一步加速。

Method: PRISM结合自适应多项式逼近和随机化草图技术：每次迭代通过草图最小二乘问题拟合当前谱的多项式代理，以最小开销适应具体实例。应用于加速Newton-Schulz类迭代的矩阵平方根和正交化计算。

Result: PRISM无需显式谱边界或奇异值估计，能自动适应演化谱。经验表明，PRISM在集成到Shampoo和Muon优化器时能加速训练。

Conclusion: PRISM提供了一个通用框架，通过自适应多项式逼近和随机化草图技术加速矩阵函数计算，在机器学习优化器中实现训练加速。
...
### [217] [Late Breaking Results: Conversion of Neural Networks into Logic Flows for Edge Computing](https://arxiv.org/abs/2601.22151)
*Daniel Stein,Shaoyi Huang,Rolf Drechsler,Bing Li,Grace Li Zhang*

Main category: cs.LG

TL;DR: 该论文提出将神经网络转换为逻辑流以提升CPU执行效率的方法，通过将神经网络转换为决策树，再压缩为if-else结构，在RISC-V CPU上实现了高达14.9%的延迟降低且无精度损失。


<details>
  <summary>Details</summary>
Motivation: 边缘设备通常使用CPU而非GPU，但CPU不擅长大规模乘加运算，更适合执行控制流逻辑。现有研究仍专注于高效执行大量MAC操作，但CPU本身不适合这种数学运算。
...
Conclusion: 通过将神经网络转换为逻辑流，能够更好地利用CPU的控制流执行能力，显著提升边缘设备上神经网络的计算效率。
...
### [218] [Discovering Hidden Gems in Model Repositories](https://arxiv.org/abs/2601.22157)
*Jonathan Kahana,Eliahu Horwitz,Yedid Hoshen*

Main category: cs.LG

TL;DR: 研究发现公共模型库中存在大量被忽视的优质微调模型（"隐藏宝石"），这些模型性能远超热门模型但下载量很少。作者提出基于多臂老虎机问题的模型发现方法，通过共享查询集和激进淘汰策略，将发现效率提升50倍以上。
...
Method: 1) 对2000多个模型进行广泛评估，验证"隐藏宝石"的存在；2) 将模型发现问题形式化为多臂老虎机问题；3) 通过共享查询集和激进淘汰策略加速Sequential Halving搜索算法。

Result: 在Llama-3.1-8B家族中，发现很少下载的检查点能将数学性能从83.2%提升到96.0%，且不增加推理成本。提出的方法仅需每个候选模型50个查询就能检索到顶级模型，将发现速度提升50倍以上。

Conclusion: 公共模型库中存在大量被忽视的优质微调模型，当前社区使用模式未能有效发现最佳模型。通过多臂老虎机方法可以高效发现这些"隐藏宝石"，显著提升模型发现效率。
...
### [219] [Diffusion-based Annealed Boltzmann Generators : benefits, pitfalls and hopes](https://arxiv.org/abs/2601.21026)
*Louis Grenioux,Maxence Noble*

Main category: stat.ML

TL;DR: 研究分析了基于退火蒙特卡洛的玻尔兹曼生成器，发现扩散模型作为骨干时，二阶去噪核比一阶表现更好，确定性积分优于随机变体，但学习型扩散模型因密度估计不准确而表现不佳。


<details>
  <summary>Details</summary>
Motivation: 传统玻尔兹曼生成器使用重要性采样等经典蒙特卡洛方法，需要可处理的似然函数且在复杂高维多模态目标上表现不佳。研究旨在探索基于退火蒙特卡洛的玻尔兹曼生成器，利用扩散模型作为骨干，以克服这些限制。

Method: 通过退火蒙特卡洛构建玻尔兹曼生成器，使用扩散模型作为骨干。在控制的多模态高斯混合模型上进行实证元分析，比较完美学习的扩散模型和从数据训练的扩散模型。测试了随机一阶/二阶去噪核以及基于一阶输运映射的确定性积分方法。

Result: 完美扩散模型下，标准一阶随机积分系统性地失败，而二阶去噪核在可获得协方差信息时显著改善性能。确定性积分方法计算成本更高但优于随机一阶变体。学习型扩散模型的所有变体都难以产生准确的玻尔兹曼生成器，主要瓶颈在于扩散模型的对数密度估计不准确。

Conclusion: 基于退火蒙特卡洛的玻尔兹曼生成器有潜力，但扩散模型作为骨干时存在挑战。二阶去噪核和确定性积分方法能改善性能，但学习型扩散模型的密度估计不准确是主要限制因素，需要改进密度估计方法才能实现有效的玻尔兹曼生成器。
...
### [220] [An efficient, accurate, and interpretable machine learning method for computing probability of failure](https://arxiv.org/abs/2601.21089)
*Jacob Zhu,Donald Estep*

Main category: stat.ML

TL;DR: 提出一种基于Gabriel编辑集的惩罚轮廓支持向量机方法，用于计算复杂系统失效概率，通过自适应采样策略减少计算机模型评估次数，同时保持决策边界几何结构。


<details>
  <summary>Details</summary>
Motivation: 在复杂系统可靠性分析中，需要基于计算机模型评估失效概率，但模型评估通常计算成本高昂。现有方法要么需要大量模型评估，要么无法准确保持决定失效概率的决策边界几何结构。

Method: 提出惩罚轮廓支持向量机方法，基于Gabriel编辑集构建。采用自适应采样策略，在失效边界附近战略性地分配采样点，通过训练点的战略聚类构建局部线性代理边界，保持原始边界的几何一致性。

Result: 证明了两个收敛性结果，在四个测试问题上与多种先进分类方法比较，性能表现优异。并将该方法应用于Lotka-Volterra竞争物种模型的生存概率计算。

Conclusion: 该方法能够显著减少计算机模型评估次数，同时准确保持失效边界的几何结构，为复杂系统可靠性分析提供了一种高效准确的计算工具。
...
### [221] [High-dimensional learning dynamics of multi-pass Stochastic Gradient Descent in multi-index models](https://arxiv.org/abs/2601.21093)
*Zhou Fan,Leda Wang*

Main category: stat.ML

TL;DR: 本文研究了高维多指标模型中多轮小批量SGD的学习动态，在样本维度与数据维度成比例增长的渐近机制下，通过动态平均场方程精确描述了SGD的坐标动态，并分析了不同学习率缩放下SGD、SME和梯度流的差异。


<details>
  <summary>Details</summary>
Motivation: 研究多轮小批量SGD在高维多指标模型中的学习动态，特别是在样本量和数据维度成比例增长的渐近机制下，理解SGD、SME和梯度流之间的动态差异，以及批量大小缩放对学习动态的影响。

Method: 采用渐近分析方法，在样本量n和数据维度d成比例增长的机制下，对于任何亚线性批量大小κ∝n^α（α∈[0,1)），在相应的"临界"学习率缩放下，通过动态平均场方程精确描述SGD的坐标动态。同时开发了SME的高斯扩散近似方法。

Result: 发现对于任何批量大小缩放α∈[0,1)，SGD的极限动态是相同的；在相应的学习率缩放下，SGD、SME和梯度流的动态相互不同，只有在线性模型特殊情况下SGD和SME的动态才重合。在小学习率极限下恢复了梯度流的动态平均场描述，在n/d→∞极限下恢复了一轮/在线SGD的描述。

Conclusion: 本文为高维多指标模型中的多轮小批量SGD提供了精确的动态描述，揭示了不同优化算法（SGD、SME、梯度流）在渐近机制下的本质差异，为理解SGD的随机采样噪声效应提供了理论框架。
...
### [222] [Multilevel and Sequential Monte Carlo for Training-Free Diffusion Guidance](https://arxiv.org/abs/2601.21104)
*Aidan Gleich,Scott C. Schmidler*

Main category: stat.ML

TL;DR: 提出基于顺序蒙特卡洛(SMC)的训练免费引导方法，通过蒙特卡洛近似整合完整去噪分布，构建无偏后验估计器，解决现有方法因点估计导致的偏差和多模态捕捉失败问题
...
Method: 提出顺序蒙特卡洛(SMC)框架，通过蒙特卡洛近似整合完整去噪分布来构建无偏的p_θ(y|x_t)估计器；为保持计算可行性，引入基于多级蒙特卡洛(MLMC)的方差缩减方案

Result: 在CIFAR-10类别条件生成上取得新的SOTA结果，达到95.6%准确率，且每次成功成本比基线低3倍；在ImageNet上，算法相比现有方法获得1.5倍的成本优势

Conclusion: 提出的SMC框架能够有效解决训练免费引导中的后验估计偏差问题，通过整合完整去噪分布实现更准确的条件生成，在计算效率和生成质量上都优于现有方法
...
### [223] [Provably Reliable Classifier Guidance through Cross-entropy Error Control](https://arxiv.org/abs/2601.21200)
*Sharan Sahu,Arisina Banerjee,Yuchen Wu*

Main category: stat.ML

TL;DR: 论文建立了分类器训练质量与扩散模型引导效果之间的定量联系，证明在平滑性假设下，分类器的交叉熵误差控制着引导向量的误差。


<details>
  <summary>Details</summary>
Motivation: 虽然分类器引导的扩散模型通过分类器梯度生成条件样本，但标准分类器训练程序是否能产生有效的扩散引导尚不清楚。需要建立分类器训练与引导对齐之间的定量联系。

Method: 在分类器满足温和平滑性假设的条件下，证明控制每个扩散步骤的交叉熵误差也能控制引导向量的误差。通过理论分析建立误差界限，并构造反例说明平滑性假设的必要性。

Result: 分类器达到条件KL散度ε²时，引导向量的均方误差为Õ(dε)。建立了采样误差的上界，类似于反向log-Sobolev型不等式。同时证明没有平滑性假设时，引导向量控制可能对几乎所有分布都失败。
...
### [224] [A Flexible Empirical Bayes Approach to Generalized Linear Models, with Applications to Sparse Logistic Regression](https://arxiv.org/abs/2601.21217)
*Dongyue Xie,Wanrong Zhu,Matthew Stephens*

Main category: stat.ML

TL;DR: 提出一种灵活的贝叶斯广义线性模型经验贝叶斯方法，采用新颖的均值场变分推断，在算法中估计先验，无需调参，直接优化后验均值和先验参数。


<details>
  <summary>Details</summary>
Motivation: 传统变分推断方法通常需要优化后验密度函数，并且常假设高斯变分分布，对于不同的似然和先验组合需要开发专门的VI方法，缺乏统一框架。

Method: 采用均值场变分推断，直接优化后验均值和先验参数（而非后验密度），减少优化参数数量，可使用L-BFGS和随机梯度下降等可扩展算法，自动确定基于先验和似然的最优后验。

Result: 该方法在稀疏逻辑回归应用中表现出优越的预测性能，通过广泛的数值研究比较，优于现有的稀疏逻辑回归方法。

Conclusion: 该方法提供了一个适用于广泛指数族分布的统一框架，无需为每种似然和先验组合开发专门的VI方法，实现了无调参的灵活贝叶斯建模。
...
### [225] [Bulk-Calibrated Credal Ambiguity Sets: Fast, Tractable Decision Making under Out-of-Sample Contamination](https://arxiv.org/abs/2601.21324)
*Mengqi Chen,Thomas B. Berrett,Theodoros Damoulas,Michele Caprio*

Main category: stat.ML

TL;DR: 提出bulk-calibrated credal ambiguity sets，通过分离处理bulk和tail来解决DRO中Huber污染导致最坏情况风险无限的问题，得到闭式有限目标函数和可优化形式。


<details>
  <summary>Details</summary>
Motivation: 传统DRO在处理Huber污染时，如果污染包含在ambiguity set中，最坏情况风险可能无限，导致目标函数无意义，除非施加强有界性或支撑集假设。需要解决这一挑战。

Method: 提出bulk-calibrated credal ambiguity sets：从数据中学习高概率质量bulk集，考虑bulk内的污染，同时单独限制剩余tail的贡献。这导致闭式的有限mean+sup鲁棒目标函数，并为常见损失和bulk几何形状提供可处理的线性或二阶锥规划。

Result: 通过该框架展示了不精确概率的上期望与最坏情况风险的等价性，实验在重尾库存控制、地理偏移房价回归和人口统计学偏移文本分类上展示了竞争性的鲁棒性-准确性权衡和高效优化时间。

Conclusion: bulk-calibrated credal ambiguity sets有效解决了DRO中Huber污染导致的风险无限问题，提供了解释性强的容忍水平，并在多种实际应用中展示了良好的性能。
...
### [226] [Statsformer: Validated Ensemble Learning with LLM-Derived Semantic Priors](https://arxiv.org/abs/2601.21410)
*Erica Zhang,Naomi Sagan,Danny Tse,Fangzhao Zhang,Mert Pilanci,Jose Blanchet*

Main category: stat.ML

TL;DR: Statsformer：通过集成学习框架将LLM知识融入统计学习，自适应校准LLM先验，保证性能不差于基础学习器的凸组合


<details>
  <summary>Details</summary>
Motivation: 现有方法将LLM指导作为未经验证的启发式方法注入（易受幻觉影响），或将语义信息嵌入单一固定学习器中，存在适应性和范围限制

Method: 采用有防护的集成架构，将LLM衍生的特征先验嵌入线性和非线性学习器集成中，通过交叉验证自适应校准其影响

Result: 信息丰富的先验带来一致的性能提升，而无信息或错误指定的LLM指导会被自动降权，减轻幻觉在各种预测任务中的影响

Conclusion: Statsformer提供了将LLM知识整合到统计学习中的原则性框架，具有oracle式保证，能自适应处理LLM指导的质量
...
### [227] [Questioning the Coverage-Length Metric in Conformal Prediction: When Shorter Intervals Are Not Better](https://arxiv.org/abs/2601.21455)
*Yizhou Min,Yizhou Lu,Lanqi Li,Zhen Zhang,Jiaye Teng*

Main category: stat.ML

TL;DR: 本文批判性地审视了传统共形预测评估指标的充分性，揭示了一种名为"偏见技巧"的欺骗性方法，该方法能在保持覆盖率有效的同时虚假地降低区间长度，但引入了区间不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 传统共形预测评估主要关注覆盖率和区间长度，但作者质疑这些标准指标的充分性。他们发现存在一种欺骗性的方法可以虚假地改善区间长度指标，而保持覆盖率不变，这暴露了当前评估体系的漏洞。

Method: 提出并分析了"偏见技巧"方法：对于任何给定的测试样本，该方法概率性地返回一个区间，该区间要么为空，要么使用调整后的置信水平构建。作者形式化推导了PT实现这种欺骗性改进的条件，并在各种回归和分类任务上提供了广泛的实证证据。此外，引入了新的评估指标"区间稳定性"来检测CP方法是否隐含地使用了类似PT的技术。

Result: 研究表明，偏见技巧确实能在保持边际覆盖率有效的同时，欺骗性地降低区间长度指标。然而，这种方法引入了严重的实际问题：同一输入在算法重复运行中可能产生完全不同的预测区间。新提出的区间稳定性指标能够有效检测这种欺骗性改进。

Conclusion: 仅依赖覆盖率和区间长度来评估共形预测方法是不够的，可能掩盖欺骗性的改进技术。需要引入区间稳定性等新指标来全面评估CP方法的实用性和可靠性，防止评估指标被操纵。
...
### [228] [A Decomposable Forward Process in Diffusion Models for Time-Series Forecasting](https://arxiv.org/abs/2601.21812)
*Francisco Caldas,Sahil Kumar,Cláudia Soares*

Main category: stat.ML

TL;DR: 提出一种模型无关的前向扩散过程，通过谱分解将信号分解为频谱分量，比标准扩散更好地保留季节性等结构化时间模式，兼容现有扩散模型骨干，在标准预测基准上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在时间序列预测中难以有效保留季节性和长期模式等结构化时间模式，标准扩散过程会破坏这些重要特征，需要改进前向扩散过程来更好地保持信号结构。

Method: 提出模型无关的前向扩散过程，使用傅里叶或小波变换将信号分解为频谱分量，根据分量能量分阶段注入噪声，使主导频率在整个扩散轨迹中保持高信噪比，从而改善长期模式的可恢复性。

Result: 在标准预测基准测试中，应用谱分解策略（傅里叶或小波变换）相比使用基线前向过程的扩散模型有持续改进，计算开销可忽略不计，能显著提升预测质量。

Conclusion: 通过谱分解调整前向扩散过程能有效保留时间序列中的结构化模式，提高扩散模型的预测性能，且该方法与现有扩散模型骨干兼容，具有实用价值。
...
### [229] [A Judge-Aware Ranking Framework for Evaluating Large Language Models without Ground Truth](https://arxiv.org/abs/2601.21817)
*Mingyuan Xu,Xinzi Tan,Jiawei Wu,Doudou Zhou*

Main category: stat.ML

TL;DR: 提出一个考虑评判者可靠性的LLM评估框架，通过引入评判者特异性参数来更准确地估计模型质量和评判者可靠性，提高与人类偏好的一致性并实现校准的不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估中，使用LLM作为评判者（LLM-as-a-judge）时，不同评判者的可靠性差异很大。如果对所有评判者同等对待，会导致有偏的排行榜和误导性的不确定性估计。在错误聚合的情况下，更多数据可能使评估更加自信地错误。

Method: 提出评判者感知的排名框架，扩展Bradley-Terry-Luce模型，引入评判者特异性判别参数，从成对比较中联合估计潜在模型质量和评判者可靠性，无需参考标签。建立了可识别性（在自然归一化下）并证明了最大似然估计的一致性和渐近正态性。

Result: 在多个公共基准和新收集的数据集上，该方法提高了与人类偏好的一致性，比未加权的基线方法具有更高的数据效率，并为LLM排名产生了校准的不确定性量化。

Conclusion: 提出的评判者感知排名框架能够更准确地评估LLM在开放任务中的表现，通过考虑评判者可靠性差异来减少偏差，提供可靠的不确定性估计，从而改进LLM评估的鲁棒性和可信度。
...
### [230] [Generative Modeling of Discrete Data Using Geometric Latent Subspaces](https://arxiv.org/abs/2601.21831)
*Daniel Gonzalez-Alvarado,Jonas Cassel,Stefania Petra,Christoph Schnörr*

Main category: stat.ML

TL;DR: 该论文提出在分类分布乘积流形的指数参数空间中使用潜在子空间，作为学习离散数据生成模型的工具。低维潜在空间编码统计依赖关系并移除冗余自由度，通过黎曼几何实现等距变换，使测地线变为直线，从而有效进行流匹配训练。


<details>
  <summary>Details</summary>
Motivation: 离散数据生成建模面临高维分类分布参数空间复杂、冗余自由度多的问题。需要一种方法能够有效捕捉变量间的统计依赖关系，同时降低模型复杂度，提高训练效率。

Method: 在分类分布乘积流形的指数参数空间中引入低维潜在子空间，为参数域配备黎曼几何结构，使得空间和距离通过等距变换相关联，从而测地线变为直线。利用流匹配方法进行模型训练，潜在空间编码统计依赖并移除冗余自由度。

Result: 实验结果表明，降低的潜在维度足以表示生成建模所需的数据。通过黎曼几何结构实现的等距变换使得流匹配训练更加有效，测地线变为直线简化了优化过程。

Conclusion: 在分类分布乘积流形的指数参数空间中使用潜在子空间是一种有效的离散数据生成建模方法。通过黎曼几何结构实现等距变换，使测地线变为直线，简化了流匹配训练，同时低维潜在空间能够充分捕捉数据统计特性。
...
### [231] [On Forgetting and Stability of Score-based Generative models](https://arxiv.org/abs/2601.21868)
*Stanislas Strasman,Gabriel Cardoso,Sylvain Le Corff,Vincent Lemaire,Antonio Ocello*

Main category: stat.ML

TL;DR: 本文通过分析反向扩散过程的马尔可夫链稳定性，为基于分数的生成模型提供了采样误差的定量界限，揭示了随机动力学在采样过程中的收缩机制。


<details>
  <summary>Details</summary>
Motivation: 理解生成模型的稳定性和长期行为是现代机器学习的基本问题。本文旨在量化基于分数的生成模型的采样误差，分析反向时间动力学中马尔可夫链的稳定性和遗忘特性。

Method: 通过建立反向过程的两个结构性条件：Lyapunov漂移条件和Doeblin型小化条件，来确保初始化误差和离散化误差的传播分析。利用马尔可夫链的稳定性理论来研究反向扩散动力学。

Result: 证明了反向扩散动力学在采样轨迹上诱导了收缩机制，从而实现了采样过程的定量稳定性。结果为分析此类方法中的误差传播提供了理论框架。

Conclusion: 研究结果阐明了随机动力学在基于分数的模型中的作用，并为分析此类方法中的误差传播提供了原则性框架，有助于理解生成模型的采样稳定性。
...
### [232] [Clustering in Deep Stochastic Transformers](https://arxiv.org/abs/2601.21942)
*Lev Fedorov,Michaël E. Sander,Romuald Elie,Pierre Marion,Mathieu Laurière*

Main category: stat.ML

TL;DR: 随机初始化噪声改变了Transformer的token动态，防止了确定性模型预测的token坍缩到单点，并导致反极点配置成为吸引子。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer理论通常假设确定性权重，预测token会坍缩到单点，但这无法捕捉标准随机初始化方案。需要研究随机初始化噪声如何改变token动态。

Method: 分析深度Transformer中值矩阵随机初始化产生的噪声，在扩散缩放和token-wise RMS归一化下，证明当层数趋于无穷时，离散token动态收敛到球面上的交互粒子系统，受共同矩阵值布朗噪声驱动。

Result: 随机初始化噪声防止了确定性模型预测的坍缩。对于两个token，证明了由交互强度和token维度控制的相变：反极点配置以正概率成为吸引子。数值实验证实了相变，显示反极点配置在多于两个token时持续存在，抑制噪声会降低准确性。

Conclusion: Transformer的随机初始化噪声对token动态有重要影响，防止坍缩并创造新的吸引子配置，这对理解Transformer的表示学习机制和设计改进的初始化方案有重要意义。
...
### [233] [Diffusion Path Samplers via Sequential Monte Carlo](https://arxiv.org/abs/2601.21951)
...
<details>
  <summary>Details</summary>
Motivation: 需要从已知归一化常数的目标分布中进行有效采样，利用扩散路径平滑插值简单基分布和目标分布的优势

Method: 开发扩散退火Langevin Monte Carlo的实用实现，使用顺序蒙特卡洛采样器估计时变分布的分数，并设计新的控制变量调度最小化分数估计方差

Result: 方法在合成和真实数据集上表现出有效性，并提供理论收敛保证

Conclusion: 提出的扩散基采样器能够有效处理已知归一化常数的目标分布采样问题，结合理论保证和实际性能
...
### [234] [Near-Optimal Private Tests for Simple and MLR Hypotheses](https://arxiv.org/abs/2601.21959)
*Yu-Wei Chen,Raghu Pasupathy,Jordan Awan*

Main category: stat.ML

TL;DR: 提出一种基于高斯差分隐私框架的近乎最优检验方法，在单调似然比条件下进行单侧和双侧检验，通过数据驱动截断的私有均值估计器实现，在保持保守第一类错误控制的同时达到与非私有最优检验相同的渐近相对效率。


<details>
  <summary>Details</summary>
Motivation: 在差分隐私框架下，如何在保护数据隐私的同时进行有效的统计检验是一个重要问题。现有方法在检验效率和隐私保护之间存在权衡，需要开发既能保持统计检验效率又能满足隐私要求的测试方法。

Method: 基于高斯差分隐私框架，开发使用数据驱动截断边界的私有均值估计器，该估计器的总体风险在私有极小极大率范围内（最多相差对数因子）。利用该估计器构建私有检验统计量，在单调似然比条件下进行简单、单侧和双侧检验。

Result: 提出的私有检验方法在保持保守第一类错误控制的同时，达到了与非私有最优检验相同的渐近相对效率。数值实验表明，该方法在中等样本量和隐私损失预算下，优于其他差分隐私方法，且与非私有最优检验的检验功效相当。

Conclusion: 该方法在差分隐私框架下实现了近乎最优的统计检验性能，在保护数据隐私的同时保持了检验的统计效率，为隐私保护下的统计推断提供了有效的解决方案。
...
### [235] [Efficient Stochastic Optimisation via Sequential Monte Carlo](https://arxiv.org/abs/2601.22003)
...
<details>
  <summary>Details</summary>
Motivation: 机器学习中经常遇到梯度不可计算的函数优化问题（如最大边际似然估计、生成模型微调）。传统随机近似方法需要内层采样循环来获取有偏随机梯度估计，计算成本高昂。

Method: 开发序列蒙特卡洛采样器来优化梯度不可计算的函数。用高效的SMC近似替代昂贵的内层采样方法，建立基本递归的收敛性理论。

Result: 该方法在能量基模型的奖励调优等多种设置中表现出有效性，能够显著降低计算成本。

Conclusion: SMC采样器为梯度不可计算的函数优化提供了高效替代方案，在保持理论收敛性的同时大幅减少计算开销。
...
### [236] [Latent-IMH: Efficient Bayesian Inference for Inverse Problems with Approximate Operators](https://arxiv.org/abs/2601.20888)
*Youguang Chen,George Biros*

Main category: stat.ML

TL;DR: Latent-IMH是一种用于贝叶斯线性逆问题的采样方法，利用近似算子Ã生成潜在变量，再用精确算子A进行精炼，将计算成本转移到离线阶段，显著提高计算效率。


<details>
  <summary>Details</summary>
Motivation: 在贝叶斯线性逆问题中，参数到观测值的算子A计算成本高昂。许多应用中A可以分解，便于构建成本效益高的近似算子Ã。需要一种方法利用这种近似来加速采样过程。

Method: 提出Latent-IMH方法，基于Metropolis-Hastings独立采样器。首先使用近似算子Ã生成中间潜在变量，然后使用精确算子A进行精炼。主要优势是将计算成本转移到离线阶段。

Result: 使用KL散度和混合时间界限进行理论分析。数值实验表明，在合理假设下，Latent-IMH在计算效率上优于NUTS等最先进方法，在某些情况下比现有方案快几个数量级。

Conclusion: Latent-IMH是一种高效的贝叶斯逆问题采样方法，通过利用算子分解和近似，显著降低了在线计算成本，在计算效率上优于现有方法。
...
### [237] [Efficient Causal Structure Learning via Modular Subgraph Integration](https://arxiv.org/abs/2601.21014)
*Haixiang Sun,Pengchao Tian,Zihan Zhou,Jielei Zhang,Peiyi Li,Andrew L. Liu*

Main category: stat.ML

TL;DR: VISTA是一个模块化因果结构学习框架，通过马尔可夫毯分解全局问题为局部子图，使用加权投票机制整合，具有模型无关、并行化支持等特性，在准确性和效率上均有显著提升。


<details>
  <summary>Details</summary>
Motivation: 高维观测数据中的因果结构学习面临计算复杂度高、搜索空间超指数增长等挑战，现有方法难以应对大规模问题。

Method: 基于马尔可夫毯将全局因果结构分解为局部子图，通过加权投票机制整合（指数衰减惩罚低支持边、自适应阈值过滤不可靠边、反馈弧集算法保证无环性）。

Result: 理论证明了有限样本误差界和渐近一致性，在合成和真实数据集上的实验显示VISTA在准确性和效率上均优于多种基线方法。

Conclusion: VISTA提供了一个模型无关、可并行化的因果结构学习框架，有效解决了高维设置下的计算挑战，具有理论保证和实际应用价值。
...
### [238] [A Diffusive Classification Loss for Learning Energy-based Generative Models](https://arxiv.org/abs/2601.21025)
*Louis Grenioux,RuiKang OuYang,José Miguel Hernández-Lobato*

Main category: stat.ML

TL;DR: DiffCLF是一种新的能量基模型训练方法，通过将EBM学习重构为跨噪声级别的监督分类问题，解决了传统方法中的模式盲区问题，同时保持计算效率。


<details>
  <summary>Details</summary>
Motivation: 基于分数的生成模型虽然取得了成功，但能量基模型（EBMs）在组合采样和玻尔兹曼生成器构建等任务中具有独特优势。然而，EBMs训练面临挑战：最大似然法计算成本高，分数匹配法存在模式盲区问题。

Method: 提出Diffusive Classification（DiffCLF）目标函数，将EBM学习重构为跨噪声级别的监督分类问题。该方法可以与标准基于分数的目标函数无缝结合，避免了模式盲区同时保持计算效率。

Result: 在分析性高斯混合案例中，DiffCLF估计的能量与真实值相符；在模型组合和玻尔兹曼生成器采样等任务中，DiffCLF训练的EBMs展现出比现有方法更高的保真度和更广泛的适用性。

Conclusion: DiffCLF为能量基模型训练提供了一种有效解决方案，克服了传统方法的局限性，使EBMs在生成和其他相关任务中具有更好的性能和实用性。
