...
Method: UniFi框架包含CSI净化管道来协调异构数据包并去除突发冗余，以及时间感知注意力模型直接从非均匀CSI序列学习而无需重采样。还创建了CommCSI-HAR数据集，包含真实世界双频通信流量的不规则采样CSI。

Result: 在该数据集和四个公共基准测试上的广泛评估表明，UniFi以紧凑的模型大小实现了最先进的精度，同时完全保持通信吞吐量。

Conclusion: UniFi是首个完全消除侵入式包注入的Wi-Fi ISAC框架，通过直接利用通信数据包的不规则采样CSI，在保持通信性能的同时实现高效感知。
...
TL;DR: 提出一种无需动态时间规整的EEG到语音直接重建框架，通过主题特定生成器从脑电信号生成梅尔频谱图，再通过预训练声码器和ASR合成语音和解码文本，在口语和想象语音上都实现了稳定的声学重建和可比的语言准确性。


<details>
  <summary>Details</summary>
Motivation: 基于EEG的语音重建面临空间分辨率有限、易受噪声影响以及想象语音缺乏时间对齐声学目标等挑战，需要开发无需时间对齐的直接重建方法。

Method: 提出EEG-to-Voice范式：使用主题特定生成器从EEG信号生成梅尔频谱图（开环方式），结合预训练声码器和ASR模块合成语音和解码文本。采用迁移学习域适应方法，先在口语上预训练再适应到想象语音。可选地使用基于语言模型的校正模块修正ASR错误。

Result: 在2秒和4秒语音条件下，口语和想象语音都实现了稳定的声学重建和可比的语言准确性。虽然较长语句的声学相似性下降，但文本级解码性能基本保持。语言模型校正持续降低了CER和WER且未引入语义失真。

Conclusion: 该研究证明了无需显式时间对齐的直接开环EEG-to-Voice重建在口语和想象语音上的可行性，为脑机接口语音通信恢复提供了新方法。
...
TL;DR: 研究将室内垂直水培与机器学习结合，优化罗勒产量并节水，比较了线性回归、LSTM和DNN三种模型，发现DNN在准确率98%与计算效率间取得最佳平衡，适合实际部署。


<details>
  <summary>Details</summary>
Motivation: 农业面临水资源短缺压力，特别是在突尼斯等地区，急需创新、资源高效的解决方案。需要开发智能系统来优化作物产量同时节约水资源。

Method: 开发基于机器学习的预测系统，使用物联网传感器收集CO2、光照等环境参数数据，建立21株罗勒的实验装置，采用Raspberry Pi和Arduino收集10k数据点，训练并评估线性回归、LSTM和深度神经网络三种模型。

Result: LSTM准确率最高达99%，但执行时间比线性回归长10倍，RAM使用量比DNN高约3倍；DNN准确率98%，在计算速度和预测质量间取得良好平衡；线性回归执行时间仅11秒，适合低复杂度或资源有限应用。

Conclusion: DNN模型在准确率和计算效率间达到最佳平衡，适合构建响应式、高精度的农业决策支持系统，未来适合边缘设备部署，为农业水资源管理提供有效解决方案。
...
TL;DR: PaperNet：一种轻量级混合架构，结合时间卷积、通道注意力机制和双向循环模块，用于EEG癫痫检测，在BEED数据集上达到0.96宏F1分数，仅需约0.6M参数。


<details>
  <summary>Details</summary>
Motivation: EEG信号具有丰富的时频结构，但受噪声、被试变异性、多尺度动态等因素影响难以建模。现有轻量级深度学习模型要么仅依赖局部卷积，要么需要沉重的循环模块，需要更高效的解决方案。

Method: 提出PaperNet混合架构：结合时间卷积、通道级残差注意力模块和轻量级双向循环块，专门用于短窗口分类。在BEED数据集上采用明确的主体独立训练协议进行评估。

Result: 在测试集上达到0.96宏F1分数，仅需约0.6M参数，所有四类性能均衡。消融研究验证了时间卷积、残差注意力和循环聚合的贡献。通道注意力权重提供了电极相关性的洞察。

Conclusion: 精心结合时间滤波、通道重加权和循环上下文建模可以在不增加过多计算成本的情况下获得强大的EEG分类性能，模型足够高效，可在资源受限系统上实际部署。
...
TL;DR: 本文提出了一种在非理想同步和非正交传输条件下的多路径SLAM方法，通过贝叶斯框架和因子图解决同步、源分离和地图构建的联合问题。


<details>
  <summary>Details</summary>
Motivation: 现有的协作多路径SLAM方法通常假设基站完美同步且传输序列正交，忽略了基站间干扰。但在实际5G系统中，基站采用静默模式仍会导致定位性能下降，需要解决同步、源分离和地图构建的联合问题。

Method: 提出了一种基于基站的数据关联和同步偏置模型，集成到联合贝叶斯框架中，通过因子图上的和积算法进行推断，分析不同系统配置下联合同步和源分离的影响。

Result: 与假设正交和同步基站的现有协作MP-SLAM方法相比，统计分析显示没有显著的性能下降，表明该方法在实际非理想条件下仍能保持良好性能。

Conclusion: 该方法成功解决了实际5G系统中的同步和干扰问题，为未来6G网络中的多路径SLAM提供了更实用的解决方案，能够在非理想条件下保持定位和地图构建性能。
...
TL;DR: 提出一种流体主动可重构智能表面(FARIS)，结合流体端口重定位和单元主动放大技术，通过交替优化框架最大化6G网络性能


<details>
  <summary>Details</summary>
Motivation: 传统FRIS/ARIS在6G网络中性能有限，需要结合流体重定位和主动放大技术来提升网络性能

Method: 提出FARIS系统，采用交替优化框架联合优化主动放大反射向量和流体主动单元离散选择，考虑实际硬件约束

Result: FARIS在多种环境下始终优于传统FRIS/ARIS，即使使用更少主动单元或更小物理孔径也能获得更高速率

Conclusion: FARIS为6G网络提供了一种有效的性能增强解决方案，通过流体重定位和主动放大的协同优化实现显著性能提升
...
TL;DR: CoDS是一个基于数字语义通信的协同感知框架，将连续语义特征转换为离散比特流，实现与现有数字V2X系统的兼容，同时保持感知精度和传输效率。


<details>
  <summary>Details</summary>
Motivation: 现有语义通信方法主要依赖模拟传输模型，与现代V2X网络的数字架构不兼容，阻碍了实际部署。需要弥合语义通信与数字系统之间的鸿沟。

Method: 1) 语义压缩编解码器提取和压缩任务导向的语义特征；2) 语义模数转换器将连续语义特征转换为离散比特流；3) 不确定性感知网络评估接收特征的可靠性并丢弃解码失败的特征。

Result: CoDS显著优于现有语义通信和传统数字通信方案，在低信噪比条件下实现最先进的感知性能，同时确保与实用数字V2X系统的兼容性。

Conclusion: CoDS成功将语义级传输效率引入实际数字通信系统，解决了语义通信与数字V2X网络兼容性的关键问题，为自动驾驶协同感知的实际部署铺平了道路。
...
TL;DR: 该论文针对稀疏观测和粗量化硬件约束下的Hermitian Toeplitz协方差矩阵估计问题，提出了Q-TSCM和2k-TSCM两种估计器，并进一步开发了基于协方差拟合准则的Q-SPA方法。
...
Method: 在三角抖动量化框架下，提出Q-TSCM估计器补偿量化偏差，其有限比特版本2k-TSCM通过截断预量化观测获得。基于复高斯假设推导非渐近误差界。进一步提出基于协方差拟合准则的Q-SPA方法，通过求解半定规划强制正半定性。

Result: 理论分析揭示了误差与量化水平的二次依赖关系，并通过覆盖系数捕捉稀疏采样模式的影响。数值实验验证了理论发现，并在波达方向估计应用中证明了所提估计器的有效性。

Conclusion: 该研究为硬件约束下的协方差矩阵估计提供了有效的解决方案，所提方法在稀疏观测和粗量化条件下表现良好，特别适用于阵列信号处理等实际应用场景。
...
TL;DR: 提出基于天线几何结构的谱混合协方差函数（GB-SMCF）作为信道先验模型，通过高斯过程回归（GPR）框架从少量导频中实时估计完整信道状态信息，减少50%的导频开销和训练能量。


<details>
  <summary>Details</summary>
Motivation: 无线通信系统中，从少量噪声观测中准确估计信道状态信息（CSI）是一个关键挑战。传统方法需要大量导频传输，导致频谱效率降低和能量消耗增加。需要一种能够利用天线阵列空间结构先验知识，从有限观测中高效恢复完整信道的方法。

Method: 将无线信道建模为发射和接收天线阵列上的复值高斯过程（GP）。提出天线几何结构的谱混合协方差函数（GB-SMCF）来表征信道协方差，捕捉天线阵列的空间结构。开发基于高斯过程回归（GPR）的信道估计框架，使用GB-SMCF作为先验协方差模型，并在线优化超参数。方案中仅从少量发射天线发送导频，在所有接收天线接收，获得噪声部分CSI，然后通过GPR框架处理这些有限观测，在线更新GB-SMCF超参数并实时重建完整CSI。

Result: 仿真结果表明，提出的基于GB-SMCF的估计器优于基线方法，相比传统方案减少了高达50%的导频开销和训练能量。该方法能够从非常有限的观测中准确估计完整信道状态信息。

Conclusion: 该工作提出了一种基于天线几何结构先验的高斯过程回归信道估计框架，能够有效利用天线阵列的空间结构信息，从少量导频观测中实时恢复完整信道状态信息，显著降低了导频开销和训练能量消耗，为大规模MIMO系统的高效信道估计提供了有前景的解决方案。
...
TL;DR: 基于5G NR PDSCH波形的400MHz带宽28GHz毫米波实时多目标检测与跟踪系统，采用RFSoC硬件平台实现低延迟处理


<details>
  <summary>Details</summary>
Motivation: 利用5G NR波形实现实时多目标检测与跟踪，探索毫米波通信与感知一体化技术，为未来智能感知系统提供硬件加速解决方案

Method: 采用RFSoC 4x2硬件平台配合Sivers EVK02001毫米波波束赋形器，在可编程逻辑中实现全硬件加速处理；使用自适应背景减除、CA-CFAR检测、DBSCAN聚类和EKF滤波进行目标检测与跟踪

Result: 成功实现了基于5G NR PDSCH波形的实时多目标检测与跟踪系统，能够处理3D距离-角度张量数据，系统集成了CPU、GPU和FPGA异构计算资源

Conclusion: 该研究展示了5G NR波形在毫米波感知应用中的可行性，为通信感知一体化系统提供了灵活、低延迟的硬件加速解决方案，具有广泛的应用前景
...
TL;DR: 该论文开发了在最小并行性和内存空间约束下的信号处理算法与实现方案，旨在提升低功耗计算硬件的能效。


<details>
  <summary>Details</summary>
Motivation: 为低功耗计算硬件开发能效优化的信号处理算法，在并行性和内存空间受限的条件下实现高效计算。

Method: 提出了四个关键技术：(1) 时钟CMOS逻辑的功耗/能耗模型以选择最优并行度；(2) 整数友好的初等函数近似方法，通过约束分段多项式构造减少查找表大小；(3) 多存储体和单端口内存上混合基流式FFT的无冲突数据放置和执行顺序；(4) 超快速Toeplitz系统求解的快速Schur算法的并行性/内存分析。

Result: 提供了构造性定理、调度方案和设计权衡，支持高效专用加速器的实现。

Conclusion: 该研究为低功耗硬件上的信号处理提供了系统性的算法和实现方案，在并行性和内存约束下实现了显著的能效提升。
...
<details>
  <summary>Details</summary>
Motivation: 多基地雷达系统需要精确的同步，传统GPS依赖方法存在局限。需要开发不依赖GPS的无线同步协议，并量化同步误差对系统性能的影响，以优化系统设计。

Method: 1) 设计分布式无线同步协议：使用双音波形交换进行频率同步，双向波形交换进行时间同步；2) 建立贝叶斯克拉美罗下界(BCRLB)框架，量化同步偏移对联合时延和多普勒估计的影响；3) 通过仿真分析同步偏移对目标定位和速度估计精度的影响；4) 优化同步链路参数以提升系统性能。

Result: 1) 建立了量化同步误差影响的BCRLB分析框架；2) 仿真结果显示残余同步偏移会降低多基地雷达性能；3) 优化同步链路参数可使多基地雷达性能超越单基地雷达并接近理想情况；4) 仿真参数表明实际实现是可行的。

Conclusion: 提出的无线同步协议能有效实现多基地雷达的同步，不依赖GPS。通过BCRLB框架可以量化同步误差影响，优化同步链路参数可显著提升系统性能，实际实现可行，为多基地雷达系统设计提供了理论依据和实用方案。
...
TL;DR: InsCom提出实例级通信，通过场景图生成识别图像实例及其关系，基于任务关键性筛选实例传输，相比语义通信减少7.82倍数据量并提升1.75-14.03 dB质量


<details>
  <summary>Details</summary>
Motivation: 智能网联车辆依赖高速数据传输，但无线资源稀缺限制了其能力。语义通信虽然能提取任务相关信息，但同一语义类别下的不同实例对下游任务的重要性并不相同，仍存在冗余。

Method: 提出实例通信(InsCom)，将通信从语义级提升到实例级：1)使用场景图生成模型识别图像实例及其相互关系；2)基于用户可配置的任务关键性标准（主体语义和关系-对象对）筛选识别出的实例；3)仅传输任务关键实例。

Result: 在多种数据集和无线信道条件下的评估显示，InsCom相比最先进的语义通信系统：1)数据量减少超过7.82倍；2)质量提升1.75-14.03 dB。

Conclusion: InsCom通过实例级通信有效解决了语义通信中残留的冗余问题，显著提高了智能网联车辆在有限无线资源下的传输效率。
...
TL;DR: 本文研究了近场可重构智能表面中相位相关幅度与设计相位偏移在相位误差下的相互耦合，提出了剩余功率指标量化RIS反射信号功率，建立了包含误差耦合的通用近场信道模型，并揭示了相位误差对性能评估的影响。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常将相位偏移误差和相位相关幅度分开处理，忽略了它们之间的相互耦合，这导致理论预测与实测结果存在差异。本文旨在量化这种耦合效应，更准确地评估RIS在实际相位误差条件下的性能。

Method: 1) 引入剩余功率指标量化RIS反射信号功率比例；2) 提出四种像素反射模型捕捉实际条件；3) 基于泰勒展开推导含误差项的剩余功率多项式上界；4) 基于Friis传输公式和投影孔径建立包含误差耦合的通用近场信道模型；5) 利用柯西-布尼亚科夫斯基-施瓦茨不等式和黎曼和推导频谱效率闭式上界。

Result: 1) 剩余功率指标渐近收敛于理论值；2) 当RIS相位偏移接近范围端点时，独立同分布相位误差下的剩余功率小于完全相关相位误差下的值，而在范围中点附近时关系反转；3) 忽略相位相关幅度中的相位误差会导致高估RIS性能增益，这解释了理论与实测结果的差异；4) 随着像素面积减小，频谱效率上界变得更紧。

Conclusion: 相位相关幅度与设计相位偏移之间的耦合对RIS性能有显著影响，忽略这种耦合会导致性能评估偏差。提出的剩余功率指标和近场信道模型为准确评估实际RIS系统性能提供了理论框架，揭示了相位误差分布对性能影响的复杂关系。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的CSI反馈算法在未见环境中的泛化性无法保证，导致部署成本高，需要提升泛化能力

Method: 1) 建模基于簇的信道分布偏移；2) 提出基于物理的分布对齐方法（多簇解耦和细粒度对齐）；3) 基于EYM定理设计高效多簇解耦算法和混合准则增强鲁棒性；4) 提出EG-CsiNet学习框架

Result: 在各种条件下的仿真和模拟到真实实验中，EG-CsiNet相比最先进方法能鲁棒地减少3dB以上的泛化误差
...
Method: 提出混合心跳检测方法，结合模板匹配和深度学习。通过置信度分析评估检测结果：基于信号形态一致性（心跳事件与检测模板的平均相关性）和心跳间隔变异性（归一化标准差）来评估置信度，保留置信度更高的结果。

Result: 在包含34名受试者、924,235个心跳的临床BCG数据集上，混合方法平均绝对间隔误差为20.73毫秒，比单独使用模板匹配减少29.28毫秒，比单独使用深度学习减少10.13毫秒。案例研究显示混合方法能结合两者的互补优势。

Conclusion: 提出的基于置信度分析的混合心跳检测方法能有效结合模板匹配和深度学习的优势，在个体差异和信号质量变化的情况下表现出更好的鲁棒性和准确性，适用于实际的BCG监测场景。
...
TL;DR: 提出AIRT-Masked-CAAE模型，结合卷积特征提取与注意力机制，通过掩码序列自编码策略提升红外热成像缺陷检测性能，减少训练时间30倍。


<details>
  <summary>Details</summary>
Motivation: 传统红外热成像自编码器难以分离细微缺陷特征与背景响应，导致在不同材料和检测条件下缺陷分析效果不佳。

Method: 提出掩码CNN-注意力自编码器(AIRT-Masked-CAAE)，整合卷积特征提取与注意力机制，采用掩码序列自编码策略，让网络从上下文线索推断缺失的热响应，同时抑制背景冗余。

Result: 在PVC、CFRP和PLA材料样本上评估，AIRT-Masked-CAAE在对比度、信噪比和神经网络指标上超越现有最佳红外热成像自编码器，训练时间减少30倍。

Conclusion: AIRT-Masked-CAAE通过掩码序列自编码策略有效提升红外热成像缺陷检测性能，具有更好的泛化能力和训练效率。
...
TL;DR: 柔性智能超表面(FIM)作为6G无线系统的突破性硬件技术，通过动态形变提供新的自由度，在统计CSI下实现多用户MISO下行链路平均和频谱效率最大化。


<details>
  <summary>Details</summary>
Motivation: 传统刚性天线阵列(RAA)发射机缺乏灵活性，而FIM技术能够动态改变物理表面形态，为提升系统性能提供新的自由度。现有研究多依赖瞬时信道状态信息(CSI)，但在实际系统中统计CSI更易获取且更稳定。

Method: 首先推导FIM辅助发射机的空间相关矩阵，然后基于梯度投影法提出迭代FIM优化算法，在统计CSI下最大化多用户MISO下行链路的平均和频谱效率。

Result: 仿真结果表明：在强空间信道相关场景下，FIM辅助系统相比RAA系统提供显著性能增益；而在弱相关信道条件下，增益减小。

Conclusion: FIM技术在统计CSI下仍能提供优于传统RAA系统的性能，特别是在空间信道相关性强的场景中，这验证了FIM作为6G无线系统关键技术的潜力。
...
TL;DR: 本文提出了几种评估线性协方差（LinCov）性能的计算技术，使用高阶统计、约束优化和无迹变换来量化线性化近似的保真度。


<details>
  <summary>Details</summary>
Motivation: 线性协方差技术在不确定性建模中广泛应用，包括航天器导航性能的初步研究。虽然LinCov方法相比蒙特卡洛方法具有更高的计算效率，但它们本质上依赖于线性化近似。理解这些近似的保真度并识别其不足对于航天器导航和任务规划至关重要，特别是在处理高度非线性系统和大状态不确定性的情况下。

Method: 提出了新的LinCov保真度度量方法，使用高阶统计、约束优化和无迹变换来评估线性协方差性能。这些计算技术能够量化线性化近似的准确性。

Result: 开发了一套计算技术来评估线性协方差方法的性能，这些技术能够识别线性化近似在高度非线性系统和大状态不确定性情况下的不足。

Conclusion: 对于航天器导航和任务规划，特别是在处理非线性系统和大不确定性时，评估线性协方差方法的保真度至关重要。本文提出的技术为理解LinCov近似的局限性提供了有效工具。
...
TL;DR: 将超大规模MIMO与正交啁啾分复用波形结合，提出面向近场ISAC的综合架构，通过专用感知子载波和天线设计简化硬件，实现高精度目标定位和三维速度测量。


<details>
  <summary>Details</summary>
Motivation: 解决近场集成感知与通信中的挑战，通过超大规模MIMO和OCDM波形结合，在简化硬件复杂度的同时实现高精度感知和通信性能提升。

Method: 1) 提出UM-MIMO与OCDM波形结合的ISAC架构；2) 使用专用感知子载波和天线设计；3) 基于FMCW检测原理简化接收硬件；4) 引入虚拟双基地感知概念，利用多天线对估计实现高精度定位；5) 利用感知结果增强信道估计。

Result: 仿真结果表明，所提ISAC方案能提高感知精度，同时改善通信性能，在空间非平稳和不相关多径等恶劣信道环境下仍能保持鲁棒性。

Conclusion: UM-MIMO与OCDM波形结合为近场ISAC提供了有效解决方案，通过专用感知子载波设计和虚拟双基地感知概念，在简化硬件的同时实现了高精度感知和通信性能提升。
...
TL;DR: 本文指出现有AirComp-FL忽略了射频功率放大器非线性带来的瞬时峰值功率约束问题，分析了限幅滤波方法的影响，发现梯度分布导致更高PAPR，在多载波OFDM系统中限幅滤波会显著降低FL性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于空中计算的联邦学习(AirComp-FL)忽略了射频功率放大器非线性带来的瞬时峰值功率约束这一关键实际问题，这在实际部署中可能导致性能下降。

Method: 分析了处理瞬时峰值功率约束的经典方法——限幅与滤波组合技术，研究了单载波和多载波OFDM系统中AirComp-FL的瞬时峰值功率约束影响，特别关注梯度值分布导致的更高峰均功率比问题。

Result: 仿真结果表明，在实际设置中，瞬时发射功率经常超过功率放大器限制；应用限幅和滤波会降低FL性能，特别是在多载波OFDM系统中，由于限幅和滤波引起的带内失真，性能下降尤为明显。

Conclusion: AirComp-FL中的瞬时峰值功率约束是一个重要实际问题，梯度分布导致更高PAPR，限幅滤波方法在多载波OFDM系统中会显著降低FL性能，需要新的解决方案来处理这一约束。
...
<details>
  <summary>Details</summary>
Motivation: 针对低概率高后果的停电场景，现有预测方法可能不够准确，需要整合多维度数据（天气、社会经济、基础设施等）来更好地理解极端条件下的停电风险

Method: 整合EAGLE-I停电记录（2014-2024）与天气、社会经济、基础设施和季节性事件数据，评估随机森林、支持向量机、自适应提升和长短期记忆四种机器学习模型

Result: 在密歇根州下半岛县区的大规模数据集上验证，LSTM网络获得最低预测误差，结果显示更强的经济条件和更发达的基础设施与更低的停电发生率相关
...
<details>
  <summary>Details</summary>
Motivation: GNN虽然能有效利用图结构和节点特征，但对抗攻击和噪声会通过消息传递被放大，导致模型脆弱性

Method: 使用对抗鲁棒性评分作为指导，选择性地剪除最可能降低模型可靠性的边，获得更干净、更鲁棒的图表示

Result: 在三个代表性GNN架构和基准数据集上的实验表明，该方法在高扰动情况下显著提升了GNN的防御能力

Conclusion: 基于对抗鲁棒性评估的图剪枝框架能有效增强GNN对攻击和噪声的抵抗力
...
TL;DR: LANCA提出了一种基于加性噪声模型的因果自编码器，通过将ANM作为强归纳偏置，将残差独立性从被动假设转变为显式优化目标，在无监督因果发现中优于现有方法。
...
Method: 提出Latent Additive Noise Model Causal Autoencoder (LANCA)：1) 将加性噪声模型(ANM)作为强归纳偏置；2) 使用确定性Wasserstein Auto-Encoder而非变分自编码器，避免随机编码模糊结构残差；3) 结合可微分的ANM层，将残差独立性从被动假设转变为显式优化目标。

Result: 理论证明：ANM约束虽不能保证一般混合情况下的唯一可识别性，但能将允许的变换从任意微分同胚限制到仿射类，解决分量级不确定性。实证结果：在合成物理基准(Pendulum, Flow)和逼真环境(CANDLE)上优于最先进基线，对复杂背景场景产生的伪相关具有更强鲁棒性。

Conclusion: LANCA通过将ANM作为归纳偏置并显式优化残差独立性，为无监督因果发现提供了有效框架。该方法在理论和实证上都展示了优越性，特别是在处理复杂场景中的伪相关时表现出更强的鲁棒性。
...
TL;DR: SoliReward是一个用于视频生成模型奖励模型训练的系统框架，通过单项目二元标注数据收集、跨提示配对策略、分层渐进查询注意力机制和改进的BT损失函数，解决现有方法中的数据噪声、架构设计和奖励黑客攻击问题。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型的后训练对齐需要有效的奖励模型，但当前方法面临三大挑战：1) 基于提示内成对标注的数据收集存在标签噪声；2) VLM-based奖励模型的架构设计特别是输出机制研究不足；3) 奖励模型在后训练中容易受到奖励黑客攻击。

Method: 提出SoliReward框架：1) 采用单项目二元标注收集高质量低成本数据；2) 使用跨提示配对策略构建偏好对；3) 设计分层渐进查询注意力机制增强特征聚合；4) 引入改进的BT损失函数，显式处理赢-平局场景，正则化正样本分数分布。

Result: 在评估物理合理性、主体变形和语义对齐的基准测试中，该方法在直接奖励模型评估指标上表现更好，并提高了视频生成模型后训练的效果。

Conclusion: SoliReward通过系统化的数据收集、架构设计和损失函数改进，有效解决了视频奖励模型训练中的关键问题，为视频生成模型的后训练对齐提供了更可靠的偏好信号。
...
TL;DR: TIDES是一个基于LLM的无线流量预测框架，通过聚类识别区域异质流量模式，使用提示工程将统计特征转换为结构化输入，并设计DeepSeek模块实现空间对齐，在保持LLM核心层冻结的同时微调轻量组件，显著提升预测准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 下一代无线网络需要智能、自适应的资源管理，而现有的深度学习和大语言模型方法大多忽略了城市尺度流量动态中固有的空间依赖性，这限制了无线流量预测的准确性和可扩展性。

Method: 1) 通过聚类机制识别区域异质流量模式，为每个区域训练个性化模型；2) 引入提示工程方案，将统计流量特征嵌入为结构化输入；3) 设计DeepSeek模块，通过跨域注意力实现空间对齐；4) 仅微调轻量组件，冻结核心LLM层。

Result: 在真实世界蜂窝流量数据集上的实验表明，TIDES在预测准确性和鲁棒性方面显著优于现有最先进基线方法，证明了将空间感知集成到LLM预测器中的有效性。
...
TL;DR: 提出Latent Sculpting框架，通过两阶段表征学习解决高维表格数据中的"泛化崩溃"问题，在零样本异常检测上取得显著提升


<details>
  <summary>Details</summary>
Motivation: 监督深度学习在高维表格数据中存在"泛化崩溃"问题：模型能学习已知分布的精确决策边界，但在面对分布外数据时表现灾难性失败。作者认为这源于潜在空间缺乏拓扑约束，导致扩散的流形使得新异常与正常数据在统计上无法区分。

Method: 提出Latent Sculpting框架：第一阶段使用混合1D-CNN和Transformer编码器，配合新颖的双中心紧凑性损失（DCCL），将正常流量"雕刻"成低熵的超球面簇；第二阶段基于此预结构化流形，使用掩码自回归流（MAF）学习精确的密度估计。
...
TL;DR: 使用Dueling Double Deep Q-Network和课程学习训练网球策略优化智能体，在模拟环境中实现高胜率，但战术分析显示存在防御性偏差。


<details>
  <summary>Details</summary>
Motivation: 网球策略优化是一个复杂的序列决策问题，涉及分层计分、随机结果、长视野信用分配、身体疲劳和对手技能适应等挑战。现有方法难以稳定处理这些复杂因素。

Method: 构建自定义网球模拟环境，集成完整计分系统（分、局、盘）、10个离散战术动作类别、对称疲劳动态和连续对手技能参数。采用Dueling Double Deep Q-Network(DDQN)架构，通过课程学习逐步提高对手难度（从0.40到0.50）。

Result: 训练后的智能体对平衡对手胜率达98-100%，发球效率63.0-67.5%，回球效率52.8-57.1%。消融研究表明，Dueling架构和课程学习对稳定收敛至关重要，标准DQN基线无法学习有效策略。

Conclusion: 尽管性能优异，但战术分析显示学习策略存在明显防御性偏差，优先避免失误和延长回合而非积极得分。这表明在简化体育模拟中，仅依赖胜率驱动的优化存在局限性，强调了奖励设计对现实体育强化学习的重要性。
...
TL;DR: 本文是系列论文的第二部分，聚焦于将物理知识和不确定性量化集成到机器学习中，用于电力变压器健康评估，介绍了物理信息神经网络及其贝叶斯扩展。


<details>
  <summary>Details</summary>
Motivation: 将基于物理的知识与机器学习模型相结合，对于电力变压器的监测、诊断和预测至关重要。第一部分介绍了神经网络基础，本文旨在进一步将物理规律和不确定性量化融入学习过程，以提高模型的可靠性和鲁棒性。

Method: 1. 介绍物理信息神经网络（PINNs）基础，应用于时空热建模和固体绝缘老化分析；2. 提出贝叶斯PINNs作为量化认知不确定性的原则性框架，在稀疏数据下提供稳健预测；3. 概述物理感知和可信机器学习的新兴研究方向。

Result: 论文提出了一个将物理知识和不确定性量化集成到变压器健康评估的完整框架，通过PINNs实现物理约束建模，通过贝叶斯PINNs处理稀疏数据下的不确定性，为关键电力资产提供更可靠的预测工具。

Conclusion: 物理感知和可信赖的机器学习方法在关键电力资产健康管理中具有重要潜力，通过集成物理知识和不确定性量化，能够提供更可靠、鲁棒的预测，为变压器监测、诊断和预测开辟了新的研究方向。
...
TL;DR: 本文是两篇系列论文的第一部分，探讨神经网络及其扩展在电力变压器状态监测和健康管理中的应用，介绍了神经网络基础概念、CNN用于多模态数据监测，以及NN与强化学习的集成。


<details>
  <summary>Details</summary>
Motivation: 电力变压器是电网关键资产，其可靠性直接影响电网韧性和稳定性。传统基于规则或纯物理模型的状态监测方法在处理不确定性、数据有限性和现代运行条件复杂性方面存在困难，需要机器学习方法进行补充和扩展。

Method: 本文作为系列论文第一部分，首先介绍神经网络基本概念，然后探讨卷积神经网络（CNN）在多模态数据条件监测中的应用，最后讨论神经网络概念与强化学习（RL）范式的集成，用于决策和控制。

Result: 论文提出了一个综合框架，将神经网络方法应用于变压器状态监测，能够实现更准确的诊断、预测和控制，克服传统方法的局限性。

Conclusion: 神经网络及其扩展为电力变压器状态监测和健康管理提供了强大的工具，能够处理复杂运行条件和有限数据问题，并指出了未来研究方向。
...
TL;DR: 本文从信号处理角度研究CNN正则化机制，发现L2正则化通过抑制高频权重能量实现低通滤波效果，揭示了准确性与鲁棒性之间的权衡关系。


<details>
  <summary>Details</summary>
Motivation: 尽管L2正则化和Dropout等正则化技术对训练深度神经网络至关重要，但其在特征频率选择方面的物理机制仍不清楚。本文旨在从频谱偏置角度理解正则化如何影响卷积神经网络的频率特性。

Method: 提出视觉诊断框架跟踪训练过程中权重频率的动态演化，引入频谱抑制比(SSR)量化不同正则器的"低通滤波"强度。通过离散径向剖面解决小卷积核(如3x3)的混叠问题，在ResNet-18和CIFAR-10上进行实证研究。

Result: L2正则化相比无正则化基线将高频能量积累抑制超过3倍。揭示了关键的准确性与鲁棒性权衡：L2模型对宽带高斯噪声敏感（因过度专注于低频），但在高频信息丢失（如低分辨率模糊）场景中表现出更强的鲁棒性，在模糊场景中性能优于基线超过6%。

Conclusion: 正则化通过强制强烈的频谱归纳偏置向低频结构，从信号处理角度提供了对泛化机制的新理解，确认了正则化在频率选择中的重要作用。
...
### [32] [Predicting Mycotoxin Contamination in Irish Oats Using Deep and Transfer Learning](https://arxiv.org/abs/2512.22243)
...
TL;DR: 使用神经网络和迁移学习模型预测爱尔兰燕麦作物中多种霉菌毒素污染，发现TabPFN模型表现最佳，收获前90天的天气模式和种子含水量是最重要的预测因子。
...
Method: 研究使用神经网络和迁移学习模型预测爱尔兰燕麦作物中的霉菌毒素污染作为多响应预测任务。评估了五种建模方法：基线多层感知器（MLP）、带预训练的MLP，以及三种迁移学习模型（TabPFN、TabNet和FT-Transformer）。使用回归（RMSE、R²）和分类（AUC、F1）指标评估模型性能，并进行基于排列的变量重要性分析。

Result: 迁移学习方法TabPFN提供了整体最佳性能，其次是基线MLP。变量重要性分析显示，收获前90天的天气历史模式是最重要的预测因子，其次是种子含水量。

Conclusion: 迁移学习模型特别是TabPFN在预测霉菌毒素污染方面表现优异，收获前90天的天气模式和种子含水量是关键的预测因素，这为早期干预和风险管理提供了重要依据。
...
### [33] [Emotion-Inspired Learning Signals (EILS): A Homeostatic Framework for Adaptive Autonomous Agents](https://arxiv.org/abs/2512.22200)
*Dhruv Tiwari*
...
<details>
  <summary>Details</summary>
Motivation: 当前AI依赖静态外部奖励函数，在封闭环境中表现优异但在开放真实环境中脆弱。标准智能体缺乏内部自主性：难以在没有密集反馈下探索、无法适应分布变化、需要大量手动调参。需要一种类似生物情感的内稳态控制机制来解决这些问题。

Method: 提出情感启发学习信号(EILS)统一框架，将情感建模为连续内稳态评估信号（好奇心、压力、信心等）。这些信号从交互历史中推导为向量值内部状态，实时动态调节智能体的优化景观：好奇心调节熵防止模式崩溃，压力调节可塑性克服不活跃，信心适应信任区域稳定收敛。

Result: 论文假设这种闭环内稳态调节能使EILS智能体在样本效率和非平稳适应方面优于标准基线方法（需要实验验证）。

Conclusion: EILS框架通过生物情感类比的内稳态控制机制，为AI在开放环境中的鲁棒自主性提供了新方向，有望解决传统外部奖励函数方法的局限性。
...
### [34] [Completed Hyperparameter Transfer across Modules, Width, Depth, Batch and Duration](https://arxiv.org/abs/2512.22382)
*Bruno Mlodozeniec,Pierre Ablin,Louis Béthune,Dan Busbridge,Michal Klein,Jason Ramapuram,Marco Cuturi*

Main category: cs.LG

TL;DR: 本文提出Complete^{(d)}参数化方法，统一处理宽度、深度、批大小和训练时长的缩放，并研究模块级超参数优化与迁移，显著提升大语言模型训练速度。


<details>
  <summary>Details</summary>
Motivation: 超参数调优对大规模模型训练稳定性和最终性能影响巨大。现有μP等方法实现了跨模型尺寸的全局超参数迁移，但需要扩展到更多缩放维度并探索模块级超参数优化。

Method: 提出Complete^{(d)}参数化方法，统一处理宽度、深度、批大小和训练时长的缩放。研究模块级超参数优化与迁移，分析高维超参数空间的挑战并提出实用指南。

Result: 实验证明，在正确的参数化下，超参数迁移在模块级超参数机制中仍然有效。使用迁移的模块级超参数能显著提升大语言模型的训练速度。

Conclusion: Complete^{(d)}参数化方法能有效统一多个缩放维度，模块级超参数优化与迁移是可行的，为大规模模型训练提供了实用的超参数调优指导。
...
TL;DR: 提出动态值注意力（DVA），用单头注意力替代多头注意力，动态为每个查询生成值，可节省37.6%训练时间并提升学习能力


<details>
  <summary>Details</summary>
Motivation: Transformer的主要内在限制是每个注意力头中对所有查询使用相同的静态值。多头注意力试图解决此问题，但头数受复杂度限制。需要更高效的方法来动态为每个查询生成值。

Method: 提出动态值注意力（DVA），为每个查询动态决定值，从而可以削减所有冗余的注意力头，只保留一个头。由于每个修订后的嵌入已经获取了足够的有用信息，后续的前馈网络也可以完全削减。

Result: DVA相比原始Transformer可节省37.6%的训练时间，同时提高了学习能力。单头动态值注意力足以替代传统的多头注意力结构。

Conclusion: 动态值注意力（DVA）是Transformer的一种更高效替代方案，通过动态为每个查询生成值，只需单头注意力即可达到甚至超越多头注意力的效果，大幅减少计算复杂度。
...
### [36] [BLISS: Bandit Layer Importance Sampling Strategy for Efficient Training of Graph Neural Networks](https://arxiv.org/abs/2512.22388)
*Omar Alsaqa,Linh Thi Hoang,Muhammed Fatih Balin*

Main category: cs.LG

TL;DR: BLISS使用多臂老虎机动态选择每层最信息丰富的节点，解决GNN在大图上的计算瓶颈，在保持或超过全批次训练精度的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 图神经网络在处理大图时面临计算成本高的挑战，因为需要处理每个节点的所有邻居，导致内存和计算瓶颈。现有静态采样方法无法适应节点重要性的动态变化。

Method: 提出BLISS（Bandit Layer Importance Sampling Strategy），使用多臂老虎机框架动态选择每层最信息丰富的节点。该方法平衡探索与利用，确保全面的图覆盖，并能适应GCN和GAT的不同聚合机制。

Result: 实验表明BLISS在保持或超过全批次训练精度的同时，显著降低了计算成本。该方法能适应不同GNN架构，展示了良好的通用性。

Conclusion: BLISS通过动态节点采样有效解决了GNN在大图上的计算瓶颈问题，为高效图神经网络训练提供了有前景的解决方案。
...
### [37] [On the Existence and Behaviour of Secondary Attention Sinks](https://arxiv.org/abs/2512.22213)
*Jeffrey T. H. Wong,Cheng Zhang,Louis Mahon,Wayne Luk,Anton Isopoussu,Yiren Zhao*

Main category: cs.LG

TL;DR: 该论文发现了一种新型注意力汇点——次级汇点，与之前研究的主要汇点不同，次级汇点主要出现在中间层，持续时间可变，吸引较小但仍显著的注意力质量。


<details>
  <summary>Details</summary>
Motivation: 先前研究已发现注意力汇点现象，但主要关注BOS等主要汇点。本文发现存在性质不同的次级汇点，需要系统研究其特性、形成机制和对注意力机制的影响。

Method: 通过11个模型家族的广泛实验，分析次级汇点的出现位置、特性、形成机制和影响。特别研究了中间层MLP模块如何形成次级汇点，以及这些向量与主要汇点方向的对齐关系。

Result: 发现：(1)次级汇点由特定中间层MLP模块形成，这些MLP将token表示映射到与该层主要汇点方向对齐的向量；(2)这些向量的ℓ₂范数决定次级汇点的汇点分数和持续时间；(3)主要汇点在中间层减弱，与次级汇点出现重合。在大规模模型中，汇点层级更确定和频繁。

Conclusion: 次级汇点是与主要汇点性质不同的新型注意力汇点，由中间层MLP模块形成，其范数决定汇点特性。在大模型中汇点层级现象更加明显，这为理解Transformer注意力机制提供了新视角。
...
### [38] [Causality-Inspired Safe Residual Correction for Multivariate Time Series](https://arxiv.org/abs/2512.22428)
*Jianxiang Xie,Yuncheng Hua*

Main category: cs.LG

TL;DR: CRC是一个因果启发的安全残差校正框架，通过解耦自变量和交叉变量动态，采用四重安全机制确保预测性能不退化，实现安全可靠的部署。


<details>
  <summary>Details</summary>
Motivation: 现有多元预测模型（如Transformers和GNNs）存在系统性误差，且缺乏部署时性能不退化的保证。传统残差校正方法虽然可能提高平均准确率，但可能过度校正可靠预测，在未见场景中导致局部失败。

Method: CRC采用分而治之策略：1）使用因果启发编码器通过解耦自变量和交叉变量动态来暴露方向感知结构；2）使用混合校正器建模残差误差；3）采用严格的四重安全机制控制校正过程，防止有害更新。

Result: 在多个数据集和预测骨干模型上的实验表明，CRC能持续提高准确率，消融研究确认其核心安全机制能确保极高的非退化率（NDR）。

Conclusion: CRC是一个即插即用的校正框架，通过因果启发设计和严格安全机制，解决了预测模型部署中的安全缺口问题，适合安全可靠的部署。
...
### [39] [Interpretable and Adaptive Node Classification on Heterophilic Graphs via Combinatorial Scoring and Hybrid Learning](https://arxiv.org/abs/2512.22221)
*Soroush Vahidi*

Main category: cs.LG

TL;DR: 提出一种基于组合推理而非深度消息传递的可解释自适应框架，用于半监督节点分类，在异配性和过渡性图基准上表现优异，兼具可解释性、可调性和计算效率。


<details>
  <summary>Details</summary>
Motivation: 图神经网络在同性配图上表现良好，但在异配性图上（相邻节点常属不同类别）表现不佳。现有基于深度消息传递的方法缺乏可解释性，需要一种更透明、能自适应处理不同图结构的方法。

Method: 1) 基于置信度排序的贪心过程分配标签，使用加性评分函数整合类别先验、邻居统计、特征相似性和训练得到的标签兼容性；2) 少量透明超参数控制各组件权重，实现从同性配到异配性的平滑适应；3) 验证门控混合策略：仅在验证性能提升时将组合预测作为先验注入轻量神经模型，否则保持可解释性。

Result: 在异配性和过渡性基准测试中，与现有GNNs相比具有竞争力，同时在可解释性、可调性和计算效率方面具有优势。所有适应信号仅从训练数据计算，确保无泄漏评估协议。

Conclusion: 提出的基于组合推理的框架为图节点分类提供了一种可解释、自适应且高效的方法，特别适用于异配性图，在保持性能的同时提供了更好的透明度和控制性。
...
### [40] [The Bayesian Geometry of Transformer Attention](https://arxiv.org/abs/2512.22471)
*Naman Aggarwal,Siddhartha R. Dalal,Vishal Misra*

Main category: cs.LG

TL;DR: Transformers在受控环境中精确执行贝叶斯推理，通过几何机制实现：残差流作为信念基底，前馈网络执行后验更新，注意力提供内容寻址路由。
...
Method: 构建"贝叶斯风洞"——后验分布已知且记忆不可能的环境。在两个任务（双射消除和隐马尔可夫模型状态跟踪）中分析小型Transformer的机制。

Result: 小型Transformer以10^-3-10^-4比特精度重现贝叶斯后验，而容量匹配的多层感知机失败。发现Transformer通过几何机制实现贝叶斯推理：残差流作为信念基底，前馈网络执行后验更新，注意力提供路由。

Conclusion: 分层注意力通过几何设计实现贝叶斯推理，解释了注意力的必要性和扁平架构的失败。贝叶斯风洞为从小型可验证系统连接到大型语言模型的推理现象提供了机制基础。
...
### [41] [Müntz-Szász Networks: Neural Architectures with Learnable Power-Law Bases](https://arxiv.org/abs/2512.22222)
...
Result: MSN在奇异目标函数的监督回归中，比MLP误差低5-8倍且参数少10倍。在PINN基准测试（包括奇异ODE和刚性边界层问题）中，MSN实现3-6倍改进，同时学习到与已知解结构匹配的可解释指数。

Conclusion: 理论引导的架构设计可以为科学驱动的函数类带来显著改进，MSN在逼近奇异函数方面比标准神经网络更高效、更准确。
...
### [42] [On Admissible Rank-based Input Normalization Operators](https://arxiv.org/abs/2512.22587)
*Taeyun Kim*

Main category: cs.LG

TL;DR: 现有基于排序的归一化方法在单调变换和批次变化下不稳定，作者提出三个公理定义稳定的排序归一化，并构造了满足这些公理的算子。


<details>
  <summary>Details</summary>
Motivation: 排序归一化在机器学习中被广泛使用，因其对尺度、单调变换和批次变化的鲁棒性。然而，现有可微排序和排名算子在严格单调变换、批次组成变化和微小扰动下不稳定，这种不稳定性源于算子结构设计而非实现细节。

Method: 提出三个公理形式化排序归一化的最小不变性和稳定性要求，证明满足这些公理的算子必须分解为：(i)特征级排序表示和(ii)单调且Lipschitz连续的标量化映射。构造满足这些标准的最小算子。

Result: 证明了现有可微排序算子不满足稳定性公理，构造了满足所有公理的最小算子，并通过实验表明这些约束在现实场景中是非平凡的。

Conclusion: 结果清晰界定了有效排序归一化算子的设计空间，将其与现有基于连续松弛的排序方法正式分离，为构建稳定排序归一化提供了理论基础。
...
### [43] [ReGAIN: Retrieval-Grounded AI Framework for Network Traffic Analysis](https://arxiv.org/abs/2512.22223)
*Shaghayegh Shajarian,Kennedy Marsh,James Benson,Sajad Khorsandroo,Mahmoud Abdelsalam*

Main category: cs.LG

TL;DR: ReGAIN是一个结合流量摘要、检索增强生成和LLM推理的多阶段框架，用于透明准确的网络流量分析，在真实攻击流量上达到95.95%-98.82%的准确率。


<details>
  <summary>Details</summary>
Motivation: 传统网络流量分析系统（基于规则或机器学习）存在高误报率和缺乏可解释性的问题，限制了分析师的信任度。

Method: 采用多阶段框架：1) 从网络流量创建自然语言摘要；2) 嵌入到多集合向量数据库；3) 使用分层检索管道（元数据过滤、MMR采样、两阶段交叉编码器重排序、弃权机制）为LLM响应提供证据引用。

Result: 在真实世界的ICMP ping flood和TCP SYN flood流量数据集上评估，准确率在95.95%到98.82%之间，优于基于规则、经典机器学习和深度学习的基线方法。

Conclusion: ReGAIN通过可信、可验证的响应提供了独特的可解释性，在保持高准确性的同时减少了幻觉并确保基于证据的推理。
...
### [44] [Understanding the Mechanisms of Fast Hyperparameter Transfer](https://arxiv.org/abs/2512.22768)
*Nikhil Ghosh,Denny Wu,Alberto Bietti*

Main category: cs.LG

TL;DR: 论文提出一个分析超参数跨规模迁移的理论框架，证明快速迁移在计算最优网格搜索中比直接调优更高效，并通过μP参数化解释实际观察到的快速迁移现象。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型规模不断增大，使得标准超参数优化变得极其昂贵。需要研究如何从小规模网格搜索中直接迁移最优超参数到大规模模型，同时保持性能损失最小。

Method: 1. 建立超参数跨规模迁移的理论框架，定义"快速迁移"概念；2. 形式化证明快速迁移在计算最优网格搜索中比直接调优更高效；3. 分析μP参数化下的迁移机制，提出优化轨迹分解假设；4. 在不同设置（包括大语言模型预训练）中进行实证验证。

Result: 1. 证明了快速迁移等价于有用迁移，即迁移在计算效率上渐近优于直接调优；2. 发现μP参数化在扩展模型宽度时表现出快速迁移；3. 通过合成设置展示了迁移性能对问题结构的依赖性；4. 实证支持了优化轨迹分解假设，即损失减少包含宽度稳定和宽度敏感两个分量。

Conclusion: 论文为超参数跨规模迁移提供了理论基础，解释了μP参数化在实践中成功的原因，并提出了优化轨迹分解机制，为大规模模型的高效超参数优化提供了新思路。
...
### [45] [DiRL: An Efficient Post-Training Framework for Diffusion Language Models](https://arxiv.org/abs/2512.22234)
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型作为自回归模型的替代方案具有潜力，但现有后训练方法存在计算效率低、训练与推理目标不匹配的问题，限制了在复杂推理任务（如数学）上的表现。

Method: 提出DiRL框架：1）整合FlexAttention加速的块训练和LMDeploy优化的推理架构；2）支持高效的两阶段后训练（监督微调+强化学习）；3）提出DiPO——首个针对dLLMs的无偏组相对策略优化实现。

Result: 在高质量数学数据上训练的DiRL-8B-Instruct模型在dLLMs中达到SOTA数学性能，在多个基准测试中超越了Qwen2.5系列的可比模型。

Conclusion: DiRL框架有效解决了dLLMs后训练的效率和目标匹配问题，通过优化的训练-推理架构和专门的策略优化方法，显著提升了扩散语言模型在复杂推理任务上的性能。
...
### [46] [Fundamental Novel Consistency Theory: $H$-Consistency Bounds](https://arxiv.org/abs/2512.22880)
*Yutao Zhong*

Main category: cs.LG

TL;DR: 该论文提出了H-一致性边界理论，用于分析代理损失与目标损失之间的估计误差关系，为机器学习中的损失函数设计提供理论保证。


<details>
  <summary>Details</summary>
Motivation: 机器学习中，由于计算不可行性或不可微分性，训练时优化的代理损失函数往往与定义任务性能的目标损失不同。需要建立理论框架来分析这两种损失之间的估计误差关系。

Method: 提出H-一致性边界理论框架，首先在二分类任务中建立紧致的分布依赖和独立边界，分析凸代理损失（线性模型和神经网络）和对抗设置。扩展到多分类，为max、sum和约束损失建立H-一致性边界。研究comp-sum损失（如交叉熵、MAE）及其平滑对抗变体。开发统一的边界推导框架，分析增长率和最小化差距。

Result: 建立了首个针对多种损失函数的H-一致性边界理论，包括二分类和多分类的凸代理损失、对抗设置、comp-sum损失等。证明了在某些情况下非平凡的H-一致性边界不可达，发现了平滑代理损失的通用平方根增长率。

Conclusion: H-一致性边界提供了比贝叶斯一致性或H-校准更强的理论保证，比超额误差边界更具信息性。该框架为损失函数设计和代理损失选择提供了理论指导，特别是在对抗性学习场景中。
...
### [47] [Masking Teacher and Reinforcing Student for Distilling Vision-Language Models](https://arxiv.org/abs/2512.22238)
...
<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型虽然性能优异但体积庞大，难以部署在移动或边缘设备上。需要紧凑但能力强的VLM，但大教师模型向小学生模型的知识蒸馏面临挑战，因为尺寸差距导致学生模型难以复现教师模型的复杂高维表示，造成学习不稳定和性能下降

Method: 提出Masters（Masking Teacher and Reinforcing Student）框架：1）首先掩码教师模型的非主导权重以减少不必要的复杂性；2）在训练过程中渐进式恢复教师模型容量；3）集成离线强化学习阶段，包含两个互补奖励：准确性奖励（衡量生成响应的正确性）和蒸馏奖励（量化从教师到学生响应的转移难易度）；4）利用掩码教师预生成的响应进行高效指导，避免在线思考-回答过程

Result: 该方法使学生模型能够以平滑稳定的方式从教师模型学习更丰富的表示，实现强大的性能，同时避免了计算昂贵的在线思考-回答过程

Conclusion: Masters框架通过掩码渐进式强化学习蒸馏，有效解决了大教师模型向小学生模型知识蒸馏时的尺寸差距问题，实现了高效的知识转移和稳定的学习过程
...
### [48] [Theory and Algorithms for Learning with Multi-Class Abstention and Multi-Expert Deferral](https://arxiv.org/abs/2512.22886)
*Anqi Mao*

Main category: cs.LG

TL;DR: 该论文系统研究了多专家延迟学习问题，包括分类中的弃权和回归中的延迟，提出了新的代理损失函数并证明了强一致性保证，在多个数据集上验证了算法有效性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型存在幻觉和高推理成本问题，通过多专家延迟机制可以提升可靠性（不确定输入交给更强专家）和效率（简单查询交给小型蒸馏模型），这激发了多专家延迟学习问题的研究。

Method: 1) 针对分类中的弃权问题，分析基于分数和预测器-拒绝器两种形式，提出新的代理损失函数族并证明强一致性保证；2) 针对多专家延迟分类，设计新的单阶段和两阶段代理损失函数；3) 针对回归延迟，提出支持多专家和多种成本结构的统一框架。

Result: 在CIFAR-10、CIFAR-100和SVHN等数据集上的实验表明，提出的算法性能优越；理论分析证明了代理损失函数具有强H-一致性边界，特别是两阶段场景对常数成本函数可实现H-一致性。

Conclusion: 该论文为多专家延迟学习提供了全面的理论框架和算法设计，解决了现有开放性问题，在分类和回归任务中均实现了可靠的延迟决策，为提升LLM的可靠性和效率提供了系统解决方案。
...
TL;DR: EvoXplain框架揭示高精度模型可能存在多种解释模式，即使预测性能相同，不同训练得到的模型可能依赖不同的内部逻辑机制。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习实践中存在一个被忽视的问题：当两个模型都达到高准确率时，它们是否依赖相同的内部逻辑？还是通过不同甚至相互竞争的机制达到相同结果？作者质疑"高准确率意味着解释正确可信"的假设。

Method: 提出EvoXplain诊断框架，将解释视为从随机优化过程中抽取的样本（而非聚合预测或构建集成模型），通过重复训练分析解释的稳定性，检测是否存在多个不同的解释模式。

Result: 在乳腺癌和COMPAS数据集上测试逻辑回归和随机森林模型，发现尽管所有模型都达到高预测准确率，但它们的解释经常表现出明显的多模态性。即使是通常被认为稳定的逻辑回归模型，在相同数据分割上重复训练也会产生多个分离的解释盆地。

Conclusion: EvoXplain不试图选择"正确"解释，而是使解释不稳定性可见且可量化，揭示单一实例或平均解释可能掩盖多种底层机制的存在。该框架将可解释性重新定义为模型类在重复实例化下的属性，而非单个训练模型的属性。
...
### [50] [Trust Region Masking for Long-Horizon LLM Reinforcement Learning](https://arxiv.org/abs/2512.23075)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的策略梯度方法存在离策略不匹配问题，传统信任区域边界随序列长度平方增长，在长序列任务中变得无效，需要更紧的边界来保证单调改进。

Method: 推导了两个更紧的边界：Pinsker-Marginal边界（O(T^{3/2})）和Mixed边界（O(T)），并提出Trust Region Masking (TRM)方法，排除任何token违反信任区域的整个序列。

Result: TRM方法提供了第一个非空单调改进保证，适用于长序列LLM强化学习，边界依赖于序列级别的最大token级KL散度。

Conclusion: TRM通过序列级别的信任区域控制，解决了长序列LLM强化学习中的离策略不匹配问题，为实际应用提供了理论保证。
...
### [51] [Enhanced geometry prediction in laser directed energy deposition using meta-learning](https://arxiv.org/abs/2512.22241)
*Abdul Malik Al Mardhouf Al Saadi,Amrita Basak*

Main category: cs.LG

TL;DR: 提出基于元学习的跨数据集知识迁移模型，用于激光定向能量沉积（L-DED）中熔道几何形状预测，解决实验数据稀缺和异构问题，能在少量数据下快速适应新沉积条件。


<details>
  <summary>Details</summary>
Motivation: 激光定向能量沉积（L-DED）中熔道几何形状的准确预测常受限于实验数据稀缺和异构性，这些数据来自不同材料、机器配置和工艺参数。需要一种能在有限数据下快速适应新沉积条件的方法。

Method: 提出基于元学习的跨数据集知识迁移框架，研究两种梯度元学习算法：模型无关元学习（MAML）和Reptile。使用来自同行评审文献和内部实验的多个数据集，评估粉末送料、线材送料和混合线材-粉末L-DED工艺。

Result: MAML和Reptile都能在仅使用3-9个训练样本的情况下，对未见目标任务实现准确的熔道高度预测，始终优于在可比数据约束下训练的传统前馈神经网络。在代表不同打印条件的多个目标任务中，元学习模型表现出强大的泛化性能，R平方值高达约0.9，平均绝对误差在0.03-0.08毫米之间。

Conclusion: 基于元学习的跨数据集知识迁移模型能有效解决L-DED中数据稀缺和异构问题，实现跨异构L-DED设置的有效知识迁移，为有限数据下的快速工艺优化提供了有前景的解决方案。
...
### [52] [Taming the Tail: Stable LLM Reinforcement Learning via Dynamic Vocabulary Pruning](https://arxiv.org/abs/2512.23087)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型的强化学习面临训练-推理不匹配的根本矛盾：高吞吐量推理引擎和数值精确训练系统从相同参数产生不同的概率分布，导致训练与推理不一致

Method: 提出将RL目标约束到动态剪枝的"安全"词汇表，排除极端低概率词元（尾部词元），用小的有界优化偏差替代大的系统性偏差不匹配

Result: 该方法实现了稳定的训练，理论上可以界定词汇表剪枝引入的优化偏差

Conclusion: 通过动态剪枝低概率词元，可以有效解决训练-推理不匹配问题，实现稳定的大语言模型强化学习训练
...
### [53] [Fairness Evaluation of Risk Estimation Models for Lung Cancer Screening](https://arxiv.org/abs/2512.22242)
*Shaurya Gaur,Michel Vitale,Alessa Hering,Johan Kwisthout,Colin Jacobs,Lena Philipp,Fennie van der Graaf*

Main category: cs.LG

TL;DR: 评估两种深度学习肺癌风险预测模型（Sybil和Venkadesh21）在不同人口亚组中的性能差异和公平性，发现存在统计学显著差异且无法用临床混杂因素解释。


<details>
  <summary>Details</summary>
Motivation: 肺癌是全球癌症相关死亡的主要原因，低剂量CT筛查可早期发现并降低死亡率，但AI模型在不同人口群体中的性能差异和公平性尚未明确，需要评估潜在的性能差异和偏见。

Method: 使用JustEFAB框架评估两种深度学习风险预测模型（Sybil和Venkadesh21）以及PanCan2b逻辑回归模型。模型基于美国国家肺癌筛查试验（NLST）数据训练，在保留的NLST验证集上评估，分析不同人口亚组的AUROC、敏感性和特异性，并探讨临床混杂因素的影响。

Result: Sybil在女性（AUROC 0.88）和男性（AUROC 0.81）之间存在统计学显著差异（p<0.001）。Venkadesh21在90%特异性下，黑人参与者敏感性（0.39）显著低于白人参与者（0.69）。这些差异无法用现有临床混杂因素解释，根据JustEFAB框架可归类为不公平偏见。
...
### [54] [How Much Data Is Enough? Uniform Convergence Bounds for Generative & Vision-Language Models under Low-Dimensional Structure](https://arxiv.org/abs/2512.23109)
*Paul M. Thompson*

Main category: cs.LG

TL;DR: 研究生成模型和视觉语言模型在科学医疗决策中的均匀收敛性，提出在低维语义表示平滑依赖假设下，可获得有限样本的均匀准确性和校准保证。


<details>
  <summary>Details</summary>
Motivation: 生成模型和视觉语言模型在科学医疗决策中需要准确且校准良好的概率预测。尽管在中等数据量下表现良好，但尚不清楚预测是否能在输入、类别或子群体间均匀泛化，而非仅平均表现良好。这在生物医学中尤为关键，因为罕见病症和特定群体可能在总体损失较低时仍出现大误差。

Method: 从有限样本角度研究问题，关注通过变化提示或语义嵌入在受限表示空间内获得的分类器族。假设模型输出对低维语义表示平滑依赖（基于文本和图像-文本嵌入的谱结构支持），应用经典均匀收敛工具。主要结果给出在提示嵌入Lipschitz稳定性下，VLM诱导分类器的准确性和校准泛函的有限样本均匀收敛界。

Result: 样本复杂度取决于内在/有效维度而非环境嵌入维度，进一步推导了谱依赖界，明确特征值衰减如何控制数据需求。结果表明在数据有限的生物医学建模中，当前数据集大小可能支持均匀可靠预测，并解释了平均校准指标可能遗漏最坏情况下的校准错误。

Conclusion: 在模型输出对低维语义表示平滑依赖的假设下，生成模型和视觉语言模型可以在有限样本下实现均匀准确和校准的预测。这对生物医学建模具有重要意义，特别是当需要确保罕见病症和特定群体的预测可靠性时，而不仅仅是平均性能。
...
### [55] [Principled Algorithms for Optimizing Generalized Metrics in Binary Classification](https://arxiv.org/abs/2512.23133)
*Anqi Mao,Mehryar Mohri,Yutao Zhong*

Main category: cs.LG

TL;DR: 提出METRO算法，用于优化类别不平衡或代价不对称场景下的广义度量指标（如Fβ、AM、Jaccard等），通过将度量优化转化为广义代价敏感学习问题，设计具有可证明H-一致性的代理损失函数。


<details>
  <summary>Details</summary>
Motivation: 在类别不平衡或代价不对称的应用中，传统二元分类损失不适用，而Fβ、AM、Jaccard等广义度量指标更合适。但优化这些度量存在计算和统计挑战，现有方法通常依赖贝叶斯最优分类器，使用基于阈值的方法，缺乏对受限假设集的定制和有限样本性能保证。

Method: 将度量优化重新表述为广义代价敏感学习问题，设计具有可证明H-一致性保证的新型代理损失函数。基于此框架开发METRO算法，提供强理论性能保证。

Result: 提出的METRO算法具有H-一致性和有限样本泛化边界。实验结果表明，该方法相比先前基线方法更有效。

Conclusion: 该工作为优化广义度量指标提供了原则性算法，具有理论保证，解决了现有方法在受限假设集和有限样本性能方面的不足。
...
### [56] [Communication Compression for Distributed Learning with Aggregate and Server-Guided Feedback](https://arxiv.org/abs/2512.22623)
*Tomas Ortega,Chun-Yin Huang,Xiaoxiao Li,Hamid Jafarkhani*

Main category: cs.LG

TL;DR: 提出两种无需客户端状态的新型压缩框架CAFe和CAFe-S，解决联邦学习中偏置压缩的通信瓶颈和隐私问题


<details>
  <summary>Details</summary>
Motivation: 联邦学习面临客户端到服务器上行传输的通信瓶颈，现有偏置压缩技术需要客户端特定的误差反馈机制，这既违反用户隐私又不兼容大规模FL中的无状态客户端

Method: 提出两种框架：1) CAFe使用前一轮全局聚合更新作为所有客户端的共享控制变量；2) CAFe-S在服务器拥有小型私有数据集时，生成服务器引导的候选更新作为更准确的预测器

Result: 理论证明CAFe在非凸且有界梯度差异情况下优于DCGD，CAFe-S收敛到平稳点且收敛速度随服务器数据代表性增强而提升；实验验证了方法优于现有压缩方案
...
### [57] [Calibrating LLM Judges: Linear Probes for Fast and Reliable Uncertainty Estimation](https://arxiv.org/abs/2512.22245)
*Bhaktipriya Radharapu,Eshika Saxena,Kenneth Li,Chenxi Whitehouse,Adina Williams,Nicola Cancedda*

Main category: cs.LG

TL;DR: 使用线性探针从LLM法官的隐藏状态中获取校准的不确定性估计，无需额外模型训练，比现有方法节省约10倍计算成本，在客观和主观任务上均表现优异。


<details>
  <summary>Details</summary>
Motivation: 随着基于LLM的法官在工业应用中变得重要，高效获取良好校准的不确定性估计对生产部署至关重要。现有方法（如语言化置信度和多生成方法）要么校准效果差，要么计算成本高。

Method: 引入基于Brier分数的损失函数训练的线性探针，从推理法官的隐藏状态中提供校准的不确定性估计，无需额外模型训练。

Result: 探针在客观任务（推理、数学、事实性、编码）和主观人类偏好判断上均优于现有方法，实现约10倍计算节省，对未见评估域具有鲁棒泛化能力，高置信度预测准确率更高。

Conclusion: 基于可解释性的不确定性估计为生产中的LLM法官提供了实用、可扩展的即插即用解决方案，虽然会产生保守估计（在简单数据集上表现不佳），但可能有利于优先考虑低误报率的安全关键部署。
...
### [58] [A Simple, Optimal and Efficient Algorithm for Online Exp-Concave Optimization](https://arxiv.org/abs/2512.23190)
*Yi-Han Wang,Peng Zhao,Zhi-Hua Zhou*

Main category: cs.LG

TL;DR: LightONS算法通过延迟昂贵的Mahalanobis投影，将OXO问题的计算复杂度从$\tilde{O}(d^ωT)$降低到$O(d^2 T + d^ω\sqrt{T \log T})$，同时保持最优$O(d \log T)$遗憾界，并解决了SXO的$\tilde{O}(d^{ω+1}/ε)$运行时间开放问题。


<details>
  <summary>Details</summary>
Motivation: 在线指数凹优化(OXO)中的标准算法ONS存在计算瓶颈，每次迭代需要$Ω(d^ω)$次算术操作，总运行时间可达$\tilde{O}(d^ωT)$。对于随机指数凹优化(SXO)，使用ONS需要$\tilde{O}(d^{ω+1}/ε)$运行时间，这是COLT'13提出的开放问题。

Method: 提出LightONS算法，这是ONS的简单变体。利用参数无关在线学习中的域转换技术，引入滞后机制，延迟昂贵的Mahalanobis投影直到必要时才执行，从而减少计算开销。

Result: LightONS将总运行时间降低到$O(d^2 T + d^ω\sqrt{T \log T})$，同时保持最优$O(d \log T)$遗憾界。对于SXO问题，运行时间降低到$\tilde{O}(d^3/ε)$，解决了开放问题。

Conclusion: LightONS算法通过延迟投影机制有效解决了OXO和SXO的计算瓶颈问题，可以作为ONS的高效替代方案，并适用于更广泛的场景，包括梯度范数自适应遗憾、参数化随机赌博机和内存高效在线学习。
...
### [59] [SNM-Net: A Universal Framework for Robust Open-Set Gas Recognition via Spherical Normalization and Mahalanobis Distance](https://arxiv.org/abs/2512.22792)
*Shuai Chen,Chen Wang,Ziran Wang*

Main category: cs.LG

TL;DR: SNM-Net：一种用于开放集气体识别的通用深度学习框架，通过几何解耦机制和马氏距离评分，有效解决电子鼻系统中的特征分布漂移和未知干扰问题。
...
Method: 提出SNM-Net框架，核心创新包括：1）通过级联批归一化和L2归一化实现几何解耦机制，将高维特征投影到单位超球面以消除信号强度波动；2）引入马氏距离作为评分机制，利用类统计信息构建自适应椭球决策边界。该框架与CNN、RNN和Transformer骨干网络兼容。

Result: 在Vergara数据集上的系统实验表明，Transformer+SNM配置达到接近理论性能：AUROC为0.9977，未知气体检测率为99.57%（5% FPR下的TPR）。性能显著优于现有方法，AUROC提高3.0%，与Class Anchor Clustering相比标准差降低91.0%。框架在不同传感器位置上表现出卓越鲁棒性，标准差低于0.0028。

Conclusion: SNM-Net有效解决了准确性与稳定性之间的权衡问题，为工业电子鼻部署提供了坚实的技术基础。该框架通过几何解耦和马氏距离评分，成功应对了开放集气体识别中的特征分布漂移和未知干扰挑战。
...
<details>
  <summary>Details</summary>
Motivation: 激活更新在梯度下降中并非沿最速下降方向，而激活对损失影响更直接，传统归一化缺乏理论依据，需要从优化原理重新理解

Method: 从梯度下降优化原理分析激活更新的不匹配问题，推导出归一化的理论依据，提出替代传统仿射归一化的新方法（包括卷积层的PatchNorm）

Result: 新提出的归一化方法在多个测试中优于传统归一化，提供了归一化的新理论框架，并质疑了"仿射变换+非线性"的模型构建范式

Conclusion: 从优化原理推导归一化提供了新的理论框架，提出的新方法性能更优，建议将归一化视为参数化缩放的激活函数，重新思考模型构建方式
...
### [61] [On the Sample Complexity of Learning for Blind Inverse Problems](https://arxiv.org/abs/2512.23405)
*Nathan Buskulic,Luca Calatroni,Lorenzo Rosasco,Silvia Villa*

Main category: cs.LG

TL;DR: 论文研究了盲逆问题中的学习理论，在线性最小均方误差估计器框架下，建立了闭式最优解表达式，证明了与Tikhonov正则化的等价性，并推导了有限样本误差界。


<details>
  <summary>Details</summary>
Motivation: 盲逆问题中前向算子部分或完全未知，传统非盲方法无法直接应用。现有数据驱动方法缺乏可解释性和理论保证，限制了在成像等应用领域的可靠性。

Method: 在线性最小均方误差估计器(LMMSE)框架下进行理论分析，推导最优估计器的闭式表达式，建立与Tikhonov正则化的等价关系，证明收敛性并推导有限样本误差界。

Result: 建立了闭式最优估计器表达式，证明了与依赖于信号、噪声和前向算子分布的Tikhonov正则化的等价性，获得了收敛结果和有限样本误差界，量化了算子随机性的影响。

Conclusion: 该研究为盲逆问题中的学习提供了严格的理论基础，建立了可解释的框架，揭示了算子随机性对性能的影响，并通过数值实验验证了理论预测的收敛行为。
...
TL;DR: 提出基于模拟的摊销推理方法，使用神经网络从合成飞行数据中学习，无需真实数据即可预测火箭气动参数，在真实飞行中实现零样本迁移。


<details>
  <summary>Details</summary>
Motivation: 传统火箭飞行性能预测方法依赖计算流体力学或经验关联，而数据驱动方法需要大量昂贵的真实飞行数据。需要一种能够利用合成数据学习并直接应用于真实场景的方法。

Method: 采用基于模拟的摊销推理方法，在物理模拟器生成的合成飞行数据上训练神经网络，学习从单个远地点测量值结合发动机和配置特征直接预测阻力系数和推力修正因子。

Result: 在10,000次合成飞行数据上训练，在8次真实飞行中评估，实现远地点预测平均绝对误差12.3米，相比OpenRocket基线预测误差有所降低，展示了零真实训练样本的模拟到真实迁移能力。

Conclusion: 该方法能够从合成数据中学习并直接应用于真实火箭飞行预测，无需真实训练数据，为业余火箭社区提供了一种有效的性能预测工具，同时揭示了理想物理模型与现实条件之间的系统性偏差。
...
### [63] [ML Compass: Navigating Capability, Cost, and Compliance Trade-offs in AI Model Deployment](https://arxiv.org/abs/2512.23487)
*Vassilis Digalakis,Ramayya Krishnan,Gonzalo Martin Fernandez,Agni Orfanoudaki*

Main category: cs.LG

TL;DR: ML Compass：一个将模型选择视为能力-成本前沿约束优化的框架，帮助组织在考虑用户效用、部署成本和合规要求时选择AI模型


<details>
  <summary>Details</summary>
Motivation: 现有能力排行榜不能直接转化为部署决策，存在能力-部署差距。组织需要综合考虑用户效用、部署成本和合规要求来选择合适的AI模型

Method: 提出ML Compass框架，将模型选择视为能力-成本前沿的约束优化问题。包括理论分析（三区制最优配置结构）和实施管道（提取内部度量、估计经验前沿、学习效用函数、推荐模型）

Result: 在对话和医疗两个案例研究中，ML Compass的推荐结果与仅基于能力的排名有显著差异，能产生部署感知的排行榜，并阐明能力、成本和安全之间的权衡如何影响最优模型选择

Conclusion: ML Compass框架通过系统级视角填补了能力-部署差距，为组织在复杂约束下选择AI模型提供了理论指导和实用工具，强调了综合考虑能力、成本和合规的重要性
...
### [64] [Temporal Visual Semantics-Induced Human Motion Understanding with Large Language Models](https://arxiv.org/abs/2512.22249)
*Zheng Xing,Weibing Zhao*

Main category: cs.LG

TL;DR: 提出一种结合时间视觉语义(TVS)与子空间聚类的人体运动分割方法，利用大语言模型从连续帧中提取文本运动信息，并通过时间正则化提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 传统的人体运动分割方法忽视了时间语义探索的重要性。本文旨在利用从人体运动序列中提取的时间视觉语义(TVS)，借助大语言模型的图像到文本能力来增强子空间聚类性能。

Method: 通过大语言模型从连续帧中提取文本运动信息，学习时间相邻信息，开发TVS集成的子空间聚类方法，包含带有时间正则化的子空间嵌入，并引入基于分割输出的反馈优化框架。

Result: 在四个基准人体运动数据集上的实验结果表明，所提方法优于现有的最先进方法。

Conclusion: 通过整合大语言模型提取的时间视觉语义与子空间聚类框架，有效提升了无监督人体运动分割的性能，证明了时间语义信息在该任务中的重要性。
...
### [65] [Trustworthy Machine Learning under Distribution Shifts](https://arxiv.org/abs/2512.23524)
*Zhuo Huang*

Main category: cs.LG

TL;DR: 该研究聚焦于分布偏移下的可信机器学习，旨在提升AI系统的鲁棒性、可解释性和适应性，以应对扰动偏移、领域偏移和模态偏移三种常见分布偏移场景。


<details>
  <summary>Details</summary>
Motivation: 尽管机器学习在AI领域取得了显著进展，从ResNet到Transformer再到LLMs都展现了强大能力，但分布偏移问题仍然是ML系统的"阿喀琉斯之踵"，限制了系统的可靠性和通用性，并引发了AI可信度问题。

Method: 研究将分布偏移分为三类：扰动偏移、领域偏移和模态偏移，并从鲁棒性、可解释性和适应性三个维度系统研究可信度问题，提出相应解决方案和理论洞见。

Result: 研究提出了针对不同分布偏移场景的有效解决方案和基础理论见解，旨在同时提升机器学习的关键问题，如效率、适应性和安全性。

Conclusion: 通过系统研究分布偏移下的可信机器学习，该工作为增强AI系统的鲁棒性、多功能性、责任性和可靠性提供了重要框架，有助于推动更可靠、更可信的AI系统发展。
...
### [66] [Interpretable Perturbation Modeling Through Biomedical Knowledge Graphs](https://arxiv.org/abs/2512.22251)
*Pascal Passigan,Kevin zhu,Angelina Ning*

Main category: cs.LG

TL;DR: 该研究构建了一个融合生物医学知识图谱与LINCS L1000数据的异质图，使用图注意力网络预测药物-细胞对中978个标志基因的表达变化，相比传统MLP基线有更好表现。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习框架主要关注知识图谱中的链接预测和二元药物-疾病关联任务，而未能深入探究药物对基因表达的扰动机制。理解小分子如何影响基因表达对于揭示药物机制、预测脱靶效应和发现药物再利用机会至关重要。

Method: 1) 构建融合PrimeKG++（增强版生物医学知识图谱）和LINCS L1000药物与细胞系节点的异质图；2) 使用MolFormerXL和BioBERT等基础模型初始化多模态嵌入；3) 训练图注意力网络（GAT）及其下游预测头，学习药物-细胞对中978个标志基因的差异表达谱。

Result: 在支架分割和随机分割下，该框架在差异表达基因预测方面优于MLP基线。消融实验（边洗牌和节点特征随机化）进一步证明生物医学知识图谱提供的边信息增强了扰动水平预测能力。

Conclusion: 该框架为机制性药物建模提供了新路径：从二元药物-疾病关联任务转向治疗干预的细粒度转录效应预测，有助于更深入地理解药物作用机制。
...
### [67] [Le Cam Distortion: A Decision-Theoretic Framework for Robust Transfer Learning](https://arxiv.org/abs/2512.23617)
*Deniz Akdemir*

Main category: cs.LG

TL;DR: 本文提出Le Cam Distortion框架，使用方向可模拟性替代对称不变性，为风险可控的迁移学习提供理论保证，在医学影像、自动驾驶等安全关键领域防止负迁移。


<details>
  <summary>Details</summary>
Motivation: 传统无监督域自适应方法强制特征不变性存在根本缺陷：当域信息量不等时（如高质量vs退化传感器），严格不变性需要破坏信息，导致"负迁移"，这在安全关键应用中可能造成灾难性后果。

Method: 基于Le Cam统计实验理论，提出决策理论框架，用方向可模拟性替代对称不变性。引入Le Cam Distortion（由缺陷距离δ(E₁,E₂)量化）作为可模拟性条件下迁移风险的严格上界。通过学习从源到目标的模拟核实现无源退化的迁移。

Result: 在五个实验中：1) HLA基因组学中实现近乎完美的频率估计（相关性r=0.999）；2) CIFAR-10图像分类中保持81.2%准确率（CycleGAN下降34.7%）；3) RL控制中实现安全策略迁移，而基于不变性的方法出现灾难性崩溃。

Conclusion: Le Cam Distortion为风险可控的迁移学习提供了首个理论框架，特别适用于医学影像、自主系统和精准医学等负迁移不可接受的领域。
...
### [68] [Graph Attention-based Adaptive Transfer Learning for Link Prediction](https://arxiv.org/abs/2512.22252)
*Huashen Lu,Wensheng Gan,Guoting Chen,Zhichao Huang,Philip S. Yu*

Main category: cs.LG

TL;DR: 提出GAATNet图注意力自适应迁移网络，结合预训练和微调解决大规模稀疏图上的链接预测问题，通过全局节点嵌入和轻量适配器实现跨数据集知识迁移。


<details>
  <summary>Details</summary>
Motivation: 现有GNN方法在处理大规模稀疏图和跨数据集迁移学习时面临挑战，自监督方法在跨图数据集泛化方面的潜力未被充分探索。

Method: 提出GAATNet，结合预训练和微调捕获跨尺度数据集的全局节点嵌入；设计两个关键策略：1)在自注意力模块中融入远邻嵌入作为偏置以捕获全局特征；2)在微调阶段引入轻量自适配器模块提升训练效率。

Result: 在7个公开数据集上的综合实验表明，GAATNet在链接预测任务中达到了最先进的性能。

Conclusion: 该研究为链接预测任务提供了一个通用且可扩展的解决方案，有效整合了GNN与迁移学习，代码和数据集已公开。
...
TL;DR: 提出基于随机特征与受控微分方程的时序学习框架，仅训练线性输出层，实现高效可扩展的时序建模。


<details>
  <summary>Details</summary>
Motivation: 传统时序学习方法计算复杂，特别是显式路径签名计算效率低下。需要一种既能保留签名核的归纳偏置，又能实现高效训练的方法。

Method: 使用大型随机参数化CDE作为连续时间储层，仅训练线性输出层。提出两个变体：RF-CDE（使用随机傅里叶特征提升输入）和R-RDE（通过log-ODE离散化处理粗糙路径输入）。

Result: 在多个时序基准测试中表现出竞争性或最先进的性能，证明了方法的有效性。

Conclusion: 该方法为显式签名计算提供了实用替代方案，既保留了签名核的归纳偏置，又受益于随机特征的高效性，统一了随机特征储层、连续时间深度架构和路径签名理论。
...
TL;DR: 研究开发了基于真实和合成数据的PCI术后心脏死亡风险预测模型，通过数据增强改善类别不平衡问题，识别出年龄、射血分数、外周动脉疾病和脑血管疾病为最重要的预测因素。


<details>
  <summary>Details</summary>
Motivation: PCI术后患者的长期预后预测对临床决策至关重要，但现有模型在处理类别不平衡（死亡病例较少）时表现不佳，需要改进少数类别的识别能力。

Method: 分析2,044例分叉病变PCI患者数据，使用多种机器学习模型预测3年死亡率。为解决类别不平衡问题，生成500个合成样本加入训练集，并应用置换特征重要性分析特征贡献。

Result: 数据增强显著提高了少数类别的召回率，同时AUROC损失最小，改善了概率质量，并得到更合理的临床风险估计。年龄、射血分数、外周动脉疾病和脑血管疾病被确定为最重要的预测因素。
...
TL;DR: 研究发现物理约束数据生成中的矛盾：显式能量守恒约束在物理一致的方程中是冗余的，而Fabry-Perot振荡对带宽预测影响最大，移除后可显著提升机器学习性能。


<details>
  <summary>Details</summary>
Motivation: 在科学领域中，真实数据稀缺，需要物理约束的数据生成方法。但现有方法往往过度约束模型，不清楚哪些物理组件是必要的，因此需要系统研究不同物理约束对数据生成和机器学习性能的影响。

Method: 对物理约束的光栅耦合器光谱生成器进行系统消融研究：选择性移除显式能量守恒约束、Fabry-Perot振荡、带宽变化和噪声，分析各组件对数据生成和下游机器学习任务的影响。

Result: 发现物理约束矛盾：显式能量守恒约束在物理一致的方程中是冗余的（约束和非约束变体达到相同的守恒精度，平均误差约7×10^-9）。Fabry-Perot振荡主导带宽变化，移除后带宽分布减少72%（从132.3nm降至37.4nm）。噪声添加和重归一化管道引入0.5%非物理负吸收值。生成器速度达200样本/秒，远快于全波求解器。下游机器学习评估显示物理可学习性权衡：移除Fabry-Perot振荡使带宽预测R²提高31.3%，RMSE降低73.8%。

Conclusion: 研究为物理信息数据集设计提供了可操作的指导，强调机器学习性能可作为评估约束相关性的诊断工具。显式能量守恒约束在物理一致的系统中是冗余的，而Fabry-Perot振荡等特定物理特征对机器学习性能有显著影响，需要针对性处理。
...
### [72] [LuxIA: A Lightweight Unitary matriX-based Framework Built on an Iterative Algorithm for Photonic Neural Network Training](https://arxiv.org/abs/2512.22264)
*Tzamn Melendez Carmona,Federico Marchesin,Marco P. Abrate,Peter Bienstman,Stefano Di Carlo,Alessandro Savino Senior*

Main category: cs.LG

TL;DR: LuxIA框架通过Slicing方法解决了光子神经网络模拟中的计算瓶颈，显著降低了内存使用和执行时间，实现了大规模PNN的可扩展模拟和训练。


<details>
  <summary>Details</summary>
Motivation: 当前光子神经网络模拟工具在训练大规模PNN时面临严重的可扩展性挑战，主要由于转移矩阵计算的计算需求导致高内存和时间消耗，限制了PNN的研究和发展。

Method: 提出了Slicing方法，一种高效的转移矩阵计算技术，兼容反向传播算法，并将其集成到统一的模拟和训练框架LuxIA中。

Result: LuxIA在多种光子架构和标准数据集（MNIST、Digits、Olivetti Faces）上的实验评估表明，它在速度和可扩展性方面始终优于现有工具，能够模拟和训练更大规模的PNN。

Conclusion: LuxIA通过解决关键计算瓶颈，推动了PNN模拟技术的发展，使探索和优化更大、更复杂的架构成为可能，促进了光子技术在AI硬件中的更广泛采用和创新。
...
TL;DR: 该论文系统研究了大语言模型在时序模体分析上的表现，提出了LLMTM基准，开发了工具增强的LLM代理，并设计了结构感知调度器来平衡准确性和成本。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在动态图处理中的应用日益广泛，但针对动态图基本单元——时序模体的分析能力尚未得到充分探索。时序模体作为动态图的重要局部特征，能直接反映异常和独特现象，对理解图演化动态和结构特征至关重要。

Method: 1) 提出LLMTM基准，包含6个专门任务和9种时序模体类型；2) 通过大量实验分析不同提示技术和9种LLM模型的影响；3) 开发工具增强的LLM代理，使用精心设计的提示解决任务；4) 提出结构感知调度器，考虑动态图结构特性和LLM认知负荷，智能分配查询到标准提示或更强大的代理。

Result: 实验表明：1) 工具增强的LLM代理能以高精度解决时序模体任务；2) 但高精度带来显著成本；3) 结构感知调度器能有效维持高精度同时降低成本。

Conclusion: 该研究填补了LLM在时序模体分析领域的空白，提出的基准和调度器框架为平衡LLM在动态图分析中的准确性和成本提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 辐射诱导接枝聚合技术虽能精确功能化聚合物薄膜，但由于基膜形态（结晶度、晶粒取向、自由体积）的未报告变异性，导致单体扩散、自由基分布和Trommsdorff效应不一致，造成空间接枝梯度和性能差异，限制了技术的可重复性。

Method: 提出SoDip分层数据驱动框架：1) 使用仅解码器Transformer编码文本过程描述符；2) TabNet和XGBoost建模多模态特征交互；3) 高斯过程回归与狄利克雷过程混合模型进行不确定性量化和异方差处理；4) 贝叶斯优化高效探索高维合成空间。使用ChemDataExtractor 2.0和WebPlotDigitizer构建包含数百项RIG研究的多样化数据集。

Result: 在交叉验证中，SoDip相比高斯过程回归实现了约33%的性能提升，同时提供校准的置信区间，能够识别低重复性区域。其堆叠架构能整合稀疏文本和不同质量的数值输入，优于先前模型。

Conclusion: SoDip框架为接枝聚合研究建立了可重复、形态感知设计的基础，通过整合多模态数据和先进建模技术，解决了辐射诱导接枝聚合中的关键可重复性问题。
...
TL;DR: Valori是一个确定性AI内存基板，用定点算术(Q16.16)替代浮点运算，确保跨平台比特级一致性，解决AI系统中因硬件差异导致的非确定性问题。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统使用浮点运算进行向量嵌入存储和搜索，导致跨硬件架构（如x86 vs ARM）的非确定性：相同模型、输入和代码会产生不同的内存状态和检索结果。这破坏了可重现性和安全部署，导致数据静默分歧，影响受监管行业的审计追踪。

Method: Valori用定点算术(Q16.16)替代浮点内存操作，将内存建模为可重放的状态机。它通过在内存边界强制执行确定性，确保比特级一致的内存状态、快照和搜索结果。

Result: Valori能保证跨平台的比特级相同内存状态、快照和搜索结果。研究表明非确定性出现在索引或检索之前，而Valori在内存边界强制执行确定性。

Conclusion: 确定性内存是可信AI系统的必要基础组件。Valori作为开源参考实现，为解决AI系统中的非确定性问题提供了可行方案。
...
TL;DR: 提出DBAW-PIKAN方法，通过动态平衡自适应加权策略结合Kolmogorov-Arnold网络架构，解决PINNs在多尺度/高频问题中的梯度流刚度和谱偏差问题，显著提升收敛速度和精度。


<details>
  <summary>Details</summary>
Motivation: PINNs在科学计算中取得重要进展，但在处理多尺度或高频特征问题时，面临梯度流刚度和谱偏差的严重挑战，限制了其预测能力。

Method: 提出动态平衡自适应加权物理信息Kolmogorov-Arnold网络(DBAW-PIKAN)，结合基于可学习B样条的Kolmogorov-Arnold网络架构和包含动态衰减上界的自适应加权策略。

Result: 相比基线模型，该方法在不增加计算复杂度的情况下，加速收敛过程并将求解精度提高至少一个数量级。在Klein-Gordon、Burgers和Helmholtz方程等数值基准测试中表现出显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有GAN方法将所有电器统一处理，忽略了间歇性和连续性电器的行为差异，导致训练不稳定和输出保真度有限。标记数据稀缺是非侵入式负载监测算法发展的主要障碍。

Method: 提出Cluster Aggregated GAN混合生成框架：1) 根据电器行为特征路由到专门分支；2) 间歇性电器使用聚类模块分组相似激活模式，为每个集群分配专用生成器；3) 连续性电器使用LSTM生成器捕捉时间演化，通过序列压缩保持训练稳定性。

Result: 在UVIC智能插座数据集上的实验表明，该框架在真实性、多样性和训练稳定性指标上均优于基线方法。将聚类作为主动生成组件显著提高了可解释性和可扩展性。

Conclusion: 该框架为非侵入式负载监测研究中的合成负载生成提供了有效方法，通过专门化处理不同电器类型解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 掩码扩散模型在训练和推理之间存在显著差异：训练使用简化的单步BERT式目标，而推理是多步迭代过程，受模型和调度策略共同控制。这种训练范式与推理轨迹级别的本质脱节，导致推理调度从未在训练中得到优化。

Method: 将MDM生成重新表述为统一的马尔可夫决策过程，联合包含模型和推理调度。应用轨迹级别的组相对策略优化，在共享奖励下协同优化模型参数和调度参数，无需通过多步生成过程进行昂贵的反向传播。

Result: 在四个基准测试（ImageReward、HPS、GenEval、DPG-Bench）上的实证结果表明，该方法能更彻底地对齐训练与推理，并显著提高生成质量。

Conclusion: Co-GRPO通过统一的MDP框架和轨迹级优化，解决了MDM训练与推理之间的脱节问题，实现了模型参数和调度参数的协同优化，显著提升了生成性能。
...
<details>
  <summary>Details</summary>
Motivation: 研究算法管理下以人为本的管理方式是否可行，传统线性方法难以捕捉工人对算法系统的复杂非线性反应

Method: 采用双机器学习框架估计调节中介模型，避免强加限制性函数形式，使用464名零工工人的调查数据

Result: 发现清晰的非线性模式：支持性HR实践提升工人福祉，但在算法监督模糊不清的中间区域，其与绩效的联系减弱；当监督透明可解释时，关系再次增强

Conclusion: 简单线性设定可能得出相反结论；平台设计需注意：部分定义的监督造成混乱，清晰的规则和可信的申诉机制能使强监督可行；双机器学习为组织研究估计条件间接效应提供了方法
...
TL;DR: 提出MHSA-GNN，通过轻量级超网络动态生成Chebyshev滤波器参数，结合双正则化策略解决图异常检测中异常节点伪装和异质性问题。


<details>
  <summary>Details</summary>
Motivation: 现有图异常检测方法在处理复杂异常模式时面临挑战，异常节点常伪装与正常节点混合，导致图中同质性和异质性共存。现有谱图神经网络使用固定全局滤波器，容易导致过平滑，丢失欺诈检测所需的高频信号，且缺乏对不同图实例的自适应能力。

Method: 提出多头谱自适应图神经网络(MHSA-GNN)：1) 设计轻量级超网络，基于包含结构统计和Rayleigh商特征的"谱指纹"，为每个实例动态生成Chebyshev滤波器参数；2) 引入双正则化策略防止多头机制模式崩溃：教师-学生对比学习保证表示准确性，Barlow Twins多样性损失强制头间正交性。

Result: 在四个真实数据集上的实验表明，该方法能有效保留高频异常信号，显著优于现有最先进方法，尤其在高度异质数据集上表现出优秀的鲁棒性。

Conclusion: MHSA-GNN通过实例自适应的谱滤波器和双正则化策略，解决了图异常检测中异常信号保留和异质图适应问题，为金融欺诈检测等应用提供了有效解决方案。
...
TL;DR: 警告性内容在训练数据中并不能有效教导语言模型避免被警告的行为，模型在警告和直接内容两种情况下生成被标记内容的概率几乎相同（76.7% vs 83.3%），因为模型学习的是统计共现而非语用解释。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解为什么包含警告框架的训练数据（如"不要使用-此代码有漏洞"）不能有效教导语言模型避免被警告的行为，探索当前架构中统计学习与语用理解之间的根本差距。

Method: 通过实验比较模型在警告框架内容和直接内容下的行为表现，使用稀疏自编码器分析潜在特征激活模式，特别是特征#8684跟踪代码执行模式，并识别"隐形滑移"现象。

Result: 模型在警告框架下生成被标记内容的概率为76.7%，与直接内容下的83.3%无统计学差异；稀疏自编码器分析显示"描述X"和"执行X"激活重叠的潜在特征；特征#8684在警告和利用情境下激活程度相似。

Conclusion: 当前架构中统计共现主导语用解释，模型学习的是上下文后倾向于出现什么，而非内容出现的原因；提示和推理时引导无法解决此问题，需要训练时的特征消融。
...
TL;DR: 本文提出了一种混合量子-经典混合专家架构，通过量子门控网络作为路由器，利用量子干涉效应实现高效的非线性决策边界建模，相比经典路由器具有参数效率和拓扑优势。


<details>
  <summary>Details</summary>
Motivation: 混合专家架构在扩展深度学习模型方面面临专家不平衡和经典路由机制计算复杂度的限制。本文旨在探索量子机器学习如何通过量子路由机制解决这些限制，特别是在建模复杂非线性决策边界方面的优势。

Method: 提出混合量子-经典混合专家架构，使用量子门控网络作为路由器（采用角度嵌入量子特征映射和波干涉机制），结合经典专家网络。通过消融研究验证量子优势来源，并在非线性可分数据上进行实验验证。

Result: 实验证明量子路由器在Two Moons等非线性可分数据集上具有显著拓扑优势，能有效"解开"经典线性路由器难以分离的数据分布。量子路由器作为高维核方法，在参数效率上优于经典对应物，且对量子噪声具有鲁棒性，适用于NISQ硬件。

Conclusion: 量子路由器通过量子干涉效应实现了高效的非线性决策边界建模，验证了干涉假说。该架构在联邦学习、隐私保护机器学习和自适应系统等应用中具有潜力，为量子增强路由范式提供了可行性证明。
...
TL;DR: 本研究扩展了Sawyer的热点分析，结合机器学习与空间建模技术，分析美国158号公路5年事故数据，识别时空事故模式，为交通安全提供针对性干预建议。


<details>
  <summary>Details</summary>
Motivation: 扩展先前的基础统计分析，通过整合先进统计方法、机器学习和空间建模技术，为美国158号公路这一重要交通走廊提供更全面的交通安全分析，并为农村公路安全研究提供方法学贡献。

Method: 使用核密度估计(KDE)、负二项回归、随机森林分类器，并与公路安全手册(HSM)安全性能函数(SPF)进行比较，同时采用莫兰I检验进行空间聚类分析。

Result: 随机森林分类器预测伤害严重程度准确率达67%，优于HSM SPF；莫兰I检验确认空间聚类(I=0.32, p<0.001)；KDE分析显示主要交叉口附近存在热点区域，验证并扩展了先前热点识别方法。

Conclusion: 研究结果为美国158号公路交通安全提供了可操作的干预建议，同时通过超越基础统计技术的方法学进步，为农村公路安全分析做出了更广泛贡献。
...
<details>
  <summary>Details</summary>
Motivation: 传统PD模型在开发时注重预测准确性，但部署后缺乏对借款人行为变化的持续适应能力，导致静态模型性能随时间退化。同时，金融机构难以将ML模型从开发环境过渡到生产环境并保持其健康状态。

Method: 采用动态端到端模型生命周期管理方法，集成持续模型监控、重新训练和验证的MLOps管道。引入动态冠军-挑战者框架，定期更新基线模型，通过最新数据重新校准独立参数，并使用时间外验证选择最佳性能模型。

Result: 基于决策树的集成模型在违约分类中表现最佳但需要频繁更新；线性模型和神经网络性能退化更严重。PDx能有效缓解数字贷款机构的价值侵蚀，特别是在借款人行为快速变化的短期小额贷款中。

Conclusion: PDx通过MLOps驱动的自适应决策系统解决了传统PD模型的局限性，在点对点借贷、商业贷款和汽车贷款等场景中验证了其可扩展性和适应性，为现代信用风险预测提供了有效解决方案。
...
TL;DR: LLMBoost是一个新颖的LLM集成微调框架，通过利用LLM的中间状态实现层次化错误校正和知识转移，在提升准确性的同时降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM集成学习方法通常将模型视为黑盒，仅组合输入或最终输出，忽略了丰富的内部表示和跨模型交互。需要打破这种限制，更有效地利用模型内部信息来提升性能。

Method: LLMBoost包含三个关键创新：1）跨模型注意力机制，使后续模型能够访问和融合前驱模型的隐藏状态；2）链式训练范式，以错误抑制为目标逐步微调连接模型；3）近并行推理范式，逐层流水线化隐藏状态，实现接近单模型解码的推理效率。

Result: 在常识推理和算术推理任务上的大量实验表明，LLMBoost能够持续提升准确性，同时降低推理延迟。理论分析证明，在有限校正假设下，顺序集成能保证单调改进。

Conclusion: LLMBoost通过利用LLM的中间状态打破了传统集成学习的限制，实现了高效的层次化错误校正和知识转移，为LLM集成学习提供了新的有效范式。
...
TL;DR: 提出OFS方法，在满足公平性和服务率约束下在线学习阈值策略，通过置信区间维护和乐观选择机制，在闭环决策系统中实现高奖励和低约束违反


<details>
  <summary>Details</summary>
Motivation: 闭环决策系统（如贷款、筛选、再犯风险评估）在公平性和服务约束下运行时会产生反馈效应：决策改变未来人群构成，导致数据非平稳并可能放大不平等。需要在线学习方法在满足约束的同时处理这些动态变化

Method: 提出乐观可行搜索（OFS）：基于网格的方法，为每个候选阈值维护奖励和约束残差的置信区间。每轮选择在置信区间下看似可行的阈值，并在其中最大化乐观奖励；若无可行阈值，则选择最小化乐观约束违反的阈值

Result: 在三个基准测试中（合成闭环基准、基于德国信贷和COMPAS的半合成基准），OFS相比无约束和原始对偶bandit基线，获得更高奖励和更小累积约束违反，接近相同扫描程序下最佳可行固定阈值的性能

Conclusion: OFS方法有效解决了闭环决策系统中的在线学习问题，在满足人口均等和服务率约束的同时，实现了接近最优的性能，特别适用于低维可解释策略类
...
<details>
  <summary>Details</summary>
Motivation: 短期降水临近预报存在高度不确定性和约束不足的问题，特别是对于快速演变的极端天气事件。现有生成方法主要依赖视觉条件，导致未来运动约束弱且模糊，需要更强的语义约束来改进预报准确性。

Method: 提出语言感知多模态临近预报框架(LangPrecip)，将气象文本作为降水演化的语义运动约束，在Rectified Flow范式下将临近预报建模为语义约束的轨迹生成问题，在潜空间中高效融合文本和雷达信息。同时构建了包含16万对雷达序列和运动描述的大规模多模态数据集LangPrecip-160k。

Result: 在瑞典和MRMS数据集上的实验表明，该方法相比最先进方法取得了一致性改进，在80分钟预报时效上，强降水的CSI指标分别提升了超过60%和19%。

Conclusion: LangPrecip框架通过引入语言作为语义运动约束，有效解决了降水临近预报中的不确定性问题，显著提升了预报精度，特别是在强降水事件中表现突出，为多模态气象预报提供了新思路。
...
TL;DR: 现有概率知识图谱嵌入方法存在关系无关的实体不确定性估计问题，无法区分新兴实体和新型关系上下文两种不同的分布外现象。本文提出将不确定性分解为语义不确定性和结构不确定性，并通过组合方法显著提升分布外检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有概率知识图谱嵌入方法使用实体级方差来量化认知不确定性，但这些方差是关系无关的，导致无法区分两种行为相反的分布外现象：新兴实体（罕见、学习不足）和新型关系上下文（熟悉实体在未观察关系中的组合）。

Method: 提出不确定性分解为互补成分：1）语义不确定性（来自实体嵌入方差，检测新兴实体）；2）结构不确定性（来自实体-关系共现统计，检测新型关系上下文）。理论证明这两种信号是非冗余的，任何凸组合都严格优于单独使用任一信号。提出的CAGP方法通过学习的权重组合语义和结构不确定性。

Result: 在三个数据集（FB15k-237、WN18RR、YAGO3-10）上验证了新型上下文三元组与频率匹配的分布内对应项完全重叠。CAGP方法在时间分布外检测上达到0.94-0.99 AUROC，相比关系无关基线相对提升60-80%。在选择性预测中，在85%回答率下减少43%错误。

Conclusion: 关系无关的实体不确定性估计存在根本局限性，无法有效检测新型关系上下文。通过将不确定性分解为语义和结构成分并适当组合，可以显著提升分布外检测性能，为知识图谱嵌入中的不确定性量化提供了更全面的框架。
...
<details>
  <summary>Details</summary>
Motivation: 比特币价格具有极端波动性和非平稳性，传统单变量时间序列模型在长期预测中表现不佳。现有研究缺乏对宏观经济因素的系统性整合，特别是全球流动性对比特币价格的影响。

Method: 使用TimeXer架构，整合来自18个主要经济体的全球M2流动性作为外生变量，采用12周滞后结构。比较了流动性条件预测模型(TimeXer-Exog)与LSTM、N-BEATS、PatchTST和标准单变量TimeXer等基准模型。

Result: 在2020年1月至2025年8月的比特币日价格数据上，TimeXer-Exog在70天预测范围内取得了1.08e8的均方误差，比单变量TimeXer基线提升了超过89%，显著稳定了长期预测。

Conclusion: 将全球流动性作为深度学习的条件变量，能够显著改善比特币价格的长期预测性能，为加密货币预测提供了新的宏观经济视角。
...
TL;DR: LoRA微调可能导致模型能力灾难性退化，但通过正则化近似重放方法（惩罚与初始模型的KL散度并混合预训练风格数据）可以几乎消除该问题，在保持通用知识的同时不阻碍新任务学习能力。


<details>
  <summary>Details</summary>
Motivation: 尽管LoRA等参数高效微调方法只修改少量参数，但实验发现基于LoRA的监督微调会灾难性地降低模型能力，即使在小数据集上训练少量步数也会出现此问题。需要找到既能保持模型原有能力又能适应新任务的微调方法。

Method: 提出正则化近似重放方法：1）惩罚与初始模型的KL散度作为正则化项；2）混合来自与预训练相似但不同的开放访问语料库的下一个词预测数据。该方法计算开销很小，应用于Qwen指令微调模型。

Result: 该方法几乎完全消除了LoRA微调导致的灾难性退化问题。在Qwen指令微调模型中，能够保持模型的通用知识，同时不阻碍对新任务的可塑性，仅增加适度的计算开销。

Conclusion: 虽然最直接的LoRA微调方法在实践中可能失败，但通过简单的训练过程调整（正则化近似重放）可以几乎消除灾难性退化问题，在保持模型原有能力的同时实现有效微调。
...
TL;DR: AFA-LoRA通过引入退火激活函数，在保持LoRA可合并性的同时，为其增加非线性表达能力，缩小了与全参数微调的性能差距。


<details>
  <summary>Details</summary>
Motivation: LoRA作为广泛使用的参数高效微调方法，其线性适应过程限制了表达能力，导致线性训练和非线性训练之间存在性能差距。

Method: 提出AFA-LoRA训练策略，核心创新是退火激活函数，在训练过程中从非线性变换逐渐过渡到线性变换，使适配器先获得更强的表示能力，最终收敛到可合并的线性形式。

Result: 在监督微调、强化学习和推测解码等任务上，AFA-LoRA显著缩小了LoRA与全参数训练之间的性能差距。

Conclusion: AFA-LoRA实现了更强大且实用的参数高效适应范式，在保持LoRA可合并性的同时增强了表达能力。
...
TL;DR: AMBIT是一个灰盒框架，通过将物理移动基线模型与可解释的树模型结合，解决OD流预测中准确性与可解释性的冲突问题。


<details>
  <summary>Details</summary>
Motivation: OD流预测在GIS和城市分析中是核心任务，但实际部署面临两个冲突需求：高准确性和清晰可解释性。传统物理模型在小时级时间分辨率下表现脆弱，需要更好的解决方案。

Method: 首先对经典空间交互模型进行全面审计，确定PPML重力模型为最强物理基线。然后在物理基线上构建残差学习器，使用梯度提升树和SHAP分析，形成物理基础残差和POI锚定残差两种方法。

Result: 审计显示大多数物理模型在小时级分辨率下脆弱；基于物理的残差方法能达到强树基预测器的准确性同时保持可解释结构；POI锚定残差在空间泛化下最具鲁棒性。

Conclusion: AMBIT框架成功平衡了OD流预测的准确性和可解释性，为城市决策提供了可复现的管道、丰富的诊断和空间误差分析工具。
...
TL;DR: GLUE提出了一种梯度自由的学习方法来统一专家模型，通过两点SPSA更新学习混合系数，只需两次前向传播，无需反向传播，在目标域上获得更好的性能。


<details>
  <summary>Details</summary>
Motivation: 在部署系统中，多个预训练的专家模型共存，但新目标域需要超越单个专家领域的泛化模型。现有方法通过启发式混合系数（基于数据大小或代理指标）初始化目标模型，但往往导致目标域测试准确率较低，而基于梯度的学习方法需要计算昂贵的反向传播。

Method: GLUE（Gradient-free Learning To Unify Experts）将目标模型初始化为固定专家模型的凸组合，通过梯度自由的两点SPSA更新学习混合系数，每个步骤只需要两次前向传播，无需反向传播。

Result: 在三个数据集和三种网络架构上的实验表明，GLUE产生的单一先验经过微调后优于基线方法。GLUE比基于数据大小的加权方法提高测试准确率高达8.5%，比基于代理指标的选择方法提高高达9.1%，与基于反向传播的完全梯度混合方法相比，要么表现更好，要么性能差距在1.4%以内。

Conclusion: GLUE提供了一种高效、梯度自由的方法来统一专家模型，为目标域扩展提供了有效的初始化策略，在保持性能的同时显著降低了计算成本。
...
TL;DR: 提出一种多类不平衡学习的协同优化Boosting模型，通过整合密度因子和置信因子设计抗噪权重更新机制和动态采样策略，实现不平衡学习与模型训练的协同优化。
...
Method: 提出一个简单但有效的协同优化Boosting模型，整合密度因子和置信因子，设计噪声抵抗的权重更新机制和动态采样策略。这些模块紧密集成，协调权重更新、样本区域划分和区域引导采样。

Result: 在20个公共不平衡数据集上的广泛实验表明，所提出的模型显著优于8个最先进的基线方法。

Conclusion: 本研究成功实现了不平衡学习与模型训练的协同优化，提出的模型在性能上超越了现有方法，为解决类别不平衡问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI模型依赖静态数据集，无法适应真实物联网NetFlow流量的动态变化和概念漂移问题。周期性重训练方法计算开销大且存在灾难性遗忘风险
...
Conclusion: 该框架消除了持续分类器重训练的需求，在动态大规模物联网环境中具有实际部署潜力，能有效应对概念漂移挑战
...
TL;DR: 论文发现彩票假设在LoRA参数高效微调中同样成立，并提出Partial-LoRA方法，通过稀疏低秩适配器减少87%可训练参数的同时保持或提升性能。
...
Method: 提出Partial-LoRA方法，系统识别稀疏子网络，训练与预训练模型任务相关子空间对齐的稀疏低秩适配器，研究稀疏性在不同层的分布对性能的影响。

Result: 在8个视觉和12个语言任务（单任务和多任务）实验中，Partial-LoRA将可训练参数减少高达87%，同时保持或提高准确率，验证了彩票假设在LoRA中的有效性。

Conclusion: 彩票假设在LoRA参数高效微调中成立，稀疏子网络性能更依赖于各层稀疏度分布而非具体权重选择，Partial-LoRA为开发更高效适配策略开辟了新途径。
...
TL;DR: 该研究探索使用元数据和幻觉信号预测LLM在牙科考试中回答的正确性，发现可提升7.14%准确率，但方法尚不足以用于高风险部署。


<details>
  <summary>Details</summary>
Motivation: LLM在医疗等高风险领域应用日益广泛，但存在产生事实错误信息（幻觉）的风险。预测LLM回答是否正确是关键但尚未充分探索的问题。

Method: 研究分析通用模型（GPT-4o）和推理中心模型（OSS-120B）在牙科多选题考试中的表现。使用三种不同提示策略下的元数据和幻觉信号，为每个（模型，提示）组合构建正确性预测器。

Result: 元数据方法可将准确率提升最高+7.14%，精确度达83.12%（相比假设所有答案正确的基线）。实际幻觉是错误回答的强指标，但仅元数据信号不能可靠预测幻觉。提示策略虽不影响总体准确率，但显著改变模型内部行为和元数据的预测效用。
...
<details>
  <summary>Details</summary>
Motivation: 大型预训练模型虽然强大，但难以精确控制其概念特定行为。任务向量（微调与预训练参数之差）可用于引导模型行为，但向量间存在概念重叠，在算术运算时会产生干扰，导致不可预测的结果。

Method: 提出原则性分解方法，将每个任务向量分解为两个组件：一个捕获多个任务向量间的共享知识，另一个隔离每个特定任务的独特信息。通过识别投影中的不变子空间来实现。

Result: 在三个领域验证有效性：1) 图像分类多任务合并提升5%（使用共享组件作为额外任务向量）；2) 扩散模型中实现干净风格混合而不降低生成质量（仅混合独特组件）；3) 语言模型中毒性降低47%同时保持通用知识任务性能（通过抵消独特组件中的毒性信息）。

Conclusion: 该方法为理解和控制任务向量算术提供了新框架，解决了模型编辑操作中的基本限制，实现了更精确的概念操控而不会意外放大或减弱其他行为。
...
TL;DR: 提出评估SNN对抗鲁棒性的新框架，包括自适应锐度替代梯度(ASSG)和稳定自适应投影梯度下降(SA-PGD)攻击方法，显著提高攻击成功率，揭示当前SNN鲁棒性被高估的问题。


<details>
  <summary>Details</summary>
Motivation: SNN使用脉冲激活模拟大脑高效信息处理，但脉冲的二值化和不连续性导致梯度消失，使得基于梯度下降的对抗鲁棒性评估不可靠。现有替代梯度方法在强对抗攻击下的有效性不明确。
...
Conclusion: 提出了更可靠的SNN对抗鲁棒性评估框架，包括ASSG和SA-PGD，实验证明其有效性并揭示现有SNN鲁棒性评估的局限性，强调需要更可靠的对抗训练方法。
...
TL;DR: TimePerceiver：统一的编码器-解码器时间序列预测框架，通过广义预测任务定义和创新的编码解码设计，在多种基准数据集上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列预测研究主要关注编码器设计，而将预测和解码视为次要问题。需要一种将编码、解码和训练策略紧密结合的统一框架。

Method: 1. 将预测任务广义化为外推、插值和填补；2. 设计灵活的编码器-解码器架构处理任意位置的输入和输出段；3. 编码器使用潜在瓶颈表示捕获时间和跨通道依赖；4. 解码器使用可学习查询对应目标时间戳检索相关信息。

Result: 在广泛的基准数据集上，TimePerceiver一致且显著地超越了先前的最先进基线方法。
...
TL;DR: 该研究分析美国NHTSA超过2500起自动驾驶车辆事故记录，通过两阶段数据挖掘框架（K-means聚类+关联规则挖掘）揭示SAE 2级和4级自动驾驶的事故动态模式。


<details>
  <summary>Details</summary>
Motivation: 尽管自动驾驶车辆有提升交通安全的潜力，但实际事故数据显示其行为可能偏离预期安全结果。现有研究多基于小规模加州数据集，且缺乏对不同自动化等级事故趋势的系统分析，需要更全面的研究来理解混合交通环境中的事故动态。

Method: 开发两阶段数据挖掘框架：首先使用K-means聚类基于时间、空间和环境因素将事故记录分为4个行为集群；然后在每个集群内应用关联规则挖掘（ARM）提取事故模式与影响因素（照明条件、路面状况、车辆动态、环境条件）之间的多元关系。

Result: 研究揭示了自动驾驶车辆事故的潜在动态模式，识别出不同集群中的关键事故影响因素组合，为理解SAE 2级和4级自动驾驶的事故特征提供了数据驱动的见解。

Conclusion: 研究结果为自动驾驶开发者、安全监管机构和政策制定者提供了可操作的指导，有助于制定更有效的自动驾驶部署策略和降低事故风险。大规模数据分析方法为理解自动驾驶安全性能提供了新视角。
...
TL;DR: EnFlow：通过能量引导采样统一生成构象多样性和能量校准的框架


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的方法存在碎片化问题：生成模型能捕获多样性但缺乏可靠的能量校准，而确定性预测器只针对单一结构且无法表示集合变异性。需要一种统一框架来同时解决构象生成和基态识别问题。

Method: EnFlow将流匹配（FM）与显式学习的能量模型通过能量引导采样方案耦合，沿着非高斯FM路径定义。在采样过程中加入能量梯度引导，将轨迹导向低能量区域，提高构象保真度。学习的能量函数还能对生成的集合进行基于能量的排序。

Result: 在GEOM-QM9和GEOM-Drugs数据集上的实验表明，EnFlow在1-2个ODE步骤内同时改进了生成指标，并减少了基态预测误差，优于现有最先进方法。

Conclusion: EnFlow提供了一个统一框架，通过能量引导采样将流匹配与能量模型结合，既能生成多样化的低能量构象集合，又能准确识别基态构象，解决了现有方法的碎片化问题。
...
TL;DR: 提出一种名为PGRU的并行门控循环单元模型，用于加密货币价格预测，通过并行独立处理不同价格特征，在减少输入数据和计算成本的同时获得更高精度。


<details>
  <summary>Details</summary>
Motivation: 随着加密货币和比特币的发展，在线投资和交易日益增多。比特币使用区块链技术确保交易安全、透明、可追溯和不可篡改，但其价格波动剧烈，投资者需要有效的价格预测方法。现有方法存在各种局限性，需要更准确高效的预测模型。

Method: 提出并行门控循环单元（PGRU）模型，使用循环神经网络以并行独立方式预测价格。并行网络处理不同的价格相关特征输入，最后通过神经网络结合各并行网络的输出，预测加密货币未来价格。

Result: 实验结果显示，提出的模型在窗口长度20和15时分别达到3.243%和2.641%的平均绝对百分比误差（MAPE）。相比现有方法，该方法用更少的输入数据和更低的计算成本实现了更高的准确性和效率。

Conclusion: PGRU模型在加密货币价格预测方面表现出色，通过并行处理不同特征提高了预测精度，同时降低了计算需求，为投资者提供了有效的价格预测工具。
...
TL;DR: 该论文提出了一种基于LSTM-MLP混合模型的金价预测算法，结合灰狼优化算法进行参数调优，在日度和月度时间框架上预测金价，并基于预测结果开发交易策略，在3个月内获得171%的回报。


<details>
  <summary>Details</summary>
Motivation: 由于全球黄金市场涉及众多金融机构和复杂的经济政治因素，准确预测金融市场具有挑战性。开发能够准确预测市场走势的模型对开发者具有重要意义。

Method: 使用两个LSTM网络分别进行日度和月度预测，结果输入MLP网络进行整合。采用灰狼优化算法优化各网络内部神经元数量，基于最小RMSE误差。数据集包含2010-2021年的宏观经济、能源市场、股票和发达国家货币状况数据。

Result: 模型预测日度收盘价的MAE为0.21美元，月度价格的MAE为22.23美元。基于预测开发的交易策略在3个月内获得171%的回报。

Conclusion: 提出的LSTM-MLP混合模型能够有效预测金价，结合优化算法和交易策略，在实际市场中表现出良好的预测精度和盈利能力。
...
TL;DR: 本文提出Selective TTS框架，通过在多阶段多智能体管道中分布计算资源并早期剪枝低质量分支，解决传统基于奖励的迭代优化在无验证奖励任务中的误差累积问题。


<details>
  <summary>Details</summary>
Motivation: 许多现实世界任务涉及多阶段管道，其最终结果缺乏可验证的奖励或足够数据来训练鲁棒的奖励模型，导致基于判断的优化容易在阶段间累积误差。

Method: 提出Selective TTS框架：1) 在多智能体管道不同阶段分布计算资源；2) 使用过程特定判断器早期剪枝低质量分支；3) 基于数据科学管道构建端到端多智能体系统生成可视化图表和报告；4) 设计可靠LLM判断模型与人类专家对齐。

Result: 在固定计算预算下，Selective TTS将平均分数从61.64提升至65.86，同时减少方差。LLM判断模型与人类专家对齐度达到Kendall's τ=0.55。

Conclusion: Selective TTS为扩展复杂、开放式且无验证奖励的任务（如科学发现和故事生成）提供了第一步，通过过程优化而非时间迭代来稳定优化过程。
...
TL;DR: 该论文首次将量子生成模型应用于计算流体动力学数据的潜在空间表示，比较了量子电路玻恩机和量子生成对抗网络与经典LSTM在流体涡量场潜在分布建模上的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然已有研究探索量子模型学习流体系统统计特性，但将离散潜在空间压缩与量子生成采样结合用于CFD数据仍未被探索。该研究旨在填补这一空白，为物理模拟与量子机器学习交叉领域建立基础。

Method: 使用GPU加速的格子玻尔兹曼方法生成流体涡量场，通过向量量化变分自编码器压缩到7维离散潜在空间。比较量子电路玻恩机、量子生成对抗网络与经典长短期记忆网络在建模该物理潜在分布上的性能。

Result: 在实验条件下，两种量子模型产生的样本与真实分布的平均最小距离均低于LSTM基线，其中量子电路玻恩机获得了最有利的指标。

Conclusion: 该工作提供了：(1)连接CFD模拟与量子机器学习的完整开源流程；(2)首次在物理模拟压缩潜在表示上进行量子生成建模的实证研究；(3)为未来该交叉领域的严格研究奠定基础。
...
<details>
  <summary>Details</summary>
Motivation: 表示学习在数据稀缺环境中广泛应用，但现有研究主要集中在中心化方法，去中心化方法研究不足。实际应用中任务数据分布在多个节点，节点间通信受限，需要开发高效的去中心化表示学习算法

Method: 提出交替投影梯度和最小化算法，用于恢复具有低秩结构的特征矩阵。算法在去中心化设置下运行，任务数据分布在多个节点，通过通信网络交换信息

Result: 算法具有可证明的精度保证，全面分析了时间、通信和样本复杂度。关键突破是通信复杂度与目标精度无关，相比先前方法显著降低通信成本。数值模拟验证了理论分析，并展示了去中心化学习优于中心化联邦方法的场景

Conclusion: 提出的去中心化多任务表示学习算法有效解决了数据分布和通信受限问题，通信成本优势明显，在特定场景下优于中心化方法，为分布式学习提供了新思路
...
TL;DR: 论文探索使用p-adic数（ℚₚ）替代实数（ℝ）作为机器学习框架的基础，利用其超度量和非阿基米德特性进行层次表示学习。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习框架基于实数域ℝ，但作者质疑这是否是唯一选择。p-adic数的层次结构和无限字符串解释使其在编码理论和层次表示学习中具有吸引力，可能提供不同于实数的新几何特性。

Method: 建立p-adic数在机器学习中的理论基础，包括分类、回归和表示学习的构建模块，提供学习模型和算法。展示如何将简单的Quillian语义网络表示为紧凑的p-adic线性网络。

Result: 证明了p-adic数在机器学习中的可行性，展示了其独特优势：能够构建实数域中不可能实现的紧凑p-adic线性网络来表示语义网络。

Conclusion: p-adic数作为ℝ的替代方案具有潜力，为机器学习开辟了新方向。论文提出了未来研究的问题和机会，包括如何利用p-adic的层次特性进行更有效的表示学习。
...
TL;DR: 该论文分析了时间序列预测深度学习架构的设计空间，指出简单、设计良好的架构往往能与最先进方法相媲美，强调需要重新思考当前有缺陷的基准测试实践，并提出了辅助预测模型卡片来表征架构的关键设计选择。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在时间序列应用中越来越受欢迎，但大量新提出的架构加上常常矛盾的实证结果，使得难以评估哪些组件对最终性能有显著贡献。作者旨在通过讨论设计维度和权衡来解释观察到的结果，理清当前时间序列预测深度学习架构的设计空间。

Method: 论文讨论了将模型设计基于时间序列组预测原则的必要性，以及如何将这些原则应用于当前模型。特别评估了局部性和全局性等概念如何应用于最近的预测架构。作者展示了考虑这些方面比采用特定序列建模层更重要，并讨论了现有架构中被忽视的实现细节如何改变预测方法的类别并影响实证结果。

Result: 研究表明，简单、设计良好的预测架构通常能与最先进方法相匹配。被忽视的实现细节会从根本上改变预测方法的类别，并显著影响观察到的实证结果。这揭示了当前基准测试实践存在缺陷。

Conclusion: 需要重新思考当前有缺陷的基准测试实践，并在设计架构时关注预测问题的基础方面。作为朝这个方向迈出的一步，作者提出了辅助预测模型卡片，其字段用于基于关键设计选择来表征现有和新的预测架构。
...
TL;DR: FoldAct框架解决了长时程RL中上下文折叠带来的非平稳观测问题，通过分离损失计算、全上下文一致性损失和选择性片段训练，实现了稳定训练和5.19倍加速。
...
Result: 方法能够稳定训练具有上下文折叠的长时程搜索智能体，解决了非平稳观测问题，同时提高了训练效率，实现了5.19倍的加速。

Conclusion: FoldAct通过明确处理上下文折叠带来的非平稳观测挑战，为长时程RL提供了稳定高效的训练框架，解决了现有方法忽视的观测空间变化问题。
...
<details>
  <summary>Details</summary>
Motivation: 测试多任务学习是否能够利用合金材料性能之间的潜在物理关联来提升预测性能
...
Conclusion: 建议对精确回归任务使用独立模型，而对需要高召回率的分类任务采用多任务学习
...
TL;DR: HEROSQL提出分层SQL表示方法，结合逻辑计划和抽象语法树，通过NMPNN进行语义验证，显著提升Text-to-SQL语义不一致检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有Text-to-SQL验证方法主要关注语法正确性，缺乏有效的语义验证（检测问题与SQL之间的错位）。语义验证面临两大挑战：同时捕捉全局用户意图和SQL结构细节，以及构建高质量细粒度子SQL标注。

Method: 提出HEROSQL分层SQL表示方法：1）集成全局意图（逻辑计划LP）和局部细节（抽象语法树AST）；2）使用嵌套消息传递神经网络（NMPNN）捕获SQL内在关系信息并聚合模式引导的语义；3）提出AST驱动的子SQL增强策略生成高质量负样本。

Result: 在Text-to-SQL验证基准测试（域内和域外设置）中，方法优于现有最先进方法，AUPRC平均提升9.40%，AUROC平均提升12.35%。能有效检测细粒度语义错误，为大语言模型提供更细粒度反馈。

Conclusion: HEROSQL通过分层表示和NMPNN增强了Text-to-SQL语义验证能力，提高了数据查询平台的可靠性和可解释性，特别是在检测细粒度语义不一致方面表现优异。
...
TL;DR: 该论文研究第三方平台面对战略代理时的定价行为，提出一种在混淆条件下学习需求的最优后悔算法，揭示了供应侧噪声对需求可学习性的根本影响。


<details>
  <summary>Details</summary>
Motivation: 平台作为收入最大化者，只能观察到均衡价格和数量，而市场特征通常会影响需求，这构成了一个在混淆条件下的一般需求学习问题。

Method: 开发了具有最优后悔的算法，利用非独立同分布行动作为工具变量学习需求，提出新颖的同胚构造方法，无需假设星形性即可建立估计界限，首次为深度神经网络学习需求提供效率保证。

Result: 算法达到最优后悔 $\Tilde{\cO}(\sqrt{T}\wedgeσ_S^{-2})$，揭示了供应侧噪声导致后悔的阶段转变，通过模拟和Zomato、Lyft的真实数据验证了方法的实用性。

Conclusion: 供应侧噪声从根本上影响需求的可学习性，非独立同分布行动可作为学习需求的工具变量，提出的同胚构造方法为深度神经网络学习需求提供了首个效率保证，方法在实际应用中具有可行性。
...
TL;DR: 提出一个微-宏观机器学习框架，整合个体层面数据（NHANES）和宏观环境特征（USDA/EPA），预测儿童肥胖风险并识别环境驱动的健康差异。


<details>
  <summary>Details</summary>
Motivation: 传统流行病学研究通常独立分析个体、家庭和环境层面的风险因素，限制了理解结构性环境条件如何与个体特征相互作用影响健康结果。儿童肥胖在美国仍然是重大公共卫生挑战。

Method: 开发微-宏观机器学习框架：1) 使用NHANES的个体层面人体测量和社会经济数据；2) 从USDA和EPA数据集中提取宏观环境特征（食品获取、空气质量、社会经济脆弱性）；3) 训练四种机器学习模型（逻辑回归、随机森林、XGBoost、LightGBM）预测肥胖；4) 构建综合环境脆弱性指数（EnvScore）。

Result: XGBoost模型表现最佳。多层比较显示，高环境负担州与全国预测的微观肥胖风险分布存在强烈地理相似性，表明环境条件与肥胖风险密切相关。

Conclusion: 该研究证明了整合多尺度数据集识别环境驱动肥胖风险差异的可行性，为公共卫生信息学提供了一个可扩展、数据驱动的多层建模管道，具有扩展到因果建模、干预规划和实时分析的潜力。
...
TL;DR: GRExplainer：首个通用、高效、用户友好的TGNN解释方法，通过节点序列统一特征表示，适用于快照式和事件式TGNN，使用BFS和时序信息提升效率，基于RNN自动生成解释。


<details>
  <summary>Details</summary>
Motivation: 现有TGNN解释方法存在三大问题：1）针对特定TGNN类型，缺乏通用性；2）计算成本高，不适用于大规模网络；3）忽略解释的结构连通性且需要先验知识，用户友好性差。需要开发通用、高效、用户友好的TGNN解释方法。

Method: 提出GRExplainer方法：1）提取节点序列作为统一特征表示，使其独立于特定输入格式，可同时适用于快照式和事件式TGNN；2）利用广度优先搜索（BFS）和时序信息构建输入节点序列，减少冗余计算；3）设计基于循环神经网络（RNN）的生成模型，实现自动化、连续的解释生成。

Result: 在6个真实世界数据集和3个目标TGNN上的实验表明，GRExplainer在通用性、效率和用户友好性方面均优于现有基线方法。

Conclusion: GRExplainer是首个通用、高效、用户友好的TGNN解释方法，解决了现有方法的局限性，为TGNN的透明性和可解释性提供了有效解决方案，有助于推动TGNN在实际应用中的采用。
...
TL;DR: Schrödinger AI是一个受量子力学启发的统一机器学习框架，包含波能求解器、动力学求解器和低秩算子演算三个组件，提供鲁棒泛化、可解释语义和涌现拓扑。


<details>
  <summary>Details</summary>
Motivation: 提出一种基于物理原理的机器学习替代方案，以克服传统交叉熵训练和Transformer注意力的局限性，实现更好的泛化能力、可解释性和动态推理能力。

Method: 框架包含三个紧密耦合组件：1) 时间无关波能求解器，将感知和分类视为学习哈密顿量下的谱分解；2) 时间相关动力学求解器，控制语义波函数随时间演化；3) 低秩算子演算，通过学习量子类跃迁算子实现符号变换。

Result: 实验显示：a) 涌现出反映人类概念关系的语义流形；b) 动态推理适应环境变化，包括实时势场扰动的迷宫导航；c) 在模运算任务上实现精确算子泛化，学习群作用并在远超训练长度的序列上组合它们。
...
TL;DR: 提出基于因果可迁移理论的零样本组合泛化算法Circuit-TR，通过模块化学习和电路传输实现跨域预测


<details>
  <summary>Details</summary>
Motivation: 跨域泛化需要结构约束来连接源域和目标域，传统方法缺乏对因果机制共享的显式建模

Method: 使用因果图表示域内结构，差异预言机表示域间机制共享，学习局部预测器模块，根据因果结构许可进行电路传输和组合

Result: 理论结果描述了基于图形电路可迁移准则的少样本可学习任务类别，将少样本泛化能力与电路大小复杂度联系起来

Conclusion: Circuit-TR实现了零样本组合泛化，并设计了无需显式因果结构的监督域适应方案，模拟实验验证了理论结果
...
TL;DR: 该研究通过分析中国公开的COVID-19追踪数据，结合自然语言处理和人口流动数据，揭示了SARS-CoV-2传播模式、区域差异和传播源随时间的变化规律。


<details>
  <summary>Details</summary>
Motivation: 通过对大规模检测、隔离和接触者追踪等公共卫生干预措施的回顾性分析，识别最有效的疫情防控机制，为未来公共卫生政策提供依据。

Method: 收集地方卫健委、中国CDC和官方社交媒体发布的病例报告，应用自然语言处理和人工整理构建传播/追踪链，结合武汉人口流动数据进行时空传播动态的量化与可视化分析。

Result: 发现显著的地区差异，大城市感染更多（可能由社交活动驱动）；79%有症状者在症状出现5天内住院，有确诊病例接触史者入院时间更短；感染源随时间变化：早期主要与湖北旅行相关，后期更多与社交活动相关。

Conclusion: 公共卫生干预措施的效果存在区域差异，传播模式随时间演变，社交活动在后期传播中起重要作用，这些发现可为针对性防控策略提供参考。
...
TL;DR: 提出基于强化学习的扩散模型蒸馏框架，将蒸馏过程视为策略优化问题，通过奖励信号动态指导学生模型，实现更少推理步骤和更高性能


<details>
  <summary>Details</summary>
Motivation: 解决扩散模型采样速度慢的问题，传统蒸馏方法依赖固定的重建或一致性损失，需要更灵活的动态优化方法

Method: 将扩散模型蒸馏视为强化学习策略优化问题，学生模型作为策略，使用基于教师模型输出的奖励信号进行训练，动态探索多个去噪路径

Result: 实验结果显示，该方法相比现有蒸馏技术，使用显著更少的推理步骤和计算资源，实现了更优的性能

Conclusion: 该强化学习驱动的蒸馏框架为扩散模型提供了通用的优化范式，模型无关且适用于各种扩散模型，能有效管理生成过程
...
TL;DR: MoR是一种新颖的混合精度训练框架，通过动态分析张量数值特性，在FP8和BF16表示之间进行选择，实现了98.38%张量量化为FP8格式，同时保持模型质量。
...
Method: 提出Mixture-of-Representations (MoR)框架，在张量和子张量级别进行动态量化。通过分析张量的数值特性，在FP8和BF16表示之间动态选择。设计了具体算法支持不同粒度的量化分区策略。

Result: 实现了98.38%的张量量化为FP8格式，达到了最先进的结果。FP8精度与现有方法相当，无需精细分区。框架还可与NVFP4等更低精度格式结合使用。

Conclusion: MoR框架展示了动态、属性感知量化的潜力，能够保持模型质量的同时提高低精度训练的鲁棒性。该方法可与其他训练方法结合，进一步利用更低精度格式。
...
TL;DR: 提出长程蒸馏方法，用自回归教师模型生成大量合成气候数据，训练单步长时步学生模型进行长期天气预报，显著提升预测技能


<details>
  <summary>Details</summary>
Motivation: 传统AI天气模型的自回归方法在长期预测中误差累积且不稳定，而单步长时步模型需要大量训练数据，但再分析数据集样本有限，导致过拟合问题

Method: 使用自回归教师模型（DLESyM）生成超过10,000年的合成气候数据，训练长时步概率学生模型直接进行长期预测，通过蒸馏方法将数百个自回归步骤替换为单步预测

Result: 在完美模型实验中，蒸馏模型优于气候学基准，接近自回归教师技能；在实际应用中，经过ERA5微调后，其S2S预测技能与ECMWF集合预报相当，且技能随合成训练数据增加而提升

Conclusion: 首次证明AI生成的合成训练数据可用于扩展长期天气预报技能，长程蒸馏方法为解决长期天气预报中的数据稀缺问题提供了有效途径
...
TL;DR: 提出一种基于时域方差驱动课程的学生-教师学习范式，通过动态选择高不确定性目标来加速目标条件强化学习
...
Method: 学生-教师学习范式，教师模块根据策略置信度（Q函数）的时域方差动态优先选择高不确定性目标，提供自适应学习信号

Result: 在11个机器人操作和迷宫导航任务中，相比最先进的课程学习和目标选择方法，取得了显著且一致的改进

Conclusion: 时域方差驱动的自适应课程学习能有效加速目标条件强化学习，该方法与算法无关，可无缝集成到现有RL框架中
...
TL;DR: 提出FMTC框架，在保护隐私的去中心化环境中实现个性化聚类，通过客户端个性化聚类模块和服务端张量相关模块解决异构客户端间的共享知识挖掘问题。
...
Method: FMTC框架包含两个核心组件：1) 客户端个性化聚类模块，学习参数化映射模型支持鲁棒的样本外推理，避免依赖伪标签；2) 服务端张量相关模块，将所有客户端模型组织为统一张量，通过低秩正则化发现其公共子空间。采用基于ADMM的高效隐私保护分布式算法进行优化。

Result: 在多个真实数据集上的实验表明，FMTC框架显著优于各种基线和最先进的联邦聚类算法。

Conclusion: FMTC框架成功解决了联邦聚类中的个性化建模和共享知识挖掘问题，在保护隐私的同时实现了优异的聚类性能，为异构客户端环境提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前处理表格日志的方法过度依赖LLM等重型模型，导致灵活性和可扩展性受限，需要更高效的日志调试方法

Method: 构建GraphLogDebugger框架：1) 为对象和事件创建异构节点 2) 连接节点间边 3) 将系统恢复为演化动态图 4) 使用简单动态GNN进行调试

Result: 在计算机系统和学术论文的真实日志数据集上验证，动态图建模使简单GNN在表格日志调试中超越LLM表现

Conclusion: 动态图建模为表格日志调试提供了更灵活、可扩展的解决方案，简单GNN即可超越复杂LLM，具有实际应用价值
...
TL;DR: 提出基于持续学习的元学习认知诊断框架MetaCD，解决长尾分布和动态数据变化问题，在多个真实数据集上优于基线方法
...
Method: 提出MetaCD框架：1) 使用元学习学习最优初始化状态，解决长尾问题；2) 采用参数保护机制的持续学习方法，适应新技能/任务的动态变化

Result: 在五个真实数据集上的综合实验表明，MetaCD在准确性和泛化能力方面均优于其他基线方法

Conclusion: MetaCD框架能有效解决认知诊断中的长尾分布和动态数据变化问题，提高模型的塑性、稳定性和泛化能力
...
TL;DR: Sat-EnQ：一个两阶段深度Q学习框架，先通过"满意"目标训练轻量级Q网络集成，再蒸馏到大型网络进行微调，显著降低训练方差和灾难性失败率。


<details>
  <summary>Details</summary>
Motivation: 深度Q学习算法在早期训练时不稳定，最大化算子会放大估计误差，导致灾难性过估计。受有限理性理论和发育学习启发，提出先学习"足够好"再优化。

Method: 两阶段框架：第一阶段使用动态基线限制早期价值增长的满意目标训练轻量级Q网络集成，产生低方差估计；第二阶段将集成蒸馏到大型网络并用标准Double DQN微调。

Result: 理论证明满意更新有界且不增加目标方差；实证显示方差降低3.8倍，消除灾难性失败（0% vs DQN的50%），在环境噪声下保持79%性能，计算需求比自举集成少2.5倍。

Conclusion: 通过先满意后优化的原则性路径，Sat-EnQ为稳健强化学习提供了新方向，显著提升了深度Q学习的稳定性和效率。
...
TL;DR: 提出MTD（多令牌散度）作为衡量语言模型上下文计算努力的简单指标，以及基于此的Divergence Steering解码方法，用于分析和控制生成文本的计算特性。


<details>
  <summary>Details</summary>
Motivation: 现有方法如下一令牌损失无法捕捉推理复杂度，而基于潜在状态可压缩性的方法具有侵入性和不稳定性，需要更简单有效的计算努力衡量指标。

Method: 提出MTD：计算模型完整输出分布与浅层辅助预测头输出分布之间的KL散度。无需额外训练，可直接从具有多个预测头的预训练模型计算。基于MTD提出Divergence Steering解码方法控制生成文本的计算特性。

Result: MTD比现有方法更能有效区分复杂任务与简单任务；在数学推理基准测试中，MTD与问题难度正相关；较低的MTD与更准确的推理相关。

Conclusion: MTD为分析和引导语言模型计算动态提供了实用、轻量级的工具，能够有效衡量计算努力并控制生成文本的计算特性。
...
TL;DR: APO提出了一种基于alpha散度的偏好优化框架，可在前向KL和后向KL之间连续插值，通过奖励和置信度引导的alpha调度实现从覆盖到利用的稳定过渡。


<details>
  <summary>Details</summary>
Motivation: 当前对齐方法主要分为两种：监督微调等前向KL方法虽然稳定但可能未充分利用高奖励模式；PPO等后向KL方法能进行模式寻求但存在模式崩溃风险。现有锚定方法通常只使用单一散度，缺乏灵活性。

Method: 提出Alpha-Divergence Preference Optimization (APO)，使用Csiszar alpha散度在锚定几何中连续插值前向和后向KL行为。推导了参数化梯度动态，分析梯度方差特性，并提出奖励和置信度引导的alpha调度策略。

Result: 在Qwen3-1.7B模型上的数学level3任务实验表明，APO在保持训练稳定性的同时，达到了与GRPO和GSPO基线相当的竞争性能。

Conclusion: APO提供了一个统一的锚定框架，通过alpha散度实现了前向和后向KL行为的连续插值，结合奖励和置信度引导的调度策略，在保持稳定性的同时实现了竞争性性能。
...
TL;DR: FLOW是一个合成的纵向数据集，用于模拟工作负荷、生活方式行为和幸福感之间的日常互动，旨在解决真实数据获取困难的问题。


<details>
  <summary>Details</summary>
Motivation: 由于隐私、伦理和后勤限制，获取关于工作与生活平衡及幸福感的纵向个人层面数据非常困难，这给压力建模、行为分析和机器学习等领域的可重复研究、方法基准测试和教育带来了挑战。

Method: 使用基于规则的反馈驱动模拟生成合成数据集，模拟压力、睡眠、情绪、身体活动和体重等变量之间的连贯时间动态。数据集包含1000名个体两年的每日数据，并提供了可配置的数据生成工具。

Result: 创建了FLOW数据集，这是一个公开可用的资源，模拟了工作负荷、生活方式行为和幸福感之间的日常互动，支持可重复实验。

Conclusion: FLOW作为一个受控的实验环境而非真实人群的替代品，支持在无法获取真实数据的情况下进行探索性分析、方法开发和基准测试。
...
TL;DR: 提出一个上下文感知且可解释的单通道EEG睡眠分期框架，特别关注改善N1期检测，在SleepEDF数据集上达到89.72%总体准确率和85.46%宏平均F1分数。


<details>
  <summary>Details</summary>
Motivation: 睡眠障碍全球普遍，自动睡眠分期对医疗保健至关重要。现有单通道EEG方法面临类别不平衡、感受野有限、可解释性不足等挑战，特别是N1期检测困难。

Method: 结合紧凑多尺度特征提取与时序建模，捕捉局部和长程依赖；使用类别加权损失函数和数据增强处理不平衡；将EEG信号分段为子时期块，通过平均softmax概率获得最终预测。

Result: 在SleepEDF数据集上达到89.72%总体准确率和85.46%宏平均F1分数，N1期F1分数达到61.7%，相比先前方法有显著提升。

Conclusion: 提出的框架有效提升了睡眠分期性能，同时保持了可解释性，适合实际临床应用，特别是在改善N1期检测方面表现出色。
...
TL;DR: 复杂多模态学习方法在标准化条件下并不比简单基线方法表现更好，研究呼吁从追求架构创新转向方法严谨性


<details>
  <summary>Details</summary>
Motivation: 挑战当前多模态学习领域的一个普遍假设：更复杂的多模态特定方法能带来更好的性能。研究者认为现有研究缺乏标准化比较和方法严谨性

Method: 提出SimBaMM（简单多模态学习基线），一个简单的后期融合Transformer架构。在标准化条件下重新实现19种高影响力方法，在9个数据集（最多23个模态）上评估，测试其泛化能力和缺失模态情况下的表现

Result: 在严格的超参数调优和标准化实验条件下，更复杂的架构并不能可靠地超越SimBaMM。统计分析显示复杂方法与SimBaMM表现相当，且经常不能可靠地超越调优良好的单模态基线，特别是在小数据场景下

Conclusion: 多模态学习领域需要从追求架构新颖性转向方法严谨性。研究提供了可靠性检查清单以促进可比、稳健和可信的未来评估
...
Result: 方法保持与任务数量无关的恒定内存复杂度，最小化任务间干扰，在多个持续学习基准测试中表现优于现有方法。

Conclusion: 提出的正交初始化和顺序合并方法有效解决了LoRA持续学习中的内存增长和任务干扰问题，具有理论和实践优势。
...
TL;DR: 通过机制可解释性分析FedAvg在非IID数据下的失败原因，发现客户端冲突更新导致电路崩溃，即负责特定类别预测的功能性子网络被破坏性干扰。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在非IID数据条件下性能显著下降，虽然这种准确率损失已被充分记录，但其内部机制原因仍然是一个黑箱。本文旨在通过机制可解释性来诊断这种失败模式。

Method: 在FL框架内训练固有可解释的权重稀疏神经网络，使用交并比(IoU)量化电路保持度，跟踪客户端和通信轮次中的电路变化，分析非IID数据分布如何导致局部电路发散。

Result: 提供了首个机制证据表明非IID数据分布导致结构上不同的局部电路发散，进而在全局模型中退化。非IID数据下电路保持度显著降低。

Conclusion: 将FL中的统计漂移问题重新定义为机制保持的具体可观察失败，为更有针对性的解决方案铺平道路。电路崩溃是非IID数据下性能下降的关键机制原因。
...
TL;DR: 提出物理信息多模态基础模型(PI-MFM)，通过自动组装PDE残差损失实现数据高效、可迁移的偏微分方程求解


<details>
  <summary>Details</summary>
Motivation: 现有多算子学习方法数据需求大且训练时忽略物理规律，需要开发能直接强制执行控制方程的数据高效PDE求解方法

Method: PI-MFM框架以PDE符号表示为输入，通过向量化导数计算自动组装PDE残差损失，支持统一物理信息目标训练和适应

Result: 在13个参数化一维时变PDE基准测试中，PI-MFM始终优于纯数据驱动方法，尤其在稀疏标记点、部分观测时间域或少量标记函数对情况下表现更佳
...
TL;DR: PGF框架通过直接在状态空间流形上计算精确解析导数，实现了相对于序列长度的O(1)内存复杂度，解决了SSM在基因组规模建模中的内存瓶颈问题。
...
Method: 引入相位梯度流（PGF）框架，通过直接在状态空间流形上操作计算精确解析导数，避免了中间计算图的物化需求。通过将SSM动态重构为平铺算子空间演化（TOSE），该方法实现了相对于序列长度的O(1)内存复杂度。

Result: PGF相比标准Autograd实现了94%的峰值VRAM减少和23倍的吞吐量提升。在128,000步序列的脉冲响应基准测试中，PGF能够在传统Autograd遇到内存不足问题的规模上稳定运行，保持接近机器精度。

Conclusion: PGF使单GPU上的染色体规模敏感性分析成为可能，弥合了理论无限上下文模型与实际硬件限制之间的差距，为大规模序列建模提供了可行的解决方案。
...
TL;DR: FLEX-MoE：一种联合优化专家分配和负载均衡的联邦MoE框架，解决边缘设备容量限制和非IID数据导致的专家负载不均衡问题


<details>
  <summary>Details</summary>
Motivation: 联邦学习中的MoE模型面临两个关键挑战：1）资源受限的边缘设备无法存储完整的专家集合；2）非IID数据分布导致严重的专家负载不均衡，降低模型性能

Method: 提出FLEX-MoE框架，引入客户端-专家适应度分数量化专家对本地数据集的适用性，采用基于优化的算法最大化客户端-专家专业化，同时在全系统范围内强制平衡专家利用率

Result: 在三个不同数据集上的综合实验表明，FLEX-MoE具有优越性能，并能在多样化资源受限场景下保持平衡的专家利用率

Conclusion: FLEX-MoE能够有效解决联邦学习中MoE模型的专家分配和负载均衡问题，相比现有仅关注个性化而忽略负载不均衡的贪婪方法，在处理异构数据时表现更优
...
TL;DR: 本文提出将掩码微调（MFT）应用于视觉语言模型（VLM），通过为权重分配可学习的门控分数来重组内部子网络，而非更新权重，实现了超越LoRA变体和全微调的高性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLM微调方法（如LoRA）主要依赖显式的权重更新，忽视了预训练模型中已编码的丰富表征结构未能充分利用。掩码微调（MFT）已被证明是语言模型的有效后训练范式，本文探索将其应用于VLM。

Method: 从结构重参数化角度重新思考VLM微调，将MFT应用于VLM的语言和投影器组件，为每个权重分配可学习的门控分数，允许模型为下游任务重组内部子网络，而不改变冻结的主干网络权重。

Result: 实验表明，MFT在不同语言骨干的VLM上一致超越LoRA变体，甚至优于全微调，在不改变冻结主干的情况下实现高性能。有效适应不仅来自权重更新，还可通过重新建立现有知识间的连接实现。

Conclusion: MFT为VLM提供了一种强大且高效的微调范式，通过重组预训练模型中的内部子网络而非更新权重，实现了优异的性能表现，揭示了模型适应性的新维度。
...
<details>
  <summary>Details</summary>
Motivation: 情感状态表现为中枢和自主系统的协调但异质生理反应，多模态表示学习面临挑战。现有自监督学习方法依赖成对对齐目标，无法捕捉多模态间的高阶交互依赖

Method: 提出多模态功能最大相关（MFMC）框架，使用双总相关（DTC）目标最大化高阶多模态依赖，通过功能最大相关分析（FMCA）的迹代理优化紧致三明治界

Result: 在三个公开情感计算基准测试中，MFMC在主体依赖和主体独立评估协议下均达到SOTA或竞争性能。CEAP-360VR上主体依赖准确率从78.9%提升至86.8%，仅使用EDA信号的主体独立准确率从27.5%提升至33.1%

Conclusion: MFMC能有效捕捉多模态间的高阶交互，对主体间变异性具有鲁棒性，为情感计算中的多模态表示学习提供了新方法
...
<details>
  <summary>Details</summary>
Motivation: 分布式系统中的数据在特定上下文中具有更深层次的意义，相互依赖的数据源可以揭示隐藏的关系和潜在结构，这对许多应用具有重要价值。

Method: OSM-L采用渗透过程，通过提取上下文信息合成密集紧凑的表示，无需原始数据交换。它迭代对齐本地数据表示，实现信息扩散和收敛到动态平衡，同时识别相关数据组作为去中心化聚类机制。

Result: 实验结果表明OSM-L在结构化数据集上具有良好的收敛性和表示能力，本地信息对齐准确率超过0.99，同时保持了上下文完整性。

Conclusion: OSM-L成功展示了从分布式数据中提取高层次潜在知识的能力，通过渗透过程实现有效的表示学习和去中心化聚类，为分布式系统提供了有价值的信息提取框架。
...
TL;DR: 提出一个统一框架，将模仿学习与强化学习结合用于LLM微调，通过梯度分解实现高效优化


<details>
  <summary>Details</summary>
Motivation: 当前LLM微调方法中，模仿学习和强化学习通常是分离的，缺乏统一的优化框架。需要一种能同时利用专家轨迹模仿和长时程奖励优化的方法。

Method: 通过分析结合轨迹级KL散度和任务奖励的复合目标函数的梯度，将其分解为两部分：1）可解析计算的密集梯度（用于token级模仿）；2）蒙特卡洛估计的稀疏梯度（用于长时程奖励优化）。密集梯度具有闭式解，可实现高效的GPU实现。

Result: 提出了一个统一的LLM微调框架，实现了模仿学习和强化学习的自然融合，其中密集梯度部分可高效计算，稀疏梯度部分通过蒙特卡洛方法估计。

Conclusion: 该框架为LLM微调提供了一个理论严谨且计算高效的统一方法，能够同时优化模仿学习和强化学习目标，有望提升模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 侵彻过程识别依赖加速度特征值，但传统方法通过长时间仿真计算获取，计算成本高、周期长，需要快速预测方案。

Method: 提出SE-MLP多层感知器架构，集成通道注意力机制和残差连接，建立物理参数与侵彻特征间的非线性映射关系。

Result: SE-MLP在预测精度、泛化能力和稳定性方面优于传统MLP、XGBoost和Transformer模型；消融实验证实注意力模块和残差结构均提升性能；数值仿真和射程恢复测试显示预测与实测差异在工程允许范围内。

Conclusion: 该方法可行且具有工程应用价值，为侵彻引信快速生成先验特征值提供了实用基础。
...
<details>
  <summary>Details</summary>
Motivation: 整合非欧几里得脑成像数据（如动态脑连接）与欧几里得表格数据（临床和人口统计信息）是医学影像分析的重大挑战，特别是在预测未来结果方面。现有方法在纵向成像研究中有效预测结果仍面临困难。

Method: 提出时间感知图神经网络与Transformer融合模型（GNN-TF），灵活整合表格数据和动态脑连接数据，在统一框架中利用这些变量的时间顺序。使用NCANDA纵向静息态fMRI数据集，整合非欧几里得和欧几里得信息源。

Result: 与多种成熟的机器学习和深度学习模型比较，GNN-TF优于这些最先进方法，在预测未来烟草使用方面提供更优的预测准确性。
...
TL;DR: 构建首个弱信号特征学习专用数据集（13,158个光谱样本），并提出PDVFN模型处理低信噪比、分布偏斜和双重不平衡问题，在弱信号场景下实现更高准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 弱信号学习在许多领域（故障诊断、医学影像、自动驾驶）是常见挑战，关键信息常被噪声和干扰掩盖，特征识别困难。但缺乏专用数据集长期制约研究。

Method: 构建首个弱信号特征学习专用数据集（13,158个光谱样本，55%以上样本SNR低于50，类别不平衡达29:1）。提出双视图表示（向量+时频图）和PDVFN模型，并行提取局部序列特征和全局频域结构，遵循局部增强、序列建模、噪声抑制、多尺度捕获、频率提取和全局感知原则。

Result: 实验表明该方法在处理弱信号、高噪声和极端类别不平衡时具有更高的准确性和鲁棒性，特别是在低SNR和不平衡场景下表现优异。
...
TL;DR: 提出一种基于扩散的去中心化投影梯度下降算法，用于多任务线性回归中的共享低维表示学习，具有理论保证和高效通信特性。


<details>
  <summary>Details</summary>
Motivation: 表示学习在数据稀缺环境中被广泛采用，但去中心化方法相对较少探索。本文旨在开发去中心化的多任务表示学习算法，解决多个线性回归模型共享低维线性表示的问题。

Method: 提出基于扩散的去中心化和联邦式交替投影梯度下降最小化算法，用于恢复低秩特征矩阵。算法采用投影梯度下降和最小化交替进行，在去中心化网络中实现多任务表示学习。

Result: 获得了构造性的理论保证：提供了所需样本复杂度的下界和迭代复杂度的上界。分析显示算法具有快速和通信高效的特点，数值模拟验证了算法性能优于基准算法。

Conclusion: 成功开发了一种去中心化的多任务表示学习算法，具有理论保证和实际有效性，为数据稀缺环境中的分布式表示学习提供了新方法。
...
TL;DR: 本文首次系统评估了12种参数高效微调方法在可验证奖励强化学习范式下的表现，发现结构变体优于标准LoRA，SVD初始化策略存在频谱崩溃问题，极端参数减少会严重限制推理能力。


<details>
  <summary>Details</summary>
Motivation: 在可验证奖励强化学习范式中，虽然LoRA等方法被广泛使用，但最适合RLVR的参数高效微调架构仍不明确。本文旨在填补这一空白，为参数高效的RL方法提供指导。

Method: 在DeepSeek-R1-Distill系列模型上，对超过12种PEFT方法在数学推理基准上进行全面评估，包括结构变体（DoRA、AdaLoRA、MiSS）和SVD初始化策略（PiSSA、MiLoRA）等，并进行消融实验和扩展实验验证。

Result: 1. 结构变体（DoRA、AdaLoRA、MiSS）一致优于标准LoRA；2. 发现SVD初始化策略存在频谱崩溃现象，归因于主成分更新与RL优化的根本错配；3. 极端参数减少（如VeRA、Rank-1）严重限制推理能力。

Conclusion: 本文挑战了标准LoRA的默认采用，为参数高效RL方法提供了明确的指导，呼吁对参数高效RL方法进行更多探索，特别是结构变体在RLVR中的优势。
...
TL;DR: HELM-BERT：首个基于HELM表示法的肽语言模型，显著优于SMILES模型，在环肽膜通透性和肽-蛋白相互作用预测任务中表现优异


<details>
  <summary>Details</summary>
Motivation: 现有分子语言模型（SMILES和氨基酸级表示）无法充分捕捉治疗性肽的化学复杂性和拓扑结构，需要更好的表示方法来加速肽药物开发

Method: 提出HELM-BERT模型，基于DeBERTa架构，使用HELM（大分子分层编辑语言）表示法训练，能够精确描述单体组成和连接性，在39,079个化学多样性肽的数据集上进行预训练

Result: HELM-BERT在环肽膜通透性预测和肽-蛋白相互作用预测等下游任务中显著优于最先进的SMILES语言模型，展示了HELM表示法在数据效率方面的优势

Conclusion: HELM的明确单体和拓扑感知表示为治疗性肽建模提供了实质性优势，填补了小分子和蛋白质语言模型之间的长期空白
...
<details>
  <summary>Details</summary>
Motivation: 声带超声检查虽然创伤小、耐受性好，但其准确性高度依赖操作者经验，需要开发自动化的辅助诊断工具
...
TL;DR: PGOT（物理-几何算子Transformer）通过谱保持几何注意力机制解决几何混叠问题，在保持线性计算复杂度的同时实现多尺度几何特征保留，并在四个标准基准测试和工业任务中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer在建模大规模非结构化网格和复杂几何时，常采用特征降维策略导致几何混叠，丢失关键的物理边界信息。需要一种能够显式保持几何特征的方法。

Method: 提出PGOT框架，包含谱保持几何注意力模块，采用"物理切片-几何注入"机制集成多尺度几何编码。通过动态路由机制，根据空间坐标将计算分配到低阶线性路径（平滑区域）和高阶非线性路径（激波和不连续区域）。

Result: PGOT在四个标准基准测试中取得一致的SOTA性能，并在翼型和汽车设计等大规模工业任务中表现优异，同时保持线性计算复杂度O(N)。

Conclusion: PGOT通过显式几何感知重建物理特征学习，有效解决几何混叠问题，实现空间自适应的高精度物理场建模，为大规模工业应用提供了有效解决方案。
...
TL;DR: FedOLF通过有序层冻结和Tensor Operation Approximation技术，在联邦学习中显著降低计算、内存和通信开销，同时保持模型精度。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在物联网边缘设备上面临计算能力、内存和带宽限制的挑战，现有方法如dropout或层冻结往往牺牲精度或忽视内存约束。

Method: 提出FedOLF框架：1）按预定义顺序冻结层以减少计算和内存需求；2）引入Tensor Operation Approximation作为轻量级量化替代方案，降低通信和能耗。

Result: 在非独立同分布数据上，FedOLF在多个数据集和模型上比现有方法精度提升0.3%-6.27%，同时具有更高的能效和更低的内存占用。

Conclusion: FedOLF有效解决了联邦学习在资源受限边缘设备上的效率问题，在保持精度的同时显著降低了计算、内存和通信开销。
...
TL;DR: FairGFL解决图联邦学习中重叠子图不平衡导致的公平性问题，通过隐私保护的重叠率估计和加权聚合提升跨客户端公平性，同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 图联邦学习中不同客户端子图存在重叠，先前研究关注重叠对数据异质性的缓解作用，但未探索不平衡重叠带来的负面影响。本文发现不平衡重叠子图会导致跨客户端不公平问题。

Method: 提出FairGFL算法：1) 通过隐私保护方法估计客户端重叠率；2) 采用可解释的加权聚合方法提升跨客户端公平性；3) 在联邦复合损失函数中设计精心构造的正则化器，平衡模型效用与公平性。

Result: 在四个基准图数据集上的实验表明，FairGFL在模型效用和公平性方面均优于四种代表性基线算法。

Conclusion: 图联邦学习中不平衡重叠子图会导致不公平问题，FairGFL通过隐私保护的公平性增强机制有效解决了这一问题，在保持模型性能的同时显著提升了跨客户端公平性。
...
TL;DR: KernelEvolve是一个面向DLRM的智能内核编码框架，通过多级编程抽象和基于图的搜索策略，自动化生成和优化跨异构硬件架构的内核，显著提升性能和开发效率。


<details>
  <summary>Details</summary>
Motivation: 深度学习推荐模型训练和推理需要高效快速，但面临三大系统挑战：模型架构多样性、内核原语多样性、硬件代际和架构异构性。需要解决大规模异构环境下的内核优化问题。

Method: 提出KernelEvolve框架，以内核规范为输入，通过多级编程抽象（从Triton/CuTe DSL到底层硬件无关语言），采用基于图的搜索策略，包含选择策略、通用算子、适应度函数和终止规则，并通过检索增强提示合成动态适应运行时执行环境。

Result: 在公开的KernelBench套件上实现100%通过率（250个问题，三个难度级别），在三个异构硬件平台上验证160个PyTorch ATen算子，100%正确性。将开发时间从数周缩短到数小时，相比PyTorch基线实现显著性能提升，并降低新AI硬件的编程门槛。

Conclusion: KernelEvolve成功解决了DLRM在异构硬件上的内核优化挑战，实现了自动化内核生成和优化，显著提升了生产推荐模型的性能和开发效率，为新型AI硬件提供了可编程性解决方案。
...
TL;DR: PFed-signal：基于联邦学习的药物不良反应信号预测模型，通过欧氏距离消除FAERS中的偏差数据，提高预测准确性


<details>
  <summary>Details</summary>
Motivation: 基于FAERS（美国FDA不良事件报告系统）中偏差记录预测的药物不良反应（ADRs）可能误导在线诊断。传统方法依赖统计方法（如ROR、PRR）无法消除偏差数据，导致信号预测不准确

Method: 提出PFed-signal联邦学习模型：1）Pfed-Split方法按ADR分割原始数据集；2）ADR-signal模型包括：基于联邦学习的偏差数据识别方法（使用欧氏距离识别并删除偏差数据生成干净数据集）和基于Transformer的ADR预测模型（在干净数据集上训练）

Result: 干净数据集上的ROR和PRR优于传统方法。PFed-Signal的准确率0.887、F1分数0.890、召回率0.913、AUC 0.957，均优于基线方法

Conclusion: PFed-signal通过联邦学习和欧氏距离有效消除FAERS中的偏差数据，显著提高了ADR信号预测的准确性，为药物安全监测提供了更可靠的方法
...
TL;DR: 研究具有有限指数矩分布之间流匹配（FM）逆问题的唯一性，在一维和高斯情况下证明解的唯一性，多维一般情况仍为开放问题
...
Method: 分析具有有限指数矩分布之间的流匹配逆问题，在一维情况和高斯分布情况下建立解的唯一性理论

Result: 证明了一维情况和高斯情况下流匹配逆问题解的唯一性，但一般多维问题的唯一性仍为开放问题

Conclusion: 流匹配逆问题在一维和高斯情况下有唯一解，但多维一般情况需要进一步研究，为流匹配模型蒸馏提供理论基础
...
TL;DR: HC-PINNs通过边界函数B作为谱滤波器重塑学习景观，NTK框架揭示边界函数设计应从启发式选择转变为谱优化问题


<details>
  <summary>Details</summary>
Motivation: 硬约束PINNs（HC-PINNs）能严格强制执行边界条件，但其训练动态的理论机制尚未被探索，需要理解边界函数如何影响学习过程

Method: 建立HC-PINNs的神经正切核（NTK）理论框架，推导显式核组合定律，通过谱分析研究边界函数B作为谱滤波器的作用机制

Result: 边界函数B作为乘法空间调制器重塑神经网络原生核的特征谱，残差核的有效秩是训练收敛的确定性预测指标，常用边界函数可能无意中导致谱崩溃

Conclusion: 边界函数设计应从启发式选择转变为原则性的谱优化问题，为科学机器学习中的几何硬约束提供坚实理论基础
...
TL;DR: Splitwise：基于Lyapunov辅助深度强化学习的LLM细粒度自适应边缘-云分割框架，通过将Transformer层分解为注意力头和前馈子块，在动态网络条件下联合优化延迟、能耗和精度，相比现有方法显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 在边缘设备上部署大语言模型面临内存和功耗限制，纯云推理延迟高成本大，而静态分割方案无法适应带宽波动，需要一种能动态适应网络条件变化的细粒度分割方法。

Method: 提出Splitwise框架：1）将Transformer层分解为注意力头和前馈子块，提供比层级分割更细的划分粒度；2）采用Lyapunov辅助的层次化深度强化学习策略，联合优化延迟、能耗和精度退化；3）通过分区检查点和指数退避恢复机制保证通信故障时的鲁棒性。

Result: 在Jetson Orin NX、Galaxy S23和Raspberry Pi 5设备上测试GPT-2 (1.5B)、LLaMA-7B和LLaMA-13B模型，相比现有分割器：端到端延迟降低1.4-2.8倍，能耗降低最高41%；相比纯云执行：95%分位数延迟降低53-61%，同时保持精度和适度内存需求。

Conclusion: Splitwise通过细粒度自适应分割，在动态网络条件下有效平衡了LLM推理的延迟、能耗和精度，为边缘-云协同推理提供了鲁棒高效的解决方案。
...
TL;DR: 该论文延续前作，使用向量化索引无关方法推导Transformer架构（包括嵌入层、多头自注意力、层归一化）的反向传播，并提供LoRA层的梯度表达式，帮助理解模型内部运作机制。


<details>
  <summary>Details</summary>
Motivation: 尽管已有自动微分工具，但手动推导反向传播能帮助研究者深入理解每个操作如何影响最终输出，填补对前向传播值传递的理解空白，从而获得对模型行为的更深刻直觉。

Method: 采用轻量级索引无关方法，将前作中CNN反向传播推导的技术扩展到Transformer架构，具体分析嵌入层、多头自注意力机制和层归一化层的梯度计算，并推导参数高效微调（LoRA）的梯度表达式。

Result: 提供了完整的最小化GPT-like网络的PyTorch实现，以及所有梯度更新的解析表达式，使读者能够清晰理解Transformer架构中每个组件的反向传播过程。

Conclusion: 通过手动推导Transformer架构的反向传播，研究者能够获得对模型内部运作的深刻理解，这种理解对于模型调试、优化和创新架构设计至关重要，而不仅仅是依赖自动微分工具的黑箱操作。
...
<details>
  <summary>Details</summary>
Motivation: 单个大语言模型的能力存在固有上限，而多模型协作（如模型路由和后处理集成）虽然快速发展，但缺乏统一的理论框架来预测多模型协作的性能缩放规律。
...
TL;DR: ECG-RAMBA：通过分离形态学和节律特征，再通过上下文感知融合重新整合，提升心电图分类的跨域鲁棒性


<details>
  <summary>Details</summary>
Motivation: 深度学习在心电图分类中在单个数据集上表现良好，但在异构采集设置下的泛化能力不足。现有模型架构通常隐式地纠缠形态波形模式和节律动态，这可能导致捷径学习并放大对分布偏移的敏感性。
...
Result: 在Chapman-Shaoxing数据集上达到宏观ROC-AUC≈0.85；在零样本迁移中，在CPSC-2021数据集上获得房颤检测的PR-AUC=0.708，显著优于原始信号Mamba基线；在PTB-XL上表现一致。

Conclusion: 确定性形态特征提供了坚实基础，而显式节律建模和长距离上下文是跨域鲁棒性的关键驱动因素。ECG-RAMBA框架有效解决了心电图分类中的跨域泛化问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有近端策略方法（如GRPO、CISPO）需要多步梯度计算和重要性采样裁剪来近似自然梯度，计算成本较高
...
TL;DR: 华为openPangu-Embedded模型通过低比特量化技术（INT8和W4A8）在昇腾NPU上实现高效CoT推理，在保持90%以上准确率的同时获得1.5倍预填充加速和显著内存节省。


<details>
  <summary>Details</summary>
Motivation: openPangu-Embedded模型的三种CoT推理模式（slow_think、auto_think、no_think）虽然增强了推理能力，但生成长推理轨迹导致内存和延迟开销巨大，难以在昇腾NPU上实际部署。

Method: 采用低比特量化技术，将FP16计算转换为更高效的整数运算，提出统一低比特推理框架，支持INT8（W8A8）和W4A8量化，专门针对Atlas A2上的openPangu-Embedded模型进行优化。

Result: 在代码生成基准测试（HumanEval和MBPP）上，INT8量化保持90%以上FP16基线准确率，Atlas A2上实现1.5倍预填充加速；W4A8量化显著减少内存消耗，但准确率有一定折衷。

Conclusion: 低比特量化技术能有效促进昇腾NPU上的高效CoT推理，在保持高模型保真度的同时解决计算约束问题，为实际部署提供可行方案。
...
TL;DR: 提出结合生成模型与神经算子的速度建模新框架，通过神经算子作为前向映射快速生成RTM扩展图像，利用自动微分更新速度模型，嵌入生成模型作为正则化器提升分辨率。


<details>
  <summary>Details</summary>
Motivation: 传统速度建模方法计算成本高、耗时，而深度学习特别是生成模型和神经算子的发展为解决传统方法局限性提供了新途径。需要高效获取高分辨率速度模型的方法。

Method: 1) 使用神经算子作为前向映射算子，快速从真实速度和偏移速度生成时间滞后逆时偏移(RTM)扩展图像；2) 通过自动微分逐步更新速度模型，使网络输出与观测数据的时滞图像匹配；3) 嵌入在真实速度模型分布上训练的生成模型作为正则化器，提升分辨率。

Result: 合成和实际数据实验表明，所提出的基于生成神经算子的速度建模方法有效，能够获得更清晰、分辨率更高的速度模型预测。

Conclusion: 提出的结合生成模型与神经算子的框架能够高效构建高分辨率速度模型，解决了传统方法计算成本高的问题，为地下成像提供了有效解决方案。
...
TL;DR: CoLog是一个用于日志异常检测的多模态框架，通过协同编码不同日志模态，使用协作transformer和多头注意力机制，在7个基准数据集上达到平均F1分数99.61%


<details>
  <summary>Details</summary>
Motivation: 传统单模态方法忽略日志数据的不同模态特性，而多模态方法未能有效处理模态间的交互。日志数据包含多种信息模态，需要更全面的异常检测方法。

Method: 提出CoLog框架，采用协作transformer和多头注意力机制学习不同模态间的交互，包含模态适应层处理异质性，能够学习数据中的细微模式和依赖关系。

Result: 在7个日志异常检测基准数据集上，CoLog达到平均精度99.63%、召回率99.59%、F1分数99.61%，优于现有最先进方法，能同时检测点和集体异常。

Conclusion: CoLog代表了日志异常检测的重要进展，通过统一框架提供复杂日志数据分析的解决方案，适用于网络安全、系统监控和运营效率提升。
...
TL;DR: FgGSL是一种端到端的图结构学习框架，通过频率引导联合学习同质性和异质性图结构，结合谱编码器，在异质图基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 图神经网络在异质图上表现不佳，因为连接节点往往标签不同，特征相似性提供的结构线索较弱。需要一种能够同时处理同质性和异质性关系的图结构学习方法。

Method: 提出频率引导图结构学习(FgGSL)：1) 使用可学习的对称特征驱动掩码函数推断互补图结构；2) 采用预设计的低通和高通图滤波器组处理；3) 引入基于标签的结构损失，显式促进同质和异质边的恢复；4) 推导结构损失的稳定性边界和滤波器组在图扰动下的鲁棒性保证。

Result: 在六个异质图基准测试中，FgGSL持续优于最先进的GNN和图重连方法，证明了结合频率信息与监督拓扑推断的优势。

Conclusion: FgGSL通过联合学习互补图结构和谱编码，有效解决了异质图上的表示学习问题，频率信息与监督拓扑推断的结合为图结构学习提供了新方向。
...
TL;DR: 论文将神经缩放定律扩展到家族模型范式，引入粒度(G)作为新的缩放变量，证明了在固定计算预算下，部署灵活性不会显著影响模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统神经缩放定律只考虑单一密集模型输出，忽略了家族模型这种在异构设备-边缘-云层次结构中实现普适智能的重要范式。家族模型通过早期退出和接力式推理，从单一共享主干生成多个可部署子模型，需要新的理论框架来量化这种"一次训练，多次部署"的范式。

Method: 提出统一函数形式L(N, D, G)，将粒度(G)作为与模型大小(N)和训练令牌(D)并列的基本缩放变量。采用严格的IsoFLOP实验设计，在固定计算预算下系统扫描模型大小和粒度，动态调整训练令牌，从而将粒度边际成本与规模效益解耦。

Result: 研究发现粒度惩罚遵循乘性幂律，且指数极小。这理论上将固定计算训练与动态架构联系起来，实践上验证了"一次训练，多次部署"的可行性，表明部署灵活性不会损害密集基线的计算最优性。

Conclusion: 通过将神经缩放定律扩展到家族模型范式，证明了在固定计算预算下，部署灵活性是可实现的，且不会显著影响模型性能，为异构设备环境中的普适智能提供了理论基础。
...
TL;DR: 提出一种解耦解几何与搜索空间的构造性方法，可将分类头压缩16倍而性能几乎无损，并引入子空间原生蒸馏新范式


<details>
  <summary>Details</summary>
Motivation: 当前深度神经网络虽然内在维度低，但仍需巨大宽度来解决非凸优化搜索问题。紧凑网络难以找到全局最优解，存在冗余

Method: 通过解耦解几何与搜索空间，在构造的子空间中定义目标，提出子空间原生蒸馏范式，为学生模型提供稳定的几何坐标系

Result: 在ResNet-50、ViT和BERT上实证表明，分类头可压缩高达16倍而性能下降可忽略

Conclusion: 该方法可绕过优化瓶颈，让学生模型完全规避高维搜索问题，实现"训练大模型，部署小模型"的愿景
...
TL;DR: STAMP是一种基于Siamese MAE框架的随机时间自编码器，通过条件变分推理学习医学图像中的不确定时间动态，在AMD和AD疾病进展预测中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法（如MAE）缺乏时间感知能力，无法捕捉纵向医学数据中的疾病进展动态，特别是疾病演化固有的不确定性。

Method: 提出STAMP框架：基于Siamese MAE，通过条件变分推理将MAE重构损失重新构建为随机过程，根据两个输入体积之间的时间差编码时间信息。

Result: 在OCT和MRI数据集上评估，STAMP预训练的ViT模型在AMD和AD疾病进展预测任务中优于现有时间MAE方法和基础模型。

Conclusion: STAMP能够有效学习疾病进展中的非确定性时间动态，为纵向医学图像分析提供了强大的时间感知表示学习方法。
...
TL;DR: 提出DSC框架解决MoE模型表示坍塌和梯度不稳定问题，通过状态依赖的稀疏基向量组合近似上下文相关权重，降低参数复杂度和内存开销
...
Method: 使用动态子空间组合框架，通过状态依赖的稀疏基向量组合近似上下文相关权重，采用星形域内的残差轨迹建模和幅度门控单纯形插值确保连续性

Result: 将参数复杂度从O(M rd)降低到O(M d)，内存流量减少到O(Kd)，同时提供动态更新的严格最坏情况边界

Conclusion: DSC框架有效解决了MoE模型的表示坍塌和梯度不稳定问题，显著降低了参数复杂度和内存需求，为动态权重更新提供了理论保证
...
TL;DR: 提出DIR方法，通过信息优化来消除奖励模型中的归纳偏差，能够处理非线性相关的复杂偏差类型，提升RLHF性能


<details>
  <summary>Details</summary>
Motivation: 奖励模型训练数据质量低，包含各种归纳偏差（如响应长度、奉承、格式等），容易导致过拟合和奖励攻击。现有去偏方法要么针对单一偏差，要么只建模简单的线性相关，无法处理复杂多样的偏差。

Method: 基于信息瓶颈理论，提出DIR方法：最大化奖励模型分数与人类偏好对之间的互信息，同时最小化奖励模型输出与偏好输入中偏差属性之间的互信息。

Result: DIR能有效缓解三种归纳偏差（响应长度、奉承、格式），不仅消除目标偏差，还提升RLHF在多种基准测试中的性能，具有更好的泛化能力。

Conclusion: DIR通过信息理论优化，能够处理非线性相关的复杂偏差类型，扩展了奖励模型去偏方法的实际应用场景，显著提升对齐效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有PEFT方法（如LoRA）在效率和表达能力之间存在权衡，受限于低秩约束，导致收敛慢、适应能力有限，难以捕捉复杂任务模式。

Method: 结合分层联合分解与旋转自由度，提取跨层全局共享基，向缩放因子注入稀疏可学习扰动，实现灵活的全秩更新。

Result: 在涵盖视觉、推理和语言理解的20个基准测试中，FRoD在相同训练预算下，仅使用1.72%可训练参数，就能达到全模型微调的准确率。

Conclusion: FRoD通过增强表达能力和效率，实现了更快、更稳健的收敛，为参数高效微调提供了新的有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 工业物联网(IIoT)网络需要支持多设备动态超可靠低延迟通信(URLLC)，但在信道状态信息(CSI)不完美的条件下，现有方法在收敛速度和可靠性方面存在不足

Method: 提出贝叶斯优化驱动的TD3方法，结合BO训练机制改进收敛速度，通过更可靠的学习方向和样本选择方法解决URLLC网络中的误差样本不平衡问题

Result: 通过大量仿真验证，所提算法相比现有解决方案具有更快的收敛速度和更高的总速率性能

Conclusion: BO驱动的TD3方法能有效处理CSI不完美、样本不平衡等问题，在IIoT网络的URLLC场景中实现了更好的联合链路自适应和设备调度性能
...
TL;DR: EGDA框架通过联合对齐全局和类别特定分布来减少脑电情感识别中的跨会话差异，同时通过图正则化保持脑电数据的内在结构，在SEED-IV数据集上实现了鲁棒的跨会话性能。


<details>
  <summary>Details</summary>
Motivation: 脑电信号是情感识别的可靠来源，但跨会话的变异性严重影响了模型的泛化能力，需要解决跨会话差异问题以实现更稳定可靠的情感识别。

Method: 提出EGDA框架，联合对齐全局（边缘）分布和类别特定（条件）分布，同时通过图正则化技术保持脑电数据的内在结构，减少跨会话差异。

Result: 在SEED-IV数据集上，EGDA在三个迁移任务中分别获得81.22%、80.15%和83.27%的准确率，超越了多个基线方法。分析发现Gamma频段最具判别性，中央-顶叶和前额叶脑区对情感识别最关键。

Conclusion: EGDA框架通过联合分布对齐和图正则化有效解决了脑电情感识别中的跨会话差异问题，为鲁棒的情感识别系统提供了可行方案，Gamma频段和特定脑区在情感识别中具有重要作用。
...
TL;DR: 提出了VL-RouterBench基准，用于系统评估视觉语言模型路由系统的整体能力，包含大规模数据集和模型覆盖，并评估了10种路由方法


<details>
  <summary>Details</summary>
Motivation: 多模型路由已从工程技术发展为关键基础设施，但现有工作缺乏系统、可复现的基准来评估视觉语言模型路由系统

Method: 基于VLMs的原始推理和评分日志构建质量和成本矩阵，覆盖14个数据集、3个任务组、30,540个样本，包含15个开源模型和2个API模型，建立包含519,180个样本-模型对的基准

Result: 观察到显著的路由能力增益，但当前最佳路由器与理想Oracle仍有明显差距，表明在路由器架构方面仍有很大改进空间

Conclusion: VL-RouterBench为多模态路由研究提供了系统评估框架，将开源完整数据构建和评估工具链以促进可比性、可复现性和实际部署
...
TL;DR: 提出结合传统统计过程控制与保形预测的混合框架，通过可视化过程不确定性和重构多变量监控为异常检测问题，增强质量控制的鲁棒性和统计严谨性。


<details>
  <summary>Details</summary>
Motivation: 传统统计过程控制依赖于经常被违反的统计假设，导致在现代复杂制造环境中监控不可靠，需要更鲁棒的方法。

Method: 提出混合框架，将保形预测的分布自由、模型不可知保证与传统SPC结合，开发两种新应用：保形增强控制图（可视化过程不确定性）和保形增强过程监控（将多变量控制重构为异常检测问题）。

Result: 该框架提供了更鲁棒和统计严谨的质量控制方法，同时保持了经典方法的可解释性和易用性。

Conclusion: 通过集成保形预测增强传统SPC，能够更好地应对现代复杂制造环境中的质量控制挑战。
...
TL;DR: BOAD框架通过多臂老虎机优化自动发现层次化多智能体系统，显著提升大语言模型在真实世界软件工程任务上的泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有单智能体系统在处理长视野、分布外真实软件工程问题时存在局限性，需要像人类工程师那样通过分解复杂问题来提升性能
...
Conclusion: 自动发现的层次化多智能体系统能显著提升大语言模型在挑战性长视野软件工程任务上的泛化能力
...
TL;DR: 论文提出将长上下文语言建模视为持续学习问题而非架构设计问题，通过测试时训练（TTT）方法，在推理时通过下一个token预测持续学习，将上下文压缩到模型权重中，同时通过元学习优化训练时的初始化。


<details>
  <summary>Details</summary>
Motivation: 传统长上下文建模方法主要关注架构设计（如滑动窗口注意力、状态空间模型等），但这些方法在扩展性和效率上存在限制。本文提出将问题重新定义为持续学习问题，让模型在测试时也能学习，从而更有效地处理长上下文。

Method: 使用标准Transformer架构配合滑动窗口注意力，但在测试时通过下一个token预测进行持续学习，将上下文信息压缩到模型权重中。训练时采用元学习方法优化模型初始化，使模型在测试时学习更高效。整个方法在训练时（通过元学习）和测试时（通过下一个token预测）都是端到端的。

Result: 在3B模型使用164B tokens训练的情况下，TTT-E2E方法在上下文长度扩展性方面与全注意力Transformer表现相当，而Mamba 2和Gated DeltaNet等方法则表现不佳。同时，TTT-E2E具有类似RNN的恒定推理延迟，在128K上下文长度下比全注意力快2.7倍。

Conclusion: 将长上下文建模重新定义为持续学习问题，通过测试时训练和元学习的端到端方法，能够在保持标准Transformer架构的同时，实现与全注意力相当的扩展性和类似RNN的恒定推理延迟，为长上下文处理提供了新的有效途径。
...
TL;DR: 利用强化学习和自评分机制训练语言模型生成更好的研究计划，通过从现有论文中自动提取研究目标和评分标准构建训练数据，在机器学习和医学领域均取得显著改进
...
Method: 1) 从多个领域的研究论文中自动提取研究目标和目标特定的评分标准构建训练语料库；2) 通过强化学习结合自评分机制训练研究计划生成模型；3) 使用初始策略的冻结副本作为评分器，通过评分标准创建生成器-验证器差距实现无外部监督的改进

Result: 1) 人类专家在70%的研究目标中更偏好微调后的Qwen3-30B-A3B模型生成的研究计划；2) 84%的自动提取目标特定评分标准获得专家认可；3) 在医学领域和新arXiv预印本上获得12-22%的相对改进，显示出显著的跨领域泛化能力

Conclusion: 提出了一种可扩展的自动化训练方法，能够有效改进AI协科学家生成研究计划的能力，即使在医学研究等难以获得执行反馈的问题设置中也表现良好
...
TL;DR: 本文系统比较了五种非负矩阵分解模型（LBA、LCA、EMA、PLSA、NMF），证明了它们在可识别性上的等价关系，并通过时间预算数据集进行实证分析。


<details>
  <summary>Details</summary>
Motivation: 不同领域（机器学习、社会科学、地理学）中存在的多种非负矩阵分解模型虽然本质相似，但使用不同名称且相似性未被充分认识，导致研究重复和沟通障碍。

Method: 通过理论分析比较五种流行模型（LBA、LCA、EMA、PLSA、NMF），重点研究可识别性问题，证明这些模型解的唯一性等价关系，并简要回顾相关算法。

Result: 证明了LBA、EMA、LCA、PLSA的解唯一当且仅当NMF的解唯一，建立了这些模型在可识别性上的等价关系，并通过时间预算数据集验证了模型应用。

Conclusion: 五种非负矩阵分解模型本质上是相似或等价的，其可识别性条件相同，这一认识有助于跨领域研究整合，减少重复工作，促进方法论的统一。
...
TL;DR: 提出Fibonacci Ensembles框架，利用斐波那契序列的数学原理改进集成学习，通过归一化权重和二阶递归动态实现方差减少和表示深度增强。


<details>
  <summary>Details</summary>
Motivation: 受自然界斐波那契序列中体现的生长、和谐与递归稳定性启发，作者认为学习系统可以从同样的内在和谐中获益。现有集成学习方法如bagging、boosting和随机森林虽然有效，但缺乏这种数学优雅性和自然启发性。

Method: 提出两种相互关联的公式：(1) 使用经过正交化和Rao-Blackwell优化的归一化斐波那契权重，实现基学习器的系统性方差减少；(2) 二阶递归集成动态，模仿斐波那契流本身，超越经典boosting的表示深度。

Result: 在一维回归实验中，使用随机傅里叶特征集成和多项式集成，斐波那契加权在某些情况下匹配或优于均匀平均，并与正交Rao-Blackwell化以原则性方式相互作用。

Conclusion: 斐波那契集成在集成学习理论中形成了一个自然且可解释的设计点，表明当学习系统受到塑造自然世界的相同内在和谐指导时，它们会蓬勃发展。
...
TL;DR: 该论文提出了一个超越传统方差减少理论的集成学习加权理论，解释了为什么即使对于方差已经很低的基础学习器，结构化加权也能提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统集成学习理论主要基于方差减少来解释其有效性，但这无法解释为什么对于方差已经通过正则化控制的稳定基础学习器（如平滑样条、核岭回归等），集成仍然有效。需要一个新的理论框架来解释这种现象。

Method: 将集成学习形式化为作用于假设空间的线性算子，赋予加权序列几何和谱约束。在这个框架下，推导了改进的偏差-方差近似分解，展示非均匀结构化权重如何通过重塑近似几何和重新分配谱复杂性来超越均匀平均。

Result: 提出了结构化加权优于均匀集成的条件，证明最优权重是约束二次规划的解。经典平均、堆叠和最近提出的基于斐波那契的集成都作为该统一理论的特例出现。
...
TL;DR: 论文分析了Transformer中交叉熵训练如何通过梯度动力学塑造注意力机制，揭示了注意力分数和值向量的更新规律，以及这些更新如何形成类似EM算法的两时间尺度过程，最终支持贝叶斯推理功能。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer在精心设计的"贝叶斯风洞"和大规模语言模型中表现出精确的概率推理能力，但基于梯度的学习如何创建所需内部几何结构的机制仍然不透明。本文旨在揭示交叉熵训练如何重塑Transformer注意力头中的注意力分数和值向量。

Method: 提供注意力分数和值向量的一阶分析，推导出注意力分数的"基于优势的路由定律"和值向量的"责任加权更新"方程。通过受控模拟（包括粘性马尔可夫链任务），将闭式EM风格更新与标准SGD进行比较，展示梯度动力学如何塑造低维流形。

Result: 推导出注意力分数梯度公式∂L/∂s_ij = α_ij(b_ij - E_α_i[b])和值向量更新Δv_j = -η∑_i α_ij u_i，这些方程诱导了路由和内容共同专业化的正反馈循环。该耦合专业化行为类似于两时间尺度EM过程：注意力权重实现E步（软责任分配），值向量实现M步（责任加权原型更新）。

Conclusion: 优化（梯度流）产生几何结构（贝叶斯流形），进而支持功能（上下文概率推理），提供了一个统一的图景。梯度最小化交叉熵的同时也塑造了实现贝叶斯推理的低维流形，揭示了Transformer内部概率推理的机制。
...
TL;DR: 该论文提出了"似然保持嵌入"理论，通过控制似然比失真度Δ_n来确保嵌入表示不改变统计推断结论，证明了Hinge定理：控制Δ_n是保持推断的必要充分条件。


<details>
  <summary>Details</summary>
Motivation: 现代机器学习嵌入虽然能有效压缩高维数据，但通常破坏了经典似然统计推断所需的几何结构。需要开发既能压缩数据又能保持统计推断有效性的嵌入表示。

Method: 引入似然比失真度Δ_n度量嵌入引起的对数似然比最大误差；提出Hinge定理证明控制Δ_n是保持推断的必要充分条件；使用神经网络作为近似充分统计量，建立训练损失与推断保证的显式界限。

Result: 当Δ_n=o_p(1)时：(1)所有基于似然比的检验和贝叶斯因子渐近保持；(2)替代最大似然估计量与全数据MLE渐近等价。证明了通用似然保持需要基本可逆嵌入，提出了模型类别特定的保证框架。

Conclusion: 该理论为构建既能压缩数据又能保持统计推断有效性的嵌入表示提供了严格框架，在分布式临床推断等应用中具有实际价值，通过控制似然比失真度可以实现可靠的压缩推断。
...
TL;DR: JADAI框架联合摊销贝叶斯自适应设计与推断，通过端到端训练策略网络、历史网络和推断网络来最大化信息增益


<details>
  <summary>Details</summary>
Motivation: 在参数估计问题中，需要主动优化设计变量以最大化信息增益，传统方法难以处理高维、多模态的后验分布

Method: 提出JADAI框架，联合训练策略网络（选择实验设计）、历史网络（编码实验历史）和推断网络（基于扩散的后验估计器），通过最小化实验序列中后验误差的增量减少来优化

Result: 在标准自适应设计基准测试中，JADAI实现了优越或具有竞争力的性能

Conclusion: JADAI为贝叶斯自适应设计和推断提供了一个有效的端到端框架，能够处理高维多模态后验，并在标准基准上表现优异
...
TL;DR: 提出在联邦学习中应用L0约束实现模型稀疏化的方法，通过概率门和连续松弛技术控制非零参数密度，在数据异质性和客户端参与异质性下保持统计性能


<details>
  <summary>Details</summary>
Motivation: 联邦学习中数据和模型的固有稀疏性未得到充分处理，导致模型过于密集且泛化能力差，特别是在数据和客户端参与异质性情况下

Method: 采用L0约束限制非零参数密度，通过概率门重参数化和连续松弛技术实现，基于联邦随机梯度下降算法进行分布式学习

Result: 在数据异质性和客户端参与异质性下，能够达到目标参数密度（ρ），在线性和非线性模型中统计性能损失最小，比基于幅度剪枝的阈值算法表现更好

Conclusion: 提出的L0约束联邦学习方法具有通信效率高、统计性能优越的特点，在多种模型和数据集上验证了其有效性
...
<details>
  <summary>Details</summary>
Motivation: 机器学习领域对因果推断存在混淆，普遍认为需要专门的因果框架或符号。作者旨在澄清这种误解，证明标准概率建模足以解决任何因果问题。

Method: 通过具体示例展示如何通过"写下所有概率"来回答因果问题，并将因果工具重新解释为标准概率建模和推断的产物。

Result: 论证了因果推断问题可以在概率建模框架内完全解决，无需专门的因果工具，阐明了现有因果工具在概率框架中的必要性和实用性。

Conclusion: 因果推断不需要专门的框架或符号，标准概率建模和推断足以回答任何因果问题，现有因果工具应被理解为概率框架的自然产物。
...
<details>
  <summary>Details</summary>
Motivation: 股票市场存在非平稳性，传统机器学习模型面临非平稳性与复杂性之间的权衡：复杂模型能减少设定误差但需要更长训练窗口，而长窗口会引入更强的非平稳性。

Method: 提出一种新颖的模型选择方法，使用锦标赛程序在非平稳验证数据上自适应评估候选模型，联合优化模型类别和训练窗口大小，平衡设定误差、估计方差和非平稳性。

Result: 在17个行业组合收益预测中，该方法持续优于标准滚动窗口基准，样本外R²平均提高14-23%。在经济衰退期间表现尤为突出，基于选定模型的交易策略累计收益平均提高31%。

Conclusion: 该方法有效解决了非平稳环境中的模型选择问题，在理论和实证上都表现出色，能够平衡设定误差、估计方差和非平稳性，实现接近事后最佳模型的性能。
...
TL;DR: MultiQT是一种在线方法，可同时保证多个分位数预测的校准性，即使面对对抗性分布漂移也能确保预测有序且不降低现有预测器的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的分位数预测器在实际应用中往往无法保证校准性，特别是在分布漂移的情况下。需要一种轻量级方法能够包装现有预测器，同时保证多个分位数水平的校准性和有序性。

Method: 提出Multi-Level Quantile Tracker (MultiQT)方法，该方法可以包装任何现有的点预测或分位数预测器，通过在线校正机制保证校准性。方法确保预测有序（如0.5分位数预测不大于0.6分位数预测），并具有无遗憾保证。

Result: 实验表明，MultiQT在流行病预测和能源预测问题上显著提高了真实预测器的校准性。方法能够有效处理对抗性分布漂移，同时保持预测的有序性。

Conclusion: MultiQT提供了一种实用且理论保证的解决方案，能够在不降低现有预测器性能的前提下，显著改善多个分位数预测的校准性，适用于存在分布漂移的现实世界预测问题。
...
<details>
  <summary>Details</summary>
Motivation: 在无限时域MDP中，离策略价值预测的校准问题尚未得到充分解决。现有方法通常需要Bellman完备性或可实现性等强假设，限制了实际应用

Method: 将经典直方图和保序校准方法扩展到动态反事实设置，通过双重稳健伪结果处理离策略数据，重复将拟合的Bellman目标回归到模型预测上，形成一维拟合价值迭代方案

Result: 在弱假设下（无需Bellman完备性或可实现性）为校准和预测提供了有限样本保证，方法可应用于任何价值估计器

Conclusion: Iterated Bellman Calibration是一种简单、模型无关、后处理的离策略价值校准方法，具有理论保证和广泛适用性
